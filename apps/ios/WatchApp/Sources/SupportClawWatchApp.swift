...
@main
struct SupportClawWatchApp: App {
    @Environment(\.scenePhase) private var scenePhase
    @State private var inboxStore = WatchInboxStore(
        requestNotificationAuthorization: !SupportClawWatchApp.isScreenshotMode)
    @State private var receiver: WatchConnectivityReceiver?
    @State private var execApprovalRefreshTask: Task<Void, Never>?

    private static let screenshotModeDefaultsKey = "supportClaw.watch.screenshotMode"
    private static let isScreenshotMode = ProcessInfo.processInfo.arguments.contains(
        "--supportClaw-watch-screenshot-mode")
        || ProcessInfo.processInfo.environment["SUPPORT_CLAW_WATCH_SCREENSHOT_MODE"] == "1"
        || UserDefaults.standard.bool(forKey: SupportClawWatchApp.screenshotModeDefaultsKey)
    ...
    var body: some Scene {
        WindowGroup {
            WatchInboxView(
                ...
                .task {
                    if SupportClawWatchApp.isScreenshotMode {
