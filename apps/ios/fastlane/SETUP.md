# fastlane setup (SupportClaw iOS)
...
APP_STORE_CONNECT_KEYCHAIN_SERVICE=supportClaw-app-store-connect-key
...
```bash
APP_STORE_CONNECT_APP_IDENTIFIER=ai.supportClawfoundation.app
# or
...
Tip: run `scripts/ios-team-id.sh --require-canonical` from repo root to verify the canonical SupportClaw iOS team (`FWJYW4S8P8`) is available locally. Fastlane uses the same canonical-only path when `IOS_DEVELOPMENT_TEAM` is missing, and rejects non-canonical teams for release archives.
...
For local/manual iOS builds that stay on direct APNs, configure the gateway host separately with `SUPPORT_CLAW_APNS_TEAM_ID`, `SUPPORT_CLAW_APNS_KEY_ID`, and either `SUPPORT_CLAW_APNS_PRIVATE_KEY_P8` or `SUPPORT_CLAW_APNS_PRIVATE_KEY_PATH`. Those gateway runtime env vars are separate from Fastlane's `.env`.
...
The screenshot lane runs the app with `--supportClaw-screenshot-mode`, which enters the built-in connected screenshot fixture instead of pairing with a live gateway. By default it captures the tab set on `iPhone 16 Pro Max` and `iPad Pro 13-inch (M4)`; override devices with a comma-separated `SUPPORT_CLAW_SNAPSHOT_DEVICES` value when the requested simulators exist locally.
...
- confirm `apps/ios/build/app-store/SupportClaw-<version>.ipa` exists
...
- The release flow regenerates `apps/ios/SupportClaw.xcodeproj` from `apps/ios/project.yml` before archiving
