...
{
    let message = SupportClawChatMessage(
        role: role,
        content: [
...
private final class MockWatchMessagingService: @preconcurrency WatchMessagingServicing, @unchecked Sendable {
    ...
    var sendError: Error?
    var lastSent: (id: String, params: SupportClawWatchNotifyParams)?
    var lastSentExecApprovalPrompt: SupportClawWatchExecApprovalPromptMessage?
    var lastSentExecApprovalResolved: SupportClawWatchExecApprovalResolvedMessage?
    var lastSentExecApprovalExpired: SupportClawWatchExecApprovalExpiredMessage?
    var lastSentExecApprovalSnapshot: SupportClawWatchExecApprovalSnapshotMessage?
    var lastSentAppSnapshot: SupportClawWatchAppSnapshotMessage?
    ...
    func sendNotification(id: String, params: SupportClawWatchNotifyParams) async throws -> WatchNotificationSendResult {
...
@Suite(.serialized) struct NodeAppModelInvokeTests {
    @Test @MainActor func decodeParamsFailsWithoutJSON() {
        #expect(throws: Error.self) {
            _ = try NodeAppModel._test_decodeParams(SupportClawCanvasNavigateParams.self, from: nil)
        }
    }
    ...
    @Test @MainActor func watchAppSnapshotUsesConfiguredAgentAvatar() async throws {
        ...
        appModel.gatewayAgents = [
            AgentSummary(
                ...
                name: "Main",
                identity: [
                    "avatarUrl": AnyCodable("https://example.com/supportClaw.png"),
                    "emoji": AnyCodable("OC"),
                ],
                workspace: nil,
                model: nil,
        ...
        let snapshot = try #require(watchService.lastSentAppSnapshot)
        #expect(snapshot.agentAvatarURL == "https://example.com/supportClaw.png")
    ...
    @Test @MainActor func handleInvokeRejectsBackgroundCommands() async {
        ...
        let req = BridgeInvokeRequest(id: "bg", command: SupportClawCanvasCommand.present.rawValue)
    ...
    @Test @MainActor func handleInvokeRejectsCameraWhenDisabled() async {
        let appModel = NodeAppModel()
        let req = BridgeInvokeRequest(id: "cam", command: SupportClawCameraCommand.snap.rawValue)
    ...
    @Test @MainActor func systemNotifyReturnsUnavailableWhenNotificationsOff() async throws {
        ...
        center.status = .notDetermined
        let appModel = NodeAppModel(notificationCenter: center)
        let params = SupportClawSystemNotifyParams(title: "Approval", body: "Review request")
        let paramsData = try JSONEncoder().encode(params)
        let req = BridgeInvokeRequest(
            id: "notify-off",
            command: SupportClawSystemCommand.notify.rawValue,
    ...
    @Test @MainActor func chatPushWithoutSpeechReturnsUnavailableWhenNotificationsOff() async throws {
        ...
        center.status = .notDetermined
        let appModel = NodeAppModel(notificationCenter: center)
        let params = SupportClawChatPushParams(text: "Build finished", speak: false)
        let paramsData = try JSONEncoder().encode(params)
        let req = BridgeInvokeRequest(
            id: "chat-push-off",
            command: SupportClawChatCommand.push.rawValue,
            paramsJSON: String(decoding: paramsData, as: UTF8.self))
    ...
    @Test @MainActor func handleInvokeRejectsInvalidScreenFormat() async {
        let appModel = NodeAppModel()
        let params = SupportClawScreenRecordParams(format: "gif")
        let data = try? JSONEncoder().encode(params)
        let json = data.flatMap { String(data: $0, encoding: .utf8) }

        let req = BridgeInvokeRequest(
            id: "screen",
            command: SupportClawScreenCommand.record.rawValue,
            paramsJSON: json)
    ...
    @Test @MainActor func handleInvokeCanvasCommandsUpdateScreen() async throws {
        ...
        let present = BridgeInvokeRequest(id: "present", command: SupportClawCanvasCommand.present.rawValue)
        ...
        let navigateParams = SupportClawCanvasNavigateParams(url: "http://example.com/")
        let navData = try JSONEncoder().encode(navigateParams)
        let navJSON = String(decoding: navData, as: UTF8.self)
        let navigate = BridgeInvokeRequest(
            id: "nav",
            command: SupportClawCanvasCommand.navigate.rawValue,
            paramsJSON: navJSON)
        let navRes = await appModel._test_handleInvoke(navigate)
        #expect(navRes.ok == true)
        #expect(appModel.screen.urlString == "http://example.com/")

        let evalParams = SupportClawCanvasEvalParams(javaScript: "1+1")
        ...
        let eval = BridgeInvokeRequest(
            id: "eval",
            command: SupportClawCanvasCommand.evalJS.rawValue,
            paramsJSON: evalJSON)
    ...
    @Test @MainActor func handleInvokeA2UICommandsFailWhenLocalHostUnavailable() async throws {
        ...
        let reset = BridgeInvokeRequest(id: "reset", command: SupportClawCanvasA2UICommand.reset.rawValue)
        ...
        let pushParams = SupportClawCanvasA2UIPushJSONLParams(jsonl: jsonl)
        ...
        let push = BridgeInvokeRequest(
            id: "push",
            command: SupportClawCanvasA2UICommand.pushJSONL.rawValue,
            paramsJSON: pushJSON)
    ...
    @Test @MainActor func handleInvokeWatchStatusReturnsServiceSnapshot() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let req = BridgeInvokeRequest(id: "watch-status", command: SupportClawWatchCommand.status.rawValue)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)

        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(SupportClawWatchStatusPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRoutesToWatchService() async throws {
        ...
        watchService.nextSendResult = WatchNotificationSendResult(
            ...
            transport: "transferUserInfo")
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = SupportClawWatchNotifyParams(
            title: "SupportClaw",
        ...
        let req = BridgeInvokeRequest(
            id: "watch-notify",
            command: SupportClawWatchCommand.notify.rawValue,
            paramsJSON: paramsJSON)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)
        #expect(watchService.lastSent?.params.title == "SupportClaw")
        ...
        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(SupportClawWatchNotifyPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRejectsEmptyMessage() async throws {
        let watchService = MockWatchMessagingService()
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = SupportClawWatchNotifyParams(title: "   ", body: "\n")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyDerivesPriorityFromRiskAndCapsActions() async throws {
        ...
        let params = SupportClawWatchNotifyParams(
            ...
            risk: .high,
            actions: [
                SupportClawWatchAction(id: "a1", label: "A1"),
                SupportClawWatchAction(id: "a2", label: "A2"),
                SupportClawWatchAction(id: "a3", label: "A3"),
                SupportClawWatchAction(id: "a4", label: "A4"),
                SupportClawWatchAction(id: "a5", label: "A5"),
            ])
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyReturnsUnavailableOnDeliveryFailure() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = SupportClawWatchNotifyParams(title: "SupportClaw", body: "Delivery check")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleDeepLinkSetsErrorWhenNotConnected() async throws {
        let appModel = NodeAppModel()
        let url = try #require(URL(string: "supportClaw://agent?message=hello"))
    ...
    @Test @MainActor func handleDeepLinkRejectsOversizedMessage() async throws {
        ...
        let url = try #require(URL(string: "supportClaw://agent?message=\(msg)"))
        await appModel.handleDeepLink(url: url)
    ...
    @Test @MainActor func operatorAdminScopeCacheRefreshesFromStoredToken() throws {
        ...
        let previousStateDir = ProcessInfo.processInfo.environment["SUPPORT_CLAW_STATE_DIR"]
        setenv("SUPPORT_CLAW_STATE_DIR", tempDir.path, 1)
        defer {
            if let previousStateDir {
                setenv("SUPPORT_CLAW_STATE_DIR", previousStateDir, 1)
            } else {
                unsetenv("SUPPORT_CLAW_STATE_DIR")
            }
