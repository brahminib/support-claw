...
@Suite(.serialized) struct GatewayConnectionSecurityTests {
    ...
    {
        let endpoint: NWEndpoint = .service(name: "Test", type: "_supportClaw-gw._tcp", domain: "local.", interface: nil)
    ...
    @Test @MainActor func manualConnectionsAllowPrivateLanPlaintext() async {
        ...
        #expect(controller._test_resolveManualUseTLS(host: "supportClaw.local", useTLS: false) == false)
