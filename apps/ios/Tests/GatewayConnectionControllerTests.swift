...
import Testing
import UIKit
@testable import SupportClawKit
...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func currentCapsReflectToggles() {
        withUserDefaults([
            ...
            "camera.enabled": true,
            "location.enabledMode": SupportClawLocationMode.always.rawValue,
            ...
            #expect(caps.contains(SupportClawCapability.canvas.rawValue))
            #expect(caps.contains(SupportClawCapability.screen.rawValue))
            #expect(caps.contains(SupportClawCapability.camera.rawValue))
            #expect(caps.contains(SupportClawCapability.location.rawValue))
            #expect(caps.contains(SupportClawCapability.voiceWake.rawValue))
            #expect(caps.contains(SupportClawCapability.talk.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsIncludeLocationWhenEnabled() {
        withUserDefaults([
            "node.instanceId": "ios-test",
            "location.enabledMode": SupportClawLocationMode.whileUsing.rawValue,
        ]) {
            let appModel = NodeAppModel()
            let controller = GatewayConnectionController(appModel: appModel, startDiscovery: false)
            let commands = Set(controller._test_currentCommands())

            #expect(commands.contains(SupportClawLocationCommand.get.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsExcludeDangerousSystemExecCommands() {
        withUserDefaults([
            ...
            #expect(commands.contains(SupportClawSystemCommand.notify.rawValue))
            #expect(!commands.contains(SupportClawSystemCommand.run.rawValue))
            #expect(!commands.contains(SupportClawSystemCommand.which.rawValue))
            #expect(!commands.contains(SupportClawSystemCommand.execApprovalsGet.rawValue))
            #expect(!commands.contains(SupportClawSystemCommand.execApprovalsSet.rawValue))
        }
    }
    ...
    @Test @MainActor func loadLastConnectionReadsSavedValues() {
        let prior = KeychainStore.loadString(service: "ai.supportClawfoundation.app.gateway", account: "lastConnection")
        defer {
            if let prior {
                _ = KeychainStore.saveString(
                    prior,
                    service: "ai.supportClawfoundation.app.gateway",
                    account: "lastConnection")
            } else {
                _ = KeychainStore.delete(service: "ai.supportClawfoundation.app.gateway", account: "lastConnection")
            }
        }
        _ = KeychainStore.delete(service: "ai.supportClawfoundation.app.gateway", account: "lastConnection")
