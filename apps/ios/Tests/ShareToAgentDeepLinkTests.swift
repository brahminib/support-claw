...
@Suite struct ShareToAgentDeepLinkTests {
    @Test func appGroupIdentifierUsesCanonicalSupportClawGroup() {
        #expect(SupportClawAppGroup.canonicalIdentifier == "group.ai.supportClawfoundation.app.shared")
    }
    ...
    @Test func buildURLEncodesAgentRoute() {
        ...
        guard case let .agent(agent)? = parsed else {
            Issue.record("Expected supportClaw://agent deep link")
            return
        }
