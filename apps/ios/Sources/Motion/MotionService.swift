...
final class MotionService: MotionServicing {
    func activities(params: SupportClawMotionActivityParams) async throws -> SupportClawMotionActivityPayload {
        ...
        let mapped: [SupportClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        SupportClawMotionActivityEntry(
        ...
        return SupportClawMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = SupportClawPedometerPayload(
