...
final class RemindersService: RemindersServicing {
    func list(params: SupportClawRemindersListParams) async throws -> SupportClawRemindersListPayload {
        ...
        let payload: [SupportClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return SupportClawReminderPayload(
        ...
        return SupportClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: SupportClawRemindersAddParams) async throws -> SupportClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = SupportClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return SupportClawRemindersAddPayload(reminder: payload)
    }
