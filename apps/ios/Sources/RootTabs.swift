...
struct RootTabs: View {
    ...
    private static var initialTab: AppTab {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--supportClaw-initial-tab") else {
            return .control
        }
    ...
    private static var requestedInitialSidebarDestination: SidebarDestination? {
        let arguments = ProcessInfo.processInfo.arguments
        guard let flagIndex = arguments.firstIndex(of: "--supportClaw-initial-destination") else {
            return nil
        }
        let valueIndex = arguments.index(after: flagIndex)
    ...
    private static var initialChatSessionKey: String? {
        let arguments = ProcessInfo.processInfo.arguments
        guard let flagIndex = arguments.firstIndex(of: "--supportClaw-chat-session") else {
            return nil
        }
        let valueIndex = arguments.index(after: flagIndex)
        guard arguments.indices.contains(valueIndex) else { return nil }
    ...
    private func sidebarNavigationSplitContent(sidebarWidth: CGFloat) -> some View {
        ...
        .background(SupportClawProBackground())
    }
    ...
    private var sidebarIdentityHeader: some View {
        HStack(spacing: 10) {
            SupportClawProMark(size: 30, shadowRadius: 3)
        ...
        .accessibilityLabel("SupportClaw \(self.sidebarGatewayStatusTitle)")
    }
    ...
    private var sidebarGatewayStatusColor: Color {
        ...
        case .connected:
            SupportClawBrand.ok
        case .connecting:
            SupportClawBrand.accent
        case .error:
            SupportClawBrand.warn
        case .disconnected:
            .secondary
        }
    }
        ...
        title: String? = nil) -> some View
    {
        ...
        .buttonStyle(.plain)
        .foregroundStyle(destination == self.selectedSidebarDestination ? SupportClawBrand.accent : .primary)
        .listRowBackground(
            destination == self.selectedSidebarDestination
                ? SupportClawBrand.accent.opacity(0.12)
                : Color.clear)
    ...
    private var sidebarHeaderLeadingAction: SupportClawSidebarHeaderAction? {
        ...
        if self.isSidebarVisible {
            return SupportClawSidebarHeaderAction(
    ...
    private func makeHomeCanvasPayload() -> RootTabsHomeCanvasPayload {
        ...
        case .connected:
            return RootTabsHomeCanvasPayload(
                ...
                footer: "SupportClaw only runs phone-side capabilities while the app is connected and permitted.")
        case .connecting:
            return RootTabsHomeCanvasPayload(
        ...
        case .error, .disconnected:
            return RootTabsHomeCanvasPayload(
                ...
                eyebrow: self.gatewayStatus == .error ? "Gateway needs attention" : "SupportClaw iOS",
                ...
                agents: Array(agents.prefix(4)),
                footer:
                "Use Settings to scan a pairing QR code or paste a setup code from your SupportClaw gateway.")
        }
    }
