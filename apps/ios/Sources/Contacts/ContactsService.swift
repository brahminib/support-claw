...
final class ContactsService: ContactsServicing {
    ...
    func search(params: SupportClawContactsSearchParams) async throws -> SupportClawContactsSearchPayload {
        ...
        return SupportClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: SupportClawContactsAddParams) async throws -> SupportClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return SupportClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return SupportClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> SupportClawContactPayload {
        SupportClawContactPayload(
