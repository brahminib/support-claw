...
@MainActor
enum SupportClawAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class SupportClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.supportClawfoundation.app", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.supportClawfoundation.app", category: "BackgroundWake")
    ...
    private static var appBundleIdentifier: String {
        guard let bundleId = Bundle.main.bundleIdentifier?.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            return "ai.supportClawfoundation.app"
        }
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? SupportClawAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "supportClaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "supportClaw.watch.promptId"
    static let sessionKeyKey = "supportClaw.watch.sessionKey"
    static let actionPrimaryIDKey = "supportClaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "supportClaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "supportClaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "supportClaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "supportClaw.watch.action.primary"
    static let actionSecondaryIdentifier = "supportClaw.watch.action.secondary"
    static let actionIdentifierPrefix = "supportClaw.watch.action."
    static let actionIDKeyPrefix = "supportClaw.watch.action.id."
    static let actionLabelKeyPrefix = "supportClaw.watch.action.label."
    static let categoryPrefix = "supportClaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: SupportClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> SupportClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return SupportClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "SupportClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [SupportClawWatchAction]) -> [UNNotificationAction] {
...
struct SupportClawApp: App {
    ...
    init() {
        ...
        #endif
        SupportClawAppModelRegistry.appModel = appModel
    ...
    private static var screenshotModeEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--supportClaw-screenshot-mode")
        #else
        false
        #endif
    }
    ...
    private static var screenshotNotificationGuidanceEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--supportClaw-screenshot-notification-guidance")
        #else
        false
        #endif
    }
    ...
    private func applyAppearancePreference() {
        ...
        UIApplication.shared.connectedScenes
            ...
            .forEach { window in
                ...
                window.tintColor = SupportClawBrand.uiAccent
            }
    }
}

extension SupportClawApp {
    @MainActor
    ...
    private static func installUncaughtExceptionLogger() {
        NSLog("SupportClaw: installing uncaught exception handler")
