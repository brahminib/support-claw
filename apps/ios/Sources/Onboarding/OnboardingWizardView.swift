...
struct OnboardingWizardView: View {
    ...
    @State private var statusLine: String = "In your SupportClaw chat, run /pair qr, then scan the code here."
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `supportClaw devices list`."
                    }()
                    let commandLine = self.currentProblem?.actionCommand ?? "supportClaw devices approve <requestId>"
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `\(commandLine)`\n"
                            + "2) `/pair approve` in your SupportClaw chat\n"
                            + "\(requestLine)\n"
                            + "SupportClaw will also retry automatically when you return to this app.")
                }
            }
    ...
    private var successStep: some View {
        VStack(spacing: 0) {
            ...
            Image(systemName: "checkmark.circle.fill")
                .font(.system(size: 64))
                .foregroundStyle(SupportClawBrand.ok)
                .padding(.bottom, 20)
...
extension OnboardingWizardView {
    ...
    private func advanceFromIntro() {
        ...
        self.statusLine = "In your SupportClaw chat, run /pair qr, then scan the code here."
        self.step = .welcome
    }
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            if let last = GatewaySettingsStore.loadLastGatewayConnection() {
                ...
                case .discovered:
                    self.manualHost = "supportClaw.local"
        ...
        if self.selectedMode == .developerLocal, self.manualHost == "supportClaw.local" {
        ...
        if !hasSavedGateway, !hasToken, !hasPassword {
            self.statusLine = "No saved pairing found. In your SupportClaw chat, run /pair qr, then scan the code here."
        }
    }
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "supportClaw.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "supportClaw.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "supportClaw.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
