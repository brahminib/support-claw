...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "supportClaw_agent_control"
    ...
    private let logger = Logger(subsystem: "ai.supportClawfoundation.app", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable]) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "SupportClaw tool call failed"]
                : ["text": completion.text ?? "SupportClaw finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: SupportClawChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }
