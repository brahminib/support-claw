...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__SUPPORT_CLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<SupportClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.supportClawfoundation.app", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = SupportClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> SupportClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = activePTTCaptureId {
            return SupportClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> SupportClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> SupportClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return SupportClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> SupportClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = SupportClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: SupportClawTalkPTTStopPayload) {
    ...
    private static func chatSendHistorySince(
        response: SupportClawChatSendResponse,
        startedAt: Double) -> Double?
    {
        self.isTerminalChatSendSuccess(response.status) ? nil : startedAt
    }

    private func sendChat(_ message: String, gateway: GatewayNodeSession) async throws -> SupportClawChatSendResponse {
        ...
        return try JSONDecoder().decode(SupportClawChatSendResponse.self, from: res)
    }
            ...
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    if evt.event == "chat" {
                        ...
                        if let text = SupportClawChatEventText.assistantText(from: chatEvent) {
                        ...
                        guard let agentEvent = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SupportClawAgentEventPayload.self)
                        else {
                            continue
                        }
