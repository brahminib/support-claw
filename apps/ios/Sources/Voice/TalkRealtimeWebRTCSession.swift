...
final class TalkRealtimeWebRTCSession: NSObject {
    private static let logger = Logger(subsystem: "ai.supportClawfoundation.app", category: "TalkRealtimeWebRTC")
    private static let consultToolName = "supportClaw_agent_consult"
    private static let controlToolName = "supportClaw_agent_control"
    private static let defaultOfferURL = "https://api.openai.com/v1/realtime/calls"
    private static let mediaStreamID = "supportClaw-ios-realtime"
    private static let audioTrackID = "supportClaw-ios-audio"
    ...
    private func handleToolDone(_ event: TalkRealtimeServerEvent) {
        ...
        self.delegate?.realtimeSession(
            self,
            didChangeStatus: name == Self.controlToolName ? "Updating SupportClaw" : "Asking SupportClaw")
    ...
    private func submitConsultToolCall(callId: String, argsJSON: String) async {
        ...
        let statusTask = Task { @MainActor [weak self] in
            ...
            guard let self, !Task.isCancelled, !self.stopped else { return }
            self.delegate?.realtimeSession(self, didChangeStatus: "Still asking SupportClaw")
        }
        defer {
            statusTask.cancel()
        ...
        do {
            ...
            self.delegate?.realtimeSession(self, didChangeStatus: "SupportClaw unavailable")
            let fallbackMessage = [
                "SupportClaw consult did not finish quickly enough.",
                "Give a brief spoken fallback from the realtime conversation",
                "and ask the user to try again if they need SupportClaw-specific context.",
            ].joined(separator: " ")
    ...
    private func submitControlToolCall(callId: String, argsJSON: String) async {
        ...
        do {
            ...
            let res = try await gateway.request(
                ...
                paramsJSON: json,
                timeoutSeconds: Self.toolCallTimeoutSeconds)
            let message = Self.controlResultMessage(from: res) ?? "SupportClaw updated the active run."
            ...
            self.submitToolResult(callId: callId, result: [
                "error": "SupportClaw could not update the active run.",
            ])
        }
        guard !Task.isCancelled, !self.stopped else { return }
        if !self.assistantAudioActive {
    ...
    private static func controlParams(sessionKey: String, argsJSON: String) throws -> [String: Any] {
        ...
        guard let text else {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 20, userInfo: [
                NSLocalizedDescriptionKey: "SupportClaw control tool call missing text",
            ])
        }
        ...
        return try await withThrowingTaskGroup(of: String.self) { group in
            group.addTask { [runId, currentSessionKey] in
                for await evt in stream {
                    ...
                    guard let chatEvent = try? GatewayPayloadDecoding.decode(
                        payload,
                        as: SupportClawChatEventPayload.self)
                    else {
                        continue
                    }
                    ...
                    if chatEvent.state == "final" {
                        return SupportClawChatEventText.assistantText(from: chatEvent) ?? "SupportClaw finished with no text."
                    }
                    if chatEvent.state == "aborted" {
                        throw NSError(domain: "TalkRealtimeWebRTC", code: 9, userInfo: [
                            NSLocalizedDescriptionKey: "SupportClaw realtime tool call aborted",
                        ])
                    }
                    if chatEvent.state == "error" {
                        throw NSError(domain: "TalkRealtimeWebRTC", code: 10, userInfo: [
                            NSLocalizedDescriptionKey: "SupportClaw realtime tool call failed",
                        ])
                    }
                }
                throw NSError(domain: "TalkRealtimeWebRTC", code: 11, userInfo: [
                    NSLocalizedDescriptionKey: "SupportClaw realtime tool event stream ended",
                ])
            }
            ...
            group.addTask {
                ...
                throw NSError(domain: "TalkRealtimeWebRTC", code: 12, userInfo: [
                    NSLocalizedDescriptionKey: "SupportClaw realtime tool call timed out",
                ])
            }
            guard let result = try await group.next() else {
                throw NSError(domain: "TalkRealtimeWebRTC", code: 13, userInfo: [
                    NSLocalizedDescriptionKey: "SupportClaw realtime tool call did not finish",
                ])
            }
            group.cancelAll()
            return result
        ...
        timeoutSeconds: Int) async throws -> String
    {
        ...
        while Date() < deadline {
            ...
            case "error":
                throw NSError(domain: "TalkRealtimeWebRTC", code: 14, userInfo: [
                    NSLocalizedDescriptionKey: wait.error ?? "SupportClaw realtime tool call failed",
                ])
            case "aborted", "cancelled", "canceled":
                throw NSError(domain: "TalkRealtimeWebRTC", code: 15, userInfo: [
                    NSLocalizedDescriptionKey: wait.stopReason ?? "SupportClaw realtime tool call aborted",
                ])
            case "timeout":
                break
            default:
        ...
        throw NSError(domain: "TalkRealtimeWebRTC", code: 16, userInfo: [
            NSLocalizedDescriptionKey: "SupportClaw realtime tool call timed out in \(phase)",
        ])
    }
        ...
        timeoutSeconds: Int) async throws -> AgentWaitResponse
    {
        ...
        guard let json = String(data: data, encoding: .utf8) else {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 17, userInfo: [
                NSLocalizedDescriptionKey: "Failed to encode SupportClaw wait request",
            ])
        }
        ...
        since: Double) async throws -> String?
    {
        ...
        guard let json = String(data: data, encoding: .utf8) else {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 18, userInfo: [
                NSLocalizedDescriptionKey: "Failed to encode SupportClaw history request",
            ])
        }
        let response = try await gateway.request(method: "chat.history", paramsJSON: json, timeoutSeconds: 15)
        let history = try JSONDecoder().decode(SupportClawChatHistoryPayload.self, from: response)
        let messages = history.messages ?? []
        let decoded: [SupportClawChatMessage] = messages.compactMap { item in
