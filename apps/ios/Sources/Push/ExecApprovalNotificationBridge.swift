...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "supportClaw.exec-approval"
    static let reviewActionIdentifier = "supportClaw.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.supportClawPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.supportClawPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func supportClawPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
