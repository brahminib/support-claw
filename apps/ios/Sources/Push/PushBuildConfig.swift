...
struct PushBuildConfig {
    ...
    static let supportClawHostedRelayHost = "ios-push-relay.supportClaw.ai"

    var usesSupportClawHostedRelay: Bool {
        ...
        return components.scheme?.lowercased() == "https"
            && components.host?.lowercased() == Self.supportClawHostedRelayHost
            && components.user == nil
    ...
    init(bundle: Bundle = .main) {
        self.transport = Self.readEnum(
            bundle: bundle,
            key: "SupportClawPushTransport",
            fallback: .direct)
        self.distribution = Self.readEnum(
            bundle: bundle,
            key: "SupportClawPushDistribution",
            fallback: .local)
        self.apnsEnvironment = Self.readEnum(
            bundle: bundle,
            key: "SupportClawPushAPNsEnvironment",
            fallback: Self.defaultAPNsEnvironment)
        self.relayBaseURL = Self.readURL(bundle: bundle, key: "SupportClawPushRelayBaseURL")
    }
