...
private struct WatchChatPreview {
    var items: [SupportClawWatchChatItem]
...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.supportClawfoundation.app", category: "DeepLink")
    ...
    private let pushWakeLogger = Logger(subsystem: "ai.supportClawfoundation.app", category: "PushWake")
    private let pendingActionLogger = Logger(subsystem: "ai.supportClawfoundation.app", category: "PendingAction")
    private let locationWakeLogger = Logger(subsystem: "ai.supportClawfoundation.app", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.supportClawfoundation.app", category: "WatchReply")
    private let watchExecApprovalLogger = Logger(subsystem: "ai.supportClawfoundation.app", category: "WatchExecApproval")
    private let execApprovalNotificationLogger = Logger(
        subsystem: "ai.supportClawfoundation.app",
    ...
    func makeChatTransport() -> any SupportClawChatTransport {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = SupportClawCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = SupportClawCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = SupportClawCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = SupportClawCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = SupportClawCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: SupportClawLocationMode) async -> Bool {
            ...
            check: { [weak self] in
                ...
                do {
                    let data = try await self.operatorGateway.request(
                        ...
                        paramsJSON: nil,
                        timeoutSeconds: 6)
                    guard let decoded = try? JSONDecoder().decode(SupportClawGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: SupportClawNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: SupportClawNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SupportClawNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(SupportClawLocationGetParams.self, from: req.paramsJSON)) ??
            SupportClawLocationGetParams()
        ...
        let payload = SupportClawLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportClawCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(SupportClawCanvasPresentParams.self, from: req.paramsJSON)) ??
                SupportClawCanvasPresentParams()
        ...
        case SupportClawCanvasCommand.hide.rawValue:
            self.screen.hideCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SupportClawCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(SupportClawCanvasNavigateParams.self, from: req.paramsJSON)
            let trimmedURL = params.url.trimmingCharacters(in: .whitespacesAndNewlines)
            self.screen.present(urlString: trimmedURL)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SupportClawCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(SupportClawCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SupportClawCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(SupportClawCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case SupportClawCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.supportClawA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing supportClawA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)

        case SupportClawCanvasA2UICommand.push.rawValue, SupportClawCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [SupportClawKit.AnyCodable]
            if command == SupportClawCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(SupportClawCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try SupportClawCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(SupportClawCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try SupportClawCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.supportClawA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportClawCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SupportClawCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(SupportClawCameraSnapParams.self, from: req.paramsJSON)) ??
                SupportClawCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SupportClawCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(SupportClawCameraClipParams.self, from: req.paramsJSON)) ??
                SupportClawCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(SupportClawScreenRecordParams.self, from: req.paramsJSON)) ??
            SupportClawScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(SupportClawSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SupportClawNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard Self.isNotificationAuthorizationAllowed(status) else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SupportClawNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SupportClawNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(SupportClawChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SupportClawNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = SupportClawChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportClawDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportClawDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(SupportClawPhotosLatestParams.self, from: req.paramsJSON)) ??
            SupportClawPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportClawContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(SupportClawContactsSearchParams.self, from: req.paramsJSON)) ??
                SupportClawContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportClawContactsCommand.add.rawValue:
            let params = try Self.decodeParams(SupportClawContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportClawCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(SupportClawCalendarEventsParams.self, from: req.paramsJSON)) ??
                SupportClawCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportClawCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(SupportClawCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportClawRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(SupportClawRemindersListParams.self, from: req.paramsJSON)) ??
                SupportClawRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportClawRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(SupportClawRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportClawMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(SupportClawMotionActivityParams.self, from: req.paramsJSON)) ??
                SupportClawMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportClawMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(SupportClawPedometerParams.self, from: req.paramsJSON)) ??
                SupportClawPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportClawTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportClawTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportClawTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportClawTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
extension NodeAppModel {
    ...
    private func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([SupportClawLocationCommand.get.rawValue]) { [weak self] req in
        ...
        register([
            ...
            SupportClawCanvasCommand.hide.rawValue,
            SupportClawCanvasCommand.navigate.rawValue,
            SupportClawCanvasCommand.evalJS.rawValue,
        ...
        register([
            ...
            SupportClawCanvasA2UICommand.push.rawValue,
            SupportClawCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([SupportClawScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([SupportClawSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([SupportClawChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SupportClawWatchCommand.status.rawValue,
            SupportClawWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([SupportClawPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SupportClawRemindersCommand.list.rawValue,
            SupportClawRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    private func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportClawWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = SupportClawWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportClawWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(SupportClawWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = SupportClawWatchNotifyPayload(
    ...
    private func locationMode() -> SupportClawLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return SupportClawLocationMode(rawValue: raw) ?? .off
    }
...
extension NodeAppModel {
    ...
    private func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "supportClaw-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "SupportClaw Share Self-Test",
            url: URL(string: "https://supportClaw.ai/share-self-test"),
    ...
    private static func makeWatchExecApprovalItem(from prompt: ExecApprovalPrompt) -> SupportClawWatchExecApprovalItem {
        let decisions = prompt.allowedDecisions.compactMap { decision in
            ...
            return SupportClawWatchExecApprovalDecision(rawValue: normalizedDecision)
        }
        let preview = Self.trimmedOrNil(prompt.commandPreview) ?? Self.trimmedOrNil(prompt.commandText)
        return SupportClawWatchExecApprovalItem(
            id: prompt.id,
    ...
    private func publishWatchExecApprovalPrompt(_ prompt: ExecApprovalPrompt, reason: String) async {
        let message = SupportClawWatchExecApprovalPromptMessage(
        ...
        source: String) async
    {
        ...
        let message = SupportClawWatchExecApprovalResolvedMessage(
    ...
    private func publishWatchExecApprovalExpired(
        approvalId: String,
        reason: SupportClawWatchExecApprovalCloseReason) async
    {
        let normalizedApprovalID = approvalId.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalizedApprovalID.isEmpty else { return }
        self.removeWatchExecApprovalPrompt(normalizedApprovalID)
        let message = SupportClawWatchExecApprovalExpiredMessage(
            approvalId: normalizedApprovalID,
            reason: reason,
    ...
    private func syncWatchExecApprovalSnapshot(reason: String) async {
        ...
        let message = SupportClawWatchExecApprovalSnapshotMessage(
            approvals: approvals,
            sentAtMs: Int(Date().timeIntervalSince1970 * 1000),
    ...
    private func makeWatchChatPreview() async -> WatchChatPreview {
        do {
            let payload: SupportClawChatHistoryPayload
            if self.isAppleReviewDemoModeEnabled {
    ...
    private nonisolated static func decodeWatchChatMessage(
        _ raw: SupportClawKit.AnyCodable) -> SupportClawChatMessage?
    {
        guard let data = try? JSONEncoder().encode(raw) else { return nil }
        return try? JSONDecoder().decode(SupportClawChatMessage.self, from: data)
    }

    private nonisolated static func makeWatchChatItems(
        from raw: [SupportClawKit.AnyCodable]) -> [SupportClawWatchChatItem]
    {
        var readableMessages: [(SupportClawChatMessage, String)] = []
        ...
        return Array(readableMessages.reversed()).enumerated().map { index, entry in
            ...
            return SupportClawWatchChatItem(
    ...
    private nonisolated static func watchChatText(from message: SupportClawChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            ...
            if let dict = content.content?.value as? [String: SupportClawKit.AnyCodable],
    ...
    private func makeWatchAppSnapshot(
        chatPreview: WatchChatPreview? = nil) -> SupportClawWatchAppSnapshotMessage
    {
        self.pruneExpiredWatchExecApprovalPrompts()
        ...
        return SupportClawWatchAppSnapshotMessage(
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.supportClawPushKind(userInfo)
        let receivedMessage =
    ...
    private static func supportClawPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["supportClaw"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["supportClaw"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        ...
        sourceReason: String? = nil) async -> ExecApprovalResolutionOutcome
    {
        ...
        guard connected else {
            ...
            return .failed(message: "SupportClaw couldn't connect to the gateway operator session.")
        }
        ...
        do {
            ...
            await self.publishWatchExecApprovalResolved(
                approvalId: normalizedApprovalID,
                decision: SupportClawWatchExecApprovalDecision(rawValue: normalizedDecision),
                source: "iphone")
            return .resolved
        } catch {
            if Self.isApprovalNotificationStaleError(error) {
            ...
            return .failed(
                message: "SupportClaw couldn't resolve this approval right now. Try again.")
        }
    }
...
extension NodeAppModel {
    ...
    nonisolated static func _test_makeWatchChatItems(from raw: [SupportClawKit.AnyCodable]) -> [SupportClawWatchChatItem] {
