...
final class LiveActivityManager {
    ...
    private let logger = Logger(subsystem: "ai.supportClawfoundation.app", category: "LiveActivity")
    private let connectingStaleSeconds: TimeInterval = 120
    private let hydrationStaleSeconds: TimeInterval = 300
    private var currentActivity: Activity<SupportClawActivityAttributes>?
    ...
    func showConnecting(statusText: String = "Connecting...", agentName: String, sessionKey: String) {
        ...
        let attributes = SupportClawActivityAttributes(agentName: agentName, sessionKey: sessionKey)
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<SupportClawActivityAttributes>.activities
    ...
    private func updateCurrent(state: SupportClawActivityAttributes.ContentState, staleDate: Date? = nil) {
    ...
    private func end(activity: Activity<SupportClawActivityAttributes>) {
        Task {
            await activity.end(
                ActivityContent(state: self.disconnectedState(), staleDate: nil),
                dismissalPolicy: .immediate)
    ...
    private func connectingState(statusText: String = "Connecting...") -> SupportClawActivityAttributes.ContentState {
        SupportClawActivityAttributes.ContentState(
    ...
    private func attentionState(statusText: String) -> SupportClawActivityAttributes.ContentState {
        SupportClawActivityAttributes.ContentState(
            statusText: statusText,
            isIdle: false,
            isDisconnected: false,
    ...
    private func disconnectedState() -> SupportClawActivityAttributes.ContentState {
        SupportClawActivityAttributes.ContentState(
