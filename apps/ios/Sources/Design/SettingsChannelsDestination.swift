...
struct SettingsChannelsDestination: View {
    ...
    private var summaryColor: Color {
        ...
        if self.errorText != nil { return SupportClawBrand.warn }
        return self.channelEntries.contains(where: { $0.running || $0.connected }) ? SupportClawBrand.ok : SupportClawBrand
            .accent
    }
...
private struct SettingsChannelEntry: Identifiable {
    ...
    var color: Color {
        if self.connected || self.running { return SupportClawBrand.ok }
        if self.lastError != nil { return SupportClawBrand.warn }
        return self.configured ? SupportClawBrand.accent : .secondary
    }
