...
extension SettingsProTab {
        ...
        ProCard(padding: 0, radius: SettingsLayout.cardRadius) {
            VStack(spacing: 0) {
                ...
                self.diagnosticCheckRow(
                    ...
                    color: self.gatewayController.gateways.isEmpty ? .secondary : SupportClawBrand.accent)
                Divider().padding(.leading, 60)
                self.diagnosticCheckRow(
                    icon: "waveform",
                    title: "Talk Config",
                ...
                self.diagnosticCheckRow(
                    ...
                    color: self.appModel.screenRecordActive ? SupportClawBrand.ok : .secondary)
                Divider().padding(.leading, 60)
                self.diagnosticCheckRow(
                    icon: "mic",
                    title: "Voice Wake",
                    ...
                    color: self.voiceWakeEnabled ? SupportClawBrand.ok : .secondary)
            }
        }
        .padding(.horizontal, SupportClawProMetric.pagePadding)
    }
    ...
    func handleLocationModeChange(_ newValue: String) {
        ...
        guard let mode = SupportClawLocationMode(rawValue: newValue) else { return }
    ...
    func applyLocationMode(
        _ mode: SupportClawLocationMode,
        rawValue: String,
    ...
    func handleNotificationAction() {
        ...
        if PushBuildConfig.current.usesSupportClawHostedRelay {
    ...
    func friendlyGatewayMessage(from raw: String) -> String? {
        ...
        if lower.contains("pairing required") {
            return "Pairing required. Run /pair approve in your SupportClaw chat, then connect again."
        }
    ...
    var gatewayStatusColor: Color {
        if self.appModel.isAppleReviewDemoModeEnabled { return SupportClawBrand.accent }
        return self.gatewayConnected ? SupportClawBrand.ok : .secondary
    }
    ...
    var approvalEmptyDetail: String {
        ...
        if self.notificationsNeedAttention {
            return "Foreground approvals still appear while SupportClaw is connected."
        }
    ...
    var gatewayTalkConfigColor: Color {
        ...
        return self.appModel.talkMode.gatewayTalkConfigLoaded ? SupportClawBrand.ok : .secondary
    }
    ...
    var gatewayServer: String {
        self.appModel.gatewayServerName ?? "SupportClaw Gateway"
    }

    var permissionsDetail: String {
        var enabled = 0
        if self.cameraEnabled { enabled += 1 }
        if self.locationModeRaw != SupportClawLocationMode.off.rawValue { enabled += 1 }
    ...
    var diagnosticsRunColor: Color {
        ...
        return diagnosticsIssueCount == 0 ? SupportClawBrand.ok : SupportClawBrand.warn
    }

    var privacyDetail: String {
        let location = SupportClawLocationMode(rawValue: self.locationModeRaw) ?? .off
        return location == .off ? "Location off" : "Location \(self.locationLabel)"
    }

    var locationLabel: String {
        switch SupportClawLocationMode(rawValue: self.locationModeRaw) ?? .off {
        case .off: "Off"
    ...
    var notificationStatusDetail: String {
        ...
        case .allowed:
            "SupportClaw can show approval prompts and event alerts when the app is not active."
        case .notAllowed:
        ...
        case .unknown:
            "SupportClaw cannot determine the current notification permission state."
        }
    }
    ...
    var notificationRelayDetail: String {
        if PushBuildConfig.current.usesSupportClawHostedRelay {
            return """
            This build uses SupportClaw's hosted push relay at ios-push-relay.supportClaw.ai for notification \
            delivery data.
            """
        }
        return "This build is not configured to use SupportClaw's hosted push relay."
    }

    var notificationRelayDisclosureMessage: String {
        "Enabling this sends delivery data through SupportClaw's hosted push relay."
    }
}
