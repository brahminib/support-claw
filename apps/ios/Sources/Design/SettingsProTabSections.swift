...
extension SettingsProTab {
    ...
    var settingsListSection: some View {
        VStack(spacing: 10) {
            self.settingsListRow(
                ...
                route: .approvals,
                color: self.pendingApproval == nil ? .secondary : SupportClawBrand.warn,
            ...
            self.settingsListRow(
                icon: "info.circle",
                title: "About",
                detail: DeviceInfoHelper.supportClawVersionString(),
                route: .about)
        }
        .padding(.horizontal, SupportClawProMetric.pagePadding)
    }
    ...
    func destination(for route: SettingsRoute) -> some View {
        ZStack {
            ...
            ScrollView {
                ...
                .padding(.top, 18)
                .padding(.bottom, SupportClawProMetric.bottomScrollInset)
            }
        }
    ...
    var approvalsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.notificationsNeedAttention ? SupportClawBrand.warn :
                    (self.pendingApproval == nil ? SupportClawBrand.ok : SupportClawBrand.warn))
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .top, spacing: 12) {
                    ProIconBadge(systemName: "bell.slash.fill", color: SupportClawBrand.warn)
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Notifications are off")
                            .font(.subheadline.weight(.semibold))
                        Text(
                            """
                            Enable Notifications to receive approval notifications while SupportClaw is not open.
                            """)
                            .font(.caption)
                            .foregroundStyle(.secondary)
                ...
                if let pendingApproval {
                    ...
                    if let errorText = self.appModel.pendingExecApprovalPromptErrorText {
                        Text(errorText)
                            ...
                            .foregroundStyle(SupportClawBrand.danger)
                    }
                    ...
                    HStack(spacing: 12) {
                        ProIconBadge(systemName: "checkmark.shield.fill", color: SupportClawBrand.ok)
                        VStack(alignment: .leading, spacing: 3) {
    ...
    var permissionsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.toggleCard(
                icon: "camera",
                title: "Camera",
                detail: "Allow the gateway to request photos or video while SupportClaw is foregrounded.",
            ...
            self.toggleCard(
                icon: "lock.display",
                title: "Keep Awake",
                detail: "Keep the screen awake while SupportClaw is open.",
    ...
    var voiceDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.talkEnabled || self.voiceWakeEnabled ? SupportClawBrand.accent : .secondary)
    ...
    var diagnosticsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.gatewayDiagnosticConnected ? SupportClawBrand.ok : SupportClawBrand.warn)
            ...
            self.detailListCard {
                ...
                Divider()
                self.detailRow("App", value: DeviceInfoHelper.supportClawVersionString())
                Divider()
    ...
    var privacyDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "hand.raised",
                title: "Privacy",
                detail: "Control what device context SupportClaw can expose to the gateway.",
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack(alignment: .top, spacing: 10) {
                        Image(systemName: "network")
                            .font(.caption.weight(.semibold))
                            .foregroundStyle(SupportClawBrand.accent)
    ...
    var aboutDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                value: DeviceInfoHelper.supportClawVersionString(),
                color: SupportClawBrand.accent)

            self.detailListCard {
                self.detailRow("Version", value: DeviceInfoHelper.supportClawVersionString())
                Divider()
                self.detailRow("Device", value: DeviceInfoHelper.deviceFamily())
                Divider()
                self.detailRow("Platform", value: DeviceInfoHelper.platformStringForDisplay())
        ...
        isOn: Binding<Bool>) -> some View
    {
        ProCard(radius: SettingsLayout.cardRadius) {
            Toggle(isOn: isOn) {
                HStack(spacing: 12) {
                    ProIconBadge(systemName: icon, color: isOn.wrappedValue ? SupportClawBrand.accent : .secondary)
                    VStack(alignment: .leading, spacing: 3) {
                        Text(title)
                            .font(.subheadline.weight(.semibold))
                        Text(detail)
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 12) {
                    ProIconBadge(
                        systemName: "location",
                        color: self.locationModeRaw == SupportClawLocationMode.off.rawValue ? .secondary : SupportClawBrand
                            .accent)
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Location")
                            .font(.subheadline.weight(.semibold))
                ...
                Picker("Location", selection: self.$locationModeRaw) {
                    Text("Off").tag(SupportClawLocationMode.off.rawValue)
                    Text("While Using").tag(SupportClawLocationMode.whileUsing.rawValue)
                    Text("Always").tag(SupportClawLocationMode.always.rawValue)
                }
                .pickerStyle(.segmented)
                ...
                if let locationStatusText {
                    Text(locationStatusText)
                        .font(.caption2)
                        .foregroundStyle(SupportClawBrand.warn)
                }
            }
        }
        .padding(.horizontal, SupportClawProMetric.pagePadding)
    ...
    func settingsSwitchIndicator(isOn: Bool) -> some View {
        Capsule()
            .fill(isOn ? SupportClawBrand.accent : Color.secondary.opacity(0.35))
