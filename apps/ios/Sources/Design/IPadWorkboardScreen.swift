            ...
            Text(IPadWorkboardDefaults.label(for: status))
                ...
                .overlay {
                    Capsule()
                        .strokeBorder(
                            self.selectedStatus == status
                                ? SupportClawBrand.accent.opacity(0.42)
                                : Color.primary.opacity(0.08),
                            lineWidth: 1)
                }
        }
        .buttonStyle(.plain)
        .foregroundStyle(self.selectedStatus == status ? SupportClawBrand.accent : .primary)
...
private struct IPadWorkboardKanbanCard: View {
    ...
    private var color: Color {
        switch self.card.status {
        case "running": SupportClawBrand.ok
        case "review": SupportClawBrand.accent
        case "blocked": SupportClawBrand.warn
        case "done": .secondary
        default: SupportClawBrand.accentHot
        }
    }
    ...
    var body: some View {
        ...
        .swipeActions(edge: .trailing, allowsFullSwipe: false) {
            if let nextStatus {
                ...
                .tint(SupportClawBrand.accentHot)
            }
            Button(self.card.metadata?.archivedAt == nil ? "Archive" : "Unarchive", action: self.archive)
                .tint(.secondary)
        }
