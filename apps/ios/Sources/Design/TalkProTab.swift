...
struct TalkProTab: View {
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 11) {
            ...
            SupportClawProMark(size: 31, shadowRadius: 9)
...
struct TalkProState: Equatable {
    ...
    var title: String {
        ...
        if self.normalizedStatus.contains("thinking") { return "Asking SupportClaw" }
    ...
    var color: Color {
        ...
        case .requestFailed, .loadFailed:
            return SupportClawBrand.danger
        case .missingScope, .requestingUpgrade, .upgradeRequested, .apiKeyMissing:
            return SupportClawBrand.warn
        default:
            if !self.isConfigLoaded { return SupportClawBrand.warn }
            return self.isEnabled ? SupportClawBrand.ok : SupportClawBrand.accentHot
        }
    }
    ...
    var primaryButtonFill: AnyShapeStyle {
        switch self.primaryAction {
        case .stop:
            AnyShapeStyle(SupportClawBrand.danger)
        case .waiting:
            AnyShapeStyle(SupportClawBrand.warn.opacity(0.72))
        default:
            AnyShapeStyle(LinearGradient(
                colors: [self.color.opacity(0.95), SupportClawBrand.accent],
