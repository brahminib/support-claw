...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.supportClawfoundation.app", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to SupportClaw", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to SupportClaw gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to SupportClaw.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "SupportClawShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "SupportClaw is not connected to a gateway yet."])
        }
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("supportClaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
