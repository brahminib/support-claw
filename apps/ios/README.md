# SupportClaw iOS (Super Alpha)

This iOS app is super-alpha and internal-use only. The first public App Store release targets iPhone and connects to an SupportClaw Gateway as a `role: node`.
...
- `xcodegen`
- `fastlane`
- Apple account signed into Xcode for the canonical SupportClaw team (`FWJYW4S8P8`)
- Fastlane Apple Developer Portal session for the canonical SupportClaw team when creating bundle IDs or enabling services
- Release-owner access to the encrypted signing repo password (`MATCH_PASSWORD`)
- App Store Connect app already created for `ai.supportClawfoundation.app`
...
- Local development uses the canonical `ai.supportClawfoundation.app*` bundle IDs when the SupportClaw team is available, and unique `ai.supportClawfoundation.app.test.*` bundle IDs only for non-canonical fallback teams.
- App Store release uses canonical `ai.supportClawfoundation.app*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/AppStoreRelease.xcconfig`.
...
- App Store release also switches the app to `SupportClawPushTransport=relay`, `SupportClawPushDistribution=official`, `SupportClawPushAPNsEnvironment=production`, and a production `aps-environment` entitlement.
...
- Release builds default to `https://ios-push-relay.supportClaw.ai`.
- Optional custom relay override: `SUPPORT_CLAW_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
3. Confirm the App Store Connect app and Apple Developer identifiers/capabilities exist for:
   - `ai.supportClawfoundation.app`
   - `ai.supportClawfoundation.app.share`
   - `ai.supportClawfoundation.app.activitywidget`
   - `ai.supportClawfoundation.app.watchkitapp`
...
4. Optional: set a custom official relay URL for the build. If unset, the release flow uses `https://ios-push-relay.supportClaw.ai`.

```bash
export SUPPORT_CLAW_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
8. Expected outputs after a successful run:
   - `apps/ios/build/app-store/SupportClaw-<version>.ipa`
   - `apps/ios/build/app-store/SupportClaw-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/SupportClaw.entitlements` derives `aps-environment` from the active build configuration/signing override.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `SupportClawPushTransport=direct`, `SupportClawPushDistribution=local`, and a development `aps-environment` entitlement.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `SUPPORT_CLAW_APNS_TEAM_ID`, `SUPPORT_CLAW_APNS_KEY_ID`, and either `SUPPORT_CLAW_APNS_PRIVATE_KEY_P8` or `SUPPORT_CLAW_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.supportClaw/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `SUPPORT_CLAW_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `SupportClawPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `supportClaw.json`. `SUPPORT_CLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine SupportClaw official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as SupportClaw node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
