...
```bash
ANDROID_SCREENSHOT_AVD=SupportClaw_QA_API35 pnpm android:screenshots
```
...
- Play build: `supportClaw-<version>-play-release.aab`
- Third-party build: `supportClaw-<version>-third-party-release.apk`
...
```bash
pnpm supportClaw gateway --port 18789 --verbose
```
...
- Canvas web content already supports live reload when loaded from Gateway `__supportClaw__/canvas/` (see `docs/platforms/android.md`).
...
Current SupportClaw Android implication:
...
2) Android app is connected to that gateway and `supportClaw nodes status` shows it as paired + connected.
3) App stays unlocked and in foreground for the whole run.
4) Open the app **Screen** tab and keep it active during the run (canvas/A2UI commands require the canvas WebView attached there).
5) Grant runtime permissions for capabilities you expect to pass (camera/mic/location/notification listener/location, etc.).
6) No interactive system dialogs should be pending before test start.
7) Canvas host is enabled and reachable from the device for remote Canvas checks (do not run gateway with `SUPPORT_CLAW_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__supportClaw__/`).
...
- `SUPPORT_CLAW_ANDROID_GATEWAY_URL=ws://...` (default: from your local SupportClaw config)
- `SUPPORT_CLAW_ANDROID_GATEWAY_TOKEN=...`
- `SUPPORT_CLAW_ANDROID_GATEWAY_PASSWORD=...`
...
- `pairing required` before tests start:
  - list pending requests (`supportClaw devices list`), then approve with the exact ID (`supportClaw devices approve <requestId>`) and rerun.
