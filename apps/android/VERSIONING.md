# SupportClaw Android Versioning
...
If `MATCH_PASSWORD` is not set, the existing manual Gradle-property signing path still works: provide `SUPPORT_CLAW_ANDROID_STORE_FILE`, `SUPPORT_CLAW_ANDROID_STORE_PASSWORD`, `SUPPORT_CLAW_ANDROID_KEY_ALIAS`, and `SUPPORT_CLAW_ANDROID_KEY_PASSWORD` through your local Gradle user properties before running release tasks.
