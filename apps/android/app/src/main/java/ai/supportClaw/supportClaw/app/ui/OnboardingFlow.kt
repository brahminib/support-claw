...
import ai.supportClaw.app.GatewayConnectionProblem
...
import ai.supportClaw.app.R
import ai.supportClaw.app.SensitiveFeatureConfig
import ai.supportClaw.app.node.DeviceNotificationListenerService
import ai.supportClaw.app.ui.design.ClawDesignTheme
import ai.supportClaw.app.ui.design.ClawErrorState
import ai.supportClaw.app.ui.design.ClawListItem
...
fun OnboardingFlow(
  ...
  ClawDesignTheme(dark = onboardingDark) {
    ...
    SupportClawSystemBarAppearance(lightAppearance = !onboardingDark && step != OnboardingStep.Welcome)
...
private fun WelcomeLogo() {
  Surface(
    ...
    Box(modifier = Modifier.fillMaxSize().padding(12.dp), contentAlignment = Alignment.Center) {
      Image(painter = painterResource(id = R.drawable.supportClaw_logo), contentDescription = "SupportClaw logo", modifier = Modifier.fillMaxSize())
    }
  }
}
      ...
      LazyColumn(
        ...
        item {
          Column(modifier = Modifier.padding(top = 10.dp, bottom = 18.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Text(
              text = "These permissions keep SupportClaw secure\nand useful.",
              style = ClawTheme.type.body,
              color = ClawTheme.colors.textMuted,
            )
          }
...
private fun PermissionTopBar(onBack: () -> Unit) {
  ...
  if (showHelp) {
    AlertDialog(
      ...
      text = {
        Text(
          "Choose what this phone can share with SupportClaw. You can change these later in Settings.",
          style = ClawTheme.type.body,
          color = ClawTheme.colors.textMuted,
        )
      },
...
internal enum class GatewayRecoveryUiState(
  ...
  Pairing(
    title = "Pairing Gateway",
    message = "Approval is in progress.\nSupportClaw will reconnect automatically.",
  ),
  Finishing(
    title = "Connecting Gateway",
    message = "SupportClaw is checking gateway and node access.",
  ),
  Failed(
  ...
  remoteAddress
    ...
    ?: if (ready) {
      ...
      recoveryGatewayApprovalCommand(gatewayConnectionProblem)
        ...
        ?: "Gateway approval is pending. Run supportClaw devices list on the gateway host, approve this phone, then retry."
      ...
      "Gateway approval is in progress. SupportClaw will retry automatically."
    } else {
      "Gateway unreachable"
    }
...
private fun copyApprovalCommand(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("SupportClaw pairing approval command", command))
...
private fun copyGatewayDiagnostic(
  ...
  val diagnostic =
    listOfNotNull(
      "SupportClaw Android gateway diagnostic",
      "Status: $statusText",
  ...
  val clipboard = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
  clipboard.setPrimaryClip(ClipData.newPlainText("SupportClaw gateway diagnostic", diagnostic))
