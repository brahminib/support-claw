...
internal fun supportClawAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this SupportClaw Android gateway connection failure.
    ...
    - tell me whether `supportClaw devices list` should show a pending pairing request
    - if more signal is needed, ask for `supportClaw qr --json`, `supportClaw devices list`, and `supportClaw nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${supportClawAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("SupportClaw gateway diagnostics", report))
