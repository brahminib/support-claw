# fastlane setup (SupportClaw Android)
...
Create a Google Play service account JSON key with Google Play Developer API access, then grant that service account access to the SupportClaw app in Play Console.
...
```bash
GOOGLE_PLAY_PACKAGE_NAME=ai.supportClaw.app
```
...
SUPPORT_CLAW_ANDROID_UPLOAD_KEYSTORE=<path-to-upload-keystore.jks> \
SUPPORT_CLAW_ANDROID_SIGNING_PROPERTIES=<path-to-android-signing.properties> \
...
```properties
SUPPORT_CLAW_ANDROID_STORE_PASSWORD=<store-password>
SUPPORT_CLAW_ANDROID_KEY_ALIAS=<upload-key-alias>
SUPPORT_CLAW_ANDROID_KEY_PASSWORD=<key-password>
```
