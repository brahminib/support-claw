...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.supportClaw.mac`) even if the shipped
    ...
    {
        if environment["SUPPORT_CLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "supportClaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "supportClaw.nixMode") { return true }
