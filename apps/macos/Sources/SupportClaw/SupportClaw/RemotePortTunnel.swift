...
final class RemotePortTunnel: @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.supportClaw", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(defaultRemotePort: Int, for sshHost: String) -> Int? {
        ...
        guard let sshKey = SupportClawConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = SupportClawConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.supportClaw.remote.tunnel.port", qos: .utility)
            do {
