...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SUPPORT_CLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/supportClaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("supportClaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("supportClaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/supportClaw/ (rolling or stdout), or nil if none exist.
