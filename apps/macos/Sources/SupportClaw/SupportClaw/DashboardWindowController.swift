...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, WKUIDelegate, NSWindowDelegate {
    ...
    private static func installNativeChromeScript(into userContentController: WKUserContentController) {
        let css = """
        html.supportClaw-native-macos {
          --supportClaw-native-titlebar-height: 50px;
        }
        @media (min-width: 700px) {
          html.supportClaw-native-macos .sidebar-shell {
            padding-top: max(14px, var(--supportClaw-native-titlebar-height)) !important;
          }
          html.supportClaw-native-macos .sidebar-shell__header {
        ...
        (() => {
          try {
            if (document.getElementById("supportClaw-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "supportClaw-native-macos-chrome";
            style.textContent = \(Self.jsStringLiteral(css));
            document.documentElement.classList.add("supportClaw-native-macos");
        ...
        auth: DashboardWindowAuth)
    {
        ...
        (() => {
          try {
            ...
            Object.defineProperty(window, "__SUPPORT_CLAW_NATIVE_CONTROL_AUTH__", {
              value: \(json),
              configurable: true,
            });
          } catch {}
