...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.supportClaw/supportClaw.json. Prefer SUPPORT_CLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
        ...
        isPaused: Bool) -> GatewayTailscaleApplyMessages
    {
        ...
        let statusMessage = if connectionMode == .local, !isPaused {
            "Saved to ~/.supportClaw/supportClaw.json. Restarting gateway…"
        } else {
            "Saved to ~/.supportClaw/supportClaw.json. Restart the gateway to apply."
        }
        return GatewayTailscaleApplyMessages(
