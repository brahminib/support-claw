...
final class AppState {
    private static let logger = Logger(subsystem: "ai.supportClaw", category: "app-state")
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = SupportClawConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            SupportClawConfigFile.canonicalHostForComparison(host) ==
                SupportClawConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = SupportClawConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: SupportClawConfigFile.loadDict(),
            draft: .init(
        ...
        guard SupportClawConfigFile.saveDict(synced.root) else {
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/supportClaw"
        state.remoteCliPath = ""
        return state
    }
}
