...
enum RuntimeLocator {
    private static let logger = Logger(subsystem: "ai.supportClaw", category: "runtime")
    ...
    static func describeFailure(_ error: RuntimeResolutionError) -> String {
        ...
        case let .notFound(searchPaths):
            [
                "supportClaw needs Node >=22.19.0 but found no runtime.",
        ...
        case let .unsupported(kind, found, required, path, searchPaths):
            [
                ...
                "PATH searched: \(searchPaths.joined(separator: ":"))",
                "Upgrade Node and rerun supportClaw.",
            ].joined(separator: "\n")
