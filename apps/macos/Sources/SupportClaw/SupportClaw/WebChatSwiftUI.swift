...
private let webChatSwiftLogger = Logger(subsystem: "ai.supportClaw", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "supportClaw.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: SupportClawChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> SupportClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(SupportClawChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            SupportClawChatSessionsDefaults(
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SupportClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SupportClawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    SupportClawChatEventPayload.self,
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    SupportClawSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    SupportClawAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: SupportClawProtocol.ModelChoice) -> SupportClawChatModelChoice {
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SupportClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SupportClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SupportClawChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: SupportClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SupportClawChatView>) -> NSViewController
    {
