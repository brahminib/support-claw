...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SupportClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.supportClaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SupportClawKit.AnyCodable]()) {
                $0[$1.key] = SupportClawKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking SupportClaw Local Network access.
                    Allow SupportClaw in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SupportClawProtocol.AnyCodable?) -> [String: SupportClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SupportClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SupportClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SupportClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("supportClaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("supportClaw.control.agent")
}
