...
final class CanvasWindowController: NSWindowController, WKNavigationDelegate, WKUIDelegate, NSWindowDelegate {
    ...
    init(sessionKey: String, root: URL, presentation: CanvasPresentation) throws {
        ...
        (() => {
          try {
            ...
            if (globalThis.__supportClawA2UIBridgeInstalled) return;
            globalThis.__supportClawA2UIBridgeInstalled = true;
            ...
            globalThis.addEventListener('a2uiaction', (evt) => {
              try {
                ...
                const handler = globalThis.webkit?.messageHandlers?.supportClawCanvasA2UIAction;
                ...
                const hasBundledA2UIHost =
                  ...
                  !!document.querySelector('supportClaw-a2ui-host');
    ...
    func snapshot(to outPath: String?) async throws -> String {
        ...
        if let outPath, !outPath.isEmpty {
            ...
            path = "/tmp/supportClaw-canvas-\(CanvasWindowController.sanitizeSessionKey(self.sessionKey))-\(ts).png"
        }
