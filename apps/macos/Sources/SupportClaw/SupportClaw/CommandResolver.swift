...
enum CommandResolver {
    private static let projectRootDefaultsKey = "supportClaw.gatewayProjectRootPath"
    private static let helperName = "supportClaw"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let supportClawEntry = root.appendingPathComponent("supportClaw.mjs").path
        if FileManager().isReadableFile(atPath: supportClawEntry) { return supportClawEntry }
        let binEntry = root.appendingPathComponent("bin/supportClaw.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__SUPPORT_CLAW_ERR__' >&2
        \(message)
        __SUPPORT_CLAW_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/supportClaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let supportClawPaths = self.supportClawManagedPaths(home: home)
        if !supportClawPaths.isEmpty {
            extras.insert(contentsOf: supportClawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + supportClawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func supportClawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".supportClaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func supportClawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSupportClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("supportClaw.mjs").path,
            root.appendingPathComponent("bin/supportClaw.js").path,
        ]
    ...
    static func hasAnySupportClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.supportClawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func supportClawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let supportClawPath = self.projectSupportClawExecutable(projectRoot: root) {
            return [supportClawPath, subcommand] + extraArgs
        }
        if let supportClawPath = self.supportClawExecutable(searchPaths: searchPaths) {
            return [supportClawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "supportClaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            supportClaw entrypoint missing (looked for dist/index.js or supportClaw.mjs); run pnpm build.
            """
    ...
    static func supportClawCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real supportClaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/supportClaw"
        ...
        if command -v supportClaw >/dev/null 2>&1; then
          CLI="$(command -v supportClaw)"
          supportClaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/supportClaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/supportClaw.mjs"
            node "$PRJ/supportClaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/supportClaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/supportClaw.js"
            node "$PRJ/bin/supportClaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent supportClaw"
          pnpm --silent supportClaw \(quotedArgs);
        else
          echo "supportClaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SupportClawConfigFile.loadDict()
