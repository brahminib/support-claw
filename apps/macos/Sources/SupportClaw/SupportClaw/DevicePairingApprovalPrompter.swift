...
final class DevicePairingApprovalPrompter {
    ...
    private let logger = Logger(subsystem: "ai.supportClaw", category: "device-pairing")
    ...
    static func alertSummary(for req: PendingRequest) -> String {
        ...
        return "Approve \(subject) to control SupportClaw. Only approve if this is yours; you can remove it later in Settings."
    }
    ...
    static func deviceName(for req: PendingRequest) -> String {
        ...
        return self.isMac(req.platform) ? "SupportClaw Mac app" : "New device"
    }
