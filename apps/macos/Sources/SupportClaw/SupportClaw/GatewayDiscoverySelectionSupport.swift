...
enum GatewayDiscoverySelectionSupport {
        ...
        state: AppState)
    {
        ...
        if preferredTransport == .direct {
            SupportClawConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.direct.rawValue)
            if !state.remoteUrl.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                SupportClawConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
            } else {
                SupportClawConfigFile.clearRemoteGatewayUrl()
            }
        } else {
            SupportClawConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.ssh.rawValue)
            SupportClawConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
        }
    }
