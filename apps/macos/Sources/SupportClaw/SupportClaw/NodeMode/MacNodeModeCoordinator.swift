...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.supportClaw", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = SupportClawConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: SupportClawNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    nonisolated static func resolvedCaps(
        ...
        cameraEnabled: Bool,
        locationMode: SupportClawLocationMode,
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        ...
        if browserControlEnabled, connectionMode == .local {
            caps.append(SupportClawCapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(SupportClawCapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(SupportClawCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps() -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: SupportClawConfigFile.browserControlEnabled(),
            cameraEnabled: UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false,
            locationMode: SupportClawLocationMode(rawValue: rawLocationMode) ?? .off,
            connectionMode: AppStateStore.shared.connectionMode)
    }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            SupportClawSystemCommand.which.rawValue,
            SupportClawSystemCommand.run.rawValue,
            SupportClawSystemCommand.execApprovalsGet.rawValue,
            SupportClawSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(SupportClawCapability.browser.rawValue) {
            commands.append(SupportClawBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(SupportClawCapability.camera.rawValue) {
        ...
        if capsSet.contains(SupportClawCapability.location.rawValue) {
    ...
    private func buildSessionBox(url: URL, connectionMode: AppState.ConnectionMode) -> WebSocketSessionBox? {
        ...
        guard let params = Self.tlsParams(
            ...
            root: SupportClawConfigFile.loadDict(),
