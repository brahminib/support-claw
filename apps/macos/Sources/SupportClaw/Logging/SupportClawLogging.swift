...
enum SupportClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SupportClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SupportClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SupportClawLogging.bootstrapIfNeeded()
        let label = SupportClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SupportClawOSLogHandler: AppLogLevelBackedHandler {
...
struct SupportClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SupportClawLogging.parseLabel(self.label)
