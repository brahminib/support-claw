...
struct SupportClawMacCLI {
    static func main() async {
        ...
        case let .unknown(exitCode):
            fputs("supportClaw-mac: unknown command\n", stderr)
            printUsage()
            exit(exitCode)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      supportClaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      supportClaw-mac discover [--timeout <ms>] [--json] [--include-local]
      supportClaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      supportClaw-mac connect
      supportClaw-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      supportClaw-mac connect --url ws://127.0.0.1:18789 --json
      supportClaw-mac discover --timeout 3000 --json
      supportClaw-mac wizard --mode local
    """)
}
