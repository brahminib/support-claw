...
private let appDefaultsSuites = ["ai.supportClaw.mac", "ai.supportClaw.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              supportClaw-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
                                          [--remote-port <port>] [--token <token>] [--password <password>]
                                          [--identity <path>] [--project-root <path>] [--cli-path <path>] [--json]
              supportClaw-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote SupportClaw checkout for CLI commands.
              --cli-path <path>   Remote supportClaw executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
...
private func configureSSHRemote(_ opts: ConfigureRemoteOptions) throws -> ConfigureRemoteOutput {
    ...
    let configURL = supportClawConfigURL()
...
private func supportClawConfigURL() -> URL {
    if let raw = ProcessInfo.processInfo.environment["SUPPORT_CLAW_CONFIG_PATH"],
       !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
        return URL(fileURLWithPath: NSString(string: raw).expandingTildeInPath)
    }
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".supportClaw/supportClaw.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String) {
    for suite in appDefaultsSuites {
        ...
        defaults.set("remote", forKey: "supportClaw.connectionMode")
        setDefaultString(defaults, key: "supportClaw.remoteTarget", value: target)
        defaults.set(true, forKey: "supportClaw.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "supportClaw.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "supportClaw.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "supportClaw.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "supportClaw.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("SupportClaw macOS Remote Config")
