...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(SupportClawCapability.browser.rawValue))
        #expect(!commands.contains(SupportClawBrowserCommand.proxy.rawValue))
        #expect(commands.contains(SupportClawCanvasCommand.present.rawValue))
