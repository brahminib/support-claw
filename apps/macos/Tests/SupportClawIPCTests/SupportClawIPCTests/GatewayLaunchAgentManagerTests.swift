...
struct GatewayLaunchAgentManagerTests {
    @Test func `attach only runtime override does not uninstall gateway launch agent`() throws {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("supportClaw-attach-only-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `launch agent plist snapshot parses args and env`() throws {
        let url = FileManager().temporaryDirectory
            .appendingPathComponent("supportClaw-launchd-\(UUID().uuidString).plist")
        let plist: [String: Any] = [
            "ProgramArguments": ["supportClaw", "gateway", "--port", "18789", "--bind", "loopback"],
            "EnvironmentVariables": [
                "SUPPORT_CLAW_GATEWAY_TOKEN": " secret ",
                "SUPPORT_CLAW_GATEWAY_PASSWORD": "pw",
            ],
        ]
