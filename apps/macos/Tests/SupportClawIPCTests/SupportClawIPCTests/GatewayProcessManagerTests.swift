...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["SUPPORT_CLAW_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "supportClaw-gateway",
                executablePath: "/tmp/supportClaw-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 supportClaw-gateway @ /tmp/supportClaw-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
