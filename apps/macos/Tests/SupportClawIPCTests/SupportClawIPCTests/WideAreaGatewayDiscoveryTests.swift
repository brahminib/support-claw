...
import Foundation
import Testing
@testable import SupportClawDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("SUPPORT_CLAW_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("SUPPORT_CLAW_WIDE_AREA_DOMAIN", "supportClaw.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("SUPPORT_CLAW_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("SUPPORT_CLAW_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._supportClaw-gw._tcp.supportClaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.supportClaw.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (SupportClaw)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/supportClaw/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (SupportClaw)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/supportClaw/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._supportClaw-gw._tcp.supportClaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
