...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: SupportClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SupportClawProtocol.AnyCodable] = [
            "presence": .init([SupportClawProtocol.AnyCodable(entry)]),
        ]
        let payload = SupportClawProtocol.AnyCodable(payloadMap)
