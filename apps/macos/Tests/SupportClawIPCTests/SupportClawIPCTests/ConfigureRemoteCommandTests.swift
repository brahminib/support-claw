import Foundation
import Testing
@testable import SupportClawMacCLI
...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("supportClaw-configure-remote-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        let defaultSuites = ["ai.supportClaw.mac", "ai.supportClaw.mac.debug"]
        let keys = [
            "supportClaw.connectionMode",
            "supportClaw.remoteTarget",
            "supportClaw.onboardingSeen",
            "supportClaw.onboardingVersion",
            "supportClaw.remoteCliPath",
        ]
        ...
        try await TestIsolation.withIsolatedState(env: ["SUPPORT_CLAW_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "supportClaw.connectionMode") == "remote")
                #expect(defaults.string(forKey: "supportClaw.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "supportClaw.onboardingSeen") == true)
                #expect(defaults.string(forKey: "supportClaw.remoteCliPath") == "/opt/homebrew/bin/supportClaw")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("supportClaw-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("supportClaw-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("supportClaw-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["SUPPORT_CLAW_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
