...
@Suite(.serialized)
struct SupportClawConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("supportClaw-config-\(UUID().uuidString)")
            .appendingPathComponent("supportClaw.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["SUPPORT_CLAW_CONFIG_PATH": override]) {
            #expect(SupportClawConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["SUPPORT_CLAW_CONFIG_PATH": override]) {
            SupportClawConfigFile.saveDict([
                "gateway": [
                    "remote": [
            ...
            #expect(SupportClawConfigFile.remoteGatewayPort() == 19999)
            #expect(SupportClawConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(SupportClawConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(SupportClawConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(SupportClawConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(SupportClawConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["SUPPORT_CLAW_CONFIG_PATH": override]) {
            ...
            SupportClawConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
            let root = SupportClawConfigFile.loadDict()
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["SUPPORT_CLAW_CONFIG_PATH": override]) {
            ...
            SupportClawConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = SupportClawConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["SUPPORT_CLAW_CONFIG_PATH": override]) {
            ...
            SupportClawConfigFile.clearRemoteGatewayUrl()
            let root = SupportClawConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("supportClaw-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "SUPPORT_CLAW_CONFIG_PATH": nil,
            "SUPPORT_CLAW_STATE_DIR": dir,
        ]) {
            #expect(SupportClawConfigFile.stateDirURL().path == dir)
            #expect(SupportClawConfigFile.url().path == "\(dir)/supportClaw.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("supportClaw-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("supportClaw.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "SUPPORT_CLAW_STATE_DIR": stateDir.path,
            "SUPPORT_CLAW_CONFIG_PATH": configPath.path,
        ]) {
            SupportClawConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-supportClaw-config-file")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = SupportClawConfigFile.loadDict()
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try SupportClawConfigFile.withTestingFileLock {
                SupportClawConfigFile.saveDict([
                    "update": ["channel": "beta"],
                ...
                _ = SupportClawConfigFile.loadDict()
                ...
                let loaded = SupportClawConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = SupportClawConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
