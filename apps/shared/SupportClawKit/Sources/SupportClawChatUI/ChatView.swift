...
@MainActor
public struct SupportClawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    @State private var viewModel: SupportClawChatViewModel
    ...
    private let talkControl: SupportClawChatTalkControl?
    ...
    public init(
        viewModel: SupportClawChatViewModel,
        ...
        talkControl: SupportClawChatTalkControl? = nil)
    {
    ...
    public var body: some View {
        ZStack {
            if self.drawsBackground, self.style == .standard {
                SupportClawChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    private var visibleMessages: [SupportClawChatMessage] {
        let base: [SupportClawChatMessage]
    ...
    private var messageListOverlay: some View {
        if self.viewModel.isLoading {
            ...
            ChatNoticeCard(
                ...
                tint: SupportClawChatTheme.accent,
                actionTitle: nil,
                action: nil)
                .padding(.horizontal, 24)
                .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    private func mergeToolResults(in messages: [SupportClawChatMessage]) -> [SupportClawChatMessage] {
        ...
        for message in messages {
            ...
            let merged = SupportClawChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: SupportClawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: SupportClawChatMessage) -> Bool {
    ...
    private func primaryText(in message: SupportClawChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            let kind = (content.type ?? "text").lowercased()
            guard kind == "text" || kind.isEmpty else { return nil }
            return content.text
        }
        return SupportClawChatMessage.displayText(
    ...
    private func hasInlineAttachments(in message: SupportClawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: SupportClawChatMessage) -> [SupportClawChatMessageContent] {
    ...
    private func inlineToolResults(in message: SupportClawChatMessage) -> [SupportClawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: SupportClawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: SupportClawChatMessage) -> String {
...
private struct ChatAssistantIntroCard: View {
    ...
    var body: some View {
        Text(self.text)
            .font(.system(size: 15))
            .lineSpacing(4)
            .foregroundStyle(SupportClawChatTheme.assistantText)
            .multilineTextAlignment(.leading)
            .padding(.vertical, 12)
            .padding(.horizontal, 14)
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(SupportClawChatTheme.assistantBubble)
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
