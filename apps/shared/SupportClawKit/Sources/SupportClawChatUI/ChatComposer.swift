...
public struct SupportClawChatTalkControl {
...
@MainActor
struct SupportClawChatComposer: View {
    @Bindable var viewModel: SupportClawChatViewModel
    let style: SupportClawChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: SupportClawChatView.ComposerChrome
    let isComposerEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: SupportClawChatTalkControl?
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(SupportClawChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(SupportClawChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(SupportClawChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SupportClawPendingAttachment.id)
                { (att: SupportClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SupportClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
                    ...
                    .padding(.vertical, 5)
                    .background(SupportClawChatTheme.accent.opacity(0.08))
                    .clipShape(Capsule())
                }
            }
        }
    ...
    private var fullEditor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(SupportClawChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(SupportClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(SupportClawChatTheme.composerBorder)))
    ...
    private func talkButton(_ talkControl: SupportClawChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: SupportClawChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            Image(systemName: talkControl.isEnabled ? "stop.fill" : "waveform")
    ...
    private func talkButtonFill(_ talkControl: SupportClawChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(SupportClawChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return SupportClawChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: SupportClawChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return SupportClawChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: SupportClawChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: SupportClawChatTalkControl) -> String {
    ...
    private var connectionPill: some View {
        ...
        .background {
            if self.composerChrome == .full {
                Capsule()
                    .fill(SupportClawChatTheme.subtleCard)
            }
        }
    }
            ...
            if self.viewModel.pendingRunCount > 0, !self.viewModel.hasDraftToSend {
                ...
                .background(
                    RoundedRectangle(cornerRadius: self.sendButtonCornerRadius, style: .continuous)
                        .fill(SupportClawChatTheme.danger))
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? SupportClawChatTheme.userBubble
    }
