...
private let chatUILogger = Logger(subsystem: "ai.supportClaw", category: "SupportClawChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class SupportClawChatViewModel {
    public static let defaultModelSelectionID = "__default__"
    static let maxAttachmentBytes = 5_000_000

    public private(set) var messages: [SupportClawChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var thinkingLevelOptions: [SupportClawChatThinkingLevelOption]
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [SupportClawChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [SupportClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SupportClawChatPendingToolCall] = []
    public private(set) var sessions: [SupportClawChatSessionEntry] = []
    private let transport: any SupportClawChatTransport
    private var sessionDefaults: SupportClawChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: SupportClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any SupportClawChatTransport,
    ...
    public var sessionChoices: [SupportClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [SupportClawChatSessionEntry] = []
    ...
    private static let baseThinkingLevelOptions: [SupportClawChatThinkingLevelOption] = [
        SupportClawChatThinkingLevelOption(id: "off", label: "off"),
        SupportClawChatThinkingLevelOption(id: "minimal", label: "minimal"),
        SupportClawChatThinkingLevelOption(id: "low", label: "low"),
        SupportClawChatThinkingLevelOption(id: "medium", label: "medium"),
        SupportClawChatThinkingLevelOption(id: "high", label: "high"),
    ]
    ...
    public func removeAttachment(_ id: SupportClawPendingAttachment.ID) {
    ...
    private func applyHistoryPayload(
        _ payload: SupportClawChatHistoryPayload,
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SupportClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SupportClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: SupportClawChatMessage) -> SupportClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> SupportClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return SupportClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: SupportClawChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: SupportClawChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: SupportClawChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private func adoptPendingLocalUserEcho(incoming: SupportClawChatMessage) -> Bool {
        ...
        updated[matchIndex] = SupportClawChatMessage(
            id: existing.id,
            role: incoming.role,
    ...
    private static func reconcileMessageIDs(
        previous: [SupportClawChatMessage],
        incoming: [SupportClawChatMessage]) -> [SupportClawChatMessage]
    {
    ...
    private static func reconcileRunRefreshMessages(
        previous: [SupportClawChatMessage],
        incoming: [SupportClawChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [SupportClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [SupportClawChatMessage]) -> [SupportClawChatMessage] {
        var result: [SupportClawChatMessage] = []
    ...
    private static func dedupeKey(for message: SupportClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SupportClawChatMessageContent] = [
            SupportClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> SupportClawChatAttachmentPayload in
            SupportClawChatAttachmentPayload(
                type: att.type,
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "SupportClawChatTransport"
    ...
    private func resolvedThinkingLevelOptions(
        for currentSession: SupportClawChatSessionEntry?) -> [SupportClawChatThinkingLevelOption]
    {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: SupportClawChatSessionEntry,
        defaults: SupportClawChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [SupportClawChatThinkingLevelOption]?) -> [SupportClawChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return SupportClawChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [SupportClawChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return SupportClawChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    private static func withCurrentThinkingOption(
        _ options: [SupportClawChatThinkingLevelOption],
        current: String) -> [SupportClawChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [SupportClawChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [SupportClawChatThinkingLevelOption]) -> [SupportClawChatThinkingLevelOption]
    {
        var result: [SupportClawChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    private func placeholderSession(key: String) -> SupportClawChatSessionEntry {
        SupportClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func updateCurrentSessionThinkingLevel(_ thinkingLevel: String?, sessionKey: String) {
        ...
        self.sessions[index] = SupportClawChatSessionEntry(
            key: current.key,
            kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: SupportClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: SupportClawSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: SupportClawChatEventPayload) {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: SupportClawChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = SupportClawChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: SupportClawChatMessage.self)
        }
    ...
    private static func isAssistantMessage(_ message: SupportClawChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: SupportClawChatMessage) -> SupportClawChatMessage {
    ...
    private func handleAgentEvent(_ evt: SupportClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SupportClawChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: SupportClawAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: SupportClawAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    private func finishPendingRunAfterTerminalOkSendAck(_ response: SupportClawChatSendResponse) {
    ...
    private func finishPendingRunIfTerminalSendAck(_ response: SupportClawChatSendResponse) -> Bool {
    ...
    private static func hasUnansweredLatestUser(in messages: [SupportClawChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    private static func latestUserTurn(in messages: [SupportClawChatMessage]) -> LatestUserTurn? {
    ...
    private static func hasAnsweredUser(
        ...
        in messages: [SupportClawChatMessage])
        -> Bool
    {
        guard let refreshKey = user.refreshKey else { return false }
        var occurrence = 0
        var latestMatchingUserIndex: [SupportClawChatMessage].Index?
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [SupportClawChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
