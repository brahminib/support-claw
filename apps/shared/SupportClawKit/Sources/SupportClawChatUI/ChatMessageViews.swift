...
struct ChatAgentAvatar: View {
    ...
    var body: some View {
        Text(self.displayText)
            ...
            .background(
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [
                                (self.tint ?? SupportClawChatTheme.accent).opacity(0.95),
            ...
            .shadow(color: (self.tint ?? SupportClawChatTheme.accent).opacity(0.18), radius: 8, y: 4)
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SupportClawChatTheme.userText : SupportClawChatTheme.assistantText
    ...
    private var inlineAttachments: [SupportClawChatMessageContent] {
    ...
    private var toolCalls: [SupportClawChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SupportClawChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SupportClawChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SupportClawChatTheme.onboardingAssistantBubble
        }
        return SupportClawChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SupportClawChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SupportClawChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SupportClawChatTheme.userText : SupportClawChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SupportClawChatMessageContent
    let isUser: Bool
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SupportClawChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: SupportClawChatTheme.assistantText)
            }
        }
    }
}
