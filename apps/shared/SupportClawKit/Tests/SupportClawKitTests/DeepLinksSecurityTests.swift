...
@Suite struct DeepLinksSecurityTests {
    @Test func dashboardDeepLinkParses() {
        let url = URL(string: "supportClaw://dashboard")!
    ...
    @Test func gatewayDeepLinkAllowsLoopbackWs() {
        let url = URL(
            string: "supportClaw://gateway?host=127.0.0.1&port=18789&tls=0&token=abc")!
        #expect(
    ...
    @Test func setupCodeAllowsPrivateLanHostPayload() {
        let payload = #"{"host":"supportClaw.local","port":18789,"tls":false,"bootstrapToken":"tok"}"#
        #expect(
            GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload)) == .init(
                host: "supportClaw.local",
                port: 18789,
