...
import Testing
@testable import SupportClawChatUI
...
fileprivate extension Array where Element == SupportClawChatMessage {
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> SupportClawChatHistoryPayload
{
    SupportClawChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> SupportClawChatSessionEntry {
...
private func thinkingOption(_ id: String, label: String? = nil) -> SupportClawChatThinkingLevelOption {
    SupportClawChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> SupportClawChatSessionEntry
{
    SupportClawChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> SupportClawChatModelChoice {
    SupportClawChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [SupportClawChatHistoryPayload],
    sessionsResponses: [SupportClawChatSessionsListResponse] = [],
    modelResponses: [[SupportClawChatModelChoice]] = [],
    ...
    sendMessageHook: (@Sendable (String) async throws -> SupportClawChatSendResponse)? = nil,
    ...
    -> (TestChatTransport, SupportClawChatViewModel)
{
...
private func sendUserMessage(_ vm: SupportClawChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, SupportClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SupportClawChatHistoryPayload]
    private let sessionsResponses: [SupportClawChatSessionsListResponse]
    private let modelResponses: [[SupportClawChatModelChoice]]
    ...
    private let sendMessageHook: (@Sendable (String) async throws -> SupportClawChatSendResponse)?
    private let waitForRunCompletionHook: (@Sendable (String, Int) async -> Bool)?
    private let healthResponses: [Bool]

    private let stream: AsyncStream<SupportClawChatTransportEvent>
    private let continuation: AsyncStream<SupportClawChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<SupportClawChatTransportEvent>.Continuation!
        ...
        parentSessionKey: String?) async throws -> SupportClawChatCreateSessionResponse
    {
        ...
        return SupportClawChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SupportClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SupportClawChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [SupportClawChatAttachmentPayload]) async throws -> SupportClawChatSendResponse
    {
        ...
        return SupportClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SupportClawChatSessionsListResponse {
        let idx = await self.state.nextSessionsCallIndex()
        if idx < self.sessionsResponses.count {
            return self.sessionsResponses[idx]
        }
        return self.sessionsResponses.last ?? SupportClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SupportClawChatTransportEvent) {
...
struct ChatViewModelTests {
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> SupportClawChatMessage {
        ...
        #expect(
            SupportClawChatMessage.errorDisplayText(
        ...
        #expect(
            SupportClawChatMessage.displayText(
                contentText: "",
                role: assistantError.role,
                stopReason: assistantError.stopReason,
                errorMessage: assistantError.errorMessage) == "stale provider failure")
    ...
    @Test func terminalTimeoutSendAckSurfacesErrorAndAllowsNextSend() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { runId in
                ...
                return SupportClawChatSendResponse(
                    runId: runId,
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(SupportClawChatTransportEvent.seqGap)
    ...
    @Test func `appends external session user message for active session`() async throws {
        ...
        transport.emit(
            .sessionMessage(
                SupportClawSessionMessageEventPayload(
                    sessionKey: "agent:aiden:main",
                    message: SupportClawChatMessage(
                        role: "user",
                        content: [
    ...
    @Test func `session choices prefer main and recent`() async throws {
        ...
        let sessions = SupportClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func `session choices use resolved main session key instead of literal main`() async throws {
        ...
        let sessions = SupportClawChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: SupportClawChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = SupportClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: SupportClawChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(SupportClawChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == SupportClawChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = SupportClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: SupportClawChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `late model completion does not replay current session selection into previous session`() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = SupportClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = SupportClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func `explicit thinking level wins over history and persists changes`() async throws {
        let history = SupportClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            SupportClawChatSessionsListResponse.self,
            from: Data(json.utf8))
    ...
    @Test func `strips inbound metadata from history messages`() async throws {
        ...
        let vm = await MainActor.run { SupportClawChatViewModel(sessionKey: "main", transport: transport) }
