...
import { buildModelAliasIndex, resolveModelRefFromString } from "supportClaw/plugin-sdk/agent-runtime";
import {
...
import { resolvePreferredSupportClawTmpDir, withTempWorkspace } from "../api.js";
...
type ThinkingPolicy = ReturnType<SupportClawPluginApi["runtime"]["agent"]["resolveThinkingPolicy"]>;

export const llmTaskToolDefinition = {
  name: "llm-task",
  label: "LLM Task",
  description:
    "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via supportClaw.invoke.",
...
function supportsThinkingPolicyLevel(
  ...
  level: ReturnType<SupportClawPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]>,
): boolean {
  return Boolean(level) && policy.levels.some((entry) => entry.id === level);
}

export function createLlmTaskTool(api: SupportClawPluginApi) {
  return {
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      let thinkLevel: ReturnType<SupportClawPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
      ...
      return await withTempWorkspace(
        { rootDir: resolvePreferredSupportClawTmpDir(), prefix: "supportClaw-llm-task-" },
        async ({ dir: tmpDir }) => {
