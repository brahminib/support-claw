...
`supportClaw.invoke --each`) without adding new SupportClaw code per workflow.
...
This extension depends on SupportClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SupportClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.supportClaw/extensions` as a standalone plugin directory.
