...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: SupportClawConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: SupportClawConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: SupportClawConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "Run SupportClaw on the Mac signed into Messages, or set cliPath to an SSH wrapper that runs imsg on that Mac.",
    "Linux/Windows hosts cannot run the default local imsg path directly.",
    "Run `imsg launch`, then `supportClaw channels status --probe` to verify private API actions.",
    "Ensure SupportClaw has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: SupportClawConfig; accountId?: string }) =>
