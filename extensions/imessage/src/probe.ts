...
import type { BaseProbeResult } from "supportClaw/plugin-sdk/channel-contract";
import {
  asDateTimestampMs,
  resolveExpiresAtMsFromDurationMs,
} from "supportClaw/plugin-sdk/number-runtime";
import { runCommandWithTimeout } from "supportClaw/plugin-sdk/process-runtime";
...
import { detectBinary } from "supportClaw/plugin-sdk/setup";
import {
...
export function resolveIMessageNonMacHostError(
  ...
  return "iMessage via the default imsg CLI must run on macOS. Run SupportClaw on the signed-in Messages Mac, or set channels.imessage.cliPath to an SSH wrapper that runs imsg on that Mac.";
}
...
// subcommand (added by supportClaw/imsg#114, which lets a single bridge call
...
export async function probeIMessagePrivateApi(
  ...
  try {
    ...
    // supportClaw/imsg#114. We do this even when the bridge is unavailable
