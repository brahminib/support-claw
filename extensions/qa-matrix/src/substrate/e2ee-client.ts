...
import type {
  ...
  MessageEventContent,
} from "@supportclaw/matrix/test-api.js";
import type {
...
type MatrixQaE2eeRuntime = typeof import("@supportclaw/matrix/test-api.js");
...
function resolveMatrixQaPluginStateNamespaceKey(options: OpenKeyedStoreOptions): string {
  return `${options.env?.SUPPORT_CLAW_STATE_DIR ?? ""}\0${options.namespace}`;
}
...
export async function loadMatrixQaE2eeRuntime(): Promise<MatrixQaE2eeRuntime> {
  const { loadQaRunnerBundledPluginTestApi } =
    await import("supportClaw/plugin-sdk/qa-runner-runtime");
