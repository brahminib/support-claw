...
import { spawn as startSupportClawCliProcess, spawnSync } from "node:child_process";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `supportClaw ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaSupportClawCliEntryPath(cwd: string): string {
...
function killMatrixQaCliChild(
  child: ReturnType<typeof startSupportClawCliProcess>,
...
export function startMatrixQaSupportClawCli(params: {
  ...
  const distEntryPath = resolveMatrixQaSupportClawCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startSupportClawCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
...
export async function runMatrixQaSupportClawCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaSupportClawCli(params).wait();
}
...
export async function createMatrixQaSupportClawCliRuntime(params: {
  accountId: string;
  accessToken: string;
