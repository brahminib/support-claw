...
const {
  ...
  resolveMatrixQaSupportClawCliEntryPath,
  runMatrixQaSupportClawCli,
  startMatrixQaSupportClawCli,
} = vi.hoisted(() => ({
  formatMatrixQaCliCommand: (args: string[]) => `supportClaw ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaSupportClawCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaSupportClawCli: vi.fn(),
  startMatrixQaSupportClawCli: vi.fn(),
}));
...
async function writeMatrixInboundDedupePluginStateEntry(params: {
  ...
  const databasePath = path.join(params.stateRoot, "state", "supportClaw.sqlite");
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaSupportClawCli.mockReset();
    startMatrixQaSupportClawCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "SupportClaw Matrix QA run",
        scenarios: [
      ...
      rooms: [
        {
          ...
          kind: "group",
          members: ["driver", "observer", "sut"],
          name: "SupportClaw Matrix QA run",
          requireMention: true,
        },
        {
          key: "driver-dm",
  ...
  it("runs the Matrix E2EE state_after regression scenario through the gateway fault proxy", async () => {
    ...
    try {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverDeviceId: "DRIVERDEVICE",
        gatewayRuntimeEnv: {
          SUPPORT_CLAW_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaSupportClawCli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaSupportClawCli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.SUPPORT_CLAW_CONFIG_PATH) {
          const cliConfig = JSON.parse(
            await readFile(String(env.SUPPORT_CLAW_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
              matrix?: {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverPassword: "driver-password",
        gatewayRuntimeEnv: {
          SUPPORT_CLAW_CONFIG_PATH: "/tmp/gateway-config.json",
          SUPPORT_CLAW_STATE_DIR: "/tmp/gateway-state",
          PATH: process.env.PATH,
        },
        outputDir,
      });
      ...
      expect(startMatrixQaSupportClawCli).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(startMatrixQaSupportClawCli, "startMatrixQaSupportClawCli").args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(mockObjectArg(startMatrixQaSupportClawCli, "startMatrixQaSupportClawCli").timeoutMs).toBe(
        16_000,
      );
      ...
      expect(registrationRequest?.deviceName).toBe(
        "SupportClaw Matrix QA CLI Self Verification Owner",
      );
      if (
      ...
      expect(loginWithPassword).toHaveBeenCalledWith({
        deviceName: "SupportClaw Matrix QA CLI Self Verification Device",
        password: "cli-owner-password",
        userId: "@cli-owner:matrix-qa.test",
      });
      const e2eeClientOptions = mockObjectArg(
      ...
      expect(runMatrixQaSupportClawCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaSupportClawCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(mockObjectArg(runMatrixQaSupportClawCli, "runMatrixQaSupportClawCli").stdin).toBe(
        "encoded-recovery-key\n",
      );
      const cliEnv = mockObjectArg(startMatrixQaSupportClawCli, "startMatrixQaSupportClawCli")
        .env as Record<string, unknown>;
      expect(cliEnv?.SUPPORT_CLAW_STATE_DIR).toContain("supportClaw-matrix-cli-qa-");
      expect(cliEnv?.SUPPORT_CLAW_CONFIG_PATH).toContain("supportClaw-matrix-cli-qa-");
      const configPath = String(cliEnv?.SUPPORT_CLAW_CONFIG_PATH);
      ...
      await expectPathMissing(String(cliEnv?.SUPPORT_CLAW_STATE_DIR));
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaSupportClawCli.mockImplementation(async ({ args, env }) => {
        if (env.SUPPORT_CLAW_CONFIG_PATH) {
      ...
      expect(runMatrixQaSupportClawCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          ...
          "cli-add-password",
          "--device-name",
          "SupportClaw Matrix QA CLI Account Add E2EE",
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SupportClaw Matrix QA CLI Account Add Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaSupportClawCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.SUPPORT_CLAW_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.SUPPORT_CLAW_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SupportClaw Matrix QA CLI Encryption Setup Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SupportClaw Matrix QA CLI Encryption Idempotent Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi
        .fn()
        .mockRejectedValue(new Error("supportClaw matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaSupportClawCli.mockReturnValue({
      ...
      expect(
        (
          mockObjectArg(startMatrixQaSupportClawCli, "startMatrixQaSupportClawCli").env as Record<
            string,
            unknown
          >
        ).SUPPORT_CLAW_CONFIG_PATH,
      ).toContain("supportClaw-matrix-e2ee-setup-qa-");
      expect(output).toHaveBeenCalledTimes(1);
      expect(wait).toHaveBeenCalledTimes(1);
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SupportClaw Matrix QA CLI Encryption Failure Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SupportClaw Matrix QA CLI Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SupportClaw Matrix QA CLI Invalid Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaSupportClawCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.SUPPORT_CLAW_CONFIG_PATH);
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "SupportClaw Matrix QA CLI Multi Account Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
    ...
    try {
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "SupportClaw Matrix QA CLI Setup Gateway" &&
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "SupportClaw Matrix QA CLI Setup Driver" &&
            request.registrationToken === "registration-token",
        ),
      ).toBe(true);
      expect(createPrivateRoom).toHaveBeenCalledWith({
