...
import { loadQaRuntimeModule } from "supportClaw/plugin-sdk/qa-runner-runtime";
import {
...
function shouldWriteMatrixQaProgress() {
  const override = process.env.SUPPORT_CLAW_QA_MATRIX_PROGRESS;
  if (override === "0") {
    return false;
  }
  if (override === "1") {
...
function createMatrixQaRunDeadline() {
  const timeoutMs = parsePositiveMatrixQaEnvMs(
    "SUPPORT_CLAW_QA_MATRIX_TIMEOUT_MS",
...
function resolveMatrixQaCanaryTimeoutMs() {
  return parsePositiveMatrixQaEnvMs(
    "SUPPORT_CLAW_QA_MATRIX_CANARY_TIMEOUT_MS",
...
async function cleanupMatrixQaResource(params: {
  ...
  const timeoutMs = parsePositiveMatrixQaEnvMs(
    "SUPPORT_CLAW_QA_MATRIX_CLEANUP_TIMEOUT_MS",
...
async function startMatrixQaLiveLaneGateway(params: {
  ...
  transport: {
    ...
    createGatewayConfig: (params: {
      baseUrl: string;
    }) => Pick<SupportClawConfig, "channels" | "messages">;
  };
...
export async function runMatrixQaLive(params: {
  ...
  const topology = buildMatrixQaTopologyForScenarios({
    defaultRoomName: `SupportClaw Matrix QA ${runSuffix}`,
    scenarios,
  });
  const observedEvents: MatrixQaObservedEvent[] = [];
  const includeObservedEventContent = process.env.SUPPORT_CLAW_QA_MATRIX_CAPTURE_CONTENT === "1";
      ...
      return await measureMatrixQaStep(() =>
        withMatrixQaRunDeadline(runDeadline, "Matrix topology provisioning", () =>
          provisionMatrixQaRoom({
            ...
            roomName: `SupportClaw Matrix QA ${runSuffix}`,
          ...
          const measuredScenario = await measureMatrixQaStep(() =>
            withMatrixQaRunDeadline(runDeadline, `Matrix scenario ${scenario.id}`, () =>
              runMatrixQaScenario(scenario, {
                ...
                gatewayStateDir: scenarioGateway.harness.gateway.runtimeEnv?.SUPPORT_CLAW_STATE_DIR,
