...
const QA_LAB_UI_OVERLAY_DIR = "/opt/supportClaw-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${yamlDoubleQuoted(context)}\n      dockerfile: Dockerfile\n      args:\n        SUPPORT_CLAW_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    volumes:
      - ./state:/opt/supportClaw-scaffold:ro
${params.bindUiDist ? `      - ${yamlDoubleQuoted(`${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro`)}\n` : ""}    healthcheck:
    ...
    command:
      - sh
      - -lc
      - SUPPORT_CLAW_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/supportClaw-scaffold/supportClaw.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://supportClaw-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
        condition: service_healthy
`
    : ""
}  supportClaw-qa-gateway:
${imageBlock}    pull_policy: never
    extra_hosts:
    ...
    environment:
      SUPPORT_CLAW_CONFIG_PATH: /tmp/supportClaw/supportClaw.json
      SUPPORT_CLAW_STATE_DIR: /tmp/supportClaw/state
      SUPPORT_CLAW_NO_RESPAWN: "1"
      SUPPORT_CLAW_SKIP_GMAIL_WATCHER: "1"
      SUPPORT_CLAW_SKIP_BROWSER_CONTROL_SERVER: "1"
      SUPPORT_CLAW_SKIP_CANVAS_HOST: "1"
      SUPPORT_CLAW_PROFILE: ""
    volumes:
      - ./state:/opt/supportClaw-scaffold:ro
      - ${yamlDoubleQuoted(`${repoMount}:/opt/supportClaw-repo:ro`)}
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/supportClaw/workspace /tmp/supportClaw/state && cp /opt/supportClaw-scaffold/supportClaw.json /tmp/supportClaw/supportClaw.json && cp -R /opt/supportClaw-scaffold/seed-workspace/. /tmp/supportClaw/workspace/ && rm -rf /tmp/supportClaw/workspace/repo && ln -s /opt/supportClaw-repo /tmp/supportClaw/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
- \`.env.example\`
- \`state/supportClaw.json\`

Suggested flow:

1. Build the prebaked image once:
   - \`docker build -t supportClaw:qa-local-prebaked --build-arg SUPPORT_CLAW_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "supportClaw:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "supportClaw.json"),
  ];
