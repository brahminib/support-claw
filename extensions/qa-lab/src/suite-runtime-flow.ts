...
import { formatMemoryDreamingDay } from "supportClaw/plugin-sdk/memory-core-host-status";
import { resolveSessionTranscriptsDirForAgent } from "supportClaw/plugin-sdk/memory-host-core";
...
function setActiveMemorySessionDisabled(
  ...
  const store = createPluginStateSyncKeyedStore<{
    ...
    env: {
      ...process.env,
      SUPPORT_CLAW_STATE_DIR: path.join(env.gateway.tempRoot, "state"),
    },
  });
