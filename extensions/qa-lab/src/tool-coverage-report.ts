...
export type QaToolCoverageRow = {
  ...
  scenarios: string[];
  sourcePaths: string[];
  supportClaw: QaToolCoverageStatus;
  codex: QaToolCoverageStatus;
  drift: QaToolCoverageDrift;
  supportClawToolCalls: number;
...
function countRuntimeToolCalls(
  ...
  const cell = runtime === "supportClaw" ? result.cells.supportClaw : result.cells.codex;
...
function buildRow(params: {
  ...
  return {
    ...
    supportClaw: result ? cellStatus(result.cells.supportClaw) : "not-run",
    codex: result ? cellStatus(result.cells.codex) : "not-run",
    drift: result?.drift ?? "not-run",
    supportClawToolCalls: countRuntimeToolCalls(result, "supportClaw", runtimeToolName),
...
function coverageFailureForRow(row: QaToolCoverageRow): string | undefined {
  ...
  if (row.supportClaw !== "pass" || row.codex !== "pass") {
    return `${row.tool} status supportClaw=${row.supportClaw} codex=${row.codex}`;
  }
  if (row.drift === "failure-mode") {
    return `${row.tool} drift=failure-mode${row.details ? ` (${row.details})` : ""}`;
  }
  if (row.runtimeToolName && row.supportClawToolCalls === 0) {
    return `${row.tool} missing supportClaw tool call ${row.runtimeToolName}`;
  }
...
export function buildQaToolCoverageReport(params: {
  ...
  return {
    ...
    dynamicIntegrationTools: rows.filter((row) => row.bucket === "supportClaw-dynamic-integration")
      .length,
    searchableDynamicTools: rows.filter(
      (row) => row.capabilityLayer === "supportClaw-dynamic-searchable",
    ).length,
...
export function renderQaToolCoverageMarkdownReport(report: QaToolCoverageReport): string {
  const lines = [
    `# SupportClaw Runtime Tool Coverage — ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
    ...
    `- SupportClaw dynamic integration tools: ${report.dynamicIntegrationTools}`,
    ...
    "",
    "| Tool | Bucket | Expected layer | Capability layer | Required | Fixtures | SupportClaw | Codex | Drift | Codex default impact | QA impact | Action | Tracking |",
    "| --- | --- | --- | --- | --- | ---: | --- | --- | --- | --- | --- | --- | --- |",
  ];
