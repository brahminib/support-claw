...
import { SUPPORT_CLAW_CRABLINE_MANIFEST_PATH } from "crabline";
...
describe("crabline transport", () => {
  it("configures SupportClaw's Telegram plugin against a Crabline fake provider server", async () => {
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      ...
      try {
        ...
        const manifest = JSON.parse(
          await fs.readFile(path.join(outputDir, SUPPORT_CLAW_CRABLINE_MANIFEST_PATH), "utf8"),
        ) as {
          provider?: string;
        };
