...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    SUPPORT_CLAW_HOME: params.homeDir,
    SUPPORT_CLAW_CONFIG_PATH: params.configPath,
    SUPPORT_CLAW_STATE_DIR: params.stateDir,
    SUPPORT_CLAW_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    SUPPORT_CLAW_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS: "2000",
    SUPPORT_CLAW_QA_PARENT_PID: String(process.pid),
    SUPPORT_CLAW_QA_TEMP_ROOT: params.tempRoot,
    ...(params.stagedBundledPluginsRoot
      ? { SUPPORT_CLAW_QA_STAGED_RUNTIME_ROOT: params.stagedBundledPluginsRoot }
      : {}),
    SUPPORT_CLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    SUPPORT_CLAW_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: SupportClawConfig) => SupportClawConfig;
  runtimeEnvPatch?: NodeJS.ProcessEnv;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-qa-suite-"),
  );
  ...
  const keepTemp = process.env.SUPPORT_CLAW_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
