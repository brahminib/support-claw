...
export type TokenEfficiencyRow = {
  ...
  supportClaw: TokenEfficiencyRuntimeUsage;
...
export type TokenEfficiencyReport = {
  ...
  aggregate: {
    supportClaw: { totalTokens: number; p50PerScenario: number; p90PerScenario: number };
...
const ZERO_AGGREGATE: TokenEfficiencyReport["aggregate"] = {
  supportClaw: { totalTokens: 0, p50PerScenario: 0, p90PerScenario: 0 },
...
function deltaPercent(supportClawTotalTokens: number, codexTotalTokens: number): number {
  if (supportClawTotalTokens === 0) {
    return codexTotalTokens === 0 ? 0 : 100;
  }
  return ((codexTotalTokens - supportClawTotalTokens) / supportClawTotalTokens) * 100;
}
...
function toolNamesForCells(supportClaw: RuntimeParityCell, codex: RuntimeParityCell): string[] {
  return [
    ...new Set([...supportClaw.toolCalls, ...codex.toolCalls].map((call) => call.tool)),
...
function buildRow(params: {
  ...
  const supportClaw = runtimeUsage(params.result.cells.supportClaw);
  const codex = runtimeUsage(params.result.cells.codex);
  const delta = deltaPercent(supportClaw.totalTokens, codex.totalTokens);
  ...
  return {
    ...
    classification,
    flagged,
    toolsUsed: toolNamesForCells(params.result.cells.supportClaw, params.result.cells.codex),
  };
}

function buildAggregate(rows: readonly TokenEfficiencyRow[]): TokenEfficiencyReport["aggregate"] {
  const supportClawTotals = rows.map((row) => row.supportClaw.totalTokens);
  const codexTotals = rows.map((row) => row.codex.totalTokens);
  const supportClawTotalTokens = supportClawTotals.reduce((sum, value) => sum + value, 0);
  const codexTotalTokens = codexTotals.reduce((sum, value) => sum + value, 0);
  return {
    supportClaw: {
      totalTokens: supportClawTotalTokens,
      p50PerScenario: percentile(supportClawTotals, 50),
      p90PerScenario: percentile(supportClawTotals, 90),
    },
    codex: {
    ...
    deltaPercent: deltaPercent(supportClawTotalTokens, codexTotalTokens),
...
function liveEvidenceFailures(row: TokenEfficiencyRow): string[] {
  ...
  if (row.supportClaw.totalTokens <= 0) {
    failures.push(`${row.scenarioId} supportClaw live usage totalTokens=${row.supportClaw.totalTokens}`);
  }
...
export function buildTokenEfficiencyReport(
  ...
  const failures = rows.flatMap((row, index) => {
    ...
    const rowFailures =
      liveUsage && result
        ? [
            ...liveUsageShapeFailures(row.scenarioId, "supportClaw", result.cells.supportClaw.usage),
  ...
  return {
    ...
    notes: [
      ...
      "Codex savings are reported as savings and do not fail the gate; only positive Codex-over-SupportClaw live deltas exceed the threshold.",
...
export function renderTokenEfficiencyMarkdownReport(report: TokenEfficiencyReport): string {
  const lines = [
    `# SupportClaw Runtime Token Efficiency - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
  ...
  lines.push(
    ...
    "| --- | ---: | ---: | ---: |",
    `| supportClaw | ${report.aggregate.supportClaw.totalTokens} | ${report.aggregate.supportClaw.p50PerScenario} | ${report.aggregate.supportClaw.p90PerScenario} |`,
  ...
  if (report.rows.length > 0) {
    lines.push(
      "## Scenario Efficiency",
      "",
      "| Scenario | Source | SupportClaw in/out/total/tools | Codex in/out/total/tools | Token delta | Classification | Flagged | Tools used |",
      "| --- | --- | ---: | ---: | ---: | --- | --- | --- |",
    );
    for (const row of report.rows) {
      lines.push(
        `| ${row.scenarioId} | ${row.usageSource} | ${row.supportClaw.inputTokens}/${row.supportClaw.outputTokens}/${row.supportClaw.totalTokens}/${row.supportClaw.toolCallCount} | ${row.codex.inputTokens}/${row.codex.outputTokens}/${row.codex.totalTokens}/${row.codex.toolCallCount} | ${formatPercent(row.deltaPercent)} | ${row.classification} | ${row.flagged ? "yes" : "no"} | ${row.toolsUsed.join(", ")} |`,
      );
    }
    lines.push("");
  }
