...
export type RuntimeId = "supportClaw" | "codex";
...
export function isRuntimeParityResultPass(result: RuntimeParityResult) {
  return (
    ...
    isRuntimeParityCellPassable(result.cells.supportClaw) &&
...
const HEARTBEAT_TRANSCRIPT_PROMPT = "[SupportClaw heartbeat poll]";
...
function classifyRuntimeParityCells(params: {
  supportClaw: RuntimeParityCell;
  codex: RuntimeParityCell;
  supportClawScenarioStatus: "pass" | "fail";
  codexScenarioStatus: "pass" | "fail";
}): Pick<RuntimeParityResult, "drift" | "driftDetails"> {
  if (
    isHardFailureRuntimeError(params.supportClaw.runtimeErrorClass) ||
    isHardFailureRuntimeError(params.codex.runtimeErrorClass) ||
    params.supportClaw.transportErrorClass ||
    params.codex.transportErrorClass
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.supportClaw.transportErrorClass || params.codex.transportErrorClass
  ...
  if (
    hasMissingToolResult(params.supportClaw.toolCalls) ||
  ...
  if (
    params.supportClawScenarioStatus === "fail" ||
    params.codexScenarioStatus === "fail" ||
    !isRuntimeParityCellPassable(params.supportClaw) ||
    !isRuntimeParityCellPassable(params.codex)
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.supportClawScenarioStatus === params.codexScenarioStatus
          ? "at least one runtime failed"
          : `scenario status differs (${params.supportClawScenarioStatus} vs ${params.codexScenarioStatus})`,
    };
  }

  const toolCallShapeDetails = compareToolCallShape(
    params.supportClaw.toolCalls,
  ...
  const supportClawTranscriptLines = params.supportClaw.transcriptBytes.trim().length
    ? params.supportClaw.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  const codexTranscriptLines = params.codex.transcriptBytes.trim().length
    ? params.codex.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  if (
    supportClawTranscriptLines !== codexTranscriptLines ||
    (!params.supportClaw.finalText && Boolean(params.codex.finalText)) ||
    (Boolean(params.supportClaw.finalText) && !params.codex.finalText)
  ) {
    return {
      drift: "structural",
      driftDetails: `transcript/final-text structure differs (${supportClawTranscriptLines} lines vs ${codexTranscriptLines})`,
    };
  }

  if (
    normalizeTextForParity(params.supportClaw.finalText) ===
...
export async function runRuntimeParityScenario(params: {
  ...
  const supportClaw = await params.runCell("supportClaw");
  const codex = await params.runCell("codex");
  const drift = classifyRuntimeParityCells({
    supportClaw: supportClaw.cell,
    codex: codex.cell,
    supportClawScenarioStatus: supportClaw.scenarioStatus,
