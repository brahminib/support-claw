...
describe("live transport artifacts", () => {
  ...
  it("preserves cleanup phase labels while redacting details", () => {
    expect(
      ...
      "credential lease release: details redacted (SUPPORT_CLAW_QA_REDACT_PUBLIC_METADATA=1)",
      "live gateway cleanup: details redacted (SUPPORT_CLAW_QA_REDACT_PUBLIC_METADATA=1)",
    ]);
  });
    ...
    expect(
      redactQaLiveLaneIssues([
        [
          ...
          "Artifacts:",
          "- gatewayDebug: /tmp/supportClaw-whatsapp-qa/gateway-debug",
        ].join("\n"),
      ]),
    ).toEqual(["details redacted (SUPPORT_CLAW_QA_REDACT_PUBLIC_METADATA=1)"]);
  });
});
