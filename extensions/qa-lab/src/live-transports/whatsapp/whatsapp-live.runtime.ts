...
} from "@supportclaw/whatsapp/api.js";
import { normalizeE164 } from "supportClaw/plugin-sdk/account-resolution";
import type { SupportClawConfig } from "supportClaw/plugin-sdk/config-contracts";
import { formatErrorMessage } from "supportClaw/plugin-sdk/error-runtime";
import { normalizeStringEntries, uniqueStrings } from "supportClaw/plugin-sdk/string-coerce-runtime";
...
const WHATSAPP_QA_CAPTURE_CONTENT_ENV = "SUPPORT_CLAW_QA_WHATSAPP_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "SUPPORT_CLAW_QA_REDACT_PUBLIC_METADATA";
...
function buildWhatsAppQuoteReplyRun(target: "dm" | "group"): WhatsAppQaMessageScenarioRun {
  ...
  const input =
    target === "group"
      ? `supportClawqa reply with only this exact marker: ${token}`
...
const WHATSAPP_QA_SCENARIOS: WhatsAppQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    buildRun: () => ({
      ...
      matchText: /SupportClaw: access not configured|Pairing code:/iu,
      target: "dm",
    }),
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `supportClawqa reply with only this exact marker: ${replyToken}`,
  ...
  },
  {
    ...
    buildRun: () => ({
      ...
      expectReply: true,
      input: "/help",
      matchText: /SupportClaw|commands|status|\/new/iu,
      target: "dm",
    }),
  },
  {
    ...
    buildRun: () => ({
      ...
      expectReply: true,
      input: "/status",
      matchText: /SupportClaw|status|session|agent/iu,
      target: "dm",
    }),
  },
  {
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `supportClawqa reply with only this exact marker under groupPolicy open: ${token}`,
        matchText: token,
        target: "group",
      };
    },
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: false,
        input: `supportClawqa groupPolicy disabled must not reply with ${token}`,
        matchText: token,
        target: "group",
      };
    },
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: false,
        input: `supportClawqa blocked group should not reply with ${quietToken}`,
...
function resolveWhatsAppQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): WhatsAppQaRuntimeEnv {
  return validateWhatsAppQaRuntimeEnv(
    {
      driverPhoneE164: resolveEnvValue(env, "SUPPORT_CLAW_QA_WHATSAPP_DRIVER_PHONE_E164"),
      sutPhoneE164: resolveEnvValue(env, "SUPPORT_CLAW_QA_WHATSAPP_SUT_PHONE_E164"),
      driverAuthArchiveBase64: resolveEnvValue(
        env,
        "SUPPORT_CLAW_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
      ),
      sutAuthArchiveBase64: resolveEnvValue(env, "SUPPORT_CLAW_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64"),
      groupJid: env.SUPPORT_CLAW_QA_WHATSAPP_GROUP_JID?.trim() || undefined,
    },
    "SUPPORT_CLAW_QA_WHATSAPP",
  );
}
...
async function writeWhatsAppQaWorkspaceFixture(
  ...
  const fixtureDir = path.join(context.gatewayWorkspaceDir, ".supportClaw", "qa-whatsapp-media");
...
export async function runWhatsAppQaLive(params: {
  ...
  try {
    ...
    tempAuthRoot = await fs.mkdtemp(
      path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-whatsapp-qa-"),
    );
