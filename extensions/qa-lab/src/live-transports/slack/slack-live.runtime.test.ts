...
describe("Slack live QA runtime helpers", () => {
  ...
  it("resolves env credential payloads", () => {
    expect(
      testing.resolveSlackQaRuntimeEnv({
        SUPPORT_CLAW_QA_SLACK_CHANNEL_ID: "C123456789",
        SUPPORT_CLAW_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        SUPPORT_CLAW_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toEqual({
  ...
  it("rejects malformed Slack channel ids", () => {
    expect(() =>
      testing.resolveSlackQaRuntimeEnv({
        SUPPORT_CLAW_QA_SLACK_CHANNEL_ID: "qa-channel",
        SUPPORT_CLAW_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        SUPPORT_CLAW_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toThrow("SUPPORT_CLAW_QA_SLACK channelId must be a Slack id like C123 or U123.");
  });
  ...
  it("resolves Slack approval checkpoint configuration from env", () => {
    expect(
      testing.resolveSlackApprovalCheckpointConfig({
        SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR: "/tmp/checkpoints",
        SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS: "5000",
      }),
    ).toEqual({
  ...
  it("writes artifacts when Convex credential acquisition fails", async () => {
    const outputDir = await fs.mkdtemp(path.join(tmpdir(), "supportClaw-slack-qa-"));
    ...
    expect(result.scenarios[0]?.details).toContain("Missing SUPPORT_CLAW_QA_CONVEX_SITE_URL");
    ...
    expect(summary.entries[0]).toMatchObject({
      ...
      result: {
        status: "fail",
        failure: {
          reason: expect.stringContaining("Missing SUPPORT_CLAW_QA_CONVEX_SITE_URL"),
        },
      },
    });
  });
