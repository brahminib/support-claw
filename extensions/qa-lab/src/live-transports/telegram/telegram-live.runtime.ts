...
import { isRecord, uniqueStrings } from "supportClaw/plugin-sdk/string-coerce-runtime";
import { z } from "zod";
import {
...
const TELEGRAM_QA_SCENARIOS: TelegramQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    buildRun: (sutUsername) =>
      telegramQaStepRun({
        ...
        expectedTextIncludes: ["SupportClaw", "Model:", "Session:", "Activation:"],
      }),
  },
  {
    ...
    buildRun: (sutUsername) => {
      const steps = [
        ...
        },
        {
          ...
          expectReply: true,
          input: `/status@${sutUsername}`,
          expectedTextIncludes: ["SupportClaw", "Session:"],
        },
        {
          expectReply: true,
          input: `/help@${sutUsername}`,
  ...
  },
  {
    ...
    buildRun: () =>
      telegramQaStepRun({
        expectReply: false,
        input: "/status@SupportClawQaOtherBot",
      }),
  },
  {
  ...
  },
  {
    ...
    regressionRefs: ["supportClaw/supportClaw#87392"],
    timeoutMs: 90_000,
    buildRun: (sutUsername) => {
  ...
  },
  {
    ...
    regressionRefs: ["supportClaw/supportClaw#39905"],
    timeoutMs: 75_000,
    buildRun: (sutUsername) =>
      telegramQaStepRun({
        allowAnySutReply: true,
...
const QA_SUITE_PROGRESS_ENV = "SUPPORT_CLAW_QA_SUITE_PROGRESS";
...
function resolveTelegramQaScenarioTimeoutMs(
  ...
  return parsePositiveTelegramQaEnvMs(env, "SUPPORT_CLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", fallbackMs);
}
...
function resolveTelegramQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): TelegramQaRuntimeEnv {
  const groupId = resolveEnvValue(env, "SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "SUPPORT_CLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "SUPPORT_CLAW_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
export async function runTelegramQaLive(params: {
  ...
  try {
    ...
    const gatewayHarness = await startQaLiveLaneGateway({
      repoRoot,
      command: params.sutSupportClawCommand
        ? {
            executablePath: params.sutSupportClawCommand,
