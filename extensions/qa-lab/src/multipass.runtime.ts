...
import type { SupportClawCrablineChannelDriverSelection } from "crabline";
import { sleep } from "supportClaw/plugin-sdk/runtime-env";
import { appendRegularFile } from "supportClaw/plugin-sdk/security-runtime";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/supportClaw-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/supportClaw";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/supportClaw-codex-home";
...
type QaMultipassPlan = {
  ...
  channelDriverSelection?: SupportClawCrablineChannelDriverSelection;
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `supportClaw-qa-${createVmSuffix()}`;
...
export function renderQaMultipassGuestScript(
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          ...
          key !== "SUPPORT_CLAW_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `SUPPORT_CLAW_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `SUPPORT_CLAW_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  await writeFile(
    plan.hostLogPath,
    `# SupportClaw QA Multipass host log\nvmName=${plan.vmName}\noutputDir=${plan.outputDir}\n\n`,
    "utf8",
  );
  await writeFile(
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm supportClaw qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredSupportClawTmpDir(), `${plan.vmName}-qa-suite-`),
  );
