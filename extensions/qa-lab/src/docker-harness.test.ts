...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    for (const expectedFile of [
      ...
      path.join(outputDir, "state", "supportClaw.json"),
    ...
    expect(compose).toContain("image: supportClaw:qa-local-prebaked");
    expect(compose).toContain("qa-mock-openai:");
    expect(services["qa-mock-openai"]?.environment).toMatchObject({
      SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI: "1",
      SUPPORT_CLAW_PROFILE: "",
    });
    expect(services["qa-mock-openai"]?.environment).not.toHaveProperty("SUPPORT_CLAW_CONFIG_PATH");
    expect(services["qa-mock-openai"]?.volumes).toBeUndefined();
    expect(services["qa-lab"]?.environment).toMatchObject({
      SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI: "1",
      SUPPORT_CLAW_CONFIG_PATH: "/opt/supportClaw-scaffold/supportClaw.json",
      SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw/state",
    });
    expect(services["qa-lab"]?.volumes).toContain("./state:/opt/supportClaw-scaffold:ro");
    expect(compose).toContain('      - "127.0.0.1:18889:18789"');
    expect(compose).toContain('      - "127.0.0.1:43124:43123"');
    expect(compose).toContain(":/opt/supportClaw-qa-lab-ui:ro");
    expect(compose).toContain("      - sh");
    expect(compose).toContain("      - -lc");
    expect(compose).toContain(
      '        - fetch("http://127.0.0.1:18789/healthz").then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))',
    );
    expect(compose).toContain("--control-ui-proxy-target http://supportClaw-qa-gateway:18789/");
    ...
    expect(compose).toContain("--ui-dist-dir /opt/supportClaw-qa-lab-ui");
    expect(compose).toContain(":/opt/supportClaw-repo:ro");
    expect(compose).toContain("./state:/opt/supportClaw-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/supportClaw-scaffold/seed-workspace/. /tmp/supportClaw/workspace/ && rm -rf /tmp/supportClaw/workspace/repo && ln -s /opt/supportClaw-repo /tmp/supportClaw/workspace/repo",
    );
    expect(compose).toContain("SUPPORT_CLAW_CONFIG_PATH: /tmp/supportClaw/supportClaw.json");
    expect(compose).toContain("SUPPORT_CLAW_STATE_DIR: /tmp/supportClaw/state");
    expect(compose).toContain('SUPPORT_CLAW_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("SUPPORT_CLAW_GATEWAY_TOKEN=qa-token");
    ...
    const configText = await readFile(path.join(outputDir, "state", "supportClaw.json"), "utf8");
    ...
    expect(configText).toContain('"supportClawToolsMcpBridge": true');
    expect(configText).toContain("/app/dist/control-ui");
    expect(configText).toContain("C-3PO QA");
    expect(configText).toContain('"/tmp/supportClaw/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    expect(result.imageName).toBe("supportClaw:qa-local-prebaked");
    expect(calls).toEqual([
      "docker build -t supportClaw:qa-local-prebaked --build-arg SUPPORT_CLAW_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/supportClaw",
    ]);
  });
  ...
  it("quotes generated compose paths so shell-sensitive repo paths survive YAML parsing", async () => {
    ...
    expect(services["qa-lab"]?.volumes).toContain(
      "../repo #hash/extensions/qa-lab/web/dist:/opt/supportClaw-qa-lab-ui:ro",
    );
    expect(services["supportClaw-qa-gateway"]?.volumes).toContain(
      "../repo #hash:/opt/supportClaw-repo:ro",
    );
  });
});
