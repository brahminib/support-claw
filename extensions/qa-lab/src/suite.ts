...
import {
  createSupportClawCrablineChannelReportNotes,
  runSupportClawCrablineChannelDriverSmoke,
  type SupportClawCrablineChannelDriverSelection,
} from "crabline";
import { disposeRegisteredAgentHarnesses } from "supportClaw/plugin-sdk/agent-harness";
...
} from "supportClaw/plugin-sdk/qa-runtime";
import { fetchWithSsrFGuard } from "supportClaw/plugin-sdk/ssrf-runtime";
...
async function createQaSuiteTransportAdapter(params: {
  channelDriverSelection?: SupportClawCrablineChannelDriverSelection | null;
  outputDir: string;
...
function shouldLogQaSuiteProgress(env: NodeJS.ProcessEnv = process.env) {
  const override = parseQaSuiteBooleanEnv(env.SUPPORT_CLAW_QA_SUITE_PROGRESS);
...
function resolveQaSuiteTransportReadyTimeoutMs(
  ...
  const raw = env.SUPPORT_CLAW_QA_TRANSPORT_READY_TIMEOUT_MS;
  if (!raw) {
    return 120_000;
  }
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.SUPPORT_CLAW_QA_DEBUG === "1") {
...
function buildRuntimeParityScenarioResult(params: {
  ...
  const supportClawCell = params.result.cells.supportClaw;
  return {
    ...
    steps: [
      {
        name: supportClawCell.runtime,
        status:
          supportClawCell.runtimeErrorClass || supportClawCell.transportErrorClass ? "fail" : "pass",
        details: formatRuntimeParityCellDetails(supportClawCell),
      },
      {
...
function createQaSuiteReportNotes(params: {
  ...
  return [
    ...
    ...createSupportClawCrablineChannelReportNotes(params.channelDriverSelection),
  ];
}
...
function buildQaRuntimeEnvPatch(params: {
  ...
  if (params.forcedRuntime) {
    patch.SUPPORT_CLAW_BUILD_PRIVATE_QA = "1";
    patch.SUPPORT_CLAW_QA_FORCE_RUNTIME = params.forcedRuntime;
  }
  if (params.forcedRuntime !== "codex" || params.providerMode !== "mock-openai") {
  ...
  patch.SUPPORT_CLAW_CODEX_APP_SERVER_ARGS = `app-server -c openai_base_url=${mockBaseUrl}/v1 --listen stdio://`;
...
function shouldCaptureGatewayHeapCheckpoints(env: NodeJS.ProcessEnv = process.env) {
  return parseQaSuiteBooleanEnv(env.SUPPORT_CLAW_QA_GATEWAY_HEAP_CHECKPOINTS) === true;
}
...
async function writeQaSuiteArtifacts(params: {
  ...
  const crablineChannelDriverSmoke = crablineChannelDriverSelection
    ? await runSupportClawCrablineChannelDriverSmoke({
        outputDir: params.outputDir,
  ...
  const report = renderQaMarkdownReport({
    title: "SupportClaw QA Scenario Suite",
    startedAt: params.startedAt,
...
export async function runQaFlowSuite(params?: QaSuiteRunParams): Promise<QaSuiteResult> {
  ...
  const enabledPluginIds = [
    ...new Set([
      ...
      ...(params?.forcedRuntime && params.forcedRuntime !== "supportClaw"
  ...
  const gateway = await startQaGatewayChild({
    ...
    mutateConfig: gatewayConfigPatch
      ? (cfg) => applyQaMergePatch(cfg, gatewayConfigPatch) as SupportClawConfig
      : undefined,
  ...
  try {
    ...
    const keepTemp = process.env.SUPPORT_CLAW_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
