...
export const QA_EVIDENCE_SUMMARY_KIND = "supportClaw.qa.evidence-summary";
...
export function resolveQaEvidenceProfile(params: {
  ...
  const envProfiles = [
    ["SUPPORT_CLAW_E2E_PROFILE", params.env?.SUPPORT_CLAW_E2E_PROFILE],
    ["SUPPORT_CLAW_QA_PROFILE", params.env?.SUPPORT_CLAW_QA_PROFILE],
  ] as const;
...
function resolveQaEvidenceRunner(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  return params.env?.SUPPORT_CLAW_QA_RUNNER?.trim() || params.fallback || "host";
}

function resolveQaEvidenceChannelDriver(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  const id =
    params.fallback?.trim() ||
    params.env?.SUPPORT_CLAW_QA_CHANNEL_DRIVER?.trim() ||
    params.env?.SUPPORT_CLAW_E2E_CHANNEL_DRIVER?.trim();
  return id ? { id } : undefined;
}

function resolveQaEvidenceEnvironment(env: NodeJS.ProcessEnv | undefined) {
  return {
    ref: env?.SUPPORT_CLAW_QA_REF?.trim() || env?.GITHUB_SHA?.trim() || null,
    os: process.platform,
...
function resolveQaEvidencePackageSource(env: NodeJS.ProcessEnv | undefined) {
  const spec = env?.SUPPORT_CLAW_QA_PACKAGE_SOURCE?.trim() || undefined;
  const sha = env?.SUPPORT_CLAW_QA_PACKAGE_SOURCE_SHA?.trim() || undefined;
  const explicitKind = env?.SUPPORT_CLAW_QA_PACKAGE_SOURCE_KIND?.trim();
  const kind =
    explicitKind ||
