...
describe("qa suite", () => {
  ...
  it("applies SUPPORT_CLAW_QA_SUITE_PROGRESS override and falls back on invalid values", () => {
    expect(
    ...
    expect(
      qaSuiteProgressTesting.shouldLogQaSuiteProgress({
        CI: "false",
        SUPPORT_CLAW_QA_SUITE_PROGRESS: "on",
      }),
    ).toBe(true);
    expect(
      qaSuiteProgressTesting.shouldLogQaSuiteProgress({
        CI: "true",
        SUPPORT_CLAW_QA_SUITE_PROGRESS: "off",
      }),
    ).toBe(false);
    expect(
      qaSuiteProgressTesting.shouldLogQaSuiteProgress({
  ...
  it("arms gateway heap checkpoint env only when requested", () => {
    expect(
      qaSuiteProgressTesting.buildQaGatewayHeapCheckpointRuntimeEnvPatch({
        SUPPORT_CLAW_QA_GATEWAY_HEAP_CHECKPOINTS: "0",
      }),
    ).toBeUndefined();
    expect(
      qaSuiteProgressTesting.buildQaGatewayHeapCheckpointRuntimeEnvPatch({
        SUPPORT_CLAW_QA_GATEWAY_HEAP_CHECKPOINTS: "1",
  ...
  it("omits mock OpenAI rewiring for non-codex runtime overrides", () => {
    expect(
      qaSuiteProgressTesting.buildQaRuntimeEnvPatch({
        providerMode: "mock-openai",
        forcedRuntime: "supportClaw",
        mockBaseUrl: "http://127.0.0.1:44080",
      }),
    ).toEqual({
      SUPPORT_CLAW_BUILD_PRIVATE_QA: "1",
      SUPPORT_CLAW_QA_FORCE_RUNTIME: "supportClaw",
    });
  });
