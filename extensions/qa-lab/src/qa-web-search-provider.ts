...
function buildQaLabSearchResult(query: string, index: number) {
  const url = `https://docs.supportClaw.ai/qa-lab/search-fixture/${index + 1}`;
  return {
    ...
    siteName: resolveSiteName(url) || "docs.supportClaw.ai",
  };
}
...
export function createQaLabWebSearchProvider(): WebSearchProviderPlugin {
  return {
    ...
    signupUrl: "https://docs.supportClaw.ai/concepts/qa-e2e-automation",
    docsUrl: "https://docs.supportClaw.ai/concepts/qa-e2e-automation",
    credentialPath: "",
    inactiveSecretPaths: [],
