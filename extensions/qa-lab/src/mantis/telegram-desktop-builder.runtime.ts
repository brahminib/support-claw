...
const TELEGRAM_PROFILE_ARCHIVE_ENV = "SUPPORT_CLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64";
const TELEGRAM_PROFILE_ARCHIVE_ENV_NAME_ENV =
  "SUPPORT_CLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_ARCHIVE_ENV";
const TELEGRAM_PROFILE_DIR_ENV = "SUPPORT_CLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_DIR";
...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.SUPPORT_CLAW_MANTIS_TELEGRAM_GROUP_ID)) {
    next.SUPPORT_CLAW_MANTIS_TELEGRAM_GROUP_ID = trimToValue(next.SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID);
  }
  if (!trimToValue(next.SUPPORT_CLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN)) {
    next.SUPPORT_CLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN = trimToValue(
      next.SUPPORT_CLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN,
    );
  }
  if (!trimToValue(next.SUPPORT_CLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN)) {
    next.SUPPORT_CLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN = trimToValue(
      next.SUPPORT_CLAW_QA_TELEGRAM_SUT_BOT_TOKEN,
    );
  }
  return next;
}
...
function resolveTelegramGatewayEnvPayload(
  ...
  const groupId = trimToValue(env.SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID);
  const driverToken = trimToValue(env.SUPPORT_CLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN);
  const sutToken = trimToValue(env.SUPPORT_CLAW_QA_TELEGRAM_SUT_BOT_TOKEN);
  if (!groupId || !driverToken || !sutToken) {
    throw new Error(
      "Telegram desktop builder requires SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID, SUPPORT_CLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN, and SUPPORT_CLAW_QA_TELEGRAM_SUT_BOT_TOKEN when using --credential-source env.",
    );
  }
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.SUPPORT_CLAW_MANTIS_TELEGRAM_GROUP_ID) &&
    trimToValue(params.env.SUPPORT_CLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN) &&
    trimToValue(params.env.SUPPORT_CLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.SUPPORT_CLAW_MANTIS_TELEGRAM_GROUP_ID = payload.groupId;
  params.env.SUPPORT_CLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN = payload.driverToken;
  params.env.SUPPORT_CLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN = payload.sutToken;
  params.env.SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID =
    trimToValue(params.env.SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID) ?? payload.groupId;
  params.env.SUPPORT_CLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN =
    trimToValue(params.env.SUPPORT_CLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN) ?? payload.driverToken;
  params.env.SUPPORT_CLAW_QA_TELEGRAM_SUT_BOT_TOKEN =
    trimToValue(params.env.SUPPORT_CLAW_QA_TELEGRAM_SUT_BOT_TOKEN) ?? payload.sutToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
if [ -n "\${SUPPORT_CLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64:-}" ]; then
  printf '%s' "$SUPPORT_CLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64" | base64 -d >"$out/telegram-profile.tgz"
...
qa_status=0
{
  ...
  if [ "$setup_gateway" = "1" ]; then
    export TELEGRAM_BOT_TOKEN="\${SUPPORT_CLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN:-\${TELEGRAM_BOT_TOKEN:-}}"
    telegram_group_id="\${SUPPORT_CLAW_MANTIS_TELEGRAM_GROUP_ID:-}"
    driver_token="\${SUPPORT_CLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN:-}"
    if [ -z "$TELEGRAM_BOT_TOKEN" ] || [ -z "$telegram_group_id" ] || [ -z "$driver_token" ]; then
      echo "Gateway setup requires SUPPORT_CLAW_MANTIS_TELEGRAM_GROUP_ID, SUPPORT_CLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN, and SUPPORT_CLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN." >&2
      exit 2
    fi
...
const token = process.env.SUPPORT_CLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
...
node --input-type=module -e 'import fs from "node:fs"; const value = JSON.parse(fs.readFileSync(process.argv[1], "utf8")); process.stdout.write(String(value.id || ""));' "$out/telegram-driver-getme.json")"
    export SUPPORT_CLAW_HOME="$HOME/.supportClaw-mantis/telegram-openclaw"
...
const token = process.env.SUPPORT_CLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
const chatId = process.env.SUPPORT_CLAW_MANTIS_TELEGRAM_GROUP_ID;
...
function renderReport(summary: MantisTelegramDesktopBuilderSummary) {
  const lines = [
    ...
    "- SupportClaw gateway log: `openclaw-gateway.log`",
...
export async function runMantisTelegramDesktopBuilder(
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Telegram desktop builder did not report a live SupportClaw gateway.");
    }
    summary = {
      artifacts: {
        reportPath,
