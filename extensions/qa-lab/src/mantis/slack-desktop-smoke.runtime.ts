...
const CRABBOX_BIN_ENV = "SUPPORT_CLAW_MANTIS_CRABBOX_BIN";
const CRABBOX_PROVIDER_ENV = "SUPPORT_CLAW_MANTIS_CRABBOX_PROVIDER";
const CRABBOX_CLASS_ENV = "SUPPORT_CLAW_MANTIS_CRABBOX_CLASS";
const CRABBOX_MARKET_ENV = "SUPPORT_CLAW_MANTIS_CRABBOX_MARKET";
const CRABBOX_LEASE_ID_ENV = "SUPPORT_CLAW_MANTIS_CRABBOX_LEASE_ID";
const CRABBOX_KEEP_ENV = "SUPPORT_CLAW_MANTIS_KEEP_VM";
const CRABBOX_IDLE_TIMEOUT_ENV = "SUPPORT_CLAW_MANTIS_CRABBOX_IDLE_TIMEOUT";
const CRABBOX_TTL_ENV = "SUPPORT_CLAW_MANTIS_CRABBOX_TTL";
const HYDRATE_MODE_ENV = "SUPPORT_CLAW_MANTIS_HYDRATE_MODE";
const SLACK_URL_ENV = "SUPPORT_CLAW_MANTIS_SLACK_URL";
const SLACK_CHANNEL_ID_ENV = "SUPPORT_CLAW_MANTIS_SLACK_CHANNEL_ID";
...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.SUPPORT_CLAW_LIVE_OPENAI_KEY) && trimToValue(next.OPENAI_API_KEY)) {
    next.SUPPORT_CLAW_LIVE_OPENAI_KEY = next.OPENAI_API_KEY;
  }
  if (!trimToValue(next.SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN) && trimToValue(next.SLACK_BOT_TOKEN)) {
    next.SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN = next.SLACK_BOT_TOKEN;
  }
  if (
    !trimToValue(next.SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(next.SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN)
  ) {
    next.SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN = next.SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN;
  }
  if (!trimToValue(next.SUPPORT_CLAW_MANTIS_SLACK_APP_TOKEN) && trimToValue(next.SLACK_APP_TOKEN)) {
    next.SUPPORT_CLAW_MANTIS_SLACK_APP_TOKEN = next.SLACK_APP_TOKEN;
  }
  if (
    !trimToValue(next.SUPPORT_CLAW_MANTIS_SLACK_APP_TOKEN) &&
    trimToValue(next.SUPPORT_CLAW_QA_SLACK_SUT_APP_TOKEN)
  ) {
    next.SUPPORT_CLAW_MANTIS_SLACK_APP_TOKEN = next.SUPPORT_CLAW_QA_SLACK_SUT_APP_TOKEN;
  }
  if (
    !trimToValue(next.SUPPORT_CLAW_MANTIS_SLACK_CHANNEL_ID) &&
    trimToValue(next.SUPPORT_CLAW_QA_SLACK_CHANNEL_ID)
  ) {
    next.SUPPORT_CLAW_MANTIS_SLACK_CHANNEL_ID = next.SUPPORT_CLAW_QA_SLACK_CHANNEL_ID;
  }
  return next;
}

function resolveSlackGatewayEnvPayload(env: NodeJS.ProcessEnv): SlackGatewayCredentialPayload {
  const channelId = trimToValue(env.SUPPORT_CLAW_QA_SLACK_CHANNEL_ID);
  const sutBotToken = trimToValue(env.SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN);
  const sutAppToken = trimToValue(env.SUPPORT_CLAW_QA_SLACK_SUT_APP_TOKEN);
  if (!channelId || !sutBotToken || !sutAppToken) {
    throw new Error(
      "Gateway setup requires SUPPORT_CLAW_QA_SLACK_CHANNEL_ID, SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN, and SUPPORT_CLAW_QA_SLACK_SUT_APP_TOKEN when using --credential-source env.",
    );
  }
  return {
    channelId,
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(params.env.SUPPORT_CLAW_MANTIS_SLACK_APP_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN = payload.sutBotToken;
  params.env.SUPPORT_CLAW_MANTIS_SLACK_APP_TOKEN = payload.sutAppToken;
  params.env.SUPPORT_CLAW_MANTIS_SLACK_CHANNEL_ID =
    trimToValue(params.env.SUPPORT_CLAW_MANTIS_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.SUPPORT_CLAW_QA_SLACK_CHANNEL_ID =
    trimToValue(params.env.SUPPORT_CLAW_QA_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN =
    trimToValue(params.env.SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN) ?? payload.sutBotToken;
  params.env.SUPPORT_CLAW_QA_SLACK_SUT_APP_TOKEN =
    trimToValue(params.env.SUPPORT_CLAW_QA_SLACK_SUT_APP_TOKEN) ?? payload.sutAppToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
remote_command_timeout_seconds="\${SUPPORT_CLAW_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS:-600}"
if [ -z "\${SUPPORT_CLAW_QA_SLACK_CHANNEL_ID:-}" ] && [ -n "$slack_channel_id" ]; then
  export SUPPORT_CLAW_QA_SLACK_CHANNEL_ID="$slack_channel_id"
fi
case "$remote_command_timeout_seconds" in
  ''|*[!0-9]*)
    echo "SUPPORT_CLAW_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS must be an integer number of seconds." >&2
    exit 2
    ;;
esac
if [ "$remote_command_timeout_seconds" -le 0 ]; then
  echo "SUPPORT_CLAW_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS must be greater than zero." >&2
  exit 2
fi
rm -rf "$out"
mkdir -p "$out"
export DISPLAY="\${DISPLAY:-:99}"
if [ -n "\${SUPPORT_CLAW_LIVE_OPENAI_KEY:-}" ] && [ -z "\${OPENAI_API_KEY:-}" ]; then
  export OPENAI_API_KEY="$SUPPORT_CLAW_LIVE_OPENAI_KEY"
fi
...
team_id="\${SUPPORT_CLAW_QA_SLACK_TEAM_ID:-}"
auth_test_token="\${SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN:-\${SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN:-}}"
...
const token = process.env.SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN || process.env.SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN;
...
if [ -z "$slack_url" ] && [ -n "$team_id" ] && [ -n "\${SUPPORT_CLAW_QA_SLACK_CHANNEL_ID:-}" ]; then
  slack_url="https://app.slack.com/client/$team_id/$SUPPORT_CLAW_QA_SLACK_CHANNEL_ID"
fi
profile="\${SUPPORT_CLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR:-$HOME/.config/openclaw-mantis/slack-chrome-profile}"
mkdir -p "$profile"
if [ "$setup_gateway" = "1" ]; then
  export SLACK_BOT_TOKEN="\${SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN:-\${SLACK_BOT_TOKEN:-}}"
  export SLACK_APP_TOKEN="\${SUPPORT_CLAW_MANTIS_SLACK_APP_TOKEN:-\${SLACK_APP_TOKEN:-}}"
  if [ -z "$SLACK_BOT_TOKEN" ] || [ -z "$SLACK_APP_TOKEN" ]; then
    echo "Gateway setup requires SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN and SUPPORT_CLAW_MANTIS_SLACK_APP_TOKEN." >&2
    exit 2
  fi
...
run_mantis_remote_body() {
  ...
  if [ "$setup_gateway" = "1" ]; then
    export SUPPORT_CLAW_HOME="$HOME/.supportClaw-mantis/slack-openclaw"
    mkdir -p "$SUPPORT_CLAW_HOME"
...
MANTIS_SLACK_PATCH
    ...
    if ! kill -0 "$gateway_pid" >/dev/null 2>&1; then
      echo "SupportClaw gateway exited during startup." >&2
    ...
    if [ "$approval_checkpoints" = "1" ]; then
      ...
      export SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR="$checkpoint_dir"
      export SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS="\${SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS:-120000}"
      export SUPPORT_CLAW_MANTIS_APPROVAL_CHECKPOINT_SCENARIOS_JSON="$approval_checkpoint_scenarios_json"
      export SUPPORT_CLAW_MANTIS_APPROVAL_BROWSER_BIN="$browser_bin"
      cat >"$out/approval-checkpoint-watcher.mjs" <<'MANTIS_APPROVAL_WATCHER'
	import { spawn } from "node:child_process";
	import fs from "node:fs/promises";
	import path from "node:path";

const checkpointDir = process.env.SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR;
const timeoutMs = Number.parseInt(
  process.env.SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS || "120000",
  10,
);
	const scenarioIds = JSON.parse(
	  process.env.SUPPORT_CLAW_MANTIS_APPROVAL_CHECKPOINT_SCENARIOS_JSON || "[]",
	);
	const browserBin = process.env.SUPPORT_CLAW_MANTIS_APPROVAL_BROWSER_BIN;

if (!checkpointDir) {
  throw new Error("SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR is required.");
}
if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
  throw new Error("SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS must be a positive integer.");
}
...
async function waitForCheckpoint(filePath) {
  ...
  throw new Error(\`Timed out waiting for approval checkpoint: \${filePath}\`);
	...
	async function captureScreenshot(screenshotPath, checkpoint) {
	  if (!browserBin) {
	    throw new Error("SUPPORT_CLAW_MANTIS_APPROVAL_BROWSER_BIN is required to render approval checkpoint screenshots.");
	  }
...
export async function runMantisSlackDesktopSmoke(
  ...
  const slackChannelId =
    ...
    trimToValue(env.SUPPORT_CLAW_QA_SLACK_CHANNEL_ID) ??
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Slack desktop gateway setup did not report a live SupportClaw gateway.");
    }
