// Qa Lab tests cover telegram desktop builder plugin behavior.
...
describe("mantis Telegram desktop builder runtime", () => {
  ...
  it("leases a desktop box, installs Telegram Desktop, configures SupportClaw, and keeps the gateway lease", async () => {
    ...
    const result = await runMantisTelegramDesktopBuilder({
      ...
      env: {
        ...
        SUPPORT_CLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver-token",
        SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID: "-1001234567890",
        SUPPORT_CLAW_QA_TELEGRAM_SUT_BOT_TOKEN: "sut-token",
    ...
    expect(runCommand?.env?.SUPPORT_CLAW_LIVE_OPENAI_KEY).toBe("openai-runtime-key");
    expect(runCommand?.env?.SUPPORT_CLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64).toBe(
      "profile-archive",
    );
    expect(runCommand?.env?.SUPPORT_CLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN).toBe("driver-token");
    expect(runCommand?.env?.SUPPORT_CLAW_MANTIS_TELEGRAM_GROUP_ID).toBe("-1001234567890");
    expect(runCommand?.env?.SUPPORT_CLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN).toBe("sut-token");
    ...
    expect(remoteScript).toContain("SUPPORT_CLAW_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64");
  ...
  it("leases Convex Telegram credentials and maps them into the VM env", async () => {
    ...
    const result = await runMantisTelegramDesktopBuilder({
      ...
      env: {
        CI: "1",
        SUPPORT_CLAW_QA_CONVEX_SECRET_CI: "convex-secret",
        SUPPORT_CLAW_QA_CONVEX_SITE_URL: "https://example.convex.site",
        PATH: process.env.PATH,
      },
      keepLease: false,
    ...
    expect(runCommand?.env?.SUPPORT_CLAW_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN).toBe("driver-leased");
    expect(runCommand?.env?.SUPPORT_CLAW_MANTIS_TELEGRAM_GROUP_ID).toBe("-100222333444");
    expect(runCommand?.env?.SUPPORT_CLAW_MANTIS_TELEGRAM_SUT_BOT_TOKEN).toBe("sut-leased");
    expect(runCommand?.env?.SUPPORT_CLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN).toBe("driver-leased");
    expect(runCommand?.env?.SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID).toBe("-100222333444");
    expect(runCommand?.env?.SUPPORT_CLAW_QA_TELEGRAM_SUT_BOT_TOKEN).toBe("sut-leased");
    expect(
      commands.some((entry) => entry.command === "/tmp/crabbox" && entry.args[0] === "stop"),
    ).toBe(true);
