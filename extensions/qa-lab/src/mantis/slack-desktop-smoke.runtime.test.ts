...
describe("mantis Slack desktop smoke runtime", () => {
  ...
  it("leases a desktop box, runs Slack QA inside it, copies artifacts, and stops on pass", async () => {
    ...
    const runtimeEnv = {
      ...
      SUPPORT_CLAW_QA_SLACK_CHANNEL_ID: "C123",
      SUPPORT_CLAW_QA_SLACK_DRIVER_BOT_TOKEN: "driver-token",
      SUPPORT_CLAW_QA_SLACK_SUT_APP_TOKEN: "app-token",
      SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN: "sut-token",
    };
    const runner = vi.fn(
    ...
    expect(
      commands.every((entry) => entry.env?.SUPPORT_CLAW_LIVE_OPENAI_KEY === "openai-runtime-key"),
    ).toBe(true);
    ...
    expect(remoteScript).toContain("SUPPORT_CLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR");
  ...
  it("runs approval checkpoint mode with default approval scenarios and records screenshots", async () => {
    ...
    expect(remoteScript).toContain('export SUPPORT_CLAW_QA_SLACK_CHANNEL_ID="$slack_channel_id"');
    ...
    expect(remoteScript).toContain("SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR");
    expect(remoteScript).toContain("SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS");
    ...
    expect(remoteScript).toContain("SUPPORT_CLAW_MANTIS_APPROVAL_BROWSER_BIN");
  ...
  it("leases Convex Slack credentials for gateway setup and maps them into the VM env", async () => {
    ...
    expect(runCommand?.env?.SUPPORT_CLAW_MANTIS_SLACK_APP_TOKEN).toBe("xapp-leased");
    expect(runCommand?.env?.SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN).toBe("xoxb-leased");
    expect(runCommand?.env?.SUPPORT_CLAW_MANTIS_SLACK_CHANNEL_ID).toBe("CLEASED");
    expect(runCommand?.env?.SUPPORT_CLAW_QA_SLACK_CHANNEL_ID).toBe("CLEASED");
    expect(runCommand?.env?.SUPPORT_CLAW_QA_SLACK_SUT_APP_TOKEN).toBe("xapp-leased");
    expect(runCommand?.env?.SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN).toBe("xoxb-leased");
  ...
  it("passes gateway setup when Crabbox returns non-zero after remote metadata proves success", async () => {
    ...
    const result = await runMantisSlackDesktopSmoke({
      ...
      env: {
        OPENAI_API_KEY: "openai-runtime-key",
        SUPPORT_CLAW_MANTIS_SLACK_APP_TOKEN: "xapp-direct",
        SUPPORT_CLAW_MANTIS_SLACK_BOT_TOKEN: "xoxb-direct",
        PATH: process.env.PATH,
      },
      gatewaySetup: true,
