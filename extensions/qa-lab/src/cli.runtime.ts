...
import {
  SUPPORT_CLAW_CRABLINE_DEFAULT_CHANNEL,
  resolveSupportClawCrablineChannelDriverSelection,
} from "crabline";
...
const QA_CREDENTIAL_PAYLOAD_MAX_BYTES_ENV = "SUPPORT_CLAW_QA_CREDENTIAL_PAYLOAD_MAX_BYTES";
...
function parseQaRuntimePair(value: string | undefined): [RuntimeId, RuntimeId] | undefined {
  ...
  if (runtimes.length !== 2) {
    throw new Error('--runtime-pair must use exactly two runtimes, e.g. "supportClaw,codex".');
  }
  const [left, right] = runtimes;
  if (!left || !right) {
    throw new Error('--runtime-pair only supports "supportClaw" and "codex".');
  }
  if (left === right) {
...
async function withTemporaryQaProfileEnv<T>(profile: string, run: () => Promise<T>): Promise<T> {
  ...
  process.env.SUPPORT_CLAW_QA_PROFILE = profile;
  try {
    return await run();
  } finally {
    ...
    channelDriver === "crabline"
      ? resolveSupportClawCrablineChannelDriverSelection({
          channel: resolveQaSuiteScenarioChannel({
            defaultChannel: SUPPORT_CLAW_CRABLINE_DEFAULT_CHANNEL,
...
export async function runQaJsonlReplayCommand(opts: {
  ...
  const repoRoot = path.resolve(opts.repoRoot ?? process.cwd());
  const runtimePair = parseQaRuntimePair(opts.runtimePair) ?? ["supportClaw", "codex"];
  if (runtimePair[0] !== "supportClaw" || runtimePair[1] !== "codex") {
    throw new Error('--runtime-pair for jsonl-replay must be "supportClaw,codex".');
  }
...
export async function runQaLabUiCommand(opts: {
  ...
  const server = await startQaLabServer({
    ...
    controlUiProxyToken: process.env.SUPPORT_CLAW_QA_CONTROL_UI_PROXY_TOKEN,
