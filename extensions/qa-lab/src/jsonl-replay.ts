...
export type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { supportClaw: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { supportClaw: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      supportClaw: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.supportClaw.push(parity.cells.supportClaw);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# SupportClaw JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
