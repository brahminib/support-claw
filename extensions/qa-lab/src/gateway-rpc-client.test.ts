...
vi.mock("supportClaw/plugin-sdk/gateway-runtime", () => ({
...
describe("startQaGatewayRpcClient", () => {
  ...
  it("calls the in-process gateway cli helper without mutating process.env", async () => {
    const originalHome = process.env.SUPPORT_CLAW_HOME;
    ...
    try {
      ...
      if (originalHome === undefined) {
        ...
        process.env.SUPPORT_CLAW_HOME = originalHome;
      }
    }

    expect(process.env.SUPPORT_CLAW_HOME).toBe(originalHome);
  });
  ...
  it("wraps request failures with gateway logs", async () => {
    ...
    const client = await startQaGatewayRpcClient({
      wsUrl: "ws://127.0.0.1:18789",
      token: "qa-token",
      logs: () => "SUPPORT_CLAW_GATEWAY_TOKEN=secret-token\nAuthorization: Bearer secret+/token=123456",
    });
