...
export const CODEX_PLUGIN_LIFECYCLE_MESSAGES = Object.freeze({
  missingPlugin:
    'Codex plugin is required for Codex runtime. Run "supportClaw doctor --fix" to install @supportclaw/codex, then retry.',
});
...
function formatPinnedOldRemediation(pluginVersion: string, hostVersion: string) {
  return `Codex plugin version ${pluginVersion} is older than SupportClaw ${hostVersion}. Run "supportClaw plugins update codex" or unpin codex, then rerun "supportClaw doctor --fix".`;
}

function formatPinnedNewRemediation(pluginVersion: string, hostVersion: string) {
  return `Codex plugin version ${pluginVersion} requires a newer SupportClaw host than ${hostVersion}. Upgrade SupportClaw or install a codex plugin version pinned to ${hostVersion}.`;
}
...
export async function seedCodexPluginAt(
  ...
  await fs.writeFile(
    path.join(targetDir, "supportClaw.plugin.json"),
