...
describe("codex plugin lifecycle: pinned-old codex plugin with new SupportClaw", () => {
  it("blocks with a precise update remediation when the plugin is older than the host", async () => {
    ...
    expect(result.remediation).toBe(
      'Codex plugin version 2026.5.19 is older than SupportClaw 2026.5.21. Run "supportClaw plugins update codex" or unpin codex, then rerun "supportClaw doctor --fix".',
    );
  });
});

describe("codex plugin lifecycle: pinned-new codex plugin with old SupportClaw", () => {
  it("blocks with a precise host-upgrade remediation when the plugin is newer than the host", async () => {
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toBe(
      "Codex plugin version 2026.5.22 requires a newer SupportClaw host than 2026.5.21. Upgrade SupportClaw or install a codex plugin version pinned to 2026.5.21.",
    );
  });
});
...
describe("codex plugin lifecycle: doctor migration safety matrix", () => {
  it.each([
    ...
    {
      name: "mixed profile with defaults SupportClaw pin",
      profileShape: "mixed" as const,
      config: { agents: { defaults: { agentRuntime: { id: "supportClaw" } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=supportClaw"],
    },
    {
      name: "mixed profile with main-agent SupportClaw pin",
      profileShape: "mixed" as const,
      config: { agents: { list: { main: { agentRuntime: { id: "supportClaw" } } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=supportClaw"],
    },
  ])(
    "keeps codex auth and strips stale SupportClaw runtime pins for $name",
