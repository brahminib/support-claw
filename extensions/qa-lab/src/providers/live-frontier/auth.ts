...
} from "supportClaw/plugin-sdk/provider-auth";
...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "SUPPORT_CLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "SUPPORT_CLAW_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "SUPPORT_CLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["SUPPORT_CLAW_LIVE_ANTHROPIC_KEY"],
  gemini: ["SUPPORT_CLAW_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "SUPPORT_CLAW_LIVE_CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: SupportClawConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: SupportClawConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: SupportClawConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.SUPPORT_CLAW_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "supportClaw") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or SUPPORT_CLAW_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host SupportClaw OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}
