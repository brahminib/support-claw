...
import {
  SUPPORT_CLAW_CRABLINE_MANIFEST_PATH,
  startSupportClawCrablineAdapter,
  type SupportClawCrablineChannelDriverSelection,
  type StartedSupportClawCrablineAdapter,
} from "crabline";
...
async function postCrablineInbound(params: {
  adapter: StartedSupportClawCrablineAdapter;
...
class QaCrablineTransport extends QaStateBackedTransportAdapter {
  readonly #adapter: StartedSupportClawCrablineAdapter;
  readonly #selection: SupportClawCrablineChannelDriverSelection;
  readonly #state: QaCrablineTransportState;

  constructor(params: {
    adapter: StartedSupportClawCrablineAdapter;
    selection: SupportClawCrablineChannelDriverSelection;
  ...
  createReportNotes = (_params: QaTransportReportParams) => [
    `Runs SupportClaw's ${this.#selection.channel} channel plugin against a Crabline fake provider server.`,
...
export async function createQaCrablineTransportAdapter(params: {
  ...
  const adapter = await startSupportClawCrablineAdapter({
    channel: params.selection.channel,
    supportClawConfig: {},
    recorderPath,
  });
  await fs.writeFile(
    path.join(params.outputDir, SUPPORT_CLAW_CRABLINE_MANIFEST_PATH),
