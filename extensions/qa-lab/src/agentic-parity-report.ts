...
type QaRuntimeParityScenarioReport = {
  ...
  driftDetails?: string;
  supportClawStatus: "pass" | "fail" | "missing";
  codexStatus: "pass" | "fail" | "missing";
  supportClawTokens: number;
  codexTokens: number;
...
function scenarioHasRuntimeToolCallEvidence(scenario: QaParityReportScenario): boolean {
  ...
  return (
    ...
    parity.cells.supportClaw.toolCalls.length > 0 &&
...
function describeLiveUsageFailure(scenarioName: string, scenario: QaRuntimeParityScenarioReport) {
  const missing = [
    scenario.supportClawTokens > 0
      ? undefined
      : `${scenario.supportClawStatus === "pass" ? "supportClaw" : "supportClaw failed"}=0`,
...
export function renderQaAgenticParityMarkdownReport(comparison: QaAgenticParityComparison): string {
  ...
  const lines = [
    `# SupportClaw Agentic Parity Report — ${comparison.candidateLabel} vs ${comparison.baselineLabel}`,
    "",
  ...
  const scenarios: QaRuntimeParityScenarioReport[] = params.summary.scenarios.map((scenario) => {
    ...
    if (!parity) {
      ...
      return {
        ...
        supportClawStatus: "missing",
        codexStatus: "missing",
        supportClawTokens: 0,
        codexTokens: 0,
        supportClawToolCalls: 0,
        codexToolCalls: 0,
      } satisfies QaRuntimeParityScenarioReport;
    }
    driftCounts[parity.drift] += 1;
    const supportClawCell = parity.cells.supportClaw;
    const codexCell = parity.cells.codex;
    const supportClawStatus = runtimeParityCellStatus(supportClawCell);
    ...
    const reportScenario = {
      ...
      supportClawStatus,
      codexStatus,
      supportClawTokens: supportClawCell.usage.totalTokens,
      codexTokens: codexCell.usage.totalTokens,
      supportClawToolCalls: supportClawCell.toolCalls.length,
...
export function renderQaRuntimeParityMarkdownReport(report: QaRuntimeParityReport): string {
  const lines = [
    `# SupportClaw Runtime Parity Report — ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
  ...
  for (const scenario of report.scenarios) {
    ...
    lines.push(
      `- supportClaw: ${scenario.supportClawStatus} (${scenario.supportClawToolCalls} tool calls, ${scenario.supportClawTokens} tokens)`,
    );
    lines.push(
