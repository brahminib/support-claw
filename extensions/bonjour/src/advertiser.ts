...
import type { PluginLogger } from "supportClaw/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "supportClaw/plugin-sdk/runtime-env";
...
/** Input data used to publish SupportClaw gateway Bonjour records. */
...
// See https://github.com/brahminib/support-claw/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.SUPPORT_CLAW_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "SupportClaw"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "SupportClaw";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(SupportClaw\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.SUPPORT_CLAW_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "supportClaw";
    ...
    const recreateAdvertiser = async (reason: string, optsLocal?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or SUPPORT_CLAW_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
