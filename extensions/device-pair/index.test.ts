...
import type { SupportClawPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredSupportClawTmpDir: vi.fn(() => path.join(os.tmpdir(), "supportClaw-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredSupportClawTmpDir: pluginApiMocks.resolvePreferredSupportClawTmpDir,
...
function createApi(params?: {
  ...
  runtime?: SupportClawPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as SupportClawPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: SupportClawPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as SupportClawPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredSupportClawTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredSupportClawTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the SupportClaw iOS app:");
    ...
    expect(text).not.toContain("![SupportClaw pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the SupportClaw iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("allows mdns cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://supportClaw.local:18789",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://supportClaw.local:18789");
  });
