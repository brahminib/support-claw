...
const {
  ...
  resolveProviderHttpRequestConfigMock: vi.fn((params: Record<string, unknown>) => {
    ...
    if (params.provider === "xai") {
      const version = process.env.SUPPORT_CLAW_VERSION?.trim() || "unknown";
      headers.set("User-Agent", `supportClaw/${version}`);
      headers.set("originator", "supportClaw");
...
vi.mock("supportClaw/plugin-sdk/provider-auth-runtime", () => ({
...
vi.mock("supportClaw/plugin-sdk/provider-http", () => ({
...
describe("xai image generation provider", () => {
  ...
  it("builds provider with correct models, default, and capabilities", () => {
    ...
    expect(isConfigured({ agentDir: "/tmp/supportClaw-xai-test" })).toBe(true);
    expect(isProviderApiKeyConfiguredMock).toHaveBeenCalledWith({
      provider: "xai",
      agentDir: "/tmp/supportClaw-xai-test",
    });
  });
  ...
  it("forwards xAI attribution User-Agent through the SDK image request", async () => {
    ...
    const request = requirePostJsonCall();
    expect(request.headers?.get("user-agent")).toBe("supportClaw/2026.3.22");
    expect(request.headers?.get("originator")).toBe("supportClaw");
