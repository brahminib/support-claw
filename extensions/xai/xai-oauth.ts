...
import type { ProviderAuthContext, ProviderAuthMethod } from "supportClaw/plugin-sdk/plugin-entry";
import {
...
import { waitForLocalOAuthCallback } from "supportClaw/plugin-sdk/provider-auth-runtime";
...
export function buildXaiOAuthAuthorizeUrl(params: {
  ...
  url.searchParams.set("referrer", "supportClaw");
  return url.toString();
}
  ...
  try {
    ...
    return buildOauthProviderAuthResult({
      ...
      notes: [
        ...
        "xAI may label the consent app as Grok Build because SupportClaw uses xAI's shared OAuth client.",
      ],
    });
  } catch (err) {
