...
function readLegacyGrokFallbackAuth(cfg?: SupportClawConfig): XaiFallbackAuth | undefined {
...
function readLegacyGrokApiKeyResult(cfg?: SupportClawConfig): ConfiguredRuntimeApiKeyResolution {
  const search = cfg?.tools?.web?.search;
  if (!search || typeof search !== "object") {
    return { status: "missing" };
  }
...
export function resolveFallbackXaiAuth(cfg?: SupportClawConfig): XaiFallbackAuth | undefined {
