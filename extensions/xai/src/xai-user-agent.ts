// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatSupportClawUserAgent`.

import { SUPPORT_CLAW_VERSION as PACKAGE_VERSION } from "supportClaw/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "supportClaw";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.SUPPORT_CLAW_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.SUPPORT_CLAW_SERVICE_VERSION) ??
...
// an empty record so the supportClaw identity is not forwarded to the proxy.
