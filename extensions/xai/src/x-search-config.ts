...
function resolveLegacyXSearchConfig(config?: SupportClawConfig): JsonRecord | undefined {
...
function resolvePluginXSearchConfig(config?: SupportClawConfig): JsonRecord | undefined {
...
function resolveLegacyGrokWebSearchConfig(config?: SupportClawConfig): JsonRecord | undefined {
  const web = config?.tools?.web as Record<string, unknown> | undefined;
...
function resolvePluginWebSearchConfig(config?: SupportClawConfig): JsonRecord | undefined {
  const pluginConfig = config?.plugins?.entries?.xai?.config;
...
export function resolveEffectiveXSearchConfig(config?: SupportClawConfig): JsonRecord | undefined {
