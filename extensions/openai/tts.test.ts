...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds SupportClaw attribution headers to native OpenAI speech requests", async () => {
      ...
      expect(headers?.originator).toBe("supportClaw");
      expect(headers?.version).toBe("2026.3.22");
      expect(headers?.["User-Agent"]).toBe("supportClaw/2026.3.22");
    });
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.SUPPORT_CLAW_DEBUG_PROXY_SESSION_ID = "tts-session";
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      ...
      process.env.SUPPORT_CLAW_DEBUG_PROXY_ENABLED = "1";
      process.env.SUPPORT_CLAW_STATE_DIR = tempDir;
      process.env.SUPPORT_CLAW_DEBUG_PROXY_SESSION_ID = "tts-patched-session";
