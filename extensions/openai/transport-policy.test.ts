...
import type { ProviderRuntimeModel } from "supportClaw/plugin-sdk/plugin-entry";
...
describe("openai transport policy", () => {
  ...
  it("builds native turn state for direct OpenAI routes", () => {
    ...
    expect(state?.headers?.["x-supportClaw-session-id"]).toBe("session-123");
    expect(state?.headers?.["x-supportClaw-turn-id"]).toBe("turn-123");
    expect(state?.headers?.["x-supportClaw-turn-attempt"]).toBe("2");
    expect(state?.metadata?.supportClaw_session_id).toBe("session-123");
    expect(state?.metadata?.supportClaw_turn_id).toBe("turn-123");
    expect(state?.metadata?.supportClaw_turn_attempt).toBe("2");
    expect(state?.metadata?.supportClaw_transport).toBe("websocket");
  });
  ...
  it("returns websocket session headers and cooldown for native routes", () => {
    ...
    expect(policy?.headers?.["x-supportClaw-session-id"]).toBe("session-123");
