...
function resolveSessionHeaders(params: {
  ...
  return {
    ...
    "x-supportClaw-session-id": sessionId,
  };
}
...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-supportClaw-turn-id": turnId,
      "x-supportClaw-turn-attempt": attempt,
    },
    metadata: {
      supportClaw_session_id: sessionHeaders["x-supportClaw-session-id"] ?? "",
      supportClaw_turn_id: turnId,
      supportClaw_turn_attempt: attempt,
      supportClaw_transport: ctx.transport,
    },
  };
}
