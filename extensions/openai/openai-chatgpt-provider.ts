...
import { CODEX_CLI_PROFILE_ID, type OAuthCredential } from "supportClaw/plugin-sdk/provider-auth";
...
import { fetchCodexUsage } from "supportClaw/plugin-sdk/provider-usage";
import {
 ...
 * entirely when they were written by older SupportClaw versions. When that row wins
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.supportClaw.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `supportClaw models auth login --provider openai` or `supportClaw configure`.";
}
