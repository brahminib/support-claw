...
import type { ProviderAuthContext } from "supportClaw/plugin-sdk/plugin-entry";
import { ensureGlobalUndiciEnvProxyDispatcher } from "supportClaw/plugin-sdk/runtime-env";
import { formatCliCommand } from "supportClaw/plugin-sdk/setup-tools";
...
const openAICodexOAuthOriginator = "supportClaw";
const localManualFallbackDelayMs = 15_000;
const localManualFallbackGraceMs = 1_000;
const openAIAuthProbeUrl =
  "https://auth.openai.com/oauth/authorize?response_type=code&client_id=supportClaw-preflight&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback&scope=openid+profile+email";
  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the SupportClaw process and then retry `supportClaw models auth login --provider openai`.",
      ].join(" "),
      error,
    );
  }
...
export async function loginOpenAICodexOAuth(params: {
  ...
  await prompter.note(
    isRemote
      ? [
          ...
          "If this SupportClaw process can receive the browser callback, sign-in may finish automatically before you paste.",
        ].join("\n")
      : [
  ...
  try {
    ...
    await prompter.note("Trouble with OAuth? See https://docs.supportClaw.ai/start/faq", "OAuth help");
