...
} from "supportClaw/plugin-sdk/image-generation";
import {
  parseOpenAiCompatibleImageResponse,
  toImageDataUrl,
} from "supportClaw/plugin-sdk/image-generation";
import { createSubsystemLogger } from "supportClaw/plugin-sdk/logging-core";
import { resolveClosestSize } from "supportClaw/plugin-sdk/media-generation-runtime";
import { extensionForMime } from "supportClaw/plugin-sdk/media-mime";
import {
...
function shouldAllowPrivateImageEndpoint(req: {
  ...
  return process.env.SUPPORT_CLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER === "1";
}
...
function hasExplicitOpenAIImageApiKeyConfig(cfg: SupportClawConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return providerConfig?.apiKey !== undefined || providerConfig?.auth === "api-key";
}

function hasExplicitDirectOpenAIImageConfig(cfg: SupportClawConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  if (!providerConfig) {
    return false;
  }
...
function hasChatGPTImageRouteConfig(cfg: SupportClawConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return (
...
function forceOpenAIImageApiKeyAuth(cfg: SupportClawConfig | undefined): SupportClawConfig | undefined {
