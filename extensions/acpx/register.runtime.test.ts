...
vi.mock("supportClaw/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.SUPPORT_CLAW_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.SUPPORT_CLAW_SKIP_ACPX_RUNTIME;
  } else {
    process.env.SUPPORT_CLAW_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/supportClaw-acpx-register-test",
    stateDir: "/tmp/supportClaw-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
