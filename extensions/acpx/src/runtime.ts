/**
 * SupportClaw ACPX runtime adapter. It wraps the upstream acpx runtime with
 * SupportClaw session metadata, lease tracking, model scoping, and cleanup policy.
 */
...
import { parseStrictPositiveInteger } from "supportClaw/plugin-sdk/number-runtime";
import { redactSensitiveText } from "supportClaw/plugin-sdk/security-runtime";
import { normalizeStringEntries } from "supportClaw/plugin-sdk/string-coerce-runtime";
...
import {
  cleanupSupportClawOwnedAcpxProcessTree,
  isSupportClawLeaseAwareAcpxProcessCommand,
...
type SupportClawAcpxRuntimeOptions = AcpRuntimeOptions & {
  supportClawWrapperRoot?: string;
  supportClawGatewayInstanceId?: string;
  supportClawProcessLeaseStore?: AcpxProcessLeaseStore;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  supportClawProcessCleanup?: AcpxProcessCleanupDeps;
};
type SupportClawRuntimeTurnInput = Parameters<NonNullable<AcpRuntime["startTurn"]>>[0];
type SupportClawRuntimeEnsureInput = Parameters<AcpRuntime["ensureSession"]>[0];
...
type SupportClawLeaseSessionMetadata = {
  supportClawLeaseId: string;
  supportClawGatewayInstanceId: string;
};

function withSupportClawManagedTurnTimeout<T extends object>(input: T): T & { timeoutMs: 0 } {
  // SupportClaw owns ACP turn deadlines. acpx treats timeout after partial agent
...
function withSupportClawLeaseSessionMetadata<T extends object>(
  record: T,
  metadata: SupportClawLeaseSessionMetadata,
): T & SupportClawLeaseSessionMetadata {
  return {
    ...record,
    supportClawLeaseId: metadata.supportClawLeaseId,
    supportClawGatewayInstanceId: metadata.supportClawGatewayInstanceId,
  };
}
...
function readSupportClawLeaseIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { supportClawLeaseId } = record as { supportClawLeaseId?: unknown };
  return typeof supportClawLeaseId === "string" ? supportClawLeaseId.trim() || undefined : undefined;
}

function readSupportClawGatewayInstanceIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { supportClawGatewayInstanceId } = record as { supportClawGatewayInstanceId?: unknown };
  return typeof supportClawGatewayInstanceId === "string"
    ? supportClawGatewayInstanceId.trim() || undefined
    : undefined;
}
...
function createResetAwareSessionStore(
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return withSupportClawLeaseSessionMetadata(record, {
        supportClawLeaseId: lease.leaseId,
        supportClawGatewayInstanceId: lease.gatewayInstanceId,
      });
    },
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        recordToSave = withSupportClawLeaseSessionMetadata(
          {
            ...record,
          ...
          {
            supportClawLeaseId: launch.leaseId,
            supportClawGatewayInstanceId: launch.gatewayInstanceId,
          },
        );
      }
...
const SUPPORT_CLAW_BRIDGE_EXECUTABLE = "supportClaw";
const SUPPORT_CLAW_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_SUPPORT_CLAW_PREFIX = "openai/";
const CLAUDE_ACP_SUPPORT_CLAW_PREFIX = "anthropic/";
...
function isSupportClawBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === SUPPORT_CLAW_BRIDGE_EXECUTABLE) {
    return parts[1] === SUPPORT_CLAW_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^supportClaw(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === SUPPORT_CLAW_BRIDGE_SUBCOMMAND;
}
...
// See supportClaw/supportClaw#73071.
...
function normalizeCodexAcpModelOverride(
  ...
  let value = raw;
  if (value.toLowerCase().startsWith(CODEX_ACP_SUPPORT_CLAW_PREFIX)) {
    value = value.slice(CODEX_ACP_SUPPORT_CLAW_PREFIX.length);
  }
...
function normalizeClaudeAcpModelOverride(rawModel: string | undefined): string | undefined {
  ...
  if (!raw.toLowerCase().startsWith(CLAUDE_ACP_SUPPORT_CLAW_PREFIX)) {
    return raw;
  }
  return raw.slice(CLAUDE_ACP_SUPPORT_CLAW_PREFIX.length).trim() || undefined;
}

function withAcpxSessionOptions(input: SupportClawRuntimeEnsureInput): AcpxDelegateEnsureInput {
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isSupportClawBridgeCommand(command);
}
...
/** SupportClaw-managed ACP runtime implementation backed by the upstream acpx runtime. */
export class AcpxRuntime implements AcpRuntime {
  ...
  constructor(options: SupportClawAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    const { supportClawProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = supportClawProcessCleanup;
    this.wrapperRoot = options.supportClawWrapperRoot;
    this.gatewayInstanceId = options.supportClawGatewayInstanceId;
    this.processLeaseStore = options.supportClawProcessLeaseStore;
  ...
  private async readCodexTurnFailureStderr(params: { handle: AcpRuntimeHandle }): Promise<string> {
    ...
    return readCodexWrapperStderrTail({
      wrapperRoot: this.wrapperRoot,
      leaseId: readSupportClawLeaseIdFromRecord(record),
    });
  }
  ...
  private async cleanupProcessTreeForRecord(
    ...
    const leaseId = readSupportClawLeaseIdFromRecord(record);
    ...
    if (lease && lease.gatewayInstanceId === this.gatewayInstanceId && lease.rootPid > 0) {
      ...
      const result = await cleanupSupportClawOwnedAcpxProcessTree({
    ...
    const expectedGatewayInstanceId = readSupportClawGatewayInstanceIdFromRecord(record);
    await cleanupSupportClawOwnedAcpxProcessTree({
      rootPid,
      rootCommand,
  ...
  async *runTurn(input: Parameters<AcpRuntime["runTurn"]>[0]): AsyncIterable<AcpRuntimeEvent> {
    ...
    try {
      for await (const event of delegate.runTurn(withSupportClawManagedTurnTimeout(input))) {
        if (
          event.type !== "error" ||
          !isCodexAcpCommand(command) ||
  ...
  startTurn(input: SupportClawRuntimeTurnInput): AcpRuntimeTurn {
    ...
    const turnPromise = Promise.all([
      ...
      try {
        return {
          command,
          turn: delegate.startTurn(withSupportClawManagedTurnTimeout(input)),
        };
      } catch (error) {
        if (!isCodexAcpCommand(command) || !isGenericInternalAcpError(error)) {
          throw error;
