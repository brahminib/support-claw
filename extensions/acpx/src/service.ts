...
import { finiteSecondsToTimerSafeMilliseconds } from "supportClaw/plugin-sdk/number-runtime";
import type {
...
const ENABLE_STARTUP_PROBE_ENV = "SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE";
const SKIP_RUNTIME_PROBE_ENV = "SUPPORT_CLAW_SKIP_ACPX_RUNTIME_PROBE";
  ...
  async function resolveRuntime(): Promise<AcpxRuntimeLike> {
    ...
    runtimePromise ??= loadRuntimeModule().then((module) => {
      runtime = new module.AcpxRuntime({
        ...
        supportClawGatewayInstanceId: params.gatewayInstanceId,
        supportClawProcessLeaseStore: params.processLeaseStore,
        supportClawWrapperRoot: params.wrapperRoot,
...
function resolveAllowedAgentsProbeAgent(ctx: SupportClawPluginServiceContext): string | undefined {
...
async function reapOpenAcpxProcessLeases(params: {
  ...
  for (const lease of leases) {
    if (lease.rootPid <= 0) {
      ...
      if (!result) {
        result = await reapStaleSupportClawOwnedAcpxOrphans({
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: SupportClawPluginServiceContext): Promise<void> {
      if (process.env.SUPPORT_CLAW_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (SUPPORT_CLAW_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (startupReap.terminatedPids.length > 0) {
        ctx.logger.info(
          `reaped ${startupReap.terminatedPids.length} stale SupportClaw-owned ACPX process${startupReap.terminatedPids.length === 1 ? "" : "es"}`,
        );
      }
    ...
    async stop(_ctx: SupportClawPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
