...
import { SUPPORT_CLAW_ACPX_LEASE_ID_ARG, SUPPORT_CLAW_GATEWAY_INSTANCE_ID_ARG } from "./process-lease.js";
...
const DOCUMENTED_SUPPORT_CLAW_BRIDGE_COMMAND =
  "env SUPPORT_CLAW_HIDE_BANNER=1 SUPPORT_CLAW_SUPPRESS_NOTES=1 supportClaw acp --url ws://127.0.0.1:18789 --token-file ~/.supportClaw/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @zed-industries/codex-acp@0.13.0";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/supportClaw/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${SUPPORT_CLAW_ACPX_LEASE_ID_ARG} lease-close ${SUPPORT_CLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "supportClaw" ? "supportClaw acp" : agentName),
        list: () => ["codex", "supportClaw"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("normalizes SupportClaw Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("strips the SupportClaw Anthropic provider prefix for Claude ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("keeps Claude ACP model ids intact after stripping the SupportClaw provider prefix", () => {
  ...
  it("adds Codex wrapper stderr tail to generic session initialization failures", async () => {
    const wrapperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-acpx-runtime-"));
    ...
    const { runtime, delegate } = makeRuntime(baseStore, {
      supportClawGatewayInstanceId: "gateway-test",
      supportClawProcessLeaseStore: leaseStore.store,
      supportClawWrapperRoot: wrapperRoot,
      agentRegistry: {
  ...
  it("adds Codex wrapper stderr tail to generic terminal turn error events", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        supportClawLeaseId: "lease-turn-event",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail to generic startTurn failure results", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        supportClawLeaseId: "lease-start-turn",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail when startTurn creation throws", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        supportClawLeaseId: "lease-start-turn-create",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("disables delegate prompt timeout for SupportClaw-managed turns", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(testing.isCodexAcpCommand("supportClaw acp")).toBe(false);
  });
  ...
  it("recognizes claude-agent-acp commands", () => {
    ...
    expect(
      testing.isClaudeAcpCommand(`node "/tmp/supportClaw/acpx/claude-agent-acp-wrapper.mjs"`),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
        `node.exe "C:/Users/runner/AppData/Local/Temp/supportClaw/acpx/claude-agent-acp-wrapper.mjs"`,
      ),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
    ...
    expect(testing.isClaudeAcpCommand("supportClaw acp")).toBe(false);
  ...
  it("cleans up SupportClaw-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/supportClaw/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate } = makeRuntime(
      baseStore,
      {
        supportClawWrapperRoot: "/tmp/supportClaw/acpx",
      },
      {
        supportClawProcessCleanup: {
          listProcesses: vi.fn(async () => [
            {
              pid: 900,
              ppid: 1,
              command: 'node "/tmp/supportClaw/acpx/codex-acp-wrapper.mjs"',
            },
            {
              pid: 901,
              ppid: 900,
              command:
                "node /tmp/supportClaw/plugin-runtime-deps/node_modules/@zed-industries/codex-acp/bin/codex-acp.js",
            },
          ]),
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    expect(lease?.wrapperPath).toBe("/tmp/supportClaw/acpx/codex-acp-wrapper.mjs");
    expect(launchCommands[0]).toContain("SUPPORT_CLAW_ACPX_LEASE_ID=");
    expect(launchCommands[0]).toContain("SUPPORT_CLAW_GATEWAY_INSTANCE_ID=gateway-test");
    expect(savedRecords[0]?.agentCommand).toBe(CODEX_ACP_WRAPPER_COMMAND);
    expect(savedRecords[0]?.supportClawGatewayInstanceId).toBe("gateway-test");
    expect(savedRecords[0]?.supportClawLeaseId).toBe(lease?.leaseId);
  });
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    leaseStore.leases.set("lease-loaded", {
      ...
      sessionKey: "agent:codex:acp:binding:test",
      wrapperRoot: "/tmp/supportClaw/acpx",
      wrapperPath: "/tmp/supportClaw/acpx/codex-acp-wrapper.mjs",
      rootPid: 777,
      commandHash: "hash",
      startedAt: 1,
      state: "open",
    ...
    expect(loadedRecord?.supportClawGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.supportClawLeaseId).toBe("lease-loaded");
  });
  ...
  it("merges the lease for the current ACPX session process when old leases exist", async () => {
    ...
    const loadedRecord = await wrappedStore.load("agent:codex:acp:binding:test");
    expect(loadedRecord?.supportClawGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.supportClawLeaseId).toBe("lease-current");
  });
  ...
  it("uses matching leases before legacy pid cleanup on close", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/supportClaw/acpx/codex-acp-wrapper.mjs"',
        supportClawLeaseId: "lease-close",
        pid: 930,
      })),
      save: vi.fn(async () => {}),
    };
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${SUPPORT_CLAW_ACPX_LEASE_ID_ARG} lease-old ${SUPPORT_CLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${SUPPORT_CLAW_ACPX_LEASE_ID_ARG} lease-current ${SUPPORT_CLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
  ...
  it("uses session lease metadata for fallback close cleanup identity checks", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        ...
        agentCommand: 'node "/tmp/supportClaw/acpx/codex-acp-wrapper.mjs"',
        supportClawGatewayInstanceId: "gateway-test",
        supportClawLeaseId: "lease-record",
        pid: 920,
      })),
      save: vi.fn(async () => {}),
    };
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 920,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${SUPPORT_CLAW_ACPX_LEASE_ID_ARG} other-lease ${SUPPORT_CLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
          ]),
          killProcess: vi.fn((pid, signal) => {
            killed.push({ pid, signal });
  ...
  it("routes supportClaw ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:supportClaw:acp:test",
      backend: "acpx",
  ...
  it("routes non-supportClaw sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for supportClaw sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:supportClaw:acp:test",
      backend: "acpx",
      runtimeSessionName: "supportClaw-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the supportClaw agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "supportClaw" ? "codex" : agentName),
        list: () => ["codex", "supportClaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the supportClaw bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "supportClaw acp" : agentName),
        list: () => ["codex", "supportClaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped supportClaw bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "supportClaw" ? DOCUMENTED_SUPPORT_CLAW_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "supportClaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node supportClaw entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "supportClaw" ? "env SUPPORT_CLAW_HIDE_BANNER=1 node supportClawCli.mjs acp" : agentName,
        list: () => ["codex", "supportClaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:supportClaw:acp:test",
        agentCommand: DOCUMENTED_SUPPORT_CLAW_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:supportClaw:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:supportClaw:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to supportClaw bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
