 ...
 * SupportClaw-owned wrapper trees after validating paths, packages, and lease ids.
 */
...
const SUPPORT_CLAW_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
export type AcpxProcessCleanupResult = {
  ...
  skippedReason?: "missing-root" | "not-supportClaw-owned" | "unverified-root";
};
...
function resolveSupportClawInstallRoot(pluginRoot: string): string {
  if (
...
function resolveOwnedAcpPackageRootCandidates(packageName: string): string[] {
  ...
  const supportClawRoot = resolveSupportClawInstallRoot(pluginRoot);
  return [
    resolvePackageRoot(packageName),
    path.join(pluginRoot, "node_modules", packageName),
    path.join(supportClawRoot, "node_modules", packageName),
...
/** Check whether a command references an SupportClaw-generated ACPX wrapper path. */
export function isSupportClawLeaseAwareAcpxProcessCommand(params: {
...
function liveCommandMatchesLeaseIdentity(params: {
  ...
  return (
    commandOptionEquals(parts, SUPPORT_CLAW_ACPX_LEASE_ID_ARG, params.expectedLeaseId) &&
    commandOptionEquals(parts, SUPPORT_CLAW_GATEWAY_INSTANCE_ID_ARG, params.expectedGatewayInstanceId)
  );
}

/** Check whether a command is owned by SupportClaw ACPX runtime packages or wrappers. */
export function isSupportClawOwnedAcpxProcessCommand(params: {
  command: string | undefined;
  wrapperRoot?: string;
}): boolean {
  const command = params.command?.trim();
  ...
  if (!normalized.includes(SUPPORT_CLAW_PLUGIN_DEPS_MARKER)) {
    return false;
  }
...
/** Terminate one validated SupportClaw-owned ACPX wrapper process tree. */
export async function cleanupSupportClawOwnedAcpxProcessTree(params: {
  rootPid?: number;
  rootCommand?: string;
  expectedLeaseId?: string;
  expectedGatewayInstanceId?: string;
  ...
  // that this PID still belongs to an SupportClaw-owned wrapper, fail closed to
...
/** Reap orphaned SupportClaw-owned ACPX wrapper trees during runtime startup. */
export async function reapStaleSupportClawOwnedAcpxOrphans(params: {
  wrapperRoot: string;
  deps?: AcpxProcessCleanupDeps;
