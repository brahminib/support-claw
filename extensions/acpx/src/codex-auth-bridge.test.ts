...
const previousEnv = {
  ...
  SUPPORT_CLAW_AGENT_DIR: process.env.SUPPORT_CLAW_AGENT_DIR,
};

async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-acpx-codex-auth-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  restoreEnv("SUPPORT_CLAW_AGENT_DIR");
...
describe("prepareAcpxCodexAuthConfig", () => {
  it("installs an isolated Codex ACP wrapper without synthesizing auth from canonical SupportClaw OAuth", async () => {
  ...
  it("launches the locally installed Codex ACP bin with isolated CODEX_HOME", async () => {
    ...
    const { stdout } = await execFileAsync(
      ...
      [
        ...
        "--supportClaw-acpx-lease-id",
        "lease-1",
        "--supportClaw-gateway-instance-id",
        "gateway-1",
      ],
      {
        cwd: root,
