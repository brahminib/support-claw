...
import { readJsonFileWithFallback } from "supportClaw/plugin-sdk/json-store";
import {
...
import {
  SUPPORT_CLAW_ACPX_LEASE_ID_ARG,
  SUPPORT_CLAW_ACPX_LEASE_ID_ENV,
  SUPPORT_CLAW_GATEWAY_INSTANCE_ID_ARG,
...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--supportClaw-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @supportclaw/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep SupportClaw's isolated CODEX_HOME wrapper, but launch the plugin-local
...
const supportClawWrapperArgs = new Set([
  ${quoteCommandPart(SUPPORT_CLAW_ACPX_LEASE_ID_ARG)},
  ${quoteCommandPart(SUPPORT_CLAW_GATEWAY_INSTANCE_ID_ARG)},
]);

function readSupportClawWrapperArg(args, name) {
...
function resolveStderrLogPath(args) {
  ...
  const leaseId =
    process.env[${JSON.stringify(SUPPORT_CLAW_ACPX_LEASE_ID_ENV)}] ||
    readSupportClawWrapperArg(args, ${quoteCommandPart(SUPPORT_CLAW_ACPX_LEASE_ID_ARG)}) ||
    "pid-" + process.pid;
...
function stripSupportClawWrapperArgs(args) {
  const stripped = [];
  for (let index = 0; index < args.length; index += 1) {
    const value = args[index];
    if (supportClawWrapperArgs.has(value)) {
      index += 1;
      continue;
    }
    stripped.push(value);
...
const configuredArgs = stripSupportClawWrapperArgs(rawConfiguredArgs);
...
if (!command) {
  console.error("[supportClaw] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[supportClaw] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in SupportClaw; fallback must not float to an unpatched newer release.
