...
import { formatPluginConfigIssue } from "supportClaw/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "supportClaw-plugin-tools";
const ACPX_SUPPORT_CLAW_TOOLS_MCP_SERVER_NAME = "supportClaw-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "supportClaw.plugin.json")) &&
...
function resolveAcpxPluginRootFromSupportClawLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common SupportClaw layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromSupportClawLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveSupportClawRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const supportClawRoot = resolveSupportClawRoot(pluginRoot);
  const distEntry = path.join(supportClawRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(supportClawRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
function resolveSupportClawToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const supportClawRoot = resolveSupportClawRoot(pluginRoot);
  const distEntry = path.join(supportClawRoot, "dist", "mcp", "supportClaw-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(supportClawRoot, "src", "mcp", "supportClaw-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  supportClawToolsMcpBridge: boolean;
  moduleUrl?: string;
  ...
  if (params.supportClawToolsMcpBridge && resolved[ACPX_SUPPORT_CLAW_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_SUPPORT_CLAW_TOOLS_MCP_SERVER_NAME} is reserved when supportClawToolsMcpBridge=true`,
    );
  }
  ...
  if (params.supportClawToolsMcpBridge) {
    resolved[ACPX_SUPPORT_CLAW_TOOLS_MCP_SERVER_NAME] = resolveSupportClawToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
/** Convert SupportClaw MCP server config into ACPX runtime MCP server entries. */
...
export function resolveAcpxPluginConfig(params: {
  ...
  const supportClawToolsMcpBridge = normalized.supportClawToolsMcpBridge === true;
  const mcpServers = resolveConfiguredMcpServers({
    mcpServers: normalized.mcpServers,
    pluginToolsMcpBridge,
    supportClawToolsMcpBridge,
