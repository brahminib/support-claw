...
const { cleanupSupportClawOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupSupportClawOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleSupportClawOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleSupportClawOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupSupportClawOwnedAcpxProcessTree: cleanupSupportClawOwnedAcpxProcessTreeMock,
  reapStaleSupportClawOwnedAcpxOrphans: reapStaleSupportClawOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { SupportClawPluginServiceContext } from "../runtime-api.js";
...
const previousEnv = {
  SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE: process.env.SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE,
  SUPPORT_CLAW_SKIP_ACPX_RUNTIME: process.env.SUPPORT_CLAW_SKIP_ACPX_RUNTIME,
  SUPPORT_CLAW_SKIP_ACPX_RUNTIME_PROBE: process.env.SUPPORT_CLAW_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  cleanupSupportClawOwnedAcpxProcessTreeMock.mockClear();
  reapStaleSupportClawOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("SUPPORT_CLAW_SKIP_ACPX_RUNTIME");
  restoreEnv("SUPPORT_CLAW_SKIP_ACPX_RUNTIME_PROBE");
  for (const dir of tempDirs.splice(0)) {
    await fs.rm(dir, { recursive: true, force: true });
  }
});

function createServiceContext(workspaceDir: string): SupportClawPluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".supportClaw-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
function createOpenKeyedStore(ctx: SupportClawPluginServiceContext) {
  const env = { ...process.env, SUPPORT_CLAW_STATE_DIR: ctx.stateDir };
...
function createAcpxRuntimeService(
  ctx: SupportClawPluginServiceContext,
...
function openGatewayInstanceStore(ctx: SupportClawPluginServiceContext) {
...
function openProcessLeaseStore(ctx: SupportClawPluginServiceContext) {
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.SUPPORT_CLAW_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupSupportClawOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupSupportClawOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale SupportClaw-owned ACPX processes");
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    await openProcessLeaseStore(ctx).register(lease.leaseId, lease);
    reapStaleSupportClawOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupSupportClawOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleSupportClawOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale SupportClaw-owned ACPX processes");
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.SUPPORT_CLAW_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.SUPPORT_CLAW_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (SUPPORT_CLAW_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
