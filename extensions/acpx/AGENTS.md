...
The ACPX extension is a thin SupportClaw wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `supportClaw/acpx`, not in this extension.
...
Use this flow when SupportClaw needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `supportClaw/acpx` repo first.
2. In SupportClaw, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild SupportClaw and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for SupportClaw ACP validation.
...
If a change feels like shared ACP runtime behavior instead of SupportClaw-specific glue, move it to `supportClaw/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
