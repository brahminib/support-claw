...
import type { RealtimeVoiceAgentControlResult } from "supportClaw/plugin-sdk/realtime-voice";
  ...
  return {
    ...
    controlRealtimeVoiceAgentRunMock: vi.fn<() => Promise<RealtimeVoiceAgentControlResult>>(
      async () => ({
        ...
        message: "There is no active SupportClaw run to steer.",
        speak: true,
        show: true,
        suppress: false,
      }),
...
vi.mock("supportClaw/plugin-sdk/routing", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/routing")>(
    "supportClaw/plugin-sdk/routing",
  );
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/agent-runtime")>(
    "supportClaw/plugin-sdk/agent-runtime",
  );
  return {
    ...actual,
    agentCommandFromIngress: agentCommandMock,
    getTtsProvider: vi.fn(() => "openai"),
    resolveAgentDir: vi.fn(() => "/tmp/supportClaw-agent"),
    resolveTtsConfig: vi.fn(() => ({
      modelOverrides: {},
      providerConfigs: {},
    })),
    resolveTtsPrefsPath: vi.fn(() => "/tmp/supportClaw-tts.json"),
  };
});

vi.mock("supportClaw/plugin-sdk/realtime-bootstrap-context", async () => {
  const actual = await vi.importActual<
    typeof import("supportClaw/plugin-sdk/realtime-bootstrap-context")
  >("supportClaw/plugin-sdk/realtime-bootstrap-context");
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/runtime-env")>(
    "supportClaw/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/realtime-voice", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/realtime-voice")>(
    "supportClaw/plugin-sdk/realtime-voice",
  );
  return {
    ...actual,
...
describe("DiscordVoiceManager", () => {
  ...
  it("uses agent-proxy realtime voice by default", async () => {
    ...
    expect(bridgeParams?.instructions).toContain("same SupportClaw agent");
    expect(bridgeParams?.instructions).toContain("short natural backchannel");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("supportClaw_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("supportClaw_agent_control");
    const player = getLastAudioPlayer();
    bridgeParams?.audioSink?.sendAudio(Buffer.alloc(24_000));
    ...
    bridgeParams?.onToolCall?.(
      {
        itemId: "item-1",
        callId: "call-1",
        name: "supportClaw_agent_consult",
    ...
    bridgeParams?.onToolCall?.(
      {
        itemId: "item-control",
        callId: "call-control",
        name: "supportClaw_agent_control",
    ...
    bridgeParams?.onToolCall?.(
      {
        ...
        args: {
          question: [
            "Speak this exact SupportClaw answer to the Discord voice channel, without adding, removing, or rephrasing words.",
  ...
  it("routes active-run realtime transcripts to voice control before forced consults", async () => {
    controlRealtimeVoiceAgentRunMock.mockResolvedValueOnce({
      ...
      active: true,
      aborted: true,
      message: "Cancelled the active SupportClaw run.",
      speak: true,
      show: true,
      suppress: false,
    });
    ...
    await vi.waitFor(() => expectUserMessageIncludes("Cancelled the active SupportClaw run."));
    expect(textToSpeechMock).not.toHaveBeenCalledWith(
      expect.objectContaining({ text: "Cancelled the active SupportClaw run." }),
    );

    const stopCallsAfterControl = player.stop.mock.calls.length;
    bridgeParams?.onTranscript?.("assistant", "Cancelled the active SupportClaw run.", true);
  ...
  it("accepts SupportClaw as a default wake name before realtime agent-proxy consults", async () => {
    agentCommandMock.mockResolvedValueOnce({ payloads: [{ text: "supportClaw wake answer" }] });
    const manager = createManager(
      {
        groupPolicy: "open",
        voice: {
    ...
    ownerTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "SupportClaw, how is it going");
    ...
    expect(lastAgentCommandArgs().message).toContain("how is it going");
    expect(lastAgentCommandArgs().message).not.toContain("SupportClaw");
    expectUserMessageIncludes("supportClaw wake answer");
  });
  ...
  it("ignores default agent wake names longer than two words", async () => {
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "SupportClaw, fallback still wakes");
  ...
  it("accepts leading fuzzy wake names before realtime agent-proxy consults", async () => {
    ...
    const supportClawTurn = entry.realtime?.beginSpeakerTurn(
      { extraSystemPrompt: undefined, senderIsOwner: true, speakerLabel: "Owner" },
      "u-owner",
    );
    supportClawTurn?.sendInputAudio(Buffer.alloc(8));
  ...
  it("does not accept configured realtime wake names longer than two words", async () => {
    ...
    fallbackTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "SupportClaw, ship it");
  ...
  it("keeps forced agent-proxy fallback diagnostics out of agent prompts", async () => {
    ...
    expect(lastAgentCommandArgs().message).not.toContain("supportClaw_agent_consult");
  ...
  it("reuses forced agent-proxy answers for late matching consult tool calls", async () => {
    ...
    expect(realtimeSessionMock.submitToolResult).toHaveBeenCalledWith(
      "call-late",
      {
        ...
        message: "SupportClaw already delivered this answer to Discord voice.",
      },
  ...
  it("starts Discord realtime voice in bidi mode with the consult tool", async () => {
    ...
    expect(bridgeParams?.autoRespondToAudio).toBe(true);
    expect(bridgeParams?.interruptResponseOnInputAudio).toBe(false);
    expect(bridgeParams?.instructions).toContain("Call supportClaw_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("supportClaw_agent_consult");
  ...
  it("adds default bootstrap profile context to realtime voice instructions", async () => {
    ...
    resolveRealtimeBootstrapContextInstructionsMock.mockResolvedValue(
      "SupportClaw realtime voice profile context:\n\n### IDENTITY.md\nName: Wilfred",
    );
    const manager = createManager({
      groupPolicy: "open",
      voice: {
    ...
    const bridgeParams = lastRealtimeBridgeParams() as
      ...
      | undefined;
    expect(bridgeParams?.instructions).toContain("SupportClaw realtime voice profile context");
