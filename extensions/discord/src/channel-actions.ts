...
import { createUnionActionGate } from "supportClaw/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "supportClaw/plugin-sdk/channel-contract";
import type { DiscordActionConfig, SupportClawConfig } from "supportClaw/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: SupportClawConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: SupportClawConfig) {
