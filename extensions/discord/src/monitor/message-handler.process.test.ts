...
import { DEFAULT_EMOJIS, DEFAULT_TIMING } from "supportClaw/plugin-sdk/channel-feedback";
import {
  recordChannelBotPairLoopAndCheckSuppression,
  type ChannelBotLoopProtectionFacts,
} from "supportClaw/plugin-sdk/channel-inbound";
import type { ReplyPayload } from "supportClaw/plugin-sdk/reply-dispatch-runtime";
import { setReplyPayloadMetadata } from "supportClaw/plugin-sdk/reply-payload-testing";
import * as runtimeEnvModule from "supportClaw/plugin-sdk/runtime-env";
...
function createNonTerminalToolWarningPayload(): ReplyPayload {
  return setReplyPayloadMetadata(
    {
      text: "⚠️ 🛠️ `run supportClaw definitely-not-a-real-subcommand (agent)` failed",
      isError: true,
    },
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveAndPersistSessionFile: vi.fn<(params?: unknown) => Promise<{ sessionFile: string }>>(
    async () => ({ sessionFile: "/tmp/supportClaw-discord-process-test-session.jsonl" }),
  ),
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/supportClaw-discord-process-test-sessions.json",
  ),
}));
...
vi.mock("supportClaw/plugin-sdk/reply-runtime", () => ({
...
vi.mock("supportClaw/plugin-sdk/conversation-runtime", () => ({
...
vi.mock("supportClaw/plugin-sdk/session-store-runtime", () => ({
...
beforeEach(() => {
  ...
  resolveAndPersistSessionFile.mockResolvedValue({
    sessionFile: "/tmp/supportClaw-discord-process-test-session.jsonl",
  });
  resolveSessionStoreEntry.mockImplementation((params) => ({
    existing: params.sessionKey ? params.store[params.sessionKey] : undefined,
  }));
  resolveStorePath.mockReturnValue("/tmp/supportClaw-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createAutomaticSourceDeliveryContext({
      cfg: {
        ...
        session: { store: "/tmp/supportClaw-discord-process-test-sessions.json" },
      },
    });
  ...
  it("pins Discord text DM main-route updates to the single configured DM owner", async () => {
    const ctx = await createBaseContext({
      ...
      cfg: {
        messages: { ackReaction: "👀" },
        session: {
          store: "/tmp/supportClaw-discord-process-test-sessions.json",
          dmScope: "main",
        },
      },
