...
describe("Discord gateway metadata", () => {
  it("resolves gateway info timeouts from strict integer config and env values", () => {
    ...
    expect(
      resolveDiscordGatewayInfoTimeoutMs({
        env: { SUPPORT_CLAW_DISCORD_GATEWAY_INFO_TIMEOUT_MS: "90000" },
      }),
    ).toBe(90_000);
    ...
    expect(
      resolveDiscordGatewayInfoTimeoutMs({
        ...
        env: { SUPPORT_CLAW_DISCORD_GATEWAY_INFO_TIMEOUT_MS: "0x1000" },
      }),
    ).toBe(30_000);
    expect(
      resolveDiscordGatewayInfoTimeoutMs({
        env: { SUPPORT_CLAW_DISCORD_GATEWAY_INFO_TIMEOUT_MS: "1e3" },
      }),
    ).toBe(30_000);
  });
