...
import { createPluginRuntimeMock } from "supportClaw/plugin-sdk/channel-test-helpers";
...
let runtimeEnvModule: typeof import("supportClaw/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): SupportClawConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("supportClaw/plugin-sdk/plugin-runtime", async () => {
    ...
    runtimeEnvModule = await import("supportClaw/plugin-sdk/runtime-env");
  ...
  it("passes SupportClaw event queue defaults to the Discord client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
