...
import * as commandRegistryModule from "supportClaw/plugin-sdk/command-auth-native";
import type {
  ChatCommandDefinition,
  CommandArgsParsing,
} from "supportClaw/plugin-sdk/command-auth-native";
import type { ModelsProviderData } from "supportClaw/plugin-sdk/command-auth-native";
import type { SupportClawConfig } from "supportClaw/plugin-sdk/config-contracts";
import * as globalsModule from "supportClaw/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  upsertSessionEntry,
} from "supportClaw/plugin-sdk/session-store-runtime";
import * as commandTextModule from "supportClaw/plugin-sdk/text-utility-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "supportClaw-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          ...
          { id: "supportClaw", label: "SupportClaw Default", description: "Use SupportClaw." },
        ],
      ],
    ]);
    const modelCommand = createModelCommandDefinition();
