...
vi.mock("supportClaw/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("supportClaw/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<SupportClawConfig["channels"]>["discord"]>["execApprovals"],
): SupportClawConfig {
  return {
    channels: {
      discord: {
