...
import type { NativeCommandSpec } from "supportClaw/plugin-sdk/command-auth-native";
import { resolveDirectStatusReplyForSession } from "supportClaw/plugin-sdk/command-status-runtime";
...
import { dispatchReplyWithDispatcher } from "supportClaw/plugin-sdk/reply-dispatch-runtime";
import {
  clearRuntimeConfigSnapshot,
  setRuntimeConfigSnapshot,
} from "supportClaw/plugin-sdk/runtime-config-snapshot";
import { getSessionEntry } from "supportClaw/plugin-sdk/session-store-runtime";
...
function createConfig(): SupportClawConfig {
  return {
    channels: {
      discord: {
...
async function createNativeCommand(cfg: SupportClawConfig, commandSpec: NativeCommandSpec) {
...
async function createPluginCommand(params: { cfg: SupportClawConfig; name: string }) {
  return createDiscordNativeCommand({
    command: {
      name: params.name,
      description: "Pair",
...
async function createStatusCommand(cfg: SupportClawConfig) {
...
describe("Discord native plugin command dispatch", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setMatchPluginCommand(
      runtimeModuleMocks.matchPluginCommand as typeof import("supportClaw/plugin-sdk/plugin-runtime").matchPluginCommand,
    );
    discordNativeCommandTesting.setExecutePluginCommand(
      runtimeModuleMocks.executePluginCommand as typeof import("supportClaw/plugin-sdk/plugin-runtime").executePluginCommand,
    );
    ...
    discordNativeCommandTesting.setGetSessionEntry(
      runtimeModuleMocks.getSessionEntry as typeof import("supportClaw/plugin-sdk/session-store-runtime").getSessionEntry,
    );
  });
  ...
  it("refreshes native command routing config between invocations", async () => {
    ...
    const resolveRouteState = vi.fn(async (params: { cfg: SupportClawConfig }) =>
      createUnboundRouteState({
        sessionKey:
  ...
  it("passes the configured binding agent to plugin-owned Discord command sessions", async () => {
    const cfg = createConfig();
    const interaction = createInteraction();
    const pluginSessionKey = "plugin-binding:supportClaw-codex-app-server:dm";
