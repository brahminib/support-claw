...
describe("maybeCreateDiscordAutoThread autoThreadName", () => {
  it("renames created thread when generated mode is enabled", async () => {
    ...
    const cfg = { agents: { defaults: { model: "anthropic/claude-opus-4-6" } } } as SupportClawConfig;
    const result = await maybeCreateDiscordAutoThread(
      createBaseParams({
        baseText: "Need help with deploy rollout",
        combinedBody: "Need help with deploy rollout",
        channelName: "supportClaw",
        channelDescription: "SupportClaw development coordination and release planning",
    ...
    expectGeneratedTitleField("channelName", "supportClaw");
    expectGeneratedTitleField(
      "channelDescription",
      "SupportClaw development coordination and release planning",
    );
