...
import { sleepWithAbort } from "supportClaw/plugin-sdk/runtime-env";
import {
...
import { resolveTargetsWithOptionalToken } from "supportClaw/plugin-sdk/target-resolver-runtime";
import {
...
function resolveDiscordStartupDelayMs(cfg: SupportClawConfig, accountId: string): number {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). SupportClaw adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
      ...
      status: createComputedAccountStatusAdapter<ResolvedDiscordAccount, DiscordProbe>({
        ...
        buildCapabilitiesDiagnostics: async ({ account, target, timeoutMs }) => {
          ...
          const statusCfg: SupportClawConfig = {
            channels: {
              discord: {
                accounts: {
                  [account.accountId]: {
