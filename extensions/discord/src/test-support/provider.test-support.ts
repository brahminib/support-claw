...
import type { RuntimeEnv } from "supportClaw/plugin-sdk/runtime-env";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: SupportClawConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): SupportClawConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("supportClaw/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/acp-runtime")>(
    "supportClaw/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/reply-runtime")>(
...
vi.mock("supportClaw/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("supportClaw/plugin-sdk/native-command-config-runtime")
  >("supportClaw/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/runtime-config-snapshot", async () => {
  const actual = await vi.importActual<
    typeof import("supportClaw/plugin-sdk/runtime-config-snapshot")
  >("supportClaw/plugin-sdk/runtime-config-snapshot");
  return {
    ...actual,
    getRuntimeConfig: () => ({}),
  };
...
vi.mock("supportClaw/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/error-runtime")>(
    "supportClaw/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
