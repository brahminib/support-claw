...
function fakeApi(): SupportClawPluginApi {
  return {
    config: {},
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.supportClaw.ai", "", "supportClaw.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.supportClaw.ai", "supportClaw.ai"],
  ...
  it("late-binds dedicated tools to the resolved runtime config snapshot", async () => {
    ...
    const registeredTools: Array<Parameters<SupportClawPluginApi["registerTool"]>[0]> = [];
    const registeredOptions: Array<Parameters<SupportClawPluginApi["registerTool"]>[1]> = [];
    ...
    await searchTool.execute("search-call", { query: "supportClaw" });
    ...
    expect(searchParams.query).toBe("supportClaw");
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as SupportClawConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
