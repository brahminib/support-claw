...
import { resolvePositiveTimeoutSeconds } from "supportClaw/plugin-sdk/provider-web-search";
import {
...
export function resolveTavilySearchConfig(cfg?: SupportClawConfig): TavilySearchConfig {
...
export function resolveTavilyApiKey(cfg?: SupportClawConfig): string | undefined {
...
export function resolveTavilyBaseUrl(cfg?: SupportClawConfig): string {
  const search = resolveTavilySearchConfig(cfg);
  const configured =
