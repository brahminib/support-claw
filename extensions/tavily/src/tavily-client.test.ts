...
vi.mock("supportClaw/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: supportClaw", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "supportClaw" });
  });
  ...
  it("runTavilyExtract sends X-Client-Source: supportClaw", async () => {
