...
export async function startRaftGatewayAccount(
  ...
  if (!ctx.channelRuntime) {
    throw new Error("Raft requires SupportClaw channel runtime support. Update SupportClaw and retry.");
  }
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (
        ...
        // Raft drains runtime activity after each wake pass. SupportClaw has no
      ...
      // Raft owns wake metadata and its schema evolution. SupportClaw accepts only
