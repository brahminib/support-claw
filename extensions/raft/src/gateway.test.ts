...
import type { ChannelGatewayContext } from "supportClaw/plugin-sdk/channel-contract";
import { createClaimableDedupe } from "supportClaw/plugin-sdk/persistent-dedupe";
...
function makeTempDir(prefix: string): string {
  // supportClaw-temp-dir: allow extension tests cannot import root test helpers
...
function createContext(accountId = "default") {
  ...
  const ctx = {
    ...
    channelRuntime: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/supportClaw-agent.sqlite"),
...
describe("Raft wake gateway", () => {
  ...
  it("persists accepted wake dedupe across restarts without crossing accounts", async () => {
    const stateDir = makeTempDir("supportClaw-raft-wake-dedupe-");
    try {
