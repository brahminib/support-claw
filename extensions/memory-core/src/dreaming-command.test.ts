...
import type { PluginCommandContext } from "supportClaw/plugin-sdk/core";
...
function resolveStoredDreaming(config: SupportClawConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: SupportClawConfig = {}) {
  let runtimeConfig: SupportClawConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: SupportClawConfig) => void }) => {
      ...
      writeConfigFile: vi.fn(async (nextConfig: SupportClawConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
