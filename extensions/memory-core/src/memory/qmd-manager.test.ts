...
import { withMockedWindowsPlatform } from "supportClaw/plugin-sdk/test-node-mocks";
...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("supportClaw.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("supportClaw.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("supportClaw.qmdEmbedQueueTail");
...
vi.mock("supportClaw/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("supportClaw/plugin-sdk/memory-core-host-engine-foundation")
  >("supportClaw/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/file-lock")>(
    "supportClaw/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { SupportClawConfig } from "supportClaw/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
  resolveMemoryBackendConfig,
} from "supportClaw/plugin-sdk/memory-core-host-engine-storage";
import { MAX_TIMER_TIMEOUT_MS } from "supportClaw/plugin-sdk/number-runtime";
import { formatSessionTranscriptMemoryHitKey } from "supportClaw/plugin-sdk/session-transcript-hit";
...
describe("QmdMemoryManager", () => {
  ...
  it("rebinds a stale in-container collection root to the host workspace (sandbox-mode transition)", async () => {
    ...
    // /home/node/.supportClaw/... stored that in-container path as the collection root. Resolved
    ...
    const containerRoot = "/home/node/.supportClaw/teams/x/workspace";
  ...
  it("parseShownCollection extracts path and pattern from qmd collection show output", async () => {
    ...
    const sampleOutput = [
      ...
      "  Path:     /home/node/.supportClaw/teams/example-team/workspace-example/memory",
      "  Pattern:  **/*.md",
      "  Include:  yes (default)",
    ].join("\n");

    const result = parser(sampleOutput);
    expect(result.path).toBe("/home/node/.supportClaw/teams/example-team/workspace-example/memory");
