...
import { clearMemoryEmbeddingProviders as clearRegistry } from "supportClaw/plugin-sdk/memory-core-host-engine-embeddings";
import { hashText } from "supportClaw/plugin-sdk/memory-core-host-engine-storage";
import { resolveSessionTranscriptsDirForAgent } from "supportClaw/plugin-sdk/memory-core-host-runtime-core";
import { resolveSupportClawAgentSqlitePath } from "supportClaw/plugin-sdk/sqlite-runtime";
import {
  closeSupportClawAgentDatabasesForTest,
  closeSupportClawStateDatabaseForTest,
  openSupportClawAgentDatabase,
} from "supportClaw/plugin-sdk/sqlite-runtime-testing";
...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-mem-fixtures-"));
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("SUPPORT_CLAW_STATE_DIR", path.join(workspaceDir, ".state-memory-index"));
    await fs.writeFile(
  ...
  async function getFtsSessionManager(params: {
    ...
    vi.stubEnv("SUPPORT_CLAW_STATE_DIR", path.join(workspaceDir, params.stateDirName));
    const cfg = createCfg({
  ...
  it("reindexes memory tables in place without deleting unrelated agent rows", async () => {
    ...
    const agentDbPath = resolveSupportClawAgentSqlitePath({ agentId: "main" });
    const agentDb = openSupportClawAgentDatabase({ agentId: "main" });
    agentDb.db
    ...
    const reopened = openSupportClawAgentDatabase({ agentId: "main" });
    expect(
      reopened.db
  ...
  it("clears dirty after sessions-only identity reindex", async () => {
    try {
      vi.stubEnv("SUPPORT_CLAW_STATE_DIR", path.join(workspaceDir, ".state-sessions-only-reindex"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
      await fs.writeFile(
  ...
  it("marks sessions-only indexes dirty when metadata is missing but chunks exist", async () => {
    try {
      vi.stubEnv("SUPPORT_CLAW_STATE_DIR", path.join(workspaceDir, ".state-sessions-missing-meta"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
      await fs.writeFile(
  ...
  it("keeps provider cutover vector search paused during targeted session sync", async () => {
    try {
      vi.stubEnv("SUPPORT_CLAW_STATE_DIR", path.join(workspaceDir, ".state-targeted-cutover"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("preserves memory dirty events raised during session identity reindex", async () => {
    try {
      vi.stubEnv("SUPPORT_CLAW_STATE_DIR", path.join(workspaceDir, ".state-dirty-during-session"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
      await fs.writeFile(
