...
describe("memory manager reindex recovery", () => {
  ...
  beforeEach(async () => {
    resetEmbeddingMocks();
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-mem-reindex-recovery-"));
    workspaceDir = path.join(fixtureRoot, "workspace");
    memoryDir = path.join(workspaceDir, "memory");
    await fs.mkdir(memoryDir, { recursive: true });
    vi.stubEnv("SUPPORT_CLAW_STATE_DIR", path.join(fixtureRoot, "state"));
  });
  ...
  async function openManager(cfg: SupportClawConfig): Promise<MemoryIndexManager> {
  ...
  it("rejects a full reindex while another process owns the build lock", async () => {
    ...
    const harness = memoryManager as unknown as ReindexHarness;
    const databasePath = resolveSupportClawAgentSqlitePath({ agentId: "main" });
