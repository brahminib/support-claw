...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "supportClaw/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("supportClaw/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): SupportClawConfig {
  return {
    agents: {
      defaults: {
        workspace: "/tmp/workspace",
...
async function expectPendingQmdReplacement(params: {
  agentId: string;
  firstCfg: SupportClawConfig;
  secondCfg: SupportClawConfig;
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("supportClaw.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-qmd-workspace-"));
