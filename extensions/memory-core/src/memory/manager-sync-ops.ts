...
import { classifyMemoryMultimodalPath } from "supportClaw/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
import {
  ...
  sessionPathForFile,
} from "supportClaw/plugin-sdk/memory-core-host-engine-qmd";
import {
  buildFileEntry,
...
const TEST_MEMORY_WATCH_FACTORY_KEY = Symbol.for("supportClaw.test.memoryWatchFactory");
const TEST_MEMORY_NATIVE_WATCH_FACTORY_KEY = Symbol.for("supportClaw.test.memoryNativeWatchFactory");
...
export abstract class MemoryManagerSyncOps {
  protected abstract readonly cfg: SupportClawConfig;
  ...
  private warnIfMemoryWatchPressure(count: number, unit: MemoryWatchPressureUnit): void {
    warnIfMemoryWatchPressureHigh(
      ...
      count,
      unit,
      "Large memory folders or extraPaths can make SupportClaw run out of file watchers or open files.",
