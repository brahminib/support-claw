...
import type {
  MemorySearchConfig,
  SupportClawConfig,
} from "supportClaw/plugin-sdk/memory-core-host-engine-foundation";
...
const {
  ...
  const chokidarKey = Symbol.for("supportClaw.test.memoryWatchFactory");
  const nativeKey = Symbol.for("supportClaw.test.memoryNativeWatchFactory");
...
const CHOKIDAR_FACTORY_KEY = Symbol.for("supportClaw.test.memoryWatchFactory");
const NATIVE_FACTORY_KEY = Symbol.for("supportClaw.test.memoryNativeWatchFactory");

vi.mock("supportClaw/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("supportClaw/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-memory-watch-"));
    vi.stubEnv("SUPPORT_CLAW_STATE_DIR", path.join(workspaceDir, "state"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): SupportClawConfig {
    const defaults: NonNullable<NonNullable<SupportClawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: SupportClawConfig) {
  ...
  it("routes Linux directories through directory-only native watchers", async () => {
    ...
    // internal/fs/recursive_watch. SupportClaw watches directories only so
