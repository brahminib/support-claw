...
} from "supportClaw/plugin-sdk/error-runtime";
import { resolveGlobalMap } from "supportClaw/plugin-sdk/global-singleton";
import * as memoryCoreHostRuntimeCoreModule from "supportClaw/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "supportClaw/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "supportClaw/plugin-sdk/session-store-runtime";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("supportClaw.memoryCore.dreamingNarrative.fileLocks");
const NARRATIVE_SESSION_LOCKS_KEY = Symbol.for(
  "supportClaw.memoryCore.dreamingNarrative.sessionLocks",
);
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("supportClaw:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("supportClaw-dreaming-backfill-");
    ...
    expect(content.match(/supportClaw:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("supportClaw-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- supportClaw:dreaming:diary:start -->");
    expect(content).toContain("<!-- supportClaw:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- supportClaw:dreaming:diary:start -->");
    const end = content.indexOf("<!-- supportClaw:dreaming:diary:end -->");
  ...
  it("skips symlinked DREAMS.md when building recent diary context", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "# Dream Diary",
        "",
        "<!-- supportClaw:dreaming:diary:start -->",
        "---",
        "",
        ...
        symlinkTargetDiary,
        "",
        "<!-- supportClaw:dreaming:diary:end -->",
        "",
      ].join("\n"),
      "utf-8",
    );
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- supportClaw:dreaming:light:start -->\n- Candidate: test\n<!-- supportClaw:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("supportClaw-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("supportClaw-dreaming-narrative-");
    const stateDir = await createTempWorkspace("supportClaw-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("supportClaw-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("supportClaw-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDirs = await Promise.all(
      Array.from({ length: 5 }, () => createTempWorkspace("supportClaw-dreaming-detach-")),
    );
    const logger = createMockLogger();
  ...
  it("serializes detached narratives that reuse a workspace and phase session", async () => {
    ...
    const workspaceDir = await createTempWorkspace("supportClaw-dreaming-detach-");
    const logger = createMockLogger();
