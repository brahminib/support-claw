    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw memory status", "Show index and provider status."],
          [
            "supportClaw memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["supportClaw memory status --deep", "Probe embedding provider readiness."],
          ["supportClaw memory index --force", "Force a full reindex."],
          ['supportClaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'supportClaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `supportClaw memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "supportClaw memory rem-backfill --path ./memory",
          ...
          ["supportClaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.supportClaw.ai/cli/memory")}\n`,
    );
