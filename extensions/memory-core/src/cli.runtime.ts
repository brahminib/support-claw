...
import type { MemoryEmbeddingProbeResult } from "supportClaw/plugin-sdk/memory-core-host-engine-storage";
import {
  resolveMemoryDreamingConfig,
  resolveMemoryRemDreamingConfig,
} from "supportClaw/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "supportClaw/plugin-sdk/routing";
...
function formatMemoryIndexIdentityWarning(
  ...
  return {
    reason,
    fix: `Run: supportClaw memory status --index --agent ${agentId}`,
  };
}
...
function resolveMemoryPluginConfig(cfg: SupportClawConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: SupportClawConfig): string {
...
function resolveAgent(cfg: SupportClawConfig, agent?: string) {
...
function resolveAgentIds(cfg: SupportClawConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: supportClaw memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-rem-backfill-"),
      );
      try {
