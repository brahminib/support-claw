...
import { peekSystemEventEntries } from "supportClaw/plugin-sdk/system-event-runtime";
...
export async function runShortTermDreamingPromotionIfTriggered(params: {
  ...
  config: ShortTermPromotionDreamingConfig;
  logger: Logger;
  subagent?: SupportClawPluginApi["runtime"]["subagent"];
...
export function registerShortTermPromotionDreaming(api: SupportClawPluginApi): void {
  ...
  const resolveCurrentConfig = (): SupportClawConfig =>
    (api.runtime.config?.current?.() ?? api.config) as SupportClawConfig;
  ...
  const reconcileManagedDreamingCron = async (params: {
    ...
    startupConfig?: SupportClawConfig;
