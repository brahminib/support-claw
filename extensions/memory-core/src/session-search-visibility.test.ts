...
import * as sessionTranscriptHit from "supportClaw/plugin-sdk/session-transcript-hit";
...
import { asSupportClawConfig } from "./tools.test-helpers.js";
...
describe("filterMemorySearchHitsBySessionVisibility", () => {
  ...
  async function createQmdArtifactIndex(params: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-qmd-session-artifact-"));
    tempRoots.push(root);
  ...
  it("drops sessions-sourced hits when requester key is missing (fail closed)", async () => {
    const cfg = asSupportClawConfig({ tools: { sessions: { visibility: "all" } } });
