...
import { resolveMemoryDreamingConfig } from "supportClaw/plugin-sdk/memory-core-host-status";
import type { SupportClawPluginApi, PluginCommandContext } from "supportClaw/plugin-sdk/plugin-entry";
import { normalizeLowercaseStringOrEmpty } from "supportClaw/plugin-sdk/string-coerce-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: SupportClawConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: SupportClawConfig, enabled: boolean): SupportClawConfig {
...
function formatStatus(cfg: SupportClawConfig): string {
...
export async function handleDreamingCommand(api: SupportClawPluginApi, ctx: PluginCommandContext) {
