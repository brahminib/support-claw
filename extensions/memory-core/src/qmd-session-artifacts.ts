...
import { requireNodeSqlite } from "supportClaw/plugin-sdk/memory-core-host-engine-storage";
import type { MemorySearchResult } from "supportClaw/plugin-sdk/memory-core-host-runtime-files";

const QMD_SESSION_ARTIFACT_TABLE = "supportClaw_qmd_session_artifacts";

export const QMD_SESSION_ARTIFACT_HIT: unique symbol = Symbol("supportClaw.qmdSessionArtifactHit");
...
function ensureQmdSessionArtifactSchema(db: DatabaseSync): void {
  ...
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_supportClaw_qmd_session_artifacts_docid
     ON ${QMD_SESSION_ARTIFACT_TABLE} (docid)`,
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_supportClaw_qmd_session_artifacts_search_path
