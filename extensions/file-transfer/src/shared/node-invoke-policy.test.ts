...
import type { SupportClawPluginNodeInvokePolicyContext } from "supportClaw/plugin-sdk/plugin-entry";
...
function createCtx(overrides: {
  ...
  approvals?: SupportClawPluginNodeInvokePolicyContext["approvals"];
}) {
  const invokeNode = vi.fn<SupportClawPluginNodeInvokePolicyContext["invokeNode"]>(
    async ({
      params,
    }: Parameters<SupportClawPluginNodeInvokePolicyContext["invokeNode"]>[0] = {}) => ({
      ok: true,
      payload: {
        ok: true,
        path:
...
function requireInvokeParams(
  invokeNode: ReturnType<typeof vi.fn<SupportClawPluginNodeInvokePolicyContext["invokeNode"]>>,
  callIndex: number,
) {
