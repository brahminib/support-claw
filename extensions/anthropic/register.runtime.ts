 ...
 */
import { formatCliCommand, parseDurationMs } from "supportClaw/plugin-sdk/cli-runtime";
import { resolveExpiresAtMsFromDurationMs } from "supportClaw/plugin-sdk/number-runtime";
import type {
...
import {
  ...
  type SupportClawConfig as ProviderAuthConfig,
...
} from "supportClaw/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "supportClaw/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("supportClaw models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("supportClaw models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("supportClaw doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: SupportClawPluginApi): void {
