...
describe("resolveClaudeCliExecutionArgs", () => {
  ...
  it("maps SupportClaw thinking levels to Claude effort args", () => {
    expect(
      resolveClaudeCliExecutionArgs({
        workspaceDir: "/tmp",
        provider: "claude-cli",
    ...
    expect(
      resolveClaudeCliExecutionArgs({
        ...
        baseArgs: [
          ...
          "--output-format",
          "stream-json",
          "--allowedTools=mcp__supportClaw__*",
          "--allowedTools",
          "Read",
          "Grep",
          "--permission-mode",
...
describe("normalizeClaudeBackendConfig", () => {
  ...
  it("derives Claude bypass from SupportClaw YOLO policy and disables it for safer policy", () => {
  ...
  it("derives Claude bypass from per-agent SupportClaw exec policy", () => {
    expect(
      resolveClaudePermissionMode({
        backendId: "claude-cli",
        agentId: "safe-agent",
  ...
  it("passes system prompt on every turn (issue #80374 — systemPromptWhen must be 'always')", () => {
    ...
    // SupportClaw system prompt context on resumed / compacted claude-cli sessions.
    const backend = buildAnthropicCliBackend();
