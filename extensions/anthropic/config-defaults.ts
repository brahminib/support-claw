 ...
 */
import type { SupportClawConfig } from "supportClaw/plugin-sdk/plugin-entry";
import {
  isRecord,
...
function usesClaudeCliModelSelection(config: SupportClawConfig): boolean {
...
function usesSelectedClaudeCliAuthProfile(config: SupportClawConfig): boolean {
  const profiles = config.auth?.profiles ?? {};
...
function collectClaudeCliRuntimeRefsFromConfig(config: SupportClawConfig): string[] {
  const refs = new Set<string>(
...
/** Apply Anthropic and Claude CLI defaults to an SupportClaw config object. */
