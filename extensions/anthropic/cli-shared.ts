 ...
 * thinking levels, and keeps SupportClaw-managed CLI runs isolated from shell env.
 */
import type {
  CliBackendConfig,
...
// steer SupportClaw-managed Claude CLI runs toward a different provider,
// endpoint, token source, plugin/config tree, or telemetry bootstrap mode.
/** Environment variables removed before launching SupportClaw-managed Claude CLI runs. */
...
function isSupportClawRequestedYolo(context?: CliBackendNormalizeConfigContext): boolean {
...
/** Resolve Claude permission mode from SupportClaw exec security settings. */
export function resolveClaudePermissionMode(context?: CliBackendNormalizeConfigContext): {
  mode?: string;
  overrideExisting: boolean;
} {
  return isSupportClawRequestedYolo(context)
...
/** Map SupportClaw thinking levels to Claude CLI effort flags for a model id. */
