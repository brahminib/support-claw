...
vi.mock("supportClaw/plugin-sdk/runtime-env", () => ({
...
import type { SupportClawPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/supportClaw-diagnostics-otel-test";
...
const ORIGINAL_SUPPORT_CLAW_OTEL_PRELOADED = process.env.SUPPORT_CLAW_OTEL_PRELOADED;
...
type OtelContextFlags = {
  ...
  protocol?: NonNullable<
    NonNullable<SupportClawPluginServiceContext["config"]["diagnostics"]>["otel"]
  >["protocol"];
...
function createTraceOnlyContext(endpoint: string): SupportClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.SUPPORT_CLAW_OTEL_PRELOADED;
  ...
  afterEach(() => {
    resetDiagnosticEventsForTest();
    if (ORIGINAL_SUPPORT_CLAW_OTEL_PRELOADED === undefined) {
      delete process.env.SUPPORT_CLAW_OTEL_PRELOADED;
    } else {
      process.env.SUPPORT_CLAW_OTEL_PRELOADED = ORIGINAL_SUPPORT_CLAW_OTEL_PRELOADED;
    }
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["supportClaw.provider"]).toBe("openai");
    for (const key of [
      "supportClaw.callId",
      "supportClaw.call_id",
      "supportClaw.chatId",
      "supportClaw.chat_id",
      "supportClaw.messageId",
      "supportClaw.message_id",
      "supportClaw.parentSpanId",
      "supportClaw.parent_span_id",
      "supportClaw.runId",
      "supportClaw.run_id",
      "supportClaw.sessionId",
      "supportClaw.session_id",
      "supportClaw.sessionKey",
      "supportClaw.session_key",
      "supportClaw.spanId",
      "supportClaw.span_id",
      "supportClaw.toolCallId",
      "supportClaw.tool_call_id",
      "supportClaw.traceId",
      "supportClaw.trace_id",
    ]) {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("supportClaw.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.channel": "telegram",
      "supportClaw.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("supportClaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "supportClaw.channel": "telegram",
      "supportClaw.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("supportClaw.message.queued")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.channel": "telegram",
      "supportClaw.source": "telegram",
    });
    expect(telemetryState.histograms.get("supportClaw.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("supportClaw.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "supportClaw.channel": "telegram",
      "supportClaw.source": "telegram",
    });
    expect(telemetryState.histograms.get("supportClaw.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "supportClaw.lane": "main",
    });
    expect(telemetryState.counters.get("supportClaw.message.processed")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.channel": "telegram",
      "supportClaw.outcome": "completed",
    });
    expect(telemetryState.counters.get("supportClaw.message.received")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.channel": "telegram",
      "supportClaw.source": "webhook",
    });
    expect(telemetryState.counters.get("supportClaw.message.received")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.channel": "unknown",
      "supportClaw.source": "unknown",
    });
    expect(
      telemetryState.counters.get("supportClaw.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("supportClaw.message.dispatch.completed")?.add,
    ...
    expect(
      ...
      expect.objectContaining({
        "supportClaw.channel": "unknown",
        "supportClaw.reason": "none",
        "supportClaw.source": "unknown",
      }),
    );
    expect(
      telemetryState.histograms.get("supportClaw.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "supportClaw.channel": "telegram",
    ...
    expect(
      telemetryState.histograms.get("supportClaw.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "supportClaw.channel": "telegram",
      "supportClaw.outcome": "completed",
    });
    expect(telemetryState.histograms.get("supportClaw.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "supportClaw.lane": "main",
      },
    );
    expect(telemetryState.counters.get("supportClaw.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("supportClaw.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.state": "processing",
    });
    expect(
      telemetryState.histograms.get("supportClaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "supportClaw.state": "processing",
    });
    expect(telemetryState.counters.get("supportClaw.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("supportClaw.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "supportClaw.agent": "agent.default",
        "supportClaw.channel": "telegram",
        "supportClaw.trigger": "user",
      },
    );

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("supportClaw.webhook.processed");
    expect(spanNames).toContain("supportClaw.message.processed");
    expect(spanNames).toContain("supportClaw.session.stuck");
    const webhookSpanOptions = startedSpanOptions("supportClaw.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("supportClaw.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("supportClaw.message.processed");
    expect(messageSpanOptions?.attributes?.["supportClaw.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["supportClaw.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["supportClaw.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("supportClaw.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("supportClaw.messageId");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.SUPPORT_CLAW_OTEL_PRELOADED = "1";
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { traces: true, metrics: true, logs: true });
    await service.start(ctx);
    ...
    const runDurationRecordCall = lastHistogramRecord("supportClaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["supportClaw.provider"]).toBe("openai");
    expect(runDurationAttributes?.["supportClaw.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("supportClaw.run");
    expect(runSpanOptions?.attributes?.["supportClaw.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("supportClaw.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "supportClaw.exporter": "diagnostics-otel",
      "supportClaw.signal": "logs",
      "supportClaw.status": "started",
      "supportClaw.reason": "configured",
    });
  ...
  test("exports trusted security events as bounded OTLP logs", async () => {
    ...
    expect(emitCall.body).toBe("supportClaw.security.event");
    expect(emitCall.severityText).toBe("WARN");
    expect(emitCall.severityNumber).toBe(13);
    expect(emitCall.attributes).toMatchObject({
      "supportClaw.security.event_id": "security-event-1",
      "supportClaw.security.category": "tool",
      "supportClaw.security.action": "tool.execution.blocked",
      "supportClaw.security.outcome": "denied",
      "supportClaw.security.severity": "medium",
      "supportClaw.security.reason": "tools.deny",
      "supportClaw.security.actor.kind": "agent",
      "supportClaw.security.actor.id_hash": "agent-hash-1",
      "supportClaw.security.actor.role": "operator",
      "supportClaw.security.actor.scopes": "operator.read,operator.approvals",
      "supportClaw.security.target.kind": "plugin",
      "supportClaw.security.target.name": "@acme/security-event-plugin",
      "supportClaw.security.target.owner": "plugin-installer",
      "supportClaw.security.policy.id": "tools.exec",
      "supportClaw.security.policy.decision": "deny",
      "supportClaw.security.policy.reason": "allowlist.miss",
      "supportClaw.security.control.id": "exec-approval",
      "supportClaw.security.control.family": "approval",
      "supportClaw.security.attribute.params_kind": "object",
      "supportClaw.security.attribute.secretish": "unknown",
    });
    ...
    expect(Object.hasOwn(emitCall.attributes ?? {}, "supportClaw.security.attribute.__proto__")).toBe(
      false,
    );
  ...
  test("exports trusted security events as stdout JSONL logs", async () => {
    ...
    try {
      ...
      expect(record.body).toBe("supportClaw.security.event");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(record.attributes).toMatchObject({
        "supportClaw.security.event_id": "security-event-stdout",
        "supportClaw.security.category": "tool",
        "supportClaw.security.action": "tool.execution.blocked",
        "supportClaw.security.outcome": "denied",
        "supportClaw.security.severity": "medium",
      ...
      expect(Object.hasOwn(record.attributes ?? {}, "supportClaw.security.attribute.__proto__")).toBe(
        false,
      );
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("supportClaw.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("supportClaw.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "supportClaw.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("supportClaw.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "supportClaw.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("supportClaw.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["supportClaw.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["supportClaw.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["supportClaw.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "supportClaw.liveness.warning");
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("supportClaw.payload.large")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.payload.action": "rejected",
      "supportClaw.payload.surface": "gateway.frame",
      "supportClaw.channel": "web",
      "supportClaw.plugin": "none",
      "supportClaw.reason": "body-too-large",
    });
    expect(
      telemetryState.histograms.get("supportClaw.payload.large_bytes")?.record,
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "supportClaw.exporter": "diagnostics-otel",
      "supportClaw.signal": "logs",
      "supportClaw.status": "failure",
      "supportClaw.reason": "emit_failed",
      "supportClaw.errorCategory": "TypeError",
    });
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    await service.start(ctx);
    telemetryState.counters.get("supportClaw.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("supportClaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["supportClaw.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["supportClaw.blocked_by"]).toBe("policy-plugin");
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("supportClaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["supportClaw.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const modelCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "supportClaw.model.call",
    );
    const attrs = (modelCall?.[1] as { attributes?: Record<string, unknown> } | undefined)
      ?.attributes;
    expect(attrs?.["supportClaw.content.input_messages"]).toBe("user prompt");
  ...
  test("exports diagnostic logs as stdout JSONL without constructing the OTLP log exporter", async () => {
    ...
    ctx.config.diagnostics!.otel!.serviceName = "rovoclaw-supportClaw";
    const stdout = captureStdoutWrites();
    ...
    try {
      ...
      expect(record.signal).toBe("supportClaw.diagnostic.log");
      expect(record["service.name"]).toBe("rovoclaw-supportClaw");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(String(record.body)).not.toContain("sk-1234567890abcdef1234567890abcdef");
      expect(String(record.body)).toContain("sk-123");
      expect(record.attributes).toMatchObject({
        "supportClaw.log.level": "WARN",
        "supportClaw.subsystem": "diagnostic",
      });
      const tokenAttr = record.attributes?.["supportClaw.token"];
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["supportClaw.token"];
    expect(tokenAttr).not.toBe("ghp_abcdefghijklmnopqrstuvwxyz123456"); // pragma: allowlist secret
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "supportClaw.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "supportClaw.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "supportClaw.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/supportClaw/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/supportClaw/src/private.ts:42",
      },
    ...
    expect(String(emitCall.attributes["supportClaw.good"])).toMatch(/^y+/);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(String(emitCall.attributes["supportClaw.good"]).length).toBeLessThanOrEqual(4200);
    expect(Object.hasOwn(emitCall.attributes, `supportClaw.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "supportClaw.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "supportClaw.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "supportClaw.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "supportClaw.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "supportClaw.code.location")).toBe(false);
    await service.stop?.(ctx);
  });
  ...
  test("does not parent diagnostic event spans from plugin-emittable trace context", async () => {
    ...
    const modelUsageCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "supportClaw.model.usage",
    );
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("supportClaw.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "supportClaw.channel": "webchat",
      "supportClaw.agent": "ops",
      "supportClaw.provider": "openai",
      "supportClaw.model": "gpt-5.4",
      "supportClaw.token": "input",
    });
  ...
  test("bounds agent identifiers on model usage metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("supportClaw.tokens")?.add).toHaveBeenCalledWith(2, {
      "supportClaw.channel": "unknown",
      "supportClaw.agent": "unknown",
      "supportClaw.provider": "openai",
      "supportClaw.model": "gpt-5.4",
      "supportClaw.token": "input",
    });
    expect(
      JSON.stringify(telemetryState.counters.get("supportClaw.tokens")?.add.mock.calls),
  ...
  test("drops session-shaped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("supportClaw.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "supportClaw.lane": "session",
      },
    );
    expect(
      JSON.stringify(telemetryState.counters.get("supportClaw.queue.lane.enqueue")?.add.mock.calls),
    ).not.toContain("Agent:qa:otel-trace-smoke");
    await service.stop?.(ctx);
  });
  ...
  test("keeps only the bounded prefix from scoped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("supportClaw.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "supportClaw.lane": "dreaming-narrative",
      },
    );
    expect(
      JSON.stringify(telemetryState.counters.get("supportClaw.queue.lane.enqueue")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("supportClaw.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "supportClaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "supportClaw.sessionId")).toBe(false);
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "supportClaw.agent": "main",
      "supportClaw.skill.activation": "read",
      "supportClaw.skill.name": "tiny-llm-brainstorm",
      "supportClaw.skill.source": "workspace",
      "supportClaw.toolName": "read",
    };
    expect(telemetryState.counters.get("supportClaw.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "supportClaw.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("supportClaw.run");
    expect(spanNames).toContain("supportClaw.model.call");
    expect(spanNames).toContain("supportClaw.harness.run");
    expect(spanNames).toContain("supportClaw.tool.execution");

    const runOptions = startedSpanOptions("supportClaw.run");
    expect(runOptions?.attributes?.["supportClaw.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["supportClaw.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["supportClaw.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["supportClaw.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "supportClaw.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "supportClaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "supportClaw.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("supportClaw.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "supportClaw.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "supportClaw.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "supportClaw.sessionKey")).toBe(false);
    ...
    const harnessCall = startedSpanCall("supportClaw.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["supportClaw.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["supportClaw.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["supportClaw.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["supportClaw.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["supportClaw.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["supportClaw.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["supportClaw.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["supportClaw.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["supportClaw.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["supportClaw.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["supportClaw.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "supportClaw.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "supportClaw.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "supportClaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "supportClaw.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("supportClaw.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["supportClaw.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["supportClaw.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["supportClaw.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["supportClaw.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["supportClaw.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "supportClaw.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "supportClaw.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "supportClaw.sessionKey")).toBe(false);
    expect(toolOptions?.startTime).toBeTypeOf("number");
    expect(toolCall?.[2]).toBeUndefined();

    const modelCallDuration = lastHistogramRecord("supportClaw.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["supportClaw.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["supportClaw.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("supportClaw.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["supportClaw.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["supportClaw.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("supportClaw.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["supportClaw.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["supportClaw.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("supportClaw.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["supportClaw.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["supportClaw.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("supportClaw.model.call");
    expect(modelSpanAttributes["supportClaw.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["supportClaw.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["supportClaw.model_call.time_to_first_byte_ms"]).toBe(45);
    const runDuration = lastHistogramRecord("supportClaw.run.duration_ms");
    expect(runDuration?.[0]).toBe(100);
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "supportClaw.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("supportClaw.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["supportClaw.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["supportClaw.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["supportClaw.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "supportClaw.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "supportClaw.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("supportClaw.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["supportClaw.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "supportClaw.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "supportClaw.runId")).toBe(false);

    const toolSpan = spanByName("supportClaw.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("supportClaw.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    await service.stop?.(ctx);
  });
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("supportClaw.model.failover");
    expect(failoverOptions?.attributes?.["supportClaw.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["supportClaw.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["supportClaw.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["supportClaw.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["supportClaw.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["supportClaw.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["supportClaw.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["supportClaw.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "supportClaw.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "supportClaw.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("supportClaw.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("supportClaw.model.failover")).toStrictEqual([
      1,
      {
        "supportClaw.failover.reason": "overloaded",
        "supportClaw.failover.suspended": "true",
        "supportClaw.lane": "main",
        "supportClaw.model": "claude-opus-4-6",
        "supportClaw.provider": "anthropic",
        "supportClaw.failover.to_model": "gpt-5.4",
        "supportClaw.failover.to_provider": "openai",
      },
    ]);
    await service.stop?.(ctx);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("supportClaw.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "supportClaw.toolName": "browser",
        "supportClaw.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "supportClaw.tool.owner": "browser-tools",
        "supportClaw.tool.params.kind": "object",
        "supportClaw.deniedReason": "tools.deny",
      },
    ]);
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalledWith(
      "supportClaw.tool.execution",
      expect.anything(),
      expect.anything(),
    );
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("supportClaw.model.failover");
    expect(failoverOptions?.attributes?.["supportClaw.lane"]).toBe("session");
  ...
  test("maps model call APIs to GenAI operation names and error type", async () => {
    ...
    const modelCallAttrs = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "supportClaw.model.call")
  ...
  test("uses latest GenAI inference span shape only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("supportClaw.model.call")).toBeUndefined();
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("supportClaw.model.call");
    expect(modelCallOptions?.attributes?.["supportClaw.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "supportClaw.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "supportClaw.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("supportClaw.provider.request", {
      "supportClaw.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("supportClaw.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["supportClaw.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "supportClaw.upstreamRequestIdHash")).toBe(
      false,
    );
    await service.stop?.(ctx);
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("supportClaw.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "supportClaw.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["supportClaw.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["supportClaw.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["supportClaw.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["supportClaw.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["supportClaw.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["supportClaw.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["supportClaw.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["supportClaw.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["supportClaw.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["supportClaw.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["supportClaw.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["supportClaw.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["supportClaw.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("supportClaw.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.toolName": "process",
      "supportClaw.loop.level": "critical",
      "supportClaw.loop.action": "block",
      "supportClaw.loop.detector": "known_poll_no_progress",
      "supportClaw.loop.count": 20,
      "supportClaw.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("supportClaw.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["supportClaw.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["supportClaw.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["supportClaw.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["supportClaw.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["supportClaw.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["supportClaw.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "supportClaw.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("supportClaw.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("supportClaw.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "supportClaw.memory.level": "critical",
        "supportClaw.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("supportClaw.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.memory.level": "critical",
      "supportClaw.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("supportClaw.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["supportClaw.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["supportClaw.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["supportClaw.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["supportClaw.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["supportClaw.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["supportClaw.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["supportClaw.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["supportClaw.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["supportClaw.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["supportClaw.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "supportClaw.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("supportClaw.diagnostic.async_queue.dropped");
    expect(counter?.add).toHaveBeenCalledWith(4, {
      "supportClaw.diagnostic.async_queue.drop_class": "total",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "supportClaw.diagnostic.async_queue.drop_class": "trusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(2, {
      "supportClaw.diagnostic.async_queue.drop_class": "untrusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "supportClaw.diagnostic.async_queue.drop_class": "priority",
    });
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "supportClaw.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "supportClaw.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "supportClaw.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["supportClaw.run"]).toBeUndefined();
    expect(parentBySpanName["supportClaw.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["supportClaw.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("supportClaw.message.processed");
    const harnessSpan = spanByName("supportClaw.harness.run");
    const runSpan = spanByName("supportClaw.run");
    const usageSpan = spanByName("supportClaw.model.usage");
    const modelCallSpan = spanByName("supportClaw.model.call");
    ...
    expect(parentBySpanName["supportClaw.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["supportClaw.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["supportClaw.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["supportClaw.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["supportClaw.model.call"]?.spanId).toBe(runSpanContext.spanId);
    await service.stop?.(ctx);
  });
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("supportClaw.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("supportClaw.message.processed")[0]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("supportClaw.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("supportClaw.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("supportClaw.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("supportClaw.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("supportClaw.message.processed")["supportClaw.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
    await service.stop?.(ctx);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("supportClaw.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("supportClaw.harness.run")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("supportClaw.run")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("does not parent sibling active runs through shared upstream aliases", async () => {
    ...
    const runContexts = startedSpanParentContextsByName("supportClaw.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("supportClaw.run").spanContext();
    const modelParentContext = startedSpanParentContexts("supportClaw.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("supportClaw.model.call");
  ...
  test("removes retained run contexts after queued diagnostics drain", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("supportClaw.model.usage")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not force remote parents for completed-only trusted lifecycle spans", async () => {
    ...
    expect(parentBySpanName["supportClaw.run"]).toBeUndefined();
    expect(parentBySpanName["supportClaw.model.call"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["supportClaw.run"]).toBeUndefined();
    expect(parentBySpanName["supportClaw.model.call"]).toBeUndefined();
    expect(parentBySpanName["supportClaw.tool.execution"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "supportClaw.run"),
    ).toHaveLength(1);
    expect(
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "supportClaw.tool.execution",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "supportClaw.harness.run",
      ),
    ).toHaveLength(1);
    await service.stop?.(ctx);
  });
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("supportClaw.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["supportClaw.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["supportClaw.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["supportClaw.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["supportClaw.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("supportClaw.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["supportClaw.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["supportClaw.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["supportClaw.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["supportClaw.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["supportClaw.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["supportClaw.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["supportClaw.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "supportClaw.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "supportClaw.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "supportClaw.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("supportClaw.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("supportClaw.exec")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("supportClaw.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "supportClaw.channel": "matrix",
      "supportClaw.delivery.kind": "text",
    });
    const deliveryDurationRecords = telemetryState.histograms.get(
      "supportClaw.message.delivery.duration_ms",
    )?.record.mock.calls as Array<[unknown, Record<string, unknown>]>;
    expect(deliveryDurationRecords[0]?.[0]).toBe(25);
    expect(deliveryDurationRecords[0]?.[1]["supportClaw.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["supportClaw.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["supportClaw.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["supportClaw.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["supportClaw.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["supportClaw.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["supportClaw.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "supportClaw.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["supportClaw.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["supportClaw.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["supportClaw.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["supportClaw.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["supportClaw.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["supportClaw.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["supportClaw.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["supportClaw.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "supportClaw.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "supportClaw.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "supportClaw.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "supportClaw.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "supportClaw.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "supportClaw.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "supportClaw.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("supportClaw.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["supportClaw.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["supportClaw.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["supportClaw.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("supportClaw.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["supportClaw.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["supportClaw.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["supportClaw.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["supportClaw.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("supportClaw.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["supportClaw.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["supportClaw.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["supportClaw.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("supportClaw.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["supportClaw.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["supportClaw.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["supportClaw.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("supportClaw.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["supportClaw.status"]).toBe("released");
    expect(telemetryState.counters.get("supportClaw.talk.event")?.add).toHaveBeenCalledWith(1, {
      "supportClaw.talk.brain": "agent-consult",
      "supportClaw.talk.event_type": "input.audio.delta",
      "supportClaw.talk.mode": "realtime",
      "supportClaw.talk.provider": "openai",
      "supportClaw.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("supportClaw.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "supportClaw.talk.brain": "agent-consult",
        "supportClaw.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("supportClaw.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "supportClaw.talk.brain": "agent-consult",
      "supportClaw.talk.event_type": "latency.metrics",
      "supportClaw.talk.mode": "realtime",
      "supportClaw.talk.provider": "openai",
      "supportClaw.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("supportClaw.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("supportClaw.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "supportClaw.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "supportClaw.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "supportClaw.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("supportClaw.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "supportClaw.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "supportClaw.content.tool_output")).toBe(
      false,
    );
    expect(toolOptions?.startTime).toBeTypeOf("number");
    await service.stop?.(ctx);
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    expect(modelAttrs?.["supportClaw.content.output_messages"]).toBe("model reply");
    expect(modelAttrs?.["supportClaw.content.system_prompt"]).toBe("system prompt");
    expect(String(modelAttrs?.["supportClaw.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["supportClaw.content.tool_input"]).toBe("tool input");
    expect(String(toolAttrs?.["supportClaw.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["supportClaw.content.tool_output"])).not.toContain("a".repeat(11));
    await service.stop?.(ctx);
  });
  ...
  test("omits absent model content fields when capture fields are opted in", async () => {
    ...
    expect(attrs["supportClaw.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "supportClaw.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "supportClaw.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "supportClaw.content.tool_definitions")).toBe(false);
  ...
  test("exports tool definitions without requiring input message capture", async () => {
    ...
    expect(Object.hasOwn(attrs ?? {}, "supportClaw.content.input_messages")).toBe(false);
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.tool.definitions"))).toEqual([
      {
        type: "function",
        name: "lookup",
    ...
    expect(JSON.parse(String(attrs?.["supportClaw.content.tool_definitions"]))).toEqual([
      {
        name: "lookup",
        description: "Lookup data",
        parameters: { type: "object" },
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("supportClaw.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["supportClaw.reason"])).toContain("…");
    expect(typeof attrs?.["supportClaw.reason"]).toBe("string");
    expect(String(attrs?.["supportClaw.reason"])).not.toContain(
