...
import { waitForDiagnosticEventsDrained } from "supportClaw/plugin-sdk/diagnostic-runtime";
import { registerUnhandledRejectionHandler } from "supportClaw/plugin-sdk/runtime-env";
import type {
...
const DEFAULT_SERVICE_NAME = "supportClaw";
...
const PRELOADED_OTEL_SDK_ENV = "SUPPORT_CLAW_OTEL_PRELOADED";
...
function writeStdoutDiagnosticLogRecord(params: {
  ...
  const line = {
    ...
    signal: "supportClaw.diagnostic.log",
...
function assignModelCallSizeTimingAttrs(
  ...
  assignPositiveNumberAttr(attrs, "supportClaw.model_call.request_bytes", evt.requestPayloadBytes);
  assignPositiveNumberAttr(attrs, "supportClaw.model_call.response_bytes", evt.responseStreamBytes);
  assignPositiveNumberAttr(
    attrs,
    "supportClaw.model_call.time_to_first_byte_ms",
...
function addUpstreamRequestIdSpanEvent(
  ...
  span.addEvent?.("supportClaw.provider.request", {
    "supportClaw.upstreamRequestIdHash": boundedHash,
  });
}
...
function assignOtelModelContentAttributes(
  ...
  if (policy.inputMessages) {
    assignOtelContentAttribute(
      attributes,
      "supportClaw.content.input_messages",
  ...
  if (policy.toolDefinitions) {
    assignOtelContentAttribute(
      attributes,
      "supportClaw.content.tool_definitions",
  ...
  if (policy.outputMessages) {
    assignOtelContentAttribute(
      attributes,
      "supportClaw.content.output_messages",
      content?.outputMessages,
    );
  }
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "supportClaw.content.system_prompt", content?.systemPrompt);
  }
}
...
function assignOtelToolContentAttributes(
  ...
  if (policy.toolInputs) {
    assignOtelContentAttribute(attributes, "supportClaw.content.tool_input", content?.toolInput);
  }
  if (policy.toolOutputs) {
    assignOtelContentAttribute(attributes, "supportClaw.content.tool_output", content?.toolOutput);
  }
}
...
function assignOtelLogEventAttributes(
  ...
  for (const rawKey in eventAttributes) {
    ...
    assignOtelLogAttribute(attributes, `supportClaw.${key}`, eventAttributes[rawKey]);
  }
}
...
function assignOtelSecurityEventAttributes(
  ...
  for (const rawKey in eventAttributes) {
    ...
    assignOtelLogAttribute(
      attributes,
      `supportClaw.security.attribute.${key}`,
...
function assignOtelSecurityAttributes(
  ...
  assignOtelLogAttribute(attributes, "supportClaw.security.event_id", evt.eventId);
  assignOtelLogAttribute(attributes, "supportClaw.security.category", evt.category);
  assignOtelLogAttribute(attributes, "supportClaw.security.action", lowCardinalityAttr(evt.action));
  assignOtelLogAttribute(attributes, "supportClaw.security.outcome", evt.outcome);
  assignOtelLogAttribute(attributes, "supportClaw.security.severity", evt.severity);
  if (evt.reason) {
    assignOtelLogAttribute(attributes, "supportClaw.security.reason", lowCardinalityAttr(evt.reason));
  }
  if (evt.actor) {
    assignOtelLogAttribute(attributes, "supportClaw.security.actor.kind", evt.actor.kind);
    if (evt.actor.idHash) {
      assignOtelLogAttribute(
        attributes,
        "supportClaw.security.actor.id_hash",
    ...
    if (evt.actor.deviceIdHash) {
      assignOtelLogAttribute(
        attributes,
        "supportClaw.security.actor.device_id_hash",
    ...
    if (evt.actor.channel) {
      assignOtelLogAttribute(
        attributes,
        "supportClaw.security.actor.channel",
    ...
    if (evt.actor.role) {
      assignOtelLogAttribute(
        attributes,
        "supportClaw.security.actor.role",
    ...
    if (evt.actor.scopes?.length) {
      assignOtelLogAttribute(
        attributes,
        "supportClaw.security.actor.scopes",
        evt.actor.scopes.map((scope) => lowCardinalityAttr(scope)).join(","),
      );
    }
  }
  if (evt.target) {
    assignOtelLogAttribute(attributes, "supportClaw.security.target.kind", evt.target.kind);
    if (evt.target.idHash) {
      assignOtelLogAttribute(
        attributes,
        "supportClaw.security.target.id_hash",
    ...
    if (evt.target.name) {
      assignOtelLogAttribute(
        attributes,
        "supportClaw.security.target.name",
    ...
    if (evt.target.owner) {
      assignOtelLogAttribute(
        attributes,
        "supportClaw.security.target.owner",
  ...
  if (evt.policy) {
    if (evt.policy.id) {
      assignOtelLogAttribute(
        attributes,
        "supportClaw.security.policy.id",
        lowCardinalityAttr(evt.policy.id),
      );
    }
    if (evt.policy.decision) {
      assignOtelLogAttribute(attributes, "supportClaw.security.policy.decision", evt.policy.decision);
    }
    if (evt.policy.reason) {
      assignOtelLogAttribute(
        attributes,
        "supportClaw.security.policy.reason",
  ...
  if (evt.control) {
    if (evt.control.id) {
      assignOtelLogAttribute(
        attributes,
        "supportClaw.security.control.id",
        lowCardinalityAttr(evt.control.id),
      );
    }
    if (evt.control.family) {
      assignOtelLogAttribute(attributes, "supportClaw.security.control.family", evt.control.family);
    }
  }
...
function addTraceAttributes(
  ...
  attributes["supportClaw.traceId"] = normalized.traceId;
  if (normalized.spanId) {
    attributes["supportClaw.spanId"] = normalized.spanId;
  }
  if (normalized.parentSpanId) {
    attributes["supportClaw.parentSpanId"] = normalized.parentSpanId;
  }
  if (normalized.traceFlags) {
    attributes["supportClaw.traceFlags"] = normalized.traceFlags;
  }
}

export function createDiagnosticsOtelService(): SupportClawPluginService {
  ...
  return {
    ...
    async start(ctx) {
      ...
      const meter = metrics.getMeter("supportClaw");
      const tracer = trace.getTracer("supportClaw");
      ...
      const tokensCounter = meter.createCounter("supportClaw.tokens", {
        unit: "1",
      ...
      const costCounter = meter.createCounter("supportClaw.cost.usd", {
        unit: "1",
        description: "Estimated model cost (USD)",
      });
      const durationHistogram = meter.createHistogram("supportClaw.run.duration_ms", {
        unit: "ms",
        description: "Agent run duration",
      });
      const harnessDurationHistogram = meter.createHistogram("supportClaw.harness.duration_ms", {
        unit: "ms",
        description: "Agent harness lifecycle duration",
      });
      const contextHistogram = meter.createHistogram("supportClaw.context.tokens", {
        unit: "1",
        description: "Context window size and usage",
      });
      const webhookReceivedCounter = meter.createCounter("supportClaw.webhook.received", {
        unit: "1",
        description: "Webhook requests received",
      });
      const webhookErrorCounter = meter.createCounter("supportClaw.webhook.error", {
        unit: "1",
        description: "Webhook processing errors",
      });
      const webhookDurationHistogram = meter.createHistogram("supportClaw.webhook.duration_ms", {
        unit: "ms",
        description: "Webhook processing duration",
      });
      const messageQueuedCounter = meter.createCounter("supportClaw.message.queued", {
        unit: "1",
        description: "Messages queued for processing",
      });
      const messageReceivedCounter = meter.createCounter("supportClaw.message.received", {
        unit: "1",
        description: "Inbound messages received",
      });
      const messageDispatchStartedCounter = meter.createCounter(
        "supportClaw.message.dispatch.started",
        {
          unit: "1",
      ...
      const messageDispatchCompletedCounter = meter.createCounter(
        "supportClaw.message.dispatch.completed",
        {
          unit: "1",
      ...
      const messageDispatchDurationHistogram = meter.createHistogram(
        "supportClaw.message.dispatch.duration_ms",
        {
          unit: "ms",
          description: "Inbound message dispatch duration",
        },
      );
      const messageProcessedCounter = meter.createCounter("supportClaw.message.processed", {
        unit: "1",
        description: "Messages processed by outcome",
      });
      const messageDurationHistogram = meter.createHistogram("supportClaw.message.duration_ms", {
        unit: "ms",
        description: "Message processing duration",
      });
      const messageDeliveryStartedCounter = meter.createCounter(
        "supportClaw.message.delivery.started",
        {
          unit: "1",
      ...
      const queueDepthHistogram = meter.createHistogram("supportClaw.queue.depth", {
        unit: "1",
        description: "Queue depth on enqueue/dequeue",
      });
      const queueWaitHistogram = meter.createHistogram("supportClaw.queue.wait_ms", {
        unit: "ms",
        description: "Queue wait time before execution",
      });
      const laneEnqueueCounter = meter.createCounter("supportClaw.queue.lane.enqueue", {
        unit: "1",
        description: "Command queue lane enqueue events",
      });
      const laneDequeueCounter = meter.createCounter("supportClaw.queue.lane.dequeue", {
        unit: "1",
        description: "Command queue lane dequeue events",
      });
      const sessionStateCounter = meter.createCounter("supportClaw.session.state", {
        unit: "1",
        description: "Session state transitions",
      });
      const sessionTurnCreatedCounter = meter.createCounter("supportClaw.session.turn.created", {
        unit: "1",
        description: "Agent session turns created",
      });
      const sessionStuckCounter = meter.createCounter("supportClaw.session.stuck", {
        unit: "1",
        description: "Sessions stuck in processing",
      });
      const sessionStuckAgeHistogram = meter.createHistogram("supportClaw.session.stuck_age_ms", {
        unit: "ms",
        description: "Age of stuck sessions",
      });
      const sessionRecoveryRequestedCounter = meter.createCounter(
        "supportClaw.session.recovery.requested",
        {
          unit: "1",
      ...
      const sessionRecoveryCompletedCounter = meter.createCounter(
        "supportClaw.session.recovery.completed",
        {
          unit: "1",
      ...
      const sessionRecoveryAgeHistogram = meter.createHistogram(
        "supportClaw.session.recovery.age_ms",
        {
          unit: "ms",
          description: "Age of sessions selected for recovery",
        },
      );
      const talkEventCounter = meter.createCounter("supportClaw.talk.event", {
        unit: "1",
        description: "Talk events emitted by type",
      });
      const talkEventDurationHistogram = meter.createHistogram("supportClaw.talk.event.duration_ms", {
        unit: "ms",
        description: "Talk event duration when reported",
      });
      const talkAudioBytesHistogram = meter.createHistogram("supportClaw.talk.audio.bytes", {
        unit: "By",
        description: "Talk audio frame byte lengths",
      });
      const runAttemptCounter = meter.createCounter("supportClaw.run.attempt", {
        unit: "1",
        description: "Run attempts",
      });
      const toolLoopCounter = meter.createCounter("supportClaw.tool.loop", {
        unit: "1",
        description: "Detected repetitive tool-call loop events",
      });
      const skillUsedCounter = meter.createCounter("supportClaw.skill.used", {
        unit: "1",
        description: "Skills used by agent runs",
      });
      const modelCallDurationHistogram = meter.createHistogram("supportClaw.model_call.duration_ms", {
        unit: "ms",
        description: "Model call duration",
      });
      const modelCallRequestBytesHistogram = meter.createHistogram(
        "supportClaw.model_call.request_bytes",
        {
          unit: "By",
      ...
      const modelCallResponseBytesHistogram = meter.createHistogram(
        "supportClaw.model_call.response_bytes",
        {
          unit: "By",
      ...
      const modelFailoverCounter = meter.createCounter("supportClaw.model.failover", {
        unit: "1",
      ...
      const toolExecutionDurationHistogram = meter.createHistogram(
        "supportClaw.tool.execution.duration_ms",
        {
          unit: "ms",
          description: "Tool execution duration",
        },
      );
      const toolExecutionBlockedCounter = meter.createCounter("supportClaw.tool.execution.blocked", {
        unit: "1",
        description: "Tool executions blocked by policy or sandbox diagnostics",
      });
      const execProcessDurationHistogram = meter.createHistogram("supportClaw.exec.duration_ms", {
        unit: "ms",
        description: "Exec process duration",
      });
      const memoryRssHistogram = meter.createHistogram("supportClaw.memory.rss_bytes", {
        unit: "By",
        description: "Resident set size reported by diagnostic memory samples",
      });
      const memoryHeapUsedHistogram = meter.createHistogram("supportClaw.memory.heap_used_bytes", {
        unit: "By",
        description: "Heap used bytes reported by diagnostic memory samples",
      });
      const memoryHeapTotalHistogram = meter.createHistogram("supportClaw.memory.heap_total_bytes", {
        unit: "By",
        description: "Heap total bytes reported by diagnostic memory samples",
      });
      const memoryExternalHistogram = meter.createHistogram("supportClaw.memory.external_bytes", {
        unit: "By",
      ...
      const livenessEventLoopDelayMaxHistogram = meter.createHistogram(
        "supportClaw.liveness.event_loop_delay_max_ms",
        {
          unit: "ms",
      ...
      const livenessEventLoopUtilizationHistogram = meter.createHistogram(
        "supportClaw.liveness.event_loop_utilization",
        {
          unit: "1",
      ...
      const livenessCpuCoreRatioHistogram = meter.createHistogram(
        "supportClaw.liveness.cpu_core_ratio",
        {
          unit: "1",
          description: "CPU core ratio reported by diagnostic liveness warnings",
        },
      );
      const telemetryExporterCounter = meter.createCounter("supportClaw.telemetry.exporter.events", {
        unit: "1",
      ...
      if (logsEnabled) {
        ...
        if (logsToOtlp) {
          ...
          otelLogger = logProvider.getLogger("supportClaw");
        }
        ...
        const buildDiagnosticLogRecord = (
          ...
          assignOtelLogAttribute(attributes, "supportClaw.log.level", logLevelName);
          if (evt.loggerName) {
            assignOtelLogAttribute(attributes, "supportClaw.logger", evt.loggerName);
          }
          if (evt.loggerParents?.length) {
            assignOtelLogAttribute(
              attributes,
              "supportClaw.logger.parents",
        ...
        const buildSecurityLogRecord = (
          ...
          const traceContext = normalizedTrustedTraceContext(evt, metadata);
          const logRecord: LogRecord = {
            body: "supportClaw.security.event",
            severityText,
      ...
      const addRunAttrs = (
        ...
        if (evt.provider) {
          spanAttrs["supportClaw.provider"] = evt.provider;
        }
        if (evt.model) {
          spanAttrs["supportClaw.model"] = evt.model;
        }
        if (evt.channel) {
          spanAttrs["supportClaw.channel"] = evt.channel;
        }
        if (evt.trigger) {
          spanAttrs["supportClaw.trigger"] = evt.trigger;
        }
      };
      ...
      const paramsSummaryAttrs = (
        ...
        return {
          "supportClaw.tool.params.kind": summary.kind,
          ...("length" in summary ? { "supportClaw.tool.params.length": summary.length } : {}),
        };
      };
      ...
      const recordModelUsage = (
        ...
        const attrs = {
          "supportClaw.channel": evt.channel ?? "unknown",
          "supportClaw.agent": lowCardinalityAttr(evt.agentId),
          "supportClaw.provider": evt.provider ?? "unknown",
          "supportClaw.model": evt.model ?? "unknown",
        };
        ...
        if (usage.input) {
          tokensCounter.add(usage.input, { ...attrs, "supportClaw.token": "input" });
        ...
        if (usage.output) {
          tokensCounter.add(usage.output, { ...attrs, "supportClaw.token": "output" });
        ...
        if (usage.cacheRead) {
          tokensCounter.add(usage.cacheRead, { ...attrs, "supportClaw.token": "cache_read" });
        }
        if (usage.cacheWrite) {
          tokensCounter.add(usage.cacheWrite, { ...attrs, "supportClaw.token": "cache_write" });
        }
        if (usage.promptTokens) {
          tokensCounter.add(usage.promptTokens, { ...attrs, "supportClaw.token": "prompt" });
        }
        if (usage.total) {
          tokensCounter.add(usage.total, { ...attrs, "supportClaw.token": "total" });
        }
        ...
        if (evt.context?.limit) {
          contextHistogram.record(evt.context.limit, {
            ...attrs,
            "supportClaw.context": "limit",
          });
        }
        if (evt.context?.used) {
          contextHistogram.record(evt.context.used, {
            ...attrs,
            "supportClaw.context": "used",
          });
        }
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "supportClaw.tokens.input": usage.input ?? 0,
          "supportClaw.tokens.output": usage.output ?? 0,
          "supportClaw.tokens.cache_read": usage.cacheRead ?? 0,
          "supportClaw.tokens.cache_write": usage.cacheWrite ?? 0,
          "supportClaw.tokens.total": usage.total ?? 0,
        };
        ...
        const span = spanWithDuration("supportClaw.model.usage", spanAttrs, evt.durationMs, {
      ...
      const recordWebhookReceived = (
        ...
        const attrs = {
          "supportClaw.channel": evt.channel ?? "unknown",
          "supportClaw.webhook": evt.updateType ?? "unknown",
        };
      ...
      const recordWebhookProcessed = (
        ...
        const attrs = {
          "supportClaw.channel": lowCardinalityAttr(evt.channel),
          "supportClaw.webhook": lowCardinalityAttr(evt.updateType),
        };
        ...
        const span = spanWithDuration("supportClaw.webhook.processed", spanAttrs, evt.durationMs);
        span.end();
      };
      ...
      const recordWebhookError = (
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "supportClaw.error": redactedError,
        };
        const span = tracer.startSpan("supportClaw.webhook.error", {
      ...
      const recordMessageQueued = (
        ...
        const attrs = {
          "supportClaw.channel": lowCardinalityAttr(evt.channel),
          "supportClaw.source": lowCardinalityAttr(evt.source),
        };
      ...
      const recordMessageDispatchStarted = (
        ...
        trackInternalOrTrustedSpan(
          evt,
          metadata,
          spanWithDuration("supportClaw.message.processed", attrs, undefined, {
      ...
      const recordMessageDispatchCompleted = (
        ...
        const attrs = {
          "supportClaw.channel": lowCardinalityAttr(evt.channel),
          "supportClaw.outcome": evt.outcome,
          "supportClaw.reason": lowCardinalityAttr(evt.reason, "none"),
          "supportClaw.source": lowCardinalityAttr(evt.source),
        };
      ...
      const recordMessageProcessed = (
        ...
        const attrs = {
          "supportClaw.channel": lowCardinalityAttr(evt.channel),
          "supportClaw.outcome": evt.outcome ?? "unknown",
        };
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        if (evt.reason) {
          spanAttrs["supportClaw.reason"] = lowCardinalityAttr(evt.reason, "unknown");
        }
        const trackedSpan = getTrackedInternalOrTrustedSpan(evt, metadata);
        const span =
          trackedSpan ??
          spanWithDuration("supportClaw.message.processed", spanAttrs, evt.durationMs, {
            parentContext: internalOrTrustedExplicitParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
      ...
      const messageDeliveryAttrs = (
        ...
        "supportClaw.channel": lowCardinalityAttr(evt.channel),
        "supportClaw.delivery.kind": lowCardinalityAttr(evt.deliveryKind, "other"),
      });
      ...
      const recordMessageDeliveryCompleted = (
        ...
        const span = spanWithDuration(
          "supportClaw.message.delivery",
          {
            ...attrs,
            "supportClaw.delivery.result_count": evt.resultCount,
          },
          evt.durationMs,
      ...
      const recordMessageDeliveryError = (
        ...
        const attrs = {
          ...messageDeliveryAttrs(evt),
          "supportClaw.outcome": "error",
          "supportClaw.errorCategory": lowCardinalityAttr(evt.errorCategory, "other"),
        };
        messageDeliveryDurationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        }
        const span = spanWithDuration("supportClaw.message.delivery", attrs, evt.durationMs, {
      ...
      const recordRunStarted = (
        ...
        const span = trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("supportClaw.run", spanAttrs, undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordLaneEnqueue = (
        ...
        const attrs = { "supportClaw.lane": lowCardinalityQueueLaneAttr(evt.lane) };
      ...
      const recordSessionState = (
        ...
        const attrs: Record<string, string> = { "supportClaw.state": evt.state };
        if (evt.reason) {
          attrs["supportClaw.reason"] = redactSensitiveText(evt.reason);
        }
      ...
      const recordSessionTurnCreated = (
        ...
        sessionTurnCreatedCounter.add(1, {
          "supportClaw.agent": lowCardinalityAttr(evt.agentId, "unknown"),
          "supportClaw.channel": lowCardinalityAttr(evt.channel, "unknown"),
          "supportClaw.trigger": evt.trigger,
        });
      };
      ...
      const recordSessionStuck = (
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        spanAttrs["supportClaw.queueDepth"] = evt.queueDepth ?? 0;
        spanAttrs["supportClaw.ageMs"] = evt.ageMs;
        const span = tracer.startSpan("supportClaw.session.stuck", { attributes: spanAttrs });
      ...
      const sessionRecoveryAttrs = (evt: SessionRecoveryDiagnosticEvent) => {
        ...
        if (evt.activeWorkKind) {
          attrs["supportClaw.active_work_kind"] = evt.activeWorkKind;
        }
        return attrs;
      };
      ...
      const recordSessionRecoveryRequested = (
        ...
        attrs["supportClaw.action"] = evt.allowActiveAbort ? "abort" : "recover";
      ...
      const recordSessionRecoveryCompleted = (
        ...
        const attrs = sessionRecoveryAttrs(evt);
        attrs["supportClaw.status"] = evt.status;
        attrs["supportClaw.action"] = lowCardinalityAttr(evt.action, "unknown");
        if (evt.outcomeReason) {
          attrs["supportClaw.reason"] = redactSensitiveText(evt.outcomeReason);
        }
      ...
      const talkEventAttrs = (evt: TalkDiagnosticEvent): Record<string, string> => ({
        "supportClaw.talk.brain": lowCardinalityAttr(evt.brain),
        "supportClaw.talk.event_type": lowCardinalityAttr(evt.talkEventType),
        "supportClaw.talk.mode": lowCardinalityAttr(evt.mode),
        "supportClaw.talk.provider": lowCardinalityAttr(evt.provider),
        "supportClaw.talk.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordRunAttempt = (evt: Extract<DiagnosticEventPayload, { type: "run.attempt" }>) => {
        runAttemptCounter.add(1, { "supportClaw.attempt": evt.attempt });
      };

      const toolLoopAttrs = (
        evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>,
      ): Record<string, string | number> => ({
        "supportClaw.toolName": lowCardinalityAttr(evt.toolName, "tool"),
        "supportClaw.loop.level": evt.level,
        "supportClaw.loop.action": evt.action,
        "supportClaw.loop.detector": evt.detector,
        "supportClaw.loop.count": evt.count,
        ...(evt.pairedToolName
          ? { "supportClaw.loop.paired_tool": lowCardinalityAttr(evt.pairedToolName, "tool") }
          : {}),
      });
      ...
      const recordToolLoop = (evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>) => {
        ...
        const span = spanWithDuration("supportClaw.tool.loop", attrs, 0, { endTimeMs: evt.ts });
      ...
      const recordMemoryPressure = (
        ...
        const attrs = {
          "supportClaw.memory.level": evt.level,
          "supportClaw.memory.reason": evt.reason,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "supportClaw.memory.rss_bytes": evt.memory.rssBytes,
          "supportClaw.memory.heap_used_bytes": evt.memory.heapUsedBytes,
          "supportClaw.memory.heap_total_bytes": evt.memory.heapTotalBytes,
          "supportClaw.memory.external_bytes": evt.memory.externalBytes,
          "supportClaw.memory.array_buffers_bytes": evt.memory.arrayBuffersBytes,
          ...(evt.thresholdBytes !== undefined
            ? { "supportClaw.memory.threshold_bytes": evt.thresholdBytes }
            : {}),
          ...(evt.rssGrowthBytes !== undefined
            ? { "supportClaw.memory.rss_growth_bytes": evt.rssGrowthBytes }
            : {}),
          ...(evt.windowMs !== undefined ? { "supportClaw.memory.window_ms": evt.windowMs } : {}),
        };
        const span = spanWithDuration("supportClaw.memory.pressure", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
      ...
      const recordRunCompleted = (
        ...
        if (evt.channel) {
          attrs["supportClaw.channel"] = evt.channel;
        }
        if (evt.blockedBy) {
          attrs["supportClaw.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        durationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.blockedBy) {
          spanAttrs["supportClaw.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        if (evt.errorCategory) {
          spanAttrs["supportClaw.errorCategory"] = lowCardinalityAttr(evt.errorCategory, "other");
        }
        ...
        const span =
          trackedSpan ??
          spanWithDuration("supportClaw.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const harnessRunMetricAttrs = (evt: HarnessRunDiagnosticEvent) => ({
        "supportClaw.harness.id": lowCardinalityAttr(evt.harnessId, "unknown"),
        "supportClaw.harness.plugin": lowCardinalityAttr(evt.pluginId),
        ...(evt.type === "harness.run.started"
          ? {}
          : {
              "supportClaw.outcome": evt.type === "harness.run.error" ? "error" : evt.outcome,
            }),
        "supportClaw.provider": lowCardinalityAttr(evt.provider, "unknown"),
        "supportClaw.model": lowCardinalityAttr(evt.model, "unknown"),
        ...(evt.channel ? { "supportClaw.channel": lowCardinalityAttr(evt.channel) } : {}),
      });
      ...
      const recordHarnessRunStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("supportClaw.harness.run", harnessRunMetricAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordHarnessRunCompleted = (
        ...
        if (evt.resultClassification) {
          spanAttrs["supportClaw.harness.result_classification"] = lowCardinalityAttr(
            evt.resultClassification,
          );
        }
        if (typeof evt.yieldDetected === "boolean") {
          spanAttrs["supportClaw.harness.yield_detected"] = evt.yieldDetected;
        }
        if (evt.itemLifecycle) {
          spanAttrs["supportClaw.harness.items.started"] = evt.itemLifecycle.startedCount;
          spanAttrs["supportClaw.harness.items.completed"] = evt.itemLifecycle.completedCount;
          spanAttrs["supportClaw.harness.items.active"] = evt.itemLifecycle.activeCount;
        }
        const trustedTrace = trustedTraceContext(evt, metadata);
        const trackedSpan = trustedTrace?.spanId
          ? activeTrustedSpans.get(trustedTrace.spanId)
        ...
        const span =
          trackedSpan ??
          spanWithDuration("supportClaw.harness.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordHarnessRunError = (
        ...
        const attrs = {
          ...harnessRunMetricAttrs(evt),
          "supportClaw.harness.phase": evt.phase,
          "supportClaw.errorCategory": errorType,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...
          ...(evt.cleanupFailed ? { "supportClaw.harness.cleanup_failed": true } : {}),
        };
        const span =
      ...
      const recordContextAssembled = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          "supportClaw.context.message_count": evt.messageCount,
          "supportClaw.context.history_text_chars": evt.historyTextChars,
          "supportClaw.context.history_image_blocks": evt.historyImageBlocks,
          "supportClaw.context.max_message_text_chars": evt.maxMessageTextChars,
          "supportClaw.context.system_prompt_chars": evt.systemPromptChars,
          "supportClaw.context.prompt_chars": evt.promptChars,
          "supportClaw.context.prompt_images": evt.promptImages,
        };
        addRunAttrs(spanAttrs, evt);
        if (evt.contextTokenBudget !== undefined) {
          spanAttrs["supportClaw.context.token_budget"] = evt.contextTokenBudget;
        }
        if (evt.reserveTokens !== undefined) {
          spanAttrs["supportClaw.context.reserve_tokens"] = evt.reserveTokens;
        }
        const span = spanWithDuration("supportClaw.context.assembled", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const recordModelFailover = (
        ...
        const metricAttrs: Record<string, string> = {
          "supportClaw.failover.reason": lowCardinalityAttr(evt.reason, "unknown"),
          "supportClaw.failover.suspended":
            evt.suspended === undefined ? "unknown" : String(evt.suspended),
          "supportClaw.lane": lowCardinalityQueueLaneAttr(evt.lane, "unknown"),
          "supportClaw.model": lowCardinalityAttr(evt.fromModel),
          "supportClaw.provider": lowCardinalityAttr(evt.fromProvider),
          "supportClaw.failover.to_model": lowCardinalityAttr(evt.toModel),
          "supportClaw.failover.to_provider": lowCardinalityAttr(evt.toProvider),
        };
        ...
        if (evt.fromProvider) {
          spanAttrs["supportClaw.provider"] = evt.fromProvider;
        }
        if (evt.fromModel) {
          spanAttrs["supportClaw.model"] = evt.fromModel;
        }
        if (evt.toProvider) {
          spanAttrs["supportClaw.failover.to_provider"] = evt.toProvider;
        }
        if (evt.toModel) {
          spanAttrs["supportClaw.failover.to_model"] = evt.toModel;
        }
        if (evt.lane) {
          spanAttrs["supportClaw.lane"] = lowCardinalityQueueLaneAttr(evt.lane, "unknown");
        }
        if (evt.suspended !== undefined) {
          spanAttrs["supportClaw.failover.suspended"] = evt.suspended;
        }
        if (evt.cascadeDepth !== undefined) {
          spanAttrs["supportClaw.failover.cascade_depth"] = evt.cascadeDepth;
        }
        const span = spanWithDuration("supportClaw.model.failover", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const modelCallMetricAttrs = (evt: ModelCallLifecycleDiagnosticEvent) => ({
        "supportClaw.provider": evt.provider,
        "supportClaw.model": evt.model,
        "supportClaw.api": lowCardinalityAttr(evt.api),
        "supportClaw.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordModelCallStarted = (
        ...
        if (evt.api) {
          spanAttrs["supportClaw.api"] = evt.api;
        }
        if (evt.transport) {
          spanAttrs["supportClaw.transport"] = evt.transport;
        }
        trackTrustedSpan(
          evt,
          metadata,
      ...
      const recordModelCallError = (
        ...
        const metricAttrs = {
          ...
          "supportClaw.errorCategory": errorType,
          ...(evt.failureKind
            ? { "supportClaw.failureKind": lowCardinalityAttr(evt.failureKind, "other") }
            : {}),
        };
        modelCallDurationHistogram.record(evt.durationMs, metricAttrs);
        recordModelCallSizeTimingMetrics(evt, metricAttrs);
        ...
        if (evt.failureKind) {
          spanAttrs["supportClaw.failureKind"] = lowCardinalityAttr(evt.failureKind, "other");
        }
        assignGenAiModelCallAttrs(spanAttrs, evt);
        if (evt.api) {
          spanAttrs["supportClaw.api"] = evt.api;
      ...
      const toolExecutionBaseAttrs = (
        ...
        "supportClaw.toolName": evt.toolName,
        "supportClaw.tool.source": lowCardinalityAttr(evt.toolSource, "core"),
        "gen_ai.tool.name": evt.toolName,
        ...(evt.toolOwner ? { "supportClaw.tool.owner": lowCardinalityAttr(evt.toolOwner) } : {}),
      ...
      const skillUsedAttrs = (
        ...
        "supportClaw.skill.name": lowCardinalityAttr(evt.skillName, "skill"),
        "supportClaw.skill.source": lowCardinalityAttr(evt.skillSource),
        "supportClaw.skill.activation": lowCardinalityAttr(evt.activation),
        ...(evt.agentId ? { "supportClaw.agent": lowCardinalityAttr(evt.agentId) } : {}),
        ...(evt.toolName ? { "supportClaw.toolName": lowCardinalityAttr(evt.toolName, "tool") } : {}),
      });
      ...
      const recordSkillUsed = (
        ...
        addRunAttrs(spanAttrs, evt);
        const span = spanWithDuration("supportClaw.skill.used", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordToolExecutionStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("supportClaw.tool.execution", toolExecutionBaseAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordToolExecutionCompleted = (
        ...
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("supportClaw.tool.execution", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordToolExecutionError = (
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.errorCode) {
          spanAttrs["supportClaw.errorCode"] = lowCardinalityAttr(evt.errorCode, "other");
        }
        assignOtelToolContentAttributes(spanAttrs, toolContent, contentCapturePolicy);
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
      ...
      const recordToolExecutionBlocked = (
        ...
        toolExecutionBlockedCounter.add(1, {
          ...toolExecutionBaseAttrs(evt),
          "supportClaw.deniedReason": lowCardinalityAttr(evt.deniedReason, "other"),
        });
        if (!tracesEnabled) {
          return;
        }
        const spanAttrs: Record<string, string | number | boolean> = {
          ...toolExecutionBaseAttrs(evt),
          "supportClaw.outcome": "blocked",
          "supportClaw.deniedReason": lowCardinalityAttr(evt.deniedReason, "other"),
        };
        addRunAttrs(spanAttrs, evt);
        const span = spanWithDuration("supportClaw.tool.execution", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordPayloadLarge = (
        ...
        const attrs = {
          "supportClaw.payload.action": evt.action,
          "supportClaw.payload.surface": lowCardinalityAttr(evt.surface, "unknown"),
          "supportClaw.channel": lowCardinalityAttr(evt.channel, "none"),
          "supportClaw.plugin": lowCardinalityAttr(evt.pluginId, "none"),
          "supportClaw.reason": lowCardinalityAttr(evt.reason, "none"),
        };
      ...
      const recordExecProcessCompleted = (
        ...
        const attrs: Record<string, string | number> = {
          "supportClaw.exec.target": evt.target,
          "supportClaw.exec.mode": evt.mode,
          "supportClaw.outcome": evt.outcome,
        };
        if (evt.failureKind) {
          attrs["supportClaw.failureKind"] = evt.failureKind;
        }
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "supportClaw.exec.command_length": evt.commandLength,
        };
        if (typeof evt.exitCode === "number") {
          spanAttrs["supportClaw.exec.exit_code"] = evt.exitCode;
        }
        if (evt.exitSignal) {
          spanAttrs["supportClaw.exec.exit_signal"] = lowCardinalityAttr(evt.exitSignal, "other");
        }
        if (evt.timedOut !== undefined) {
          spanAttrs["supportClaw.exec.timed_out"] = evt.timedOut;
        }

        const span = spanWithDuration("supportClaw.exec", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
      ...
      const recordHeartbeat = (
        ...
        queueDepthHistogram.record(evt.queued, { "supportClaw.channel": "heartbeat" });
      };
      ...
      const recordLivenessWarning = (
        ...
        const attrs = {
          "supportClaw.liveness.reason": lowCardinalityAttr(reason, "unknown"),
        };
        livenessWarningCounter.add(1, attrs);
        queueDepthHistogram.record(evt.queued, { "supportClaw.channel": "liveness" });
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "supportClaw.liveness.active": evt.active,
          "supportClaw.liveness.waiting": evt.waiting,
          "supportClaw.liveness.queued": evt.queued,
          "supportClaw.liveness.interval_ms": evt.intervalMs,
          ...(evt.eventLoopDelayP99Ms !== undefined
            ? { "supportClaw.liveness.event_loop_delay_p99_ms": evt.eventLoopDelayP99Ms }
            : {}),
          ...(evt.eventLoopDelayMaxMs !== undefined
            ? { "supportClaw.liveness.event_loop_delay_max_ms": evt.eventLoopDelayMaxMs }
            : {}),
          ...(evt.eventLoopUtilization !== undefined
            ? { "supportClaw.liveness.event_loop_utilization": evt.eventLoopUtilization }
            : {}),
          ...(evt.cpuUserMs !== undefined
            ? { "supportClaw.liveness.cpu_user_ms": evt.cpuUserMs }
            : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "supportClaw.liveness.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "supportClaw.liveness.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "supportClaw.liveness.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        const span = spanWithDuration("supportClaw.liveness.warning", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
        span.setStatus({
          code: SpanStatusCode.ERROR,
      ...
      const recordDiagnosticPhaseCompleted = (
        ...
        const spanAttrs: Record<string, string | number> = {
          "supportClaw.phase": lowCardinalityAttr(evt.name, "unknown"),
          ...(evt.cpuUserMs !== undefined ? { "supportClaw.phase.cpu_user_ms": evt.cpuUserMs } : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "supportClaw.phase.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "supportClaw.phase.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "supportClaw.phase.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        for (const [key, value] of Object.entries(evt.details ?? {})) {
          spanAttrs[`supportClaw.phase.detail.${key}`] =
            typeof value === "boolean" ? String(value) : value;
        }
        const span = spanWithDuration("supportClaw.diagnostic.phase", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      };
      ...
      const recordTelemetryExporter = (
        ...
        telemetryExporterCounter.add(1, {
          "supportClaw.exporter": lowCardinalityAttr(evt.exporter, "unknown"),
          "supportClaw.signal": evt.signal,
          "supportClaw.status": evt.status,
          ...(evt.reason ? { "supportClaw.reason": evt.reason } : {}),
          ...(evt.errorCategory
            ? { "supportClaw.errorCategory": lowCardinalityAttr(evt.errorCategory, "other") }
            : {}),
        });
  ...
  } satisfies SupportClawPluginService;
}
