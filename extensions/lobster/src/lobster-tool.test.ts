...
import type { SupportClawPluginApi, SupportClawPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<SupportClawPluginApi> = {}): SupportClawPluginApi {
...
function fakeCtx(overrides: Partial<SupportClawPluginToolContext> = {}): SupportClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", () => {
    const api = fakeApi();
    const factoryTool = (ctx: SupportClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
