...
- Extension production code should import from `supportClaw/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `supportClaw.plugin.json` and the package
  `supportClaw` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `supportClaw/plugin-sdk/*`.
