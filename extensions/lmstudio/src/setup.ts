...
import { withAgentModelAliases } from "supportClaw/plugin-sdk/provider-onboard";
import {
...
import { WizardCancelledError, type WizardPrompter } from "supportClaw/plugin-sdk/setup";
...
function resolveLmstudioSetupDefaultBaseUrl(env: NodeJS.ProcessEnv = process.env): string {
  return isTruthyEnvValue(env.SUPPORT_CLAW_DOCKER_SETUP)
    ? LMSTUDIO_DOCKER_HOST_BASE_URL
    : LMSTUDIO_DEFAULT_BASE_URL;
}
...
function stripLmstudioStoredAuthConfig(cfg: SupportClawConfig): SupportClawConfig {
...
function mergeDiscoveredLmstudioAllowlistEntries(params: {
  existing?: NonNullable<NonNullable<SupportClawConfig["agents"]>["defaults"]>["models"];
  discoveredModels: ModelDefinitionConfig[];
}) {
