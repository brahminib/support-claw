...
import { CUSTOM_LOCAL_AUTH_MARKER } from "supportClaw/plugin-sdk/provider-auth";
import type { SupportClawConfig } from "supportClaw/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "supportClaw/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "supportClaw/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "supportClaw/plugin-sdk/provider-setup";
import type { WizardPrompter } from "supportClaw/plugin-sdk/setup";
...
vi.mock("supportClaw/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("supportClaw/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("supportClaw/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("supportClaw/plugin-sdk/provider-auth");
  vi.doUnmock("supportClaw/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): SupportClawConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.SUPPORT_CLAW_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("SUPPORT_CLAW_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
