---
name: canvas
description: "Present HTML on connected SupportClaw node canvases, navigate/eval/snapshot, and debug canvas host URLs."
metadata: { "supportClaw": { "emoji": "🖼️" } }
---
...
Active config: `$SUPPORT_CLAW_CONFIG_PATH` or `~/.supportClaw/supportClaw.json`.
...
4. Present the hosted URL: `/__supportClaw__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__supportClaw__/canvas/index.html
http://<gateway-host>:<gateway.port>/__supportClaw__/canvas/games/snake.html
```

Path mapping:

- `/__supportClaw__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__supportClaw__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`
