...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.SUPPORT_CLAW_SPARSE_PROFILE || process.env.SUPPORT_CLAW_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because SUPPORT_CLAW_A2UI_SKIP_MISSING=1 or SUPPORT_CLAW_SPARSE_PROFILE is set.",
      );
      return;
    }
