...
import { defaultRuntime } from "supportClaw/plugin-sdk/runtime-env";
...
describe("canvas host", () => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-canvas-fixtures-"));
  });
  ...
  it("injects live reload script", () => {
    ...
    expect(out).toContain("supportClawCanvasA2UIAction");
    expect(out).toContain("supportClawSendUserAction");
  });
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(response.body).toContain("supportClawSendUserAction");
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    await fs.writeFile(
      ...
      `<supportClaw-a2ui-host></supportClaw-a2ui-host>
<script>supportClawCanvasA2UIAction</script>`,
      "utf8",
    );
    await fs.writeFile(path.join(a2uiRoot, "a2ui.bundle.js"), "window.supportClawA2UI = {};", "utf8");
    ...
    try {
      ...
      expect(html).toContain("supportClaw-a2ui-host");
      expect(html).toContain("supportClawCanvasA2UIAction");

      const bundleRes = await captureA2uiFixtureResponse(a2uiRoot, `${A2UI_PATH}/a2ui.bundle.js`);
      const js = bundleRes.body;
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("supportClawA2UI");
