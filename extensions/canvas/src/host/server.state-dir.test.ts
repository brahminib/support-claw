...
import { withStateDirEnv } from "supportClaw/plugin-sdk/test-env";
...
describe("canvas host state dir defaults", () => {
  ...
  it("uses SUPPORT_CLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("supportClaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("SupportClaw Canvas");
      } finally {
        await handler.close();
      }
    });
