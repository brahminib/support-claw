...
import { themeContext } from "@supportclaw/a2ui-theme-context";
...
class SupportClawA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.supportClawA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["supportClaw:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.supportClawCanvasA2UIAction ??
      globalThis.supportClawCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.supportClawCanvasA2UIAction) {
...
if (!customElements.get("supportClaw-a2ui-host")) {
  customElements.define("supportClaw-a2ui-host", SupportClawA2UIHost);
}
