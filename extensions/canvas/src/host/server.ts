...
import { detectMime } from "supportClaw/plugin-sdk/media-mime";
import { isTruthyEnvValue, type RuntimeEnv } from "supportClaw/plugin-sdk/runtime-env";
import { resolveStateDir } from "supportClaw/plugin-sdk/state-paths";
import {
  lowercasePreservingWhitespace,
  normalizeOptionalString,
} from "supportClaw/plugin-sdk/string-coerce-runtime";
import { ensureDir, resolveUserPath } from "supportClaw/plugin-sdk/text-utility-runtime";
...
<title>SupportClaw Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.supportClawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.supportClawCanvasA2UIAction &&
        typeof window.supportClawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.supportClawSendUserAction === "function";
  ...
  window.addEventListener("supportClaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android SupportClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.supportClawSendUserAction === "function"
        ? window.supportClawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SUPPORT_CLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>SupportClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
