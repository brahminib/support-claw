...
import type { AnyAgentTool, SupportClawConfig } from "supportClaw/plugin-sdk/plugin-entry";
...
async function writeBase64ToTempFile(params: { base64: string; ext: string }): Promise<string> {
  const dir = resolvePreferredSupportClawTmpDir();
  await fs.mkdir(dir, { recursive: true, mode: 0o700 });
  const ext = `.${normalizeCanvasSnapshotFileExtension(params.ext)}`;
  const filePath = path.join(dir, `supportClaw-canvas-snapshot-${randomUUID()}${ext}`);
