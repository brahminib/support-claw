...
export function isCanvasPluginEnabled(config?: SupportClawConfig): boolean {
  if (!config) {
    return true;
  }
...
export function isCanvasHostEnabled(config?: SupportClawConfig): boolean {
...
export const canvasConfigSchema: CanvasPluginConfigSchema = {
  ...
  uiHints: {
    ...
    "host.root": {
      ...
      help: "Directory to serve. Defaults to the SupportClaw state canvas directory.",
      advanced: true,
    },
    "host.port": {
