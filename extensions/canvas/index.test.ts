...
import type { AnyAgentTool, SupportClawPluginApi } from "supportClaw/plugin-sdk/plugin-entry";
...
function registerCanvas() {
  const routes: Array<Parameters<SupportClawPluginApi["registerHttpRoute"]>[0]> = [];
  const services: Array<Parameters<SupportClawPluginApi["registerService"]>[0]> = [];
  const resolvers: Array<Parameters<SupportClawPluginApi["registerHostedMediaResolver"]>[0]> = [];
  const tools: Array<Parameters<SupportClawPluginApi["registerTool"]>[0]> = [];
  const cliFeatures: Array<{
    registrar: Parameters<SupportClawPluginApi["registerNodeCliFeature"]>[0];
    opts: Parameters<SupportClawPluginApi["registerNodeCliFeature"]>[1];
  }> = [];
...
describe("Canvas plugin entry", () => {
  ...
  it("defers Canvas host implementation until a registered route is used", async () => {
    ...
    await routes[0]?.handler({ url: "/__supportClaw__/canvas" } as never, {} as never);
  ...
  it("defers Canvas resolver, CLI, and tool implementations until use", async () => {
    ...
    await expect(resolvers[0]?.("/__supportClaw__/canvas/documents/id/index.html")).resolves.toBe(
      "/tmp/canvas-asset",
    );
