...
import type { PluginStateSyncKeyedStore } from "supportClaw/plugin-sdk/plugin-state-runtime";
...
export function createCopilotAgentHarness(
  ...
  >();
  // Maps SupportClaw session id (from AgentHarnessAttemptParams.sessionId) to
  ...
  return {
    ...
    async runAttempt(params: AgentHarnessAttemptParams): Promise<AgentHarnessAttemptResult> {
      const attemptPromise = (async () => {
        ...
        const supportClawSessionId =
          typeof params.sessionId === "string" ? params.sessionId : undefined;

        // Dogfood finding #4: reuse the SDK session across turns within
        // the same SupportClaw session so that the GitHub Copilot agent runtime's prompt
        ...
        const compactionCleanupPending =
          supportClawSessionId !== undefined && hasPendingDeferredCompactionCleanup(supportClawSessionId);
        const replayBlocked =
          supportClawSessionId !== undefined &&
          (compactionCleanupPending || resetBlockedStoredSessions.has(supportClawSessionId));
        const tracked =
          supportClawSessionId && !replayBlocked ? trackedSessions.get(supportClawSessionId) : undefined;
        const stored = supportClawSessionId
          ? replayBlocked
            ? undefined
            : lookupStoredBinding(options?.sessionStore, supportClawSessionId)
          : undefined;
        ...
        return runCopilotAttempt(effectiveParams, {
          pool,
          onSessionEstablished: supportClawSessionId
            ? ({
                sdkSessionId,
                pooledClient,
                sessionConfig,
                ...
                trackedSessions.set(supportClawSessionId, {
                  sdkSessionId,
                ...
                registerStoredBinding(options?.sessionStore, supportClawSessionId, {
                  schemaVersion: 2,
                  sdkSessionId,
                  compatKey: currentCompatKey,
                  compactKey: currentCompactKey,
                ...
                resetBlockedStoredSessions.delete(supportClawSessionId);
              }
            : undefined,
          onDeferredCompaction: supportClawSessionId
            ? ({
                abort,
                cleanup,
                sdkSessionId,
                ...
                const trackedBinding = trackedSessions.get(supportClawSessionId);
                const storedBinding = lookupStoredBinding(options?.sessionStore, supportClawSessionId);
                ...
                trackDeferredCompactionCleanup({
                  abort,
                  cleanup,
                  sessionId: supportClawSessionId,
                  sdkSessionId,
                });
                ...
                resetBlockedStoredSessions.add(supportClawSessionId);
                void cleanup.then((outcome) => {
                  const currentTracked = trackedSessions.get(supportClawSessionId);
                  const currentStored = lookupStoredBinding(
                    options?.sessionStore,
                    supportClawSessionId,
                  );
                  ...
                  if (stillOwnsTrackedSession) {
                    trackedSessions.delete(supportClawSessionId);
                  }
                  if (stillOwnsStoredSession) {
                    deleteStoredBinding(options?.sessionStore, supportClawSessionId);
                  }
                  if (stillOwnsTrackedSession || stillOwnsStoredSession) {
                    resetBlockedStoredSessions.add(supportClawSessionId);
                  }
    ...
    async reset(params: AgentHarnessResetParams): Promise<void> {
      const supportClawSessionId = typeof params.sessionId === "string" ? params.sessionId : undefined;
      if (!supportClawSessionId) {
        return;
      }
      ...
      const tracked = trackedSessions.get(supportClawSessionId);
      const stored = lookupStoredBinding(options?.sessionStore, supportClawSessionId);
      resetBlockedStoredSessions.add(supportClawSessionId);
      await abortDeferredCompactionCleanups(supportClawSessionId);
      const currentStored = lookupStoredBinding(options?.sessionStore, supportClawSessionId);
      const stillOwnsStoredSession =
        stored !== undefined && currentStored?.sdkSessionId === stored.sdkSessionId;
      if (stillOwnsStoredSession) {
        if (deleteStoredBinding(options?.sessionStore, supportClawSessionId)) {
          resetBlockedStoredSessions.delete(supportClawSessionId);
        }
      } else {
        resetBlockedStoredSessions.delete(supportClawSessionId);
      ...
      if (trackedSessions.get(supportClawSessionId)?.sdkSessionId === tracked.sdkSessionId) {
        trackedSessions.delete(supportClawSessionId);
      }
      try {
    ...
    async compact(
      ...
      // The SDK owns Copilot history compaction. SupportClaw only resumes
      // the tracked SDK session and calls the session-scoped RPC; durable
      // SupportClaw session/transcript state stays in SQLite, with no marker
      ...
      if (hasPendingDeferredCompactionCleanup(supportClawSessionId)) {
        return {
          ok: false,
          compacted: false,
      ...
      if (!compatibleTracked) {
        ...
        // needs the live SessionConfig with SupportClaw hooks/tools, so preserve the
