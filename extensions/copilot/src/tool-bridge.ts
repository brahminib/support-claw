...
type CreateSupportClawCodingTools =
  (typeof import("supportClaw/plugin-sdk/agent-harness"))["createSupportClawCodingTools"];
type SupportClawCodingToolsOptions = NonNullable<Parameters<CreateSupportClawCodingTools>[0]>;
 ...
 * authoritative `createSupportClawCodingTools({...})` call shape).
 *
 * Declared as `Partial<EmbeddedRunAttemptParams>` (imported from the
 * `supportClaw/plugin-sdk/agent-harness-runtime` boundary, *not* from
...
export interface CopilotToolBridgeInput {
   ...
   * model, and run-trace fields to `createSupportClawCodingTools` so the
  ...
  createSupportClawCodingTools?: (opts: unknown) => AnyAgentTool[] | Promise<AnyAgentTool[]>;
  beforeExecute?: (ctx: {
    toolName: string;
    toolCallId: string;
    args: unknown;
...
export async function createCopilotToolBridge(
  ...
  const createSupportClawCodingTools =
    input.createSupportClawCodingTools ??
    (await import("supportClaw/plugin-sdk/agent-harness")).createSupportClawCodingTools;

  const toolOptions = buildSupportClawCodingToolsOptions(input, effectiveToolPlan);

  let sourceTools: unknown;
  try {
    sourceTools = await createSupportClawCodingTools(toolOptions);
  } catch (error: unknown) {
    throw createError(
      `[copilot-tool-bridge] createSupportClawCodingTools failed: ${toError(error).message}`,
      error,
    );
  }
  ...
  if (!Array.isArray(sourceTools)) {
    throw new Error(
      "[copilot-tool-bridge] createSupportClawCodingTools must return an array of tools",
    );
  }
  ...
  return {
    sdkTools: filteredTools.map((sourceTool) =>
      convertSupportClawToolToSdkTool(sourceTool, {
...
/**
 * Builds the full `createSupportClawCodingTools` options bag mirroring the
 * PI in-tree call at `src/agents/pi-embedded-runner/run/attempt.ts:1029-1117`.
 *
 * Why PI parity matters: bridged SupportClaw tools register with the SDK
 * as `overridesBuiltInTool: true, skipPermission: true` (see
 * `convertSupportClawToolToSdkTool` below). That means the wrapped-tool
 * enforcement layer
 ...
 */
function buildSupportClawCodingToolsOptions(
  input: CopilotToolBridgeInput,
  toolPlan: ReturnType<typeof resolveEmbeddedAttemptToolConstructionPlan>,
): SupportClawCodingToolsOptions {
  ...
  const modelCompat =
    ...
    typeof model.compat === "object"
      ? (model.compat as SupportClawCodingToolsOptions["modelCompat"])
      : undefined;
...
export function convertSupportClawToolToSdkTool(
  sourceTool: AnyAgentTool,
  ctx: {
    abortSignal?: AbortSignal;
  ...
  return {
    ...
    // SupportClaw owns its bridged tools by design (the harness docs:
    // "SupportClaw still owns ... SupportClaw dynamic tools (bridged)"). The bundled
    // Copilot CLI ships built-in tools whose names (edit, read, write, bash,
    // ...) collide with SupportClaw's coding-tool set. Mark every bridged tool as
    ...
    // same name." SupportClaw's tool layer is the source of truth for these
    // names within a copilot attempt.
    overridesBuiltInTool: true,
    parameters: sourceTool.parameters as Record<string, unknown> | undefined,
    // Bridged SupportClaw tools enforce their own permission/policy decisions
    ...
    // `kind: "custom-tool"` would either short-circuit SupportClaw's richer
    // enforcement (if we allow-all) or block every call (if we
    // reject-all) — neither matches PI parity. The in-tree codex harness
    // takes the same approach: bridged SupportClaw tools are wrapped with
