...
import type { AgentHarnessAttemptParams } from "supportClaw/plugin-sdk/agent-harness-runtime";
...
vi.mock("supportClaw/plugin-sdk/agent-harness", async (importOriginal) => {
  const actual = await importOriginal<typeof import("supportClaw/plugin-sdk/agent-harness")>();

  return {
    ...actual,
    createSupportClawCodingTools: vi.fn(() => [
      {
...
const LIVE = isLiveTestEnabled(["SUPPORT_CLAW_COPILOT_AGENT_LIVE_TEST"]);
const TOKEN =
  process.env.SUPPORT_CLAW_COPILOT_AGENT_LIVE_TOKEN ||
...
describeLive("copilot agent runtime live smoke", () => {
  it("runs one turn on gpt-4.1 with one custom tool", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "supportClaw-copilot-live-"));
