...
describe("resolveCopilotAuth - copilotHome resolution", () => {
  ...
  it("synthesises per-agent default from homeDir when no path is given", () => {
    ...
    expect(result.copilotHome).toBe(
      resolve(join(FAKE_HOME, ".supportClaw", "agents", "agent-1", "copilot")),
    );
  });

  it("respects SUPPORT_CLAW_HOME env var as the home root", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: { SUPPORT_CLAW_HOME: "/custom/supportClaw" } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.copilotHome).toBe(
      resolve(join("/custom/supportClaw", ".supportClaw", "agents", "agent-1", "copilot")),
    );
  });
  ...
  it("uses the default agent id when agentId is invalid/missing", () => {
    ...
    expect(result.copilotHome).toBe(
      resolve(join(FAKE_HOME, ".supportClaw", "agents", COPILOT_DEFAULT_AGENT_ID, "copilot")),
    );
  });
...
describe("resolveCopilotAuth - contract-resolved auth (resolvedApiKey + authProfileId)", () => {
  ...
  it("contract resolvedApiKey takes precedence over env fallback", () => {
    const result = resolveCopilotAuth({
      ...
      authProfileId: "p",
      env: {
        SUPPORT_CLAW_GITHUB_TOKEN: "env-should-be-ignored",
...
describe("resolveCopilotAuth - env var fallbacks", () => {
  ...
  it("SUPPORT_CLAW_GITHUB_TOKEN takes precedence over GITHUB_TOKEN", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: {
        SUPPORT_CLAW_GITHUB_TOKEN: "supportClaw-tok",
        GITHUB_TOKEN: "github-tok",
      } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.gitHubToken).toBe("supportClaw-tok");
    expect(result.authProfileId).toBe("env:SUPPORT_CLAW_GITHUB_TOKEN");
    expect(result.authProfileVersion).toBe(tokenFingerprint("supportClaw-tok"));
  });
  ...
  it("SUPPORT_CLAW_GITHUB_TOKEN takes precedence over COPILOT_GITHUB_TOKEN, GH_TOKEN and GITHUB_TOKEN", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: {
        SUPPORT_CLAW_GITHUB_TOKEN: "supportClaw-tok",
  ...
  it("explicit auth.useLoggedInUser=true wins over env tokens", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      auth: { useLoggedInUser: true },
      env: { SUPPORT_CLAW_GITHUB_TOKEN: "env-tok" } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.authMode).toBe("useLoggedInUser");
  });
...
describe("resolveCopilotAuth - defaults wiring", () => {
  ...
  it("uses os.homedir() when homeDir is not injected", () => {
    ...
    expect(result.copilotHome.endsWith(join(".supportClaw", "agents", "agent-1", "copilot"))).toBe(
      true,
    );
  });
  ...
  it("falls back to process.cwd() if homeDir throws", () => {
    ...
    expect(result.copilotHome.includes(join(".supportClaw", "agents", "agent-1", "copilot"))).toBe(
      true,
    );
  });
});
