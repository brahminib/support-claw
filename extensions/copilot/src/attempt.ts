...
// supportClaw/plugin-sdk/agent-harness-runtime declares auth, messages,
// onAssistantDelta, and initialReplayState.sdkSessionId fields. Tracked by
// project supportClaw-copilot-harness; reviewer-attempt-bridge note.
...
type AttemptParamsLike = AgentHarnessAttemptParams & {
  ...
  // current-turn user message for the SupportClaw audit transcript so
...
export interface CopilotAttemptDeps {
   ...
   * `supportClaw/plugin-sdk/agent-harness-runtime#resolveSandboxContext`, which is
   ...
   * harness uses this to track the supportClawSessionId -> sdkSessionId
...
export async function runCopilotAttempt(
  ...
  try {
    ...
    try {
      const toolBridge = await createToolBridge({
        ...
        // tool-bridge.ts buildSupportClawCodingToolsOptions().
        attemptParams: input,
        sessionRef,
        onYieldDetected: () => {
          yieldDetected = true;
    ...
    // Load SupportClaw workspace bootstrap files (SOUL.md, IDENTITY.md,
    ...
    // SupportClaw bootstrap block (SDK still loads AGENTS.md natively).
    ...
    const emitLlmInput = (prompt: string, additionalContext?: string) => {
      runAgentHarnessLlmInputHook({
        event: {
          ...
          // Copilot SDK sessions own their own transcript. SupportClaw's
    ...
    if (abortRequested || params.abortSignal?.aborted) {
      ...
      if (!bridge.recordSendResult(result) && !aborted) {
        ...
        // capability inventory. Do not call session.abort() here: SupportClaw may
  ...
  //
  // Without this synthesis the SupportClaw audit transcript never sees
  ...
  // (user/assistant/toolResult) into the SupportClaw audit transcript at
  // params.sessionFile, alongside the SDK's own session storage. The
  // SupportClaw shell (attempt-execution.ts) writes only the user prompt
  ...
  if (sessionFileForMirror && messagesSnapshot.length > 0) {
    ...
    await dualWriteCopilotTranscriptBestEffort({
      ...
      // authoritative; only the SupportClaw audit transcript would be
...
function createSessionConfig(
  ...
  return {
    ...
    // tool-bridge.ts) so 100% of tool calls go through SupportClaw's
    ...
    ...(resolvedAuth.authMode === "gitHubToken" && resolvedAuth.gitHubToken
      ...
      : {}),
    // SupportClaw workspace bootstrap plus per-turn runtime guidance
    // injected via the SDK's `systemMessage` field in append mode:
    // SDK foundation + SupportClaw context. Append keeps every SDK
    ...
    // L1036). Omitted when there is no SupportClaw-owned context so the
...
function hasMirrorIdentity(message: AgentMessage): boolean {
  const record = message as unknown as { __supportClaw?: unknown };
