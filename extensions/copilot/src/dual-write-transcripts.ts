 ...
 * SupportClaw audit transcript that sits next to (but is distinct from) the
 * SDK's own session storage.
 *
 * The SupportClaw shell (src/agents/command/attempt-execution.ts) already
 ...
 * assistant + toolResult) into the SupportClaw transcript via the same
...
export function attachCopilotMirrorIdentity<T extends AgentMessage>(
  ...
  const existing = record["__supportClaw"];
  const baseMeta =
  ...
  return {
    ...record,
    __supportClaw: { ...baseMeta, [MIRROR_IDENTITY_META_KEY]: identity },
  } as unknown as T;
}
 ...
 * the SupportClaw audit trail just misses the intermediate messages for
 * this turn.
 */
