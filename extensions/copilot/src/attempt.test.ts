...
import type { SandboxContext } from "supportClaw/plugin-sdk/agent-harness-runtime";
import {
  initializeGlobalHookRunner,
  resetGlobalHookRunner,
} from "supportClaw/plugin-sdk/hook-runtime";
...
const dualWriteMock = vi.hoisted(() => ({
  ...
  attachCopilotMirrorIdentity: <T>(message: T, identity: string): T => {
    ...
    return {
      ...record,
      __supportClaw: { ...(record["__supportClaw"] as object | undefined), mirrorIdentity: identity },
    } as unknown as T;
  },
}));
...
describe("runCopilotAttempt", () => {
  ...
  describe("workspace bootstrap (systemMessage)", () => {
    ...
    it("forwards rendered bootstrap instructions into SDK SessionConfig.systemMessage (append mode)", async () => {
      ...
      // while layering SupportClaw context after it. See
    ...
    it("forwards rendered bootstrap instructions to resumeSession on the resume path", async () => {
      ...
      // otherwise resumed sessions would silently lose SupportClaw
  ...
  describe("session-level gitHubToken (independent of client-level)", () => {
    ...
    it("SessionConfig.gitHubToken is omitted when default mode is useLoggedInUser (no auth signal)", async () => {
      ...
      const prevSupportClaw = process.env.SUPPORT_CLAW_GITHUB_TOKEN;
      const prevGithub = process.env.GITHUB_TOKEN;
      delete process.env.SUPPORT_CLAW_GITHUB_TOKEN;
      ...
      try {
        ...
        if (prevSupportClaw !== undefined) {
          process.env.SUPPORT_CLAW_GITHUB_TOKEN = prevSupportClaw;
        }
  ...
  describe("dual-write transcript mirror", () => {
    ...
    it("tags mirrored messages with copilot mirror identity per role and position", async () => {
      ...
      const args = dualWriteMock.dualWriteCopilotTranscriptBestEffort.mock.calls[0]?.[0] as {
        messages: Array<{ role: string; __supportClaw?: { mirrorIdentity?: string } }>;
      };
      for (const [index, message] of args.messages.entries()) {
        ...
        const identity = message["__supportClaw"]?.mirrorIdentity ?? "";
    ...
    // SupportClaw audit transcript (mirrors codex event-projector pattern).
    //
    // Without this synthesis the dashboard / CLI history shows only
    // assistant bubbles — the user's typed turn is lost — because the
    // SupportClaw shell's `persistTextTurnTranscript` skips its own user
    ...
    it("injects synthetic user message with runId:prompt identity when caller passes no history", async () => {
      ...
      const args = dualWriteMock.dualWriteCopilotTranscriptBestEffort.mock.calls[0]?.[0] as {
        messages: Array<{
          role: string;
          content: unknown;
          __supportClaw?: { mirrorIdentity?: string };
        }>;
      };
      expect(args.messages.length).toBe(2);
      expect(args.messages[0]?.role).toBe("user");
      expect(args.messages[0]?.content).toBe("what's my name?");
      expect(args.messages[0]?.["__supportClaw"]?.mirrorIdentity).toBe("run-A:prompt");
      expect(args.messages[1]?.role).toBe("assistant");
      expect(args.messages[1]?.["__supportClaw"]?.mirrorIdentity).toBe("run-A:assistant:final");
    });
    ...
    it("two attempts that share the same sdkSessionId but differ by runId produce distinct user/assistant mirror identities", async () => {
      ...
      expect(turn1User?.["__supportClaw"]?.mirrorIdentity).toBe("run-1:prompt");
      expect(turn2User?.["__supportClaw"]?.mirrorIdentity).toBe("run-2:prompt");
      expect(turn1Assistant?.["__supportClaw"]?.mirrorIdentity).toBe("run-1:assistant:final");
      expect(turn2Assistant?.["__supportClaw"]?.mirrorIdentity).toBe("run-2:assistant:final");
    });
    ...
    it("two attempts with identical prompts but different runIds remain distinct (no content-fingerprint collapse)", async () => {
      ...
      ).messages.find((m) => m.role === "user")?.["__supportClaw"]?.mirrorIdentity;
      const id2 = (
        calls[1][0] as {
          messages: Array<{ role: string; __supportClaw?: { mirrorIdentity?: string } }>;
        }
  ...
  // bypass SupportClaw's wrapped-tool enforcement under any permissive
