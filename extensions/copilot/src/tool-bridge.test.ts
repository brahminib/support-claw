...
import type { AnyAgentTool, SandboxContext } from "supportClaw/plugin-sdk/agent-harness-runtime";
import { afterEach, describe, expect, it, vi } from "vitest";
import {
  createCopilotToolBridge,
  convertSupportClawToolToSdkTool,
  supportsModelTools,
} from "./tool-bridge.js";
...
describe("supportsModelTools", () => {
  it("returns true for github-copilot and false otherwise", () => {
    ...
    expect(supportsModelTools("supportClaw")).toBe(false);
...
describe("createCopilotToolBridge", () => {
  it("returns empty arrays for unsupported providers without calling the seam", async () => {
    const createSupportClawCodingTools = vi.fn(async () => [makeTool()]);
    ...
    expect(createSupportClawCodingTools).toHaveBeenCalledTimes(0);
  });

  it("forwards supported fields to injected createSupportClawCodingTools", async () => {
    ...
    expect(createSupportClawCodingTools).toHaveBeenCalledTimes(1);
    ...
    expect(createSupportClawCodingTools).toHaveBeenCalledWith(
  ...
  it("returns sdkTools and sourceTools with matching lengths", async () => {
    ...
    const result = await createCopilotToolBridge({
      agentId: "agent-1",
      createSupportClawCodingTools: async () => sourceTools,
      modelId: "gpt-4o",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("throws when createSupportClawCodingTools returns a non-array", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createSupportClawCodingTools: async () => ({ tools: [] }) as never,
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      }),
    ).rejects.toThrow("createSupportClawCodingTools must return an array");
  });

  it("throws when createSupportClawCodingTools rejects and includes the cause", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createSupportClawCodingTools: async () => {
  ...
  // F6: PI-parity tool context. The bridged SupportClaw tools register
  ...
  describe("PI-parity attempt context (F6)", () => {
    function captureCall() {
      ...
      return {
        createSupportClawCodingTools,
        getOpts: () =>
          (createSupportClawCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as Record<
            string,
            unknown
          >,
      };
    ...
    it("forwards identity, owner/policy, and channel/routing fields from attemptParams", async () => {
      const { createSupportClawCodingTools, getOpts } = captureCall();
    ...
    it("prefers the unscoped toolAuthProfileStore when building SupportClaw tools", async () => {
      const { createSupportClawCodingTools, getOpts } = captureCall();
  ...
  describe("sandbox forwarding (PR #86155 [P1])", () => {
    ...
    it("defaults sandbox to undefined and derives spawnWorkspaceDir from workspaceDir when no sandbox is passed (back-compat)", async () => {
      ...
      const opts = (createSupportClawCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        sandbox?: unknown;
    ...
    it("forwards an explicit sandbox and spawnWorkspaceDir verbatim to createSupportClawCodingTools", async () => {
  ...
  // SupportClaw attempt would suppress. These tests pin the contract.
  describe("tool-surface gating (PR #86155 [P1] round-6)", () => {
    it("short-circuits when attemptParams.disableTools is true and never calls createSupportClawCodingTools", async () => {
      const createSupportClawCodingTools = vi.fn(async () => [makeTool()]);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
    ...
    it("filters constructed tools to exactly the allowlist when toolsAllow is narrow", async () => {
      const createSupportClawCodingTools = vi.fn(async () => [
        makeTool({ name: "read" }),
        makeTool({ name: "edit" }),
    ...
    it("leaves the tool list unchanged when toolsAllow is undefined", async () => {
      ...
      const createSupportClawCodingTools = vi.fn(async () => tools);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
  ...
  describe("tool-name aliases (PR #86155 [P1] round-7)", () => {
    ...
    it("keeps core tools available for glob allowlists", async () => {
      ...
      const options = (createSupportClawCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        toolConstructionPlan?: { includeSupportClawTools?: boolean };
      };
      expect(options?.toolConstructionPlan?.includeSupportClawTools).toBe(true);
    });
...
describe("convertSupportClawToolToSdkTool", () => {
  it("throws on empty and non-string names", () => {
    expect(() => convertSupportClawToolToSdkTool(makeTool({ name: "" as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
    expect(() => convertSupportClawToolToSdkTool(makeTool({ name: 42 as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
  });

  it("throws on non-function execute", () => {
    expect(() => convertSupportClawToolToSdkTool(makeTool({ execute: "nope" as never }), {})).toThrow(
  ...
  it("preserves name, description, and parameters exactly", () => {
    ...
    const result = convertSupportClawToolToSdkTool(sourceTool, {});
  ...
  it("sets skipPermission: true so SupportClaw's wrapped-tool internal enforcement handles permission decisions (PI-parity model)", () => {
    // Per the harness docs: every bridged SupportClaw tool comes from
    // `createSupportClawCodingTools`, which already wraps each tool with
    ...
    // for kind: "custom-tool" would either short-circuit SupportClaw's
    // richer enforcement (allow-all) or block every call (reject-all).
    // Setting `skipPermission: true` lets the wrapped execute() run
    // SupportClaw's hook with the right context — mirrors codex
    // (`extensions/codex/src/app-server/dynamic-tools.ts`).
    const result = convertSupportClawToolToSdkTool(makeTool(), {}) as SdkTool & {
  ...
  it("marks every bridged tool as overridesBuiltInTool so SupportClaw owns names that collide with Copilot CLI built-ins (edit/read/write/bash/...)", () => {
    // Real-world dogfood found that supportClaw's createSupportClawCodingTools
    ...
    for (const name of ["edit", "read", "write", "bash", "live_echo"]) {
      const result = convertSupportClawToolToSdkTool(makeTool({ name }), {}) as SdkTool & {
  ...
  it("returns a failure result when the signal is already aborted", async () => {
    ...
    controller.abort();
    const sourceTool = makeTool();
    const sdkTool = convertSupportClawToolToSdkTool(sourceTool, { abortSignal: controller.signal });
  ...
  it("calls beforeExecute with the invocation context before execute", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertSupportClawToolToSdkTool(sourceTool, { beforeExecute });
  ...
  it("returns a failure result when beforeExecute throws", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertSupportClawToolToSdkTool(sourceTool, {
  ...
  it("calls prepareArguments and passes the prepared args and toolCallId to execute", async () => {
    ...
    const sdkTool = convertSupportClawToolToSdkTool(sourceTool, { onToolCompleted });
  ...
  it("converts single text content to an exact textResultForLlm", async () => {
    ...
    const sdkTool = convertSupportClawToolToSdkTool(makeTool({}, sourceResult), { onAgentToolResult });
  ...
  it("reports terminal tool results to the harness lifecycle bridge", async () => {
    ...
    const sdkTool = convertSupportClawToolToSdkTool(makeTool({}, sourceResult), {
      onToolCompleted,
    });
  ...
  it("reports thrown tool failures to the private result observer", async () => {
    ...
    const onAgentToolResult = vi.fn();
    const sdkTool = convertSupportClawToolToSdkTool(
      makeTool({
        execute: vi.fn(async () => {
          throw error;
        }),
  ...
  it("reports returned SupportClaw error results to both tool observers", async () => {
    const onAgentToolResult = vi.fn();
    const onToolCompleted = vi.fn();
    const sourceResult = {
