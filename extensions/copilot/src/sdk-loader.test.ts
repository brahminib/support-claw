...
describe("sdk-loader", () => {
  ...
  it("default fallback importer resolves and imports the installed SDK entry", async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        "module.exports = { supportClawDefaultImporterSentinel: true };",
      );
      ...
      })) as unknown as { supportClawDefaultImporterSentinel?: boolean };

      expect(sdk.supportClawDefaultImporterSentinel).toBe(true);
      expect(primaryImport).toHaveBeenCalledTimes(1);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("throws an actionable error with plugin install instructions when both probes fail", async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining("supportClaw plugins install @supportclaw/copilot"),
    });
  ...
  it("error message includes the fallback path and underlying primary error", async () => {
    ...
    expect(message).toContain("supportClaw plugins install @supportclaw/copilot");
  });
  ...
  it("resolves the fallback dir from SUPPORT_CLAW_STATE_DIR for relocated profiles", () => {
    expect(
      resolveCopilotSdkFallbackDir({
        ...process.env,
        SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-state",
      }),
    ).toBe(path.join("/tmp/supportClaw-state", "npm-runtime", "copilot"));
  });
