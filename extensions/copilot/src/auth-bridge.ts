 ...
 *     use cases.
 *   - Falls back to SUPPORT_CLAW_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<supportClawHome>/.supportClaw/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `SUPPORT_CLAW_HOME` for the home directory root.
 ...
 *   4. SUPPORT_CLAW_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with SupportClaw's own files
  ...
  const supportClawHome = readString(args.env.SUPPORT_CLAW_HOME);
  const rootHome = supportClawHome ? resolve(supportClawHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <supportClawHome>/.supportClaw/agents/<agentId>/copilot
  return resolve(join(rootHome, ".supportClaw", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // SUPPORT_CLAW_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "SUPPORT_CLAW_GITHUB_TOKEN", value: readString(env.SUPPORT_CLAW_GITHUB_TOKEN) },
