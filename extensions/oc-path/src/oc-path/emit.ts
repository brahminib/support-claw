 ...
 * `__SUPPORT_CLAW_REDACTED__` literal anywhere in the output throws
 ...
 *
 * @module @supportclaw/oc-path/emit
 */
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__SUPPORT_CLAW_REDACTED__")) {
      guardSentinel("__SUPPORT_CLAW_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
