...
describe("sentinel literal at format boundary", () => {
  it("formatOcPath rejects a struct carrying the redaction sentinel", () => {
    expect(() => formatOcPath({ file: "AGENTS.md", section: "__SUPPORT_CLAW_REDACTED__" })).toThrow(
      /sentinel literal/,
    );
  });
});
...
describe("numeric segments dispatch by node kind", () => {
  it("negative numeric key on object resolves as literal key (supportClaw#59934)", () => {
