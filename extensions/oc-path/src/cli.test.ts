/**
 * Smoke tests for the `supportClaw path` CLI handlers.
 *
...
describe("supportClaw path CLI", () => {
  ...
  describe("set", () => {
    ...
    it("CLI-S08 sets slash-deep JSONC paths and parsed JSON values", async () => {
      const filePath = join(workspaceDir, "supportClaw.json");
      writeFileSync(
        filePath,
      ...
      await pathSetCommand(
        "oc://supportClaw.json/gateway/auth/token",
      ...
      await pathSetCommand(
        "oc://supportClaw.json/agents/list/0/tools/exec/security",
        "allowlist",
        { cwd: workspaceDir, json: true },
        rt2,
      );
