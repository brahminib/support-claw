/**
 * `supportClaw path` — shell access to the OcPath substrate verbs.
 *
...
export function registerPathCli(program: Command): void {
  const path = program
    ...
    .addHelpText("after", "\nDocs: https://docs.supportClaw.ai/cli/path\n");
  ...
  // Bare `supportClaw path` prints help and exits 0 (matches the core
  // applyParentDefaultHelpAction contract — see supportClaw#73077).
  path.action(() => {
    path.outputHelp();
    process.exitCode = 0;
  });
