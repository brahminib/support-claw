...
describe("codex media understanding provider", () => {
  ...
  it("runs image understanding through a bounded Codex app-server turn", async () => {
    ...
    expect(requests[1]?.params).toEqual({
      ...
      developerInstructions:
        "You are SupportClaw's bounded image-understanding worker. Describe only the provided image content. Do not call tools, edit files, or ask follow-up questions.",
      config: {
        "features.apps": false,
  ...
  it("preserves configured WebSocket transport for media turns", async () => {
    ...
    expect(requests[1]?.params).toEqual(expect.objectContaining({ cwd: "/tmp/supportClaw-agent" }));
    expect(requests[2]?.params).toEqual(expect.objectContaining({ cwd: "/tmp/supportClaw-agent" }));
  });
  ...
  it("runs structured extraction through the same bounded Codex app-server path", async () => {
    ...
    expect(requests[1]?.params).toEqual({
      ...
      serviceName: "SupportClaw",
      developerInstructions:
        "You are SupportClaw's bounded structured-extraction worker. Return only the requested extraction. Do not call tools, edit files, ask follow-up questions, or include secrets.",
      config: {
        "features.apps": false,
        "features.code_mode": false,
        "features.code_mode_only": false,
    ...
    expect(turnParams?.cwd).toBe("/tmp/supportClaw-agent");
