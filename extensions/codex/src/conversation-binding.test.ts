...
import type { ExecApprovalsFile } from "supportClaw/plugin-sdk/exec-approvals-runtime";
import { upsertSessionEntry } from "supportClaw/plugin-sdk/session-store-runtime";
...
vi.mock("supportClaw/plugin-sdk/agent-harness-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("supportClaw/plugin-sdk/agent-harness-runtime")>();
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/exec-approvals-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("supportClaw/plugin-sdk/exec-approvals-runtime")>();
  return {
    ...actual,
    loadExecApprovals: execApprovalsRuntimeMocks.loadExecApprovals,
  };
});
vi.mock("supportClaw/plugin-sdk/agent-runtime", () => agentRuntimeMocks);
...
describe("codex conversation binding", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-codex-binding-"));
  });
  ...
  it("rejects binding when configured exec auto mode may need unrouted human approvals", async () => {
    ...
    await expect(
      ...
      "SupportClaw native Codex conversation binding cannot route interactive approvals yet",
    );
  ...
  it("blocks bound Codex app-server turns when the current SupportClaw session is sandboxed", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    await fs.writeFile(
      `${sessionFile}.codex-app-server.json`,
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because SupportClaw sandboxing is active for this session.",
        ),
      },
    });
  ...
  it("blocks bound Codex app-server turns when exec host=node is active", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because SupportClaw exec host=node is active for this session.",
        ),
      },
    });
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when the binding agent uses node exec without a session key", async () => {
    ...
    expect(result?.reply?.text).toContain("SupportClaw exec host=node is active");
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  });
  ...
  it("blocks bound Codex CLI node turns when the current SupportClaw session is sandboxed", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex CLI node conversation binding is unavailable because SupportClaw sandboxing is active for this session.",
        ),
      },
    });
  ...
  it("refreshes stale network-proxy bound app-server threads before the turn", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        ...
        threadId: "thread-old",
        cwd: tempDir,
        networkProxyProfileName: "supportClaw-network-stale",
