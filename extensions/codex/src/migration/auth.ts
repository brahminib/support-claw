...
import { loadAuthProfileStoreWithoutExternalProfiles } from "supportClaw/plugin-sdk/agent-runtime";
import {
  createMigrationItem,
  markMigrationItemConflict,
  markMigrationItemError,
  markMigrationItemSkipped,
} from "supportClaw/plugin-sdk/migration";
import type { MigrationItem, MigrationProviderContext } from "supportClaw/plugin-sdk/plugin-entry";
import {
...
type AgentDefaultModelConfigs = NonNullable<
  NonNullable<NonNullable<SupportClawConfig["agents"]>["defaults"]>["models"]
>;
...
async function buildCodexOAuthCredential(source: CodexSource): Promise<CodexAuthCredential | null> {
  ...
  } satisfies Partial<SupportClawConfig>;
...
function replaceConfigDraft(draft: SupportClawConfig, next: SupportClawConfig): void {
  for (const key of Object.keys(draft) as Array<keyof SupportClawConfig>) {
    delete draft[key];
  }
...
function existingAuthProfileConfigIsCompatible(
  existing: NonNullable<NonNullable<SupportClawConfig["auth"]>["profiles"]>[string],
...
function hasCurrentAuthProfileConfigConflict(
  ...
  try {
    config = (ctx.runtime?.config?.current?.() as SupportClawConfig | undefined) ?? config;
  } catch {
...
function applyDefaultModelIfMissing(cfg: SupportClawConfig): SupportClawConfig {
...
async function applyCodexAuthProfileConfig(
  ctx: MigrationProviderContext,
  profile: CodexAuthProfileConfig,
  applyConfig: (config: SupportClawConfig) => SupportClawConfig,
