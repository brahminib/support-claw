...
export function createCodexCliSessionNodeHostCommands(): SupportClawPluginNodeHostCommand[] {
  return [
    {
...
export function createCodexCliSessionNodeInvokePolicies(): SupportClawPluginNodeInvokePolicy[] {
  return [
    {
...
async function runCodexExecResume(params: {
  ...
  const outputPath = path.join(
    await fs.mkdtemp(path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-codex-cli-")),
    "last-message.txt",
  );
  try {
    const args = [
