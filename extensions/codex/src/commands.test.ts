...
function expectedDiagnosticsTargetBlock(params: {
  ...
  return [
    ...
    ...(params.sessionKey ? [`SupportClaw session key: \`${params.sessionKey}\``] : []),
    ...(params.sessionId ? [`SupportClaw session id: \`${params.sessionId}\``] : []),
...
describe("codex command", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-codex-command-"));
    vi.stubEnv("SUPPORT_CLAW_STATE_DIR", tempDir);
  });
  ...
  it("lists Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(result, "supportClaw.json");
  });
  ...
  it("enables and disables Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(disabled, "google-calendar: disabled in supportClaw.json");
    ...
    expectResultTextContains(enabled, "google-calendar: enabled in supportClaw.json");
  ...
  it("attaches the current session to an existing Codex thread", async () => {
    ...
    await expect(
      ...
      text: "Attached this SupportClaw session to Codex thread thread-123.",
    });
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because SupportClaw sandboxing is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because SupportClaw exec host=node is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
    expect(startCodexConversationThread).not.toHaveBeenCalled();
    expect(steerCodexConversationTurn).not.toHaveBeenCalled();
  ...
  it("blocks config-level exec host=node without a session key", async () => {
    ...
    expect(result.text).toContain(
      "Codex-native /codex bind is unavailable because SupportClaw exec host=node is active for this session.",
    );
    expect(startCodexConversationThread).not.toHaveBeenCalled();
  });
  ...
  it("explains compaction when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: "No Codex thread is attached to this SupportClaw session yet.",
    });
  });
  ...
  it("asks before sending diagnostics feedback for the attached Codex thread", async () => {
    ...
    expect(safeCodexControlRequest).toHaveBeenCalledWith(
      ...
      CODEX_CONTROL_METHODS.feedback,
      {
        ...
        includeLogs: true,
        tags: {
          source: "supportClaw-diagnostics",
          channel: "test",
        },
      },
    );
  ...
  it("uploads all Codex diagnostics sessions and reports their channel/thread breakdown", async () => {
    ...
    expect(request.text).toContain("SupportClaw session key: `agent:main:whatsapp:one`");
    expect(request.text).toContain("SupportClaw session id: `session-one`");
    expect(request.text).toContain("Codex thread id: `thread-111`");
    expect(request.text).toContain("SupportClaw session key: `agent:main:discord:two`");
    expect(request.text).toContain("SupportClaw session id: `session-two`");
