...
/**
 * Lightweight read/write surface over the SupportClaw config file. Plugged in by
...
// Plugin lifecycle changes (enable/disable) write to supportClaw.json
...
export async function handleCodexPluginsSubcommand(
  ...
  if (normalized === "enable" || normalized === "disable") {
    ...
    return {
      text: `${formatCodexDisplayText(target)}: ${wantEnabled ? "enabled" : "disabled"} in supportClaw.json. ${POLICY_REFRESH_HINT}`,
    };
  }
...
export function buildPluginsHelp(): string {
  return [
    "Codex sub-plugin management (writes only to ~/.supportClaw/supportClaw.json, never to ~/.codex/config.toml):",
...
export function formatPluginList(
  ...
  return [
    "Codex sub-plugins in SupportClaw config (~/.supportClaw/supportClaw.json):",
    "",
    ...rows.map(
      (r) =>
