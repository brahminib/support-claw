...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("supportClaw.codex.conversationControl");
...
async function requireThreadBinding(sessionFile: string, lookup: CodexAppServerBindingLookup = {}) {
  ...
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this SupportClaw session yet.");
  }
  return binding;
}
