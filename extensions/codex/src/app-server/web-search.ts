...
function hasManagedSearchProvider(config: SupportClawConfig | undefined): boolean {
  return normalizeOptionalString(config?.tools?.web?.search?.provider) !== undefined;
}

function hasNativeDomainRestrictions(config: SupportClawConfig | undefined): boolean {
  return (
