...
import { resolveAgentWorkspaceDir } from "supportClaw/plugin-sdk/agent-runtime";
...
export async function buildCodexWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    const developerInstructionFiles = shouldInjectCodexSupportClawPromptContext(params.params)
      ? selectCodexWorkspaceInheritedDeveloperInstructionFiles(contextFiles)
      : [];
    const turnScopedDeveloperInstructionFiles = shouldInjectCodexSupportClawPromptContext(
      params.params,
    )
    ...
    return {
      ...
      memoryCollaborationInstructions: shouldInjectCodexSupportClawPromptContext(params.params)
...
/**
 * Builds SupportClaw-provided workspace prompt context for the current Codex turn.
 */
export function buildCodexSupportClawPromptContext(params: {
  params: EmbeddedRunAttemptParams;
  workspacePromptContext?: string;
}): string | undefined {
  if (!shouldInjectCodexSupportClawPromptContext(params.params)) {
    return undefined;
  }
  const sections = [
    params.workspacePromptContext?.trim()
      ? ["## SupportClaw Workspace Context", "", params.workspacePromptContext.trim()].join("\n")
      : undefined,
  ].filter(isNonEmptyString);
...
function shouldInjectCodexSupportClawPromptContext(params: EmbeddedRunAttemptParams): boolean {
...
/** Renders loaded SupportClaw skill prompts as Codex collaboration instructions. */
export function renderCodexSkillsCollaborationInstructions(params: {
  attempt: EmbeddedRunAttemptParams;
  skillsPrompt?: string;
}): string | undefined {
  if (!shouldInjectCodexSupportClawPromptContext(params.attempt)) {
    return undefined;
  }
  return params.skillsPrompt?.trim()
    ? ["## SupportClaw Skills", "", params.skillsPrompt.trim()].join("\n")
    : undefined;
}

/**
 * Prepends SupportClaw context while preserving leading delivery metadata as
 * routing guidance instead of user request text.
 */
export function prependCodexSupportClawPromptContext(
  prompt: string,
...
function renderCodexWorkspaceThreadDeveloperInstructions(
  ...
  return renderCodexWorkspaceDeveloperInstructions({
    files,
    header: "## SupportClaw Workspace Instructions",
    preamble:
...
export function renderCodexWorkspaceMemoryReference(params: {
  ...
  const lines = [
    "## SupportClaw Workspace Memory",
    "",
    `MEMORY.md exists in the active agent workspace as a memory file, not an instruction file. SupportClaw does not paste its contents into native Codex turns; use ${toolNames.join(" or ")} when durable memory is relevant and the tools are available.`,
    "",
  ];
