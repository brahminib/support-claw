...
describe("Codex app-server policy", () => {
  it("keeps implicit Codex yolo approval policy when untrusted approvals are disallowed", () => {
    ...
    const resolved = resolveCodexAppServerForSupportClawToolPolicy({
      appServer,
  ...
  it("promotes implicit yolo approval policy when SupportClaw tool policy requires review", () => {
    const appServer = resolveCodexAppServerRuntimeOptions({ env: {}, requirementsToml: null });
  ...
  it("preserves explicit operator app-server policy", () => {
    ...
    const explicitConfig = resolveCodexAppServerForSupportClawToolPolicy({
      appServer,
    ...
    const explicitEnv = resolveCodexAppServerForSupportClawToolPolicy({
      appServer,
      pluginConfig: readCodexPluginConfig({}),
      env: { SUPPORT_CLAW_CODEX_APP_SERVER_APPROVAL_POLICY: "never" },
      shouldPromote: true,
      canUseUntrustedApprovalPolicy: true,
    });
    const explicitRequirements = resolveCodexAppServerForSupportClawToolPolicy({
