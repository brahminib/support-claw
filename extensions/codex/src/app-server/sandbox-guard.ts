/**
 * Blocks direct Codex app-server requests that would bypass SupportClaw sandbox or
...
type DirectMethodPolicy =
  ...
  | "requires-supportClaw-environment";
...
const DIRECT_METHOD_POLICIES = new Map<string, DirectMethodPolicy>([
  ...
  ["thread/start", "requires-supportClaw-environment"],
...
/** Returns a block message when a direct app-server method would bypass SupportClaw execution policy. */
export function resolveCodexAppServerDirectSandboxBypassBlock(params: {
  ...
  if (
    policy === "requires-supportClaw-environment" &&
    hasSupportClawSandboxEnvironmentSelection(params.requestParams)
  ) {
    return undefined;
  }
  return sandboxBlock;
...
function hasSupportClawSandboxEnvironmentSelection(value: unknown): boolean {
  ...
  return (
    ...
    environments.every((entry) => {
      ...
      return (
        ...
        environment.environmentId.startsWith("supportClaw-sandbox-") &&
...
function formatCodexNativeSandboxBlock(params: { surface: string }): string {
  return [
    `Codex-native ${params.surface} is unavailable because SupportClaw sandboxing is active for this session.`,
    "This mode cannot route execution through the SupportClaw sandbox backend.",
