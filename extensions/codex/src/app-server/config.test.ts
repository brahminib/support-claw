...
import { MAX_TIMER_TIMEOUT_MS } from "supportClaw/plugin-sdk/number-runtime";
...
import {
  ...
  resolveSupportClawExecModeForCodexAppServer,
  resolveSupportClawExecModeFromConfig,
  resolveSupportClawExecPolicyForCodexAppServer,
...
describe("Codex app-server config", () => {
  it("only auto-approves app-server approvals for full yolo runtime policy", () => {
    ...
    expect(
      shouldAutoApproveCodexAppServerApprovals({
        ...
        sandbox: "danger-full-access",
        networkProxy: {
          profileName: "supportClaw-network",
          configFingerprint: "network-proxy-v1",
          configPatch: {
            "features.network_proxy.enabled": true,
            default_permissions: "supportClaw-network",
            permissions: {},
          },
        },
      }),
  ...
  it("parses typed plugin config before falling back to environment knobs", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        SUPPORT_CLAW_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        SUPPORT_CLAW_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
  ...
  it("uses read-only filesystem rules for read-only network proxy profiles", () => {
    ...
    expect(profileName).toMatch(/^supportClaw-network-[a-f0-9]{16}$/u);
    ...
    expect(
      readCodexPluginConfig({
        appServer: {
          ...
          remoteWorkspace: {
            localRoot: "/Users/kevinlin/code/supportClaw",
            remoteRoot: "/home/oai/supportClaw-workspaces",
          },
        },
      }),
    ).toStrictEqual({});
    ...
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          ...
          url: "wss://codex-app-server.example.internal/ws",
          authToken: "capability-token",
          remoteWorkspaceRoot: " /home/oai/supportClaw-workspaces ",
        },
      },
    });
  ...
  it("treats only explicit OpenAI model context as safe for Codex-backed auto-review", () => {
    ...
    for (const openAIProvider of [
      ...
      {
        baseUrl: "https://api.openai.com/v1",
        headers: { "x-supportClaw-reviewer-proxy": "local" },
        models: [],
      },
      {
        baseUrl: "https://api.openai.com/v1",
...
`,
  ...
  it("keeps explicit yolo mode when requirements disallow full access", () => {
    ...
    expectRuntimePolicy(
      resolveRuntimeForTest({
        pluginConfig: {},
        env: { SUPPORT_CLAW_CODEX_APP_SERVER_MODE: "yolo" },
        requirementsToml,
      }),
      {
        approvalPolicy: "never",
  ...
  it("rejects the retired dynamic tool profile key", () => {
    expect(
      readCodexPluginConfig({
        codexDynamicToolsProfile: "supportClaw-compat",
        codexDynamicToolsLoading: "direct",
      }),
    ).toEqual({});
  });
  ...
  it("treats configured and environment commands as explicit overrides", () => {
    expectFields(
      resolveRuntimeForTest({
        ...
        env: { SUPPORT_CLAW_CODEX_APP_SERVER_BIN: "/usr/local/bin/codex" },
      }).start,
      "configured start",
      {
  ...
  it("rejects Codex app-server command overrides that include inline arguments", () => {
    ...
    ).toThrow("SUPPORT_CLAW_CODEX_APP_SERVER_BIN must be only the Codex app-server executable path");
  });
  ...
  it("resolves Computer Use setup from plugin config and environment fallbacks", () => {
    expect(
      resolveCodexComputerUseConfig({
        ...
        env: {
          SUPPORT_CLAW_CODEX_COMPUTER_USE_PLUGIN_NAME: "env-fallback-plugin",
        },
      }),
    ).toEqual({
      enabled: true,
    ...
    expectFields(
      resolveCodexComputerUseConfig({
        pluginConfig: {},
        env: {
          SUPPORT_CLAW_CODEX_COMPUTER_USE: "1",
          SUPPORT_CLAW_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE: "github:example/plugins",
          SUPPORT_CLAW_CODEX_COMPUTER_USE_AUTO_INSTALL: "true",
          SUPPORT_CLAW_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: "30000",
        },
      }),
      "computer use config",
      {
    ...
    for (const value of ["0x10", "1e3"]) {
      expectFields(
        resolveCodexComputerUseConfig({
          ...
          env: {
            SUPPORT_CLAW_CODEX_COMPUTER_USE: "1",
            SUPPORT_CLAW_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: value,
          },
        }),
        "computer use config",
        {
  ...
  it("allows environment mode fallback to opt in to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      modelProvider: "openai",
      env: { SUPPORT_CLAW_CODEX_APP_SERVER_MODE: "guardian" },
    });
  ...
  it("maps normalized SupportClaw auto exec mode to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "auto",
      modelProvider: "openai",
  ...
  it("forces guarded app-server policy fields for auto mode", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        SUPPORT_CLAW_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        SUPPORT_CLAW_CODEX_APP_SERVER_SANDBOX: "danger-full-access",
      },
      execMode: "auto",
      modelProvider: "openai",
    });
  ...
  it("preserves explicit read-only app-server sandbox for auto mode", () => {
    ...
    const envRuntime = resolveRuntimeForTest({
      ...
      modelProvider: "openai",
      env: {
        SUPPORT_CLAW_CODEX_APP_SERVER_MODE: "yolo",
        SUPPORT_CLAW_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        SUPPORT_CLAW_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
    });
  ...
  it.each(["deny", "allowlist"] as const)(
    "blocks Codex app-server local execution for normalized SupportClaw %s exec mode",
    (execMode) => {
      expect(() =>
        resolveRuntimeForTest({
          pluginConfig: {},
  ...
  it("maps normalized SupportClaw ask exec mode away from Codex yolo", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "ask",
    });
  ...
  it("fails closed when normalized SupportClaw ask mode cannot use user approvals", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "ask",
  ...
  it.each([
    ...
    "fails closed when normalized SupportClaw $execMode mode can only use $policies approvals",
    ({ execMode, policies }) => {
      expect(() =>
        resolveRuntimeForTest({
          pluginConfig: {},
  ...
  it("keeps normalized SupportClaw full exec mode on default Codex yolo", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "full",
    });
  ...
  it("fails closed when normalized SupportClaw auto mode can only use on-failure approvals", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "auto",
  ...
  it("fails closed when normalized SupportClaw auto mode cannot force prompting over yolo", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "auto",
  ...
  it("uses user approvals when normalized SupportClaw auto mode cannot use Codex auto-review", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "auto",
      requirementsToml:
  ...
  it("keeps normalized SupportClaw auto mode when legacy app-server yolo was schema-defaulted", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          command: "codex",
  ...
  it("forces guarded policy fields for normalized SupportClaw auto mode", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          approvalPolicy: "never",
  ...
  it("resolves agent-scoped normalized SupportClaw exec mode for Codex app-server mapping", () => {
    const config = {
      tools: {
        exec: {
          mode: "ask",
    ...
    expect(resolveSupportClawExecModeFromConfig({ config, agentId: "codex-agent" })).toBe("auto");
    expect(resolveSupportClawExecModeFromConfig({ config, agentId: "other-agent" })).toBe("ask");
  });

  it("keeps legacy exec security overrides ahead of normalized SupportClaw exec mode", () => {
    expect(
      resolveSupportClawExecModeFromConfig({
        config: {
          tools: {
  ...
  it.each(["always"] as const)(
    ...
    (ask) => {
      ...
      const execPolicy = resolveSupportClawExecPolicyForCodexAppServer({ config });

      expect(resolveSupportClawExecModeForCodexAppServer({ config })).toBe("ask");
      expectRuntimePolicy(
        resolveRuntimeForTest({
          pluginConfig: {
            appServer: {
  ...
  it("keeps legacy full exec security with ask=on-miss on default Codex yolo", () => {
    ...
    expect(resolveSupportClawExecModeForCodexAppServer({ config })).toBe("full");
  ...
  it("fails closed when legacy full exec with ask cannot use full Codex sandbox", () => {
    ...
    expect(() =>
      resolveRuntimeForTest({
        execPolicy: resolveSupportClawExecPolicyForCodexAppServer({ config }),
        requirementsToml: 'allowed_sandbox_modes = ["read-only", "workspace-write"]\n',
      }),
    ).toThrow("legacy full exec security with ask requires Codex app-server danger-full-access");
  });

  it("clamps legacy full exec with ask when an SupportClaw sandbox is active", () => {
    const config = {
      tools: {
        exec: {
          security: "full",
  ...
  it("applies host exec approval security floors before starting Codex app-server", () => {
    const execPolicy = resolveSupportClawExecPolicyForCodexAppServer({
      config: {
        tools: {
          exec: {
            mode: "full",
  ...
  it("treats ask-only legacy overrides as normalized mode overrides", () => {
    ...
    expect(resolveSupportClawExecModeFromConfig({ config, agentId: "codex-agent" })).toBe("allowlist");
    const execMode = resolveSupportClawExecModeForCodexAppServer({
      config,
      agentId: "main",
      execOverrides: {
        ask: "always",
  ...
  it("ignores removed SUPPORT_CLAW_CODEX_APP_SERVER_GUARDIAN fallback", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      env: { SUPPORT_CLAW_CODEX_APP_SERVER_GUARDIAN: "1" },
    });
  ...
  it("keeps runtime config keys aligned with manifest schema and UI hints", async () => {
    const manifest = JSON.parse(
      await fs.readFile(new URL("../../supportClaw.plugin.json", import.meta.url), "utf8"),
    ) as {
      configSchema: {
        properties: {
