 ...
 * SupportClaw-owned tools, Codex native-tool fallback rules, sandbox shell shims,
...
import { resolveAgentDir } from "supportClaw/plugin-sdk/agent-runtime";
import { isToolAllowed } from "supportClaw/plugin-sdk/sandbox";
...
type SupportClawCodingToolsOptions = NonNullable<
  Parameters<(typeof import("supportClaw/plugin-sdk/agent-harness"))["createSupportClawCodingTools"]>[0]
>;
type SupportClawExecOptions = NonNullable<SupportClawCodingToolsOptions["exec"]>;

/** Factory seam for constructing SupportClaw runtime tools without eagerly loading agent-harness. */
export type SupportClawCodingToolsFactory =
  (typeof import("supportClaw/plugin-sdk/agent-harness"))["createSupportClawCodingTools"];
type SupportClawDynamicTool = ReturnType<SupportClawCodingToolsFactory>[number];
type SupportClawSandboxContext = Awaited<ReturnType<typeof resolveSandboxContext>>;
...
export type DynamicToolBuildParams = {
  ...
  sandbox: SupportClawSandboxContext;
...
let supportClawCodingToolsFactoryForTests: SupportClawCodingToolsFactory | undefined;

/** Overrides the runtime tool factory for tests that need deterministic tool catalogs. */
export function setSupportClawCodingToolsFactoryForTests(factory: SupportClawCodingToolsFactory): void {
  supportClawCodingToolsFactoryForTests = factory;
}

/** Clears the test-only runtime tool factory override. */
export function resetSupportClawCodingToolsFactoryForTests(): void {
  supportClawCodingToolsFactoryForTests = undefined;
}

/** Splits sandbox and run session keys so tool calls can bind to both scopes when needed. */
export function resolveSupportClawCodingToolsSessionKeys(
  params: EmbeddedRunAttemptParams,
  sandboxSessionKey: string,
): Pick<SupportClawCodingToolsOptions, "sessionKey" | "runSessionKey"> {
  return {
...
export async function buildDynamicTools(input: DynamicToolBuildParams) {
  ...
  const agentHarness = await import("supportClaw/plugin-sdk/agent-harness");
  const createSupportClawCodingTools =
    supportClawCodingToolsFactoryForTests ?? agentHarness.createSupportClawCodingTools;
  toolBuildStages.mark("load-agent-harness-tools");
  const sessionKeys = resolveSupportClawCodingToolsSessionKeys(params, input.sandboxSessionKey);
  const nativeExecutionPolicy = resolveCodexNativeExecutionPolicyForDynamicTools(input);
  const allTools = createSupportClawCodingTools({
    ...
    modelCompat:
      params.model.compat && typeof params.model.compat === "object"
        ? (params.model.compat as SupportClawCodingToolsOptions["modelCompat"])
        : undefined,
  ...
  toolBuildStages.mark("create-supportClaw-coding-tools");
...
export function shouldEnableCodexAppServerNativeToolSurface(
  ...
  // capability, so narrow SupportClaw allowlists must fail closed rather than
...
/** Returns true when SupportClaw policy requires the Node-owned exec/process tools instead. */
...
function canCodexAppServerNativeToolSurfaceHonorSandbox(
  sandbox: SupportClawSandboxContext | undefined,
  ...
  // active SupportClaw sandboxing must disable the native surface and route shell
...
/** Requires a Codex sandbox environment only when native tools must run inside SupportClaw sandboxing. */
...
/** Projects a local SupportClaw workspace cwd into the remote Codex app-server workspace root. */
export function mapCodexAppServerRemoteWorkspacePath(params: {
  ...
  if (!normalizedValue.startsWith(prefix)) {
    throw new Error(
      `Codex remoteWorkspaceRoot is configured but cwd ${params.value} is outside SupportClaw workspace root ${params.localWorkspaceRoot}; refusing to send a gateway-local cwd to the remote Codex app-server.`,
    );
  }
...
/** Converts SupportClaw sandbox networking into Codex's external-sandbox policy shape. */
export function resolveCodexExternalSandboxPolicyForSupportClawSandbox(
  sandbox: SupportClawSandboxContext | undefined,
): CodexSandboxPolicy {
  return {
    type: "externalSandbox",
    networkAccess: codexNetworkAccessForSupportClawSandbox(sandbox) ? "enabled" : "restricted",
  };
}

function codexNetworkAccessForSupportClawSandbox(
  sandbox: SupportClawSandboxContext | undefined,
): boolean {
...
export function addSandboxShellDynamicToolsIfAvailable(
  filteredTools: SupportClawDynamicTool[],
  allTools: SupportClawDynamicTool[],
  input: DynamicToolBuildParams,
): SupportClawDynamicTool[] {
  if (
  ...
  const sandboxExecTool: SupportClawDynamicTool = {
    ...execTool,
    name: "sandbox_exec",
    description:
      "Run a shell command through SupportClaw's configured sandbox backend for this session. Use when SupportClaw sandboxing is active or when a command must execute in the sandbox backend, such as an SSH-backed sandbox or Docker container-path bind layout. Use Codex's native shell only when no SupportClaw sandbox is active and native Code Mode is available.",
  ...
  const sandboxProcessTool: SupportClawDynamicTool = {
    ...processTool,
    name: "sandbox_process",
    description:
      "Manage sandbox_exec sessions that were started through SupportClaw's configured sandbox backend for this session: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use only for sandbox_exec follow-up; use Codex's native shell session handling only when no SupportClaw sandbox is active and native Code Mode is available.",
  };
...
function addNodeShellDynamicToolsIfNeeded(
  ...
  const toolsToAppend: SupportClawDynamicTool[] = [];
  if (
...
function createNodeExecDynamicTool(
  execTool: SupportClawDynamicTool,
  ...
  return {
    ...
    description:
      "Run a shell command on the SupportClaw configured remote node for this session. This tool always uses SupportClaw host=node internally and follows the existing node exec approval and allowlist policy. Use node_process for follow-up on backgrounded node_exec sessions. Use Codex's native shell for local app-server work.",
...
function createNodeProcessDynamicTool(processTool: SupportClawDynamicTool): SupportClawDynamicTool {
  return {
    ...processTool,
    name: CODEX_NODE_PROCESS_DYNAMIC_TOOL_NAME,
    description:
      "Manage node_exec sessions that were started on the SupportClaw configured remote node for this session: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use only for node_exec follow-up; use Codex's native shell session handling for local app-server work.",
  };
}
...
function hideNodeExecDynamicToolParameters(parameters: SupportClawDynamicTool["parameters"]) {
...
): Pick<SupportClawExecOptions, "host" | "node"> | undefined {
