/**
 * Hosts the local SupportClaw sandbox exec-server that Codex app-server native
...
import { embeddedAgentLog } from "supportClaw/plugin-sdk/agent-harness-runtime";
import type { SandboxContext } from "supportClaw/plugin-sdk/sandbox";
...
const SANDBOX_EXEC_SERVERS = new Map<string, Promise<SupportClawExecServer>>();
  ...
  await Promise.all(
    servers.map(async (entry) => {
      if (entry.status === "fulfilled") {
        ...
        await closeSupportClawExecServer(entry.value);
      }
    }),
  );
}
...
export async function ensureCodexSandboxExecServerEnvironment(params: {
  ...
  if (!canExposeLocalExecServerToAppServer(params.appServerStartOptions)) {
    throw new Error(
      "SupportClaw Codex exec-server uses a local loopback URL and cannot be registered with a remote Codex app-server.",
    );
  }
  assertCodexSandboxExecServerSupported(params.client);
  const execServer = await acquireSupportClawExecServer(params.sandbox);
  try {
    ...
    await releaseSupportClawExecServer(execServer);
...
export async function releaseCodexSandboxExecServerEnvironment(
  ...
  if (server) {
    await releaseSupportClawExecServer(server);
  }
}
...
function assertCodexSandboxExecServerSupported(client: CodexAppServerClient): void {
  ...
  if (
    ...
    throw new Error(
      `Codex app-server ${MIN_CODEX_SANDBOX_EXEC_SERVER_APP_SERVER_VERSION} or newer is required for SupportClaw sandbox exec-server environments, but detected ${
...
async function acquireSupportClawExecServer(sandbox: SandboxContext): Promise<SupportClawExecServer> {
  const key = sandbox.runtimeId;
  while (true) {
    const existing = SANDBOX_EXEC_SERVERS.get(key);
    const promise = existing ?? startAndRememberSupportClawExecServer(sandbox);
...
function startAndRememberSupportClawExecServer(sandbox: SandboxContext): Promise<SupportClawExecServer> {
  const created = startSupportClawExecServer(sandbox);
  const key = sandbox.runtimeId;
...
async function startSupportClawExecServer(sandbox: SandboxContext): Promise<SupportClawExecServer> {
  ...
  if (!address || typeof address === "string") {
    throw new Error("SupportClaw Codex exec-server did not bind to a TCP port.");
  }
  const environmentId = buildEnvironmentId(sandbox);
  const authPath = `/supportClaw-${randomUUID()}`;
  const url = `ws://127.0.0.1:${(address as AddressInfo).port}${authPath}`;
  const execServer: SupportClawExecServer = {
    authPath,
    closed: false,
    environmentId,
    refCount: 0,
...
async function releaseSupportClawExecServer(execServer: SupportClawExecServer): Promise<void> {
  ...
  await closeSupportClawExecServer(execServer);
}

async function closeSupportClawExecServer(execServer: SupportClawExecServer): Promise<void> {
  if (execServer.closed) {
    return;
  }
...
function buildEnvironmentId(sandbox: SandboxContext): string {
  ...
  return `supportClaw-sandbox-${hash}`;
}

function isAuthorizedExecServerRequest(
  execServer: SupportClawExecServer,
...
function handleConnection(execServer: SupportClawExecServer, socket: WebSocket): void {
...
async function dispatchRequest(
  ...
  switch (request.method) {
    ...
    default:
      throw new Error(`Unsupported SupportClaw sandbox exec-server method: ${request.method}`);
  }
}
