...
import { resetDiagnosticEventsForTest } from "supportClaw/plugin-sdk/diagnostic-runtime";
import { clearInternalHooks, resetGlobalHookRunner } from "supportClaw/plugin-sdk/hook-runtime";
import { clearMemoryPluginState } from "supportClaw/plugin-sdk/memory-core-host-runtime-core";
import { clearPluginCommands } from "supportClaw/plugin-sdk/plugin-runtime";
...
export function setupRunAttemptTestHooks(): void {
  beforeEach(async () => {
    ...
    vi.stubEnv("SUPPORT_CLAW_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-codex-run-"));
  });
  ...
  afterEach(async () => {
    ...
    testing.resetSupportClawCodingToolsFactoryForTests();
