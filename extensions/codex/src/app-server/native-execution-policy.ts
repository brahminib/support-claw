 ...
 * or whether SupportClaw must keep exec/process on a configured node host.
 */
import type { SupportClawConfig } from "supportClaw/plugin-sdk/config-contracts";
import { resolveSandboxRuntimeStatus } from "supportClaw/plugin-sdk/sandbox";
import { getSessionEntry, type SessionEntry } from "supportClaw/plugin-sdk/session-store-runtime";
...
type AgentEntry = NonNullable<NonNullable<SupportClawConfig["agents"]>["list"]>[number];
...
export function resolveCodexNativeExecutionPolicy(params: {
  ...
  return {
    ...
    node,
    blockReason:
      "SupportClaw exec host=node is active for this session. Codex app-server native execution cannot route shell, filesystem, MCP, or app-backed work through the selected SupportClaw node.",
  };
}
...
export function formatCodexNativeNodeExecBlock(params: {
  ...
  return [
    `Codex-native ${params.surface} is unavailable because SupportClaw exec host=node is active for this session.`,
    params.reason ??
      "Codex app-server native execution cannot route execution through the selected SupportClaw node.",
    "Use a normal Codex harness turn so SupportClaw exec/process tools run on the node, or switch exec host to gateway for native Codex app-server execution.",
  ].join(" ");
}
...
function listAgentEntries(config: SupportClawConfig): AgentEntry[] {
