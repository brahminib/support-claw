/**
 * Bridges Codex native hook callbacks into SupportClaw's native hook relay so
 * app-server tool events can still run SupportClaw policy and diagnostics.
 */
...
/** Codex hook events that can be registered through SupportClaw's native relay. */
...
/** Registers an SupportClaw native hook relay for a Codex app-server turn. */
...
export function resolveCodexNativeHookRelayEvents(params: {
  ...
  // pre-guardian SupportClaw plugin approval prompt.
...
export function buildCodexNativeHookRelayId(params: {
  ...
  hash.update("supportClaw:codex:native-hook-relay:v1");
  hash.update("\0");
