...
import { SessionManager } from "supportClaw/plugin-sdk/agent-sessions";
import {
  onInternalDiagnosticEvent,
  waitForDiagnosticEventsDrained,
  type DiagnosticEventPayload,
} from "supportClaw/plugin-sdk/diagnostic-runtime";
import { initializeGlobalHookRunner, registerInternalHook } from "supportClaw/plugin-sdk/hook-runtime";
import { registerMemoryCapability } from "supportClaw/plugin-sdk/memory-core-host-runtime-core";
import { MESSAGE_TOOL_DELIVERY_HINTS } from "supportClaw/plugin-sdk/message-tool-delivery-hints";
import { registerPluginCommand } from "supportClaw/plugin-sdk/plugin-runtime";
import { createMockPluginRegistry } from "supportClaw/plugin-sdk/plugin-test-runtime";
import { describe, expect, it, vi } from "vitest";
import WebSocket from "ws";
import { CODEX_GPT5_BEHAVIOR_CONTRACT } from "../../prompt-overlay.js";
import { defaultCodexAppInventoryCache } from "./app-inventory-cache.js";
import {
  buildCodexSupportClawPromptContext,
  buildCodexSystemPromptReport,
  buildCodexWorkspaceBootstrapContext,
  getCodexWorkspaceMemoryToolNames,
  prependCodexSupportClawPromptContext,
...
import {
  CODEX_SUPPORT_CLAW_DYNAMIC_TOOL_NAMESPACE,
...
async function buildCodexTurnContextForTest(
  ...
  const threadDeveloperInstructions = [
    ...
    .join("\n\n");
  const supportClawPromptContext = buildCodexSupportClawPromptContext({
    params,
    workspacePromptContext: workspaceBootstrapContext.promptContext,
  });
  const codexTurnPromptText = prependCodexSupportClawPromptContext(
    params.prompt,
    supportClawPromptContext,
  );
...
describe("runCodexAppServerAttempt", () => {
  ...
  it("starts active SupportClaw sandbox threads with Codex native execution disabled", async () => {
    testing.setSupportClawCodingToolsFactoryForTests(() => [
  ...
  it("routes native Codex execution through an SupportClaw sandbox exec-server when opted in", async () => {
    const appServer = {
    ...
    try {
      ...
      expect(environmentAddParams?.environmentId).toMatch(/^supportClaw-sandbox-/);
  ...
  it("starts Codex threads without duplicate SupportClaw workspace tools by default", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("includes Codex app-server scoped plugin command guidance in developer instructions", () => {
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "SupportClaw main command guidance.",
          surfaces: ["supportClaw_main"],
        },
      ],
    ...
    expect(instructions).not.toContain("SupportClaw main command guidance.");
  });

  it("passes SupportClaw skills as turn collaboration developer instructions", async () => {
    const llmInput = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "llm_input", handler: llmInput }]),
    );
    vi.stubEnv("SUPPORT_CLAW_TRAJECTORY", "1");
    vi.stubEnv("SUPPORT_CLAW_TRAJECTORY_DIR", path.join(tempDir, "trajectory"));
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    ...
    const collaborationInstructions =
      turnStartParams.collaborationMode?.settings?.developer_instructions ?? "";
    expect(collaborationInstructions).toContain("## SupportClaw Skills");
    expect(collaborationInstructions).toContain("<available_skills>");
    const inputText = turnStartParams.input?.[0]?.text ?? "";
    expect(inputText).not.toContain("## SupportClaw Skills");
    ...
    expect(compiledContext?.data?.systemPrompt).toContain("## SupportClaw Skills");
  ...
  it("emits TUI-compatible tool events for Codex dynamic tool calls", async () => {
    ...
    await expect(
      ...
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown SupportClaw tool: python" }],
    });
    await harness.completeTurn({ threadId: "thread-1", turnId: "turn-1" });
    await run;
    ...
    expect(onRunAgentEvent).toHaveBeenCalledWith({
      ...
      data: {
        ...
        isError: true,
        result: {
          content: [{ type: "text", text: "Unknown SupportClaw tool: python" }],
        },
      },
    });
  ...
  it("keeps leading delivery hints out of the Codex current user request", async () => {
    for (const [index, deliveryHint] of MESSAGE_TOOL_DELIVERY_HINTS.entries()) {
      ...
      const inputText = turnStartParams.input?.[0]?.text ?? "";
      expect(inputText).toContain("SupportClaw delivery metadata:");
  ...
  it("keeps searchable SupportClaw dynamic tools when code-mode-only is enabled", () => {
    const tools = [
      createRuntimeDynamicTool("message"),
    ...
    expect(webSearch?.namespace).toBe(CODEX_SUPPORT_CLAW_DYNAMIC_TOOL_NAMESPACE);
    expect(webSearch?.deferLoading).toBe(true);
    expect(heartbeat?.namespace).toBe(CODEX_SUPPORT_CLAW_DYNAMIC_TOOL_NAMESPACE);
    expect(heartbeat?.deferLoading).toBe(true);
    expect(sessionsSpawn?.namespace).toBe(CODEX_SUPPORT_CLAW_DYNAMIC_TOOL_NAMESPACE);
  ...
  it("registers heartbeat response durably without advertising it on normal turns", async () => {
    testing.setSupportClawCodingToolsFactoryForTests((options) => [
      createRuntimeDynamicTool("message"),
    ...
    expect(normalInstructions).toContain(
      "Deferred searchable SupportClaw dynamic tools available: message.",
    );
    expect(normalInstructions).not.toContain(
      "Deferred searchable SupportClaw dynamic tools available: heartbeat_respond",
    );
  ...
  it("keeps message in the registered schema when disabled for an internal turn", async () => {
    ...
    await expect(
      ...
      contentItems: [
        {
          type: "inputText",
          text: "SupportClaw tool is not available for this turn: message",
        },
      ],
    });
  });
  ...
  it("fails closed for Codex app defaults when restricted native tools have no plugin config", async () => {
    testing.setSupportClawCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("message")]);
    const params = createParams(
      path.join(tempDir, "session.jsonl"),
      path.join(tempDir, "workspace"),
    );
  ...
  it("applies before_prompt_build to Codex developer instructions and turn input", async () => {
    ...
    const wrappedPluginSystemContext = (text: string) =>
      `---\n\nSupportClaw plugin-injected system context. This block is not workspace file content.\n\n${text}\n\n---`;
  ...
  it("projects bounded continuity when starting Codex without a native thread binding", async () => {
    ...
    const harness = createStartedThreadHarness();
    const params = createParams(sessionFile, workspaceDir);
    params.prompt = "make the default webpage supportClaw";
    ...
    expect(inputText).toContain("SupportClaw assembled context for this turn:");
    ...
    expect(inputText).toContain("Current user request:");
    expect(inputText).toContain("make the default webpage supportClaw");
  });
  ...
  it("does not replay mirrored history already covered by an existing Codex binding", async () => {
    ...
    expect(inputText).not.toContain("SupportClaw assembled context for this turn:");
  ...
  it("projects only newer visible history when a resumed Codex binding is stale", async () => {
    ...
    const copilotMirrorMessage = {
      ...
      __supportClaw: { mirrorIdentity: "copilot:assistant-1" },
    } as ReturnType<typeof assistantMessage> & { __supportClaw: { mirrorIdentity: string } };
  ...
  it("does not project Codex mirrored transcript echoes as stale binding continuity", async () => {
    ...
    const codexMirrorAssistantMessage = {
      ...
      __supportClaw: { mirrorIdentity: "codex-app-server:assistant-1" },
    } as ReturnType<typeof assistantMessage> & { __supportClaw: { mirrorIdentity: string } };
  ...
  it("does not project mirrored messages on consecutive resumes", async () => {
    ...
    const firstInputText =
      (firstTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(firstInputText).toContain("SupportClaw assembled context for this turn:");
    ...
    const secondInputText =
      (secondTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(secondInputText).not.toContain("SupportClaw assembled context for this turn:");
  ...
  it("passes stable workspace files as Codex developer instructions and routes MEMORY.md through tools", async () => {
    ...
    expect(threadDeveloperInstructions).toContain("SupportClaw Workspace Instructions");
    ...
    expect(collaborationInstructions).toContain("SupportClaw Agent Soul");
    ...
    expect(collaborationInstructions).toContain("SupportClaw Workspace Memory");
    ...
    expect(inputText).not.toContain("SupportClaw runtime context for this turn:");
    ...
    expect(inputText).not.toContain("SupportClaw Workspace Memory");
  ...
  it("adds memory recall guidance when dated memory notes exist without root MEMORY.md", async () => {
    ...
    expect(collaborationInstructions).toContain("memory_search");
    expect(collaborationInstructions).toContain("memory_get");
    expect(collaborationInstructions).not.toContain("SupportClaw Workspace Memory");
  ...
  it("sends workspace bootstrap instructions through Codex app-server payloads", async () => {
    ...
    expect(threadStartParams.developerInstructions).toContain("SupportClaw Workspace Instructions");
  ...
  it("routes MEMORY.md through memory_get when search is unavailable", async () => {
    ...
    await fs.writeFile(path.join(workspaceDir, "MEMORY.md"), memorySummary);
    registerMemoryPromptForTest();
    testing.setSupportClawCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("memory_get")]);
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    setAgentWorkspaceForTest(params, workspaceDir);
  ...
  it("omits heartbeat Codex workspace pointers for empty HEARTBEAT.md files", async () => {
    ...
    expect(collaborationInstructions).toContain("This is an SupportClaw heartbeat turn");
    expect(collaborationInstructions).not.toContain("HEARTBEAT.md exists");
  });

  it("keeps lightweight cron Codex turns out of SupportClaw bootstrap context", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const exactCommand =
      "cd /Users/phaedrus/Projects/supportClaw && /Users/phaedrus/clawd/scripts/clawsweeper-related-scan.py";
    await fs.mkdir(workspaceDir, { recursive: true });
  ...
  it("keeps lightweight cron delivery hints byte-for-byte without SupportClaw prompt context", async () => {
  ...
  it("promotes implicit Codex yolo approval policy when SupportClaw tool policy exists", async () => {
    initializeGlobalHookRunner(
    ...
    expect(info).toHaveBeenCalledWith(
      "codex app-server approval policy promoted for SupportClaw tool policy",
      {
        from: "never",
        to: "untrusted",
  ...
  it("keeps explicit Codex yolo mode unpromoted when SupportClaw tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const sessionFile = path.join(tempDir, "session.jsonl");
  ...
  it("keeps normalized full exec mode unpromoted when SupportClaw tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const sessionFile = path.join(tempDir, "session.jsonl");
  ...
  it("ignores invalid Codex app-server env overrides when promoting tool policy approval", async () => {
    ...
    vi.stubEnv("SUPPORT_CLAW_CODEX_APP_SERVER_MODE", " ");
    vi.stubEnv("SUPPORT_CLAW_CODEX_APP_SERVER_APPROVAL_POLICY", "always");
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const harness = createStartedThreadHarness();
  ...
  it("keeps managed web_search for provider-qualified Codex model overrides", async () => {
    testing.setSupportClawCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("web_search")]);
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const { requests, waitForMethod, completeTurn } = createStartedThreadHarness(async (method) => {
  ...
  it("announces Codex app-server fast auto progress after the crossing tool result", async () => {
    ...
    expect(payloads[offIndex]?.channelData).toEqual({
      supportClawProgressKind: "fast-mode-auto",
    });
