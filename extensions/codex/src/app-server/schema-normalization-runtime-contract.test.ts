...
describe("Codex app-server dynamic tool schema boundary contract", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-codex-schema-contract-"));
  });
  ...
  it("passes prepared executable dynamic tool schemas through legacy thread start specs", async () => {
    ...
    expect(startPayload?.serviceName).toBe("SupportClaw");
    ...
    expect(startPayload?.developerInstructions).toContain("SupportClaw");
  });
