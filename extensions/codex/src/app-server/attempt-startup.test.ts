...
function createAttemptPaths(): AttemptPaths {
  const root = path.join(os.tmpdir(), `supportClaw-codex-attempt-startup-${randomUUID()}`);
  tempRoots.add(root);
  return {
...
async function answerInitialize(harness: ClientHarness): Promise<void> {
  ...
  harness.send({ id: initialize.id, result: { userAgent: "supportClaw/0.125.0 (macOS; test)" } });
}
