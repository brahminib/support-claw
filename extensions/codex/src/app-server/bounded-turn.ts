...
import type { AuthProfileStore } from "supportClaw/plugin-sdk/agent-runtime";
...
const SUPPORT_CLAW_CODEX_APP_SERVER_ARGS_ENV_VAR = "SUPPORT_CLAW_CODEX_APP_SERVER_ARGS";
...
function buildPrivateCodexAppServerStartOptions(
  ...
  const privateEnv = Object.fromEntries(
    Object.entries(start.env ?? {}).filter(
      ([name]) => name.trim().toUpperCase() !== SUPPORT_CLAW_CODEX_APP_SERVER_ARGS_ENV_VAR,
    ),
  );
  const clearEnv = (start.clearEnv ?? []).filter((name) => {
    const normalized = name.trim().toUpperCase();
    return normalized !== "CODEX_HOME" && normalized !== SUPPORT_CLAW_CODEX_APP_SERVER_ARGS_ENV_VAR;
  });
  return {
    ...start,
    ...
    clearEnv: [...clearEnv, SUPPORT_CLAW_CODEX_APP_SERVER_ARGS_ENV_VAR],
  };
}
...
function createCodexBoundedApprovalHandler(taskLabel: string) {
  return (request: { method: string }): JsonValue | undefined => {
    if (
      ...
      return {
        decision: "decline",
        reason: `SupportClaw Codex ${taskLabel} does not grant tool or file approvals.`,
      };
    }
    ...
    if (request.method.includes("requestApproval")) {
      return {
        decision: "decline",
        reason: `SupportClaw Codex ${taskLabel} does not grant native approvals.`,
      };
    }
