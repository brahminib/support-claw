/**
 * Bridges SupportClaw runtime tools into Codex app-server dynamic tool specs and
 * tool-call responses.
 */
...
import { emitTrustedDiagnosticEvent } from "supportClaw/plugin-sdk/diagnostic-runtime";
import type { ImageContent, TextContent } from "supportClaw/plugin-sdk/llm";
...
/** Namespace attached to SupportClaw-owned dynamic tools exposed to Codex. */
export const CODEX_SUPPORT_CLAW_DYNAMIC_TOOL_NAMESPACE = "supportClaw";

// Keep SupportClaw session spawning searchable in Codex mode so Codex's native
...
/**
 * Creates dynamic tool specs and a call handler that executes SupportClaw tools,
    ...
    handleToolCall: async (call, options) => {
      ...
      if (!toolEntry) {
        const message = registeredToolNames.has(call.tool)
          ? `SupportClaw tool is not available for this turn: ${call.tool}`
          : `Unknown SupportClaw tool: ${call.tool}`;
        finalizeToolTerminalPresentation({
...
function createCodexDynamicToolSpecs(params: {
  ...
  if (namespaceTools.length > 0) {
    specs.push({
      type: "namespace",
      name: CODEX_SUPPORT_CLAW_DYNAMIC_TOOL_NAMESPACE,
      description: "",
...
function convertToolContents(
  ...
  const noticeText = `...(SupportClaw truncated dynamic tool result: original ${totalTextChars} chars, showing ${maxChars}; rerun with narrower args.)`;
  const notice = `\n${noticeText}`;
