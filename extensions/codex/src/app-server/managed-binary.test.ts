...
describe("managed Codex app-server binary", () => {
  ...
  it("resolves the plugin-local bundled Codex binary", async () => {
    const pluginRoot = path.join("/tmp", "supportClaw", "extensions", "codex");
  ...
  it("resolves Windows Codex command shims", () => {
    const pluginRoot = path.win32.join("C:\\", "SupportClaw", "dist", "extensions", "codex");
  ...
  it("uses the package root when the resolver is bundled into a dist chunk", () => {
    expect(testing.resolveDefaultCodexPluginRoot("/repo/supportClaw/dist")).toBe("/repo/supportClaw");
    expect(testing.resolveDefaultCodexPluginRoot("/repo/supportClaw/dist-runtime")).toBe(
      "/repo/supportClaw",
    );
    expect(
      testing.resolveDefaultCodexPluginRoot("/repo/supportClaw/extensions/codex/src/app-server"),
    ).toBe("/repo/supportClaw/extensions/codex");
  });

  it("finds Codex in the package install root used by packaged plugins", async () => {
    const installRoot = path.join("/tmp", "supportClaw-plugin-package", "codex");
  ...
  it("finds Codex bins hoisted into an isolated npm project root", async () => {
    const projectRoot = path.join("/tmp", "state", "npm", "projects", "supportClaw-codex-hash");
    const pluginRoot = path.join(projectRoot, "node_modules", "@supportclaw", "codex");
  ...
  it("finds Windows Codex shims hoisted into an isolated npm project root", async () => {
    const projectRoot = path.win32.join(
      ...
      "npm",
      "projects",
      "supportClaw-codex-hash",
    );
    const pluginRoot = path.win32.join(projectRoot, "node_modules", "@supportclaw", "codex");
  ...
  it("falls back to the resolved Codex package bin when no command shim exists", async () => {
    const installRoot = await mkdtemp(path.join(os.tmpdir(), "supportClaw-codex-package-"));
    const pluginRoot = path.join(installRoot, "dist", "extensions", "codex");
  ...
  it("fails clearly when the managed Codex binary is missing", async () => {
    await expect(
      resolveManagedCodexAppServerStartOptions(startOptions("managed"), {
        platform: "darwin",
        pluginRoot: path.join("/tmp", "supportClaw", "extensions", "codex"),
