...
import { nativeHookRelayTesting } from "supportClaw/plugin-sdk/agent-harness-runtime";
import {
...
const createSupportClawCodingToolsMock = vi.fn();
...
vi.mock("supportClaw/plugin-sdk/agent-harness", () => ({
  createSupportClawCodingTools: (...args: unknown[]) => createSupportClawCodingToolsMock(...args),
}));
...
async function runSideQuestionWithManagedWebSearchCall(
  ...
  if (!options.preserveToolFactory) {
    createSupportClawCodingToolsMock.mockReturnValue([
      {
        name: "web_search",
...
describe("runCodexAppServerSideQuestion", () => {
  beforeEach(() => {
    ...
    createSupportClawCodingToolsMock.mockReset();
  ...
  it("forks an ephemeral side thread and returns the completed assistant text", async () => {
    ...
    const [toolOptions] = mockCall(createSupportClawCodingToolsMock);
  ...
  it("disables hosted search when side-question sender policy removes managed web_search", async () => {
    createSupportClawCodingToolsMock.mockImplementation((options: { senderId?: string }) =>
  ...
  it("applies native search restrictions to side forks and suppresses managed search", async () => {
    ...
    expect(toolResponse).toEqual({
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown SupportClaw tool: web_search" }],
    });
  ...
  it("preserves managed web_search while planning hosted search for Responses side questions", async () => {
    createSupportClawCodingToolsMock.mockImplementation(
  ...
  it("rejects /btw before forking when the current SupportClaw session is sandboxed", async () => {
    await expect(
      runCodexAppServerSideQuestion(
        sideParams({
      ...
      "Codex-native /btw side-question mode is unavailable because SupportClaw sandboxing is active for this session.",
    );
  ...
  it("rejects /btw before forking when exec host=node is active", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because SupportClaw exec host=node is active for this session.",
    );
  ...
  it("bridges side-thread dynamic tool requests to SupportClaw tools", async () => {
    const client = createFakeClient();
    let toolResponse: unknown;
    client.request.mockImplementation(async (method: string) => {
      if (method === "thread/fork") {
  ...
  it("normalizes hook channel ids for side-thread dynamic tool requests", async () => {
    ...
    expect(createSupportClawCodingToolsMock).toHaveBeenCalledWith(
