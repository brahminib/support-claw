...
import { resolveUserPath } from "supportClaw/plugin-sdk/agent-harness-runtime";
import type {
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.SUPPORT_CLAW_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.SUPPORT_CLAW_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
