...
/** Tool names owned by Codex app-server and normally excluded from SupportClaw dynamic tools. */
...
type CodexDynamicToolProfileEnv = {
  SUPPORT_CLAW_BUILD_PRIVATE_QA?: string;
  SUPPORT_CLAW_QA_FORCE_RUNTIME?: string;
};

/** Normalizes SupportClaw/Codex tool names before filtering and allowlist checks. */
...
export function isForcedPrivateQaCodexRuntime(
  ...
  return (
    ...
    env.SUPPORT_CLAW_QA_FORCE_RUNTIME?.trim().toLowerCase() === "codex"
  );
}
...
/** Filters SupportClaw tools that Codex owns natively or config explicitly excludes. */
