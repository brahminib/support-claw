...
describe("Codex app-server dynamic tool build", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-codex-tools-"));
  });
  ...
  it("maps local gateway workspace suffixes to the remote Codex app-server root", () => {
    expect(
      mapCodexAppServerRemoteWorkspacePath({
        value: "/Users/kevinlin/code/supportClaw/packages/example",
        localWorkspaceRoot: "/Users/kevinlin/code/supportClaw",
        remoteWorkspaceRoot: "/home/oai/supportClaw-workspaces",
      }),
    ).toBe("/home/oai/supportClaw-workspaces/packages/example");
    expect(
      mapCodexAppServerRemoteWorkspacePath({
        value: "/Users/kevinlin/code/supportClaw",
        localWorkspaceRoot: "/Users/kevinlin/code/supportClaw",
        remoteWorkspaceRoot: "/home/oai/supportClaw-workspaces",
      }),
    ).toBe("/home/oai/supportClaw-workspaces");
  });
  ...
  it("fails closed when remote cwd projection cannot stay under the remote workspace root", () => {
    ...
    ).toThrow("outside SupportClaw workspace root");
  });

  it("maps Windows child paths through remote Codex app-server workspaces", () => {
    expect(
      mapCodexAppServerRemoteWorkspacePath({
        value: "C:\\Users\\kevinlin\\code\\supportClaw\\packages\\example",
        localWorkspaceRoot: "C:\\Users\\kevinlin\\code\\supportClaw",
        remoteWorkspaceRoot: "/home/oai/supportClaw-workspaces",
      }),
    ).toBe("/home/oai/supportClaw-workspaces/packages/example");
  });
  ...
  it("maps sandbox exec-server cwd through the remote workspace mapping", () => {
    expect(
      resolveCodexAppServerExecutionCwd({
        effectiveCwd: "/Users/kevinlin/code/supportClaw",
        environment: {
          id: "sandbox-1",
          cwd: "/Users/kevinlin/code/supportClaw/sandbox",
        } as never,
        nativeToolSurfaceEnabled: true,
        localWorkspaceRoot: "/Users/kevinlin/code/supportClaw",
        remoteWorkspaceRoot: "/home/oai/supportClaw-workspaces",
      }),
    ).toBe("/home/oai/supportClaw-workspaces/sandbox");
  });
  ...
  it("reports hosted search denied when effective tool policy removes web_search", async () => {
    ...
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    setSupportClawCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("message")]);
  ...
  it("exposes app-server-owned tools directly for forced private QA Codex runtime", () => {
    ...
    const privateQaCodexEnv = {
      SUPPORT_CLAW_BUILD_PRIVATE_QA: "1",
  ...
  it("quarantines unreadable tool entries before Codex-specific filtering", async () => {
    ...
    setSupportClawCodingToolsFactoryForTests(() => sourceTools);
  ...
  it("quarantines non-object plugin schemas before Codex-specific filtering", async () => {
    ...
    setSupportClawCodingToolsFactoryForTests(() => [brokenTool, messageTool]);
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("limits Codex memory flush runs to managed read and write tools", async () => {
    ...
    setSupportClawCodingToolsFactoryForTests((options) => {
  ...
  it("exposes SupportClaw sandbox shell tools under distinct names for non-Docker sandbox backends", async () => {
    setSupportClawCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("read"),
      createRuntimeDynamicTool("write"),
  ...
  it("exposes Docker sandbox shell tools when SupportClaw sandboxing disables native Code Mode", async () => {
    setSupportClawCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("exec"),
      createRuntimeDynamicTool("process"),
      createRuntimeDynamicTool("message"),
  ...
  it("exposes Docker sandbox shell tools when native Code Mode cannot honor sandbox paths", async () => {
    ...
    const tools = await buildDynamicToolsForTest(params, workspaceDir, {
      sandbox: {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/supportClaw-data:/data:rw"] },
      } as never,
      nativeToolSurfaceEnabled: false,
    });
  ...
  it("preserves before-tool wrapping through Codex runtime normalization", async () => {
    ...
    setSupportClawCodingToolsFactoryForTests(() => [wrappedTool]);
  ...
  it("enables gateway subagent binding for forced private QA Codex runs", async () => {
    vi.stubEnv("SUPPORT_CLAW_BUILD_PRIVATE_QA", "1");
    vi.stubEnv("SUPPORT_CLAW_QA_FORCE_RUNTIME", "codex");
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("disables Codex native tool surfaces whenever an SupportClaw sandbox is active", () => {
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(path.join(tempDir, "session.jsonl"), workspaceDir);
    params.disableTools = false;
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/supportClaw-data:/tmp/supportClaw-data:rw"] },
      } as never),
    ).toBe(false);
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        ...
        docker: {
          binds: [
            "/tmp/supportClaw-data:/tmp/supportClaw-data:rw",
            "/tmp/supportClaw-data/secrets:/tmp/supportClaw-data/secrets:ro",
          ],
        },
      } as never),
    ).toBe(false);
  ...
  it("passes the live run session key to Codex dynamic tools when sandbox policy uses another key", () => {
    ...
    expect(
      resolveSupportClawCodingToolsSessionKeys(params, "agent:main:telegram:default:direct:1234"),
    ).toEqual({
      sessionKey: "agent:main:telegram:default:direct:1234",
      runSessionKey: "agent:main:main",
    });

    expect(resolveSupportClawCodingToolsSessionKeys(params, "agent:main:main")).toEqual({
