/**
 * Policy promotion for Codex app-server runs that can safely use SupportClaw tool
 * approvals.
 */
import {
 ...
 * requirements allow SupportClaw to handle tool approvals.
 */
export function resolveCodexAppServerForSupportClawToolPolicy(params: {
  ...
  const explicitMode =
    ...
    isCodexAppServerPolicyMode(params.env.SUPPORT_CLAW_CODEX_APP_SERVER_MODE);
  const explicitApprovalPolicy =
    params.pluginConfig.appServer?.approvalPolicy !== undefined ||
    isCodexAppServerApprovalPolicy(params.env.SUPPORT_CLAW_CODEX_APP_SERVER_APPROVAL_POLICY) ||
