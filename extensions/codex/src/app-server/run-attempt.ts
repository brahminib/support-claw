...
import { loadExecApprovals } from "supportClaw/plugin-sdk/exec-approvals-runtime";
import { pathExists } from "supportClaw/plugin-sdk/security-runtime";
import {
  resolveCodexAppServerForModelProvider,
  resolveCodexAppServerForSupportClawToolPolicy,
...
import {
  ...
  type SupportClawExecPolicyForCodexAppServer,
} from "./config.js";
import {
...
import {
  ...
  resolveSupportClawCodingToolsSessionKeys,
  resetSupportClawCodingToolsFactoryForTests,
  setSupportClawCodingToolsFactoryForTests,
...
export async function runCodexAppServerAttempt(
  ...
  let configuredAppServer = resolveCodexAppServerRuntimeOptions({
    ...
    config: params.config,
    agentDir,
    supportClawSandboxActive: sandbox?.enabled === true,
  });
  ...
  let policyAppServer = resolveCodexAppServerForSupportClawToolPolicy({
  ...
  if (configuredAppServer.approvalPolicy === "never" && appServer.approvalPolicy === "untrusted") {
    embeddedAgentLog.info("codex app-server approval policy promoted for SupportClaw tool policy", {
      from: "never",
      to: "untrusted",
  ...
  policyAppServer = resolveCodexAppServerForSupportClawToolPolicy({
    appServer: configuredAppServer,
    pluginConfig,
    env: process.env,
    shouldPromote:
  ...
  // SupportClaw transcript is persistence/search state. Context-engine output is
  ...
  const decorateCodexTurnPromptText = (promptBuildResult: {
    ...
    const turnPromptText = prependCodexSupportClawPromptContext(
  ...
  const selectNewerVisibleHistoryAfterBinding = (binding: CodexAppServerThreadBinding) => {
    ...
    return historyMessages.filter((message) => {
      ...
      const meta = record["__supportClaw"];
  ...
  const pendingSupportClawDynamicToolCompletionIds = new Set<string>();
  ...
  const releaseTurnAfterTerminalDynamicTool = (paramsValue: {
    ...
    if (
      !shouldReleaseTurnAfterTerminalDynamicTool({
        ...
        pendingSupportClawDynamicToolCompletionIdsCount: pendingSupportClawDynamicToolCompletionIds.size,
      })
    ) {
      return;
    }
  ...
  const emitFastModeAutoProgress = async (payload: {
    ...
    try {
      await params.onToolResult?.({
        text: summary,
        channelData: { supportClawProgressKind: FAST_MODE_AUTO_PROGRESS_KIND },
      });
    } catch (error) {
  ...
  const handleNotification = async (notification: CodexServerNotification) => {
    ...
    const notificationState = applyCodexTurnNotificationState({
      ...
      activeTurnItemIds,
      activeAppServerTurnRequests,
      pendingSupportClawDynamicToolCompletionIds,
    ...
    // See supportClaw/supportClaw#67996.
  ...
  const requestCleanup = client.addRequestHandler(async (request) => {
    ...
    try {
      ...
      pendingSupportClawDynamicToolCompletionIds.add(call.callId);
      ...
      try {
        ...
        pendingSupportClawDynamicToolCompletionIds.delete(call.callId);
  ...
  if (activeNativeTurnIds.length > 0) {
    ...
    // Starting an SupportClaw turn before that native turn completes can wedge the
  ...
  try {
    ...
    if (isCodexActiveCompactTurnError(turnStartError)) {
      ...
      // the next SupportClaw turn collides with that compact turn, wait for the
    ...
    if (
      ...
      // Do not try to pre-compact or summarize through SupportClaw here. Codex owns
      // automatic compaction; SupportClaw may only discard a stale projection thread
  ...
  try {
    ...
    const canUseCompletedAssistantTextAfterClientClose =
      ...
      activeTurnItemIds.size === 0 &&
      pendingSupportClawDynamicToolCompletionIds.size === 0;
...
function handleApprovalRequest(params: {
  ...
  execPolicy?: Pick<SupportClawExecPolicyForCodexAppServer, "mode">;
