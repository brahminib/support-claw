...
import { isSilentReplyPayloadText } from "supportClaw/plugin-sdk/reply-chunking";
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-codex-delivery-contract-"));
