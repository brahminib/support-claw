 ...
 * access through the active SupportClaw sandbox backend.
 */
...
import { SsrFBlockedError, isBlockedHostnameOrIp } from "supportClaw/plugin-sdk/ssrf-runtime";
...
import type { HttpHeader, SupportClawExecServer } from "./types.js";
...
async function runStreamingSandboxHttpRequest(
  ...
  if (!command) {
    throw new Error("SupportClaw sandbox HTTP exec spec did not provide a command.");
  }
...
tmp=$(mktemp "$TMPDIR/supportClaw-http.XXXXXX.py" 2>/dev/null || mktemp "/tmp/supportClaw-http.XXXXXX.py") || exit 1
trap 'rm -f "$tmp"' EXIT
cat > "$tmp" <<'PY'
import base64
import json
