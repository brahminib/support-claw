 ...
 */
import { asBoolean } from "supportClaw/plugin-sdk/string-coerce-runtime";
import {
...
/** Detects completion for an SupportClaw dynamic tool result still awaited by Codex. */
export function isPendingSupportClawDynamicToolCompletionNotification(
  notification: CodexServerNotification,
  pendingSupportClawDynamicToolCompletionIds: ReadonlySet<string>,
): boolean {
  if (notification.method !== "item/completed" || !isJsonObject(notification.params)) {
    return false;
  }
  const itemId = readNotificationItemId(notification);
  if (!itemId || !pendingSupportClawDynamicToolCompletionIds.has(itemId)) {
    return false;
  }
  const item = isJsonObject(notification.params.item) ? notification.params.item : undefined;
