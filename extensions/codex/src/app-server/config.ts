...
} from "supportClaw/plugin-sdk/exec-approvals-runtime";
import { resolvePositiveTimerTimeoutMs } from "supportClaw/plugin-sdk/number-runtime";
import { normalizeAgentId } from "supportClaw/plugin-sdk/routing";
import {
  buildSecretInputSchema,
  normalizeResolvedSecretInputString,
  type SecretInput,
} from "supportClaw/plugin-sdk/secret-input";
import { normalizeTrimmedStringList } from "supportClaw/plugin-sdk/string-coerce-runtime";
import { detectWindowsSpawnCommandInlineArgs } from "supportClaw/plugin-sdk/windows-spawn";
import { z } from "zod";
import type { CodexSandboxPolicy, CodexServiceTier, JsonObject, JsonValue } from "./protocol.js";

const START_OPTIONS_KEY_SECRET_SYMBOL = Symbol.for("supportClaw.codexAppServerStartOptionsKeySecret");
...
type SupportClawExecMode = "deny" | "allowlist" | "ask" | "auto" | "full";
type SupportClawExecSecurity = "deny" | "allowlist" | "full";
type SupportClawExecAsk = "off" | "on-miss" | "always";
type SupportClawExecApprovalFloorsForCodexAppServer = {
  security?: SupportClawExecSecurity;
  ask?: SupportClawExecAsk;
};
export type SupportClawExecPolicyForCodexAppServer = {
  mode?: SupportClawExecMode;
  security: SupportClawExecSecurity;
  ask: SupportClawExecAsk;
  touched: boolean;
};
type SupportClawExecPolicy = SupportClawExecPolicyForCodexAppServer;
...
const DEFAULT_CODEX_APP_SERVER_NETWORK_PROXY_PROFILE_PREFIX = "supportClaw-network";
...
export function resolveCodexAppServerRuntimeOptions(
  params: {
    ...
    execMode?: SupportClawExecMode;
    execPolicy?: SupportClawExecPolicyForCodexAppServer;
    modelProvider?: string;
    model?: string;
    config?: ProviderAuthAliasConfig;
    env?: NodeJS.ProcessEnv;
    ...
    hostName?: string;
    supportClawSandboxActive?: boolean;
  } = {},
  ...
  const envCommand = readNonEmptyString(env.SUPPORT_CLAW_CODEX_APP_SERVER_BIN);
  ...
  const args = resolveArgs(config.args, env.SUPPORT_CLAW_CODEX_APP_SERVER_ARGS);
  ...
  const execMode = resolveEffectiveSupportClawExecModeForCodexAppServer({
    execMode: params.execMode,
    execPolicy: params.execPolicy,
  });
  assertCodexAppServerAllowedForSupportClawExecMode(execMode);
  const explicitPolicyMode =
    resolvePolicyMode(config.mode) ?? resolvePolicyMode(env.SUPPORT_CLAW_CODEX_APP_SERVER_MODE);
  const configuredSandbox =
    resolveSandbox(config.sandbox) ?? resolveSandbox(env.SUPPORT_CLAW_CODEX_APP_SERVER_SANDBOX);
  const explicitApprovalsReviewer = resolveApprovalsReviewer(config.approvalsReviewer);
  const normalizedPolicyMode = resolveCodexPolicyModeForSupportClawExecMode(execMode);
  ...
  const execModeRequiringPromptingApprovals: Extract<SupportClawExecMode, "auto" | "ask"> | undefined =
        ...
        sandbox: preserveExplicitAutoSandbox
          ...
          : forceDangerFullAccessSandbox
            ? selectForcedDangerFullAccessSandbox({
                configuredSandbox,
                defaultPolicy,
                supportClawSandboxActive: Boolean(params.supportClawSandboxActive),
              })
  ...
  const envApprovalPolicy = resolveApprovalPolicy(env.SUPPORT_CLAW_CODEX_APP_SERVER_APPROVAL_POLICY);
...
export function resolveCodexComputerUseConfig(
  ...
  const marketplaceSource =
    ...
    readNonEmptyString(env.SUPPORT_CLAW_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE);
  const marketplacePath =
    readNonEmptyString(params.overrides?.marketplacePath) ??
    readNonEmptyString(config.marketplacePath) ??
    readNonEmptyString(env.SUPPORT_CLAW_CODEX_COMPUTER_USE_MARKETPLACE_PATH);
  const marketplaceName =
    readNonEmptyString(params.overrides?.marketplaceName) ??
    readNonEmptyString(config.marketplaceName) ??
    readNonEmptyString(env.SUPPORT_CLAW_CODEX_COMPUTER_USE_MARKETPLACE_NAME);
  const autoInstall =
    params.overrides?.autoInstall ??
    config.autoInstall ??
    readBooleanEnv(env.SUPPORT_CLAW_CODEX_COMPUTER_USE_AUTO_INSTALL) ??
    false;
  const marketplaceDiscoveryTimeoutMs = normalizePositiveNumber(
    params.overrides?.marketplaceDiscoveryTimeoutMs ??
      ...
      readNumberEnv(env.SUPPORT_CLAW_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS),
    DEFAULT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS,
  );
  const enabled =
    params.overrides?.enabled ??
    config.enabled ??
    readBooleanEnv(env.SUPPORT_CLAW_CODEX_COMPUTER_USE) ??
  ...
  return {
    ...
    pluginName:
      ...
      readNonEmptyString(env.SUPPORT_CLAW_CODEX_COMPUTER_USE_PLUGIN_NAME) ??
    ...
    mcpServerName:
      ...
      readNonEmptyString(env.SUPPORT_CLAW_CODEX_COMPUTER_USE_MCP_SERVER_NAME) ??
...
function resolveDefaultCodexAppServerPolicy(params: {
  ...
  execModeRequiringPromptingApprovals?: Extract<SupportClawExecMode, "auto" | "ask">;
  execModeRequiringUserReviewer?: SupportClawExecMode;
  env?: NodeJS.ProcessEnv;
  requirementsToml?: string | null;
  requirementsPath?: string;
  readRequirementsFile?: (path: string) => string | undefined;
...
function selectGuardianApprovalsReviewer(
  ...
  execModeRequiringAutoReviewer?: Extract<SupportClawExecMode, "auto">,
...
function selectForcedDangerFullAccessSandbox(params: {
  ...
  if (params.defaultPolicy?.dangerFullAccessAllowed === false) {
    if (params.supportClawSandboxActive) {
...
export function resolveSupportClawExecModeFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): SupportClawExecMode | undefined {
  const policy = resolveSupportClawExecPolicyFromConfig(params);
  return policy.touched ? policy.mode : undefined;
}

function resolveSupportClawExecPolicyFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): SupportClawExecPolicy {
  const root = readRecord(params.config);
  const globalExec = readRecord(readRecord(root?.tools)?.exec);
  const globalPolicy = applySupportClawExecPolicyLayer(createDefaultSupportClawExecPolicy(), globalExec);
  ...
  return applySupportClawExecPolicyLayer(globalPolicy, agentExec);
}

export function resolveSupportClawExecModeForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  const policy = resolveSupportClawExecPolicyForCodexAppServer(params);
  return policy.touched ? policy.mode : undefined;
}

export function resolveSupportClawExecPolicyForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  config?: unknown;
  agentId?: string;
}): SupportClawExecPolicyForCodexAppServer {
  const basePolicy = resolveSupportClawExecPolicyFromConfig({
    config: params.config,
    agentId: params.agentId,
  });
  const overridePolicy = applySupportClawExecPolicyLayer(basePolicy, params.execOverrides);
  const approvalFloors = resolveSupportClawExecApprovalFloorsForCodexAppServer({
    approvals: params.approvals,
    agentId: params.agentId,
    policy: overridePolicy,
  });
  return applySupportClawExecApprovalFloors(overridePolicy, approvalFloors);
}

function resolveEffectiveSupportClawExecModeForCodexAppServer(params: {
  execMode?: SupportClawExecMode;
  execPolicy?: SupportClawExecPolicyForCodexAppServer;
}): SupportClawExecMode | undefined {
...
function resolveCodexPolicyModeForSupportClawExecMode(
  mode: SupportClawExecMode | undefined,
...
function assertCodexAppServerAllowedForSupportClawExecMode(mode: SupportClawExecMode | undefined): void {
...
function createDefaultSupportClawExecPolicy(): SupportClawExecPolicy {
  return {
    security: "full",
    ask: "off",
    touched: false,
...
function applySupportClawExecPolicyLayer(
  base: SupportClawExecPolicy,
  ...
  if (mode !== undefined) {
    return {
      ...resolveSupportClawExecPolicyForMode(mode),
      touched: true,
    };
  }
  ...
  return {
    mode: resolveSupportClawExecModeFromPolicy({ security: nextSecurity, ask: nextAsk }),
...
function resolveSupportClawExecApprovalFloorsForCodexAppServer(params: {
  approvals?: ExecApprovalsFile;
  agentId?: string;
  policy: SupportClawExecPolicy;
}): SupportClawExecApprovalFloorsForCodexAppServer | undefined {
  if (!params.approvals) {
    return undefined;
  }
...
function applySupportClawExecApprovalFloors(
  base: SupportClawExecPolicy,
  approvalFloors?: SupportClawExecApprovalFloorsForCodexAppServer,
): SupportClawExecPolicy {
  if (!approvalFloors) {
    return base;
  }
  const nextSecurity = approvalFloors.security
    ? minSupportClawExecSecurity(base.security, approvalFloors.security)
    : base.security;
  const nextAsk = approvalFloors.ask ? maxSupportClawExecAsk(base.ask, approvalFloors.ask) : base.ask;
...
function resolveSupportClawExecPolicyForMode(
  mode: SupportClawExecMode,
): Omit<SupportClawExecPolicy, "touched"> {
  switch (mode) {
    case "deny":
...
function resolveSupportClawExecModeFromPolicy(params: {
  security: SupportClawExecSecurity;
  ask: SupportClawExecAsk;
}): SupportClawExecMode {
...
function minSupportClawExecSecurity(
  left: SupportClawExecSecurity,
  right: SupportClawExecSecurity,
): SupportClawExecSecurity {
  const order: Record<SupportClawExecSecurity, number> = { deny: 0, allowlist: 1, full: 2 };
  return order[left] <= order[right] ? left : right;
}

function maxSupportClawExecAsk(left: SupportClawExecAsk, right: SupportClawExecAsk): SupportClawExecAsk {
  const order: Record<SupportClawExecAsk, number> = { off: 0, "on-miss": 1, always: 2 };
  return order[left] >= order[right] ? left : right;
}

function readExecMode(value: unknown): SupportClawExecMode | undefined {
  return value === "deny" ||
    value === "allowlist" ||
    value === "ask" ||
    value === "auto" ||
...
function readExecSecurity(value: unknown): SupportClawExecSecurity | undefined {
  return value === "deny" || value === "allowlist" || value === "full" ? value : undefined;
}

function readExecAsk(value: unknown): SupportClawExecAsk | undefined {
