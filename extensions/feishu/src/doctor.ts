...
function collectConfiguredAgentIds(cfg: SupportClawConfig): string[] {
...
function resolveConfiguredDefaultAgentId(cfg: SupportClawConfig): string {
...
function formatPreviewWarning(inspection: FeishuDoctorInspection): string {
  ...
  return [
    ...
    '- Run "supportClaw doctor --fix" to rebuild Feishu local state.',
  ].join("\n");
}
...
function hasConfiguredFeishuChannel(cfg: SupportClawConfig): boolean {
