...
async function withIsolatedHome<T>(run: () => Promise<T>): Promise<T> {
  ...
  return await withTempDir("supportClaw-feishu-media-", async (tempHome) => {
    try {
...
describe("sendMediaFeishu msg_type routing", () => {
  ...
  afterAll(() => {
    ...
    vi.doUnmock("supportClaw/plugin-sdk/media-runtime");
    vi.resetModules();
  });
  ...
  it("passes mediaLocalRoots as localRoots to loadWebMedia for local paths (#27884)", async () => {
    ...
    const roots = ["/allowed/workspace", "/tmp/supportClaw"];
    await sendMediaFeishu({
      cfg: emptyConfig,
      to: "user:ou_target",
...
describe("saveMessageResourceFeishu", () => {
  ...
  it("saves message resource streams directly to the media store", async () => {
    await withIsolatedHome(async () => {
      ...
      expect(result.saved.path).toContain(`${path.sep}.supportClaw${path.sep}media${path.sep}inbound`);
