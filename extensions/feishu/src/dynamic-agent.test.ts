...
beforeEach(async () => {
  tempRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "supportClaw-feishu-agent-"));
});
...
function createRuntime(
  currentCfg?: SupportClawConfig,
  persistedCfg?: SupportClawConfig,
  mutationCfg?: SupportClawConfig,
) {
  let runtimeCfg = structuredClone(currentCfg ?? ({} as SupportClawConfig));
  const commitConfig = vi.fn();
  const mutateConfigFile = vi.fn(
    async (params: {
      mutate: (draft: SupportClawConfig, context: { snapshot: never; previousHash: null }) => unknown;
    }) => {
...
describe("maybeCreateDynamicAgent", () => {
  ...
  it("rechecks current ingress inside the config mutation lock", async () => {
    ...
    const canCreateForConfig = vi
      .fn<(cfg: SupportClawConfig) => Promise<boolean>>()
  ...
  it("preserves concurrent runtime config when creating from a stale request snapshot", async () => {
    ...
    const result = await maybeCreateDynamicAgent({
      cfg: { agents: { list: [] }, bindings: [] } as SupportClawConfig,
      runtime,
      accountId: "default",
      senderOpenId: "ou_sender",
      canCreateForConfig: async () => true,
