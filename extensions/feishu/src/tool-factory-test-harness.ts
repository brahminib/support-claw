...
export function createToolFactoryHarness(cfg: SupportClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<SupportClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as SupportClawPluginApi,
    resolveTool,
    registered,
  };
}
