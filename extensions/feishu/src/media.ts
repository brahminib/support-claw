...
import type { MessageReceipt } from "supportClaw/plugin-sdk/channel-outbound";
import { mediaKindFromMime } from "supportClaw/plugin-sdk/media-mime";
import {
  MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS,
  runFfmpeg,
  runFfprobe,
} from "supportClaw/plugin-sdk/media-runtime";
import { saveMediaBuffer, saveMediaStream, type SavedMedia } from "supportClaw/plugin-sdk/media-store";
import { readRegularFile, writeExternalFileWithinRoot } from "supportClaw/plugin-sdk/security-runtime";
...
} from "supportClaw/plugin-sdk/temp-path";
...
async function saveMessageResourceWithType(params: {
  ...
  const saved = await saveFeishuResponseMedia({
    response,
    tmpDirPrefix: "supportClaw-feishu-resource-",
...
async function transcodeToFeishuVoiceOpus(params: {
  ...
  return await withTempWorkspace(
    { rootDir: resolvePreferredSupportClawTmpDir(), prefix: "feishu-voice-" },
    async (workspace) => {
      const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
...
async function probeAudioDurationMs(buffer: Buffer): Promise<number | undefined> {
  try {
    return await withTempWorkspace(
      { rootDir: resolvePreferredSupportClawTmpDir(), prefix: "feishu-audio-probe-" },
      async (workspace) => {
