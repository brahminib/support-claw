...
function captureEnv(): EnvSnapshot {
  return {
    ...
    SUPPORT_CLAW_HOME: process.env.SUPPORT_CLAW_HOME,
...
function feishuConfig(): SupportClawConfig {
  return {
    channels: {
      feishu: {
        appId: "cli_xxx",
...
function stateDir(): string {
  const dir = process.env.SUPPORT_CLAW_STATE_DIR;
  if (!dir) {
...
describe("Feishu doctor state repair", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-feishu-doctor-"));
    process.env.HOME = tempHome;
    process.env.SUPPORT_CLAW_HOME = tempHome;
    process.env.SUPPORT_CLAW_STATE_DIR = path.join(tempHome, ".supportClaw");
    fs.mkdirSync(process.env.SUPPORT_CLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  });
  ...
  it("warns before repair when Feishu local state is corrupt", async () => {
    ...
    expect(result.warningNotes.join("\n")).toContain("supportClaw doctor --fix");
  });
