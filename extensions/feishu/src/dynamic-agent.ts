...
import { resolveChannelConfigWrites } from "supportClaw/plugin-sdk/channel-config-writes";
import { normalizeAccountId, resolveAgentRoute } from "supportClaw/plugin-sdk/routing";
import type { SupportClawConfig, PluginRuntime } from "../runtime-api.js";
import { resolveFeishuAccount } from "./accounts.js";
import type { DynamicAgentCreationConfig } from "./types.js";

type MaybeCreateDynamicAgentResult = {
  created: boolean;
  updatedCfg: SupportClawConfig;
  agentId?: string;
};
...
class DynamicAgentMutationSkipped extends Error {
  constructor(readonly cfg: SupportClawConfig) {
...
export async function maybeCreateDynamicAgent(params: {
  ...
  accountId: string;
  senderOpenId: string;
  canCreateForConfig: (cfg: SupportClawConfig) => Promise<boolean>;
  log: (msg: string) => void;
  ...
  const currentCfg = runtime.config.current() as SupportClawConfig;
  ...
  let skippedCfg: SupportClawConfig | undefined;
      ...
      mutate: async (draft) => {
        ...
        if (!agentExists) {
          const workspaceTemplate =
            dynamicCfg.workspaceTemplate ?? "~/.supportClaw/workspace-{agentId}";
          const agentDirTemplate =
            dynamicCfg.agentDirTemplate ?? "~/.supportClaw/agents/{agentId}/agent";
  ...
  return {
    ...
    updatedCfg: runtime.config.current() as SupportClawConfig,
