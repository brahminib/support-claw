...
vi.mock("supportClaw/plugin-sdk/media-store", () => ({
...
describe("downloadLineMedia", () => {
  ...
  afterAll(() => {
    ...
    vi.doUnmock("supportClaw/plugin-sdk/media-store");
    vi.resetModules();
  });
  ...
  beforeEach(() => {
    ...
    saveMediaStreamMock.mockImplementation(
      async (stream: AsyncIterable<Buffer>, contentType?: string, subdir?: string) => {
        ...
        return {
          path: `/home/user/.supportClaw/media/${subdir ?? "unknown"}/saved-media`,
  ...
  it("persists inbound media with the shared media store", async () => {
    ...
    expect(result).toEqual({
      path: "/home/user/.supportClaw/media/inbound/saved-media",
