...
import { createMockIncomingRequest } from "supportClaw/plugin-sdk/test-env";
import { WEBHOOK_IN_FLIGHT_DEFAULTS } from "supportClaw/plugin-sdk/webhook-request-guards";
...
vi.mock("supportClaw/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/webhook-ingress")>(
    "supportClaw/plugin-sdk/webhook-ingress",
  );
  return {
    ...actual,
...
describe("monitorLineProvider lifecycle", () => {
  ...
  afterAll(() => {
    vi.doUnmock("./bot.js");
    vi.doUnmock("supportClaw/plugin-sdk/reply-runtime");
    vi.doUnmock("supportClaw/plugin-sdk/runtime-env");
    vi.doUnmock("supportClaw/plugin-sdk/webhook-ingress");
