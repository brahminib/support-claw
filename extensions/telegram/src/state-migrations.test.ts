...
import { resolvePersistentDedupePluginStateNamespace } from "supportClaw/plugin-sdk/persistent-dedupe";
import {
...
describe("telegram state migrations", () => {
  it("detects legacy bot-info cache import", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "supportClaw-telegram-state-migration-"));
    const env = { ...process.env, SUPPORT_CLAW_STATE_DIR: dir };
