...
import { resolveFetch } from "supportClaw/plugin-sdk/fetch-runtime";
import { MAX_DATE_TIMESTAMP_MS } from "supportClaw/plugin-sdk/number-runtime";
...
beforeEach(() => {
  vi.unstubAllEnvs();
  for (const key of [
    "SUPPORT_CLAW_DEBUG_PROXY_ENABLED",
    "SUPPORT_CLAW_DEBUG_PROXY_URL",
    "ALL_PROXY",
    "all_proxy",
    "HTTP_PROXY",
    "http_proxy",
    ...
    "SUPPORT_CLAW_PROXY_ACTIVE",
    "SUPPORT_CLAW_PROXY_CA_FILE",
  ]) {
    vi.stubEnv(key, "");
  }
...
function writeTempCa(contents: string): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "supportClaw-telegram-proxy-ca-"));
  tempDirs.push(dir);
...
describe("resolveTelegramFetch", () => {
  ...
  it("uses the SupportClaw debug proxy URL when no explicit proxy fetch is provided", async () => {
    vi.stubEnv("SUPPORT_CLAW_DEBUG_PROXY_ENABLED", "1");
    vi.stubEnv("SUPPORT_CLAW_DEBUG_PROXY_URL", "http://127.0.0.1:7777");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("uses SUPPORT_CLAW_PROXY_URL as a Telegram explicit proxy when proxy env is absent", async () => {
    vi.stubEnv("SUPPORT_CLAW_PROXY_URL", "http://127.0.0.1:7788");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
    ...
    expect(ProxyAgentCtor).toHaveBeenCalledTimes(1);
    const proxyOptions = constructorOptions(ProxyAgentCtor, "SupportClaw proxy") as {
      allowH2?: boolean;
      uri?: string;
  ...
  it("preserves caller-provided custom fetch when SUPPORT_CLAW_PROXY_URL is present", async () => {
    vi.stubEnv("SUPPORT_CLAW_PROXY_URL", "http://127.0.0.1:7788");
    const proxyFetch = vi.fn(async () => ({ ok: true }) as Response) as unknown as typeof fetch;
  ...
  it("prefers standard proxy env over SUPPORT_CLAW_PROXY_URL for Telegram", async () => {
    vi.stubEnv("SUPPORT_CLAW_PROXY_URL", "http://127.0.0.1:7788");
    vi.stubEnv("https_proxy", "http://127.0.0.1:7890");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("keeps resolver-scoped transport policy for SupportClaw proxy fetches", async () => {
  ...
  it("skips sticky IPv4 fallback when the DNS result order env override is verbatim", async () => {
    vi.stubEnv("SUPPORT_CLAW_TELEGRAM_DNS_RESULT_ORDER", "verbatim");
    undiciFetch.mockResolvedValueOnce({ ok: true } as Response);
