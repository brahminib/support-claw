...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("supportClaw/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
type DispatchReplyHarnessParams = Parameters<DispatchReplyWithBufferedBlockDispatcherFn>[0];
type ReadRemoteMediaBufferFn =
  typeof import("supportClaw/plugin-sdk/media-runtime").readRemoteMediaBuffer;
...
function ensureMediaHarnessStoreRoot(): string {
  mediaHarnessStoreRoot ??= mkdtempSync(path.join(os.tmpdir(), "supportClaw-telegram-media-e2e-"));
...
const defaultRuntimeConfig = (() =>
  ...
  }) as SupportClawConfig) as TelegramBotDeps["getRuntimeConfig"];
