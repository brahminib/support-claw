...
const { resetLogger, setLoggerOverride } = await import("supportClaw/plugin-sdk/runtime-env");
const {
  buildInlineKeyboard,
...
function captureInfoLogs(): string {
  logCaptureCounter += 1;
  const logFile = `/tmp/supportClaw-telegram-send-log-${process.pid}-${logCaptureCounter}.jsonl`;
...
describe("sent-message-cache", () => {
  ...
  it("keeps sent-message ownership across restart", async () => {
    const persistedStorePath = `/tmp/supportClaw-telegram-send-tests-${process.pid}-restart.json`;
  ...
  it("keeps expired custom-store cleanup away from the default store", () => {
    const customStorePath = `/tmp/supportClaw-telegram-send-tests-${process.pid}-custom-cleanup.json`;
  ...
  it("keeps default and custom stores isolated while both are loaded", () => {
    const customStorePath = `/tmp/supportClaw-telegram-send-tests-${process.pid}-custom-isolated.json`;
    const customCfg = { session: { store: customStorePath } };
...
describe("sendMessageTelegram", () => {
  ...
  it("records sent text messages into the Telegram prompt context cache", async () => {
    const storePath = `/tmp/supportClaw-telegram-send-context-${process.pid}-${Date.now()}.json`;
  ...
  it.each([
    {
      name: "local path",
      markdown:
        "See [scripts/yougile.py](/home/user/.supportClaw/workspace/scripts/yougile.py#L41) and [docs](https://example.com/docs)",
      rejectedAnchor: '<a href="/home',
      visibleLabel: "<code>scripts/yougile.py</code>",
    },
    {
      name: "relative path",
      markdown: "Edit [config](./supportClaw.json) or see [docs](https://example.com/docs)",
