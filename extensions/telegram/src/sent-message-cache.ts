...
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("supportClaw.telegramSentMessagesState");
const TELEGRAM_SENT_MESSAGES_STORE_FOR_TEST_KEY = Symbol.for(
  "supportClaw.telegramSentMessagesStoreForTest",
);
...
function resolveSentMessageStorePath(cfg?: Pick<SupportClawConfig, "session">): string {
  return `${resolveStorePath(cfg?.session?.store)}.telegram-sent-messages.json`;
}

function resolveSentMessageScopeKey(cfg?: Pick<SupportClawConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<SupportClawConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<SupportClawConfig, "session">): SentMessageStore {
...
export function recordSentMessage(
  ...
  messageId: number,
  cfg?: Pick<SupportClawConfig, "session">,
): void {
