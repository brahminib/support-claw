...
import type { ChannelLegacyStateMigrationPlan } from "supportClaw/plugin-sdk/channel-contract";
import { resolveChannelAllowFromPath } from "supportClaw/plugin-sdk/channel-pairing";
...
} from "supportClaw/plugin-sdk/persistent-dedupe";
import { createPluginStateSyncKeyedStore } from "supportClaw/plugin-sdk/runtime-doctor";
import { statRegularFileSync } from "supportClaw/plugin-sdk/security-runtime";
...
function detectTelegramMessageDispatchLegacyStateMigration(params: {
  ...
  const storePath = resolveStorePath(params.cfg.session?.store, { env: params.env });
  const legacyStorePath = resolveLegacySessionStorePath(params);
  const env = params.stateDir ? { ...params.env, SUPPORT_CLAW_STATE_DIR: params.stateDir } : params.env;
