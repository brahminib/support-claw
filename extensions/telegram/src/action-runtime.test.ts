...
import { resolveStorePath } from "supportClaw/plugin-sdk/session-store-runtime";
...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): SupportClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as SupportClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): SupportClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  function topicCacheScopeFor(cfg: SupportClawConfig, accountId: string): string {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["SUPPORT_CLAW_STATE_DIR", "TELEGRAM_BOT_TOKEN"]);
    resetTopicNameCacheForTest();
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as SupportClawConfig;
    await expect(
  ...
  it("persists sendMessage action deliveries before Telegram platform send", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-telegram-action-durable-"));
    const {
      createOutboundTestPlugin,
      createTestRegistry,
      readQueuedDeliveryEntriesForTest,
      setActivePluginRegistry,
    } = await import("supportClaw/plugin-sdk/plugin-test-runtime");
  ...
  it("stores created forum topic names in the account-scoped cache", async () => {
    ...
    const cfg = {
      ...
      session: { store: path.join(os.tmpdir(), "supportClaw-telegram-action-sessions.json") },
    } as SupportClawConfig;
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: SupportClawConfig, accountId = "media") {
    await handleTelegramAction(
