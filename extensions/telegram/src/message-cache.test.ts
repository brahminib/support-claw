...
describe("telegram message cache", () => {
  ...
  it("parses legacy sidecar records for doctor migration only", async () => {
    const storePath = `/tmp/supportClaw-telegram-message-cache-legacy-${process.pid}-${Date.now()}.json`;
  ...
  it("filters conversation context nodes when an include predicate is supplied", async () => {
    ...
    for (const msg of [
      ...
      },
      {
        ...
        message_id: 601,
        date: 1736380660,
        text: "@supportclaw_bot please check this",
    ...
    const context = await buildTelegramConversationContext({
      ...
      recentLimit: 10,
      replyTargetWindowSize: 1,
      includeNode: (node) => node.body?.includes("@supportclaw_bot") === true,
    });
  ...
  it("does not select messages before the latest session reset command", async () => {
    ...
    const staleInstruction = "okay so we just flip in supportClaw? if yes do it up";
    const record = (params: {
      id: number;
      text: string;
      timestampMs: number;
