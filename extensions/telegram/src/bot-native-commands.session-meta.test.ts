...
import type { ResolvedAgentRoute } from "supportClaw/plugin-sdk/routing";
...
type ResolveConfiguredBindingRouteFn =
  typeof import("supportClaw/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("supportClaw/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("supportClaw/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
type LoadModelCatalogFn = typeof import("supportClaw/plugin-sdk/agent-runtime").loadModelCatalog;
...
vi.mock("supportClaw/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/conversation-runtime")>(
    "supportClaw/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/session-store-runtime")>(
    "supportClaw/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/command-auth-native")>(
    "supportClaw/plugin-sdk/command-auth-native",
  );
...
vi.mock("supportClaw/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/plugin-runtime")>(
    "supportClaw/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  beforeEach(() => {
    ...
    sessionMocks.resolveAndPersistSessionFile.mockClear().mockImplementation(async (params) => {
      const sessionFile =
        params.fallbackSessionFile ?? `/tmp/supportClaw-sessions/${params.sessionId}.jsonl`;
      return {
        sessionFile,
        sessionEntry: {
          ...params.sessionEntry,
    ...
    sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/supportClaw-sessions.json");
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.loadSessionStore).toHaveBeenCalledWith("/tmp/supportClaw-sessions.json");
    expectSendMessageCall({
      sendMessage,
      chatId: 100,
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/supportClaw-sessions/sessions.json");
    sessionMocks.loadSessionStore.mockReturnValue({
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as SupportClawConfig,
    ...
    expectRecordFields(
      firstMockArg(sessionMocks.resolveAndPersistSessionFile, "resolveAndPersistSessionFile"),
      {
        ...
        storePath: "/tmp/supportClaw-sessions/sessions.json",
        sessionsDir: "/tmp/supportClaw-sessions",
        fallbackSessionFile: path.resolve("/tmp/supportClaw-sessions", "sess-topic-topic-42.jsonl"),
      },
    ...
    expectRecordFields(
      (pluginRuntimeMocks.executePluginCommand.mock.calls as unknown as Array<[unknown]>)[0]?.[0],
      {
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        sessionId: "sess-topic",
        sessionFile: path.resolve("/tmp/supportClaw-sessions", "sess-topic-topic-42.jsonl"),
