...
import { escapeRegExp, formatEnvelopeTimestamp } from "supportClaw/plugin-sdk/channel-test-helpers";
import type { TelegramGroupConfig } from "supportClaw/plugin-sdk/config-contracts";
import type { GetReplyOptions, MsgContext } from "supportClaw/plugin-sdk/reply-runtime";
import { withEnvAsync } from "supportClaw/plugin-sdk/test-env";
import { sanitizeTerminalText } from "supportClaw/plugin-sdk/test-fixtures";
...
const pluginStateTestRuntime = await import("supportClaw/plugin-sdk/plugin-state-test-runtime");
const conversationRuntime = await import("supportClaw/plugin-sdk/conversation-runtime");
const configMutation = await import("supportClaw/plugin-sdk/config-mutation");
const sessionStoreRuntime = await import("supportClaw/plugin-sdk/session-store-runtime");
...
function createTelegramBotTestStateDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "supportClaw-telegram-bot-"));
...
describe("createTelegramBot", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.SUPPORT_CLAW_STATE_DIR;
    process.env.SUPPORT_CLAW_STATE_DIR = createTelegramBotTestStateDir();
    resetTelegramForumFlagCacheForTest();
  ...
  it("passes startup probe botInfo to grammY", () => {
    const botInfo = {
      id: 123456,
      is_bot: true,
      first_name: "SupportClaw",
      username: "supportClaw_bot",
  ...
  it.each(["stop", "/stop@supportclaw_bot"] as const)(
    ...
    try {
      ...
      await runTelegramMiddlewareChain({
        ctx: {
          ...
          message: {
            chat: { id: -1007, type: "supergroup", title: "SupportClaw Ops" },
            text: "first",
            date: 1736380804,
            message_id: 104,
            from: { id: 42, first_name: "Ada" },
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("supportClaw pairing approve telegram");
      expectRecordFields(
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "SupportClaw", username: "supportClaw_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
          from: { id: 7, is_bot: true, first_name: "SupportClaw", username: "supportClaw_bot" },
          text: "Binding: Review pull request 54118 (supportClaw)",
        },
      },
      me: { id: 7, is_bot: true, first_name: "SupportClaw", username: "supportClaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("ignores group self-authored message updates instead of re-processing bot output", async () => {
    ...
    await handler({
      message: {
        chat: { id: -1001234, type: "supergroup", title: "SupportClaw Ops" },
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "SupportClaw", username: "supportClaw_bot" },
  ...
  it("reloads topic agent overrides between messages without recreating the bot", async () => {
    ...
    const sendTopicMessage = async (chatId: number, messageId: number, text: string) => {
      await handler({
        ...
        me: { username: "supportClaw_bot", has_topics_enabled: true },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      });
    };
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "supportClaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("marks explicit Telegram bot-handle mentions in the inbound context", async () => {
    ...
    await dispatchMessage({
      message: {
        chat: { id: 7, type: "group", title: "Test Group" },
        text: "@supportclaw_bot status",
        entities: [{ type: "mention", offset: 0, length: "@supportclaw_bot".length }],
        date: 1736380800,
        message_id: 4,
        from: { id: 9, first_name: "Ada" },
      },
      me: { id: 999, username: "supportClaw_bot" },
    });
    ...
    expect(payload.BotUsername).toBe("supportClaw_bot");
  });
  ...
  it("honors routed group activation from session store", async () => {
    const storePath = "/tmp/supportClaw-telegram-group-activation.json";
