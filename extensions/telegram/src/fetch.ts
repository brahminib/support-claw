...
} from "supportClaw/plugin-sdk/fetch-runtime";
import {
...
} from "supportClaw/plugin-sdk/proxy-capture";
import { resolveRequestUrl } from "supportClaw/plugin-sdk/request-url";
...
// fix for the leak observed in supportClaw#68128 is the transport lifecycle that
...
function resolveSupportClawProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.SUPPORT_CLAW_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveSupportClawProxyUrlForTelegram() : undefined;
