...
vi.mock("supportClaw/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/config-mutation")>(
    "supportClaw/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/cron-store-runtime")>(
    "supportClaw/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/supportClaw.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);
