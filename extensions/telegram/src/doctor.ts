...
function formatTelegramAccountConfigPath(cfg: SupportClawConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
export function maybeRepairTelegramApiRoots(cfg: SupportClawConfig): {
...
async function repairTelegramConfig(params: { cfg: SupportClawConfig }): Promise<{
  config: SupportClawConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: SupportClawConfig): Promise<{
  config: SupportClawConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("supportClaw/plugin-sdk/runtime");
