...
import { closeActiveMemorySearchManager } from "supportClaw/plugin-sdk/memory-host-search";
import {
  asDateTimestampMs,
...
} from "supportClaw/plugin-sdk/plugin-config-runtime";
import { definePluginEntry, type SupportClawPluginApi } from "supportClaw/plugin-sdk/plugin-entry";
import { parseAgentSessionKey, parseThreadSessionSuffix } from "supportClaw/plugin-sdk/routing";
import { isPathInside } from "supportClaw/plugin-sdk/security-runtime";
import {
...
import { tempWorkspace, resolvePreferredSupportClawTmpDir } from "supportClaw/plugin-sdk/temp-path";
...
function resolveDefaultToolsAllow(cfg: SupportClawConfig | undefined): string[] {
  return cfg?.plugins?.slots?.memory === "memory-lancedb"
    ? [...LANCEDB_ACTIVE_MEMORY_TOOLS_ALLOW]
    : [...DEFAULT_ACTIVE_MEMORY_TOOLS_ALLOW];
}

function resolveToolsAllow(params: { pluginToolsAllow: unknown; cfg?: SupportClawConfig }): string[] {
  return (
...
function resolvePersistentTranscriptBaseDir(api: SupportClawPluginApi, agentId: string): string {
  return path.join(
...
function openActiveMemoryToggleStore(api: SupportClawPluginApi) {
...
function isActiveMemoryGloballyEnabled(cfg: SupportClawConfig): boolean {
...
function resolveActiveMemoryCleanupConfig(api: SupportClawPluginApi): SupportClawConfig | undefined {
  try {
    return (
      (api.runtime.config?.current?.() as SupportClawConfig | undefined) ??
      (api.config as SupportClawConfig | undefined)
    );
  } catch {
    return api.config as SupportClawConfig | undefined;
  }
}
...
async function runRecallSubagent(params: {
  ...
  const transientWorkspace = params.config.persistTranscripts
    ? undefined
    : await tempWorkspace({
        rootDir: resolvePreferredSupportClawTmpDir(),
        prefix: "supportClaw-active-memory-",
      });
...
export default definePluginEntry({
  ...
  register(api: SupportClawPluginApi) {
    const readCurrentConfig = (): SupportClawConfig | undefined => {
      try {
        return (
          (api.runtime.config?.current?.() as SupportClawConfig | undefined) ??
          (api.config as SupportClawConfig | undefined)
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (isGlobal) {
          const currentConfig = api.runtime.config.current() as SupportClawConfig;
          if (action === "status") {
            return {
