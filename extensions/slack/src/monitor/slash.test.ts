...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, /^supportClaw_cmdarg/, "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "supportClaw_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(
      Array.from(testHarness.actions.keys()).some(
        (key) => key instanceof RegExp && String(key) === String(/^supportClaw_cmdarg/),
      ),
    ).toBe(true);
    expect(testHarness.options.has("supportClaw_cmdarg")).toBe(true);
  ...
  it("shows a button menu when required args are omitted", async () => {
    ...
    expect(actions?.elements?.[0]?.action_id).toBe("supportClaw_cmdarg_0_0");
    expect(actions?.elements?.[1]?.action_id).toBe("supportClaw_cmdarg_0_1");
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("supportClaw_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("supportClaw_cmdarg");
    expect(blockId).toContain("supportClaw_cmdarg_ext:");
    const token = (blockId ?? "").slice("supportClaw_cmdarg_ext:".length);
...
describe("slack slash command session metadata", () => {
  ...
  it("refreshes slash routing config between invocations", async () => {
    ...
    const sourceCfg = (harness.ctx as { cfg: SupportClawConfig }).cfg;
    const runtimeCfg = {
      ...sourceCfg,
      session: { dmScope: "per-channel-peer" },
    } as SupportClawConfig;
    resolveAgentRouteMock.mockImplementation((params: { cfg: SupportClawConfig }) => ({
      agentId: "main",
      accountId: "acct",
      sessionKey:
