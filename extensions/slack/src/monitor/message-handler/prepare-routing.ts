...
type SlackRouteBinding = NonNullable<SupportClawConfig["bindings"]>[number];
type SlackRouteBindingPeer = NonNullable<SlackRouteBinding["match"]["peer"]>;

const slackRouteBindingConfigCache = new WeakMap<
  SupportClawConfig,
  { bindingsRef: SupportClawConfig["bindings"]; normalizedCfg: SupportClawConfig }
>();
...
function normalizeSlackRouteBindingConfig(cfg: SupportClawConfig): SupportClawConfig {
  ...
  const normalizedCfg = changed
    ? ({ ...cfg, bindings: normalizedBindings } as SupportClawConfig)
    : cfg;
