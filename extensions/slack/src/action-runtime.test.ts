...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): SupportClawConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as SupportClawConfig;
  ...
  function expectLastSlackSend(content: string, cfg: SupportClawConfig, threadTs?: string) {
  ...
  async function resolveReadToken(cfg: SupportClawConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: SupportClawConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/supportClaw-media/report.pdf",
    ...
    expect(String(firstContent.text)).toContain("/tmp/supportClaw-media/report.pdf");
    ...
    expect(details.media).toEqual({
      mediaUrl: "/tmp/supportClaw-media/report.pdf",
      outbound: false,
      contentType: "application/pdf",
    });
  });
