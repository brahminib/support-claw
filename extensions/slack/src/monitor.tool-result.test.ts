...
import { expectPairingReplyText } from "supportClaw/plugin-sdk/channel-test-helpers";
import { HISTORY_CONTEXT_MARKER } from "supportClaw/plugin-sdk/reply-history";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bsupportClaw\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("supportClaw: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("supportClaw: hello <@U2>");
  });
