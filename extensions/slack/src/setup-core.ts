...
import { hasConfiguredSecretInput } from "supportClaw/plugin-sdk/secret-input";
import {
...
} from "supportClaw/plugin-sdk/setup-runtime";
import { formatDocsLink } from "supportClaw/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: SupportClawConfig, accountId: string): SupportClawConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: SupportClawConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: SupportClawConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: SupportClawConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: SupportClawConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: SupportClawConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: SupportClawConfig; accountId: string }) =>
    ...
    disable: (cfg: SupportClawConfig) => setSetupChannelEnabled(cfg, channel, false),
