...
export function installDebugProxyTestResetHooks() {
  ...
  afterEach(async () => {
    const { closeDebugProxyCaptureStore } = await import("supportClaw/plugin-sdk/proxy-capture");
    const { closeSupportClawStateDatabaseForTest } =
      await import("supportClaw/plugin-sdk/sqlite-runtime-testing");
