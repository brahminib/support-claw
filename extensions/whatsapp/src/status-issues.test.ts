...
describe("collectWhatsAppStatusIssues", () => {
  beforeEach(() => {
    vi.stubEnv("SUPPORT_CLAW_CONTAINER_HINT", "");
    vi.stubEnv("SUPPORT_CLAW_PROFILE", "");
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Run: supportClaw channels login (scan QR on the gateway host).",
      },
    ]);
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Run: supportClaw doctor (or restart the gateway). If it persists, relink via channels login and check logs.",
      },
    ]);
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Watch: supportClaw logs --follow and run supportClaw channels status --probe if disconnects continue. If it keeps flapping, restart the gateway or relink via channels login.",
      },
    ]);
  });
