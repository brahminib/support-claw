...
import { resetInboundDedupe } from "supportClaw/plugin-sdk/reply-runtime";
...
const WEB_AUTO_REPLY_SOCKETS_KEY = Symbol.for("supportClaw:webAutoReplySessionSockets");
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-session-"));
