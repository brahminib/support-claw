...
import { resolveAccountEntry } from "supportClaw/plugin-sdk/account-core";
import { CHANNEL_APPROVAL_NATIVE_RUNTIME_CONTEXT_CAPABILITY } from "supportClaw/plugin-sdk/approval-handler-runtime";
import { resolveInboundDebounceMs } from "supportClaw/plugin-sdk/channel-inbound-debounce";
import { registerChannelRuntimeContext } from "supportClaw/plugin-sdk/channel-runtime-context";
import { formatCliCommand } from "supportClaw/plugin-sdk/cli-runtime";
import { isControlCommandMessage } from "supportClaw/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "supportClaw/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "supportClaw/plugin-sdk/reply-history";
import { resolveAgentRoute } from "supportClaw/plugin-sdk/routing";
import { logVerbose } from "supportClaw/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "supportClaw/plugin-sdk/runtime-env";
import { getChildLogger } from "supportClaw/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  warn,
  type RuntimeEnv,
} from "supportClaw/plugin-sdk/runtime-env";
    ...
    while (true) {
        ...
        if (setupDecision) {
          ...
          if (setupDecision.action === "stop") {
            ...
            if (setupDecision.healthState === "logged-out") {
              runtime.error(
                `WhatsApp session logged out during setup. Run \`${formatCliCommand("supportClaw channels login --channel whatsapp")}\` to relink.`,
              );
            } else if (setupDecision.healthState === "conflict") {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then restart the channel. To force a fresh QR, run \`${formatCliCommand("supportClaw channels logout --channel whatsapp")}\` before \`${formatCliCommand("supportClaw channels login --channel whatsapp")}\`. Stopping web monitoring.`,
              );
            } else {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}) after ${setupDecision.reconnectAttempts}/${reconnectPolicy.maxAttempts} attempts. Relink with \`${formatCliCommand("supportClaw channels login --channel whatsapp")}\` if the issue persists.`,
              );
            }
            await controller.shutdown();
            break;
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("supportClaw channels login --channel whatsapp")}\` to relink.`,
          );
