...
let resetLogger: typeof import("supportClaw/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("supportClaw/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: SupportClawConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("supportClaw/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `supportClaw-outbound-${crypto.randomUUID()}.log`);
