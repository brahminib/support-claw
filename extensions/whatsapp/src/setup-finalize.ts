...
import type { ChannelSetupWizard } from "supportClaw/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "supportClaw/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<SupportClawConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: SupportClawConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: SupportClawConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: SupportClawConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: SupportClawConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("supportClaw channels login"),
      }),
      "WhatsApp",
    );
  }
