...
import { VERSION } from "supportClaw/plugin-sdk/cli-runtime";
import {
  createHttp1EnvHttpProxyAgent,
  createHttp1ProxyAgent,
  createNodeProxyAgent,
} from "supportClaw/plugin-sdk/fetch-runtime";
import { danger, success } from "supportClaw/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "supportClaw/plugin-sdk/runtime-env";
...
export async function createWaSocket(
  ...
  const sock = makeWASocket({
    ...
    browser: ["supportClaw", "cli", VERSION],
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("supportClaw channels login")}`,
              ),
            );
          }
        }
