...
import { info, success } from "supportClaw/plugin-sdk/runtime-env";
import { getChildLogger } from "supportClaw/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "supportClaw/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("supportClaw channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
