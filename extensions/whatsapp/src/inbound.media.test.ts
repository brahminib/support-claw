...
vi.mock("supportClaw/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/channel-pairing")>(
...
vi.mock("supportClaw/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/media-store")>(
    "supportClaw/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
vi.mock("./runtime.js", async () => {
  const { createChannelIngressQueueForTests: createChannelIngressQueue } = await Promise.resolve(
    vi.importActual<typeof import("supportClaw/plugin-sdk/plugin-state-test-runtime")>(
      "supportClaw/plugin-sdk/plugin-state-test-runtime",
    ),
  );
  const stateDir = `/tmp/supportClaw-whatsapp-inbound-media-${Date.now()}-${Math.random()}`;
  return {
...
const HOME = path.join(os.tmpdir(), `supportClaw-inbound-media-${crypto.randomUUID()}`);
