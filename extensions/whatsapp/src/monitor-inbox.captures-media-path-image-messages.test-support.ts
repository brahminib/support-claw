...
vi.mock("supportClaw/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/logging-core")>(
    "supportClaw/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("SupportClaw WhatsApp listener close");
