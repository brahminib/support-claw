...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "supportClaw-test-web-logout-"));
    previousOAuthDir = process.env.SUPPORT_CLAW_OAUTH_DIR;
    process.env.SUPPORT_CLAW_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete through symlinked auth dirs inside the SupportClaw auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the SupportClaw auth root", async () => {
