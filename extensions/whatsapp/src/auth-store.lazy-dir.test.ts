...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["SUPPORT_CLAW_STATE_DIR", "SUPPORT_CLAW_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-wa-profile-"));
  ...
  it("resolves the default web auth dir from SUPPORT_CLAW_STATE_DIR at call time", async () => {
