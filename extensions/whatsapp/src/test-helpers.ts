...
import { formatEnvelopeTimestamp } from "supportClaw/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("supportClaw:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("supportClaw:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[supportClaw]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/supportClaw-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("supportClaw:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("supportClaw:lastSocket")];
  if (typeof getter === "function") {
