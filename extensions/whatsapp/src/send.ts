...
import { generateSecureUuid } from "supportClaw/plugin-sdk/core";
import { redactIdentifier } from "supportClaw/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "supportClaw/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "supportClaw/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "supportClaw/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "supportClaw/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: SupportClawConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`supportClaw channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: SupportClawConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
