...
import { createQueuedWizardPrompter } from "supportClaw/plugin-sdk/plugin-test-runtime";
import { DEFAULT_ACCOUNT_ID } from "supportClaw/plugin-sdk/routing";
...
vi.mock("supportClaw/plugin-sdk/setup", async () => {
  ...
  return {
    ...
    setSetupChannelEnabled: (cfg: SupportClawConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<SupportClawConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
