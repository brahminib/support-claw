...
import { resolveStateDir } from "supportClaw/plugin-sdk/state-paths";
import { resolvePreferredSupportClawTmpDir } from "supportClaw/plugin-sdk/temp-path";
import { captureEnv } from "supportClaw/plugin-sdk/test-env";
...
import { withMockedWindowsPlatform, withRestoredMocks } from "supportClaw/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "supportClaw/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub SUPPORT_CLAW_STATE_DIR.
    // Also keep it outside the SupportClaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["SUPPORT_CLAW_STATE_DIR"]);
    process.env.SUPPORT_CLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "supportClaw-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredSupportClawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/supportClaw` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredSupportClawTmpDir();
  ...
  it("allows default SupportClaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default SupportClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
