...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__supportClaw_channel_id";
...
type InteractionCallbackConfig = Pick<SupportClawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "supportClaw-mattermost-interactions").update(botToken).digest("hex");
}
