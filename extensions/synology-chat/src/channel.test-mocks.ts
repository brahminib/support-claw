...
vi.mock("supportClaw/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("supportClaw/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("supportClaw/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("supportClaw/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/supportClaw/synology-chat-sessions.json"),
