  ...
  if (await hasLegacyHermesAuthJson(params.source.authPath)) {
    items.push(
      createMigrationManualItem({
        ...
        message:
          "Hermes auth.json contains legacy OAuth credentials. SupportClaw no longer imports those into live auth during Hermes migration.",
        recommendation:
          "Run supportClaw models auth login --provider openai after migration, or run supportClaw doctor --fix for existing SupportClaw legacy auth state.",
      }),
    );
  }
