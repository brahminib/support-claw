...
import { resolveAuthStorePathForDisplay } from "supportClaw/plugin-sdk/agent-runtime";
...
vi.mock("supportClaw/plugin-sdk/provider-auth", async (importOriginal) => ({
  ...(await importOriginal<typeof import("supportClaw/plugin-sdk/provider-auth")>()),
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-hermes-secret-failure-"));
  tempRoots.add(root);
  return root;
}
