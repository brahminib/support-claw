...
import { createCapturedPluginRegistration } from "supportClaw/plugin-sdk/plugin-test-runtime";
...
describe("Hermes migration provider", () => {
  ...
  it("resolves tilde source paths against the OS home when SUPPORT_CLAW_HOME is set", () => {
    const previous = process.env.SUPPORT_CLAW_HOME;
    process.env.SUPPORT_CLAW_HOME = path.join(path.sep, "tmp", "supportClaw-home");
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.SUPPORT_CLAW_HOME = previous;
      }
    }
  });
