...
import { canResolveEnvSecretRefInReadOnlyPath } from "supportClaw/plugin-sdk/extension-shared";
import { resolvePositiveTimeoutSeconds } from "supportClaw/plugin-sdk/provider-web-fetch";
import { resolveSecretInputString, normalizeSecretInput } from "supportClaw/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<SupportClawConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<SupportClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: SupportClawConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: SupportClawConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: SupportClawConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: SupportClawConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: SupportClawConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: SupportClawConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: SupportClawConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: SupportClawConfig, override?: number): number {
