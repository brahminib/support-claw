...
import { buildOauthProviderAuthResult } from "supportClaw/plugin-sdk/provider-auth";
import { createProviderApiKeyAuthMethod } from "supportClaw/plugin-sdk/provider-auth-api-key";
...
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "supportClaw/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "supportClaw/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: SupportClawConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: SupportClawConfig): string | undefined {
...
export function registerMinimaxProviders(api: SupportClawPluginApi) {
