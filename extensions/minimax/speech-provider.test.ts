...
describe("buildMinimaxSpeechProvider", () => {
  ...
  describe("isConfigured", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "supportClaw-minimax-tts-auth-"));
  ...
  describe("synthesize", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "supportClaw-minimax-tts-synth-"));
      tempAgentDir = path.join(tempStateDir, "agents", "main", "agent");
      await mkdir(tempAgentDir, { recursive: true });
      process.env = {
        ...savedEnv,
        SUPPORT_CLAW_AGENT_DIR: tempAgentDir,
        SUPPORT_CLAW_STATE_DIR: tempStateDir,
      };
      clearMinimaxAuthEnv();
      clearRuntimeAuthProfileStoreSnapshots();
