...
const EAGER_BROWSER_CONTROL_SERVICE_ENV = "SUPPORT_CLAW_EAGER_BROWSER_CONTROL_SERVER";
...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage SupportClaw's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via SupportClaw's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated SupportClaw-managed browser (`supportClaw`).",
...
function createBrowserToolOptions(ctx: SupportClawPluginToolContext): {
  sandboxBridgeUrl?: string;
  allowHostControl?: boolean;
  agentSessionKey?: string;
  agentDir?: string;
...
export const browserPluginNodeHostCommands: SupportClawPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: SupportClawPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): SupportClawPluginService {
  let service: SupportClawPluginService | null = null;
...
export function registerBrowserPlugin(api: SupportClawPluginApi) {
  api.registerTool(((ctx: SupportClawPluginToolContext) =>
    createLazyBrowserTool(createBrowserToolOptions(ctx))) as SupportClawPluginToolFactory);
  api.registerCli(
    async ({ program }) => {
