 ...
 */
import type { SupportClawPluginSecurityAuditContext } from "supportClaw/plugin-sdk/plugin-entry";
...
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "supportClaw/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: SupportClawPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("supportClaw security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.SUPPORT_CLAW_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.SUPPORT_CLAW_GATEWAY_PASSWORD) ||
