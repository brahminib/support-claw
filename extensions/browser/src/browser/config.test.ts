...
import {
  ...
  SUPPORT_CLAW_BROWSER_HEADLESS_ENV,
...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("supportClaw");
    expect(profile?.driver).toBe("supportClaw");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const supportClaw = resolveProfile(resolved, "supportClaw");
    expect(supportClaw?.driver).toBe("supportClaw");
    expect(supportClaw?.cdpPort).toBe(18800);
    expect(supportClaw?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from SUPPORT_CLAW_GATEWAY_PORT when unset", () => {
    withEnv({ SUPPORT_CLAW_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const supportClaw = resolveProfile(resolved, "supportClaw");
      expect(supportClaw?.cdpPort).toBe(19012);
      expect(supportClaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ SUPPORT_CLAW_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const supportClaw = resolveProfile(resolved, "supportClaw");
      expect(supportClaw?.cdpPort).toBe(19022);
      expect(supportClaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(supportClaw?.cdpPort).toBe(19000);
    expect(supportClaw?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "supportClaw");
  ...
  describe("managed browser headless mode", () => {
    const noDisplayEnv = {
      ...
      [SUPPORT_CLAW_BROWSER_HEADLESS_ENV]: undefined,
    };
    ...
    it("lets explicit profile headless=false beat the Linux no-display fallback", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        profiles: {
          supportClaw: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      });
      const profile = resolveProfile(resolved, "supportClaw")!;
    ...
    it("lets SUPPORT_CLAW_BROWSER_HEADLESS override profile/global config", () => {
      const resolved = resolveBrowserConfig({
        profiles: {
          supportClaw: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          platform: "linux",
          env: { ...noDisplayEnv, [SUPPORT_CLAW_BROWSER_HEADLESS_ENV]: "1" },
        }),
    ...
    it("lets request-local headless override beat env and profile/global config", () => {
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          ...
          platform: "linux",
          env: { ...noDisplayEnv, [SUPPORT_CLAW_BROWSER_HEADLESS_ENV]: "0" },
        }),
    ...
    it("returns an actionable error only when headed mode is explicitly selected", () => {
      ...
      const defaultProfile = resolveProfile(defaultResolved, "supportClaw")!;
      expect(
      ...
      const profile = resolveProfile(profileResolved, "supportClaw")!;
      expect(
        getManagedBrowserMissingDisplayError(profileResolved, profile, {
          platform: "linux",
          env: noDisplayEnv,
        }),
      ).toContain("browser.profiles.supportClaw.headless=false");
    });
  });
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "supportClaw")!;
  ...
  describe("default profile preference", () => {
    it("defaults to supportClaw profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("supportClaw");
    });

    it("keeps supportClaw default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("supportClaw");
    ...
    it("keeps supportClaw default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("supportClaw");
    ...
    it("keeps supportClaw default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
