...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "supportClaw" | "existing-session";
      attachOnly?: boolean;
    }
  >;
  tabUrl: string;
...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.SUPPORT_CLAW_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "supportClaw",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/supportClaw" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    supportClaw: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("../config/config.js", async () => {
  ...
  const mutateConfigFile = vi.fn(
    async (params: {
      ...
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/supportClaw.json" } });
      await writeConfigFile(draft);
      return {
        path: "/tmp/supportClaw.json",
        previousHash: "test-hash",
        persistedHash: "test-hash",
        snapshot: { path: "/tmp/supportClaw.json" },
        nextConfig: draft,
        result,
        attempts: 1,
...
vi.mock("./chrome.js", () => ({
  ...
  launchSupportClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveSupportClawUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopSupportClawChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "supportClaw";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.SUPPORT_CLAW_GATEWAY_PORT;
  process.env.SUPPORT_CLAW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.SUPPORT_CLAW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.SUPPORT_CLAW_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.SUPPORT_CLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.SUPPORT_CLAW_GATEWAY_PASSWORD;
  } else {
    process.env.SUPPORT_CLAW_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
