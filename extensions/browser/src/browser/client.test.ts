...
describe("browser client", () => {
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    expect(doctorResult.profile).toBe("supportClaw");
    ...
    expect(deepDoctorResult.profile).toBe("supportClaw");
    ...
    expect(urls.some((url) => url.endsWith("/doctor?profile=supportClaw&deep=true"))).toBe(true);
    ...
    const deepDoctor = calls.find((c) => c.url.endsWith("/doctor?profile=supportClaw&deep=true"));
