...
async function runBrowserLiveProbe(req: BrowserRequest, ctx: BrowserRouteContext) {
  ...
  try {
    ...
    return {
      ...
      status: "fail" as const,
      summary: String(err),
      fixHint: "Run supportClaw browser start, then retry with supportClaw browser doctor --deep.",
    };
  }
}
...
function parseHeadlessStartOverride(params: {
  ...
  if (
    params.profileCtx.profile.driver !== "supportClaw" ||
    ...
    jsonError(
      params.res,
      400,
      `Headless start override is only supported for locally launched supportClaw profiles. Profile "${params.profileCtx.profile.name}" is attach-only, remote, or existing-session.`,
    );
    return { ok: false };
  }
...
export function registerBrowserBasicRoutes(app: BrowserRouteRegistrar, ctx: BrowserRouteContext) {
  ...
  app.post(
    ...
    asyncBrowserRoute(async (req, res) => {
      ...
      if (driver && driver !== "supportClaw" && driver !== "clawd" && driver !== "existing-session") {
        return jsonError(
          res,
          400,
          `unsupported profile driver "${driver}"; use "supportClaw", "clawd", or "existing-session"`,
        );
      }
          ...
          await service.createProfile({
            ...
            driver:
              driver === "existing-session"
                ? "existing-session"
                : driver === "supportClaw" || driver === "clawd"
                  ? "supportClaw"
                  : undefined,
          }),
      });
