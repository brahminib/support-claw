 ...
 */
import { parseBrowserHttpUrl, redactCdpUrl } from "supportClaw/plugin-sdk/browser-config";
  ...
  try {
    ...
    const res = await withManagedProxyForCdpUrl(fetchUrl, () =>
      withNoProxyForCdpUrl(url, async () => {
        ...
        // Loopback CDP is an SupportClaw control plane, not page navigation. Allow
