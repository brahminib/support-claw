...
vi.mock("supportClaw/plugin-sdk/ssrf-runtime-internal", () => ({
...
vi.mock("../infra/tmp-supportClaw-dir.js", () => ({
  resolvePreferredSupportClawTmpDir: () => "/tmp/supportClaw-browser-test",
}));
...
import {
  buildSupportClawChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchSupportClawChrome,
  resolveSupportClawUserDataDir,
  stopSupportClawChrome,
} from "./chrome.js";
 ...
 * not exercise: launchSupportClawChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopSupportClawChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildSupportClawChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveSupportClawUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveSupportClawUserDataDir();
      expect(dir.endsWith(path.join("supportClaw", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveSupportClawUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildSupportClawChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildSupportClawChromeLaunchArgs({
    ...
    it("lets a request headless override beat env and profile headed settings", () => {
      const args = buildSupportClawChromeLaunchArgs({
        ...
        env: { SUPPORT_CLAW_BROWSER_HEADLESS: "0" },
      });
      expect(args).toContain("--headless=new");
      expect(args).toContain("--disable-gpu");
    });
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchSupportClawChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchSupportClawChrome(resolved, profile);
  ...
  describe("launchSupportClawChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "supportClaw-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchSupportClawChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it.each([
      ...
      async ({ cdpUrl, configuredProbeHost }) => {
        ...
        await expect(launchSupportClawChrome(makeResolved(), profile)).rejects.toThrow(portBusy);
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchSupportClawChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("accepts a ready CDP diagnostic after the launch HTTP probe expires", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          const port = new URL(baseUrl).port;
          const profile = makeProfile(Number(port));
          const running = await launchSupportClawChrome(
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      const configPath = path.join(tmpDir, "supportClaw.json");
      await fsp.writeFile(
        configPath,
        JSON.stringify({
          logging: {
      ...
      vi.stubEnv("SUPPORT_CLAW_CONFIG_PATH", configPath);
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const userDataDir = resolveSupportClawUserDataDir(profile.name);
        ...
        for (const testCase of [
          ...
          await withMockChromeCdpServer({
            ...
            run: async (baseUrl) => {
              ...
              const userDataDir = resolveSupportClawUserDataDir(`${profile.name}-${testCase.pid}`);
              ...
              try {
                await expect(
                  launchSupportClawChrome(makeResolved(), profileWithUniqueName),
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchSupportClawChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("stopSupportClawChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopSupportClawChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopSupportClawChrome>[0],
        1,
      );
  ...
  describe("launchSupportClawChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "supportClaw-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveSupportClawUserDataDir(profileName));
        ...
        const staged = resolveSupportClawUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_SUPPORT_CLAW_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      const configDir = await fsp.mkdtemp(path.join(os.tmpdir(), "supportClaw-redact-off-"));
      ...
      let message = "";
      try {
        await launchSupportClawChrome(resolved, profile);
      } catch (err) {
    ...
    it("logs a warning when decorateSupportClawProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateSupportClawProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateSupportClawProfile }, "decorateSupportClawProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".supportClaw-profile-decorated") || s.endsWith("Preferences")) {
  ...
  describe("launchSupportClawChrome managed-proxy CDP bypass", () => {
    const makeLoopbackProfile = (cdpPort: number): ResolvedBrowserProfile =>
      ({
        name: "supportClaw-bypass",
        color: "#FF4500",
        cdpPort,
        cdpUrl: `http://127.0.0.1:${cdpPort}`,
        cdpIsLoopback: true,
    ...
    it("surfaces loopbackMode=block as BrowserProfileUnavailableError without spawning Chrome", async () => {
      ...
      await expect(launchSupportClawChrome(makeResolved(), profile)).rejects.toBeInstanceOf(
    ...
    it("does not register a bypass for a remote attachOnly CDP URL (loopback gate)", async () => {
      stubExecutableAndPrefsExist();
      // For this test we want launchSupportClawChrome to reject before any
      ...
      const remoteProfile = {
        name: "supportClaw-remote",
        color: "#FF4500",
        cdpPort: 19222,
        cdpUrl: "http://browserless.example.com:19222",
        cdpIsLoopback: false,
      } as unknown as ResolvedBrowserProfile;
      await expect(launchSupportClawChrome(makeResolved(), remoteProfile)).rejects.toThrow(
        /is remote; cannot launch local Chrome/,
      );
