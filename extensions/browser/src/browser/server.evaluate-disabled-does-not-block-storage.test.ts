...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        supportClaw: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.SUPPORT_CLAW_GATEWAY_PORT;
    process.env.SUPPORT_CLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.SUPPORT_CLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.SUPPORT_CLAW_GATEWAY_PASSWORD;
