...
function localSupportClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "supportClaw",
...
function createLocalSupportClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localSupportClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveSupportClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-reset-"));
    const profileDir = path.join(tempRoot, "supportClaw");
    ...
    const ops = createLocalSupportClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveSupportClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "supportClaw");
    fs.mkdirSync(profileDir, { recursive: true });
