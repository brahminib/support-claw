/**
 * SupportClaw-managed Chrome profile decoration.
 *
...
import { loadJsonFile, saveJsonFile } from "supportClaw/plugin-sdk/json-store";
import {
...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".supportClaw-profile-decorated");
}
 ...
 */
export function decorateSupportClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SUPPORT_CLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SUPPORT_CLAW_BROWSER_COLOR).toUpperCase();
