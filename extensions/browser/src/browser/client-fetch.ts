 ...
 */
import { parseBrowserHttpUrl } from "supportClaw/plugin-sdk/browser-config";
...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-supportClaw-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-supportClaw-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-supportClaw-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function resolveBrowserFetchOperatorHint(
  ...
  if (opts?.ownership === "external-browser") {
    return (
      "The browser profile is external to SupportClaw; make sure its browser/CDP endpoint " +
      "is running and reachable. Restarting the SupportClaw gateway will not launch it."
    );
  }
  const isLocal = !isAbsoluteHttp(url);
  return isLocal
    ? `Restart the SupportClaw gateway (SupportClaw.app menubar, or \`${formatCliCommand("supportClaw gateway")}\`).`
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  if (kind === "timeout") {
    return new Error(
      `Can't reach the SupportClaw browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`,
  ...
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the SupportClaw browser control service. ${operatorHint} (${msg})`,
    ),
    err instanceof Error ? { cause: err } : undefined,
  );
}
