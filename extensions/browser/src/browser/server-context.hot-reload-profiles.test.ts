...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", () => {
    // Start with only supportClaw profile
    ...
    // 2. Simulate adding a new profile to config (like user editing supportClaw.json)
  ...
  it("forProfile refreshes existing profile config after getRuntimeConfig cache updates", () => {
    ...
    mockState.cfgProfiles.supportClaw = { cdpPort: 19999, color: "#FF4500" };
    ...
    expect(state.resolved.profiles.supportClaw?.cdpPort).toBe(19999);
  });
  ...
  it("marks existing runtime state for reconcile when profile invariants change", () => {
    const cfg = getRuntimeConfig();
    const resolved = resolveBrowserConfig(cfg.browser, cfg);
    const supportClawProfile = requireValue(
      resolveProfile(resolved, "supportClaw"),
      "supportClaw profile missing",
    );
    ...
    const runtime = requireValue(state.profiles.get("supportClaw"), "supportClaw runtime missing");
  ...
  it("marks local managed runtime state for reconcile when profile headless changes", () => {
    ...
    expect(supportClawProfile.headless).toBe(true);
    const state: BrowserServerState = {
      server: null,
      port: 18791,
      resolved,
    ...
    mockState.cfgProfiles.supportClaw = {
      cdpPort: 18800,
      color: "#FF4500",
      headless: false,
    };
  ...
  it("marks local managed runtime state for reconcile when profile executablePath changes", () => {
    ...
    expect(supportClawProfile.executablePath).toBe("/usr/bin/chrome-old");
    const state: BrowserServerState = {
      server: null,
      port: 18791,
      resolved,
  ...
  it("does not reconcile remote cdp runtime when only headless changes", () => {
    ...
    expect(remoteProfile.driver).toBe("supportClaw");
    ...
    const runtime = requireValue(state.profiles.get("remote"), "remote runtime missing");
    expect(runtime.profile.driver).toBe("supportClaw");
