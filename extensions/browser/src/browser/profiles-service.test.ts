...
import { resolveSupportClawUserDataDir } from "./chrome.js";
import type { BrowserRouteContext, BrowserServerState } from "./server-context.js";
import { movePathToTrash } from "./trash.js";

const configMocks = vi.hoisted(() => ({
  getRuntimeConfig: vi.fn<() => SupportClawConfig>(),
  writeConfigFile: vi.fn<(cfg: SupportClawConfig) => Promise<void>>(async (_cfg) => {}),
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (draft: SupportClawConfig, context: { snapshot: { path: string } }) => unknown;
    }) => {
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: SupportClawConfig }) => {
...
vi.mock("./chrome.js", () => ({
  resolveSupportClawUserDataDir: vi.fn(() => "/tmp/supportClaw-test/supportClaw/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "supportClaw-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveSupportClawUserDataDir).mockReturnValue(userDataDir);
