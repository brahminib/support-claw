/**
 * SupportClaw-managed Chrome lifecycle and CDP helpers.
 *
...
import { prepareOomScoreAdjustedSpawn } from "supportClaw/plugin-sdk/process-runtime";
...
import {
  DEFAULT_SUPPORT_CLAW_BROWSER_COLOR,
  DEFAULT_SUPPORT_CLAW_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
async function ensureManagedChromePortAvailable(
  ...
  // Chromium tries IPv4 loopback first, while SupportClaw polls the configured endpoint.
...
function chromeLaunchHints(params: {
  ...
  if (CHROME_MISSING_DISPLAY_PATTERN.test(params.stderrOutput) && !headlessMode.headless) {
    hints.push(
      "No DISPLAY/X server was detected. Set SUPPORT_CLAW_BROWSER_HEADLESS=1, remove the headed override, start Xvfb, or run the Gateway in a desktop session.",
    );
  }
  if (CHROME_SINGLETON_IN_USE_PATTERN.test(params.stderrOutput)) {
    hints.push(
      `The Chromium profile "${params.profile.name}" is locked. Stop the existing browser or remove stale Singleton* lock files under ~/.supportClaw/browser/${params.profile.name}/user-data.`,
    );
  }
...
export type RunningChrome = {
   ...
   * through stopSupportClawChrome without type churn.
   */
...
/** Resolve the user-data-dir path for a managed SupportClaw Chrome profile. */
export function resolveSupportClawUserDataDir(profileName = DEFAULT_SUPPORT_CLAW_BROWSER_PROFILE_NAME) {
...
/** Build Chrome launch arguments for the managed SupportClaw browser. */
export function buildSupportClawChromeLaunchArgs(params: {
  resolved: ResolvedBrowserConfig;
  profile: ResolvedBrowserProfile;
  userDataDir: string;
...
/** Launch or attach to the managed SupportClaw Chrome profile. */
export async function launchSupportClawChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  launchOptions: ManagedBrowserHeadlessOptions = {},
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_SUPPORT_CLAW_BROWSER_COLOR).toUpperCase(),
  ...
  const spawnOnce = () => {
    ...
    if (process.platform === "linux") {
      const chromiumStateDir = path.join(resolvePreferredSupportClawTmpDir(), ".chromium");
  ...
  if (needsDecorate) {
    try {
      decorateSupportClawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
        downloadDir: DEFAULT_DOWNLOAD_DIR,
      });
      log.info(`🦞 supportClaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`supportClaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`supportClaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  const launchOnceAndWait = async (allowSingletonRecovery: boolean): Promise<RunningChrome> => {
    ...
    try {
      ...
      log.info(
        `🦞 supportClaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
      );
...
export async function stopSupportClawChrome(
