...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredSupportClawTmpDir: vi.fn(() => "/tmp/supportClaw"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-supportClaw-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-supportClaw-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredSupportClawTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredSupportClawTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredSupportClawTmpDir.mockReturnValue("/tmp/supportClaw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredSupportClawTmpDir.mockReturnValue("/tmp/supportClaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "supportClaw-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "supportClaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredSupportClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).not.toBe(
      path.resolve(path.join(path.sep, "tmp", "supportClaw-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toContain(path.basename(res.path));
    expect(path.basename(outPath)).toMatch(/\.part$/);
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "supportClaw-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredSupportClawTmpDir.mockReturnValue(tempDir);
