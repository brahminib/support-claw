...
describe("normalizeBrowserEvaluateFunctionSource", () => {
  ...
  it("wraps expressions as page functions", () => {
    expect(normalizeBrowserEvaluateFunctionSource("document.title")).toBe(
      [
        "() => {",
        "const __supportClawEvaluateExpressionResult = (document.title);",
        'return typeof __supportClawEvaluateExpressionResult === "function" ? __supportClawEvaluateExpressionResult() : __supportClawEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
  });
  ...
  it("preserves function-valued expression invocation", () => {
    expect(normalizeBrowserEvaluateFunctionSource("extractTitle")).toBe(
      [
        "() => {",
        "const __supportClawEvaluateExpressionResult = (extractTitle);",
        'return typeof __supportClawEvaluateExpressionResult === "function" ? __supportClawEvaluateExpressionResult() : __supportClawEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
    expect(normalizeBrowserEvaluateFunctionSource("extractText", { argumentName: "el" })).toBe(
      [
        "(el) => {",
        "const __supportClawEvaluateExpressionResult = (extractText);",
        'return typeof __supportClawEvaluateExpressionResult === "function" ? __supportClawEvaluateExpressionResult(el) : __supportClawEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
  });
