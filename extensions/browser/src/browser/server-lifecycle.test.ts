...
const { stopSupportClawChromeMock } = vi.hoisted(() => ({
  stopSupportClawChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopSupportClawChrome: stopSupportClawChromeMock,
}));
...
beforeEach(() => {
  ...
  stopSupportClawChromeMock.mockClear();
});

describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["supportClaw", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      supportClaw: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.supportClaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopSupportClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("supportClaw browser stop failed: Error: oops");
  });
});
