...
describe("profile CRUD endpoints", () => {
  ...
  it("validates profile create/delete endpoints", async () => {
    ...
    const createDuplicate = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "supportClaw" }),
    });
    ...
    const explicitUserDataDir = "/tmp/supportClaw-brave-profile";
    ...
    const deleteDefault = await realFetch(`${base}/profiles/supportClaw`, {
      method: "DELETE",
    });
