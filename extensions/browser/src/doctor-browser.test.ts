...
describe("browser doctor readiness", () => {
  ...
  it("warns when managed browser profiles have no local executable", async () => {
    ...
    expect(noteFn).toHaveBeenCalledWith(
      [
        "- SupportClaw-managed browser profile(s) are configured: supportClaw.",
  ...
  it("warns about legacy clawd managed browser profile residue", async () => {
    const noteFn = vi.fn();
    const configDir = "/tmp/supportClaw-home";
    ...
    expect(note).toContain("/tmp/supportClaw-home/browser/clawd");
    expect(note).toContain("/tmp/supportClaw-home/browser/supportClaw/user-data");
    expect(note).toContain("supportClaw doctor --fix");
  });
  ...
  it("does not warn when clawd is still configured as a browser profile", async () => {
    ...
    await noteChromeMcpBrowserReadiness(
      ...
      },
      {
        ...
        env: { DISPLAY: ":99" },
        getUid: () => 1000,
        configDir: "/tmp/supportClaw-home",
        pathExists: () => true,
        resolveManagedExecutable: () => ({ kind: "chrome", path: "/usr/bin/google-chrome" }),
      },
    );
...
describe("legacy clawd browser profile cleanup", () => {
  it("archives stale clawd residue with the safe trash mover", async () => {
    const movePathToTrash = vi.fn(async () => "/tmp/supportClaw-home/browser/.trash/clawd");
    ...
    expect(movePathToTrash).toHaveBeenCalledWith("/tmp/supportClaw-home/browser/clawd");
    expect(result.warnings).toStrictEqual([]);
    expect(result.changes.join("\n")).toContain(
      "Archived legacy clawd managed browser profile residue.",
    );
    expect(result.changes.join("\n")).toContain("/tmp/supportClaw-home/browser/supportClaw/user-data");
  });
