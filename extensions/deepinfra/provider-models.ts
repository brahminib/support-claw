...
import { isProviderApiKeyConfigured } from "supportClaw/plugin-sdk/provider-auth";
import {
...
import { asPositiveSafeInteger } from "supportClaw/plugin-sdk/string-coerce-runtime";
...
export const DEEPINFRA_MODELS_URL = `${DEEPINFRA_BASE_URL}/models?sort_by=supportClaw&filter=with_meta`;
...
// Static fallback. Chat rows live in supportClaw.plugin.json (manifest-validated);
...
// `/v1/openai/models?sort_by=supportClaw&filter=with_meta` projection is the
...
// `isProviderApiKeyConfigured` helper (covers SecretRef / `SUPPORT_CLAW_LIVE_*`
// indirection too).
