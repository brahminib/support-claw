...
async function createWorkspace(): Promise<string> {
  const workspaceDir = await mkdtemp(path.join(tmpdir(), "supportClaw-voice-context-"));
...
describe("buildRealtimeVoiceInstructions", () => {
  it("injects bounded identity and workspace context", async () => {
    ...
    expect(instructions).toContain("SupportClaw agent voice context:");
    expect(instructions).toContain("Consult behavior:");
    expect(instructions).toContain("Call supportClaw_agent_consult before answering requests");
