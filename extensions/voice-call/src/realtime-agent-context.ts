...
import { buildRealtimeVoiceAgentConsultPolicyInstructions } from "supportClaw/plugin-sdk/realtime-voice";
import { root } from "supportClaw/plugin-sdk/security-runtime";
import { normalizeOptionalString as normalizeString } from "supportClaw/plugin-sdk/string-coerce-runtime";
...
export async function buildRealtimeVoiceInstructions(params: {
  ...
  const capsule: string[] = [
    "SupportClaw agent voice context:",
    `- Agent id: ${agentId}`,
    "- Use this context to match the SupportClaw agent's personality and standing preferences on fast voice turns.",
    "- Treat this as compact context only; call supportClaw_agent_consult when the caller needs the full agent brain, tools, memory, or workspace state.",
  ];

  if (contextConfig.includeIdentity) {
    const identity = params.agentRuntime.resolveAgentIdentity(
      params.coreConfig as SupportClawConfig,
      agentId,
