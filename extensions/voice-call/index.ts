// Voice Call plugin entrypoint registers its SupportClaw integration.
import { formatErrorMessage } from "supportClaw/plugin-sdk/error-runtime";
import { ErrorCodes, errorShape } from "supportClaw/plugin-sdk/gateway-runtime";
...
const voiceCallConfigSchema = {
  ...
  uiHints: {
    ...
    "realtime.toolPolicy": {
      ...
      help: "Controls the shared supportClaw_agent_consult tool.",
      advanced: true,
    },
    "realtime.consultPolicy": {
      label: "Realtime Consult Policy",
      help: "Guides when the realtime voice model should call supportClaw_agent_consult.",
      advanced: true,
    },
...
const VoiceCallToolSchema = Type.Union([
  Type.Object({
    ...
    sessionKey: Type.Optional(Type.String({ description: "SupportClaw session key for the call" })),
    requesterSessionKey: Type.Optional(
      Type.String({ description: "SupportClaw session key that initiated the call" }),
    ),
...
function isCliOnlyProcess(): boolean {
  return process.env.SUPPORT_CLAW_CLI === "1" && !process.argv.slice(2).includes("gateway");
}

const VOICE_CALL_RUNTIME_KEY = Symbol.for("supportClaw.voice-call.runtime");
const VOICE_CALL_RUNTIME_PROMISE_KEY = Symbol.for("supportClaw.voice-call.runtimePromise");
const VOICE_CALL_RUNTIME_STOP_PROMISE_KEY = Symbol.for("supportClaw.voice-call.runtimeStopPromise");
