...
import { readPluginPackageVersion } from "supportClaw/plugin-sdk/extension-shared";
...
const USER_AGENT = `supportClaw-parallel/${PLUGIN_VERSION} (${process.platform})`;
...
export async function executeParallelWebSearchProviderTool(
  ...
  // Generic `query` arg fallback: supportClaw's operator-facing CLI
  // (`supportClaw capability web.search ...`) always passes the shared
  ...
  // Always pass max_results so Parallel matches the supportClaw web_search default
