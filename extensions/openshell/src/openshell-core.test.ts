...
import type { CreateSandboxBackendParams } from "supportClaw/plugin-sdk/sandbox";
import {
...
describe("openshell backend manager", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await makeTempDir("supportClaw-openshell-real-");
      const root = path.join(realParent, "sandbox");
      const target = path.join(root, ".supportClaw", "sandbox-skills");
  ...
  it("checks runtime status with config override from SupportClaw config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "supportClaw-session-1234",
        backendId: "openshell",
        runtimeLabel: "supportClaw-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "supportClaw-session-1234",
        config: expectedConfig,
      },
      args: ["sandbox", "get", "supportClaw-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "supportClaw-session-5678",
        backendId: "openshell",
        runtimeLabel: "supportClaw-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "supportClaw",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "supportClaw-session-5678",
        config: expectedConfig,
      },
      args: ["sandbox", "delete", "supportClaw-session-5678"],
    });
  });
  ...
  it("preserves a local sandbox skills shadow when mirror sync crosses filesystems", async () => {
    const workspaceDir = await makeTempDir("supportClaw-openshell-workspace-");
    const shadowFile = path.join(workspaceDir, ".supportClaw", "sandbox-skills", "user-note.txt");
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        ...
        await fs.mkdir(path.join(tmpDir, ".supportClaw", "sandbox-skills", "skills"), {
          recursive: true,
        });
        await fs.writeFile(
          path.join(tmpDir, ".supportClaw", "sandbox-skills", "skills", "generated.txt"),
          "generated",
          "utf8",
        );
      }
    ...
    try {
      ...
      await expectPathMissing(
        path.join(workspaceDir, ".supportClaw", "sandbox-skills", "skills", "generated.txt"),
      );
    } finally {
  ...
  it("drops non-directory materialized sandbox skills from mirror downloads", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = args[4];
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.mkdir(path.join(tmpDir, ".supportClaw"), { recursive: true });
        await fs.writeFile(path.join(tmpDir, ".supportClaw", "sandbox-skills"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expectPathMissing(path.join(workspaceDir, ".supportClaw", "sandbox-skills"));
  });
  ...
  it("restores a local sandbox skills shadow when mirror download has a file parent", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = args[4];
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.writeFile(path.join(tmpDir, ".supportClaw"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expect(fs.readFile(shadowFile, "utf8")).resolves.toBe("local shadow");
    expect((await fs.stat(path.join(workspaceDir, ".supportClaw"))).isDirectory()).toBe(true);
  });
});
...
function createOpenShellBackendSandboxConfig(): CreateSandboxBackendParams["cfg"] {
  return {
    ...
    ssh: createSandboxSshConfig("/tmp/supportClaw-sandboxes"),
...
async function makeExecutable(params: { name: string; script: string }): Promise<string> {
  const dir = await makeTempDir("supportClaw-openshell-bin-");
...
describe("openshell fs bridges", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const stateDir = await makeTempDir("supportClaw-openshell-remote-pin-");
  ...
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("supportClaw-openshell-fs-");
  ...
  it("rejects symlink-parent writes instead of escaping the local mount root", async () => {
    const workspaceDir = await makeTempDir("supportClaw-openshell-fs-");
    const outsideDir = await makeTempDir("supportClaw-openshell-outside-");
  ...
  it("reads materialized sandbox skills from the protected skills workspace", async () => {
    const workspaceDir = await makeTempDir("supportClaw-openshell-fs-");
    const skillsWorkspaceDir = await makeTempDir("supportClaw-openshell-skills-");
    ...
    await expect(
      bridge.readFile({
        filePath: "/sandbox/.supportClaw/sandbox-skills/skills/demo/SKILL.md",
      }),
    ).resolves.toEqual(Buffer.from("# Demo\nmaterialized\n"));
    await expect(
      bridge.readFile({
        filePath: ".supportClaw/sandbox-skills/skills/demo/SKILL.md",
      }),
    ).resolves.toEqual(Buffer.from("# Demo\nmaterialized\n"));
    await expect(
      bridge.writeFile({
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("supportClaw-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("supportClaw-openshell-agent-");
