...
import { createSandboxTestContext } from "supportClaw/plugin-sdk/test-fixtures";
import {
...
const SUPPORT_CLAW_OPENSHELL_E2E = process.env.SUPPORT_CLAW_E2E_OPENSHELL === "1";
const SUPPORT_CLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const SUPPORT_CLAW_OPENSHELL_COMMAND =
  process.env.SUPPORT_CLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "supportClaw-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && SUPPORT_CLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: SUPPORT_CLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(SUPPORT_CLAW_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(SUPPORT_CLAW_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-openshell-e2e-"));
      ...
      const gatewayName = `supportClaw-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `supportClaw-policy-allow-${scopeSuffix}`;
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: SUPPORT_CLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("supportClaw-openshell-e2e");
