...
metadata: { "supportClaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == SupportClaw `sessions_spawn`
- **File I/O** == SupportClaw `read`/`write`
- **Remote fetch** == SupportClaw `web_fetch` (or `exec` with curl when POST is required)
