...
function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): SupportClawConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): SupportClawConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
function expectOllamaWebSearchRequest(
  ...
  expect(request).toEqual({
    ...
    init: {
      ...
      body: JSON.stringify({
        query: params.query ?? "supportClaw",
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the local Ollama proxy endpoint", async () => {
    ...
    expect(result.query).toBe("supportClaw docs");
    expect(result.provider).toBe("ollama");
    expect(result.count).toBe(1);
    expectSingleSearchResultUrl(result.results, "https://supportClaw.ai/docs");
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest supportClaw release" })).rejects.toThrow(
      "ollama signin",
    );
  });
