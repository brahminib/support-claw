...
import { isBlockedHostnameOrIp } from "supportClaw/plugin-sdk/ssrf-runtime";
import { asRecord, normalizeOptionalString } from "supportClaw/plugin-sdk/string-coerce-runtime";
...
export function getGoogleMeetSetupStatus(
  ...
  checks.push({
    ...
    message: config.chrome.browserProfile
      ? "Local Chrome uses the SupportClaw browser profile; chrome.browserProfile is passed to chrome-node hosts"
      : "Local Chrome uses the SupportClaw browser profile; configure browser.defaultProfile to choose another profile",
  });
