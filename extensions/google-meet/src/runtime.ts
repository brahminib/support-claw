...
import type { PluginRuntime, RuntimeLogger } from "supportClaw/plugin-sdk/plugin-runtime";
import { sleep } from "supportClaw/plugin-sdk/runtime-env";
import { normalizeOptionalString, uniqueStrings } from "supportClaw/plugin-sdk/string-coerce-runtime";
import type {
  GoogleMeetConfig,
  GoogleMeetMode,
  GoogleMeetModeInput,
  ...
  if (health?.manualActionRequired) {
    return {
      ...
      message:
        ...
        "Resolve the Google Meet browser prompt before asking SupportClaw to speak.",
    };
  }
  if (health?.inCall === true) {
    if (health.micMuted === true) {
      return {
        ...
        message: "Turn on the SupportClaw Google Meet microphone before asking SupportClaw to speak.",
      };
    }
    if (session.chrome.audioBridge) {
      return { ready: true };
