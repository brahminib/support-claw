...
async function evaluatePolicyUncached(ctx: HealthCheckContext): Promise<PolicyEvaluation> {
  ...
  if (
    ...
    findings.push({
      ...
      requirement: "oc://supportClaw.config/plugins/entries/policy/config/expectedHash",
  ...
  return [
    {
      ...
      requirement: "oc://supportClaw.config/plugins/entries/policy/config/expectedAttestationHash",
...
function isGroupIngressDisabled(
  ...
  const channelDefaultsParent = "oc://supportClaw.config/channels/defaults";
    ...
    if (invalidIndex >= 0) {
      findings.push(
        policyShapeFinding(
          ...
          "Use non-empty source names such as env, file, exec, or supportClaw.",
        ),
      );
    }
  }
...
function resolvePolicyArtifactHomeDir(): string | undefined {
  const explicitHome = normalizedEnvValue(process.env.SUPPORT_CLAW_HOME);
  ...
  return [
    ...new Set(
      findings
        ...
        .map((finding) => finding.ocPath?.match(/^oc:\/\/supportClaw\.config\/channels\/(.+)$/)?.[1])
...
function canonicalExecApprovalsPath(): string {
  return "~/.supportClaw/exec-approvals.json";
}
...
function execApprovalsArtifactLocation(ctx: HealthCheckContext): {
  ...
  const stateDir = normalizedEnvValue(process.env.SUPPORT_CLAW_STATE_DIR);
