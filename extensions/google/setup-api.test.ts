...
import type { CliBackendPlugin } from "supportClaw/plugin-sdk/cli-backend";
...
describe("google gemini cli backend auth bridge", () => {
  it("materializes selected SupportClaw OAuth credentials into a persistent profile-scoped Gemini CLI home", async () => {
    const backend = buildGoogleGeminiCliBackend();
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-test-workspace-"));
    ...
    try {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify({
          ...
          mcp: { allowed: ["supportClaw"] },
          mcpServers: { supportClaw: { url: "http://127.0.0.1:23119/mcp" } },
        })}\n`,
        "utf8",
      );
      ...
      expect(
        path.relative(resolvePreferredSupportClawTmpDir(), path.dirname(systemSettingsPath ?? "")),
      ).toMatch(/^supportClaw-gemini-cli-/);
