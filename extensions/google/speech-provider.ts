...
import { transcodeAudioBufferToOpus } from "supportClaw/plugin-sdk/media-runtime";
import {
...
import { asObject, trimToUndefined } from "supportClaw/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: SupportClawConfig): string | undefined {
...
function isSupportClawGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isSupportClawGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
