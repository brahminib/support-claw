...
import type { FixedWindowRateLimiter } from "supportClaw/plugin-sdk/webhook-ingress";
...
vi.mock("supportClaw/plugin-sdk/webhook-request-guards", () => ({
  readJsonWebhookBodyOrReject,
}));

vi.mock("supportClaw/plugin-sdk/webhook-targets", () => ({
...
describe("googlechat monitor webhook", () => {
  ...
  afterAll(() => {
    vi.doUnmock("supportClaw/plugin-sdk/webhook-request-guards");
    vi.doUnmock("supportClaw/plugin-sdk/webhook-targets");
    vi.doUnmock("./auth.js");
    vi.resetModules();
  });
    ...
    expect(processEvent).toHaveBeenCalledWith(
      {
        ...
        action: {
          parameters: [
            { key: "supportClaw_action", value: "approval" },
