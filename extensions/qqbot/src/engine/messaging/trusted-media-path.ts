import { resolvePreferredSupportClawTmpDir } from "supportClaw/plugin-sdk/sandbox";
import { resolveLocalPathFromRootsSync } from "supportClaw/plugin-sdk/security-runtime";
...
function trustedSupportClawTmpRoot(): string | null {
  if (cachedTrustedTmpRoot === undefined) {
    try {
      cachedTrustedTmpRoot = resolvePreferredSupportClawTmpDir();
    } catch {
      return null;
    }
  }
 ...
 * roots, this also trusts SupportClaw's permission-hardened temp root, where
...
export function resolveTrustedOutboundMediaPath(
  ...
  const tmpRoot = trustedSupportClawTmpRoot();
  if (!tmpRoot) {
    return null;
  }
  return (
    resolveLocalPathFromRootsSync({
      filePath: p,
      roots: [tmpRoot],
      label: "SupportClaw temp media root",
