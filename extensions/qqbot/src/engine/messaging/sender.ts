...
let supportClawVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${pluginVersion} (Node/${process.versions.node}; ${os.platform()}; SupportClaw/${supportClawVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; supportClawVersion?: string }): void {
  if (options.pluginVersion) {
    pluginVersion = options.pluginVersion;
  }
  if (options.supportClawVersion) {
    supportClawVersion = options.supportClawVersion;
  }
}

/** Update the SupportClaw framework version in the User-Agent (called after runtime injection). */
export function setSupportClawVersion(version: string): void {
  if (version) {
