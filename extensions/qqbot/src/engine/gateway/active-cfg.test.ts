...
const getRuntimeConfigMock = vi.hoisted(() => vi.fn<() => SupportClawConfig>());

vi.mock("supportClaw/plugin-sdk/runtime-config-snapshot", () => ({
  getRuntimeConfig: getRuntimeConfigMock,
}));

function asCfg(shape: { bindings: Array<{ id: string }> }): SupportClawConfig {
  return shape as unknown as SupportClawConfig;
}
