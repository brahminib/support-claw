...
import { isImplicitSameChatApprovalAuthorization } from "supportClaw/plugin-sdk/approval-auth-runtime";
...
async function handleApprovalButtonInteraction(params: {
  ...
  event: InteractionEvent;
  getActiveCfg?: () => SupportClawConfig | Record<string, unknown>;
  log?: EngineLogger;
  ...
  try {
    cfg = params.getActiveCfg() as SupportClawConfig;
  } catch (err) {
    await acknowledgeApprovalInteraction(params.creds, params.event, params.log, {
      content: "Approval is unavailable.",
    });
