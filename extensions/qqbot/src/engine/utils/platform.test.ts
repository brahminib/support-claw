...
describe("qqbot local media path remapping", () => {
  ...
  function createSupportClawTestRoot() {
    const actualHome = getHomeDir();
    const supportClawDir = path.join(actualHome, ".supportClaw");
    fs.mkdirSync(supportClawDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(supportClawDir, "qqbot-platform-test-"));
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createSupportClawTestRoot();
  ...
  it("allows structured payload files inside sibling SupportClaw media subdirectories", () => {
    ...
    // trust the shared `~/.supportClaw/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".supportClaw", "media", "outbound");
...
// Regression coverage for https://github.com/brahminib/support-claw/issues/83562 —
// when HOME and SUPPORT_CLAW_HOME diverge (Docker, multi-user hosts), QQ Bot media
// paths must be anchored on SUPPORT_CLAW_HOME so files written under
// `$SUPPORT_CLAW_HOME/.supportClaw/media/qqbot/` are accepted by the outbound
// allowlist.
//
...
// home as the baseline and only varies `process.env.SUPPORT_CLAW_HOME`.
describe("qqbot media path resolution honors SUPPORT_CLAW_HOME (#83562)", () => {
  ...
  function makeFakeSupportClawHome(): string {
  ...
  it("accepts files under $SUPPORT_CLAW_HOME/.supportClaw/media/qqbot when SUPPORT_CLAW_HOME differs from HOME", () => {
    const fakeSupportClawHome = makeFakeSupportClawHome();
    vi.stubEnv("SUPPORT_CLAW_HOME", fakeSupportClawHome);

    const mediaFile = path.join(fakeSupportClawHome, ".supportClaw", "media", "qqbot", "repro.png");
    ...
    const oldHomeMediaRoot = path.join(realOsHome, ".supportClaw", "media", "qqbot");
    expect(isPathInsideOrEqual(mediaFile, oldHomeMediaRoot)).toBe(false);
    fs.mkdirSync(path.dirname(mediaFile), { recursive: true });
    fs.writeFileSync(mediaFile, "image", "utf8");

    expect(getQQBotMediaPath()).toBe(path.join(fakeSupportClawHome, ".supportClaw", "media", "qqbot"));
    expect(resolveQQBotPayloadLocalFilePath(mediaFile)).toBe(fs.realpathSync(mediaFile));
  });

  it("expands tilde-prefixed SUPPORT_CLAW_HOME against the OS home", () => {
    ...
    vi.stubEnv("SUPPORT_CLAW_HOME", `~/${sub}`);

    expect(getQQBotMediaPath()).toBe(path.join(expectedHome, ".supportClaw", "media", "qqbot"));

    const mediaFile = path.join(expectedHome, ".supportClaw", "media", "qqbot", "tilde.png");
    fs.mkdirSync(path.dirname(mediaFile), { recursive: true });
    fs.writeFileSync(mediaFile, "image", "utf8");
  ...
  it("falls back to OS home when SUPPORT_CLAW_HOME is unset (no regression)", () => {
    vi.stubEnv("SUPPORT_CLAW_HOME", "");

    expect(getQQBotMediaPath()).toBe(path.join(realOsHome, ".supportClaw", "media", "qqbot"));
  });

  it("treats sentinel strings 'undefined' and 'null' as unset", () => {
    for (const sentinel of ["undefined", "null"]) {
      vi.stubEnv("SUPPORT_CLAW_HOME", sentinel);
      expect(getQQBotMediaPath()).toBe(path.join(realOsHome, ".supportClaw", "media", "qqbot"));
    }
  });
  ...
  it("keeps persisted QQ Bot data anchored on the OS home (compatibility)", () => {
    ...
    // operator adds SUPPORT_CLAW_HOME — otherwise existing deployments would lose
    // their session state. Only the media root follows SUPPORT_CLAW_HOME.
    expect(getQQBotDataPath()).toBe(path.join(realOsHome, ".supportClaw", "qqbot"));
  });

  it("rejects files that live under HOME tree when SUPPORT_CLAW_HOME is the active root", () => {
    const fakeSupportClawHome = makeFakeSupportClawHome();
    vi.stubEnv("SUPPORT_CLAW_HOME", fakeSupportClawHome);

    // File under the HOME-side mirror — exactly the path that *worked* on
    // current main and *broke* the SUPPORT_CLAW_HOME setup. After the fix the
    // active media root is SUPPORT_CLAW_HOME, so a file under HOME is no longer
    ...
    const homeOnlyFile = path.join(realOsHome, ".supportClaw", "media", "qqbot", stale);
  ...
  it("remaps workspace paths under either HOME or SUPPORT_CLAW_HOME to the SUPPORT_CLAW_HOME media root", () => {
    const fakeSupportClawHome = makeFakeSupportClawHome();
    vi.stubEnv("SUPPORT_CLAW_HOME", fakeSupportClawHome);

    const baseName = `remap-${process.pid}-${Date.now()}`;

    // Real file lives under the SUPPORT_CLAW_HOME media tree.
    const mediaFile = path.join(
      fakeSupportClawHome,
      ".supportClaw",
      "media",
    ...
    const homeWorkspaceDir = path.join(realOsHome, ".supportClaw", "workspace", "qqbot");
    ...
    // HOME-side path, so nothing to clean up there beyond the SUPPORT_CLAW_HOME tree.
    expect(resolveQQBotLocalMediaPath(homeWorkspacePath)).toBe(mediaFile);

    // Same path but under SUPPORT_CLAW_HOME should also remap.
    const supportClawWorkspacePath = path.join(
      fakeSupportClawHome,
      ".supportClaw",
      "workspace",
      "qqbot",
    ...
    expect(resolveQQBotLocalMediaPath(supportClawWorkspacePath)).toBe(mediaFile);
  });
});
