...
import { withEnv } from "supportClaw/plugin-sdk/test-env";
...
describe("qqbot legacy credential backup paths", () => {
  ...
  it("scopes legacy credential backup imports to the active SUPPORT_CLAW_STATE_DIR", () => {
  ...
  it("keeps legacy account import paths isolated across different state directories", () => {
    ...
    const gatewayAPath = withEnv({ SUPPORT_CLAW_STATE_DIR: stateDirA }, () =>
      getCredentialBackupFile("default"),
    );
    const gatewayBPath = withEnv({ SUPPORT_CLAW_STATE_DIR: stateDirB }, () =>
      getCredentialBackupFile("default"),
    );
  ...
  it("uses SUPPORT_CLAW_HOME for default legacy credential backup imports", () => {
    const homeDir = createTempDir("qqbot-supportClaw-home-");
    withEnv({ SUPPORT_CLAW_STATE_DIR: "", SUPPORT_CLAW_HOME: homeDir }, () => {
      expect(getCredentialBackupFile("default")).toBe(
        path.join(homeDir, ".supportClaw", "qqbot", "data", "credential-backup-default.json"),
      );
    });
  });
  ...
  it("expands tilde state-dir overrides through the canonical state resolver", () => {
    ...
    withEnv({ HOME: homeDir, SUPPORT_CLAW_HOME: "", SUPPORT_CLAW_STATE_DIR: "~/gateway-a" }, () => {
      expect(getCredentialBackupFile("default")).toBe(
