 ...
 * silk-wasm availability checks — all without importing `supportClaw/plugin-sdk`.
 ...
 * `SUPPORT_CLAW_HOME`. QQ Bot still checks this tree for legacy state imports and
...
/**
 * Resolve the effective SupportClaw home directory.
 *
 * Mirrors the contract from core (`src/infra/home-dir.ts::resolveEffectiveHomeDir`)
 * so QQ Bot media roots live under the same tree the rest of SupportClaw treats as
 * `~`. The extension cannot import the core helper directly (it is a separate
 * package with `supportClaw` as a peer dependency), so this re-implements the
 * minimal contract:
 *
 * 1. `SUPPORT_CLAW_HOME` when set (with `~` / `~/...` expanded against the OS home).
 ...
 */
function resolveSupportClawHome(): string {
  const raw = process.env.SUPPORT_CLAW_HOME?.trim();
...
/**
 * Return a legacy path under `~/.supportClaw/qqbot` without creating it.
 *
...
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".supportClaw", "qqbot", ...subPaths);
}

/** Return a path under `~/.supportClaw/qqbot`, creating it on demand. */
...
/**
 * Return a path under `<supportClaw-home>/.supportClaw/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under SupportClaw's core media allowlist
 * so downloaded images and audio can be accessed by framework media tooling.
 * The base honors `SUPPORT_CLAW_HOME` (when set) so files written by agents into
 * the SupportClaw-managed media tree are reachable by this plugin even when
 * `HOME` and `SUPPORT_CLAW_HOME` differ (Docker, multi-user hosts). Fixes #83562.
 */
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(resolveSupportClawHome(), ".supportClaw", "media", "qqbot", ...subPaths);
}

/** Return a path under `<supportClaw-home>/.supportClaw/media/qqbot`, creating it on demand. */
...
/**
 * Return `<supportClaw-home>/.supportClaw/media`, SupportClaw's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `supportClaw/src/media/local-roots.ts`). Using it as a
 ...
 * {@link getQQBotMediaPath}, the base honors `SUPPORT_CLAW_HOME`.
 */
function getSupportClawMediaDir(): string {
  return path.join(resolveSupportClawHome(), ".supportClaw", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const supportClawHomeDir = resolveSupportClawHome();
  const mediaRoot = getQQBotMediaPath();
  const dataRoot = getQQBotDataPath();
  // When SUPPORT_CLAW_HOME differs from HOME we have to consider workspace roots
  // under both trees: agents may be configured with `~`-relative paths (HOME)
  // or with the SupportClaw-managed home tree. Deduplicate when they match.
  const workspaceRoots = Array.from(
    new Set([
      path.join(osHomeDir, ".supportClaw", "workspace", "qqbot"),
      path.join(supportClawHomeDir, ".supportClaw", "workspace", "qqbot"),
    ]),
  );
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and SupportClaw's shared `~/.supportClaw/media`
  ...
  const allowedRoots = [getSupportClawMediaDir(), getQQBotMediaPath()];
