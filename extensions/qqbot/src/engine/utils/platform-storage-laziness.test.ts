...
describe("qqbot storage laziness", () => {
  ...
  it("does not create ~/.supportClaw/qqbot from module imports or read-only probes", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".supportClaw", "qqbot");
  ...
  it("creates storage when qqbot persists runtime state", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".supportClaw", "qqbot");
    const sqlitePath = path.join(stateDir, "state", "supportClaw.sqlite");
