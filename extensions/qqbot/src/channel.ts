...
import { getExecApprovalReplyMetadata } from "supportClaw/plugin-sdk/approval-runtime";
import {
...
import { sanitizeAssistantVisibleText } from "supportClaw/plugin-sdk/text-chunking";
...
import { toGatewayAccount, writeSupportClawConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped supportClaw.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // supportClaw.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeSupportClawConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe supportClaw.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeSupportClawConfigThroughRuntime(getQQBotRuntime(), nextCfg as SupportClawConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as SupportClawConfig, accountId);
