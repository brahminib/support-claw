...
import { coerceSecretRef, normalizeSecretInputString } from "supportClaw/plugin-sdk/secret-input";
...
export function listQQBotAccountIds(cfg: SupportClawConfig): string[] {
  return listAccountIds(cfg as unknown as Record<string, unknown>);
}

/** Resolve the default QQBot account ID. */
export function resolveDefaultQQBotAccountId(cfg: SupportClawConfig): string {
...
/** Apply account config updates back into the SupportClaw config object. */
