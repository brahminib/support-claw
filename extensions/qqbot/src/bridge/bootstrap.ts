...
let mediaRuntimeModulePromise: Promise<typeof import("supportClaw/plugin-sdk/media-runtime")> | null =
  null;

const loadMediaRuntimeModule = async () => {
  mediaRuntimeModulePromise ??= import("supportClaw/plugin-sdk/media-runtime");
...
function createBuiltinAdapter(): PlatformAdapter {
  return {
    ...
    async resolveApproval(approvalId: string, decision: string): Promise<boolean> {
      try {
        const { getRuntimeConfig } = await import("supportClaw/plugin-sdk/runtime-config-snapshot");
        const { resolveApprovalOverGateway } =
          await import("supportClaw/plugin-sdk/approval-gateway-runtime");
