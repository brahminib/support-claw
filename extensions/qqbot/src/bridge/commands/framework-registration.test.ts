...
function registerCommands(): SupportClawPluginCommandDefinition[] {
  ensurePlatformAdapter();
  const commands: SupportClawPluginCommandDefinition[] = [];
  const api = {
    logger: {},
    registerCommand: (command: SupportClawPluginCommandDefinition) => {
...
function findCommand(
  commands: SupportClawPluginCommandDefinition[],
  name: string,
): SupportClawPluginCommandDefinition {
...
describe("registerQQBotFrameworkCommands", () => {
  ...
  it("preserves the private-chat guard for bot-streaming on generic framework calls", async () => {
    ...
    const writes: SupportClawConfig[] = [];
