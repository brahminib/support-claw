...
import type { GroupToolPolicyConfig } from "supportClaw/plugin-sdk/channel-policy";
import type { SupportClawConfig } from "supportClaw/plugin-sdk/config-contracts";
import { asObjectRecord } from "supportClaw/plugin-sdk/runtime-doctor";
...
export const legacyConfigRules: ChannelDoctorLegacyConfigRule[] = [
  {
    ...
    message:
      'channels.qqbot.groups.<id>.toolPolicy is legacy and was ignored by QQBot group tool enforcement; use channels.qqbot.groups.<id>.tools instead. Run "supportClaw doctor --fix".',
    match: hasLegacyGroupToolPolicy,
  },
  {
    path: ["channels", "qqbot", "accounts"],
    message:
      'channels.qqbot.accounts.<id>.groups.<groupId>.toolPolicy is legacy and was ignored by QQBot group tool enforcement; use channels.qqbot.accounts.<id>.groups.<groupId>.tools instead. Run "supportClaw doctor --fix".',
...
export function normalizeCompatibilityConfig({
  ...
  return {
    config: {
      ...
      channels: {
        ...cfg.channels,
        qqbot: updated as unknown as NonNullable<SupportClawConfig["channels"]>["qqbot"],
