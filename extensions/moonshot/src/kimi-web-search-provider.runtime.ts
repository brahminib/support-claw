...
function resolveKimiBaseUrl(kimi?: KimiConfig, supportClawConfig?: SupportClawConfig): string {
  ...
  const moonshotBaseUrl = supportClawConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: SupportClawConfig; searchConfig?: SearchConfigRecord },
  ...
  if (!result.grounded) {
    return {
      ...
      provider: "kimi",
      model,
      docs: "https://docs.supportClaw.ai/tools/kimi-search",
