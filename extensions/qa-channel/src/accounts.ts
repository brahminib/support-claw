...
import { resolveMergedAccountConfig } from "supportClaw/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "supportClaw";
  const botDisplayName = merged.botDisplayName?.trim() || "SupportClaw QA";
  return {
    accountId,
    enabled,
