...
function hasTrustedSupportClawRootIndicator(packageRoot, packageJson) {
  ...
  const hasSupportClawBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("supportClaw")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.supportClaw === "string");
  const hasSupportClawEntrypoint = fs.existsSync(path.join(packageRoot, "supportClawCli.mjs"));
  return hasCliEntryExport || hasSupportClawBin || hasSupportClawEntrypoint;
}

function findSupportClawPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "supportClaw" && hasTrustedSupportClawRootIndicator(cursor, pkg)) {
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findSupportClawPackageRoot(moduleDir);
  if (location) {
