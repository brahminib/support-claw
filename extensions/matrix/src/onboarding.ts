...
import type { DmPolicy } from "supportClaw/plugin-sdk/config-contracts";
import {
...
import { isPrivateNetworkOptInEnabled } from "supportClaw/plugin-sdk/ssrf-policy";
import {
...
async function configureMatrixInviteAutoJoin(params: {
  ...
  await params.prompter.note(
    [
      ...
      "SupportClaw agents will not join invited rooms or fresh DM-style invites unless you set autoJoin.",
  ...
  const deviceName =
    normalizeStringifiedOptionalString(
      await params.prompter.text({
        ...
        initialValue: existing.deviceName ?? "SupportClaw Gateway",
      }),
    ) ?? "";
