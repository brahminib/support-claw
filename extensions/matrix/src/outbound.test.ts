...
describe("matrixOutbound cfg threading", () => {
  ...
  it("passes resolved cfg to sendMessageMatrix for media sends", async () => {
    ...
    await matrixOutbound.sendMedia!({
      ...
      mediaLocalRoots: ["/tmp/supportClaw"],
      accountId: "default",
      audioAsVoice: true,
    });
    ...
    expect(options.mediaLocalRoots).toEqual(["/tmp/supportClaw"]);
  ...
  it("renders MessagePresentation into Matrix custom content metadata", async () => {
    ...
    expect(matrixData.extraContent?.["com.supportClaw.presentation"]).toEqual({
      ...presentation,
      version: 1,
    ...
    await matrixOutbound.sendPayload!({
      ...
      payload: {
        ...
        channelData: {
          matrix: {
            extraContent: {
              "com.supportClaw.presentation": presentationContent,
            },
          },
        },
      },
