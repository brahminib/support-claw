...
import { writeJsonFileAtomically } from "supportClaw/plugin-sdk/json-store";
import { resolveRequiredHomeDir, resolveStateDir } from "supportClaw/plugin-sdk/state-paths";

const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "supportClaw-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  outputDir?: string;
  createBackupArchive?: typeof import("supportClaw/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("supportClaw/plugin-sdk/runtime")).createBackupArchive;
