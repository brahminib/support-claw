...
import { createReplyToFanout } from "supportClaw/plugin-sdk/channel-outbound";
import {
  renderMessagePresentationFallbackText,
  type MessagePresentation,
} from "supportClaw/plugin-sdk/interactive-runtime";
import { resolvePayloadMediaUrls } from "supportClaw/plugin-sdk/reply-payload";
...
const MATRIX_SUPPORT_CLAW_PRESENTATION_KEY = "com.supportClaw.presentation" as const;
const MATRIX_SUPPORT_CLAW_PRESENTATION_TYPE = "message.presentation" as const;
...
function buildMatrixPresentationContent(presentation: MessagePresentation) {
  return {
    ...presentation,
    version: 1,
    type: MATRIX_SUPPORT_CLAW_PRESENTATION_TYPE,
  };
}
...
function resolveMatrixPresentationContent(
  ...
  const presentation = toRecord(extraContent?.[MATRIX_SUPPORT_CLAW_PRESENTATION_KEY]);
  if (
    !presentation ||
    presentation.version !== 1 ||
    presentation.type !== MATRIX_SUPPORT_CLAW_PRESENTATION_TYPE
  ) {
    return undefined;
  }
  return presentation;
  ...
  return {
    ...
    channelData: {
      ...
      matrix: {
        ...matrixData,
        extraContent: {
          [MATRIX_SUPPORT_CLAW_PRESENTATION_KEY]: buildMatrixPresentationContent(params.presentation),
        },
      },
    },
  };
...
function resolveMatrixExtraContent(payload: ReplyPayload): MatrixExtraContentFields | undefined {
  ...
  return presentation ? { [MATRIX_SUPPORT_CLAW_PRESENTATION_KEY]: presentation } : undefined;
}
