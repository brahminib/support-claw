...
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "supportClaw/plugin-sdk/account-id";
  ...
  if (!channel) {
    return {
      warning:
        ...
        'Configure Matrix, then rerun "supportClaw doctor --fix" or restart the gateway.',
    };
  }
  if (requiresExplicitMatrixDefaultAccount(params.cfg)) {
    return {
      warning:
        ...
        'Set "channels.matrix.defaultAccount" to the intended target account before rerunning "supportClaw doctor --fix" or restarting the gateway.',
    };
  }
  ...
  if (!target) {
    ...
    return {
      warning:
        ...
        'Start the gateway once with a working Matrix login, or rerun "supportClaw doctor --fix" after cached credentials are available.',
    };
  }
