...
import { getSessionBindingService, testing } from "supportClaw/plugin-sdk/session-binding-runtime";
...
describe("matrix thread bindings", () => {
  ...
  async function readPersistedBindings(bindingsPath: string) {
    const store = createPluginStateKeyedStoreForTests<{
      ...
      maxEntries: 10_000,
      env: { ...process.env, SUPPORT_CLAW_STATE_DIR: path.dirname(bindingsPath) },
    });
    return {
      version: 1,
