...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'supportClaw matrix verify status --verbose --account ${accountId}' and 'supportClaw matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another SupportClaw runtime is using the same account. " +
    `Check 'supportClaw matrix verify status --verbose --account ${accountId}' and 'supportClaw matrix devices list --account ${accountId}'.`
  );
}
