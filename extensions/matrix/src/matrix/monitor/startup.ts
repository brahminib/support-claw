...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleSupportClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale SupportClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleSupportClawDevices.map((device) => device.deviceId).join(", ")}. Run 'supportClaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
