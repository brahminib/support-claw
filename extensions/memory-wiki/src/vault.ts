...
import { FsSafeError, pathExists, root as fsRoot } from "supportClaw/plugin-sdk/security-runtime";
...
- Use \`.supportClaw-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the SupportClaw memory-wiki plugin.
...
- \`.supportClaw-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.

## Notes
<!-- supportClaw:human:start -->
<!-- supportClaw:human:end -->
`);
}
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(rootDir, ".supportClaw-wiki/log.jsonl", "", createdFiles);
