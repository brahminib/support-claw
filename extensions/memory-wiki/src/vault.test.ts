...
describe("initializeMemoryWikiVault", () => {
  it("creates the wiki layout and seed files", async () => {
    ...
    await expect(fs.access(path.join(rootDir, ".supportClaw-wiki", "state.json"))).rejects.toThrow(
      /ENOENT/,
    );
    await expect(fs.access(path.join(rootDir, ".supportClaw-wiki", "locks"))).rejects.toThrow(
      /ENOENT/,
    );
  });
