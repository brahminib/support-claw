...
import type { PluginStateEntry } from "supportClaw/plugin-sdk/plugin-state-runtime";
...
export function createMemoryWikiTestHarness() {
  ...
  async function createTempDir(prefix: string): Promise<string> {
    const tempDir = await fs.mkdtemp(path.join(resolvePreferredSupportClawTmpDir(), prefix));
