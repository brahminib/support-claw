...
import { runTasksWithConcurrency } from "supportClaw/plugin-sdk/concurrency-runtime";
import { retryTransientMemoryRead } from "supportClaw/plugin-sdk/memory-core-host-engine-storage";
import {
  replaceManagedMarkdownBlock,
  withTrailingNewline,
} from "supportClaw/plugin-sdk/memory-host-markdown";
...
const AGENT_DIGEST_PATH = ".supportClaw-wiki/cache/agent-digest.json";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- supportClaw:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- supportClaw:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- supportClaw:wiki:index:start -->",
      endMarker: "<!-- supportClaw:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- supportClaw:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- supportClaw:wiki:${group.dir}:index:end -->`,
