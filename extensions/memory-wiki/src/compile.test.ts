...
describe("compileMemoryWikiVault", () => {
  ...
  it("writes root and directory indexes for native markdown", async () => {
    ...
    const agentDigest = JSON.parse(
      await fs.readFile(path.join(rootDir, ".supportClaw-wiki", "cache", "agent-digest.json"), "utf8"),
    ) as {
      claimCount: number;
    ...
    await expect(
      fs.readFile(path.join(rootDir, ".supportClaw-wiki", "cache", "claims.jsonl"), "utf8"),
