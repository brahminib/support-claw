...
import { MAX_DATE_TIMESTAMP_MS, MAX_TIMER_TIMEOUT_MS } from "supportClaw/plugin-sdk/number-runtime";
import type {
  SupportClawConfig,
...
type RegisteredMemoryEmbeddingProvider = Parameters<
  SupportClawPluginApi["registerMemoryEmbeddingProvider"]
>[0];
type RegisteredProvider = Parameters<SupportClawPluginApi["registerProvider"]>[0];
type GithubCopilotTestProvider = RegisteredProvider & {
  auth: Array<{
    run: (ctx: unknown) => Promise<ProviderAuthResult | null>;
    runNonInteractive: (ctx: unknown) => Promise<SupportClawConfig | null>;
  }>;
  catalog: {
...
async function createAgentDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-github-copilot-test-"));
  tempDirs.push(dir);
  return dir;
}
...
function registerProviderAndCatalogWithPluginConfig(pluginConfig: Record<string, unknown>) {
  const registerProviderMock = vi.fn<SupportClawPluginApi["registerProvider"]>();
  const registerModelCatalogProviderMock =
    vi.fn<SupportClawPluginApi["registerModelCatalogProvider"]>();
...
describe("github-copilot plugin", () => {
  ...
  it("registers embedding provider", () => {
    const registerMemoryEmbeddingProviderMock =
      vi.fn<SupportClawPluginApi["registerMemoryEmbeddingProvider"]>();
