...
import { createProviderUsageFetch, makeResponse } from "supportClaw/plugin-sdk/test-env";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { deriveCopilotApiBaseUrlFromToken, resolveCopilotApiToken } from "./token.js";
import { fetchCopilotUsage } from "./usage.js";

vi.mock("supportClaw/plugin-sdk/provider-model-shared", async (importOriginal) => ({
  ...(await importOriginal<typeof import("supportClaw/plugin-sdk/provider-model-shared")>()),
...
vi.mock("supportClaw/plugin-sdk/json-store", () => ({
  loadJsonFile: jsonStoreMocks.loadJsonFile,
  saveJsonFile: jsonStoreMocks.saveJsonFile,
}));

vi.mock("supportClaw/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/supportClaw-state",
}));

import type { ProviderResolveDynamicModelContext } from "supportClaw/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/supportClaw-state/credentials/github-copilot.token.json";
