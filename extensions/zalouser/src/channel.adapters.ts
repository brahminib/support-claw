...
import { createScopedDmSecurityResolver } from "supportClaw/plugin-sdk/channel-config-helpers";
...
function resolveZalouserOutboundChunkMode(cfg: SupportClawConfig, accountId?: string) {
  return getZalouserRuntime().channel.text.resolveChunkMode(cfg, "zalouser", accountId);
}

function resolveZalouserOutboundTextChunkLimit(cfg: SupportClawConfig, accountId?: string) {
...
export const zalouserPairingTextAdapter = {
  ...
  notify: async ({ cfg, id, message }: { cfg: SupportClawConfig; id: string; message: string }) => {
    const { sendMessageZalouser } = await loadZalouserChannelRuntime();
