/**
 * SupportClaw Memory (LanceDB) Plugin
 *
...
import type { AgentToolResult } from "supportClaw/plugin-sdk/agent-core";
import {
  optionalFiniteNumberSchema,
  optionalPositiveIntegerSchema,
} from "supportClaw/plugin-sdk/channel-actions";
import { BUNDLED_CHAT_CHANNEL_ENVELOPE_PREFIXES } from "supportClaw/plugin-sdk/chat-channel-ids";
import type { SupportClawConfig } from "supportClaw/plugin-sdk/config-contracts";
import type { MemoryEmbeddingProvider } from "supportClaw/plugin-sdk/memory-core-host-engine-embeddings";
import { MESSAGE_TOOL_DELIVERY_HINTS } from "supportClaw/plugin-sdk/message-tool-delivery-hints";
import {
  parseStrictPositiveInteger,
  resolveTimerTimeoutMs,
} from "supportClaw/plugin-sdk/number-runtime";
import { readFiniteNumberParam, readPositiveIntegerParam } from "supportClaw/plugin-sdk/param-readers";
import { resolveLivePluginConfigObject } from "supportClaw/plugin-sdk/plugin-config-runtime";
import { ensureGlobalUndiciEnvProxyDispatcher } from "supportClaw/plugin-sdk/runtime-env";
import {
  asOptionalRecord as asRecord,
  normalizeLowercaseStringOrEmpty,
} from "supportClaw/plugin-sdk/string-coerce-runtime";
import { truncateUtf16Safe } from "supportClaw/plugin-sdk/text-utility-runtime";
import { Type } from "typebox";
import { definePluginEntry, type SupportClawPluginApi } from "./api.js";
import {
...
let memoryEmbeddingProviderModulePromise:
  | Promise<typeof import("supportClaw/plugin-sdk/memory-core-host-engine-embeddings")>
  | undefined;
function loadMemoryEmbeddingProviderModule(): Promise<
  typeof import("supportClaw/plugin-sdk/memory-core-host-engine-embeddings")
> {
  memoryEmbeddingProviderModulePromise ??=
    import("supportClaw/plugin-sdk/memory-core-host-engine-embeddings");
  return memoryEmbeddingProviderModulePromise;
}

let memoryHostCoreModulePromise:
  | Promise<typeof import("supportClaw/plugin-sdk/memory-host-core")>
  | undefined;
function loadMemoryHostCoreModule(): Promise<
  typeof import("supportClaw/plugin-sdk/memory-host-core")
> {
  memoryHostCoreModulePromise ??= import("supportClaw/plugin-sdk/memory-host-core");
...
class ProviderAdapterEmbeddings implements Embeddings {
  ...
  constructor(
    private api: SupportClawPluginApi,
  ...
  private async createProvider(): Promise<MemoryEmbeddingProvider> {
    const cfg = (this.api.runtime.config?.current?.() ?? this.api.config) as SupportClawConfig;
...
function createEmbeddings(api: SupportClawPluginApi, cfg: MemoryConfig): Embeddings {
...
/**
 * Matches JSON blobs that look like SupportClaw transport envelope metadata.
...
/**
 * Returns true if `text` looks like it contains SupportClaw-injected envelope or
...
/**
 * Strips SupportClaw-injected envelope metadata from a user message so that only
    ...
    const resolveCurrentHookConfig = () => {
      const runtimePluginConfig = resolveLivePluginConfigObject(
        api.runtime.config?.current
          ? () => api.runtime.config.current() as SupportClawConfig
          : undefined,
        "memory-lancedb",
