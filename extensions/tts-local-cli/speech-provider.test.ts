...
import type { SpeechProviderConfig, SpeechSynthesisRequest } from "supportClaw/plugin-sdk/speech-core";
...
const TEST_CFG = {} as SupportClawConfig;

function createCliFixture(): { dir: string; script: string } {
  const dir = mkdtempSync(path.join(os.tmpdir(), "supportClaw-cli-tts-test-"));
...
describe("buildCliSpeechProvider", () => {
  ...
  it("can synthesize through a real local CLI fixture and ffmpeg", async () => {
    if (process.env.SUPPORT_CLAW_LIVE_TEST !== "1") {
      return;
    }
    const fixture = createCliFixture();
    const rawFfmpeg = await vi.importActual<typeof import("supportClaw/plugin-sdk/media-runtime")>(
