// Thread Ownership plugin entrypoint registers its SupportClaw integration.
...
function resolveOwnershipAgent(config: SupportClawConfig): { id: string; name: string } {
...
export default definePluginEntry({
  ...
  register(api: SupportClawPluginApi) {
    const resolveCurrentState = () => {
      const currentConfig = (api.runtime.config?.current?.() ?? api.config) as SupportClawConfig;
