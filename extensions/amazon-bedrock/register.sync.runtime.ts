...
import { registerApiProvider, streamSimple } from "supportClaw/plugin-sdk/llm";
import { resolvePluginConfigObject } from "supportClaw/plugin-sdk/plugin-config-runtime";
import type {
...
import { streamWithPayloadPatch } from "supportClaw/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When SupportClaw's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if SupportClaw identifies this as an Anthropic model via the ARN heuristic.
 ...
 * same matcher SupportClaw uses for direct model IDs.
 */
 ...
 * the SupportClaw config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: SupportClawPluginApi): void {
  ...
  function resolveCurrentPluginConfig(
    config: SupportClawConfig | undefined,
