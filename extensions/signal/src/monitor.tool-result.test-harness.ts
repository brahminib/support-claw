...
import type { MockFn } from "supportClaw/plugin-sdk/plugin-test-runtime";
...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/supportClaw-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("supportClaw/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/security-runtime")>(
    "supportClaw/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("supportClaw/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("supportClaw/plugin-sdk/system-event-runtime")>(
    "supportClaw/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("supportClaw/plugin-sdk/reply-runtime"),
      import("supportClaw/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
