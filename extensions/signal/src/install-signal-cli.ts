...
import { runPluginCommandWithTimeout } from "supportClaw/plugin-sdk/run-command";
import type { RuntimeEnv } from "supportClaw/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "supportClaw/plugin-sdk/setup-tools";
...
export async function installSignalCliFromRelease(
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-signal-"));
