...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["supportClawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["supportClaw", "pollId"]) ??
    readNestedString(value, ["supportClaw", "poll", "id"]) ??
    readNestedString(value, ["data", "supportClawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "supportClaw", "pollId"]) ??
    // Action.Execute (Universal Action Model) payload shape: value.action.data
    readNestedString(value, ["action", "data", "supportClawPollId"]) ??
    ...
    actions: [
      {
        type: "Action.Execute",
        title: "Vote",
        verb: "supportClaw.poll.vote",
        data: {
          supportClawPollId: pollId,
          pollId,
        },
      },
    ],
