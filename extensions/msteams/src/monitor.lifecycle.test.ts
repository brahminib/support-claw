...
function createConfig(port: number): SupportClawConfig {
  return {
    channels: {
      msteams: {
        enabled: true,
...
function updateMSTeamsConfig(
  ...
  patch: NonNullable<NonNullable<SupportClawConfig["channels"]>["msteams"]>,
): void {
...
function requireRegisteredMSTeamsConfig(): SupportClawConfig {
  ...
  it("gates poll card votes before recording them", async () => {
    ...
    const response = await cardActionHandler({
      activity: {
        ...
        value: { action: { data: { supportClawPollId: "poll-1", choices: "0" } } },
      },
    });
