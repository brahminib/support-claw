...
// pulls from `supportClaw/plugin-sdk/outbound-media` (post-migration). Mock the
// canonical source so the re-export carries our stub through.
vi.mock("supportClaw/plugin-sdk/outbound-media", () => ({
  loadOutboundMediaFromUrl: mockState.loadOutboundMediaFromUrl,
}));

vi.mock("supportClaw/plugin-sdk/markdown-table-runtime", () => ({
  resolveMarkdownTableMode: mockState.resolveMarkdownTableMode,
}));

vi.mock("supportClaw/plugin-sdk/text-chunking", async (importOriginal) => {
  const actual = await importOriginal<typeof import("supportClaw/plugin-sdk/text-chunking")>();
  return {
    ...actual,
