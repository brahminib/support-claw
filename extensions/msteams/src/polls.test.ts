...
describe("msteams polls", () => {
  ...
  it("stores and records poll votes", async () => {
    const home = await fs.promises.mkdtemp(path.join(os.tmpdir(), "supportClaw-msteams-polls-"));
...
const createStateStore = async () => {
  const stateDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "supportClaw-msteams-polls-"));
...
describe("state poll store", () => {
  ...
  it("deletes vote buckets when pruning over the poll cap", async () => {
    const stateDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "supportClaw-msteams-polls-"));
    const env = { ...process.env, SUPPORT_CLAW_STATE_DIR: stateDir };
