...
import { buildSupportClawUserAgentFragment } from "./user-agent.js";
...
type MSTeamsAppOn = {
  ...
  // delegates back into the SDK handlers after SupportClaw sender policy passes.
...
export async function createMSTeamsApp(
  ...
  // backend can identify SupportClaw traffic for usage telemetry. Teams SDK
  ...
  const appOptions: Record<string, unknown> = {
    client: options?.httpClient ?? {
      headers: { "User-Agent": buildSupportClawUserAgentFragment() },
    },
