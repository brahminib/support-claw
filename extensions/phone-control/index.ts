// Phone Control plugin entrypoint registers its SupportClaw integration.
import {
  asDateTimestampMs,
...
function openArmStateStore(api: SupportClawPluginApi) {
...
async function readArmState(api: SupportClawPluginApi): Promise<ArmStateFile | null> {
  return (await openArmStateStore(api).lookup(ARM_STATE_KEY)) ?? null;
}

async function writeArmState(api: SupportClawPluginApi, state: ArmStateFile | null): Promise<void> {
...
export default definePluginEntry({
  ...
  register(api: SupportClawPluginApi) {
    ...
    const timerService: SupportClawPluginService = {
