...
Read-only diff viewer plugin for **SupportClaw** agents.

## Install

```bash
supportClaw plugins install @supportclaw/diffs
```
...
- Plugin id: `diffs`
- Package: `@supportclaw/diffs`
- Minimum SupportClaw host: `2026.4.30`
...
Before:
SupportClaw supports plugins.

After:
SupportClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/supportClaw-diffs`).
