...
export function registerDiffsPlugin(api: SupportClawPluginApi): void {
  const store = new DiffArtifactStore({
    rootDir: path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-diffs"),
    logger: api.logger,
  });
...
export function resolveDiffsLanguagePackAvailability(api: SupportClawPluginApi): boolean {
  const currentConfig = (api.runtime.config?.current?.() ?? api.config) as SupportClawConfig;
...
function hasSiblingLanguagePackRuntime(rootDir: string | undefined): boolean {
  ...
  return (
    fs.existsSync(path.join(languagePackRoot, "supportClaw.plugin.json")) &&
