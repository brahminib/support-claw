...
const disableAutoStartKey = Symbol.for("supportClaw.diffs.disableAutoStart");
...
function renderCard(payloadOverride?: string): void {
  ...
  document.body.insertAdjacentHTML(
    ...
    `<section class="oc-diff-card">
      <div data-supportClaw-diff-host></div>
      <script type="application/json" data-supportClaw-diff-payload>${payload}</script>
    </section>`,
  );
}
...
describe("hydrateViewer", () => {
  beforeEach(() => {
    ...
    delete document.documentElement.dataset.supportClawDiffsError;
    delete document.documentElement.dataset.supportClawDiffsReady;
    vi.clearAllMocks();
  });
  ...
  it("continues hydrating later cards when one card throws", async () => {
    ...
    expect(document.documentElement.dataset.supportClawDiffsError).toBeUndefined();
    warn.mockRestore();
  });
...
describe("ensureShadowRoot", () => {
  ...
  it("attaches shadow root from template and removes template element", async () => {
    renderCard();
    const host = document.querySelector<HTMLElement>("[data-supportClaw-diff-host]")!;
