...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-supportClaw-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-supportClaw-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.supportClawDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.supportClawDiffsError = "true";
    console.error("Failed to hydrate diff viewer", error);
  }
}

export const disableAutoStartKey = Symbol.for("supportClaw.diffs.disableAutoStart");
