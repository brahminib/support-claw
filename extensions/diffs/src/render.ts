...
function renderDiffCard(payload: DiffViewerPayload): string {
  return `<section class="oc-diff-card">
    <diffs-container class="oc-diff-host" data-supportClaw-diff-host>
      <template shadowrootmode="open">${payload.prerenderedHTML}</template>
    </diffs-container>
    <script type="application/json" data-supportClaw-diff-payload>${escapeJsonScript(payload)}</script>
  </section>`;
}
...
<html lang="en">
  ...
  <body data-theme="${params.theme}">
    <main class="oc-frame" data-render-mode="${params.runtimeMode}">
      <div data-supportClaw-diff-root>
        ${params.bodyHtml}
      </div>
    </main>
