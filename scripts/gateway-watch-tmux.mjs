...
const TMUX_CWD_ENV_KEY = "SUPPORT_CLAW_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@supportclaw.gateway_watch.cwd";
const TMUX_CHILD_ENV_KEYS = [
  "NODE_OPTIONS",
  "SUPPORT_CLAW_CONFIG_PATH",
  "SUPPORT_CLAW_DIAGNOSTICS",
  "SUPPORT_CLAW_DIAGNOSTICS_EVENT_LOOP",
  "SUPPORT_CLAW_DIAGNOSTICS_TIMELINE_PATH",
  "SUPPORT_CLAW_GATEWAY_PORT",
  "SUPPORT_CLAW_GATEWAY_RESTART_TRACE",
  "SUPPORT_CLAW_GATEWAY_STARTUP_TRACE",
  "SUPPORT_CLAW_HOME",
...
const resolveGatewayWatchBenchmarkArgs = ({ args = [], env = process.env } = {}) => {
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.SUPPORT_CLAW_TRACE_SYNC_IO ??= "0";
    if (nextEnv.SUPPORT_CLAW_TRACE_SYNC_IO === "1") {
...
export const resolveGatewayWatchTmuxSessionName = ({ args = [], env = process.env } = {}) => {
  const profile =
    env.SUPPORT_CLAW_PROFILE ||
    readArgValue(args, "--profile") ||
    (args.includes("--dev") ? "dev" : null);
  const port = env.SUPPORT_CLAW_GATEWAY_PORT || readArgValue(args, "--port");
  const parts = [
    "supportClaw",
    "gateway",
    "watch",
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    "env",
    ...colorEnv.options,
    `SUPPORT_CLAW_GATEWAY_WATCH_TMUX_CHILD=1`,
    `SUPPORT_CLAW_GATEWAY_WATCH_SESSION=${sessionName}`,
...
const log = (stderr, message) => {
  stderr.write(`[supportClaw] ${message}\n`);
};
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.SUPPORT_CLAW_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.SUPPORT_CLAW_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.SUPPORT_CLAW_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,
