...
run_prepare_push_retry_gates() {
  ...
  if [ "${SUPPORT_CLAW_TESTBOX:-}" = "1" ]; then
...
prepare_gates() {
  ...
  if [ "${SUPPORT_CLAW_TESTBOX:-}" != "1" ] && [ "$docs_only" = "true" ] && [ -n "$previous_last_verified_head" ] && git merge-base --is-ancestor "$previous_last_verified_head" HEAD 2>/dev/null; then
  ...
  if [ "${SUPPORT_CLAW_TESTBOX:-}" = "1" ]; then
    ...
    if [ "$docs_only" = "true" ]; then
      ...
      gates_mode="full"
      if [ -n "${SUPPORT_CLAW_VITEST_MAX_WORKERS:-}" ]; then
        echo "Running pnpm test with SUPPORT_CLAW_VITEST_MAX_WORKERS=$SUPPORT_CLAW_VITEST_MAX_WORKERS."
        run_quiet_logged \
          "pnpm test" \
          ".local/gates-test.log" \
          env SUPPORT_CLAW_VITEST_MAX_WORKERS="$SUPPORT_CLAW_VITEST_MAX_WORKERS" pnpm test
      else
