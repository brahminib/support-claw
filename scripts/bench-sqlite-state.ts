// SQLite state benchmark seeds SupportClaw DBs and reports hot-query proof lines.
...
} from "../src/state/supportClaw-agent-db.js";
import {
...
function printUsage(): void {
  console.log(`SupportClaw SQLite state benchmark
...
function main(): void {
  ...
  const stateDir =
    options.stateDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sqlite-perf-"));
  const env = { SUPPORT_CLAW_STATE_DIR: stateDir };
  const started = nowMs();
  try {
    const stateDatabase = openSupportClawStateDatabase({ env });
    const agentDatabases = Array.from({ length: config.agentCount }, (_, index) =>
      openSupportClawAgentDatabase({ agentId: `perf-agent-${index}`, env }),
    );
