...
EXTRA_MOUNTS="${SUPPORT_CLAW_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${SUPPORT_CLAW_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${SUPPORT_CLAW_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${SUPPORT_CLAW_DOCKER_SOCKET:-}"
TIMEZONE="${SUPPORT_CLAW_TZ:-}"
RAW_SKIP_ONBOARDING="${SUPPORT_CLAW_SKIP_ONBOARDING:-}"
SKIP_ONBOARDING=""
DOCKER_PULL_TIMEOUT="${SUPPORT_CLAW_DOCKER_SETUP_PULL_TIMEOUT:-600s}"
OFFLINE_MODE=""
DEFAULT_SANDBOX_IMAGE="supportClaw-sandbox:bookworm-slim"
DEFAULT_SANDBOX_BROWSER_IMAGE="supportClaw-sandbox-browser:bookworm-slim"
...
read_config_gateway_token() {
  local config_path="$SUPPORT_CLAW_CONFIG_DIR/supportClaw.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == SUPPORT_CLAW_GATEWAY_TOKEN=* ]]; then
      token="${line#SUPPORT_CLAW_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${SUPPORT_CLAW_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$SUPPORT_CLAW_GATEWAY_PORT" "$SUPPORT_CLAW_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$SUPPORT_CLAW_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$SUPPORT_CLAW_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network supportClaw-cli service because it
  ...
  # Host SUPPORT_CLAW_* paths are Compose bind-mount sources. Setup-time CLI writes
  ...
  run_prestart_gateway \
    ...
    --entrypoint node supportClaw-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" supportClaw-cli "$@"
}
...
run_gateway_up() {
  ...
  docker compose "${compose_args[@]}" "${up_args[@]}" supportClaw-gateway
}
...
resolve_offline_sandbox_images() {
  ...
  printf '%s' "$agents_json" | run_prestart_gateway \
    -T --entrypoint node supportClaw-gateway -e '
const fs = require("node:fs");
...
const matchesBrowser = (rawPattern) => {
  ...
  if (pattern === "group:supportClaw" || pattern === "group:ui") {
    return true;
  }
  if (!pattern) {
    return false;
...
validate_offline_sandbox_prerequisites() {
      ...
      browser)
        if ! browser_contract="$(
          docker --host "unix://$DOCKER_SOCKET_PATH" image inspect \
            -f '{{ index .Config.Labels "org.supportClaw.sandbox-browser.contract" }}' \
  ...
  if ! run_compose_one_off --rm --entrypoint docker supportClaw-gateway --version >/dev/null 2>&1; then
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "SUPPORT_CLAW_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "SUPPORT_CLAW_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
SUPPORT_CLAW_CONFIG_DIR="${SUPPORT_CLAW_CONFIG_DIR:-$HOME/.supportClaw}"
SUPPORT_CLAW_WORKSPACE_DIR="${SUPPORT_CLAW_WORKSPACE_DIR:-$HOME/.supportClaw/workspace}"
SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR="${SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR:-$HOME/.supportClaw-auth-profile-secrets}"

validate_mount_path_value "SUPPORT_CLAW_CONFIG_DIR" "$SUPPORT_CLAW_CONFIG_DIR"
validate_mount_path_value "SUPPORT_CLAW_WORKSPACE_DIR" "$SUPPORT_CLAW_WORKSPACE_DIR"
validate_mount_path_value "SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR" "$SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "SUPPORT_CLAW_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "SUPPORT_CLAW_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "SUPPORT_CLAW_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "SUPPORT_CLAW_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "SUPPORT_CLAW_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "SUPPORT_CLAW_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$SUPPORT_CLAW_CONFIG_DIR"
mkdir -p "$SUPPORT_CLAW_WORKSPACE_DIR"
mkdir -p "$SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$SUPPORT_CLAW_CONFIG_DIR/identity"
mkdir -p "$SUPPORT_CLAW_CONFIG_DIR/agents/main/agent"
mkdir -p "$SUPPORT_CLAW_CONFIG_DIR/agents/main/sessions"

export SUPPORT_CLAW_CONFIG_DIR
export SUPPORT_CLAW_WORKSPACE_DIR
export SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR
export SUPPORT_CLAW_GATEWAY_PORT="${SUPPORT_CLAW_GATEWAY_PORT:-18789}"
export SUPPORT_CLAW_BRIDGE_PORT="${SUPPORT_CLAW_BRIDGE_PORT:-18790}"
export SUPPORT_CLAW_GATEWAY_BIND="${SUPPORT_CLAW_GATEWAY_BIND:-lan}"
export SUPPORT_CLAW_DISABLE_BONJOUR="${SUPPORT_CLAW_DISABLE_BONJOUR:-}"
export SUPPORT_CLAW_IMAGE="$IMAGE_NAME"
export SUPPORT_CLAW_IMAGE_APT_PACKAGES="${SUPPORT_CLAW_IMAGE_APT_PACKAGES-${SUPPORT_CLAW_DOCKER_APT_PACKAGES:-}}"
export SUPPORT_CLAW_IMAGE_PIP_PACKAGES="${SUPPORT_CLAW_IMAGE_PIP_PACKAGES:-}"
export SUPPORT_CLAW_EXTENSIONS="${SUPPORT_CLAW_EXTENSIONS:-}"
export SUPPORT_CLAW_INSTALL_BROWSER="${SUPPORT_CLAW_INSTALL_BROWSER:-}"
export SUPPORT_CLAW_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export SUPPORT_CLAW_HOME_VOLUME="$HOME_VOLUME_NAME"
export SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS="${SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS:-}"
export SUPPORT_CLAW_SANDBOX="$SANDBOX_ENABLED"
export SUPPORT_CLAW_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export SUPPORT_CLAW_DOCKER_SETUP=1
export SUPPORT_CLAW_TZ="$TIMEZONE"
...
export SUPPORT_CLAW_OTEL_PRELOADED="${SUPPORT_CLAW_OTEL_PRELOADED:-}"
export SUPPORT_CLAW_SKIP_ONBOARDING="$SKIP_ONBOARDING"
...
if [[ -z "${SUPPORT_CLAW_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    SUPPORT_CLAW_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $SUPPORT_CLAW_CONFIG_DIR/supportClaw.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      SUPPORT_CLAW_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      SUPPORT_CLAW_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      SUPPORT_CLAW_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
export SUPPORT_CLAW_GATEWAY_TOKEN
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${SUPPORT_CLAW_CONFIG_DIR}:/home/node/.supportClaw"
    gateway_workspace_mount="${SUPPORT_CLAW_WORKSPACE_DIR}:/home/node/.supportClaw/workspace"
    gateway_auth_profile_secret_mount="${SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR}:/home/node/.config/supportClaw"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${SUPPORT_CLAW_INSTALL_DOCKER_CLI:-}" ]]; then
    export SUPPORT_CLAW_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR \
  SUPPORT_CLAW_GATEWAY_PORT \
  SUPPORT_CLAW_BRIDGE_PORT \
  SUPPORT_CLAW_GATEWAY_BIND \
  SUPPORT_CLAW_DISABLE_BONJOUR \
  SUPPORT_CLAW_GATEWAY_TOKEN \
  SUPPORT_CLAW_IMAGE \
  SUPPORT_CLAW_EXTRA_MOUNTS \
  SUPPORT_CLAW_HOME_VOLUME \
  SUPPORT_CLAW_IMAGE_APT_PACKAGES \
  SUPPORT_CLAW_IMAGE_PIP_PACKAGES \
  SUPPORT_CLAW_EXTENSIONS \
  SUPPORT_CLAW_INSTALL_BROWSER \
  SUPPORT_CLAW_SANDBOX \
  SUPPORT_CLAW_DOCKER_SOCKET \
  DOCKER_GID \
  SUPPORT_CLAW_INSTALL_DOCKER_CLI \
  SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS \
  SUPPORT_CLAW_TZ \
  ...
  SUPPORT_CLAW_OTEL_PRELOADED \
  SUPPORT_CLAW_SKIP_ONBOARDING

if [[ -n "$OFFLINE_MODE" ]]; then
  require_local_docker_image "$IMAGE_NAME"
  echo "==> Using preloaded Docker image: $IMAGE_NAME"
elif [[ "$IMAGE_NAME" == "supportClaw:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  run_docker_build \
    --build-arg "SUPPORT_CLAW_IMAGE_APT_PACKAGES=${SUPPORT_CLAW_IMAGE_APT_PACKAGES}" \
    --build-arg "SUPPORT_CLAW_IMAGE_PIP_PACKAGES=${SUPPORT_CLAW_IMAGE_PIP_PACKAGES}" \
    --build-arg "SUPPORT_CLAW_EXTENSIONS=${SUPPORT_CLAW_EXTENSIONS}" \
    --build-arg "SUPPORT_CLAW_INSTALL_BROWSER=${SUPPORT_CLAW_INSTALL_BROWSER}" \
    --build-arg "SUPPORT_CLAW_INSTALL_DOCKER_CLI=${SUPPORT_CLAW_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the SupportClaw metadata subdirectory
# (.supportClaw/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh supportClaw-gateway -c \
  'find /home/node/.supportClaw -xdev -exec chown node:node {} +; \
   chown node:node /home/node/.config; \
   find /home/node/.config/supportClaw -xdev -exec chown node:node {} +; \
   [ -d /home/node/.supportClaw/workspace/.supportClaw ] && chown -R node:node /home/node/.supportClaw/workspace/.supportClaw || true'

echo ""
if [[ -n "$SKIP_ONBOARDING" ]]; then
  echo "==> Skipping onboarding (SUPPORT_CLAW_SKIP_ONBOARDING is set)"
else
  echo "==> Onboarding (interactive)"
  echo "Docker setup pins Gateway mode to local."
  echo "Gateway runtime bind comes from SUPPORT_CLAW_GATEWAY_BIND (default: lan)."
  echo "Current runtime bind: $SUPPORT_CLAW_GATEWAY_BIND"
  if is_truthy_value "$SUPPORT_CLAW_DISABLE_BONJOUR"; then
    echo "Bonjour/mDNS advertising: force disabled (SUPPORT_CLAW_DISABLE_BONJOUR=$SUPPORT_CLAW_DISABLE_BONJOUR)."
  elif [[ -z "$SUPPORT_CLAW_DISABLE_BONJOUR" ]]; then
    echo "Bonjour/mDNS advertising: auto (disabled inside the Gateway container unless explicitly enabled)."
  else
    echo "Bonjour/mDNS advertising: explicitly enabled (SUPPORT_CLAW_DISABLE_BONJOUR=$SUPPORT_CLAW_DISABLE_BONJOUR)."
  fi
  ...
  run_prestart_cli onboard \
    ...
    --no-install-daemon \
    --gateway-auth token \
    --gateway-token-ref-env SUPPORT_CLAW_GATEWAY_TOKEN \
    --skip-ui \
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm supportClaw-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm supportClaw-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm supportClaw-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.supportClaw.ai/channels"
...
# --- Sandbox setup (opt-in via SUPPORT_CLAW_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -z "$OFFLINE_MODE" && ! -S "$DOCKER_SOCKET_PATH" ]]; then
    echo "WARNING: SUPPORT_CLAW_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
  ...
  if [[ -n "$SANDBOX_ENABLED" && -z "$OFFLINE_MODE" ]] &&
    ! run_compose_one_off --rm --entrypoint docker supportClaw-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg SUPPORT_CLAW_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (SUPPORT_CLAW_IMAGE=supportClaw:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in SupportClaw config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.supportClaw.ai/gateway/sandboxing"
...
echo "Config: $SUPPORT_CLAW_CONFIG_DIR"
echo "Workspace: $SUPPORT_CLAW_WORKSPACE_DIR"
echo "Token: stored in Docker environment/config (not printed)."
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f supportClaw-gateway"
echo "  ${COMPOSE_HINT} exec supportClaw-gateway sh -lc 'node dist/index.js health --token \"\$SUPPORT_CLAW_GATEWAY_TOKEN\"'"
