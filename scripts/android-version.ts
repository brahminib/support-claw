// Android Version script supports SupportClaw repository automation.
...
function main(argv = process.argv.slice(2)): number {
  ...
  if (options.format === "shell") {
    process.stdout.write(
      [
        `SUPPORT_CLAW_ANDROID_VERSION_NAME=${version.canonicalVersion}`,
        `SUPPORT_CLAW_ANDROID_VERSION_CODE=${version.versionCode}`,
      ].join("\n") + "\n",
    );
  } else {
