...
const DEFAULT_WORKFLOW = "supportClaw-live-and-e2e-checks-reusable.yml";
...
function ghWorkflowCommand(lanes, ref, workflow, reuseInputs = {}) {
  const workflowRef =
    ...
    process.env.SUPPORT_CLAW_DOCKER_E2E_WORKFLOW_REF ||
...
function main() {
  ...
  if (isLocalJson) {
    ...
    const outputDir =
      options.dir || path.join(os.tmpdir(), `supportClaw-docker-e2e-rerun-${options.input}`);
