 ...
 * `supportClaw update`. Use it to verify the completion installation flow
...
\`supportClaw update\`, \`supportClaw doctor\`, and \`supportClaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SupportClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`supportClaw completion --install\` later to enable.`));
    return;
  }
