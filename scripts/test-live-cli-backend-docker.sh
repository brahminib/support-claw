...
DEFAULT_PROVIDER="${SUPPORT_CLAW_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
CLI_MODEL="${SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL:-}"
CLI_PROVIDER="${CLI_MODEL%%/*}"
CLI_DISABLE_MCP_CONFIG="${SUPPORT_CLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG:-}"
CLI_AUTH_MODE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_AUTH:-auto}"
...
case "$CLI_AUTH_MODE" in
  auto | api-key | subscription)
    ;;
  *)
    echo "ERROR: SUPPORT_CLAW_LIVE_CLI_BACKEND_AUTH must be one of: auto, api-key, subscription." >&2
    exit 1
    ;;
esac

if [[ "$CLI_AUTH_MODE" == "subscription" && "$CLI_PROVIDER" != "claude-cli" ]]; then
  echo "ERROR: SUPPORT_CLAW_LIVE_CLI_BACKEND_AUTH=subscription is only supported for claude-cli." >&2
  exit 1
fi
...
export SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
export SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
export SUPPORT_CLAW_LIVE_CLI_BACKEND_MCP_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
...
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  ...
  if [[ -z "${SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]]; then
    if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" ]]; then
      export SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV='["CLAUDE_CODE_OAUTH_TOKEN"]'
    else
      export SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="[]"
    fi
  fi
  if [[ "$SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV" == *ANTHROPIC_API_KEY* ]]; then
    echo "ERROR: subscription auth smoke must not preserve Anthropic API-key env vars." >&2
    exit 1
  fi
  if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" && "$SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV" != *CLAUDE_CODE_OAUTH_TOKEN* ]]; then
  ...
  export SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
  export SUPPORT_CLAW_LIVE_CLI_BACKEND_RESUME_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_RESUME_PROBE:-1}"
  export SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
  export SUPPORT_CLAW_LIVE_CLI_BACKEND_MCP_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
fi
...
if [[ -n "${SUPPORT_CLAW_DOCKER_AUTH_DIRS:-}" ]]; then
  ...
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(supportClaw_live_collect_auth_dirs_from_csv "$CLI_PROVIDER")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(supportClaw_live_collect_auth_files_from_csv "$CLI_PROVIDER")
fi
if [[ "${CLAUDE_SUBSCRIPTION_AUTH_SOURCE:-}" == "env-token" ]]; then
  retained_auth_files=()
...
run_setup_command() {
  local timeout_value="${SUPPORT_CLAW_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:?missing live CLI backend setup timeout seconds}s"
  local timeout_bin=""
...
provider="${SUPPORT_CLAW_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
default_command="${SUPPORT_CLAW_DOCKER_CLI_BACKEND_COMMAND_DEFAULT:-}"
docker_package="${SUPPORT_CLAW_DOCKER_CLI_BACKEND_NPM_PACKAGE:-}"
binary_name="${SUPPORT_CLAW_DOCKER_CLI_BACKEND_BINARY_NAME:-}"
if [ -z "$binary_name" ] && [ -n "$default_command" ]; then
  binary_name="$(basename "$default_command")"
fi
if [ -z "${SUPPORT_CLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -n "$binary_name" ]; then
  export SUPPORT_CLAW_LIVE_CLI_BACKEND_COMMAND="$NPM_CONFIG_PREFIX/bin/$binary_name"
fi
...
if [ -n "${SUPPORT_CLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ ! -x "${SUPPORT_CLAW_LIVE_CLI_BACKEND_COMMAND}" ] && [ -n "$docker_package" ]; then
  run_setup_command npm install -g "$docker_package"
elif [ -n "$docker_package" ] && package_has_explicit_version "$docker_package"; then
  run_setup_command npm install -g "$docker_package"
fi
if [ -n "${SUPPORT_CLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -x "${SUPPORT_CLAW_LIVE_CLI_BACKEND_COMMAND}" ]; then
  echo "==> CLI backend binary: ${SUPPORT_CLAW_LIVE_CLI_BACKEND_COMMAND}"
  "${SUPPORT_CLAW_LIVE_CLI_BACKEND_COMMAND}" -V || "${SUPPORT_CLAW_LIVE_CLI_BACKEND_COMMAND}" --version || true
fi
if [ "$provider" = "claude-cli" ]; then
  auth_mode="${SUPPORT_CLAW_LIVE_CLI_BACKEND_AUTH:-auto}"
...
if [ -n "\${SUPPORT_CLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${SUPPORT_CLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${SUPPORT_CLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${SUPPORT_CLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  if [ -z "${SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]; then
    export SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'
  fi
  if [ "$auth_mode" = "subscription" ]; then
    claude --version
    direct_token="SUPPORT_CLAW-CLAUDE-SUBSCRIPTION-DIRECT"
...
DOCKER_AUTH_ENV=(
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_AUTH="$CLI_AUTH_MODE"
)
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  DOCKER_AUTH_ENV+=(
    -e CLAUDE_CODE_OAUTH_TOKEN="${CLAUDE_CODE_OAUTH_TOKEN:-}"
    -e SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="$SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV"
  )
else
  DOCKER_AUTH_ENV+=(
    -e ANTHROPIC_API_KEY
    -e ANTHROPIC_API_KEY_OLD
    -e SUPPORT_CLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
    -e SUPPORT_CLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}"
    -e SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="${SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}"
  )
fi

DOCKER_RUN_ARGS=()
supportClaw_live_init_docker_run_args DOCKER_RUN_ARGS "${SUPPORT_CLAW_LIVE_CLI_BACKEND_DOCKER_RUN_TIMEOUT:-2700s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS="$CLI_SETUP_TIMEOUT_SECONDS" \
  -e SUPPORT_CLAW_DOCKER_CLI_BACKEND_PROVIDER="$CLI_PROVIDER" \
  -e SUPPORT_CLAW_DOCKER_CLI_BACKEND_COMMAND_DEFAULT="$CLI_DEFAULT_COMMAND" \
  -e SUPPORT_CLAW_DOCKER_CLI_BACKEND_NPM_PACKAGE="$CLI_DOCKER_NPM_PACKAGE" \
  -e SUPPORT_CLAW_DOCKER_CLI_BACKEND_BINARY_NAME="$CLI_DOCKER_BINARY_NAME" \
  -e SUPPORT_CLAW_LIVE_TEST=1 \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND=1 \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_DEBUG="${SUPPORT_CLAW_LIVE_CLI_BACKEND_DEBUG:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_ADVISORY="${SUPPORT_CLAW_LIVE_CLI_BACKEND_ADVISORY:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP="${SUPPORT_CLAW_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP:-}" \
  ...
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL="$CLI_MODEL" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_COMMAND="${SUPPORT_CLAW_LIVE_CLI_BACKEND_COMMAND:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_ARGS="${SUPPORT_CLAW_LIVE_CLI_BACKEND_ARGS:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_RESUME_ARGS="${SUPPORT_CLAW_LIVE_CLI_BACKEND_RESUME_ARGS:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_CLEAR_ENV="${SUPPORT_CLAW_LIVE_CLI_BACKEND_CLEAR_ENV:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG="$CLI_DISABLE_MCP_CONFIG" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_RESUME_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_RESUME_PROBE:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_MCP_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_MCP_PROBE:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_ARG="${SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_ARG:-}" \
  -e SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_MODE="${SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_MODE:-}")
