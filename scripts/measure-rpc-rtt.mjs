...
function resolveSupportClawLaunchArgs(repoRoot, sourceEntryExists = existsSync) {
  const sourceEntry = path.join(repoRoot, "src", "entry.ts");
  if (sourceEntryExists(sourceEntry)) {
    return ["--import", "tsx", sourceEntry];
  }
  return [path.join(repoRoot, "supportClawCli.mjs")];
}
...
export async function startGateway({
  ...
  let child;
  const launcherArgs = resolveSupportClawLaunchArgs(repoRoot, sourceEntryExists);
  try {
    child = spawnImpl(
      process.execPath,
      [
      ...
      ],
      {
        ...
        env: {
          ...
          SUPPORT_CLAW_STATE_DIR: path.join(tempRoot, "state"),
...
async function main() {
  ...
  const repoRoot = path.resolve(args.repoRoot ?? process.env.SUPPORT_CLAW_REPO_ROOT ?? process.cwd());
  ...
  try {
    ...
    const requireFromSupportClaw = createRequire(path.join(repoRoot, "package.json"));
    const WebSocket = requireFromSupportClaw("ws");
    ...
    const connect = await client.request(
      "connect",
      {
        ...
        client: {
          id: "gateway-client",
          displayName: "supportClaw-rtt rpc probe",
          version: "rtt",
          platform: process.platform,
          mode: "backend",
          instanceId: `supportClaw-rtt-rpc-${randomUUID()}`,
        },
        locale: "en-US",
        userAgent: "supportClaw-rtt-rpc",
        role: "operator",
