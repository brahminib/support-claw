// Ios Version script supports SupportClaw repository automation.
...
function main(argv = process.argv.slice(2)): number {
  ...
  if (options.format === "shell") {
    process.stdout.write(
      [
        `SUPPORT_CLAW_IOS_VERSION=${version.canonicalVersion}`,
        `SUPPORT_CLAW_MARKETING_VERSION=${version.marketingVersion}`,
        `SUPPORT_CLAW_BUILD_VERSION=${version.buildVersion}`,
      ].join("\n") + "\n",
    );
  } else {
