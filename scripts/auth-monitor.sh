...
#
# Suggested cron: */30 * * * * /path/to/supportClaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send SupportClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., supportClaw-alerts)
...
STATE_FILE="$HOME/.supportClaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SupportClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use supportClaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SupportClaw to $NOTIFY_PHONE..."
            supportClaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: SupportClaw Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SupportClaw is down. Run on the SupportClaw host: ${SCRIPT_DIR}/mobile-reauth.sh" "urgent"
    exit 1
