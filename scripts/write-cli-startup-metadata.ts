// Write Cli Startup Metadata script supports SupportClaw repository automation.
...
import type { SupportClawConfig } from "../src/config/config.js";
...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.supportClaw?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.supportClaw?.channel?.order;
      const labelRaw = parsed.supportClaw?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".supportClaw-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "supportClaw-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "supportClaw-build",
...
async function renderSourceBrowserHelpText(
  ...
  const inlineModule = [
    ...
    `registerBrowserCli(program, ["node", "supportClaw", "browser", "--help"]);`,
...
async function renderSourceCommandHelpText(
  ...
  return await spawnText(["supportClawCli.mjs", command, "--help"], {
    cwd: rootDir,
    env: {
      ...renderContext.env,
      SUPPORT_CLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
