// Generate Prompt Snapshots script supports SupportClaw repository automation.
...
export async function createFormattedPromptSnapshotFiles(): Promise<PromptSnapshotFile[]> {
  ...
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-prompt-snapshots-"));
  try {
