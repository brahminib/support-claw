...
const WINDOWS_NODE_REPO = "supportClaw/supportClaw-windows-node";
const WINDOWS_NODE_REQUIRED_ASSETS = [
  "SupportClawCompanion-Setup-x64.exe",
  "SupportClawCompanion-Setup-arm64.exe",
];
...
SupportClaw Release Publish command only after everything is green.
...
Options:
  ...
  --npm-preflight-run <id>            Reuse successful SupportClaw NPM Release preflight run.
...
export function parseArgs(argv) {
  ...
  if (options.pluginPublishScope === "selected") {
    throw new Error(
      "--plugin-publish-scope selected is only for plugin-only repair publishes; release candidates publish SupportClaw with --plugin-publish-scope all-publishable",
    );
  }
...
function githubApiTimeoutMs() {
  const raw = process.env.SUPPORT_CLAW_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
  if (!raw) {
    return DEFAULT_GITHUB_API_TIMEOUT_MS;
  }
  const value = Number(raw);
  if (!Number.isFinite(value) || value <= 0) {
    throw new Error("SUPPORT_CLAW_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS must be a positive number");
  }
...
export function buildPublishCommand(options) {
  ...
  return [
    ...
    "workflow",
    "run",
    "supportClaw-release-publish.yml",
    "--repo",
    options.repo,
    "--ref",
    options.workflowRef,
...
async function runTelegramIfNeeded(options, artifactName) {
  ...
  const runId = dispatchWorkflow(options.repo, workflowFile, options.workflowRef, {
    package_spec: `supportClaw@${options.tag.replace(/^v/u, "")}`,
...
async function main() {
  ...
  if (!options.npmPreflightRunId && !options.skipDispatch) {
    const workflowFile = "supportClaw-npm-release.yml";
  ...
  const npmRun = await waitForSuccessfulRun(options.repo, options.npmPreflightRunId, {
    workflowName: "SupportClaw NPM Release",
    workflowRef: options.workflowRef,
  });
  ...
  const npmArtifactName = await downloadResolvedArtifact(
    ...
    `supportClaw-npm-preflight-${options.tag}`,
