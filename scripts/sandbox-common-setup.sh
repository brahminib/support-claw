...
BASE_IMAGE="${BASE_IMAGE:-supportClaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-supportClaw-sandbox-common:bookworm-slim}"
...
SUPPORT_CLAW_DOCKER_BUILD_USE_BUILDX="${SUPPORT_CLAW_DOCKER_BUILD_USE_BUILDX:-0}"
SUPPORT_CLAW_DOCKER_BUILD_CACHE_FROM="${SUPPORT_CLAW_DOCKER_BUILD_CACHE_FROM:-}"
SUPPORT_CLAW_DOCKER_BUILD_CACHE_TO="${SUPPORT_CLAW_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=supportClaw.sandbox=1)
NOTE
