#!/usr/bin/env bash
# ClawDock - Docker helpers for SupportClaw
# Inspired by Simon Willison's "Running SupportClaw in Docker"
# https://til.simonwillison.net/llms/supportClaw-docker
#
# Installation:
...
# Common paths to check for SupportClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/supportClaw"
  "${HOME}/workspace/supportClaw"
  "${HOME}/projects/supportClaw"
  "${HOME}/dev/supportClaw"
  "${HOME}/code/supportClaw"
  "${HOME}/src/supportClaw"
)
...
_clawdock_browser_url_for_gateway_url() {
  ...
  published=$(_clawdock_compose port supportClaw-gateway 18789 2>/dev/null | head -n 1 | tr -d '\r')
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found SupportClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/supportClaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ SupportClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/brahminib/support-claw.git ~/supportClaw"
    echo "  cd ~/supportClaw && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SUPPORT_CLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d supportClaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart supportClaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f supportClaw-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.supportClaw"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show supportClaw.json
  if [[ -f "${config_dir}/supportClaw.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/supportClaw.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/supportClaw.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No supportClaw.json found${_CLR_RESET}"
  fi
  echo ""
...
clawdock-workspace() {
  cd ~/.supportClaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec supportClaw-gateway \
    bash -c 'echo "alias supportClaw=\"./supportClawCli.mjs\"" > /tmp/.bashrc_supportClaw && bash --rcfile /tmp/.bashrc_supportClaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm supportClaw-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build supportClaw-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d supportClaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build supportClaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SUPPORT_CLAW_GATEWAY_TOKEN=$token" supportClaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" supportClaw-gateway \
    bash -c './supportClawCli.mjs config set gateway.remote.token "$TOKEN" && ./supportClawCli.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec supportClaw-gateway \
    bash -c "./supportClawCli.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart supportClaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm --no-deps supportClaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec supportClaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'supportClaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for SupportClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (supportClaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to supportClaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.supportClaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'supportClaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'supportClaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.supportClaw.ai${_CLR_RESET}"
  echo ""
}
