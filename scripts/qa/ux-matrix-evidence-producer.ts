...
const SUITE_COMMAND = `pnpm supportClaw qa suite --scenario ${SCENARIO_ID}`;
...
function buildExecution(params: {
  ...
  return {
    ...
    environment: {
      ref: process.env.SUPPORT_CLAW_QA_REF?.trim() || process.env.GITHUB_SHA?.trim() || null,
      os: process.platform,
...
async function writeProducerMetadata(params: {
  ...
  await writeJson(path.join(params.artifactBase, "release-ledger.json"), {
    ...
    kind: "supportClaw.qa.ux-matrix.release-ledger",
  });
  await writeText(
