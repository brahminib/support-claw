...
// Code Mode Namespace Live script supports SupportClaw repository automation.
...
async function runOne(mode: Mode, task: Task, model: string, apiKey: string): Promise<RunMetrics> {
  ...
  return {
    ...
    ...(process.env.SUPPORT_CLAW_CODE_MODE_LIVE_DEBUG === "1" ? { toolResults } : {}),
  };
}
...
export async function main() {
  const model = readArg("model") ?? process.env.SUPPORT_CLAW_CODE_MODE_LIVE_MODEL ?? "gpt-5.4-mini";
  ...
  const taskLimit = parseTaskLimit(
    taskArg ?? process.env.SUPPORT_CLAW_CODE_MODE_LIVE_TASKS,
    taskArg === undefined ? "SUPPORT_CLAW_CODE_MODE_LIVE_TASKS" : "--tasks",
  );
