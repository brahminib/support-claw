...
- SUPPORT_CLAW_IOS_VERSION = exact canonical iOS version
- SUPPORT_CLAW_MARKETING_VERSION = short iOS/App Store version
- SUPPORT_CLAW_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    SUPPORT_CLAW_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    SUPPORT_CLAW_MARKETING_VERSION)
...
SUPPORT_CLAW_IOS_VERSION = ${IOS_VERSION}
SUPPORT_CLAW_MARKETING_VERSION = ${MARKETING_VERSION}
SUPPORT_CLAW_BUILD_VERSION = ${BUILD_NUMBER}
EOF
