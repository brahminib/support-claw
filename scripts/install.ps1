# SupportClaw Installer for Windows
# Usage: powershell -c "irm https://supportClaw.ai/install.ps1 | iex"
#        powershell -c "& ([scriptblock]::Create((irm https://supportClaw.ai/install.ps1))) -Tag beta -NoOnboard -DryRun"
...
function Complete-Install {
    ...
    throw "SupportClaw installation failed with exit code $($script:InstallExitCode)."
}

Write-Host ""
Write-Host "  SupportClaw Installer" -ForegroundColor Cyan
Write-Host ""
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:SUPPORT_CLAW_INSTALL_METHOD)) {
        $InstallMethod = $env:SUPPORT_CLAW_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:SUPPORT_CLAW_GIT_DIR)) {
        $GitDir = $env:SUPPORT_CLAW_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:SUPPORT_CLAW_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:SUPPORT_CLAW_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:SUPPORT_CLAW_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    ...
    $GitDir = (Join-Path $userHome "supportClaw")
}
...
function Get-SupportClawDepsRoot {
    ...
    return (Join-Path $localAppData "SupportClaw\deps")
}

function Get-PortableNodeRoot {
    return (Join-Path (Get-SupportClawDepsRoot) "portable-node")
}
...
function Ensure-PortableNodeOnUserPath {
    ...
    if (Add-ToUserPath $nodeDir) {
        Write-Host "[!] Added $nodeDir to user PATH (restart terminal if node or supportClaw is not found)" -ForegroundColor Yellow
    }
}
...
function Check-ExistingSupportClaw {
    if (Get-SupportClawCommandPath) {
        Write-Host "[*] Existing SupportClaw installation detected" -ForegroundColor Yellow
        return $true
    }
    return $false
}
...
function Get-PortableGitRoot {
    return (Join-Path (Get-SupportClawDepsRoot) "portable-git")
}
...
function Resolve-PortableGitDownload {
    ...
    $headers = @{
        "User-Agent" = "supportClaw-installer"
...
function Install-PortableGit {
    ...
    $portableParent = Split-Path -Parent $portableRoot
    $tmpZip = Join-Path $env:TEMP $download.Name
    $tmpExtract = Join-Path $env:TEMP ("supportClaw-portable-git-" + [guid]::NewGuid().ToString("N"))
...
function Ensure-Git {
    ...
    Write-Host ""
    Write-Host "Error: Git is required to install SupportClaw." -ForegroundColor Red
...
function Get-SupportClawCommandPath {
    $supportClawCmd = Get-Command supportClaw.cmd -ErrorAction SilentlyContinue
    if ($supportClawCmd -and $supportClawCmd.Source) {
        return $supportClawCmd.Source
    }

    $supportClaw = Get-Command supportClaw -ErrorAction SilentlyContinue
    if ($supportClaw -and $supportClaw.Source) {
        return $supportClaw.Source
    }

    return $null
}

function Invoke-SupportClawCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )

    $commandPath = Get-SupportClawCommandPath
    if (-not $commandPath) {
        throw "supportClaw command not found on PATH."
    }

    & $commandPath @Arguments
}

function Invoke-InteractiveSupportClawCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
    ...
    if ($process.ExitCode -ne 0) {
        throw "supportClaw $($Arguments -join ' ') failed with exit code $($process.ExitCode)."
    }
}
...
function Ensure-SupportClawOnPath {
    if (Get-SupportClawCommandPath) {
        return $true
    }
    ...
    foreach ($npmBin in $npmBins) {
        if (-not (Test-Path (Join-Path $npmBin "supportClaw.cmd"))) {
            continue
        }
    ...
    Write-Host "[!] supportClaw is not on PATH yet." -ForegroundColor Yellow
...
function Resolve-NpmSupportClawInstallSpec {
    param(
        [string]$PackageName,
        [string]$RequestedTag
    )
...
function Test-SupportClawSourcePackageInstallSpec {
    ...
    if ($normalizedTag.StartsWith("supportClaw@")) {
        $normalizedTag = $normalizedTag.Substring("supportClaw@".Length)
    }

    if ($normalizedTag -eq "main") {
        return $true
    }
    if ($normalizedTag -match '^github:supportClaw/supportClaw($|[#/])') {
        return $true
    }
    ...
    return (
        $normalizedTag -match '^https?://github\.com/supportClaw/supportClaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^ssh://git@github\.com[:/]supportClaw/supportClaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^git://github\.com/supportClaw/supportClaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^git@github\.com:supportClaw/supportClaw(\.git)?($|[?#])'
    )
}
...
function Install-SupportClaw {
    if ([string]::IsNullOrWhiteSpace($Tag)) {
        $Tag = "latest"
    }
    if (Test-SupportClawSourcePackageInstallSpec -RequestedTag $Tag) {
        Write-Host "Error: npm installs do not support SupportClaw GitHub source targets like '$Tag'." -ForegroundColor Red
    ...
    # Use supportClaw package for beta, supportClaw for stable
    $packageName = "supportClaw"
    if ($Tag -eq "beta" -or $Tag -match "^beta\.") {
        $packageName = "supportClaw"
    }
    $installSpec = Resolve-NpmSupportClawInstallSpec -PackageName $packageName -RequestedTag $Tag
    Write-Host "[*] Installing SupportClaw ($installSpec)..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host '  powershell -c "irm https://supportClaw.ai/install.ps1 | iex"' -ForegroundColor Cyan
            }
    ...
    Write-Host "[OK] SupportClaw installed" -ForegroundColor Green
    return $true
}

# Install SupportClaw from GitHub
function Install-SupportClawFromGit {
    param(
        [string]$RepoDir,
        [switch]$SkipUpdate
    )
    ...
    $repoUrl = "https://github.com/brahminib/support-claw.git"
    Write-Host "[*] Installing SupportClaw from GitHub ($repoUrl)..." -ForegroundColor Yellow
    ...
    if (-not (Test-Path $entryPath)) {
        Write-Host "[!] SupportClaw build did not produce $entryPath" -ForegroundColor Red
        return $false
    }
    ...
    $cmdPath = Join-Path $binDir "supportClaw.cmd"
    ...
    Write-Host "[OK] SupportClaw wrapper installed to $cmdPath" -ForegroundColor Green
...
function Run-Doctor {
    ...
    try {
        Invoke-SupportClawCommand doctor --non-interactive
    } catch {
...
function Test-GatewayServiceLoaded {
    try {
        $statusJson = (Invoke-SupportClawCommand daemon status --json 2>$null)
...
function Refresh-GatewayServiceIfLoaded {
    if (-not (Get-SupportClawCommandPath)) {
        return
    }
    ...
    try {
        Invoke-SupportClawCommand gateway install --force | Out-Null
    } catch {
    ...
    try {
        Invoke-SupportClawCommand gateway restart | Out-Null
        Invoke-SupportClawCommand gateway status --json | Out-Null
...
function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:SUPPORT_CLAW_GIT_DIR)) {
        return $env:SUPPORT_CLAW_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "supportClaw")
}
...
function Main {
    ...
    $isUpgrade = Check-ExistingSupportClaw
    ...
    if ($InstallMethod -eq "git") {
        try {
            ...
            if ($npmCommand) {
                Invoke-NpmCommand -Arguments @("uninstall", "-g", "supportClaw") 2>$null | Out-Null
                Write-Host "[OK] Removed npm global install if present" -ForegroundColor Green
            }
        } catch { }
        $finalGitDir = $GitDir
        $gitInstallResults = @(Install-SupportClawFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate)
        if (-not (Test-BooleanSuccessResult -Results $gitInstallResults)) {
            return (Fail-Install)
        }
    } else {
        $gitWrapper = Join-Path (Join-Path $env:USERPROFILE ".local\\bin") "supportClaw.cmd"
        ...
        $npmInstallResults = @(Install-SupportClaw)
        if (-not (Test-BooleanSuccessResult -Results $npmInstallResults)) {
            return (Fail-Install)
        }
    }

    if (-not (Ensure-SupportClawOnPath)) {
        Write-Host "Install completed, but SupportClaw is not on PATH yet." -ForegroundColor Yellow
        Write-Host "Open a new terminal, then run: supportClaw doctor" -ForegroundColor Cyan
        return
    }
    ...
    try {
        $installedVersion = (Invoke-SupportClawCommand --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.supportClaw -and $npmList.dependencies.supportClaw.version) {
                $installedVersion = $npmList.dependencies.supportClaw.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    Write-Host ""
    if ($installedVersion) {
        Write-Host "SupportClaw installed successfully ($installedVersion)!" -ForegroundColor Green
    } else {
        Write-Host "SupportClaw installed successfully!" -ForegroundColor Green
    }
    Write-Host ""
    if ($isUpgrade) {
        $updateMessages = @(
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\supportClaw.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "supportClaw doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "supportClaw onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
            Write-Host "Starting setup..." -ForegroundColor Cyan
            Write-Host ""
            Invoke-InteractiveSupportClawCommand onboard
        }
    }
