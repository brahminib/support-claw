...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.SUPPORT_CLAW_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[supportClaw] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[supportClaw] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[supportClaw] This usually means a file in node_modules is empty or truncated.");
  console.error("[supportClaw] Recommended recovery:");
  console.error("[supportClaw]   rm -rf node_modules");
  console.error("[supportClaw]   pnpm store prune");
  console.error("[supportClaw]   pnpm install");
  console.error("");
  console.error("[supportClaw] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.SUPPORT_CLAW_WATCH_MODE = "1";
  childEnv.SUPPORT_CLAW_WATCH_SESSION = watchSession;
  ...
  childEnv.SUPPORT_CLAW_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.SUPPORT_CLAW_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `supportClaw doctor --fix --non-interactive` once.",
        deps,
      );
