...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[supportClaw] warning: detected ${packageManager} for install lifecycle.`,
    "[supportClaw] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[supportClaw] prefer: corepack pnpm install",
  ].join("\n");
}
