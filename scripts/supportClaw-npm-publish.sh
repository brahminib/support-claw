...
usage() {
  echo "usage: bash scripts/supportClaw-npm-publish.sh --publish [package.tgz]"
}
...
mapfile -t publish_plan < <(
  PACKAGE_VERSION="${package_version}" REQUESTED_PUBLISH_TAG="${SUPPORT_CLAW_NPM_PUBLISH_TAG:-}" \
    node --import tsx --input-type=module <<'EOF'
import { resolveNpmPublishPlan } from "./scripts/supportClaw-npm-release-check.ts";
