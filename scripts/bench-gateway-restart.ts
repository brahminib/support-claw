// Bench Gateway Restart script supports SupportClaw repository automation.
...
const GATEWAY_CASES: readonly GatewayBenchCase[] = [
  {
    ...
    env: { SUPPORT_CLAW_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "skipChannelsAcpxProbe",
    name: "gateway restart, skip channels, ACPX startup probe on",
    env: { SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE: "1", SUPPORT_CLAW_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "skipChannelsNoAcpxProbe",
    name: "gateway restart, skip channels, ACPX startup probe off",
    env: { SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE: "0", SUPPORT_CLAW_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "default",
...
function printUsage(): void {
  console.log(`SupportClaw Gateway restart benchmark
...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "supportClaw.json");
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "supportClaw-gateway-restart-bench-"));
