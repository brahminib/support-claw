// Plugin Clawhub Release script supports SupportClaw repository automation.
...
const SUPPORT_CLAW_PLUGIN_CLAWHUB_REPOSITORY = "supportClaw/supportClaw";
const SUPPORT_CLAW_PLUGIN_CLAWHUB_WORKFLOW_FILENAME = "plugin-clawhub-release.yml";
...
const CLAWHUB_SHARED_RELEASE_INPUT_PATHS = [
  ...
  "scripts/supportClaw-npm-release-check.ts",
...
export function collectClawHubPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.supportClaw?.release?.publishToClawHub !== true) {
      continue;
    }
...
export function collectClawHubVersionGateErrors(params: {
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.supportClaw?.release?.publishToClawHub !== true) {
      continue;
    }
    const baseVersion =
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    try {
      if (!retryRateLimit) {
        ...
        return isSupportClawPluginTrustedPublisher(trustedPublisherDetail.trustedPublisher);
      }
    } finally {
      request.clearTimeout();
    }
...
function isSupportClawPluginTrustedPublisher(value: unknown): boolean {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return false;
  }
  const trustedPublisher = value as ClawHubTrustedPublisherConfig;
  return (
    trustedPublisher.repository === SUPPORT_CLAW_PLUGIN_CLAWHUB_REPOSITORY &&
    trustedPublisher.workflowFilename === SUPPORT_CLAW_PLUGIN_CLAWHUB_WORKFLOW_FILENAME &&
