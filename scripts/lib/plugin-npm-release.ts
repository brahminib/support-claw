// Plugin Npm Release script supports SupportClaw repository automation.
...
import { parseReleaseVersion } from "../supportClaw-npm-release-check.ts";
...
export const SUPPORT_CLAW_PLUGIN_NPM_REPOSITORY_URL = "https://github.com/brahminib/support-claw";
...
export function collectPublishablePluginPackageErrors(
  ...
  const installNpmSpec = normalizeOptionalString(packageJson.supportClaw?.install?.npmSpec);
  const repositoryUrl =
    typeof packageJson.repository === "string"
      ? packageJson.repository.trim()
      : (packageJson.repository?.url?.trim() ?? "");
  const extensions = packageJson.supportClaw?.extensions ?? [];

  if (!packageName.startsWith("@supportclaw/")) {
    errors.push(
      `package name must start with "@supportclaw/"; found "${packageName || "<missing>"}".`,
    );
  }
  ...
  if (repositoryUrl !== SUPPORT_CLAW_PLUGIN_NPM_REPOSITORY_URL) {
    errors.push(
      `package.json repository.url must be "${SUPPORT_CLAW_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "${repositoryUrl || "<missing>"}".`,
    );
  }
  if (!packageVersion) {
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("supportClaw.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("supportClaw.extensions must contain only non-empty strings.");
  }
  if (!installNpmSpec) {
    errors.push("supportClaw.install.npmSpec must be a non-empty string for publishable plugins.");
  }
  errors.push(
...
export function collectPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    publishable.push({
      ...
      installNpmSpec: normalizeOptionalString(packageJson.supportClaw?.install?.npmSpec),
    });
  }
...
function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-plugin-npm-view-"));
