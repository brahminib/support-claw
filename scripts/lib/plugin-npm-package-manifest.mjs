...
function packageOptsOutOfBundledRuntimeDependencies(packageJson) {
  return packageJson?.supportClaw?.release?.bundleRuntimeDependencies === false;
}
...
export function resolveAugmentedPluginNpmManifest(params) {
  const repoRoot = path.resolve(params.repoRoot ?? ".");
  const packageDir = resolvePackageDir(repoRoot, params.packageDir);
  const manifestPath = path.join(packageDir, "supportClaw.plugin.json");
...
function main(argv = process.argv.slice(2)) {
  ...
  return withAugmentedPluginNpmManifestForPackage(
    {
      packageDir,
      bundleDependencies: process.env.SUPPORT_CLAW_PLUGIN_NPM_BUNDLE_DEPENDENCIES,
    },
    ({ packageDir: cwd }) => {
