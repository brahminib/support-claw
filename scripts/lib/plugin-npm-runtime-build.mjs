...
export function isPublishablePluginPackage(packageJson) {
  return (
    packageJson.supportClaw?.release?.publishToNpm === true ||
    packageJson.supportClaw?.release?.publishToClawHub === true
  );
}
...
function createNeverBundleDependencyMatcher(packageJson) {
  ...
  return (id) => {
    if (id === "supportClaw" || id.startsWith("supportClaw/")) {
      return true;
    }
...
export function resolvePluginNpmRuntimePackageFiles(plan) {
  ...
  merged.add("dist/**");
  if (packageRelativePathExists(plan.packageDir, "supportClaw.plugin.json")) {
    merged.add("supportClaw.plugin.json");
  }
...
function normalizeSupportClawPeerRange(value) {
...
function resolveSupportClawPeerRange(packageJson, rootPackageJson) {
  return (
    normalizeSupportClawPeerRange(packageJson.supportClaw?.compat?.pluginApi) ||
    normalizeSupportClawPeerRange(packageJson.peerDependencies?.supportClaw) ||
    normalizeSupportClawPeerRange(packageJson.supportClaw?.build?.supportClawVersion) ||
    normalizeSupportClawPeerRange(rootPackageJson?.version) ||
    normalizeSupportClawPeerRange(packageJson.version)
  );
}

/** Resolve package peer dependency metadata for the SupportClaw plugin API. */
export function resolvePluginNpmRuntimePackagePeerMetadata(plan) {
  const supportClawPeerRange = resolveSupportClawPeerRange(plan.packageJson, plan.rootPackageJson);
  if (!supportClawPeerRange) {
    throw new Error(
      `cannot infer supportClaw peerDependency range for ${plan.pluginDir}; set supportClaw.compat.pluginApi or package version`,
    );
  }
  ...
  const existingSupportClawMeta = getRecord(existingPeerDependenciesMeta.supportClaw);
  return {
    peerDependencies: {
      ...existingPeerDependencies,
      supportClaw: supportClawPeerRange,
    },
    peerDependenciesMeta: {
      ...existingPeerDependenciesMeta,
      supportClaw: {
        ...existingSupportClawMeta,
        optional: true,
      },
    },
  };
...
export function resolvePluginNpmRuntimeBuildPlan(params) {
  ...
  const plan = {
    ...
    runtimeExtensions: (Array.isArray(packageJson.supportClaw?.extensions)
      ? packageJson.supportClaw.extensions
      : []
    )
      .map(normalizePackageEntry)
      .filter(Boolean)
      .map(toPackageRuntimeEntry),
    runtimeSetupEntry: normalizePackageEntry(packageJson.supportClaw?.setupEntry)
      ? toPackageRuntimeEntry(packageJson.supportClaw.setupEntry)
      : undefined,
  };
  return {
    ...plan,
