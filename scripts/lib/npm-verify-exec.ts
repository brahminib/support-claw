// Npm Verify Exec script supports SupportClaw repository automation.
...
export function runNpmVerifyCommand(
  ...
  const timeoutMs =
    options.timeoutMs ??
    positiveEnvInt("SUPPORT_CLAW_NPM_VERIFY_COMMAND_TIMEOUT_MS", DEFAULT_NPM_VERIFY_COMMAND_TIMEOUT_MS);
  const maxBuffer =
    options.maxBufferBytes ??
    positiveEnvInt(
      "SUPPORT_CLAW_NPM_VERIFY_COMMAND_MAX_BUFFER_BYTES",
