...
docker_build_on_missing_enabled() {
  case "${SUPPORT_CLAW_DOCKER_BUILD_ON_MISSING:-}" in
    1 | true | TRUE | yes | YES)
      return 0
      ;;
    0 | false | FALSE | no | NO)
  ...
  [ "${SUPPORT_CLAW_TESTBOX:-0}" = "1" ]
}

docker_build_command() {
  local build_cmd=(docker build)
  if [ "${SUPPORT_CLAW_DOCKER_BUILD_USE_BUILDX:-0}" = "1" ] || docker_build_args_need_buildx "$@"; then
    build_cmd=(docker buildx build --load)
    if [ -n "${SUPPORT_CLAW_DOCKER_BUILD_CACHE_FROM:-}" ]; then
      build_cmd+=(--cache-from "${SUPPORT_CLAW_DOCKER_BUILD_CACHE_FROM}")
    fi
    if [ -n "${SUPPORT_CLAW_DOCKER_BUILD_CACHE_TO:-}" ]; then
      build_cmd+=(--cache-to "${SUPPORT_CLAW_DOCKER_BUILD_CACHE_TO}")
    fi
  fi
...
docker_build_retry_count() {
  local configured="${SUPPORT_CLAW_DOCKER_BUILD_RETRIES:-2}"
  if [[ "$configured" =~ ^[0-9]+$ ]]; then
    echo "$((10#$configured))"
    return 0
  fi
  echo "invalid SUPPORT_CLAW_DOCKER_BUILD_RETRIES: $configured" >&2
  return 2
}

docker_build_timeout_required() {
  case "${SUPPORT_CLAW_DOCKER_BUILD_REQUIRE_TIMEOUT:-0}" in
    1 | true | TRUE | yes | YES)
      return 0
      ;;
  esac
...
docker_build_heartbeat_seconds() {
  local configured="${SUPPORT_CLAW_DOCKER_BUILD_HEARTBEAT_SECONDS:-30}"
  if [[ "$configured" =~ ^[0-9]+$ ]] && [ "$configured" -ge 1 ]; then
    echo "$((10#$configured))"
    return 0
  fi
  echo "invalid SUPPORT_CLAW_DOCKER_BUILD_HEARTBEAT_SECONDS: $configured" >&2
  return 2
}
...
docker_build_with_retries() {
  ...
  local timeout_value="${SUPPORT_CLAW_DOCKER_BUILD_TIMEOUT:-3600s}"
  while true; do
...
docker_build_run() {
  ...
  SUPPORT_CLAW_DOCKER_BUILD_REQUIRE_TIMEOUT="${SUPPORT_CLAW_DOCKER_BUILD_REQUIRE_TIMEOUT:-1}" \
