...
export function resolvePlainGhBin(env = process.env) {
  if (env.SUPPORT_CLAW_GH_BIN) {
    if (isExecutable(env.SUPPORT_CLAW_GH_BIN)) {
      return env.SUPPORT_CLAW_GH_BIN;
    }
    throw new Error(`SUPPORT_CLAW_GH_BIN is not executable: ${env.SUPPORT_CLAW_GH_BIN}`);
  }
