...
supportClaw_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${SUPPORT_CLAW_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "SUPPORT_CLAW_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
  ...
  tar -C /src \
    ...
    --exclude=.worktrees \
    --exclude=__supportClaw_vitest__ \
    --exclude=relay.sock \
    --exclude='*.sock' \
    --exclude='*/*.sock' \
    --exclude='apps/*/.build' \
...
supportClaw_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"
...
supportClaw_live_stage_node_modules() {
  local dest_dir="${1:?destination directory required}"
...
supportClaw_live_scrub_staged_plugin_index() {
  local dest_dir="${1:?destination directory required}"
  local db_path="$dest_dir/state/supportClaw.sqlite"
...
supportClaw_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.supportClaw"
  ...
  if [ -d "$source_dir" ]; then
    ...
    supportClaw_live_scrub_staged_plugin_index "$dest_dir"
    if [ -d "$source_dir/workspace" ] && [ ! -e "$dest_dir/workspace" ]; then
      ln -s "$source_dir/workspace" "$dest_dir/workspace"
    fi
  fi

  export SUPPORT_CLAW_STATE_DIR="$dest_dir"
  export SUPPORT_CLAW_CONFIG_PATH="$dest_dir/supportClaw.json"
}

supportClaw_live_prepare_staged_config() {
  if [ ! -f "${SUPPORT_CLAW_CONFIG_PATH:-}" ]; then
    return 0
  fi
