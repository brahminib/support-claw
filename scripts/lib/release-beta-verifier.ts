// Release Beta Verifier script supports SupportClaw repository automation.
...
const DEFAULT_REPO = "supportClaw/supportClaw";
...
export function parseReleaseVerifyBetaArgs(argv: string[]): ReleaseVerifyBetaArgs {
  ...
  if (!version || version.startsWith("-")) {
    throw new Error(
      "Usage: pnpm release:verify-beta -- <version> [--workflow-ref REF] [--clawhub-workflow-ref REF] [--full-release-validation-run ID] [--supportClaw-npm-run ID] [--plugin-npm-run ID] [--plugin-clawhub-run ID] [--plugin-clawhub-bootstrap-run ID] [--npm-telegram-run ID] [--skip-github-release] [--skip-clawhub]",
    );
  }
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--full-release-validation-run":
        ...
        break;
      case "--supportClaw-npm-run":
        parsed.workflowRuns.supportClawNpm = next();
        break;
      case "--plugin-npm-run":
...
export async function verifyBetaRelease(
  ...
  const supportClawNpm = await verifyNpmPackage("supportClaw", args.version, args.distTag);
  lines.push(`supportClaw npm OK: ${args.version} (${args.distTag})`);
  ...
  if (!args.skipPostpublish) {
    ...
    lines.push("supportClaw postpublish verifier OK");
  }
  ...
  if (args.workflowRuns.supportClawNpm !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.supportClawNpm,
        label: "SupportClaw NPM Release",
        repo: args.repo,
        expectedWorkflowName: "SupportClaw NPM Release",
        expectedHeadBranch: args.workflowRef,
        rerunFailed: false,
      }),
    );
      ...
      `${JSON.stringify(
        {
          ...
          supportClawNpmIntegrity: supportClawNpm.integrity,
          supportClawNpmTarball: supportClawNpm.tarball,
