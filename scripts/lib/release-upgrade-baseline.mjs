...
export function compareSupportClawVersions(leftVersion, rightVersion) {
  const left = parseVersion(leftVersion);
  const right = parseVersion(rightVersion);
  if (!left || !right) {
    throw new Error(`cannot compare SupportClaw versions: ${leftVersion} ${rightVersion}`);
  }
...
function normalizePublishedVersions(publishedVersions) {
  return [...new Set(publishedVersions.map((version) => String(version).trim()).filter(Boolean))]
    ...
    .toSorted((left, right) => compareSupportClawVersions(right, left));
}

export function resolveDefaultReleaseUpgradeBaseline(candidateVersion, publishedVersions) {
  const candidate = parseVersion(candidateVersion);
  if (!candidate) {
    throw new Error(`invalid candidate SupportClaw version: ${candidateVersion}`);
  }

  const versions = normalizePublishedVersions(publishedVersions);
  const older = versions.find((version) => compareSupportClawVersions(version, candidate.version) < 0);
  if (older) {
    return `supportClaw@${older}`;
  }

  const same = versions.find(
    (version) => compareSupportClawVersions(version, candidate.version) === 0,
  );
  if (same) {
    return `supportClaw@${same}`;
  }

  throw new Error(`no published SupportClaw baseline is <= candidate ${candidate.version}`);
}
...
function readPublishedVersions(args) {
  ...
  const raw = execFileSync("npm", ["view", "supportClaw", "versions", "--json", "--silent"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "inherit"],
  });
  const parsed = JSON.parse(raw);
  if (!Array.isArray(parsed)) {
    throw new Error("npm returned a non-array supportClaw versions payload");
  }
  return parsed;
}
