...
function isSystemThrottleDisabled(env) {
  const normalized = env.SUPPORT_CLAW_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestEnv(env = process.env) {
  const normalizedLocalCheck = env.SUPPORT_CLAW_LOCAL_CHECK?.trim().toLowerCase();
