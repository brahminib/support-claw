// Verifies installed packages can bootstrap the default SupportClaw workspace files.
...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "supportClaw-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "supportClawCli.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".supportClaw", "workspace");
