...
function isCloseoutEvidenceAsset(assetName, tag) {
  ...
  return (
    assetName === `supportClaw-${releaseVersion}-stable-main-closeout.json` ||
    assetName === `supportClaw-${releaseVersion}-stable-main-closeout.json.sha256`
  );
}
...
export function verifyStableMainCloseout(params) {
  ...
  const expectedMacAssets = [
    `SupportClaw-${macAssetVersion}.zip`,
    `SupportClaw-${macAssetVersion}.dmg`,
    `SupportClaw-${macAssetVersion}.dSYM.zip`,
  ];
