#!/usr/bin/env node
// Test Env Mutation Report script supports SupportClaw repository automation.
...
const DEFAULT_ALLOWED_FILES = new Map([
  ["src/test-utils/supportClaw-test-state.ts", "canonical SupportClaw test state helper"],
  ...
  ["test/setup-supportClaw-runtime.ts", "global runtime-test setup owns process env isolation"],
  [
...
export function renderTestEnvMutationReport(
  ...
  const lines = [
    "SupportClaw test env mutation report",
...
function printHelp(): void {
  process.stdout.write(`SupportClaw test env mutation report
