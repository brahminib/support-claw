...
const TOOLING_ISOLATED_TEST_TARGET = "test/scripts/supportClaw-e2e-instance.test.ts";
...
const INCLUDE_FILE_ENV_KEY = "SUPPORT_CLAW_VITEST_INCLUDE_FILE";
const FS_MODULE_CACHE_PATH_ENV_KEY = "SUPPORT_CLAW_VITEST_FS_MODULE_CACHE_PATH";
...
const BROAD_CHANGED_FALLBACK_PATTERNS = [
  /^package\.json$/u,
  /^pnpm-lock\.yaml$/u,
  /^test\/setup(?:\.shared|\.extensions|-supportClaw-runtime)?\.ts$/u,
...
const GITHUB_WORKFLOW_OWNER_TEST_TARGETS = new Map([
  ...
  [
    ...
    [
      ...
      "test/scripts/supportClaw-cross-os-release-workflow.test.ts",
      "test/scripts/package-acceptance-workflow.test.ts",
    ],
  ],
  [
  ...
  [
    ".github/workflows/supportClaw-performance.yml",
    ["test/scripts/supportClaw-performance-workflow.test.ts"],
  ],
  [
    ".github/workflows/supportClaw-release-checks.yml",
    [
      "test/scripts/supportClaw-cross-os-release-checks.test.ts",
      "test/scripts/package-acceptance-workflow.test.ts",
      "test/scripts/plugin-prerelease-test-plan.test.ts",
  ...
  [
    ".github/workflows/supportClaw-scheduled-live-checks.yml",
    ["test/scripts/package-acceptance-workflow.test.ts"],
  ],
  [
    ".github/workflows/supportClaw-stable-main-closeout.yml",
    ["test/scripts/package-acceptance-workflow.test.ts"],
  ],
  [
...
const TOOLING_SOURCE_TEST_TARGETS = new Map([
  ...
  [
    "scripts/github/resolve-supportClaw-ref.sh",
    ["test/scripts/resolve-supportClaw-ref.test.ts"],
  ],
  ["scripts/ci-hydrate-testbox-env.sh", ["test/scripts/ci-hydrate-testbox-env.test.ts"]],
  [
    "scripts/github/run-supportClaw-cross-os-release-checks.sh",
    ["test/scripts/supportClaw-cross-os-release-workflow.test.ts"],
  ],
  ...
  ["scripts/lib/supportClaw-test-state.mjs", ["test/scripts/supportClaw-test-state.test.ts"]],
  [
  ...
  ["scripts/supportClaw-npm-prepublish-verify.ts", ["test/supportClaw-npm-prepublish-verify.test.ts"]],
  ["scripts/supportClaw-npm-postpublish-verify.ts", ["test/supportClaw-npm-postpublish-verify.test.ts"]],
  ...
  [
    "scripts/check-supportClaw-package-tarball.mjs",
    ["test/scripts/check-supportClaw-package-tarball.test.ts"],
  ],
  ...
  [
    ...
    ["test/scripts/docker-build-helper.test.ts", "test/scripts/supportClaw-test-state.test.ts"],
  ],
  [
  ...
  [
    "scripts/supportClaw-performance-source-summary.mjs",
    ["test/scripts/supportClaw-performance-source-summary.test.ts"],
  ],
...
const TOOLING_TEST_TARGETS = new Map([
  ...
  ["test/scripts/supportClaw-test-state.test.ts", ["test/scripts/supportClaw-test-state.test.ts"]],
...
const SOURCE_TEST_TARGETS = new Map([
  ...
  ["src/test-utils/supportClaw-test-state.ts", ["src/test-utils/supportClaw-test-state.test.ts"]],
  [
...
const BROAD_CHANGED_ENV_KEY = "SUPPORT_CLAW_TEST_CHANGED_BROAD";
const VITEST_NO_OUTPUT_TIMEOUT_ENV_KEY = "SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS";
const VITEST_NO_OUTPUT_HEARTBEAT_ENV_KEY = "SUPPORT_CLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS";
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "SUPPORT_CLAW_VITEST_NO_OUTPUT_RETRY";
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = parsePositiveInt(
    process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL,
    "SUPPORT_CLAW_TEST_PROJECTS_PARALLEL",
  );
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.SUPPORT_CLAW_TEST_PROJECTS_SERIAL !== "1" && env.CI !== "true" && env.GITHUB_ACTIONS !== "true"
  );
}
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(
    env.SUPPORT_CLAW_VITEST_MAX_WORKERS ?? env.SUPPORT_CLAW_TEST_WORKERS,
    env.SUPPORT_CLAW_VITEST_MAX_WORKERS === undefined
      ? "SUPPORT_CLAW_TEST_WORKERS"
...
export function resolveParallelFullSuiteConcurrency(specCount, envInput, hostInfo) {
  ...
  const override = parsePositiveInt(
    env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL,
    "SUPPORT_CLAW_TEST_PROJECTS_PARALLEL",
  );
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.SUPPORT_CLAW_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(
          ...
          `supportClaw-vitest-include-${randomUUID()}-${index}.json`,
        )
      : null;
    return {
      config: plan.config,
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.SUPPORT_CLAW_TEST_HEAVY_CHECK_LOCK_HELD === "1") {
    return false;
  }

  if (env.SUPPORT_CLAW_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
