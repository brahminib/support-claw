...
LIVE_GATEWAY_MAX_MODELS="$(supportClaw_live_read_positive_int_env SUPPORT_CLAW_LIVE_GATEWAY_MAX_MODELS 8)"
LIVE_GATEWAY_STEP_TIMEOUT_MS="$(supportClaw_live_read_positive_int_env SUPPORT_CLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS 45000)"
LIVE_GATEWAY_MODEL_TIMEOUT_MS="$(supportClaw_live_read_positive_int_env SUPPORT_CLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS 90000)"
TEMP_DIRS=()
DOCKER_HOME_MOUNT=()
...
elif [[ -n "${SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(supportClaw_live_collect_auth_dirs_from_csv "${SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(supportClaw_live_collect_auth_files_from_csv "${SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
DOCKER_RUN_ARGS=()
supportClaw_live_init_docker_run_args DOCKER_RUN_ARGS "${SUPPORT_CLAW_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e SUPPORT_CLAW_LIVE_GATEWAY_MODELS="${SUPPORT_CLAW_LIVE_GATEWAY_MODELS:-modern}" \
  -e SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS="${SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS:-}" \
  -e SUPPORT_CLAW_LIVE_GATEWAY_THINKING="${SUPPORT_CLAW_LIVE_GATEWAY_THINKING:-}" \
  -e SUPPORT_CLAW_LIVE_GATEWAY_SMOKE="${SUPPORT_CLAW_LIVE_GATEWAY_SMOKE:-1}" \
  -e SUPPORT_CLAW_LIVE_GATEWAY_MAX_MODELS="$LIVE_GATEWAY_MAX_MODELS" \
  -e SUPPORT_CLAW_LIVE_GATEWAY_HEARTBEAT_MS="${SUPPORT_CLAW_LIVE_GATEWAY_HEARTBEAT_MS:-}" \
  -e SUPPORT_CLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS="$LIVE_GATEWAY_STEP_TIMEOUT_MS" \
  -e SUPPORT_CLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS="$LIVE_GATEWAY_MODEL_TIMEOUT_MS" \
