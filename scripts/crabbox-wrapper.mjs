...
const ignoreRepoBinary = process.env.SUPPORT_CLAW_CRABBOX_WRAPPER_IGNORE_REPO_BINARY === "1";
...
function shouldRequireBrokeredAws(commandArgs, providerName) {
  if (process.env.SUPPORT_CLAW_CRABBOX_ALLOW_DIRECT_AWS === "1") {
    return false;
  }
...
function enforceBrokeredAws(commandArgs, providerName) {
  ...
  console.error(
    [
      "[crabbox] provider=aws requires a configured Crabbox broker for SupportClaw proof.",
      "[crabbox] run `crabbox login --url https://crabbox.supportClaw.ai --provider aws`, then retry.",
      "[crabbox] for intentional direct AWS provider debugging, set SUPPORT_CLAW_CRABBOX_ALLOW_DIRECT_AWS=1.",
    ].join("\n"),
  );
  process.exit(2);
}
...
function remoteGitBootstrapForChangedGate(changedGateBase) {
  ...
  return [
    "supportClaw_changed_gate_base=${SUPPORT_CLAW_CHANGED_GATE_BASE:-" + quotedBase + "};",
    'if ! command -v git >/dev/null 2>&1; then echo "git is required for SupportClaw remote changed-gate sync" >&2; exit 2; fi;',
    'supportClaw_changed_gate_remote_base="$(git rev-parse --verify refs/remotes/origin/main 2>/dev/null || true)";',
    'if ! git status --short >/dev/null 2>&1 || [ "$supportClaw_changed_gate_remote_base" != "$supportClaw_changed_gate_base" ]; then',
    "rm -rf .git;",
    "git init -q;",
    "git remote add origin https://github.com/brahminib/support-claw.git 2>/dev/null || git remote set-url origin https://github.com/brahminib/support-claw.git;",
    ...
    "git add -A;",
    "if ! git diff --cached --quiet; then git -c user.name=SupportClaw -c user.email=ci@supportclaw.local commit -q --no-gpg-sign -m remote-changed-gate-tree; fi;",
    "fi",
  ].join(" ");
}
...
function remoteWindowsHydratedNodeModulesBootstrap() {
  return [
    "$supportClawModulesDir = $env:PNPM_CONFIG_MODULES_DIR",
    "if ($supportClawModulesDir) {",
    'if (-not (Test-Path $supportClawModulesDir)) { throw "PNPM_CONFIG_MODULES_DIR does not exist: $supportClawModulesDir" }',
    '$supportClawWorkspaceModules = Join-Path (Get-Location).Path "node_modules"',
    '$supportClawSelfModules = Join-Path $supportClawModulesDir "node_modules"',
    'if (-not (Test-Path $supportClawSelfModules)) { cmd /c mklink /J "$supportClawSelfModules" "$supportClawModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link hydrated pnpm node_modules" } }',
    'if (-not (Test-Path $supportClawWorkspaceModules)) { cmd /c mklink /J "$supportClawWorkspaceModules" "$supportClawModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link workspace node_modules" } }',
    "}",
  ].join("; ");
}
...
function remoteAwsMacosJsBootstrap({ packageManager = false, bun = false } = {}) {
  const nodeVersion = process.env.SUPPORT_CLAW_CRABBOX_MACOS_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "supportClaw_crabbox_bootstrap_macos_js() {",
    'tool_root="${SUPPORT_CLAW_CRABBOX_MACOS_TOOLCHAIN_DIR:-$HOME/.supportClaw-crabbox-toolchain}";',
    ...
    'macos_locale="${SUPPORT_CLAW_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}";',
    ...
    'ready_marker="$node_dir/.supportClaw-crabbox-node-ready";',
    ...
    "supportClaw_crabbox_env() {",
    "supportClaw_env_args=();",
    "supportClaw_env_ignore=0;",
    "supportClaw_env_path_seen=0;",
    'while [ "$#" -gt 0 ]; do',
    'case "$1" in',
    '-i|--ignore-environment) supportClaw_env_ignore=1; supportClaw_env_args+=("$1"); shift ;;',
    '-S|--split-string|-S*|--split-string=*) command env "${supportClaw_env_args[@]}" "$@"; return ;;',
    '-[!-]*i*) supportClaw_env_ignore=1; supportClaw_env_args+=("$1"); shift ;;',
    '-u|--unset|-C|--chdir) supportClaw_env_args+=("$1"); shift; if [ "$#" -gt 0 ]; then supportClaw_env_args+=("$1"); shift; fi ;;',
    '--unset=*|--chdir=*) supportClaw_env_args+=("$1"); shift ;;',
    'PATH=*) if [ "$supportClaw_env_ignore" = "1" ]; then supportClaw_env_args+=("PATH=$PATH:${1#PATH=}"); else supportClaw_env_args+=("$1"); fi; supportClaw_env_path_seen=1; shift ;;',
    '[A-Za-z_]*=*) supportClaw_env_args+=("$1"); shift ;;',
    '--) supportClaw_env_args+=("--"); shift; break ;;',
    "*) break ;;",
    "esac;",
    "done;",
    'if [ "$supportClaw_env_ignore" = "1" ] && [ "$supportClaw_env_path_seen" = "0" ]; then supportClaw_env_args+=("PATH=$PATH"); fi;',
    'command env "${supportClaw_env_args[@]}" "$@";',
    "};",
  ];
  if (packageManager) {
    bootstrap.push(
  ...
  if (bun) {
    bootstrap.push(
      ...
      'bun_ready_marker="$bun_root/.supportClaw-crabbox-bun-ready";',
  ...
  bootstrap.push("};", "supportClaw_crabbox_bootstrap_macos_js");
...
function scopedAwsMacosEnvCommand(commandArgs) {
  ...
  return {
    ...
    shellCommand: `supportClaw_crabbox_env ${shellJoin(commandArgs.slice(1))}`,
  };
}
...
function remoteAwsMacosSwiftBootstrap() {
  return [
    "supportClaw_crabbox_require_macos_swift_62() {",
    'supportClaw_xcode="";',
    'for supportClaw_candidate in /Applications/Xcode_26.1.app /Applications/Xcode_26*.app /Applications/Xcode-26*.app; do if [ -d "$supportClaw_candidate" ]; then supportClaw_xcode="$supportClaw_candidate"; fi; done;',
    'if [ -n "$supportClaw_xcode" ]; then supportClaw_developer="$supportClaw_xcode/Contents/Developer"; if [ ! -d "$supportClaw_developer" ]; then supportClaw_developer="$supportClaw_xcode"; fi; sudo xcode-select -s "$supportClaw_developer" || return 1; fi;',
    'supportClaw_swift_version="$(swift --version 2>&1)" || { status=$?; printf "%s\\n" "$supportClaw_swift_version" >&2; return "$status"; };',
    'printf "%s\\n" "$supportClaw_swift_version" >&2;',
    'supportClaw_swift_major_minor="$(printf "%s\\n" "$supportClaw_swift_version" | sed -nE "s/.*Apple Swift version ([0-9]+)\\.([0-9]+).*/\\1 \\2/p" | head -n 1)";',
    'if [ -z "$supportClaw_swift_major_minor" ]; then echo "[crabbox] SupportClaw macOS app proof requires Swift tools 6.2+; unable to parse swift --version." >&2; return 2; fi;',
    "set -- $supportClaw_swift_major_minor;",
    'if [ "$1" -lt 6 ] || { [ "$1" -eq 6 ] && [ "$2" -lt 2 ]; }; then',
    'echo "[crabbox] SupportClaw macOS app proof requires Swift tools 6.2+ (Xcode 26.x)." >&2;',
...
function prepareAwsMacosScriptStdinBootstrap(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "supportClaw-crabbox-macos-script-"));
...
function createAwsMacosScriptStdinWrapper(script) {
  ...
  return [
    ...
    'tmp_script="$(mktemp "${TMPDIR:-/tmp}/supportClaw-crabbox-script.XXXXXX")" || exit $?',
    'cleanup_supportClaw_crabbox_script() { rm -f "$tmp_script"; }',
    "trap cleanup_supportClaw_crabbox_script EXIT",
...
function uniqueHereDocDelimiter(script) {
  let index = 0;
  for (;;) {
    const delimiterLocal = `SUPPORT_CLAW_CRABBOX_SCRIPT_${index}`;
...
function defaultFullCheckoutSyncRoot() {
  const home = homedir();
  if (home) {
    return resolve(home, ".cache", "supportClaw", "crabbox-sync");
  }
  return resolve(tmpdir(), "supportClaw-crabbox-sync");
}

function fullCheckoutSyncRoot() {
  const configured = process.env.SUPPORT_CLAW_CRABBOX_SYNC_TMPDIR?.trim();
...
function assertFullCheckoutSyncDisk(root) {
  const requiredBytes = parseNonNegativeIntegerEnv(
    "SUPPORT_CLAW_CRABBOX_SYNC_MIN_FREE_BYTES",
    1024 * 1024 * 1024,
    "byte count",
  );
  if (requiredBytes === 0) {
  ...
  throw new Error(
    [
      ...
      "set SUPPORT_CLAW_CRABBOX_SYNC_TMPDIR to a roomier filesystem or lower SUPPORT_CLAW_CRABBOX_SYNC_MIN_FREE_BYTES if you know this checkout fits",
    ].join("; "),
  );
}
...
function prepareFullCheckoutForSync(options = {}) {
  ...
  const dir = mkdtempSync(resolve(syncRoot, "supportClaw-crabbox-sync-"));
  let active = false;
...
function fullCheckoutKeepaliveIntervalMs() {
  return parseNonNegativeIntegerEnv(
    "SUPPORT_CLAW_CRABBOX_SYNC_KEEPALIVE_MS",
    5000,
  ...
  if (!satisfiesMinimumCrabboxVersion(version.text, minimumBlacksmithCrabboxVersion)) {
    console.error(
      [
        ...
        "[crabbox] if using ../crabbox, rebuild it: version=$(git -C ../crabbox describe --tags --always --dirty | sed 's/^v//') && go build -C ../crabbox -trimpath -ldflags \"-s -w -X github.com/supportClaw/crabbox/internal/cli.version=${version}\" -o bin/crabbox ./cmd/crabbox",
      ].join("\n"),
    );
    process.exit(2);
  }
...
if (
  ...
  if (isAwsMacosRemoteTarget(normalizedArgs, provider)) {
    ...
    console.error(
      `[crabbox] warning: provider=aws raw boxes may lack Node/Corepack/pnpm/Bun for ${runtimeEntrypoint}; hydrate first (${hydrate}) or pass --provider blacksmith-testbox for SupportClaw CI-like proof; not switching providers automatically`,
    );
  }
}
...
if (
  ...
  console.error(
    "[crabbox] provider=docker enabling host Docker socket pass-through for SupportClaw Docker tests",
  );
}
if (
  isLocalContainerProvider(provider) &&
  ...
  childEnv.CRABBOX_LOCAL_CONTAINER_WORK_ROOT = "/tmp/supportClaw-crabbox-docker-work";
  console.error(
