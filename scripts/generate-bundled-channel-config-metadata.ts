#!/usr/bin/env node
// Generate Bundled Channel Config Metadata script supports SupportClaw repository automation.
...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const supportClawMeta =
    source.packageJson &&
    typeof source.packageJson === "object" &&
    !Array.isArray(source.packageJson) &&
    "supportClaw" in source.packageJson
      ? (source.packageJson.supportClaw as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    supportClawMeta &&
    typeof supportClawMeta.channel === "object" &&
    supportClawMeta.channel &&
    !Array.isArray(supportClawMeta.channel)
      ? (supportClawMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
