// Tool Search Gateway E2E script supports SupportClaw repository automation.
...
export function readToolSearchGatewayFetchLimits(
  ...
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "SUPPORT_CLAW_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES",
      1024 * 1024,
      env,
    ),
    timeoutMs: readPositiveIntEnv(
      "SUPPORT_CLAW_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS",
      180_000,
      env,
    ),
  };
...
export function snapshotToolSearchGatewayEnv(
  ...
  return {
    configPath: env.SUPPORT_CLAW_CONFIG_PATH,
    stateDir: env.SUPPORT_CLAW_STATE_DIR,
    testFast: env.SUPPORT_CLAW_TEST_FAST,
  };
}
...
function restoreEnvValue(
  ...
  key: keyof Pick<
    NodeJS.ProcessEnv,
    "SUPPORT_CLAW_CONFIG_PATH" | "SUPPORT_CLAW_STATE_DIR" | "SUPPORT_CLAW_TEST_FAST"
  >,
...
export function restoreToolSearchGatewayEnv(
  ...
  restoreEnvValue(env, "SUPPORT_CLAW_CONFIG_PATH", snapshot.configPath);
  restoreEnvValue(env, "SUPPORT_CLAW_STATE_DIR", snapshot.stateDir);
  restoreEnvValue(env, "SUPPORT_CLAW_TEST_FAST", snapshot.testFast);
}
...
async function writeFakePlugin(params: {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      {
        name: "@supportclaw/tool-search-e2e-fixture",
        version: "0.0.0",
        type: "module",
        supportClaw: {
          extensions: ["./index.js"],
  ...
  try {
    ...
    const response = await fetchJson(`http://127.0.0.1:${gatewayPort}/v1/responses`, {
      ...
      headers: {
        ...
        "x-supportClaw-agent": "qa",
      },
      body: JSON.stringify({
        model: "supportClaw/qa",
        input: [
          {
            type: "message",
            role: "user",
...
export async function main() {
  ...
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-tool-search-"));
