// Sync Plugin Versions script supports SupportClaw repository automation.
...
const SUPPORT_CLAW_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncSupportClawDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.supportClaw;
  if (!current || current === "workspace:*" || !SUPPORT_CLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  ...
  deps.supportClaw = next;
  return true;
}

function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.supportClaw?.compat;
  const current = compat?.pluginApi;
  if (!current || !SUPPORT_CLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildSupportClawVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.supportClaw?.build;
  const current = build?.supportClawVersion;
  if (!current) {
    return false;
  }
  ...
  build.supportClawVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncSupportClawDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncSupportClawDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildSupportClawVersionChanged = syncBuildSupportClawVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildSupportClawVersionChanged;
    if (!packageChanged) {
