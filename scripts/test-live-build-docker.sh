...
DOCKER_BUILD_EXTENSIONS="${SUPPORT_CLAW_DOCKER_BUILD_EXTENSIONS:-${SUPPORT_CLAW_EXTENSIONS:-}}"
LIVE_IMAGE_PULL_ATTEMPTS="${SUPPORT_CLAW_LIVE_DOCKER_PULL_ATTEMPTS:-3}"
LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS="${SUPPORT_CLAW_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"

if ! [[ "$LIVE_IMAGE_PULL_ATTEMPTS" =~ ^[1-9][0-9]*$ ]]; then
  echo "SUPPORT_CLAW_LIVE_DOCKER_PULL_ATTEMPTS must be a positive integer, got: $LIVE_IMAGE_PULL_ATTEMPTS" >&2
  exit 2
fi
if ! [[ "$LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" =~ ^[0-9]+$ ]]; then
  echo "SUPPORT_CLAW_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS must be a non-negative integer, got: $LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" >&2
  exit 2
fi
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "SUPPORT_CLAW_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi
...
if [[ "${SUPPORT_CLAW_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  if ! docker_build_on_missing_enabled; then
    ...
    echo "Build it first or unset SUPPORT_CLAW_SKIP_DOCKER_BUILD." >&2
    exit 1
  fi
  echo "==> Live-test image not available; building because SUPPORT_CLAW_DOCKER_BUILD_ON_MISSING/SUPPORT_CLAW_TESTBOX allows fallback."
fi
