...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/supportClaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SupportClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/supportClaw/scripts/mobile-reauth.sh'" --id supportClaw-auth
        ;;
    *)
