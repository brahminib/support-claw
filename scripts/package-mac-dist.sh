...
# - dist/SupportClaw-<version>.zip
# - dist/SupportClaw-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-ai.supportClaw.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/supportClaw-sparkle-build.XXXXXX")" || {
...
APP="$ROOT_DIR/dist/SupportClaw.app"
if [[ ! -d "$APP" ]]; then
...
ZIP="$ROOT_DIR/dist/SupportClaw-$VERSION.zip"
DMG="$ROOT_DIR/dist/SupportClaw-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/SupportClaw-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/SupportClaw-$VERSION.dSYM.zip"
