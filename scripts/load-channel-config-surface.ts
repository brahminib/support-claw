// Load Channel Config Surface script supports SupportClaw repository automation.
...
const jitiFactoryOverrideKey = Symbol.for("supportClaw.channelConfigSurfaceJitiFactoryOverride");
...
export async function loadChannelConfigSurfaceModule(
  ...
  const loadViaBun = (candidatePath: string) => {
    const script = `
      ...
      const modulePath = process.env.SUPPORT_CLAW_CONFIG_SURFACE_MODULE;
      if (!modulePath) {
        throw new Error("missing SUPPORT_CLAW_CONFIG_SURFACE_MODULE");
      }
    ...
    const result = spawnSync("bun", ["-e", script], {
      ...
      env: {
        ...process.env,
        SUPPORT_CLAW_CONFIG_SURFACE_MODULE: path.resolve(candidatePath),
      },
    });
    if (result.error) {
  ...
  const loadViaJiti = (candidatePath: string) => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "supportClaw/plugin-sdk": pluginSdkAlias } : {}),
      ...resolvePluginSdkScopedAliasMap({
        modulePath: resolvedPath,
        pluginSdkResolution: "src",
      }),
