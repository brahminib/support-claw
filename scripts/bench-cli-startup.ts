// Bench Cli Startup script supports SupportClaw repository automation.
...
const DEFAULT_ENTRY = "supportClawCli.mjs";
...
function parseGatewayPortEnv(raw: string | undefined): number {
  ...
  if (bracketHostMatch) {
    return parsePositiveInt(bracketHostMatch[1], 32123, "SUPPORT_CLAW_GATEWAY_PORT");
  }
  ...
  return parsePositiveInt(portRaw, 32123, "SUPPORT_CLAW_GATEWAY_PORT");
}
...
function buildConfigFixture(commandCase: CommandCase): Record<string, unknown> | null {
  ...
  const port = parseGatewayPortEnv(process.env.SUPPORT_CLAW_GATEWAY_PORT);
  return {
    gateway: {
      auth: { mode: "none" },
      bind: "loopback",
...
async function runSample(params: {
  ...
  const runRoot = mkdtempSync(path.join(os.tmpdir(), "supportClaw-cli-bench-home-"));
  const stateDir = path.join(runRoot, ".supportClaw");
...
function printUsage(): void {
  console.log(`SupportClaw CLI benchmark
...
Options:
  ...
  --entry <path>               Primary entry file (default: supportClawCli.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "supportClaw-cli-bench-"));
