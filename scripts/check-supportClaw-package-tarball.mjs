...
function usage() {
  return "Usage: node scripts/check-supportClaw-package-tarball.mjs <supportClaw.tgz>";
}
...
function parseArgs(argv) {
  ...
  if (tarball.startsWith("-")) {
    throw new Error(`Unknown SupportClaw package tarball check option: ${tarball}`);
  }
  const extraArg = args[1]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected SupportClaw package tarball check argument: ${extraArg}`);
  }
...
if (!fs.existsSync(tarball)) {
  fail(`SupportClaw package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.SUPPORT_CLAW_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-supportClaw-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-package-tarball-"));
try {
...
if (!entrySet.has("npm-shrinkwrap.json")) {
  ...
  try {
    ...
    if (shrinkwrap.name !== "supportClaw") {
      errors.push("npm-shrinkwrap.json root name must be supportClaw");
    }
    ...
    if (!rootPackage || rootPackage.name !== "supportClaw") {
      errors.push("npm-shrinkwrap.json packages root must name supportClaw");
    }
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`SupportClaw package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`SupportClaw package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("SupportClaw package tarball integrity passed.");
