...
const SKIP_GATEWAY_ENV = {
  ...
  SUPPORT_CLAW_SKIP_ACPX_RUNTIME: "1",
  SUPPORT_CLAW_SKIP_ACPX_RUNTIME_PROBE: "1",
...
Options:
  ...
  --keep                         Keep the synthetic SUPPORT_CLAW_HOME and workspace after the run.
...
export function parseArgs(argv) {
  ...
  const options = {
    fileCount: undefined,
    mode: process.env.SUPPORT_CLAW_MEMORY_FD_REPRO_MODE || "fixed",
    ...
    keep: process.env.SUPPORT_CLAW_MEMORY_FD_REPRO_KEEP === "1",
    allowNonDarwin: process.env.SUPPORT_CLAW_MEMORY_FD_REPRO_ALLOW_NON_DARWIN === "1",
  };
  ...
  options.fileCount ??= readPositiveNumberEnv("SUPPORT_CLAW_MEMORY_FD_REPRO_FILES", DEFAULT_FILE_COUNT);
  options.maxWorkspaceRegFds ??= readNumberEnv(
    "SUPPORT_CLAW_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS",
    DEFAULT_MAX_WORKSPACE_REG_FDS,
  );
  options.invokeTimeoutMs ??= readTimerTimeoutNumberEnv("SUPPORT_CLAW_MEMORY_FD_REPRO_TIMEOUT_MS", 30_000);
  options.sampleDelayMs ??= readTimerTimeoutNumberEnv(
    "SUPPORT_CLAW_MEMORY_FD_REPRO_SAMPLE_DELAY_MS",
    1_000,
    0,
  );
  options.settleDelayMs ??= readTimerTimeoutNumberEnv(
    "SUPPORT_CLAW_MEMORY_FD_REPRO_SETTLE_DELAY_MS",
    5_000,
    0,
  );
...
/**
 * Writes isolated SupportClaw config for the synthetic memory workspace.
 */
export function writeConfig({ homeDir, workspaceDir, port, token }) {
  const configDir = path.join(homeDir, ".supportClaw");
...
async function main() {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-memory-fd-repro-"));
  ...
  const env = {
    ...
    ...SKIP_GATEWAY_ENV,
    HOME: homeDir,
    SUPPORT_CLAW_STATE_DIR: path.join(homeDir, ".supportClaw"),
