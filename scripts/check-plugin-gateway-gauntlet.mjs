...
export function parseArgs(argv) {
  ...
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("SUPPORT_CLAW_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("SUPPORT_CLAW_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
    ...
    qaTimeoutMs: 900_000,
    allowEmpty: false,
    failOnObservation: process.env.SUPPORT_CLAW_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION === "1",
    keepRunRoot: process.env.SUPPORT_CLAW_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT === "1",
  };
  const envIds = normalizeCsv(process.env.SUPPORT_CLAW_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
Environment:
  SUPPORT_CLAW_PLUGIN_GATEWAY_GAUNTLET_IDS   Comma-separated plugin ids to include
  SUPPORT_CLAW_PLUGIN_GATEWAY_GAUNTLET_TOTAL Total plugin shards
  SUPPORT_CLAW_PLUGIN_GATEWAY_GAUNTLET_INDEX Zero-based shard index
  SUPPORT_CLAW_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION=1
  SUPPORT_CLAW_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT=1
  SUPPORT_CLAW_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES  QA summary read ceiling
`);
}
...
function supportClawCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function sourceSupportClawCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    SUPPORT_CLAW_LOG_DIR: path.join(runRoot, "logs"),
    SUPPORT_CLAW_QA_SUITE_PROGRESS: process.env.SUPPORT_CLAW_QA_SUITE_PROGRESS ?? "1",
...
function buildSlashHelpProbe(params) {
  ...
  return {
    ...
    ...supportClawCommand(params.repoRoot, [command, "--help"]),
...
async function runPluginLifecycleCommand(params) {
  ...
  params.rows.push(
    await runMeasuredCommand({
      ...
      ...supportClawCommand(params.repoRoot, ["plugins", ...params.args]),
      label: params.label,
...
async function runQaChunks(params) {
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const row = await runMeasuredCommand({
      ...
      ...sourceSupportClawCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-plugin-gauntlet-"));
