...
export function parseCliArgs(args, env = process.env) {
  let shardSpec = env.SUPPORT_CLAW_ADDITIONAL_BOUNDARY_SHARD ?? "";
  let help = false;
  ...
  try {
    ...
    if (cliArgs.help) {
      ...
      const concurrencyRaw =
        process.env.SUPPORT_CLAW_ADDITIONAL_BOUNDARY_CONCURRENCY ??
        process.env.SUPPORT_CLAW_EXTENSION_BOUNDARY_CONCURRENCY;
      const concurrencyLabel =
        process.env.SUPPORT_CLAW_ADDITIONAL_BOUNDARY_CONCURRENCY === undefined
          ? "SUPPORT_CLAW_EXTENSION_BOUNDARY_CONCURRENCY"
          : "SUPPORT_CLAW_ADDITIONAL_BOUNDARY_CONCURRENCY";
      const concurrency = resolveConcurrency(concurrencyRaw, 4, concurrencyLabel);
      const checkTimeoutMs = resolvePositiveInteger(
        process.env.SUPPORT_CLAW_ADDITIONAL_BOUNDARY_TIMEOUT_MS,
        DEFAULT_CHECK_TIMEOUT_MS,
        "SUPPORT_CLAW_ADDITIONAL_BOUNDARY_TIMEOUT_MS",
      );
      const outputMaxBytes = resolvePositiveInteger(
        process.env.SUPPORT_CLAW_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES,
        DEFAULT_OUTPUT_MAX_BYTES,
        "SUPPORT_CLAW_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES",
      );
