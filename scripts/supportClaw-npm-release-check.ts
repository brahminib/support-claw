...
// SupportClaw Npm Release Check script supports SupportClaw repository automation.
...
const EXPECTED_REPOSITORY_URL = "https://github.com/brahminib/support-claw";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@supportclaw/fs-safe";
...
const skipPackValidationEnv = "SUPPORT_CLAW_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return positiveEnvInt(
    "SUPPORT_CLAW_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "supportClaw") {
    errors.push(`package.json name must be "supportClaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.supportClaw !== "supportClawCli.mjs") {
    errors.push(
      `package.json bin.supportClaw must be "supportClawCli.mjs"; found "${pkg.bin?.supportClaw ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`supportClaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `supportClaw-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
