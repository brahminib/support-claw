...
boot_emulator() {
  ...
  EMULATOR_LOG="$(mktemp "${TMPDIR:-/tmp}/supportClaw-android-screenshot-emulator.XXXXXX.log")"
...
for scene in "${SCENES[@]}"; do
  output_path="${OUTPUT_DIR}/supportClaw-${scene}.png"
  "$ADB_BIN" -s "$ADB_SERIAL" shell am force-stop ai.supportClaw.app >/dev/null
  "$ADB_BIN" -s "$ADB_SERIAL" shell am start -W \
    -n ai.supportClaw.app/.MainActivity \
    --ez supportClaw.screenshotMode true \
    --es supportClaw.screenshotScene "$scene" >/dev/null
