...
export function prepareMirroredDocsDir(sourceDir = DOCS_DIR, options = {}) {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-docs-link-audit-"));
  try {
...
export function prepareAnchorAuditDocsDir(sourceDir = DOCS_DIR) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-docs-anchor-audit-"));
  try {
