...
REMOTE_URL="${SUPPORT_CLAW_REF_REMOTE:-https://github.com/brahminib/support-claw.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-supportClaw-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves SupportClaw branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve SupportClaw ref: ${REF}" >&2
exit 1
