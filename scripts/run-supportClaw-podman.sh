#!/usr/bin/env bash
# Rootless SupportClaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-supportClaw-podman.sh launch        # Start gateway
#   ./scripts/run-supportClaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   supportClaw --container supportClaw dashboard --no-open
#   supportClaw --container supportClaw channels login
#
...
run_podman_detached() {
  supportClaw_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"
}
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      SUPPORT_CLAW_GATEWAY_TOKEN|SUPPORT_CLAW_PODMAN_CONTAINER|SUPPORT_CLAW_PODMAN_IMAGE|SUPPORT_CLAW_IMAGE|SUPPORT_CLAW_PODMAN_PULL|SUPPORT_CLAW_PODMAN_RUN_TIMEOUT|SUPPORT_CLAW_PODMAN_GATEWAY_HOST_PORT|SUPPORT_CLAW_GATEWAY_PORT|SUPPORT_CLAW_PODMAN_BRIDGE_HOST_PORT|SUPPORT_CLAW_BRIDGE_PORT|SUPPORT_CLAW_GATEWAY_BIND|SUPPORT_CLAW_PODMAN_USERNS|SUPPORT_CLAW_BIND_MOUNT_OPTIONS|SUPPORT_CLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-supportClaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${SUPPORT_CLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.supportClaw}"
ENV_FILE="${SUPPORT_CLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${SUPPORT_CLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.supportClaw}"
ENV_FILE="${SUPPORT_CLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${SUPPORT_CLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${SUPPORT_CLAW_PODMAN_CONTAINER:-supportClaw}"
SUPPORT_CLAW_IMAGE="${SUPPORT_CLAW_PODMAN_IMAGE:-${SUPPORT_CLAW_IMAGE:-supportClaw:local}}"
PODMAN_PULL="${SUPPORT_CLAW_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${SUPPORT_CLAW_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${SUPPORT_CLAW_PODMAN_GATEWAY_HOST_PORT:-${SUPPORT_CLAW_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${SUPPORT_CLAW_PODMAN_BRIDGE_HOST_PORT:-${SUPPORT_CLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${SUPPORT_CLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$SUPPORT_CLAW_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  SUPPORT_CLAW_CONTAINER="" SUPPORT_CLAW_CONFIG_DIR="$config_dir" \
    supportClaw config get gateway.bind 2>/dev/null || true
}
...
# SUPPORT_CLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${SUPPORT_CLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'SUPPORT_CLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v supportClaw >/dev/null 2>&1; then
    echo "Warning: supportClaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    SUPPORT_CLAW_CONTAINER="" SUPPORT_CLAW_CONFIG_DIR="$config_dir" \
      supportClaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    SUPPORT_CLAW_CONTAINER="" SUPPORT_CLAW_CONFIG_DIR="$config_dir" \
      supportClaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  SUPPORT_CLAW_CONTAINER="" SUPPORT_CLAW_CONFIG_DIR="$config_dir" \
    supportClaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${SUPPORT_CLAW_GATEWAY_TOKEN:-}" ]]; then
  export SUPPORT_CLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "SUPPORT_CLAW_GATEWAY_TOKEN" "$SUPPORT_CLAW_GATEWAY_TOKEN"
  echo "Generated SUPPORT_CLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/supportClaw.json"
...
PODMAN_USERNS="${SUPPORT_CLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported SUPPORT_CLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (SUPPORT_CLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${SUPPORT_CLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${SUPPORT_CLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$SUPPORT_CLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.supportClaw/.npm \
    -e SUPPORT_CLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.supportClaw:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.supportClaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$SUPPORT_CLAW_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME supportClaw dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME supportClaw devices approve --latest  # if pairing required"
