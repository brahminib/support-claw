// Mcp Code Mode Gateway E2E script supports SupportClaw repository automation.
...
export async function main() {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-mcp-code-mode-"));
  const keep = process.env.SUPPORT_CLAW_MCP_CODE_MODE_GATEWAY_E2E_KEEP === "1";
  const previousEnv = {
  ...
  try {
    ...
    await provider?.stop();
    resetConfigRuntimeState();
    restoreEnvValue("SUPPORT_CLAW_STATE_DIR", previousEnv.stateDir);
    restoreEnvValue("SUPPORT_CLAW_CONFIG_PATH", previousEnv.configPath);
    restoreEnvValue("SUPPORT_CLAW_TEST_FAST", previousEnv.testFast);
    if (!keep) {
