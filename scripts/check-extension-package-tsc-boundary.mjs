...
export function resolveCompileConcurrency(
  ...
  const raw = env.SUPPORT_CLAW_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "SUPPORT_CLAW_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.SUPPORT_CLAW_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
