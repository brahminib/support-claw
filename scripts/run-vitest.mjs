...
export function resolveVitestNodeArgs(env = process.env) {
  if (isTruthyEnvValue(env.SUPPORT_CLAW_VITEST_ENABLE_MAGLEV)) {
    return [];
  }
...
export function resolveVitestCliEntry({
  ...
  try {
    ...
    if (isMissingVitestResolveError(error)) {
      ...
      wrappedError.code = "SUPPORT_CLAW_MISSING_VITEST";
      throw wrappedError;
    }
    throw error;
  }
...
function shouldApplyNativeWorkerBudget(env) {
  ...
  return (
    env.SUPPORT_CLAW_TEST_PROJECTS_SERIAL === "1" || resolveExplicitVitestWorkerBudget(env) !== null
  );
}
...
function resolveExplicitVitestWorkerBudget(env) {
  return parsePositiveInt(env.SUPPORT_CLAW_VITEST_MAX_WORKERS ?? env.SUPPORT_CLAW_TEST_WORKERS);
}
  ...
  try {
    return fsImpl.readdirSync(extensionsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fsImpl.existsSync(path.join(pluginDir, "supportClaw.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveLinkedSourceBundledPluginsEnv(
  ...
  if (env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR?.trim()) {
    return {};
  }
  ...
  return {
    SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: extensionsDir,
    SUPPORT_CLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR:
      env.SUPPORT_CLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR?.trim() || "1",
  };
}
...
function main(argv = process.argv.slice(2), env = process.env) {
  ...
  try {
    ...
    if (error instanceof Error && error.code === "SUPPORT_CLAW_MISSING_VITEST") {
