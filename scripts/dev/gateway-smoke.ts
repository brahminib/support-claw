// Gateway Smoke script supports SupportClaw repository automation.
...
function parseGatewaySmokeCli(
  ...
  return {
    help: has("--help") || has("-h"),
    token: getArg("--token") ?? env.SUPPORT_CLAW_GATEWAY_TOKEN,
    urlRaw: getArg("--url") ?? env.SUPPORT_CLAW_GATEWAY_URL,
  };
}
...
export async function runGatewaySmoke(
  ...
  try {
    ...
    const connectRes = await request("connect", {
      ...
      client: {
        id: "supportClaw-ios",
        displayName: "supportClaw gateway smoke test",
        version: "dev",
        platform: "dev",
        mode: "ui",
        instanceId: "supportClaw-dev-smoke",
      },
      locale: "en-US",
