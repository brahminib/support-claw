// Ios Node E2E script supports SupportClaw repository automation.
...
function usage(): string {
  return [
    ...
    "Or set env: SUPPORT_CLAW_GATEWAY_URL / SUPPORT_CLAW_GATEWAY_TOKEN",
    "",
    "Options:",
...
const urlRaw = getArg("--url") ?? process.env.SUPPORT_CLAW_GATEWAY_URL;
const token = getArg("--token") ?? process.env.SUPPORT_CLAW_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.SUPPORT_CLAW_RUN_DANGEROUS === "1";
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "supportClaw ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "supportClaw-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "SupportClaw E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
