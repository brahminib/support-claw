#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports SupportClaw repository automation.
...
type DriverMode = "token" | "webhook" | "supportClaw";
...
const DEFAULT_SUPPORT_CLAW_CLI_TIMEOUT_MS = 60_000;
...
function resolveStateDir(): string {
  const override = process.env.SUPPORT_CLAW_STATE_DIR?.trim();
  if (override) {
    if (override === "~") {
  ...
  const home = process.env.SUPPORT_CLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".supportClaw");
}
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "supportClaw") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or supportClaw.`,
  );
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver supportClaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) SupportClaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|supportClaw> Driver transport mode (default: token)\n" +
    ...
    "  --state-dir <p>              Override SupportClaw state dir for plugin-state polling\n" +
    "  --supportClaw-bin <path>        SupportClaw CLI binary for driver=supportClaw (default: supportClaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  SUPPORT_CLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  SUPPORT_CLAW_DISCORD_SMOKE_DRIVER\n" +
    "  SUPPORT_CLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  SUPPORT_CLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  SUPPORT_CLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  SUPPORT_CLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  SUPPORT_CLAW_DISCORD_SMOKE_AGENT\n" +
    "  SUPPORT_CLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  SUPPORT_CLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  SUPPORT_CLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  SUPPORT_CLAW_STATE_DIR\n" +
    "  SUPPORT_CLAW_DISCORD_SMOKE_SUPPORT_CLAW_BIN"
  );
}
...
function parseArgs(argv = process.argv.slice(2)): Args {
  ...
  const channelId =
    resolveArg("--channel", argv) || process.env.SUPPORT_CLAW_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver", argv) || process.env.SUPPORT_CLAW_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token", argv) || process.env.SUPPORT_CLAW_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix", argv) ||
    process.env.SUPPORT_CLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX ||
    "Bot";
  const botToken =
    resolveArg("--bot-token", argv) ||
    process.env.SUPPORT_CLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix", argv) ||
    process.env.SUPPORT_CLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent", argv) || process.env.SUPPORT_CLAW_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention", argv) ||
    process.env.SUPPORT_CLAW_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction", argv) ||
    process.env.SUPPORT_CLAW_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms", argv) || process.env.SUPPORT_CLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms", argv) || process.env.SUPPORT_CLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  const stateDir = path.resolve(resolveArg("--state-dir", argv) || resolveStateDir());
  const supportClawBin =
    resolveArg("--supportClaw-bin", argv) ||
    process.env.SUPPORT_CLAW_DISCORD_SMOKE_SUPPORT_CLAW_BIN ||
    "supportClaw";
...
async function supportClawCliJson<T>(params: {
  supportClawBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.supportClawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_SUPPORT_CLAW_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`supportClaw ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithSupportClaw(params: {
  supportClawBin: string;
  target: string;
  limit: number;
  timeoutMs?: number;
}): Promise<DiscordMessage[]> {
  const response = await supportClawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    supportClawBin: params.supportClawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function readThreadBindings(stateDir: string): Promise<ThreadBindingRecord[]> {
  const store = createPluginStateKeyedStore<ThreadBindingRecord>("discord", {
    ...
    env: { ...process.env, SUPPORT_CLAW_STATE_DIR: stateDir },
  });
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "supportClaw") {
    return await readMessagesWithSupportClaw({
      supportClawBin: params.args.supportClawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        timeoutMs: remainingTimeoutMs(deadline),
        body: {
          name: `supportClaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      minBindingBoundAt = Date.now() - 3_000;
      const sent = await supportClawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        supportClawBin: args.supportClawBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("supportClaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "supportClaw"
            ? await readMessagesWithSupportClaw({
                supportClawBin: args.supportClawBin,
                target: threadId,
                limit: 50,
                timeoutMs: remainingTimeoutMs(deadline),
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from SupportClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
