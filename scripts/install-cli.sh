...
# SupportClaw CLI installer (non-interactive, no onboarding)
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://supportClaw.ai/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
resolve_supportClaw_effective_home() {
  ...
  case "$supportClaw_home" in
    \~)
      echo "$HOME"
      ;;
    \~/*)
      echo "${HOME}/${supportClaw_home#~/}"
      ;;
    *)
...
SUPPORT_CLAW_EFFECTIVE_HOME="$(resolve_supportClaw_effective_home)"
PREFIX="${SUPPORT_CLAW_PREFIX:-${HOME}/.supportClaw}"
SUPPORT_CLAW_VERSION="${SUPPORT_CLAW_VERSION:-latest}"
NODE_VERSION="${SUPPORT_CLAW_NODE_VERSION:-22.22.0}"
NODE_VERSION_REQUESTED=0
if [[ -n "${SUPPORT_CLAW_NODE_VERSION:-}" ]]; then
  NODE_VERSION_REQUESTED=1
fi
MIN_NODE_VERSION="22.19.0"
APK_NODE_BIN_DIR="/usr/bin"
NPM_LOGLEVEL="${SUPPORT_CLAW_NPM_LOGLEVEL:-error}"
INSTALL_METHOD="${SUPPORT_CLAW_INSTALL_METHOD:-npm}"
GIT_DIR="${SUPPORT_CLAW_GIT_DIR:-${SUPPORT_CLAW_EFFECTIVE_HOME}/supportClaw}"
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.supportClaw; use \$SUPPORT_CLAW_PREFIX to override)
  --install-method, --method npm|git  Install via npm (default) or from a git checkout
  --npm                               Shortcut for --install-method npm
  --git, --github                     Shortcut for --install-method git
  --git-dir, --dir <path>             Checkout directory (default: ~/supportClaw, or \$SUPPORT_CLAW_HOME/supportClaw)
  --version <ver>                     SupportClaw version (default: latest)
...
Environment variables:
  ...
  SUPPORT_CLAW_HOME=...
  SUPPORT_CLAW_PREFIX=...
  SUPPORT_CLAW_VERSION=latest|next|<semver>
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${SUPPORT_CLAW_GIT_DIR:-${SUPPORT_CLAW_EFFECTIVE_HOME}/supportClaw}}"
...
resolve_git_supportClaw_ref() {
  ...
  case "$requested" in
    ""|latest)
      resolved_version="$("$(npm_bin)" view "supportClaw" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
install_supportClaw() {
  local requested="${SUPPORT_CLAW_VERSION:-latest}"
  if is_supportClaw_source_package_install_spec "$requested"; then
    fail "npm installs do not support SupportClaw GitHub source targets like '${requested}'. Use --install-method git --version main, latest, beta, an exact version, or a built .tgz package."
  fi
  ...
  emit_json "{\"event\":\"step\",\"name\":\"supportClaw\",\"status\":\"start\",\"version\":\"${requested}\"}"
  log "Installing SupportClaw (${requested})..."
  if [[ "$SET_NPM_PREFIX" -eq 1 ]]; then
    fix_npm_prefix_if_needed
  fi

  if [[ "${requested}" == "latest" ]]; then
    if ! env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "supportClaw@latest"; then
      log "npm install supportClaw@latest failed; retrying supportClaw@next"
      emit_json "{\"event\":\"step\",\"name\":\"supportClaw\",\"status\":\"retry\",\"version\":\"next\"}"
      env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "supportClaw@next"
      requested="next"
    fi
  else
    env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "supportClaw@${requested}"
  fi

  mkdir -p "${PREFIX}/bin"
  rm -f "${PREFIX}/bin/supportClaw"
  cat > "${PREFIX}/bin/supportClaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "${PREFIX}/tools/node/bin/node" "$(node_dir)/lib/node_modules/supportClaw/dist/entry.js" "\$@"
EOF
  chmod +x "${PREFIX}/bin/supportClaw"
  emit_json "{\"event\":\"step\",\"name\":\"supportClaw\",\"status\":\"ok\",\"version\":\"${requested}\"}"
}
...
install_supportClaw_from_git() {
  ...
  emit_json "{\"event\":\"step\",\"name\":\"supportClaw\",\"status\":\"start\",\"method\":\"git\",\"repo\":\"${repo_url//\"/\\\"}\"}"
  if [[ -d "$repo_dir/.git" ]]; then
    log "Installing SupportClaw from git checkout: ${repo_dir}"
  else
    log "Installing SupportClaw from GitHub (${repo_url})..."
  fi
...
EOF
  chmod +x "${PREFIX}/bin/supportClaw"
  emit_json "{\"event\":\"step\",\"name\":\"supportClaw\",\"status\":\"ok\",\"method\":\"git\"}"
}

resolve_supportClaw_version() {
  local version=""
  if [[ -x "${PREFIX}/bin/supportClaw" ]]; then
    version="$("${PREFIX}/bin/supportClaw" --version 2>/dev/null | head -n 1 | tr -d '\r')"
  fi
  echo "$version"
}
...
refresh_gateway_service_if_loaded() {
  local claw="${PREFIX}/bin/supportClaw"
  if [[ ! -x "$claw" ]]; then
    return 0
  fi
...
main() {
  ...
  if [[ "${SUPPORT_CLAW_NO_ONBOARD:-0}" == "1" ]]; then
    RUN_ONBOARD=0
  fi
  ...
  if [[ "$INSTALL_METHOD" == "git" ]]; then
    install_supportClaw_from_git "$GIT_DIR"
  ...
  if [[ -n "$installed_version" ]]; then
    ...
    log "SupportClaw installed (${installed_version})."
  else
    emit_json "{\"event\":\"done\",\"ok\":true}"
    log "SupportClaw installed."
  fi

  if [[ "$RUN_ONBOARD" -eq 1 ]]; then
    "${PREFIX}/bin/supportClaw" onboard
  fi
}

if [[ "${SUPPORT_CLAW_INSTALL_CLI_SH_NO_RUN:-0}" != "1" ]]; then
  main "$@"
fi
