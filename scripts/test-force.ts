...
// Test Force script supports SupportClaw repository automation.
...
function usage(): string {
  return [
    ...
    "",
    "Clears the configured SupportClaw gateway port, then runs the local test suite.",
    "",
    "Options:",
...
function runTests() {
  const isolatedLock =
    process.env.SUPPORT_CLAW_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `supportClaw-gateway.lock.test.${Date.now()}`);
  const result = spawnSync(process.execPath, ["scripts/test-projects.mjs"], {
    stdio: "inherit",
    env: {
      ...process.env,
      SUPPORT_CLAW_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
