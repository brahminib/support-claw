// Check No Monolithic Plugin Sdk Entry Imports script supports SupportClaw repository automation.
import fs from "node:fs";
import path from "node:path";
import { discoverSupportClawPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']supportClaw\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']supportClaw\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']supportClaw\/plugin-sdk\/channel-runtime["']/,
    label: "supportClaw/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']supportClaw\/plugin-sdk\/config-runtime["']/,
    label: "supportClaw/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']supportClaw\/plugin-sdk\/infra-runtime["']/,
    label: "supportClaw/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic supportClaw/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy supportClaw/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused supportClaw/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
