#!/usr/bin/env node
// Builds the SupportClaw package artifact used by Docker E2E.
...
function resolvePackedSupportClawFileName(value) {
  const filename = value.trim();
  if (
    !filename.endsWith(".tgz") ||
    (!filename.startsWith("supportClaw-") &&
      !filename.includes(":") &&
      !filename.includes("/") &&
      !filename.includes("\\"))
  ) {
  ...
  if (
    ...
    throw new Error(`npm pack reported unsafe SupportClaw tarball filename: ${filename}`);
  }
  return filename;
}
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building SupportClaw package artifacts",
    command: "node",
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      ...
      timeoutMs: resolveTimeoutMs(
        "SUPPORT_CLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestSupportClawTarball(outputDir, packOutput) {
  let fromOutput = "";
  for (const line of packOutput.split(/\r?\n/u)) {
    const filename = resolvePackedSupportClawFileName(line);
    if (filename) {
      fromOutput = filename;
    }
  }
  ...
  const packed = entries
    .filter((entry) => {
      try {
        return resolvePackedSupportClawFileName(entry) === entry;
      } catch {
        return false;
      }
    })
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed SupportClaw tarball in ${outputDir}`);
  }
...
export async function packSupportClawPackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  console.error("==> Packing SupportClaw package");
  ...
  try {
    ...
    packOutput = await runCaptureImpl(
      ...
      sourceDir,
      {
        ...
        timeoutMs: resolveTimeoutMs(
          "SUPPORT_CLAW_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
  ...
  return await newestSupportClawTarball(outputDir, packOutput);
}
...
async function main() {
  ...
  console.error("==> Writing SupportClaw package inventory");
  await run(
    "node",
    [
      "--import",
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "SUPPORT_CLAW_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
        DEFAULT_PACKAGE_INVENTORY_TIMEOUT_MS,
      ),
    },
  );

  let tarball = await packSupportClawPackageForDocker(sourceDir, outputDir);
  ...
  console.error("==> Checking SupportClaw package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-supportClaw-package-tarball.mjs"), tarball],
    sourceDir,
    {
      timeoutMs: resolveTimeoutMs(
        "SUPPORT_CLAW_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
