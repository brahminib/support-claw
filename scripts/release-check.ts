...
// Release Check script supports SupportClaw repository automation.
...
export const CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS = [
  "supportClaw/plugin-sdk/core",
  "supportClaw/plugin-sdk/provider-entry",
  "supportClaw/plugin-sdk/runtime",
] as const;
export const CRITICAL_PLUGIN_SDK_IMPORT_SMOKE_SPECIFIERS = ["supportClaw/plugin-sdk/core"] as const;
...
export function runReleaseCheckCommand(
  ...
  const output = execFileSync(invocation.command, invocation.args, {
    ...
    maxBuffer:
      options.maxBuffer ??
      positiveEnvInt(
        "SUPPORT_CLAW_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES",
    ...
    timeout:
      options.timeoutMs ??
      positiveEnvInt(
        "SUPPORT_CLAW_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
...
export function collectPackedInstalledPackageVerificationErrors(params: {
  ...
  if (
    ...
    errors.push(
      `installed supportClaw binary version mismatch: expected ${params.expectedVersion}, found ${params.installedBinaryVersion || "<missing>"}.`,
    );
  }
...
export function createPackedPluginSdkTypescriptSmokeProject(params: {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        name: "supportClaw-plugin-sdk-type-smoke",
        private: true,
        type: "module",
        dependencies: {
          supportClaw: params.packageSpec,
        },
      },
      null,
      2,
...
function runPackedPluginSdkTypescriptSmoke(tarballPath: string, tmpRoot: string): void {
  ...
  const installedSupportClawRoot = join(consumerDir, "node_modules", "supportClaw");
  const tscPath = [
    join(consumerDir, "node_modules", "typescript", "bin", "tsc"),
    join(installedSupportClawRoot, "node_modules", "typescript", "bin", "tsc"),
...
export function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".supportClaw", "supportClaw.json");
  mkdirSync(join(homeDir, ".supportClaw"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
        ...
        models: {
          providers: {
            openai: {
              apiKey: "sk-supportClaw-release-check",
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-supportClaw-release-check",
  });
  ...
  runReleaseCheckCommand(
    {
      command: process.execPath,
      args: [join(packageRoot, "supportClawCli.mjs"), ...PACKED_BUNDLED_RUNTIME_DEPS_REPAIR_ARGS],
    },
    {
      cwd: packageRoot,
      stdio: "inherit",
  ...
  runReleaseCheckCommand(
    { command: process.execPath, args: [join(packageRoot, "supportClawCli.mjs"), "plugins", "doctor"] },
    {
      cwd: packageRoot,
      stdio: "inherit",
      env,
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "supportClaw-release-pack-smoke-"));
  try {
    ...
    const packageRoot = join(resolveGlobalRoot(prefixDir, tmpRoot), "supportClaw");
    ...
    runReleaseCheckCommand(
      {
        command: process.execPath,
        args: [join(packageRoot, "supportClawCli.mjs"), ...PACKED_COMPLETION_SMOKE_ARGS],
      },
      {
        cwd: packageRoot,
        stdio: "inherit",
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.supportClaw-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.supportClaw-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from supportClaw/plugin-sdk.
...
export function collectCriticalPluginSdkEntrypointSizeErrors(rootDir = process.cwd()): string[] {
  ...
  for (const specifier of CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS) {
    const subpath = specifier.slice("supportClaw/plugin-sdk/".length);
