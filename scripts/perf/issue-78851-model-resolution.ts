// Issue 78851 Model Resolution script supports SupportClaw repository automation.
...
function printUsage(): void {
  process.stdout.write(`SupportClaw issue #78851 model-resolution profiler
...
function buildConfig(options: Options, workspaceDir: string): SupportClawConfig {
  const providers: NonNullable<NonNullable<SupportClawConfig["models"]>["providers"]> = {};
...
async function measurePhase(params: {
  ...
  const ensureResult = await ensureSupportClawModelsJson(params.config, params.agentDir, {
...
async function main(): Promise<void> {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "supportClaw-issue-78851-"));
