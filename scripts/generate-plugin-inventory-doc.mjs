...
const MANUAL_SECTION_START = "<!-- supportClaw-plugin-reference:manual-start -->";
const MANUAL_SECTION_END = "<!-- supportClaw-plugin-reference:manual-end -->";
...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving SupportClaw messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to SupportClaw.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an SupportClaw plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.supportClaw?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    const release = packageJson.supportClaw?.release;
    if (release?.publishToClawHub === true || release?.publishToNpm === true) {
      return `included in SupportClaw; ${resolveInstallRoute(packageJson, "external")}`;
    }
    return "included in SupportClaw";
  }
  const install = packageJson.supportClaw?.install;
  const release = packageJson.supportClaw?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.supportClaw?.install?.clawhubSpec === "string" ||
    typeof packageJson.supportClaw?.install?.npmSpec === "string";
...
function renderReferenceIndex(records) {
  ...
  return `---
summary: "Generated index of SupportClaw plugin reference pages"
read_when:
  - You need a reference page for a specific SupportClaw plugin
...
\`supportClaw.plugin.json\`. Regenerate it with:
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "supportClaw.plugin.json");
