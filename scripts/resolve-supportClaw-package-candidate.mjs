...
const DEFAULT_OUTPUT_NAME = "supportClaw-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "SUPPORT_CLAW_TRUSTED_PACKAGE_TOKEN";
...
export const SUPPORT_CLAW_PACKAGE_SPEC_RE =
  /^supportClaw@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-supportClaw-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
function resolvePackedSupportClawTarballFilename(value) {
  const filename = typeof value === "string" ? value.trim() : "";
  if (
    !/^supportClaw-[A-Za-z0-9._-]+\.tgz$/u.test(filename) ||
    filename.includes("\0") ||
    filename !== path.basename(filename) ||
    filename !== path.win32.basename(filename)
  ) {
    throw new Error(
      `npm pack reported unsafe SupportClaw tarball filename: ${JSON.stringify(filename)}`,
    );
  }
  return filename;
}

export function validateSupportClawPackageSpec(spec) {
  if (!SUPPORT_CLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be supportClaw@alpha, supportClaw@beta, supportClaw@latest, or an exact SupportClaw release version; got: ${spec}`,
    );
  }
}

export function resolveNpmPackageCandidatePackRunner(packageSpec, outputDir, params = {}) {
  validateSupportClawPackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an SupportClaw branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `supportClaw-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (Array.isArray(parsed)) {
    ...
    if (packedFilename) {
      filename = resolvePackedSupportClawTarballFilename(packedFilename);
    }
  }
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (
        ...
        (trimmed.startsWith("supportClaw-") ||
          trimmed.includes(":") ||
          trimmed.includes("/") ||
          trimmed.includes("\\"))
      ) {
        filename = resolvePackedSupportClawTarballFilename(trimmed);
      }
    }
  }
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => {
        try {
          return resolvePackedSupportClawTarballFilename(entry) === entry;
        } catch {
          return false;
        }
      })
  ...
  if (!filename) {
    throw new Error(`npm pack produced no SupportClaw tarball in ${outputDir}`);
  }
...
async function cleanPackedSupportClawTarballs(outputDir) {
  let entries;
  try {
...
export const cleanPackedSupportClawTarballsForTest = cleanPackedSupportClawTarballs;
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await cleanPackedSupportClawTarballs(outputDir);
  ...
  console.error(`Checking SupportClaw package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-supportClaw-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `SupportClaw package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "supportClaw") {
    throw new Error(`package candidate must be named "supportClaw"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
