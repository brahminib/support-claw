...
const MAX_RSS_MARKER = "__SUPPORT_CLAW_MAX_RSS_KB__=";
const DEFAULT_COMMAND_TIMEOUT_MS = 60_000;
const COMMAND_TIMEOUT_MS = readPositiveIntEnv(
  "SUPPORT_CLAW_STARTUP_MEMORY_TIMEOUT_MS",
...
function parseArgs(argv) {
  const options = {
    jsonPath:
      readNonEmptyEnv("SUPPORT_CLAW_STARTUP_MEMORY_JSON_PATH") ??
      path.join(repoRoot, ".artifacts", "startup-memory", "startup-memory.json"),
    summaryPath:
      readNonEmptyEnv("SUPPORT_CLAW_STARTUP_MEMORY_SUMMARY_PATH") ??
...
const cases = [
  {
    id: "help",
    label: "--help",
    args: ["supportClawCli.mjs", "--help"],
    limitMb: readPositiveNumberEnv("SUPPORT_CLAW_STARTUP_MEMORY_HELP_MB", DEFAULT_LIMITS_MB.help),
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["supportClawCli.mjs", "status", "--json"],
    limitMb: readPositiveNumberEnv(
      "SUPPORT_CLAW_STARTUP_MEMORY_STATUS_JSON_MB",
  ...
  },
  {
    ...
    args: ["supportClawCli.mjs", "gateway", "status"],
    limitMb: readPositiveNumberEnv(
      "SUPPORT_CLAW_STARTUP_MEMORY_GATEWAY_STATUS_MB",
...
function formatFixGuidance(testCase, details) {
  ...
  const guidance = [
    ...
    "LLM prompt:",
    `"SupportClaw startup-memory CI failed for '${testCase.label}'. Analyze this failure, identify the first runtime/import side effect that makes startup heavier or broken, and propose the smallest safe patch. Failure output:\n${details}"`,
  ];
...
function runStartupMemoryCheck(argv = process.argv.slice(2), params = {}) {
  ...
  tmpHome = mkdtempSync(path.join(os.tmpdir(), "supportClaw-startup-memory-"));
