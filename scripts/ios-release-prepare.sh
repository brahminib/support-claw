...
Optional custom relay:
  SUPPORT_CLAW_PUSH_RELAY_BASE_URL=https://relay.example.com \
    scripts/ios-release-prepare.sh --build-number 7 [--team-id TEAMID]
...
- regenerates apps/ios/SupportClaw.xcodeproj via xcodegen
EOF
}
...
DEFAULT_IOS_PUSH_RELAY_BASE_URL="https://ios-push-relay.supportClaw.ai"
PUSH_RELAY_BASE_URL="${SUPPORT_CLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-${DEFAULT_IOS_PUSH_RELAY_BASE_URL}}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid SUPPORT_CLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid SUPPORT_CLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid SUPPORT_CLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid SUPPORT_CLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ "${TEAM_ID}" != "${CANONICAL_TEAM_ID}" ]]; then
  echo "iOS App Store release must use canonical SupportClaw Team ID ${CANONICAL_TEAM_ID}; got ${TEAM_ID}." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(SUPPORT_CLAW_URL_SLASH)$(SUPPORT_CLAW_URL_SLASH)#g'
)"
...
${RELEASE_SIGNING_XCCONFIG}
SUPPORT_CLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
SUPPORT_CLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
SUPPORT_CLAW_APP_BUNDLE_ID = ai.supportClawfoundation.app
SUPPORT_CLAW_SHARE_BUNDLE_ID = ai.supportClawfoundation.app.share
SUPPORT_CLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.supportClawfoundation.app.activitywidget
SUPPORT_CLAW_WATCH_APP_BUNDLE_ID = ai.supportClawfoundation.app.watchkitapp
SUPPORT_CLAW_APNS_ENTITLEMENT_ENVIRONMENT = production
SUPPORT_CLAW_PUSH_TRANSPORT = relay
SUPPORT_CLAW_PUSH_DISTRIBUTION = official
SUPPORT_CLAW_URL_SLASH = /
SUPPORT_CLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
SUPPORT_CLAW_PUSH_APNS_ENVIRONMENT = production
EOF
