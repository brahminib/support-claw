// Debug Claude Usage script supports SupportClaw repository automation.
...
Options:
  --agent <id>          SupportClaw agent id to inspect (default: main)
...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.SUPPORT_CLAW_STATE_DIR?.trim() || path.join(os.homedir(), ".supportClaw");
...
const resolveFetchTimeoutMs = (raw = process.env.SUPPORT_CLAW_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "SUPPORT_CLAW_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};
