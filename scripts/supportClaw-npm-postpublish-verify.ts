...
// SupportClaw Npm Postpublish Verify script supports SupportClaw repository automation.
...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./supportClaw-npm-release-check.ts";
...
export type SupportClawNpmPostpublishVerifyArgs =
  | {
      help: false;
      version: string;
    }
...
export function supportClawNpmPostpublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/supportClaw-npm-postpublish-verify.ts <version>";
}

export function parseSupportClawNpmPostpublishVerifyArgs(
  argv: readonly string[],
): SupportClawNpmPostpublishVerifyArgs {
  ...
  if (!version) {
    throw new Error(supportClawNpmPostpublishVerifyUsage());
  }
  if (version.startsWith("-")) {
    throw new Error(`Unknown supportClaw npm postpublish verifier option: ${version}`);
  }
  const extraArg = args[1]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected supportClaw npm postpublish verifier argument: ${extraArg}`);
  }
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  const exactSpec = `supportClaw@${version}`;
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`supportClaw@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
const NPM_PROVENANCE_REPOSITORY = "https://github.com/brahminib/support-claw";
const NPM_PROVENANCE_WORKFLOW_PATH = ".github/workflows/supportClaw-npm-release.yml";
...
function resolveNpmProvenanceVerificationPolicy(
  ...
  if (
    ...
    throw new Error(
      `npm provenance attestation does not bind ${version} to the trusted SupportClaw GitHub release workflow.`,
    );
  }
...
export function collectInstalledPluginSdkDeclarationErrors(packageRoot: string): string[] {
  ...
  const errors: string[] = [];
  const forbiddenPrivateWorkspaceSpecifiers = ["@supportclaw/llm-core"];
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32"
    ? pathWin32.join(prefixDir, "supportClaw.cmd")
    : pathPosix.join(prefixDir, "bin", "supportClaw");
}
...
async function verifyPublishedRegistryProvenanceOnce(version: string): Promise<void> {
  ...
  console.log(
    `supportClaw-npm-postpublish-verify: registry signature and provenance attestation verified (${version})`,
  );
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `supportClaw-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed supportClaw binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`supportClaw-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
async function main(argv = process.argv.slice(2)): Promise<void> {
  const args = parseSupportClawNpmPostpublishVerifyArgs(argv);
  if (args.help) {
    console.log(supportClawNpmPostpublishVerifyUsage());
    return;
  }
  ...
  console.log(
    `supportClaw-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`supportClaw-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
