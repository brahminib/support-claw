...
# Build and bundle SupportClaw into a minimal .app we can open.
# Outputs to dist/SupportClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT_DIR/scripts/lib/plistbuddy.sh"
APP_ROOT="$ROOT_DIR/dist/SupportClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="SupportClaw"
MLX_TTS_HELPER_PRODUCT="supportClaw-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.supportClaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/supportClaw/supportClaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/SupportClaw/Resources/Info.plist"
...
plist_set_string_required "$APP_ROOT/Contents/Info.plist" SupportClawBuildTimestamp "$BUILD_TS"
plist_set_string_required "$APP_ROOT/Contents/Info.plist" SupportClawGitCommit "$GIT_COMMIT"
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/SupportClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/SupportClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/SupportClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/SupportClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/SupportClaw/Resources/SupportClaw.icns" "$APP_ROOT/Contents/Resources/SupportClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/SupportClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SupportClawKit resources"
SUPPORT_CLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SupportClawKit_SupportClawKit.bundle"
if [ -d "$SUPPORT_CLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SupportClawKit_SupportClawKit.bundle"
  cp -R "$SUPPORT_CLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SupportClawKit_SupportClawKit.bundle"
else
  echo "ERROR: SupportClawKit resource bundle not found at $SUPPORT_CLAWKIT_BUNDLE" >&2
  exit 1
fi
...
running_packaged_app_pids() {
  ...
  local app_binary="$APP_ROOT/Contents/MacOS/SupportClaw"
  local pid
...
stop_packaged_app_if_running() {
  ...
  echo "⏹  Stopping packaged SupportClaw bundle (${pids[*]})"
  ...
  echo "ERROR: Packaged SupportClaw bundle did not exit: ${pids[*]}" >&2
  return 1
}
