...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.SUPPORT_CLAW_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "supportClaw.plugin.json");
    ...
    if (packageJson.supportClaw && "extensions" in packageJson.supportClaw) {
      packageJson.supportClaw = {
        ...packageJson.supportClaw,
        extensions: rewritePackageExtensions(packageJson.supportClaw.extensions),
        ...(typeof packageJson.supportClaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.supportClaw.setupEntry) }
          : {}),
      };
    }
