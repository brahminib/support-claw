...
process.env.SUPPORT_CLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "SUPPORT_CLAW_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
const installedLayoutEnv = "SUPPORT_CLAW_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";
...
function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/supportClaw");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "supportClaw");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    const packageJson = readJson(packageJsonPath);
    if (!packageJson.supportClaw?.channel) {
      continue;
    }
    ...
    const extensionEntries =
      Array.isArray(packageJson.supportClaw.extensions) && packageJson.supportClaw.extensions.length > 0
    ...
    const setupEntry = packageJson.supportClaw.setupEntry;
