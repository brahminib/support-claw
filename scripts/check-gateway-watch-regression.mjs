...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  SUPPORT_CLAW_RUNTIME_POSTBUILD_STATIC_ASSETS: "0",
...
function formatCapturedWatchLog(text, truncated) {
  return truncated
    ? `[supportClaw] log truncated to last ${WATCH_LOG_CAPTURE_MAX_CHARS} chars\n${text}`
    : text;
}
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const isolatedStateDir = path.join(isolatedHomeDir, ".supportClaw");
  const isolatedConfigPath = path.join(isolatedStateDir, "supportClaw.json");
  const shellSource = [
    'echo "$$" > "$SUPPORT_CLAW_WATCH_PID_FILE"',
    'mkdir -p "$SUPPORT_CLAW_STATE_DIR"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$SUPPORT_CLAW_CONFIG_PATH"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    SUPPORT_CLAW_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    SUPPORT_CLAW_HOME: isolatedHomeDir,
    SUPPORT_CLAW_CONFIG_PATH: isolatedConfigPath,
    SUPPORT_CLAW_STATE_DIR: isolatedStateDir,
...
export async function runTimedWatch(options, outputDir, deps = {}) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-gateway-watch-"));
