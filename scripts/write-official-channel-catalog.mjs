...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.supportClaw) ? packageJson.supportClaw : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.supportClaw?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.supportClaw?.channel?.id) || left.name;
    const rightId = trimString(right.supportClaw?.channel?.id) || right.name;
