...
# SupportClaw Auth Widget for Termux
...
# Check auth status
termux-toast "Checking SupportClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/supportClaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/supportClaw/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|SUPPORT_CLAW_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/supportClaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|SUPPORT_CLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: SupportClaw won't work until re-auth"
                ;;
        esac
        ;;
