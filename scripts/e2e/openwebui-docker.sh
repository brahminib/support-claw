#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized SupportClaw Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "supportClaw-openwebui-e2e" SUPPORT_CLAW_OPENWEBUI_E2E_IMAGE)"
OPENWEBUI_IMAGE="${OPENWEBUI_IMAGE:-ghcr.io/open-webui/open-webui:v0.8.10}"
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env SUPPORT_CLAW_OPENWEBUI_MAX_MEMORY_MIB 8192)"
MAX_CPU_PERCENT="$(docker_e2e_read_nonnegative_decimal_env SUPPORT_CLAW_OPENWEBUI_MAX_CPU_PERCENT 1600)"
...
MODEL="${SUPPORT_CLAW_OPENWEBUI_MODEL:-openai/gpt-5.5}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${SUPPORT_CLAW_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="$(docker_e2e_read_tcp_port_env SUPPORT_CLAW_OPENWEBUI_GATEWAY_PORT 18789)"
WEBUI_PORT="$(docker_e2e_read_tcp_port_env SUPPORT_CLAW_OPENWEBUI_PORT 8080)"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${SUPPORT_CLAW_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${SUPPORT_CLAW_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="supportClaw-openwebui-e2e-$$"
GW_NAME="supportClaw-openwebui-gateway-$$"
OW_NAME="supportClaw-openwebui-$$"
PROVIDER_TIMEOUT_SECONDS="${SUPPORT_CLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS:-900}"
DOCKER_PULL_TIMEOUT="${SUPPORT_CLAW_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
PROBE_FETCH_TIMEOUT_MS="${SUPPORT_CLAW_OPENWEBUI_FETCH_TIMEOUT_MS:-$((PROVIDER_TIMEOUT_SECONDS_DECIMAL * 1000 + 60000))}"
validate_positive_int SUPPORT_CLAW_OPENWEBUI_FETCH_TIMEOUT_MS "$PROBE_FETCH_TIMEOUT_MS"
DOCKER_COMMAND_TIMEOUT="${SUPPORT_CLAW_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-$((PROVIDER_TIMEOUT_SECONDS_DECIMAL + 90))s}"
...
PROFILE_FILE="${SUPPORT_CLAW_TESTBOX_PROFILE_FILE:-$HOME/.supportClaw-testbox-live.profile}"
...
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/supportClaw-openwebui-stats.XXXXXX")"
PROBE_LOG="$(mktemp "${TMPDIR:-/tmp}/supportClaw-openwebui-probe.XXXXXX")"
STATS_STOP_FILE="$(mktemp "${TMPDIR:-/tmp}/supportClaw-openwebui-stats-stop.XXXXXX")"
STATS_PIDS=()
...
docker_e2e_docker_cmd run -d \
  ...
  -e "SUPPORT_CLAW_OPENWEBUI_MODEL=$MODEL" \
  ...
  -e "SUPPORT_CLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=$PROVIDER_TIMEOUT_SECONDS" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/supportClaw-openwebui-config.XXXXXX.json)"
    SUPPORT_CLAW_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
    node "$entry" config set --batch-file "$batch_file" >/dev/null
    rm -f "$batch_file"
    node scripts/e2e/lib/fixture.mjs openwebui-workspace

    supportClaw_e2e_exec_gateway "$entry" '"$PORT"' lan /tmp/openwebui-gateway.log
  ' >/dev/null
start_openwebui_stats_sampler

echo "Waiting for gateway HTTP surface..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "SUPPORT_CLAW_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="SupportClaw E2E" \
  -e WEBUI_SECRET_KEY="supportClaw-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="SupportClaw E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="supportClaw/default" \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "SUPPORT_CLAW_HTTP_PROBE_BEARER='$TOKEN' SUPPORT_CLAW_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> SupportClaw smoke..."
set +e
