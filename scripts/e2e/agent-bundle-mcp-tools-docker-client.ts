// SupportClaw bundle MCP tools Docker harness.
...
server.tool("docker_probe", "Docker SupportClaw MCP tool availability probe", async () => ({
...
async function main() {
  const tempState = await createE2eStateDir("supportClaw-agent-bundle-mcp-");
