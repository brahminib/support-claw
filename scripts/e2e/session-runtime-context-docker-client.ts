...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".supportClaw", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(
    runtimeContextMessage.customType === "supportClaw.runtime-context",
  ...
  assert(
    !userText.includes("SUPPORT_CLAW_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-session-runtime-context-"));
  process.env.HOME = root;
  process.env.SUPPORT_CLAW_STATE_DIR = path.join(root, ".supportClaw");
  process.env.SUPPORT_CLAW_CONFIG_PATH = path.join(process.env.SUPPORT_CLAW_STATE_DIR, "supportClaw.json");
  try {
    ...
    if (process.env.SUPPORT_CLAW_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
