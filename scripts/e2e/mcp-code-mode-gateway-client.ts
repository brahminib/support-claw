// Mcp Code Mode Gateway Client script supports SupportClaw repository automation.
...
export function readMcpCodeModeClientFetchLimits(
  ...
  return {
    ...
    timeoutMs: readPositiveIntEnv("SUPPORT_CLAW_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", 300_000, env),
  };
}
...
async function main() {
  ...
  const model = process.env.SUPPORT_CLAW_MCP_CODE_MODE_MODEL?.trim() || "supportClaw/main";
  assert(gatewayUrl, "missing GW_URL");
  assert(gatewayToken, "missing GW_TOKEN");
  assert(stateDir, "missing SUPPORT_CLAW_STATE_DIR");
