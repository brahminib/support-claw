SUPPORT_CLAW_PLUGINS_FIXTURE_PID_FILES=()
SUPPORT_CLAW_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=0
SUPPORT_CLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=""

supportClaw_plugins_read_positive_int_env() {
...
supportClaw_plugins_read_nonnegative_decimal_env() {
  local name="${1:?missing environment variable name}"
  local fallback="${2:?missing fallback value}"
  local value="${!name-}"
  if [[ -z "${!name+x}" ]]; then
...
supportClaw_plugins_cleanup_fixture_servers() {
  local pid_file
  local pid
  for pid_file in "${SUPPORT_CLAW_PLUGINS_FIXTURE_PID_FILES[@]:-}"; do
    [[ -f "$pid_file" ]] || continue
    pid="$(cat "$pid_file" 2>/dev/null || true)"
    if [[ "$pid" =~ ^[0-9]+$ ]]; then
      supportClaw_plugins_stop_fixture_process "$pid"
    fi
    rm -f "$pid_file"
  done
}

supportClaw_plugins_signal_fixture_process() {
  local pid="$1"
  local signal="$2"
...
supportClaw_plugins_fixture_process_alive() {
  local pid="$1"
  kill -0 "$pid" >/dev/null 2>&1 || kill -0 -- "-$pid" >/dev/null 2>&1
}

supportClaw_plugins_stop_fixture_process() {
  local pid="$1"
  local _
  local attempts interval
  attempts="$(supportClaw_plugins_read_positive_int_env SUPPORT_CLAW_PLUGINS_FIXTURE_STOP_ATTEMPTS 40)" || return $?
  interval="$(supportClaw_plugins_read_nonnegative_decimal_env SUPPORT_CLAW_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS 0.25)" || return $?
  if declare -F supportClaw_e2e_stop_process >/dev/null 2>&1; then
    supportClaw_e2e_stop_process "$pid"
    return
  fi
  supportClaw_plugins_signal_fixture_process "$pid" TERM
  for _ in $(seq 1 "$attempts"); do
    ! supportClaw_plugins_fixture_process_alive "$pid" && { wait "$pid" >/dev/null 2>&1 || true; return; }
    sleep "$interval"
  done
  supportClaw_plugins_signal_fixture_process "$pid" KILL
  wait "$pid" >/dev/null 2>&1 || true
}

supportClaw_plugins_print_fixture_log() {
  local log_file="$1"
  ...
  local max_bytes
  max_bytes="$(supportClaw_plugins_read_positive_int_env SUPPORT_CLAW_DOCKER_E2E_LOG_PRINT_BYTES 65536)" || return $?
...
supportClaw_plugins_validate_fixture_log_print_bytes() {
  supportClaw_plugins_read_positive_int_env SUPPORT_CLAW_DOCKER_E2E_LOG_PRINT_BYTES 65536 >/dev/null
}

supportClaw_plugins_register_fixture_pid_file() {
  local pid_file="$1"
  SUPPORT_CLAW_PLUGINS_FIXTURE_PID_FILES+=("$pid_file")
  supportClaw_plugins_install_fixture_cleanup_trap
}

supportClaw_plugins_install_fixture_cleanup_trap() {
  if [[ "${SUPPORT_CLAW_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED:-0}" = "1" ]]; then
    return
  fi

  local existing_trap
  existing_trap="$(trap -p EXIT || true)"
  if [[ -n "$existing_trap" && "$existing_trap" != *supportClaw_plugins_fixture_exit_trap* ]]; then
    local existing_action="${existing_trap#trap -- }"
    existing_action="${existing_action% EXIT}"
    eval "SUPPORT_CLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=$existing_action"
  fi

  SUPPORT_CLAW_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=1
  trap supportClaw_plugins_fixture_exit_trap EXIT
}

supportClaw_plugins_fixture_exit_trap() {
  local status="$?"
  supportClaw_plugins_cleanup_fixture_servers
  if [[ -n "${SUPPORT_CLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION:-}" ]]; then
    eval "$SUPPORT_CLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION"
  fi
  exit "$status"
}
...
packageJson.supportClaw.extensions = ["./index.js", " "];
