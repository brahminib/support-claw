...
const scratchRoot = process.env.SUPPORT_CLAW_PLUGINS_TMP_DIR || os.tmpdir();
...
function readClawHubPreflightLimits() {
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES",
      1024 * 1024,
    ),
    timeoutMs: readPositiveIntEnv("SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS", 30_000),
  };
}
...
function getInstallRecords() {
  const configPath = supportClawConfigPath();
  const config = readSupportClawConfig();
  const allowLegacyCompat = process.env.SUPPORT_CLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function supportClawConfigPath() {
  return path.join(process.env.HOME, ".supportClaw", "supportClaw.json");
}

function readSupportClawConfig() {
  const configPath = supportClawConfigPath();
  return fs.existsSync(configPath) ? readRequiredSupportClawConfig() : {};
}

function readRequiredSupportClawConfig() {
  const configPath = supportClawConfigPath();
  try {
    return readJson(configPath);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`failed to read SupportClaw config ${configPath}: ${message}`, { cause: error });
  }
}
...
function recordFixturePluginTrust() {
  ...
  const configPath = path.join(process.env.HOME, ".supportClaw", "supportClaw.json");
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".supportClaw", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  const supportClawPeerPath = path.join(installPath, "node_modules", "supportClaw");
  if (!fs.existsSync(supportClawPeerPath)) {
    throw new Error(`missing ClawHub supportClaw peer symlink: ${supportClawPeerPath}`);
  }
  if (!fs.lstatSync(supportClawPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub supportClaw peer is not a symlink: ${supportClawPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(supportClawPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub supportClaw peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@supportclaw/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@supportclaw/demo-plugin-npm") {
...
function assertInvalidSupportClawExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  for (const expected of ["supportClaw.extensions[1]", "non-empty string"]) {
    assertTextFileIncludes(
      scratchFile("plugins-invalid-supportClaw-extensions.log"),
      expected,
      "malformed metadata install output",
    );
  }

  const list = readJson(scratchFile("plugins-invalid-supportClaw-extensions-list.json"));
  ...
  const managedInstallPath = path.join(process.env.HOME, ".supportClaw", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.SUPPORT_CLAW_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.SUPPORT_CLAW_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const extensionsRoot = path.join(process.env.HOME, ".supportClaw", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".supportClaw", "supportClaw.json");
...
const commands = {
  ...
  "invalid-supportClaw-extensions": assertInvalidSupportClawExtensionsRejected,
