...
const agentTurnTimeoutSeconds = readPositiveIntEnv(
  "SUPPORT_CLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS",
  300,
);
...
const AGENT_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "SUPPORT_CLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
const SESSION_FILE_LIST_LIMIT = 20;
const SESSION_SCAN_MAX_ENTRIES = readPositiveIntEnv(
  "SUPPORT_CLAW_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES",
  50_000,
);
...
function stateDir() {
  return process.env.SUPPORT_CLAW_STATE_DIR || path.join(process.env.HOME, ".supportClaw");
}

function configPath() {
  return process.env.SUPPORT_CLAW_CONFIG_PATH || path.join(stateDir(), "supportClaw.json");
}

function agentOutputPath() {
  return process.env.SUPPORT_CLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_PATH || "/tmp/supportClaw-agent.json";
}

function agentErrorPath() {
  return process.env.SUPPORT_CLAW_LIVE_PLUGIN_TOOL_AGENT_ERROR_PATH || "/tmp/supportClaw-agent.err";
}
...
function pluginInstallPath() {
  ...
  const inspect = fs.existsSync("/tmp/supportClaw-plugin-inspect.json")
    ? readJson("/tmp/supportClaw-plugin-inspect.json")
    : {};
...
function writeFixture() {
  ...
  writeJson(path.join(dir, "supportClaw.plugin.json"), {
    id: pluginId,
    name: "E2E Slug Tool",
...
function assertInstalled() {
  ...
  const list = readJson("/tmp/supportClaw-plugins-list.json");
  ...
  const inspect = readJson("/tmp/supportClaw-plugin-inspect.json");
