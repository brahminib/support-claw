...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/supportClaw-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/supportClaw-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${SUPPORT_CLAW_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${SUPPORT_CLAW_SECRET_ENV_NAME:-}" ] && [ -n "${SUPPORT_CLAW_SECRET_FILE:-}" ] && [ -f "$SUPPORT_CLAW_SECRET_FILE" ]; then
  secret_value="$(cat "$SUPPORT_CLAW_SECRET_FILE")"
  rm -f "$SUPPORT_CLAW_SECRET_FILE"
  export "${SUPPORT_CLAW_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "SUPPORT_CLAW_SECRET_ENV_NAME=$api_key_env" \
      "SUPPORT_CLAW_SECRET_FILE=$secret_path" \
      "SUPPORT_CLAW_WRAPPER_FILE=$wrapper_path" \
