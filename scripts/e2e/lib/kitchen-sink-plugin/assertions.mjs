...
function shouldScanLogFile(entry) {
  if (!(/\.(?:log|jsonl)$/u.test(entry) || /supportClaw-kitchen-sink-/u.test(path.basename(entry)))) {
    return false;
  }
...
function scanLogs() {
  ...
  const roots = [scratchRoot, path.join(process.env.HOME, ".supportClaw")];
  const deny = [
  ...
  if (scannedFiles === 0) {
    throw new Error(
      "kitchen-sink log scan found no files under the isolated scratch root or SupportClaw home",
    );
  }
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(supportClawPeerPath)) {
    throw new Error(`missing kitchen-sink supportClaw peer symlink: ${supportClawPeerPath}`);
  }
  if (!fs.lstatSync(supportClawPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink supportClaw peer is not a symlink: ${supportClawPeerPath}`);
  }
  ...
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink supportClaw peer ${linkedHostRoot} to target ${hostRoot}`);
  }
