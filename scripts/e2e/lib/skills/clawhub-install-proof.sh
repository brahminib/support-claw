...
SUPPORT_CLAW_TEST_STATE_SCRIPT_B64="${SUPPORT_CLAW_TEST_STATE_SCRIPT_B64:-}"
supportClaw_skill_install_owns_home=0
cleanup_clawhub_skill_install_home() {
  if [ "$supportClaw_skill_install_owns_home" = "1" ] && [ -n "${HOME:-}" ]; then
    rm -rf "$HOME"
  fi
}
trap cleanup_clawhub_skill_install_home EXIT

if [ -n "$SUPPORT_CLAW_TEST_STATE_SCRIPT_B64" ]; then
  supportClaw_e2e_eval_test_state_from_b64 "$SUPPORT_CLAW_TEST_STATE_SCRIPT_B64"
else
  export HOME="$(mktemp -d "${TMPDIR:-/tmp}/supportClaw-skill-install-home.XXXXXX")"
  supportClaw_skill_install_owns_home=1
  export USERPROFILE="$HOME"
  export SUPPORT_CLAW_HOME="$HOME"
  export SUPPORT_CLAW_STATE_DIR="$HOME/.supportClaw"
...
if [ -n "${SUPPORT_CLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
  ...
  supportClaw_e2e_install_package /tmp/supportClaw-skill-install-npm.log
fi

if [ -n "${SUPPORT_CLAW_CURRENT_PACKAGE_TGZ:-}" ] && command -v supportClaw >/dev/null 2>&1; then
  SUPPORT_CLAW_CMD=(supportClaw)
elif command -v pnpm >/dev/null 2>&1 && [ -f package.json ]; then
  if [ "${SUPPORT_CLAW_SKILL_INSTALL_E2E_BUILD_SOURCE:-0}" = "1" ]; then
    pnpm build >/tmp/supportClaw-skill-install-build.log 2>&1
  fi
  SUPPORT_CLAW_CMD=(pnpm --silent supportClaw)
elif command -v supportClaw >/dev/null 2>&1; then
  SUPPORT_CLAW_CMD=(supportClaw)
else
  echo "supportClaw command not found; install package first or run from repo with pnpm" >&2
  exit 1
fi

mkdir -p "$(dirname "$SUPPORT_CLAW_CONFIG_PATH")"
node --input-type=module - "$SUPPORT_CLAW_CONFIG_PATH" <<'NODE'
...
query="${SUPPORT_CLAW_SKILL_INSTALL_E2E_QUERY:-homeassistant}"
requested_slug="${SUPPORT_CLAW_SKILL_INSTALL_E2E_SLUG:-}"
preferred_slug="${SUPPORT_CLAW_SKILL_INSTALL_E2E_PREFERRED_SLUG:-homeassistant-skill}"
search_json="/tmp/supportClaw-skill-install-search.json"
resolve_json="/tmp/supportClaw-skill-install-resolved.json"
install_log="/tmp/supportClaw-skill-install.log"
info_json="/tmp/supportClaw-skill-install-info.json"

echo "Searching live ClawHub skills for: $query"
"${SUPPORT_CLAW_CMD[@]}" skills search "$query" --limit 8 --json >"$search_json"
...
if ! "${SUPPORT_CLAW_CMD[@]}" skills install "$slug" --force >"$install_log" 2>&1; then
  echo "Skill install failed" >&2
  supportClaw_e2e_dump_logs /tmp/supportClaw-skill-install-npm.log "$search_json" "$resolve_json" "$install_log"
  exit 1
fi

workspace_dir="$HOME/.supportClaw/workspace"
skill_dir="$workspace_dir/skills/$slug"
origin_json="$skill_dir/.clawhub/origin.json"
lock_json="$workspace_dir/.clawhub/lock.json"

supportClaw_e2e_assert_file "$skill_dir/SKILL.md"
supportClaw_e2e_assert_file "$origin_json"
supportClaw_e2e_assert_file "$lock_json"

"${SUPPORT_CLAW_CMD[@]}" skills info "$slug" --json >"$info_json"

node --input-type=module - "$SUPPORT_CLAW_CONFIG_PATH" "$skill_dir" "$origin_json" "$lock_json" "$info_json" "$slug" <<'NODE'
import fs from "node:fs";
