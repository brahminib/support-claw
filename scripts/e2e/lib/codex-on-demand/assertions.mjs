...
const inspect = readJson("/tmp/supportClaw-codex-inspect.json");
...
if (!String(codexRecord.spec || "").includes("@supportclaw/codex")) {
  throw new Error(`expected @supportclaw/codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @supportclaw/codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@supportclaw/codex") {
  throw new Error(`unexpected codex package name: ${codexPackage.name}`);
}

const npmProjectRoot = npmProjectRootForInstalledPackage(installPath, "@supportclaw/codex");
...
function readAuthProfileStoreText(agentDir) {
  const dbPath = path.join(agentDir, "supportClaw-agent.sqlite");
...
assertOpenAiEnvAuthProfileStore(authRaw, {
  ...
  rawKeyNeedle: "sk-supportClaw-codex-on-demand-e2e",
});
