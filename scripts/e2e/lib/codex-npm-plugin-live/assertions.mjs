...
const allowBetaCompatDiagnostics =
  process.env.SUPPORT_CLAW_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS === "1";
...
const MAX_ERROR_TAIL_BYTES = readPositiveIntEnv(
  "SUPPORT_CLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES",
  64 * 1024,
);
const MAX_TRANSCRIPT_FILES = readPositiveIntEnv(
  "SUPPORT_CLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES",
  64,
);
const MAX_TRANSCRIPT_WALK_ENTRIES = readPositiveIntEnv(
  "SUPPORT_CLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES",
  4096,
);
const MAX_TRANSCRIPT_SCAN_BYTES = readPositiveIntEnv(
  "SUPPORT_CLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES",
  2 * 1024 * 1024,
);
...
function assertPlugin() {
  const spec = process.argv[3] || "npm:@supportclaw/codex";
  const list = readJson("/tmp/supportClaw-codex-plugins-list.json");
  const inspect = readJson("/tmp/supportClaw-codex-plugin-inspect.json");
...
function codexNpmProjectRoot() {
  return npmProjectRootForInstalledPackage(codexInstallPath(), "@supportclaw/codex");
}
...
function assertNpmDeps() {
  ...
  if (!fs.existsSync(pluginPackageJson)) {
    throw new Error(`missing npm-installed @supportclaw/codex package.json: ${pluginPackageJson}`);
  }
  ...
  if (pluginPackage.name !== "@supportclaw/codex") {
  ...
  if (!openAiCodexPackageJson) {
    throw new Error("missing @openai/codex dependency under .supportClaw/npm");
  }
...
function assertPreflight() {
  ...
  const output = readTextFileBounded("/tmp/supportClaw-codex-preflight.log", "Codex preflight log");
...
function assertAgentTurn() {
  ...
  const stdout = readTextFileBounded("/tmp/supportClaw-codex-agent.json", "SupportClaw agent JSON");
  const stderr = readTextFileTail("/tmp/supportClaw-codex-agent.err", "SupportClaw agent stderr");
  const response = JSON.parse(stdout);
  const text = extractAgentReplyTexts(JSON.stringify(response)).join("\n");
  if (!text.includes(marker)) {
    throw new Error(
      `SupportClaw agent reply did not contain ${marker}:\nstdout=${stdout}\nstderr=${stderr}`,
    );
  }
  ...
  if (typeof entry.sessionFile !== "string" || !fs.existsSync(entry.sessionFile)) {
    throw new Error(`missing SupportClaw session file: ${entry.sessionFile}`);
  }
...
function assertUninstalled() {
  ...
  const list = readJson("/tmp/supportClaw-codex-plugins-list-after-uninstall.json");
  const plugin = (list.plugins || []).find((entry) => entry.id === "codex");
  if (plugin?.status === "loaded" || plugin?.enabled === true) {
...
function assertAgentError() {
  ...
  if (!Number.isInteger(status) || status === 0) {
    throw new Error(
      `expected SupportClaw agent to fail after Codex uninstall, got status ${process.argv[3]}`,
    );
  }
  const stdout = fs.existsSync("/tmp/supportClaw-codex-agent-after-uninstall.json")
    ? readTextFileTail(
        "/tmp/supportClaw-codex-agent-after-uninstall.json",
        "post-uninstall agent stdout",
      )
    : "";
  const stderr = fs.existsSync("/tmp/supportClaw-codex-agent-after-uninstall.err")
    ? readTextFileTail(
