...
const AGENTS_DELETE_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "SUPPORT_CLAW_FIXTURE_AGENTS_DELETE_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.SUPPORT_CLAW_WORKSPACE_DIR || path.join(process.env.HOME, ".supportClaw", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.SUPPORT_CLAW_STATE_DIR, "SUPPORT_CLAW_STATE_DIR");
  const sharedWorkspace = requireArg(process.env.SHARED_WORKSPACE, "SHARED_WORKSPACE");
  const gatewayToken = process.env.SUPPORT_CLAW_GATEWAY_TOKEN?.trim();
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.SUPPORT_CLAW_STATE_DIR, "supportClaw.json"))?.agents?.list ?? [];
