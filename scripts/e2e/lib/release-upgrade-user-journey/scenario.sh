...
export OPENAI_API_KEY="sk-supportClaw-release-upgrade-user-journey"
export CLICKCLACK_BOT_TOKEN="clickclack-release-upgrade-token"

PORT="18789"
MOCK_PORT="44210"
CLICKCLACK_PORT="44211"
SUCCESS_MARKER="SUPPORT_CLAW_E2E_OK_RELEASE_UPGRADE"
...
candidate_version="$(
  tar -xOf "${SUPPORT_CLAW_CURRENT_PACKAGE_TGZ:?missing SUPPORT_CLAW_CURRENT_PACKAGE_TGZ}" package/package.json |
...
if [ -n "${SUPPORT_CLAW_RELEASE_UPGRADE_BASELINE_SPEC:-}" ]; then
  BASELINE_SPEC="$SUPPORT_CLAW_RELEASE_UPGRADE_BASELINE_SPEC"
else
...
cleanup() {
  ...
  supportClaw_e2e_stop_process "${clickclack_pid:-}"
...
if ! supportClaw_e2e_maybe_timeout "${SUPPORT_CLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g "$BASELINE_SPEC" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
  cat "$BASELINE_INSTALL_LOG" >&2 || true
  exit 1
fi
command -v supportClaw >/dev/null
baseline_root="$(supportClaw_e2e_package_root)"
baseline_entry="$(supportClaw_e2e_package_entrypoint "$baseline_root")"
supportClaw_e2e_enable_supportClaw_cli_timeout

mock_pid="$(supportClaw_e2e_start_mock_openai "$MOCK_PORT" "$OPENAI_LOG")"
...
for _ in $(seq 1 100); do
  if supportClaw_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200 >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
supportClaw_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200
...
supportClaw plugins install "$plugin_dir" >"$PLUGIN_INSTALL_LOG" 2>&1
supportClaw release-upgrade ping >"$PLUGIN_CLI_BEFORE_LOG" 2>&1
...
supportClaw_e2e_install_package "$CANDIDATE_INSTALL_LOG" "candidate SupportClaw package"
...
supportClaw release-upgrade ping >"$PLUGIN_CLI_AFTER_LOG" 2>&1
...
supportClaw plugins install "$clickclack_plugin_dir" >"$CLICKCLACK_PLUGIN_INSTALL_LOG" 2>&1

supportClaw channels status --json >"$STATUS_JSON" 2>"$STATUS_ERR"
node scripts/e2e/lib/release-user-journey/assertions.mjs assert-channel-status clickclack "$STATUS_JSON"
supportClaw message send \
  --channel clickclack \
