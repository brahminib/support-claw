    ...
    [
      {
        id: "logging-file",
        intent: "logging",
        argv: ["config", "set", "logging.file", "~/supportClaw-upgrade-survivor/gateway.jsonl"],
      },
    ],
  ],
  [
...
function selectedScenario() {
  return process.env.SUPPORT_CLAW_UPGRADE_SURVIVOR_SCENARIO || "base";
}
...
export function resolveUpgradeSurvivorSupportClawCommand(argv, params = {}) {
  const platform = params.platform ?? process.platform;
  if (platform === "win32") {
    const comSpec = params.comSpec ?? resolveWindowsCmdExePath(params.env ?? process.env);
    return {
      command: comSpec,
      args: ["/d", "/s", "/c", buildCmdExeCommandLine("supportClaw.cmd", argv)],
      commandLabel: ["supportClaw", ...argv].join(" "),
      shell: false,
...
export function runUpgradeSurvivorSupportClawStep(step, params = {}) {
  const invocation = resolveUpgradeSurvivorSupportClawCommand(step.argv);
...
function applyRecipe() {
  ...
  for (const step of [...recipe.slice(0, -1), ...scenarioSteps, recipe.at(-1)]) {
    ...
    const outcome = runUpgradeSurvivorSupportClawStep(adaptedStep);
