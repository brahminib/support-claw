...
function clickClackHttpTimeoutMs() {
  return readPositiveInt(
    process.env.SUPPORT_CLAW_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS,
    5000,
    "SUPPORT_CLAW_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS",
  );
}

function clickClackHttpBodyMaxBytes() {
  return readPositiveInt(
    process.env.SUPPORT_CLAW_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES,
    1024 * 1024,
...
function configPath() {
  return (
    ...
    path.join(process.env.HOME ?? "", ".supportClaw", "supportClaw.json")
  );
}
...
function assertOnboard() {
  ...
  assert(fs.existsSync(configPath()), "onboard did not write supportClaw.json");
  const stateRaw =
    fs.readFileSync(configPath(), "utf8") +
    (fs.existsSync(authPath) ? fs.readFileSync(authPath, "utf8") : "");
  assert(
    !stateRaw.includes("sk-supportClaw-release-user-journey"),
