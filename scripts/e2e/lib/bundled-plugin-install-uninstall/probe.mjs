...
function resolveStateDir() {
  if (process.env.SUPPORT_CLAW_STATE_DIR) {
    return process.env.SUPPORT_CLAW_STATE_DIR;
  }
  return path.join(process.env.HOME || os.homedir(), ".supportClaw");
}
...
function resolveSupportClawEntry() {
  if (process.env.SUPPORT_CLAW_ENTRY) {
    return process.env.SUPPORT_CLAW_ENTRY;
  }
  ...
  throw new Error("Missing SUPPORT_CLAW_ENTRY and dist/index.(m)js");
}

function readPluginsList() {
  const entry = resolveSupportClawEntry();
...
async function loadManifestEntries() {
  const explicit = (process.env.SUPPORT_CLAW_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `SUPPORT_CLAW_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("SUPPORT_CLAW_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("SUPPORT_CLAW_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `SUPPORT_CLAW_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.SUPPORT_CLAW_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }
