...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/supportClaw-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.SUPPORT_CLAW_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".supportClaw-rename-tmp");
...
run_supportClaw() {
  HOME="$HOME_DIR" \
  SUPPORT_CLAW_HOME="$STATE_DIR" \
  SUPPORT_CLAW_STATE_DIR="$STATE_DIR" \
  SUPPORT_CLAW_CONFIG_PATH="$CONFIG_PATH" \
  SUPPORT_CLAW_PLUGIN_STAGE_DIR="$STAGE_DIR" \
...
run_supportClaw status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run supportClaw doctor --fix" "$BEFORE" >/dev/null

run_supportClaw doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_supportClaw status --all --timeout 1 > "$AFTER"
