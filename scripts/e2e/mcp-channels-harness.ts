...
// against the SupportClaw npm tarball installed in the functional image.
...
export async function connectMcpClient(params: {
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      SUPPORT_CLAW_STATE_DIR: tempState.stateDir,
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[supportClaw mcp] ${String(chunk)}`);
  });
