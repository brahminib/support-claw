...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.SUPPORT_CLAW_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsResult?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "supportClaw/default") ?? modelIds.find((id) => id === "supportClaw");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing supportClaw model: ${JSON.stringify(modelIds)}`;
...
if (!targetModel) {
  throw new Error(
    `supportClaw model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {
