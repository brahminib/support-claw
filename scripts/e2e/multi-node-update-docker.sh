...
# SupportClaw under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `supportClaw update`. Verifies that:
#
...
#
# Requires: Docker, a built supportClaw-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="$(docker_e2e_resolve_image "supportClaw-multi-node-update-e2e" SUPPORT_CLAW_MULTI_NODE_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${SUPPORT_CLAW_MULTI_NODE_UPDATE_E2E_SKIP_BUILD:-0}"
DOCKER_RUN_TIMEOUT="${SUPPORT_CLAW_MULTI_NODE_DOCKER_TIMEOUT:-300s}"
...
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz multi-node-update "${SUPPORT_CLAW_CURRENT_PACKAGE_TGZ:-}")"
...
docker_e2e_run_with_harness \
  ...
  -e SUPPORT_CLAW_DISABLE_BONJOUR=1 \
...
echo ""
echo "── Step 2: Install SupportClaw under node-A ──"

# Use node-A to install supportClaw with npm prefix A.
...
echo "Installing SupportClaw package under node-A prefix: $NPM_PREFIX_A"
supportClaw_e2e_install_package "$ARTIFACTS/install-a.log" "SupportClaw package under node-A prefix" "$NPM_PREFIX_A"
echo "Installed. Checking supportClaw location..."

SUPPORT_CLAW_A="$(command -v supportClaw)"
echo "supportClaw binary: $SUPPORT_CLAW_A"
echo "supportClaw version: $(supportClaw --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/supportClaw"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/supportClaw-gateway.service"
...
start_gateway() {
  ...
  (
    ...
    export SUPPORT_CLAW_NO_RESPAWN=1
...
if ! supportClaw gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err"; then
...
# but supportClaw is NOT installed there.
...
echo "which supportClaw: $(command -v supportClaw)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run supportClaw update (this is the bug) ──"
...
echo "Running supportClaw update --yes --json..."
UPDATE_EXIT=0
supportClaw update --yes --json \
  --tag /tmp/supportClaw-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: supportClaw update failed before reaching the package install step"
...
if [ "$BAKED_NODE_AFTER" = "$NODE_B" ] && [ "$BAKED_NODE_BEFORE" != "$NODE_B" ]; then
  ...
  echo "   but SupportClaw package is still under node-A prefix ($PACKAGE_ROOT_A)."
...
# Check 2: Is the SupportClaw package installed under node-B npm prefix?
if [ -f "$NPM_PREFIX_B/lib/node_modules/supportClaw/package.json" ]; then
  echo "WARNING: SupportClaw was ALSO installed under node-B prefix (split install)"
else
  echo "OK: SupportClaw is NOT under node-B prefix (expected: only under node-A)"
fi
...
if [ -f "$ARTIFACTS/update.err" ]; then
  if grep -qi "Shell SupportClaw root differs" "$ARTIFACTS/update.err" 2>/dev/null; then
