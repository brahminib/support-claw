...
IMAGE_NAME="$(docker_e2e_resolve_image "supportClaw-mcp-code-mode-gateway-live-e2e" SUPPORT_CLAW_IMAGE)"
PORT="$(docker_e2e_read_tcp_port_env SUPPORT_CLAW_MCP_CODE_MODE_LIVE_GATEWAY_PORT 18789)"
...
CONTAINER_NAME="supportClaw-mcp-code-mode-live-e2e-$$"
PROFILE_FILE="${SUPPORT_CLAW_MCP_CODE_MODE_LIVE_PROFILE_FILE:-${SUPPORT_CLAW_TESTBOX_PROFILE_FILE:-$HOME/.supportClaw-testbox-live.profile}}"

CLIENT_LOG="$(mktemp -t supportClaw-mcp-code-mode-live-log.XXXXXX)"
...
SUPPORT_CLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway-live empty)"

# The profile is only a credential source. Keep this lane's SupportClaw runtime
# isolated from host/testbox mode flags that can change packaged behavior.
unset SUPPORT_CLAW_TESTBOX
...
docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "SUPPORT_CLAW_DOCKER_OPENAI_BASE_URL=${OPENAI_BASE_URL:-https://api.openai.com/v1}" \
  ...
  -e "SUPPORT_CLAW_MCP_CODE_MODE_MODEL=${SUPPORT_CLAW_MCP_CODE_MODE_LIVE_MODEL:-supportClaw/main}" \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail
    ...
    gateway_pid=\"\$(supportClaw_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-live-gateway.log)\"
    supportClaw_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-live-gateway.log 480 $PORT
