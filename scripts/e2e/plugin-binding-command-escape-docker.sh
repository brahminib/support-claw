...
IMAGE_NAME="${SUPPORT_CLAW_PLUGIN_BINDING_COMMAND_ESCAPE_E2E_IMAGE:-supportClaw-plugin-binding-command-escape-e2e}"
CONTAINER_NAME="supportClaw-plugin-binding-command-escape-e2e-$$"
DOCKER_RUN_TIMEOUT="${SUPPORT_CLAW_PLUGIN_BINDING_COMMAND_ESCAPE_DOCKER_RUN_TIMEOUT:-900s}"
RUN_LOG="$(mktemp -t supportClaw-plugin-binding-command-escape-log.XXXXXX)"
...
DOCKER_COMMAND_TIMEOUT="$DOCKER_RUN_TIMEOUT" docker_e2e_docker_run_cmd run --rm \
  ...
  -e SUPPORT_CLAW_VITEST_FS_MODULE_CACHE_PATH=/tmp/supportClaw-vitest-cache \
  "$IMAGE_NAME" \
