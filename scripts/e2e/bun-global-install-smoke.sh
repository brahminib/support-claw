...
BUN_BIN="${BUN_BIN:-bun}"
HOST_BUILD="${SUPPORT_CLAW_BUN_GLOBAL_SMOKE_HOST_BUILD:-1}"
DIST_IMAGE="${SUPPORT_CLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE:-}"
PACKAGE_TGZ="${SUPPORT_CLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ:-}"
...
resolve_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "SUPPORT_CLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ -n "$DIST_IMAGE" ]; then
    ...
    echo "==> Skipping host build (SUPPORT_CLAW_BUN_GLOBAL_SMOKE_HOST_BUILD=0)"
  fi

  if [ ! -d "$ROOT_DIR/dist" ]; then
    echo "dist/ is missing; run pnpm build or set SUPPORT_CLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE" >&2
    exit 1
  fi
  ...
  PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/supportClaw-bun-pack.XXXXXX")"
  pack_json_file="$PACK_DIR/pack.json"

  echo "==> Pack SupportClaw tarball"
...
main() {
  ...
  echo "==> Bun global install packed SupportClaw"
  "$bun_path" install -g "$PACKAGE_TGZ" --no-progress

  supportClaw_bin="$BUN_INSTALL/bin/supportClaw"
  if [ ! -x "$supportClaw_bin" ]; then
    supportClaw_bin="$(command -v supportClaw || true)"
  fi
  if [ -z "$supportClaw_bin" ] || [ ! -x "$supportClaw_bin" ]; then
    echo "Bun global install did not create an executable supportClaw binary" >&2
    exit 1
  fi

  echo "==> SupportClaw version through Bun global install"
  run_with_timeout "$COMMAND_TIMEOUT_MS" "$supportClaw_bin" --version

  echo "==> SupportClaw image providers through Bun global install"
  local providers_json
  providers_json="$(run_with_timeout "$COMMAND_TIMEOUT_MS" "$supportClaw_bin" infer image providers --json)"
  SUPPORT_CLAW_IMAGE_PROVIDERS_JSON="$providers_json" node scripts/e2e/lib/bun-global-install/assertions.mjs assert-image-providers
}
