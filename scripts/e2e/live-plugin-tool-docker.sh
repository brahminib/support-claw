...
IMAGE_NAME="$(docker_e2e_resolve_image "supportClaw-live-plugin-tool-e2e" SUPPORT_CLAW_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${SUPPORT_CLAW_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${SUPPORT_CLAW_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${SUPPORT_CLAW_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${SUPPORT_CLAW_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${SUPPORT_CLAW_TESTBOX_PROFILE_FILE:-$HOME/.supportClaw-testbox-live.profile}}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${SUPPORT_CLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "SUPPORT_CLAW_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires SUPPORT_CLAW_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
SUPPORT_CLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e SUPPORT_CLAW_LIVE_PLUGIN_TOOL_MODEL="${SUPPORT_CLAW_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.5}" \
  ...
  -e "SUPPORT_CLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${SUPPORT_CLAW_LIVE_PLUGIN_TOOL_MODEL:?missing SUPPORT_CLAW_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@supportclaw/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="SupportClaw E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  local status="$1"
  local agent_output_dump_bytes="${SUPPORT_CLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES:-16384}"
  echo "Live plugin tool scenario failed with exit code $status" >&2
  if [ -f /tmp/supportClaw-agent.json ]; then
    echo "--- /tmp/supportClaw-agent.json (last ${agent_output_dump_bytes} bytes) ---" >&2
    tail -c "$agent_output_dump_bytes" /tmp/supportClaw-agent.json >&2 || true
    echo >&2
  fi
  supportClaw_e2e_dump_logs \
    /tmp/supportClaw-install.log \
    /tmp/supportClaw-plugin-install.log \
    /tmp/supportClaw-plugin-enable.log \
    /tmp/supportClaw-plugins-list.json \
    /tmp/supportClaw-plugin-inspect.json \
    /tmp/supportClaw-live-plugin-tool-pack.log \
...
fixture_dir="$(mktemp -d /tmp/supportClaw-live-plugin-tool.XXXXXX)"
...
(cd "$plugin_dir" && npm pack --pack-destination "$fixture_dir" --silent) \
  >/tmp/supportClaw-live-plugin-tool-pack.log 2>&1
plugin_tgzs=()
...
if [ "${#plugin_tgzs[@]}" -ne 1 ]; then
  ...
  supportClaw_e2e_dump_logs /tmp/supportClaw-live-plugin-tool-pack.log
  exit 1
fi
plugin_tgz="${plugin_tgzs[0]}"

echo "Installing fixture plugin from npm-pack: $plugin_tgz"
supportClaw plugins install "npm-pack:$plugin_tgz" --force >/tmp/supportClaw-plugin-install.log 2>&1
node scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
supportClaw plugins enable "$PLUGIN_ID" >/tmp/supportClaw-plugin-enable.log 2>&1
supportClaw plugins list --json >/tmp/supportClaw-plugins-list.json
supportClaw plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/supportClaw-plugin-inspect.json
...
supportClaw agent --local \
  ...
  --thinking off \
  --timeout "${SUPPORT_CLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/supportClaw-agent.json 2>/tmp/supportClaw-agent.err
