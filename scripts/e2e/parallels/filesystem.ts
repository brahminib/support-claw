// Filesystem script supports SupportClaw repository automation.
...
const SUPPORT_CLAW_VERSION_PATTERN = /SupportClaw\s+([0-9][^\s]*)/gi;
...
export async function extractLastSupportClawVersionFromLog(
  logPath: string,
  pattern = SUPPORT_CLAW_VERSION_PATTERN,
  maxBytes = DEFAULT_TEXT_FILE_TAIL_BYTES,
): Promise<string> {
...
export async function makeTempDir(prefix: string): Promise<string> {
  const root =
    process.env.SUPPORT_CLAW_PARALLELS_ARTIFACT_ROOT || path.join(repoRoot, ".artifacts", "parallels");
