// Plugin Isolation script supports SupportClaw repository automation.
...
export function posixCodexPlatformPackageRepairFunction(): string {
  ...
  state_home="\${SUPPORT_CLAW_PARALLELS_HOME:-\${HOME:-}}"
  codex_manifest=""
  for candidate in "$state_home"/.supportClaw/npm/projects/*/node_modules/@supportclaw/codex/package.json; do
  ...
  project_root="\${codex_manifest%/node_modules/@supportclaw/codex/package.json}"
  cache_dir="$(mktemp -d "\${TMPDIR:-/tmp}/supportClaw-npm-cache.XXXXXX")"
...
export function windowsCodexPlatformPackageRepairFunction(): string {
  ...
  $projectsRoot = Join-Path $env:USERPROFILE '.supportClaw\npm\projects'
  $codexManifest = Get-ChildItem -Path $projectsRoot -Filter package.json -File -Recurse -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -match 'node_modules[\\/]@supportclaw[\\/]codex[\\/]package\.json$' } |
  ...
  $cacheDir = Join-Path ([System.IO.Path]::GetTempPath()) ('supportClaw-npm-cache-' + [guid]::NewGuid().ToString('N'))
...
export function posixProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  const homeEnv = options.homeFallback
    ? `SUPPORT_CLAW_PARALLELS_HOME=${shellQuote(options.homeFallback)} `
    : "";
...
export function windowsProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  return `$env:SUPPORT_CLAW_PARALLELS_PLUGIN_ISOLATION = @'
${payloadJson}
'@
$isolationScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'supportClaw-parallels-plugin-isolation.cjs'
@'
...
Remove-Item Env:SUPPORT_CLAW_PARALLELS_PLUGIN_ISOLATION -Force -ErrorAction SilentlyContinue`;
}
...
function providerOnlyPluginIsolationNodeScript(options: PluginIsolationOptions): string {
  ...
  return `process.env.SUPPORT_CLAW_PARALLELS_PLUGIN_ISOLATION = ${JSON.stringify(payloadJson)};
${providerOnlyPluginIsolationNodeSource()}`;
}
...
const payload = JSON.parse(process.env.SUPPORT_CLAW_PARALLELS_PLUGIN_ISOLATION || "{}");
const home =
  process.env.SUPPORT_CLAW_PARALLELS_HOME ||
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@supportclaw", "codex"), {
  recursive: true,
  force: true,
});
