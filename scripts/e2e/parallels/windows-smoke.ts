...
// Windows Smoke script supports SupportClaw repository automation.
...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://supportClaw.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-supportClaw-native-e2e-2026-03-12",
  targetPackageSpec: "",
  upgradeFromPackedMain: false,
  vmName: "Windows 11",
});

const windowsPortableGitPathScript = `$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'SupportClaw\\deps') 'portable-git') ''
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-supportClaw-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --provider <openai|anthropic|minimax>
  --model <provider/model>    Override the model used for the agent-turn smoke.
  --api-key-env <var>        Host env var name for provider API key.
  --openai-api-key-env <var> Alias for --api-key-env (backward compatible)
  --install-url <url>        Installer URL for latest release. Default: https://supportClaw.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run supportClaw update --channel dev.
...
class WindowsSmoke extends SmokeRunController<WindowsOptions> {
  ...
  private gatewayRecoveryAfterMs =
    readPositiveIntEnv("SUPPORT_CLAW_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S", 180) * 1000;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("supportClaw-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("supportClaw-parallels-windows-tgz.");
    try {
  ...
  protected async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () => this.installMain("supportClaw-main-fresh.tgz"));
  ...
  private guestPowerShell(
    ...
    return this.guest.powershell(`${windowsSupportClawResolver}\n${script}`, options);
  }
  ...
  private logGuestPreflight(cleanSupportClaw: boolean): void {
    const cleanScript = cleanSupportClaw
      ? "npm.cmd uninstall -g supportClaw --no-fund --no-audit --loglevel=error 2>$null; $global:LASTEXITCODE = 0"
      : "";
...
if ($LASTEXITCODE -ne 0) { throw "supportClaw --version failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestPowerShell("Invoke-SupportClaw --version");
...
Invoke-SupportClaw onboard --non-interactive --mode local --auth-choice ${psSingleQuote(this.auth.authChoice)} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json
if ($LASTEXITCODE -ne 0) { throw "supportClaw onboard failed with exit code $LASTEXITCODE" }
${this.windowsPluginIsolationScript()}`,
  ...
  private async guestPowerShellBackground(
    ...
    await runWindowsBackgroundPowerShell({
      ...
      label,
      onLaunchRetry: warn,
      script: `${windowsSupportClawResolver}\n${script}`,
      timeoutMs,
...
${windowsScopedEnvFunction}
$script:SupportClawUpdateExit = 0
Invoke-WithScopedEnv @{ SUPPORT_CLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'; SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS = '1' } {
  Invoke-SupportClaw update --channel dev --yes --json
  $script:SupportClawUpdateExit = $LASTEXITCODE
}
if ($script:SupportClawUpdateExit -ne 0) { throw "supportClaw update failed with exit code $script:SupportClawUpdateExit" }
Invoke-SupportClaw --version
Invoke-SupportClaw update status --json`,
...
Invoke-SupportClaw update status --json`,
  ...
  private gatewayAction(action: "restart" | "stop"): Promise<void> {
    return this.guestPowerShellBackground(
      ...
      `$ErrorActionPreference = 'Continue'
$PSNativeCommandUseErrorActionPreference = $false
Invoke-SupportClaw gateway ${action}
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    while (Date.now() < deadline) {
      const probe = this.guestPowerShell(
        "Invoke-SupportClaw gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json",
      ...
      if (!recoveryTried && Date.now() - start >= this.gatewayRecoveryAfterMs) {
        ...
        this.guestPowerShell("Invoke-SupportClaw gateway start", {
          check: false,
          timeoutMs: 120_000,
        });
        recoveryTried = true;
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestPowerShell("Invoke-SupportClaw gateway status --help", {
      check: false,
    });
    const suffix = help.includes("--require-rpc") ? "--deep --require-rpc" : "--deep";
    this.guestPowerShell(`Invoke-SupportClaw gateway status ${suffix}`);
  }
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  $output = Invoke-SupportClaw @args 2>&1
...
if (-not $agentOk) { throw 'supportClaw agent finished without OK response' }`,
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastSupportClawVersion(this.runDir, phaseName, /SupportClaw\s+([0-9][^\s]*)/gi);
  }
