...
// Linux Smoke script supports SupportClaw repository automation.
...
function parseSupportClawPackageVersion(value: string): string | null {
  return value.match(/\b(\d{4}\.\d{1,2}\.\d{1,2}(?:-[A-Za-z0-9.]+)?)\b/u)?.[1] ?? null;
}

function compareSupportClawPackageVersions(left: string, right: string): number {
  const parse = (value: string): [number, number, number] => {
    const match = parseSupportClawPackageVersion(value)?.match(/^(\d{4})\.(\d+)\.(\d+)/u);
    if (!match) {
      return [0, 0, 0];
    }
...
class LinuxSmoke extends SmokeRunController<LinuxOptions> {
  ...
  private disableBonjour = parseBoolEnv(process.env.SUPPORT_CLAW_PARALLELS_LINUX_DISABLE_BONJOUR);
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("supportClaw-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("supportClaw-parallels-linux-tgz.");
    try {
  ...
  protected async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () =>
      this.installMainTgz("supportClaw-main-fresh.tgz"),
    );
  ...
  protected async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("supportClaw-main-upgrade.tgz"),
    );
...
run_apt_with_lock_retry apt-get -o DPkg::Lock::Timeout=30 install -y curl ca-certificates`);
  ...
  private installLatestRelease(): void {
    this.downloadGuestFile(this.options.installUrl, "/tmp/supportClaw-install.sh");
    if (this.options.installVersion) {
      this.guestExec([
        "/usr/bin/env",
        "SUPPORT_CLAW_NO_ONBOARD=1",
        "bash",
        "/tmp/supportClaw-install.sh",
        "--version",
    ...
    this.guestExec(["supportClaw", "--version"]);
  }
...
if command -v curl >/dev/null 2>&1; then
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["supportClaw", "--version"]);
  ...
  private injectBadPluginFixture(): void {
    this.guestBash(String.raw`set -euo pipefail
plugin_dir=/root/.supportClaw/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@supportclaw/test-bad-plugin","version":"1.0.0","supportClaw":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/supportClaw.plugin.json" <<'JSON'
...
config_path = Path("/root/.supportClaw/supportClaw.json")
...
PY`);
  ...
  private shouldExpectBadPluginDiagnostic(lane: "fresh" | "upgrade"): boolean {
    const version = parseSupportClawPackageVersion(this.versionForLane(lane));
    if (!version) {
      return true;
    }
    return compareSupportClawPackageVersions(version, BAD_PLUGIN_DIAGNOSTIC_MIN_VERSION) >= 0;
  }
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " SUPPORT_CLAW_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "supportClaw gateway run" >/dev/null 2>&1 || true
rm -f /tmp/supportClaw-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
          `exec env SUPPORT_CLAW_HOME=/root SUPPORT_CLAW_STATE_DIR=/root/.supportClaw SUPPORT_CLAW_CONFIG_PATH=/root/.supportClaw/supportClaw.json SUPPORT_CLAW_ALLOW_ROOT=1${bonjourEnv} ${this.auth.apiKeyEnv}=${shellQuote(
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["supportClaw", "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["supportClaw", "gateway", "status", "--deep", "--require-rpc"]
      : ["supportClaw", "gateway", "status", "--deep"];
    const result = run(
      "prlctl",
      ["exec", this.options.vmName, "/usr/bin/env", "HOME=/root", "SUPPORT_CLAW_ALLOW_ROOT=1", ...args],
      {
        check: false,
        quiet: true,
        timeoutMs: this.remainingPhaseTimeoutMs(),
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.supportClaw/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.supportClaw/test-bad-plugin`);
  }
  ...
  private verifyLocalTurn(): void {
    this.guestExec(["supportClaw", "models", "set", this.auth.modelId]);
...
JSON
supportClaw config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["supportClaw", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env SUPPORT_CLAW_ALLOW_ROOT=1 ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} supportClaw agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private async extractLastVersion(phaseId: string): Promise<string> {
    return await extractLastSupportClawVersion(
      this.runDir,
      phaseId,
      /(SupportClaw [^\r\n]+ \([0-9a-f]{7,}\))/g,
    );
