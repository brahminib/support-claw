...
// Macos Smoke script supports SupportClaw repository automation.
...
const guestSupportClaw = "supportClaw";
const guestSupportClawEntry = '"$(npm root -g)/supportClaw/supportClawCli.mjs"';
const guestSupportClawEntryRunner = `node ${guestSupportClawEntry}`;
...
const defaultOptions = (): MacosOptions => ({
  ...
  installUrl: "https://supportClaw.ai/install.sh",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://supportClaw.ai/install.sh
...
class MacosSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await makeTempDir("supportClaw-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("supportClaw-parallels-macos-tgz.");
    try {
      ...
      if (await this.needsHostTgz()) {
        this.artifact = await packSupportClaw({
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
      this.installMain("supportClaw-main-fresh.tgz"),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    if (this.options.targetPackageSpec) {
      await this.phase("upgrade.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
        this.installMain("supportClaw-main-upgrade.tgz"),
      );
  ...
  private guestSupportClawEntryExec(
    args: string[],
    options: { check?: boolean; env?: Record<string, string> } = {},
  ): string {
    ...
    return this.guestSh(
      `set -e
entry="$(npm root -g)/supportClaw/supportClawCli.mjs"
exec node "$entry" ${argv}`,
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'supportClaw.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'supportClaw-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'supportClawCli.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g supportClaw >/dev/null 2>&1 || true
rm -rf "$HOME/.supportClaw"
...
curl -fsSL --connect-timeout 10 --max-time 120 --retry 2 --retry-delay 2 ${shellQuote(
      ...
      )} -o /tmp/supportClaw-install.sh
bash /tmp/supportClaw-install.sh --version ${shellQuote(this.installVersion)}
${guestSupportClaw} --version`,
    );
  }
...
${guestSupportClaw} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestSupportClaw, "--version"]);
...
check_path "$root/supportClaw"
check_path "$root/supportClaw/extensions"
if [ -d "$root/supportClaw/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/supportClaw/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/supportClaw-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private runDevChannelUpdate(): void {
    ...
    this.guestSh(
      `set -eu
rm -rf ${shellQuote(`${home}/supportClaw`)}
export PATH=${shellQuote(`/tmp/supportClaw-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".supportClaw", "supportClaw.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=8192 SUPPORT_CLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS=1 ${guestSupportClawEntryRunner} update --channel dev --yes --json
${guestSupportClawEntryRunner} --version
${guestSupportClawEntryRunner} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestSupportClawEntryExec(["update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'supportClaw.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'supportClaw-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'supportClawCli.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} SUPPORT_CLAW_HOME=${shellQuote(home)} SUPPORT_CLAW_STATE_DIR=${shellQuote(`${home}/.supportClaw`)} SUPPORT_CLAW_CONFIG_PATH=${shellQuote(
        `${home}/.supportClaw/supportClaw.json`,
      )} ${guestSupportClawEntryRunner} gateway run --bind loopback --port 18789 --force </dev/null >/tmp/supportClaw-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private verifyGateway(): void {
    for (let attempt = 1; attempt <= 8; attempt++) {
      const result = this.guestSupportClaw(
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestSupportClaw, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestSupportClaw(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestSupportClaw(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`supportClaw ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/supportClaw-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>SupportClaw Control</title>' /tmp/supportClaw-dashboard-smoke.html >/dev/null &&
      grep -F '<supportClaw-app></supportClaw-app>' /tmp/supportClaw-dashboard-smoke.html >/dev/null; then
      asset_paths="$(
        sed -nE 's/.*<(script|link)[^>]*(src|href)=["'"'"']([^"'"'"']+)["'"'"'].*/\3/p' /tmp/supportClaw-dashboard-smoke.html |
          grep -E '(^|/)assets/' |
          grep -Ev '^(https?:)?//' |
          sort -u
      )"
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestSupportClawEntryExec(["models", "set", this.auth.modelId]);
...
JSON
${guestSupportClawEntryRunner} config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    }
    this.guestSupportClawEntryExec([
      "config",
      "set",
    ...
    this.guestSupportClawEntryExec(["config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestSupportClawEntryRunner} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private ensureDiscordGatewayReady(): void {
    ...
    this.verifyGateway();
    const status = this.guestSupportClawEntryExec(["channels", "status", "--probe", "--json"]);
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastSupportClawVersionFromLog(path.join(this.runDir, `${phaseName}.log`));
  }
