// Powershell script supports SupportClaw repository automation.
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:SUPPORT_CLAW_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.supportClaw\\supportClaw.json' }
$agentTurnVersionText = Invoke-SupportClaw --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'SupportClaw\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:SUPPORT_CLAW_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:SUPPORT_CLAW_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:SUPPORT_CLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'supportClaw-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.SUPPORT_CLAW_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.SUPPORT_CLAW_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.SUPPORT_CLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@supportclaw", "codex"), { recursive: true, force: true });
...
if (selectedModelEntry && typeof selectedModelEntry === "object" && !Array.isArray(selectedModelEntry)) {
  if (canWriteAgentRuntime) {
    selectedModelEntry.agentRuntime = { id: "supportClaw" };
  } else {
...
Remove-Item Env:SUPPORT_CLAW_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:SUPPORT_CLAW_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:SUPPORT_CLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsSupportClawResolver = String.raw`$portableNode = if ($env:LOCALAPPDATA) { Join-Path $env:LOCALAPPDATA 'Programs\nodejs' } else { $null }
if ($portableNode -and (Test-Path (Join-Path $portableNode 'node.exe'))) {
  $env:PATH = "$portableNode;$env:PATH"
}
function Resolve-SupportClawCommand {
  if ($script:SupportClawResolvedCommand) { return $script:SupportClawResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\supportClaw.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\supportClaw.ps1'
  }
  foreach ($name in @('supportClaw.cmd', 'supportClaw.ps1', 'supportClaw')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'supportClaw.cmd'
    $shimCandidates += Join-Path $npmPrefix 'supportClaw.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:SupportClawResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:SupportClawResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\supportClaw\supportClawCli.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:SupportClawResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:SupportClawResolvedCommand
    }
  }
  throw 'supportClaw command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-SupportClaw {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $SupportClawArgs)
  $command = Resolve-SupportClawCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @SupportClawArgs
    } else {
      & $command.Path @SupportClawArgs
    }
  } finally {
