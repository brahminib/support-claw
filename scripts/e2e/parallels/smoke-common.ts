// Smoke Common helper supports SupportClaw script workflows.
import { readFile, rm } from "node:fs/promises";
import path from "node:path";
import { extractLastSupportClawVersionFromLog } from "./filesystem.ts";
...
export async function packAndServeSmokeArtifact(
  ...
  const artifact = await packSupportClaw({
    destination: tgzDir,
    packageSpec,
    requireControlUi,
  });
...
export async function extractLastSupportClawVersion(
  runDir: string,
  phaseName: string,
  pattern: RegExp,
): Promise<string> {
  return await extractLastSupportClawVersionFromLog(path.join(runDir, `${phaseName}.log`), pattern);
}
