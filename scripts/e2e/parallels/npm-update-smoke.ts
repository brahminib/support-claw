...
// Npm Update Smoke script supports SupportClaw repository automation.
...
import {
  ...
  resolveSupportClawRegistryVersion,
  resolveProviderAuth,
...
const updateTimeoutSeconds = readPositiveIntEnv("SUPPORT_CLAW_PARALLELS_NPM_UPDATE_TIMEOUT_S", 1200);
...
const freshLaneTimeoutKillGraceMs = readPositiveIntEnv(
  "SUPPORT_CLAW_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_KILL_GRACE_MS",
  2_000,
);
...
export function freshLaneTimeoutMs(platform: Platform): number {
  ...
  return resolveSecondsTimerMs(
    readPositiveIntEnv("SUPPORT_CLAW_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S", defaultSeconds),
  );
}
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: supportClaw@latest
  --update-target <target>    Target passed to guest 'supportClaw update --tag'.
...
function supportClawVersionFamily(version: string): string {
  return /^(\d{4}\.\d{1,2}\.\d{1,2})(?:[-.]|$)/u.exec(version.trim())?.[1] ?? "";
}

function parseSupportClawPackageSpecVersion(spec: string): string {
  const value = spec.trim();
  if (!value) {
    return "";
  }
  return resolveSupportClawRegistryVersion(value) || "";
}
...
export class NpmUpdateSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await this.makeRunTempDir("supportClaw-parallels-npm-update.");
    this.tgzDir = await this.makeRunTempDir("supportClaw-parallels-npm-update-tgz.");
    try {
      await this.runSteps();
    } finally {
  ...
  protected async runSteps(): Promise<void> {
    ...
    this.packageSpec = this.options.packageSpec || `supportClaw@${this.latestVersion}`;
    ...
    say(`Run same-guest supportClaw update to ${this.updateTargetEffective}`);
    ...
    if (this.options.platforms.has("linux")) {
      jobs.push(
        this.spawnFresh("Linux", "linux", ["--vm", this.linuxVm], {
          SUPPORT_CLAW_PARALLELS_LINUX_DISABLE_BONJOUR: "1",
        }),
      );
    }
  ...
  private async prepareUpdateTarget(): Promise<void> {
    ...
    this.updateExpectedNeedle = this.isExplicitPackageTarget(this.updateTargetEffective)
      ? ""
      : resolveSupportClawRegistryVersion(this.updateTargetEffective) || this.updateTargetEffective;
  ...
  private resolveRegistryPackageMetadata(target: string): {
    ...
    const spec = target.startsWith("supportClaw@") ? target : `supportClaw@${target}`;
  ...
  private async guestMacos(
    ...
    const scriptPath = this.writeGuestScript(
      this.macosVm,
      script,
      "supportClaw-parallels-npm-update-macos",
    );
  ...
  private preflightRegistryUpdateTarget(): void {
    ...
    const baseline = resolveSupportClawRegistryVersion(this.packageSpec);
    const target = resolveSupportClawRegistryVersion(this.options.updateTarget);
    if (baseline && target && baseline === target) {
      die(
        `--update-target ${this.options.updateTarget} resolves to supportClaw@${target}, same as baseline ${this.packageSpec}; publish or choose a newer --update-target before running VM update coverage`,
      );
    }
  }
  ...
  private async extractLastVersion(logPath: string): Promise<string> {
    return await extractLastSupportClawVersionFromLog(logPath);
  }
  ...
  private async configureTargets(): Promise<void> {
    ...
    if (this.options.betaValidation) {
      const version = resolveSupportClawRegistryVersion(this.options.betaValidation);
      if (!version) {
        die(`could not resolve beta validation target: ${this.options.betaValidation}`);
      }
      this.options.updateTarget = version;
      this.options.freshTargetSpec = `supportClaw@${version}`;
      say(`Beta validation target: supportClaw@${version}`);
    } else if (
      this.options.updateTarget &&
      this.options.updateTarget !== "local-main" &&
      !this.isExplicitPackageTarget(this.options.updateTarget)
    ) {
      const version = resolveSupportClawRegistryVersion(this.options.updateTarget);
      if (version) {
        this.options.updateTarget = version;
      }
    }
    ...
    if (this.options.freshTargetSpec) {
      const version = resolveSupportClawRegistryVersion(this.options.freshTargetSpec);
      this.freshTargetSpec = version ? `supportClaw@${version}` : this.options.freshTargetSpec;
    }
  }

  private assertPublishedTargetMatchesHarnessCheckout(): void {
    if (process.env.SUPPORT_CLAW_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH === "1") {
      return;
    }
    const candidateVersion =
      this.targetTarballVersion ||
      (this.freshTargetSpec
        ? parseSupportClawPackageSpecVersion(this.freshTargetSpec)
        : parseSupportClawPackageSpecVersion(this.options.updateTarget));
    const targetFamily = supportClawVersionFamily(candidateVersion);
    if (!targetFamily) {
      return;
    }
    this.harnessTargetFamily = targetFamily;
    const checkoutFamily = supportClawVersionFamily(this.harnessCheckoutVersion);
    if (checkoutFamily === targetFamily) {
      return;
    }
    die(
      `refusing to run Parallels ${candidateVersion} target with harness checkout ${this.harnessCheckoutVersion || "unknown"}; checkout the matching release branch or set SUPPORT_CLAW_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH=1 for an intentional cross-version harness run`,
    );
  }
