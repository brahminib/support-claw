// Guest Transports script supports SupportClaw repository automation.
...
function guestScriptName(extension: string): string {
  return `supportClaw-parallels-${randomUUID()}.${extension}`;
}
...
export async function runWindowsBackgroundPowerShell(
  ...
  const fileBase = `supportClaw-parallels-${nonce}`;
  const logLengthPrefix = `__SUPPORT_CLAW_LOG_LENGTH__:${nonce}:`;
  const logOffsetPrefix = `__SUPPORT_CLAW_LOG_OFFSET__:${nonce}:`;
  const backgroundExitPrefix = `__SUPPORT_CLAW_BACKGROUND_EXIT__:${nonce}:`;
  const backgroundDoneMarker = `__SUPPORT_CLAW_BACKGROUND_DONE__:${nonce}`;
...
$exitPath = "$base.exit"
$pidPath = "$base.pid"
function Write-SupportClawUtf8File([string]$Path, [string]$Value) {
  [System.IO.File]::WriteAllText($Path, $Value, [System.Text.UTF8Encoding]::new($false))
}`;
  const payload = `$ErrorActionPreference = 'Stop'
$PSNativeCommandUseErrorActionPreference = $false
${pathsScript}
function Add-SupportClawBackgroundLog {
...
try {
  & {
${options.script}
  } *>&1 | Add-SupportClawBackgroundLog
  Write-SupportClawUtf8File $exitPath '0'
} catch {
  $_ | Add-SupportClawBackgroundLog
  Write-SupportClawUtf8File $exitPath '1'
} finally {
  Write-SupportClawUtf8File $donePath 'done'
}`;
...
Write-SupportClawUtf8File $pidPath ([string]$process.Id)
'started'`),
        ],
...
function cleanupWindowsBackground(
  ...
  const stopProcessTree = options.stopProcessTree
    ? `function Stop-SupportClawBackgroundProcessTree([int]$ProcessId) {
  Get-CimInstance Win32_Process -Filter "ParentProcessId=$ProcessId" -ErrorAction SilentlyContinue | ForEach-Object {
    Stop-SupportClawBackgroundProcessTree ([int]$_.ProcessId)
  }
...
export class LinuxGuest {
  ...
  private transportArgs(args: string[]): string[] {
    return ["exec", this.vmName, "/usr/bin/env", "HOME=/root", "SUPPORT_CLAW_ALLOW_ROOT=1", ...args];
  }
