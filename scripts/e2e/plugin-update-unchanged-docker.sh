#!/usr/bin/env bash
# Verifies `supportClaw plugins update` is a no-op for an already-current plugin.
...
IMAGE_NAME="$(docker_e2e_resolve_image "supportClaw-plugin-update-e2e" SUPPORT_CLAW_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${SUPPORT_CLAW_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
cleanup() {
  docker_e2e_cleanup_package_tgz "${PACKAGE_TGZ:-}"
}
trap cleanup EXIT

PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz plugin-update "${SUPPORT_CLAW_CURRENT_PACKAGE_TGZ:-}")"
...
SUPPORT_CLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 plugin-update empty)"
