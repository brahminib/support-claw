...
const PLUGIN_SPEC =
  process.env.SUPPORT_CLAW_KITCHEN_SINK_NPM_SPEC || "npm:@supportclaw/kitchen-sink@latest";
const PLUGIN_ID = process.env.SUPPORT_CLAW_KITCHEN_SINK_PLUGIN_ID || "supportClaw-kitchen-sink-fixture";
...
Runs the external Kitchen Sink plugin RPC walk against a built SupportClaw entry.

Environment:
  SUPPORT_CLAW_ENTRY                         Built SupportClaw entrypoint. Defaults to dist/index.mjs or dist/index.js.
  SUPPORT_CLAW_KITCHEN_SINK_NPM_SPEC         Plugin package spec. Default: npm:@supportclaw/kitchen-sink@latest.
  SUPPORT_CLAW_KITCHEN_SINK_PLUGIN_ID        Plugin id. Default: supportClaw-kitchen-sink-fixture.
  SUPPORT_CLAW_KITCHEN_SINK_PERSONALITY      Plugin fixture personality. Default: conformance.
  SUPPORT_CLAW_KITCHEN_SINK_RPC_PORT         Gateway loopback port. Default: OS-selected free port.
  SUPPORT_CLAW_KITCHEN_SINK_RPC_READY_MS     Gateway readiness timeout.
  SUPPORT_CLAW_KITCHEN_SINK_RPC_COMMAND_MS   SupportClaw command timeout.
  SUPPORT_CLAW_KITCHEN_SINK_RPC_INSTALL_MS   Plugin install timeout.
  SUPPORT_CLAW_KITCHEN_SINK_RPC_CALL_MS      RPC call timeout.
  SUPPORT_CLAW_KITCHEN_SINK_RPC_FETCH_MS     HTTP readiness probe timeout.
  SUPPORT_CLAW_KITCHEN_SINK_RPC_FETCH_BODY_BYTES  HTTP readiness probe response ceiling.
  SUPPORT_CLAW_KITCHEN_SINK_MAX_RSS_MIB      Gateway RSS ceiling.
  SUPPORT_CLAW_KITCHEN_SINK_COMMAND_MAX_RSS_MIB  Install/CLI command RSS ceiling.
  SUPPORT_CLAW_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS  Per-command stdout/stderr capture ceiling.
  SUPPORT_CLAW_KITCHEN_SINK_KEEP_TMP=1       Preserve the isolated temp home.
`;
}
...
export function resolveKitchenSinkRpcConfig(env = process.env) {
  const commandTimeoutMs = readPositiveTimerMs(
    env.SUPPORT_CLAW_KITCHEN_SINK_RPC_COMMAND_MS,
    DEFAULT_COMMAND_TIMEOUT_MS,
    "SUPPORT_CLAW_KITCHEN_SINK_RPC_COMMAND_MS",
  );
  return {
    commandMaxRssMiB: readPositiveInt(
      env.SUPPORT_CLAW_KITCHEN_SINK_COMMAND_MAX_RSS_MIB,
      DEFAULT_MAX_COMMAND_RSS_MIB,
      "SUPPORT_CLAW_KITCHEN_SINK_COMMAND_MAX_RSS_MIB",
    ),
    commandTimeoutMs,
    fetchBodyMaxBytes: readPositiveInt(
      env.SUPPORT_CLAW_KITCHEN_SINK_RPC_FETCH_BODY_BYTES,
      DEFAULT_FETCH_BODY_MAX_BYTES,
      "SUPPORT_CLAW_KITCHEN_SINK_RPC_FETCH_BODY_BYTES",
    ),
    fetchTimeoutMs: readPositiveTimerMs(
      env.SUPPORT_CLAW_KITCHEN_SINK_RPC_FETCH_MS,
      DEFAULT_FETCH_TIMEOUT_MS,
      "SUPPORT_CLAW_KITCHEN_SINK_RPC_FETCH_MS",
    ),
    installTimeoutMs: readPositiveTimerMs(
      env.SUPPORT_CLAW_KITCHEN_SINK_RPC_INSTALL_MS,
      Math.max(commandTimeoutMs, DEFAULT_INSTALL_TIMEOUT_MS),
      "SUPPORT_CLAW_KITCHEN_SINK_RPC_INSTALL_MS",
    ),
    maxRssMiB: readPositiveInt(
      env.SUPPORT_CLAW_KITCHEN_SINK_MAX_RSS_MIB,
      DEFAULT_MAX_RSS_MIB,
      "SUPPORT_CLAW_KITCHEN_SINK_MAX_RSS_MIB",
    ),
    outputCaptureChars: readPositiveInt(
      env.SUPPORT_CLAW_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS,
      DEFAULT_OUTPUT_CAPTURE_CHARS,
      "SUPPORT_CLAW_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS",
    ),
    readyTimeoutMs: readPositiveTimerMs(
      env.SUPPORT_CLAW_KITCHEN_SINK_RPC_READY_MS,
      DEFAULT_READY_TIMEOUT_MS,
      "SUPPORT_CLAW_KITCHEN_SINK_RPC_READY_MS",
    ),
    rpcTimeoutMs: readPositiveTimerMs(
      env.SUPPORT_CLAW_KITCHEN_SINK_RPC_CALL_MS,
      DEFAULT_RPC_TIMEOUT_MS,
      "SUPPORT_CLAW_KITCHEN_SINK_RPC_CALL_MS",
    ),
  };
}
...
export async function resolveKitchenSinkRpcPort(env = process.env, options = {}) {
  const rawPort = (env.SUPPORT_CLAW_KITCHEN_SINK_RPC_PORT || "").trim();
  if (rawPort) {
    const port = readPositiveInt(rawPort, 0, "SUPPORT_CLAW_KITCHEN_SINK_RPC_PORT");
    if (port > 65535) {
      throw new Error(
        `SUPPORT_CLAW_KITCHEN_SINK_RPC_PORT must be a TCP port from 1 to 65535. Got: ${JSON.stringify(rawPort)}`,
      );
    }
    return port;
  }
...
function resolveSupportClawRunner() {
  if (process.env.SUPPORT_CLAW_ENTRY) {
    return {
      command: "node",
      baseArgs: [process.env.SUPPORT_CLAW_ENTRY],
      label: process.env.SUPPORT_CLAW_ENTRY,
    };
  }
  ...
  return { pnpm: true, baseArgs: ["supportClaw"], label: "pnpm supportClaw" };
}

export function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      SUPPORT_CLAW_SKIP_PROVIDERS: "0",
      SUPPORT_CLAW_KITCHEN_SINK_PERSONALITY:
        process.env.SUPPORT_CLAW_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
async function runSupportClaw(runner, args, env, options = {}) {
  const config = resolveKitchenSinkRpcConfig(env);
  const command = await resolveSupportClawCommand(runner, args, env, {
    stdio: ["ignore", "pipe", "pipe"],
  });
...
async function resolveSupportClawCommand(runner, args, env, options = {}) {
  if (runner.pnpm) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = module
    ? await module.callGateway({
        config: readJson(options.env.SUPPORT_CLAW_CONFIG_PATH),
        configPath: options.env.SUPPORT_CLAW_CONFIG_PATH,
...
async function loadCallGatewayModule(runner) {
  if (!usesBuiltSupportClawEntry(runner)) {
    return null;
  }
...
export function usesBuiltSupportClawEntry(runner, cwd = process.cwd(), env = process.env) {
  if (runner?.pnpm || !runner?.baseArgs?.[0]) {
    return false;
  }
  const entry = runner.baseArgs[0];
  if (env.SUPPORT_CLAW_ENTRY && entry === env.SUPPORT_CLAW_ENTRY) {
    return true;
  }
...
function configureKitchenSink(env, port) {
  const configPath = env.SUPPORT_CLAW_CONFIG_PATH;
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.SUPPORT_CLAW_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function startGateway(runner, port, env, logPath) {
  ...
  const command = await resolveSupportClawCommand(
    runner,
...
async function samplePosixProcessTree(pid, run, commandLineNeedles) {
  ...
  try {
    ...
    const gatewayTitleMatches = descendants.filter((row) =>
      row.command.toLowerCase().includes("supportClaw-gateway"),
    );
...
export async function main() {
  const config = resolveKitchenSinkRpcConfig();
  let runner = resolveSupportClawRunner();
  const port = await resolveKitchenSinkRpcPort();
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");
  const keepTmp = process.env.SUPPORT_CLAW_KITCHEN_SINK_KEEP_TMP === "1";
  let failed = false;
  let child;
  ...
  try {
    ...
    const inspect = parseJsonOutput(
      (
        await runSupportClaw(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env, {
          ...commandResourceOptions,
    ...
    assertKitchenSinkUiDescriptors(uiDescriptors, {
      expectDescriptor: env.SUPPORT_CLAW_KITCHEN_SINK_PERSONALITY !== "conformance",
    });
