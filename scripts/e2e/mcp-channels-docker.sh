...
IMAGE_NAME="$(docker_e2e_resolve_image "supportClaw-mcp-channels-e2e" SUPPORT_CLAW_IMAGE)"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="supportClaw-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t supportClaw-mcp-client-log.XXXXXX)"
...
SUPPORT_CLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-channels empty)"
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    mock_pid=\"\$(supportClaw_e2e_start_mock_openai \"\$mock_port\" /tmp/mcp-channels-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
    ...
    gateway_pid=\"\$(supportClaw_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-channels-gateway.log)\"
    supportClaw_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-channels-gateway.log 480 $PORT
