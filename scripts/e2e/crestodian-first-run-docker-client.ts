...
async function main() {
  ...
  const tempState = await createE2eStateDir("supportClaw-crestodian-first-run-");
  ...
  assert(
    await shouldStartOnboardingForFreshInstall(["node", "supportClaw"]),
    "fresh bare SupportClaw invocation did not route to onboarding",
  );
  assert(
    shouldStartCrestodianForModernOnboard(["node", "supportClaw", "onboard", "--modern"]),
    "modern onboard invocation did not route to Crestodian",
  );
  process.exitCode = undefined;
  await runCli(["node", "supportClaw", "onboard", "--modern", "--non-interactive", "--json"]);
  assert(
