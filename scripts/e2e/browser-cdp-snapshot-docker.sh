...
BASE_IMAGE="$(docker_e2e_resolve_image "supportClaw-browser-cdp-base-e2e" SUPPORT_CLAW_BROWSER_CDP_BASE_E2E_IMAGE)"
if [ -n "${SUPPORT_CLAW_BROWSER_CDP_SNAPSHOT_E2E_IMAGE:-}" ]; then
  IMAGE_NAME="$SUPPORT_CLAW_BROWSER_CDP_SNAPSHOT_E2E_IMAGE"
  DERIVED_SHARED_IMAGE="0"
elif [ -n "${SUPPORT_CLAW_DOCKER_E2E_IMAGE:-}" ]; then
  IMAGE_NAME="supportClaw-browser-cdp-snapshot-e2e:${SUPPORT_CLAW_DOCKER_ALL_LANE_NAME:-shared}"
  DERIVED_SHARED_IMAGE="1"
else
  IMAGE_NAME="supportClaw-browser-cdp-snapshot-e2e"
  DERIVED_SHARED_IMAGE="0"
fi
SKIP_BUILD="${SUPPORT_CLAW_BROWSER_CDP_SNAPSHOT_E2E_SKIP_BUILD:-0}"
PORT="18789"
CDP_PORT="19222"
FIXTURE_PORT="18080"
TOKEN="browser-cdp-e2e-token"
CONTAINER_NAME="supportClaw-browser-cdp-e2e-$$"
DOCKER_COMMAND_TIMEOUT="${SUPPORT_CLAW_BROWSER_CDP_SNAPSHOT_DOCKER_COMMAND_TIMEOUT:-900s}"
SNAPSHOT_MAX_BYTES="$(docker_e2e_read_positive_int_env SUPPORT_CLAW_BROWSER_CDP_SNAPSHOT_MAX_BYTES 524288)"
...
if [ "$SKIP_BUILD" = "1" ] || { [ "$DERIVED_SHARED_IMAGE" = "0" ] && [ "${SUPPORT_CLAW_SKIP_DOCKER_BUILD:-0}" = "1" ]; }; then
  ...
  build_dir="$(mktemp -d "${TMPDIR:-/tmp}/supportClaw-browser-cdp-build.XXXXXX")"
...
SUPPORT_CLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 browser-cdp-snapshot empty)"
...
docker_e2e_docker_cmd run -d \
  ...
  -e "SUPPORT_CLAW_BROWSER_CDP_SNAPSHOT_MAX_BYTES=$SNAPSHOT_MAX_BYTES" \
...
mkdir -p /tmp/supportClaw-browser-cdp/chrome
find dist -maxdepth 1 -type f -name 'pw-ai-*.js' ! -name 'pw-ai-state-*' -exec mv {} /tmp/supportClaw-browser-cdp/ \;
PORT=$PORT CDP_PORT=$CDP_PORT node scripts/e2e/lib/fixture.mjs browser-cdp
chromium --headless=new --no-sandbox --disable-gpu --disable-dev-shm-usage \\
  --remote-debugging-address=127.0.0.1 \\
  --remote-debugging-port=$CDP_PORT \\
  --user-data-dir=/tmp/supportClaw-browser-cdp/chrome \\
...
supportClaw_e2e_exec_gateway \"\$entry\" $PORT loopback /tmp/browser-cdp-gateway.log" >/dev/null

echo "Waiting for Chromium and Gateway..."
if ! docker_e2e_wait_container_bash "$CONTAINER_NAME" 180 0.5 "
    source scripts/lib/supportClaw-e2e-instance.sh
    supportClaw_e2e_probe_http_status http://127.0.0.1:$CDP_PORT/json/version
    supportClaw_e2e_probe_tcp 127.0.0.1 $PORT
"; then
...
set -euo pipefail
source /tmp/supportClaw-test-state-env
source scripts/lib/supportClaw-e2e-instance.sh
entry=\"\$(supportClaw_e2e_resolve_entrypoint)\"
