...
import type { SupportClawConfig } from "../../dist/config/types.supportClaw.js";
...
function makeParams(commandBody: string, cfg: SupportClawConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function invoke(commandBody: string, cfg: SupportClawConfig, isGroup = false): Promise<string> {
...
async function main() {
  const tempState = await createE2eStateDir("supportClaw-crestodian-");
  tempState.registerExitCleanup();
  const stateDir = tempState.stateDir;
  const configPath = process.env.SUPPORT_CLAW_CONFIG_PATH ?? path.join(stateDir, "supportClaw.json");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env SUPPORT_CLAW_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke("/crestodian create agent work workspace /tmp/supportClaw-work", cfg);
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/supportClaw-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as SupportClawConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "SUPPORT_CLAW_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/supportClaw-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/supportClaw-work",
    ),
