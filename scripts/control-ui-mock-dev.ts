// Control Ui Mock Dev script supports SupportClaw repository automation.
...
function createChatPickerScenario(): ControlUiMockGatewayScenario {
  ...
  const workspaceListCases = ["main", "alpha", "supportClaw-mock"].map((agentId) => ({
    match: { agentId },
    response: {
      agentId,
  ...
  const workspaceFileCases = ["main", "alpha", "supportClaw-mock"].flatMap((agentId) =>
  ...
  <text x="320" y="326" text-anchor="middle" font-family="ui-sans-serif, system-ui" font-size="24" fill="#f6f7f9">supportClaw session artifact</text>
</svg>`;
  const lobsterArtifact = {
    id: "artifact-supportClaw-lobster",
    type: "image",
    title: "supportClaw-lobster-preview.svg",
  ...
  return {
    assistantAgentId: "supportClaw-mock",
    assistantName: "SupportClaw mock",
    defaultAgentId: "supportClaw-mock",
...
function createMockGatewayPlugin(scenario: ControlUiMockGatewayScenario): Plugin {
  ...
  return {
    ...
    name: "supportClaw-control-ui-mock-gateway",
    transformIndexHtml(html) {
      return html.replace(
        "</head>",
        `    <script data-supportClaw-control-ui-mock-gateway>\n${initScript}\n    </script>\n  </head>`,
      );
    },
  };
}
...
const server = await createServer({
  ...
  define: {
    SUPPORT_CLAW_CONTROL_UI_BUILD_ID: JSON.stringify("mock"),
  },
  logLevel: "error",
  optimizeDeps: {
