...
function parseArgs(argv, env = process.env) {
  const opts = parseFlagArgs(
    argv,
    {
      ...
      maxWallMs: readBudgetEnvNumber("SUPPORT_CLAW_TEST_PERF_MAX_WALL_MS", env),
      baselineWallMs: readBudgetEnvNumber("SUPPORT_CLAW_TEST_PERF_BASELINE_WALL_MS", env),
      maxRegressionPct: readBudgetEnvNumber("SUPPORT_CLAW_TEST_PERF_MAX_REGRESSION_PCT", env) ?? 10,
      reportOnly: readBooleanEnv("SUPPORT_CLAW_TEST_PERF_REPORT_ONLY", env),
    },
    [
...
function main() {
  ...
  const reportPath = runVitestJsonReport({
    config: opts.config,
    prefix: "supportClaw-vitest-perf",
  });
