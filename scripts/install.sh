...
# SupportClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://supportClaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one SupportClaw."
...
resolve_supportClaw_effective_home() {
    local supportClaw_home="${SUPPORT_CLAW_HOME:-}"
    if [[ -z "$supportClaw_home" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$supportClaw_home" == "~" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$supportClaw_home" == \~/* ]]; then
        echo "${HOME}${supportClaw_home:1}"
        return
    fi
    echo "$supportClaw_home"
}
...
GUM_VERSION="${SUPPORT_CLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 SupportClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://supportClaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$SUPPORT_CLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.supportClaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_supportClaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.supportClaw-* "$npm_root"/supportClaw 2>/dev/null || true
}

extract_supportClaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_supportClaw_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/supportClaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/supportClaw"|"/usr/local/bin/supportClaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/supportClaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale supportClaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing supportClaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing SupportClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi

    ui_info "Installing SupportClaw package"
    "${cmd[@]}" >"$log" 2>&1
}
...
install_supportClaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "SupportClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*supportClaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_supportClaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_supportClaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "SupportClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an supportClaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${SUPPORT_CLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${SUPPORT_CLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((SUPPORT_CLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${SUPPORT_CLAW_NO_ONBOARD:-0}
NO_PROMPT=${SUPPORT_CLAW_NO_PROMPT:-0}
DRY_RUN=${SUPPORT_CLAW_DRY_RUN:-0}
INSTALL_METHOD=${SUPPORT_CLAW_INSTALL_METHOD:-}
SUPPORT_CLAW_VERSION=${SUPPORT_CLAW_VERSION:-latest}
USE_BETA=${SUPPORT_CLAW_BETA:-0}
GIT_DIR_DEFAULT="$(resolve_supportClaw_effective_home)/supportClaw"
GIT_DIR=${SUPPORT_CLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${SUPPORT_CLAW_GIT_UPDATE:-1}
NPM_LOGLEVEL="${SUPPORT_CLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${SUPPORT_CLAW_VERBOSE:-0}"
VERIFY_INSTALL="${SUPPORT_CLAW_VERIFY_INSTALL:-0}"
SUPPORT_CLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
SupportClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://supportClaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/supportClaw)
...
Environment variables:
  SUPPORT_CLAW_INSTALL_METHOD=git|npm
  SUPPORT_CLAW_VERSION=latest|next|<semver>|<spec>
  SUPPORT_CLAW_BETA=0|1
  SUPPORT_CLAW_GIT_DIR=...
  SUPPORT_CLAW_GIT_UPDATE=0|1
  SUPPORT_CLAW_NO_PROMPT=1
  SUPPORT_CLAW_VERIFY_INSTALL=1
  SUPPORT_CLAW_DRY_RUN=1
  SUPPORT_CLAW_NO_ONBOARD=1
  SUPPORT_CLAW_VERBOSE=1
  SUPPORT_CLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://supportClaw.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://supportClaw.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://supportClaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://supportClaw.ai/install.sh | bash -s -- --install-method git --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://supportClaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected SupportClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a SupportClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_supportClaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"supportClaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://supportClaw.ai/install.sh | bash"
}
    ...
    for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
        if [[ -f "$rc" ]]; then
            if [[ "$(sed -n '1p' "$rc")" != "$path_line" ]]; then
                local tmp_rc="${rc}.supportClaw-tmp"
                {
                    printf '%s\n' "$path_line"
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future SupportClaw updates; use npm i -g supportClaw@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_supportClaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/supportClaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/supportClaw" ]]; then
        ln -sf "$npm_root/supportClaw/dist/entry.js" "${npm_bin}/supportClaw"
        ui_info "Created supportClaw bin link at ${npm_bin}/supportClaw"
    fi
    return 0
}

# Check for existing SupportClaw installation
check_existing_supportClaw() {
    if [[ -n "$(type -P supportClaw 2>/dev/null || true)" ]]; then
        ui_info "Existing SupportClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_supportClaw_ref() {
    local requested="${SUPPORT_CLAW_VERSION:-latest}"
    local resolved_version=""

    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "supportClaw" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_supportClaw_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${SUPPORT_CLAW_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_supportClaw_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/supportClaw"
        ...
        local version=""
        version="$(supportClaw_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_supportClaw_npm_root_candidates)
}

warn_duplicate_supportClaw_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_supportClaw_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple SupportClaw global installs detected"
    echo "  Different Node/npm environments can run different SupportClaw versions."

    local active_node active_npm active_supportClaw
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_supportClaw="${SUPPORT_CLAW_BIN:-}"
    if [[ -z "$active_supportClaw" ]]; then
        active_supportClaw="$(type -P supportClaw 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active supportClaw: ${INFO}${active_supportClaw:-none}${NC}"
    echo ""
    echo "  Found installs:"
    ...
    echo "    npm uninstall -g supportClaw"
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make supportClaw show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

supportClaw_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "supportClaw"
        return 0
...
bounded_probe_output() {
    local label="$1"
    shift
    local timeout_seconds="${SUPPORT_CLAW_INSTALL_PROBE_TIMEOUT_SECONDS:-5}"
...
warn_supportClaw_not_found() {
    ui_warn "Installed, but supportClaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t supportClaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named supportClaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_supportClaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P supportClaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/supportClaw" ]]; then
        echo "${npm_bin}/supportClaw"
        return 0
    fi
...
install_supportClaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/brahminib/support-claw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing SupportClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing SupportClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        ...
        run_quiet_step "Cloning SupportClaw" git clone "$repo_url" "$repo_dir"
    fi

    local git_ref
    git_ref="$(resolve_git_supportClaw_ref)"
    if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
        ui_info "Using git ref: ${git_ref}"
        checkout_git_supportClaw_ref "$repo_dir" "$git_ref"
    else
    ...
    run_quiet_step "Building SupportClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/supportClaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/supportClaw"
    ui_success "SupportClaw wrapper installed to \$HOME/.local/bin/supportClaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view supportClaw dist-tags.beta 2>/dev/null || true)"
...
is_supportClaw_source_package_install_spec() {
    local value="${1:-}"
    local normalized_value=""
    normalized_value="$(to_lowercase_ascii "$value")"
    normalized_value="${normalized_value#supportClaw@}"

    [[ "$normalized_value" == "main" ]] && return 0
    [[ "$normalized_value" =~ ^github:supportClaw/supportClaw($|[#/]) ]] && return 0

    normalized_value="${normalized_value#git+}"
    [[ "$normalized_value" =~ ^https?://github\.com/supportClaw/supportClaw(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^ssh://git@github\.com[:/]supportClaw/supportClaw(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git://github\.com/supportClaw/supportClaw(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git@github\.com:supportClaw/supportClaw(\.git)?($|[?#]) ]] && return 0
    return 1
}
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:supportClaw/supportClaw#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_supportClaw() {
    local package_name="supportClaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            SUPPORT_CLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="supportClaw"
        else
            SUPPORT_CLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${SUPPORT_CLAW_VERSION}" ]]; then
        SUPPORT_CLAW_VERSION="latest"
    fi

    if is_supportClaw_source_package_install_spec "${SUPPORT_CLAW_VERSION}"; then
        ui_error "npm installs do not support SupportClaw GitHub source targets like '${SUPPORT_CLAW_VERSION}'."
    ...
    local resolved_version=""
    if can_resolve_registry_package_version "${SUPPORT_CLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${SUPPORT_CLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing SupportClaw v${resolved_version}"
    else
        ui_info "Installing SupportClaw (${SUPPORT_CLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${SUPPORT_CLAW_VERSION}")"

    if ! install_supportClaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_supportClaw_paths
        install_supportClaw_npm "${install_spec}"
    fi

    if [[ "${SUPPORT_CLAW_VERSION}" == "latest" && "${package_name}" == "supportClaw" ]]; then
        if ! resolve_supportClaw_bin &> /dev/null; then
            ui_warn "npm install supportClaw@latest failed; retrying supportClaw@next"
            cleanup_npm_supportClaw_paths
            install_supportClaw_npm "supportClaw@next"
        fi
    fi

    ensure_supportClaw_bin_link || true

    ui_success "SupportClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${SUPPORT_CLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_supportClaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (supportClaw not on PATH yet)"
        warn_supportClaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${SUPPORT_CLAW_PROFILE:-default}"
    local effective_home
    effective_home="$(resolve_supportClaw_effective_home)"
    if [[ "${profile}" != "default" ]]; then
        echo "${effective_home}/.supportClaw/workspace-${profile}"
    else
        echo "${effective_home}/.supportClaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local effective_home
    effective_home="$(resolve_supportClaw_effective_home)"
    local config_path="${SUPPORT_CLAW_CONFIG_PATH:-$effective_home/.supportClaw/supportClaw.json}"
    local legacy_config_path="${HOME}/.supportClaw/supportClaw.json"
    local legacy_clawdbot_path="${HOME}/.clawdbot/clawdbot.json"
    if [[ -f "${config_path}" || -f "$effective_home/.clawdbot/clawdbot.json" ]]; then
        return
    fi
    if [[ -z "${SUPPORT_CLAW_CONFIG_PATH:-}" && "${effective_home}" != "${HOME}" ]]; then
    ...
    if ! is_promptable; then
        local user_claw
        user_claw="$(supportClaw_command_for_user "${SUPPORT_CLAW_BIN:-}")"
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but supportClaw not on PATH; skipping onboarding"
        warn_supportClaw_not_found
        return
    fi
    ...
    "$claw" onboard || {
        local user_claw
        user_claw="$(supportClaw_command_for_user "$claw")"
...
if ! declare -F extract_supportClaw_semver >/dev/null 2>&1; then
...
resolve_supportClaw_version() {
    ...
    local claw="${SUPPORT_CLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v supportClaw &> /dev/null; then
        claw="$(command -v supportClaw)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_supportClaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/supportClaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/supportClaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$(bounded_probe_output "supportClaw daemon status --json" "$claw" daemon status --json || true)"
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: supportClaw not on PATH yet"
        warn_supportClaw_not_found
        return 1
    fi

    run_quiet_step "Checking SupportClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: supportClaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_supportClaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found SupportClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set SUPPORT_CLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_supportClaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing SupportClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g supportClaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g supportClaw 2>/dev/null || true
        ...
        install_supportClaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/supportClaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/supportClaw"
    ...
    SUPPORT_CLAW_BIN="$(resolve_supportClaw_bin || true)"
    warn_duplicate_supportClaw_global_installs || true
    ...
    installed_version=$(resolve_supportClaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 SupportClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 SupportClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/supportClaw"
        ui_kv "Update command" "supportClaw update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://supportClaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            ui_info "Running supportClaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                SUPPORT_CLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                SUPPORT_CLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                SUPPORT_CLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
        ...
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if is_promptable; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (supportClaw not on PATH yet)"
                    warn_supportClaw_not_found
                    return 0
                fi
                exec </dev/tty
    ...
    if command -v supportClaw &> /dev/null; then
        local claw="${SUPPORT_CLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_supportClaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (supportClaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if SUPPORT_CLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: supportClaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${SUPPORT_CLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_install_stage_total
    configure_verbose
    main
