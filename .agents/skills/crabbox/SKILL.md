---
name: crabbox
description: Use the Crabbox wrapper for SupportClaw remote validation across Linux, macOS, Windows, and WSL2, including delegated Blacksmith Testbox proof. Report the actual provider and id.
---

# Crabbox

Use the Crabbox wrapper when SupportClaw needs remote Linux proof for broad tests,
...
For SupportClaw maintainer broad `pnpm` gates, Blacksmith Testbox through the
...
comparison. Use `--provider blacksmith-testbox` when the task needs SupportClaw
...
- SupportClaw scripts prefer `../crabbox/bin/crabbox` when present. The user PATH
  shim can be stale.
...
- The brokered AWS default is a Linux developer image in `eu-west-1`; the repo
  ...
  region/AZ placement, and FSR state before blaming SupportClaw.
- For broad SupportClaw maintainer `pnpm` gates, prefer the repo wrapper with
...
- Do not treat inherited shell env as operator intent. In particular,
  `SUPPORT_CLAW_LOCAL_CHECK_MODE=throttled` from the local shell is not permission
  to move broad `pnpm check:changed`, `pnpm test:changed`, full `pnpm test`, or
  lint/typecheck fan-out onto the laptop.
- Only use `SUPPORT_CLAW_LOCAL_CHECK_MODE=throttled|full` when the user explicitly
...
Use these only when the task needs an existing non-Linux host. SupportClaw broad
...
Do not silently substitute AWS macOS for normal SupportClaw Linux proof. Report
...
Use this for SupportClaw maintainer broad/heavy `pnpm` gates when the prepared CI
...
node scripts/crabbox-wrapper.mjs run \
  --provider blacksmith-testbox \
  --cache-volume pnpm-store=supportClaw-node24-pnpm-lock:/tmp/supportClaw-pnpm-store \
  --timing-json \
  -- \
  corepack pnpm check:changed
```
...
install SupportClaw as the user would, run the same setup/update/CLI/Gateway/API
...
- Prefer `SUPPORT_CLAW_CURRENT_PACKAGE_TGZ` with Docker/package lanes when testing a
...
- Prefer deterministic arrow navigation over search typing for Clack-style
  ...
  sequences.
- Isolate mutable state with `SUPPORT_CLAW_STATE_DIR=$(mktemp -d)`. Plugin npm
  installs live under that state dir (`npm/node_modules/...`), not under
  `SUPPORT_CLAW_CONFIG_DIR`. Verify downloads by checking the state dir, package
  lock, and installed package metadata.
- To test automatic setup installs against local package artifacts, use
  `SUPPORT_CLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1` plus
  `SUPPORT_CLAW_PLUGIN_INSTALL_OVERRIDES='{"plugin-id":"npm-pack:/tmp/plugin.tgz"}'`.
  Pack with `npm pack`, set an isolated `SUPPORT_CLAW_STATE_DIR`, and verify the
...
- For OpenAI/Codex onboarding proof, the useful markers are the UI line
  `Installed Codex plugin`, `npm/node_modules/@supportclaw/codex`, and the
...
- Bad local config: inspect `.crabbox.yaml`, `crabbox config show`, and
  `crabbox whoami`; normal SupportClaw proof should use brokered AWS without
...
```sh
blacksmith auth login --non-interactive --organization supportClaw
```
...
Use AWS for normal SupportClaw remote proof. The repo `.crabbox.yaml` already
...
```sh
brew install supportClaw/tap/crabbox
...
- If broker auth is missing, run `crabbox login --url https://crabbox.supportClaw.ai --provider aws`.
- If the CLI asks for `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, or AWS
  profile setup during normal SupportClaw validation, assume the agent selected
...
- Ask for AWS keys only for explicit direct-provider/account administration,
  not for normal brokered SupportClaw proof.
- Trusted automation may still use
  `printf '%s' "$CRABBOX_COORDINATOR_TOKEN" | crabbox login --url https://crabbox.supportClaw.ai --provider aws --token-stdin`.
...
for SupportClaw lanes. Let that config drive normal validation.
...
Do not add SupportClaw-specific setup to Crabbox itself. Put repo setup in the
