...
1. Reproduce the failing shape first.
   ...
   - `pnpm canvas:a2ui:bundle && SUPPORT_CLAW_TEST_MEMORY_TRACE=1 SUPPORT_CLAW_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 SUPPORT_CLAW_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap SUPPORT_CLAW_TEST_WORKERS=2 SUPPORT_CLAW_TEST_MAX_OLD_SPACE_SIZE_MB=6144 pnpm test`
   - Keep `SUPPORT_CLAW_TEST_MEMORY_TRACE=1` enabled so the wrapper prints per-file RSS summaries alongside the snapshots.
