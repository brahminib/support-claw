...
```bash
SUPPORT_CLAW_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
...
```bash
SUPPORT_CLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
...
- That item is the first place to look for:
  ...
  - `SUPPORT_CLAW_QA_PROVIDER_MODE`
  - `SUPPORT_CLAW_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `OpenClaw`
  - item: `SUPPORT_CLAW_QA_CONVEX_SITE_URL`
  - item: `SUPPORT_CLAW_QA_CONVEX_SECRET_MAINTAINER`
  - item: `SUPPORT_CLAW_QA_CONVEX_SECRET_CI`
...
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `OpenClaw/SUPPORT_CLAW_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
SUPPORT_CLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
SUPPORT_CLAW_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
SUPPORT_CLAW_QA_PROVIDER_MODE="mock-openai" \
SUPPORT_CLAW_NPM_TELEGRAM_PACKAGE_SPEC="openclaw@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `SUPPORT_CLAW_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `SUPPORT_CLAW_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `SUPPORT_CLAW_QA_PROVIDER_MODE`, map it explicitly to `SUPPORT_CLAW_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    ...
    - `SUPPORT_CLAW_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `SUPPORT_CLAW_HOME` sandboxed.
