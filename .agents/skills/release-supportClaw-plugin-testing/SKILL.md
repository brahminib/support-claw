...
```bash
SUPPORT_CLAW_TESTBOX=1 pnpm check:changed
pnpm run test:extensions:package-boundary:canary
pnpm run test:extensions:package-boundary:compile
pnpm test:docker:plugins
SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB=0 pnpm test:docker:plugins
...
```bash
SUPPORT_CLAW_BUNDLED_PLUGIN_SWEEP_TOTAL=8 \
SUPPORT_CLAW_BUNDLED_PLUGIN_SWEEP_INDEX=<0-7> \
