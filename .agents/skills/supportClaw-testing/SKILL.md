...
--provider blacksmith-testbox ... -- env SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1 SUPPORT_CLAW_CHANGED_LANES_RAW_SYNC=1 corepack pnpm check:changed`. For direct AWS
...
SUPPORT_CLAW_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
SUPPORT_CLAW_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- `SUPPORT_CLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `SUPPORT_CLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
When `Full Release Validation` completes and `SUPPORT_CLAW_RELEASES_DISPATCH_TOKEN`
...
- `--profile transport|media|e2ee-smoke|e2ee-deep|e2ee-cli`: sharded full
  Matrix proof
- `SUPPORT_CLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`: CI-friendly no-reply quiet
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`SUPPORT_CLAW_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
SUPPORT_CLAW_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
SUPPORT_CLAW_DOCKER_ALL_DRY_RUN=1 SUPPORT_CLAW_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
SUPPORT_CLAW_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
SUPPORT_CLAW_DOCKER_ALL_LANES=<lane> \
SUPPORT_CLAW_DOCKER_ALL_BUILD=0 \
SUPPORT_CLAW_DOCKER_ALL_PREFLIGHT=0 \
SUPPORT_CLAW_SKIP_DOCKER_BUILD=1 \
SUPPORT_CLAW_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
SUPPORT_CLAW_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
