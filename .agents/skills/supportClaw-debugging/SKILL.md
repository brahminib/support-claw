...
```bash
SUPPORT_CLAW_DEBUG_MODEL_TRANSPORT=1 openclaw gateway
SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=tools SUPPORT_CLAW_DEBUG_SSE=events openclaw gateway
SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=full-redacted SUPPORT_CLAW_DEBUG_SSE=peek openclaw gateway
```

Useful flags:

- `SUPPORT_CLAW_DEBUG_MODEL_TRANSPORT=1`: request start, fetch response, SDK
  headers, first SSE event, stream done, and transport errors at `info`.
- `SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=summary`: bounded payload summary.
- `SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=tools`: all model-facing tool names.
- `SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=full-redacted`: capped, redacted JSON payload.
  Use only while debugging; prompts/message text may still appear.
- `SUPPORT_CLAW_DEBUG_SSE=events`: first-event and stream-completion timing.
- `SUPPORT_CLAW_DEBUG_SSE=peek`: first five redacted SSE events.
- `SUPPORT_CLAW_DEBUG_CODE_MODE=1`: code-mode tool-surface diagnostics.
