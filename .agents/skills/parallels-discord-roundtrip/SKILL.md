...
```bash
export SUPPORT_CLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.supportClaw/supportClaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env SUPPORT_CLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `supportClaw` wrapper for guest `message send/read`; `node supportClawCli.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/supportClaw-parallels-smoke.*`
...
- host inbound nonce appears in `supportClaw message read` output
