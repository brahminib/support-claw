...
- Gateway Service Lifecycle: Per-user Gateway LaunchAgent install, launchctl bootstrap, LaunchAgent labels, Gateway token/env handling, App-managed LaunchAgent handoff, supportClaw update package/git handoff, Managed service refresh, Stale updater launchd job detection, supportClaw uninstall, Stranded service recovery
- Diagnostics and Observability: LaunchAgent log paths, supportClaw gateway status --deep, Gateway silently stops responding, Stale updater jobs
