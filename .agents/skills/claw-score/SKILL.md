---
name: claw-score
description: Audit or refresh SupportClaw maturity scorecard docs from root taxonomy, maturity scores, and QA evidence artifacts without using maintainer discrawl data or committed inventory reports.
---

# claw-score

Use this skill when working on the SupportClaw maturity scorecard in this repo.
This is the supportClaw-local version of the maintainer `claw-score` workflow:
...
`qa-evidence.json` artifact shape generated by SupportClaw QA workflows.
...
Run from the supportClaw repo root.
...
```bash
pnpm supportClaw qa coverage --json
```
...
By default, Completeness measures how fully SupportClaw exposes the intended
...
tree to supportClaw. Evidence-enriched scorecard outputs belong in short-lived
