...
- In maintainer Testbox mode, use `SUPPORT_CLAW_TESTBOX=1 scripts/pr prepare-run
<PR>` only after the exact PR head has passed `CI` and every scheduled
  ...
  `SUPPORT_CLAW_TESTBOX=1 scripts/pr prepare-sync-head <PR>`, wait for those hosted
...
target_ref=<full-pr-sha> -f include_android=true -f release_gate=true`.
  ...
  cancelled, or been skipped. Then rerun `SUPPORT_CLAW_TESTBOX=1 scripts/pr
prepare-run <PR>`.
...
```bash
  SUPPORT_CLAW_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```
...
- Use `pnpm test:live:media video` for bounded video-provider smoke when video
  ...
  each provider operation with `SUPPORT_CLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS`
  (`180000` by default).
...
- Set `SUPPORT_CLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` only when intentionally
...
- Post-published beta verification roster:
  ...
  - published npm Telegram proof: dispatch Actions > `NPM Telegram Beta E2E`
    ...
    `SUPPORT_CLAW_NPM_TELEGRAM_PACKAGE_SPEC` and Convex CI env only as a fallback
    or debugging path.
...
- For beta-style full e2e batteries, hard-cap top-level long lanes instead of letting them run indefinitely. Use host `timeout --foreground`/`gtimeout --foreground` caps such as:
  - `45m` for `SUPPORT_CLAW_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  ...
  automatically when `SUPPORT_CLAW_PUBLIC_REPO_RELEASE_TOKEN` is present in the
