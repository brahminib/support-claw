...
test:extensions:batch <plugin[,plugin...]>` or plugin-inspector command
   ...
   - For import-heavy suspicion add:
     `SUPPORT_CLAW_VITEST_IMPORT_DURATIONS=1 SUPPORT_CLAW_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
4. For broad or package-heavy plugin proof, use Crabbox-backed Blacksmith
   ...
   - `pnpm crabbox:run -- --provider blacksmith-testbox --timing-json -- SUPPORT_CLAW_TESTBOX=1 pnpm test:extensions:batch <ids>`
...
| import duration | broad barrel/runtime loads         | `SUPPORT_CLAW_VITEST_IMPORT_DURATIONS=1`                                        |
...
```bash
SUPPORT_CLAW_TEST_MEMORY_TRACE=1 \
SUPPORT_CLAW_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 \
SUPPORT_CLAW_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap \
SUPPORT_CLAW_TEST_WORKERS=2 \
SUPPORT_CLAW_TEST_MAX_OLD_SPACE_SIZE_MB=6144 \
pnpm test
```
...
- Parallel Vitest runs sharing `node_modules/.experimental-vitest-cache` without
  distinct `SUPPORT_CLAW_VITEST_FS_MODULE_CACHE_PATH` values.
...
timeout 240 /usr/bin/time -l env \
  SUPPORT_CLAW_VITEST_IMPORT_DURATIONS=1 \
  SUPPORT_CLAW_VITEST_PRINT_IMPORT_BREAKDOWN=1 \
