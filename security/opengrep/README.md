...
`precise.yml` is SupportClaw's shipped precise OpenGrep rulepack. Each rule is tied
...
```bash
# from the supportClaw repo root
...
Some SupportClaw paths are excluded by `.gitignore` for build reasons even though
