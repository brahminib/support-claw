...
const COMPILE_CACHE_DISABLED_RESPAWNED_ENV = "SUPPORT_CLAW_COMPILE_CACHE_DISABLED_RESPAWNED";
...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `supportClaw: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[supportClaw] Failed to respawn launcher: ${
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.SUPPORT_CLAW_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    SUPPORT_CLAW_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["supportClaw: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:supportClaw/supportClaw#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g supportClaw@latest`.");
  return lines.join("\n");
};
...
const hasLauncherContainerTarget = (argv) => {
  if (normalizeLauncherMetadataValue(process.env.SUPPORT_CLAW_CONTAINER)) {
    return true;
  }
...
const isHelpFastPathDisabled = () =>
  process.env.SUPPORT_CLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.SUPPORT_CLAW_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.SUPPORT_CLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.SUPPORT_CLAW_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "supportClaw.json"), path.join(stateDir, "clawdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".supportClaw", "supportClaw.json"),
    path.join(homeDir, ".supportClaw", "clawdbot.json"),
    path.join(homeDir, ".clawdbot", "supportClaw.json"),
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    ...
    process.stdout.write(commit ? `SupportClaw ${version} (${commit})\n` : `SupportClaw ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.SUPPORT_CLAW_BUNDLED_VERSION) ?? "0.0.0";
}
