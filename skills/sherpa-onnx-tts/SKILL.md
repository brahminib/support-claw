...
1. Download the runtime for your OS (extracts into `$SUPPORT_CLAW_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.supportClaw/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$SUPPORT_CLAW_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.supportClaw/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active SupportClaw config file (`$SUPPORT_CLAW_CONFIG_PATH`, default `~/.supportClaw/supportClaw.json`):
