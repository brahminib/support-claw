...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${SUPPORT_CLAW_CONFIG_PATH:-${SUPPORT_CLAW_STATE_DIR:-$HOME/.supportClaw}/supportClaw.json}"
...
```text
${SUPPORT_CLAW_STATE_DIR:-$HOME/.supportClaw}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with supportClaw message send if route provided.
```
