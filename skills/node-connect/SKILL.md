---
name: node-connect
description: "Diagnose SupportClaw Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify SupportClaw is advertising that route, then fix pairing/auth.
...
- whether `supportClaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `supportClaw qr --json`. It uses the same setup-code payload Android scans.

```bash
supportClaw config get gateway.mode
supportClaw config get gateway.bind
supportClaw config get gateway.tailscale.mode
supportClaw config get gateway.remote.url
supportClaw config get gateway.auth.mode
supportClaw config get gateway.auth.allowTailscale
supportClaw config get plugins.entries.device-pair.config.publicUrl
supportClaw qr --json
supportClaw devices list
supportClaw nodes status
```

If this SupportClaw instance is pointed at a remote gateway, also run:

```bash
supportClaw qr --remote --json
```
...
`supportClaw qr --json` success means:
...
If `supportClaw qr --json` says `Gateway is only bound to loopback`:
...
```bash
supportClaw devices list
supportClaw devices approve --latest   # preview only; copy the requestId from output
...
- `supportClaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run supportClaw qr again, rescan, then approve the pending device pairing.`
