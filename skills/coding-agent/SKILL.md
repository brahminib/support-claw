...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.supportClaw`, `$SUPPORT_CLAW_STATE_DIR`, or active SupportClaw state dirs.
...
- Worker must send completion/failure via `supportClaw message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/supportClaw`; use an isolated checkout.
...
supportClaw message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use supportClaw system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t supportClaw-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
<notification block>
EOF
