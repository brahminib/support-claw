---
summary: "Frequently asked questions about SupportClaw setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/supportClaw/supportClaw-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   supportClaw health --json
   supportClaw health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  ...
  <Accordion title="What are the top five everyday use cases for SupportClaw?">
  ...
  <Accordion title="Can SupportClaw help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    SupportClaw draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    SupportClaw is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use SupportClaw when you
    ...
    Showcase: [https://supportClaw.ai/showcase](https://supportClaw.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.supportClaw/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.supportClaw/supportClaw.json`). Precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.supportClaw/skills` → bundled → `skills.load.extraDirs`, so managed overrides still win over bundled skills without touching git. If you need the skill installed globally but only visible to some agents, keep the shared copy in `~/.supportClaw/skills` and control visibility with `agents.defaults.skills` and `agents.list[].skills`. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.supportClaw/supportClaw.json` (lowest precedence). Default precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.supportClaw/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which SupportClaw treats as `<workspace>/skills` on the next session. If the skill should only be visible to certain agents, pair that with `agents.defaults.skills` or `agents.list[].skills`.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, SupportClaw falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, SupportClaw intentionally suppresses the announce instead of posting stale earlier progress.
    - Tool/toolResult output is not promoted into child result text; the result is the child's latest visible assistant reply.

    Debug:

    ```bash
    supportClaw tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `SUPPORT_CLAW_SKIP_CRON` is not set.
    ...
    ```bash
    supportClaw cron run <jobId>
    supportClaw cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `supportClaw skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.ai](https://clawhub.ai).

    ```bash
    supportClaw skills search "calendar"
    supportClaw skills search --limit 20
    supportClaw skills install <skill-slug>
    supportClaw skills install <skill-slug> --version <version>
    supportClaw skills install <skill-slug> --force
    supportClaw skills install <skill-slug> --global
    supportClaw skills update --all
    supportClaw skills update --all --global
    supportClaw skills list --eligible
    supportClaw skills check
    ```

    Native `supportClaw skills install` writes into the active workspace `skills/`
  ...
  <Accordion title="Can SupportClaw run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.supportClaw.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. SupportClaw can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.supportClaw/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "supportClaw": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across all local agents, use `supportClaw skills install <slug> --global` (or place them manually in `~/.supportClaw/skills/<name>/SKILL.md`). If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills), [Skills config](/tools/skills-config), and [ClawHub](/tools/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with SupportClaw?">
    ...
    supportClaw browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `SUPPORT_CLAW_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `SUPPORT_CLAW_IMAGE_APT_PACKAGES`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    SupportClaw validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
  ...
  <Accordion title="How does memory work?">
    SupportClaw memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    SupportClaw also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, SupportClaw uses OpenAI embeddings. Legacy
...
<AccordionGroup>
  <Accordion title="Is all data used with SupportClaw saved locally?">
    No - **SupportClaw's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.supportClaw` + your workspace directory).
  ...
  <Accordion title="Where does SupportClaw store its data?">
    Everything lives under `$SUPPORT_CLAW_STATE_DIR` (default: `~/.supportClaw`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$SUPPORT_CLAW_STATE_DIR/supportClaw.json`                             | Main config (JSON5)                                                |
    | `$SUPPORT_CLAW_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$SUPPORT_CLAW_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$SUPPORT_CLAW_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$SUPPORT_CLAW_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$SUPPORT_CLAW_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$SUPPORT_CLAW_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$SUPPORT_CLAW_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$SUPPORT_CLAW_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.supportClaw/agent/*` (migrated by `supportClaw doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.supportClaw/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.supportClaw`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md`, `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
      Lowercase root `memory.md` is legacy repair input only; `supportClaw doctor --fix`
      can merge it into `MEMORY.md` when both files exist.
    - **State dir (`~/.supportClaw`)**: config, channel/provider state, auth profiles, sessions, logs,
      and shared skills (`~/.supportClaw/skills`).

    Default workspace is `~/.supportClaw/workspace`, configurable via:

    ```json5
    {
      agents: { defaults: { workspace: "~/.supportClaw/workspace" } },
    }
    ```
  ...
  <Accordion title="Can I make SOUL.md bigger?">
    ...
    Change the shared defaults in your SupportClaw config:
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.supportClaw` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall SupportClaw?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The SupportClaw repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    SupportClaw reads an optional **JSON5** config from `$SUPPORT_CLAW_CONFIG_PATH` (default: `~/.supportClaw/supportClaw.json`):

    ```
    $SUPPORT_CLAW_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.supportClaw/workspace`).
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `SUPPORT_CLAW_GATEWAY_PASSWORD`) instead.
  ...
  <Accordion title="Why do I need a token on localhost now?">
    SupportClaw enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and generates a runtime-only token for that startup, so **local WS clients must authenticate**. Configure `gateway.auth.token`, `gateway.auth.password`, `SUPPORT_CLAW_GATEWAY_TOKEN`, or `SUPPORT_CLAW_GATEWAY_PASSWORD` explicitly when clients need a stable secret across restarts. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `supportClaw doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one SupportClaw.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `SUPPORT_CLAW_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `supportClaw configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, SupportClaw auto-detects the first ready fetch fallback provider from available credentials. The official Firecrawl plugin provides that fallback.
    - Daemons read env vars from `~/.supportClaw/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current SupportClaw protects many accidental clobbers:

    - SupportClaw-owned config writes validate the full post-change config before writing.
    - Invalid or destructive SupportClaw-owned writes are rejected and saved as `supportClaw.json.rejected.*`.
    - If a direct edit breaks startup or hot reload, Gateway fails closed or skips the reload; it does not rewrite `supportClaw.json`.
    - `supportClaw doctor --fix` owns repair and can restore last-known-good while saving the rejected file as `supportClaw.json.clobbered.*`.

    Recover:

    - Check `supportClaw logs --follow` for `Invalid config at`, `Config write rejected:`, or `config reload skipped (invalid config)`.
    - Inspect the newest `supportClaw.json.clobbered.*` or `supportClaw.json.rejected.*` beside the active config.
    - Run `supportClaw config validate` and `supportClaw doctor --fix`.
    - Copy only the intended keys back with `supportClaw config set` or `config.patch`.
    - If you have no last-known-good or rejected payload, restore from backup, or re-run `supportClaw doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `supportClaw config set` for small changes.
    - Use `supportClaw configure` for interactive edits.
  ...
  <Accordion title="Can the SupportClaw browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="How can my agent access my computer if the Gateway is hosted remotely?">
    ...
    5. Approve the node on the Gateway:
       ...
       ```bash
       supportClaw devices list
       supportClaw devices approve <requestId>
       ```
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `supportClaw gateway status`
    - Gateway health: `supportClaw status`
    - Channel health: `supportClaw channels status`
  ...
  <Accordion title="Can two SupportClaw instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `supportClaw agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    supportClaw agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    ```bash
    supportClaw gateway --tailscale serve
    ```
...
<AccordionGroup>
  <Accordion title="How does SupportClaw load environment variables?">
    SupportClaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.supportClaw/.env` (aka `$SUPPORT_CLAW_STATE_DIR/.env`)
    ...
    `.env` and should live in the process environment, `~/.supportClaw/.env`, or config `env`.
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.supportClaw/.env` so they're picked up even when the service doesn't inherit your shell env.
    ...
    `SUPPORT_CLAW_LOAD_SHELL_ENV=1`, `SUPPORT_CLAW_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `supportClaw models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means SupportClaw won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.supportClaw/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of SupportClaw instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset SupportClaw but keep it installed?">
    ...
    ```bash
    supportClaw onboard --install-daemon
    ```

    Notes:

    - Onboarding also offers **Reset** if it sees an existing config. See [Onboarding (CLI)](/start/wizard).
    - If you used profiles (`--profile` / `SUPPORT_CLAW_PROFILE`), reset each state dir (defaults are `~/.supportClaw-<profile>`).
    - Dev reset: `supportClaw gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    or empty checklist stubs), SupportClaw skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. SupportClaw runs on **your own account**, so if you're in the group, SupportClaw can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    supportClaw directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does SupportClaw not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.supportClaw/agents/<agentId>/sessions/`.
    ...
    - Use `supportClaw doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > SUPPORT_CLAW_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does supportClaw gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    ...
    Use `supportClaw gateway status` and trust these lines:
  ...
  <Accordion title='Why does supportClaw gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `SUPPORT_CLAW_STATE_DIR` mismatch).
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    SupportClaw enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `supportClaw gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run SupportClaw in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `supportClaw gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `supportClaw dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `supportClaw doctor --generate-gateway-token`.
    - If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
    - Shared-secret mode: set `gateway.auth.token` / `SUPPORT_CLAW_GATEWAY_TOKEN` or `gateway.auth.password` / `SUPPORT_CLAW_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `supportClaw devices list`
      - `supportClaw devices rotate --device <id> --role operator`
    ...
    - Still stuck? Run `supportClaw status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `SUPPORT_CLAW_CONFIG_PATH` (per-instance config)
    - `SUPPORT_CLAW_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `supportClaw --profile <name> ...` per instance (auto-creates `~/.supportClaw-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `supportClaw --profile <name> gateway install`.

    Profiles also suffix service names (`ai.supportClaw.<profile>`; legacy `com.supportClaw.*`, `supportClaw-gateway-<profile>.service`, `SupportClaw Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    supportClaw tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    ```
    /tmp/supportClaw/supportClaw-YYYY-MM-DD.log
    ```
    ...
    - macOS launchd stdout: `~/Library/Logs/supportClaw/gateway.log` (profiles use `gateway-<profile>.log`; stderr is suppressed)
    - Linux: `journalctl --user -u supportClaw-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "SupportClaw Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `supportClaw gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart SupportClaw?">
    There are **three Windows install modes**:

    **1) Windows Hub local setup:** the native app manages a local app-owned WSL Gateway.

    Open **SupportClaw Companion** from the Start menu or tray, then use
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `supportClaw gateway status`
    2. Is the Gateway healthy? `supportClaw status`
    3. Does the UI have the right token? `supportClaw dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    supportClaw channels status
    supportClaw channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. SupportClaw already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: supportClaw gateway restart vs supportClaw gateway">
    - `supportClaw gateway restart`: restarts the **background service** (launchd/systemd).
    - `supportClaw gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `supportClaw gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must use structured media fields such as `media`, `mediaUrl`, `path`, or `filePath`. See [SupportClaw assistant setup](/start/supportClaw) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    supportClaw message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose SupportClaw to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `supportClaw pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `supportClaw pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `supportClaw doctor` to surface risky DM policies.
  ...
  <Accordion title="Is SupportClaw less safe because it uses TypeScript/Node instead of Rust/WASM?">
    Language and runtime matter, but they are not the main risk for a personal
    agent. The practical SupportClaw risks are gateway exposure, who can message the
    ...
    - run `supportClaw security audit --deep` after config changes

    Details: [Security](/gateway/security), [Sandboxing](/gateway/sandboxing).

  </Accordion>

  <Accordion title="I saw reports about exposed SupportClaw instances. What should I check?">
    First check your actual deployment:

    ```bash
    supportClaw security audit --deep
    supportClaw gateway status
    ```
  ...
  <Accordion title="Are ClawHub skills and third-party plugins safe to install?">
    ...
    complete security boundary. SupportClaw does not run built-in local
  ...
  <Accordion title="I ran /start in Telegram but did not get a pairing code">
    ...
    ```bash
    supportClaw pairing list telegram
    ```
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. SupportClaw only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    supportClaw pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    supportClaw pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    SupportClaw blocks **cross-provider** messaging by default. If a tool call is bound
