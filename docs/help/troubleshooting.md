---
summary: "Symptom first troubleshooting hub for SupportClaw"
read_when:
  - SupportClaw is not working and you need the fastest path to a fix
...
- `supportClaw status` → shows configured channels and no obvious auth errors.
- `supportClaw status --all` → full report is present and shareable.
- `supportClaw gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `supportClaw gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `supportClaw doctor` → no blocking config/service errors.
- `supportClaw channels status --probe` → reachable gateway returns live per-account
...
- `supportClaw logs --follow` → steady activity, no repeating fatal errors.
...
and run `supportClaw status --all` again. See [Tools](/tools) for the profile
...
## Local OpenAI-compatible backend works directly but fails in SupportClaw

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `supportClaw infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on SupportClaw agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger SupportClaw prompts crash the
...
## Plugin install fails with missing supportClaw extensions

If install fails with `package.json missing supportClaw.extensions`, the plugin package
is using an old shape that SupportClaw no longer accepts.

Fix in the plugin package:

1. Add `supportClaw.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `supportClaw plugins install <package>` again.

Example:

```json
{
  "name": "@supportclaw/my-plugin",
  "version": "1.2.3",
  "supportClaw": {
...
Install policy runs on plugin installs and updates. SupportClaw-owned plugin
versions normally move with the SupportClaw release, so an SupportClaw update can
also need matching `@supportclaw/*` plugin updates during post-update sync.

Avoid these broad policy shapes unless you also maintain the matching upgrade
rule:

- Freezing SupportClaw-owned plugins to one exact old version, such as allowing
  only `@supportclaw/*@2026.5.3`.
...
- Approving plugin versions without considering the policy request's
  `supportClawVersion` and the plugin candidate metadata.

Safer policy rules allow trusted SupportClaw-owned plugin updates when the
candidate is compatible with the current SupportClaw host, instead of pinning a
single release forever. If you block npm by default, make a narrow exception
for the trusted `@supportclaw/*` plugin packages or plugin ids you use. If you
...
If the policy is intentionally strict, relax it for the trusted SupportClaw upgrade
window, rerun `supportClaw plugins update --all`, then restore the stricter rule.
...
If `supportClaw doctor`, setup, or startup warnings show:
...
them. Do not remove the plugin config. Fix the file ownership or run SupportClaw as
...
```mermaid
flowchart TD
  A[SupportClaw is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `supportClaw gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    supportClaw config get tools.exec.host
    supportClaw config get tools.exec.security
    supportClaw config get tools.exec.ask
    ...
    ```bash
    supportClaw config set tools.exec.host gateway
    supportClaw config set tools.exec.security full
    supportClaw config set tools.exec.ask off
    supportClaw gateway restart
    ```
  ...
  <Accordion title="Browser tool fails">
    ...
    - `supportClaw` starts, or `user` can see local Chrome tabs.
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `supportClaw browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
