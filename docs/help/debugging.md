...
This is handy when you need to toggle obscure settings without editing `supportClaw.json`.

Examples:

```
/debug show
/debug set messages.responsePrefix="[supportClaw]"
...
Use `SUPPORT_CLAW_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
SUPPORT_CLAW_PLUGIN_LIFECYCLE_TRACE=1 supportClaw plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm supportClaw ...`
...
`SUPPORT_CLAW_RUN_NODE_CPU_PROF_DIR`:

```bash
SUPPORT_CLAW_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm supportClaw status
```
...
```bash
SUPPORT_CLAW_TRACE_SYNC_IO=1 pnpm supportClaw gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `SUPPORT_CLAW_TRACE_SYNC_IO=1` when you explicitly want Node
...
`supportClaw-gateway-watch-main` (or a profile/port-specific variant such as
`supportClaw-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t supportClaw-gateway-watch-main
```
...
# or
SUPPORT_CLAW_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
SUPPORT_CLAW_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`SUPPORT_CLAW_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`SUPPORT_CLAW_PROFILE`, `SUPPORT_CLAW_CONFIG_PATH`, `SUPPORT_CLAW_STATE_DIR`,
`SUPPORT_CLAW_GATEWAY_PORT`, and `SUPPORT_CLAW_SKIP_CHANNELS` into the pane. Put
...
`supportClaw doctor --fix --non-interactive` once and restarts the Gateway child.
Use `SUPPORT_CLAW_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `supportClaw.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.supportClaw-dev` and
...
```bash
pnpm gateway:dev
SUPPORT_CLAW_PROFILE=dev supportClaw tui
```

If you don't have a global install yet, run the CLI via `pnpm supportClaw ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `SUPPORT_CLAW_PROFILE=dev`
   - `SUPPORT_CLAW_STATE_DIR=~/.supportClaw-dev`
   - `SUPPORT_CLAW_CONFIG_PATH=~/.supportClaw-dev/supportClaw.json`
   - `SUPPORT_CLAW_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`SUPPORT_CLAW_SKIP_CHANNELS=1`).
...
```bash
SUPPORT_CLAW_PROFILE=dev supportClaw gateway --dev --reset
```
...
## Raw stream logging (SupportClaw)

SupportClaw can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.supportClaw/logs/raw-stream.jsonl
```
...
SUPPORT_CLAW_RAW_STREAM_PATH=~/.supportClaw/logs/raw-stream.jsonl
```

Default file:

`~/.supportClaw/logs/raw-stream.jsonl`
...
```bash
SUPPORT_CLAW_RAW_STREAM_PATH=~/.supportClaw/logs/raw-openai-completions.jsonl
```

Default file:

`~/.supportClaw/logs/raw-openai-completions.jsonl`
...
- If you need to use the built SupportClaw CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./supportClawCli.mjs` or create a shell alias like `alias supportClaw-build="node $(pwd)/supportClawCli.mjs"`
