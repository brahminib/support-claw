...
An owner is a ClawHub publisher handle, such as `@alice` or `@supportclaw`.
...
`@supportclaw/dronzer`, it can only be published as `@supportclaw`. If you publish as
...
```text
Package scope "@supportclaw" must match selected owner "@vintageayu".
Publish as "@supportclaw" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-supportClaw --to opik
```
...
This protects org namespaces. A package named `@supportclaw/dronzer` claims the
`@supportclaw` namespace, so only publishers with access to the `@supportclaw` owner
can publish it.
