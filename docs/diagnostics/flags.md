...
```bash
SUPPORT_CLAW_DIAGNOSTICS=telegram.http,telegram.payload
```

Disable all flags:

```bash
SUPPORT_CLAW_DIAGNOSTICS=0
```

`SUPPORT_CLAW_DIAGNOSTICS=0` is a process-level disable override: it disables
...
```bash
SUPPORT_CLAW_DIAGNOSTICS=profiler supportClaw gateway run
```

Enable only reply-dispatch profiler spans:

```bash
SUPPORT_CLAW_DIAGNOSTICS=reply.profiler supportClaw gateway run
```

Enable only Codex app-server startup/tool/thread profiler spans:

```bash
SUPPORT_CLAW_DIAGNOSTICS=codex.profiler supportClaw gateway run
```
...
```bash
SUPPORT_CLAW_DIAGNOSTICS=0 supportClaw gateway run
```
...
SUPPORT_CLAW_DIAGNOSTICS_TIMELINE_PATH=/tmp/supportClaw-timeline.jsonl \
...
`SUPPORT_CLAW_DIAGNOSTICS_TIMELINE_PATH`. When `timeline` is enabled only from
config, the earliest config-loading spans are not emitted because SupportClaw has
not read config yet; subsequent startup spans use the config flag.

`SUPPORT_CLAW_DIAGNOSTICS=1`, `SUPPORT_CLAW_DIAGNOSTICS=all`, and
`SUPPORT_CLAW_DIAGNOSTICS=*` also enable the timeline because they enable every
diagnostics flag. Prefer `timeline` when you only want the JSONL timing
artifact.

Timeline records use the `supportClaw.diagnostics.v1` envelope. Events can include
...
```bash
ls -t /tmp/supportClaw/supportClaw-*.log | head -n 1
```

Filter for Telegram HTTP diagnostics:

```bash
rg "telegram http error" /tmp/supportClaw/supportClaw-*.log
```

Filter for Brave Search HTTP diagnostics:

```bash
rg "brave http" /tmp/supportClaw/supportClaw-*.log
```

Or tail while reproducing:

```bash
tail -f /tmp/supportClaw/supportClaw-$(date +%F).log | rg "telegram http error"
```

For remote gateways, you can also use `supportClaw logs --follow` (see [/cli/logs](/cli/logs)).
