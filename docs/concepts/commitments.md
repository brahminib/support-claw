...
read_when:
  - You want SupportClaw to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, SupportClaw can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. SupportClaw may check in afterward.
- You say you are exhausted. SupportClaw may ask later whether you slept.
- The agent says it will follow up after something changes. SupportClaw may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: SupportClaw remembers a
...
```bash
supportClaw config set commitments.enabled true
supportClaw config set commitments.maxPerDay 3
```

Equivalent `supportClaw.json`:
...
After an agent reply, SupportClaw may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, SupportClaw stores a commitment with:
...
without SupportClaw tools.

SupportClaw never delivers an inferred commitment immediately after writing it.
...
supportClaw commitments dismiss cm_abc123
```

See [`supportClaw commitments`](/cli/commitments) for the command reference.
...
Stored commitments are local SupportClaw state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
supportClaw config set commitments.enabled false
```
...
- Check `supportClaw commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`supportClaw commitments`](/cli/commitments)
