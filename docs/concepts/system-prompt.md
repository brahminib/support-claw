---
summary: "What the SupportClaw system prompt contains and how it is assembled"
read_when:
...
SupportClaw builds a custom system prompt for every agent run. The prompt is **SupportClaw-owned** and does not use a runtime default prompt.

The prompt is assembled by SupportClaw and injected into each agent run.
...
the full SupportClaw-owned prompt. The provider runtime can:
...
- **SupportClaw Control**: tells the model to prefer the `gateway` tool for
  config/restart work and to avoid inventing CLI commands.
- **SupportClaw Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to SupportClaw docs/source and when to read them.
...
SupportClaw keeps large stable content, including **Project Context**, above the
...
SupportClaw can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Memory Recall**, **SupportClaw
...
For channel auto-reply runs, SupportClaw omits the generic **Silent Replies**
...
SupportClaw keeps committed prompt snapshots for the Codex runtime happy path under
...
SupportClaw developer instructions, turn-scoped collaboration-mode instructions
when SupportClaw provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after SupportClaw sends
...
On the native Codex harness, SupportClaw avoids repeating stable workspace files
...
Codex developer instructions. The compact SupportClaw skills list is also forwarded
...
SupportClaw prompt according to their existing gates. `HEARTBEAT.md` is omitted on
...
occurs, SupportClaw can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, SupportClaw injects a compact **available skills list**
...
points to the local SupportClaw docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.supportClaw.ai](https://docs.supportClaw.ai).

The same section also includes the SupportClaw source location. Git checkouts expose the local
...
authority for SupportClaw self-knowledge before the model understands how SupportClaw works,
...
notes, and `memory_search` as instruction context or user memory rather than SupportClaw
design or implementation knowledge. If docs are silent or stale, the model should say so
and inspect source. The prompt also tells the model to run `supportClaw status` itself when
