...
read_when:
  ...
  - Explaining how SupportClaw updates one channel message while work is in progress
...
When progress drafts are enabled, SupportClaw creates one visible work-in-progress
...
That is usually enough. SupportClaw will pick an automatic one-word label, wait
...
SupportClaw sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from SupportClaw's built-in
...
SupportClaw renders only the `progress.text` in the channel progress UI. The
...
SupportClaw uses the same formatter for progress drafts and `/verbose`:
...
SupportClaw truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, SupportClaw still suppresses the older standalone
...
When the final answer is ready, SupportClaw tries to keep the chat clean:

- If the draft can safely become the final answer, SupportClaw edits it in place.
- If the channel uses native progress streaming, SupportClaw finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, SupportClaw sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, SupportClaw keeps the
