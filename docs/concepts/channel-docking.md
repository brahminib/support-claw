---
summary: "Move one SupportClaw session's reply route between linked chat channels"
...
Channel docking is call forwarding for one SupportClaw session.
...
Alice can message SupportClaw on Telegram and Discord:
...
SupportClaw keeps the current session context and changes the reply route:
...
4. Keep the same SupportClaw session, but receive future replies in Discord.
