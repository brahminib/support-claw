---
summary: "How SupportClaw manages conversation sessions"
read_when:
...
SupportClaw organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `supportClaw security audit`.
...
SupportClaw automatically bounds session storage over time. By default, it runs
...
`supportClaw sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `supportClaw sessions cleanup --dry-run`.

## Inspecting sessions

- `supportClaw status` -- session store path and recent activity.
- `supportClaw sessions --json` -- all sessions (filter with `--active <minutes>`).
