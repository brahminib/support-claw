...
The private QA stack is meant to exercise SupportClaw in a more realistic,
...
Every QA flow runs under `pnpm supportClaw qa <subcommand>`. Many have `pnpm qa:*`
...
| `qa suite`                                          | Run repo-backed scenarios against the QA gateway lane. Aliases: `pnpm supportClaw qa suite --runner multipass` for a disposable Linux VM.                                                                                                                                  |
| `qa coverage`                                       | Print the YAML scenario-coverage inventory (`--json` for machine output).                                                                                                                                                                                               |
| `qa parity-report`                                  | Compare two `qa-suite-summary.json` files and write the agentic parity report, or use `--runtime-axis --token-efficiency` to write Codex-vs-SupportClaw runtime parity and token-efficiency reports from one runtime-pair summary.                                         |
...
channels. When a command also needs an SupportClaw root profile, put the root
profile before the QA command:

```bash
pnpm supportClaw --profile work qa run --qa-profile smoke-ci
```
...
```bash
pnpm supportClaw qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`supportClaw.run`, `supportClaw.harness.run`, a latest GenAI semantic-convention
model-call span, `supportClaw.context.assembled`, and `supportClaw.message.delivery`
...
`supportClaw.content.*` attributes must stay out of the trace. The raw OTLP
...
```bash
pnpm supportClaw qa matrix --profile fast --fail-fast
```
...
frontier credentials, `--fast`, and `SUPPORT_CLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`.
...
pnpm supportClaw qa discord
pnpm supportClaw qa slack
pnpm supportClaw qa whatsapp
```
...
```bash
pnpm supportClaw qa mantis slack-desktop-smoke \
  --gateway-setup \
...
With `--gateway-setup`, Mantis leaves a persistent SupportClaw Slack gateway
...
```bash
pnpm supportClaw qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `supportClaw infer image describe`
...
`supportClaw/plugin-sdk/qa-live-transport-scenarios`.
...
```bash
pnpm supportClaw qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds SupportClaw
...
- `SUPPORT_CLAW_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
...
The implicit default set always covers canary, mention gating, native command replies, command addressing, and bot-to-bot group replies. `mock-openai` defaults also include deterministic reply-chain and final-message streaming checks. `telegram-current-session-status-tool` remains opt-in because it is only stable when threaded directly after canary, not after arbitrary native command replies. Use `pnpm supportClaw qa telegram --list-scenarios --provider-mode mock-openai` to print the current default/optional split with regression refs.
...
```bash
SUPPORT_CLAW_QA_CREDENTIAL_SOURCE=convex \
pnpm test:docker:npm-telegram-live
```

When `SUPPORT_CLAW_QA_CREDENTIAL_SOURCE=convex` is set, the package live wrapper
...
`SUPPORT_CLAW_NPM_TELEGRAM_RTT_SAMPLES`, `SUPPORT_CLAW_NPM_TELEGRAM_RTT_TIMEOUT_MS`,
or `SUPPORT_CLAW_NPM_TELEGRAM_RTT_MAX_FAILURES` to tune RTT measurement without
...
Targets one real private Discord guild channel with two bots: a driver bot controlled by the harness and a SUT bot started by the child SupportClaw gateway through the bundled Discord plugin. Verifies channel mention handling, that the SUT bot has registered the native `/help` command with Discord, and opt-in Mantis evidence scenarios.

Required env when `--credential-source env`:

- `SUPPORT_CLAW_QA_DISCORD_GUILD_ID`
- `SUPPORT_CLAW_QA_DISCORD_CHANNEL_ID`
- `SUPPORT_CLAW_QA_DISCORD_DRIVER_BOT_TOKEN`
- `SUPPORT_CLAW_QA_DISCORD_SUT_BOT_TOKEN`
- `SUPPORT_CLAW_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id returned by Discord (the lane fails fast otherwise).

Optional:

- `SUPPORT_CLAW_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `SUPPORT_CLAW_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for `discord-voice-autojoin`; without it, the scenario picks the first visible voice/stage channel for the SUT bot.
...
- `discord-qa-observed-messages.json` - bodies redacted unless `SUPPORT_CLAW_QA_DISCORD_CAPTURE_CONTENT=1`.
...
Targets one real private Slack channel with two distinct bots: a driver bot controlled by the harness and a SUT bot started by the child SupportClaw gateway through the bundled Slack plugin.

Required env when `--credential-source env`:

- `SUPPORT_CLAW_QA_SLACK_CHANNEL_ID`
- `SUPPORT_CLAW_QA_SLACK_DRIVER_BOT_TOKEN`
- `SUPPORT_CLAW_QA_SLACK_SUT_BOT_TOKEN`
- `SUPPORT_CLAW_QA_SLACK_SUT_APP_TOKEN`

Optional:

- `SUPPORT_CLAW_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR` enables visual approval
...
- `SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS` overrides the checkpoint
...
- `slack-qa-report.md`
- `qa-evidence.json` - evidence entries for the live transport checks.
- `slack-qa-observed-messages.json` - bodies redacted unless `SUPPORT_CLAW_QA_SLACK_CAPTURE_CONTENT=1`.
- `approval-checkpoints/` - only when Mantis sets
  `SUPPORT_CLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR`; contains checkpoint JSON,
...
{
  "display_information": {
    "name": "SupportClaw QA Driver",
    "description": "Test driver bot for SupportClaw QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "SupportClaw QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
{
  "display_information": {
    "name": "SupportClaw QA SUT",
    "description": "SupportClaw QA SUT connector for SupportClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "SupportClaw QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#supportClaw-qa`) and invite both bots from inside the channel:

```
/invite @SupportClaw QA Driver
/invite @SupportClaw QA SUT
```
...
Two options. Use env vars for single-machine debugging (set the four `SUPPORT_CLAW_QA_SLACK_*` variables and pass `--credential-source env`), or seed the shared Convex pool so CI and other maintainers can lease them.
...
With `SUPPORT_CLAW_QA_CONVEX_SITE_URL` and `SUPPORT_CLAW_QA_CONVEX_SECRET_MAINTAINER` exported in your shell, register and verify:

```bash
pnpm supportClaw qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm supportClaw qa credentials list --kind slack --status all --json
```
...
the harness and a SUT account started by the child SupportClaw gateway through the
bundled WhatsApp plugin.

Required env when `--credential-source env`:

- `SUPPORT_CLAW_QA_WHATSAPP_DRIVER_PHONE_E164`
- `SUPPORT_CLAW_QA_WHATSAPP_SUT_PHONE_E164`
- `SUPPORT_CLAW_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64`
- `SUPPORT_CLAW_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64`

Optional:

- `SUPPORT_CLAW_QA_WHATSAPP_GROUP_JID` enables group scenarios such as
  `whatsapp-mention-gating` and `whatsapp-group-allowlist-block`.
- `SUPPORT_CLAW_QA_WHATSAPP_CAPTURE_CONTENT=1` keeps message bodies in
...
`@supportclaw/whatsapp/api.js` package surface instead of reaching into private
...
- `qa-evidence.json` - evidence entries for the live transport checks.
- `whatsapp-qa-observed-messages.json` - bodies redacted unless `SUPPORT_CLAW_QA_WHATSAPP_CAPTURE_CONTENT=1`.

### Convex credential pool

Telegram, Discord, Slack, and WhatsApp lanes can lease credentials from a shared Convex pool instead of reading the env vars above. Pass `--credential-source convex` (or set `SUPPORT_CLAW_QA_CREDENTIAL_SOURCE=convex`); QA Lab acquires an exclusive lease, heartbeats it for the duration of the run, and releases it on shutdown. Pool kinds are `"telegram"`, `"discord"`, `"slack"`, and `"whatsapp"`.
...
- `mock-openai` is the scenario-aware SupportClaw mock. It remains the default
...
SupportClaw's normal channel plugins against them. `live` is reserved for real
...
- the `supportClaw qa` command root
...
- how `supportClaw qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `supportClaw qa <runner>` instead of registering a competing root command. Runner plugins should declare `qaRunners` in `supportClaw.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`. Keep `runtime-api.ts` light; lazy CLI and runner execution should stay behind separate entrypoints.
...
For the inventory of available scenarios - useful when sizing follow-up work or wiring a new transport - run `pnpm supportClaw qa coverage` (add `--json` for machine-readable output).
When choosing focused proof for a touched behavior or file path, run `pnpm supportClaw qa coverage --match <query>`.
...
```bash
pnpm supportClaw qa character-eval \
