...
SupportClaw handles inbound messages through a pipeline of session resolution, queueing, streaming, tool execution, and reasoning visibility. This page maps the path from inbound message to reply.
...
Channels can redeliver the same message after reconnects. SupportClaw keeps a
...
SupportClaw keeps that boundary explicit:
...
SupportClaw separates the **prompt body** from the **command body**:
...
SupportClaw can expose or hide model reasoning:
...
When a turn also has pending tool media, such as generated TTS audio, SupportClaw
strips the silent text but still delivers the media attachment.
SupportClaw resolves that behavior by conversation type:

- Direct conversations never receive `NO_REPLY` prompt guidance. If a direct
  run accidentally returns a bare silent token, SupportClaw suppresses it instead
...
SupportClaw also uses silent replies for generic internal runner failures in
