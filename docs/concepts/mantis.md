---
summary: "Mantis is the visual end-to-end verification system for reproducing SupportClaw bugs on live transports, capturing before and after evidence, and attaching artifacts to PRs."
title: "Mantis"
read_when:
  - Building or running live visual QA for SupportClaw bugs
...
Mantis is the SupportClaw end-to-end verification system for bugs that need a real
...
Mantis lives in the SupportClaw QA stack.

- SupportClaw owns the scenario runtime, transport adapters, evidence schema, and
  local CLI under `pnpm supportClaw qa mantis`.
...
- SupportClaw agents drive Mantis through Codex when a scenario needs agentic setup,
  debugging, or stuck-state reporting.

This boundary keeps transport knowledge in SupportClaw, machine scheduling in
...
```bash
pnpm supportClaw qa mantis discord-smoke \
...
```bash
pnpm supportClaw qa mantis run \
  --transport discord \
...
thread, calls SupportClaw's `message.thread-reply` action with a repo-local
...
```bash
pnpm supportClaw qa mantis desktop-browser-smoke \
...
`SUPPORT_CLAW_MANTIS_CRABBOX_PROVIDER` when running against another Crabbox fleet.

Useful desktop smoke flags:

- `--lease-id <cbx_...>` or `SUPPORT_CLAW_MANTIS_CRABBOX_LEASE_ID` reuses a warmed desktop.
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome user-data-dir archive from the named environment variable before launching the browser. Use this for logged-in witnesses such as Discord Web. The default env var is `SUPPORT_CLAW_MANTIS_BROWSER_PROFILE_TGZ_B64`.
...
- `--keep-lease` or `SUPPORT_CLAW_MANTIS_KEEP_VM=1` keeps a newly created passing lease open for VNC inspection. Failed runs keep the lease by default when one was created so an operator can reconnect.
...
REST. When `SUPPORT_CLAW_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also
writes a Discord Web URL artifact. When `SUPPORT_CLAW_QA_DISCORD_KEEP_THREADS=1` is
...
the VM, runs `pnpm supportClaw qa slack` inside that VM, opens Slack Web in the VNC
...
shape where the SUT SupportClaw gateway and the browser both live inside the same
Linux desktop VM.

With `--gateway-setup`, the command prepares a persistent disposable SupportClaw
home at `$HOME/.supportClaw-mantis/slack-supportClaw`, patches Slack Socket Mode
configuration for the selected channel, starts `supportClaw gateway run` on port
...
- `SUPPORT_CLAW_LIVE_OPENAI_KEY` for the remote model lane. If only
  `OPENAI_API_KEY` is set locally, Mantis maps it to `SUPPORT_CLAW_LIVE_OPENAI_KEY`
  before invoking Crabbox so Crabbox's `SUPPORT_CLAW_*` env forwarding can carry it
  into the VM.
...
channel id, Socket Mode app token, and bot token as the `SUPPORT_CLAW_MANTIS_SLACK_*`
...
- `--gateway-setup` starts a persistent SupportClaw Slack gateway in the VM instead of only running the bot-to-bot QA lane.
...
- `SUPPORT_CLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome profile inside the VM. The default is `$HOME/.config/supportClaw-mantis/slack-chrome-profile`, so a manual Slack Web login survives reruns on the same lease.
...
Crabbox CLI from
`supportClaw/crabbox` main so it can use the current desktop/browser lease flags
...
runs `pnpm supportClaw qa mantis slack-desktop-smoke --gateway-setup` against that
...
worktree, runs `pnpm supportClaw qa telegram --credential-source convex
...
`@supportclaw-mantis telegram desktop proof`, from the Actions UI with freeform
...
SupportClaw with the leased Telegram SUT bot token, starts `supportClaw gateway run`
...
token never logs Telegram Desktop in; it only configures SupportClaw. The desktop
...
- `--no-gateway-setup` installs and opens Telegram Desktop without configuring SupportClaw.
...
comment or creates one. The workflows publish to `supportClaw-crabbox-artifacts`
with public URLs under `https://artifacts.supportClaw.ai`. They provide bucket,
...
```text
@supportclaw-mantis discord status reactions
```
...
```text
@supportclaw-mantis discord status reactions baseline=origin/main candidate=HEAD
```
...
6. Start a child SupportClaw Gateway with an isolated state directory.
...
- It exercises a real SupportClaw Gateway, Discord bot auth, message dispatch,
...
- `pnpm supportClaw qa discord` already runs a live Discord lane with driver and
  SUT bots.
...
- **Headless automation**: default for CI. Chrome runs with CDP enabled, and
  Playwright or SupportClaw browser control captures screenshots.
...
- Node 22 and pnpm
- SupportClaw checkout and dependency cache
- Playwright Chromium browser cache when Playwright is used
- enough CPU and memory for one SupportClaw Gateway, one browser, and one model run
...
- `SUPPORT_CLAW_QA_DISCORD_MANTIS_BOT_TOKEN`
...
- `SUPPORT_CLAW_QA_DISCORD_NOTIFY_CHANNEL_ID`
- `SUPPORT_CLAW_QA_REDACT_PUBLIC_METADATA=1` for public GitHub artifact uploads
- `SUPPORT_CLAW_QA_CONVEX_SITE_URL`
- `SUPPORT_CLAW_QA_CONVEX_SECRET_CI`
- `SUPPORT_CLAW_QA_MANTIS_CRABBOX_COORDINATOR`
- `SUPPORT_CLAW_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`
...
`SUPPORT_CLAW_QA_REDACT_PUBLIC_METADATA=1` for this reason.
