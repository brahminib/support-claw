...
- `/usage cost` in chats: local cost summary aggregated from SupportClaw session logs.
- CLI: `supportClaw status --usage` prints a full per-provider breakdown.
- CLI: `supportClaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
{
  "messages": {
    "usageTemplate": "~/.supportClaw/usage-footer.json"
  }
}
```
...
```jsonc
{
  "schema": "supportClaw.usageBar.v1",
  "scales": {
    "braille": "⠐⡀⡄⡆⡇⣇⣧⣷⣿",
    "block": "░▏▎▍▌▋▊▉█",
    "shade": "░▒▓█",
...
- **MiniMax**: API key or MiniMax OAuth auth profile. SupportClaw treats
  ...
  quota, so SupportClaw inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, SupportClaw prefers the
...
- **DeepSeek**: API key via env/config/auth store (`DEEPSEEK_API_KEY`).
  SupportClaw calls DeepSeek's balance endpoint and shows the provider-reported
...
can supply plugin-specific usage auth logic; otherwise SupportClaw falls back to
