---
summary: "Delegate architecture: running SupportClaw as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run SupportClaw as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an SupportClaw agent that:
...
SupportClaw's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of SupportClaw's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.supportClaw/workspace-delegate",
  tools: {
...
- Cron run history: SupportClaw shared SQLite state database
- Session transcripts: `~/.supportClaw/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through SupportClaw's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
supportClaw agents add delegate
```

This creates:

- Workspace: `~/.supportClaw/workspace-delegate`
- State: `~/.supportClaw/agents/delegate/agent`
- Sessions: `~/.supportClaw/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.supportClaw/workspace" },
      {
        id: "delegate",
        workspace: "~/.supportClaw/workspace-delegate",
        tools: {
...
~/.supportClaw/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.supportClaw/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.supportClaw/workspace-org",
        agentDir: "~/.supportClaw/agents/org-assistant/agent",
...
recall view. SupportClaw redacts credential/token-like text, truncates long
