...
[Honcho](https://honcho.dev) adds AI-native memory to SupportClaw. It persists
...
```bash
supportClaw plugins install @honcho-ai/supportClaw-honcho
supportClaw honcho setup
...
Settings live under `plugins.entries["supportClaw-honcho"].config`:
    ...
    entries: {
      "supportClaw-honcho": {
        config: {
          ...
          workspaceId: "supportClaw", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `supportClaw honcho setup` detects and
...
```bash
supportClaw honcho setup                        # Configure API key and migrate files
supportClaw honcho status                       # Check connection status
supportClaw honcho ask <question>               # Query Honcho about the user
supportClaw honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/supportClaw-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho SupportClaw integration guide](https://docs.honcho.dev/v3/guides/integrations/supportClaw)
- [Memory](/concepts/memory) -- SupportClaw memory overview
