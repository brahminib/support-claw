---
summary: "How SupportClaw separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between SupportClaw, Codex, ACP, or another native agent runtime
...
the finished turn to SupportClaw.
...
| ------------- | -------------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `github-copilot`      | How SupportClaw authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`                 | The model selected for the agent turn.                              |
| Agent runtime | `supportClaw`, `codex`, `copilot`, `claude-cli` | The low level loop or backend that executes the prepared turn.      |
| Channel       | Telegram, Discord, Slack, WhatsApp           | Where messages enter and leave SupportClaw.                            |
...
`supportClaw doctor --fix` removes old whole-agent runtime pins and rewrites
...
- **Embedded harnesses** run inside SupportClaw's prepared agent loop. Today this
  is the built-in `supportClaw` runtime plus registered plugin harnesses such as
  `codex` and `copilot`.
...
| Surface                                          | SupportClaw name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
the native app-server features available; `supportClaw doctor --fix` owns legacy
...
That means SupportClaw selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or SupportClaw session store
becomes Codex.
...
3. If the user explicitly chooses **SupportClaw for an OpenAI model**, keep the model ref
   as `openai/<model>` and set provider/model runtime policy to
   `agentRuntime.id: "supportClaw"`. A selected `openai` OAuth profile is routed
   internally through SupportClaw's Codex-auth transport.
4. If legacy config still contains **legacy Codex model refs**, repair it to
   `openai/<model>` with `supportClaw doctor --fix`; doctor keeps the Codex auth
   ...
   app-server route; SupportClaw no longer keeps a bundled Codex CLI backend.
...
| Surface                     | SupportClaw embedded                             | Codex app-server                                                            |
| --------------------------- | --------------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | SupportClaw through the SupportClaw embedded runner | Codex app-server                                                            |
| Canonical thread state      | SupportClaw transcript                           | Codex thread, plus SupportClaw transcript mirror                               |
| SupportClaw dynamic tools      | Native SupportClaw tool loop                     | Bridged through the Codex adapter                                           |
| Native shell and file tools | SupportClaw path                                 | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native SupportClaw context assembly              | SupportClaw projects assembled context into the Codex turn                     |
| Compaction                  | SupportClaw or selected context engine           | Codex-native compaction, with SupportClaw notifications and mirror maintenance |
| Channel delivery            | SupportClaw                                      | SupportClaw                                                                    |

This ownership split is the main design rule:

- If SupportClaw owns the surface, SupportClaw can provide normal plugin hook behavior.
- If the native runtime owns the surface, SupportClaw needs runtime events or native hooks.
- If the native runtime owns canonical thread state, SupportClaw should mirror and project context, not rewrite unsupported internals.

## Runtime selection

SupportClaw chooses an embedded runtime after provider and model resolution:
...
3. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
4. If no runtime claims a turn in `auto` mode, SupportClaw uses `supportClaw` as the
...
`SUPPORT_CLAW_AGENT_RUNTIME`, session `agentHarnessId`/`agentRuntimeOverride` state,
`agents.defaults.agentRuntime`, and `agents.list[].agentRuntime`. Run
`supportClaw doctor --fix` to remove stale whole-agent runtime config and convert
legacy runtime model refs where SupportClaw can preserve the intent.
...
selection/runtime error; it is never silently routed back to SupportClaw.
...
harness. Explicit SupportClaw runtime config remains an opt-in compatibility route for
`openai/*` agent turns; when paired with a selected `openai` OAuth profile,
SupportClaw routes that path internally through the Codex-auth transport while
keeping the public model ref as `openai/*`. Stale OpenAI runtime session pins are
ignored by runtime selection and can be cleaned with `supportClaw doctor --fix`.

If `supportClaw doctor` warns that the `codex` plugin is enabled while
legacy Codex model refs remain in config, treat that as legacy route state. Run
`supportClaw doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## GitHub Copilot agent runtime

The external `@supportclaw/copilot` plugin registers an opt-in `copilot` runtime
...
`supportClaw doctor` auto-loads. For configuration, auth, transcript mirroring,
...
When a runtime is not SupportClaw, it should document what SupportClaw surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether SupportClaw can edit history or only mirror it.                                   |
| Do SupportClaw dynamic tools work?        | Messaging, sessions, cron, and SupportClaw-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around SupportClaw-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume SupportClaw equivalence where the native runtime owns more state.            |
