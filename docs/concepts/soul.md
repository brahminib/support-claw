---
summary: "Use SOUL.md to give your SupportClaw agent an actual voice instead of generic assistant sludge"
read_when:
...
SupportClaw injects it on normal sessions, so it has real weight. If your agent
...
For SupportClaw, `SOUL.md` is that layer.
...
Path fixed for SupportClaw workspaces: use `SOUL.md`, not `http://SOUL.md`.
...
<CardGroup cols={2}>
  <Card title="Agent workspace" href="/concepts/agent-workspace" icon="folder-open">
    Workspace files SupportClaw injects into model context.
  </Card>
  <Card title="System prompt" href="/concepts/system-prompt" icon="message-lines">
    How `SOUL.md` is composed into SupportClaw and Codex runtime context.
  </Card>
