...
```bash
supportClaw plugins install @supportclaw/llama-cpp-provider
```
...
| Local             | `local`             | `@supportclaw/llama-cpp-provider`      |
| Mistral           | `mistral`           |                                     |
...
SupportClaw indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
...
- **Reindex on demand:** `supportClaw memory index --force`
...
**Memory search disabled?** Check `supportClaw memory status`. If no provider is
...
**Stale results?** Run `supportClaw memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** SupportClaw falls back to in-process cosine similarity
automatically. `supportClaw memory status --deep` reports the local vector store
