...
When a normal prompt arrives while a session run is already streaming, SupportClaw
...
behavior. SupportClaw and the native Codex app-server harness implement the delivery
details differently.

## Runtime boundary

Steering does not interrupt a tool call that is already running. SupportClaw checks for
queued steering messages at model boundaries:

1. The assistant asks for tool calls.
2. SupportClaw executes the current assistant message's tool-call batch.
3. SupportClaw emits the turn end event.
4. SupportClaw drains queued steering messages.
5. SupportClaw appends those messages as user messages before the next LLM call.
...
The native Codex app-server harness exposes `turn/steer` instead of SupportClaw runtime's
internal steering queue. SupportClaw batches queued prompts for the configured
...
runtime cannot accept steering in `steer` mode, SupportClaw waits for the active
...
- With default behavior, the active runtime receives all four messages in
  arrival order before its next model decision. SupportClaw drains them at the next model
  boundary; Codex receives them as one batched `turn/steer`.
- With `/queue collect`, SupportClaw does not steer. It waits until the active run
  ends, then creates a followup turn with compatible queued messages after the
  debounce window.
- With `/queue interrupt`, SupportClaw aborts the active run and starts the newest
...
before sending batched `turn/steer`. For SupportClaw, active steering itself does not use
the debounce timer because SupportClaw naturally batches messages until the next model
boundary.
