...
alongside SupportClaw. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, SupportClaw falls back to the
...
SupportClaw creates a self-contained QMD home under
`~/.supportClaw/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- SupportClaw creates collections from your workspace memory files and any
...
- If QMD fails entirely, SupportClaw falls back to the builtin SQLite engine.
  ...
  `supportClaw memory status` and one-shot CLI probes still recheck QMD directly.
...
SupportClaw keeps the QMD search path compatible with both current and older QMD
installs.

On startup, SupportClaw checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, SupportClaw searches all
...
Older QMD builds only accept one collection filter. When SupportClaw detects one
...
process, so you can tune QMD globally without adding new SupportClaw config:
...
collection under `~/.supportClaw/agents/<id>/qmd/sessions/`.
...
When scope denies a search, SupportClaw logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If SupportClaw
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but SupportClaw still reports
...
with `supportClaw memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs SupportClaw uses.

**Many QMD subprocesses during search?** Update QMD if possible. SupportClaw uses
...
`memory.qmd.searchMode = "search"`. SupportClaw treats that mode as lexical-only,
...
the next startup reconciliation. SupportClaw recreates stale managed collections
...
SupportClaw's builtin symlink rules. Keep temporary monorepo checkouts under
