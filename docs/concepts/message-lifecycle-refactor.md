...
That shape fixes local bugs, but it leaves SupportClaw with too many public
...
That gives SupportClaw at-least-once recovery. Exactly-once behavior exists only
...
- Structured SupportClaw-origin metadata for operational/system output so visible
...
SupportClaw should borrow the vocabulary, not copy the surface.

What SupportClaw needs beyond that model:
...
`thread.post()` style promises are not enough for SupportClaw. They hide the
...
Origin describes who produced a message and how SupportClaw should treat echoes of
that message. It is separate from relation: a message can be a reply to a user
and still be SupportClaw-originated operational output.
...
Core owns the meaning of SupportClaw-originated output. Channels own how that
...
SupportClaw operational output must not be accepted as bot-authored input in shared
...
- **Transport ack:** tells the platform webhook or socket that SupportClaw accepted
...
- **Inbound record ack:** confirms SupportClaw persisted enough inbound metadata to
...
Before bot authorization, receive must apply the shared SupportClaw echo policy
when the channel can decode message origin metadata:

```typescript
function shouldDropSupportClawEcho(params: {
  origin?: MessageOrigin;
  isBotAuthor: boolean;
  isRoomish: boolean;
}): boolean {
  return (
    params.isBotAuthor &&
    params.isRoomish &&
    params.origin?.source === "supportClaw" &&
...
same visible gateway-failure text but without SupportClaw origin metadata still
...
middleware chain, but SupportClaw persists only the safe completed update id after
...
source if we need platform-level redelivery beyond SupportClaw's restart
...
process dies there, SupportClaw cannot know whether the platform message exists
...
```typescript
import { defineChannelMessageAdapter } from "supportClaw/plugin-sdk/channel-outbound";
```
...
Before preflight authorization, core must run the shared SupportClaw echo predicate
whenever `origin.decode` returns SupportClaw-origin metadata. The receive adapter
...
- iMessage monitor delivery records sent messages in an echo cache after a
  ...
  SupportClaw can re-ingest its own final replies as inbound user messages.
...
- Direct-DM helpers can have a reply callback that is the only correct transport
  ...
  that callback.
- SupportClaw gateway failure output must stay visible to humans, but tagged
...
| Slack           | Send adapter handles normal chat posts. Live adapter chooses native stream when thread shape supports it, otherwise draft preview. Receipts preserve thread timestamps. Origin adapter maps SupportClaw gateway failures to Slack `chat.postMessage.metadata` and drops tagged bot-room echoes before `allowBots` authorization.                                  |
...
| Matrix          | Live adapter owns draft event edits, finalization, redaction, encrypted media constraints, and reply-target mismatch fallback. Receive adapter owns encrypted event hydration and dedupe. Origin adapter should encode SupportClaw gateway-failure origin into Matrix event content and drop configured-bot room echoes before `allowBots` handling.              |
...
| Google Chat     | Simple receive plus send adapter with thread relation mapped to spaces and thread ids. Audit `allowBots=true` room behavior for tagged SupportClaw gateway-failure echoes.                                                                                                                                                                                        |
...
- Persist message origin through durable send intents so recovery, replay, and
  chunked sends preserve SupportClaw operational provenance.
...
- Add `supportClaw/plugin-sdk/channel-outbound`.
...
- Include `MessageOrigin`, origin encode/decode hooks, and the shared
  `shouldDropSupportClawEcho` predicate in the channel-outbound SDK surface.
...
This is where the model stops being "agent replies" and becomes "SupportClaw sends
messages".
...
- Gateway-failure origin factory and `shouldDropSupportClawEcho` predicate.
...
- Slack tagged SupportClaw gateway failure messages stay visible outbound, tagged
...
- Matrix tagged SupportClaw gateway-failure room echoes from configured bot
...
- Whether Telegram should eventually replace the grammY runner source with a
  ...
  only SupportClaw's persisted restart watermark.
...
- SupportClaw-originated gateway failure output is visible to humans but tagged
