...
<AccordionGroup>
  <Accordion title="Model refs and CLI helpers">
    ...
    - CLI helpers: `supportClaw onboard`, `supportClaw models list`, `supportClaw models set <provider/model>`.
  ...
  <Accordion title="Adding provider auth does not change your primary model">
    `supportClaw configure` preserves an existing `agents.defaults.model.primary` when you add or reauth a provider. `supportClaw models auth login` does the same unless you pass `--set-default`. Provider plugins may still return a recommended default model in their auth config patch, but SupportClaw treats that as "make this model available" when a primary model already exists, not "replace the current primary model."

    To intentionally switch the default model, use `supportClaw models set <provider/model>` or `supportClaw models auth login --provider <id> --set-default`.
  ...
  <Accordion title="OpenAI provider/runtime split">
    ...
    - `openai/<model>` plus provider/model `agentRuntime.id: "supportClaw"` uses SupportClaw's built-in runtime for explicit API-key or compatibility routes.
    ...
    GPT-5.5 is available through the native Codex app-server harness by default on `openai/gpt-5.5`, and through the SupportClaw runtime when provider/model runtime policy explicitly selects `supportClaw`.
  ...
  <Accordion title="CLI runtimes">
    ...
    Legacy `claude-cli/*` and `google-gemini-cli/*` refs migrate back to canonical provider refs with the runtime recorded separately. Legacy `codex-cli/*` refs migrate to `openai/*` and use the Codex app-server route; SupportClaw no longer keeps a bundled Codex CLI backend.
...
<AccordionGroup>
  <Accordion title="Key sources and priority">
    ...
    - `SUPPORT_CLAW_LIVE_<PROVIDER>_KEY` (single live override, highest priority)
...
- Provider: `openai`
- Auth: `OPENAI_API_KEY`
- Optional rotation: `OPENAI_API_KEYS`, `OPENAI_API_KEY_1`, `OPENAI_API_KEY_2`, plus `SUPPORT_CLAW_LIVE_OPENAI_KEY` (single override)
- Example models: `openai/gpt-5.5`, `openai/gpt-5.4-mini`
- Verify account/model availability with `supportClaw models list --provider openai` if a specific install or API key behaves differently.
- CLI: `supportClaw onboard --auth-choice openai-api-key`
- Default transport is `auto`; SupportClaw passes the transport choice to the shared model runtime.
...
- Hidden SupportClaw attribution headers (`originator`, `version`, `User-Agent`) apply only on native OpenAI traffic to `api.openai.com`, not generic OpenAI-compatible proxies
...
- `openai/gpt-5.3-codex-spark` is available through ChatGPT/Codex OAuth subscription auth when your signed-in account exposes it; SupportClaw still suppresses direct OpenAI API-key and Azure API-key routes for this model because those transports reject it
...
- Optional rotation: `ANTHROPIC_API_KEYS`, `ANTHROPIC_API_KEY_1`, `ANTHROPIC_API_KEY_2`, plus `SUPPORT_CLAW_LIVE_ANTHROPIC_KEY` (single override)
- Example model: `anthropic/claude-opus-4-6`
- CLI: `supportClaw onboard --auth-choice apiKey`
- Direct public Anthropic requests support the shared `/fast` toggle and `params.fastMode`, including API-key and OAuth-authenticated traffic sent to `api.anthropic.com`; SupportClaw maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
<Note>
Anthropic staff told us SupportClaw-style Claude CLI usage is allowed again, so SupportClaw treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy. Anthropic setup-token remains available as a supported SupportClaw token path, but SupportClaw now prefers Claude CLI reuse and `claude -p` when available.
</Note>
...
- CLI: `supportClaw onboard --auth-choice openai` or `supportClaw models auth login --provider openai`
...
- Hidden SupportClaw attribution headers (`originator`, `version`, `User-Agent`) are only attached on native Codex traffic to `chatgpt.com/backend-api`, not generic OpenAI-compatible proxies
- Shares the same `/fast` toggle and `params.fastMode` config as direct `openai/*`; SupportClaw maps that to `service_tier=priority`
...
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like SupportClaw.
...
- Use provider/model `agentRuntime.id: "supportClaw"` only when you want the built-in SupportClaw route; otherwise keep `openai/gpt-5.5` on the default Codex harness.
- legacy Codex GPT refs are legacy state, not a live provider route. Use `openai/gpt-5.5` on the native Codex runtime for new agent config, and run `supportClaw doctor --fix` to migrate old legacy Codex model refs to canonical `openai/*` refs.
...
- CLI: `supportClaw onboard --auth-choice opencode-zen` or `supportClaw onboard --auth-choice opencode-go`
...
- Provider: `google`
- Auth: `GEMINI_API_KEY`
- Optional rotation: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY` fallback, and `SUPPORT_CLAW_LIVE_GEMINI_KEY` (single override)
- Example models: `google/gemini-3.1-pro-preview`, `google/gemini-3-flash-preview`
- Compatibility: legacy SupportClaw config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- Alias: `google/gemini-3.1-pro` is accepted and normalized to Google's live Gemini API id, `google/gemini-3.1-pro-preview`
- CLI: `supportClaw onboard --auth-choice gemini-api-key`
...
- Direct Gemini runs also accept `agents.defaults.models["google/<model>"].params.cachedContent` (or legacy `cached_content`) to forward a provider-native `cachedContents/...` handle; Gemini cache hits surface as SupportClaw `cacheRead`
...
<Warning>
Gemini CLI OAuth in SupportClaw is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
</Warning>
...
<Steps>
  ...
  <Step title="Enable plugin">
    ```bash
    supportClaw plugins enable google
    ```
  </Step>
  <Step title="Login">
    ```bash
    supportClaw models auth login --provider google-gemini-cli --set-default
    ```

    Default model: `google-gemini-cli/gemini-3-flash-preview`. You do **not** paste a client id or secret into `supportClaw.json`. The CLI login flow stores tokens in auth profiles on the gateway host.
...
Gemini CLI uses `stream-json` by default. SupportClaw reads assistant stream
...
- CLI: `supportClaw onboard --auth-choice zai-api-key`
...
- CLI: `supportClaw onboard --auth-choice ai-gateway-api-key`
...
Install `@supportclaw/moonshot-provider` before onboarding. Add an explicit `models.providers.moonshot` entry only when you need to override the base URL or model metadata:

- Provider: `moonshot`
- Auth: `MOONSHOT_API_KEY`
- Example model: `moonshot/kimi-k2.6`
- CLI: `supportClaw onboard --auth-choice moonshot-api-key` or `supportClaw onboard --auth-choice moonshot-api-key-cn`
...
- CLI: `supportClaw onboard --auth-choice volcengine-api-key`
...
In onboarding/configure model pickers, the Volcengine auth choice prefers both `volcengine/*` and `volcengine-plan/*` rows. If those models are not loaded yet, SupportClaw falls back to the unfiltered catalog instead of showing an empty provider-scoped picker.
...
- CLI: `supportClaw onboard --auth-choice byteplus-api-key`
...
In onboarding/configure model pickers, the BytePlus auth choice prefers both `byteplus/*` and `byteplus-plan/*` rows. If those models are not loaded yet, SupportClaw falls back to the unfiltered catalog instead of showing an empty provider-scoped picker.
...
- CLI: `supportClaw onboard --auth-choice synthetic-api-key`
...
<Note>
On MiniMax's Anthropic-compatible streaming path, SupportClaw disables thinking by default for the M2.x family unless you explicitly set it; MiniMax-M3 (and M3.x) stays on the provider's omitted/adaptive thinking path by default. `/fast on` rewrites `MiniMax-M2.7` to `MiniMax-M2.7-highspeed`.
</Note>
...
SupportClaw uses LM Studio's native `/api/v1/models` and `/api/v1/models/load` for discovery + auto-load, with `/v1/chat/completions` for inference by default. If you want LM Studio JIT loading, TTL, and auto-evict to own model lifecycle, set `models.providers.lmstudio.params.preload: false`. See [/providers/lmstudio](/providers/lmstudio) for setup and troubleshooting.
...
Ollama is detected locally at `http://127.0.0.1:11434` when you opt in with `OLLAMA_API_KEY`, and the bundled provider plugin adds Ollama directly to `supportClaw onboard` and the model picker. See [/providers/ollama](/providers/ollama) for onboarding, cloud/local mode, and custom configuration.
...
<AccordionGroup>
  <Accordion title="Default optional fields">
    For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional. When omitted, SupportClaw defaults to:
  ...
  <Accordion title="Proxy-route shaping rules">
    - For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), SupportClaw forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
    - Proxy-style OpenAI-compatible routes also skip native OpenAI-only request shaping: no `service_tier`, no Responses `store`, no Completions `store`, no prompt-cache hints, no OpenAI reasoning-compat payload shaping, and no hidden SupportClaw attribution headers.
...
```bash
supportClaw onboard --auth-choice opencode-zen
supportClaw models set opencode/claude-opus-4-6
