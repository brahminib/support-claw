...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.supportClaw/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.supportClaw/agents/<agentId>/sessions`.
...
a local profile, but SupportClaw does not clone OAuth refresh tokens into the
...
Skills are loaded from each agent workspace plus shared roots such as `~/.supportClaw/skills`, then filtered by the effective agent skill allowlist when configured. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for per-agent replacement. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent skill allowlists](/tools/skills#agent-allowlists).
...
- State dir: `~/.supportClaw` (or `SUPPORT_CLAW_STATE_DIR`)
- Workspace: `~/.supportClaw/workspace` (or `~/.supportClaw/workspace-<agentId>`)
- Agent dir: `~/.supportClaw/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.supportClaw/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, SupportClaw runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.supportClaw/workspace` (or `~/.supportClaw/workspace-<profile>` when `SUPPORT_CLAW_PROFILE` is set).
- State defaults to `~/.supportClaw/agents/main/agent`.
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    ```bash
    supportClaw agents add coding
    supportClaw agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.supportClaw/agents/<agentId>`.
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.supportClaw/workspace-alex" },
      { id: "mia", workspace: "~/.supportClaw/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  ...
  <Accordion title="Account-scope detail">
    ...
    - If you later add the same binding for the same agent with an explicit account id, SupportClaw upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
If you want a channel-wide default account when `accountId` is omitted, set `channels.<channel>.defaultAccount` (optional). When unset, SupportClaw falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.supportClaw/workspace-main" },
          { id: "coding", workspace: "~/.supportClaw/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.supportClaw/workspace-main" },
          { id: "alerts", workspace: "~/.supportClaw/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    supportClaw channels login --channel whatsapp --account personal
    supportClaw channels login --channel whatsapp --account biz
    ```

    `~/.supportClaw/supportClaw.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.supportClaw/workspace-home",
            agentDir: "~/.supportClaw/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.supportClaw/workspace-work",
            agentDir: "~/.supportClaw/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.supportClaw/credentials/whatsapp/personal
              // authDir: "~/.supportClaw/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.supportClaw/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.supportClaw/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.supportClaw/workspace-opus",
