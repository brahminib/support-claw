---
summary: "How SupportClaw remembers things across sessions"
...
SupportClaw remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.supportClaw/workspace`).
...
If `MEMORY.md` grows past the bootstrap file budget, SupportClaw keeps the file on
...
`supportClaw doctor` to see raw vs injected sizes and truncation status.
...
Memory can preserve approval context, but it does not enforce policy. Use SupportClaw approval settings, sandboxing, and scheduled tasks for hard operational controls.
...
for that case. SupportClaw infers them in a hidden background pass, scopes them to
...
<Info>
SupportClaw uses OpenAI embeddings by default. Set
...
Before [compaction](/concepts/compaction) summarizes your conversation, SupportClaw
...
```bash
supportClaw memory rem-backfill --rollback
supportClaw memory rem-backfill --rollback-short-term
```

## CLI

```bash
supportClaw memory status          # Check index status and provider
supportClaw memory search "query"  # Search from the command line
supportClaw memory index --force   # Rebuild the index
```
