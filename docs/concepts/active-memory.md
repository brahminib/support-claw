...
Paste this into `supportClaw.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, SupportClaw can show:
...
case-insensitive. If `allowedChatIds` is non-empty and SupportClaw cannot resolve a
...
`config.promptOverride` replaces the default Active Memory prompt. SupportClaw
...
5. Verify memory search itself works with `supportClaw memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, SupportClaw uses OpenAI embeddings. Set
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `supportClaw memory status --deep` to inspect the memory-search backend
      and index health.
