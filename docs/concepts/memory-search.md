...
`@supportclaw/llama-cpp-provider` and set `provider: "local"`. Source checkouts
...
SupportClaw runs two retrieval paths in parallel and merges the results:
...
- **Vector search** finds notes with similar meaning ("gateway host" matches
  "the machine running SupportClaw").
...
**No results?** Run `supportClaw memory status` to check the index. If empty, run
