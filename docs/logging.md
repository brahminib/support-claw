...
read_when:
  - You need a beginner-friendly overview of SupportClaw logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

SupportClaw has two main log surfaces:
...
SupportClaw keeps up to five numbered archives beside the active file, such as
`supportClaw-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.supportClaw/supportClaw.json`:

```json
{
  "logging": {
    "file": "/path/to/supportClaw.log"
  }
}
```
...
or times out before `logs.tail` answers, `supportClaw logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `supportClaw logs --follow` is stricter: on Linux it uses the active
...
```bash
supportClaw channels logs --channel whatsapp
```
...
SupportClaw preserves the original structured log arguments alongside these fields
...
`supportClaw gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
supportClaw gateway
supportClaw gateway --verbose --ws-log compact
supportClaw gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.supportClaw/supportClaw.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/supportClaw/supportClaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`SUPPORT_CLAW_LOG_LEVEL`** environment variable (e.g. `SUPPORT_CLAW_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `supportClaw.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `supportClaw --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
SUPPORT_CLAW_DEBUG_MODEL_TRANSPORT=1 supportClaw gateway
SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=tools SUPPORT_CLAW_DEBUG_SSE=events supportClaw gateway
```

Available flags:

- `SUPPORT_CLAW_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `SUPPORT_CLAW_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `SUPPORT_CLAW_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `SUPPORT_CLAW_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal SupportClaw logging, so `supportClaw logs --follow`
...
`SUPPORT_CLAW_DEBUG_MODEL_TRANSPORT`, so basic model transport hygiene is visible
without debug flags.

### Trace correlation

File logs are JSONL. When a log call carries a valid diagnostic trace context,
SupportClaw writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
SupportClaw can redact sensitive tokens before they hit console output, file logs,
...
policy. SupportClaw still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `SUPPORT_CLAW_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `supportClaw doctor` first.
