...
Explicit copy flows, such as `supportClaw agents add`, use this portability policy:
...
has such a marker, `supportClaw doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but SupportClaw cannot resolve a probeable model
