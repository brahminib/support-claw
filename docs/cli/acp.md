...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an SupportClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`supportClaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to SupportClaw channel
conversations instead of hosting an ACP harness session, use
[`supportClaw mcp serve`](/cli/mcp) instead.

## What this is not

This page is often confused with ACP harness sessions.

`supportClaw acp` means:

- SupportClaw acts as an ACP server
- an IDE or ACP client connects to SupportClaw
- SupportClaw forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where SupportClaw runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to SupportClaw: use `supportClaw acp`
- SupportClaw should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include SupportClaw parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the SupportClaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
supportClaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
supportClaw acp --url wss://gateway-host:18789 --token-file ~/.supportClaw/gateway.token

# Attach to an existing session key
supportClaw acp --session agent:main:main

# Attach by label (must already exist)
supportClaw acp --session-label "support inbox"

# Reset the session key before the first prompt
supportClaw acp --session agent:main:main --reset-session
```
...
```bash
supportClaw acp client

# Point the spawned bridge at a remote Gateway
supportClaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.supportClaw/gateway.token

# Override the server command (default: supportClaw)
supportClaw acp client --server "node" --server-args supportClawCli.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run SupportClaw through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`supportClaw acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `supportClaw gateway call sessions.list` as the only ACP proof. That
...
it to drive an SupportClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `supportClaw acp` over stdio.

Example config (persisted):

```bash
supportClaw config set gateway.remote.url wss://gateway-host:18789
supportClaw config set gateway.remote.token <token>
```
...
```bash
supportClaw acp --session agent:main:main
supportClaw acp --session agent:design:main
supportClaw acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see SupportClaw plugin tools or selected
...
[SupportClaw tools MCP bridge](/tools/acp-agents-setup#supportClaw-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
SupportClaw bot over ACP, use `acpx` with its built-in `supportClaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx supportClaw` at `supportClaw acp`.
3. Target the SupportClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default SupportClaw ACP session
acpx supportClaw exec "Summarize the active SupportClaw session state."

# Persistent named session for follow-up turns
acpx supportClaw sessions ensure --name codex-bridge
acpx supportClaw -s codex-bridge --cwd /path/to/repo \
  "Ask my SupportClaw work agent for recent context relevant to this repo."
```

If you want `acpx supportClaw` to target a specific Gateway and session key every
time, override the `supportClaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "supportClaw": {
      "command": "env SUPPORT_CLAW_HIDE_BANNER=1 SUPPORT_CLAW_SUPPRESS_NOTES=1 supportClaw acp --url ws://127.0.0.1:18789 --token-file ~/.supportClaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local SupportClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env SUPPORT_CLAW_HIDE_BANNER=1 SUPPORT_CLAW_SUPPRESS_NOTES=1 node supportClawCli.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an SupportClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select "SupportClaw ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`SUPPORT_CLAW_GATEWAY_TOKEN`, `SUPPORT_CLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`SUPPORT_CLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `SUPPORT_CLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `supportClaw acp client` sets `SUPPORT_CLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `supportClaw`).
