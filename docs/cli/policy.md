---
summary: "CLI reference for `supportClaw policy` conformance checks"
read_when:
  - You want to check SupportClaw settings against an authored policy.jsonc
...
`supportClaw policy` is provided by the bundled Policy plugin. Policy is an
enterprise conformance layer over existing SupportClaw settings. It does not add a
second configuration system. `policy.jsonc` defines authored requirements,
SupportClaw observes the active workspace as evidence, and policy health checks
...
data-handling posture, SupportClaw config secret provider/auth profile posture, and governed tool
...
posture, require SupportClaw config SecretRefs to use managed providers, require
...
repeatable way to prove that SupportClaw still conforms to that statement. Use
...
```bash
supportClaw plugins enable policy
```
...
exposure, agent workspace posture, configured sandbox runtime posture, SupportClaw
...
when a concrete rule is present. SupportClaw reads current `channels.*` settings
...
posture, SupportClaw config agent sandbox workspace access and tool deny posture,
...
`edit`, and `apply_patch`; SupportClaw config `group:fs` covers file mutation tools
...
present in `policy.jsonc`. The observed state is existing SupportClaw config or
...
present in `agents.list[]`, SupportClaw evaluates the scoped rule against inherited
...
Container posture policy is evaluated only against evidence SupportClaw can
...
artifact. By default this is `~/.supportClaw/exec-approvals.json`; when
`SUPPORT_CLAW_STATE_DIR` is set, Policy reads
`$SUPPORT_CLAW_STATE_DIR/exec-approvals.json`. Actual posture rules such as
...
```bash
supportClaw policy check
supportClaw policy check --json
supportClaw policy check --severity-min error
```

`policy check` runs only the policy check set and emits evidence, findings, and
attestation hashes. The same findings also appear in `supportClaw doctor --lint`
when the Policy plugin is enabled.

Compare an operator policy file to an authored baseline policy file:

```bash
supportClaw policy compare --baseline official.policy.jsonc
supportClaw policy compare --baseline official.policy.jsonc --policy policy.jsonc --json
```

`policy compare` compares policy file syntax to policy file syntax. It does not
inspect SupportClaw runtime state, evidence, credentials, or secrets. The command
...
```json
{
  ...
  "evidence": {
    "channels": [
      {
        "id": "telegram",
        "provider": "telegram",
        "source": "oc://supportClaw.config/channels/telegram",
        "enabled": false
      }
    ],
    "mcpServers": [
      {
        "id": "docs",
        "transport": "stdio",
        "source": "oc://supportClaw.config/mcp/servers/docs",
        "command": "npx"
      }
    ],
    "modelProviders": [
      {
        "id": "openai",
        "source": "oc://supportClaw.config/models/providers/openai"
      }
    ],
    "modelRefs": [
      {
        ...
        "provider": "openai",
        "model": "gpt-5.5",
        "source": "oc://supportClaw.config/agents/defaults/model"
      }
    ],
    "network": [
      {
        "id": "browser-private-network",
        "source": "oc://supportClaw.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
        "value": false
      }
    ],
    "gatewayExposure": [
      {
        "id": "gateway-bind",
        "kind": "bind",
        "source": "oc://supportClaw.config/gateway/bind",
        "value": "loopback",
        "nonLoopback": false,
        "explicit": true
      }
    ...
    "agentWorkspace": [
      {
        ...
        "value": "ro",
        "sandboxMode": "all",
        "sandboxModeSource": "oc://supportClaw.config/agents/defaults/sandbox/mode",
        "sandboxEnabled": true,
        "explicit": true
      },
      {
    ...
    "secrets": [
      {
        "id": "vault",
        "kind": "provider",
        "source": "oc://supportClaw.config/secrets/providers/vault",
        "providerSource": "env"
      },
      {
        "id": "oc://supportClaw.config/models/providers/openai/apiKey",
        "kind": "input",
    ...
    "authProfiles": [
      {
        "id": "github",
        "source": "oc://supportClaw.config/auth/profiles/github",
        "validMetadata": true,
        "provider": "github",
        "mode": "token"
      }
...
records the observed SupportClaw state used by the policy checks. The
...
2. Run `supportClaw policy check --json`.
...
5. Re-run `supportClaw doctor --lint` in CI or release gates.
...
`supportClaw policy watch` runs the same check repeatedly and reports when the
current evidence no longer matches `expectedAttestationHash`:

```bash
supportClaw policy watch --json
```
...
```json
{
  ...
  "target": "oc://supportClaw.config/mcp/servers/remote",
...
```json
{
  ...
  "target": "oc://supportClaw.config/agents/defaults/model/fallbacks/#0",
...
```json
{
  ...
  "target": "oc://supportClaw.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
...
```json
{
  ...
  "target": "oc://supportClaw.config/gateway/bind",
...
```json
{
  ...
  "target": "oc://supportClaw.config/agents/defaults/sandbox/workspaceAccess",
...
In this version, repair can disable channels that are enabled in SupportClaw config
