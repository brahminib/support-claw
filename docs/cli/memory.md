---
summary: "CLI reference for `supportClaw memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
supportClaw memory status
supportClaw memory status --deep
supportClaw memory status --fix
supportClaw memory index --force
supportClaw memory search "meeting notes"
supportClaw memory search --query "deployment" --max-results 20
supportClaw memory promote --limit 10 --min-score 0.75
supportClaw memory promote --apply
supportClaw memory promote --json --min-recall-count 0 --min-unique-queries 0
supportClaw memory promote-explain "router vlan"
supportClaw memory promote-explain "router vlan" --json
supportClaw memory rem-harness
supportClaw memory rem-harness --json
supportClaw memory status --json
supportClaw memory status --deep --index
supportClaw memory status --deep --index --verbose
supportClaw memory status --agent main
supportClaw memory index --agent main --verbose
```
...
```bash
supportClaw memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `supportClaw cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
supportClaw memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
supportClaw memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
