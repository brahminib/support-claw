---
summary: "CLI reference for `supportClaw workboard` cards, dispatch, and worker runs"
read_when:
...
`supportClaw workboard` is the terminal surface for the bundled
...
```bash
supportClaw plugins enable workboard
...
```bash
supportClaw workboard list [--board <id>] [--status <status>] [--json]
supportClaw workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
supportClaw workboard show <id> [--json]
supportClaw workboard dispatch [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
```bash
supportClaw workboard list
supportClaw workboard list --board default --status ready
supportClaw workboard list --json
```
...
```bash
supportClaw workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
supportClaw workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
supportClaw workboard show 7f4a2c10
supportClaw workboard show 7f4a2c10 --json
```
...
```bash
supportClaw workboard dispatch
supportClaw workboard dispatch --json
supportClaw workboard dispatch --url http://127.0.0.1:18789 --token "$SUPPORT_CLAW_GATEWAY_TOKEN"
```
...
Local `list`, `create`, and `show` commands operate on the local SupportClaw state
directory used by the current profile. Use `--dev` or `--profile <name>` on the
top-level `supportClaw` command when you need a different state root.
...
```bash
supportClaw plugins inspect workboard --runtime --json
```
...
Then retry `supportClaw workboard dispatch`. Data-only fallback is useful for local
...
```bash
supportClaw workboard list --status ready
```
