---
summary: "CLI reference for `supportClaw tasks` (background task ledger and Task Flow state)"
read_when:
  - You want to inspect, audit, or cancel background task records
  - You are documenting Task Flow commands under `supportClaw tasks flow`
title: "`supportClaw tasks`"
---

Inspect durable background tasks and Task Flow state. With no subcommand,
`supportClaw tasks` is equivalent to `supportClaw tasks list`.
...
```bash
supportClaw tasks list [--runtime <name>] [--status <name>] [--json]
```
...
```bash
supportClaw tasks show <lookup> [--json]
```
...
```bash
supportClaw tasks audit [--severity <warn|error>] [--code <name>] [--limit <n>] [--json]
```
...
```bash
supportClaw tasks flow list [--status <name>] [--json]
