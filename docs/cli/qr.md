---
summary: "CLI reference for `supportClaw qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
supportClaw qr
supportClaw qr --setup-code-only
supportClaw qr --json
supportClaw qr --remote
supportClaw qr --url wss://gateway.example/ws
```
...
- With `--remote`, SupportClaw requires either `gateway.remote.url` or
