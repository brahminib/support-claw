---
summary: "CLI reference for `supportClaw webhooks` (Gmail Pub/Sub setup and runner)"
read_when:
  - You want to wire Gmail Pub/Sub events into SupportClaw
  - You need the full flag list and default values
title: "Webhooks"
---

# `supportClaw webhooks`
...
```bash
supportClaw webhooks gmail setup --account <email> [...]
supportClaw webhooks gmail run   [--account <email>] [...]
```

| Subcommand    | Description                                                                                  |
| ------------- | -------------------------------------------------------------------------------------------- |
| `gmail setup` | Configure Gmail watch, Pub/Sub topic/subscription, and the SupportClaw webhook delivery target. |
| `gmail run`   | Run `gog watch serve` plus the watch auto-renew loop.                                        |

## `webhooks gmail setup`

Configure Gmail watch, Pub/Sub, and SupportClaw webhook delivery.

```bash
supportClaw webhooks gmail setup --account you@example.com
supportClaw webhooks gmail setup --account you@example.com --project my-gcp-project --json
supportClaw webhooks gmail setup --account you@example.com --hook-url https://gateway.example.com/hooks/gmail
```
...
### SupportClaw delivery options

| Flag                   | Default | Description                                |
| ---------------------- | ------- | ------------------------------------------ |
| `--hook-url <url>`     | (none)  | SupportClaw webhook URL.                      |
| `--hook-token <token>` | (none)  | SupportClaw webhook token.                    |
...
```bash
supportClaw webhooks gmail run --account you@example.com
```

`run` accepts the same `gog watch serve`, SupportClaw delivery, Pub/Sub, and Tailscale flags as `setup`, except:
...
| SupportClaw delivery | `--hook-url`, `--hook-token`, `--push-token`                                     |
