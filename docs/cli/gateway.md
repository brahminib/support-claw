---
summary: "SupportClaw Gateway CLI (`supportClaw gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is SupportClaw's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `supportClaw gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How SupportClaw advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.supportClaw/supportClaw.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `supportClaw onboard --mode local` and `supportClaw setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `SUPPORT_CLAW_GATEWAY_TOKEN` for the process).
</ParamField>
...
<ParamField path="--bind custom + gateway.customBindHost" type="string">
  Expects an IPv4 address today. For IPv6-only BYOH, place an IPv4 sidecar or proxy in front of the Gateway and point SupportClaw at that IPv4 endpoint.
</ParamField>
...
supportClaw gateway restart --safe
supportClaw gateway restart --safe --skip-deferral
supportClaw gateway restart --force
```

`supportClaw gateway restart --safe` asks the running Gateway to preflight active SupportClaw work before restarting. If queued operations, reply delivery, embedded runs, or task runs are active, the Gateway reports the blockers, coalesces duplicate safe restart requests, and restarts once the active work drains. Plain `restart` keeps the existing service-manager behavior for compatibility. Use `--force` only when you explicitly want the immediate override path.

`supportClaw gateway restart --safe --skip-deferral` runs the same SupportClaw-aware coordinated restart as `--safe`, but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Use it as the operator escape hatch when a deferral has been pinned by a stuck task run and `--safe` alone would wait indefinitely. `--skip-deferral` requires `--safe`.
...
- Set `SUPPORT_CLAW_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
- Set `SUPPORT_CLAW_GATEWAY_RESTART_TRACE=1` to log restart-scoped `restart trace:` lines for restart signal handling, active-work drain, shutdown phases, next start, ready timing, and memory metrics.
- Set `SUPPORT_CLAW_DIAGNOSTICS=timeline` with `SUPPORT_CLAW_DIAGNOSTICS_TIMELINE_PATH=<path>` to write a best-effort JSONL startup diagnostics timeline for external QA harnesses. You can also enable the flag with `diagnostics.flags: ["timeline"]` in config; the path is still env-provided. Add `SUPPORT_CLAW_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
supportClaw gateway health --url ws://127.0.0.1:18789
supportClaw gateway health --port 18789
```
...
<ParamField path="--port <port>" type="number">
  Target a local loopback Gateway on this port. This overrides `SUPPORT_CLAW_GATEWAY_URL` and `SUPPORT_CLAW_GATEWAY_PORT` for the health call.
</ParamField>
...
```bash
supportClaw gateway usage-cost
supportClaw gateway usage-cost --days 7
supportClaw gateway usage-cost --agent work --json
supportClaw gateway usage-cost --all-agents
supportClaw gateway usage-cost --json
```
...
```bash
supportClaw gateway stability
supportClaw gateway stability --type payload.large
supportClaw gateway stability --bundle latest
supportClaw gateway stability --bundle latest --export
supportClaw gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, SupportClaw writes the same diagnostic snapshot to `~/.supportClaw/logs/stability/supportClaw-stability-*.json` when the recorder has events. Inspect the newest bundle with `supportClaw gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
supportClaw gateway diagnostics export --output supportClaw-diagnostics.zip
supportClaw gateway diagnostics export --json
```
...
supportClaw gateway status --require-rpc
```
...
supportClaw gateway probe --port 18789
```
...
<AccordionGroup>
  ...
  <Accordion title="Common warning codes">
    ...
    - `multiple_gateways`: distinct gateway identities were reachable, or SupportClaw could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway does not trigger this warning.
...
```bash
supportClaw gateway call status
supportClaw gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
responsible for eventually exec'ing `supportClaw` or Node with those args.

```bash
cat > ~/.local/bin/supportClaw-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- supportClaw "$@"
EOF
chmod +x ~/.local/bin/supportClaw-doppler

supportClaw gateway install --wrapper ~/.local/bin/supportClaw-doppler --force
supportClaw gateway restart
```
...
`SUPPORT_CLAW_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
SUPPORT_CLAW_WRAPPER="$HOME/.local/bin/supportClaw-doppler" supportClaw gateway install --force
supportClaw doctor
```

To remove a persisted wrapper, clear `SUPPORT_CLAW_WRAPPER` while reinstalling:

```bash
SUPPORT_CLAW_WRAPPER= supportClaw gateway install --force
supportClaw gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway restart --safe` asks the running Gateway to preflight active SupportClaw work and defer the restart until reply delivery, embedded runs, and task runs drain. `--safe` cannot be combined with `--force` or `--wait`.
    ...
    - `gateway restart --safe --skip-deferral` runs the SupportClaw-aware safe restart but bypasses the deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch for stuck-task-run deferrals; requires `--safe`.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `SUPPORT_CLAW_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `SUPPORT_CLAW_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_supportClaw-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `supportClaw.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
supportClaw gateway discover --timeout 4000
supportClaw gateway discover --json | jq '.beacons[].wsUrl'
```
