---
summary: "CLI reference for `supportClaw migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into SupportClaw
...
```bash
supportClaw migrate list
supportClaw migrate claude --dry-run
supportClaw migrate codex --dry-run
supportClaw migrate codex --skill gog-vault77-google-workspace
supportClaw migrate codex --plugin google-calendar --dry-run
supportClaw migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
supportClaw migrate hermes --dry-run
supportClaw migrate hermes
supportClaw migrate apply codex --yes --skill gog-vault77-google-workspace
supportClaw migrate apply codex --yes --plugin google-calendar
supportClaw migrate apply codex --yes
supportClaw migrate apply claude --yes
supportClaw migrate apply hermes --yes
supportClaw migrate apply hermes --include-secrets --yes
supportClaw onboard --flow import
supportClaw onboard --import-from claude --import-source ~/.claude
supportClaw onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `supportClaw migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local SupportClaw state exists.
</ParamField>
...
`supportClaw migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `supportClaw migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an SupportClaw backup before applying the migration. If no local SupportClaw state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the SupportClaw agent workspace.
...
- Claude command Markdown files converted into SupportClaw skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. SupportClaw does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the SupportClaw Codex harness and you want to
...
Running `supportClaw migrate codex` in an interactive terminal previews the full
...
start checked unless the target SupportClaw Codex plugin config already has that
...
```bash
supportClaw migrate codex --dry-run --skill gog-vault77-google-workspace
supportClaw migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
supportClaw migrate codex --dry-run --plugin google-calendar
supportClaw migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  SupportClaw agent workspace when you want per-agent ownership.
- Source-installed `openai-curated` Codex plugins discovered through Codex
  ...
  native Codex harness; they are not exposed to SupportClaw provider runs,
...
- `SOUL.md` and `AGENTS.md` into the SupportClaw agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for SupportClaw file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that SupportClaw cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live SupportClaw config or credentials. This preserves opaque or unsafe state without pretending SupportClaw can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `supportClaw.plugin.json`:
...
Provider plugins can use `supportClaw/plugin-sdk/migration` for item construction and summary counts, plus `supportClaw/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `supportClaw onboard --flow import` and `supportClaw setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh SupportClaw setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move SupportClaw to a new machine.
