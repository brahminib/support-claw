---
summary: "CLI reference for `supportClaw devices` (device pairing + token rotation/revocation)"
read_when:
...
### `supportClaw devices remove <deviceId>`
...
```
supportClaw devices remove <deviceId>
supportClaw devices remove <deviceId> --json
```

### `supportClaw devices clear --yes [--pending]`

Clear paired devices in bulk.

```
supportClaw devices clear --yes
supportClaw devices clear --yes --pending
supportClaw devices clear --yes --pending --json
```

### `supportClaw devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, SupportClaw only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), SupportClaw supersedes the previous pending entry and issues a new `requestId`. Run `supportClaw devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
SupportClaw keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `supportClaw devices list`
or use `supportClaw devices approve --latest` to preview the exact upgrade before
approving it.
...
```
supportClaw devices approve
supportClaw devices approve <requestId>
supportClaw devices approve --latest
```

## Paperclip / `supportClaw_gateway` first-run approval

When a new Paperclip agent connects through the `supportClaw_gateway` adapter for the first time, the Gateway may require a one-time device pairing approval before runs can succeed. If Paperclip reports `supportClaw_gateway_pairing_required`, approve the pending device and retry.
...
The preview prints the exact `supportClaw devices approve <requestId>` command. Verify the request details, then rerun that command with the request ID to approve it.

For remote gateways or explicit credentials, pass the same options while previewing and approving:

```bash
supportClaw devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `supportClaw devices list` first to confirm a pending request exists.
...
### `supportClaw devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
supportClaw devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `supportClaw devices revoke --device <id> --role <role>`
...
```
supportClaw devices revoke --device <deviceId> --role node
```
...
```bash
supportClaw devices rotate --device <deviceId> --role operator
```
