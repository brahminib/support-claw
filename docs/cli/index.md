---
summary: "SupportClaw CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `supportClaw` subcommand
  - Looking up global flags or output styling rules
title: "CLI reference"
---

`supportClaw` is the main CLI entry point. Each core command has either a
...
- `supportClaw setup` creates the baseline config and workspace without walking the full guided onboarding flow.
- `supportClaw onboard` is the full guided first-run path for gateway, model auth, workspace, channels, skills, and health.
- `supportClaw configure` changes targeted parts of an existing setup, such as model auth, gateway, channels, plugins, or skills.
- `supportClaw channels add` configures channel accounts after the baseline exists; run it without flags for guided channel setup or with channel-specific flags for scripts.
...
| Flag                    | Purpose                                                               |
| ----------------------- | --------------------------------------------------------------------- |
| `--dev`                 | Isolate state under `~/.supportClaw-dev` and shift default ports         |
| `--profile <name>`      | Isolate state under `~/.supportClaw-<name>`                              |
| `--container <name>`    | Target a named container for execution                                |
| `--no-color`            | Disable ANSI colors (`NO_COLOR=1` is also respected)                  |
| `--update`              | Shorthand for [`supportClaw update`](/cli/update) (source installs only) |
...
```
supportClaw [--dev] [--profile <name>] <command>
  crestodian
  setup
  onboard
  configure
...
[`supportClaw workboard`](/cli/workboard) or `supportClaw voicecall`.
...
`supportClaw status --usage` and the Control UI surface provider usage/quota when
