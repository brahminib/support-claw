---
summary: "CLI reference for `supportClaw logs` (tail gateway logs via RPC)"
read_when:
...
`supportClaw logs` also accepts the standard Gateway client flags:
...
supportClaw logs --follow --interval 2000
supportClaw logs --limit 500 --max-bytes 500000
supportClaw logs --json
supportClaw logs --plain
supportClaw logs --no-color
supportClaw logs --limit 500
supportClaw logs --local-time
supportClaw logs --utc
supportClaw logs --follow --local-time
supportClaw logs --url ws://127.0.0.1:18789 --token "$SUPPORT_CLAW_GATEWAY_TOKEN"
```

## Notes

- Timestamps render in your local timezone by default. Use `--utc` for UTC output.
- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `supportClaw logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets do not use this fallback.
- `supportClaw logs --follow` does not follow configured-file fallbacks after implicit local Gateway RPC failures. On Linux, it uses the active user-systemd Gateway journal by PID when available and prints the selected log source; otherwise it keeps retrying the live Gateway instead of tailing a potentially stale side-by-side file.
