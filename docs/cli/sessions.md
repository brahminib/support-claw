---
summary: "CLI reference for `supportClaw sessions` (list stored sessions + usage)"
read_when:
  - You want to list stored sessions and see recent activity
title: "Sessions"
---

# `supportClaw sessions`
...
until a message is processed. Use `supportClaw channels status --probe`,
`supportClaw status --deep`, or `supportClaw health --verbose` when you need live
channel connectivity.

`supportClaw sessions` and Gateway `sessions.list` responses are bounded by
...
```bash
supportClaw sessions
supportClaw sessions --agent work
supportClaw sessions --all-agents
supportClaw sessions --active 120
supportClaw sessions --limit 25
supportClaw sessions --verbose
supportClaw sessions --json
```
...
```bash
supportClaw sessions tail
supportClaw sessions tail --follow
supportClaw sessions tail --session-key "agent:main:telegram:direct:123" --tail 25
supportClaw sessions --agent work tail --follow
supportClaw sessions --all-agents tail --follow
```

`supportClaw sessions tail` renders recent trajectory JSONL events as compact progress lines. Without `--session-key`, it tails running sessions first, then the latest stored session. `--tail <count>` controls how many existing events print before follow mode; the default is `80`, and `0` starts at the current end. `--follow` keeps watching the selected trajectory files, including relocated files referenced by `<session>.trajectory-path.json`.
...
```bash
supportClaw sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
supportClaw sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.supportClaw/trajectory-exports/` under the selected workspace.

`supportClaw sessions --all-agents` reads configured agent stores. Gateway and ACP
...
`supportClaw sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.supportClaw/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.supportClaw/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
supportClaw sessions cleanup --agent work --dry-run
supportClaw sessions cleanup --all-agents --dry-run
supportClaw sessions cleanup --enforce
supportClaw sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
supportClaw sessions cleanup --dry-run --fix-dm-scope
supportClaw sessions cleanup --json
```

`supportClaw sessions cleanup` uses `session.maintenance` settings from config:

- Scope note: `supportClaw sessions cleanup` maintains session stores, transcripts, and trajectory sidecars. It does not prune cron run history, which is managed by `cron.runLog.keepLines` in [Cron configuration](/automation/cron-jobs#configuration) and explained in [Cron maintenance](/automation/cron-jobs#maintenance).
...
`supportClaw sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.supportClaw/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.supportClaw/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
...
Reclaim context budget for a wedged or oversized session. `supportClaw sessions compact <key>` is the first-class wrapper around the `sessions.compact` gateway RPC and requires a running gateway.

```bash
supportClaw sessions compact "agent:main:main"
supportClaw sessions compact "agent:main:main" --max-lines 200
supportClaw sessions compact "agent:work:main" --agent work --json
```
...
> Note: `supportClaw agent --message '/compact ...'` is **not** a compaction path. Slash commands from the CLI are rejected by the authorized-sender check; that invocation exits non-zero with guidance pointing here instead of silently no-opping.

### sessions.compact RPC

`supportClaw gateway call sessions.compact --params '<json>'` accepts:
...
```json
{
  ...
  "key": "agent:main:main",
  "compacted": true,
  "archived": "/home/user/.supportClaw/agents/main/sessions/transcripts/<id>.jsonl.bak",
  "kept": 200
}
```
