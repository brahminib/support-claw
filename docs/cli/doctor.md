---
summary: "CLI reference for `supportClaw doctor` (health checks + guided repairs)"
read_when:
...
`supportClaw doctor` is the SupportClaw health surface. Use it when the gateway,
...
| ------- | ------------------------ | ------------------------------------------------------------------------------- |
| Inspect | `supportClaw doctor`        | Human-oriented checks and guided prompts.                                       |
| Repair  | `supportClaw doctor --fix`  | Applies supported repairs, using prompts unless non-interactive repair is safe. |
| Lint    | `supportClaw doctor --lint` | Read-only structured findings for CI, preflight, and review gates.              |
...
supportClaw doctor --lint --allow-exec
supportClaw doctor --deep
supportClaw doctor --fix
supportClaw doctor --fix --non-interactive
supportClaw doctor --generate-gateway-token
supportClaw doctor --post-upgrade
supportClaw doctor --post-upgrade --json
```

For channel-specific permissions, use the channel probes instead of `doctor`:

```bash
supportClaw channels capabilities --channel discord --target channel:<channel-id>
...
`supportClaw doctor --lint` is the read-only automation posture for doctor checks.
...
doctor --lint: ran 6 check(s), 1 finding(s)
  [warning] core/doctor/gateway-config gateway.mode - gateway.mode is unset; gateway start will be blocked.
    fix: Run `supportClaw configure` and set Gateway mode (local/remote), or `supportClaw config set gateway.mode local`.
```
  ...
  "findings": [
    {
      ...
      "path": "gateway.mode",
      "fixHint": "Run `supportClaw configure` and set Gateway mode (local/remote), or `supportClaw config set gateway.mode local`."
    }
  ]
}
```
...
example, `supportClaw doctor --lint --severity-min error` can print no findings and
...
command path. The `supportClaw/plugin-sdk/health` subpath exposes the same
...
```bash
supportClaw doctor --lint --only core/doctor/gateway-config --json
supportClaw doctor --lint --skip core/doctor/skills-readiness
```
...
`supportClaw doctor --post-upgrade` runs plugin compatibility probes intended to be
...
- In Nix mode (`SUPPORT_CLAW_NIX_MODE=1`), read-only doctor checks still work, but `doctor --fix`, `doctor --repair`, `doctor --yes`, and `doctor --generate-gateway-token` are disabled because `supportClaw.json` is immutable. Edit the Nix source for this install instead; for nix-supportClaw, use the agent-first [Quick Start](https://github.com/supportClaw/nix-supportClaw#quick-start).
...
- By default, doctor does not execute `exec` SecretRefs while checking secrets. Use `supportClaw doctor --allow-exec` or `supportClaw doctor --lint --allow-exec` only when you intentionally want doctor to run those configured secret resolvers.
- `--fix` (alias for `--repair`) writes a backup to `~/.supportClaw/supportClaw.json.bak` and drops unknown config keys, listing each removal.
...
- `doctor --fix --non-interactive` reports missing or stale gateway service definitions but does not install or rewrite them outside update repair mode. Run `supportClaw gateway install` for a missing service, or `supportClaw gateway install --force` when you intentionally want to replace the launcher.
...
- Doctor also scans `~/.supportClaw/cron/jobs.json` (or `cron.store`) for legacy cron job shapes and rewrites them before importing canonical rows into SQLite.
...
- On Linux, doctor warns when the user's crontab still runs legacy `~/.supportClaw/bin/ensure-whatsapp.sh`; that script is no longer maintained and can log false WhatsApp gateway outages when cron lacks the systemd user-bus environment.
- When WhatsApp is enabled, doctor checks for a degraded Gateway event loop with local `supportClaw-tui` clients still running. `doctor --fix` stops only verified local TUI clients so WhatsApp replies are not queued behind stale TUI refresh loops.
...
- Doctor cleans legacy plugin dependency staging state created by older SupportClaw versions and relinks the host `supportClaw` package for managed npm plugins that declare it as a peer dependency. It also repairs missing downloadable plugins that are referenced by config, such as `plugins.entries`, configured channels, configured provider/search settings, or configured agent runtimes. During package updates, doctor skips package-manager plugin repair until the package swap is complete; rerun `supportClaw doctor --fix` afterward if a configured plugin still needs recovery. If the download fails, doctor reports the install error and preserves the configured plugin entry for the next repair attempt.
...
- Set `SUPPORT_CLAW_SERVICE_REPAIR_POLICY=external` when another supervisor owns the gateway lifecycle. Doctor still reports gateway/service health and applies non-service repairs, but skips service install/start/restart/bootstrap and legacy service cleanup.
- On Linux, doctor ignores inactive extra gateway-like systemd units and does not rewrite command/entrypoint metadata for a running systemd gateway service during repair. Stop the service first or use `supportClaw gateway install --force` when you intentionally want to replace the active launcher.
...
- Doctor includes a memory-search readiness check and can recommend `supportClaw configure --section model` when embedding credentials are missing.
...
- Doctor reports an info note when Codex-mode agents are configured and personal Codex CLI assets exist in the operator's Codex home. Local Codex app-server launches use isolated per-agent homes, so install the Codex plugin first if needed, then use `supportClaw migrate plan codex` to inventory assets that should be promoted deliberately.
...
- If sandbox mode is enabled but Docker is unavailable, doctor reports a high-signal warning with remediation (`install Docker` or `supportClaw config set agents.defaults.sandbox.mode off`).
- If legacy sandbox registry files or shard directories are present (`~/.supportClaw/sandbox/containers.json`, `~/.supportClaw/sandbox/browsers.json`, `~/.supportClaw/sandbox/containers/`, or `~/.supportClaw/sandbox/browsers/`), doctor reports them; `supportClaw doctor --fix` migrates valid entries into SQLite and quarantines invalid legacy files.
...
If you previously ran `launchctl setenv SUPPORT_CLAW_GATEWAY_TOKEN ...` (or `...PASSWORD`), that value overrides your config file and can cause persistent "unauthorized" errors.

```bash
launchctl getenv SUPPORT_CLAW_GATEWAY_TOKEN
launchctl getenv SUPPORT_CLAW_GATEWAY_PASSWORD

launchctl unsetenv SUPPORT_CLAW_GATEWAY_TOKEN
launchctl unsetenv SUPPORT_CLAW_GATEWAY_PASSWORD
```
