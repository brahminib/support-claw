---
summary: "CLI reference for `supportClaw node` (headless node host)"
read_when:
...
`supportClaw node run` and `supportClaw node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `SUPPORT_CLAW_GATEWAY_TOKEN` / `SUPPORT_CLAW_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `supportClaw.json` config
key.
`supportClaw node install` persists it into the supervised node service when it is
...
```bash
supportClaw node install --host <gateway-host> --port 18789
```
...
```bash
supportClaw node status
supportClaw node start
supportClaw node stop
supportClaw node restart
supportClaw node uninstall
```

Use `supportClaw node run` for a foreground node host (no service).
...
- `supportClaw approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, SupportClaw prepares a canonical `systemRunPlan`
