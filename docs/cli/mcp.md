---
summary: "Expose SupportClaw channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to SupportClaw-backed channels
  - Running `supportClaw mcp serve`
  - Managing SupportClaw-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`supportClaw mcp` has two jobs:

- run SupportClaw as an MCP server with `supportClaw mcp serve`
- manage SupportClaw-managed outbound MCP server definitions with `list`, `show`, `status`, `doctor`, `probe`, `add`, `set`, `configure`, `tools`, `login`, `logout`, `reload`, and `unset`

In other words:

- `serve` is SupportClaw acting as an MCP server
- the other subcommands are SupportClaw acting as an MCP client-side registry for MCP servers its runtimes may consume later

<Note>
  `list`, `show`, `set`, and `unset` only read and write SupportClaw-managed `mcp.servers` entries in SupportClaw config. They do not include mcporter servers from `config/mcporter.json`; use `mcporter list` for that registry.
</Note>

Use [`supportClaw acp`](/cli/acp) when SupportClaw should host a coding harness session itself and route that runtime through ACP.

## Choose the right MCP path

SupportClaw has several MCP surfaces. Pick the one that matches who owns the agent runtime and who owns the tools.

| Goal                                                                | Use                                                                  | Why                                                                                                             |
| ------------------------------------------------------------------- | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Let an external MCP client read/send SupportClaw channel conversations | `supportClaw mcp serve`                                                 | SupportClaw is the MCP server and exposes Gateway-backed conversations over stdio.                                 |
| Save third-party MCP servers for SupportClaw-managed agent runs        | `supportClaw mcp add`, `set`, `configure`, `tools`, `login`             | SupportClaw is the MCP client-side registry and later projects those servers into eligible runtimes.               |
| Check a saved server without running an agent turn                  | `supportClaw mcp status`, `doctor`, `probe`                             | `status` and `doctor` inspect config; `probe` opens a live MCP connection and lists capabilities.               |
...
| Run ACP-hosted harness sessions                                     | [`supportClaw acp`](/cli/acp) and [ACP Agents](/tools/acp-agents-setup) | ACP bridge mode does not accept per-session MCP server injection; configure gateway/plugin bridges instead.     |

<Tip>
If you are not sure which path you need, start with `supportClaw mcp status --verbose`. It shows what SupportClaw has saved without starting any MCP servers.
</Tip>

## SupportClaw as an MCP server

This is the `supportClaw mcp serve` path.

### When to use `serve`

Use `supportClaw mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to SupportClaw-backed channel conversations
- you already have a local or remote SupportClaw Gateway with routed sessions
- you want one MCP server that works across SupportClaw's channel backends instead of running separate per-channel bridges

Use [`supportClaw acp`](/cli/acp) instead when SupportClaw should host the coding runtime itself and keep the agent session inside SupportClaw.

### How it works

`supportClaw mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote SupportClaw Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `supportClaw mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the SupportClaw Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `supportClaw agent` and `supportClaw infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by SupportClaw (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when SupportClaw already has session state with a known route such as:
...
<Tabs>
  <Tab title="Local Gateway">
    ```bash
    supportClaw mcp serve
    ```
  </Tab>
  <Tab title="Remote Gateway (token)">
    ```bash
    supportClaw mcp serve --url wss://gateway-host:18789 --token-file ~/.supportClaw/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    supportClaw mcp serve --url wss://gateway-host:18789 --password-file ~/.supportClaw/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    supportClaw mcp serve --verbose
    supportClaw mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the SupportClaw equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
`supportClaw mcp serve` supports:
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying SupportClaw channel configuration
...
SupportClaw ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `supportClaw mcp serve`
...
## SupportClaw as an MCP client registry

This is the `supportClaw mcp list`, `show`, `status`, `doctor`, `probe`, `add`, `set`,
`configure`, `tools`, `login`, `logout`, `reload`, and `unset` path.

These commands do not expose SupportClaw over MCP. They manage SupportClaw-managed MCP server definitions under `mcp.servers` in SupportClaw config. They do not read mcporter servers from `config/mcporter.json`.

Those saved definitions are for runtimes that SupportClaw launches or configures later, such as embedded SupportClaw and other runtime adapters. SupportClaw stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write SupportClaw config
    ...
    - embedded SupportClaw exposes configured MCP tools in normal `coding` and `messaging` tool profiles; `minimal` still hides them, and `tools.deny: ["bundle-mcp"]` disables them explicitly
    - per-server `toolFilter.include` and `toolFilter.exclude` filter discovered MCP tools before they become SupportClaw tools
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded SupportClaw consumes SupportClaw `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

Codex app-server also honors an optional `codex` block on each server. This is
SupportClaw projection metadata for Codex app-server threads only; it does not
change ACP sessions, generic Codex harness config, or other runtime adapters.
Use non-empty `codex.agents` to project a server only into specific SupportClaw
...
SupportClaw strips the `codex` metadata before handing the native `mcp_servers`
config to Codex.

### Saved MCP server definitions

SupportClaw also stores a lightweight MCP server registry in config for surfaces that want SupportClaw-managed MCP definitions.

Commands:

- `supportClaw mcp list`
- `supportClaw mcp show [name]`
- `supportClaw mcp status [--verbose]`
- `supportClaw mcp doctor [name] [--probe]`
- `supportClaw mcp probe [name]`
- `supportClaw mcp add <name> [flags]`
- `supportClaw mcp set <name> <json>`
- `supportClaw mcp configure <name> [flags]`
- `supportClaw mcp tools <name> [--include csv] [--exclude csv] [--clear]`
- `supportClaw mcp login <name> [--code code]`
- `supportClaw mcp logout <name>`
- `supportClaw mcp reload`
- `supportClaw mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `supportClaw mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
supportClaw mcp list
supportClaw mcp show context7 --json
supportClaw mcp status --verbose
supportClaw mcp doctor --probe
supportClaw mcp probe context7 --json
supportClaw mcp add memory --command npx --arg -y --arg @modelcontextprotocol/server-memory
supportClaw mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
supportClaw mcp tools context7 --include 'resolve-library-id,get-library-docs'
supportClaw mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
supportClaw mcp configure docs --timeout 20 --connect-timeout 5 --include 'search,read_*'
supportClaw mcp configure docs --auth oauth --oauth-scope 'docs.read'
supportClaw mcp login docs
supportClaw mcp logout docs
supportClaw mcp unset context7
```

### Common server recipes

These examples save server definitions only. Run `supportClaw mcp doctor --probe` afterward to prove that the server starts and exposes tools.

<Tabs>
  <Tab title="Filesystem">
    ```bash
    supportClaw mcp add files \
      --command npx \
      --arg -y \
      --arg @modelcontextprotocol/server-filesystem \
      --arg "$HOME/Documents" \
      --include 'read_file,list_directory,search_files'
    supportClaw mcp doctor files --probe
    ```
  ...
  <Tab title="Memory">
    ```bash
    supportClaw mcp add memory \
      --command npx \
      --arg -y \
      --arg @modelcontextprotocol/server-memory
    supportClaw mcp probe memory --json
    ```
  ...
  <Tab title="Local script">
    ```bash
    supportClaw mcp add local-tools \
      --command node \
      --arg ./dist/mcp-server.js \
      --cwd /srv/supportClaw-tools \
  ...
  <Tab title="Remote HTTP">
    ...
    supportClaw mcp doctor docs --probe
    ```
  ...
  <Tab title="Desktop/CUA">
    ```bash
    supportClaw mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
    supportClaw mcp tools cua-driver --include 'list_apps,observe,click,type'
    supportClaw mcp doctor cua-driver --probe
    ```
...
<AccordionGroup>
  <Accordion title="status --json">
    ```json
    {
      "path": "/home/user/.supportClaw/supportClaw.json",
      "servers": [
        {
          "name": "docs",
          "configured": true,
      ...
      "servers": [
        {
          ...
          "issues": [
            {
              "level": "error",
              "message": "OAuth credentials are not authorized; run supportClaw mcp login docs"
            }
          ]
        }
      ]
...
| `auth: "oauth"`                | Use MCP OAuth token storage and `supportClaw mcp login`             |
...
Sensitive values in `url` (userinfo) and `headers` are redacted in logs and status output. `supportClaw mcp doctor` warns when sensitive-looking `headers` or `env` entries contain literal values, so operators can move those values out of committed config.
...
<Steps>
  <Step title="Save the server">
    ...
    ```bash
    supportClaw mcp set docs '{"url":"https://mcp.example.com/mcp","transport":"streamable-http","auth":"oauth","oauth":{"scope":"docs.read"}}'
    ```
  ...
  <Step title="Start login">
    ...
    SupportClaw prints the authorization URL and stores temporary OAuth verifier state under the SupportClaw state directory.

  </Step>
  <Step title="Finish with the code">
    After approving in the browser, pass the returned code back to SupportClaw.

    ```bash
    supportClaw mcp login docs --code abc123
    ```
...
If the provider rotates tokens or the authorization state gets stuck, run `supportClaw mcp logout <name>`, then repeat `login`. `logout` can clear credentials for a saved HTTP server even after `auth: "oauth"` has been removed from config, as long as the server name and URL still identify the credential store entry.
...
| ------------------------------ | -------------------------------------------------------------------------------------- |
| `url`                          | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`                    | Set to `"streamable-http"` to select this transport; when omitted, SupportClaw uses `sse` |
| `headers`                      | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs`          | Per-server connection timeout in ms (optional)                                         |
| `connectTimeout`               | Per-server connection timeout in seconds (optional)                                    |
| `timeout` / `requestTimeoutMs` | Per-server MCP request timeout in seconds or ms                                        |
...
SupportClaw config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `supportClaw mcp set` and repaired by `supportClaw doctor --fix` in existing config, but `transport` is what embedded SupportClaw consumes directly.
...
Use the page for operator edits and quick inventory. Use `supportClaw mcp doctor --probe` or `supportClaw mcp probe` when you need live server proof.
...
7. Run `supportClaw mcp doctor --probe` when you need live proof that the edited server starts and lists tools.
...
- active runtimes may need `supportClaw mcp reload`, Gateway config publish, or process restart depending on which process owns the MCP clients
