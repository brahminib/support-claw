---
summary: "CLI reference for `supportClaw health` (gateway health snapshot via RPC)"
read_when:
...
supportClaw health --timeout 2500
supportClaw health --verbose
supportClaw health --debug
```

Notes:

- Default `supportClaw health` asks the running gateway for its health snapshot. When the
