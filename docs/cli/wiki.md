---
summary: "CLI reference for `supportClaw wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `supportClaw wiki`
title: "Wiki"
---
...
Use `supportClaw wiki` when you want a compiled knowledge vault with:
...
```bash
supportClaw wiki status
supportClaw wiki doctor
supportClaw wiki init
supportClaw wiki ingest ./notes/alpha.md
supportClaw wiki okf import ./knowledge-catalog/okf/bundles/ga4
supportClaw wiki compile
supportClaw wiki lint
supportClaw wiki search "alpha"
supportClaw wiki search "who should I ask about Teams?" --mode route-question
supportClaw wiki get entity.alpha --from 1 --lines 80

supportClaw wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

supportClaw wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

supportClaw wiki bridge import
supportClaw wiki unsafe-local import

supportClaw wiki obsidian status
supportClaw wiki obsidian search "alpha"
supportClaw wiki obsidian open syntheses/alpha-summary.md
supportClaw wiki obsidian command workspace:quick-switcher
supportClaw wiki obsidian daily
```
...
```bash
supportClaw wiki okf import ./bundles/ga4
supportClaw wiki okf import ./bundles/ga4 --json
supportClaw wiki search "BigQuery Table" --mode source-evidence --json
supportClaw wiki get <path-from-json-result>
```
...
- `.supportClaw-wiki/cache/agent-digest.json`
- `.supportClaw-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `supportClaw memory search` when the
...
```bash
supportClaw wiki search "bgroux" --mode find-person
supportClaw wiki search "who knows Teams rollout?" --mode route-question
supportClaw wiki search "maintainer-whois" --mode source-evidence
supportClaw wiki search "strong route Teams" --mode raw-claim --json
```
...
```bash
supportClaw wiki get entity.alpha
supportClaw wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`supportClaw wiki` behavior is shaped by:
