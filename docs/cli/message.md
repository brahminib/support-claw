---
summary: "CLI reference for `supportClaw message` (send + channel actions)"
read_when:
...
```
supportClaw message <subcommand> [flags]
```
...
- `supportClaw message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, SupportClaw will attempt a live directory lookup when the provider supports it.
...
- `supportClaw message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, SupportClaw does not force a `default` account SecretRef scope
...
```
supportClaw message send --channel discord \
...
```bash
supportClaw message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
supportClaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
supportClaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
supportClaw message poll --channel msteams \
...
```
supportClaw message react --channel slack \
...
```
supportClaw message react --channel signal \
...
```
supportClaw message send --channel telegram --target @mychat --message "Choose:" \
...
```
supportClaw message send --channel telegram --target 123456789 --message "Open app:" \
...
```bash
supportClaw message send --channel telegram --target @mychat \
