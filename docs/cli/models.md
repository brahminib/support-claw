---
summary: "CLI reference for `supportClaw models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
supportClaw models set <model-or-alias>
supportClaw models scan
```

`supportClaw models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise SupportClaw falls back to matching OAuth/API-key
...
the command uses `SUPPORT_CLAW_AGENT_DIR` if set, otherwise the
configured default agent.
Probe rows can come from auth profiles, env credentials, or `models.json`.
For OpenAI ChatGPT/Codex OAuth troubleshooting, `supportClaw models status`,
`supportClaw models auth list --provider openai`, and
`supportClaw config get agents.defaults.model --json` are the quickest way to
...
- If you omit the provider, SupportClaw resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, SupportClaw
...
By default SupportClaw tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `SUPPORT_CLAW_AGENT_DIR`)
...
- `no_model`: provider auth exists, but SupportClaw could not resolve a probeable
...
```bash
supportClaw models auth add
supportClaw models auth list [--provider <id>] [--json]
supportClaw models auth login --provider <id>
supportClaw models auth login --provider openai --profile-id openai:work
supportClaw models auth paste-api-key --provider <id>
supportClaw models auth setup-token --provider <id>
supportClaw models auth paste-token
```
...
`supportClaw plugins list` to see which providers are installed.
Use `supportClaw models auth --agent <id> <subcommand>` to write auth results to a
...
usually as a backup for Codex subscription limits. Run `supportClaw doctor --fix`
to migrate older legacy OpenAI Codex prefix auth/profile state to `openai`.

Examples:

```bash
supportClaw models auth login --provider openai --set-default
supportClaw models auth login --provider openai --method api-key
supportClaw models auth paste-api-key --provider openai
...
- `paste-api-key` accepts API keys generated elsewhere, prompts for the key
  ...
  `printf "%s\n" "$OPENAI_API_KEY" | supportClaw models auth paste-api-key --provider openai`.
...
- Anthropic note: Anthropic staff told us SupportClaw-style Claude CLI usage is allowed again, so SupportClaw treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported SupportClaw token path, but SupportClaw now prefers Claude CLI reuse and `claude -p` when available.
