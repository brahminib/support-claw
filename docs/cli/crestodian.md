...
read_when:
  - You run supportClaw with no command after setup and want to understand Crestodian
  - You need a configless-safe way to inspect or repair SupportClaw
  - You are designing or enabling message-channel rescue mode
title: "Crestodian"
---

# `supportClaw crestodian`

Crestodian is SupportClaw's local setup, repair, and configuration helper. It is
designed to stay reachable when the normal agent path is broken.

Running `supportClaw` with no command starts classic onboarding first when the
active config file is missing or has no authored settings (empty or
metadata-only). After a config file has authored settings, running `supportClaw`
with no command starts Crestodian in an interactive terminal. Running
`supportClaw crestodian` starts the same helper explicitly.

## What Crestodian shows

On startup, interactive Crestodian opens the same TUI shell used by
`supportClaw tui`, with a Crestodian chat backend. The chat log starts with a short
greeting:
...
Crestodian uses the same SupportClaw reference discovery as regular agents. In a Git checkout,
...
[https://github.com/brahminib/support-claw](https://github.com/brahminib/support-claw), with explicit
...
supportClaw
supportClaw crestodian
supportClaw crestodian --json
supportClaw crestodian --message "models"
supportClaw crestodian --message "validate config"
supportClaw crestodian --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
supportClaw crestodian --message "set default model openai/gpt-5.5" --yes
supportClaw onboard --modern
```
...
config set-ref gateway.auth.token env SUPPORT_CLAW_GATEWAY_TOKEN
gateway status
restart gateway
agents
...
plugin install clawhub:supportClaw-codex-app-server
plugin uninstall supportClaw-codex-app-server
talk to work agent
...
- `supportClaw.json` is missing
- `supportClaw.json` is invalid
- the Gateway is down
- plugin command registration is unavailable
- no agent has been configured yet

`supportClaw --help` and `supportClaw --version` still use the normal fast paths.
Noninteractive bare `supportClaw` exits with a short message instead of printing
...
```text
~/.supportClaw/audit/crestodian.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`supportClaw onboard --modern` starts Crestodian as the modern onboarding preview.
Plain `supportClaw onboard` still runs classic onboarding.
...
planner turn through SupportClaw's normal runtime paths. It first uses the
configured SupportClaw model. If no configured model is usable yet, it can fall
...
`supportClaw tui`, `supportClaw chat`, and `supportClaw terminal` still open the normal
...
SupportClaw: Crestodian rescue mode. Gateway reachable: no. Config valid: no.
You: /crestodian restart gateway
SupportClaw: Plan: restart the Gateway. Reply /crestodian yes to apply.
You: /crestodian yes
SupportClaw: Applied. Audit entry written.
```
...
- Remote rescue cannot open the local TUI or switch into an interactive agent
  session. Use local `supportClaw` for agent handoff.
