---
summary: "CLI reference for `supportClaw path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
supportClaw plugins enable oc-path
```

## Why use it

SupportClaw state is spread across human-edited markdown, commented JSONC config,
...
`supportClaw path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `supportClaw path` for ordinary whole-file edits, rich
...
```bash
supportClaw path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
supportClaw path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
supportClaw path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
supportClaw path resolve 'oc://AGENTS.md/runtime-safety/supportClaw-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
supportClaw path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
`supportClaw path` does four things:
...
# Read a leaf
supportClaw path resolve 'oc://gateway.jsonc/version'

# Wildcard search
supportClaw path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
supportClaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
supportClaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
supportClaw path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
supportClaw path emit ./AGENTS.md
```
...
supportClaw path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
supportClaw path set 'oc://supportClaw.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
supportClaw path set 'oc://supportClaw.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
supportClaw path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
supportClaw path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
supportClaw path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
supportClaw path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
supportClaw path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
supportClaw path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
supportClaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
supportClaw path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
supportClaw path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
supportClaw path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
supportClaw path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ supportClaw path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ supportClaw path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ supportClaw path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ supportClaw path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ supportClaw path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ supportClaw path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ supportClaw path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: supportClaw.invoke
```

```bash
$ supportClaw path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ supportClaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
supportClaw path resolve 'oc://AGENTS.md/tools/gh/risk' --human
supportClaw path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
supportClaw path find 'oc://AGENTS.md/tools/**/risk'
supportClaw path find 'oc://session.jsonl/[event=action]/userId'
supportClaw path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
supportClaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
supportClaw path set 'oc://gateway.jsonc/version' '2.0'
supportClaw path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ supportClaw path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
supportClaw path emit ./AGENTS.md
supportClaw path emit ./gateway.jsonc --json
```
...
`supportClaw path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__SUPPORT_CLAW_REDACTED__` (verbatim or as a substring) is refused at write
  time.
