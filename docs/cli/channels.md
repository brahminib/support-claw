---
summary: "CLI reference for `supportClaw channels` (accounts, status, login/logout, logs)"
read_when:
...
Do not use `supportClaw sessions`, Gateway `sessions.list`, or the agent
...
```bash
supportClaw channels add --channel telegram --token <bot-token>
supportClaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
supportClaw channels remove --channel telegram --delete
```

<Tip>
`supportClaw channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, SupportClaw uses the channel's default install source without opening the interactive plugin install prompt.

When you run `supportClaw channels add` without flags, the interactive wizard can prompt:
...
- Run `supportClaw status --deep` for a broad probe.
- Use `supportClaw doctor` for guided fixes.
- `supportClaw channels list` no longer prints model provider usage/quota snapshots. For those, use `supportClaw status` (overview) or `supportClaw models list` (per-provider).
- `supportClaw channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
supportClaw channels resolve --channel slack "#general" "@jane"
supportClaw channels resolve --channel discord "My Server/#support" "@someone"
supportClaw channels resolve --channel matrix "Project Room"
```
