---
summary: "CLI reference for `supportClaw configure` (interactive configuration prompts)"
read_when:
...
Use `supportClaw onboard` for the full guided first-run journey, `supportClaw setup` for the baseline config/workspace only, and `supportClaw channels add` when you only need channel account setup.
...
<Tip>
`supportClaw config` without a subcommand opens the same wizard. Use `supportClaw config get|set|unset` for non-interactive edits.
</Tip>

For web search, `supportClaw configure --section web` lets you choose a provider
...
supportClaw configure --section model --section channels
supportClaw configure --section gateway --section daemon
```
