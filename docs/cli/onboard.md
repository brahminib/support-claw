---
summary: "CLI reference for `supportClaw onboard` (interactive onboarding)"
read_when:
...
Full guided onboarding for local or remote Gateway setup. Use this when you want SupportClaw to walk through model auth, workspace, gateway, channels, skills, and health in one flow.
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How SupportClaw onboarding fits together.
  </Card>
...
supportClaw onboard --flow quickstart
supportClaw onboard --flow manual
supportClaw onboard --flow import
supportClaw onboard --import-from hermes --import-source ~/.hermes
supportClaw onboard --skip-bootstrap
supportClaw onboard --mode remote --remote-url wss://gateway-host:18789
```

`--flow import` uses plugin-owned migration providers such as Hermes. It only runs against a fresh SupportClaw setup; if existing config, credentials, sessions, or workspace memory/identity files are present, reset or choose a fresh setup before importing.

`--modern` starts the Crestodian conversational onboarding preview. Without
`--modern`, `supportClaw onboard` keeps the classic onboarding flow.

On a fresh install where the active config file is missing or has no authored
settings (empty or metadata-only), bare `supportClaw` also starts the classic
onboarding flow. Once a config file has authored settings, bare `supportClaw`
...
`SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
```bash
SUPPORT_CLAW_LOCALE=zh-CN supportClaw onboard
```
...
SupportClaw marks common vision model IDs as image-capable automatically. Pass `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
```bash
export SUPPORT_CLAW_GATEWAY_TOKEN="your-token"
supportClaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `supportClaw gateway run`.
...
<AccordionGroup>
  ...
  <Accordion title="Other behaviors">
    ...
    - Fastest first chat: `supportClaw dashboard` (Control UI, no channel setup).
...
supportClaw agents add <name>
```

Use `supportClaw setup` instead when you only need the baseline config/workspace. Use `supportClaw configure` later for targeted changes and `supportClaw channels add` for channel-only setup.
