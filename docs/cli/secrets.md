---
summary: "CLI reference for `supportClaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `supportClaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
supportClaw secrets reload
supportClaw secrets reload --json
supportClaw secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan SupportClaw state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `supportClaw.json` refs)
...
```bash
supportClaw secrets audit
supportClaw secrets audit --check
supportClaw secrets audit --json
...
```bash
supportClaw secrets configure
supportClaw secrets configure --plan-out /tmp/supportClaw-secrets-plan.json
supportClaw secrets configure --apply --yes
supportClaw secrets configure --providers-only
supportClaw secrets configure --skip-provider-setup
supportClaw secrets configure --agent ops
supportClaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `supportClaw.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, SupportClaw fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
supportClaw secrets apply --from /tmp/supportClaw-secrets-plan.json --json
```
...
- `supportClaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.supportClaw/.env` known secret keys whose values were migrated
