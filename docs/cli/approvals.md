---
summary: "CLI reference for `supportClaw approvals` and `supportClaw exec-policy`"
read_when:
...
# `supportClaw approvals`

Manage exec approvals for the **local host**, **gateway host**, or a **node host**.
By default, commands target the local approvals file on disk. Use `--gateway` to target the gateway, or `--node` to target a specific node.

Alias: `supportClaw exec-approvals`
...
## `supportClaw exec-policy`

`supportClaw exec-policy` is the local convenience command for keeping the requested
...
```bash
supportClaw exec-policy show
supportClaw exec-policy show --json

supportClaw exec-policy preset yolo
supportClaw exec-policy preset cautious --json

supportClaw exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `supportClaw exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `supportClaw approvals set --gateway`
or `supportClaw approvals set --node <id|name|ip>`.

## Common commands

```bash
supportClaw approvals get
supportClaw approvals get --node <id|name|ip>
supportClaw approvals get --gateway
```

`supportClaw approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
supportClaw approvals set --file ./exec-approvals.json
supportClaw approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off", askFallback: "full" } }
EOF
supportClaw approvals set --node <id|name|ip> --file ./exec-approvals.json
supportClaw approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested SupportClaw policy aligned, also set:
...
```bash
supportClaw approvals allowlist add "~/Projects/**/bin/rg"
supportClaw approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
supportClaw approvals allowlist add --agent "*" "/usr/bin/uname"

supportClaw approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `supportClaw nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host in the SupportClaw state dir
  (`$SUPPORT_CLAW_STATE_DIR/exec-approvals.json`, or
  `~/.supportClaw/exec-approvals.json` when the variable is unset).
