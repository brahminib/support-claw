...
read_when:
  - You want an SupportClaw agent to join a Google Meet call
  - You want an SupportClaw agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for SupportClaw — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured SupportClaw agent answers, and regular SupportClaw TTS speaks into Meet.
...
provider plus regular SupportClaw TTS. OpenAI is the default transcription
...
```bash
supportClaw googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `supportClaw googlemeet setup --json` for
...
```bash
supportClaw googlemeet setup --transport twilio
```
...
```bash
supportClaw googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
supportClaw googlemeet create --transport chrome-node --mode agent
```
...
```bash
supportClaw googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. SupportClaw uses the
  ...
  the SupportClaw Chrome profile on the node to already be signed in to Google.
...
SupportClaw's microphone/camera permission grant and avoid the Meet **Use
...
Use `supportClaw googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in SupportClaw browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by SupportClaw. For
...
You do **not** need a full SupportClaw Gateway or model API key inside a macOS VM
...
- Gateway host: SupportClaw Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: SupportClaw CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update SupportClaw in the VM, then enable the bundled plugin there:

```bash
supportClaw plugins enable google-meet
```

Start the node host in the VM:

```bash
supportClaw node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
```bash
SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  supportClaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  supportClaw node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
supportClaw node restart
```

`SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`supportClaw.json` setting. `supportClaw node install` stores it in the LaunchAgent
...
```bash
supportClaw googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, SupportClaw browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, SupportClaw auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `supportClaw node run` in the VM,
  approve pairing, and make sure `supportClaw plugins enable google-meet` and
  `supportClaw plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses SupportClaw
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. SupportClaw
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by SupportClaw; use separate virtual devices or Loopback-style routing
...
SupportClaw does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with SupportClaw, review BlackHole's
...
Chrome transport opens the Meet URL through SupportClaw browser control and joins
as the signed-in SupportClaw browser profile. On macOS, the plugin checks for
...
```bash
supportClaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
supportClaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local SupportClaw audio
...
meeting; SupportClaw does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an SupportClaw agent. Be brief.",
...
secrets out of `supportClaw.json`:
...
```bash
supportClaw config validate
supportClaw plugins list | grep -E 'google-meet|voice-call'
supportClaw googlemeet setup
```
...
`SUPPORT_CLAW_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes SupportClaw requests:
...
`meetings.space.readonly` lets SupportClaw resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets SupportClaw pass `SpaceConfig` settings such as
...
```bash
supportClaw googlemeet auth login --json
```
...
```bash
SUPPORT_CLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
SUPPORT_CLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
supportClaw googlemeet auth login --json
```
...
SUPPORT_CLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
SUPPORT_CLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
supportClaw googlemeet auth login --json --manual
```
...
support existed, rerun `supportClaw googlemeet auth login --json` so the refresh
...
```bash
supportClaw googlemeet doctor --oauth --json
```
...
```bash
supportClaw googlemeet doctor --oauth --create-space --json
supportClaw googlemeet create --no-join --json
```
...
```bash
supportClaw googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
supportClaw googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `supportClaw googlemeet auth login
...
- `SUPPORT_CLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `SUPPORT_CLAW_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `SUPPORT_CLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `SUPPORT_CLAW_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `SUPPORT_CLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `SUPPORT_CLAW_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `SUPPORT_CLAW_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
supportClaw googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
supportClaw googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
supportClaw googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
supportClaw googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
supportClaw googlemeet latest --today
supportClaw googlemeet calendar-events --today --json
supportClaw googlemeet artifacts --event "Weekly sync"
supportClaw googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
supportClaw googlemeet latest --meeting https://meet.google.com/abc-defg-hij
supportClaw googlemeet artifacts --conference-record conferenceRecords/abc123 --json
supportClaw googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
supportClaw googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
SupportClaw accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops SupportClaw's local/session
...
```bash
supportClaw googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
supportClaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
supportClaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
supportClaw googlemeet export --conference-record conferenceRecords/abc123 \
...
SUPPORT_CLAW_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
supportClaw googlemeet setup --transport chrome-node --mode transcribe
supportClaw googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `SUPPORT_CLAW_LIVE_TEST=1` enables guarded live tests.
- `SUPPORT_CLAW_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `SUPPORT_CLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `SUPPORT_CLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `SUPPORT_CLAW_GOOGLE_MEET_CLIENT_SECRET`,
  `SUPPORT_CLAW_GOOGLE_MEET_ACCESS_TOKEN`, and
  `SUPPORT_CLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `SUPPORT_CLAW_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the SupportClaw browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the SupportClaw browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, SupportClaw reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured SupportClaw TTS provider.
...
- `chrome.guestName: "SupportClaw Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through SupportClaw browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  SupportClaw agent in a per-meeting sub-agent session, and spoken back through the
  normal SupportClaw TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `supportClaw_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `supportClaw_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional SupportClaw agent id for
  `supportClaw_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `supportClaw_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured SupportClaw
agent produces the answer, and regular SupportClaw TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means SupportClaw did
...
transcripts are routed through the configured SupportClaw agent, and the answer is
spoken through the normal SupportClaw TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured SupportClaw agent | Normal SupportClaw TTS runtime            | You want "my agent is in the meeting" behavior        |
...
information, or normal SupportClaw tools, it can call `supportClaw_agent_consult`.

The consult tool runs the regular SupportClaw agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
SupportClaw sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated SupportClaw agent workspace, model defaults,
...
```bash
supportClaw googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
```bash
supportClaw googlemeet setup
supportClaw nodes status --connected
supportClaw nodes invoke \
  --node parallels-macos \
...
- `supportClaw logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
supportClaw plugins list | grep google-meet
supportClaw googlemeet setup
```
...
```bash
supportClaw plugins enable google-meet
supportClaw plugins enable browser
SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  supportClaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; SupportClaw
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, SupportClaw
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `SUPPORT_CLAW_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `supportClaw googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the SupportClaw Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. SupportClaw should click **Use microphone** or, for
...
```bash
supportClaw googlemeet setup
supportClaw googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> SupportClaw agent -> TTS talk-back path,
...
run `supportClaw googlemeet status --json <session-id>` after participants speak
...
utterance but SupportClaw did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  SupportClaw. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
supportClaw googlemeet recover-tab
supportClaw googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
supportClaw googlemeet setup --transport twilio
supportClaw voicecall setup
supportClaw voicecall smoke
```

`voicecall smoke` is readiness-only by default. To dry-run a specific number:

```bash
supportClaw voicecall smoke --to "+15555550123"
```
...
```bash
supportClaw voicecall smoke --to "+15555550123" --yes
```
...
- Run `supportClaw googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `supportClaw voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `supportClaw voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `supportClaw logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `supportClaw googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `supportClaw logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: SupportClaw owns the
