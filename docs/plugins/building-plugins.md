---
summary: "Create your first SupportClaw plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new SupportClaw plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend SupportClaw without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the SupportClaw repository. Publish
the package to [ClawHub](/clawhub) and users install it with:

```bash
supportClaw plugins install clawhub:<package-name>
```
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because SupportClaw loads bundled
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect SupportClaw to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through SupportClaw model fallback.
  </Card>
...
```json supportClaw.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to SupportClaw",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    must appear in `contracts.tools` so SupportClaw can discover ownership without
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    supportClaw plugins inspect my-plugin --runtime --json
    ```
    ...
    `supportClaw demo-plugin ping`.

    For a bundled plugin in this repository, SupportClaw discovers source-checkout
  ...
  <Step title="Install">
    ...
    ```bash
    supportClaw plugins install clawhub:your-org/your-plugin
    ```
...
operator, installed plugin code, or a modified SupportClaw runtime. Sensitive local
...
aligned with `api.registerTool(..., { optional: true })` so SupportClaw can avoid
...
```typescript
import { definePluginEntry } from "supportClaw/plugin-sdk";
```
...
`supportClaw/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `supportClaw` metadata</Check>
<Check>**supportClaw.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [supportClaw/supportClaw](https://github.com/brahminib/support-claw/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official SupportClaw X account [@supportclaw](https://x.com/supportClaw) for release announcements.
