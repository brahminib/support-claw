...
Plugin hooks are in-process extension points for SupportClaw plugins. Use them
...
current plugin options; SupportClaw injects it per handler without mutating the
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when SupportClaw has resolved the child session's native model before launch.
...
use `supportClaw sessions` or the Gateway session/status surfaces. When debugging
...
- SupportClaw strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, SupportClaw stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when SupportClaw can
...
identifier when SupportClaw can derive one from the session key or delivery
metadata.
...
forever. A timeout is logged and SupportClaw continues; it does not cancel
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when SupportClaw can derive a
...
Codex native `Stop` hooks are relayed into this hook as SupportClaw
...
reach the next model turn exactly once. SupportClaw drains queued injections before
...
policy runs from SupportClaw config, covers CLI install and update paths, and fails
closed when enabled but unavailable.

`before_install` is a plugin-runtime lifecycle hook. It runs after
`security.installPolicy` only in the SupportClaw process where plugin hooks have
...
field remains in the event payload for compatibility, but SupportClaw no longer
...
context when syncing external wake schedulers, and keep SupportClaw as the
