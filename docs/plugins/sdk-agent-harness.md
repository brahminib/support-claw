...
An **agent harness** is the low level executor for one prepared SupportClaw agent
...
runtime and the normal SupportClaw provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the SupportClaw
  session transcript
...
Before a harness is selected, SupportClaw has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the SupportClaw transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an SupportClaw-owned
policy bundle for runtime decisions that must stay shared across SupportClaw and native
harnesses:
...
Harnesses may use the plan for decisions that need to match SupportClaw behavior, but
should still treat it as host-owned attempt state. Do not mutate it or use it to
switch providers/models inside a turn.

## Register a harness

**Import:** `supportClaw/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "supportClaw/plugin-sdk/agent-harness";
...
SupportClaw chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, SupportClaw uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, SupportClaw does not
...
`agents.list[].agentRuntime`, and `SUPPORT_CLAW_AGENT_RUNTIME`. `/status` shows the
...
SupportClaw. The harness then claims that provider in `supports(...)`.
...
- app-server request: SupportClaw sends the bare model id to Codex and lets the
...
SupportClaw requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
SupportClaw only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning SupportClaw to the newer tested stable line.
...
seam is for async tool-result transforms that must run before SupportClaw or Codex
...
`supportClaw/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so SupportClaw's fallback policy can decide whether to retry on a
different model. `planning-only` requires the harness's explicit `planText`
field; SupportClaw does not infer it from assistant prose. The helper intentionally
...
`supportClaw/plugin-sdk/agent-harness-runtime` after they finalize an attempt. It
dispatches the portable `agent_end` hook and SupportClaw's research capture without
...
The bundled `codex` harness is the native Codex mode for embedded SupportClaw
...
`supportClaw doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. SupportClaw still owns the chat channel,
...
By default, SupportClaw uses `auto` provider/model runtime policy: registered
...
fail hard. This does not block an explicit provider/model `agentRuntime.id: "supportClaw"`.
...
Keep that binding explicitly associated with the SupportClaw session, and keep
mirroring user-visible assistant/tool output into the SupportClaw transcript.

The SupportClaw transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in SupportClaw harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so SupportClaw can
clear it when the owning SupportClaw session is reset.

## Tool and media results

Core constructs the SupportClaw tool list and passes it into the prepared attempt.
...
on the same delivery path as SupportClaw-backed runs.
