---
summary: "Set up Codex Computer Use for Codex-mode SupportClaw agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode SupportClaw agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. SupportClaw
...
Use this page when SupportClaw is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## SupportClaw.app and Peekaboo

SupportClaw.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want SupportClaw.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode SupportClaw agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an SupportClaw node and exposes mobile
...
SupportClaw-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through SupportClaw's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the SupportClaw command:

```bash
cua-driver mcp-config --client supportClaw
```
...
available as a normal SupportClaw MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. SupportClaw
...
Computer Use in and lets SupportClaw install or re-enable it before the turn:
...
With this config, SupportClaw checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, SupportClaw asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, SupportClaw also tries to
...
plugin command surface is available. These are SupportClaw chat/runtime commands,
not `supportClaw codex ...` CLI subcommands:
...
SupportClaw uses the same app-server API that Codex itself exposes. The
...
During install, SupportClaw polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, SupportClaw prefers
...
`computer-use` is registered, SupportClaw tries to add the standard bundled
...
## What SupportClaw checks

SupportClaw reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If SupportClaw says Computer Use
...
SupportClaw intentionally fails closed when `computerUse.enabled` is true. A
...
fresh SupportClaw session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook could not reach an active SupportClaw relay through the local bridge or
Gateway fallback. Start a fresh SupportClaw session with `/new` or `/reset`. If it
works once and then fails again on a later tool call, `/new` is only clearing the
current attempt; restart the Codex app-server or SupportClaw Gateway so old threads
