...
Message presentation is SupportClaw's shared contract for rich outbound chat UI.
...
```bash
supportClaw message send --channel slack \
  --target channel:C123 \
...
```bash
supportClaw message send --channel telegram \
...
Use helpers from `supportClaw/plugin-sdk/interactive-runtime` when bridging older
code:
