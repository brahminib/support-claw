...
read_when:
  - You want to place an outbound voice call from SupportClaw
...
Voice calls for SupportClaw via a plugin. Supports outbound notifications,
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        supportClaw plugins install @supportclaw/voice-call
        ```
      </Tab>
      <Tab title="From a local folder (dev)">
        ```bash
        PLUGIN_SRC=./path/to/local/voice-call-plugin
        supportClaw plugins install "$PLUGIN_SRC"
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `supportClaw doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `supportClaw doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `supportClaw_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal SupportClaw tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `supportClaw_agent_consult`.
...
configured SupportClaw agent without paying a full agent-consult round trip on
...
`supportClaw_agent_consult` still run the full SupportClaw agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call supportClaw_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `supportClaw doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
supportClaw voicecall call --to "+15555550123" --message "Hello from SupportClaw"
supportClaw voicecall start --to "+15555550123"   # alias for call
supportClaw voicecall continue --call-id <id> --message "Any questions?"
supportClaw voicecall speak --call-id <id> --message "One moment"
supportClaw voicecall dtmf --call-id <id> --digits "ww123456#"
supportClaw voicecall end --call-id <id>
supportClaw voicecall status --call-id <id>
supportClaw voicecall tail
supportClaw voicecall latency                      # summarize turn latency from logs
supportClaw voicecall expose --mode funnel
```
...
```bash
supportClaw voicecall setup
supportClaw voicecall setup --json
```
...
Provider signatures are checked against the public URL SupportClaw reconstructs
...
Use `supportClaw logs --follow` for the live phase trace. A healthy Twilio Meet
...
`supportClaw voicecall tail` still shows persisted call records; it is useful for
...
- `supportClaw logs --follow` shows realtime TwiML served, the realtime bridge
