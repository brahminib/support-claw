---
summary: "Run SupportClaw embedded agent turns through the bundled Codex app-server harness"
title: "Codex harness"
read_when:
  - You want to use the bundled Codex app-server harness
  - You need Codex harness config examples
  - You want Codex-only deployments to fail instead of falling back to SupportClaw
---

The bundled `codex` plugin lets SupportClaw run embedded OpenAI agent turns
through Codex app-server instead of the built-in SupportClaw harness.
...
app-server execution. SupportClaw still owns chat channels, session files, model
selection, SupportClaw dynamic tools, approvals, media delivery, and the visible
transcript mirror.
...
When no SupportClaw sandbox is active, SupportClaw starts Codex app-server threads
...
SupportClaw dynamic tools continue through the app-server `item/tool/call` bridge.
Active SupportClaw sandboxing and restricted tool policies disable native code mode
entirely unless you opt into the experimental sandbox exec-server path.

This Codex-native feature is separate from
[SupportClaw code mode](/reference/code-mode), which is an opt-in QuickJS-WASI
runtime for generic SupportClaw runs with a different `exec` input shape.
...
- SupportClaw with the bundled `codex` plugin available.
...
- Codex auth available through `supportClaw models auth login --provider openai`,
...
Most users who want Codex in SupportClaw want this path: sign in with a
...
```bash
supportClaw models auth login --provider openai
```
...
harness options in SupportClaw config, and use the CLI only for Codex auth:

| Need                                   | Set                                                                              | Where                              |
| -------------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------- |
| Enable the harness                     | `plugins.entries.codex.enabled: true`                                            | SupportClaw config                    |
| Keep an allowlisted plugin install     | Include `codex` in `plugins.allow`                                               | SupportClaw config                    |
| Route OpenAI agent turns through Codex | `agents.defaults.model` or `agents.list[].model` as `openai/gpt-*`               | SupportClaw agent config              |
| Sign in with ChatGPT/Codex OAuth       | `supportClaw models auth login --provider openai`                                   | CLI auth profile                   |
| Add API-key backup for Codex runs      | `openai:*` API-key profile listed after subscription auth in `auth.order.openai` | CLI auth profile + SupportClaw config |
| Fail closed when Codex is unavailable  | Provider or model `agentRuntime.id: "codex"`                                     | SupportClaw model/provider config     |
| Use direct OpenAI API traffic          | Provider or model `agentRuntime.id: "supportClaw"` with normal OpenAI auth          | SupportClaw model/provider config     |
...
Codex compacts through its native app-server thread state, so SupportClaw ignores
those local summarizer overrides at runtime and `supportClaw doctor --fix` removes
...
`agents.defaults.compaction.provider`. `supportClaw doctor --fix` migrates the old
...
the bound thread. SupportClaw does not wait for completion, impose an SupportClaw
...
turns. The API key is only an auth fallback, not a request to switch to SupportClaw or
...
- Do not use legacy Codex GPT refs in config. Run `supportClaw doctor --fix` to
...
- `agentRuntime.id: "supportClaw"` opts a provider or model into the SupportClaw
...
| Direct OpenAI API-key traffic through SupportClaw       | Provider or model `agentRuntime.id: "supportClaw"` and normal OpenAI auth | `/status` shows SupportClaw runtime        | Use only when SupportClaw is intentional |
| Legacy config                                        | legacy Codex GPT refs                                                  | `supportClaw doctor --fix` rewrites it     | Do not write new config this way      |
...
With Codex forced, SupportClaw fails early if the Codex plugin is disabled, the
app-server is too old, or the app-server cannot start.

## App-server policy

By default, the plugin starts SupportClaw's managed Codex binary locally with stdio
...
implicit YOLO posture, SupportClaw selects allowed guardian permissions instead.
When an SupportClaw sandbox is active for the session, SupportClaw disables Codex
...
through SupportClaw sandbox-backed dynamic tools such as `sandbox_exec` and
`sandbox_process` when the normal exec/process tools are available.

Use normalized SupportClaw exec mode when you want Codex native auto-review before
...
For Codex app-server sessions, SupportClaw maps `tools.exec.mode: "auto"` to Codex
...
In `tools.exec.mode: "auto"`, SupportClaw does not preserve legacy unsafe Codex
...
works, but `tools.exec.mode: "auto"` is the normalized SupportClaw surface.
...
supports SupportClaw text commands.
...
- `/codex resume <thread-id>` attaches the current SupportClaw session to an
...
1. Ordered OpenAI auth profiles for the agent, preferably under
   `auth.order.openai`. Run `supportClaw doctor --fix` to migrate older
...
When SupportClaw sees a ChatGPT subscription-style Codex auth profile, it removes
...
When native Codex plugins are configured, SupportClaw installs or refreshes those
plugins through the connected app-server before exposing plugin-owned apps to
the Codex thread.

If a subscription profile hits a Codex usage limit, SupportClaw records the reset
...
For local stdio app-server launches, SupportClaw sets `CODEX_HOME` to a per-agent
...
default. SupportClaw preserves the normal process `HOME`; Codex-run subprocesses
...
SupportClaw removes `CODEX_HOME` and `HOME` from this list during local launch
normalization: `CODEX_HOME` stays per-agent, and `HOME` stays inherited so
subprocesses can use normal user-home state.

Codex dynamic tools default to `searchable` loading. SupportClaw does not expose
...
SupportClaw integration tools such as messaging, media, cron, browser, nodes,
gateway, and `heartbeat_respond` are available through Codex tool search under
the `supportClaw` namespace, keeping the initial model context smaller. Web search
uses Codex's hosted `web_search` tool by default when search is enabled and no
managed provider is selected. Native hosted search and SupportClaw's managed
`web_search` dynamic tool are mutually exclusive so managed search cannot bypass
native domain restrictions. SupportClaw uses the managed tool when hosted search is
unavailable, explicitly disabled, or replaced by a selected managed provider.
SupportClaw keeps Codex's standalone `web.run` extension disabled because
...
SupportClaw or ACP delegation is still available through the `supportClaw` dynamic
...
| -------------------------- | -------------- | ---------------------------------------------------------------------------------------- |
| `codexDynamicToolsLoading` | `"searchable"` | Use `"direct"` to put SupportClaw dynamic tools directly in the initial Codex tool context. |
| `codexDynamicToolsExclude` | `[]`           | Additional SupportClaw dynamic tool names to omit from Codex app-server turns.              |
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after SupportClaw builds its inherited environment. SupportClaw keeps per-agent `CODEX_HOME` and inherited `HOME` for local launches.                                                                                                                                                                              |
| `codeModeOnly`                                | `false`                                                | Opt into Codex's code-mode-only tool surface. SupportClaw dynamic tools remain registered with Codex so nested `tools.*` calls return through the app-server `item/tool/call` bridge.                                                                                                                                                                                                              |
| `remoteWorkspaceRoot`                         | unset                                                  | Remote Codex app-server workspace root. When set, SupportClaw infers the local workspace root from the resolved SupportClaw workspace, preserves the current cwd suffix under this remote root, and sends only the final app-server cwd to Codex. If the cwd is outside the resolved SupportClaw workspace root, SupportClaw fails closed instead of sending a gateway-local path to the remote app-server. |
| `requestTimeoutMs`                            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                                                                                                                                                                                     |
| `turnCompletionIdleTimeoutMs`                 | `60000`                                                | Quiet window after Codex accepts a turn or after a turn-scoped app-server request while SupportClaw waits for `turn/completed`.                                                                                                                                                                                                                                                                    |
| `postToolRawAssistantCompletionIdleTimeoutMs` | `300000`                                               | Completion-idle and progress guard used after a tool handoff, native tool completion, post-tool raw assistant progress, raw reasoning completion, or reasoning progress while SupportClaw waits for `turn/completed`. Use this for trusted or heavy workloads where post-tool synthesis can legitimately stay quiet longer than the final assistant release budget.                                |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an SupportClaw sandbox is active, `danger-full-access` turns use Codex `workspace-write` with network access derived from the SupportClaw sandbox egress setting.                                                                                     |
| `approvalsReviewer`                           | `"user"` or an allowed guardian reviewer               | Use `"auto_review"` to let Codex review native approval prompts when allowed, otherwise `guardian_subagent` or `user`. `guardian_subagent` remains a legacy alias.                                                                                                                                                                                                                              |
| `serviceTier`                                 | unset                                                  | Optional Codex app-server service tier. `"priority"` enables fast-mode routing, `"flex"` requests flex processing, `null` clears the override, and legacy `"fast"` is accepted as `"priority"`.                                                                                                                                                                                                 |
| `networkProxy`                                | disabled                                               | Opt into Codex permissions-profile networking for app-server commands. SupportClaw defines the selected `permissions.<profile>.network` config and selects it with `default_permissions` instead of sending `sandbox`.                                                                                                                                                                             |
| `experimental.sandboxExecServer`              | `false`                                                | Preview opt-in that registers an SupportClaw sandbox-backed Codex environment with Codex app-server 0.132.0 or newer so native Codex execution can run inside the active SupportClaw sandbox.                                                                                                                                                                                                         |

`appServer.networkProxy` is explicit because it changes the Codex sandbox
contract. When enabled, SupportClaw also sets `features.network_proxy.enabled` and
`default_permissions` in the Codex thread config so the generated permission
profile can start Codex managed networking. By default, SupportClaw generates a
collision-resistant `supportClaw-network-<fingerprint>` profile name from the
...
SupportClaw-owned dynamic tool calls are bounded independently from
`appServer.requestTimeoutMs`: Codex `item/tool/call` requests use a 90 second
SupportClaw watchdog by default. A positive per-call `timeoutMs` argument extends
...
capped at 600000 ms. On timeout, SupportClaw aborts the tool signal
...
After Codex accepts a turn, and after SupportClaw responds to a turn-scoped
...
quiet for `appServer.turnCompletionIdleTimeoutMs`, SupportClaw best-effort
interrupts the Codex turn, records a diagnostic timeout, and releases the
SupportClaw session lane so follow-up chat messages are not queued behind a stale
...
longer post-tool idle budget: after SupportClaw returns an `item/tool/call`
...
without `turn/completed`, SupportClaw best-effort interrupts the native turn and
...
timeouts still retire the stuck app-server client and release the SupportClaw
...
- `SUPPORT_CLAW_CODEX_APP_SERVER_BIN`
- `SUPPORT_CLAW_CODEX_APP_SERVER_ARGS`
- `SUPPORT_CLAW_CODEX_APP_SERVER_MODE=yolo|guardian`
- `SUPPORT_CLAW_CODEX_APP_SERVER_APPROVAL_POLICY`
- `SUPPORT_CLAW_CODEX_APP_SERVER_SANDBOX`

`SUPPORT_CLAW_CODEX_APP_SERVER_BIN` bypasses the managed binary when
`appServer.command` is unset.

`SUPPORT_CLAW_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`SUPPORT_CLAW_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is
...
capabilities in the same Codex thread as the SupportClaw harness turn. SupportClaw
does not translate Codex plugins into synthetic `codex_plugin_*` SupportClaw
dynamic tools.
...
Thread app config is computed when SupportClaw establishes a Codex harness session
...
The short version: SupportClaw does not vendor the desktop-control app or execute
...
- SupportClaw dynamic tools are supported. Codex asks SupportClaw to execute those
  tools, so SupportClaw remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  SupportClaw can observe or block selected native events through the supported
  relay, but it does not rewrite native tool arguments.
- Codex owns native compaction. SupportClaw keeps a transcript mirror for channel
  history, search, `/new`, `/reset`, and future model or harness switching, but
  it does not replace Codex compaction with an SupportClaw or context-engine
  summarizer.
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching SupportClaw provider/model settings.
- `tool_result_persist` applies to SupportClaw-owned transcript tool results, not
...
**SupportClaw uses the built-in harness instead of Codex:** make sure the model ref is
...
Ask affected collaborators to run this read-only command on their SupportClaw host:

```bash
(
  pattern='openai/gpt-5\.[45]|openai[-]codex|agentRuntime(\.id)?|harnessRuntime|Runtime: OpenAI Codex|legacy OpenAI Codex prefix|resolveSelectedOpenAIRuntimeProvider|candidateProvider[": ]+openai|status[": ]+401|Incorrect API key|No API key|api-key path|API-key path|OAuth'

  if ls /tmp/supportClaw/supportClaw-*.log >/dev/null 2>&1; then
    grep -E -i -n "$pattern" /tmp/supportClaw/supportClaw-*.log 2>/dev/null || true
  else
    journalctl --user -u supportClaw-gateway --since today --no-pager 2>/dev/null \
...
**Legacy Codex model refs config remains:** run `supportClaw doctor --fix`.
...
`0.125.0-alpha.2` or `0.125.0+custom` are rejected because SupportClaw tests the
...
the Codex thread is still trying to use a native hook relay id that SupportClaw no
...
app-server or SupportClaw Gateway so old threads are dropped and native hook
