---
summary: "Run SupportClaw embedded agent turns through the external GitHub Copilot SDK harness"
...
read_when:
  ...
  - You are wiring an agent to subscription Copilot (github / supportClaw / copilot) and want it to run through the Copilot CLI
---

The external `@supportclaw/copilot` plugin lets SupportClaw run embedded subscription
...
SupportClaw still owns chat channels, session files, model selection, SupportClaw
...
[Side questions (`/btw`)](#side-questions-btw)), and `supportClaw doctor`.
...
- SupportClaw with the `@supportclaw/copilot` plugin installed.
- If your config uses `plugins.allow`, include `copilot` (the manifest
  id declared by the plugin). A restrictive
  allowlist that uses the npm-style `@supportclaw/copilot` package name
...
- A writable `copilotHome` directory. The harness defaults to
  `<agentDir>/copilot` when SupportClaw provides an agent directory, otherwise
  `~/.supportClaw/agents/<agentId>/copilot` for full per-agent isolation.

`supportClaw doctor` runs the plugin
...
The Copilot runtime is an external plugin so the core `supportClaw` package does
...
Without the opt-in, supportClaw uses its built-in GitHub Copilot provider
and never installs the runtime plugin.

The runtime resolves the SDK in this order:

1. `import("@github/copilot-sdk")` from the installed `@supportclaw/copilot`
   package.
2. The well-known fallback dir `~/.supportClaw/npm-runtime/copilot/` (the
...
4. **Env-var fallback** for direct CLI / dogfood runs where no auth
   ...
   setup:
   1. `SUPPORT_CLAW_GITHUB_TOKEN` -- harness-specific override; set this
      to pin a token for the SupportClaw harness without disturbing
...
(isolating SDK state from SupportClaw's `models.json` / `auth-profiles.json` in
the same directory), or `~/.supportClaw/agents/<agentId>/copilot` otherwise.
...
- `model` — string or `{ provider, id, api? }`. When omitted, SupportClaw uses
...
- `reasoningEffort` — `"low" | "medium" | "high" | "xhigh"`. Maps from
  SupportClaw's `ThinkLevel` / `ReasoningLevel` resolution in
...
- `hooksConfig` — optional native Copilot SDK `SessionHooks` compatibility
  ...
  It is separate from SupportClaw's portable lifecycle hooks.
- `permissionPolicy` — optional override for the SDK's
  ...
  invokes any of those kinds because every bridged SupportClaw tool is
  registered with `overridesBuiltInTool: true` and
  `skipPermission: true` so 100% of tool calls flow through SupportClaw's
...
SupportClaw plugin hooks do not need Copilot-specific attempt configuration. The
...
`before_compaction` and `after_compaction`. Bridged SupportClaw tools continue to
...
Nothing in the rest of SupportClaw needs to know about these fields. Other
...
The SupportClaw side transcript mirror (see below) continues to receive the
...
SupportClaw audit transcript via
...
deliberately leaves `harness.runSideQuestion` undefined, so SupportClaw's `/btw`
...
asserted in
[`extensions/copilot/harness.test.ts`](https://github.com/brahminib/support-claw/blob/main/extensions/copilot/harness.test.ts)
...
- **Interactive `ask_user` is not yet wired.** The SDK's
  ...
  `UserInputRequest`s through the SupportClaw channel/TUI prompt path.

## Permissions and ask_user

Permission enforcement for bridged SupportClaw tools happens **inside the
...
`createSupportClawCodingTools` to every coding tool: loop detection,
...
`createSupportClawCodingTools` — identity (`senderIsOwner`,
