...
The plugin is included with SupportClaw, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
Use the WebSocket RPC path for SupportClaw clients and interactive tools that can keep a Gateway WebSocket connection open.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    supportClaw plugins enable admin-http-rpc
...
```bash
supportClaw plugins disable admin-http-rpc
supportClaw gateway restart
```
...
- For `token` and `password` auth, narrower `x-supportClaw-scopes` headers are ignored and the normal full operator defaults are restored.
- Trusted identity-bearing HTTP modes honor `x-supportClaw-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for SupportClaw clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.
