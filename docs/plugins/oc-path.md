---
summary: "Bundled `oc-path` plugin: ships the `supportClaw path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`supportClaw path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the SupportClaw repo under
...
- **Local automation**: shell scripts can resolve or update one workspace value
  with `supportClaw path … --json` instead of carrying separate markdown, JSONC,
...
supportClaw path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
supportClaw path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `supportClaw` CLI** on the host where you
...
The plugin metadata lives in `extensions/oc-path/supportClaw.plugin.json`:
...
`supportClaw path …`, so installs that never use the verb pay no cost.
...
state. Bare `supportClaw path` invocations work immediately on the same host —
the CLI loads the plugin on demand.

Disable with:

```bash
supportClaw plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `supportClaw path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__SUPPORT_CLAW_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`supportClaw path` CLI reference](/cli/path)
