...
read_when:
  ...
  - You need the SupportClaw plugin that owns the node-llama-cpp runtime
...
The main `supportClaw` npm package does not include `node-llama-cpp`. Keeping the
native dependency in this plugin prevents normal SupportClaw npm updates from
deleting a manually installed runtime inside the SupportClaw package directory.
