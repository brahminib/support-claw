...
read_when:
  - You maintain an SupportClaw plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

SupportClaw keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core SupportClaw repo as a separate
...
```sh
supportClaw-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. SupportClaw
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `supportClaw` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against SupportClaw plugin packages.
Run it from a clean SupportClaw checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @supportclaw/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @supportclaw/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @supportclaw/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
```
...
SupportClaw should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `supportClaw/plugin-sdk/compat`
...
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `supportClaw/extension-api`,
  `supportClaw/plugin-sdk/channel-runtime`, `supportClaw/plugin-sdk/command-auth`
  status builders, `supportClaw/plugin-sdk/test-utils` (replaced by focused
  `supportClaw/plugin-sdk/*` test subpaths), and the `ClawdbotConfig` /
  `SupportClawSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `supportClaw/plugin-sdk/channel-route`
...
- channel `showConfigured` / `showInSetup` metadata while channel packages move
  to `supportClaw.channel.exposure`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `supportClaw plugins registry --refresh` and
