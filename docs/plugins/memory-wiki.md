...
If the active memory plugin exposes shared recall artifacts, SupportClaw can search
...
currently exposing public bridge inputs yet. Run `supportClaw wiki doctor` first,
...
`supportClaw wiki status`, `supportClaw wiki doctor`, and `supportClaw wiki bridge
...
artifact, then let `memory-wiki` turn it into SupportClaw-native concept pages and
compiled digests.
...
supportClaw wiki apply synthesis "Alpha Summary" --body "..." --source-id source.alpha
