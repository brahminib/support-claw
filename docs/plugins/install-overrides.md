...
[`supportClaw plugins install`](/cli/plugins).
...
```bash
export SUPPORT_CLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1
export SUPPORT_CLAW_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/supportClaw-codex-2026.5.8.tgz",
  "supportClaw-web-search": "npm:@supportclaw/web-search@2026.5.8"
}'
```
...
SupportClaw uses the override source instead of the catalog, bundled, or default
...
entry normally represents an SupportClaw-owned package, an override is treated as
operator-supplied test input.

Workspace `.env` files cannot enable install overrides. Set these variables in
the trusted shell, CI job, or remote test command that launches SupportClaw.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal SupportClaw state:

```bash
npm pack extensions/codex --pack-destination /tmp

SUPPORT_CLAW_STATE_DIR="$(mktemp -d)" \
SUPPORT_CLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1 \
SUPPORT_CLAW_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/supportClaw-codex-2026.5.8.tgz"}' \
pnpm supportClaw onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$SUPPORT_CLAW_STATE_DIR/npm/projects" -path '*/node_modules/@supportclaw/codex/package.json' -print
grep -R '"@supportclaw/codex"' "$SUPPORT_CLAW_STATE_DIR/npm/projects"/*/package-lock.json
```
