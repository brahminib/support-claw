...
By default, SupportClaw starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after SupportClaw builds its inherited environment.                                                                                                                                                                                                                                                             |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active SupportClaw sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows SupportClaw sandbox egress.                                                                                                                                                                                       |
...
SupportClaw treats non-loopback WebSocket app-server URLs as remote and requires
...
shorthand before SupportClaw builds app-server start options, and unresolved
structured SecretRefs fail before any token or header is sent. When native Codex
plugins are configured, SupportClaw uses the connected app-server's plugin control
plane to install or refresh those plugins and then refreshes app inventory so
plugin-owned apps are visible to the Codex thread. Only connect SupportClaw to
remote app-servers that are trusted to accept SupportClaw-managed plugin installs
...
unattended SupportClaw turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, SupportClaw treats the implicit default as guardian
...
When an SupportClaw sandbox is active, the local Codex app-server process still
runs on the Gateway host. SupportClaw therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the SupportClaw sandbox
backend. Shell access is exposed through SupportClaw sandbox-backed dynamic tools
...
`workspace-write` without active SupportClaw sandboxing. If you see
`bwrap: setting up uid map: Permission denied` or
`bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`, run
`supportClaw doctor` and fix the reported host namespace policy for the SupportClaw
...
The stable default is fail-closed: active SupportClaw sandboxing disables native
...
try Codex's remote environment support with SupportClaw's sandbox backend. This
...
When the flag is on and the current SupportClaw session is sandboxed, SupportClaw
...
SupportClaw-owned environment. If the app-server cannot register the environment,
...
loopback exec-server unless it is running on the same host, so SupportClaw rejects
that combination.
...
1. An explicit SupportClaw Codex auth profile for the agent.
...
Stdio app-server launches inherit SupportClaw's process environment by default.
SupportClaw owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's SupportClaw state. That keeps Codex config,
accounts, plugin cache/data, and thread state scoped to the SupportClaw agent
instead of leaking in from the operator's personal `~/.codex` home.

SupportClaw does not rewrite `HOME` for normal local app-server launches. Codex-run
subprocesses such as `supportClaw`, `gh`, `git`, cloud CLIs, and shell commands see
...
SupportClaw plugins and SupportClaw skill snapshots still flow through SupportClaw's own
...
part of an SupportClaw agent, inventory them explicitly:

```bash
supportClaw migrate codex --dry-run
supportClaw migrate apply codex --yes
```
...
Most remaining SupportClaw integration tools, such as messaging, media, cron,
browser, nodes, gateway, `heartbeat_respond`, and `web_search`, are available
through Codex tool search under the `supportClaw` namespace. This keeps the initial
...
surface, while explicit SupportClaw or ACP delegation is still available through
the `supportClaw` dynamic tool namespace.
...
Dynamic tool budgets are capped at 600000 ms. On timeout, SupportClaw aborts the
...
post-tool idle budget: after SupportClaw returns an `item/tool/call` response, after
...
`turn/completed`, SupportClaw best-effort interrupts the native turn and releases
...
availability is owned by Codex app-server, so the list can change when SupportClaw
...
If discovery fails or times out, SupportClaw uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. SupportClaw
...
For SupportClaw workspace parity, the Codex harness resolves the other bootstrap
files. `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, and `USER.md` are forwarded as
SupportClaw Codex developer instructions because they define the active agent,
available workspace guidance, and user profile. The compact SupportClaw skills
...
`BOOTSTRAP.md` when present is forwarded as SupportClaw turn input reference
context.
