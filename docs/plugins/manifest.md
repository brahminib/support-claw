...
read_when:
  - You are building an SupportClaw plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native SupportClaw plugin manifest** only.
...
SupportClaw auto-detects those bundle layouts too, but they are not validated
against the `supportClaw.plugin.json` schema described here.

For compatible bundles, SupportClaw currently reads bundle metadata plus declared
...
SupportClaw runtime expectations.

Every native SupportClaw plugin **must** ship a `supportClaw.plugin.json` file in the
**plugin root**. SupportClaw uses this manifest to validate configuration
...
`supportClaw.plugin.json` is the metadata SupportClaw reads **before it loads your
...
- QA runner metadata the shared `supportClaw qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; SupportClaw still reads this during the deprecation window.                                                             |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                                      |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that SupportClaw can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                                        |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `supportClaw qa` host before plugin runtime loads.                                                                                                                                                  |
...
SupportClaw reads these fields before provider runtime loads so core tools can
...
ownership. `toolMetadata` declares cheap availability evidence so SupportClaw can
...
If a tool has no `toolMetadata`, SupportClaw preserves the existing behavior and
...
SupportClaw reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                                              | User-facing label. If omitted, SupportClaw falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. SupportClaw
...
the shared `supportClaw qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `supportClaw qa`, for example `matrix`.    |
...
SupportClaw also includes `setup.providers[].envVars` in generic provider auth and
...
SupportClaw can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. SupportClaw treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `supportClaw.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
SupportClaw reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that SupportClaw can
read without importing the plugin runtime.

```json
{
  "contracts": {
    "agentToolResultMiddleware": ["supportClaw", "codex"],
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `supportClaw migrate`.                                                                         |
...
SupportClaw reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, SupportClaw can still load the plugin, but
...
the same defaults through `package.json#supportClaw.channel.commands` alongside
...
When `channels.chat` is configured, SupportClaw considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, SupportClaw disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, SupportClaw
...
Use `modelSupport` when SupportClaw should infer your provider plugin from
...
SupportClaw applies this precedence:
...
Use `modelCatalog` when SupportClaw should know provider model metadata before
...
provider-filtered list uses an alias, SupportClaw can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching SupportClaw model config compatibility.  |
...
refreshable rows are not authoritative by themselves. Use `runtime` when SupportClaw
...
exec provider preset. SupportClaw reads this metadata before plugin runtime loads,
...
The map key is the integration id. If `providerAlias` is omitted, SupportClaw uses
...
When an operator selects the preset, SupportClaw writes a provider reference like:
...
At startup/reload, SupportClaw resolves that provider by loading current plugin
...
SupportClaw materializes it at startup/reload to the current Node executable and
...
SupportClaw derives `trustedDirs` for manifest presets from the plugin root and,
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the SupportClaw provider id, for example `z-ai` for a `zai` provider. |
...
### SupportClaw Provider Index

The SupportClaw Provider Index is SupportClaw-owned preview metadata for providers
...
4. SupportClaw Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `supportClaw doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `supportClaw.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `supportClaw` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if SupportClaw must know it before loading plugin code, put it in `supportClaw.plugin.json`
...
`supportClaw` block instead of `supportClaw.plugin.json`.
`supportClaw.bundle` and `supportClaw.bundle.json` are not SupportClaw plugin contracts;
native plugins must use `supportClaw.plugin.json` plus the supported
`package.json#supportClaw` fields below.

Important examples:

| Field                                                                                      | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `supportClaw.extensions`                                                                      | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `supportClaw.runtimeExtensions`                                                               | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `supportClaw.setupEntry`                                                                      | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `supportClaw.runtimeSetupEntry`                                                               | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `supportClaw.channel`                                                                         | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `supportClaw.channel.commands`                                                                | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `supportClaw.channel.configuredState`                                                         | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `supportClaw.channel.persistedAuthState`                                                      | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `supportClaw.install.clawhubSpec` / `supportClaw.install.npmSpec` / `supportClaw.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `supportClaw.install.defaultChoice`                                                           | Preferred install path when multiple install sources are available.                                                                                                                  |
| `supportClaw.install.minHostVersion`                                                          | Minimum supported SupportClaw host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                             |
| `supportClaw.compat.pluginApi`                                                                | Minimum SupportClaw plugin API range required by this package, using a semver floor like `>=2026.5.27`.                                                                                 |
| `supportClaw.install.expectedIntegrity`                                                       | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `supportClaw.install.allowInvalidConfigRecovery`                                              | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `supportClaw.install.requiredPlatformPackages`                                                | npm package aliases that must materialize when their lockfile platform constraints match the current host.                                                                           |
| `supportClaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Lets setup-runtime channel surfaces load before listen, then defers the full configured channel plugin until post-listen activation.                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#supportClaw.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `supportClaw.plugin.json`.

`supportClaw.install.minHostVersion` is enforced during install and manifest
...
`supportClaw.install.requiredPlatformPackages` is for npm packages that expose
...
SupportClaw verifies only the declared alias whose lockfile constraints match the
current host. If npm reports success but omits that alias, SupportClaw retries once
with a fresh cache and rolls back the install if the alias is still missing.

`supportClaw.compat.pluginApi` is enforced during package install for non-bundled
plugin sources. Use it for the SupportClaw plugin SDK/runtime API floor that the
...
flows. Official SupportClaw release sync bumps existing official plugin API floors
to the SupportClaw release version by default, but plugin-only releases can keep a
lower floor when the package intentionally supports older hosts. Do not use the
package version alone as the compatibility contract. `peerDependencies.supportClaw`
remains npm package metadata; SupportClaw uses the `supportClaw.compat.pluginApi`
...
`"npmSpec": "@wecom/wecom-supportClaw-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `supportClaw.setupEntry` when status, channel list,
...
entrypoint fields. For example, `supportClaw.runtimeExtensions` cannot make an
escaping `supportClaw.extensions` path loadable.

`supportClaw.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `supportClaw doctor --fix`.

`supportClaw.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`supportClaw.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
SupportClaw discovers plugins from several roots. For the raw filesystem scan
...
2. **Bundled** — plugins shipped with SupportClaw
3. **Global install** — plugins installed into the global SupportClaw plugin root
...
- When extending or forking a bundled plugin with new config keys, update that plugin's `supportClaw.plugin.json` `configSchema` at the same time. Bundled plugin schemas are strict, so adding `plugins.entries.<id>.config.myNewKey` in user config without adding `myNewKey` to `configSchema.properties` will be rejected before the plugin runtime loads.
...
- The manifest is **required for native SupportClaw plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `SupportClawPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
