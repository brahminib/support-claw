---
summary: "Configure migrated native Codex plugins for Codex-mode SupportClaw agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode SupportClaw agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode SupportClaw agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the SupportClaw turn.

SupportClaw does not translate Codex plugins into synthetic `codex_plugin_*`
SupportClaw dynamic tools. Plugin calls stay in the native Codex transcript, and
...
- The selected SupportClaw agent runtime must be the native Codex harness.
...
`codexPlugins` has no effect on SupportClaw runs, normal OpenAI provider runs, ACP
...
```bash
supportClaw migrate codex --dry-run --verify-plugin-apps
```
...
`enable` and `disable` write only to SupportClaw config at
`~/.supportClaw/supportClaw.json`; they do not edit `~/.codex/config.toml` or install
...
- Enabled: SupportClaw config is willing to make the plugin available to Codex
  harness turns.
...
Migration is the durable install/eligibility step. During planning, SupportClaw
...
SupportClaw reads Codex app inventory through app-server `app/list`, caches it for
one hour, and refreshes stale or missing entries asynchronously. The cache is
in memory only; restarting the CLI or gateway drops it, and SupportClaw rebuilds it
...
A plugin app is exposed only when SupportClaw can map it back to the migrated
...
SupportClaw injects a restrictive `config.apps` patch for the Codex thread:
`_default` is disabled and only apps owned by enabled migrated plugins are
enabled.

SupportClaw sets app-level `destructive_enabled` from the effective global or
...
plugin apps are emitted with `open_world_enabled: true`; SupportClaw does not
...
- Per-plugin `allow_destructive_actions` overrides the global policy for that
  plugin.
- When policy is `false`, SupportClaw returns a deterministic decline.
- When policy is `true`, SupportClaw auto-accepts only safe schemas it can map to
  an approval response, such as a boolean approve field.
- When policy is `"auto"`, SupportClaw exposes destructive plugin actions to
  Codex but turns ownership-proven MCP approval elicitations into SupportClaw
...
empty or stale cache. SupportClaw schedules an async refresh and excludes plugin
...
Codex thread bindings keep the app config they started with until SupportClaw
