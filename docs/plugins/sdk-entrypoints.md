...
when SupportClaw loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, SupportClaw will use a
...
**Import:** `supportClaw/plugin-sdk/tool-plugin`
...
schemas, wraps plain return values in the SupportClaw tool-result format, and
exposes static metadata that `supportClaw plugins build` writes into the plugin
manifest.
...
- `configSchema` is optional. When omitted, SupportClaw uses a strict empty object
...
- Tool names are static. `supportClaw plugins build` derives `contracts.tools`
...
- Runtime loading stays strict. Installed plugins still need
  `supportClaw.plugin.json` and `package.json` `supportClaw.extensions`; SupportClaw does
  not execute plugin code to infer missing manifest data.

## `definePluginEntry`

**Import:** `supportClaw/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `SupportClawPluginConfigSchema \| () => SupportClawPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: SupportClawPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `supportClaw.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- SupportClaw resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `supportClaw/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: SupportClawPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: SupportClawPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. SupportClaw may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and SupportClaw
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `supportClaw nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; SupportClaw resolves it to the
...
SupportClaw loads this instead of the full entry when a channel is disabled,
...
- `supportClaw/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
...
- `supportClaw/plugin-sdk/channel-setup` for optional-install setup surfaces
- `supportClaw/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`supportClaw/plugin-sdk/channel-entry-contract` instead. That contract lets the
setup entry keep setup-safe plugin/secrets exports while still exposing a
runtime setter:

```typescript
import { defineBundledChannelSetupEntry } from "supportClaw/plugin-sdk/channel-entry-contract";
...
the plugin entry and the channel plugin object so SupportClaw can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want SupportClaw to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; SupportClaw rejects descriptor names outside
...
SupportClaw classifies loaded plugins by their registration behavior:
...
Use `supportClaw plugins inspect <id>` to see a plugin's shape.
