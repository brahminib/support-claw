...
At startup, SupportClaw does roughly this:
...
The manifest is the control-plane source of truth. SupportClaw uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, SupportClaw
...
SupportClaw does not cache plugin discovery results or direct manifest registry
...
SupportClaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, SupportClaw calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that SupportClaw no longer calls, such as
...
| --  | _(built-in model lookup)_         | SupportClaw tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
| 24  | `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared SupportClaw refreshers                                                                                          |
...
that still runs on SupportClaw's normal inference loop.

`resolveUsageAuth` decides whether SupportClaw should call `fetchUsageSnapshot` or
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of SupportClaw's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as SupportClaw adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while SupportClaw owns the
...
- SupportClaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "SupportClaw plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-supportClaw-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-supportClaw-scopes` only when the header is explicitly present
  - if `x-supportClaw-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-supportClaw-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `supportClaw/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `supportClaw/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `supportClaw/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `supportClaw/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `supportClaw/plugin-sdk/config-schema` | Root `supportClaw.json` Zod schema (`SupportClawSchema`) |
...
<Info>
`supportClaw/plugin-sdk/channel-runtime`, `supportClaw/plugin-sdk/channel-lifecycle`,
small channel helper facades, `supportClaw/plugin-sdk/outbound-runtime`,
`supportClaw/plugin-sdk/outbound-send-deps`, `supportClaw/plugin-sdk/config-runtime`,
and `supportClaw/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `supportClaw/plugin-sdk/*` subpaths. Never
...
`supportClaw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape SupportClaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to SupportClaw's
...
- if both `catalog` and `discovery` are registered, SupportClaw uses `catalog`
...
- Read-only command paths such as `supportClaw status`, `supportClaw status --all`,
  `supportClaw channels status`, `supportClaw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `supportClaw.extensions`:
...
Security guardrail: every `supportClaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `supportClaw plugins install` installs plugin dependencies with a
...
Optional: `supportClaw.setupEntry` can point at a lightweight setup-only module.
When SupportClaw needs setup surfaces for a disabled channel plugin, or
...
Optional: `supportClaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let SupportClaw load the
...
Channel plugins can advertise setup/discovery metadata via `supportClaw.channel` and
install hints via `supportClaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@supportclaw/nextcloud-talk",
  "supportClaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@supportclaw/nextcloud-talk",
...
Useful `supportClaw.channel` fields beyond the minimal example:
...
SupportClaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.supportClaw/mpm/plugins.json`
- `~/.supportClaw/mpm/catalog.json`
- `~/.supportClaw/plugins/catalog.json`

Or point `SUPPORT_CLAW_PLUGIN_CATALOG_PATHS` (or `SUPPORT_CLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "supportClaw": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `supportClaw.install` block. The
...
2. add typed plugin registration/runtime surfaces
   Extend `SupportClawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how SupportClaw stays opinionated without becoming hardcoded to one
