...
SupportClaw Gateway; it is not a replacement for GitHub Issues, Linear, Jira, or
...
state directory and move with the rest of that Gateway's SupportClaw state.
...
under the SupportClaw state directory. Boards, cards, labels, lifecycle events,
...
`supportClaw doctor --fix` to migrate the shipped legacy plugin-state namespaces
...
system processes; normal SupportClaw subagent sessions still own execution. The
...
SupportClaw records the orchestration intent and exposes it in worker context; the
...
supportClaw workboard create "Fix stale card lifecycle" --priority high --labels bug,workboard
supportClaw workboard show <card-id>
supportClaw workboard dispatch
```

`supportClaw workboard dispatch` calls the running Gateway so worker starts use the
...
```bash
supportClaw plugins disable workboard
supportClaw gateway restart
```
