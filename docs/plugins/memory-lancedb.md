...
The plugin is published to npm and is not bundled into the SupportClaw runtime image.
...
Set `dimensions` for non-standard embedding models. SupportClaw knows the
...
`memory_forget` query path, and `supportClaw ltm search`. Auto-recall prefers the
...
```bash
supportClaw ltm list
supportClaw ltm search "project preferences"
supportClaw ltm stats
```
...
```bash
supportClaw ltm query --cols id,text,createdAt --limit 20
supportClaw ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.supportClaw/memory/lancedb`. Override the
path with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "~/.supportClaw/memory/lancedb",
          embedding: {
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/supportClaw",
          storageOptions: {
...
SupportClaw treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade SupportClaw and restart the
Gateway.
...
```bash
supportClaw ltm stats
supportClaw ltm search "recent preference"
```
