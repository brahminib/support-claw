---
summary: "How SupportClaw installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged SupportClaw installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

SupportClaw keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the SupportClaw
package directory.
...
- SDK/core imports are peer or supplied SupportClaw imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into SupportClaw-owned package roots

SupportClaw owns only the plugin lifecycle:
...
SupportClaw uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.supportClaw/npm/projects/<encoded-package>`
- git packages clone under `~/.supportClaw/git`
...
```bash
cd ~/.supportClaw/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`supportClaw plugins install npm-pack:<path.tgz>` uses that same per-plugin npm
project root for a local npm-pack tarball. SupportClaw reads the tarball's npm
...
`node_modules` beside the plugin package. SupportClaw scans the managed project
...
publishable lockfile during install, and SupportClaw's managed npm project root
supports it through the normal npm install path. SupportClaw-owned publishable
...
SupportClaw does not require it for community packages, but npm will respect it
when present.

SupportClaw-owned npm plugin packages can also publish with explicit
...
with `supportClaw.release.bundleRuntimeDependencies: false`; those packages still
...
`supportClaw` package does not bundle its full dependency tree.

Plugins that import `supportClaw/plugin-sdk/*` declare `supportClaw` as a peer
dependency. SupportClaw does not let npm install a separate registry copy of the
...
resolution/materialization and SupportClaw reasserts plugin-local
`node_modules/supportClaw` links for installed packages that declare the host peer
...
Local plugins are treated as developer-controlled directories. SupportClaw does not
...
supportClaw plugins install <source>
supportClaw doctor --fix
```

`doctor --fix` can clean legacy SupportClaw-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of SupportClaw.
...
In source checkouts, SupportClaw treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g supportClaw`        | Built runtime tree inside the package | SupportClaw package and explicit plugin install/update/doctor flows     |
...
| `supportClaw plugins install ...`   | Managed npm project/git/ClawHub root  | The plugin install/update flow                                       |

## Legacy cleanup

Older SupportClaw versions generated bundled-plugin dependency roots at startup or
...
`.supportClaw-runtime-deps*` manifests, generated plugin `node_modules`, install
...
Older npm installs also used a shared `~/.supportClaw/npm/node_modules` root.
