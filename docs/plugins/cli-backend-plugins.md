...
read_when:
  ...
  - You need to map a third-party CLI into SupportClaw's text fallback runner
---

CLI backend plugins let SupportClaw call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points SupportClaw at the plugin runtime module              |
| Manifest ownership   | `supportClaw.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ```json package.json
    {
      "name": "@acme/supportClaw-acme-cli",
      "version": "1.0.0",
      "type": "module",
      "supportClaw": {
        "extensions": ["./index.ts"],
    ...
    entry is `./src/index.ts`, add `supportClaw.runtimeExtensions` that points at
  ...
  <Step title="Declare backend ownership">
    ```json supportClaw.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through SupportClaw",
    ...
    `cliBackends` is the runtime ownership list. It lets SupportClaw auto-load the
  ...
  <Step title="Register the backend">
    ...
    } from "supportClaw/plugin-sdk/cli-backend";
...
`CliBackendConfig` describes how SupportClaw should launch and parse the CLI:
...
| `modelAliases`                            | Map SupportClaw model ids to CLI-native ids                    |
| `sessionArg` / `sessionArgs`              | How to pass a session id                                    |
| `sessionMode`                             | `always`, `existing`, or `none`                             |
| `sessionIdFields`                         | JSON fields SupportClaw reads from CLI output                  |
...
| `defaultAuthProfileId`             | Prefer a specific SupportClaw auth profile                                     |
...
| `bundleMcp` / `bundleMcpMode`      | Opt into SupportClaw's loopback MCP tool bridge                                |
| `ownsNativeCompaction`             | Backend owns its own compaction - SupportClaw defers                           |
...
otherwise SupportClaw fails closed when BTW requires a no-tools CLI run.

### `ownsNativeCompaction`: opting out of SupportClaw compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so SupportClaw's safeguard summarizer never runs against its
...
stay over budget / go stale (SupportClaw no longer rescues it):
...
CLI backends do not receive SupportClaw tools by default. If the CLI can consume an
...
"always-on"` so SupportClaw can fail closed when a caller requires no native tools.
...
```bash
supportClaw plugins inspect acme-cli --runtime --json
supportClaw agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `supportClaw.extensions` and built runtime entries for published packages</Check>
<Check>`supportClaw.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
