---
summary: "Build simple typed agent tools with defineToolPlugin and supportClaw plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple SupportClaw plugin that only adds agent tools
...
Tool plugins add agent-callable tools to SupportClaw without adding a channel,
model provider, hook, service, or setup backend. Use `defineToolPlugin` when the
plugin owns a fixed list of tools and you want SupportClaw to generate the manifest
metadata that keeps those tools discoverable without loading runtime code.

The recommended flow is:

1. Scaffold a package with `supportClaw plugins init`.
2. Write tools with `defineToolPlugin`.
3. Build JavaScript.
4. Generate `supportClaw.plugin.json` and `package.json` metadata with
   `supportClaw plugins build`.
...
- `supportClaw >=2026.5.17`, the first SupportClaw version that exports
  `supportClaw/plugin-sdk/tool-plugin`.
- A package root that can ship `dist/`, `supportClaw.plugin.json`, and
  `package.json`.
...
- `supportClaw.plugin.json`: generated manifest metadata for the initial tool.
...
import { defineToolPlugin } from "supportClaw/plugin-sdk/tool-plugin";
...
`supportClaw plugins build` writes the matching `toolMetadata.<tool>.optional`
manifest entry, so SupportClaw can discover the tool without loading plugin
runtime code.
...
`defineToolPlugin` wraps plain return values into the SupportClaw tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and SupportClaw to keep the original value in `details`.
...
`configSchema` is optional. If you omit it, SupportClaw uses a strict empty object
...
SupportClaw reads plugin config from the plugin entry in the Gateway config. Do not
...
SupportClaw discovers installed plugins from cold metadata. It must be able to read
the plugin manifest before importing plugin runtime code. `defineToolPlugin`
therefore exposes static metadata, and `supportClaw plugins build` writes that
...
`contracts.tools` is the important discovery contract. It tells SupportClaw which
...
For the simple tool-plugin workflow, `supportClaw plugins build` aligns
`package.json` to the selected single runtime entry:

```json
{
  "type": "module",
  "files": ["dist", "supportClaw.plugin.json", "README.md"],
  "dependencies": {
    "typebox": "^1.1.38"
  },
  "peerDependencies": {
    "supportClaw": ">=2026.5.17"
  },
  "supportClaw": {
...
- `supportClaw.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `supportClaw.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate SupportClaw checkout or installed CLI, install the package path:

```bash
supportClaw plugins install ./stock-quotes
supportClaw plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
supportClaw plugins install npm-pack:./supportClaw-plugin-stock-quotes-0.1.0.tgz
supportClaw plugins inspect stock-quotes --runtime --json
```
...
```bash
supportClaw plugins install clawhub:your-org/stock-quotes
```

Bare npm package specs remain supported during the launch cutover, but ClawHub
is the preferred discovery and distribution surface for SupportClaw plugins.
...
`supportClaw plugins build --entry ./dist/index.js` or
`supportClaw plugins validate --entry ./dist/index.js`.
...
### `supportClaw.plugin.json generated metadata is stale`
...
Commit both `supportClaw.plugin.json` and `package.json` changes.

### `package.json supportClaw.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`supportClaw plugins build --entry ./dist/index.js` so the generator aligns the
...
1. `supportClaw plugins inspect <plugin-id> --runtime`
2. `supportClaw plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `supportClaw.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `supportClaw.extensions: ["./dist/index.js"]`.
