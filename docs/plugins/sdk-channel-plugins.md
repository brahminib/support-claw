---
summary: "Step-by-step guide to building a messaging channel plugin for SupportClaw"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect SupportClaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects SupportClaw to a
...
<Info>
  If you have not built any SupportClaw plugin before, read
...
Channel plugins do not need their own send/edit/react tools. SupportClaw keeps one
...
`defineChannelMessageAdapter` from `supportClaw/plugin-sdk/channel-outbound`. The
...
receive/send lifecycle helpers on `supportClaw/plugin-sdk/channel-outbound`.

Channels migrating inbound authorization can use the experimental
`supportClaw/plugin-sdk/channel-ingress-runtime` subpath from runtime receive
...
`supportClaw/plugin-sdk/outbound-media` with plugin state stores. Keep platform
...
Use `supportClaw/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `supportClaw/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If custom approval auth intentionally allows only same-chat fallback, return `markImplicitSameChatApprovalAuthorization({ authorized: true })` from `supportClaw/plugin-sdk/approval-auth-runtime`; otherwise core treats the result as explicit approver authorization.
...
- `supportClaw/plugin-sdk/approval-auth-runtime`
- `supportClaw/plugin-sdk/approval-client-runtime`
- `supportClaw/plugin-sdk/approval-delivery-runtime`
- `supportClaw/plugin-sdk/approval-gateway-runtime`
- `supportClaw/plugin-sdk/approval-handler-adapter-runtime`
- `supportClaw/plugin-sdk/approval-handler-runtime`
- `supportClaw/plugin-sdk/approval-native-runtime`
- `supportClaw/plugin-sdk/approval-reply-runtime`
- `supportClaw/plugin-sdk/channel-runtime-context`

Likewise, prefer `supportClaw/plugin-sdk/setup-runtime`,
`supportClaw/plugin-sdk/setup-runtime`,
`supportClaw/plugin-sdk/reply-runtime`,
`supportClaw/plugin-sdk/reply-dispatch-runtime`,
`supportClaw/plugin-sdk/reply-reference`, and
`supportClaw/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `supportClaw/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `supportClaw/plugin-sdk/setup-runtime` includes the env-aware adapter seam for
  `createEnvPatchedAccountSetupAdapter`
- `supportClaw/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `supportClaw.setupEntry` in
...
- use the broader `supportClaw/plugin-sdk/setup` seam only when you also need the
...
- `supportClaw/plugin-sdk/account-core`,
  `supportClaw/plugin-sdk/account-id`,
  `supportClaw/plugin-sdk/account-resolution`, and
  `supportClaw/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `supportClaw/plugin-sdk/inbound-envelope` and
  `supportClaw/plugin-sdk/channel-inbound` for inbound route/envelope and
  record-and-dispatch wiring
- `supportClaw/plugin-sdk/channel-targets` for target parsing helpers
- `supportClaw/plugin-sdk/outbound-media` for media loading and
  `supportClaw/plugin-sdk/channel-outbound` for outbound identity/send delegates
  and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `supportClaw/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `supportClaw/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `supportClaw/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `supportClaw/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `supportClaw/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `supportClaw/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`supportClaw/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#supportClaw-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/supportClaw-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "supportClaw": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect SupportClaw to Acme Chat."
        }
      }
    }
    ```

    ```json supportClaw.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { SupportClawConfig } from "supportClaw/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "supportClaw/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so SupportClaw
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "supportClaw/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    SupportClaw loads this instead of the full entry when the channel is disabled
    ...
    `supportClaw/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    SupportClaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to SupportClaw.
...
├── package.json              # supportClaw.channel metadata
├── supportClaw.plugin.json      # Manifest with config schema
