...
read_when:
  ...
  - You want a reference for all registration methods on SupportClawPluginApi
...
<Note>
  This page is for plugin authors using `supportClaw/plugin-sdk/*` inside
  SupportClaw. For external apps, scripts, dashboards, CI jobs, and IDE extensions
...
prefer `supportClaw/plugin-sdk/channel-core`; keep `supportClaw/plugin-sdk/core` for
...
`supportClaw.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. SupportClaw's
...
<Warning>
  ...
  `supportClaw/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`supportClaw/plugin-sdk/discord` and `supportClaw/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `SupportClawPluginApi` object with these
methods:
...
agentPromptGuidance: [
  "Global command hint.",
  { text: "Only show this in the main SupportClaw prompt.", surfaces: ["supportClaw_main"] },
];
```

Structured `surfaces` may include `supportClaw_main`, `codex_app_server`,
`cli_backend`, `acp_backend`, or `subagent`. `pi_main` remains a deprecated alias
for `supportClaw_main`. Omit `surfaces` for intentional all-surface guidance. Do
...
| `api.registerNodeCliFeature(registrar, opts?)` | Node feature CLI under `supportClaw nodes` |
...
runtime, for example `["supportClaw", "codex"]`. Installed plugins without that
contract, or without explicit enablement, cannot register this middleware; keep
normal SupportClaw plugin hooks for work that does not need pre-model tool-result
timing. The old
...
Gateway on a local discovery transport such as mDNS/Bonjour. SupportClaw calls the
...
`supportClaw nodes canvas` explicit plugin-owned node features.
...
- User config still wins. SupportClaw merges `agents.defaults.cliBackends.<id>` over the
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping SupportClaw thinking levels to a native effort
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `supportClaw/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep SupportClaw as the source of truth for due checks and execution.
...
| `api.config`             | `SupportClawConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `supportClaw/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when SupportClaw is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through SupportClaw's plugin
...
- **`@supportclaw/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@supportclaw/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `supportClaw/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `supportClaw/plugin-sdk/speech`, `.../provider-model-shared`, or another
