...
The Policy plugin contributes doctor health checks for policy-managed SupportClaw
...
posture, ingress/channel access posture, data-handling posture, and SupportClaw config secret
provider/auth profile posture.

Policy stores authored requirements in `policy.jsonc`, observes existing
SupportClaw settings and workspace declarations as evidence, and reports drift
through `supportClaw policy check` and `supportClaw doctor --lint`. A clean policy
check emits policy, evidence, findings, and attestation hashes that operators
can record for audit.

`supportClaw policy compare --baseline <file>` compares one policy file to another
