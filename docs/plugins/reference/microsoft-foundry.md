---
summary: "Adds Microsoft Foundry model provider support to SupportClaw."
read_when:
...
Adds Microsoft Foundry model provider support to SupportClaw.

## Distribution

- Package: `@supportclaw/microsoft-foundry`
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. SupportClaw refreshes Microsoft Foundry runtime tokens through
...
SupportClaw uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so SupportClaw
...
deployment through onboarding or include model metadata so SupportClaw can verify
