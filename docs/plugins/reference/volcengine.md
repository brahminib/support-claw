---
summary: "Adds Volcengine, Volcengine Plan model provider support to SupportClaw."
read_when:
  - You are installing, configuring, or auditing the volcengine plugin
title: "Volcengine plugin"
---

# Volcengine plugin

Adds Volcengine, Volcengine Plan model provider support to SupportClaw.

## Distribution

- Package: `@supportclaw/volcengine-provider`
- Install route: included in SupportClaw

## Surface

providers: volcengine, volcengine-plan; contracts: speechProviders

## Related docs

- [volcengine](/providers/volcengine)
