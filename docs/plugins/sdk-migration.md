...
read_when:
  - You see the SUPPORT_CLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the SUPPORT_CLAW_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before SupportClaw 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external SupportClaw plugin
---

SupportClaw has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`supportClaw/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`supportClaw/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`supportClaw/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`supportClaw/extension-api`** - a bridge that gave plugins direct access to
...
SupportClaw does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`supportClaw/plugin-sdk/\<subpath\>`)
...
`supportClaw/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `supportClaw doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `supportClaw/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `SupportClawConfig` | `supportClaw/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `supportClaw/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `supportClaw/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `supportClaw/plugin-sdk/config-mutation` |
    | Session store helpers | `supportClaw/plugin-sdk/session-store-runtime` |
    | Markdown table config | `supportClaw/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `supportClaw/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `supportClaw/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `supportClaw/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    ```typescript
    // SupportClaw and Codex runtime dynamic tools
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `supportClaw/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `supportClaw/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "supportClaw/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "supportClaw/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "supportClaw/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "supportClaw/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "supportClaw/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedAgent } from "supportClaw/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `supportClaw/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `supportClaw/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `supportClaw/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `supportClaw/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `supportClaw/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `supportClaw/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `supportClaw/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `supportClaw/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `supportClaw/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `supportClaw/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `supportClaw/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `supportClaw/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `supportClaw/plugin-sdk/error-runtime` |
    | Transport readiness waits | `supportClaw/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `supportClaw/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `supportClaw/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `supportClaw/plugin-sdk/number-runtime` |
    | Process-local async lock | `supportClaw/plugin-sdk/async-lock-runtime` |
    | File locks | `supportClaw/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `supportClaw/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `SupportClawSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | SupportClaw-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@supportclaw/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`supportClaw/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`supportClaw/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "supportClaw/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "supportClaw/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `supportClaw/plugin-sdk/channel-inbound` or
    `supportClaw/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `supportClaw/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `supportClaw/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `supportClaw/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `supportClaw/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    SupportClaw no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. SupportClaw downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="SupportClawSchemaType alias → SupportClawConfig">
    `SupportClawSchemaType` re-exported from `supportClaw/plugin-sdk` is now a
    one-line alias for `SupportClawConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { SupportClawSchemaType } from "supportClaw/plugin-sdk";
    // After
    import type { SupportClawConfig } from "supportClaw/plugin-sdk/config-schema";
    ```
...
```bash
SUPPORT_CLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 supportClaw gateway run
SUPPORT_CLAW_SUPPRESS_EXTENSION_API_WARNING=1 supportClaw gateway run
```
