---
summary: "Install and use Codex, Claude, and Cursor bundles as SupportClaw plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how SupportClaw maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

SupportClaw can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
SupportClaw maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native SupportClaw plugins. Native plugins run
...
of requiring authors to rewrite them as native SupportClaw plugins, SupportClaw
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    supportClaw plugins install ./my-bundle

    # Archive
    supportClaw plugins install ./my-bundle.tgz
...
## What SupportClaw maps from bundles

Not every bundle feature runs in SupportClaw today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal SupportClaw skills                                                 | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                        | Claude, Cursor |
| Hook packs    | SupportClaw-style `HOOK.md` + `handler.ts` layouts                                                   | Codex          |
| MCP tools     | Bundle MCP config merged into embedded SupportClaw settings; supported stdio and HTTP servers loaded | All formats    |
| LSP servers   | Claude `.lsp.json` and manifest-declared `lspServers` merged into embedded SupportClaw LSP defaults  | Claude         |
| Settings      | Claude `settings.json` imported as embedded SupportClaw defaults                                     | Claude         |

#### Skill content

- bundle skill roots load as normal SupportClaw skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal SupportClaw skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal SupportClaw hook-pack
  layout. Today this is primarily the Codex-compatible case:
  - `HOOK.md`
  - `handler.ts` or `handler.js`

#### MCP for embedded SupportClaw

- enabled bundles can contribute MCP server config
- SupportClaw merges bundle MCP config into the effective embedded SupportClaw settings as
  `mcpServers`
- SupportClaw exposes supported bundle MCP tools during embedded SupportClaw agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, SupportClaw uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in SupportClaw config. `supportClaw mcp set` and `supportClaw doctor --fix` normalize the common alias.
...
SupportClaw registers bundle MCP tools with provider-safe names in the form
...
#### Embedded SupportClaw settings

- Claude `settings.json` is imported as default embedded SupportClaw settings when the
  bundle is enabled
- SupportClaw sanitizes shell override keys before applying them
...
#### Embedded SupportClaw LSP

- enabled Claude bundles can contribute LSP server config
- SupportClaw loads `.lsp.json` plus any manifest-declared `lspServers` paths
- bundle LSP config is merged into the effective embedded SupportClaw LSP defaults
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `supportClaw plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but SupportClaw does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit SupportClaw best when they use skill roots and SupportClaw-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded SupportClaw settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded SupportClaw
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded SupportClaw LSP defaults
...
SupportClaw checks for native plugin format first:

1. `supportClaw.plugin.json` or valid `package.json` with `supportClaw.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, SupportClaw uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `supportClaw plugins install` and ship everything
  they need in the installed plugin directory.
- SupportClaw-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `supportClaw doctor --fix` removes legacy staged dependency directories and can
...
- SupportClaw does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `supportClaw plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded SupportClaw settings from `settings.json` are supported. SupportClaw does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    SupportClaw hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
