...
CLI onboarding is the **recommended** terminal setup path for SupportClaw on
...
`SUPPORT_CLAW_LOCALE`, then `LC_ALL`, then `LC_MESSAGES`, then `LANG`, and falls
...
Names and stable identifiers stay literal: `SupportClaw`, `Gateway`, `Tailscale`,
commands, config keys, URLs, provider IDs, model IDs, and plugin/channel labels
are not translated.

<Info>
Fastest first chat: open the Control UI (no channel setup needed). Run
`supportClaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`supportClaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.supportClaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `supportClaw doctor` first.
</Note>
...
Use `supportClaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.supportClaw/workspace-<agentId>`.
...
- CLI command reference: [`supportClaw onboard`](/cli/onboard)
