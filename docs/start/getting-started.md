---
summary: "Get SupportClaw installed and run your first chat in minutes."
read_when:
...
Install SupportClaw, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.supportClaw/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.supportClaw/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run SupportClaw as a service account or want custom paths:

- `SUPPORT_CLAW_HOME` — home directory for internal path resolution
- `SUPPORT_CLAW_STATE_DIR` — override the state directory
- `SUPPORT_CLAW_CONFIG_PATH` — override the config file path
