...
On the first agent run, SupportClaw bootstraps the workspace (default
...
For embedded/local model runs, SupportClaw keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `supportClaw onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.supportClaw/workspace`).
</Note>
