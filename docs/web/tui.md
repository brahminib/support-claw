...
```bash
supportClaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
supportClaw chat
# or
supportClaw tui --local
```

Notes:

- `supportClaw chat` and `supportClaw terminal` are aliases for `supportClaw tui --local`.
- `--local` cannot be combined with `--url`, `--token`, or `--password`.
- Local mode uses the embedded agent runtime directly. Most local tools work, but Gateway-only features are unavailable.
- After a config file has authored settings, `supportClaw` and `supportClaw crestodian` also use this TUI shell, with Crestodian as the local setup and repair chat backend.
...
- To show the Gateway host for non-local URL-backed connections, opt in with:
  ...
  ```bash
  supportClaw config set tui.footer.showRemoteHost true
  ```
...
- Turn delivery on:
  ...
  - or start with `supportClaw tui --deliver`
...
- Local shell commands receive `SUPPORT_CLAW_SHELL=tui-local` in their environment.
...
If `supportClaw config validate` is already failing, start with `supportClaw configure`
or `supportClaw doctor --fix` first. `supportClaw chat` does not bypass the invalid-
config guard.
...
4. Apply narrow changes with `supportClaw config set` or `supportClaw configure`, then rerun `!supportClaw config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!supportClaw doctor --fix`.

Tips:

- Prefer `supportClaw config set` or `supportClaw configure` over hand-editing `supportClaw.json`.
- `supportClaw docs "<query>"` searches the live docs index from the same machine.
- `supportClaw config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- If your terminal uses a light background and auto-detection is wrong, set `SUPPORT_CLAW_THEME=light` before launching `supportClaw tui`.
- To force the original dark palette instead, set `SUPPORT_CLAW_THEME=dark`.
...
- Check the Gateway logs: `supportClaw logs --follow`.
- Confirm the agent can run: `supportClaw status` and `supportClaw models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `supportClaw agents list` and your routing config.
- Empty session picker: you might be in global scope or have no sessions yet.

## Related

- [Control UI](/web/control-ui) — web-based control interface
- [Config](/cli/config) — inspect, validate, and edit `supportClaw.json`
