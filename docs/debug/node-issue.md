...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in SupportClaw
...
Running SupportClaw via Node with `tsx` fails at startup with:

```
[supportClaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `supportClaw status` and `gateway:watch` worked.
...
- Use `tsgo` for repo type checking, then run the built output:
  ...
  ```bash
  pnpm tsgo
  node supportClawCli.mjs status
  ```
