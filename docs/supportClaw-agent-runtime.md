---
summary: "Developer workflow for SupportClaw agent runtime: build, test, and live validation"
title: "SupportClaw agent runtime workflow"
read_when:
  - Working on SupportClaw agent runtime code or tests
  - Running agent-runtime lint, typecheck, and live test flows
---

A sane workflow for working on the SupportClaw agent runtime in SupportClaw.
...
```bash
SUPPORT_CLAW_LIVE_TEST=1 pnpm test src/agents/embedded-agent-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm supportClaw agent --message "Hello" --thinking low`
...
State lives under the SupportClaw state directory. Default is `~/.supportClaw`. If `SUPPORT_CLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `supportClaw.json` for config
...
- [SupportClaw agent runtime architecture](/agent-runtime-architecture)
