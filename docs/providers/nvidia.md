---
summary: "Use NVIDIA's OpenAI-compatible API in SupportClaw"
read_when:
  - You want to use open models in SupportClaw for free
...
[build.nvidia.com](https://build.nvidia.com/settings/api-keys). SupportClaw
...
<Steps>
  ...
  <Step title="Export the key and run onboarding">
    ...
    supportClaw onboard --auth-choice nvidia-api-key
    ```
  </Step>
  <Step title="Set an NVIDIA model">
    ```bash
    supportClaw models set nvidia/nvidia/nemotron-3-ultra-550b-a55b
    ```
  </Step>
</Steps>
...
```bash
supportClaw onboard --auth-choice nvidia-api-key --nvidia-api-key "nvapi-..."
```
...
When an NVIDIA API key is configured, SupportClaw setup and model-selection paths
...
SupportClaw release. When the live feed is available, the first returned model is
...
malformed, SupportClaw falls back to the bundled catalog and bundled default below.

## Nemotron 3 Ultra

Nemotron 3 Ultra is the default NVIDIA model in SupportClaw. NVIDIA's build page for
...
<AccordionGroup>
  ...
  <Accordion title="Catalog and pricing">
    SupportClaw prefers NVIDIA's public featured-model catalog when NVIDIA auth is
  ...
  <Accordion title="Nemotron 3 Ultra reasoning params">
    ...
    and `reasoning_budget` for reasoning output. SupportClaw's bundled Ultra row
