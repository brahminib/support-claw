---
summary: "Use OpenAI via API keys or Codex subscription in SupportClaw"
read_when:
  - You want to use OpenAI models in SupportClaw
...
ChatGPT-plan coding agent through OpenAI's Codex clients. SupportClaw uses one
provider id, `openai`, for both auth shapes.

SupportClaw uses `openai/*` as the canonical OpenAI model route. Embedded agent
...
- **Legacy config** - legacy Codex model refs are repaired by
  `supportClaw doctor --fix` to `openai/*` plus the Codex runtime.

OpenAI explicitly supports subscription OAuth usage in external tools and workflows like SupportClaw.
...
| Direct API-key billing through explicit SupportClaw     | `openai/gpt-5.5` plus provider/model runtime `supportClaw`  | Select a normal `openai` API-key profile.                             |
| Latest ChatGPT Instant API alias                     | `openai/chat-latest`                                     | Direct API-key only. Moving alias for experiments, not the default.   |
| ChatGPT/Codex subscription auth through SupportClaw     | `openai/gpt-5.5` plus provider/model runtime `supportClaw`  | Select an `openai` OAuth profile for the compatibility route.         |
...
| legacy OpenAI Codex prefix              | Legacy prefix     | Older model/profile namespace. `supportClaw doctor --fix` migrates it to `openai`.                   |
| `codex` plugin                          | Plugin            | Bundled SupportClaw plugin that provides native Codex app-server runtime and `/codex` chat controls. |
...
`auth.order.openai` for config; `supportClaw doctor --fix` rewrites legacy
...
SupportClaw runtime config remains available as an opt-in compatibility route. When SupportClaw is
explicitly selected with an `openai` OAuth profile, SupportClaw keeps the
public model ref as `openai/*` and routes internally through the Codex-auth
transport. Run `supportClaw doctor --fix` to repair stale
legacy Codex model refs, `codex-cli/*`, or old runtime session pins that do not come from
explicit runtime config.
</Note>

## SupportClaw feature coverage

| OpenAI capability         | SupportClaw surface                                                                              | Status                                                                 |
| ------------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
...
the `openai` API-key auth profile created by `supportClaw onboard --auth-choice openai-api-key`,
...
SupportClaw install, but they do not configure Realtime voice.
</Note>

## Memory embeddings

SupportClaw can use OpenAI, or an OpenAI-compatible embedding endpoint, for
...
`queryInputType` and `documentInputType` under `memorySearch`. SupportClaw forwards
    ...
    **Best for:** direct API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        supportClaw onboard --auth-choice openai-api-key
        ```

        Or pass the key directly:

        ```bash
        supportClaw onboard --openai-api-key "$OPENAI_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        supportClaw models list --provider openai
        ```
      </Step>
    </Steps>
    ...
    | `openai/gpt-5.5`      | provider/model `agentRuntime.id: "supportClaw"`              | SupportClaw embedded runtime      | Selected `openai` profile |
    ...
    non-agent OpenAI API surfaces. Run `supportClaw doctor --fix` to migrate older
    ...
    SupportClaw normalizes incompatible OpenAI text-verbosity overrides for this
    model.

    <Warning>
    SupportClaw does **not** expose `gpt-5.3-codex-spark` on the direct OpenAI API-key route. It is available only through Codex subscription catalog entries when your signed-in account exposes it.
    </Warning>
    ...
    **Best for:** using your ChatGPT/Codex subscription with native Codex app-server execution instead of a separate API key. Codex cloud requires ChatGPT sign-in.
    ...
    <Steps>
      <Step title="Run Codex OAuth">
        ```bash
        supportClaw onboard --auth-choice openai
        ```
        ...
        ```bash
        supportClaw models auth login --provider openai --device-code
        ```
      </Step>
      <Step title="Use the canonical OpenAI model route">
        ```bash
        supportClaw config set agents.defaults.model.primary openai/gpt-5.5
        ```

        No runtime config is required for the default path. OpenAI agent turns
        select the native Codex app-server runtime automatically, and SupportClaw
    ...
    | `openai/gpt-5.5` | provider/model `agentRuntime.id: "supportClaw"` | SupportClaw embedded runtime with internal Codex-auth transport | Selected `openai` OAuth profile |
    ...
    legacy Codex GPT refs are legacy SupportClaw routes, not the native Codex runtime
    path; run `supportClaw doctor --fix` when you want to migrate them to canonical
    ...
    auth order under `openai`. SupportClaw will try the subscription first, then
    ...
    <Note>
    Onboarding no longer imports OAuth material from `~/.codex`. Sign in with browser OAuth (default) or the device-code flow above — SupportClaw manages the resulting credentials in its own agent auth store.
    </Note>
    ...
    supportClaw models auth list --provider openai
    supportClaw config get agents.defaults.model --json
    supportClaw config get models.providers.openai.agentRuntime --json
    ```

    For a specific agent, add `--agent <id>`:

    ```bash
    supportClaw models status --agent <id>
    supportClaw models auth list --agent <id> --provider openai
    ```
    ...
    ```bash
    supportClaw models auth login --provider openai
    supportClaw models status --probe --probe-provider openai
    ```
    ...
    ```bash
    supportClaw models auth login --provider openai --profile-id openai:ritsuko
    supportClaw models auth login --provider openai --profile-id openai:lain
    ```

    `openai/*` is the model route for OpenAI agent turns through Codex. Run
    `supportClaw doctor --fix` to migrate older legacy OpenAI Codex prefix profile ids and
    ...
    config explicitly pins SupportClaw.

    ### Doctor warning

    If legacy Codex model refs or stale OpenAI runtime pins remain in config or
    session state, `supportClaw doctor --fix` rewrites them to `openai/*` with the
    Codex runtime unless SupportClaw is explicitly configured.

    ### Context window cap

    SupportClaw treats model metadata and the runtime context cap as separate values.
    ...
    SupportClaw uses upstream Codex catalog metadata for `gpt-5.5` when it is
    present. If live Codex discovery omits the `gpt-5.5` row while
    the account is authenticated, SupportClaw synthesizes that OAuth model row so
...
still account-based. SupportClaw selects auth in this order:
...
profile is selected, SupportClaw also keeps `CODEX_API_KEY` and `OPENAI_API_KEY`
...
Codex usage limit, SupportClaw can rotate to the next ordered `openai:*` API-key
...
still accepted. SupportClaw also protects the public OpenAI and
...
`supportClaw infer image edit` when starting from an input file.
...
`openai` OAuth profile is configured, SupportClaw resolves that stored OAuth
...
`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; SupportClaw keeps
...
```
/tool image_generate model=openai/gpt-image-2 prompt="A polished launch poster for SupportClaw on macOS" size=3840x2160 count=1
```
...
SupportClaw adds a shared GPT-5 prompt contribution for GPT-5-family runs on SupportClaw-assembled prompt surfaces. It applies by model id, so SupportClaw/provider routes such as legacy pre-repair refs (legacy Codex GPT-5.5 ref), `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5`, and other compatible GPT-5 refs receive the same overlay. Older GPT-4.x models do not.

The bundled native Codex harness does not receive this SupportClaw GPT-5 overlay through Codex app-server developer instructions. Native Codex keeps Codex-owned base, model, and project-doc behavior, while SupportClaw disables Codex's built-in personality for native threads so agent workspace personality files stay authoritative. SupportClaw contributes only runtime context such as channel delivery, SupportClaw dynamic tools, ACP delegation, workspace context, and SupportClaw skills.

The GPT-5 contribution adds a tagged behavior contract for persona persistence, execution safety, tool discipline, output shape, completion checks, and verification on matching SupportClaw-assembled prompts. Channel-specific reply and silent-message behavior stays in the shared SupportClaw system prompt and outbound delivery policy. The friendly interaction-style layer is separate and configurable.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    supportClaw config set agents.defaults.promptOverlays.gpt5.personality off
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Speech synthesis (TTS)">
    ...
    `extraBody` is merged into `/audio/speech` request JSON after SupportClaw's generated fields, so use it for OpenAI-compatible endpoints that require additional keys such as `lang`. Prototype keys are ignored.
  ...
  <Accordion title="Speech-to-text">
    ...
    SupportClaw's media-understanding transcription surface.

    - Default model: `gpt-4o-transcribe`
    - Endpoint: OpenAI REST `/v1/audio/transcriptions`
    - Input path: multipart audio file upload
    - Supported by SupportClaw wherever inbound audio transcription uses
  ...
  <Accordion title="Realtime voice">
    ...
    model has emitted audio in that session. SupportClaw currently exposes the
...
generation by overriding the base URL. On the image-generation path, SupportClaw
...
SupportClaw recognizes these Azure host suffixes for the Azure image-generation
route:
...
For image-generation requests on a recognized Azure host, SupportClaw:
...
SupportClaw 2026.4.22 or later. Earlier versions treat any custom
...
routed through the bundled `openai` provider, the `model` field in SupportClaw
...
SupportClaw. If an Azure request fails with a validation error, check the
...
SupportClaw's hidden attribution headers — see the **Native vs OpenAI-compatible
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    SupportClaw uses WebSocket-first with SSE fallback (`"auto"`) for `openai/*`.

    In `"auto"` mode, SupportClaw:
  ...
  <Accordion title="Fast mode">
    SupportClaw exposes a shared fast-mode toggle for `openai/*`:

    - **Chat/UI:** `/fast status|auto|on|off`
    - **Config:** `agents.defaults.models["<provider>/<model>"].params.fastMode`

    When enabled, SupportClaw maps fast mode to OpenAI priority processing (`service_tier = "priority"`). Existing `service_tier` values are preserved, and fast mode does not rewrite `reasoning` or `text.verbosity`. `fastMode: "auto"` starts new model calls fast until the auto cutoff, then starts later retry, fallback, tool-result, or continuation calls without fast mode. The cutoff defaults to 60 seconds; set `params.fastAutoOnSeconds` on the active model to change it.
  ...
  <Accordion title="Priority processing (service_tier)">
    OpenAI's API exposes priority processing via `service_tier`. Set it per model in SupportClaw:
    ...
    <Warning>
    `serviceTier` is only forwarded to native OpenAI endpoints (`api.openai.com`) and native Codex endpoints (`chatgpt.com/backend-api`). If you route either provider through a proxy, SupportClaw leaves `service_tier` untouched.
    </Warning>

  </Accordion>

  <Accordion title="Server-side compaction (Responses API)">
    For direct OpenAI Responses models (`openai/*` on `api.openai.com`), the OpenAI plugin's SupportClaw stream wrapper auto-enables server-side compaction:

    - Forces `store: true` (unless model compat sets `supportsStore: false`)
    - Injects `context_management: [{ type: "compaction", compact_threshold: ... }]`
    - Default `compact_threshold`: 70% of `contextWindow` (or `80000` when unavailable)

    This applies to the built-in SupportClaw runtime path and to OpenAI provider hooks used by embedded runs. The native Codex app-server harness manages its own context through Codex and is configured by OpenAI's default agent route or provider/model runtime policy.
  ...
  <Accordion title="Strict-agentic GPT mode">
    For GPT-5-family runs on `openai/*`, SupportClaw can use a stricter embedded execution contract:
    ...
    With `strict-agentic`, SupportClaw:
    ...
    SupportClaw does not classify assistant prose to decide whether a turn is a plan, progress update, or final answer.
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    SupportClaw treats direct OpenAI, Codex, and Azure OpenAI endpoints differently from generic OpenAI-compatible `/v1` proxies:
