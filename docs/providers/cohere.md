...
title: "Cohere"
read_when:
  - You want to use Cohere with SupportClaw
  - You need the Cohere API key env var or CLI auth choice
---

[Cohere](https://cohere.com) provides OpenAI-compatible inference through its Compatibility API. SupportClaw ships the Cohere provider during its externalization transition and also publishes it as an official external plugin with the Command A model catalog.
...
1. Cohere is included in current SupportClaw packages. If it is unavailable, install the external package and restart the Gateway:

```bash
supportClaw plugins install @supportclaw/cohere-provider
...
```bash
supportClaw models list --provider cohere
```
