---
summary: "Use Qwen Cloud through its SupportClaw plugin"
read_when:
  - You want to use Qwen with SupportClaw
  - You previously used Qwen OAuth
title: "Qwen"
---

SupportClaw now treats Qwen as a first-class provider plugin with canonical id
...
```bash
supportClaw plugins install @supportclaw/qwen-provider
    ...
    **Best for:** subscription-based access through the Qwen Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        supportClaw onboard --auth-choice qwen-api-key
        ```

        For the **China** endpoint:

        ```bash
        supportClaw onboard --auth-choice qwen-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        supportClaw models list --provider qwen
        ```
      </Step>
    </Steps>
    ...
    **Best for:** pay-as-you-go access through the Standard Model Studio endpoint, including models like `qwen3.6-plus` that may not be available on the Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        supportClaw onboard --auth-choice qwen-standard-api-key
        ```

        For the **China** endpoint:

        ```bash
        supportClaw onboard --auth-choice qwen-standard-api-key-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
    ...
    **Best for:** a Qwen Portal token against `https://portal.qwen.ai/v1`.
    ...
    <Steps>
      <Step title="Provide your portal token">
        ```bash
        supportClaw onboard --auth-choice qwen-oauth
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
      ...
      <Step title="Verify the model is available">
        ```bash
        supportClaw models list --provider qwen-oauth
        ```
      </Step>
    </Steps>
    ...
    through SupportClaw onboarding.
    </Note>
...
SupportClaw currently ships this Qwen static catalog. The configured catalog is
...
For reasoning-enabled Qwen Cloud models, the provider maps SupportClaw
...
<AccordionGroup>
  ...
  <Accordion title="Qwen 3.6 Plus availability">
    ...
    SupportClaw's Qwen static catalog does not advertise `qwen3.6-plus` on Coding
  ...
  <Accordion title="Video generation details">
    For video generation, SupportClaw maps the configured Qwen region to the matching
  ...
  <Accordion title="Environment and daemon setup">
    ...
    available to that process (for example, in `~/.supportClaw/.env` or via
    `env.shellEnv`).
  </Accordion>
</AccordionGroup>
