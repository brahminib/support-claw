---
summary: "Use Synthetic's Anthropic-compatible API in SupportClaw"
read_when:
...
SupportClaw registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    supportClaw onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
SupportClaw's Anthropic client appends `/v1` to the base URL automatically, so use
...
`supportClaw models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that SupportClaw appends `/v1` automatically.
