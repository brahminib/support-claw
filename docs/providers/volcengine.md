...
read_when:
  - You want to use Volcano Engine or Doubao models with SupportClaw
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    supportClaw onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    supportClaw models list --provider volcengine
    supportClaw models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
Then enable it in `supportClaw.json`:
...
For voice-note targets, SupportClaw asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `supportClaw onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, SupportClaw falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.supportClaw/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running SupportClaw as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about SupportClaw setup.
  </Card>
</CardGroup>
