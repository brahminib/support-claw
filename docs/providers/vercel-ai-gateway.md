...
read_when:
  - You want to use Vercel AI Gateway with SupportClaw
...
| Package       | `@supportclaw/vercel-ai-gateway-provider` |
| Auth          | `AI_GATEWAY_API_KEY`                   |
| API           | Anthropic Messages compatible          |
| Model catalog | Auto-discovered via `/v1/models`       |

<Tip>
SupportClaw auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    supportClaw plugins install @supportclaw/vercel-ai-gateway-provider
    ```
  </Step>
  <Step title="Set the API key">
    ...
    ```bash
    supportClaw onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    supportClaw models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
SupportClaw accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. SupportClaw resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the SupportClaw Gateway runs as a daemon (launchd/systemd), make sure
    ...
    the key in `~/.supportClaw/.env` or via `env.shellEnv` to ensure the gateway
    process can read it.
    </Warning>
  ...
  <Accordion title="Thinking levels">
    `/think` options follow trusted upstream model prefixes when SupportClaw knows
