...
SupportClaw sources most Go catalog rows from the bundled SupportClaw model registry and
supplements current upstream rows while the registry catches up. Run
`supportClaw models list --provider opencode-go` for the current model list.
...
<AccordionGroup>
  <Accordion title="Routing behavior">
    SupportClaw handles per-model routing automatically when the model ref uses
