---
summary: "Use GMI Cloud's OpenAI-compatible API with SupportClaw"
read_when:
  - You want to run SupportClaw with GMI Cloud models
  - You need the GMI provider id, key, or endpoint
title: "GMI Cloud"
---

GMI Cloud is a hosted inference platform for frontier and open-weight models
behind an OpenAI-compatible API. In SupportClaw it is an official external provider
...
This provider uses OpenAI-compatible chat semantics. SupportClaw owns the provider
...
```bash
supportClaw plugins install @supportclaw/gmi-provider
...
```bash
supportClaw onboard --auth-choice gmi-api-key
```
...
all times. Use SupportClaw's model listing command to see what the configured
provider reports in your environment:

```bash
supportClaw models list --provider gmi
```

## Troubleshooting

- `401` or `403`: check that `GMI_API_KEY` is set for the process running
  SupportClaw, or re-run onboarding to store the key in the provider auth profile.
- Unknown model errors: confirm the model exists in your GMI account and use the
  full `gmi/<route-id>` ref shown by `supportClaw models list --provider gmi`.
