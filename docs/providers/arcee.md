...
title: "Arcee AI"
read_when:
  - You want to use Arcee AI with SupportClaw
...
```bash
supportClaw plugins install @supportclaw/arcee-provider
...
<Tabs>
  <Tab title="Direct (Arcee platform)">
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        supportClaw onboard --auth-choice arceeai-api-key
        ```
      </Step>
  ...
  <Tab title="Via OpenRouter">
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        supportClaw onboard --auth-choice arceeai-openrouter
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
SupportClaw currently ships this Arcee static catalog:
...
<AccordionGroup>
  ...
  <Accordion title="OpenRouter routing">
    ...
    SupportClaw handles routing transparently based on your auth choice. See the
