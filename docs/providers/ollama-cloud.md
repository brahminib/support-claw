---
summary: "Use Ollama Cloud directly with SupportClaw"
read_when:
...
Ollama Cloud is Ollama's hosted model API. It lets SupportClaw call Ollama-hosted
...
`/api/chat` style, not the OpenAI-compatible `/v1` route. SupportClaw registers it
...
- You want the same native Ollama chat API shape SupportClaw uses for local
...
SupportClaw discovers Ollama Cloud models from the live hosted catalog. Commonly
...
```bash
supportClaw models list --provider ollama-cloud
supportClaw models set ollama-cloud/kimi-k2.6
```
...
SUPPORT_CLAW_LIVE_OLLAMA_MODEL=kimi-k2.6 \
...
- Unknown model errors: run `supportClaw models list --provider ollama-cloud` and
