---
summary: "Use Venice AI privacy-focused models in SupportClaw"
read_when:
  - You want privacy-focused inference in SupportClaw
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    supportClaw plugins install @supportclaw/venice-provider
    ```
  </Step>
  ...
  <Step title="Configure SupportClaw">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        supportClaw onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    supportClaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, SupportClaw shows all available Venice models. Pick based on your needs:
...
```bash
supportClaw models set venice/kimi-k2-5
supportClaw models set venice/claude-opus-4-6
```

List all available models:

```bash
supportClaw models list --all --provider venice
```

You can also run `supportClaw configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, SupportClaw fills the required DeepSeek V4
...
SupportClaw keeps that provider-specific replay fix separate from the native
...
SupportClaw ships a manifest-backed Venice seed catalog for read-only model listing. Runtime refresh can still discover models from the Venice API, and falls back to the manifest catalog if the API is unreachable.
...
supportClaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
supportClaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
supportClaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
supportClaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
supportClaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  <Accordion title="API key not recognized">
    ```bash
    echo $VENICE_API_KEY
    supportClaw models list | grep venice
    ```
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `supportClaw models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
