...
read_when:
  - You want to use Tencent Hy3 preview with SupportClaw
...
| Package          | `@supportclaw/tencent-provider`                          |
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    supportClaw plugins install @supportclaw/tencent-provider
    ```
  </Step>
...
```bash Onboarding
supportClaw onboard --auth-choice tokenhub-api-key
```
...
```
  ...
  <Step title="Verify the model">
    ```bash
    supportClaw models list --provider tencent-tokenhub
    ```
  </Step>
</Steps>
...
<Tip>
  The model id is `hy3-preview`. Do not confuse it with Tencent's `HY-3D-*` models, which are 3D generation APIs and are not the SupportClaw chat model configured by this provider.
</Tip>
...
<AccordionGroup>
  <Accordion title="Endpoint override">
    SupportClaw defaults to Tencent Cloud's `https://tokenhub.tencentmaas.com/v1` endpoint. Tencent also documents an international TokenHub endpoint:

    ```bash
    supportClaw config set models.providers.tencent-tokenhub.baseUrl "https://tokenhub-intl.tencentmaas.com/v1"
    ```
  ...
  <Accordion title="Environment availability for the daemon">
    If the Gateway runs as a managed service (launchd, systemd, Docker), `TOKENHUB_API_KEY` must be visible to that process. Set it in `~/.supportClaw/.env` or via `env.shellEnv` so launchd, systemd, or Docker exec environments can read it.
