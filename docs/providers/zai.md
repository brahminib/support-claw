---
summary: "Use Z.AI (GLM models) with SupportClaw"
read_when:
  - You want Z.AI / GLM models in SupportClaw
...
SupportClaw uses the `zai` provider with a Z.AI API key.

| Property | Value                                        |
| -------- | -------------------------------------------- |
| Provider | `zai`                                        |
| Package  | `@supportclaw/zai-provider`                     |
| Auth     | `ZAI_API_KEY` (legacy alias: `Z_AI_API_KEY`) |
| API      | Z.AI Chat Completions (Bearer auth)          |

## GLM models

GLM is a model family, not a separate provider. In SupportClaw, GLM models use
...
```bash
supportClaw plugins install @supportclaw/zai-provider
```

<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. SupportClaw probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        supportClaw onboard --auth-choice zai-api-key
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
        supportClaw models list --all --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        supportClaw onboard --auth-choice zai-coding-global

        # Coding Plan CN (China region)
        supportClaw onboard --auth-choice zai-coding-cn

        # General API
        supportClaw onboard --auth-choice zai-global

        # General API CN (China region)
        supportClaw onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
...
<Tip>
`zai-api-key` lets SupportClaw detect the matching Z.AI endpoint from the key and
...
<Tip>
GLM-5.2 supports `off`, `low`, `high`, and `max` thinking levels. SupportClaw maps
...
can change; run `supportClaw models list --all --provider zai` to see the catalog
...
<AccordionGroup>
  ...
  <Accordion title="Thinking and preserved thinking">
    Z.AI thinking follows SupportClaw's `/think` controls. With thinking off,
    SupportClaw sends `thinking: { type: "disabled" }` to avoid responses that
    ...
    When enabled and thinking is on, SupportClaw sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; SupportClaw copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full SupportClaw config schema, including provider and model settings.
  </Card>
</CardGroup>
