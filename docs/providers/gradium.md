---
summary: "Use Gradium text-to-speech in SupportClaw"
read_when:
...
[Gradium](https://gradium.ai) is a text-to-speech provider for SupportClaw. The plugin can render normal audio replies (WAV), voice-note-compatible Opus output, and 8 kHz u-law audio for telephony surfaces.
...
```bash
supportClaw plugins install @supportclaw/gradium-speech
supportClaw gateway restart
```

## Setup

Create a Gradium API key, then expose it to SupportClaw with either an env var or the config key.
...
The output audio format is selected automatically by the runtime based on the target surface and is not configurable from `supportClaw.json`. See [Output](#output) below.
...
Among configured TTS providers, Gradium's auto-select order is `30`. See [Text-to-Speech](/tools/tts) for how SupportClaw picks the active provider when `messages.tts.provider` is not pinned.
