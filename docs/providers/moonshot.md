...
Kimi K2.7 Code always uses native thinking. SupportClaw exposes only the `on`
thinking state for this model and omits outbound `thinking` and
`reasoning_effort` controls, as required by Moonshot. SupportClaw also omits
...
<Tabs>
    ...
    **Best for:** Kimi K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        supportClaw onboard --auth-choice moonshot-api-key
        ```

        Or for the China endpoint:

        ```bash
        supportClaw onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        supportClaw models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        SUPPORT_CLAW_CONFIG_PATH=/tmp/supportClaw-kimi/supportClaw.json \
        SUPPORT_CLAW_STATE_DIR=/tmp/supportClaw-kimi \
  ...
  <Tab title="Kimi Coding">
    ...
    ```bash
    supportClaw plugins install @supportclaw/kimi-provider
    ...
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        supportClaw onboard --auth-choice kimi-code-api-key
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
      ...
      <Step title="Verify the model is available">
        ```bash
        supportClaw models list --provider kimi
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    ...
    omit the `thinking` field for this model, so SupportClaw exposes only `on` and
    ignores stale `off` settings. K2.7 also fixes `temperature`, `top_p`, `n`,
    `presence_penalty`, and `frequency_penalty`; SupportClaw omits configured
    ...
    SupportClaw maps runtime `/think` levels for those models:
    ...
    <Warning>
    When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. SupportClaw normalizes incompatible values to `auto`. This includes Kimi K2.7 Code, whose thinking mode cannot be disabled to preserve a pinned tool choice.
    </Warning>
    ...
    default strategy. SupportClaw only forwards `thinking.keep` for
    `moonshot/kimi-k2.6` and strips it from other models. Kimi K2.7 Code
    preserves full reasoning history by default while SupportClaw omits the entire
    `thinking` field.
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    shared `openai-completions` transport. SupportClaw keys that off endpoint
