...
title: "Chutes"
read_when:
  - You want to use Chutes with SupportClaw
...
OpenAI-compatible API. SupportClaw supports both browser OAuth and direct API-key
...
```bash
supportClaw plugins install @supportclaw/chutes-provider
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run the OAuth onboarding flow">
        ```bash
        supportClaw onboard --auth-choice chutes
        ```
        SupportClaw launches the browser flow locally, or shows a URL + redirect-paste
        flow on remote/headless hosts. OAuth tokens auto-refresh through SupportClaw auth
        profiles.
      </Step>
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run the API key onboarding flow">
        ```bash
        supportClaw onboard --auth-choice chutes-api-key
        ```
      </Step>
      <Step title="Verify the default model">
        After onboarding, the default model is set to
...
When Chutes auth is available, SupportClaw queries the Chutes catalog with that
credential and uses the discovered models. If discovery fails, SupportClaw falls
back to a static catalog so onboarding and startup still work.

## Default aliases

SupportClaw registers three convenience aliases for the Chutes static catalog:
