---
summary: "Use OpenRouter's unified API to access many models in SupportClaw"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in SupportClaw
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run OAuth onboarding">
        ```bash
        supportClaw onboard --auth-choice openrouter-oauth
        ```

        SupportClaw opens OpenRouter's browser sign-in flow, exchanges the PKCE
        code for an OpenRouter API key, and stores that key in the default
        OpenRouter auth profile. On remote/headless hosts, SupportClaw prints the
      ...
      <Step title="(Optional) Switch to a specific model">
        ...
        ```bash
        supportClaw models set openrouter/<provider>/<model>
        ```
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run API-key onboarding">
        ```bash
        supportClaw onboard --auth-choice openrouter-api-key
        ```
      </Step>
      <Step title="(Optional) Switch to a specific model">
        Onboarding defaults to `openrouter/auto`. Pick a concrete model later:
...
SupportClaw submits text-to-video and image-to-video jobs to OpenRouter, polls
...
`google/lyria-3-clip-preview`. SupportClaw sends `modalities: ["text",
...
SupportClaw sends OpenRouter STT requests as JSON with base64 audio under
`input_audio` (OpenRouter STT contract), not as multipart OpenAI form uploads.

## Fusion router

Use OpenRouter Fusion when you want one SupportClaw model ref to ask several
...
the upstream model slug is `openrouter/fusion`, the SupportClaw model ref includes
both the SupportClaw provider prefix and the upstream OpenRouter namespace:

```bash
supportClaw models set openrouter/openrouter/fusion
```
...
`"required"` in normal SupportClaw agent/chat turns to try to force Fusion;
SupportClaw turns may include SupportClaw tool definitions, and a top-level required
tool choice can require one of those tools instead of the Fusion router. When
this Fusion plugin config is present, SupportClaw also adds a sanitized
...
Fusion is slower by design. OpenRouter may send the same SupportClaw prompt to
...
```bash
supportClaw infer model run --local \
...
OAuth is a PKCE login flow that issues an OpenRouter API key, so SupportClaw stores
...
```bash
supportClaw models auth login --provider openrouter --method oauth
```

Use `supportClaw models auth login --provider openrouter --method api-key` when
you want to paste a key you created manually at OpenRouter.

On real OpenRouter requests (`https://openrouter.ai/api/v1`), SupportClaw also adds
OpenRouter's documented app-attribution headers:

| Header                    | Value                                                                                                  |
| ------------------------- | ------------------------------------------------------------------------------------------------------ |
| `HTTP-Referer`            | `https://supportClaw.ai`                                                                                  |
| `X-OpenRouter-Title`      | `SupportClaw`                                                                                             |
| `X-OpenRouter-Categories` | `cli-agent,cloud-agent,programming-app,creative-writing,writing-assistant,general-chat,personal-agent` |

<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, SupportClaw
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    SupportClaw sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that SupportClaw uses for
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, SupportClaw maps the selected thinking level to
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    required follow-up shape. SupportClaw sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: SupportClaw keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    SupportClaw forwards that object to OpenRouter as the request `provider`
