---
summary: "Use NovitaAI's OpenAI-compatible API with SupportClaw"
read_when:
  - You want to run SupportClaw with NovitaAI models
...
API. In SupportClaw it is a bundled model provider, so the provider id is
...
This provider uses Novita's OpenAI-compatible endpoint. SupportClaw handles
...
```bash
supportClaw onboard --auth-choice novita-api-key
```
...
The catalog is a starting point for SupportClaw model selection. Your account,
...
```bash
supportClaw models list --provider novita
```

## Troubleshooting

- `401` or `403`: verify the key in Novita's key management page and re-run
  `supportClaw onboard --auth-choice novita-api-key` if the stored profile is
  stale.
