---
summary: "Use Mistral models and Voxtral transcription with SupportClaw"
read_when:
  - You want to use Mistral models in SupportClaw
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

SupportClaw includes a bundled Mistral plugin that registers four contracts: chat completions, media understanding (Voxtral batch transcription), realtime STT for Voice Call (Voxtral Realtime), and memory embeddings (`mistral-embed`).
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    supportClaw onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    supportClaw onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    supportClaw models list --provider mistral
    ```
  </Step>
</Steps>
...
SupportClaw currently ships this bundled Mistral catalog:
...
```bash
supportClaw models list --all --provider mistral --plain
```
...
<Note>
SupportClaw defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning">
    ...
    SupportClaw maps the session **thinking** level to Mistral's API:

    | SupportClaw thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
    ...
    answers, turn thinking off/minimal so SupportClaw sends
