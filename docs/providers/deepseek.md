...
title: "DeepSeek"
read_when:
  - You want to use DeepSeek with SupportClaw
...
```bash
supportClaw plugins install @supportclaw/deepseek-provider
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    supportClaw onboard --auth-choice deepseek-api-key
    ```
  ...
  <Step title="Verify models are available">
    ```bash
    supportClaw models list --provider deepseek
    ```
    ...
    ```bash
    supportClaw models list --all --provider deepseek
    ```
...
<Tip>
V4 models support DeepSeek's `thinking` control. SupportClaw also replays
...
`reasoning_content` on follow-up requests. SupportClaw handles this inside the
...
DeepSeek `reasoning_content`. SupportClaw fills that missing field on replayed
assistant messages for DeepSeek V4 thinking requests so the provider can accept
the history without requiring `/new`.

When thinking is disabled in SupportClaw (including the UI **None** selection),
SupportClaw sends DeepSeek `thinking: { type: "disabled" }` and strips replayed
...
```bash
SUPPORT_CLAW_LIVE_PROVIDERS=deepseek \
SUPPORT_CLAW_LIVE_MODELS="deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro" \
