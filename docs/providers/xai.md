---
summary: "Use xAI Grok models in SupportClaw"
read_when:
  - You want to use Grok models in SupportClaw
  - You are configuring xAI auth or model ids
title: "xAI"
---

SupportClaw ships a bundled `xai` provider plugin for Grok models. For most
users, the recommended path is Grok OAuth with an eligible SuperGrok or X Premium
subscription. SupportClaw stays local-first: the Gateway, config, routing, and
...
app. xAI may still show Grok Build on the consent screen because SupportClaw uses
xAI's shared OAuth client.

## Choose your setup path

Use the path that matches your SupportClaw install state:

<Steps>
  <Step title="New SupportClaw install">
    ...
    ```bash
    supportClaw onboard --install-daemon --auth-choice xai-device-code
    ```

    OAuth does not require an xAI API key. SupportClaw does not require the Grok
    Build app. xAI may still label the consent app as Grok Build because
    SupportClaw uses xAI's shared OAuth client.

  </Step>
  <Step title="Existing SupportClaw install">
    If SupportClaw is already configured, sign in to xAI only. Do not rerun full
    onboarding or reinstall the daemon just to connect Grok:

    ```bash
    supportClaw models auth login --provider xai --method oauth
    ```
    ...
    ```bash
    supportClaw models auth login --provider xai --device-code
    ```

    To make Grok the default model after signing in, apply it separately:

    ```bash
    supportClaw models set xai/grok-4.3
    ```
  ...
  <Step title="API-key path">
    ...
    ```bash
    supportClaw models auth login --provider xai --method api-key
...
<Note>
SupportClaw uses the xAI Responses API as the bundled xAI transport. The same
credential from `supportClaw models auth login --provider xai --method oauth`,
`supportClaw models auth login --provider xai --device-code`, or
`supportClaw models auth login --provider xai --method api-key` can also power first-class
...
- To inspect saved xAI auth profiles, run:
  ...
  ```bash
  supportClaw models auth list --provider xai
...
<Tip>
Use `xai-device-code` when signing in from SSH, Docker, or a VPS. SupportClaw
...
SupportClaw includes the current xAI chat models out of the box, ordered newest
...
normalize to `grok-build-0.1`; SupportClaw no longer shows the other retired
...
The bundled plugin maps xAI's current public API surface onto SupportClaw's shared
...
| xAI capability             | SupportClaw surface                          | Status                                                              |
| -------------------------- | ----------------------------------------- | ------------------------------------------------------------------- |
...
| Streaming TTS              | -                                         | Not exposed; SupportClaw's TTS contract returns complete audio buffers |
| Batch speech-to-text       | `tools.media.audio` / media understanding | Yes                                                                 |
| Streaming speech-to-text   | Voice Call `streaming.provider: "xai"`    | Yes                                                                 |
| Realtime voice             | -                                         | Not exposed yet; different session/WebSocket contract               |
| Files / batches            | Generic model API compatibility only      | Not a first-class SupportClaw tool                                     |

<Note>
SupportClaw uses xAI's REST image/video/TTS/STT APIs for media generation,
...
code-execution tools. Features that need different SupportClaw contracts, such as
...
<AccordionGroup>
  <Accordion title="Web search">
    ...
    ```bash
    supportClaw models auth login --provider xai --method oauth
    supportClaw config set tools.web.search.provider grok
    ```
  ...
  <Accordion title="Video generation">
    ...
    SupportClaw can encode those as data URLs for xAI.
    </Warning>
  ...
  <Accordion title="Image generation">
    ...
    SupportClaw asks xAI for `b64_json` image responses so generated media can be
    ...
    such as `1:2`, `2:1`, `9:20`, and `20:9`. SupportClaw forwards only the
  ...
  <Accordion title="Text-to-speech">
    ...
    <Note>
    SupportClaw uses xAI's batch `/v1/tts` endpoint. xAI also offers streaming TTS
    over WebSocket, but the SupportClaw speech provider contract currently expects
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through SupportClaw's
    ...
    transcription request. Prompt hints are accepted by the shared SupportClaw
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an SupportClaw tool for searching
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an SupportClaw tool for
  ...
  <Accordion title="Known limits">
    - xAI auth can use an API key, environment variable, plugin config fallback,
      ...
      the consent page may show Grok Build even though SupportClaw does not require
      the Grok Build app.
    - SupportClaw does not currently expose the xAI multi-agent model family. xAI
      serves these models through the Responses API, but they do not accept the
      client-side or custom tools used by SupportClaw's shared agent loop. See the
      [xAI multi-agent limitations](https://docs.x.ai/developers/model-capabilities/text/multi-agent#limitations).
    - xAI Realtime voice is not registered as an SupportClaw provider yet. It
  ...
  <Accordion title="Advanced notes">
    - SupportClaw applies xAI-specific tool-schema and tool-call compatibility fixes
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as SupportClaw
      tools. SupportClaw enables the specific xAI built-in it needs inside each tool
...
SUPPORT_CLAW_LIVE_TEST=1 SUPPORT_CLAW_LIVE_TEST_QUIET=1 pnpm test:live -- extensions/xai/xai.live.test.ts
SUPPORT_CLAW_LIVE_TEST=1 SUPPORT_CLAW_LIVE_TEST_QUIET=1 SUPPORT_CLAW_LIVE_IMAGE_GENERATION_PROVIDERS=xai pnpm test:live -- test/image-generation.runtime.live.test.ts
```
...
shared image live file verifies the same xAI provider through SupportClaw's
