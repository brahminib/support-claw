---
summary: "Inworld streaming text-to-speech for SupportClaw replies"
read_when:
...
Inworld is a streaming text-to-speech (TTS) provider. In SupportClaw it
...
SupportClaw posts to Inworld's streaming TTS endpoint, concatenates the
...
```bash
supportClaw plugins install @supportclaw/inworld-speech
...
<Steps>
  ...
  <Step title="Send a message">
    Send a reply through any connected channel. SupportClaw synthesizes the
...
<AccordionGroup>
  ...
  <Accordion title="Audio outputs">
    ...
    SupportClaw asks Inworld for `OGG_OPUS` so the audio plays as a native
...
<CardGroup cols={2}>
  ...
  <Card title="Providers" href="/providers" icon="grid">
    All supported SupportClaw providers.
  </Card>
