...
read_when:
  - Setting up SupportClaw for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

SupportClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.supportClaw/supportClaw.json`.
The active config path must be a regular file. Symlinked `supportClaw.json`
layouts are unsupported for SupportClaw-owned writes; an atomic write may replace
the path instead of preserving the symlink. If you keep config outside the
default state directory, point `SUPPORT_CLAW_CONFIG_PATH` directly at the real file.

If the file is missing, SupportClaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `supportClaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    supportClaw onboard       # full onboarding flow
    supportClaw configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.supportClaw/supportClaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
SupportClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`supportClaw config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`supportClaw doctor`, `supportClaw logs`, `supportClaw health`, `supportClaw status`)
- Run `supportClaw doctor` to see exact issues
- Run `supportClaw doctor --fix` (or `--yes`) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically. If `supportClaw.json`
...
Run `supportClaw doctor --fix` (or `--yes`) to repair prefixed/clobbered config or
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `supportClaw config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
        ...
        list: [
          {
            id: "main",
            groupChat: {
              mentionPatterns: ["@supportclaw", "supportClaw"],
            },
          },
        ],
      },
  ...
  <Accordion title="Tune gateway WebSocket handshake timeout">
    ...
    - `SUPPORT_CLAW_HANDSHAKE_TIMEOUT_MS` still takes precedence for one-off service or shell overrides.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push uses the hosted SupportClaw relay by default: `https://ios-push-relay.supportClaw.ai`.
    ...
    ```bash
    supportClaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `SUPPORT_CLAW_APNS_RELAY_BASE_URL` and `SUPPORT_CLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - Custom gateway relay URLs must match the relay base URL baked into the official/TestFlight iOS build.
    - `SUPPORT_CLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Use a dedicated `hooks.token`; do not reuse active Gateway auth secrets (`gateway.auth.token` / `SUPPORT_CLAW_GATEWAY_TOKEN` or `gateway.auth.password` / `SUPPORT_CLAW_GATEWAY_PASSWORD`).
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-supportClaw-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **SupportClaw-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      SupportClaw updates that included file and leaves `supportClaw.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for SupportClaw-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `supportClaw.json`. To share a tree across machines or users, set
      `SUPPORT_CLAW_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.supportClaw/supportClaw.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `supportClaw.json`. SupportClaw-owned config
...
config`, inspect the config, run `supportClaw config validate`, then run `supportClaw
...
When you edit a source file that is referenced through `$include`, SupportClaw plans
...
```bash
supportClaw gateway call config.get --params '{}'  # capture payload.hash
supportClaw gateway call config.patch --params '{
...
SupportClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.supportClaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, SupportClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `SUPPORT_CLAW_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${SUPPORT_CLAW_GATEWAY_TOKEN}" } },
