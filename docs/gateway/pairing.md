...
supportClaw nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), SupportClaw treats
...
Pairing state is stored under the Gateway state directory (default `~/.supportClaw`):

- `~/.supportClaw/nodes/paired.json`
- `~/.supportClaw/nodes/pending.json`

If you override `SUPPORT_CLAW_STATE_DIR`, the `nodes/` folder moves with it.
