...
SupportClaw supports OAuth and API keys for model providers. For always-on gateway
...
setup, but SupportClaw also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `supportClaw gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.supportClaw/.env` so the daemon can read it:
...
API keys for daemon use: `supportClaw onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.supportClaw/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in SupportClaw as a supported token
path. Anthropic staff has since told us that SupportClaw-style Claude CLI usage is
allowed again, so SupportClaw treats Claude CLI reuse and `claude -p` usage as
...
2. Tell SupportClaw to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching SupportClaw auth profile.
...
```bash
supportClaw models auth paste-token --provider openrouter
```
...
```bash
supportClaw models status --check
```
...
- If auth exists but SupportClaw cannot resolve a probeable model candidate for
...
- Anthropic staff told us this SupportClaw integration path is allowed again.
- SupportClaw therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `SUPPORT_CLAW_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- SupportClaw retries with the next key only for rate-limit errors (for example
...
When provider auth is removed through the Gateway control plane, SupportClaw deletes
...
Use `supportClaw models auth login --provider <id> --profile-id <profileId>` for
...
```bash
supportClaw models auth login --provider anthropic --force
```
...
When you debug order issues, `supportClaw models status --probe` shows omitted
...
Run `supportClaw models status` to confirm which profile is expiring. If an
