...
SupportClaw can create a local diagnostics zip for bug reports. It combines
...
2. SupportClaw sends the diagnostics preamble and asks for one explicit exec
   approval. The approval runs `supportClaw gateway diagnostics export --json`.
   Do not approve diagnostics through an allow-all rule.
3. After approval, SupportClaw replies with a pasteable report containing the local
   bundle path, manifest summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but SupportClaw does not
...
that the diagnostics flow was sent privately. If SupportClaw cannot find a private
owner route, the command fails closed and asks the owner to run it from a DM.

When the active SupportClaw session is using the native OpenAI Codex harness,
the same exec approval also covers an OpenAI feedback upload for the Codex
runtime threads SupportClaw knows about. That upload is separate from the local
...
the channels, SupportClaw session ids, Codex thread ids, and local resume commands
for the threads that were sent to OpenAI servers. If you deny or ignore the
approval, SupportClaw does not run the export, does not send Codex feedback, and
...
Persisted bundles live under `~/.supportClaw/logs/stability/` when events exist.

## Useful options

```bash
supportClaw gateway diagnostics export \
  --output supportClaw-diagnostics.zip \
  --log-lines 5000 \
  --log-bytes 1000000
```
