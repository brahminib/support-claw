---
summary: "Schema-accurate configuration examples for common SupportClaw setups"
read_when:
  - Learning how to configure SupportClaw
...
Save to `~/.supportClaw/supportClaw.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.supportClaw/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  cron: {
    enabled: true,
    store: "~/.supportClaw/cron/cron.json",
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/supportClaw" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.supportClaw/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
