...
SupportClaw has two log "surfaces":
...
- Default rolling log file is under `/tmp/supportClaw/` (one file per day): `supportClaw-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.supportClaw/supportClaw.json`:
  - `logging.file`
  - `logging.level`
...
SupportClaw can mask sensitive tokens before log or transcript output leaves the
...
`supportClaw gateway` supports a per-gateway style switch:
