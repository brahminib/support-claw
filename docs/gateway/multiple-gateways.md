---
summary: "Run multiple SupportClaw Gateways on one host (isolation, ports, and profiles)"
read_when:
...
supportClaw --profile rescue onboard
supportClaw --profile rescue gateway install --port 19789
```

If your main bot is already running, that is usually all you need.

During `supportClaw --profile rescue onboard`:
...
`supportClaw --profile rescue onboard` uses the normal onboarding flow, but it
...
- config file
- state directory
- workspace (by default `~/.supportClaw/workspace-rescue`)
...
# extra gateway
supportClaw --profile ops setup
supportClaw --profile ops gateway --port 19789
```

If you want both Gateways to use named profiles, that also works:

```bash
supportClaw --profile main setup
supportClaw --profile main gateway --port 18789
...
supportClaw --profile ops gateway install --port 19789
```
...
- `SUPPORT_CLAW_CONFIG_PATH` — per-instance config file
- `SUPPORT_CLAW_STATE_DIR` — per-instance sessions, creds, caches
...
Base port = `gateway.port` (or `SUPPORT_CLAW_GATEWAY_PORT` / `--port`).
...
```bash
SUPPORT_CLAW_CONFIG_PATH=~/.supportClaw/main.json \
SUPPORT_CLAW_STATE_DIR=~/.supportClaw \
supportClaw gateway --port 18789

SUPPORT_CLAW_CONFIG_PATH=~/.supportClaw/rescue.json \
SUPPORT_CLAW_STATE_DIR=~/.supportClaw-rescue \
supportClaw gateway --port 19789
```
...
supportClaw --profile rescue gateway status --deep
supportClaw --profile rescue gateway probe
supportClaw status
supportClaw --profile rescue status
supportClaw --profile rescue browser status
```

Interpretation:

- `gateway status --deep` helps catch stale launchd/systemd/schtasks services from older installs.
- `gateway probe` warning text such as `multiple reachable gateway identities detected` is expected only when you intentionally run more than one isolated gateway, or when SupportClaw cannot prove reachable probe targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is one gateway with multiple transports, even when transport ports differ.
