...
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to SupportClaw
...
External apps should talk to SupportClaw through the Gateway protocol today. Use
...
<Warning>
  There is no public npm client package yet. Do not add SupportClaw client package
...
<Note>
  This page is for code outside the SupportClaw process. Plugin code that runs
  inside SupportClaw should use documented `supportClaw/plugin-sdk/*` subpaths instead.
</Note>
...
| [`supportClaw agent`](/cli/agent)          | Ready  | One-shot script integration when shelling out to the CLI is enough.                           |
| [`supportClaw message`](/cli/message)      | Ready  | Sending messages or channel actions from scripts.                                             |
...
4. Pin the SupportClaw version you test against.
5. Recheck the RPC reference when upgrading SupportClaw.
...
Use Gateway RPC when code lives outside SupportClaw:
...
Use the Plugin SDK when code runs inside SupportClaw:
...
External apps should not import `supportClaw/plugin-sdk/*`; those subpaths are for
plugins loaded by SupportClaw.
