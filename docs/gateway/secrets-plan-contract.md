...
read_when:
  - Generating or reviewing `supportClaw secrets apply` plans
  - Debugging `Invalid plan target path` errors
  - Understanding target type and path validation behavior
title: "Secrets apply plan contract"
---

This page defines the strict contract enforced by `supportClaw secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`supportClaw secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `providerUpserts` — an object keyed by provider alias. Each value is a
  ...
  `secrets.providers.<alias>` in `supportClaw.json`, e.g. an `exec` or `file`
  provider).
...
configured in `supportClaw.json` fail with `provider "<alias>" is not
configured`.
...
- `secrets apply` writes supported `supportClaw.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `supportClaw secrets configure` or fix the target path to a supported shape above.
