---
summary: "Use OpenShell as a managed sandbox backend for SupportClaw agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for SupportClaw. Instead of running Docker
containers locally, SupportClaw delegates sandbox lifecycle to the `openshell` CLI,
...
- OpenShell plugin installed (`supportClaw plugins install @supportclaw/openshell-sandbox`)
- The `openshell` CLI installed and on `PATH` (or set a custom path via
  `plugins.entries.openshell.config.command`)
- An OpenShell account with sandbox access
- SupportClaw Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, SupportClaw creates an OpenShell
...
- You edit files locally outside SupportClaw and want those changes visible in the
...
- When the sandbox is first created, SupportClaw seeds the remote workspace from
...
- SupportClaw does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside SupportClaw after the initial seed, the remote sandbox does **not** see those changes. Use `supportClaw sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"supportClaw"`  | Sandbox source for first-time create                  |
...
1. SupportClaw calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. SupportClaw calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `supportClaw sandbox` commands
