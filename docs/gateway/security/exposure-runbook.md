---
summary: "Pre-flight and rollback checklist before exposing an SupportClaw Gateway beyond loopback"
...
- Backup location for `~/.supportClaw/supportClaw.json` and credentials.
...
```bash
supportClaw gateway probe --url ws://127.0.0.1:18789 --token "$SUPPORT_CLAW_GATEWAY_TOKEN"
```
...
Run `supportClaw security audit --deep` after proxy changes. Trusted-proxy findings
...
1. Re-run `supportClaw security audit --deep`.
...
5. Re-run `supportClaw security audit --deep`.
