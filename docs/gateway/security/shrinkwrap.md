---
summary: "Plain-English and technical explanation of npm shrinkwrap in SupportClaw releases"
read_when:
  - You want to know what npm shrinkwrap means in an SupportClaw release
...
SupportClaw source checkouts use `pnpm-lock.yaml`. Published SupportClaw npm
...
For SupportClaw releases, that means:
...
it does not replace host isolation, `supportClaw security audit`, package
...
| --------------------- | ------------------------ | --------------------------------- |
| `pnpm-lock.yaml`      | SupportClaw source checkout | Maintainer dependency graph       |
| `npm-shrinkwrap.json` | Published npm package    | npm install graph for users       |
| `package-lock.json`   | Local npm apps           | Not the SupportClaw publish contract |

## Why SupportClaw uses it

SupportClaw is a gateway, plugin host, model router, and agent runtime. A default
...
The root `supportClaw` npm package and SupportClaw-owned npm plugin packages include
`npm-shrinkwrap.json` when they publish. Suitable SupportClaw-owned plugin
...
SupportClaw package validators require shrinkwrap in new root package tarballs.
...
validators reject `package-lock.json` for published SupportClaw packages.

To inspect a published root package:

```bash
npm pack supportClaw@<version> --json --pack-destination /tmp/supportClaw-pack
tar -tf /tmp/supportClaw-pack/supportClaw-<version>.tgz | grep '^package/npm-shrinkwrap.json$'
```

To inspect an SupportClaw-owned plugin package:

```bash
npm pack @supportclaw/discord@<version> --json --pack-destination /tmp/supportClaw-plugin-pack
tar -tf /tmp/supportClaw-plugin-pack/supportClaw-discord-<version>.tgz | grep '^package/npm-shrinkwrap.json$'
tar -tf /tmp/supportClaw-plugin-pack/supportClaw-discord-<version>.tgz | grep '^package/node_modules/'
```
