...
SupportClaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
agent can inspect, including `supportClaw.json`, `auth-profiles.json`, `.env`, or
...
`supportClaw secrets audit --check` reports no plaintext secret residue.
</Warning>
...
- legacy plaintext residue has been scrubbed from `supportClaw.json`,
  `auth-profiles.json`, `.env`, and generated `models.json` files
- `supportClaw secrets audit --check` is clean after the migration
...
<AccordionGroup>
  <Accordion title="Examples of inactive surfaces">
    ...
    - `gateway.auth.token` SecretRef is inactive for startup auth resolution when `SUPPORT_CLAW_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, SupportClaw runs preflight validation before saving:
...
<AccordionGroup>
  ...
  <Accordion title="Exec provider">
    ...
    - Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). SupportClaw validates the resolved target path.
    ...
    - Plugin-managed exec providers can use `pluginIntegration` instead of
      copied `command`/`args`. SupportClaw resolves the current command details
...
<AccordionGroup>
      ...
      secrets: {
        providers: {
          onepassword_openai: {
            ...
            args: ["read", "op://Personal/SupportClaw QA API Key/password"],
            passEnv: ["HOME"],
            jsonOnly: false,
          },
        },
  ...
  <Accordion title="Bitwarden Secrets Manager (`bws`)">
    ...
    `scripts/secrets/supportClaw-bws-resolver.mjs`; install or copy it to an absolute
    ...
    ```json5
    {
      secrets: {
        providers: {
          bws: {
            source: "exec",
            command: "/usr/local/bin/supportClaw-bws-resolver.mjs",
        ...
        providers: {
          openai: {
            ...
            apiKey: {
              source: "exec",
              provider: "bws",
              id: "supportClaw/providers/openai/apiKey",
            },
          },
        },
      },
    ...
    SecretRef id contract, such as `supportClaw/providers/openai/apiKey`; env-var
    ...
    ```bash
    supportClaw secrets audit --allow-exec
    ```
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            allowSymlinkCommand: true, // required for Homebrew symlinked binaries
            trustedDirs: ["/opt/homebrew"],
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/supportClaw"],
  ...
  <Accordion title="password-store (`pass`)">
    ...
    `/usr/local/bin/supportClaw-pass-resolver`. The `#!/usr/bin/env node` shebang
    ...
    ```json5
    {
      secrets: {
        providers: {
          pass_store: {
            source: "exec",
            command: "/usr/local/bin/supportClaw-pass-resolver",
    ...
    ```bash
    supportClaw secrets audit --check
    supportClaw secrets audit --allow-exec
    ```
...
- SupportClaw resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__SUPPORT_CLAW_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `supportClaw.json` refs)
...
When reload-time activation fails after a healthy state, SupportClaw enters degraded secrets state.
...
<Tabs>
  <Tab title="Strict command paths">
    For example `supportClaw memory` remote-memory paths and `supportClaw qr --remote` when it needs remote shared-secret refs. They read from the active snapshot and fail fast when a required SecretRef is unavailable.
  </Tab>
  <Tab title="Read-only command paths">
    For example `supportClaw status`, `supportClaw status --all`, `supportClaw channels status`, `supportClaw channels resolve`, `supportClaw security audit`, and read-only doctor/config repair flows. They also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `supportClaw secrets reload`.
...
<Steps>
  ...
  <Step title="Configure and apply SecretRefs">
    ```bash
    supportClaw secrets configure --apply
    ```
  </Step>
  <Step title="Re-audit">
    ```bash
...
with `supportClaw secrets apply --from <plan-path>` before the re-audit.

<AccordionGroup>
  <Accordion title="secrets audit">
    Findings include:

    - plaintext values at rest (`supportClaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
    - plaintext sensitive provider header residues in generated `models.json` entries
    - unresolved refs
    - precedence shadowing (`auth-profiles.json` taking priority over `supportClaw.json` refs)
    - legacy residues (`auth.json`, OAuth reminders)

    Exec note:

    - By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
    - Use `supportClaw secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - lets you select supported secret-bearing fields in `supportClaw.json` plus `auth-profiles.json` for one agent scope
    ...
    - `supportClaw secrets configure --providers-only`
    - `supportClaw secrets configure --skip-provider-setup`
    - `supportClaw secrets configure --agent <id>`
  ...
  <Accordion title="secrets apply">
    ...
    ```bash
    supportClaw secrets apply --from /tmp/supportClaw-secrets-plan.json
    supportClaw secrets apply --from /tmp/supportClaw-secrets-plan.json --allow-exec
    supportClaw secrets apply --from /tmp/supportClaw-secrets-plan.json --dry-run
    supportClaw secrets apply --from /tmp/supportClaw-secrets-plan.json --dry-run --allow-exec
    ```
...
<Warning>
SupportClaw intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
