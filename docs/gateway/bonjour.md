...
SupportClaw can use Bonjour (mDNS / DNS-SD) to discover an active Gateway (WebSocket endpoint).
...
2. Publish DNS-SD records for `_supportClaw-gw._tcp` under a dedicated zone
   (example: `supportClaw.internal.`).
3. Configure Tailscale **split DNS** so your chosen domain resolves via that
   DNS server for clients (including iOS).

SupportClaw supports any discovery domain; `supportClaw.internal.` is just an example.
...
- serve your chosen domain (example: `supportClaw.internal.`) from `~/.supportClaw/dns/<domain>.db`

Validate from a tailnet-connected machine:

```bash
dns-sd -B _supportClaw-gw._tcp supportClaw.internal.
dig @<TAILNET_IPV4> -p 53 _supportClaw-gw._tcp.supportClaw.internal PTR +short
```
...
`_supportClaw-gw._tcp` in your discovery domain without multicast.
...
- Set `gateway.bind: "tailnet"` in `~/.supportClaw/supportClaw.json`.
- Restart the Gateway (or restart the macOS menubar app).

## What advertises

Only the Gateway advertises `_supportClaw-gw._tcp`. LAN multicast advertising is
provided by the bundled `bonjour` plugin when the plugin is enabled; wide-area
DNS-SD publishing remains Gateway-owned.

## Service types

- `_supportClaw-gw._tcp` - gateway transport beacon (used by macOS/iOS/Android nodes).
...
- Browse instances:
  ...
  ```bash
  dns-sd -B _supportClaw-gw._tcp local.
  ```

- Resolve one instance (replace `<instance>`):

  ```bash
  dns-sd -L "<instance>" _supportClaw-gw._tcp local.
  ```
...
in-progress states. If the service never reaches `announced`, SupportClaw eventually
...
invalid DNS-label character, SupportClaw falls back to `supportClaw.local`. Set
`SUPPORT_CLAW_MDNS_HOSTNAME=<name>` before starting the Gateway when you need an
explicit host label.

## Debugging on iOS node

The iOS node uses `NWBrowser` to discover `_supportClaw-gw._tcp`.
...
```bash
supportClaw plugins enable bonjour
```
...
discovery plugin for that SupportClaw config:

```bash
supportClaw plugins disable bonjour
```

## Docker gotchas

The bundled Bonjour plugin auto-disables LAN multicast advertising in detected
containers when `SUPPORT_CLAW_DISABLE_BONJOUR` is unset. Docker bridge networks
...
- Disabling Bonjour does not change `gateway.bind`; Docker still defaults to
  `SUPPORT_CLAW_GATEWAY_BIND=lan` so the published host port can work.
...
- Reusing the same `SUPPORT_CLAW_CONFIG_DIR` outside Docker does not persist the
  container auto-disable policy.
- Set `SUPPORT_CLAW_DISABLE_BONJOUR=0` only for host networking, macvlan, or another
...
1. Confirm whether the Gateway is running in auto, forced-on, or forced-off mode:
   ...
   ```bash
   docker compose config | grep SUPPORT_CLAW_DISABLE_BONJOUR
   ```
...
4. If you deliberately enabled the Bonjour plugin in Docker and forced advertising
   with `SUPPORT_CLAW_DISABLE_BONJOUR=0`, test multicast from the host:
   ...
   cancellations, restore `SUPPORT_CLAW_DISABLE_BONJOUR=1` and use a direct or
   Tailnet route.
...
- **Advertiser stuck in probing/announcing**: hosts with blocked multicast,
  ...
  non-announced state. SupportClaw retries a few times and then disables Bonjour
...
- **Docker bridge networking**: Bonjour auto-disables in detected containers.
  Set `SUPPORT_CLAW_DISABLE_BONJOUR=0` only for host, macvlan, or another
...
- `supportClaw plugins enable bonjour` enables the bundled LAN discovery plugin on hosts where it is not default-enabled.
- `supportClaw plugins disable bonjour` disables LAN multicast advertising by disabling the bundled plugin.
- `SUPPORT_CLAW_DISABLE_BONJOUR=1` disables LAN multicast advertising without changing plugin config; accepted truthy values are `1`, `true`, `yes`, and `on` (legacy: `SUPPORT_CLAW_DISABLE_BONJOUR`).
- `SUPPORT_CLAW_DISABLE_BONJOUR=0` forces LAN multicast advertising on, including inside detected containers; accepted falsy values are `0`, `false`, `no`, and `off`.
- When the Bonjour plugin is enabled and `SUPPORT_CLAW_DISABLE_BONJOUR` is unset, Bonjour advertises on normal hosts and auto-disables inside detected containers.
- `gateway.bind` in `~/.supportClaw/supportClaw.json` controls the Gateway bind mode.
- `SUPPORT_CLAW_SSH_PORT` overrides the SSH port when `sshPort` is advertised (legacy: `SUPPORT_CLAW_SSH_PORT`).
- `SUPPORT_CLAW_TAILNET_DNS` publishes a MagicDNS hint in TXT when mDNS full mode is enabled (legacy: `SUPPORT_CLAW_TAILNET_DNS`).
- `SUPPORT_CLAW_CLI_PATH` overrides the advertised CLI path (legacy: `SUPPORT_CLAW_CLI_PATH`).
