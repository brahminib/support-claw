...
- Use the macOS app's remote mode (Settings → General → SupportClaw runs).
...
- `supportClaw health` and `supportClaw status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `supportClaw gateway status`, `supportClaw gateway health`, `supportClaw gateway probe`, and `supportClaw gateway call` can also target the forwarded URL via `--url` when needed.

<Note>
Replace `18789` with your configured `gateway.port` (or `--port` or `SUPPORT_CLAW_GATEWAY_PORT`).
</Note>
...
- URL override safety:
  ...
  - Env URL overrides (`SUPPORT_CLAW_GATEWAY_URL`) may use env credentials only (`SUPPORT_CLAW_GATEWAY_TOKEN` / `SUPPORT_CLAW_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `SUPPORT_CLAW_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `SUPPORT_CLAW_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `SUPPORT_CLAW_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `SUPPORT_CLAW_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `SUPPORT_CLAW_GATEWAY_*` only.
...
```bash
supportClaw config set gateway.remote.token "<your-token>"
```

#### Step 4: create the LaunchAgent

Save this as `~/Library/LaunchAgents/ai.supportClaw.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.supportClaw.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.supportClaw.ssh-tunnel.plist
```

The tunnel will start automatically at login, restart on crash, and keep the forwarded port live.

<Note>
If you have a leftover `com.supportClaw.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
</Note>
...
```bash
launchctl kickstart -k gui/$UID/ai.supportClaw.ssh-tunnel
```

Stop the tunnel:

```bash
launchctl bootout gui/$UID/ai.supportClaw.ssh-tunnel
```
