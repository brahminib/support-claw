...
```bash
supportClaw nodes describe --node <idOrNameOrIp>
supportClaw nodes canvas snapshot --node <idOrNameOrIp>
supportClaw logs --follow
```
...
supportClaw nodes status
supportClaw approvals get --node <idOrNameOrIp>
supportClaw approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`supportClaw approvals get --node ...`), not in the gateway pairing record.
