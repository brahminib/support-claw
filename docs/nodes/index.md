...
`supportClaw nodes …` works against this Mac). In remote gateway mode, browser
automation is handled by the CLI node host (`supportClaw node run` or the
...
supportClaw devices reject <requestId>
...
`supportClaw devices list` before approving.
...
- `node.pair.*` (CLI: `supportClaw nodes pending/approve/reject/remove/rename`) is a separate gateway-owned
  node pairing store; it does **not** gate the WS `connect` handshake.
- `supportClaw nodes remove --node <id|name|ip>` removes a node pairing. For a
...
- **Approvals**: enforced on the node host via `~/.supportClaw/exec-approvals.json`.

Approval note:

- Approval-backed node runs bind exact request context.
- For direct shell/runtime file executions, SupportClaw also best-effort binds one concrete local
  file operand and denies the run if that file changes before execution.
- If SupportClaw cannot identify exactly one concrete local file for an interpreter/runtime command,
...
```bash
supportClaw node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
export SUPPORT_CLAW_GATEWAY_TOKEN="<gateway-token>"
supportClaw node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

Notes:

- `supportClaw node run` supports token or password auth.
- Env vars are preferred: `SUPPORT_CLAW_GATEWAY_TOKEN` / `SUPPORT_CLAW_GATEWAY_PASSWORD`.
...
- Node-host auth resolution only honors `SUPPORT_CLAW_GATEWAY_*` env vars.

### Start a node host (service)

```bash
supportClaw node install --host <gateway-host> --port 18789 --display-name "Build Node"
supportClaw node start
...
If the node retries with changed auth details, re-run `supportClaw devices list`
and approve the current `requestId`.

Naming options:

- `--display-name` on `supportClaw node run` / `supportClaw node install` (persists in `~/.supportClaw/node.json` on the node).
- `supportClaw nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
```bash
supportClaw approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
supportClaw approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

Approvals live on the node host at `~/.supportClaw/exec-approvals.json`.
...
```bash
supportClaw config set tools.exec.host node
supportClaw config set tools.exec.security allowlist
supportClaw config set tools.exec.node "<id-or-name>"
```
...
```bash
supportClaw nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
supportClaw nodes canvas snapshot --node <idOrNameOrIp> --format png
supportClaw nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas controls

```bash
supportClaw nodes canvas present --node <idOrNameOrIp> --target https://example.com
supportClaw nodes canvas hide --node <idOrNameOrIp>
supportClaw nodes canvas navigate https://example.com --node <idOrNameOrIp>
supportClaw nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
supportClaw nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
supportClaw nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
supportClaw nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
supportClaw nodes camera list --node <idOrNameOrIp>
supportClaw nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
supportClaw nodes camera snap --node <idOrNameOrIp> --facing front
```

Video clips (`mp4`):

```bash
supportClaw nodes camera clip --node <idOrNameOrIp> --duration 10s
supportClaw nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
supportClaw nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
supportClaw nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
supportClaw nodes location get --node <idOrNameOrIp>
supportClaw nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
supportClaw nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from SupportClaw"}'
```
...
```bash
supportClaw nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
supportClaw nodes invoke --node <idOrNameOrIp> --command device.apps --params '{"limit":10}'
supportClaw nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
supportClaw nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
```bash
supportClaw nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
supportClaw nodes invoke --node <idOrNameOrIp> --command system.which --params '{"name":"git"}'
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.supportClaw/exec-approvals.json`).
...
```bash
supportClaw config set tools.exec.node "node-id-or-name"
```
...
```bash
supportClaw config unset tools.exec.node
supportClaw config unset 'agents.list[0].tools.exec.node'
```
...
SupportClaw can run a **headless node host** (no UI) that connects to the Gateway
...
```bash
supportClaw node run --host <gateway-host> --port 18789
```

Notes:

- Pairing is still required (the Gateway will show a device pairing prompt).
- The node host stores its node id, token, display name, and gateway connection info in `~/.supportClaw/node.json`.
- Exec approvals are enforced locally via `~/.supportClaw/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host executes `system.run` locally by default. Set
  `SUPPORT_CLAW_NODE_EXEC_HOST=app` to route `system.run` through the companion app exec host; add
  `SUPPORT_CLAW_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `supportClaw nodes …` works against this Mac).
