...
## Config (`~/.supportClaw/supportClaw.json`)
...
- `consultThinkingLevel`: optional thinking level override for the full SupportClaw agent run behind realtime `supportClaw_agent_consult` calls.
- `consultFastMode`: optional fast-mode override for realtime `supportClaw_agent_consult` calls.
...
- `realtime.consultRouting`: `provider-direct` preserves the provider's direct reply when it skips `supportClaw_agent_consult`; `force-agent-consult` makes Gateway relay route finalized user transcripts through SupportClaw instead.
- `realtime.instructions`: appends provider-facing system instructions to SupportClaw's built-in realtime prompt. Use it for voice style and tone; SupportClaw keeps the default `supportClaw_agent_consult` guidance.
...
- Browser realtime Talk uses `talk.client.toolCall` for `supportClaw_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `supportClaw-mlx-tts` helper when present, or an executable on `PATH`. Set `SUPPORT_CLAW_MLX_TTS_BIN` to point at a custom helper binary during development.
