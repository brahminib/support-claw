...
frontmatter and a markdown body. SupportClaw loads bundled skills plus any local
...
SupportClaw loads from these sources, **highest precedence first**. When the same
...
| 4           | Managed / local skills | `~/.supportClaw/skills`                    |
| 5           | Bundled skills         | shipped with the install                |
| 6 — lowest  | Extra directories      | `skills.load.extraDirs` + plugin skills |

Skill roots support grouped layouts. SupportClaw discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** an SupportClaw
  skill root. Use `supportClaw migrate plan codex` to inventory those skills, then
  `supportClaw migrate codex` to copy them into your SupportClaw workspace.
</Note>
...
| Shared managed | `~/.supportClaw/skills`         | All agents on this machine  |
...
`supportClaw.plugin.json` (paths relative to the plugin root). Plugin skills load
...
skill overrides them. Gate them via `metadata.supportClaw.requires.config` on the
...
```bash
supportClaw skills workshop list
supportClaw skills workshop inspect <proposal-id>
supportClaw skills workshop apply <proposal-id>
```
...
`supportClaw skills` commands for install and update, or the `clawhub` CLI for
publish and sync.

| Action                             | Command                                                |
| ---------------------------------- | ------------------------------------------------------ |
| Install a skill into the workspace | `supportClaw skills install <slug>`                       |
| Install from a Git repository      | `supportClaw skills install git:owner/repo@ref`           |
| Install a local skill directory    | `supportClaw skills install ./path/to/skill --as my-tool` |
| Install for all local agents       | `supportClaw skills install <slug> --global`              |
| Update all workspace skills        | `supportClaw skills update --all`                         |
| Update a shared managed skill      | `supportClaw skills update <slug> --global`               |
| Update all shared managed skills   | `supportClaw skills update --all --global`                |
| Verify a skill's trust envelope    | `supportClaw skills verify <slug>`                        |
| Print the generated Skill Card     | `supportClaw skills verify <slug> --card`                 |
| Publish / sync via ClawHub CLI     | `clawhub sync --all`                                   |

<AccordionGroup>
  <Accordion title="Install details">
    `supportClaw skills install` installs into the active workspace `skills/`
    directory by default. Add `--global` to install into the shared
    `~/.supportClaw/skills` directory, visible to all local agents unless agent
    ...
    `supportClaw skills update` tracks ClawHub installs only — reinstall Git or
    local sources to refresh them.

  </Accordion>
  <Accordion title="Verification and security scanning">
    `supportClaw skills verify <slug>` asks ClawHub for the skill's
  ...
  <Accordion title="Private archive installs">
    ...
    `supportClaw.json`. Normal ClawHub installs never need that setting.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Path containment">
    ...
    Managed `~/.supportClaw/skills` and personal `~/.agents/skills` may contain
...
<Note>
  SupportClaw follows the [AgentSkills](https://agentskills.io) spec. The
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, SupportClaw keeps the skill's instructions out of the agent's normal
...
SupportClaw filters skills at load time using `metadata.supportClaw` (single-line
JSON in the frontmatter). A skill with no `metadata.supportClaw` block is always
...
<ParamField path="requires.config" type="string[]">
  Each `supportClaw.json` path must be truthy.
</ParamField>
...
<Note>
  ...
  `metadata.supportClaw` is absent, so older installed skills keep their
  dependency gates and installer hints. New skills should use
  `metadata.supportClaw`.
</Note>
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, SupportClaw lists each entry so you can
      see all available artifacts.
    - Specs can include `os: ["darwin"|"linux"|"win32"]` to filter by platform.
    - Node installs honor `skills.install.nodeManager` in `supportClaw.json`
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** SupportClaw does not auto-install Homebrew or translate brew
    ...
    - **Download:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`),
      ...
      `targetDir` (default: `~/.supportClaw/tools/<skillKey>`).
  </Accordion>
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience field for skills that declare `metadata.supportClaw.primaryEnv`.
...
<Note>
  ...
  `metadata.supportClaw.skillKey`, use that key under `skills.entries`. Quote
  hyphenated names: JSON5 allows quoted keys.
</Note>

## Environment injection

When an agent run starts, SupportClaw:

<Steps>
  <Step title="Reads skill metadata">
    SupportClaw resolves the effective skill list for the agent, applying gating
...
For the bundled `claude-cli` backend, SupportClaw also materializes the same
...
SupportClaw snapshots eligible skills **when a session starts** and reuses that
...
allowlist changes, SupportClaw refreshes the snapshot to keep visible skills
aligned.

<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, SupportClaw watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, SupportClaw can treat macOS-only skills as eligible when
    ...
    answering bin probes, SupportClaw clears its cached bin matches.
...
When skills are eligible, SupportClaw injects a compact XML block into the system
