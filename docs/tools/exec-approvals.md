...
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `supportClaw approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `supportClaw exec-policy show`                                      | Local-machine merged view.                                                             |
| `supportClaw exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `supportClaw` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, SupportClaw also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
`SUPPORT_CLAW_STATE_DIR` is set, the file follows that state directory;
otherwise it uses the default SupportClaw state directory:

```text
$SUPPORT_CLAW_STATE_DIR/exec-approvals.json
# otherwise
~/.supportClaw/exec-approvals.json
```

The default approval socket follows the same root:
`$SUPPORT_CLAW_STATE_DIR/exec-approvals.sock`, or
`~/.supportClaw/exec-approvals.sock` when the variable is unset.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.supportClaw/exec-approvals.sock",
...
- `auto` - use allowlist policy, run deterministic matches directly, and send approval misses through SupportClaw's native auto reviewer before falling back to a human approval route.
...
<ParamField path="askFallback" type='"deny" | "allowlist" | "full"'>
  ...
  field is omitted, SupportClaw defaults to `deny`.
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, SupportClaw treats inline code-eval forms as approval-only
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  ...
  SupportClaw may attach parser-derived command spans so Web approval
...
**both** policy layers - requested exec policy in SupportClaw config
(`tools.exec.*`) **and** host-local approvals policy in
the execution host approvals file.

SupportClaw defaults omitted `askFallback` to `deny`. Set host
...
- In YOLO mode, SupportClaw does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when SupportClaw's effective exec
policy is YOLO. For SupportClaw-managed Claude live sessions, SupportClaw's
...
If you want a more conservative setup, tighten SupportClaw exec policy back to
...
<Steps>
  ...
  <Step title="Match the host approvals file">
    ```bash
    supportClaw approvals set --stdin <<'EOF'
    {
      version: 1,
      defaults: {
        security: "full",
...
```bash
supportClaw exec-policy preset yolo
```
...
approvals remotely, use `supportClaw approvals set --gateway` or
`supportClaw approvals set --node <id|name|ip>`.
...
```bash
supportClaw approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `supportClaw exec-policy` does not synchronize node approvals.
- `supportClaw exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `supportClaw approvals --node ...`.
...
specific argument shape. SupportClaw evaluates the regular expression
...
entries instead of hand-editing the encoded value. If SupportClaw cannot
...
CLI: `supportClaw approvals` supports gateway or node editing - see
...
SupportClaw posts the denial back into that session as an internal followup so the
agent can stop waiting on the async command and avoid a missing-result repair.
If there is no session or the session cannot be resumed, SupportClaw can still
...
When an async exec approval is denied, SupportClaw treats the host command as
...
session delivery is unavailable, SupportClaw falls back to a concise operator or
