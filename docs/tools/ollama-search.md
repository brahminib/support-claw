...
SupportClaw supports **Ollama Web Search** as a bundled `web_search` provider. It
...
- an Ollama host that is reachable from SupportClaw
- `ollama signin`
...
If no explicit Ollama base URL is set, SupportClaw uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, SupportClaw reuses
...
- If the Ollama host is auth-protected, SupportClaw reuses the normal Ollama
  provider API key when present.
- If `baseUrl` is `https://ollama.com`, SupportClaw calls
...
- If the configured host does not expose web search and `OLLAMA_API_KEY` is set,
  SupportClaw can fall back to `https://ollama.com/api/web_search` without sending
  that env key to the local host.
- SupportClaw warns during setup if Ollama is unreachable or not signed in, but
  it does not block selection.
- SupportClaw does not auto-select Ollama Web Search when no higher-priority
