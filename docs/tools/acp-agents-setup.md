...
read_when:
  ...
  - Enabling the plugin-tools or SupportClaw-tools MCP bridge
...
Codex has two SupportClaw routes:
...
When SupportClaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal SupportClaw `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by SupportClaw before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, SupportClaw/acpx cannot force a model selection.
...
Packaged installs use the official `@supportclaw/acpx` runtime plugin for ACP.
...
```bash
supportClaw plugins install ./path/to/local/acpx-plugin
```
...
`ready` signal. Set `SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE=0` or
`SUPPORT_CLAW_SKIP_ACPX_RUNTIME_PROBE=1` only for scripts or environments that
...
- `command` accepts an absolute path, relative path (resolved from the SupportClaw workspace), or command name.
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before SupportClaw passes it through the current acpx command-string registry.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install SupportClaw globally with `npm install -g supportClaw`, the acpx
...
normally and reports the missing dependency through `supportClaw acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose SupportClaw plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
SupportClaw plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
supportClaw config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `supportClaw-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled SupportClaw
  plugins.
...
### SupportClaw tools MCP bridge

By default, ACPX sessions also do **not** expose built-in SupportClaw tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
supportClaw config set plugins.entries.acpx.config.supportClawToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `supportClaw-tools` into ACPX session
  bootstrap.
- Exposes selected built-in SupportClaw tools. The initial server exposes `cron`.
...
```bash
supportClaw config set plugins.entries.acpx.config.timeoutSeconds 180
```

Runtime turns use SupportClaw agent/run timeouts, including `/acp timeout`.
...
```bash
supportClaw config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from SupportClaw exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.

For the broader comparison between SupportClaw `tools.exec.mode`, Codex Guardian
...
```bash
supportClaw config set plugins.entries.acpx.config.permissionMode approve-all
supportClaw config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
SupportClaw defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
