...
SupportClaw supports [SearXNG](https://docs.searxng.org/) as a **self-hosted,
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    supportClaw plugins install @supportclaw/searxng-plugin
    ```
  </Step>
...
- **Category fallback** -- if a non-`general` category request succeeds but
  returns zero results, SupportClaw retries the same query once with `general`
