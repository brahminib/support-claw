...
`~/.supportClaw/supportClaw.json`. Agent-specific visibility lives under
...
<ParamField path="skills.install.nodeManager" type='"npm" | "pnpm" | "yarn" | "bun"' default='"npm"'>
  ...
  for WhatsApp/Telegram). Use `supportClaw setup --node-manager` for npm, pnpm,
...
runs after SupportClaw has staged source material and before the install or update
...
```json5
{
  security: {
    installPolicy: {
      ...
      exec: {
        source: "exec",
        command: "/usr/local/bin/supportClaw-install-policy",
        args: ["--json"],
        timeoutMs: 10000,
        noOutputTimeoutMs: 10000,
        maxOutputBytes: 1048576,
        passEnv: ["SUPPORT_CLAW_STATE_DIR", "PATH"],
...
<ParamField path="security.installPolicy.exec.command" type="string">
  Absolute path to the trusted policy executable. SupportClaw runs it without a
...
<ParamField path="security.installPolicy.exec.passEnv" type="string[]">
  Environment variable names copied from the SupportClaw process into the policy
...
`supportClawVersion`, `targetType`, `targetName`, `sourcePath`, `sourcePathKind`,
...
SupportClaw does not execute install policy during normal Gateway startup. Installs
and updates fail closed when policy is enabled but unavailable. `supportClaw doctor`
performs static validation, and `supportClaw doctor --deep` executes a synthetic
...
{
  "protocolVersion": 1,
  "supportClawVersion": "2026.6.1",
  "targetType": "skill",
  "targetName": "weather",
  "sourcePath": "/var/folders/.../supportClaw-skill-clawhub/root",
  ...
  "origin": {
    "type": "clawhub",
    "registry": "https://clawhub.supportClaw.ai",
    "slug": "weather",
    "version": "1.0.0"
  },
  "request": {
...
`metadata.supportClaw.skillKey`, use that key instead. Quote hyphenated names
...
Managed `~/.supportClaw/skills` and personal `~/.agents/skills` directories
