...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in SupportClaw
...
Today this applies to SupportClaw embedded runs and SupportClaw dynamic tools in the Codex
app-server harness. Tokenjuice hooks SupportClaw's tool-result middleware and
...
```bash
supportClaw plugins install clawhub:@supportclaw/tokenjuice
```
...
```bash
supportClaw config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
supportClaw plugins disable tokenjuice
```
