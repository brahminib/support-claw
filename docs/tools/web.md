...
SupportClaw also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Configure">
    ```bash
    supportClaw configure --section web
    ```
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "SupportClaw plugin SDK" });
    ```
...
search and SupportClaw's managed `web_search` dynamic tool are mutually exclusive,
so managed search cannot bypass native domain restrictions. SupportClaw uses the
managed tool when hosted search is unavailable, explicitly disabled, or
replaced by a selected managed provider. SupportClaw keeps Codex's standalone
...
- Set `tools.web.search.provider` to a managed provider such as `brave` to use
  SupportClaw's managed `web_search` instead
...
use the managed `web_search` fallback through SupportClaw's dynamic tool namespace.
Use an explicit managed provider when you need SupportClaw's provider-specific
...
Codex app-server first with `supportClaw models auth login --provider openai`.
The parent agent can use any model or runtime; only the bounded search worker
runs through Codex.

## Network safety

Managed HTTP `web_search` provider calls use SupportClaw's guarded fetch path. For
trusted provider API hosts, SupportClaw allows Surge, Clash, and sing-box fake-IP
...
If no `provider` is set, SupportClaw checks providers in this order and uses the
...
`supportClaw configure --section web`. SupportClaw does not send managed
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, SupportClaw resolves only the
...
Grok can also reuse an xAI OAuth auth profile from `supportClaw models auth login
...
SupportClaw keeps startup resilient and reports a warning so you can reinstall the
plugin or run `supportClaw doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let SupportClaw auto-detect the first ready web-fetch
...
When you choose **Kimi** during `supportClaw onboard` or
`supportClaw configure --section web`, SupportClaw can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `supportClaw doctor --fix`.
When you choose Grok during `supportClaw onboard` or `supportClaw configure --section web`,
SupportClaw can also offer optional `x_search` setup with the same credential.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, SupportClaw does not
...
<Tabs>
  <Tab title="Config file">
    Run `supportClaw configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.supportClaw/.env`.
...
optional structured filters. SupportClaw only enables the built-in xAI `x_search`
