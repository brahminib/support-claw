...
SupportClaw can use **Firecrawl** in three ways:
...
```bash
supportClaw plugins install @supportclaw/firecrawl-plugin
...
- Choosing Firecrawl in onboarding or `supportClaw configure --section web` enables the installed Firecrawl plugin automatically.
...
- The explicitly selected Firecrawl `web_fetch` fallback works without an API key. When configured, SupportClaw sends `plugins.entries.firecrawl.config.webFetch.apiKey` or `FIRECRAWL_API_KEY` for higher limits.
- Choosing Firecrawl during onboarding or `supportClaw configure --section web` enables the plugin and selects Firecrawl for `web_fetch` unless another fetch provider is already configured.
- `firecrawl_scrape` requires an API key.
- `maxAgeMs` controls how old cached results can be (ms). Default is 2 days.
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `supportClaw doctor --fix`.
...
when you run Firecrawl yourself. SupportClaw accepts `http://` only for loopback,
...
SupportClaw always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, SupportClaw
