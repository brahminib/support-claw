...
read_when:
  - You want SupportClaw to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current SupportClaw session.
...
SupportClaw waits for the next fresh session token snapshot and uses that as the
...
cost reporting, and context-window behavior still use the normal SupportClaw
usage and model controls.

## Model tools

SupportClaw exposes three core goal tools to agent harnesses:
...
The `/goal` command works in command-capable SupportClaw sessions, including the
...
token snapshot yet. Usage refreshes as SupportClaw records session usage and
