...
SupportClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, SupportClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `supportClaw configure --section web`. It stores the key in
`~/.supportClaw/supportClaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.supportClaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
