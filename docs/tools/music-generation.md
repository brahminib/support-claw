...
For session-backed agent runs, SupportClaw starts music generation as a
...
from the completion reply, SupportClaw sends an idempotent direct fallback with
...
<Note>
Not all providers support all parameters. SupportClaw still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, SupportClaw
...
Provider request timeouts are operator configuration only. SupportClaw uses
...
- **Status lookup:** `supportClaw tasks list` or `supportClaw tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** SupportClaw injects an internal completion event back
...
```bash
SUPPORT_CLAW_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
