...
SupportClaw supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), SupportClaw reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.supportClaw/.env`.

If you omit `baseUrl`, SupportClaw defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, SupportClaw defaults to `kimi-k2.6`.
...
SupportClaw treats Kimi `web_search` as successful only after Moonshot returns
...
SupportClaw returns a structured `kimi_web_search_ungrounded` error instead of
