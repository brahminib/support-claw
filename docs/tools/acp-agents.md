---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, SupportClaw ACP, OpenCode) through the ACP backend"
read_when:
...
let SupportClaw run external coding harnesses (for example Claude Code,
Cursor, Copilot, Droid, SupportClaw ACP, OpenCode, Gemini CLI, and other
...
directly to existing SupportClaw channel conversations, use
[`supportClaw mcp serve`](/cli/mcp) instead of ACP.
</Note>
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ SupportClaw | This page                             | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                                                   |
| Expose an SupportClaw Gateway session _as_ an ACP server for an editor or client                   | [`supportClaw acp`](/cli/acp)            | Bridge mode. IDE/client talks ACP to SupportClaw over stdio/WebSocket                                                                                                                            |
| Reuse a local AI CLI as a text-only fallback model                                              | [CLI Backends](/gateway/cli-backends) | Not ACP. No SupportClaw tools, no ACP controls, no harness runtime                                                                                                                               |
...
```bash
supportClaw plugins install @supportclaw/acpx
supportClaw config set plugins.entries.acpx.enabled true
```
...
SupportClaw only teaches agents about ACP spawning when ACP is **truly
...
<AccordionGroup>
  <Accordion title="First-run gotchas">
    ...
    - Codex ACP runs with an isolated `CODEX_HOME`; SupportClaw copies trusted project entries plus safe model/provider routing config from the host Codex config, while auth, notifications, and hooks stay on the host config.
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. SupportClaw owns routing,
    ...
    Before blaming SupportClaw, verify:
...
SupportClaw plugin tools and built-in SupportClaw tools are **not** exposed to
...
| `supportClaw` | SupportClaw Gateway bridge through `supportClaw acp` | Lets an ACP-aware harness talk back to an SupportClaw Gateway session.                 |
| `qwen`     | Qwen Code / Qwen CLI                           | Requires Qwen-compatible auth on the host.                                          |

Custom acpx agent aliases can be configured in acpx itself, but SupportClaw
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the SupportClaw session store, and may create a background task when the run is parent-owned.
    ...
    - `close` ends the ACP session from SupportClaw's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
    - The acpx plugin cleans up SupportClaw-owned wrapper and adapter process trees after `close`, and reaps stale SupportClaw-owned ACPX orphans during Gateway startup.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    SupportClaw dynamic tools still execute through SupportClaw, while
    Codex-native tools such as shell/apply-patch execute inside Codex.
    For Codex-native tool events, SupportClaw injects a per-turn native
    hook relay so plugin hooks can block `before_tool_call`, observe
    `after_tool_call`, and route Codex `PermissionRequest` events
    through SupportClaw approvals. Codex `Stop` hooks are relayed to
    SupportClaw `before_agent_finalize`, where plugins can request one more
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    SupportClaw picks `runtime: "acp"`, resolves the harness `agentId`,
    ...
    SupportClaw config agent id from `agents_list` unless that entry is
    explicitly configured with `agents.list[].runtime.type="acp"`;
    otherwise use the default sub-agent runtime. When an SupportClaw agent
    is configured with `runtime.type="acp"`, SupportClaw uses
...
plugin is enabled. Use **sub-agents** when you want SupportClaw-native
delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | SupportClaw native sub-agent runtime  |
...
1. SupportClaw ACP session control plane.
2. Official `@supportclaw/acpx` runtime plugin.
...
- **ACP session** - the durable Codex/Claude/Gemini runtime state SupportClaw routes to.
...
spawned ACP session - no child thread, same chat surface. SupportClaw keeps
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` only works on channels that advertise current-conversation binding; SupportClaw returns a clear unsupported message otherwise. Bindings persist across gateway restarts.
    - On Discord, `spawnSessions` gates child thread creation for `--thread auto|here` - not `--bind here`.
    - If you spawn to a different ACP agent without `--cwd`, SupportClaw inherits the **target agent's** workspace by default. Missing inherited paths (`ENOENT`/`ENOTDIR`) fall back to the backend default; other access errors (e.g. `EACCES`) surface as spawn errors.
    - Gateway management commands stay local in bound conversations - `/acp ...` commands are handled by SupportClaw even when normal follow-up text routes to the bound ACP session; `/status` and `/unfocus` also stay local whenever command handling is enabled for that surface.

  </Accordion>
  <Accordion title="Thread-bound sessions">
    When thread bindings are enabled for a channel adapter:

    - SupportClaw binds a thread to a target ACP session.
    ...
    adapter does not support thread bindings, SupportClaw returns a clear
...
<ParamField path="bindings[].agentId" type="string">
  The owning SupportClaw agent id.
</ParamField>
...
- SupportClaw ensures the configured ACP session exists after channel-specific admission and before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, SupportClaw inherits the target agent workspace from agent config.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    for ACP sessions. If `agentId` is omitted, SupportClaw uses
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, SupportClaw may default to persistent behaviour per
...
<ParamField path="model" type="string">
  ...
  `session/set_model`; otherwise SupportClaw/acpx fails clearly instead of
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    What SupportClaw sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. SupportClaw agents get SupportClaw's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_SUPPORT_CLAW_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in SupportClaw where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. SupportClaw converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see SupportClaw-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    peer sessions, SupportClaw uses an agent-to-agent (A2A) follow-up path
    ...
    SupportClaw skips the A2A follow-up only when the requester is the
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not an SupportClaw channel session key; SupportClaw still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new SupportClaw session you are creating, so `mode: "session"` still requires `thread: true`.
...
- SupportClaw's sandbox policy does **not** wrap ACP harness execution.
- SupportClaw still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced SupportClaw-native work.
...
If no target resolves, SupportClaw returns a clear error
...
| ---------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, SupportClaw normalizes `openai/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai/gpt-5.4/high` to `reasoning_effort`.                                         |
| `/acp set thinking <level>`  | canonical option `thinking`          | SupportClaw sends the backend-advertised equivalent when present, preferring `thinking`, then `effort`, `reasoning_effort`, or `thought_level`. For Codex ACP, the adapter maps values to `reasoning_effort`. |
| `/acp permissions <profile>` | canonical option `permissionProfile` | SupportClaw sends the backend-advertised equivalent when present, such as `approval_policy`, `permission_profile`, `permissions`, or `permission_mode`.                                                       |
| `/acp timeout <seconds>`     | canonical option `timeoutSeconds`    | SupportClaw sends the backend-advertised equivalent when present, such as `timeout` or `timeout_seconds`.                                                                                                     |
...
aliases), the plugin-tools and SupportClaw-tools MCP bridges, and ACP
...
| Vendor auth error from the harness                                          | SupportClaw is healthy, but the target CLI/provider is not logged in.                                                     | Log in or provide the required provider key on the Gateway host environment.                                                                                             |
...
| ACP session stalls indefinitely after completing work                       | Harness process finished but ACP session did not report completion.                                                    | Update SupportClaw; current acpx cleanup reaps SupportClaw-owned stale wrapper and adapter processes on close and Gateway startup.                                             |
| Harness sees `<<<BEGIN_SUPPORT_CLAW_INTERNAL_CONTEXT>>>`                        | Internal event envelope leaked across the ACP boundary.                                                                | Update SupportClaw and rerun the completion flow; external harnesses should receive plain completion prompts only.                                                          |
...
native tool call, restart the Codex app-server or SupportClaw Gateway instead of
...
- [`supportClaw acp` (bridge mode)](/cli/acp)
