...
Skill Workshop is SupportClaw's governed path for creating and updating workspace
skills.
...
```bash
supportClaw skills workshop propose-update trip-planning --proposal ./PROPOSAL.md
```
...
- `autonomous.enabled`: allows SupportClaw to create pending proposals from durable
...
```text
<SUPPORT_CLAW_STATE_DIR>/skill-workshop/
  proposals.json
...
Default state directory: `~/.supportClaw`.
...
| Proposal does not show in list                 | Check the selected `--agent` workspace and `SUPPORT_CLAW_STATE_DIR`.                                                                                                                                            |
...
- [Skills CLI](/cli/skills) for `supportClaw skills` commands
