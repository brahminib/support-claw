...
It is modeled after Claude Code's `/btw` behavior, but adapted to SupportClaw's
...
instead of falling back to a direct provider call. SupportClaw seeds sanitized
conversation context into a fresh one-shot CLI invocation, disables SupportClaw MCP
...
If the main run is currently active, SupportClaw snapshots the current message
