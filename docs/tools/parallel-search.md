...
```bash
supportClaw plugins install @supportclaw/parallel-plugin
      ...
      parallel: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://api.parallel.ai", // optional; SupportClaw appends /v1/search
          },
        },
      },
    },
...
example, the Cloudflare AI Gateway). SupportClaw normalizes bare hosts by
...
SupportClaw exposes Parallel's native search shape so the model can fill in both
...
- Result excerpts come back as the `excerpts` array and are also joined into
  ...
  contract
- Parallel returns a `session_id` on every response; SupportClaw surfaces it as
...
- `searchId`, `warnings`, and `usage` from Parallel are passed through when
  present
- SupportClaw always forwards a resolved result count to Parallel as
  `advanced_settings.max_results`. The caller's `count` arg wins, then the
  top-level `tools.web.search.maxResults` setting, otherwise SupportClaw's
