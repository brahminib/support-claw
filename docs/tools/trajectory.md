---
summary: "Export redacted trajectory bundles for debugging an SupportClaw agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an SupportClaw session
...
Trajectory capture is SupportClaw's per-session flight recorder. It records a
...
SupportClaw writes the bundle under the workspace:

```text
.supportClaw/trajectory-exports/supportClaw-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.supportClaw/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, SupportClaw sends the
...
Trajectory capture is on by default for SupportClaw agent runs.
...
```json
{
  "traceSchema": "supportClaw-trajectory",
  "schemaVersion": 1
}
```
...
| `metadata.json`       | SupportClaw version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
SupportClaw also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `SUPPORT_CLAW_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export SUPPORT_CLAW_TRAJECTORY_DIR=/var/lib/supportClaw/trajectories
```

When this variable is set, SupportClaw writes one JSONL file per session id in that
directory.
...
Set `SUPPORT_CLAW_TRAJECTORY=0` before starting SupportClaw:

```bash
export SUPPORT_CLAW_TRAJECTORY=0
```
...
SupportClaw flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`SUPPORT_CLAW_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting SupportClaw:

```bash
export SUPPORT_CLAW_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when SupportClaw logs an `supportClaw-trajectory-flush` timeout and continues.
It does not change the trajectory size caps. To tune all agent cleanup steps
that do not pass an explicit timeout, set `SUPPORT_CLAW_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are designed for support and debugging, not public posting.
SupportClaw redacts sensitive values before writing export files:
...
- confirm SupportClaw was started without `SUPPORT_CLAW_TRAJECTORY=0`
- check whether `SUPPORT_CLAW_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.supportClaw/trajectory-exports/`
