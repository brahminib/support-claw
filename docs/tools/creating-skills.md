...
summary: "Build, test, and publish custom SKILL.md workspace skills for your SupportClaw agents."
read_when:
...
SupportClaw loads skills from several roots in a defined [precedence order](/tools/skills#loading-order).
...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.supportClaw/workspace/skills/hello-world
    ```
    ...
    ```bash
    mkdir -p ~/.supportClaw/workspace/skills/personal/hello-world
  ...
  <Step title="Verify the skill loaded">
    ...
    SupportClaw watches `SKILL.md` files under skills roots by default. If the
  ...
  <Step title="Test it">
    ...
    ```bash
    supportClaw agent --message "give me a greeting"
    ```
...
metadata: { "supportClaw": { "requires": { "bins": ["gemini"] }, "primaryEnv": "GEMINI_API_KEY" } }
---
```
...
<AccordionGroup>
  <Accordion title="Gating options">
    ...
    | `requires.config` | Each `supportClaw.json` path must be truthy |
  ...
  <Accordion title="Environment and API keys">
    Wire an API key to a skill entry in `supportClaw.json`:
...
supportClaw skills workshop propose-update hello-world \
  --proposal ./PROPOSAL.md \
...
<Steps>
  <Step title="Ensure your SKILL.md is complete">
    Make sure `name`, `description`, and any `metadata.supportClaw` gating fields
  ...
  <Step title="Install the ClawHub skill">
    ...
    ```bash
    supportClaw skills install clawhub-publish
    ```
...
<Tip>
  ...
  - **Test locally** — use `supportClaw agent --message "..."` before sharing.
