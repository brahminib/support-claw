...
SupportClaw supports Gemini models with built-in
...
For a gateway install, put env keys in `~/.supportClaw/.env`.
...
`pd`, `pw`, `pm`, and `py`. SupportClaw converts these values, or an explicit
