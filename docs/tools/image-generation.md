...
SupportClaw records a background task, returns the task id immediately, and wakes
...
generated images are still missing from the completion reply, SupportClaw sends an
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai` OAuth profile is configured, SupportClaw routes image requests
...
nearby geometry option instead of the exact requested one, SupportClaw remaps to
...
SupportClaw tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when SupportClaw can
  ...
  <Accordion title="Timeouts">
    ...
    SupportClaw's 600000 ms dynamic-tool bridge maximum.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai` OAuth profile is configured, SupportClaw reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. SupportClaw
    ...
    SupportClaw forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible SupportClaw maps
    ...
    transparency-capable OpenAI image model. SupportClaw routes default
  ...
  <Accordion title="Microsoft Foundry MAI image models">
    ...
    onboarding/model metadata so SupportClaw can verify that the deployment is
  ...
  <Accordion title="OpenRouter image models">
    ...
    SupportClaw forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="fal Krea 2">
    ...
    `image_size` schema used by Flux. SupportClaw sends:
    ...
    Krea; SupportClaw maps `size` to the closest supported Krea aspect ratio and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as SupportClaw-managed image attachments

    SupportClaw intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for SupportClaw image generation" size=3840x2160 count=1
```
  </Tab>
...
`--openai-moderation` flags are available on `supportClaw infer image edit`;
