---
summary: "Install, configure, and manage SupportClaw plugins"
read_when:
...
Plugins extend SupportClaw with channels, model providers, agent harnesses, tools,
...
- an SupportClaw checkout or installation with the `supportClaw` CLI available
...
<Steps>
  <Step title="Find the plugin">
    ...
    they match an official plugin id. Raw `@supportclaw/*` package specs that match
    bundled plugins use the bundled copy from the current SupportClaw build. Use an
  ...
  <Step title="Configure and enable it">
    ...
    ```bash
    supportClaw plugins enable <plugin-id>
    ```
    ...
    `supportClaw plugins install` adds the installed id to an existing
  ...
  <Step title="Let the Gateway reload">
    ...
    enabled, SupportClaw detects the changed plugin install record and restarts the
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| ClawHub     | You want SupportClaw-native discovery, scans, version metadata, and install hints | `supportClaw plugins install clawhub:<package>`                   |
| npm         | You need direct npm registry or dist-tag workflows                             | `supportClaw plugins install npm:<package>`                       |
| git         | You need a branch, tag, or commit from a repository                            | `supportClaw plugins install git:github.com/<owner>/<repo>@<ref>` |
...
a bundled plugin id, SupportClaw uses that bundled source. If it matches an
official external plugin id, SupportClaw uses the official package catalog. Other
ordinary bare package specs install through npm during the launch cutover. Raw
`@supportclaw/*` package specs that match bundled plugins also resolve to the
bundled copy before npm fallback. Use `npm:@supportclaw/<plugin>@<version>` when
...
deterministic source selection. See [`supportClaw plugins`](/cli/plugins#install)
for the full command contract.

For npm installs, unpinned package specs and `@latest` choose the newest stable
package that advertises compatibility with this SupportClaw build. If npm's
current latest release declares a newer `supportClaw.compat.pluginApi` or
`supportClaw.install.minHostVersion`, SupportClaw scans older stable package versions
...
SupportClaw processes where plugin hooks are loaded, so use `security.installPolicy`
...
not bypass install policy or SupportClaw's built-in plugin dependency denylist.
...
Run `supportClaw doctor` or `supportClaw doctor --fix` when config validation reports
stale plugin ids, allowlist/tool mismatches, or legacy bundled plugin paths.

## Understand plugin formats

SupportClaw recognizes two plugin formats:

| Format                 | How it loads                                                                 | Use when                                                               |
| ---------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Native SupportClaw plugin | `supportClaw.plugin.json` plus a runtime module loaded in process               | You are installing or building SupportClaw-specific runtime capabilities  |
| Compatible bundle      | Codex, Claude, or Cursor plugin layout mapped into SupportClaw plugin inventory | You are reusing compatible skills, commands, hooks, or bundle metadata |

Both formats appear in `supportClaw plugins list`, `supportClaw plugins inspect`,
`supportClaw plugins enable`, and `supportClaw plugins disable`. See
...
Plugin-managed internal hooks show up in `supportClaw hooks list` with
`plugin:<id>`. You cannot enable or disable them through `supportClaw hooks`;
enable or disable the plugin instead.

## Verify the active Gateway

`supportClaw plugins list` and plain `supportClaw plugins inspect` read cold config,
...
```bash
supportClaw gateway status --deep --require-rpc
supportClaw plugins inspect <plugin-id> --runtime --json
supportClaw gateway restart
```
...
sure any manual restart targets the actual `supportClaw gateway run` child that
...
| Symptom                                                        | Check                                                                                                                                      | Fix                                                                                                     |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| Plugin appears in `plugins list` but runtime hooks do not run  | Use `supportClaw plugins inspect <id> --runtime --json` and confirm the active Gateway with `gateway status --deep --require-rpc`             | Restart the live Gateway after install, update, config, or source changes                               |
| Duplicate channel or tool ownership diagnostics appear         | Run `supportClaw plugins list --enabled --verbose`, inspect each suspected plugin with `--runtime --json`, and compare channel/tool ownership | Disable one owner, remove stale installs, or use manifest `preferOver` for intentional replacement      |
...
| Config is invalid during install                               | Read the validation message and run `supportClaw doctor --fix` when it points to stale plugin state                                           | Doctor can quarantine invalid plugin config by disabling the entry and removing the invalid payload     |
| Plugin path is blocked for suspicious ownership or permissions | Inspect the diagnostic before the config error                                                                                             | Fix filesystem ownership/permissions, then run `supportClaw plugins registry --refresh`                    |
| `SUPPORT_CLAW_NIX_MODE=1` blocks lifecycle commands                | Confirm the install is managed by Nix                                                                                                      | Change plugin selection in the Nix source instead of using plugin mutator commands                      |
| Dependency import fails at runtime                             | Check whether the plugin was installed through npm/git/ClawHub or loaded from a local path                                                 | Run `supportClaw plugins update <id>`, reinstall the source, or install local plugin dependencies yourself |
...
other channel. Run `supportClaw doctor --fix` to remove stale plugin and channel
...
plugin id. If both plugins are explicitly enabled, SupportClaw keeps that request
...
SupportClaw needs at runtime. Update or reinstall after the publisher ships
...
and config validation follows with `plugin present but blocked`, SupportClaw found
...
SupportClaw as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted SupportClaw config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run SupportClaw as root, repair the managed plugin root to
root ownership instead:

```bash
sudo chown -R root:root /path/to/supportClaw-config/npm
```

After fixing ownership, rerun `supportClaw doctor --fix` or
`supportClaw plugins registry --refresh` so the persisted plugin registry matches
the repaired files.
...
```bash
supportClaw config set logging.level trace
...
SupportClaw caches successful plugin tool factory results for repeated resolutions
