...
When a site requires login, **sign in manually** in the **host** browser profile (the supportClaw browser).
...
SupportClaw controls a **dedicated Chrome profile** (named `supportClaw`, orange-tinted UI). This is separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent should use its isolated `supportClaw` browser.
...
supportClaw browser open https://x.com
```

If you have multiple profiles, pass `--browser-profile <name>` (the default is `supportClaw`).
...
```bash
supportClaw browser open https://x.com --browser-profile supportClaw
```
