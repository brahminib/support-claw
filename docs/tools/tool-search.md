---
summary: "Tool Search: compact large SupportClaw tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want SupportClaw agents to use a large tool catalog without adding every tool schema to the prompt
  - You want SupportClaw tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for SupportClaw runs
---

Tool Search is an experimental SupportClaw agent runtime feature. It gives agents one
...
This page documents SupportClaw Tool Search. It is not the Codex-native tool
...
When enabled for SupportClaw runs, the model receives one `tool_search_code` tool
by default. That tool runs a short JavaScript body in an isolated Node
subprocess with an `supportClaw.tools` bridge:

```js
const hits = await supportClaw.tools.search("create a GitHub issue");
const tool = await supportClaw.tools.describe(hits[0].id);
return await supportClaw.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include SupportClaw tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through SupportClaw.

Codex harness runs do not receive these experimental SupportClaw Tool Search
controls. SupportClaw passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the SupportClaw embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible SupportClaw and plugin tools.
...
6. Expose the SupportClaw code bridge, the structured fallback tools, or the
   compact directory surface to the model.

At execution time every real tool call returns to SupportClaw. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`supportClaw.tools.call(...)` crosses the bridge back into the Gateway, where the
...
- `directory`: exposes `tool_search`, `tool_describe`, and `tool_call` plus a
  ...
  providers that should see tool names without every full schema. SupportClaw can
  also expose a small bounded set of likely or required tool schemas directly
  for the current turn.

All modes use the same policy-filtered catalog and normal SupportClaw execution
...
for the current run while SupportClaw tools, plugin tools, and MCP tools can be
...
All modes are experimental. Prefer direct tool exposure for small SupportClaw tool
...
catalog includes eligible SupportClaw, MCP, and client tools after normal policy
filtering.
...
`supportClaw.tools.search(query, options?)`

Searches the effective catalog for the current run. Results are compact and safe
to put back into prompt context.

```js
const hits = await supportClaw.tools.search("calendar event", { limit: 5 });
```

`supportClaw.tools.describe(id)`

Loads full metadata for one search result, including the exact input schema.

```js
const calendarCreate = await supportClaw.tools.describe("mcp:calendar:create_event");
```

`supportClaw.tools.call(id, args)`

Calls a selected tool through SupportClaw.

```js
await supportClaw.tools.call(calendarCreate.id, {
  summary: "Planning",
...
the model requests an exact hidden directory tool name directly, SupportClaw
hydrates it from the authorized catalog before normal execution.
Directory-mode client tool names must not collide with SupportClaw, plugin, or MCP
...
network grants, and no child-process or worker grants. SupportClaw enforces a
...
- `supportClaw.tools.search`
- `supportClaw.tools.describe`
- `supportClaw.tools.call`

Normal SupportClaw behavior still applies to final calls:
...
Enable Tool Search for SupportClaw runs with the default code bridge:

```bash
supportClaw config set tools.toolSearch true
```
...
Use the structured fallback tools instead for SupportClaw runs:
...
Use the compact directory surface instead for SupportClaw runs:
...
- final tool calls executed through SupportClaw
...
- whether the result came from SupportClaw, MCP, or a client tool

## E2E validation

The gateway E2E runner proves both paths with the SupportClaw runtime:
