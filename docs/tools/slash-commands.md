...
the SupportClaw command handler, and `/status` plus `/unfocus` stay local whenever
...
<ParamField path="commands.config" type="boolean" default="false">
  Enables `/config` (reads/writes `supportClaw.json`). Owner-only.
</ParamField>

<ParamField path="commands.mcp" type="boolean" default="false">
  Enables `/mcp` (reads/writes SupportClaw-managed MCP config under `mcp.servers`). Owner-only.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Owner-only writes and admin">
    ...
    | --- | --- | --- |
    | `/config show\|get\|set\|unset` | `commands.config: true` | Read or write `supportClaw.json`. Owner-only |
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | Read or write SupportClaw-managed MCP server config. Owner-only |
    ...
    | `/restart` | `commands.restart: true` (default) | Restart SupportClaw |
...
/config set messages.responsePrefix="[supportClaw]"
...
`/mcp` stores config in SupportClaw config, not embedded-agent project settings.
...
<AccordionGroup>
  ...
  <Accordion title="Slack specifics">
    `channels.slack.slashCommand` supports a single `/supportClaw`-style command.
...
- **Execution vs runtime:** `/status` reports `Execution` for the effective sandbox path and `Runtime` for who is running the session: `SupportClaw Default`, `OpenAI Codex`, a CLI backend, or an ACP backend.
