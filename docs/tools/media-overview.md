...
read_when:
  - Looking for an overview of SupportClaw's media capabilities
...
SupportClaw generates images, videos, and music, understands inbound media
...
For async tools, SupportClaw submits the request to the provider, returns a task
...
SupportClaw wakes the agent with the generated media paths so it can tell the
...
SupportClaw sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until SupportClaw has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in SupportClaw until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
