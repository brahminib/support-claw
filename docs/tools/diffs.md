...
<Steps>
  <Step title="Install the plugin">
    ```bash
    supportClaw plugins install diffs
    ```
  </Step>
...
SupportClaw includes syntax highlighting for common source, config, and documentation languages:
...
Set plugin-wide defaults in `~/.supportClaw/supportClaw.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/supportClaw",
        },
      },
    },
  },
...
- Artifacts are stored under the temp subfolder: `$TMPDIR/supportClaw-diffs`.
...
<Steps>
  <Step title="Config">
    `browser.executablePath` in SupportClaw config.
  </Step>
