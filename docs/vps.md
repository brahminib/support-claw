---
summary: "Run SupportClaw on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for SupportClaw
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the SupportClaw Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install SupportClaw on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep SupportClaw bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/supportClaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- Add service env for a stable startup path:
  ...
  - `NODE_COMPILE_CACHE=/var/tmp/supportClaw-compile-cache`
...
For the standard `supportClaw onboard --install-daemon` path, edit the user unit:
...
`supportClaw-gateway.service` via `sudo systemctl edit supportClaw-gateway.service`.
