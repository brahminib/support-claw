...
read_when:
  ...
  - Wiring external triggers (webhooks, Gmail) into SupportClaw
...
<Steps>
  <Step title="Add a one-shot reminder">
    ```bash
    supportClaw cron create "2026-02-01T16:00:00Z" \
      --name "Reminder" \
      --session main \
  ...
  <Step title="Check your jobs">
    ...
    supportClaw cron get <job-id>
    supportClaw cron show <job-id>
    ```
  </Step>
  <Step title="See run history">
    ```bash
    supportClaw cron runs --id <job-id>
    ```
  </Step>
</Steps>
...
- Job definitions, runtime state, and run history persist in SupportClaw's shared SQLite state database so restarts do not lose schedules.
- On upgrade, run `supportClaw doctor --fix` to import legacy `~/.supportClaw/cron/jobs.json`, `jobs-state.json`, and `runs/*.jsonl` files into SQLite and rename them with a `.migrated` suffix. Malformed job rows are skipped from runtime and copied to `jobs-quarantine.json` for later repair or review.
- `cron.store` still names the logical cron store key and doctor import path. After import, editing that JSON file no longer changes active cron jobs; use `supportClaw cron add|edit|remove` or the Gateway cron RPC methods instead.
...
- Isolated cron runs also guard against stale acknowledgement replies. If the first result is just an interim status update (`on it`, `pulling everything together`, and similar hints) and no descendant subagent run is still responsible for the final answer, SupportClaw re-prompts once for the actual result before delivery.
...
This fires ~5–6 times per month instead of 0–1 times per month. SupportClaw uses Croner's default OR behavior here. To require both conditions, use Croner's `+` day-of-week modifier (`0 9 15 * +1`) or schedule on one field and guard the other in your job's prompt or command.
...
<AccordionGroup>
  ...
  <Accordion title="Subagent and Discord delivery">
    When isolated cron runs orchestrate subagents, delivery also prefers the final descendant output over stale parent interim text. If descendants are still running, SupportClaw suppresses that partial parent update instead of announcing it.

    For text-only Discord announce targets, SupportClaw sends the canonical final assistant text once instead of replaying both streamed/intermediate text payloads and the final answer. Media and structured Discord payloads are still delivered as separate payloads so attachments and components are not dropped.
...
For isolated jobs, chat delivery is shared. If a chat route is available, the agent can use the `message` tool even when the job uses `--no-deliver`. If the agent sends to the configured/current target, SupportClaw skips the fallback announce. Otherwise `announce`, `webhook`, and `none` only control what the runner does with the final reply after the agent turn.

When an agent creates an isolated reminder from an active chat, SupportClaw stores the preserved live delivery target for the fallback announce route. Internal session keys may be lowercase; provider delivery targets are not reconstructed from those keys when current chat context is available.
...
<Tabs>
  ...
  <Tab title="Webhook output">
    ```bash
    supportClaw cron create "0 18 * * 1-5" \
      "Summarize today's deploys as JSON." \
      --name "Deploy digest" \
      --webhook "https://example.invalid/supportClaw/cron"
    ```
  </Tab>
...
Wire Gmail inbox triggers to SupportClaw via Google PubSub.

<Note>
**Prerequisites:** `gcloud` CLI, `gog` (gogcli), SupportClaw hooks enabled, Tailscale for the public HTTPS endpoint.
</Note>

### Wizard setup (recommended)

```bash
supportClaw webhooks gmail setup --account supportClaw@gmail.com
```
...
When `hooks.enabled=true` and `hooks.gmail.account` is set, the Gateway starts `gog gmail watch serve` on boot and auto-renews the watch. Set `SUPPORT_CLAW_SKIP_GMAIL_WATCHER=1` to opt out.
...
supportClaw cron get <jobId>

# Show one job, including resolved delivery route
supportClaw cron show <jobId>

# Edit a job
supportClaw cron edit <jobId> --message "Updated prompt" --model "opus"

# Force run a job now
supportClaw cron run <jobId>

# Force run a job now and wait for its terminal status
supportClaw cron run <jobId> --wait --wait-timeout 10m --poll-interval 2s

# Run only if due
supportClaw cron run <jobId> --due

# View run history
supportClaw cron runs --id <jobId> --limit 50

# View one exact run
supportClaw cron runs --id <jobId> --run-id <runId>

# Delete a job
supportClaw cron remove <jobId>

# Agent selection (multi-agent setups)
supportClaw cron create "0 6 * * *" "Check ops queue" --name "Ops sweep" --session isolated --agent ops
supportClaw cron edit <jobId> --clear-agent
```

`supportClaw cron run <jobId>` returns after enqueueing the manual run. Use `--wait` for shutdown hooks, maintenance scripts, or other automation that must block until the queued run finishes. Wait mode polls the exact returned `runId`; it exits `0` for status `ok` and non-zero for `error`, `skipped`, or a wait timeout.
...
- `supportClaw cron edit <job-id> --clear-model` clears that override from the CLI (same effect as the `model: null` patch) and cannot be combined with `--model`.
- Configured fallback chains still apply because cron `--model` is a job primary, not a session `/model` override.
- `supportClaw cron add|edit --fallbacks ...` sets payload `fallbacks`, replacing configured fallbacks for that job; `--fallbacks ""` disables fallback and makes the run strict. `supportClaw cron edit <job-id> --clear-fallbacks` clears the per-job override.
...
```json5
{
  cron: {
    enabled: true,
    store: "~/.supportClaw/cron/jobs.json",
    maxConcurrentRuns: 8,
    retry: {
      maxAttempts: 3,
...
`cron.store` is a logical store key and legacy doctor import path. Run `supportClaw doctor --fix` to import existing JSON stores into SQLite and archive them; future cron changes should go through the CLI or Gateway API.

Disable cron: `cron.enabled: false` or `SUPPORT_CLAW_SKIP_CRON=1`.
...
<AccordionGroup>
  <Accordion title="Cron not firing">
    - Check `cron.enabled` and `SUPPORT_CLAW_SKIP_CRON` env var.
    - Confirm the Gateway is running continuously.
    - For `cron` schedules, verify timezone (`--tz`) vs the host timezone.
    - `reason: not-due` in run output means manual run was checked with `supportClaw cron run <jobId> --due` and the job was not due yet.
  ...
  <Accordion title="Cron fired but no delivery">
    ...
    - If the isolated run returns only the silent token (`NO_REPLY` / `no_reply`), SupportClaw suppresses direct outbound delivery and also suppresses the fallback queued summary path, so nothing is posted back to chat.
  ...
  <Accordion title="Cron or heartbeat appears to prevent /new-style rollover">
    ...
    - For legacy rows created before those fields existed, SupportClaw can recover `sessionStartedAt` from the transcript JSONL session header when the file is still available. Legacy idle rows without `lastInteractionAt` use that recovered start time as their idle baseline.
