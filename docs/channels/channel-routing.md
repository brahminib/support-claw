...
SupportClaw routes replies **back to the channel where a message came from**. The
...
SupportClaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, SupportClaw still records inbound session metadata, but it
...
when a guarded path must not create a new SupportClaw session. In that mode,
SupportClaw may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when SupportClaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.supportClaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.supportClaw`):

- `~/.supportClaw/agents/<agentId>/sessions/sessions.json`
