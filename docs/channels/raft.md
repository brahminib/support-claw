...
read_when:
  - You want to connect SupportClaw to a Raft workspace
...
Raft support connects an SupportClaw agent to a Raft External Agent through the local
...
```bash
supportClaw plugins install @supportclaw/raft
...
- The Raft CLI installed on the same host as the SupportClaw Gateway.
...
```bash
supportClaw channels setup raft
```
...
7. Starts one serialized SupportClaw agent turn for each accepted wake.

The bridge owns Raft delivery retries and reconnects. The SupportClaw turn receives
only a wake notice, not a copied Raft message body. It uses the CLI to read
pending messages and to send its response:

```bash
raft --profile supportClaw message check
raft --profile supportClaw message send
```

<Note>
Raft is not a normal push-message transport. SupportClaw does not automatically
...
Check that SupportClaw can find the CLI and has a configured profile:

```bash
supportClaw channels status --probe
supportClaw plugins inspect raft --runtime --json
```
