...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when SupportClaw would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When SupportClaw would reply in this chat, it will run all three agents.
...
<Steps>
  ...
  <Step title="Route and admission">
    SupportClaw applies channel allowlists, group activation rules, and configured ACP binding ownership.
  </Step>
  <Step title="Broadcast check">
    If no configured ACP binding owns the route, SupportClaw checks whether the peer ID is in `broadcast`.
  </Step>
  ...
  <Step title="If broadcast does not apply">
    SupportClaw dispatches the ordinary route or the configured ACP session route selected during routing.
  </Step>
</Steps>
...
<Tabs>
  <Tab title="Alfred's context">
    ...
    Workspace: /Users/user/supportClaw-alfred/
  ...
  <Tab title="Bärbel's context">
    ...
    Workspace: /Users/user/supportClaw-baerbel/
    Tools: read only
    ```
  </Tab>
</Tabs>
...
<Note>
**Precedence:** `broadcast` takes priority over ordinary route bindings. Configured ACP bindings (`bindings[].type="acp"`) are exclusive: when one matches, SupportClaw dispatches to the configured ACP session instead of fan-out broadcast.
</Note>
...
<AccordionGroup>
  <Accordion title="Agents not responding">
    ...
    **Debug:**
    ...
    ```bash
    tail -f ~/.supportClaw/logs/gateway.log | grep broadcast
    ```
