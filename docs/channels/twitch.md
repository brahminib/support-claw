...
read_when:
  - Setting up Twitch chat integration for SupportClaw
title: "Twitch"
sidebarTitle: "Twitch"
---

Twitch chat support via IRC connection. SupportClaw connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

<Note>
Twitch ships as a bundled plugin in current SupportClaw releases, so normal packaged builds do not need a separate install.
</Note>
...
<Tabs>
  <Tab title="npm registry">
    ```bash
    supportClaw plugins install @supportclaw/twitch
    ```
  </Tab>
  <Tab title="Local checkout">
    ```bash
    supportClaw plugins install ./path/to/local/twitch-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Ensure plugin is available">
    Current packaged SupportClaw releases already bundle it. Older/custom installs can add it manually with the commands above.
  </Step>
  ...
  <Step title="Configure the token">
    - Env: `SUPPORT_CLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "supportClaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SUPPORT_CLAW_TWITCH_ACCESS_TOKEN env var)
...
<Tabs>
  <Tab title="Env var (default account only)">
    ```bash
    SUPPORT_CLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
    ```
  </Tab>
  <Tab title="Config">
    ```json5
