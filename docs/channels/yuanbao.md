...
Tencent Yuanbao is Tencent's AI assistant platform. The SupportClaw channel plugin
connects Yuanbao bots to SupportClaw over WebSocket so they can interact with users
...
> **Requires SupportClaw 2026.4.10 or above.** Run `supportClaw --version` to check. Upgrade with `supportClaw update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  supportClaw channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
supportClaw channels login --channel yuanbao
```
...
```bash
supportClaw pairing list yuanbao
supportClaw pairing approve yuanbao <CODE>
```
