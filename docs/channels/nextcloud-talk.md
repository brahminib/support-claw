...
Nextcloud Talk ships as a bundled plugin in current SupportClaw releases, so
...
```bash
supportClaw plugins install @supportclaw/nextcloud-talk
```
...
```bash
supportClaw plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "SupportClaw" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure SupportClaw:
   ...
   ```bash
   supportClaw channels add --channel nextcloud-talk \
...
- Approve via:
  - `supportClaw pairing list nextcloud-talk`
  - `supportClaw pairing approve nextcloud-talk <CODE>`
