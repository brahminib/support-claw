...
read_when:
  - Setting up Zalo Personal for SupportClaw
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside SupportClaw.
...
Zalo Personal ships as a bundled plugin in current SupportClaw releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `supportClaw plugins install @supportclaw/zalouser`
- Pinned version: `supportClaw plugins install @supportclaw/zalouser@2026.5.2`
- Or from a source checkout: `supportClaw plugins install ./path/to/local/zalouser-plugin`
...
1. Ensure the Zalo Personal plugin is available.
   - Current packaged SupportClaw releases already bundle it.
   - Older/custom installs can add it manually with the commands above.
2. Login (QR, on the Gateway machine):
   - `supportClaw channels login --channel zalouser`
...
```bash
supportClaw directory self --channel zalouser
supportClaw directory peers list --channel zalouser --query "name"
supportClaw directory groups list --channel zalouser --query "work"
```
...
- `supportClaw pairing list zalouser`
- `supportClaw pairing approve zalouser <code>`
...
- On startup, SupportClaw resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, SupportClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in SupportClaw state. Example:
...
Profile names select the saved Zalo login credentials in SupportClaw state. Resolution order is:
...
- SupportClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, SupportClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `supportClaw channels logout --channel zalouser && supportClaw channels login --channel zalouser`
...
- The channel now runs fully in SupportClaw without external CLI binaries.
