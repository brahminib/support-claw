---
summary: "Synology Chat webhook setup and SupportClaw config"
read_when:
  - Setting up Synology Chat with SupportClaw
...
Synology Chat ships as a bundled plugin in current SupportClaw releases, so normal
...
```bash
supportClaw plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `supportClaw onboard` now shows Synology Chat in the same channel setup list as `supportClaw channels add`.
   - Non-interactive setup: `supportClaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your SupportClaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in SupportClaw.
   - Guided: `supportClaw onboard`
   - Direct: `supportClaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- SupportClaw accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `supportClaw pairing list synology-chat`
  - `supportClaw pairing approve synology-chat <CODE>`
...
```bash
supportClaw message send --channel synology-chat --target 123456 --text "Hello from SupportClaw"
supportClaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
supportClaw message send --channel synology-chat --target synology:123456 --text "Short prefix"
```

Media sends are supported by URL-based file delivery.
Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before SupportClaw forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. SupportClaw now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached SupportClaw
- `Rate limit exceeded`:
