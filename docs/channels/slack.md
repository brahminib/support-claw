...
| Request signing              | Not used (auth is the App-Level Token)                                                                                                               | Slack signs every request; SupportClaw verifies with `signingSecret`                                              |
| Recovery on connection drop  | Slack SDK auto-reconnect is enabled; SupportClaw also restarts failed Socket Mode sessions with bounded backoff. Pong-timeout transport tuning applies. | No persistent connection to drop; retries are per-request from Slack                                           |
...
Relay mode separates Slack ingress from the SupportClaw gateway. A trusted router owns the
...
```bash
supportClaw plugins install @supportclaw/slack
```
...
```json Recommended
{
  "display_information": {
    "name": "SupportClaw",
    "description": "Slack connector for SupportClaw"
  },
  "features": {
    "bot_user": { "display_name": "SupportClaw", "always_online": true },
    "app_home": {
    ...
    "assistant_view": {
      "assistant_description": "SupportClaw connects Slack assistant threads to SupportClaw agents.",
      "suggested_prompts": [
    ...
    "slash_commands": [
      {
        "command": "/supportClaw",
        "description": "Send a message to SupportClaw",
        "should_escape": false
      }
    ]
  },
...
JSON5
supportClaw config patch --file ./slack.socket.patch.json5 --dry-run
supportClaw config patch --file ./slack.socket.patch.json5
```
...
```
        ...
        <Info>
          The three URL fields (`slash_commands[].url`, `event_subscriptions.request_url`, and `interactivity.request_url` / `message_menu_options_url`) all point at the same SupportClaw endpoint. Slack's manifest schema requires them named separately, but SupportClaw routes by payload type so a single `webhookPath` (default `/slack/events`) is enough. Slash commands without `slash_commands[].url` will silently no-op in HTTP mode.
        </Info>
...
JSON5
supportClaw config patch --file ./slack.http.patch.json5 --dry-run
supportClaw config patch --file ./slack.http.patch.json5
```
...
SupportClaw sets the Slack SDK client pong timeout to 15 seconds by default for Socket Mode. Override the transport settings only when you need workspace- or host-specific tuning:
...
- Only `clientPingTimeout` has an SupportClaw default (`15000`). `serverPingTimeout` and `pingPongLoggingEnabled` are passed to the Slack SDK only when configured.
...
The default manifest enables the Slack App Home **Home** tab and subscribes to `app_home_opened`. When a workspace member opens the Home tab, SupportClaw publishes a safe default Home view with `views.publish`; no conversation payload or private configuration is included. The **Messages** tab remains enabled for Slack DMs. The manifest also enables Slack assistant threads with `features.assistant_view`, `assistant:write`, `assistant_thread_started`, and `assistant_thread_context_changed`; assistant threads route to their own SupportClaw thread sessions and keep Slack-provided thread context available to the agent.
...
<Tabs>
  <Tab title="DM policy">
    ...
    Legacy `channels.slack.dm.policy` and `channels.slack.dm.allowFrom` still read for compatibility. `supportClaw doctor --fix` migrates them to `dmPolicy` and `allowFrom` when it can do so without changing access.

    Pairing in DMs uses `supportClaw pairing approve slack <code>`.
...
- SupportClaw seeds an eligible top-level channel root into `agent:<agentId>:slack:channel:<channelId>:thread:<rootTs>` when that root is expected to start a visible Slack thread, so the root and later thread replies share one SupportClaw session. This applies to `app_mention` events, explicit bot or configured mention-pattern matches, and `requireMention: false` channels with non-`off` `replyToMode`.
...
When a `message` tool call runs inside a Slack thread and targets the same channel, SupportClaw normally inherits the current Slack thread according to `replyToMode`. Set `topLevel: true` on `action: "send"` or `action: "upload-file"` to force a new parent-channel message instead. `threadId: null` is accepted as the same top-level opt-out.
...
`ackReaction` sends an acknowledgement emoji while SupportClaw is processing an inbound message. `ackReactionScope` decides _when_ that emoji is actually sent.
...
- Top-level Slack DMs stay off-thread by default, so they do not show Slack's thread-style native stream/status preview; SupportClaw posts and edits a draft preview in the DM instead.
...
- If streaming fails mid-reply, SupportClaw falls back to normal delivery for remaining payloads.
...
- Run `supportClaw doctor --fix` to rewrite persisted Slack streaming config to the canonical keys.

## Typing reaction fallback

`typingReaction` adds a temporary reaction to the inbound Slack message while SupportClaw is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
<AccordionGroup>
  <Accordion title="Inbound attachments">
    ...
    Downloads use bounded idle and total timeouts. If Slack file retrieval stalls or fails, SupportClaw keeps processing the message and falls back to the file placeholder.
...
- The interactive callback values are SupportClaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, SupportClaw falls back to the original text reply instead of sending an invalid blocks payload.

### Plugin-owned modal submissions

Slack plugins that register an interactive handler can also receive modal
`view_submission` and `view_closed` lifecycle events before SupportClaw compacts
the payload for the agent-visible system event. Use one of these routing
patterns when opening a Slack modal:

- Set `callback_id` to `supportClaw:<namespace>:<payload>`.
...
```bash
supportClaw pairing list slack
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `supportClaw channels status --probe --json` shows `botTokenStatus` or
...
1. SupportClaw downloads the file from Slack's private URL using the bot token.
...
| Forwarded/shared attachments           | Text and Slack-hosted image/file media are best-effort                       | Re-share directly in the SupportClaw thread                                   |
...
- [PDF tool](/tools/pdf)
- Epic: [#51349](https://github.com/brahminib/support-claw/issues/51349) — Slack attachment vision enablement
- Regression tests: [#51353](https://github.com/brahminib/support-claw/issues/51353)
- Live verification: [#51354](https://github.com/brahminib/support-claw/issues/51354)
