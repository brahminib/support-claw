...
Tlon is a decentralized messenger built on Urbit. SupportClaw connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current SupportClaw releases, so normal packaged
...
```bash
supportClaw plugins install @supportclaw/tlon
```
...
```bash
supportClaw plugins install ./path/to/local/tlon-plugin
```
...
By default, SupportClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `supportClaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, SupportClaw replies in-thread.
