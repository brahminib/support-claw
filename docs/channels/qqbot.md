...
read_when:
  - You want to connect SupportClaw to QQ
  - You need QQ Bot credential setup
  - You want QQ Bot group or private chat support
title: QQ bot
---

QQ Bot connects to SupportClaw via the official QQ Bot API (WebSocket gateway). The
...
```bash
supportClaw plugins install @supportclaw/qqbot
```
...
```bash
supportClaw channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `supportClaw channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single SupportClaw instance:
...
```bash
supportClaw channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
...
Old QQBot `toolPolicy` entries are retired. Run `supportClaw doctor --fix` to migrate them to `tools`.
...
| `/bot-version` | Show the SupportClaw framework version                                                                      |
...
- Inbound, outbound, and gateway bridge paths share a single media payload root under `~/.supportClaw/media`, so uploads, downloads, and transcode caches land under one guarded directory instead of a per-subsystem tree.
...
- Credentials can be backed up and restored as part of standard SupportClaw credential snapshots; the engine re-attaches each account's resource stack on restore without requiring a fresh QR-code pair.

## QR-code onboarding

As an alternative to pasting `AppID:AppSecret` manually, the engine supports a QR-code onboarding flow for linking a QQ Bot to SupportClaw:

1. Run the QQ Bot setup path (for example `supportClaw channels add --channel qqbot`) and pick the QR-code flow when prompted.
2. Scan the generated QR code with the phone app tied to the target QQ Bot.
3. Approve the pairing on the phone. SupportClaw persists the returned credentials into `credentials/` under the right account scope.

Approval prompts generated by the bot itself (for example, "allow this action?" flows exposed by the QQ Bot API) surface as native SupportClaw prompts that you can accept with `/bot-approve` rather than replying through the raw QQ client.
...
- **Repeated self-replies:** SupportClaw records QQ outbound ref indexes as
