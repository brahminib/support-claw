---
summary: "Zalo ClawBot channel setup through the external supportClaw-zaloclawbot plugin"
read_when:
  - You want a personal Zalo assistant bot with QR-code login
  - You are installing or troubleshooting the supportClaw-zaloclawbot channel plugin
title: "Zalo ClawBot"
---

SupportClaw connects to Zalo ClawBot through the catalog-listed external
`@zalo-platforms/supportClaw-zaloclawbot` plugin. Login uses a Zalo Mini App QR
code.

## Compatibility

| Plugin Version | SupportClaw Version | npm dist-tag | Status        |
| -------------- | ---------------- | ------------ | ------------- |
...
- Node.js **>= 22**
- [SupportClaw](https://docs.supportClaw.ai/install) must be installed (`supportClaw` CLI available).
- A Zalo account on a mobile device to scan the login QR code.

## Install with onboard (recommended)

Run the SupportClaw onboarding wizard and pick **Zalo ClawBot** from the channel menu:
...
```bash
supportClaw plugins install "@zalo-platforms/supportClaw-zaloclawbot@0.1.4"
```

Use the exact pinned version shown above (it matches the official catalog entry), so SupportClaw verifies the package against the catalog integrity hash during install.

### 2. Enable the plugin in config

```bash
supportClaw config set plugins.entries.supportClaw-zaloclawbot.enabled true
```

### 3. Generate QR code and log in

```bash
supportClaw channels login --channel supportClaw-zaloclawbot
```
...
The external plugin manages bot credentials under the SupportClaw state directory.
Treat that directory as sensitive and include it in the same access-control and
backup policy as the rest of your SupportClaw state.
...
- **Gateway Fails to Load:** Ensure your SupportClaw host version is `2026.4.10` or higher. Older versions do not support the external npm-plugin installation ledger.
