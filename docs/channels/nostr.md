...
read_when:
  - You want SupportClaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables SupportClaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current SupportClaw releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`supportClaw onboard`) and `supportClaw channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
supportClaw plugins install @supportclaw/nostr
```
...
```bash
supportClaw plugins install --link <path-to-local-nostr-plugin>
```
...
supportClaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "supportClaw",
        displayName: "SupportClaw",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "supportClaw@example.com",
        lud16: "supportClaw@example.com",
      },
    },
  },
}
