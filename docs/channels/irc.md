...
title: IRC
read_when:
  - You want to connect SupportClaw to IRC channels or DMs
  - You are configuring IRC allowlists, group policy, or mention gating
---

Use IRC when you want SupportClaw in classic channels (`#room`) and direct messages.
...
```bash
supportClaw plugins install @supportclaw/irc
```

2. Enable IRC config in `~/.supportClaw/supportClaw.json`.
3. Set at least:
...
- IRC uses raw TCP/TLS sockets outside SupportClaw operator-managed forward proxy routing. In deployments that require all egress through that forward proxy, set `channels.irc.enabled=false` unless direct IRC egress is explicitly approved.
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, SupportClaw defaults to **mention-gating** in group contexts.
