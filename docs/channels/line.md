...
read_when:
  - You want to connect SupportClaw to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to SupportClaw via the LINE Messaging API. The plugin runs as a webhook
...
```bash
supportClaw plugins install @supportclaw/line
```

Local checkout (when running from a git repo):

```bash
supportClaw plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so SupportClaw applies strict pre-auth body limits and timeout before verification.
- SupportClaw processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
supportClaw pairing list line
supportClaw pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.supportClaw/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. SupportClaw validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
