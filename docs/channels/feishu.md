...
<Note>
Requires SupportClaw 2026.5.29 or above. Run `supportClaw --version` to check. Upgrade with `supportClaw update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  supportClaw channels login --channel feishu
  ```
...
```bash
supportClaw pairing list feishu
supportClaw pairing approve feishu <CODE>
```
...
4. Check logs: `supportClaw logs --follow`
...
5. Ensure the gateway is running: `supportClaw gateway status`
6. Check logs: `supportClaw logs --follow`

### QR setup does not react in the Feishu mobile app

1. Rerun setup: `supportClaw channels login --channel feishu`
...
3. Restart the gateway: `supportClaw gateway restart`
...
If a named account created an unscoped dynamic agent on an older release, that legacy agent still counts toward `maxAgents`. Confirm that it is not used by the default account before removing it, or temporarily increase `maxAgents`; SupportClaw cannot safely infer which account owns ambiguous legacy state.
</Note>
  ...
  channels: {
    feishu: {
      ...
      dynamicAgentCreation: {
        enabled: true,
        workspaceTemplate: "~/.supportClaw/workspace-{agentId}",
        agentDirTemplate: "~/.supportClaw/agents/{agentId}/agent",
      },
    },
  },
  session: {
...
| `channels.feishu.dynamicAgentCreation.workspaceTemplate` | Path template for dynamic agent workspaces | `~/.supportClaw/workspace-{agentId}`    |
| `channels.feishu.dynamicAgentCreation.agentDirTemplate`  | Agent directory name template              | `~/.supportClaw/agents/{agentId}/agent` |
...
workspace: /Users/you/.supportClaw/workspace-feishu-ou_xxxxxx
...
of raw `file_key` JSON. When `tools.media.audio` is configured, SupportClaw
...
conversion fails, SupportClaw falls back to a file attachment and logs the reason.
...
topic session key. If a native topic starter event omits `thread_id`, SupportClaw
hydrates it from Feishu before routing the turn. Normal group replies that
SupportClaw turns into threads keep using the reply root message ID (`om_*`) so the
