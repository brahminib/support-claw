...
read_when:
  - You want to connect SupportClaw to SMS through Twilio
  - You need SMS webhook or allowlist setup
title: "SMS"
---

SupportClaw can receive and send SMS through a Twilio phone number or Messaging Service. The Gateway registers an inbound webhook route, validates Twilio request signatures by default, and sends replies back through Twilio's Messages API.
...
- The official SMS plugin installed with `supportClaw plugins install @supportclaw/sms`.
- A Twilio account with an SMS-capable phone number, or a Twilio Messaging Service.
- The Twilio Account SID and Auth Token.
- A public HTTPS URL that reaches your SupportClaw Gateway.
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    supportClaw plugins install @supportclaw/sms
    ```
  </Step>
...
```bash
supportClaw config patch --file ./sms.patch.json5 --dry-run
supportClaw config patch --file ./sms.patch.json5
```
...
```bash
supportClaw pairing list sms
supportClaw pairing approve sms <CODE>
```
...
`authToken` can be a SecretRef. Use this when the Gateway should resolve the Twilio Auth Token from the SupportClaw secrets runtime instead of storing plaintext config:
...
```bash
supportClaw message send --channel sms --target sms:+15551234567 --message "hello"
```
...
```bash
supportClaw message send --target twilio-sms:+15551234567 --message "hello"
```
...
SMS output is plain text. SupportClaw strips markdown, flattens fenced code blocks, preserves readable links, and chunks long replies before sending them through Twilio.
...
```bash
supportClaw channels capabilities --channel sms
supportClaw channels status --channel sms --probe --json
```

3. Send an SMS to the Twilio number from your phone.
4. Run `supportClaw pairing list sms`.
5. Approve the pairing code with `supportClaw pairing approve sms <CODE>`.
6. Send another SMS and confirm the agent replies.

For outbound-only testing, use:

```bash
supportClaw message send --channel sms --target sms:+15557654321 --message "SupportClaw SMS test"
```
...
```bash
imsg send --to "+15551234567" --service sms --text "SupportClaw SMS E2E $(date -u +%Y%m%dT%H%M%SZ)" --json
supportClaw pairing list sms
supportClaw pairing approve sms <CODE>
...
By default, SupportClaw validates `X-Twilio-Signature` using `publicWebhookUrl` and `authToken`. Keep `publicWebhookUrl` byte-for-byte aligned with the URL configured in Twilio, including scheme, host, path, and query string.
...
### Twilio returns 403 or SupportClaw rejects the webhook
