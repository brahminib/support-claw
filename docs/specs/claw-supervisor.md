...
summary: "Fleet supervision plan for Codex app-server sessions controlled by SupportClaw."
read_when:
  - Designing Codex fleet supervision
  - Building SupportClaw tools that read, steer, or spawn Codex sessions
...
Claw Supervisor lets one always-on SupportClaw instance monitor and drive a fleet of Codex sessions without changing the normal Codex user experience. A user can SSH into a host, start Codex, work in the TUI, and still have the supervisor read the session, steer it, interrupt it, spawn related sessions, and accept handoffs. Codex sessions can also call back into SupportClaw through MCP.

## Product Model

Codex remains the primary work surface. SupportClaw supervises Codex rather than hiding Codex inside an opaque SupportClaw subagent.

The SupportClaw plugin is named `codex-supervisor`. `crabfleet` remains the deployment
...
- Supervisor Claw: an always-on SupportClaw agent with tools for fleet state, transcript reads, steering, interruption, spawning, and handoff.

SupportClaw may use its existing subagent machinery internally, but the external contract is an attachable Codex session with a Codex thread id.
...
- SupportClaw tools for Claw-to-Codex control.
...
Every supervised Codex gets an MCP server named `supportClaw-codex-supervisor`.
...
- Supervisor state lives in SupportClaw Gateway storage.
...
- Transcript reads redact secrets at SupportClaw boundary.
...
Phase 2: SupportClaw integration
...
- Durable state owner: SupportClaw Gateway DB, Cloudflare Durable Object, or VPS database.
