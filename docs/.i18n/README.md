...
- repo: `supportClaw/docs`
- local checkout: `~/Projects/supportClaw-docs`

## Source of truth

- English docs are authored in `supportClaw/supportClaw`.
...
1. Edit English docs in `supportClaw/supportClaw`.
2. Push to `main`.
3. `supportClaw/supportClaw/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `supportClaw/docs`.
...
5. `supportClaw/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `supportClaw/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `supportClaw/docs` instead.
...
- Sync metadata is written to `.supportClaw-sync/source.json` in the publish repo.
- Source repo secret: `SUPPORT_CLAW_DOCS_SYNC_TOKEN`
- Publish repo secret: `SUPPORT_CLAW_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `supportClaw/docs` first.
