---
summary: "Host SupportClaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up SupportClaw on Oracle Cloud
  - Looking for free VPS hosting for SupportClaw
  - Want 24/7 SupportClaw on a small server
title: "Oracle Cloud"
---

Run a persistent SupportClaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname supportClaw
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=supportClaw
    ```

    From now on, connect via Tailscale: `ssh ubuntu@supportclaw`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    supportClaw config set gateway.bind loopback
    supportClaw config set gateway.auth.mode token
    supportClaw doctor --generate-gateway-token
    supportClaw config set gateway.tailscale.mode serve
    supportClaw config set gateway.trustedProxies '["127.0.0.1"]'
  ...
  <Step title="Verify">
    ...
    ```
    https://supportClaw.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.supportClaw` to restrict credential file permissions.
- `supportClaw security audit` for an SupportClaw-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most SupportClaw features work fine; a small number of native binaries need ARM builds:
...
- `~/.supportClaw/` — `supportClaw.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.supportClaw/workspace/` — the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@supportclaw
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=supportClaw --reset` to re-authenticate.

**Gateway will not start** -- Run `supportClaw doctor --non-interactive` and check logs with `journalctl --user -u supportClaw-gateway.service -n 50`.
