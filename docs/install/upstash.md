---
summary: "Host SupportClaw on Upstash Box with keep-alive and SSH tunnel access"
read_when:
  - Deploying SupportClaw to Upstash Box
  - You want a managed Linux environment for SupportClaw with SSH-tunneled dashboard access
title: "Upstash Box"
---

Run a persistent SupportClaw Gateway on Upstash Box, a managed Linux environment
...
Upstash maintains its current SupportClaw Box walkthrough at
[SupportClaw Setup](https://upstash.com/docs/box/guides/supportClaw-setup).

## Connect with an SSH tunnel

Forward the SupportClaw dashboard port to your local machine. Use your Box API key
...
```bash
sudo npm install -g supportClaw
```
...
```bash
supportClaw config set gateway.bind lan
nohup supportClaw gateway > gateway.log 2>&1 &
```
...
- [Updating SupportClaw](/install/updating)
