---
summary: "Optional Docker-based setup and onboarding for SupportClaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run SupportClaw on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export SUPPORT_CLAW_IMAGE="ghcr.io/supportClaw/supportClaw:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/brahminib/support-claw/pkgs/container/supportClaw).
  ...
  <Step title="Airgapped rerun">
    ...
    ```bash
    docker load -i supportClaw-image.tar
    export SUPPORT_CLAW_IMAGE="ghcr.io/supportClaw/supportClaw:latest"
    ./scripts/docker/setup.sh --offline
    ```

    `--offline` verifies that `SUPPORT_CLAW_IMAGE` already exists locally, disables
    ...
    If `SUPPORT_CLAW_SANDBOX=1`, offline setup also checks the configured default
    and active per-agent sandbox images on the daemon behind
    `SUPPORT_CLAW_DOCKER_SOCKET`. Docker-backed browser images must also carry the
    current SupportClaw browser contract label. When a required image is missing or
  ...
  <Step title="Complete onboarding">
    ...
    `supportClaw-gateway` directly. `supportClaw-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm supportClaw-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm supportClaw-cli channels login

    # Telegram
    docker compose run --rm supportClaw-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm supportClaw-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t supportClaw:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node supportClaw-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node supportClaw-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d supportClaw-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `SUPPORT_CLAW_EXTRA_MOUNTS`
or `SUPPORT_CLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
...
<Note>
Because `supportClaw-cli` shares `supportClaw-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d supportClaw-gateway`, run onboarding
and setup-time config writes through `supportClaw-gateway` with
...
| Variable                                   | Purpose                                                               |
| ------------------------------------------ | --------------------------------------------------------------------- |
| `SUPPORT_CLAW_IMAGE`                           | Use a remote image instead of building locally                        |
| `SUPPORT_CLAW_IMAGE_APT_PACKAGES`              | Install extra apt packages during build (space-separated)             |
| `SUPPORT_CLAW_IMAGE_PIP_PACKAGES`              | Install extra Python packages during build (space-separated)          |
| `SUPPORT_CLAW_EXTENSIONS`                      | Pre-install plugin dependencies at build time (space-separated names) |
| `SUPPORT_CLAW_EXTRA_MOUNTS`                    | Extra host bind mounts (comma-separated `source:target[:opts]`)       |
| `SUPPORT_CLAW_HOME_VOLUME`                     | Persist `/home/node` in a named Docker volume                         |
| `SUPPORT_CLAW_SANDBOX`                         | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)                |
| `SUPPORT_CLAW_SKIP_ONBOARDING`                 | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`)       |
| `SUPPORT_CLAW_DOCKER_SOCKET`                   | Override Docker socket path                                           |
| `SUPPORT_CLAW_DISABLE_BONJOUR`                 | Disable Bonjour/mDNS advertising (defaults to `1` for Docker)         |
| `SUPPORT_CLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS` | Disable bundled plugin source bind-mount overlays                     |
...
| `SUPPORT_CLAW_OTEL_PRELOADED`                  | Skip starting a second OpenTelemetry SDK when one is preloaded        |

The official Docker image does not ship Homebrew. During onboarding, SupportClaw
...
`SUPPORT_CLAW_IMAGE_APT_PACKAGES` during image build. The legacy
`SUPPORT_CLAW_DOCKER_APT_PACKAGES` name is still accepted.
For Python dependencies, use `SUPPORT_CLAW_IMAGE_PIP_PACKAGES`. This runs
...
`SUPPORT_CLAW_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
...
```bash
export SUPPORT_CLAW_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="supportClaw-gateway"
./scripts/docker/setup.sh
```

Install the official `@supportclaw/diagnostics-otel` plugin from ClawHub in
packaged Docker installs before enabling export. Custom source-built images can
still include the local plugin source with
`SUPPORT_CLAW_EXTENSIONS=diagnostics-otel`. To enable export, allow and enable the
...
`clawhub:@supportclaw/diagnostics-prometheus`, enable the
...
```bash
docker compose exec supportClaw-gateway node dist/index.js health --token "$SUPPORT_CLAW_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `SUPPORT_CLAW_GATEWAY_BIND=lan` so host access to
...
When SupportClaw runs in Docker, `127.0.0.1` inside the container is the container
...
`SUPPORT_CLAW_DISABLE_BONJOUR=1` so the Gateway does not crash-loop or repeatedly
...
Set `SUPPORT_CLAW_DISABLE_BONJOUR=0` only when running with host networking, macvlan,
...
Docker Compose bind-mounts `SUPPORT_CLAW_CONFIG_DIR` to `/home/node/.supportClaw`,
`SUPPORT_CLAW_WORKSPACE_DIR` to `/home/node/.supportClaw/workspace`, and
`SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR` to `/home/node/.config/supportClaw`, so those
...
That mounted config directory is where SupportClaw keeps:

- `supportClaw.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `SUPPORT_CLAW_GATEWAY_TOKEN`
...
but separate from `SUPPORT_CLAW_CONFIG_DIR`.

Installed downloadable plugins store their package state under the mounted
SupportClaw home, so plugin install records and package roots survive container
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/supportClaw/supportClaw/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export SUPPORT_CLAW_SANDBOX=1
    export SUPPORT_CLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
    ...
    `workspace-write` while the SupportClaw sandbox is active; do not mount the
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm supportClaw-cli gateway probe
    docker compose run -T --rm supportClaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `supportClaw-cli` uses `network_mode: "service:supportClaw-gateway"` so CLI
    ...
    `no-new-privileges` on both `supportClaw-gateway` and `supportClaw-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in supportClaw-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network
    `supportClaw-cli` sidecar after `NET_RAW` is dropped, which shows up as
    `EAI_AGAIN` during npm-backed commands such as `supportClaw plugins install`.
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm supportClaw-cli plugins install <package>
    ```

    If you already created a long-running `supportClaw-cli` container, recreate it
  ...
  <Accordion title="Permissions and EACCES">
    ...
    `/home/node/.supportClaw`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/supportClaw-config /path/to/supportClaw-workspace
    ```
    ...
    `/path/to/supportClaw-config/npm` to `root:root` if you intentionally run
    SupportClaw as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export SUPPORT_CLAW_HOME_VOLUME="supportClaw_home"`
    2. **Bake system deps**: `export SUPPORT_CLAW_IMAGE_APT_PACKAGES="git curl jq"`
    3. **Bake Python deps**: `export SUPPORT_CLAW_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **Bake Playwright Chromium**: `export SUPPORT_CLAW_INSTALL_BROWSER=1`
    5. **Or install Playwright browsers into a persisted volume**:
       ```bash
       docker compose run --rm supportClaw-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    6. **Persist browser downloads**: use `SUPPORT_CLAW_HOME_VOLUME` or
       `SUPPORT_CLAW_EXTRA_MOUNTS`. SupportClaw auto-detects the Docker image's
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/brahminib/support-claw/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    SupportClaw runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm supportClaw-cli dashboard --no-open
    docker compose run --rm supportClaw-cli devices list
    docker compose run --rm supportClaw-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm supportClaw-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm supportClaw-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping SupportClaw up to date
