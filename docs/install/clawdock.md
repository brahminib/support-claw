---
summary: "ClawDock shell helpers for Docker-based SupportClaw installs"
read_when:
  - You run SupportClaw with Docker often and want shorter day-to-day commands
  - You want a helper layer for dashboard, logs, token setup, and pairing flows
title: "ClawDock"
---

ClawDock is a small shell-helper layer for Docker-based SupportClaw installs.
...
| `clawdock-cli <command>`  | Run SupportClaw CLI commands in Docker           |
...
| `clawdock-config`      | Open `~/.supportClaw`                      |
...
- `~/.supportClaw/.env` for env-backed provider keys and bot tokens
- `~/.supportClaw/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.supportClaw/supportClaw.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `supportClaw.json` quickly. It redacts `.env` values in its printed output.

## Related

<CardGroup cols={2}>
  <Card title="Docker" href="/install/docker" icon="docker">
    Canonical Docker install for SupportClaw.
  </Card>
  ...
  <Card title="Updating" href="/install/updating" icon="arrow-up-right-from-square">
    Updating the SupportClaw package and managed services.
  </Card>
</CardGroup>
