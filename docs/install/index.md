---
summary: "Install SupportClaw - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs SupportClaw, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://supportClaw.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want SupportClaw and Node kept under a local prefix such as
`~/.supportClaw`, without depending on a system-wide Node install:

```bash
curl -fsSL https://supportClaw.ai/install-cli.sh | bash
```
...
`supportClaw update --channel dev` and `supportClaw update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ```bash
    npm install -g supportClaw@latest
    supportClaw onboard --install-daemon
    ```

    <Note>
    The hosted installer clears npm freshness filters such as `min-release-age`
    for the SupportClaw package install. If you install manually with npm, your own
...
Or skip the link and use `pnpm supportClaw ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
supportClaw --version      # confirm the CLI is available
supportClaw doctor         # check for config issues
supportClaw gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `supportClaw onboard --install-daemon` or `supportClaw gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy SupportClaw on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove SupportClaw completely.
  </Card>
</CardGroup>

## Troubleshooting: `supportClaw` not found

If the install succeeded but `supportClaw` is not found in your terminal:
