---
summary: "Deploy SupportClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run SupportClaw on a Kubernetes cluster
  - You want to test SupportClaw in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running SupportClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

SupportClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/supportClaw 18789:18789 -n supportClaw
...
```bash
kubectl get secret supportClaw-secrets -n supportClaw -o jsonpath='{.data.SUPPORT_CLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: supportClaw (configurable via SUPPORT_CLAW_NAMESPACE)
├── Deployment/supportClaw        # Single pod, init container + gateway
├── Service/supportClaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/supportClaw-config  # supportClaw.json + AGENTS.md
└── Secret/supportClaw-secrets    # Gateway token + API keys
```
...
Edit `supportClaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret supportClaw-secrets -n supportClaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/supportClaw -n supportClaw
```

### Custom namespace

```bash
SUPPORT_CLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/supportClaw/supportClaw:latest # or pin to a specific version from https://github.com/brahminib/support-claw/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # supportClaw.json + AGENTS.md
