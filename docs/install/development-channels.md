...
SupportClaw ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/supportClaw`, or
  `$SUPPORT_CLAW_HOME/supportClaw` when `SUPPORT_CLAW_HOME` is set; override with
  `SUPPORT_CLAW_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
supportClaw update --tag 2026.4.1-beta.1
...
supportClaw update --tag supportClaw@2026.4.1-beta.1
...
- The tag is not persisted. Your next `supportClaw update` uses your configured
  channel as usual.
- For package installs, SupportClaw pre-packs GitHub/git source specs into a
...
- Downgrade protection: if the target version is older than your current version,
  SupportClaw prompts for confirmation (skip with `--yes`).
...
Preview what `supportClaw update` would do without making changes:

```bash
supportClaw update --dry-run
supportClaw update --channel beta --dry-run
supportClaw update --tag 2026.4.1-beta.1 --dry-run
supportClaw update --dry-run --json
```
...
When you switch channels with `supportClaw update`, SupportClaw also syncs plugin
sources:
