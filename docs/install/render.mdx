---
summary: "Deploy SupportClaw on Render with Infrastructure-as-Code"
read_when:
  - Deploying SupportClaw to Render
...
Deploy SupportClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/brahminib/support-claw)
...
repository configures everything needed to run SupportClaw:
...
services:
  - type: web
    ...
    envVars:
      - key: SUPPORT_CLAW_GATEWAY_PORT
        value: "8080"
      - key: SUPPORT_CLAW_STATE_DIR
        value: /data/.supportClaw
      - key: SUPPORT_CLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: SUPPORT_CLAW_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: supportClaw-data
      mountPath: /data
      sizeGB: 1
```
...
your fork's `render.yaml` (but note: no persistent disk means SupportClaw state
...
`SUPPORT_CLAW_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original SupportClaw repository, Render will not auto-deploy your SupportClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For SupportClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with SupportClaw state plus any configured
...
- Missing `SUPPORT_CLAW_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `SUPPORT_CLAW_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `supportClaw backup create` in the Render shell.
