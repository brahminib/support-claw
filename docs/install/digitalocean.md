---
summary: "Host SupportClaw on a DigitalOcean Droplet"
read_when:
  - Setting up SupportClaw on DigitalOcean
  - Looking for a simple paid VPS for SupportClaw
title: "DigitalOcean"
---

Run a persistent SupportClaw Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own SupportClaw state and services.
    adduser supportClaw
    usermod -aG sudo supportClaw
    loginctl enable-linger supportClaw

    su - supportClaw
    supportClaw --version
    ```

    Use the root shell only for system bootstrap. Run SupportClaw commands as the non-root `supportClaw` user so state lives under `/home/supportClaw/.supportClaw/` and the Gateway installs as that user's systemd service.
  ...
  <Step title="Access the Control UI">
    ...
    **Option C: Tailnet bind (no Serve)**
    ...
    ```bash
    supportClaw config set gateway.bind tailnet
    supportClaw gateway restart
    ```
...
DigitalOcean snapshots back the whole Droplet up; `supportClaw backup create` is portable across hosts.
