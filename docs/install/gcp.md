---
summary: "Run SupportClaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want SupportClaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent SupportClaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "SupportClaw 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the SupportClaw Gateway in Docker
- Persist `~/.supportClaw` + `~/.supportClaw/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.supportClaw` state includes `supportClaw.json`, per-agent
...
3. SSH into the VM
4. Install Docker
5. Clone SupportClaw repository
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-supportClaw-project --name="SupportClaw Gateway"
    gcloud config set project my-supportClaw-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create supportClaw-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh supportClaw-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the SupportClaw repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    SUPPORT_CLAW_IMAGE=supportClaw:latest
    SUPPORT_CLAW_GATEWAY_TOKEN=
    SUPPORT_CLAW_GATEWAY_BIND=lan
    SUPPORT_CLAW_GATEWAY_PORT=18789

    SUPPORT_CLAW_CONFIG_DIR=/home/$USER/.supportClaw
    SUPPORT_CLAW_WORKSPACE_DIR=/home/$USER/.supportClaw/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.supportClaw
    ```

    Set `SUPPORT_CLAW_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, SupportClaw uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `SUPPORT_CLAW_GATEWAY_TOKEN`.
    ...
    services:
      supportClaw-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - SUPPORT_CLAW_GATEWAY_BIND=${SUPPORT_CLAW_GATEWAY_BIND}
          - SUPPORT_CLAW_GATEWAY_PORT=${SUPPORT_CLAW_GATEWAY_PORT}
        ...
        ports:
          ...
          - "127.0.0.1:${SUPPORT_CLAW_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`SUPPORT_CLAW_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm supportClaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh supportClaw-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop supportClaw-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type supportClaw-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start supportClaw-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create supportClaw-deploy \
     --display-name="SupportClaw Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-supportClaw-project \
     --member="serviceAccount:supportClaw-deploy@my-supportClaw-project.iam.gserviceaccount.com" \
