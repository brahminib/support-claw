...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In SupportClaw it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to SupportClaw primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In SupportClaw it ships as a plugin that installs an OpenProse skill
...
<Steps>
  <Step title="Enable the plugin">
    ...
    ```bash
    supportClaw plugins enable open-prose
    ```
  ...
  <Step title="Verify">
    ```bash
    supportClaw plugins list | grep prose
    ```
...
For a local checkout: `supportClaw plugins install ./path/to/local/open-prose-plugin`
...
## SupportClaw runtime mapping

OpenProse programs map to SupportClaw primitives:

| OpenProse concept         | SupportClaw tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review them before running. Use SupportClaw tool
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    SupportClaw's native multi-agent coordination layer.
  </Card>
