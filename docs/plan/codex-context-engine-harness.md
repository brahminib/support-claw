...
summary: "Specification for making the bundled Codex app-server harness honor SupportClaw context-engine plugins"
read_when:
  ...
  - You are comparing embedded SupportClaw and Codex app-server context behavior
---
...
Make the bundled Codex app-server harness honor the same SupportClaw context-engine
lifecycle contract that embedded SupportClaw turns already honor.
...
SupportClaw-level compaction policy as far as the Codex app-server boundary allows.
...
as built-in SupportClaw attempts:

- `extensions/codex/src/app-server/run-attempt.ts`

That means the required hook point is in SupportClaw-controlled code. The external
boundary is the Codex app-server protocol itself: SupportClaw can control what it
...
Built-in SupportClaw attempts call the context-engine lifecycle directly:
...
For Codex harness turns, SupportClaw should preserve this lifecycle:

1. Read the mirrored SupportClaw session transcript.
...
8. Mirror the Codex result back into the SupportClaw transcript.
...
11. Preserve Codex native compaction signals and SupportClaw compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. SupportClaw should
not try to mutate the app-server's internal history except through supported
protocol calls.

SupportClaw's transcript mirror remains the source for SupportClaw features:
...
The context-engine interface returns SupportClaw `AgentMessage[]`, not a Codex
...
- `runtime: "supportClaw"` selects the built-in SupportClaw harness
- `runtime: "codex"` selects the registered Codex harness
- `runtime: "auto"` lets plugin harnesses claim supported providers
- unmatched `auto` runs use the built-in SupportClaw harness
...
- Label it clearly as SupportClaw assembled context.
...
inside SupportClaw and preserves context-engine semantics.
...
we need strict SupportClaw parity, run context-engine assembly before hook
...
1. SupportClaw context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit SupportClaw compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
SupportClaw compaction should prefer the context engine's `compact()` result for
the SupportClaw transcript mirror and plugin state.
...
the SupportClaw session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
SupportClaw session lifecycle paths. Do not add Codex-specific cleanup unless the
context-engine lifecycle currently misses reset/delete events for all harnesses.

### 10. Error handling

Follow built-in SupportClaw semantics:
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole SupportClaw compaction when the context engine is primary.
...
Avoid requiring lossless-claw in SupportClaw core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for SupportClaw/plugin state.
