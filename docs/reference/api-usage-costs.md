...
SupportClaw features that can generate provider usage or paid API calls.
...
- If SupportClaw has usage metadata and local pricing for the active model,
...
Anthropic note: Anthropic staff told us SupportClaw-style Claude CLI usage is
...
Anthropic still does not expose a per-message dollar estimate that SupportClaw can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `supportClaw status --usage` and `supportClaw channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so SupportClaw inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, SupportClaw prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise SupportClaw falls back to matching OAuth/API-key
...
SupportClaw can pick up credentials from:
...
SupportClaw's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's SupportClaw Claude-login path with **Extra Usage** enabled.
...
- `supportClaw models status --json`
...
`supportClaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
