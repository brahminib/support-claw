...
beta publish, pass `release_package_spec=supportClaw@YYYY.M.PATCH-beta.N` to reuse the
...
`release_package_spec` values derive `codex_plugin_spec=npm:@supportclaw/codex@<version>`;
...
`SupportClaw Release Checks` is the largest child workflow. It resolves the target
...
| `release-checks`    | All SupportClaw Release Checks stages.                                                             |
...
QA release-check failures block normal release validation. Required SupportClaw
...
- `release-package-under-test` from `SupportClaw Release Checks`
