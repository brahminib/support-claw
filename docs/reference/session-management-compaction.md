...
SupportClaw manages sessions end-to-end across these areas:
...
SupportClaw is designed around a single **Gateway process** that owns session state.
...
SupportClaw persists sessions in two layers:
...
- Transcripts: `~/.supportClaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

SupportClaw resolves these via `src/config/sessions.ts`.
...
SupportClaw no longer creates automatic `sessions.json.bak.*` rotation backups during Gateway writes. The legacy `session.maintenance.rotateBytes` key is ignored and `supportClaw doctor --fix` removes it from older configs.
...
`SUPPORT_CLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS`, `SUPPORT_CLAW_SESSION_WRITE_LOCK_STALE_MS`, and
`SUPPORT_CLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS`.
...
In `mode: "warn"`, SupportClaw reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
supportClaw sessions cleanup --dry-run
supportClaw sessions cleanup --enforce
```
...
- **Parent fork policy** uses SupportClaw's active branch when creating a thread or subagent fork. If that branch is too large, SupportClaw starts the child with isolated context instead of failing or inheriting unusable history. The sizing policy is automatic; legacy `session.parentForkMaxTokens` config is removed by `supportClaw doctor --fix`.
...
Transcripts are managed by `supportClaw/plugin-sdk/agent-sessions`'s `SessionManager`.
...
SupportClaw intentionally does **not** "fix up" transcripts; the Gateway uses `SessionManager` to read/write them.
...
SupportClaw does not append AGENTS.md excerpts on top of the compaction summary.

Compaction is **persistent** (unlike session pruning). See [/concepts/session-pruning](/concepts/session-pruning).

## Compaction chunk boundaries and tool pairing

When SupportClaw splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, SupportClaw
...
- If a trailing tool-result block would otherwise push the chunk over target,
  SupportClaw preserves that pending tool block and keeps the unsummarized tail
  intact.
- Aborted/error tool-call blocks do not hold a pending split open.

---

## When auto-compaction happens (SupportClaw runtime)

In the embedded SupportClaw agent, auto-compaction triggers in two cases:
...
exceeded`, and similar provider-shaped variants) → compact → retry.
   When the provider reports the attempted token count, SupportClaw forwards that
   observed count into overflow recovery compaction. If the provider confirms
   overflow but does not expose a parseable count, SupportClaw passes a minimally
   over-budget synthetic count to compaction engines and diagnostics.
   If overflow recovery still fails, SupportClaw surfaces explicit guidance to the
...
These are SupportClaw runtime semantics.

SupportClaw can also trigger a preflight local compaction before opening the next
...
reopen cost, not raw archival: SupportClaw still runs normal semantic compaction,
and it requires `truncateAfterCompaction` so the compacted summary can become a
new successor transcript.

For embedded SupportClaw runs, `agents.defaults.compaction.midTurnPrecheck.enabled: true`
adds an opt-in tool-loop guard. After a tool result is appended and before the
next model call, SupportClaw estimates the prompt pressure using the same preflight
budget logic used at turn start. If the context no longer fits, the guard does
not compact inside SupportClaw runtime's `transformContext` hook. It raises a structured
...
before a turn opens, while mid-turn precheck runs later in the embedded SupportClaw tool
...
SupportClaw runtime's compaction settings live in agent settings:
...
SupportClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, SupportClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it's already higher, SupportClaw leaves it alone.
- Manual `/compact` honors an explicit `agents.defaults.compaction.keepRecentTokens`
  and keeps SupportClaw runtime's recent-tail cut point. Without an explicit keep budget,
...
- When `agents.defaults.compaction.truncateAfterCompaction` is enabled,
  SupportClaw rotates the active transcript to a compacted successor JSONL after
...
- If the provider fails or returns an empty result, SupportClaw falls back to built-in LLM summarization automatically.
...
- `supportClaw status` (CLI)
- `supportClaw sessions` / `sessions --json`
- Gateway logs (`pnpm gateway:watch` or `supportClaw logs --follow`): `embedded run auto-compaction start` + `complete`
...
SupportClaw supports "silent" turns for background tasks where the user should not see intermediate output.
...
- SupportClaw strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, SupportClaw also suppresses **draft/typing streaming** when a
...
SupportClaw uses the **pre-threshold flush** approach:

1. Monitor session context usage.
2. When it crosses a "soft threshold" (below SupportClaw runtime's compaction threshold), run a silent
...
- The flush runs only for embedded SupportClaw sessions (CLI backends skip it).
- The flush is skipped when the session workspace is read-only (`workspaceAccess: "ro"` or `"none"`).
- See [Memory](/concepts/memory) for the workspace file layout and write patterns.

SupportClaw also exposes a `session_before_compact` hook in the extension API, but SupportClaw's
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `supportClaw status`.
