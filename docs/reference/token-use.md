---
summary: "How SupportClaw builds prompt context and reports token usage + costs"
read_when:
...
SupportClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

SupportClaw assembles its own system prompt on every run. It includes:
...
`toolResultMaxChars` is an advanced ceiling (up to `1000000` characters). When it is unset, SupportClaw chooses
...
For images, SupportClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage off|tokens|full` → appends a **per-response usage footer** to every reply.
  ...
  - `/usage full` shows estimated cost only when SupportClaw has usage metadata and
    local pricing for the active model. Otherwise it shows tokens only.
- `/usage cost` → shows a local cost summary from SupportClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `supportClaw status --usage` and `supportClaw channels list` show
...
available; otherwise SupportClaw falls back to matching OAuth/API-key credentials
...
SupportClaw keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, SupportClaw shows tokens only. Cost display is
...
After sidecars and channels reach the Gateway ready path, SupportClaw starts an
...
Provider prompt caching only applies within the cache TTL window. SupportClaw can
...
SupportClaw sizes GA-capable Claude 4.x models such as Opus 4.8, Opus 4.7, Opus 4.6, and
...
Older configs can keep `context1m: true`, but SupportClaw no longer sends
...
SupportClaw preserves the OAuth-required Anthropic beta headers while stripping the
