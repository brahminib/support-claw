---
summary: "SupportClaw code mode: an opt-in exec/wait tool surface backed by QuickJS-WASI and a hidden run-scoped tool catalog"
title: "Code mode"
sidebarTitle: "Code mode"
read_when:
  - You want to enable SupportClaw code mode for an agent run
...
Code mode is an experimental SupportClaw agent-runtime feature. It is off by
default. When you enable it, SupportClaw changes what the model sees for one run:
instead of exposing every enabled tool schema directly, the model sees only
`exec` and `wait`.

This page documents SupportClaw code mode. It is not Codex Code mode. The two
...
- SupportClaw code mode is disabled unless `tools.codeMode.enabled: true` is
  configured. It runs in the SupportClaw generic agent runtime, where the model
...
surfaces. SupportClaw code mode is an SupportClaw-owned experimental tool-surface
adapter for generic SupportClaw runs. It uses `quickjs-wasi`, a hidden SupportClaw
tool catalog, and the normal SupportClaw tool executor.

## What is this?

SupportClaw code mode lets the model write a small JavaScript or TypeScript program
...
- `exec` evaluates model-generated JavaScript or TypeScript in a constrained
  QuickJS-WASI worker.
- Normal SupportClaw tools are hidden from the model prompt and exposed inside the
  guest program through `ALL_TOOLS` and `tools`.
- Guest code can search the hidden catalog, describe a tool, and call a tool
  through the same SupportClaw execution path used by normal agent turns.
...
surface. It does not replace SupportClaw tools, plugin tools, MCP tools, auth,
...
- Provider neutral: it works for SupportClaw, plugin, MCP, and client tools without
  depending on provider-native code execution.
- Existing policy stays in force: nested tool calls still go through SupportClaw
...
- Clear failure mode: when code mode is explicitly enabled and the runtime is
  unavailable, SupportClaw fails closed instead of falling back to broad direct tool
  exposure.
...
```bash
SUPPORT_CLAW_DEBUG_CODE_MODE=1 \
SUPPORT_CLAW_DEBUG_MODEL_TRANSPORT=1 \
SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=tools \
supportClaw gateway
```
...
`SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
- Stability: experimental SupportClaw surface; Codex Code mode is a separate stable
  Codex harness surface.
- Target surface: generic SupportClaw agent runs.
...
**Code mode** is the SupportClaw runtime mode that hides normal model tools and
...
If code mode is enabled but QuickJS-WASI cannot load, SupportClaw fails closed for
...
2. Build the effective SupportClaw tool list.
...
- If `language` is `"typescript"`, SupportClaw transpiles before evaluation.
...
final value is JSON-compatible after SupportClaw's output adapter runs.
...
`wait` exists because nested SupportClaw tools can be slow, interactive, approval
...
2. On suspension, SupportClaw snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot.
4. SupportClaw re-registers host callbacks by stable names.
5. SupportClaw delivers nested tool results into the restored VM.
6. SupportClaw drains QuickJS pending jobs.
...
type ToolCatalogEntry = {
  ...
  label?: string;
  description: string;
  source: "supportClaw" | "plugin" | "mcp" | "client";
  sourceName?: string;
};
```
...
const hits = await tools.web_search({ query: "SupportClaw code mode" });
```
...
state directory. For each code-mode `exec` call, SupportClaw builds the run-scoped
...
inside the QuickJS program while SupportClaw still shows only `exec` and `wait` to
the model.
...
7. SupportClaw auto-drains ready namespace bridge calls inside the active
...
- `globalName` collisions with built-in code-mode globals such as `tools`,
  `namespaces`, `text`, `json`, `yield_control`, or `__supportClaw*`
...
catalog for arbitrary enabled SupportClaw, plugin, and client tools; use `MCP` for
...
- images and files travel through ordinary SupportClaw tools, not through the
  code-mode bridge
...
Code mode supersedes the SupportClaw Tool Search model surface for runs where it is
active.

When `tools.codeMode.enabled` is true and code mode activates:

- SupportClaw does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same SupportClaw executor path that Tool Search
  uses.

The existing [Tool Search](/tools/tool-search) page describes the SupportClaw compact
catalog bridge. Code mode is the generic SupportClaw alternative for runs that can
use `exec` and `wait`.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal SupportClaw
shell `exec` tool is enabled, it is hidden from the model and cataloged like any
other tool.

Inside the guest runtime:

- `tools.call("supportClaw:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, SupportClaw omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters SupportClaw.
...
SupportClaw loads `quickjs-wasi` as a direct dependency in the owning package. The
...
The runtime executes outside SupportClaw's main event loop in a worker. A guest
...
inputs beyond existing SupportClaw trajectory policy.
...
SUPPORT_CLAW_DEBUG_MODEL_TRANSPORT=1 \
SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=tools \
SUPPORT_CLAW_DEBUG_SSE=events \
supportClaw gateway
```

For payload-shape debugging, use `SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `SUPPORT_CLAW_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, and `tools.call` work for SupportClaw tools
...
5. Send an agent turn with SupportClaw, plugin, MCP, and client test tools.
...
8. In `exec`, call SupportClaw/plugin/client tools through `tools.search`,
