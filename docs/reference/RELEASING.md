...
SupportClaw has three public release lanes:
...
- Every stable SupportClaw release ships the npm package, macOS app, and signed
...
6. Run `SupportClaw NPM Release` with `preflight_only=true`. Before a tag exists,
...
9. For a tagged beta candidate, run
   ...
   `SupportClaw Release Publish` command only after the evidence bundle is green.
   `SupportClaw Release Publish` dispatches the selected or all-publishable plugin
   packages to npm and the same set to ClawHub in parallel, and then promotes the
   prepared SupportClaw npm preflight artifact with the matching dist-tag as soon as
   plugin npm publish succeeds.
   After the SupportClaw npm publish child succeeds, it creates or updates the
   ...
   evidence as soon as SupportClaw npm publish succeeds, waits for ClawHub whenever
   SupportClaw npm is being published, then runs `pnpm release:verify-beta` and
   ...
   `supportClaw@YYYY.M.PATCH-beta.N` or
   `supportClaw@beta` package. If a pushed or published prerelease needs a fix,
...
10. For stable, continue only after the vetted beta or release candidate has the
    ...
    `SupportClaw Release Publish`, reusing the successful preflight artifact via
    ...
    readiness requires the signed `SupportClawCompanion-Setup-x64.exe`,
    `SupportClawCompanion-Setup-arm64.exe`, and
    `SupportClawCompanion-SHA256SUMS.txt` assets on the SupportClaw GitHub release.
    Pass the exact signed `supportClaw/supportClaw-windows-node` release tag as
    `windows_node_tag` and its candidate-approved installer digest map as
    `windows_node_installer_digests`; `SupportClaw Release Publish` keeps the
...
5. Run `pnpm release:generated:check`, `pnpm deps:shrinkwrap:check`, and
   `SUPPORT_CLAW_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main`
...
6. Keep the repository variables `RELEASE_ROLLBACK_DRILL_ID` and
   ...
   `SupportClaw Stable Main Closeout` starts from the `main` push that carries the
...
- Plugin version sync updates official plugin package versions and existing
  `supportClaw.compat.pluginApi` floors to the SupportClaw release version by
  ...
  compatible with older SupportClaw hosts, keep the floor at the oldest supported
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `SupportClaw Release Checks` for install smoke, package acceptance, cross-OS
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `supportClaw@beta`, `supportClaw@latest`, or an exact release version; `source=ref`
  ...
  Example: `gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=supportClaw@beta -f suite_profile=product -f published_upgrade_survivor_baseline=supportClaw@2026.4.26 -f telegram_mode=mock-openai`
  Common profiles:
...
- `SupportClaw NPM Release` preflight generates dependency release evidence before
...
- The preflight uploads dependency evidence as
  `supportClaw-release-dependency-evidence-<tag>` and also embeds it under
  ...
  to the GitHub release as `supportClaw-<version>-dependency-evidence.zip`.
- Run `SupportClaw Release Publish` for the mutating publish sequence after the
  tag exists. Dispatch it from `release/YYYY.M.PATCH` (or `main` when publishing a
  main-reachable tag), pass the release tag, successful SupportClaw npm
  ...
  ClawHub publish, and SupportClaw npm publish so the core package is not published
  before its externalized plugins.
- Stable `SupportClaw Release Publish` requires an exact `windows_node_tag` after
  the matching non-prerelease `supportClaw/supportClaw-windows-node` release exists.
  ...
  while the SupportClaw release is still a draft, carrying the pinned installer
  ...
  signatures use the expected SupportClaw Foundation signer on a Windows runner,
  writes a SHA-256 manifest, and uploads the installers plus manifest onto the
  canonical SupportClaw GitHub release, then re-downloads the promoted assets and
  ...
  rejects unexpected `SupportClawCompanion-*` asset names before replacing the
  ...
  approved source release. Website download links should target exact SupportClaw
...
- Release checks now run in a separate manual workflow:
  `SupportClaw Release Checks`
- `SupportClaw Release Checks` also runs the QA Lab mock parity lane plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `SupportClaw Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/supportClaw-cross-os-release-checks-reusable.yml` directly
...
- `SupportClaw Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an SupportClaw branch or release tag
- `SupportClaw NPM Release` validation-only preflight also accepts the current
...
- That workflow runs
  `SUPPORT_CLAW_LIVE_TEST=1 SUPPORT_CLAW_LIVE_CACHE_TEST=1 pnpm test:live:cache`
...
- Before tagging a release candidate locally, run
  ...
  UI build, and `release:supportClaw:npm:check` in the order that catches common
  approval-blocking mistakes before the GitHub publish workflow starts.
- Run `RELEASE_TAG=vYYYY.M.PATCH node --import tsx scripts/supportClaw-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/supportClaw-npm-postpublish-verify.ts YYYY.M.PATCH`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `SUPPORT_CLAW_NPM_TELEGRAM_PACKAGE_SPEC=supportClaw@YYYY.M.PATCH-beta.N SUPPORT_CLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex SUPPORT_CLAW_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `SUPPORT_CLAW_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - token-based npm dist-tag mutation now lives in
    `supportClaw/releases/.github/workflows/supportClaw-npm-dist-tags.yml` because
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=supportClaw@YYYY.M.PATCH-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, then dispatches `SupportClaw Release Checks`.
`SupportClaw Release Checks` fans out install smoke, cross-OS release checks,
...
`SupportClaw Release Checks` uses the trusted workflow ref to resolve the target
...
After a beta is already on npm, set `release_package_spec=supportClaw@YYYY.M.PATCH-beta.N`
...
The cross-OS OpenAI install smoke uses `SUPPORT_CLAW_CROSS_OS_OPENAI_MODEL` when the
...
gh workflow run full-release-validation.yml \
  ...
  -f release_package_spec=supportClaw@YYYY.M.PATCH-beta.N \
  -f evidence_package_spec=supportClaw@YYYY.M.PATCH-beta.N \
...
validation, including required SupportClaw dynamic tool drift in the standard tier.
...
`SUPPORT_CLAW_RELEASE_QA_*_LIVE_CI_ENABLED` repo variable must be enabled; otherwise
...
The Docker box lives in `SupportClaw Release Checks` through
`supportClaw-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `SupportClaw Release Checks`. It is the agentic
...
`scripts/resolve-supportClaw-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `supportClaw@beta`, `supportClaw@latest`, or an exact SupportClaw release
  version
...
`SupportClaw Release Checks` runs Package Acceptance with `source=artifact`, the
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=supportClaw@2026.4.26
```
...
`SupportClaw Release Publish` is the normal mutating publish entrypoint. It
...
6. Dispatch `SupportClaw NPM Release` with the release tag, npm dist-tag, and
...
gh workflow run supportClaw-release-publish.yml \
  ...
  -f windows_node_installer_digests='{"SupportClawCompanion-Setup-x64.exe":"sha256:<approved-x64-sha256>","SupportClawCompanion-Setup-arm64.exe":"sha256:<approved-arm64-sha256>"}' \
  -f preflight_run_id=<successful-supportClaw-npm-preflight-run-id> \
  -f full_release_validation_run_id=<successful-full-release-validation-run-id> \
  -f npm_dist_tag=beta
```
...
only for focused repair or republish work. `SupportClaw Release Publish` rejects
`plugin_publish_scope=selected` when `publish_supportClaw_npm=true` so the core
package cannot ship without every publishable official plugin, including
`@supportclaw/diffs-language-pack`. For a selected plugin repair, set
`publish_supportClaw_npm=false` with `plugin_publish_scope=selected` and
`plugins=@supportclaw/name`, or dispatch the child workflow directly.

## NPM workflow inputs

`SupportClaw NPM Release` accepts these operator-controlled inputs:
...
`SupportClaw Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `SupportClaw NPM Release` preflight run id;
  required when `publish_supportClaw_npm=true`
- `full_release_validation_run_id`: successful `Full Release Validation` run
  id; required when `publish_supportClaw_npm=true`
- `windows_node_tag`: exact non-prerelease `supportClaw/supportClaw-windows-node`
  release tag; required for stable SupportClaw publish
- `windows_node_installer_digests`: candidate-approved compact JSON map of the
  ...
  for stable SupportClaw publish
- `npm_dist_tag`: npm target tag for the SupportClaw package
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only
  for focused plugin-only repair work with `publish_supportClaw_npm=false`
- `plugins`: comma-separated `@supportclaw/*` package names when
  `plugin_publish_scope=selected`
- `publish_supportClaw_npm`: defaults to `true`; set `false` only when using the
...
`SupportClaw Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an SupportClaw branch or
  release tag.
...
- For `SupportClaw NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `SupportClaw Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `SupportClaw NPM Release` with `preflight_only=true`
...
5. Select the exact non-prerelease `supportClaw/supportClaw-windows-node` release tag
...
7. Run `SupportClaw Release Publish` with the same `tag`, the same `npm_dist_tag`,
   ...
   SupportClaw npm package
8. If the release landed on `beta`, use the
   `supportClaw/releases/.github/workflows/supportClaw-npm-dist-tags.yml`
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/brahminib/support-claw/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/brahminib/support-claw/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/supportClaw-npm-release.yml`](https://github.com/brahminib/support-claw/blob/main/.github/workflows/supportClaw-npm-release.yml)
- [`.github/workflows/supportClaw-release-checks.yml`](https://github.com/brahminib/support-claw/blob/main/.github/workflows/supportClaw-release-checks.yml)
- [`.github/workflows/supportClaw-cross-os-release-checks-reusable.yml`](https://github.com/brahminib/support-claw/blob/main/.github/workflows/supportClaw-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-supportClaw-package-candidate.mjs`](https://github.com/brahminib/support-claw/blob/main/scripts/resolve-supportClaw-package-candidate.mjs)
- [`scripts/supportClaw-npm-release-check.ts`](https://github.com/brahminib/support-claw/blob/main/scripts/supportClaw-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/brahminib/support-claw/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/brahminib/support-claw/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`supportClaw/maintainers/release/README.md`](https://github.com/supportClaw/maintainers/blob/main/release/README.md)
