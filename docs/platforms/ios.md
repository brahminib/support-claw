...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `supportClaw.internal.`), **or**
...
Run `supportClaw devices list` again before approval.
...
supportClaw gateway call node.list --params "{}"
```
...
By default, official/TestFlight builds and gateways use the hosted relay at `https://ios-push-relay.supportClaw.ai`.
...
- `SUPPORT_CLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
- `SUPPORT_CLAW_PUSH_RELAY_BASE_URL` still works as a temporary env override for official/TestFlight iOS builds.
...
- Only genuine SupportClaw iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight SupportClaw builds.
...
```bash
export SUPPORT_CLAW_APNS_TEAM_ID="TEAMID"
export SUPPORT_CLAW_APNS_KEY_ID="KEYID"
export SUPPORT_CLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.supportClaw/credentials/apns
chmod 700 ~/.supportClaw/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.supportClaw/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.supportClaw/credentials/apns/AuthKey_KEYID.p8
export SUPPORT_CLAW_APNS_PRIVATE_KEY_PATH="$HOME/.supportClaw/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_supportClaw-gw._tcp` on `local.` and, when configured, the same
...
`supportClaw.internal.`) and Tailscale split DNS.
...
```bash
supportClaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__supportClaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__supportClaw__/canvas/` and `/__supportClaw__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through SupportClaw node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through SupportClaw by invoking node
...
```bash
supportClaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__supportClaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
supportClaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `supportClaw devices list` and approve manually.
