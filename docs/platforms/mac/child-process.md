...
service via the external `supportClaw` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.supportClaw.gateway`
  (or `ai.supportClaw.<profile>` when using `--profile`/`SUPPORT_CLAW_PROFILE`; legacy `com.supportClaw.*` is supported).
