...
- Same path: run the app from a fixed location (for SupportClaw, `dist/SupportClaw.app`).
...
Prefer granting Accessibility to SupportClaw.app, Peekaboo.app, or another signed
...
```bash
sudo tccutil reset Accessibility ai.supportClaw.mac
sudo tccutil reset ScreenCapture ai.supportClaw.mac
...
Workaround: move files into the SupportClaw workspace (`~/.supportClaw/workspace`) if you want to avoid per-folder grants.
