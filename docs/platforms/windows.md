...
read_when:
  - Installing SupportClaw on Windows
  - Choosing between Windows Hub, native Windows, and WSL2
  - Setting up the Windows companion app or Windows node mode
title: "Windows"
---

SupportClaw ships a native **Windows Hub** companion app plus Windows CLI support.
...
x64 and ARM64 installers on SupportClaw releases.

Download the latest stable installer from the [SupportClaw releases page](https://github.com/brahminib/support-claw/releases):

- [SupportClawCompanion-Setup-x64.exe](https://github.com/brahminib/support-claw/releases/download/v2026.6.5/SupportClawCompanion-Setup-x64.exe)
- [SupportClawCompanion-Setup-arm64.exe](https://github.com/brahminib/support-claw/releases/download/v2026.6.5/SupportClawCompanion-Setup-arm64.exe)
- [Checksums](https://github.com/brahminib/support-claw/releases/download/v2026.6.5/SupportClawCompanion-SHA256SUMS.txt)

If a download link above returns a 404, visit the [releases page](https://github.com/brahminib/support-claw/releases) and look for the `SupportClawCompanion-Setup-*` assets on the latest release.

After install, launch **SupportClaw Companion** from the Start menu or the system
...
`SupportClawGateway` WSL distro, installs the Gateway inside it, and pairs the app.
...
Windows Hub can register as a first-class SupportClaw node. The agent can then use
...
supportClaw devices approve <request-id>
...
Windows capabilities without a running SupportClaw Gateway.
...
For terminal-first use, install SupportClaw from PowerShell:
...
```powershell
supportClaw onboard --non-interactive --skip-health
...
Then install SupportClaw inside WSL with the Linux quickstart:
...
```bash
systemctl --user is-enabled supportClaw-gateway.service
systemctl --user status supportClaw-gateway.service --no-pager
```
...
Check Task Manager for `SupportClaw.Tray.WinUI.exe`. If it is running, open the
hidden tray-icons area and pin it. If it is not running, launch **SupportClaw
...
```powershell
notepad "$env:LOCALAPPDATA\SupportClawTray\Logs\Setup\easy-setup-latest.txt"
```
