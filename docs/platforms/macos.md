---
summary: "SupportClaw macOS companion app (menu bar + gateway broker)"
read_when:
...
The macOS app is the **menu-bar companion** for SupportClaw. It owns permissions,
...
- Installs the global CLI (`supportClaw`) on request via npm, pnpm, or bun (the app prefers npm, then pnpm, then bun; Node remains the recommended Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `supportClaw gateway install`.
...
The app manages a per-user LaunchAgent labeled `ai.supportClaw.gateway`
(or `ai.supportClaw.<profile>` when using `--profile`/`SUPPORT_CLAW_PROFILE`; legacy `com.supportClaw.*` still unloads).
...
Replace the label with `ai.supportClaw.<profile>` when running a named profile.
...
The app registers the `supportClaw://` URL scheme for local actions.
...
```bash
open 'supportClaw://agent?message=Hello%20from%20deep%20link'
```
...
1. Install and launch **SupportClaw.app**.
...
Avoid putting your SupportClaw state dir in iCloud or other cloud-synced folders.
...
If `supportClaw doctor` detects state under:
...
- `swift run SupportClaw` (or Xcode)
...
```bash
cd apps/macos
swift run supportClaw-mac connect --json
swift run supportClaw-mac discover --timeout 3000 --json
```
...
<Tip>
Compare against `supportClaw gateway discover --json` to see whether the macOS app's discovery pipeline (`local.` plus the configured wide-area domain, with wide-area and Tailscale Serve fallbacks) differs from the Node CLI's `dns-sd` based discovery.
</Tip>
