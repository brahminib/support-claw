...
<Note>
The official Android app is available on [Google Play](https://play.google.com/store/apps/details?id=ai.supportClaw.app&hl=en_IN). It is a companion node and requires a running SupportClaw Gateway. The source code is also available in the [SupportClaw repository](https://github.com/brahminib/support-claw) under `apps/android`; see [apps/android/README.md](https://github.com/brahminib/support-claw/blob/main/apps/android/README.md) for build instructions.
</Note>
...
- Install: [Google Play](https://play.google.com/store/apps/details?id=ai.supportClaw.app&hl=en_IN) for the app, [Getting Started](/start/getting-started) for the Gateway, then [Pairing](/channels/pairing).
...
- You can run the CLI (`supportClaw`) on the gateway machine (or via SSH).
...
```bash
supportClaw gateway discover --json
```
...
1. Set up a DNS-SD zone (example `supportClaw.internal.`) on the gateway host and publish `_supportClaw-gw._tcp` records.
...
1. Create `~/.supportClaw/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
supportClaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__supportClaw__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__supportClaw__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The Gateway also serves `/__supportClaw__/a2ui/`, but the Android app treats remote A2UI pages as render-only. Action-capable A2UI commands use the bundled app-owned A2UI page before applying messages.
...
Android supports launching SupportClaw from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
SupportClaw..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set SupportClaw as the default assistant app.
</Note>
