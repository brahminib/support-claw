...
`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"supportClaw":{"emoji":"disk","events":["command:new"]}}');

    const parsed = JSON5.parse(result.metadata);
    expect(parsed.supportClaw?.emoji).toBe("disk");
  });
...
---`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"supportClaw":true}');
  });
