...
/** Resolve the underlying OS home before applying SupportClaw overrides. */
...
/** Resolve raw home with SUPPORT_CLAW_HOME tilde expansion. */
function resolveRawHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string | undefined {
  const explicitHome = normalize(env.SUPPORT_CLAW_HOME);
  if (explicitHome) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  return explicitHome ? { home, prefix: "$SUPPORT_CLAW_HOME" } : { home, prefix: "~" };
}

/** Replace the effective home path with "~" or "$SUPPORT_CLAW_HOME" for terminal display. */
