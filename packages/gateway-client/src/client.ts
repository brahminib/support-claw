...
import type {
  ...
  RequestFrame,
  ResponseFrame,
} from "@supportclaw/gateway-protocol";
import {
  GATEWAY_CLIENT_MODES,
  GATEWAY_CLIENT_NAMES,
  type GatewayClientMode,
  type GatewayClientName,
} from "@supportclaw/gateway-protocol/client-info";
import {
...
} from "@supportclaw/gateway-protocol/connect-error-details";
import { resolveGatewayStartupRetryAfterMs } from "@supportclaw/gateway-protocol/startup-unavailable";
import { MIN_CLIENT_PROTOCOL_VERSION, PROTOCOL_VERSION } from "@supportclaw/gateway-protocol/version";
...
// The package stays reusable by depending on host callbacks for SupportClaw-owned
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    this.deps = {
      // Defaults keep the package inert outside SupportClaw; device signing throws
  ...
  start() {
    ...
    const allowPrivateWs =
      (this.opts.env ?? process.env).SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
    ...
    if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          (allowPrivateWs
            ? ""
            : "Break-glass (trusted private networks only): set SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS=1. ") +
          "Run `supportClaw doctor --fix` for guidance.",
      );
