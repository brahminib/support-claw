...
/** Read SupportClaw package.json blocks without trusting caller input shape. */
function readSupportClawBlock(packageJson: unknown) {
  const root = isRecord(packageJson) ? packageJson : undefined;
  const supportClaw = isRecord(root?.supportClaw) ? root.supportClaw : undefined;
  const compat = isRecord(supportClaw?.compat) ? supportClaw.compat : undefined;
  const build = isRecord(supportClaw?.build) ? supportClaw.build : undefined;
  const install = isRecord(supportClaw?.install) ? supportClaw.install : undefined;
  return { root, supportClaw, compat, build, install };
}
...
export function normalizeExternalPluginCompatibility(
  ...
  const { root, compat, build, install } = readSupportClawBlock(packageJson);
  ...
  const builtWithSupportClawVersion = normalizeOptionalString(build?.supportClawVersion) ?? version;
  if (builtWithSupportClawVersion) {
    compatibility.builtWithSupportClawVersion = builtWithSupportClawVersion;
  }
...
export function listMissingExternalCodePluginFieldPaths(packageJson: unknown): string[] {
  const { compat, build } = readSupportClawBlock(packageJson);
  const missing: string[] = [];
  if (!normalizeOptionalString(compat?.pluginApi)) {
    missing.push("supportClaw.compat.pluginApi");
  }
  if (!normalizeOptionalString(build?.supportClawVersion)) {
    missing.push("supportClaw.build.supportClawVersion");
  }
  return missing;
}
