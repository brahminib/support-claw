...
export type MemorySessionSyncTarget = {
  /** Owning SupportClaw agent. Omit only when the active manager scope already supplies it. */
  agentId?: string;
...
export type MemorySyncParams = {
   ...
   * During the deprecation window only canonical SupportClaw transcript paths are accepted.
   */
  sessionFiles?: string[];
