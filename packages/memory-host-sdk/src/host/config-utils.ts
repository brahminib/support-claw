...
export { splitShellArgs } from "./supportClaw-runtime-io.js";

// Shared SupportClaw config helpers used by memory host, QMD, and agent context code.
...
/** Narrow SupportClaw config shape consumed by memory host utilities. */
...
/** Resolve the underlying OS home before applying SupportClaw-specific overrides. */
...
/** Resolve SUPPORT_CLAW_HOME or the OS home, falling back to cwd for hermetic tests. */
function resolveRequiredHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string {
  const explicitHome = normalizeHomeValue(env.SUPPORT_CLAW_HOME);
...
/** Resolve absolute user paths, including "~" against the effective SupportClaw home. */
...
export function resolveStateDir(
  ...
  if (env.SUPPORT_CLAW_TEST_FAST === "1" || fs.existsSync(nextDir)) {
    return nextDir;
  }
  // Existing legacy state remains authoritative until an explicit migration creates .supportClaw.
...
/** Resolve the default agent workspace, partitioned by SUPPORT_CLAW_PROFILE when set. */
function resolveDefaultAgentWorkspaceDir(env: NodeJS.ProcessEnv = process.env): string {
  const home = resolveRequiredHomeDir(env, os.homedir);
  const profile = env.SUPPORT_CLAW_PROFILE?.trim();
  if (profile && normalizeLowercaseStringOrEmpty(profile) !== "default") {
    return path.join(home, ".supportClaw", `workspace-${profile}`);
  }
  return path.join(home, ".supportClaw", "workspace");
}

/** Return configured agent entries after dropping nullish placeholders. */
function listAgentEntries(cfg: SupportClawConfig): AgentConfig[] {
...
function resolveDefaultAgentId(cfg: SupportClawConfig): string {
...
function resolveAgentConfig(cfg: SupportClawConfig, agentId: string): AgentConfig | undefined {
