...
import type { SupportClawConfig } from "./config-utils.js";
...
const rootMemoryConfig = (workspaceDir: string): SupportClawConfig =>
  ({
    agents: {
...
describe("resolveMemoryBackendConfig", () => {
  it("defaults to builtin backend when config missing", () => {
    const cfg = { agents: { defaults: { workspace: "/tmp/memory-test" } } } as SupportClawConfig;
  ...
  it("preserves unquoted Windows absolute qmd command paths", () => {
    ...
    const cfg = {
      agents: { defaults: { workspace: String.raw`C:\Users\penny\.supportClaw\workspace` } },
      memory: {
        backend: "qmd",
        qmd: {
          command,
