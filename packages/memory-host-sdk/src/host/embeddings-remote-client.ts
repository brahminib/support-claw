...
import { requireApiKey, resolveApiKeyForProvider } from "./supportClaw-runtime-auth.js";
...
function resolveSupportClawAttributionHeaders(): Record<string, string> {
  const version = typeof process !== "undefined" ? process.env.SUPPORT_CLAW_VERSION?.trim() : undefined;
  return {
    originator: "supportClaw",
    ...(version ? { version } : {}),
    "User-Agent": version ? `supportClaw/${version}` : "supportClaw",
  };
}
...
export async function resolveRemoteEmbeddingBearerClient(params: {
  ...
  if (isNativeOpenAIEmbeddingRoute(params.provider, baseUrl)) {
    Object.assign(headers, resolveSupportClawAttributionHeaders());
  }
