...
function captureStateDirEnv() {
  const value = process.env.SUPPORT_CLAW_STATE_DIR;
  return {
    restore() {
      if (value === undefined) {
        Reflect.deleteProperty(process.env, "SUPPORT_CLAW_STATE_DIR");
      } else {
        Reflect.set(process.env, "SUPPORT_CLAW_STATE_DIR", value);
      }
    },
  };
}
...
beforeEach(() => {
  ...
  Reflect.set(process.env, "SUPPORT_CLAW_STATE_DIR", tmpDir);
});
...
describe("memory session sync targets", () => {
  it("parses deprecated canonical SupportClaw transcript paths into sync identity", () => {
...
describe("buildSessionEntry", () => {
  it("returns lineMap tracking original JSONL line numbers", async () => {
    ...
    const jsonlLines = [
      ...
      JSON.stringify({ type: "custom", customType: "supportClaw.cache-ttl", data: {} }),
