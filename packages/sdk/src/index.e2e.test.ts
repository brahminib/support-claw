...
import { GatewayClientTransport, SupportClaw } from "./index.js";
...
describe("SupportClaw SDK websocket e2e", () => {
  afterEach(async () => {
    await Promise.all(
...
describe("SupportClaw SDK real Gateway e2e", () => {
...
const liveGatewayUrl = process.env.SUPPORT_CLAW_SDK_LIVE_GATEWAY_URL;
const liveGatewayToken = process.env.SUPPORT_CLAW_SDK_LIVE_GATEWAY_TOKEN;
...
liveGatewayDescribe("SupportClaw SDK live Gateway e2e", () => {
  it("connects to a configured Gateway, streams a real run, and waits for completion", async () => {
    ...
    try {
      ...
      const agent = await oc.agents.get(process.env.SUPPORT_CLAW_SDK_LIVE_AGENT_ID ?? "main");
      const run = await agent.run({
        input: "Reply with exactly: SUPPORT_CLAW_SDK_LIVE_OK",
      ...
      expect(events.text).toContain("SUPPORT_CLAW_SDK_LIVE_OK");
    } finally {
      await oc.close();
    }
  }, 240_000);
