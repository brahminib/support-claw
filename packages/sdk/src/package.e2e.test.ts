// SupportClaw SDK tests cover package behavior.
...
const WORKSPACE_PACKAGE_NAMES = [
  "@supportclaw/gateway-protocol",
  "@supportclaw/gateway-client",
  "@supportclaw/sdk",
] as const;
...
function normalizeWorkspaceDependencies(
  ...
  for (const [name, spec] of Object.entries(dependencies)) {
    normalized[name] =
      name.startsWith("@supportclaw/") && spec === "workspace:*" ? "0.0.0-private" : spec;
  }
  return normalized;
}
...
async function startSupportClawRegistry(packages: PackedPackage[]): Promise<{
  registryUrl: string;
  close: () => Promise<void>;
}> {
...
describe("SupportClaw SDK package e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-sdk-consumer-"));
    ...
    const sdkTarball =
      packedPackages.find((pkg) => pkg.manifest.name === "@supportclaw/sdk")?.tarball ?? "";
    expect(sdkTarball).not.toBe("");
    const registry = await startSupportClawRegistry(packedPackages);

    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ private: true, type: "module" }),
    );
    await fs.writeFile(path.join(tempDir, ".npmrc"), `@supportclaw:registry=${registry.registryUrl}`);
    try {
    ...
    const importScript = `
      import { GatewayClientTransport, SupportClaw, normalizeGatewayEvent } from "@supportclaw/sdk";
      if (typeof GatewayClientTransport !== "function") throw new Error("missing transport export");
      if (typeof SupportClaw !== "function") throw new Error("missing client export");
