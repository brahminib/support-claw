// SupportClaw SDK tests cover index behavior.
import { describe, expect, it } from "vitest";
import { EventHub, SupportClaw, normalizeGatewayEvent } from "./index.js";
import type {
  GatewayEvent,
...
class FakeTransport implements SupportClawTransport {
...
class EventsOnlyTransport implements SupportClawTransport {
...
describe("SupportClaw SDK", () => {
  it("runs an agent through the Gateway agent method", async () => {
    ...
    const oc = new SupportClaw({ transport });
  ...
  it("splits provider-qualified model refs and rejects unsupported run options", async () => {
    ...
    await expect(
      ...
      "SupportClaw Gateway does not support per-run SDK options yet: workspace, runtime, environment, approvals",
    );
  });
  ...
  it("throws explicit unsupported errors for SDK namespaces without Gateway RPCs", async () => {
    ...
    await expect(oc.environments.create({ provider: "testbox" })).rejects.toThrow(
      "oc.environments.create is not supported by the current SupportClaw Gateway yet",
    );
    await expect(oc.environments.delete("environment_123")).rejects.toThrow(
      "oc.environments.delete is not supported by the current SupportClaw Gateway yet",
    );
    expect(transport.calls).toStrictEqual([]);
  });
  ...
  it("keeps close terminal when it races a pending connect", async () => {
    ...
    await expect(connect).rejects.toThrow("SupportClaw SDK client is closed");
    await close;
    await expect(oc.agents.list()).rejects.toThrow("SupportClaw SDK client is closed");
    await expect(oc.events()[Symbol.asyncIterator]().next()).rejects.toThrow(
      "SupportClaw SDK client is closed",
    );
    expect(() => oc.rawEvents()).toThrow("SupportClaw SDK client is closed");
  ...
  it("rejects normalized event streams when the event pump fails before yielding", async () => {
    ...
    let futureIterator: AsyncIterator<SupportClawEvent> | undefined;
  ...
  it("uses cumulative text for the first replayed chat projection", async () => {
    ...
    let text = "";
    let iterator: AsyncIterator<SupportClawEvent> | undefined;
