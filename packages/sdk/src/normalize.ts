// SupportClaw SDK helper module supports normalize behavior.
import type { GatewayEvent, JsonObject, SupportClawEvent, SupportClawEventType } from "./types.js";
...
function normalizeLifecycleEndEventType(data: JsonObject): SupportClawEventType {
  const status = readLowerString(data.status);
  const stopReason = readLowerString(data.stopReason);
...
function normalizeAgentEventType(payload: JsonObject): SupportClawEventType {
...
function normalizeNamedEventType(event: GatewayEvent): SupportClawEventType {
...
export function normalizeGatewayEvent(event: GatewayEvent): SupportClawEvent {
  const payload = asRecord(event.payload);
