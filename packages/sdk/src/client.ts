// SupportClaw SDK module implements client behavior.
...
// High-level SupportClaw SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the SupportClaw SDK client. */
export type SupportClawOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
  requestTimeoutMs?: number;
  transport?: SupportClawTransport;
};

function resolveGatewayUrl(options: SupportClawOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `SupportClaw Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current SupportClaw Gateway yet`);
}
...
function readChatProjection(event: SupportClawEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: SupportClawEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: SupportClawEvent): boolean {
  return (
...
export class SupportClaw {
  ...
  private readonly transport: SupportClawTransport;
  private readonly normalizedEvents = new EventHub<SupportClawEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, SupportClawEvent[]>();
  ...
  constructor(options: SupportClawOptions = {}) {
    this.transport =
      options.transport ??
  ...
  events(filter?: (event: SupportClawEvent) => boolean): AsyncIterable<SupportClawEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: SupportClawEvent) => boolean,
  ): AsyncIterable<SupportClawEvent> {
  ...
  private assertOpen(): void {
    if (this.closed) {
      throw new Error("SupportClaw SDK client is closed");
    }
  }
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: SupportClawEvent): SupportClawEvent | undefined => {
    ...
    const matches = (event: SupportClawEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: SupportClawEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): SupportClawEvent[] {
...
export class Agent {
  constructor(
    private readonly client: SupportClaw,
    readonly id: string,
  ) {}
...
export class AgentsNamespace {
  constructor(private readonly client: SupportClaw) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<SupportClawEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: SupportClaw,
...
export class TasksNamespace extends RpcNamespace {
  constructor(client: SupportClaw) {
    super(client, "tasks");
  }
