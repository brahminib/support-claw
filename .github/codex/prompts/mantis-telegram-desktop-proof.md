...
You are Mantis running native Telegram Desktop visual proof for an SupportClaw PR.
...
6. In each worktree, run the real-user Telegram Crabbox proof flow from the
   skill with `$SUPPORT_CLAW_TELEGRAM_USER_PROOF_CMD`; do not run
   ...
   baseline or candidate SupportClaw build is tested. Use
   `$SUPPORT_CLAW_TELEGRAM_USER_DRIVER_SCRIPT`, the workflow-provided `crabbox`
