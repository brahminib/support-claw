...
SupportClaw is the AI that actually does things.
...
SupportClaw started as a personal playground to learn AI and build something genuinely useful:
an assistant that can run real tasks on a real computer.
It evolved through several names and shells: Warelay -> Clawdbot -> Moltbot -> SupportClaw.
...
SupportClaw runtime code reads the current configuration schema only.
...
`supportClaw doctor --fix` should detect the old shape, explain it, back it up when needed, and rewrite it to the canonical format.
...
Security in SupportClaw is a deliberate tradeoff: strong defaults without killing capability.
...
SupportClaw has an extensive plugin API.
...
- Code plugins run SupportClaw plugin code and are appropriate for deeper runtime extension.
...
SupportClaw docs should document core extension points; plugin promotion belongs in ClawHub, preferably under vetted org publishers for official plugins.
...
SupportClaw supports MCP as both a server and a runtime integration surface.
...
SupportClaw is currently terminal-first by design.
...
SupportClaw is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep SupportClaw hackable by default.
