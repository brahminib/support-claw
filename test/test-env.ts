...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.SUPPORT_CLAW_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "SUPPORT_CLAW_TEST_FAST", value: process.env.SUPPORT_CLAW_TEST_FAST },
    {
      key: "SUPPORT_CLAW_STRICT_FAST_REPLY_CONFIG",
      value: process.env.SUPPORT_CLAW_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "SUPPORT_CLAW_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.SUPPORT_CLAW_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "SUPPORT_CLAW_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.SUPPORT_CLAW_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "SUPPORT_CLAW_STATE_DIR", value: process.env.SUPPORT_CLAW_STATE_DIR },
    { key: "SUPPORT_CLAW_CONFIG_PATH", value: process.env.SUPPORT_CLAW_CONFIG_PATH },
    { key: "SUPPORT_CLAW_GATEWAY_PORT", value: process.env.SUPPORT_CLAW_GATEWAY_PORT },
    { key: "SUPPORT_CLAW_BRIDGE_ENABLED", value: process.env.SUPPORT_CLAW_BRIDGE_ENABLED },
    { key: "SUPPORT_CLAW_BRIDGE_HOST", value: process.env.SUPPORT_CLAW_BRIDGE_HOST },
    { key: "SUPPORT_CLAW_BRIDGE_PORT", value: process.env.SUPPORT_CLAW_BRIDGE_PORT },
    { key: "SUPPORT_CLAW_CANVAS_HOST_PORT", value: process.env.SUPPORT_CLAW_CANVAS_HOST_PORT },
    { key: "SUPPORT_CLAW_TEST_HOME", value: process.env.SUPPORT_CLAW_TEST_HOME },
    { key: "SUPPORT_CLAW_AGENT_DIR", value: process.env.SUPPORT_CLAW_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SUPPORT_CLAW_TEST_HOME = tempHome;
  process.env.SUPPORT_CLAW_TEST_FAST = "1";
  process.env.SUPPORT_CLAW_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.SUPPORT_CLAW_ALLOW_SLOW_REPLY_TESTS;

  // Ensure test runs never touch the developer's real config/state, even if they have overrides set.
  delete process.env.SUPPORT_CLAW_CONFIG_PATH;
  ...
  delete process.env.SUPPORT_CLAW_AGENT_DIR;
  // Prefer test-controlled ports over developer overrides (avoid port collisions across tests/workers).
  delete process.env.SUPPORT_CLAW_GATEWAY_PORT;
  delete process.env.SUPPORT_CLAW_BRIDGE_ENABLED;
  delete process.env.SUPPORT_CLAW_BRIDGE_HOST;
  delete process.env.SUPPORT_CLAW_BRIDGE_PORT;
  delete process.env.SUPPORT_CLAW_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.SUPPORT_CLAW_STATE_DIR = path.join(tempHome, ".supportClaw");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.SUPPORT_CLAW_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.SUPPORT_CLAW_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".supportClaw");
  const priorIsolatedHome = params.env.SUPPORT_CLAW_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".supportClaw")
  ) {
    realStateDir = path.join(params.realHome, ".supportClaw");
  }
  const tempStateDir = path.join(params.tempHome, ".supportClaw");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.SUPPORT_CLAW_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.SUPPORT_CLAW_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "supportClaw.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "supportClaw.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.SUPPORT_CLAW_LIVE_TEST === "1" ||
    process.env.SUPPORT_CLAW_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.SUPPORT_CLAW_LIVE_USE_REAL_HOME);
