...
const SHARED_TEST_SETUP = Symbol.for("supportClaw.sharedTestSetup");
const EMBEDDED_RUN_STATE = Symbol.for("supportClaw.embeddedRunState");
const REPLY_RUN_REGISTRY = Symbol.for("supportClaw.replyRunRegistry");
...
function getSharedTestHome(): string | undefined {
  ...
  return globalState[SHARED_TEST_SETUP]?.tempHome ?? process.env.SUPPORT_CLAW_TEST_HOME;
}
...
function restoreSharedTestHomeAfterEnvUnstub(testHomeRaw: string | undefined): void {
  ...
  process.env.SUPPORT_CLAW_TEST_HOME = testHome;
...
function resetSupportClawGlobalRunState(): void {
...
export default class SupportClawNonIsolatedRunner extends TestRunner {
  override onCollectStart(file: { filepath: string }) {
    super.onCollectStart(file);
    restoreRealTimers();
    restoreNativeTimerGlobals();
    restoreSharedTestHomeAfterEnvUnstub(getSharedTestHome());
    const orderLogPath = process.env.SUPPORT_CLAW_VITEST_FILE_ORDER_LOG?.trim();
    if (orderLogPath) {
  ...
  override async onAfterRunSuite(suite: RunnerTestSuite) {
    ...
    vi.clearAllMocks();
    resetSupportClawGlobalRunState();
    vi.resetModules();
