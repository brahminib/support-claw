...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.SUPPORT_CLAW_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.SUPPORT_CLAW_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.SUPPORT_CLAW_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.SUPPORT_CLAW_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.SUPPORT_CLAW_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.SUPPORT_CLAW_VITEST_PRINT_IMPORT_BREAKDOWN)) {
