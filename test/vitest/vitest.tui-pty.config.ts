...
export function createTuiPtyVitestConfig(env?: Record<string, string | undefined>) {
  ...
  const includeLocal = configEnv.SUPPORT_CLAW_TUI_PTY_INCLUDE_LOCAL === "1";
  const include = [
    "tui/tui-pty-harness.e2e.test.ts",
    ...(includeLocal ? ["tui/tui-pty-local.e2e.test.ts"] : []),
  ];
  const includeFromEnv = toTuiPtyIncludePatterns(
    loadPatternListFromEnv("SUPPORT_CLAW_VITEST_INCLUDE_FILE", configEnv),
  );
  ...
  return defineConfig({
    ...
    test: {
      ...
      setupFiles: [
        ...new Set(
          [...(baseTest.setupFiles ?? []), "test/setup-supportClaw-runtime.ts"].map(
            resolveRepoRootPath,
          ),
        ),
      ],
