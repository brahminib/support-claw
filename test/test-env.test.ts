...
function createTempHome(): string {
  return makeTempDir(tempDirs, "supportClaw-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-supportClaw.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.supportClaw/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.supportClaw/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".supportClaw", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".supportClaw", "external-plugins", "glueclaw", "supportClaw.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".supportClaw", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.SUPPORT_CLAW_LIVE_TEST_QUIET = "1";
    process.env.SUPPORT_CLAW_CONFIG_PATH = "~/custom-supportClaw.json5";
    process.env.SUPPORT_CLAW_TEST_HOME = priorIsolatedHome;
    process.env.SUPPORT_CLAW_STATE_DIR = path.join(priorIsolatedHome, ".supportClaw");
    ...
    expect(process.env.SUPPORT_CLAW_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".supportClaw", "supportClaw.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".supportClaw", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".supportClaw", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.SUPPORT_CLAW_LIVE_TEST = "1";
    process.env.SUPPORT_CLAW_LIVE_USE_REAL_HOME = "1";
    process.env.SUPPORT_CLAW_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.SUPPORT_CLAW_LIVE_TEST;
    delete process.env.SUPPORT_CLAW_LIVE_GATEWAY;
    delete process.env.SUPPORT_CLAW_LIVE_USE_REAL_HOME;
    delete process.env.SUPPORT_CLAW_LIVE_TEST_QUIET;
