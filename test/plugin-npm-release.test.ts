...
import { bundledPluginFile, bundledPluginRoot } from "supportClaw/plugin-sdk/test-fixtures";
...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @supportclaw/zalo, @supportclaw/feishu  @supportclaw/zalo "),
    ).toEqual(["@supportclaw/feishu", "@supportclaw/zalo"]);
  });
});
...
describe("collectPublishablePluginPackageErrors", () => {
  ...
  it("flags invalid publishable plugin metadata", () => {
    expect(
      ...
      'package name must start with "@supportclaw/"; found "broken".',
      "package.json private must not be true.",
      'package.json type must be "module" so built .js runtime entries load as ESM.',
      `package.json repository.url must be "${SUPPORT_CLAW_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "<missing>".`,
      'package.json version must match YYYY.M.PATCH, YYYY.M.PATCH-N, YYYY.M.PATCH-alpha.N, or YYYY.M.PATCH-beta.N; found "latest".',
      "supportClaw.extensions must contain only non-empty strings.",
      "supportClaw.install.npmSpec must be a non-empty string for publishable plugins.",
    ]);
  });
  ...
  it("requires npm install metadata for publishable plugins", () => {
    ...
    ).toEqual(["supportClaw.install.npmSpec must be a non-empty string for publishable plugins."]);
  });
  ...
  it("requires the external plugin package compatibility contract for npm publish", () => {
    expect(
      ...
      "supportClaw.build.supportClawVersion is required for external code plugin packages.",
    ]);
  });
...
describe("collectPluginReleaseVersionFloorErrors", () => {
  it("blocks selected plugin stable and beta releases below the June 2026 floor", () => {
    expect(
      ...
      '@supportclaw/demo@2026.6.4-beta.1: June 2026 stable and beta release trains must use patch 5 or higher because 2026.6.5-beta.1 is already published; found "2026.6.4-beta.1".',
    ]);
  });
...
describe("collectPublishablePluginPackages", () => {
  it("keeps publishable plugin dist trees out of the core npm package unless bundled", () => {
    ...
    for (const plugin of publishablePlugins) {
      ...
      if (packageJson.supportClaw?.build?.bundledDist === true) {
  ...
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "supportClaw-plugin-npm-release-");
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@supportclaw/demo-plugin",
      },
    ]);
  });
  ...
  it("does not validate unselected publishable plugin manifests", () => {
    ...
    writeJsonFile(join(repoDir, "extensions", "private-plugin", "package.json"), {
      name: "@supportclaw/private-plugin",
      version: "2026.4.10-beta.1",
      private: true,
      supportClaw: {
        extensions: ["./index.ts"],
        ...externalPluginContract("2026.4.10-beta.1"),
        install: {
          npmSpec: "@supportclaw/private-plugin",
        },
        release: {
          publishToNpm: true,
        },
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@supportclaw/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @supportclaw/missing.");
  });
});
