// QA OTEL Smoke runtime supports SupportClaw repository automation.
...
import { resolveTimerTimeoutMs } from "@supportclaw/normalization-core/number-coercion";
...
const DEFAULT_DOCKER_COLLECTOR_IMAGE =
  process.env.SUPPORT_CLAW_QA_OTEL_COLLECTOR_IMAGE || "otel/opentelemetry-collector:0.104.0";
...
const REQUIRED_METRIC_NAMES = ["supportClaw.harness.duration_ms"] as const;
...
const MAX_OTLP_COMPRESSED_BODY_BYTES = readPositiveIntegerEnv(
  "SUPPORT_CLAW_QA_OTEL_MAX_COMPRESSED_BODY_BYTES",
  2 * 1024 * 1024,
);
const MAX_OTLP_DECODED_BODY_BYTES = readPositiveIntegerEnv(
  "SUPPORT_CLAW_QA_OTEL_MAX_DECODED_BODY_BYTES",
  8 * 1024 * 1024,
);
const MAX_CAPTURED_BODY_TEXT_BYTES = readPositiveIntegerEnv(
  "SUPPORT_CLAW_QA_OTEL_MAX_CAPTURED_BODY_TEXT_BYTES",
  512 * 1024,
);
const QA_SUITE_TIMEOUT_MS = readPositiveIntegerEnv(
  "SUPPORT_CLAW_QA_OTEL_SUITE_TIMEOUT_MS",
  10 * 60 * 1000,
);
const QA_SUITE_KILL_GRACE_MS = readPositiveIntegerEnv("SUPPORT_CLAW_QA_OTEL_SUITE_KILL_GRACE_MS", 5000);
const MAX_STDOUT_DIAGNOSTIC_LINE_BYTES = readPositiveIntegerEnv(
  "SUPPORT_CLAW_QA_OTEL_MAX_STDOUT_DIAGNOSTIC_LINE_BYTES",
  512 * 1024,
);
...
function isStdoutDiagnosticLogRecord(value: unknown): value is StdoutDiagnosticLogRecord {
  return (
    ...
    value !== null &&
    !Array.isArray(value) &&
    objectValue(value, "signal") === "supportClaw.diagnostic.log"
  );
}
...
async function startDockerOtelCollector(
  ...
  const tempDir = await makeTempDir(path.join(osTmpdir(), "supportClaw-otel-collector-"));
  const configPath = path.join(tempDir, "collector.yaml");
  const containerName = `supportClaw-otel-smoke-${makeUuid()}`;
...
exporters:
  otlphttp/supportClaw:
    endpoint: ${receiverEndpoint}
service:
  pipelines:
    traces:
      receivers: [otlp]
      exporters: [otlphttp/supportClaw]
    metrics:
      receivers: [otlp]
      exporters: [otlphttp/supportClaw]
    logs:
...
function supportClawEntryArgs(): string[] {
  if (existsSync(path.join(process.cwd(), "scripts", "run-node.mjs"))) {
    return ["scripts/run-node.mjs"];
  }
  return ["supportClawCli.mjs"];
}

function spawnSupportClaw(args: string[], env: NodeJS.ProcessEnv): ChildProcess {
  return spawn(process.execPath, [...supportClawEntryArgs(), ...args], {
...
async function waitForChild(
  ...
  throw new Error(`supportClaw qa suite timed out after ${resolvedTimeoutMs}ms`);
}
...
function buildQaEnv(port: number): NodeJS.ProcessEnv {
  ...
  env.OTEL_SERVICE_NAME = "supportClaw-qa-lab-otel-smoke";
  env.OTEL_SEMCONV_STABILITY_OPT_IN = "gen_ai_latest_experimental";
  env.SUPPORT_CLAW_QA_SUITE_PROGRESS = env.SUPPORT_CLAW_QA_SUITE_PROGRESS ?? "1";
  return env;
}
...
function isLatestGenAiModelCallSpan(span: CapturedSpan): boolean {
  ...
  return (
    ...
    typeof span.attributes["supportClaw.provider"] === "string" &&
    typeof span.attributes["supportClaw.model"] === "string"
  );
}
...
function assertSmoke(params: {
  ...
  if (spanNames.has("supportClaw.model.call")) {
    failures.push("legacy supportClaw.model.call span exported with GenAI semconv opt-in");
  }
  ...
  const contentKeys = [...attributeKeys].filter((key) => key.startsWith("supportClaw.content."));
  ...
  const modelErrorSpans = modelSpans.filter((span) => {
    ...
    return (
      ...
      Object.hasOwn(span.attributes, "supportClaw.errorCategory") ||
...
async function main() {
  ...
  try {
    ...
    const child = spawnSupportClaw(buildQaArgs(options), buildQaEnv(exportPort));
