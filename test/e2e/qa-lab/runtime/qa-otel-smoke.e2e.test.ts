...
describe("qa-otel-smoke receiver bounds", () => {
  ...
  beforeAll(() => {
    configuredBodyLimitLoad = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          SUPPORT_CLAW_QA_OTEL_MAX_CAPTURED_BODY_TEXT_BYTES: "1024",
          SUPPORT_CLAW_QA_OTEL_MAX_COMPRESSED_BODY_BYTES: "2048",
          SUPPORT_CLAW_QA_OTEL_MAX_DECODED_BODY_BYTES: "4096",
        },
      },
    );
  });
  ...
  function makePassingSmokeAssertionInput(): Parameters<typeof testing.assertSmoke>[0] {
    return {
      ...
      metrics: [{ name: "supportClaw.harness.duration_ms" }],
      requests: [
        {
          path: "/v1/traces",
          signal: "traces",
      ...
      stdoutLogRecords: [],
      spans: [
        { name: "supportClaw.run", parent: false, attributes: {} },
        { name: "supportClaw.harness.run", parent: true, attributes: {} },
        { name: "supportClaw.context.assembled", parent: true, attributes: {} },
        { name: "supportClaw.message.delivery", parent: true, attributes: {} },
        {
          name: "chat gpt-5.5",
          parent: true,
          attributes: {
  ...
  it("scrubs inherited OpenTelemetry exporter env before running the QA suite", () => {
    ...
    expect(env.OTEL_SERVICE_NAME).toBe("supportClaw-qa-lab-otel-smoke");
  });
    ...
    input.stdoutLogRecords = [
      {
        ...
        "service.name": "supportClaw-qa-lab-otel-smoke",
        severityText: "INFO",
        severityNumber: 9,
        body: "log",
        attributes: {
          "supportClaw.log.level": "INFO",
        },
      },
    ];
  ...
  it("streams gateway stdout artifact records without requiring them in the tail", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "supportClaw-qa-otel-stdout-stream-"));
  ...
  it("keeps gateway stdout artifact fallback parsing bounded", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "supportClaw-qa-otel-stdout-artifact-"));
  ...
  it("times out and kills a wedged QA suite child with a detached gateway", async () => {
    ...
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "supportClaw-qa-otel-child-"));
    ...
    try {
      ...
      await expect(testing.waitForChild(child, 100, 100)).rejects.toThrow(
        "supportClaw qa suite timed out after 100ms",
      );
  ...
  it("cleans Docker collector containers and temp config after readiness failures", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "supportClaw-qa-otel-collector-"));
    ...
    try {
      ...
      expect(stopDockerContainer).toHaveBeenCalledWith(
        "supportClaw-otel-smoke-00000000-0000-4000-8000-000000000000",
      );
  ...
  it("reports bounded Docker collector output when readiness exits", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "supportClaw-qa-otel-collector-output-"));
    const collectorDir = path.join(tempRoot, "collector");
    const child = new EventEmitter() as EventEmitter & {
      stderr: EventEmitter;
      stdout: EventEmitter;
