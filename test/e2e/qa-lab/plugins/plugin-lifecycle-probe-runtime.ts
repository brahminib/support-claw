...
type ProbeEnv = Pick<NodeJS.ProcessEnv, "HOME" | "SUPPORT_CLAW_CONFIG_PATH" | "SUPPORT_CLAW_STATE_DIR">;
...
function stateDir(env: ProbeEnv = process.env) {
  return env.SUPPORT_CLAW_STATE_DIR || path.join(env.HOME ?? os.homedir(), ".supportClaw");
}

function configPath(env: ProbeEnv = process.env) {
  return env.SUPPORT_CLAW_CONFIG_PATH || path.join(stateDir(env), "supportClaw.json");
}
...
function createMatrixStateEnv(resourceDir: string): MatrixEnv {
  ...
  const configFile = path.join(stateDir, "supportClaw.json");
  ...
  return {
    ...
    SUPPORT_CLAW_CONFIG_PATH: configFile,
    SUPPORT_CLAW_TEST_WORKSPACE_DIR: workspaceDir,
    SUPPORT_CLAW_AUTH_PROFILE_SECRET_KEY: randomBytes(32).toString("hex"),
  };
}
...
function packageEntrypoint(prefix: string) {
  ...
  throw new Error(`SupportClaw package entrypoint not found under ${packageRoot}/dist/`);
}
...
async function installSupportClawPackage(prefix: string, env: MatrixEnv) {
  const packageTgz = env.SUPPORT_CLAW_CURRENT_PACKAGE_TGZ;
  assertProbe(packageTgz, "SUPPORT_CLAW_CURRENT_PACKAGE_TGZ is required");
  const installLog = "/tmp/supportClaw-plugin-lifecycle-install.log";
  process.stdout.write("Installing mounted SupportClaw package...\n");
  await runCommand(
    "npm",
    ["install", "-g", "--prefix", prefix, packageTgz, "--no-fund", "--no-audit"],
    {
      ...
      timeoutMs: parseDurationMs(env.SUPPORT_CLAW_E2E_NPM_INSTALL_TIMEOUT, "600s"),
    },
  );
}
...
export async function runPluginLifecycleMatrix() {
  ...
  const packageName = "@supportclaw/lifecycle-claw";
  const resourceDir = tempDirs.make("supportClaw-plugin-lifecycle-matrix-");
  ...
  try {
    await installSupportClawPackage(npmPrefix, env);
