// SupportClaw launcher E2E tests validate launcher process behavior.
...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = makeTempDir(fixtureRoots, "supportClaw-launcher-");
  await fs.copyFile(
    path.resolve(process.cwd(), "supportClawCli.mjs"),
    path.join(fixtureRoot, "supportClawCli.mjs"),
  );
...
async function makeLauncherProbeFixture(
  ...
  const launcherPath = path.join(fixtureRoot, "supportClawCli.mjs");
  const launcher = await fs.readFile(launcherPath, "utf8");
  const bootstrapStart = "\nif (!waitingForCompileCacheRespawn) {";
  const bootstrapIndex = launcher.indexOf(bootstrapStart);
  if (bootstrapIndex < 0) {
    throw new Error("supportClaw launcher bootstrap block was not found");
  }
  await fs.writeFile(
    launcherPath,
...
async function addGitMarker(fixtureRoot: string): Promise<void> {
  await fs.writeFile(path.join(fixtureRoot, ".git"), "gitdir: .git/worktrees/supportClaw\n", "utf8");
}
...
async function addCompileCacheProbe(fixtureRoot: string): Promise<void> {
  await fs.writeFile(
    ...
    [
      ...
      '  `${module.getCompileCacheDir?.() ? "cache:enabled" : "cache:disabled"};respawn:${process.env.SUPPORT_CLAW_COMPILE_CACHE_DISABLED_RESPAWNED ?? "0"}`',
      ");",
    ].join("\n"),
    "utf8",
  );
...
function launcherEnv(extra: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  ...
  delete env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR;
  delete env.SUPPORT_CLAW_CONFIG_PATH;
  delete env.SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS;
  delete env.SUPPORT_CLAW_HOME;
  delete env.SUPPORT_CLAW_STATE_DIR;
  delete env.SUPPORT_CLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
describe("supportClaw launcher", () => {
  ...
  it("keeps the bootstrap Node floor aligned with package and runtime guards", async () => {
    const [launcher, runtimeGuard, packageJsonRaw] = await Promise.all([
      fs.readFile(path.resolve(process.cwd(), "supportClawCli.mjs"), "utf8"),
    ...
    if (!launcherMatch) {
      throw new Error("supportClawCli.mjs MIN_NODE_* constants were not found");
    }
    if (!runtimeMatch) {
    ...
    expect(
      ...
      "supportClawCli.mjs MIN_NODE_* must match package.json engines.node",
    ...
    expect(result.stderr).toContain(
      `supportClaw: Node.js v${launcherMinimumLabel}+ is required (current: v${mockedNodeVersion}).`,
    );
  });
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      'import "missing-supportClaw-launcher-dep";\nexport {};\n',
      "utf8",
    );

    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "supportClawCli.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
    ...
    expect(result.stderr).toContain("missing-supportClaw-launcher-dep");
  ...
  it("prints root version without importing the runtime entry", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "supportClawCli.mjs"), "--version"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
    ...
    expect(result.stdout).toBe("SupportClaw 1.2.3-test (abcdef0)\n");
  ...
  it("defers container-targeted root version to the runtime entry", async () => {
    ...
    await fs.writeFile(
      path.join(fixtureRoot, "package.json"),
      JSON.stringify({ name: "supportClaw", version: "1.2.3-test" }),
      "utf8",
    );
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "supportClawCli.mjs"), "--container", "demo", "--version"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
    ...
    const envResult = spawnSync(
      ...
      {
        cwd: fixtureRoot,
        env: launcherEnv({ SUPPORT_CLAW_CONTAINER: "demo" }),
        encoding: "utf8",
      },
    );
  ...
  it("treats Bun direct optional import misses as direct launcher misses", async () => {
    const fixtureRoot = await makeLauncherProbeFixture(
      ...
      [
        ...
        "    { message: \"Cannot find module './nested.js' from '/pkg/supportClaw/dist/entry.js'\" },",
        "    './dist/entry.js',",
        "  ),",
        "  sameSpecifierTransitive: isDirectModuleNotFoundError(",
        "    { message: \"Cannot find module './dist/entry.js' from '/pkg/supportClaw/dist/entry.js'\" },",
        "    './dist/entry.js',",
        "  ),",
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "supportClawCli.mjs")], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
  ...
  it.runIf(process.env.SUPPORT_CLAW_TEST_BUN_LAUNCHER === "1" && hasBunRuntime())(
  ...
  it.runIf(process.env.SUPPORT_CLAW_TEST_BUN_LAUNCHER === "1" && hasBunRuntime())(
    ...
    async () => {
      ...
      const result = spawnSync(
        process.env.BUN_BIN ?? "bun",
        [path.join(fixtureRoot, "supportClawCli.mjs"), "--help"],
        {
          cwd: fixtureRoot,
          env: launcherEnv(),
          encoding: "utf8",
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "supportClawCli.mjs"), params.command, "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it.each(["doctor", "gateway", "models", "plugins", "sessions", "tasks"])(
    ...
    async (command) => {
      ...
      const result = spawnSync(
        process.execPath,
        [path.join(fixtureRoot, "supportClawCli.mjs"), command, "--help"],
        {
          cwd: fixtureRoot,
          env: launcherEnv(),
          encoding: "utf8",
  ...
  it("uses precomputed subcommand help with leading root options", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "supportClawCli.mjs"), "--profile", "work", "--no-color", "models", "-h"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it("defers precomputed subcommand help to the runtime entry when container env is set", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "supportClawCli.mjs"), "models", "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv({ SUPPORT_CLAW_CONTAINER: "demo" }),
        encoding: "utf8",
  ...
  it.each([
    {
      name: "container env",
      args: ["browser", "--help"],
      env: { SUPPORT_CLAW_CONTAINER: "demo" },
    },
    {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "supportClawCli.mjs"), ...params.args],
      {
        cwd: fixtureRoot,
  ...
  it("defers root help to the runtime entry when plugin config can change help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const configPath = path.join(fixtureRoot, "supportClaw.json");
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "cli-startup-metadata.json"),
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "supportClawCli.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ SUPPORT_CLAW_CONFIG_PATH: configPath }),
      encoding: "utf8",
    });
  ...
  it("defers nodes help to the runtime entry when plugin config can change help", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "supportClawCli.mjs"), "nodes", "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv({ SUPPORT_CLAW_CONFIG_PATH: configPath }),
        encoding: "utf8",
  ...
  it("checks the SUPPORT_CLAW_HOME default config path before using precomputed root help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const supportClawHome = path.join(fixtureRoot, "home");
    const configDir = path.join(supportClawHome, ".supportClaw");
    ...
    await fs.writeFile(
      path.join(configDir, "supportClaw.json"),
      JSON.stringify({ plugins: { slots: { memory: "memory-lancedb" } } }),
      "utf8",
    );
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "supportClawCli.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ SUPPORT_CLAW_HOME: supportClawHome }),
      encoding: "utf8",
    });
  ...
  it("checks legacy config candidates before using precomputed root help", async () => {
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "supportClawCli.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ HOME: home, SUPPORT_CLAW_HOME: undefined }),
      encoding: "utf8",
    });
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:supportClaw/supportClaw#<ref>");
  });
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          'process.title = "supportClaw-launcher-sigterm-test-child";',
      ...
      const launcher = spawn(process.execPath, [path.join(fixtureRoot, "supportClawCli.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv({
          NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
        }),
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          'import { writeFileSync } from "node:fs";',
          `writeFileSync(${JSON.stringify(childInfoPath)}, JSON.stringify({ pid: process.pid }) + "\\n");`,
          'process.title = "supportClaw-launcher-sigterm-ignore-test-child";',
          'process.on("SIGTERM", () => {});',
          "setInterval(() => {}, 1000);",
          "",
        ].join("\n"),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await addGitMarker(fixtureRoot);
      await addCompileCacheProbe(fixtureRoot);
      const linkParent = makeTempDir(fixtureRoots, "supportClaw-launcher-link-");
      const linkedRoot = path.join(linkParent, "supportClaw-linked");
      await fs.symlink(fixtureRoot, linkedRoot, "dir");

      const result = spawnSync(process.execPath, [path.join(linkedRoot, "supportClawCli.mjs")], {
        cwd: linkParent,
        env: launcherEnv({
  ...
  it("scopes packaged launcher compile cache inside configured cache roots", async () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join(".node-compile-cache", "supportClaw", "2026.4.29"));
  });

  it("falls back to the default packaged launcher compile cache when NODE_COMPILE_CACHE is empty", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const runCwd = makeTempDir(fixtureRoots, "supportClaw-launcher-cwd-");
    const tmpRoot = makeTempDir(fixtureRoots, "supportClaw-launcher-tmp-");
    await fs.writeFile(path.join(fixtureRoot, "package.json"), '{"version":"2026.4.29"}\n');
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      [
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join("node-compile-cache", "supportClaw", "2026.4.29"));
    expect(result.stdout).not.toContain(path.join(runCwd, "supportClaw"));
  });
  ...
  it("skips compile cache for Windows packaged launchers on early Node 24.x", async () => {
    for (const nodeVersion of ["24.1.0", "24.14.0"]) {
      ...
      const result = spawnSync(
        process.execPath,
        ["--import", pathToFileURL(mockRuntime).href, path.join(fixtureRoot, "supportClawCli.mjs")],
        {
          cwd: fixtureRoot,
          env: launcherEnv({
            TMP: tmpRoot,
