...
import {
  ...
  buildSupportClawReleaseClawHubRuntimeState,
  parseSupportClawReleaseClawHubPlanArgs,
} from "../scripts/lib/supportClaw-release-clawhub-plan.ts";
import {
...
import {
  ...
  SUPPORT_CLAW_PLUGIN_NPM_REPOSITORY_URL,
...
describe("collectClawHubPublishablePluginPackages", () => {
  it("requires the ClawHub external plugin contract", () => {
    ...
    expect(() => collectClawHubPublishablePluginPackages(repoDir)).toThrow(
      "supportClaw.compat.pluginApi is required for external code plugin packages.",
    );
  });
  ...
  it("validates only selected package names when filters are provided", () => {
    ...
    expect(
      collectClawHubPublishablePluginPackages(repoDir, {
        packageNames: ["@supportclaw/demo-plugin"],
      }).map((plugin) => plugin.packageName),
    ).toEqual(["@supportclaw/demo-plugin"]);
  });
});

describe("SupportClaw dual-published plugin metadata", () => {
  const dualPublishedPlugins = [
    {
      extensionId: "cohere",
      packageName: "@supportclaw/cohere-provider",
      install: {
        clawhubSpec: "clawhub:@supportclaw/cohere-provider",
        defaultChoice: "npm",
        minHostVersion: ">=2026.6.8",
        npmSpec: "@supportclaw/cohere-provider",
      },
    },
    {
      extensionId: "diagnostics-otel",
      packageName: "@supportclaw/diagnostics-otel",
      install: {
    ...
    },
    {
      ...
      packageName: "@supportclaw/diagnostics-prometheus",
      install: {
        clawhubSpec: "clawhub:@supportclaw/diagnostics-prometheus",
        defaultChoice: "npm",
        minHostVersion: ">=2026.4.25",
        npmSpec: "@supportclaw/diagnostics-prometheus",
      },
    },
    {
      extensionId: "gmi",
      packageName: "@supportclaw/gmi-provider",
      install: {
        clawhubSpec: "clawhub:@supportclaw/gmi-provider",
        defaultChoice: "npm",
        minHostVersion: ">=2026.6.8",
        npmSpec: "@supportclaw/gmi-provider",
      },
    },
  ] as const;
  ...
  it("keeps dual-published plugins selectable through both ClawHub and npm release paths", () => {
    ...
    for (const plugin of dualPublishedPlugins) {
      ...
      expect(packageJson.supportClaw?.install).toEqual(plugin.install);
      expect(packageJson.supportClaw?.release).toEqual({
        publishToClawHub: true,
        publishToNpm: true,
      });
    }
...
describe("collectClawHubVersionGateErrors", () => {
  it("requires a version bump when a publishable plugin changes", () => {
    ...
    expect(errors).toEqual([
      "@supportclaw/demo-plugin@2026.4.1: changed publishable plugin still has the same version in package.json.",
    ]);
  });
  ...
  it("does not require a version bump for the first ClawHub opt-in", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@supportclaw/demo-plugin",
          version: "2026.4.1",
          type: "module",
          repository: {
            type: "git",
            url: SUPPORT_CLAW_PLUGIN_NPM_REPOSITORY_URL,
          },
          supportClaw: {
            extensions: ["./index.ts"],
            compat: {
            ...
            install: {
              npmSpec: "@supportclaw/demo-plugin",
            },
            build: {
              supportClawVersion: "2026.4.1",
            },
            release: {
              publishToClawHub: true,
            },
...
describe("collectPluginClawHubReleasePlan", () => {
    ...
    const { fetchImpl, requests } = createClawHubPlanFetch({
      ...
      trustedPublishers: {
        "@supportclaw/demo-plugin": {
          ...
          body: {
            trustedPublisher: {
              repository: "supportClaw/supportClaw",
      ...
      versions: {
        "@supportclaw/demo-plugin@2026.4.1": 404,
      },
    });

    const plan = await collectPluginClawHubReleasePlan({
      rootDir: repoDir,
      selection: ["@supportclaw/demo-plugin"],
      fetchImpl,
      registryBaseUrl: "https://clawhub.ai",
    });

    expect(plan.candidates.map((plugin) => plugin.packageName)).toEqual(["@supportclaw/demo-plugin"]);
    expect(plan.bootstrapCandidates).toStrictEqual([]);
    expect(plan.missingTrustedPublisher).toStrictEqual([]);
    expect(requests).toEqual([
      "/api/v1/packages/%40supportClaw%2Fdemo-plugin",
      "/api/v1/packages/%40supportClaw%2Fdemo-plugin/trusted-publisher",
      "/api/v1/packages/%40supportClaw%2Fdemo-plugin/versions/2026.4.1",
    ]);
  });
  ...
  it("cancels unused ClawHub package and version response bodies", async () => {
    ...
    const fetchImpl: typeof fetch = async (input) => {
      ...
      if (url.pathname === "/api/v1/packages/%40supportClaw%2Fdemo-plugin") {
        return new Response(
      ...
      if (url.pathname === "/api/v1/packages/%40supportClaw%2Fdemo-plugin/trusted-publisher") {
        return new Response(
          JSON.stringify({
            trustedPublisher: {
              repository: "supportClaw/supportClaw",
      ...
      if (url.pathname === "/api/v1/packages/%40supportClaw%2Fdemo-plugin/versions/2026.4.1") {
        return new Response(
          new ReadableStream<Uint8Array>({
            cancel() {
  ...
  it("retries a rate-limited trusted publisher lookup", async () => {
    ...
    const fetchImpl: typeof fetch = async (input) => {
      ...
      if (pathname === "/api/v1/packages/%40supportClaw%2Fdemo-plugin") {
        return new Response("{}", { status: 200 });
      }
      if (pathname === "/api/v1/packages/%40supportClaw%2Fdemo-plugin/trusted-publisher") {
      ...
      if (pathname === "/api/v1/packages/%40supportClaw%2Fdemo-plugin/versions/2026.4.1") {
  ...
  it("routes missing package rows to bootstrap candidates instead of normal candidates", async () => {
    ...
    expect(plan.bootstrapCandidates[0]).toMatchObject({
      ...
      artifactName: "clawhub-package-supportClaw-demo-plugin-2026.4.1",
      packageName: "@supportclaw/demo-plugin",
      version: "2026.4.1",
    });
    expect(plan.missingTrustedPublisher).toStrictEqual([]);
  });
  ...
  it("skips versions that already exist on ClawHub", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@supportclaw/demo-plugin@2026.4.1": 200,
      },
    });
  ...
  it("plans selected packages without validating unrelated publishable packages", async () => {
    ...
    expect(plan.candidates.map((plugin) => plugin.artifactName)).toEqual([
      "clawhub-package-supportClaw-demo-plugin-2026.4.1",
    ]);
  });
});

describe("buildSupportClawReleaseClawHubPlan", () => {
  it("emits a dispatch plan that keeps ClawHub children on the release tag", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      packages: {
        ...
        "@supportclaw/demo-two": {
          status: 404,
        },
        "@supportclaw/demo-three": {
          status: 200,
          body: {
            package: {},
            owner: {},
      ...
      versions: {
        "@supportclaw/demo-plugin@2026.4.1": 404,
        "@supportclaw/demo-three@2026.4.1": 404,
      },
    });

    const plan = await buildSupportClawReleaseClawHubPlan(
      {
    ...
    expect(plan.normal).toEqual({
      ...
      shouldDispatch: true,
      packages: ["@supportclaw/demo-plugin"],
      inputs: {
        publish_scope: "selected",
        plugins: "@supportclaw/demo-plugin",
    ...
    expect(plan.bootstrap).toEqual({
      ...
      ref: "v2026.4.1-beta.1",
      shouldDispatch: true,
      packages: ["@supportclaw/demo-two", "@supportclaw/demo-three"],
      inputs: {
        plugins: "@supportclaw/demo-two,@supportclaw/demo-three",
        release_publish_run_id: "12345",
        release_publish_branch: "main",
      },
    });
    ...
    expect(plan.summary).toEqual({
      ...
      normalPlugins: "@supportclaw/demo-plugin",
      bootstrapPlugins: "@supportclaw/demo-two,@supportclaw/demo-three",
      missingTrustedPlugins: "@supportclaw/demo-three",
    });
  ...
  it("routes already-published packages missing trusted publisher config to bootstrap repair", async () => {
    ...
    expect(plan.summary).toMatchObject({
      ...
      bootstrapCount: 1,
      missingTrustedPublisherCount: 1,
      bootstrapPlugins: "@supportclaw/demo-plugin",
      missingTrustedPlugins: "@supportclaw/demo-plugin",
    });
  });
...
describe("buildSupportClawReleaseClawHubRuntimeState", () => {
  it("includes the normal ClawHub run in verifier args when the release waits for it", () => {
    const state = buildSupportClawReleaseClawHubRuntimeState({
      repository: "supportClaw/supportClaw",
      waitForClawHub: true,
    ...
    expect(state.proofLines.normal).toBe(
      "- plugin ClawHub publish: https://github.com/brahminib/support-claw/actions/runs/111",
    );
  ...
  it("includes a completed bootstrap run even when there is no normal ClawHub run", () => {
    ...
    expect(state.proofLines.bootstrap).toBe(
      "- plugin ClawHub bootstrap: https://github.com/brahminib/support-claw/actions/runs/222",
    );
  });
  ...
  it("skips ClawHub verification for non-awaited incomplete runs while keeping proof links", () => {
    ...
    expect(state.proofLines.normal).toBe(
      "- plugin ClawHub publish: dispatched separately, not awaited by this proof: https://github.com/brahminib/support-claw/actions/runs/111",
    );
    expect(state.proofLines.bootstrap).toBe(
      "- plugin ClawHub bootstrap: dispatched separately, not awaited by this proof: https://github.com/brahminib/support-claw/actions/runs/222",
    );
  });
...
if [[ "\${1:-}" == "package" && "\${2:-}" == "pack" ]]; then
  ...
  pack_path="$pack_destination/supportClaw-demo-plugin-2026.4.1.tgz"
  printf 'fake tgz\\n' > "$pack_path"
  printf '{"path":"%s","name":"@supportclaw/demo-plugin","version":"2026.4.1"}\\n' "$pack_path"
fi
exit 0
`,
    );
...
`,
    ...
    execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          SUPPORT_CLAW_CLAWHUB_MANUAL_OVERRIDE_REASON:
...
`,
    ...
    const output = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          SUPPORT_CLAW_CLAWHUB_PACK_OUTPUT_DIR: outputDir,
          PATH: `${binDir}${delimiter}${process.env.PATH ?? ""}`,
        },
      },
    );
    ...
    expect(existsSync(join(outputDir, "supportClaw-demo-plugin-2026.4.1.tgz"))).toBe(true);
    const invocations = readFileSync(markerPath, "utf8");
...
function createTempPluginRepo(
  ...
  const repoDir = makeTempRepoRoot(tempDirs, "supportClaw-clawhub-release-");
  const extensionId = options.extensionId ?? "demo-plugin";
  const extensionIds = [extensionId, ...(options.extraExtensionIds ?? [])];

  writeFileSync(
    join(repoDir, "package.json"),
    JSON.stringify({ name: "supportClaw-test-root", type: "module" }, null, 2),
  );
  ...
  for (const currentExtensionId of extensionIds) {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: `@supportclaw/${currentExtensionId}`,
          version: "2026.4.1",
          type: "module",
          repository: {
            type: "git",
          ...
          supportClaw: {
            ...
            install: {
              npmSpec: `@supportclaw/${currentExtensionId}`,
            },
            release: {
