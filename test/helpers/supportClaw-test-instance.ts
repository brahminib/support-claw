// SupportClaw test instance helper spawns isolated SupportClaw processes.
...
import {
  ...
  type SupportClawTestStateOptions,
} from "../../src/test-utils/supportClaw-test-state.js";
import { sleep } from "../../src/utils.js";

export type SupportClawTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<SupportClawTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type SupportClawTestInstanceCommandResult = {
  code: number | null;
...
export type SupportClawTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  ) => Promise<SupportClawTestInstanceCommandResult>;
...
type SupportClawTestChildProcess = Pick<ChildProcessWithoutNullStreams, "kill" | "pid">;
...
async function prepareGatewayEntrypoint(cwd: string): Promise<string[]> {
  ...
  const child = spawn("node", ["scripts/run-node.mjs", "--help"], {
    ...
    detached: shouldUseSupportClawTestProcessGroup(),
  });
  ...
  if (completed === null) {
    signalSupportClawTestProcess(child, "SIGKILL");
...
export async function createSupportClawTestInstance(
  options: SupportClawTestInstanceOptions,
): Promise<SupportClawTestInstance> {
  ...
  const instance: SupportClawTestInstance = {
    name: options.name,
    port,
    ...
    stopGateway: async () => {
      ...
      if (!hasChildExited(child) && !child.killed) {
        try {
          signalSupportClawTestProcess(child, "SIGTERM");
        } catch {
          // ignore
        }
      }
...
function shouldUseSupportClawTestProcessGroup(): boolean {
  return process.platform !== "win32";
}

function signalSupportClawTestProcess(
  child: SupportClawTestChildProcess,
  signal: NodeJS.Signals,
  killProcess: (pid: number, signal: NodeJS.Signals) => boolean = process.kill,
): void {
  if (shouldUseSupportClawTestProcessGroup() && typeof child.pid === "number") {
    try {
