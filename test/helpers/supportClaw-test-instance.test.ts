// SupportClaw test instance tests cover spawned test instance lifecycle.
import fs from "node:fs/promises";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { createSupportClawTestInstance, testing } from "./supportClaw-test-instance.js";
...
describe("supportClaw test instance", () => {
  ...
  it("signals test instance process groups on POSIX", () => {
    ...
    testing.signalSupportClawTestProcess(child, "SIGKILL", killProcess);
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    const inst = await createSupportClawTestInstance({
      name: "instance-unit",
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".supportClaw"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "supportClaw.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.SUPPORT_CLAW_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.SUPPORT_CLAW_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.SUPPORT_CLAW_SKIP_CRON).toBe("0");
