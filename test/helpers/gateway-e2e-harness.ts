...
import { createSupportClawTestInstance, type SupportClawTestInstance } from "./supportClaw-test-instance.js";

export { extractFirstTextBlock };

export type GatewayInstance = SupportClawTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createSupportClawTestInstance({ name });
  try {
