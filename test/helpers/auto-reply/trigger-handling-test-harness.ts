...
const embeddedAgentMocks = getSharedMocks("supportClaw.trigger-handling.embedded-agent-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("supportClaw.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("supportClaw.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("supportClaw.trigger-handling.web-session-mocks", () => ({
...
type TempHomeEnvSnapshot = {
  ...
  supportClawHome: string | undefined;
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    supportClawHome: process.env.SUPPORT_CLAW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("SUPPORT_CLAW_HOME", snapshot.supportClawHome);
  restoreKey("SUPPORT_CLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.SUPPORT_CLAW_STATE_DIR = join(home, ".supportClaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "supportClaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".supportClaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): SupportClawConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-7" },
        workspace: join(home, "supportClaw"),
