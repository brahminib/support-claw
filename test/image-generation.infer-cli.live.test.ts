...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.SUPPORT_CLAW_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through supportClaw infer", () => {
    const outputBase = path.join(os.tmpdir(), `supportClaw-infer-image-${process.pid}.png`);
