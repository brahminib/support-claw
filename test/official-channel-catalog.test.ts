...
import { bundledPluginRoot } from "supportClaw/plugin-sdk/test-fixtures";
...
type OfficialChannelInstall = NonNullable<
  NonNullable<OfficialChannelCatalogEntry["supportClaw"]>["install"]
>;
...
function requireInstall(entry: OfficialChannelCatalogEntry | undefined): OfficialChannelInstall {
  const install = entry?.supportClaw?.install;
  if (!install) {
...
function summarizeCatalogEntry(entry: OfficialChannelCatalogEntry) {
  return {
    ...
    source: entry.source,
    plugin: entry.supportClaw?.plugin,
    channel: entry.supportClaw?.channel,
    install: entry.supportClaw?.install,
  };
}
...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("supportClaw-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@supportclaw/whatsapp",
      version: "2026.3.23",
      description: "SupportClaw WhatsApp channel plugin",
      supportClaw: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@wecom/wecom-supportClaw-plugin"),
      ),
    ).toEqual({
      name: "@wecom/wecom-supportClaw-plugin",
      description: "SupportClaw WeCom channel plugin by the Tencent WeCom team.",
      source: "external",
      plugin: {
      ...
      install: {
        npmSpec: "@wecom/wecom-supportClaw-plugin@2026.5.7",
        defaultChoice: "npm",
        expectedIntegrity:
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "supportClaw-plugin-yuanbao"),
      ),
    ).toEqual({
      name: "supportClaw-plugin-yuanbao",
      description: "SupportClaw Yuanbao channel plugin by the Tencent Yuanbao team.",
      source: "external",
      plugin: {
      ...
      install: {
        npmSpec: "supportClaw-plugin-yuanbao@2.15.0",
        defaultChoice: "npm",
        expectedIntegrity:
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@supportclaw/whatsapp"),
      ),
    ).toEqual({
      name: "@supportclaw/whatsapp",
      description: "SupportClaw WhatsApp channel plugin",
  ...
  it("keeps third-party official external catalog npm sources exactly pinned", () => {
    const repoRoot = makeRepoRoot("supportClaw-official-channel-catalog-policy-");
    const entries = buildOfficialChannelCatalog({ repoRoot }).entries.filter(
      (entry) => entry.source === "external" && !entry.name?.startsWith("@supportclaw/"),
    );
  ...
  it("allows official SupportClaw channel npm specs without integrity during launch", () => {
    const repoRoot = makeRepoRoot("supportClaw-official-channel-catalog-supportClaw-policy-");
    const twitch = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (entry) => entry.supportClaw?.channel?.id === "twitch",
    );

    expect({
      name: twitch?.name,
      install: twitch?.supportClaw?.install,
    }).toEqual({
  ...
  it("preserves ClawHub specs when generating publishable channel catalog entries", () => {
    const repoRoot = makeRepoRoot("supportClaw-official-channel-catalog-clawhub-");
    writeJson(path.join(repoRoot, "extensions", "storepack-chat", "package.json"), {
      name: "@supportclaw/storepack-chat",
      supportClaw: {
        channel: {
          id: "storepack-chat",
        ...
        install: {
          clawhubSpec: "clawhub:@supportclaw/storepack-chat",
          npmSpec: "@supportclaw/storepack-chat",
          defaultChoice: "clawhub",
        },
        release: {
          publishToNpm: true,
    ...
    const entry = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (candidate) => candidate.supportClaw?.channel?.id === "storepack-chat",
    );
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("supportClaw-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@supportclaw/whatsapp",
      supportClaw: {
        channel: {
    ...
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "@wecom/wecom-supportClaw-plugin",
    );
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "supportClaw-plugin-yuanbao",
    );
    const whatsappEntry = findCatalogEntry(
      entries,
      (entry: { supportClaw?: { channel?: { id?: string } } }) =>
        entry.supportClaw?.channel?.id === "whatsapp",
    );
