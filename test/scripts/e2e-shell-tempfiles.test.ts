...
describe("e2e shell tempfile hygiene", () => {
  ...
  it("preserves wizard exit status when reporting failures", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "supportClaw-onboard-status-test-"));
...
export SUPPORT_CLAW_ONBOARD_SCENARIO_SOURCE_ONLY=1
export SUPPORT_CLAW_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
SUPPORT_CLAW_ENTRY=node
supportClaw_test_state_create() { :; }
...
`,
  ...
  it("checks local onboarding logs for systemd noise", async () => {
    ...
    expect(contents).toContain('SUPPORT_CLAW_E2E_LOG_DIR="$ONBOARD_TMP_DIR/logs"');
    ...
    expect(contents).toContain('validate_local_basic_log "$SUPPORT_CLAW_E2E_LAST_LOG_PATH"');
    expect(contents).not.toContain(
      "validate_local_basic_log /tmp/supportClaw-onboard-local-basic.log",
    );
    expect(contents).toContain(
  ...
  it("probes onboarding gateway readiness through TCP", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "supportClaw-onboard-gateway-log-"));
...
supportClaw_e2e_probe_tcp() { return 0; }
sleep 30 &
GATEWAY_PID="$!"
...
`,
  ...
  it("rejects onboarding gateway readiness when the TCP probe fails", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "supportClaw-onboard-gateway-tcp-"));
...
export SUPPORT_CLAW_ONBOARD_SCENARIO_SOURCE_ONLY=1
export SUPPORT_CLAW_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
export SUPPORT_CLAW_ONBOARD_GATEWAY_WAIT_ATTEMPTS=2
export SUPPORT_CLAW_ONBOARD_GATEWAY_WAIT_INTERVAL_S=0.1
SUPPORT_CLAW_ENTRY=node
source scripts/e2e/lib/onboard/scenario.sh

supportClaw_e2e_probe_tcp() { return 1; }
sleep 30 &
GATEWAY_PID="$!"
printf 'listening on ws://127.0.0.1:18789\\n' >"$GATEWAY_LOG_PATH"
...
`,
  ...
  it("rejects invalid onboarding gateway wait attempts before probing", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "supportClaw-onboard-gateway-attempts-"));
...
export SUPPORT_CLAW_ONBOARD_SCENARIO_SOURCE_ONLY=1
export SUPPORT_CLAW_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
export SUPPORT_CLAW_ONBOARD_GATEWAY_WAIT_ATTEMPTS=2x
SUPPORT_CLAW_ENTRY=node
source scripts/e2e/lib/onboard/scenario.sh
...
`,
    ...
    try {
      ...
      expect(result.stderr).toContain("invalid SUPPORT_CLAW_ONBOARD_GATEWAY_WAIT_ATTEMPTS: 2x");
  ...
  it("removes fallback ClawHub skill install HOME on failure", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "supportClaw-clawhub-home-test-"));
...
`,
    ...
    try {
      const result = spawnSync("bash", ["scripts/e2e/lib/skills/clawhub-install-proof.sh"], {
        ...
        env: {
          ...process.env,
          SUPPORT_CLAW_CURRENT_PACKAGE_TGZ: "",
          SUPPORT_CLAW_TEST_STATE_SCRIPT_B64: "",
      ...
      expect(
        scratchEntries.filter((entry) => entry.startsWith("supportClaw-skill-install-home.")),
      ).toEqual([]);
    } finally {
      await rm(tempRoot, { force: true, recursive: true });
    }
  ...
  it("rejects ClawHub skill info paths that only share a resolved prefix", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "supportClaw-clawhub-info-path-"));
