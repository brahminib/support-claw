...
function makeFakeCrabbox(helpText: string): string {
  ...
  const binDir = mkdtempSync(path.join(tmpdir(), "supportClaw-fake-crabbox-"));
...
function writeFakeCrabbox(binDir: string, helpText: string): string {
  ...
  if (process.platform !== "win32") {
    ...
    const script = [
      "#!/bin/sh",
      'if [ "$1" = "--version" ]; then',
      '  printf "%s\\n" "${SUPPORT_CLAW_FAKE_CRABBOX_VERSION:-crabbox 0.22.1}"',
      "  exit 0",
      "fi",
      ...
      '  for arg in "$@"; do',
      '    if [ "$arg" = "--json" ]; then',
      '      status="${SUPPORT_CLAW_FAKE_CRABBOX_CONFIG_STATUS:-0}"',
      '      if [ "$status" != "0" ]; then',
      '        printf "%s\\n" "config unavailable" >&2',
      '        exit "$status"',
      "      fi",
      '      if [ -n "${SUPPORT_CLAW_FAKE_CRABBOX_CONFIG_JSON+x}" ]; then',
      '        printf "%s" "$SUPPORT_CLAW_FAKE_CRABBOX_CONFIG_JSON"',
      "      else",
      ...
      "fi",
      'if [ "$1" = "whoami" ]; then',
      '  status="${SUPPORT_CLAW_FAKE_CRABBOX_WHOAMI_STATUS:-0}"',
      '  if [ "$status" != "0" ]; then',
      ...
      '  previous_arg="$arg"',
      "done",
      'if [ "${SUPPORT_CLAW_FAKE_CRABBOX_DELETE_CWD_AND_EXIT:-}" = "1" ]; then',
      '  deleted_cwd="$PWD"',
      "  cd / || exit 1",
      '  rm -rf "$deleted_cwd"',
      "  exit 0",
      "fi",
      'if [ "${SUPPORT_CLAW_FAKE_CRABBOX_DELETE_CWD_ONCE:-}" = "1" ]; then',
      '  deleted_cwd="$PWD"',
      "  cd / || exit 1",
      '  rm -rf "$deleted_cwd"',
      "  deadline=100",
      ...
      "fi",
      'if [ -n "${SUPPORT_CLAW_FAKE_CRABBOX_DESCENDANT_PID_PATH:-}" ]; then',
      `  ${shellSingleQuote(process.execPath)} --input-type=module --eval ${shellSingleQuote(signalIgnoringDescendantScript)} &`,
      '  printf "%s" "$!" > "$SUPPORT_CLAW_FAKE_CRABBOX_DESCENDANT_PID_PATH"',
      '  trap "exit 0" INT TERM HUP',
      "  while :; do sleep 1; done",
      "fi",
      'printf "%s\\0" "__SUPPORT_CLAW_FAKE_CRABBOX_V1__"',
      'printf "%s\\0" "$PWD"',
      'printf "%s\\0" "$#"',
      'for arg in "$@"; do',
      '  printf "%s\\0" "$arg"',
  ...
  const helperScript = [
    ...
    "  const status = Number.parseInt(process.env.SUPPORT_CLAW_FAKE_CRABBOX_CONFIG_STATUS || '0', 10);",
    "  if (status !== 0) {",
    "    process.stderr.write('config unavailable\\n');",
    "    process.exit(status);",
    "  }",
    '  process.stdout.write(process.env.SUPPORT_CLAW_FAKE_CRABBOX_CONFIG_JSON || \'{"coordinator":"configured-broker","brokerAuth":"configured"}\');',
    "  process.exit(0);",
    "}",
    'if (args[0] === "whoami") {',
    "  const status = Number.parseInt(process.env.SUPPORT_CLAW_FAKE_CRABBOX_WHOAMI_STATUS || '0', 10);",
    "  if (status !== 0) {",
  ...
  const script = [
    ...
    "const args = process.argv.slice(2);",
    'if (args[0] === "--version") {',
    "  console.log(process.env.SUPPORT_CLAW_FAKE_CRABBOX_VERSION || 'crabbox 0.22.1');",
    "  process.exit(0);",
    "}",
...
function makeSlowVersionCrabbox(helpText: string): string {
  const binDir = mkdtempSync(path.join(tmpdir(), "supportClaw-slow-crabbox-"));
  tempDirs.push(binDir);
  const crabboxPath = path.join(binDir, "crabbox");
...
function makeFakeGit(
  ...
  const binDir = mkdtempSync(path.join(tmpdir(), "supportClaw-fake-git-"));
  tempDirs.push(binDir);
  ...
  const script = [
    ...
    "const responses = new Map(Object.entries(JSON.parse(process.env.SUPPORT_CLAW_FAKE_GIT_RESPONSES || '{}')));",
    "const args = process.argv.slice(2);",
...
function wrapperEnv(helpText: string, options: WrapperOptions): NodeJS.ProcessEnv {
  ...
  return {
    ...
    PATH: [...(options.extraPathEntries ?? []), binDir, gitBinDir, process.env.PATH ?? ""]
      ...
      .join(path.delimiter),
    CRABBOX_PROVIDER: "",
    SUPPORT_CLAW_CRABBOX_ALLOW_DIRECT_AWS: "",
    SUPPORT_CLAW_CRABBOX_SYNC_MIN_FREE_BYTES: "0",
    SUPPORT_CLAW_CRABBOX_WRAPPER_IGNORE_REPO_BINARY: "1",
    ...(options.configJson
      ? { SUPPORT_CLAW_FAKE_CRABBOX_CONFIG_JSON: JSON.stringify(options.configJson) }
      : {}),
    ...(options.configStatus
      ? { SUPPORT_CLAW_FAKE_CRABBOX_CONFIG_STATUS: String(options.configStatus) }
      : {}),
    ...options.env,
    SUPPORT_CLAW_FAKE_GIT_RESPONSES: JSON.stringify(gitResponses),
  };
}
...
function parseFakeCrabboxOutput(result: ReturnType<typeof runWrapper>): {
  ...
  const marker = "__SUPPORT_CLAW_FAKE_CRABBOX_V1__\0";
...
async function runSignalCleanupProof(sendSignals: (pid: number) => Promise<void>): Promise<void> {
  const root = mkdtempSync(path.join(tmpdir(), "supportClaw-crabbox-descendant-"));
  tempDirs.push(root);
  ...
  const runner = spawnWrapper(
    ...
    {
      env: {
        SUPPORT_CLAW_FAKE_CRABBOX_DESCENDANT_PID_PATH: descendantPidPath,
      },
    },
  );
...
const remoteChangedGateEnvPrefix =
  "SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1 SUPPORT_CLAW_CHANGED_LANES_RAW_SYNC=1 CI=1";
const remoteChangedGateExport = `export ${remoteChangedGateEnvPrefix};`;
const remoteChangedGateFetch =
  'git fetch -q --depth=1 origin "$supportClaw_changed_gate_base:refs/remotes/origin/main"';

function expectChangedGateGitBootstrap(remoteCommand: string): void {
  expect(remoteCommand).toContain("command -v git");
  expect(remoteCommand).toContain(
    "supportClaw_changed_gate_base=${SUPPORT_CLAW_CHANGED_GATE_BASE:-abc123}",
  );
  expect(remoteCommand).toContain(
    'supportClaw_changed_gate_remote_base="$(git rev-parse --verify refs/remotes/origin/main 2>/dev/null || true)"',
  );
  expect(remoteCommand).toContain(
    '[ "$supportClaw_changed_gate_remote_base" != "$supportClaw_changed_gate_base" ]',
  );
...
describe.concurrent("scripts/crabbox-wrapper", () => {
  ...
  it("requires a current Crabbox binary for Blacksmith Testbox runs", () => {
    const result = runWrapper(
      ...
      { env: { SUPPORT_CLAW_FAKE_CRABBOX_VERSION: "crabbox 0.21.9" } },
    );
  ...
  it("rejects prerelease Crabbox builds at the Blacksmith minimum boundary", () => {
    const result = runWrapper(
      "provider: hetzner, aws, local-container, blacksmith-testbox, or cloudflare\n",
      ["run", "--provider", "blacksmith-testbox", "--", "echo ok"],
      { env: { SUPPORT_CLAW_FAKE_CRABBOX_VERSION: "crabbox 0.22.0-rc.1" } },
    );
  ...
  it("rejects unsafe Crabbox version numbers at the Blacksmith minimum gate", () => {
    const result = runWrapper(
      "provider: hetzner, aws, local-container, blacksmith-testbox, or cloudflare\n",
      ["run", "--provider", "blacksmith-testbox", "--", "echo ok"],
      { env: { SUPPORT_CLAW_FAKE_CRABBOX_VERSION: "crabbox 0.9007199254740993.0" } },
    );
  ...
  it("accepts post-release Crabbox describe builds at the Blacksmith minimum boundary", () => {
    const result = runWrapper(
      "provider: hetzner, aws, local-container, blacksmith-testbox, or cloudflare\n",
      ["run", "--provider", "blacksmith-testbox", "--", "echo ok"],
      { env: { SUPPORT_CLAW_FAKE_CRABBOX_VERSION: "crabbox 0.22.0-3-gabc1234" } },
    );
  ...
  it("rejects reused Blacksmith Testboxes that were not created by Crabbox", () => {
    const home = mkdtempSync(path.join(tmpdir(), "supportClaw-crabbox-home-"));
    tempDirs.push(home);
  ...
  it("only forces the short local-container Docker work root on Linux", () => {
    ...
    const expectedMessage =
      "[crabbox] provider=docker using short host-visible work root for SupportClaw Docker tests";
    ...
    const result = runWrapper(
      ...
      ["run", "--target", "windows", "--", "echo ok"],
      {
        env: {
          ...
          SUPPORT_CLAW_CRABBOX_ALLOW_DIRECT_AWS: "1",
        },
      },
    );
  ...
  it("fails closed for AWS proof when broker auth is missing", () => {
    ...
    expect(result.stderr).toContain(
      "crabbox login --url https://crabbox.supportClaw.ai --provider aws",
    );
  });
  ...
  it("fails closed for AWS proof when broker auth is stale", () => {
    const result = runWrapper(
      ...
      ["run", "--provider", "aws", "--", "echo ok"],
      {
        configJson: { coordinator: "https://crabbox.supportClaw.ai", brokerAuth: "configured" },
        env: { SUPPORT_CLAW_FAKE_CRABBOX_WHOAMI_STATUS: "1" },
      },
    );
  ...
  it("allows explicit direct AWS debugging without broker auth", () => {
    const result = runWrapper(
      ...
      {
        configJson: { coordinator: "", brokerAuth: "missing" },
        env: { SUPPORT_CLAW_CRABBOX_ALLOW_DIRECT_AWS: "1" },
      },
    );
  ...
  it("bootstraps only Node for raw AWS macOS node commands", () => {
    ...
    expect(remoteCommand).toContain("supportClaw_crabbox_bootstrap_macos_js");
  ...
  it("preflights Swift 6.2 for raw AWS macOS Swift app builds", () => {
    ...
    expect(result.status).toBe(0);
    expect(output.args).toContain("--shell");
    expect(remoteCommand).toContain("supportClaw_crabbox_require_macos_swift_62");
    expect(remoteCommand).toContain("/Applications/Xcode_26.1.app");
    expect(remoteCommand).toContain("/Applications/Xcode-26*.app");
    expect(remoteCommand).toContain('sudo xcode-select -s "$supportClaw_developer"');
    expect(remoteCommand).toContain("SupportClaw macOS app proof requires Swift tools 6.2+");
    expect(remoteCommand).not.toContain("supportClaw_crabbox_bootstrap_macos_js");
    expectGroupedShellCommand(
      remoteCommand,
      "swift build --package-path apps/macos --product SupportClaw",
    );
  });
  ...
  it("preserves sanitized env pnpm package commands when Swift preflight is needed", () => {
    ...
    expect(remoteCommand).toContain("supportClaw_crabbox_bootstrap_macos_js");
    expect(remoteCommand).toContain("supportClaw_crabbox_require_macos_swift_62");
    expectGroupedShellCommand(remoteCommand, "supportClaw_crabbox_env -i pnpm mac:package");
  });
  ...
  it("normalizes inherited Linux UTF-8 locale names for raw AWS macOS bootstrap", () => {
    ...
    const remoteCommand = normalizeShellLineEndings(output.args.at(-1) ?? "");
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain('macos_locale="${SUPPORT_CLAW_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}"');
    expect(remoteCommand).toContain(
  ...
  it("bootstraps Bun for raw AWS macOS env-prefixed bun commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain("bun --version >&2 || return 1");
    expectGroupedShellCommand(remoteCommand, "supportClaw_crabbox_env -i bun --version");
  });
  ...
  it("bootstraps Corepack for raw AWS macOS pnpm commands", () => {
    ...
    expect(remoteCommand).toContain('ready_marker="$node_dir/.supportClaw-crabbox-node-ready"');
    expect(remoteCommand).toContain(
  ...
  it("bootstraps Corepack for raw AWS macOS env-prefixed pnpm commands", () => {
    ...
    expect(remoteCommand).toContain('corepack enable --install-directory "$PNPM_HOME"');
    expect(remoteCommand).toContain("pnpm --version >&2");
    expectGroupedShellCommand(remoteCommand, "supportClaw_crabbox_env pnpm --version");
  });
  ...
  it("bootstraps Corepack for raw AWS macOS env option pnpm commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain("supportClaw_crabbox_bootstrap_macos_js");
    expect(remoteCommand).toContain("supportClaw_crabbox_env");
    expect(remoteCommand).not.toContain("export -f env supportClaw_crabbox_env");
    expect(remoteCommand).not.toContain('env() { supportClaw_crabbox_env "$@"; };');
    expect(remoteCommand).toContain("PATH=$PATH:${1#PATH=}");
    expect(remoteCommand).toContain("pnpm --version >&2");
    expectGroupedShellCommand(
      remoteCommand,
      "supportClaw_crabbox_env -i PATH=/usr/bin:/bin pnpm --version",
    );
  });
  ...
  it("bootstraps Corepack for raw AWS macOS env options before ignore-environment", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      "supportClaw_crabbox_env -u FOO -i PATH=/usr/bin:/bin pnpm --version",
    );
  });
  ...
  it("injects the bootstrapped PATH for raw AWS macOS absolute env -i commands", () => {
    ...
    expect(remoteCommand).toContain("supportClaw_crabbox_bootstrap_macos_js");
    expect(remoteCommand).toContain(
      'if [ "$supportClaw_env_ignore" = "1" ] && [ "$supportClaw_env_path_seen" = "0" ]; then supportClaw_env_args+=("PATH=$PATH"); fi;',
    );
    expectGroupedShellCommand(remoteCommand, "supportClaw_crabbox_env -i pnpm --version");
  });
  ...
  it("bootstraps Corepack for raw AWS macOS env split-string pnpm commands", () => {
    ...
    expectGroupedShellCommand(remoteCommand, "supportClaw_crabbox_env -S 'pnpm --version'");
  });
  ...
  it("bootstraps Corepack for AWS macOS node changed-gate commands", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      `supportClaw_crabbox_env ${remoteChangedGateEnvPrefix} node scripts/check-changed.mjs`,
    );
  });
  ...
  it("bootstraps Corepack for AWS macOS node option changed-gate commands", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      `supportClaw_crabbox_env ${remoteChangedGateEnvPrefix} node --max-old-space-size 4096 --env-file-if-exists .env --unhandled-rejections strict --trace-warnings --import=tsx scripts/check-changed.mjs`,
    );
  });
  ...
  it("does not treat node script arguments as changed-gate commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain("supportClaw_crabbox_bootstrap_macos_js");
    expect(remoteCommand).not.toContain("SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1");
    expectGroupedShellCommand(
      remoteCommand,
  ...
  it("bootstraps AWS macOS script-stdin runs before the uploaded script body", () => {
    ...
    expect(output.scriptContent).toContain("supportClaw_crabbox_bootstrap_macos_js");
    expect(output.scriptContent).toContain('if [ ! -d "$TMPDIR" ]; then mkdir -p "$TMPDIR"');
    expect(output.scriptContent).toContain("supportClaw_crabbox_bootstrap_macos_js || exit $?");
  ...
  it("preserves AWS macOS script-stdin shebang payloads behind the bootstrap wrapper", () => {
    ...
    expect(output.scriptContent).toContain("cat >\"$tmp_script\" <<'SUPPORT_CLAW_CRABBOX_SCRIPT_0'");
    expect(output.scriptContent).toContain(`\n${script}\nSUPPORT_CLAW_CRABBOX_SCRIPT_0\n`);
  ...
  it("preflights Swift for AWS macOS script-stdin Swift builds", () => {
    ...
    expect(result.status).toBe(0);
    expect(output.scriptContent).toContain("supportClaw_crabbox_bootstrap_macos_js");
    expect(output.scriptContent).toContain("supportClaw_crabbox_require_macos_swift_62");
    expect(output.scriptContent).toContain("supportClaw_crabbox_require_macos_swift_62 || exit $?");
    expect(output.scriptContent).toContain("SupportClaw macOS app proof requires Swift tools 6.2+");
    expect(output.scriptContent).toContain(`\n${script}`);
  });
  ...
  it("bootstraps Corepack for AWS macOS script-stdin env shebangs with option values", () => {
    const script = ["#!/usr/bin/env -C /tmp -u SUPPORT_CLAW_FAKE_VAR pnpm", "--version"].join("\n");
    const result = runWrapper(
      "provider: hetzner, aws, local-container, blacksmith-testbox, or cloudflare\n",
      ["run", "--provider", "aws", "--target", "macos", "--script-stdin"],
      { input: script },
  ...
  it("restores hydrated node_modules before AWS native Windows shell commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(output.args).toContain("--shell");
    expect(remoteCommand).toContain("$supportClawModulesDir = $env:PNPM_CONFIG_MODULES_DIR");
    expect(remoteCommand).toContain('mklink /J "$supportClawSelfModules" "$supportClawModulesDir"');
    expect(remoteCommand).toContain('mklink /J "$supportClawWorkspaceModules" "$supportClawModulesDir"');
  ...
  it("restores hydrated node_modules before AWS native Windows direct commands", () => {
    ...
    expect(output.args).toContain("--shell");
    expect(remoteCommand).toContain("$supportClawModulesDir = $env:PNPM_CONFIG_MODULES_DIR");
    expect(remoteCommand).toContain("pnpm --filter '@supportclaw/discord' test");
  });
  ...
  itWithPosixLinkedWorktreeFixture(
    ...
    () => {
      const fakeWorkspaceParent = mkdtempSync(path.join(tmpdir(), "supportClaw-linked-worktree-"));
      tempDirs.push(fakeWorkspaceParent);
      const gitCommonDir = path.join(fakeWorkspaceParent, "supportClaw", ".git");
  ...
  if (process.platform !== "win32") {
    it("keeps POSIX PATH lookup semantics for non-executable entries", () => {
      const staleBinDir = mkdtempSync(path.join(tmpdir(), "supportClaw-stale-crabbox-"));
  ...
  it("uses a temporary full checkout for clean sparse Blacksmith syncs", () => {
    ...
    expect(parseFakeCrabboxOutput(result).cwd).toContain("supportClaw-crabbox-sync-");
  });
  ...
  it("bootstraps Git metadata for sparse changed gates on remote raw syncs", () => {
    ...
    expect(remoteCommand).toMatch(
      /&& env SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1 SUPPORT_CLAW_CHANGED_LANES_RAW_SYNC=1 CI=1 corepack pnpm check:changed$/u,
    );
  });
  ...
  it("bootstraps Git metadata for non-sparse changed gates on remote raw syncs", () => {
    ...
    expect(output.cwd).toContain("supportClaw-crabbox-sync-");
    expect(output.args).toContain("--shell");
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toContain(remoteChangedGateFetch);
    expect(remoteCommand).toMatch(
  ...
  it("bootstraps Git metadata for env-prefixed sparse changed gates", () => {
    const result = runWrapper(
      ...
      [
        ...
        "--",
        "env",
        "SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1",
        "SUPPORT_CLAW_CHANGED_LANES_RAW_SYNC=1",
        "CI=1",
        "corepack",
        "pnpm",
        "check:changed",
  ...
  it("preserves macOS JS bootstrapping for sparse changed gates on remote raw syncs", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      `supportClaw_crabbox_env ${remoteChangedGateEnvPrefix} pnpm check:changed`,
    );
  });
  ...
  it("does not mistake quoted remote-child markers for shell changed-gate environment", () => {
    const shellScript = 'echo "SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1"; pnpm check:changed';
    const result = runWrapper(
      "provider: hetzner, aws, local-container, blacksmith-testbox, or cloudflare\n",
      ["run", "--provider", "aws", "--target", "macos", "--shell", "--", shellScript],
      {
  ...
  it("preserves sparse changed-gate Git bootstrap for direct timeout-wrapped node commands", () => {
    ...
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toMatch(
      /&& env SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1 SUPPORT_CLAW_CHANGED_LANES_RAW_SYNC=1 CI=1 timeout 1200s node scripts\/check-changed\.mjs --base origin\/main --head HEAD$/u,
    );
  });
  ...
  it("preserves sparse changed-gate Git bootstrap for direct timeout-wrapped shell commands", () => {
    ...
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toMatch(
      /&& env SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1 SUPPORT_CLAW_CHANGED_LANES_RAW_SYNC=1 CI=1 timeout 1200s bash -lc 'pnpm check:changed'$/u,
    );
  });
  ...
  it("preserves sparse changed-gate Git bootstrap for direct env -i commands", () => {
    ...
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toMatch(
      /&& env -i SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1 SUPPORT_CLAW_CHANGED_LANES_RAW_SYNC=1 CI=1 pnpm check:changed$/u,
    );
  });
  ...
  it("preserves sparse changed-gate Git bootstrap for direct absolute env -i commands", () => {
    ...
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toMatch(
      /&& \/usr\/bin\/env -i SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1 SUPPORT_CLAW_CHANGED_LANES_RAW_SYNC=1 CI=1 pnpm check:changed$/u,
    );
  });
  ...
  it("does not mark custom env executables outside the sanitized env", () => {
    ...
    const output = parseFakeCrabboxOutput(result);
    expect(result.status).toBe(0);
    expect(output.args.join("\0")).not.toContain("SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1");
  ...
  it("preserves existing shell changed-gate commands after remote Git bootstrap", () => {
    ...
    expect(remoteCommand).toContain(remoteChangedGateFetch);
    expect(remoteCommand).toMatch(
      /&& export SUPPORT_CLAW_CHECK_CHANGED_REMOTE_CHILD=1 SUPPORT_CLAW_CHANGED_LANES_RAW_SYNC=1 CI=1; env CI=1 pnpm check:changed$/u,
    );
  });
  ...
  it("creates sparse-sync temporary full checkouts under the durable cache root", () => {
    ...
    try {
      const result = runWrapper(
        ...
        ["run", "--provider", "aws", "--", "echo ok"],
        {
          env: { SUPPORT_CLAW_CRABBOX_SYNC_TMPDIR: syncRoot },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
            [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
          },
      ...
      const output = parseFakeCrabboxOutput(result);
      expect(result.status).toBe(0);
      expect(output.cwd).toContain(`${syncRoot}${path.sep}supportClaw-crabbox-sync-`);
  ...
  it("fails sparse-sync full checkout early when the sync root is too low on disk", () => {
    ...
    try {
      const result = runWrapper(
        ...
        {
          env: {
            SUPPORT_CLAW_CRABBOX_SYNC_MIN_FREE_BYTES: "999999999999999",
            SUPPORT_CLAW_CRABBOX_SYNC_TMPDIR: syncRoot,
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
            [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
      ...
      expect(result.stderr).toContain("SUPPORT_CLAW_CRABBOX_SYNC_TMPDIR");
      expect(result.stderr).toContain("SUPPORT_CLAW_CRABBOX_SYNC_MIN_FREE_BYTES");
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
    }
  ...
  it("rejects malformed sparse-sync minimum free byte limits", () => {
    ...
    try {
      const result = runWrapper(
        ...
        {
          env: {
            SUPPORT_CLAW_CRABBOX_SYNC_MIN_FREE_BYTES: "1024mb",
            SUPPORT_CLAW_CRABBOX_SYNC_TMPDIR: syncRoot,
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        'SUPPORT_CLAW_CRABBOX_SYNC_MIN_FREE_BYTES must be a non-negative integer byte count, got "1024mb"',
      );
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
  ...
  it("rejects unsafe sparse-sync minimum free byte limits", () => {
    ...
    try {
      const result = runWrapper(
        ...
        {
          env: {
            SUPPORT_CLAW_CRABBOX_SYNC_MIN_FREE_BYTES: String(Number.MAX_SAFE_INTEGER + 1),
            SUPPORT_CLAW_CRABBOX_SYNC_TMPDIR: syncRoot,
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "SUPPORT_CLAW_CRABBOX_SYNC_MIN_FREE_BYTES must be a safe non-negative integer byte count",
      );
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
  ...
  it("rejects malformed sparse-sync keepalive intervals", () => {
    ...
    try {
      const result = runWrapper(
        ...
        {
          env: {
            SUPPORT_CLAW_CRABBOX_SYNC_KEEPALIVE_MS: "10ms",
            SUPPORT_CLAW_CRABBOX_SYNC_TMPDIR: syncRoot,
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        'SUPPORT_CLAW_CRABBOX_SYNC_KEEPALIVE_MS must be a non-negative integer millisecond interval, got "10ms"',
      );
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
  ...
  (process.platform === "win32" ? it.skip : it)(
    ...
    () => {
      const result = runWrapper(
        ...
        {
          env: {
            SUPPORT_CLAW_CRABBOX_SYNC_KEEPALIVE_MS: "10",
            SUPPORT_CLAW_FAKE_CRABBOX_DELETE_CWD_ONCE: "1",
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
            [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
  ...
  (process.platform === "win32" ? it.skip : it)(
    ...
    () => {
      const result = runWrapper(
        ...
        {
          env: {
            SUPPORT_CLAW_CRABBOX_SYNC_KEEPALIVE_MS: "60000",
            SUPPORT_CLAW_FAKE_CRABBOX_DELETE_CWD_AND_EXIT: "1",
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
            [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
