  ...
  it("parses child run IDs and repair flags", () => {
    expect(
      parseReleaseVerifyBetaArgs([
        ...
        "v2026.5.10-beta.3",
        "--plugins",
        "@supportclaw/plugin-a,@supportclaw/plugin-b",
      ...
      pluginSelection: ["@supportclaw/plugin-a", "@supportclaw/plugin-b"],
    ...
    expect(
      parseNpmViewFields(
        JSON.stringify({
          ...
          "dist.tarball": "https://registry.example/supportClaw.tgz",
        }),
        "beta",
      ),
    ).toEqual({
      ...
      tarball: "https://registry.example/supportClaw.tgz",
    });
  });
...
describe("runNpmViewWithRetry", () => {
  it("retries transient registry failures with online metadata reads", async () => {
    ...
    await expect(
      runNpmViewWithRetry(["view", "supportClaw@2026.5.10-beta.3", "version", "--json"], {
        attempts: 3,
        delay: async (delayMs) => {
          delays.push(delayMs);
        },
