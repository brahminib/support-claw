...
const originalSupportClawCodexRepo = process.env.SUPPORT_CLAW_CODEX_REPO;

afterEach(() => {
  if (originalSupportClawCodexRepo === undefined) {
    delete process.env.SUPPORT_CLAW_CODEX_REPO;
  } else {
    process.env.SUPPORT_CLAW_CODEX_REPO = originalSupportClawCodexRepo;
  }
});
...
describe("codex app-server protocol source resolver", () => {
  ...
  it("allows an explicit local disk headroom override", () => {
    expect(resolveCodexProtocolMinFreeBytes({ SUPPORT_CLAW_CODEX_PROTOCOL_MIN_FREE_BYTES: "0" })).toBe(
      0,
    );
    expect(() =>
      validateCodexProtocolGenerationHeadroom({
  ...
  it("rejects malformed local disk headroom overrides", () => {
    expect(() =>
      resolveCodexProtocolMinFreeBytes({ SUPPORT_CLAW_CODEX_PROTOCOL_MIN_FREE_BYTES: "nope" }),
  ...
  it("uses SUPPORT_CLAW_CODEX_REPO when provided", async () => {
    const root = createTempDir("supportClaw-protocol-source-root-");
    const codexRepo = createTempDir("supportClaw-protocol-source-codex-");
    createProtocolSchema(codexRepo);
    process.env.SUPPORT_CLAW_CODEX_REPO = codexRepo;
  ...
  it("finds the primary checkout sibling from a git worktree", async () => {
    const parentDir = createTempDir("supportClaw-protocol-source-parent-");
    const primarySupportClaw = path.join(parentDir, "supportClaw");
    const codexRepo = path.join(parentDir, "codex");
    const worktreeRoot = createTempDir("supportClaw-protocol-source-worktree-");
    fs.mkdirSync(path.join(primarySupportClaw, ".git", "worktrees", "codex-harness"), {
      recursive: true,
    });
    fs.mkdirSync(worktreeRoot, { recursive: true });
    fs.writeFileSync(
      path.join(worktreeRoot, ".git"),
      `gitdir: ${path.join(primarySupportClaw, ".git", "worktrees", "codex-harness")}\n`,
    );
    createProtocolSchema(codexRepo);
    delete process.env.SUPPORT_CLAW_CODEX_REPO;
