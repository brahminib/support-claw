...
import { buildMarkdown, parseArgs } from "../../scripts/supportClaw-performance-source-summary.mjs";

const tmpRoots: string[] = [];

function mkTmpRoot() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-source-summary-"));
  tmpRoots.push(root);
  return root;
}
...
function runCli(...args: string[]) {
  return spawnSync(process.execPath, ["scripts/supportClaw-performance-source-summary.mjs", ...args], {
    cwd: path.resolve("."),
    encoding: "utf8",
  });
}
