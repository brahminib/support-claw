...
function writeStartupMetadataSourceSignatureFixture(rootDir: string): void {
  const fixtures = new Map<string, string>([
    ...
    ["src/cli/banner.ts", "export const banner = 'supportClaw';\n"],
    [
...
describe("write-cli-startup-metadata", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = createTempDir("supportClaw-startup-metadata-timeout-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = createTempDir("supportClaw-startup-metadata-signal-");
  ...
  it("writes startup metadata with populated root help text when dist falls back to source rendering", async () => {
    const tempRoot = createTempDir("supportClaw-startup-metadata-");
    ...
    await writeCliStartupMetadata({
      ...
      renderSourceRootHelpText: () => "Usage: supportClaw\n",
      renderSourceBrowserHelpText: () => "Usage: supportClaw browser\n",
      renderSourceSecretsHelpText: () => "Usage: supportClaw secrets\n",
      renderSourceNodesHelpText: () => "Usage: supportClaw nodes\n",
      renderSourceSubcommandHelpTextRecord: () => ({
        doctor: "Usage: supportClaw doctor\n",
        gateway: "Usage: supportClaw gateway\n",
        models: "Usage: supportClaw models\n",
        plugins: "Usage: supportClaw plugins\n",
        sessions: "Usage: supportClaw sessions\n",
        tasks: "Usage: supportClaw tasks\n",
      }),
    });
    ...
    expect(written.browserHelpText).toContain("supportClaw browser");
    expect(written.secretsHelpText).toContain("Usage:");
    expect(written.secretsHelpText).toContain("supportClaw secrets");
    expect(written.nodesHelpText).toContain("Usage:");
    expect(written.nodesHelpText).toContain("supportClaw nodes");
    expect(written.rootHelpText).toContain("Usage:");
    expect(written.rootHelpText).toContain("supportClaw");
    expect(written.subcommandHelpText.doctor).toContain("supportClaw doctor");
    expect(written.subcommandHelpText.gateway).toContain("supportClaw gateway");
    expect(written.subcommandHelpText.models).toContain("supportClaw models");
    expect(written.subcommandHelpText.plugins).toContain("supportClaw plugins");
    expect(written.subcommandHelpText.sessions).toContain("supportClaw sessions");
    expect(written.subcommandHelpText.tasks).toContain("supportClaw tasks");
  });

  it("renders independent startup help snapshots concurrently", async () => {
    const tempRoot = createTempDir("supportClaw-startup-metadata-concurrency-");
    const distDir = path.join(tempRoot, "dist");
    const extensionsDir = path.join(tempRoot, "extensions");
    const outputPath = path.join(distDir, "cli-startup-metadata.json");
    ...
    const writePromise = writeCliStartupMetadata({
      ...
      renderBundledRootHelpText: async () => "Usage: supportClaw\n",
      renderSourceBrowserHelpText: renderAfterUnblock("browser", "Usage: supportClaw browser\n"),
      renderSourceSecretsHelpText: renderAfterUnblock("secrets", "Usage: supportClaw secrets\n"),
      renderSourceNodesHelpText: renderAfterUnblock("nodes", "Usage: supportClaw nodes\n"),
  ...
  it("regenerates nodes help when bundled canvas CLI help sources change", async () => {
    const tempRoot = createTempDir("supportClaw-startup-metadata-signature-");
    const distDir = path.join(tempRoot, "dist");
    const extensionsDir = path.join(tempRoot, "extensions");
    const outputPath = path.join(distDir, "cli-startup-metadata.json");
    ...
    const writeMetadata = async (): Promise<void> => {
      await writeCliStartupMetadata({
        ...
        renderSourceNodesHelpText: () => {
          nodesRenderCount += 1;
          return `Usage: supportClaw nodes ${nodesRenderCount}\n`;
        },
        renderSourceSubcommandHelpTextRecord: () => ({
          doctor: "Usage: supportClaw doctor\n",
          gateway: "Usage: supportClaw gateway\n",
    ...
    expect(written.nodesHelpText).toContain("supportClaw nodes 3");
  });
});
