...
describe("install.ps1 failure handling", () => {
  ...
  it("keeps failure termination in the top-level completion handler", () => {
    ...
    expect(completeInstallBody).toMatch(/\bthrow "SupportClaw installation failed with exit code/);
  ...
  it("runs npm install through the resolved command with quiet CI defaults", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-SupportClaw");
  ...
  it("runs Windows command shims from a Windows-local cwd", () => {
    ...
    const supportClawPathBody = extractFunctionBody(source, "Ensure-SupportClawOnPath");
    ...
    expect(supportClawPathBody).toContain('Invoke-NpmCommand -Arguments @("config", "get", "prefix")');
    ...
    expect(mainBody).toContain('Invoke-NpmCommand -Arguments @("uninstall", "-g", "supportClaw")');
  ...
  it("rejects SupportClaw GitHub source targets for npm installs", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-SupportClaw");
    const sourceTargetBody = extractFunctionBody(source, "Test-SupportClawSourcePackageInstallSpec");
    expect(sourceTargetBody).toContain('$normalizedTag -eq "main"');
    expect(sourceTargetBody).toContain("^github:supportClaw/supportClaw");
    expect(npmInstallBody).toContain("Test-SupportClawSourcePackageInstallSpec -RequestedTag $Tag");
    expect(npmInstallBody).toContain("npm installs do not support SupportClaw GitHub source targets");
  ...
  it("preserves caller-relative local tarball install specs before safe-cwd npm calls", () => {
    const resolveSpecBody = extractFunctionBody(source, "Resolve-NpmSupportClawInstallSpec");
  ...
  it("falls back to a user-local portable Node.js bootstrap when package managers are absent", () => {
    ...
    const depsRootBody = extractFunctionBody(source, "Get-SupportClawDepsRoot");
    ...
    expect(depsRootBody).toContain("SupportClaw\\deps");
  ...
  it("persists user-local portable Git for future git-backed updates", () => {
    ...
    expect(portableGitRootBody).toContain("Get-SupportClawDepsRoot");
  ...
  runIfPowerShell("selects native ARM64 MinGit when the release publishes it", () => {
    const tempDir = harness.createTempDir("supportClaw-install-ps1-");
  ...
  it("activates the repo-pinned pnpm version for git installs", () => {
    ...
    const ensurePnpmBody = extractFunctionBody(source, "Ensure-Pnpm");
    const gitInstallBody = extractFunctionBody(source, "Install-SupportClawFromGit");
    ...
    expect(mainBody).toContain("$gitInstallResults = @(Install-SupportClawFromGit");
    expect(mainBody).toContain("Test-BooleanSuccessResult -Results $gitInstallResults");
    expect(mainBody).toContain("$npmInstallResults = @(Install-SupportClaw)");
    ...
    expect(gitInstallBody).toContain('Write-Host "[!] SupportClaw build did not produce $entryPath"');
  ...
  it("launches interactive onboarding outside Main's captured output", () => {
    const interactiveCommandBody = extractFunctionBody(source, "Invoke-InteractiveSupportClawCommand");
    const mainBody = extractFunctionBody(source, "Main");
    ...
    expect(mainBody).toContain("Invoke-InteractiveSupportClawCommand onboard");
  });
  ...
  runIfPowerShell("fails install when interactive onboarding exits non-zero", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Check-ExistingSupportClaw { return $false }",
        "function Get-NpmCommandPath { return 'npm.cmd' }",
        "function Install-SupportClaw { return $true }",
        "function Ensure-SupportClawOnPath { return $true }",
        "function Add-ToUserPath { param([string]$Path) }",
        "function Get-SupportClawCommandPath { return 'cmd.exe' }",
    ...
    expect(`${result.stdout}\n${result.stderr}`).toContain(
      "supportClaw onboard failed with exit code 17",
    );
  });
  ...
  runIfPowerShell("throws without killing the caller when run as a scriptblock", () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("caught=SupportClaw installation failed with exit code 1.");
  ...
  runIfPowerShell("treats noisy Git install false as failure", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "  return $false",
        "}",
        "function Ensure-SupportClawOnPath { throw 'should not continue after failed git install' }",
        "$InstallMethod = 'git'",
        "$GitDir = 'C:\\\\supportClaw-test'",
        "$NoOnboard = $true",
        "$result = Main",
  ...
  runIfPowerShell("keeps npm chatter out of Main's success return value", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Install-SupportClaw { Write-Output 'npm stdout'; return $true }",
        "function Ensure-SupportClawOnPath { return $true }",
        "function Refresh-GatewayServiceIfLoaded { }",
        "function Invoke-SupportClawCommand { return 'SupportClaw test-version' }",
        "$NoOnboard = $true",
        "$result = Main",
