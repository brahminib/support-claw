...
const tmpFixtureFiles = ["/tmp/supportClaw-codex-inspect.json", "/tmp/supportClaw-plugins-list.json"];
...
function writeAuthProfileStoreSqlite(agentDir: string) {
  ...
  const db = new DatabaseSync(path.join(agentDir, "supportClaw-agent.sqlite"));
  try {
    db.exec(`
...
function runCodexOnDemandAssertions(root: string) {
  return spawnSync(process.execPath, [ASSERTIONS_SCRIPT], {
    ...
    env: {
      ...
      SUPPORT_CLAW_CONFIG_PATH: path.join(root, "state", "supportClaw.json"),
...
function createCodexInstallFixture(root: string) {
  ...
  const installPath = path.join(npmRoot, "projects", "codex", "node_modules", "@supportclaw", "codex");
  const projectRoot = npmProjectRootForInstalledPackage(installPath, "@supportclaw/codex");
  writeJson(path.join(installPath, "package.json"), { name: "@supportclaw/codex" });
  ...
  writeJson(path.join(stateDir, "supportClaw.json"), {
    ...
    plugins: {
      installs: {
        codex: {
          installPath,
          source: "npm",
          spec: "npm:@supportclaw/codex",
        },
      },
    },
  });
  writeJson("/tmp/supportClaw-codex-inspect.json", {
    plugin: { id: "codex", status: "loaded", agentHarnessIds: ["codex"] },
  });
  writeJson("/tmp/supportClaw-plugins-list.json", {
...
describe("Codex install helpers", () => {
  it("resolves package roots and package manifests inside managed npm installs", () => {
    const root = makeTempDir(tempDirs, "supportClaw-codex-install-utils-");
    ...
    const projectRoot = npmProjectRootForInstalledPackage(packageRoot, "@supportclaw/codex");
  ...
  it("accepts a complete on-demand Codex npm install fixture", () => {
    const root = makeTempDir(tempDirs, "supportClaw-codex-on-demand-");
  ...
  it("rejects on-demand fixtures missing the managed @openai/codex dependency", () => {
    const root = makeTempDir(tempDirs, "supportClaw-codex-on-demand-missing-");
    createCodexInstallFixture(root);
  ...
  it("rejects on-demand fixtures missing the managed Codex executable", () => {
    const root = makeTempDir(tempDirs, "supportClaw-codex-on-demand-missing-bin-");
    createCodexInstallFixture(root);
    rmSync(
      path.join(
        root,
