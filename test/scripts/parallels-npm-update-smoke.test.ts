...
function makeTempDir(): string {
  const root = mkdtempSync(path.join(tmpdir(), "supportClaw-parallels-npm-update-"));
  tempDirs.push(root);
  return root;
}
...
function extractWindowsBackgroundControlMarkers(decoded: string): {
  ...
  return {
    done: marker("__SUPPORT_CLAW_BACKGROUND_DONE__", false),
    exitPrefix: marker("__SUPPORT_CLAW_BACKGROUND_EXIT__", true),
    lengthPrefix: marker("__SUPPORT_CLAW_LOG_LENGTH__", true),
    offsetPrefix: marker("__SUPPORT_CLAW_LOG_OFFSET__", true),
  };
}
...
describe("parallels npm update smoke", () => {
  it("accepts one prepared tarball target for update and fresh install", () => {
    expect(parseArgs(["--target-tarball", "/tmp/supportClaw-candidate.tgz"])).toMatchObject({
      targetTarball: "/tmp/supportClaw-candidate.tgz",
      updateTarget: "",
      freshTargetSpec: undefined,
    });
    expect(() =>
      parseArgs(["--target-tarball", "/tmp/supportClaw-candidate.tgz", "--update-target", "beta"]),
  ...
  it("stops the host artifact server when the wrapper fails mid-run", async () => {
    ...
    await withEnvAsync({ OPENAI_API_KEY: "test-key" }, async () => {
      const smoke = new FailingNpmUpdateSmoke({
        ...TEST_AUTH,
        json: false,
        packageSpec: "supportClaw@latest",
...
printf '%s\\n' "$*" >>"$log_path"
args=" $* "
if [[ "$args" == *" /usr/bin/tee /tmp/supportClaw-parallels-npm-update-linux-"* ]]; then
  cat >/dev/null
  exit 0
fi
if [[ "$args" == *" /bin/chmod 755 /tmp/supportClaw-parallels-npm-update-linux-"* ]]; then
  echo "chmod denied" >&2
  exit 7
fi
if [[ "$args" == *" /bin/rm -f /tmp/supportClaw-parallels-npm-update-linux-"* ]]; then
  printf 'cleanup\\n' >>"$log_path"
  exit 0
fi
exit 1
`,
      ...
      () => {
        ...
        expect(() =>
          writeGuestScript.call(
            ...
            "Linux VM",
            "echo update",
            "supportClaw-parallels-npm-update-linux",
          ),
    ...
    expect(log).toContain("/bin/chmod 755 /tmp/supportClaw-parallels-npm-update-linux-");
    expect(log).toContain("/bin/rm -f /tmp/supportClaw-parallels-npm-update-linux-");
  ...
  it("has a one-command beta validation mode with fresh target coverage", () => {
    ...
    expect(script).toContain("resolveSupportClawRegistryVersion");
    expect(script).toContain("this.options.updateTarget = version");
    expect(script).toContain("this.options.freshTargetSpec = `supportClaw@${version}`");
  ...
  it("accepts keyed and nested npm metadata for published update targets", () => {
    ...
    expect(
      parseRegistryPackageMetadata(
        JSON.stringify({
          ...
          "dist.tarball": "https://registry.example/supportClaw-keyed.tgz",
          gitHead: "abcdef0123456789",
        }),
      ),
    ).toEqual({
      version: "2026.5.20-beta.1",
      tarball: "https://registry.example/supportClaw-keyed.tgz",
      gitHead: "abcdef0123456789",
    });
    ...
    expect(
      parseRegistryPackageMetadata(
        JSON.stringify({
          version: "2026.5.20-beta.1",
          dist: { tarball: "https://registry.example/supportClaw-nested.tgz" },
        }),
      ),
    ).toEqual({
      version: "2026.5.20-beta.1",
      tarball: "https://registry.example/supportClaw-nested.tgz",
      gitHead: "",
    });
  });
  ...
  it("guards beta validation against cross-version harness checkouts", () => {
    ...
    expect(script).toContain("supportClawVersionFamily");
    expect(script).toContain("SUPPORT_CLAW_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH");
  ...
  it("bounds POSIX guest failure logs", () => {
    ...
    expect(scripts).toContain("SUPPORT_CLAW_PARALLELS_NPM_UPDATE_LOG_TAIL_BYTES");
    expect(scripts).toContain('print_log_tail "$output_file"');
    expect(scripts).toContain("print_log_tail /tmp/supportClaw-parallels-macos-gateway.log >&2");
    expect(scripts).toContain("print_log_tail /tmp/supportClaw-parallels-linux-gateway.log >&2");
    expect(scripts).not.toContain('cat "$output_file"');
    expect(scripts).not.toContain("cat /tmp/supportClaw-parallels-");
  });
  ...
  it("passes platform model timeouts to POSIX update agent turns", () => {
    ...
    withEnv(
      {
        SUPPORT_CLAW_PARALLELS_LINUX_MODEL_TIMEOUT_S: undefined,
        SUPPORT_CLAW_PARALLELS_MACOS_MODEL_TIMEOUT_S: undefined,
        SUPPORT_CLAW_PARALLELS_MODEL_TIMEOUT_S: undefined,
      },
      () => {
    ...
    withEnv(
      {
        SUPPORT_CLAW_PARALLELS_LINUX_MODEL_TIMEOUT_S: "321",
        SUPPORT_CLAW_PARALLELS_MACOS_MODEL_TIMEOUT_S: "654",
      },
      () => {
  ...
  it("sets platform-aware fresh lane timeouts", () => {
    withEnv({ SUPPORT_CLAW_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: undefined }, () => {
    ...
    withEnv({ SUPPORT_CLAW_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: "3" }, () => {
      expect(freshLaneTimeoutMs("macos")).toBe(3000);
    });

    withEnv(
      { SUPPORT_CLAW_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: String(Number.MAX_SAFE_INTEGER) },
      () => {
  ...
  it("clears update stream timers when spawning the guest command fails", async () => {
    ...
    await expect(
      runStreamingToJobLog.call(smoke, "supportClaw-definitely-missing-command", [], 60 * 60 * 1000, {
        append: () => undefined,
        logPath: "",
  ...
  it("cleans timed-out Windows background work and reads bounded log chunks", async () => {
    ...
    expect(commands).toContain("function Write-SupportClawUtf8File");
    expect(commands).toContain("[System.Text.UTF8Encoding]::new($false)");
    expect(payloads).toContain("Write-SupportClawUtf8File $exitPath '0'");
    expect(payloads).toContain("Write-SupportClawUtf8File $donePath 'done'");
    expect(commands).toContain("Write-SupportClawUtf8File $pidPath ([string]$process.Id)");
    ...
    expect(commands).toContain("Stop-SupportClawBackgroundProcessTree ([int]$backgroundPid)");
  ...
  it("does not treat Windows background log text as completion control", async () => {
    ...
    const fakeRun: typeof hostCommandRun = (_command, args) => {
      ...
      if (decoded.includes("__SUPPORT_CLAW_LOG_LENGTH__")) {
        ...
        return {
          ...
          stdout: [
            ...
            "__SUPPORT_CLAW_BACKGROUND_EXIT__:0",
            "__SUPPORT_CLAW_BACKGROUND_DONE__",
            "",
          ].join("\n"),
        };
      }
    ...
    expect(decodedCommands.join("\n")).toContain(
      "Stop-SupportClawBackgroundProcessTree ([int]$backgroundPid)",
    );
  });
  ...
  it("drains completed Windows background logs before cleanup", async () => {
    ...
    expect(decodedCommands.join("\n")).not.toContain("Stop-SupportClawBackgroundProcessTree");
    expect(decodedCommands.join("\n")).toContain(
      "Remove-Item -Path $scriptPath, $logPath, $donePath, $exitPath, $pidPath",
    );
  });
...
`,
  ...
  it("scrubs future plugin entries before invoking old same-guest updaters", () => {
    ...
    expect(script).toContain("Invoke-WithScopedEnv @{ SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS = '1'");
    expect(macosScript).toContain('SUPPORT_CLAW_BIN="$(resolve_required_command supportClaw)"');
    expect(macosScript).toContain("/usr/local/bin:/usr/local/sbin");
    expect(macosScript).toContain(
      'SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS=1 "$SUPPORT_CLAW_BIN" update --tag',
    );
    expect(macosScript).not.toContain("/opt/homebrew/bin/supportClaw");
    expect(script).toContain("SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS=1 supportClaw update --tag");
    expect(macosScript).toContain(
      'SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS=1 "$SUPPORT_CLAW_BIN" gateway stop',
    );
    expect(script).toContain(
      "SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS=1 SUPPORT_CLAW_ALLOW_ROOT=1 supportClaw gateway stop",
    );
  });
  ...
  it("reenables bundled plugins before Windows post-update verification", () => {
    ...
    const updateIndex = script.indexOf("Invoke-SupportClaw update --tag");
    const scopedIndex = script.indexOf("Invoke-WithScopedEnv @{ SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS");
    const versionIndex = script.indexOf("Invoke-SupportClaw --version", scopedIndex);
    const restartIndex = script.indexOf("Invoke-SupportClaw gateway restart");
    const agentIndex = script.indexOf("Invoke-SupportClaw agent --local");
    ...
    expect(script).not.toContain("$env:SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS = '1'");
  });
  ...
  it("generates a .NET-safe Windows stale import regex in the update-failure guard", () => {
    ...
    expect(staleImportPattern).toBe(
      String.raw`node_modules\\supportClaw\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`,
    );
    expect(staleImportPattern).not.toContain("node_modules\\supportClaw\\dist\\");
    expect(staleImportPattern.match(/\\\\/g)).toHaveLength(4);
    const representativeUpdateFailure = String.raw`Error [ERR_MODULE_NOT_FOUND]: Cannot find module 'C:\Users\runner\AppData\Roaming\npm\node_modules\supportClaw\dist\main-a1_B2.js' imported from C:\Users\runner\AppData\Roaming\npm\node_modules\supportClaw\dist\cli.js`;
    const generatedRegex = new RegExp(staleImportPattern);
    expect(generatedRegex.test(representativeUpdateFailure)).toBe(true);
    expect(generatedRegex.test(String.raw`node_modules\supportClaw\dist\main.js`)).toBe(false);
  });
});
