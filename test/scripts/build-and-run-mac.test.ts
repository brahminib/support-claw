...
function runStopExistingLocalApp(params: { fakeLsof?: string; fakePgrep: string }) {
  const root = mkdtempSync(join(tmpdir(), "supportClaw-build-run-mac-test-"));
  tempRoots.push(root);
  ...
  writeFileSync(
    ...
    [
      "#!/usr/bin/env bash",
      "set -euo pipefail",
      'BIN_ABS="/worktree/apps/macos/.build-local/debug/SupportClaw"',
      'BIN=".build-local/debug/SupportClaw"',
      'APP_CWD="/worktree/apps/macos"',
      "kill() {",
      '  printf "%s\\n" "$*" >> "$SUPPORT_CLAW_TEST_KILL_CALLS"',
      '  touch "$SUPPORT_CLAW_TEST_KILLED_MARKER"',
      "  return 0",
      "}",
      stopFunction,
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      SUPPORT_CLAW_TEST_KILLED_MARKER: join(root, "killed"),
      SUPPORT_CLAW_TEST_KILL_CALLS: killCallsPath,
      SUPPORT_CLAW_TEST_PGREP_CALLS: pgrepCallsPath,
      SUPPORT_CLAW_TEST_PGREP_COUNT: join(root, "pgrep-count.txt"),
...
describe("scripts/build-and-run-mac.sh", () => {
  ...
  it("keeps launch logs isolated unless an explicit log path is provided", () => {
    ...
    expect(script).toContain(
      'LOG_PATH="${SUPPORT_CLAW_MAC_RUN_LOG:-$(mktemp "${TMPDIR:-/tmp}/supportClaw-${PRODUCT}.XXXXXX.log")}"',
    );
    expect(script).toContain('nohup "$BIN_ABS" >"$LOG_PATH" 2>&1 &');
    expect(script).toContain('printf "Started $PRODUCT (PID $PID). Logs: $LOG_PATH\\n"');
    expect(script).not.toContain("/tmp/supportClaw.log");
  });
  ...
  it("stops only the local debug app binary before relaunching", () => {
    ...
    const { killCalls, pgrepCalls, result } = runStopExistingLocalApp({
      fakePgrep: [
        "#!/usr/bin/env bash",
        `printf '%s\\n' "$*" >> "$SUPPORT_CLAW_TEST_PGREP_CALLS"`,
        'count="$(cat "$SUPPORT_CLAW_TEST_PGREP_COUNT" 2>/dev/null || echo 0)"',
        'next="$((count + 1))"',
        'printf "%s\\n" "$next" > "$SUPPORT_CLAW_TEST_PGREP_COUNT"',
        'if [[ "$2" == "/worktree/apps/macos/.build-local/debug/SupportClaw" ]]; then exit 1; fi',
        'if [[ "$2" == ".build-local/debug/SupportClaw" && "$count" == "1" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
    ...
    expect(pgrepCalls).toContain("-f /worktree/apps/macos/.build-local/debug/SupportClaw");
    expect(pgrepCalls).toContain("-f .build-local/debug/SupportClaw");
  ...
  it("fails when the scoped local app process survives cleanup", () => {
    const { result } = runStopExistingLocalApp({
      fakePgrep: [
        "#!/usr/bin/env bash",
        'if [[ "$2" == ".build-local/debug/SupportClaw" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
