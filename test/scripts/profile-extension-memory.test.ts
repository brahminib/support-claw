...
describe("scripts/profile-extension-memory", () => {
  ...
  it("bounds noisy child output without losing RSS samples", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-extension-memory-test-"));
    try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = mkdtempSync(path.join(tmpdir(), "supportClaw-extension-memory-timeout-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = mkdtempSync(path.join(tmpdir(), "supportClaw-extension-memory-parent-signal-"));
      const hookPath = path.join(root, "rss-hook.mjs");
