// Check SupportClaw Package Tarball tests cover check supportClaw package tarball script behavior.
...
const CHECK_SCRIPT = "scripts/check-supportClaw-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "supportClaw-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "supportClaw", version }));
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><supportClaw-app></supportClaw-app>",
    ...
    const tarball = join(root, "supportClaw.tgz");
...
describe("check-supportClaw-package-tarball", () => {
  ...
  it("rejects option-like and extra arguments before tar inspection", () => {
    ...
    expect(unknown.stderr).toContain("Unknown SupportClaw package tarball check option: --tag");
    expect(unknown.stderr).not.toContain("SupportClaw package tarball does not exist");

    const extra = spawnSync("node", [CHECK_SCRIPT, "supportClaw.tgz", "extra"], {
      encoding: "utf8",
    });

    expect(extra.status).not.toBe(0);
    expect(extra.stderr).toContain("Unexpected SupportClaw package tarball check argument: extra");
    expect(extra.stderr).not.toContain("SupportClaw package tarball does not exist");
  });

  it.runIf(process.platform !== "win32")(
    "removes the extract dir when tar extraction fails",
    () => {
      const root = mkdtempSync(join(tmpdir(), "supportClaw-package-tarball-extract-fail-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.SUPPORT_CLAW_TEST_EXTRACT_DIR_FILE, outputDir);",
        ...
        const result = spawnSync("node", [CHECK_SCRIPT, tarball], {
          ...
          env: {
            ...process.env,
            SUPPORT_CLAW_TEST_EXTRACT_DIR_FILE: extractDirFile,
  ...
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("SupportClaw package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
