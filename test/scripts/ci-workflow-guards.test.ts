...
describe("ci workflow guards", () => {
  it("makes the hosted release-gate fallback explicit and exact-SHA only", () => {
    ...
    expect(readFileSync(".github/workflows/ci.yml", "utf8")).toContain(
      "SUPPORT_CLAW_CI_RUN_ANDROID: ${{ github.event_name == 'workflow_dispatch' && (inputs.release_gate || inputs.include_android) && 'true' || steps.changed_scope.outputs.run_android || 'false' }}",
    );
  });
  ...
  it("debounces canonical main pushes before Blacksmith admission", () => {
    ...
    expect(admission.steps[0].run).toContain('sleep "${SUPPORT_CLAW_MAIN_CI_DEBOUNCE_SECONDS}"');
    expect(admission.env.SUPPORT_CLAW_MAIN_CI_DEBOUNCE_SECONDS).toBe("90");
    ...
    expect(source).toContain(
      "cancel-in-progress: ${{ github.event_name == 'pull_request' || (github.event_name == 'push' && github.repository == 'supportClaw/supportClaw' && github.ref == 'refs/heads/main') }}",
    );
  });
  ...
  it("fails and retries quiet Node test shard stalls quickly", () => {
    ...
    expect(runStep.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("300000");
    expect(runStep.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_RETRY).toBe("1");
    expect(runStep.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL).toBe("2");
    expect(runStep.env.SUPPORT_CLAW_NODE_TEST_ENV_JSON).toBe("${{ toJson(matrix.env) }}");
    expect(runStep.run).toContain("env: JSON.parse(process.env.SUPPORT_CLAW_NODE_TEST_ENV_JSON");
  ...
  it("keeps push docs validation ClawHub-backed", () => {
    ...
    expect(workflow).toContain("repository: supportClaw/clawhub");
    expect(workflow).toContain("path: clawhub-source");
    expect(workflow).toContain(
      "SUPPORT_CLAW_DOCS_SYNC_CLAWHUB_REPO: ${{ github.workspace }}/clawhub-source",
    );
  });
