...
describe("QA UX Matrix evidence producer CLI", () => {
  ...
  it("reports short flag values without treating them as help", () => {
    ...
    const repoRootResult = runCli("--artifact-base", "/tmp/supportClaw-ux-test", "--repo-root", "-h");
  ...
  it("sanitizes local checkout paths from generated evidence artifacts", () => {
    const artifactBase = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-ux-evidence-test-"));
    const fakeRepoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-ux-repo-test-"));
    try {
      const result = runCli(
        "--artifact-base",
        artifactBase,
