// Resolve SupportClaw ref tests cover the release workflow ref resolver script.
import { execFileSync, spawnSync } from "node:child_process";
import { join } from "node:path";
import { afterEach, describe, expect, it } from "vitest";
import { createTempDirTracker } from "../helpers/temp-dir.js";

const SCRIPT_PATH = "scripts/github/resolve-supportClaw-ref.sh";
...
function createRemoteRepo() {
  const repo = tempDirs.make("supportClaw-ref-remote-");
...
function runResolver(remote: string, args: string[]) {
  return spawnSync("bash", [SCRIPT_PATH, ...args], {
    ...
    env: {
      ...process.env,
      GITHUB_OUTPUT: "",
      SUPPORT_CLAW_REF_REMOTE: remote,
    },
  });
}
...
describe("scripts/github/resolve-supportClaw-ref.sh", () => {
  ...
  it("writes fallback outputs for unresolved refs when a caller supplies an expected SHA", () => {
    const { repo, sha } = createRemoteRepo();
    const outputPath = join(tempDirs.make("supportClaw-ref-output-"), "github-output.txt");
  ...
  it("does not let fallback mode hide remote lookup failures", () => {
    const missingRemote = join(tempDirs.make("supportClaw-ref-missing-"), "missing.git");
