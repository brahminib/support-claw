...
function runAssertionCommand(command: string, root: string, env: Record<string, string> = {}) {
  return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, command], {
    ...
    env: {
      ...
      SUPPORT_CLAW_LIVE_PLUGIN_TOOL_AGENT_ERROR_PATH: path.join(root, "agent.err"),
      SUPPORT_CLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_PATH: path.join(root, "agent.json"),
      SUPPORT_CLAW_STATE_DIR: path.join(root, "state"),
      PLUGIN_ID: "e2e-live-plugin-tool",
      PLUGIN_NAME: "@supportclaw/e2e-live-plugin-tool",
...
describe("live plugin tool assertions", () => {
  it("rejects loose timeout env values instead of parsing numeric prefixes", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-live-plugin-tool-"));
    try {
      const result = runAssertionCommand("configure", root, {
        SUPPORT_CLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS: "1e3",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("invalid SUPPORT_CLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS: 1e3");
    } finally {
  ...
  it("writes strict positive timeout values into generated config", () => {
    ...
    try {
      const result = runAssertionCommand("configure", root, {
        SUPPORT_CLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS: "240",
      });

      expect(result.status).toBe(0);
      const config = JSON.parse(readFileSync(path.join(root, "state", "supportClaw.json"), "utf8"));
  ...
  it("bounds session transcript traversal before scanning unbounded trees", () => {
    ...
    try {
      ...
      const result = runAssertion(root, {
        SUPPORT_CLAW_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES: "2",
      });
  ...
  it("rejects oversized agent output before parsing it", () => {
    ...
    try {
      ...
      const result = runAssertion(root, {
        SUPPORT_CLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES: "1024",
      });
