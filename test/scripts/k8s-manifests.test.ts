...
describe("k8s manifests", () => {
  ...
  it("keeps gateway service selectors and ports aligned with deployment labels", () => {
    ...
    expect(deployment).toMatchObject({
      apiVersion: "apps/v1",
      kind: "Deployment",
      metadata: { name: "supportClaw" },
    });
    expect(matchLabels).toEqual({ app: "supportClaw" });
  ...
  it("keeps deployment mounts, secrets, and security posture deployable", () => {
    ...
    expect(findNamed(env, "SUPPORT_CLAW_CONFIG_DIR")).toMatchObject({ value: "/home/node/.supportClaw" });
    expect(findNamed(env, "SUPPORT_CLAW_GATEWAY_TOKEN")).toMatchObject({
      valueFrom: { secretKeyRef: { key: "SUPPORT_CLAW_GATEWAY_TOKEN", name: "supportClaw-secrets" } },
    });
    expect(findNamed(volumes, "supportClaw-home")).toMatchObject({
      persistentVolumeClaim: { claimName: "supportClaw-home-pvc" },
    });
    expect(findNamed(volumes, "config")).toMatchObject({ configMap: { name: "supportClaw-config" } });
  ...
  it("keeps config and persistence manifests aligned with the gateway", () => {
    ...
    const config = JSON.parse(String(data["supportClaw.json"])) as Record<string, unknown>;
    const gateway = asRecord(config.gateway, "supportClaw config gateway");
    const auth = asRecord(gateway.auth, "supportClaw config auth");
    const agents = asRecord(config.agents, "supportClaw config agents");
    const defaults = asRecord(agents.defaults, "supportClaw config agent defaults");
    ...
    expect(configMap).toMatchObject({
      apiVersion: "v1",
      kind: "ConfigMap",
      metadata: { name: "supportClaw-config" },
    });
    expect(gateway).toMatchObject({ mode: "local", port: 18789 });
    expect(auth).toMatchObject({ mode: "token" });
    expect(defaults).toMatchObject({ workspace: "~/.supportClaw/workspace" });
    expect(data["AGENTS.md"]).toContain("SupportClaw Assistant");
    expect(pvc).toMatchObject({
      apiVersion: "v1",
      kind: "PersistentVolumeClaim",
      metadata: { name: "supportClaw-home-pvc" },
    });
