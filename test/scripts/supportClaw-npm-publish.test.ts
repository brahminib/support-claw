// SupportClaw NPM Publish tests cover publish wrapper argument safety.
...
const scriptPath = "scripts/supportClaw-npm-publish.sh";
...
describe("supportClaw npm publish wrapper", () => {
  it("prints help without resolving release metadata", () => {
    const result = runPublishWrapper(["--help"]);

    expect(result.status).toBe(0);
    expect(result.stdout.trim()).toBe(
      "usage: bash scripts/supportClaw-npm-publish.sh --publish [package.tgz]",
    );
  ...
  it("rejects extra publish arguments before npm publish", () => {
    const tempRoot = makeTempDir("supportClaw-npm-publish-");
    const tarball = path.join(tempRoot, "supportClaw.tgz");
