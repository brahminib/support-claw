...
describe("dev tooling safety helpers", () => {
  ...
  it("redacts home paths and masks opaque ids", () => {
    expect(redactHomePath("/home/alice/.supportClaw/state.json", "/home/alice")).toBe(
      "~/.supportClaw/state.json",
    );
...
describe("script-specific dev tooling hardening", () => {
  ...
  it("rejects unknown Discord smoke args before live Discord/SupportClaw work", () => {
  ...
  it("reads TUI PTY mirror updates incrementally with a bounded chunk", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-tui-watch-test-"));
  ...
  it("rejects invalid OpenAI realtime smoke timeout values", () => {
    ...
    expect(() => realtimeSmokeTesting.resolveOpenAIHttpTimeoutMs("2s")).toThrow(
      /SUPPORT_CLAW_REALTIME_OPENAI_HTTP_TIMEOUT_MS must be an integer/u,
    );
  });
  ...
  it("reads only the bounded Anthropic prompt probe gateway log tail", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-prompt-probe-log-"));
    tempDirs.push(tempRoot);
  ...
  it("cleans Anthropic prompt probe temp dirs unless explicitly kept", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-prompt-probe-test-"));
    const keepRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-prompt-probe-test-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-direct-prompt-tree-"));
      tempDirs.push(tempRoot);
      ...
      const probe = spawn(
        ...
        ["--import", "tsx", "scripts/anthropic-prompt-probe.ts"],
        {
          ...
          env: {
            ...
            SUPPORT_CLAW_PROMPT_TEXT: "timeout cleanup proof",
            SUPPORT_CLAW_PROMPT_TIMEOUT_MS: "1000",
            SUPPORT_CLAW_PROMPT_TRANSPORT: "direct",
          },
          stdio: "ignore",
        },
      );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-direct-parent-signal-"));
      tempDirs.push(tempRoot);
      const descendantPidPath = path.join(tempRoot, "descendant.pid");
      let descendantPid = 0;
      const fakeClaudeBin = await writeFakePromptCli(tempRoot, descendantPidPath);
      const probe = spawn(
        ...
        ["--import", "tsx", "scripts/anthropic-prompt-probe.ts"],
        {
          ...
          env: {
            ...process.env,
            CLAUDE_BIN: fakeClaudeBin,
            SUPPORT_CLAW_PROMPT_TEXT: "parent signal cleanup proof",
            SUPPORT_CLAW_PROMPT_TIMEOUT_MS: "10000",
            SUPPORT_CLAW_PROMPT_TRANSPORT: "direct",
          },
          stdio: "ignore",
        },
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-prompt-gateway-tree-"));
      tempDirs.push(tempRoot);
      const descendantPidPath = path.join(tempRoot, "descendant.pid");
      let descendantPid = 0;
      const descendantScript = [
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-prompt-parent-signal-"));
      tempDirs.push(tempRoot);
      const descendantPidPath = path.join(tempRoot, "descendant.pid");
  ...
  it("rejects invalid Claude usage timeout values", () => {
    ...
    expect(() => claudeUsageTesting.resolveFetchTimeoutMs("1.5")).toThrow(
      /SUPPORT_CLAW_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS must be an integer/u,
    );
  });
