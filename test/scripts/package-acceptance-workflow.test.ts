...
const LIVE_E2E_WORKFLOW = ".github/workflows/supportClaw-live-and-e2e-checks-reusable.yml";
const LIVE_MEDIA_RUNNER_DOCKERFILE = ".github/images/live-media-runner/Dockerfile";
const LIVE_MEDIA_RUNNER_IMAGE = "ghcr.io/supportClaw/supportClaw-live-media-runner:ubuntu-24.04";
...
const RELEASE_CHECKS_WORKFLOW = ".github/workflows/supportClaw-release-checks.yml";
const RELEASE_PUBLISH_WORKFLOW = ".github/workflows/supportClaw-release-publish.yml";
const STABLE_MAIN_CLOSEOUT_WORKFLOW = ".github/workflows/supportClaw-stable-main-closeout.yml";
...
const SCHEDULED_LIVE_CHECKS_WORKFLOW = ".github/workflows/supportClaw-scheduled-live-checks.yml";
const CI_HYDRATE_LIVE_AUTH_SCRIPT = "scripts/ci-hydrate-live-auth.sh";
const VERIFY_PROVIDER_SECRETS_SCRIPT =
  ".agents/skills/release-supportClaw-ci/scripts/verify-provider-secrets.mjs";
...
describe("package acceptance workflow", () => {
  it("verifies immutable postpublish evidence before stable closeout reads it", () => {
    ...
    expect(workflow).toContain(
      'awk -v asset="supportClaw-${release_version}-stable-main-closeout.json"',
    );
  ...
  it("keeps Crabbox hydration compatible with local Actions replay", () => {
    ...
    expect(hydrateWindowsPnpm.run).toContain('"supportClaw",');
    expect(hydrateWindowsPnpm.run).toContain(
    ...
    expect(workflowText).toContain("SUPPORT_CLAW_CRABBOX_HYDRATE_DOWNLOAD_TIMEOUT_SECONDS:-300");
    expect(workflowText).toContain("SUPPORT_CLAW_CRABBOX_HYDRATE_DOWNLOAD_RETRIES:-3");
  ...
  it("resolves candidate package sources before reusing Docker E2E lanes", () => {
    ...
    expect(workflow).toContain("scripts/resolve-supportClaw-package-candidate.mjs");
    ...
    expect(workflow).toContain(
      "uses: ./.github/workflows/supportClaw-live-and-e2e-checks-reusable.yml",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain(
      "node scripts/check-supportClaw-package-tarball.mjs .artifacts/docker-e2e-package/supportClaw-current.tgz",
    );
  ...
  it("offers bounded product profiles and can run Telegram against the resolved artifact", () => {
    ...
    expect(workflow).toContain(
      "package_label: supportClaw@${{ needs.resolve_package.outputs.package_version }}",
    );
  ...
  it("requires pinned full release child workflows to run at the resolved target SHA", () => {
    ...
    expect(releaseChecksWorkflow).toContain(
      'codex_plugin_spec="npm:@supportclaw/codex@${BASH_REMATCH[1]}"',
    );
    expect(releaseChecksWorkflow).toContain(
    ...
    expect(releaseChecksWorkflow).toContain(
      "package_spec: ${{ needs.resolve_target.outputs.package_acceptance_package_spec || needs.resolve_target.outputs.release_package_spec || 'supportClaw@beta' }}",
    );
  });
...
describe("package artifact reuse", () => {
  it("lets reusable Docker E2E consume an already resolved package artifact", () => {
    ...
    expect(workflow).toContain("SUPPORT_CLAW_DOCKER_E2E_SELECTED_SHA:");
    expect(workflow).toContain(
      "SUPPORT_CLAW_UPGRADE_SURVIVOR_BASELINE_SPEC: ${{ inputs.published_upgrade_survivor_baseline }}",
    );
    expect(workflow).toContain(
      "SUPPORT_CLAW_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ matrix.group.published_upgrade_survivor_baselines || inputs.published_upgrade_survivor_baselines }}",
    );
    expect(workflow).toContain(
      "SUPPORT_CLAW_UPGRADE_SURVIVOR_SCENARIOS: ${{ inputs.published_upgrade_survivor_scenarios }}",
    );
    expect(workflow).toContain("Download current-run SupportClaw Docker E2E package");
    expect(workflow).toContain("Download previous-run SupportClaw Docker E2E package");
    ...
    expect(workflow).toContain("SUPPORT_CLAW_DOCKER_E2E_REPO_ROOT:");
    ...
    expect(workflowStep(prepareDockerImage, "Plan Docker E2E images").env).toEqual({
      ...
      SUPPORT_CLAW_UPGRADE_SURVIVOR_BASELINE_SPEC: "${{ inputs.published_upgrade_survivor_baseline }}",
      SUPPORT_CLAW_UPGRADE_SURVIVOR_BASELINE_SPECS:
    ...
    expect(workflow).toContain(
      "SUPPORT_CLAW_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ inputs.published_upgrade_survivor_baselines }}",
    );
    ...
    expect(scheduler).toContain(
      "published_upgrade_survivor_baseline=${shellQuote(process.env.SUPPORT_CLAW_UPGRADE_SURVIVOR_BASELINE_SPEC)}",
    );
    expect(scheduler).toContain(
      "published_upgrade_survivor_baselines=${shellQuote(process.env.SUPPORT_CLAW_UPGRADE_SURVIVOR_BASELINE_SPECS)}",
    );
    expect(scheduler).toContain(
      '["SUPPORT_CLAW_UPGRADE_SURVIVOR_BASELINE_SPEC", baseEnv.SUPPORT_CLAW_UPGRADE_SURVIVOR_BASELINE_SPEC]',
    );
    expect(scheduler).toContain('["SUPPORT_CLAW_UPGRADE_SURVIVOR_BASELINE_SPECS",');
    expect(scheduler).toContain('["SUPPORT_CLAW_UPGRADE_SURVIVOR_SCENARIOS",');
    expect(packageJson).toContain("SUPPORT_CLAW_UPGRADE_SURVIVOR_PUBLISHED_BASELINE=1");
    expect(packageJson).toContain("test:docker:update-restart-auth");
    expect(packageJson).toContain("SUPPORT_CLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE=auto-auth");
    expect(publishedUpgradeSurvivor).toContain("validate_baseline_package_spec");
    expect(publishedUpgradeSurvivor).toContain("SUPPORT_CLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE");
    ...
    expect(publishedUpgradeSurvivor).toContain(
      "env -u SUPPORT_CLAW_GATEWAY_TOKEN -u SUPPORT_CLAW_GATEWAY_PASSWORD supportClaw",
    );
    expect(publishedUpgradeSurvivor).toContain("phase prepare-update-restart-probe");
    expect(publishedUpgradeSurvivor).toContain("supportClaw@(alpha|beta|latest|");
    ...
    expect(publishedUpgradeSurvivor).toContain('"id": "opik-supportClaw"');
  ...
  it("bounds shared Docker image pulls so package acceptance cannot stall forever", () => {
    ...
    expect(pullHelper).toContain("SUPPORT_CLAW_DOCKER_PULL_ATTEMPTS");
    expect(pullHelper).toContain("SUPPORT_CLAW_DOCKER_PULL_TIMEOUT_SECONDS");
    expect(pullHelper).toContain('timeout_seconds="${SUPPORT_CLAW_DOCKER_PULL_TIMEOUT_SECONDS:-180}"');
    expect(pullHelper).toContain(
      'retry_delay_seconds="${SUPPORT_CLAW_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"',
    );
    expect(pullHelper).toContain('source "$SCRIPT_DIR/lib/host-timeout.sh"');
    expect(pullHelper).toContain("supportClaw_host_timeout_bin");
    ...
    expect(dockerE2ePlanAction).not.toContain('docker pull "${SUPPORT_CLAW_DOCKER_E2E_');
  });
  ...
  it("shards broad native live tests instead of one serial live-all job", () => {
    ...
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_COMMAND: ${{ matrix.command }}");
    ...
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS=deepseek,fireworks");
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS=opencode-go,openrouter");
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS=xai,zai");
    expect(workflow).toContain("inputs.live_suite_filter == 'live-gateway-advisory-docker'");
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL=claude-cli/claude-sonnet-4-6");
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_CLI_BACKEND_AUTH=api-key");
    expect(workflow).not.toContain("SUPPORT_CLAW_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG=1");
    expect(workflow).not.toContain('service_tier=\\"fast\\"');
    expect(workflow).not.toContain("SUPPORT_CLAW_LIVE_CLI_BACKEND_ARGS=");
    expect(workflow).not.toContain("SUPPORT_CLAW_LIVE_CLI_BACKEND_RESUME_ARGS=");
    expect(workflow).not.toContain(
      'SUPPORT_CLAW_LIVE_CLI_BACKEND_ARGS=["exec","--json","--color","never","--sandbox","danger-full-access","--skip-git-repo-check"]',
    );
    ...
    expect(workflow).toContain(
      "command: SUPPORT_CLAW_LIVE_CODEX_HARNESS=1 SUPPORT_CLAW_LIVE_CODEX_HARNESS_AUTH=api-key node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-core",
    );
    expect(workflow).toContain(
      "command: SUPPORT_CLAW_LIVE_CODEX_HARNESS=1 SUPPORT_CLAW_LIVE_CODEX_HARNESS_AUTH=api-key node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-backends",
    );
    expect(workflow).toContain("suite_id: native-live-src-infra");
    expect(workflow).toContain(
      "command: SUPPORT_CLAW_LIVE_APNS_REACHABILITY=1 node .release-harness/scripts/test-live-shard.mjs native-live-src-infra",
    );
    ...
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_GATEWAY_MODELS=anthropic/claude-opus-4-8");
    ...
    expect(workflow).toContain(
      "command: SUPPORT_CLAW_LIVE_GATEWAY_THINKING=off SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS=openai SUPPORT_CLAW_LIVE_GATEWAY_MODELS=openai/gpt-5.5 SUPPORT_CLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000 SUPPORT_CLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "SUPPORT_CLAW_LIVE_GATEWAY_MODELS=google/gemini-3.1-pro-preview node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-profiles",
    );
    expect(workflow).toContain(
      "SUPPORT_CLAW_LIVE_GATEWAY_MODELS=minimax/MiniMax-M2.7,minimax-portal/MiniMax-M2.7 SUPPORT_CLAW_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).not.toContain(
      "SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS=deepseek,opencode-go,openrouter,xai,zai",
    );
    expect(workflow).toContain("suite_id: live-gateway-anthropic-docker");
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_GATEWAY_MAX_MODELS=2");
    expect(workflow).toContain(
      "SUPPORT_CLAW_LIVE_GATEWAY_THINKING=off SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS=openai SUPPORT_CLAW_LIVE_GATEWAY_MODELS=openai/gpt-5.5 SUPPORT_CLAW_LIVE_GATEWAY_MAX_MODELS=1 SUPPORT_CLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS=90000 SUPPORT_CLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "SUPPORT_CLAW_LIVE_GATEWAY_MODELS=anthropic/claude-sonnet-4-6,anthropic/claude-haiku-4-5 SUPPORT_CLAW_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000");
    ...
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_SUITE_ADVISORY: ${{ matrix.advisory }}");
    expect(workflow).toContain("Advisory live suite failed with exit code");
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?SUPPORT_CLAW_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}[\s\S]*?shell: bash[\s\S]*?Advisory live suite failed with exit code/u,
    );
    expect(workflow).toMatch(
      /suite_id: live-gateway-advisory-docker-deepseek-fireworks[\s\S]*?advisory: true/u,
    );
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?SUPPORT_CLAW_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}/u,
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_VIDEO_GENERATION_PROVIDERS=google,minimax");
    expect(workflow).toContain("SUPPORT_CLAW_LIVE_VIDEO_GENERATION_PROVIDERS=openai,openrouter,xai");
    ...
    expect(retryHelper).toContain("SUPPORT_CLAW_LIVE_COMMAND_ATTEMPTS:-2");
    ...
    expect(retryHelper).toContain("SUPPORT_CLAW_LIVE_COMMAND_RATE_LIMIT_RETRY_DELAY_SECONDS:-60");
  ...
  it("runs Docker live harnesses from trusted helper scripts", () => {
    ...
    expect(workflow).toContain(
      'run: SUPPORT_CLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-models-docker.sh',
    );
    expect(workflow).toContain(
      "command: SUPPORT_CLAW_LIVE_GATEWAY_THINKING=off SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS=openai SUPPORT_CLAW_LIVE_GATEWAY_MODELS=openai/gpt-5.5 SUPPORT_CLAW_LIVE_GATEWAY_MAX_MODELS=1",
    );
    expect(workflow).toContain(
      "command: SUPPORT_CLAW_LIVE_GATEWAY_PROVIDERS=minimax,minimax-portal SUPPORT_CLAW_LIVE_GATEWAY_MODELS=minimax/MiniMax-M2.7,minimax-portal/MiniMax-M2.7 SUPPORT_CLAW_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain(
      'command: SUPPORT_CLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-cli-backend-docker.sh',
    );
    expect(workflow).toContain(
      'command: SUPPORT_CLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-acp-bind-docker.sh',
    );
    expect(workflow).toContain(
      'command: SUPPORT_CLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-codex-harness-docker.sh',
    );
    expect(workflow).toContain(
      'command: SUPPORT_CLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 20m bash .release-harness/scripts/test-live-subagent-announce-docker.sh',
    );
    ...
    expect(scenarios).toContain("process.env.SUPPORT_CLAW_DOCKER_E2E_REPO_ROOT");
    expect(scenarios).toContain(
      'harness="\\${SUPPORT_CLAW_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"',
    );
    ...
    expect(liveDockerAuth).toContain("supportClaw_live_init_docker_run_args()");
    expect(liveDockerAuth).toContain("supportClaw_live_stage_profile_into_home()");
    expect(liveDockerAuth).toContain("supportClaw_live_chown_bind_dirs_for_container_user()");
    expect(liveDockerAuth).toContain("supportClaw_live_uses_managed_bind_dirs()");
    expect(liveDockerAuth).toContain('supportClaw_live_truthy "${SUPPORT_CLAW_TESTBOX:-}"');
    expect(liveDockerAuth).toContain('[[ -n "${SUPPORT_CLAW_DOCKER_CACHE_HOME_DIR:-}" ]]');
    expect(liveDockerAuth).toContain(
      'timeout_value="${2:-${SUPPORT_CLAW_LIVE_DOCKER_RUN_TIMEOUT:-2700s}}"',
    );
    ...
    for (const script of [harness, ...sharedLiveScripts]) {
      ...
      expect(script).toContain("supportClaw_live_init_docker_run_args DOCKER_RUN_ARGS");
      expect(script).toContain("supportClaw_live_prepare_bind_dir_for_container_user");
    ...
    for (const script of sharedLiveScripts) {
      expect(script).toContain("supportClaw_live_uses_managed_bind_dirs");
    ...
    for (const script of [
      ...
      expect(script).not.toContain('timeout --kill-after=30s "${SUPPORT_CLAW_LIVE_');
    }
    expect(readFileSync("scripts/test-live-models-docker.sh", "utf8")).toContain(
      "SUPPORT_CLAW_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-gateway-models-docker.sh", "utf8")).toContain(
      "SUPPORT_CLAW_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      "SUPPORT_CLAW_LIVE_CLI_BACKEND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      'CLI_SETUP_TIMEOUT_SECONDS="$(supportClaw_live_read_positive_int_env SUPPORT_CLAW_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS 180)"',
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      'timeout_value="${SUPPORT_CLAW_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:?missing live CLI backend setup timeout seconds}s"',
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      "SUPPORT_CLAW_LIVE_ACP_BIND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      'ACP_SETUP_TIMEOUT_SECONDS="$(supportClaw_live_read_positive_int_env SUPPORT_CLAW_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS 180)"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      'timeout_value="${SUPPORT_CLAW_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:?missing live ACP bind setup timeout seconds}s"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      "SUPPORT_CLAW_LIVE_CODEX_HARNESS_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      'timeout_value="${SUPPORT_CLAW_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS:?missing live Codex harness setup timeout seconds}s"',
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-subagent-announce-docker.sh", "utf8")).toContain(
      "SUPPORT_CLAW_LIVE_SUBAGENT_DOCKER_RUN_TIMEOUT:-1200s",
    );
    expect(build).toContain('ROOT_DIR="${SUPPORT_CLAW_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"');
    ...
    expect(build).toContain('LIVE_IMAGE_PULL_ATTEMPTS="${SUPPORT_CLAW_LIVE_DOCKER_PULL_ATTEMPTS:-3}"');
    ...
    expect(stage).toContain(
      'local scripts_dir="${SUPPORT_CLAW_LIVE_DOCKER_SCRIPTS_DIR:-/src/scripts}"',
    );
  ...
  it("fails Droid ACP Docker live proof when Factory auth is missing", () => {
    ...
    expect(script).toContain("supportClaw_live_acp_bind_load_factory_api_key_from_profile");
    expect(script).not.toContain('source "$PROFILE_FILE"');
    expect(script.indexOf("supportClaw_live_acp_bind_load_factory_api_key_from_profile")).toBeLessThan(
  ...
  it("plumbs live credentials through planned Docker E2E live lanes", () => {
    ...
    const testboxProviderSecretKeys = [
      ...
      "SUPPORT_CLAW_LIVE_BROWSER_CDP_URL",
      "SUPPORT_CLAW_LIVE_SETUP_TOKEN",
      "SUPPORT_CLAW_LIVE_SETUP_TOKEN_MODEL",
      "SUPPORT_CLAW_LIVE_SETUP_TOKEN_PROFILE",
      "SUPPORT_CLAW_LIVE_SETUP_TOKEN_VALUE",
      "GEMINI_API_KEY",
      "GOOGLE_API_KEY",
      "OPENROUTER_API_KEY",
      "QWEN_API_KEY",
      ...
      "SUPPORT_CLAW_CODEX_AUTH_JSON",
      "SUPPORT_CLAW_CODEX_CONFIG_TOML",
      "SUPPORT_CLAW_CLAUDE_JSON",
      "SUPPORT_CLAW_CLAUDE_CREDENTIALS_JSON",
      "SUPPORT_CLAW_CLAUDE_SETTINGS_JSON",
      "SUPPORT_CLAW_CLAUDE_SETTINGS_LOCAL_JSON",
      "SUPPORT_CLAW_GEMINI_SETTINGS_JSON",
      "FIREWORKS_API_KEY",
    ];
    ...
    expectTextToIncludeAll(dockerPlanAction, [
      ...
      "require_any Codex SUPPORT_CLAW_CODEX_AUTH_JSON",
      'if [[ "$credentials" == *",anthropic,"* ]]; then',
      "require_any Anthropic ANTHROPIC_API_TOKEN ANTHROPIC_API_KEY SUPPORT_CLAW_CLAUDE_CREDENTIALS_JSON SUPPORT_CLAW_CLAUDE_JSON",
      'if [[ "$credentials" == *",factory,"* ]]; then',
      "require_any Factory FACTORY_API_KEY",
      'if [[ "$credentials" == *",gemini,"* ]]; then',
      "require_any Gemini GEMINI_API_KEY GOOGLE_API_KEY SUPPORT_CLAW_GEMINI_SETTINGS_JSON",
    ...
    expect(reusableWorkflow.match(/SUPPORT_CLAW_LIVE_CLI_BACKEND_AUTH=subscription/g)).toHaveLength(2);
    expect(
      reusableWorkflow.match(
        /if \[\[ -n "\$\{SUPPORT_CLAW_CLAUDE_CREDENTIALS_JSON:-\}" \|\| -n "\$\{CLAUDE_CODE_OAUTH_TOKEN:-\}" \]\]; then/g,
      ),
    ).toHaveLength(2);
  });
  ...
  it("fails Testbox changed-check delegation when the remote command fails", () => {
    ...
    expect(workflow).toContain('PNPM_CONFIG_STORE_DIR: "/tmp/supportClaw-pnpm-store"');
  ...
  it("allows the Telegram lane to run from reusable package acceptance artifacts", () => {
    ...
    expect(workflow).toContain("SUPPORT_CLAW_NPM_TELEGRAM_PACKAGE_TGZ");
  ...
  it("includes package acceptance in release checks", () => {
    ...
    expect(workflow).toContain(
      "SUPPORT_CLAW_QA_CONVEX_SITE_URL: ${{ secrets.SUPPORT_CLAW_QA_CONVEX_SITE_URL }}",
    );
    expect(workflow).toContain(
      "SUPPORT_CLAW_QA_CONVEX_SECRET_CI: ${{ secrets.SUPPORT_CLAW_QA_CONVEX_SECRET_CI }}",
    );
    ...
    expect(workflow).toContain("SUPPORT_CLAW_RELEASE_QA_*_LIVE_CI_ENABLED");
    expect(workflow).not.toContain(
  ...
  it("detects Matrix fail-fast support for older release refs", () => {
    ...
    expect(releaseWorkflow).toContain(
      'pnpm supportClaw qa matrix --help 2>/dev/null | grep -F -q -- "--fail-fast"',
    );
    expect(releaseWorkflow).toContain("matrix_args+=(--fail-fast)");
    expect(releaseWorkflow).toContain(
      'pnpm supportClaw qa matrix --output-dir "${attempt_output_dir}" "${matrix_args[@]}"',
    );
    expect(releaseWorkflow).toContain(
  ...
  it("runs live transport lanes nightly while release checks stay gated", () => {
    ...
    for (const channel of ["DISCORD", "WHATSAPP", "SLACK"]) {
      ...
      expect(releaseWorkflow).toContain(
        `RELEASE_QA_${channel}_LIVE_CI_ENABLED: \${{ vars.SUPPORT_CLAW_RELEASE_QA_${channel}_LIVE_CI_ENABLED || 'false' }}`,
      );
      ...
      expect(releaseWorkflow).not.toContain(
        `vars.SUPPORT_CLAW_RELEASE_QA_${channel}_LIVE_CI_ENABLED == 'true'`,
      );
      expect(qaWorkflow).not.toContain(`SUPPORT_CLAW_QA_${channel}_LIVE_CI_ENABLED`);
    }
  });
  ...
  it("documents the full-release Telegram package path in operator summaries", () => {
    ...
    expectTextToIncludeAll(workflow, [
      ...
      "Package Telegram E2E: SupportClaw Release Checks Package Acceptance",
  ...
  it("lets npm Telegram consume current-run or release-run package artifacts", () => {
    ...
    expectTextToIncludeAll(validateStep.run, [
      ...
      "package_spec must be supportClaw@alpha",
    ]);
    expectTextToIncludeAll(runStep.run, [
      'export SUPPORT_CLAW_NPM_TELEGRAM_PACKAGE_TGZ="${package_tgzs[0]}"',
    ]);
  });
  ...
  it("lets CI Telegram consumers wait on Convex leases instead of GitHub concurrency", () => {
    ...
    for (const [workflowPath, jobName, stepName] of telegramJobs) {
      ...
      expect(step.env?.SUPPORT_CLAW_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS).toBe("1800000");
    }
  });
  ...
  it("keeps release QA and repo E2E lanes off scarce 32-core runners", () => {
    ...
    expectTextToIncludeAll(liveE2eWorkflow, [
      "SUPPORT_CLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000",
      "SUPPORT_CLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    ]);
  });
  ...
  it("keeps release publish creation compatible with gh api and prerelease notes", () => {
    ...
    const npmWorkflow = readFileSync(".github/workflows/supportClaw-npm-release.yml", "utf8");
    const fullReleaseWorkflow = readFileSync(FULL_RELEASE_VALIDATION_WORKFLOW, "utf8");

    expect(workflow).toContain("timeout-minutes: 120");
    expect(workflow).toContain("environment: npm-release");
    expect(workflow).toContain("Download SupportClaw npm preflight manifest");
    expect(workflow).toContain("Validate SupportClaw npm preflight manifest");
    ...
    expect(workflow).toContain(
      "publish_supportClaw_npm=true requires plugin_publish_scope=all-publishable",
    );
    ...
    expect(fullReleaseWorkflow).not.toContain("node /app/supportClawCli.mjs agent");
    expect(fullReleaseWorkflow).toContain('SUPPORT_CLAW_EXTENSIONS="diagnostics-otel,codex"');
  ...
  it("keeps SupportClaw npm release pack tarball paths local before preflight upload", () => {
    const npmWorkflow = readFileSync(".github/workflows/supportClaw-npm-release.yml", "utf8");
  ...
  it("gates stable GitHub publication on the Windows Hub release asset contract", () => {
    ...
    const releaseSkill = readFileSync(
      ".agents/skills/release-supportClaw-maintainer/SKILL.md",
      "utf8",
    );

    expect(releaseWorkflow).toContain(
      "Stable SupportClaw publish requires an explicit windows_node_tag.",
    );
    expect(releaseWorkflow).toContain(
      "Stable SupportClaw publish requires candidate-approved windows_node_installer_digests.",
    );
    ...
    expect(releaseWorkflow).toContain(
      "Stable release SupportClawCompanion asset names do not exactly match the current contract",
    );
    expect(releaseWorkflow).toContain('select(.name | startswith("SupportClawCompanion-"))');
    expect(releaseWorkflow).toContain(
    ...
    expect(windowsWorkflow).toContain("must be an explicit supportClaw-windows-node release tag");
    ...
    expect(windowsWorkflow).toContain(
      "--repo supportClaw/supportClaw-windows-node --json tagName,isDraft,isPrerelease,assets,url",
    );
    expect(windowsWorkflow).toContain(
    ...
    expect(windowsWorkflow).toContain(
      "CN=SupportClaw Foundation, O=SupportClaw Foundation, L=Mill Valley, S=California, C=US",
    );
    expect(windowsWorkflow).toContain("has unexpected signer subject");
    expect(windowsWorkflow).toContain("SupportClawCompanion-SHA256SUMS.txt");
    expect(windowsWorkflow).toContain("Verify promoted release asset contract");
    expect(windowsWorkflow).toContain(
      "Promoted SupportClawCompanion asset names do not exactly match the current contract",
    );
    expect(windowsWorkflow).toContain(
  ...
  it("rejects unsafe direct Windows recovery before uploading assets", () => {
    ...
    const rejectUnexpectedTargetAssetsIndex = windowsWorkflow.indexOf(
      "Target SupportClaw release contains unexpected SupportClawCompanion assets before upload",
    );
  ...
  it("keeps beta release verification and ClawHub publish repair hooks wired", () => {
    ...
    const supportClawNpmWorkflow = readFileSync(".github/workflows/supportClaw-npm-release.yml", "utf8");
    ...
    const clawHubReleasePlanScript = readFileSync(
      "scripts/lib/supportClaw-release-clawhub-plan.ts",
      "utf8",
    );
    ...
    expect(clawHubWorkflow).toContain(
      "uses: supportClaw/clawhub/.github/workflows/package-publish.yml@9d49df109d4ad3dc8a6ecf05d26b39f46d294721",
    );
    ...
    expect(releaseWorkflow).toContain("scripts/supportClaw-release-clawhub-plan.ts");
    expect(releaseWorkflow).toContain("scripts/supportClaw-release-clawhub-runtime-state.ts");
    expect(isExecutable("scripts/supportClaw-release-clawhub-plan.ts")).toBe(true);
    expect(isExecutable("scripts/supportClaw-release-clawhub-runtime-state.ts")).toBe(true);
    expect(releaseWorkflow).toContain("supportClaw-release-clawhub-plan.json");
    expect(releaseWorkflow).toContain("supportClaw-release-clawhub-runtime-state");
    ...
    expect(releaseWorkflow).toContain("SupportClaw npm run ID");
    ...
    expect(releaseWorkflow).toContain("supportClawNpmTarball");
    expect(releaseWorkflow).not.toContain('npm view "supportClaw@${release_version}" dist.tarball');
    ...
    expect(releaseWorkflow).toContain("supportClaw-release-postpublish-evidence");
    ...
    expect(supportClawNpmWorkflow).toContain("Validate release publish approval run");
    ...
    expect(supportClawNpmWorkflow).toContain("Direct SupportClaw npm publish");
    ...
    expect(supportClawNpmWorkflow).toContain('GITHUB_ACTOR}" != "github-actions[bot]"');
    ...
    expect(supportClawNpmWorkflow).toContain("Direct SupportClaw npm recovery");
    ...
    expect(supportClawNpmWorkflow).toContain("validate-release-publish-approval.mjs");
    ...
    expect(clawHubNewWorkflow).not.toContain(
      "uses: supportClaw/clawhub/.github/workflows/package-publish.yml",
    );
    ...
    expect(clawHubNewWorkflow).toContain(
      "CLAW-277 03 - Split SupportClaw plugin ClawHub publishing into OIDC release and token bootstrap workflows",
    );
    ...
    expect(clawHubNewWorkflow).toContain(
      'SUPPORT_CLAW_CLAWHUB_MANUAL_OVERRIDE_REASON="GitHub Actions trusted publisher repair before OIDC migration"',
    );
    ...
    expect(clawHubNewWorkflow).toContain('SUPPORT_CLAW_PLUGIN_NPM_RUNTIME_BUILD: "0"');
    ...
    expect(clawHubNewWorkflow).toContain('trustedPublisher?.repository !== "supportClaw/supportClaw"');
    expect(supportClawNpmWorkflow).toContain("environment: npm-release");
  ...
  it("keeps release workflow setup and timeout budgets bounded", () => {
    ...
    const crossOs = readWorkflow(".github/workflows/supportClaw-cross-os-release-checks-reusable.yml");
    const liveE2e = readWorkflow(LIVE_E2E_WORKFLOW);
    const releaseWorkflowPaths = [
      FULL_RELEASE_VALIDATION_WORKFLOW,
      RELEASE_CHECKS_WORKFLOW,
      ".github/workflows/supportClaw-cross-os-release-checks-reusable.yml",
      LIVE_E2E_WORKFLOW,
      NPM_TELEGRAM_WORKFLOW,
      ".github/workflows/supportClaw-release-publish.yml",
      ".github/workflows/supportClaw-npm-release.yml",
