...
function makeTempRoot(): string {
  return mkdtempSync(path.join(tmpdir(), "supportClaw-fixture-config-"));
}
...
function runFixture(
  ...
  return spawnSync(process.execPath, [fixturePath, command, ...args], {
    ...
    env: {
      ...process.env,
      SUPPORT_CLAW_CONFIG_BATCH_PATH: path.join(root, "batch.json"),
      SUPPORT_CLAW_CONFIG_PATH: path.join(root, "supportClaw.json"),
      SUPPORT_CLAW_GATEWAY_TOKEN: "test-token",
      SUPPORT_CLAW_OPENWEBUI_MODEL: "openai/gpt-5.4-mini",
...
describe("scripts/e2e/lib/fixture.mjs config commands", () => {
  ...
  it("rejects loose Open WebUI provider timeout values", () => {
    ...
    try {
      const result = runFixture(root, "openwebui-config", ["test-key"], {
        SUPPORT_CLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS: "300s",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("invalid SUPPORT_CLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS: 300s");
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("writes strict positive Open WebUI provider timeouts into generated config", () => {
    ...
    try {
      const result = runFixture(root, "openwebui-config", ["test-key"], {
        SUPPORT_CLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS: "300",
      });
