...
function readDefaultPublicFunctionExportBudget() {
  ...
  const match =
    /publicFunctionExports:\s*readBudgetEnv\("SUPPORT_CLAW_PLUGIN_SDK_MAX_PUBLIC_FUNCTION_EXPORTS",\s*(\d+)\)/u.exec(
      source,
    );
...
describe("plugin SDK surface report", () => {
  it("rejects loose numeric budget env vars before collecting SDK stats", () => {
    const result = runSurfaceReport({
      SUPPORT_CLAW_PLUGIN_SDK_MAX_PUBLIC_EXPORTS: "1e9",
    });

    expect(result.status).toBe(1);
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain(
      "SUPPORT_CLAW_PLUGIN_SDK_MAX_PUBLIC_EXPORTS must be a non-negative integer",
    );
  ...
  it("rejects unsafe budget env vars before collecting SDK stats", () => {
    const result = runSurfaceReport({
      SUPPORT_CLAW_PLUGIN_SDK_MAX_PUBLIC_EXPORTS: "9007199254740992",
    });

    expect(result.status).toBe(1);
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain(
      "SUPPORT_CLAW_PLUGIN_SDK_MAX_PUBLIC_EXPORTS must be a safe non-negative integer",
    );
    expect(result.stderr).not.toContain("at ");
  });
  ...
  it("accepts exact deprecated export budget overrides by public entrypoint", () => {
    const result = runSurfaceReport({
      SUPPORT_CLAW_PLUGIN_SDK_MAX_PUBLIC_DEPRECATED_EXPORTS_BY_ENTRYPOINT: JSON.stringify({ core: 2 }),
    });
  ...
  it("keeps generated package declarations out of source surface counts", () => {
    ...
    const result = runSurfaceReport({
      SUPPORT_CLAW_PLUGIN_SDK_MAX_PUBLIC_FUNCTION_EXPORTS: String(budget - 1),
    });
  ...
  it("rejects deprecated export growth by public entrypoint", () => {
    const result = runSurfaceReport({
      SUPPORT_CLAW_PLUGIN_SDK_MAX_PUBLIC_DEPRECATED_EXPORTS_BY_ENTRYPOINT: JSON.stringify({ core: 1 }),
    });
