...
function makeFixture(bundleId: string): { root: string; script: string; logFile: string } {
  const root = mkdtempSync(path.join(os.tmpdir(), "supportClaw-ios-run-"));
  tempDirs.push(root);
...
app_dir="$derived/Build/Products/$configuration-iphonesimulator/SupportClaw.app"
mkdir -p "$app_dir"
...
describe("scripts/ios-run.sh", () => {
  ...
  it("installs and launches the configured app bundle identifier", () => {
    const fixture = makeFixture("ai.supportClawfoundation.app");

    runIosRun(fixture, { SIMCTL_BOOT_MODE: "booted" });

    expect(readFileSync(fixture.logFile, "utf8")).toContain(
      "simctl launch iPhone 17 ai.supportClawfoundation.app",
    );
  });
