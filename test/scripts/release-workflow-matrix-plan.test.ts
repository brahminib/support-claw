...
function workflow() {
  return parse(readFileSync(".github/workflows/supportClaw-live-and-e2e-checks-reusable.yml", "utf8"));
}
...
describe("scripts/plan-release-workflow-matrix.mjs", () => {
  ...
  it("wires filtered matrices into the reusable live and E2E workflow", () => {
    ...
    expect(jobs.validate_live_models_docker.env.SUPPORT_CLAW_LIVE_MODELS).toBe(
      "${{ matrix.models || 'modern' }}",
    );
    expect(jobs.validate_live_models_docker.env.SUPPORT_CLAW_LIVE_MAX_MODELS).toBe(
      "${{ matrix.max_models || '6' }}",
    );
  });
