...
describe("dependency guard workflow", () => {
  ...
  it("keeps contents write scoped to the conditional autoscrub job", () => {
    ...
    expect(detectSteps[1].env?.SUPPORT_CLAW_DEPENDENCY_GUARD_MODE).toBe("detect");
    ...
    expect(autoscrubSteps[3].env?.SUPPORT_CLAW_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN).toBe(
      "${{ steps.app-token.outputs.token || steps.app-token-fallback.outputs.token }}",
    );
    expect(autoscrubSteps[3].env?.SUPPORT_CLAW_DEPENDENCY_GUARD_MODE).toBe("autoscrub");
    expect(finalSteps[1].env?.SUPPORT_CLAW_DEPENDENCY_GUARD_MODE).toBe("enforce");
  });
  ...
  it("uses a dedicated checked-in script and bounded sticky comments", () => {
    ...
    expect(runStep.env?.SUPPORT_CLAW_SECURITY_TEAM_SLUG).toBe("supportClaw-secops");
    expect(runStep.env?.SUPPORT_CLAW_SECURITY_APPROVERS).toBe("vincentkoc,steipete,joshavant");
  ...
  it("blocks package lockfile and manifest graph changes unless secops approves the current head sha", () => {
    ...
    expect(script).toContain("supportClaw-secops");
  ...
  it("requires secops review for future workflow or guard changes", () => {
    ...
    expect(codeowners).toContain(
      "/.github/workflows/dependency-guard.yml @supportclaw/supportClaw-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/dependency-guard-workflow.test.ts @supportclaw/supportClaw-secops",
    );
    expect(codeowners).toContain("/scripts/github/dependency-guard.mjs @supportclaw/supportClaw-secops");
    expect(codeowners).toContain("/package-lock.json @supportclaw/supportClaw-secops");
    expect(codeowners).toContain("/npm-shrinkwrap.json @supportclaw/supportClaw-secops");
    expect(codeowners).toContain("/extensions/*/package-lock.json @supportclaw/supportClaw-secops");
    expect(codeowners).toContain("/extensions/*/npm-shrinkwrap.json @supportclaw/supportClaw-secops");
  });
});
