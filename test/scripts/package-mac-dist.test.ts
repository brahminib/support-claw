...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "supportClaw-dist-plist-"));
  tempDirs.push(dir);
...
describe("package-mac-dist plist validation", () => {
  it("fails closed for required Info.plist reads", () => {
    ...
    const readBlock = script.slice(
      ...
      script.indexOf('ZIP="$ROOT_DIR/dist/SupportClaw-$VERSION.zip"'),
    );
  ...
  it("prefers repo Corepack pnpm over a global pnpm shim", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "supportClaw-dist-pnpm-root-"));
    const outerRoot = mkdtempSync(path.join(tmpdir(), "supportClaw-dist-pnpm-outer-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "supportClaw-dist-pnpm-tools-"));
  ...
  it("keeps dependency bootstrap output out of captured Sparkle build values", () => {
    ...
    const dir = mkdtempSync(path.join(tmpdir(), "supportClaw-dist-sparkle-"));
    tempDirs.push(dir);
    ...
    const result = runHelper(`
      ...
      SUPPORT_CLAW_ROOT=${JSON.stringify(process.cwd())}
      SUPPORT_CLAW_MARKER=${JSON.stringify(marker)}
      PATH=${JSON.stringify(tools)}:/usr/bin:/bin
      export SUPPORT_CLAW_MARKER SUPPORT_CLAW_ROOT PATH
      ${helpers}
