  ...
  it("flags wildcard re-exports from plugin-sdk subpaths", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export * from "supportClaw/plugin-sdk/foo";',
          'export type * from "supportClaw/plugin-sdk/bar";',
          'export { named } from "supportClaw/plugin-sdk/foo";',
        ].join("\n"),
      ),
    ).toEqual([
      { line: 1, text: 'export * from "supportClaw/plugin-sdk/foo";' },
      { line: 2, text: 'export type * from "supportClaw/plugin-sdk/bar";' },
    ]);
  });
  ...
  it("allows explicit SDK exports and local wildcard barrels", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export { named } from "supportClaw/plugin-sdk/foo";',
          'export type { Named } from "supportClaw/plugin-sdk/foo";',
