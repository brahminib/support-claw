// SupportClaw Performance Workflow tests cover supportClaw performance workflow script behavior.
import { readFileSync } from "node:fs";
import { describe, expect, it } from "vitest";
import { parse } from "yaml";

const WORKFLOW = ".github/workflows/supportClaw-performance.yml";
...
describe("SupportClaw performance workflow", () => {
  it("uses an optional dispatch identifier to name parent-owned runs", () => {
    const workflow = readFileSync(WORKFLOW, "utf8");

    expect(workflow).toContain(
      "run-name: ${{ inputs.dispatch_id != '' && format('SupportClaw Performance {0}', inputs.dispatch_id) || 'SupportClaw Performance' }}",
    );
  ...
  it("uses the clawgrit reports token for every report repo push path", () => {
    ...
    expect(prepare.run).toContain(
      'remote add origin "https://x-access-token:${CLAWGRIT_REPORTS_TOKEN}@github.com/supportClaw/clawgrit-reports.git"',
    );
    expect(publish.run).toContain(
      'remote set-url origin "https://x-access-token:${CLAWGRIT_REPORTS_TOKEN}@github.com/supportClaw/clawgrit-reports.git"',
    );
