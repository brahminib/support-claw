      ...
      return (
        ...
        (params.bundledPlugin &&
          (normalized === "/repo/extensions/codex/package.json" ||
            normalized === "/repo/extensions/codex/supportClaw.plugin.json"))
      );
    },
...
describe("scripts/run-vitest", () => {
  ...
  it("restores the workspace node_modules link from a hydrated pnpm modules directory", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { PNPM_CONFIG_MODULES_DIR: "/runner/supportClaw-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/supportClaw-pnpm-node-modules/vitest/package.json",
    ...
    expect(symlinks).toEqual([
      {
        target: "/runner/supportClaw-pnpm-node-modules",
        path: "/runner/supportClaw-pnpm-node-modules/node_modules",
        type: "junction",
      },
      {
        target: "/runner/supportClaw-pnpm-node-modules",
  ...
  it("self-links hydrated pnpm modules when pnpm lowercases the env key", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { npm_config_modules_dir: "/runner/supportClaw-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/supportClaw-pnpm-node-modules/vitest/package.json",
        ...
        platform: "win32",
        requireResolve: () => "/runner/supportClaw-pnpm-node-modules/vitest/package.json",
      }),
    ).toBe("/repo/node_modules/vitest/vitest.mjs");
    expect(symlinks).toEqual([
      {
  ...
  it("keeps tooling-excluded explicit tests on existing routing", () => {
    const argv = ["run", "test/scripts/supportClaw-e2e-instance.test.ts"];
    expect(resolveImplicitVitestArgs(argv)).toBe(argv);
  });
  ...
  it("allows opting back into Maglev explicitly", () => {
    expect(
      resolveVitestNodeArgs({
        SUPPORT_CLAW_VITEST_ENABLE_MAGLEV: "1",
        PATH: "/usr/bin",
      }),
    ).toStrictEqual([]);
  });
  ...
  it("parses the optional no-output timeout env", () => {
    ...
    expect(resolveVitestNoOutputTimeoutMs({ SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500" })).toBe(
      2500,
    );
    expect(
      resolveVitestNoOutputTimeoutMs({ SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "0" }),
    ).toBeNull();
    expect(
      resolveVitestNoOutputTimeoutMs({ SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "1e3" }),
    ).toBeNull();
    expect(
      resolveVitestNoOutputTimeoutMs({ SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500ms" }),
    ).toBeNull();
  });
  ...
  it("defaults direct non-watch runs to the stall watchdog", () => {
    expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run"])).toEqual({
      PATH: "/usr/bin",
      SUPPORT_CLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
      SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "120000",
    });
    ...
    expect(
      resolveRunVitestSpawnEnv({ SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "0", PATH: "/usr/bin" }, [
        "run",
      ]),
    ).toEqual({
  ...
  it("uses a longer default stall watchdog for broad e2e and project shard configs", () => {
    ...
    for (const configArg of [
      ...
      expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run", configArg])).toEqual({
        PATH: "/usr/bin",
        SUPPORT_CLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
        SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: timeout,
      });
    }
    for (const configArg of [
      ...
      expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run", configArg])).toEqual({
        PATH: "/usr/bin",
        SUPPORT_CLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
        SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: extraLongTimeout,
      });
    }
    expect(
      resolveDefaultVitestNoOutputTimeoutMs([
  ...
  it("points symlinked source worktree Vitest runs at local bundled plugins", () => {
    expect(
      ...
      SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: nodePath.join("/repo", "extensions"),
  ...
  it("keeps explicit bundled plugin env ahead of symlinked worktree defaults", () => {
    expect(
      resolveLinkedSourceBundledPluginsEnv(
        { SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: "/custom/extensions", PATH: "/usr/bin", PWD: "/repo" },
        {
          baseDir: "/repo",
          fsImpl: createRunVitestFs({ nodeModulesSymlink: true, bundledPlugin: true }),
        },
  ...
  it("does not force the stall watchdog into delegated runner environments", () => {
    ...
    expect(
      resolveTestProjectsRunnerEnv({
        SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
        PATH: "/usr/bin",
      }),
    ).toEqual({
      SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
  ...
  posixIt("cleans delegated test-project children when the wrapper is signaled", async () => {
    ...
    const childPidPath = nodePath.join(
      os.tmpdir(),
      `supportClaw-run-vitest-delegated-child-${process.pid}-${Date.now()}.pid`,
    );
    const descendantPidPath = nodePath.join(
      os.tmpdir(),
      `supportClaw-run-vitest-delegated-descendant-${process.pid}-${Date.now()}.pid`,
    );
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "  fs.writeFileSync(process.env.SUPPORT_CLAW_DELEGATED_SIGNAL_CHILD_PID!, String(process.pid));",
        "  fs.writeFileSync(process.env.SUPPORT_CLAW_DELEGATED_SIGNAL_DESCENDANT_PID!, String(child.pid));",
        "  await new Promise(() => {});",
        "});",
        "",
      ].join("\n"),
    ...
    const runner = spawn(
      ...
      ["scripts/run-vitest.mjs", fixturePath, "--reporter=verbose"],
      {
        env: {
          ...process.env,
          SUPPORT_CLAW_DELEGATED_SIGNAL_CHILD_PID: childPidPath,
          SUPPORT_CLAW_DELEGATED_SIGNAL_DESCENDANT_PID: descendantPidPath,
        },
        stdio: "ignore",
      },
    );
  ...
  posixIt("terminates a silent Vitest child through the watchdog", async () => {
    const watched = spawnWatchedVitestProcess({
      ...
      env: { SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "100" },
    });
  ...
  it("caps native Rust worker pools for serial Vitest runs", () => {
    expect(
      resolveVitestSpawnParams(
        {
          SUPPORT_CLAW_TEST_PROJECTS_SERIAL: "1",
          PATH: "/usr/bin",
        },
        "darwin",
      ).env,
  ...
  it("keeps explicit native Rust worker pool settings", () => {
    expect(
      resolveVitestSpawnParams(
        {
          SUPPORT_CLAW_VITEST_MAX_WORKERS: "2",
          PATH: "/usr/bin",
          RAYON_NUM_THREADS: "8",
  ...
  it("does not truncate malformed native worker budgets", () => {
    expect(
      resolveVitestSpawnParams(
        {
          SUPPORT_CLAW_TEST_PROJECTS_SERIAL: "1",
          SUPPORT_CLAW_VITEST_MAX_WORKERS: "8x",
          PATH: "/usr/bin",
        },
        "darwin",
      ).env,
  ...
  it("parses the optional watchdog heartbeat interval", () => {
    expect(
      resolveVitestNoOutputHeartbeatMs({ SUPPORT_CLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "120000" }),
    ).toBe(120000);
    expect(
      resolveVitestNoOutputHeartbeatMs({ SUPPORT_CLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "0" }),
    ).toBeNull();
  });
});
