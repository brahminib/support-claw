...
const scrubbedEnvKeys = [
  ...
  "MOCK_PORT",
  "MOCK_REQUEST_LOG",
  "SUPPORT_CLAW_CONFIG_RELOAD_LOG_MAX_READ_BYTES",
  "SUPPORT_CLAW_CONFIG_RELOAD_LOG_PATH",
  "SUPPORT_CLAW_CONFIG_RELOAD_LOG_TIMEOUT_MS",
  "SUPPORT_CLAW_MOCK_OPENAI_PORT",
  "RAW_SCHEMA_ERROR",
  "SUCCESS_MARKER",
];
...
describe("e2e mock and config helper numeric limits", () => {
  it("rejects loose mock OpenAI port env values", () => {
    ...
    const fallbackPort = runScript(mockOpenAiPath, {
      SUPPORT_CLAW_MOCK_OPENAI_PORT: "44080http",
    });
    expect(fallbackPort.status).not.toBe(0);
    expect(fallbackPort.stderr).toContain("invalid SUPPORT_CLAW_MOCK_OPENAI_PORT: 44080http");
  });
  ...
  it("rejects out-of-range mock OpenAI port env values", () => {
    ...
    const fallbackPort = runScript(mockOpenAiPath, {
      SUPPORT_CLAW_MOCK_OPENAI_PORT: "65536",
    });
    expect(fallbackPort.status).not.toBe(0);
    expect(fallbackPort.stderr).toContain("invalid SUPPORT_CLAW_MOCK_OPENAI_PORT: 65536");
  });
  ...
  it("rejects loose config-reload log timeout env values", () => {
    const result = runScript(configReloadAssertPath, {
      SUPPORT_CLAW_CONFIG_RELOAD_LOG_TIMEOUT_MS: "30000ms",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid SUPPORT_CLAW_CONFIG_RELOAD_LOG_TIMEOUT_MS: 30000ms");
  });

  it("rejects loose config-reload log read caps", () => {
    const result = runScript(configReloadAssertPath, {
      SUPPORT_CLAW_CONFIG_RELOAD_LOG_MAX_READ_BYTES: "256kb",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid SUPPORT_CLAW_CONFIG_RELOAD_LOG_MAX_READ_BYTES: 256kb");
  });

  it("returns a clear error when mock OpenAI cannot append request logs", async () => {
    const requestLogDirectory = await mkdtemp(join(tmpdir(), "supportClaw-mock-request-log-"));
    try {
      await withMockServer(
        ...
        async (baseUrl, output) => {
          const response = await fetch(`${baseUrl}/v1/responses`, {
            ...
            body: JSON.stringify({ input: "SUPPORT_CLAW_E2E_OK" }),
          });
  ...
  it("returns a clear error when web-search mock cannot append request logs", async () => {
    const requestLogDirectory = await mkdtemp(join(tmpdir(), "supportClaw-web-search-log-"));
    try {
      await withMockServer(
        webSearchMockPath,
        {
          ...
          SUCCESS_MARKER: "SUPPORT_CLAW_SCHEMA_E2E_OK",
        },
        async (baseUrl, output) => {
          const response = await fetch(`${baseUrl}/v1/responses`, {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({
              input: "SUPPORT_CLAW_SCHEMA_E2E_OK",
