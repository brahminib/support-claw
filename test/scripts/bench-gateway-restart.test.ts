...
import type { DB as SupportClawStateKyselyDatabase } from "../../src/state/supportClaw-state-db.generated.js";
import {
  closeSupportClawStateDatabaseForTest,
  openSupportClawStateDatabase,
} from "../../src/state/supportClaw-state-db.js";
import { registerStopChildBehaviorTests } from "./bench-gateway-child-test-support.js";

type GatewayRestartIntentDatabase = Pick<SupportClawStateKyselyDatabase, "gateway_restart_intent">;
...
describe("gateway restart benchmark script", () => {
  ...
  it("prints help without running benchmark cases", () => {
    ...
    expect(helpResult.stdout).toContain("SupportClaw Gateway restart benchmark");
...
`),
  ...
  it("enables both startup and restart trace in the child gateway environment", () => {
    const env = testing.sanitizedEnv("/tmp/supportClaw-bench", "/tmp/supportClaw-bench/config.json", {
      config: {},
      id: "skipChannels",
      name: "gateway restart, skip channels",
    });

    expect(env.SUPPORT_CLAW_GATEWAY_STARTUP_TRACE).toBe("1");
    expect(env.SUPPORT_CLAW_GATEWAY_RESTART_TRACE).toBe("1");
    expect(env.SUPPORT_CLAW_NO_RESPAWN).toBe("1");
    expect(env.SUPPORT_CLAW_LOCAL_CHECK).toBeUndefined();
  });

  it("can pin ACPX startup probe policy per benchmark case", () => {
    const probeOffEnv = testing.sanitizedEnv(
      "/tmp/supportClaw-bench",
      "/tmp/supportClaw-bench/config.json",
      {
        config: {},
        env: { SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE: "0" },
        id: "skipChannelsNoAcpxProbe",
        name: "gateway restart, skip channels, ACPX startup probe off",
      },
    );

    expect(probeOffEnv.SUPPORT_CLAW_ACPX_RUNTIME_STARTUP_PROBE).toBe("0");
  });
  ...
  it("writes restart intent files for the target gateway pid", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-restart-bench-test-"));
    try {
      const env = { SUPPORT_CLAW_STATE_DIR: path.join(root, "state") };
  ...
  it("writes plugin fixtures as a parent load path with explicit startup activation", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-restart-bench-config-test-"));
    try {
      ...
      const manifest = JSON.parse(
        fs.readFileSync(
          path.join(root, "plugins", "bench-plugin-01", "supportClaw.plugin.json"),
          "utf8",
        ),
