...
function makeEnvMutationFixture(): string {
  const repoRoot = createTempDir("supportClaw-test-env-mutations-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
process.env.SUPPORT_CLAW_STATE_DIR = "ignored because this is not a test file";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/example.test.ts",
    `
process.env.SUPPORT_CLAW_STATE_DIR = "state";
delete process.env["SUPPORT_CLAW_CONFIG_PATH"];
vi.stubEnv("HOME", "home");
vi.stubEnv("SUPPORT_CLAW_WORKSPACE_DIR", "workspace");
process.env = { ...process.env, SUPPORT_CLAW_HOME: "home", OTHER_KEY: "ignored" };
const dynamicEnvKey = "SUPPORT_CLAW_STATE_DIR";
process.env[dynamicEnvKey] = "dynamic";
delete process.env[dynamicEnvKey];
const fixture = 'process.env.SUPPORT_CLAW_HOME = "not code"';
process.env.NOT_SUPPORT_CLAW = "ignored";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.SUPPORT_CLAW_CONFIG_PATH = "script-harness";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.SUPPORT_CLAW_AGENT_DIR = "agent";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/test-utils/supportClaw-test-state.ts",
    `
...
describe("collectTestEnvMutationReport", () => {
  it("reports active SupportClaw env mutations while ignoring strings and non-test files", () => {
    ...
    expect(
    ...
    ).toEqual([
      {
        ...
        key: "SUPPORT_CLAW_CONFIG_PATH",
        operation: "assign",
      },
      {
        file: "src/agents/auth-profiles/oauth-test-utils.ts",
        key: "SUPPORT_CLAW_AGENT_DIR",
        operation: "assign",
      },
      { file: "src/example.test.ts", key: "SUPPORT_CLAW_STATE_DIR", operation: "assign" },
      { file: "src/example.test.ts", key: "SUPPORT_CLAW_CONFIG_PATH", operation: "delete" },
      { file: "src/example.test.ts", key: "HOME", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "SUPPORT_CLAW_WORKSPACE_DIR", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "SUPPORT_CLAW_HOME", operation: "replace" },
  ...
  it("renders a non-blocking text report for active findings", () => {
    ...
    expect(rendered).toContain("SupportClaw test env mutation report");
    ...
    expect(rendered).toContain("L2 SUPPORT_CLAW_STATE_DIR assign process.env");
    ...
    for (const limit of ["1e3", ""]) {
      const result = spawnSync(
        ...
        [
          ...
          limit,
          "--repo-root",
          createTempDir("supportClaw-env-limit-"),
        ],
        {
          encoding: "utf8",
        },
