...
describe("scripts/test-docker-all scheduler", () => {
  ...
  it("prints CLI help without a stack trace", () => {
    ...
    expect(result.stdout).toContain("SUPPORT_CLAW_DOCKER_ALL_* env vars");
  });
  ...
  it("rejects loose numeric runner env vars without a stack trace", () => {
    const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs", "--plan-json"], {
      ...
      env: {
        ...process.env,
        SUPPORT_CLAW_DOCKER_ALL_PARALLELISM: "1e3",
      },
    });

    expect(result.status).toBe(1);
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain("SUPPORT_CLAW_DOCKER_ALL_PARALLELISM must be a positive integer");
    expect(result.stderr).not.toContain("at ");
  });

  it("rejects loose numeric resource limit env vars before scheduling lanes", () => {
    const logDir = mkdtempSync(`${tmpdir()}/supportClaw-docker-all-`);
    try {
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...process.env,
          SUPPORT_CLAW_DOCKER_ALL_BUILD: "0",
          SUPPORT_CLAW_DOCKER_ALL_DOCKER_LIMIT: "1e3",
          SUPPORT_CLAW_DOCKER_ALL_DRY_RUN: "1",
          SUPPORT_CLAW_DOCKER_ALL_LOG_DIR: logDir,
          SUPPORT_CLAW_DOCKER_ALL_PREFLIGHT: "0",
          SUPPORT_CLAW_DOCKER_ALL_TIMINGS: "0",
        },
      });

      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "SUPPORT_CLAW_DOCKER_ALL_DOCKER_LIMIT must be a positive integer",
      );
      expect(result.stderr).not.toContain("at ");
    } finally {
  ...
  it("rejects release-path configs that schedule zero Docker lanes", () => {
    ...
    try {
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...process.env,
          SUPPORT_CLAW_DOCKER_ALL_CHUNK: "openwebui",
          SUPPORT_CLAW_DOCKER_ALL_DRY_RUN: "1",
          SUPPORT_CLAW_DOCKER_ALL_INCLUDE_OPENWEBUI: "0",
          SUPPORT_CLAW_DOCKER_ALL_LOG_DIR: logDir,
          SUPPORT_CLAW_DOCKER_ALL_PREFLIGHT: "0",
  ...
  posixIt("writes Docker run artifacts when cleanup smoke fails", () => {
    const root = mkdtempSync(`${tmpdir()}/supportClaw-docker-all-cleanup-`);
    const logDir = path.join(root, "logs");
    const packageTgz = path.join(root, "supportClaw-current.tgz");
...
`,
    ...
    try {
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...process.env,
          SUPPORT_CLAW_CURRENT_PACKAGE_TGZ: packageTgz,
          SUPPORT_CLAW_DOCKER_ALL_BUILD: "0",
          SUPPORT_CLAW_DOCKER_ALL_LIVE_MODE: "skip",
          SUPPORT_CLAW_DOCKER_ALL_LOG_DIR: logDir,
          SUPPORT_CLAW_DOCKER_ALL_PARALLELISM: "16",
          SUPPORT_CLAW_DOCKER_ALL_PREFLIGHT: "0",
          SUPPORT_CLAW_DOCKER_ALL_START_STAGGER_MS: "0",
          SUPPORT_CLAW_DOCKER_ALL_STATUS_INTERVAL_MS: "0",
          SUPPORT_CLAW_DOCKER_ALL_TAIL_PARALLELISM: "16",
          SUPPORT_CLAW_DOCKER_ALL_TIMINGS: "0",
  ...
  it("cleans stale stopped containers from all named Docker E2E lanes", () => {
    expect(
      dockerPreflightContainerNames(`
supportClaw-gateway-e2e-123 Exited (1) 2 minutes ago
supportClaw-config-reload-e2e-234 Created
supportClaw-plugin-binding-command-escape-e2e-345 Dead
supportClaw-kitchen-sink-rpc-e2e-456 Exited (137) 10 seconds ago
supportClaw-openwebui-gateway-567 Exited (1) 3 minutes ago
supportClaw-openwebui-678 Created
supportClaw-not-an-e2e-container Exited (1) 2 minutes ago
postgres Created
`),
    ).toEqual([
      "supportClaw-gateway-e2e-123",
      "supportClaw-config-reload-e2e-234",
      "supportClaw-plugin-binding-command-escape-e2e-345",
      "supportClaw-kitchen-sink-rpc-e2e-456",
      "supportClaw-openwebui-gateway-567",
      "supportClaw-openwebui-678",
    ]);
  });
  ...
  it("reads bounded lane log tails instead of full noisy logs", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-docker-all-log-tail-"));
    try {
  ...
  posixIt("kills timed-out shell command groups when the leader exits first", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-docker-all-timeout-"));
...
`,
  ...
  posixIt("clamps oversized shell command kill grace before scheduling", async () => {
    const root = createTempDir("supportClaw-docker-all-oversized-grace-");
    const scriptPath = path.join(root, "leader-exits.mjs");
...
`,
  ...
  posixIt("lets timed-out shell command descendants exit during kill grace", async () => {
    const root = createTempDir("supportClaw-docker-all-grace-");
    const scriptPath = path.join(root, "leader-exits.mjs");
    const donePath = path.join(root, "done");
    const readyPath = path.join(root, "ready");
    const childScript = [
...
`,
  ...
  posixIt("lets timed-out shell capture descendants exit during kill grace", async () => {
    const root = createTempDir("supportClaw-docker-all-capture-grace-");
    const scriptPath = path.join(root, "leader-exits.mjs");
    const donePath = path.join(root, "done");
    const readyPath = path.join(root, "ready");
    const childScript = [
...
`,
  ...
  posixIt("cleans active shell command groups before parent signal exit", async () => {
    const root = createTempDir("supportClaw-docker-all-parent-signal-");
