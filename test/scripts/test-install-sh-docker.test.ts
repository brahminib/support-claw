...
const PODMAN_QUADLET_TEMPLATE_PATH = "scripts/podman/supportClaw.container.in";
const PODMAN_RUN_PATH = "scripts/run-supportClaw-podman.sh";
...
const RELEASE_CHECKS_WORKFLOW_PATH = ".github/workflows/supportClaw-release-checks.yml";
const LIVE_E2E_WORKFLOW_PATH = ".github/workflows/supportClaw-live-and-e2e-checks-reusable.yml";
...
function normalizeInstallE2eAgentOutput(output: string) {
  const root = mkdtempSync(join(tmpdir(), "supportClaw-install-e2e-agent-output-"));
...
describe("test-install-sh-docker", () => {
  it("defaults ARM hosts to native arm64 while keeping x64 CI on amd64", () => {
    ...
    expect(
      runDefaultSmokePlatform({ SUPPORT_CLAW_INSTALL_SMOKE_PLATFORM: "linux/s390x" }, "x86_64"),
    ).toBe("linux/s390x");
  });
  ...
  it("supports npm update package specs without a separate expected-version env", () => {
    ...
    expect(script).toContain(
      'UPDATE_EXPECT_VERSION="${SUPPORT_CLAW_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"',
    );
  ...
  it("uses npm latest as the update baseline and resolves it to the concrete packed version", () => {
    ...
    expect(script).toContain(
      'UPDATE_BASELINE_VERSION="${SUPPORT_CLAW_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"',
    );
    ...
    expect(runner).toContain(
      'UPDATE_BASELINE_VERSION="${SUPPORT_CLAW_INSTALL_UPDATE_BASELINE:-latest}"',
    );
    ...
    expect(workflow).toContain(
      "SUPPORT_CLAW_INSTALL_SMOKE_UPDATE_BASELINE: ${{ inputs.update_baseline_version || 'latest' }}",
    );
  });
  ...
  it("keeps shared install helpers parsing and verifying installed CLI versions", () => {
    const root = tempDirs.make("supportClaw-install-helper-");
    const binDir = join(root, "bin");
    mkdirSync(binDir, { recursive: true });
    writeFileSync(
      join(binDir, "supportClaw"),
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        'case "${1:-}" in',
        "  --version)",
        "    printf 'SupportClaw v2026.6.21-beta.1\\r\\n'",
        "    ;;",
        "  --help)",
        "    printf 'usage\\n'",
        "    ;;",
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          "printf 'parsed=%s\\n' \"$(extract_supportClaw_semver 'SupportClaw v2026.6.21-beta.1+build.7')\"",
          "verify_installed_cli supportClaw 2026.6.21-beta.1",
        ].join("\n"),
      ],
      {
        cwd: process.cwd(),
    ...
    expect(result.stdout).toContain(
      "cli=supportClaw installed=2026.6.21-beta.1 expected=2026.6.21-beta.1",
    );
  ...
  it("can reuse dist from the already-built root Docker smoke image", () => {
    ...
    expect(script).toContain('UPDATE_DIST_IMAGE="${SUPPORT_CLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"');
    ...
    expect(script).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${SUPPORT_CLAW_INSTALL_SMOKE_DOCKER_COMMAND_TIMEOUT:-600s}}"',
    );
  ...
  it("bounds installer smoke container runs", () => {
    ...
    expect(script).toContain(
      'INSTALL_SMOKE_DOCKER_RUN_TIMEOUT="${SUPPORT_CLAW_INSTALL_SMOKE_DOCKER_RUN_TIMEOUT:-2700s}"',
    );
  ...
  it("passes the baked browser build arg through Docker setup", () => {
    ...
    expect(script).toContain('export SUPPORT_CLAW_INSTALL_BROWSER="${SUPPORT_CLAW_INSTALL_BROWSER:-}"');
    expect(script).toContain("SUPPORT_CLAW_INSTALL_BROWSER \\");
    expect(script).toContain('--build-arg "SUPPORT_CLAW_INSTALL_BROWSER=${SUPPORT_CLAW_INSTALL_BROWSER}"');
  });
  ...
  it("bounds Docker setup image pulls", () => {
    ...
    expect(script).toContain('DOCKER_PULL_TIMEOUT="${SUPPORT_CLAW_DOCKER_SETUP_PULL_TIMEOUT:-600s}"');
    expect(script).toContain("run_docker_pull()");
    expect(script).toContain(
      'supportClaw_host_timeout_cmd "$DOCKER_PULL_TIMEOUT" docker pull "$image"',
    );
  ...
  it("bounds Podman setup image pulls", () => {
    ...
    expect(script).toContain('PODMAN_PULL_TIMEOUT="${SUPPORT_CLAW_PODMAN_SETUP_PULL_TIMEOUT:-600s}"');
    expect(script).toContain("run_podman_pull()");
    expect(script).toContain(
      'supportClaw_host_timeout_cmd "$PODMAN_PULL_TIMEOUT" podman pull "$image"',
    );
    expect(script).toContain('run_podman_pull "$SUPPORT_CLAW_IMAGE"');
    expect(script).not.toContain('podman pull "$SUPPORT_CLAW_IMAGE"');
  });
  ...
  it("bounds Podman setup image builds", () => {
    ...
    expect(script).toContain(
      'PODMAN_BUILD_TIMEOUT="${SUPPORT_CLAW_PODMAN_SETUP_BUILD_TIMEOUT:-1800s}"',
    );
    expect(script).toContain("run_podman_build()");
    expect(script).toContain('supportClaw_host_timeout_cmd "$PODMAN_BUILD_TIMEOUT" podman build "$@"');
    expect(script).toContain('run_podman_build -t "$SUPPORT_CLAW_IMAGE"');
    expect(script).not.toContain('podman build -t "$SUPPORT_CLAW_IMAGE"');
  });

  it("bounds detached Podman launches without timing out onboarding", () => {
    const script = readFileSync(PODMAN_RUN_PATH, "utf8");

    expect(script).toContain('PODMAN_RUN_TIMEOUT="${SUPPORT_CLAW_PODMAN_RUN_TIMEOUT:-600s}"');
    expect(script).toContain("SUPPORT_CLAW_PODMAN_RUN_TIMEOUT|SUPPORT_CLAW_PODMAN_GATEWAY_HOST_PORT");
    expect(script).toContain('source "$SCRIPT_DIR/lib/host-timeout.sh"');
    expect(script).toContain("run_podman_detached()");
    expect(script).toContain('supportClaw_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"');
  ...
  it("passes image-scoped pip packages through Docker and Podman setup", () => {
    ...
    expect(dockerfile).toContain("ARG SUPPORT_CLAW_IMAGE_PIP_PACKAGES");
    expect(dockerfile).toContain(
      "python3 -m pip install --no-cache-dir --break-system-packages $SUPPORT_CLAW_IMAGE_PIP_PACKAGES",
    );
    expect(dockerSetup).toContain(
      'export SUPPORT_CLAW_IMAGE_PIP_PACKAGES="${SUPPORT_CLAW_IMAGE_PIP_PACKAGES:-}"',
    );
    expect(dockerSetup).toContain("SUPPORT_CLAW_IMAGE_PIP_PACKAGES \\");
    expect(dockerSetup).toContain(
      '--build-arg "SUPPORT_CLAW_IMAGE_PIP_PACKAGES=${SUPPORT_CLAW_IMAGE_PIP_PACKAGES}"',
    );
    expect(dockerSetup).not.toContain("SUPPORT_CLAW_DOCKER_PIP_PACKAGES");
    expect(podmanSetup).toContain('SUPPORT_CLAW_IMAGE_PIP_PACKAGES="${SUPPORT_CLAW_IMAGE_PIP_PACKAGES:-}"');
    expect(podmanSetup).toContain(
      'BUILD_ARGS+=(--build-arg "SUPPORT_CLAW_IMAGE_PIP_PACKAGES=${SUPPORT_CLAW_IMAGE_PIP_PACKAGES}")',
    );
    expect(podmanSetup).not.toContain("SUPPORT_CLAW_DOCKER_PIP_PACKAGES");
  });
  ...
  it("keeps the Podman Quadlet template aligned with setup substitutions", () => {
    ...
    expect(setupScript).toContain(
      'QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/supportClaw.container.in"',
    );
    for (const placeholder of [
      "SUPPORT_CLAW_CONFIG_DIR",
      "SUPPORT_CLAW_WORKSPACE_DIR",
      "IMAGE_NAME",
      "CONTAINER_NAME",
    ]) {
    ...
    expect(template).toContain("Volume={{SUPPORT_CLAW_CONFIG_DIR}}:/home/node/.supportClaw:Z");
    expect(template).toContain(
      "Volume={{SUPPORT_CLAW_WORKSPACE_DIR}}:/home/node/.supportClaw/workspace:Z",
    );
    expect(template).toContain("EnvironmentFile={{SUPPORT_CLAW_CONFIG_DIR}}/.env");
  ...
  it("allows repository branch history and release tags for secret-backed Docker release checks", () => {
    ...
    expect(workflow).toContain("reachable from an SupportClaw branch or release tag");
  });
  ...
  it("rejects path-like npm pack tarball filenames in update smoke metadata", () => {
    expect(runReadPackTarballFilename("supportClaw-2026.6.17.tgz")).toMatchObject({
      status: 0,
      stdout: "supportClaw-2026.6.17.tgz",
    });

    const unsafeFilenames = [
      "../supportClaw.tgz",
      "nested/supportClaw.tgz",
      "nested\\supportClaw.tgz",
      "/tmp/supportClaw.tgz",
      "C:\\temp\\supportClaw.tgz",
      "supportClaw.tar.gz",
    ];
  ...
  it("writes the package dist inventory before packing ignore-scripts tarballs", () => {
    ...
    expect(script).toContain("node scripts/check-supportClaw-package-tarball.mjs");
  });
  ...
  it("runs candidate tarballs through the installer script instead of direct npm", () => {
    ...
    expect(wrapper).toContain('-v "$ROOT_DIR/scripts/install.sh:/tmp/supportClaw-install.sh:ro"');
  ...
  it("uses public npm latest as the non-root installer expectation", () => {
    ...
    expect(wrapper).toContain('-e SUPPORT_CLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION"');
  });
});
...
describe("install-sh E2E runner", () => {
  it("normalizes Docker wrapper timing and toggle knobs before forwarding", () => {
    ...
    expect(wrapper).toContain(
      'SESSION_SCAN_BYTES="$(\n  docker_e2e_read_positive_int_env SUPPORT_CLAW_INSTALL_E2E_SESSION_SCAN_BYTES 16777216\n)"',
    );
    expect(wrapper).toContain(
      'SESSION_LINE_BYTES="$(\n  docker_e2e_read_positive_int_env SUPPORT_CLAW_INSTALL_E2E_SESSION_LINE_BYTES 1048576\n)"',
    );
    expect(wrapper).toContain(
      'SESSION_SCAN_DEPTH="$(docker_e2e_read_positive_int_env SUPPORT_CLAW_INSTALL_E2E_SESSION_SCAN_DEPTH 64)"',
    );
    expect(wrapper).toContain(
      'SESSION_SCAN_NODES="$(docker_e2e_read_positive_int_env SUPPORT_CLAW_INSTALL_E2E_SESSION_SCAN_NODES 100000)"',
    );
    expect(wrapper).toContain(
      '-e SUPPORT_CLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS="$OPENAI_PROVIDER_TIMEOUT_SECONDS"',
    );
    expect(wrapper).toContain(
      '-e SUPPORT_CLAW_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS="$AGENT_TURN_TIMEOUT_SECONDS"',
    );
    expect(wrapper).toContain(
      '-e SUPPORT_CLAW_INSTALL_E2E_AGENT_TURNS_PARALLEL="$AGENT_TURNS_PARALLEL"',
    );
    expect(wrapper).toContain('-e SUPPORT_CLAW_INSTALL_E2E_AGENT_TOOL_SMOKE="$AGENT_TOOL_SMOKE"');
    expect(wrapper).toContain('-e SUPPORT_CLAW_INSTALL_E2E_SESSION_SCAN_BYTES="$SESSION_SCAN_BYTES"');
    expect(wrapper).toContain('-e SUPPORT_CLAW_INSTALL_E2E_SESSION_LINE_BYTES="$SESSION_LINE_BYTES"');
    expect(wrapper).toContain('-e SUPPORT_CLAW_INSTALL_E2E_SESSION_SCAN_DEPTH="$SESSION_SCAN_DEPTH"');
    expect(wrapper).toContain('-e SUPPORT_CLAW_INSTALL_E2E_SESSION_SCAN_NODES="$SESSION_SCAN_NODES"');
    expect(wrapper).not.toContain(
      'SUPPORT_CLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS="${SUPPORT_CLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS:-}"',
    );
  });

  it.each([
    ["turn timeout", "SUPPORT_CLAW_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS", "300s"],
    ["provider timeout", "SUPPORT_CLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS", "1e3"],
    ["parallel toggle", "SUPPORT_CLAW_INSTALL_E2E_AGENT_TURNS_PARALLEL", "2"],
    ["tool smoke toggle", "SUPPORT_CLAW_INSTALL_E2E_AGENT_TOOL_SMOKE", "false"],
    ["session scan bytes", "SUPPORT_CLAW_INSTALL_E2E_SESSION_SCAN_BYTES", "16mb"],
    ["session line bytes", "SUPPORT_CLAW_INSTALL_E2E_SESSION_LINE_BYTES", "1mb"],
    ["session scan depth", "SUPPORT_CLAW_INSTALL_E2E_SESSION_SCAN_DEPTH", "0"],
    ["session scan nodes", "SUPPORT_CLAW_INSTALL_E2E_SESSION_SCAN_NODES", "100k"],
  ...
  it("validates agent timing and toggle knobs before running provider setup", () => {
    ...
    expect(script).toContain(
      'OPENAI_PROVIDER_TIMEOUT_SECONDS="$(read_positive_int_env SUPPORT_CLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS "$AGENT_TURN_TIMEOUT_SECONDS")"',
    );
  ...
  it.each([
    ...
    expect(result.status).toBe(2);
    expect(result.stderr).toContain(`invalid ${envName}: ${value}`);
    expect(result.stderr).not.toContain("SUPPORT_CLAW_E2E_MODELS=both requires");
  });
});
...
describe("install-sh smoke runner", () => {
  it("wraps long npm/update operations with heartbeat and install-size audits", () => {
    ...
    expect(script).toContain(
      'HEARTBEAT_INTERVAL="$(read_nonnegative_int_env SUPPORT_CLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL 60)"',
    );
    expect(script).toContain(
      'INSTALL_COMMAND_TIMEOUT="$(read_positive_int_env SUPPORT_CLAW_INSTALL_SMOKE_COMMAND_TIMEOUT 900)"',
    );
    ...
    expect(script).toContain("supportClaw update --tag");
  ...
  it.each([
    ["command timeout", "SUPPORT_CLAW_INSTALL_SMOKE_COMMAND_TIMEOUT", "900s"],
    ["heartbeat interval", "SUPPORT_CLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL", "60s"],
    ...
    expect(result.status).toBe(2);
    expect(result.stderr).toContain(`invalid ${envName}: ${value}`);
    expect(result.stderr).not.toContain("unsupported SUPPORT_CLAW_INSTALL_SMOKE_MODE");
  });
  ...
  it("covers plain npm global installs and npm-driven updates", () => {
    ...
    expect(script).toContain('SKIP_NPM_GLOBAL="${SUPPORT_CLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"');
    expect(script).toContain('NPM_CACHE_DIR="${SUPPORT_CLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"');
    ...
    expect(script).toContain("SUPPORT_CLAW_INSTALL_SMOKE_MODE=npm-global");
  ...
  it("forwards smoke-runner control knobs into Docker containers", () => {
    ...
    for (const envName of [
      "SUPPORT_CLAW_INSTALL_ALLOW_LEGACY_UPDATE_WARNING",
      "SUPPORT_CLAW_INSTALL_SELF_UPDATE_WARNING_FIXED_VERSION",
      "SUPPORT_CLAW_INSTALL_SMOKE_COMMAND_TIMEOUT",
      "SUPPORT_CLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL",
      "SUPPORT_CLAW_INSTALL_SMOKE_PREVIOUS",
      "SUPPORT_CLAW_INSTALL_SMOKE_SKIP_PREVIOUS",
    ]) {
...
describe("bun global install smoke", () => {
  it("packs the current tree and verifies image-provider discovery through Bun", () => {
    ...
    expect(script).toContain("SUPPORT_CLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE");
    expect(script).toContain('source "$ROOT_DIR/scripts/lib/docker-e2e-container.sh"');
    expect(script).toContain(
      'COMMAND_TIMEOUT_MS="$(read_positive_int_env SUPPORT_CLAW_BUN_GLOBAL_SMOKE_TIMEOUT_MS 180000)"',
    );
    expect(script).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${SUPPORT_CLAW_BUN_GLOBAL_SMOKE_DOCKER_COMMAND_TIMEOUT:-600s}}"',
    );
    expect(script).toContain('container_id="$(docker_e2e_docker_cmd create "$image")"');
    expect(script).toContain(
  ...
  it("rejects invalid Bun global install command timeouts before Bun setup", () => {
    const result = spawnSync("bash", [BUN_GLOBAL_SMOKE_PATH], {
      ...
      env: {
        ...process.env,
        SUPPORT_CLAW_BUN_GLOBAL_SMOKE_TIMEOUT_MS: "180000ms",
      },
    });

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid SUPPORT_CLAW_BUN_GLOBAL_SMOKE_TIMEOUT_MS: 180000ms");
  ...
  it("rejects path-like npm pack tarball filenames in Bun smoke metadata", () => {
    const safeResult = runResolvePackTarballPath("supportClaw-2026.6.17.tgz");

    expect(safeResult.status).toBe(0);
    expect(safeResult.stdout).toMatch(/\/supportClaw-2026\.6\.17\.tgz$/u);
  ...
  it.runIf(process.platform !== "win32" && existsSync("/usr/bin/time"))(
    ...
    () => {
      const tempDir = tempDirs.make("supportClaw-bun-global-timeout-grace-");
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            SUPPORT_CLAW_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS: "1000",
          },
          timeout: 5_000,
        },
      );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = tempDirs.make("supportClaw-bun-global-parent-signal-");
      const readyPath = path.join(tempDir, "ready");
      ...
      const runner = spawn(
        ...
        ],
        {
          env: {
            ...process.env,
            SUPPORT_CLAW_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS: "100",
          },
  ...
  it("gates workflow Bun install smoke to scheduled and release-check runs", () => {
    ...
    expect(workflow).toContain(
      "SUPPORT_CLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE: ${{ needs.root_dockerfile_image.outputs.image_ref }}",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain("SUPPORT_CLAW_CI_WORKFLOW_BUN_GLOBAL_INSTALL_SMOKE");
    ...
    expect(workflow).toContain("SUPPORT_CLAW_INSTALL_URL: file:///tmp/supportClaw-install.sh");
    expect(workflow).toContain("SUPPORT_CLAW_INSTALL_CLI_URL: file:///tmp/supportClaw-install-cli.sh");
    expect(workflow).toContain('SUPPORT_CLAW_INSTALL_SMOKE_SKIP_CLI: "0"');
    ...
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/supportClaw-cli");
    ...
    expect(workflow).toContain('SUPPORT_CLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL: "1"');
  ...
  it("kills Bun global install smoke commands that ignore TERM after timeout", () => {
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          SUPPORT_CLAW_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS: "50",
        },
        timeout: 5000,
      },
    );
