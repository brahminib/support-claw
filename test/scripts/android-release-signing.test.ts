...
function makeTempRoot(): string {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-android-signing-"));
    ...
    `${JSON.stringify(
      {
        ...
        gradlePropertyNames: [
          "SUPPORT_CLAW_ANDROID_STORE_FILE",
          "SUPPORT_CLAW_ANDROID_STORE_PASSWORD",
          "SUPPORT_CLAW_ANDROID_KEY_ALIAS",
          "SUPPORT_CLAW_ANDROID_KEY_PASSWORD",
        ],
      },
      null,
      2,
...
function writeSigningSources(tempRoot: string) {
  ...
  fs.writeFileSync(
    propertiesPath,
    [
      `SUPPORT_CLAW_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`,
      "SUPPORT_CLAW_ANDROID_KEY_ALIAS=supportClaw-upload",
      `SUPPORT_CLAW_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`,
      "",
    ].join("\n"),
  );
...
describe("scripts/android-release-signing.mjs", () => {
  ...
  it("documents the canonical Android release signing plan", () => {
    ...
    expect(result.stdout).toContain("Signing repo: git@github.com:supportClaw/apps-signing.git");
    expect(result.stdout).toContain("Signing assets: android/supportClaw");
  ...
  it.runIf(commandAvailable("openssl"))(
    ...
    () => {
      ...
      const encryptedProperties = fs.readFileSync(
        path.join(remoteCheck, "android", "supportClaw", "gradle.properties.enc"),
        "utf8",
      );
      ...
      expect(materializedProperties).toContain(
        `SUPPORT_CLAW_ANDROID_STORE_FILE=${path.join(materializedDir, "upload-keystore.jks")}`,
      );
      expect(materializedProperties).toContain(`SUPPORT_CLAW_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`);
      expect(materializedProperties).toContain("SUPPORT_CLAW_ANDROID_KEY_ALIAS=supportClaw-upload");
      expect(materializedProperties).toContain(`SUPPORT_CLAW_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`);
