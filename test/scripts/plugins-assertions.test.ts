...
function writeFixtureServerShims(binDir: string, pidPath: string): void {
  ...
  writeFileSync(
    ...
    [
      "#!/bin/bash",
      'printf "%s\\n" "$$" >"$SUPPORT_CLAW_TEST_FIXTURE_SERVER_PID"',
      "trap 'exit 0' TERM",
...
describe("plugins Docker assertions", () => {
  it("rejects loose ClawHub preflight limits instead of parsing prefixes", () => {
    const timeoutResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-preflight"], {
      ...
      env: {
        ...process.env,
        CLAWHUB_PLUGIN_SPEC: "clawhub:@supportclaw/kitchen-sink",
        SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
      },
    });
    expect(timeoutResult.status).not.toBe(0);
    expect(timeoutResult.stderr).toContain(
      "invalid SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: 1e3",
    );
    ...
    const bodyLimitResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-preflight"], {
      ...
      env: {
        ...process.env,
        CLAWHUB_PLUGIN_SPEC: "clawhub:@supportclaw/kitchen-sink",
        SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: "1000bytes",
      },
    });
    expect(bodyLimitResult.status).not.toBe(0);
    expect(bodyLimitResult.stderr).toContain(
      "invalid SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: 1000bytes",
    );
  });
  ...
  it("keeps sweep artifact paths aligned with the assertion scratch root", () => {
    ...
    for (const scriptPath of scripts) {
      ...
      const scriptWithoutDefaultScratch = script.replace(
        'mktemp -d "/tmp/supportClaw-plugins.XXXXXX"',
        "",
      );
      expect(script).toContain("SUPPORT_CLAW_PLUGINS_TMP_DIR");
      expect(scriptWithoutDefaultScratch).not.toMatch(
        /\/tmp\/(?:plugins|marketplace|demo-plugin|is-number|supportClaw-plugin|supportClaw-clawhub)/,
      );
    }
  });

  it("cleans the default plugin sweep scratch root", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-plugin-sweep-cleanup-"));
    const marker = path.join(root, "scratch-path.txt");
    try {
      const result = runPluginsSweepShell(
        `
set -euo pipefail
export SUPPORT_CLAW_PLUGINS_SWEEP_SOURCE_ONLY=1
source scripts/e2e/lib/plugins/sweep.sh
printf '%s\\n' "$SUPPORT_CLAW_PLUGINS_TMP_DIR" > "$MARKER"
test -d "$SUPPORT_CLAW_PLUGINS_TMP_DIR"
cleanup_supportClaw_plugins_sweep
test ! -e "$SUPPORT_CLAW_PLUGINS_TMP_DIR"
`,
        { MARKER: marker },
      );
      ...
      expect(scratchRoot).toContain("/tmp/supportClaw-plugins.");
  ...
  it("preserves caller-provided plugin sweep scratch roots", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-plugin-sweep-caller-"));
...
set -euo pipefail
export SUPPORT_CLAW_PLUGINS_SWEEP_SOURCE_ONLY=1
export SUPPORT_CLAW_PLUGINS_TMP_DIR="$SCRATCH_ROOT"
source scripts/e2e/lib/plugins/sweep.sh
test -d "$SUPPORT_CLAW_PLUGINS_TMP_DIR"
cleanup_supportClaw_plugins_sweep
test -d "$SUPPORT_CLAW_PLUGINS_TMP_DIR"
`,
  ...
  it("scans plugin assertion logs without echoing whole files on failure", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-plugin-update-log-"));
    try {
      ...
      const pass = await runAssertionAsync(["plugin-dir-update-skipped"], {
        SUPPORT_CLAW_PLUGINS_TMP_DIR: passRoot,
      });
      ...
      const fail = await runAssertionAsync(["plugin-dir-update-skipped"], {
        SUPPORT_CLAW_PLUGINS_TMP_DIR: failRoot,
      });
      ...
      writeFileSync(
        path.join(invalidRoot, "plugins-invalid-supportClaw-extensions.log"),
        `supportClaw.extensions[1]\n${"x".repeat(256 * 1024)}\nmissing validation tail`,
        "utf8",
      );
      writeJson(path.join(invalidRoot, "plugins-invalid-supportClaw-extensions-list.json"), {
        plugins: [],
      });
      const invalid = await runAssertionAsync(["invalid-supportClaw-extensions"], {
        HOME: invalidHome,
        SUPPORT_CLAW_PLUGINS_TMP_DIR: invalidRoot,
      });
  ...
  it("cleans npm fixture registry children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-plugin-npm-fixture-cleanup-"));
    try {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            SUPPORT_CLAW_TEST_FIXTURE_SERVER_PID: pidPath,
  ...
  it("force-kills stubborn npm fixture registry children during cleanup", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-plugin-npm-fixture-kill-"));
    try {
      const binDir = path.join(root, "bin");
      const fixtureDir = path.join(root, "fixture");
      const pidPath = path.join(root, "server.pid");
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            SUPPORT_CLAW_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2",
            SUPPORT_CLAW_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: "0.05",
            SUPPORT_CLAW_TEST_FIXTURE_SERVER_PID: pidPath,
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          "set -euo pipefail",
          "source scripts/e2e/lib/plugins/fixtures.sh",
          "supportClaw_plugins_signal_fixture_process() { echo signal; }",
          "supportClaw_plugins_fixture_process_alive() { echo probe; return 1; }",
          "set +e",
          "supportClaw_plugins_stop_fixture_process 12345",
          'status="$?"',
          "set -e",
          'exit "$status"',
        ].join("\n"),
      ],
      {
        ...
        env: {
          ...process.env,
          SUPPORT_CLAW_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2x",
        },
      },
    );

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid SUPPORT_CLAW_PLUGINS_FIXTURE_STOP_ATTEMPTS: 2x");
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          SUPPORT_CLAW_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2",
          SUPPORT_CLAW_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: "soon",
        },
      },
    );

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid SUPPORT_CLAW_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: soon");
    expect(result.stdout).not.toContain("signal");
    expect(result.stdout).not.toContain("probe");
  });

  it("bounds npm fixture registry logs when readiness fails", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-plugin-npm-fixture-log-"));
    try {
      const binDir = path.join(root, "bin");
      const fixtureDir = path.join(root, "fixture");
      mkdirSync(fixtureDir);
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            SUPPORT_CLAW_DOCKER_E2E_LOG_PRINT_BYTES: "80",
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
      );
  ...
  it("keeps npm fixture registry alive after malformed package paths", async () => {
    ...
    const root = makeTempDir(tempDirs, "supportClaw-plugin-npm-fixture-request-");
    ...
    try {
      ...
      const valid = await requestFixtureRegistry(port, "/@supportclaw%2Fdemo-plugin-npm");

      expect(valid.statusCode, stderr.text()).toBe(200);
      expect(JSON.parse(valid.body)).toMatchObject({
        name: "@supportclaw/demo-plugin-npm",
  ...
  it("rejects invalid plugin fixture log byte limits before npm fixture setup", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-plugin-npm-fixture-log-invalid-"));
    try {
      const binDir = path.join(root, "bin");
      const fixtureDir = path.join(root, "fixture");
      mkdirSync(binDir, { recursive: true });
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            SUPPORT_CLAW_DOCKER_E2E_LOG_PRINT_BYTES: "64kb",
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
      );
      ...
      expect(result.status).toBe(2);
      expect(result.stderr).toContain("invalid SUPPORT_CLAW_DOCKER_E2E_LOG_PRINT_BYTES: 64kb");
  ...
  it("cleans ClawHub fixture children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-plugin-clawhub-fixture-cleanup-"));
    try {
      const binDir = path.join(root, "bin");
      const cleanupPath = path.join(root, "caller-cleanup");
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            SUPPORT_CLAW_PLUGINS_E2E_LIVE_CLAWHUB: "0",
            SUPPORT_CLAW_PLUGINS_TMP_DIR: tmpDir,
            SUPPORT_CLAW_TEST_FIXTURE_SERVER_PID: pidPath,
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
  ...
  it("rejects invalid plugin fixture log byte limits before ClawHub fixture setup", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-plugin-clawhub-fixture-log-invalid-"));
    try {
      const binDir = path.join(root, "bin");
      const tmpDir = path.join(root, "scratch");
      mkdirSync(binDir, { recursive: true });
  ...
  it("bounds ClawHub fixture server logs when readiness fails", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-plugin-clawhub-fixture-log-"));
    try {
      const binDir = path.join(root, "bin");
      const tmpDir = path.join(root, "scratch");
      mkdirSync(tmpDir);
  ...
  it("uses the configured scratch root and resolves Windows home-relative install paths", () => {
    const root = mkdtempSync(path.join(tmpdir(), "supportClaw-plugins-assertions-"));
    ...
    try {
      ...
      writeJson(path.join(home, ".supportClaw", "plugins", "installs.json"), {
        installRecords: {
          "demo-plugin-tgz": {
            source: "archive",
      ...
      const result = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "plugin-tgz"], {
        ...
        env: {
          ...
          HOME: home,
          SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
          SUPPORT_CLAW_PLUGINS_TMP_DIR: scratchRoot,
        },
      });
  ...
  it("compares local plugin source paths by canonical path", () => {
    ...
    const installPath = path.join(home, ".supportClaw", "extensions", "demo-plugin-dir");
  ...
  it("still requires archive managed install directories to be removed", () => {
    ...
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const installPath = path.join(home, ".supportClaw", "extensions", "demo-plugin-tgz");
    mkdirSync(installPath, { recursive: true });
  ...
  it("rejects unreadable config during plugin uninstall proof", () => {
    ...
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const removedInstallPath = path.join(home, ".supportClaw", "extensions", "demo-plugin-tgz");
    ...
    try {
      ...
      writeFileSync(path.join(home, ".supportClaw", "supportClaw.json"), "{ malformed\n", "utf8");
      ...
      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("failed to read SupportClaw config");
    } finally {
      rmSync(root, { force: true, recursive: true });
    }
  });
  ...
  it("rejects ClawHub install paths that resolve outside the managed extensions root", () => {
    const tempDirs: string[] = [];
    const root = makeTempDir(tempDirs, "supportClaw-plugins-clawhub-path-");
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const extensionsRoot = path.join(home, ".supportClaw", "extensions");
    ...
    try {
      writeJson(path.join(scratchRoot, "plugins-clawhub-installed.json"), {
        plugins: [{ id: "supportClaw-kitchen-sink-fixture", status: "loaded" }],
      });
      writeJson(path.join(scratchRoot, "plugins-clawhub-inspect.json"), {
        plugin: { id: "supportClaw-kitchen-sink-fixture" },
      });
      writeJson(path.join(home, ".supportClaw", "plugins", "installs.json"), {
        installRecords: {
          "supportClaw-kitchen-sink-fixture": {
            artifactFormat: "zip",
            artifactKind: "legacy-zip",
            clawhubFamily: "code-plugin",
            clawhubPackage: "@supportclaw/kitchen-sink",
            installPath: escapedInstallPath,
            source: "clawhub",
            spec: "clawhub:@supportclaw/kitchen-sink",
          },
        },
      });
      ...
      const result = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-installed"], {
        ...
        env: {
          ...process.env,
          CLAWHUB_PLUGIN_ID: "supportClaw-kitchen-sink-fixture",
          CLAWHUB_PLUGIN_SPEC: "clawhub:@supportclaw/kitchen-sink",
          HOME: home,
          SUPPORT_CLAW_PLUGINS_TMP_DIR: scratchRoot,
        },
  ...
  it("times out stalled ClawHub package metadata requests", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        CLAWHUB_PLUGIN_ID: "supportClaw-kitchen-sink-fixture",
        CLAWHUB_PLUGIN_SPEC: "clawhub:@supportclaw/kitchen-sink",
        SUPPORT_CLAW_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "25",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight for @supportclaw/kitchen-sink timed out after 25ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
  ...
  it("times out stalled ClawHub package metadata bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        ...
        CLAWHUB_PLUGIN_SPEC: "clawhub:@supportclaw/kitchen-sink",
        SUPPORT_CLAW_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "75",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight response for @supportclaw/kitchen-sink timed out after 75ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
        server.close(() => resolve());
  ...
  it("bounds ClawHub package metadata response bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        ...
        CLAWHUB_PLUGIN_SPEC: "clawhub:@supportclaw/kitchen-sink",
        SUPPORT_CLAW_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: "16",
        SUPPORT_CLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1000",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight response for @supportclaw/kitchen-sink response body exceeded 16 bytes",
      );
