...
} from "@supportclaw/normalization-core/number-coercion";
import { afterEach, beforeAll, describe, expect, it, vi } from "vitest";
import {
  extractLastSupportClawVersionFromLog,
...
describe("Parallels smoke model selection", () => {
  ...
  it("extracts the last SupportClaw version from a bounded log tail", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-log-tail-"));
    const logPath = join(tempDir, "phase.log");
    try {
      writeFileSync(logPath, ["SupportClaw 0.0.1", "x".repeat(4096), "SupportClaw 2026.6.7"].join("\n"));

      await expect(extractLastSupportClawVersionFromLog(logPath, undefined, 128)).resolves.toBe(
        "2026.6.7",
      );
    } finally {
  ...
  beforeAll(() => {
    ...
    invalidModelTimeoutResult = spawnNodeEvalSync(
      `process.env.SUPPORT_CLAW_PARALLELS_MACOS_MODEL_TIMEOUT_S = "1800s"; const { resolveParallelsModelTimeoutSeconds } = await import("./${TS_PATHS.common}"); resolveParallelsModelTimeoutSeconds("macos");`,
      { env: process.env, imports: ["tsx"] },
    );
    ...
    invalidLinuxAgentTimeoutResult = spawnNodeEvalSync(
      `process.env.SUPPORT_CLAW_PARALLELS_LINUX_AGENT_TIMEOUT_S = "1e3"; process.argv = ["node", "${TS_PATHS.linux}"]; await import("./${TS_PATHS.linux}");`,
      { env: process.env, imports: ["tsx"] },
    );
    invalidWindowsAgentTimeoutResult = spawnNodeEvalSync(
      `process.env.SUPPORT_CLAW_PARALLELS_WINDOWS_AGENT_TIMEOUT_S = "2700s"; process.argv = ["node", "${TS_PATHS.windows}"]; await import("./${TS_PATHS.windows}");`,
      { env: process.env, imports: ["tsx"] },
    );
    invalidWindowsUpdateTimeoutResult = spawnNodeEvalSync(
      `process.env.SUPPORT_CLAW_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S = "12.5"; process.argv = ["node", "${TS_PATHS.windows}"]; await import("./${TS_PATHS.windows}");`,
      { env: process.env, imports: ["tsx"] },
    );
  });
  ...
  it("accepts leading package-manager separators and still honors later terminators", () => {
    ...
    expect(parseNpmUpdateSmokeArgs(["--", "--package-spec", "supportClaw@2026.5.1"]).packageSpec).toBe(
      "supportClaw@2026.5.1",
    );
    expect(
  ...
  it("keeps provider auth and model defaults in the shared TypeScript helper", () => {
    ...
    expect(providerAuth).toContain("SUPPORT_CLAW_PARALLELS_OPENAI_MODEL");
    expect(providerAuth).toContain("SUPPORT_CLAW_PARALLELS_WINDOWS_OPENAI_MODEL");
  ...
  it("keeps snapshot, host, package, and quote helpers shared", () => {
    ...
    expect(packageArtifact).toContain("export async function packSupportClaw");
  ...
  it("keeps fresh package locks with malformed owner pids", async () => {
    const lockDir = makeTempDir(tempDirs, "supportClaw-parallels-package-lock-");
  ...
  it("removes a just-created package lock when owner writing fails", async () => {
    const parentDir = makeTempDir(tempDirs, "supportClaw-parallels-package-lock-parent-");
  ...
  it("uses a temporary npmrc file and cleans it after resolving the latest package version", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "supportClaw-parallels-version-"));
    let userConfigPath = "";
    try {
      const version = resolveLatestVersion("", {
        createTempDir: (prefix) => {
          expect(prefix).toBe(join(tmpdir(), "supportClaw-npm-"));
          return mkdtempSync(join(tempRoot, "npm-"));
        },
        runCommand: (command, args, options) => {
          userConfigPath = args.at(-1) ?? "";
          expect(command).toBe("npm");
          expect(args).toEqual(["view", "supportClaw", "version", "--userconfig", userConfigPath]);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-host-server-"));
...
`,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-host-server-signal-"));
      const fakePython = join(tempDir, "python3");
      writeFileSync(
        fakePython,
        `#!/usr/bin/env bash
...
`,
  ...
  it("quotes shell args and resolves fuzzy snapshot hints through the shared TypeScript helper", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-helper-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("resolves a latest snapshot hint to the matching version before older LATEST labels", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-snapshot-latest-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("rejects skip-restore for combined Parallels smoke lanes", () => {
    ...
    expect(invalidSkipBothResult.stderr).toContain(
      "SUPPORT_CLAW_PARALLELS_SKIP_SNAPSHOT_RESTORE=1 requires --mode fresh or --mode upgrade",
    );
    expect(() =>
  ...
  it("uses one Ubuntu VM fallback resolver for Linux lanes", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-vm-helper-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("uses the only macOS VM when the default name is unavailable", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-macos-vm-helper-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("does not infer destructive macOS smoke targets from arbitrary names", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-macos-vm-guard-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("resumes suspended Parallels VMs", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-vm-resume-"));
...
`,
  ...
  it("keeps Linux bad-plugin diagnostics gated for historical update baselines", () => {
    ...
    expect(script).toContain("parseSupportClawPackageVersion");
      ...
      withEnv(
        {
          ...
          SUPPORT_CLAW_PARALLELS_WINDOWS_OPENAI_MODEL: "openai/custom-windows",
        },
        () => resolveWindowsProviderAuth({ provider: "openai" }),
      ),
    ).toEqual({
  ...
  it("cleans POSIX guest scripts after the phase deadline is exhausted", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-posix-cleanup-"));
...
printf '%s\\n' "$*" >>"$log_path"
args=" $* "
if [[ "$args" == *" dd of=/tmp/supportClaw-parallels-"* || "$args" == *" /bin/dd of=/tmp/supportClaw-parallels-"* ]]; then
  cat >/dev/null
  exit 0
fi
if [[ "$args" == *" bash /tmp/supportClaw-parallels-"* || "$args" == *" /bin/bash /tmp/supportClaw-parallels-"* ]]; then
  exit 1
fi
if [[ "$args" == *" /bin/rm -f /tmp/supportClaw-parallels-"* ]]; then
  printf 'cleanup\\n' >>"$log_path"
  exit 0
fi
exit 0
...
`,
  ...
  it("streams full phase logs to disk while bounding the failure tail", async () => {
    const runDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-phase-"));
  ...
  it("clamps oversized phase timers before scheduling", async () => {
    const runDir = makeTempDir(tempDirs, "supportClaw-parallels-phase-timeout-");
    const phaseRunner = new PhaseRunner(runDir, 128);
  ...
  it("preseeds dev update channel before stable-to-dev update lanes", () => {
    ...
    expect(macos).toContain("SUPPORT_CLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1");
    expect(windows).toContain("SUPPORT_CLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS");
  });
  ...
  it("passes aggregate model overrides into each OS fresh lane", () => {
    ...
    expect(script).toContain("SUPPORT_CLAW_PARALLELS_LINUX_DISABLE_BONJOUR");
  });
  ...
  it("keeps the Windows update config scrub compatible with PowerShell 5.1", () => {
    ...
    expect(script).toContain("function Get-SupportClawJsonProperty");
    expect(script).toContain("function Remove-SupportClawJsonProperty");
    expect(script).toContain("Remove-SupportClawJsonProperty $entries $pluginId");
  });
  ...
  it("keeps aggregate update guest scripts isolated from the npm-update orchestrator", () => {
    ...
    expect(updateScripts).toContain("Invoke-SupportClaw update");
  ...
  it("resolves macOS smoke commands from the guest PATH", () => {
    ...
    expect(macos).toContain('const guestSupportClaw = "supportClaw"');
    expect(macos).toContain('const guestNode = "node"');
    expect(macos).toContain('const guestNpm = "npm"');
    expect(macos).toContain("$(npm root -g)/supportClaw/supportClawCli.mjs");
    expect(macos).toContain("guestSupportClawEntryExec");
    expect(macos).not.toContain('const guestSupportClaw = "/opt/homebrew/bin/supportClaw"');
    ...
    expect(macos).not.toContain("/opt/homebrew/lib/node_modules/supportClaw/supportClawCli.mjs");
  });

  it("keeps Windows gateway reachability on a real deadline with start recovery", () => {
    const script = readFileSync(TS_PATHS.windows, "utf8");

    expect(script).toContain("SUPPORT_CLAW_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S");
  ...
  it("runs Windows ref onboarding through a detached done-file runner", () => {
    ...
    expect(transports).toContain("__SUPPORT_CLAW_BACKGROUND_DONE__");
    expect(transports).toContain("__SUPPORT_CLAW_BACKGROUND_EXIT__");
    expect(transports).toContain("__SUPPORT_CLAW_LOG_OFFSET__");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const tempDir = makeTempDir(tempDirs, "supportClaw-parallels-host-command-drain-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-host-command-"));
...
`,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = makeTempDir(tempDirs, "supportClaw-parallels-host-command-signal-");
      ...
      try {
        const grandchildScript = [
          "const { writeFileSync } = require('node:fs');",
          "writeFileSync(process.env.SUPPORT_CLAW_TEST_GRANDCHILD_PID, String(process.pid));",
          "process.on('SIGTERM', () => {});",
          "setInterval(() => {}, 1000);",
        ].join("\n");
        const parentScript = [
          ...
          "writeFileSync(process.env.SUPPORT_CLAW_TEST_READY_FILE, 'ready');",
          "process.on('SIGTERM', () => process.exit(0));",
          "setInterval(() => {}, 1000);",
        ].join("\n");
        writeFileSync(
          ...
          [
            ...
            "  env: {",
            "    ...process.env,",
            `    SUPPORT_CLAW_TEST_GRANDCHILD_PID: ${JSON.stringify(grandchildPidPath)},`,
            `    SUPPORT_CLAW_TEST_READY_FILE: ${JSON.stringify(readyPath)},`,
            "  },",
            "  quiet: true,",
            "  timeoutMs: 30_000,",
            "});",
  ...
  it.runIf(process.platform !== "win32")("preserves timed host command spawn errors", () => {
    expect(() =>
      run("supportClaw-definitely-missing-host-command", [], {
        check: false,
        quiet: true,
        timeoutMs: 50,
      }),
  ...
  it("rejects streaming host commands when log writes fail", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "supportClaw-parallels-host-command-log-"));
    try {
      await expect(
  ...
  it("clears streaming host command timers when spawn fails", async () => {
    ...
    try {
      await expect(
        runStreaming("supportClaw-definitely-missing-host-command", [], {
          quiet: true,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = makeTempDir(tempDirs, "supportClaw-parallels-streaming-host-command-drain-");
      const readyFile = join(tempDir, "ready");
      const drainFile = join(tempDir, "drained");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = makeTempDir(tempDirs, "supportClaw-parallels-streaming-host-command-signal-");
      const runnerPath = join(tempDir, "runner.mjs");
      const readyPath = join(tempDir, "ready");
      const grandchildPidPath = join(tempDir, "grandchild.pid");
      const logPath = join(tempDir, "stream.log");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const result = run(
        process.execPath,
        ["-e", "process.stderr.write('__SUPPORT_CLAW_HOST_COMMAND_SPAWN_ERROR__{}\\n')"],
        {
          check: false,
          quiet: true,
          timeoutMs: 500,
  ...
  it("routes Windows host pnpm and npm shims through safe runners", () => {
    ...
    expect(
      resolveHostCommandInvocation("npm", ["view", "supportClaw", "version"], {
        env: { ComSpec: comSpec },
        execPath,
      ...
      args: ["/d", "/s", "/c", `${npmCmdPath} view supportClaw version`],
      command: comSpec,
      shell: false,
      windowsVerbatimArguments: true,
    });
  ...
  it("runs the Windows agent turn through the detached done-file runner", () => {
    ...
    expect(script).toContain('guestPowerShellBackground(\n      "agent-turn"');
    expect(script).toContain("SUPPORT_CLAW_PARALLELS_WINDOWS_AGENT_TIMEOUT_S");
    expect(script).toContain(
      'readPositiveIntEnv(\n    "SUPPORT_CLAW_PARALLELS_WINDOWS_AGENT_TIMEOUT_S"',
    );
  ...
  it("gives GPT-5.5 enough Parallels model time on slower desktop guests", () => {
    ...
    expect(readFileSync(TS_PATHS.macos, "utf8")).toContain(
      'this.agentTimeoutSeconds = readPositiveIntEnv("SUPPORT_CLAW_PARALLELS_MACOS_AGENT_TIMEOUT_S", 2700)',
    );
  ...
  it("rejects loose Parallels numeric limits before starting smoke lanes", () => {
    expect(
      withEnv({ SUPPORT_CLAW_PARALLELS_MODEL_TIMEOUT_S: "1200" }, () =>
        resolveParallelsModelTimeoutSeconds("linux"),
      ),
    ).toBe(1200);
    expect(
      withEnv({ SUPPORT_CLAW_PARALLELS_NUMERIC_TEST: " 42 " }, () =>
        readPositiveIntEnv("SUPPORT_CLAW_PARALLELS_NUMERIC_TEST", 7),
      ),
    ).toBe(42);

    expect(invalidModelTimeoutResult.status).toBe(1);
    expect(invalidModelTimeoutResult.stderr).toContain(
      "invalid SUPPORT_CLAW_PARALLELS_MACOS_MODEL_TIMEOUT_S: 1800s",
    );
    ...
    expect(invalidLinuxAgentTimeoutResult.stderr).toContain(
      "invalid SUPPORT_CLAW_PARALLELS_LINUX_AGENT_TIMEOUT_S: 1e3",
    );

    expect(invalidWindowsAgentTimeoutResult.status).toBe(1);
    expect(invalidWindowsAgentTimeoutResult.stderr).toContain(
      "invalid SUPPORT_CLAW_PARALLELS_WINDOWS_AGENT_TIMEOUT_S: 2700s",
    );

    expect(invalidWindowsUpdateTimeoutResult.status).toBe(1);
    expect(invalidWindowsUpdateTimeoutResult.stderr).toContain(
      "invalid SUPPORT_CLAW_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S: 12.5",
    );

    expect(readFileSync(TS_PATHS.macos, "utf8")).toContain(
      'this.updateDevTimeoutSeconds = readPositiveIntEnv(\n      "SUPPORT_CLAW_PARALLELS_MACOS_UPDATE_DEV_TIMEOUT_S"',
    );
    expect(readFileSync(TS_PATHS.linux, "utf8")).toContain(
      'readPositiveIntEnv(\n    "SUPPORT_CLAW_PARALLELS_LINUX_AGENT_TIMEOUT_S"',
    );
    expect(readFileSync(TS_PATHS.windows, "utf8")).toContain(
      'readPositiveIntEnv(\n    "SUPPORT_CLAW_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S"',
    );
    expect(readFileSync(TS_PATHS.packageArtifact, "utf8")).toContain(
      'readPositiveIntEnv("SUPPORT_CLAW_PARALLELS_PACKAGE_LOCK_TIMEOUT_MS", 30 * 60_000)',
    );
    expect(readFileSync(TS_PATHS.npmUpdate, "utf8")).toContain(
      'readPositiveIntEnv("SUPPORT_CLAW_PARALLELS_NPM_UPDATE_TIMEOUT_S", 1200)',
    );
  });
  ...
  it("keeps Windows update-only env flags scoped before verification", () => {
    ...
    expect(windows).toContain(
      "Invoke-WithScopedEnv @{ SUPPORT_CLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS",
    );
    expect(windows).toContain("$script:SupportClawUpdateExit = $LASTEXITCODE");
    expect(windows).not.toContain("$env:SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS = '1'");
  });
  ...
  it("resolves Windows SupportClaw commands without assuming the npm shim path", () => {
    const powershell = readFileSync(TS_PATHS.powershell, "utf8");
    const windows = readFileSync(TS_PATHS.windows, "utf8");

    expect(powershell).toContain("windowsSupportClawResolver");
    expect(powershell).toContain("SUPPORT_CLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED");
    expect(powershell).toContain("Programs\\nodejs");
    expect(powershell).toContain('selectedModelEntry.agentRuntime = { id: "supportClaw" }');
    ...
    expect(powershell).toContain("Resolve-SupportClawCommand");
    expect(powershell).toContain("npm\\node_modules\\supportClaw\\supportClawCli.mjs");
    ...
    expect(windows).toContain("windowsSupportClawResolver");
    expect(windows).toContain("Invoke-SupportClaw gateway");
    expect(windows).not.toContain("Join-Path $env:APPDATA 'npm\\\\supportClaw.cmd'");
  });
});
