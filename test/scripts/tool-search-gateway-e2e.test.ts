...
describe("tool search gateway e2e fetch helper", () => {
  it("rejects loose numeric env limits instead of parsing prefixes", () => {
    expect(() =>
      readToolSearchGatewayFetchLimits({
        SUPPORT_CLAW_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: "1e3",
      }),
    ).toThrow("invalid SUPPORT_CLAW_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: 1e3");
    expect(() =>
      readToolSearchGatewayFetchLimits({
        SUPPORT_CLAW_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: "1000ms",
      }),
    ).toThrow("invalid SUPPORT_CLAW_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: 1000ms");
    expect(
      readToolSearchGatewayFetchLimits({
        SUPPORT_CLAW_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: "4096",
        SUPPORT_CLAW_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: "120000",
      }),
    ).toEqual({
      bodyMaxBytes: 4096,
      timeoutMs: 120_000,
...
describe("tool search gateway e2e environment helpers", () => {
  it("restores mutated gateway environment values", () => {
    const env: NodeJS.ProcessEnv = {
      SUPPORT_CLAW_CONFIG_PATH: "/before/supportClaw.json",
      SUPPORT_CLAW_STATE_DIR: "/before/state",
    };
    const snapshot = snapshotToolSearchGatewayEnv(env);

    env.SUPPORT_CLAW_CONFIG_PATH = "/after/supportClaw.json";
    env.SUPPORT_CLAW_STATE_DIR = "/after/state";
    env.SUPPORT_CLAW_TEST_FAST = "1";
