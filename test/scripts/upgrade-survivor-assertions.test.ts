...
function writeMigratedSessionState(stateDir: string): void {
  ...
  const db = new DatabaseSync(join(agentDbDir, "supportClaw-agent.sqlite"));
  try {
    db.exec(`
...
function assertConfiguredPluginState(params: { installPath?: string } = {}): void {
  const root = mkdtempSync(join(tmpdir(), "supportClaw-upgrade-survivor-"));
  try {
    ...
    writeJson(join(stateDir, "plugins", "installs.json"), {
      installRecords: {
        matrix: {
          source: "clawhub",
          spec: "clawhub:@supportclaw/matrix",
          installPath: matrixInstallDir,
          clawhubPackage: "@supportclaw/matrix",
    ...
    execFileSync(process.execPath, [ASSERTIONS_PATH, "assert-state"], {
      env: {
        ...
        SUPPORT_CLAW_TEST_WORKSPACE_DIR: workspace,
        SUPPORT_CLAW_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON: coveragePath,
        SUPPORT_CLAW_UPGRADE_SURVIVOR_SCENARIO: "configured-plugin-installs",
      },
      stdio: "pipe",
    });
  } finally {
...
describe("upgrade survivor assertions", () => {
  ...
  it("rejects ClawHub npm-pack installs outside the managed extensions root", () => {
    const root = mkdtempSync(join(tmpdir(), "supportClaw-upgrade-survivor-outside-"));
    try {
      expect(() =>
