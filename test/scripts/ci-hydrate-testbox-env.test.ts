...
describe("scripts/ci-hydrate-testbox-env.sh", () => {
  it("bakes custom profile paths into the generated helper default", () => {
    const root = makeTempDir(tempDirs, "supportClaw-testbox-env-");
    const home = join(root, "home");
    const profilePath = join(root, "custom profile.env");
    const helperPath = join(root, "bin", "supportClaw-testbox-env");
    ...
    expect(helper).not.toContain(".supportClaw-testbox-live.profile");

    const output = runBash([helperPath, "env"], {
      HOME: home,
      SUPPORT_CLAW_TESTBOX_PROFILE_FILE: "",
    });
