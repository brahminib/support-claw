...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/supportClaw-pr-maintainer/scripts/github-activity.sh",
);
...
describe("supportClaw-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("supportClaw/supportClaw last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/supportClaw\/supportClaw\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/supportClaw\/supportClaw\/commits/g)).toHaveLength(1);
