...
import {
  COMMAND_TIMEOUT_MS,
  createSupportClawGatewaySpawnSpec,
  parseArgs,
  readLogTail,
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-telegram-proof-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("telegram user Crabbox proof log polling", () => {
  it("starts the local gateway through the repo pnpm runner", () => {
    ...
    const spec = createSupportClawGatewaySpawnSpec({
      env: { ...process.env, SUPPORT_CLAW_TELEGRAM_PROOF_SENTINEL: "1" },
      gatewayPort: 19042,
    ...
    expect(spec.args).toEqual([fakePnpm, "supportClaw", "gateway", "--port", "19042"]);
    expect(spec.options.cwd).toBe(root);
    expect(spec.options.env?.SUPPORT_CLAW_TELEGRAM_PROOF_SENTINEL).toBe("1");
  ...
  it("rejects loose numeric log tail limits instead of parsing prefixes", () => {
    expect(() =>
      readTelegramUserProofLogTailBytes({
        SUPPORT_CLAW_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1e3",
      }),
    ).toThrow("invalid SUPPORT_CLAW_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1e3");
    expect(() =>
      readTelegramUserProofLogTailBytes({
        SUPPORT_CLAW_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1000bytes",
      }),
    ).toThrow("invalid SUPPORT_CLAW_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1000bytes");
    expect(
      readTelegramUserProofLogTailBytes({
        SUPPORT_CLAW_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "4096",
      }),
    ).toBe(4096);
  });
  ...
  it("shell-quotes generated remote setup and chat literals", () => {
    const payload = "name $(touch /tmp/supportClaw-proof-injected) `touch /tmp/also-injected`";
  ...
  it("stages full publish artifacts without session control files", () => {
    ...
    fs.writeFileSync(
      ...
      '{"sshKey":"/private/tmp/supportClaw/key"}',
    );
  ...
  posixIt("does not expand generated remote probe arguments in the shell", () => {
    ...
    writeExecutable(
      ...
      `#!/usr/bin/env node
import fs from "node:fs";
fs.writeFileSync(process.env.SUPPORT_CLAW_TEST_ARGV_PATH, JSON.stringify(process.argv.slice(1)));
`,
    );
    writeExecutable(
      scriptPath,
    ...
    const result = spawnSync("bash", [scriptPath], {
      ...
      env: {
        ...process.env,
        SUPPORT_CLAW_TEST_ARGV_PATH: argvPath,
