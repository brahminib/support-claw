...
function runInstallShell(script: string, env: NodeJS.ProcessEnv = {}) {
  const home = mkdtempSync(join(tmpdir(), "supportClaw-install-home-"));
  try {
...
describe("install.sh", () => {
  ...
  it("runs installer snippets without inherited shell startup files", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-shell-env-"));
    const bashEnvPath = join(tmp, "bash_env");
    writeFileSync(bashEnvPath, "export SUPPORT_CLAW_BASH_ENV_LEAKED=1\n");

    try {
      const result = runInstallShell('printf "leaked=%s\\n" "${SUPPORT_CLAW_BASH_ENV_LEAKED:-0}"', {
  ...
  it("installs Git with apk on Alpine", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-git-apk-"));
  ...
  it("does not select apk Git on non-Alpine hosts", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-git-native-"));
    const bin = join(tmp, "bin");
    const apkLog = join(tmp, "apk-args.txt");
    mkdirSync(bin, { recursive: true });
    const fakeApk = join(bin, "apk");
  ...
  it("does not emit --before when raw user npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-npmrc-"));
    const bin = join(tmp, "bin");
    ...
    try {
      const result = runInstallShell(
        [
          ...
          `run_npm_global_install supportClaw@latest ${JSON.stringify(join(tmp, "install.log"))}`,
  ...
  it("does not emit --before when default global npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-global-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("does not emit --before when builtin npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-builtin-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("uses SUPPORT_CLAW_HOME for git and onboarding defaults", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-home-"));
    const osHome = join(tmp, "os-home");
    const supportClawHome = join(tmp, "supportClaw-home");
    mkdirSync(osHome, { recursive: true });
    mkdirSync(supportClawHome, { recursive: true });
    ...
    expect(output).toContain(`git=${join(supportClawHome, "supportClaw")}`);
    expect(output).toContain(`workspace=${join(supportClawHome, ".supportClaw", "workspace")}`);
    expect(output).toContain(
      `workspaceProfile=${join(supportClawHome, ".supportClaw", "workspace-work")}`,
    );
  ...
  it("skips bootstrap onboarding when legacy HOME config exists with SUPPORT_CLAW_HOME", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-legacy-config-"));
    const osHome = join(tmp, "os-home");
    const supportClawHome = join(tmp, "supportClaw-home");
    const legacyConfigDir = join(osHome, ".supportClaw");
    const bootstrapDir = join(supportClawHome, ".supportClaw", "workspace");
    mkdirSync(legacyConfigDir, { recursive: true });
    mkdirSync(bootstrapDir, { recursive: true });
    writeFileSync(join(legacyConfigDir, "supportClaw.json"), "{}\n");
  ...
  it("rejects SupportClaw GitHub source targets for npm installs", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      set +e
      SUPPORT_CLAW_VERSION=main
      USE_BETA=0
      install_supportClaw
      status=$?
      printf 'status=%s\\n' "$status"
    ...
    expect(result.stdout).toContain("npm installs do not support SupportClaw GitHub source targets");
  ...
  it("does not emit before args when npmrc min-release-age computes a before cutoff", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-npm-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("ignores project npmrc when choosing global install freshness args", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-global-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("bounds installer npm prefix probes during finalization helpers", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-npm-probe-"));
    ...
    try {
      const result = runInstallShell(
        ...
        {
          SUPPORT_CLAW_INSTALL_PROBE_TIMEOUT_SECONDS: "1",
          PATH: `${tmp}:${process.env.PATH ?? ""}`,
        },
      );

      expect(result.status).toBe(0);
      expect(result.stdout.trim()).toBe("/tmp/supportClaw-npm/bin");
  ...
  it("bounds daemon status probes during finalization helpers", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-probe-"));
    const claw = join(tmp, "supportClaw");
    writeFileSync(
      claw,
      [
        "#!/usr/bin/env bash",
    ...
    try {
      const result = runInstallShell(
        ...
        { SUPPORT_CLAW_INSTALL_PROBE_TIMEOUT_SECONDS: "0.1" },
      );

      expect(result.status).toBe(0);
      expect(result.stdout.trim()).toBe("not-loaded");
      expect(result.stderr).toContain(
        "timed out during installer finalization probe: supportClaw daemon status --json",
      );
    } finally {
      rmSync(tmp, { force: true, recursive: true });
    }
  ...
  it("loads nvm before checking Node.js so stale system Node does not win", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-nvm-"));
    const home = join(tmp, "home");
  ...
  it("promotes a supported Linux Node binary over stale PATH entries", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-node-promote-"));
  ...
  it("uses the package engine floor when accepting existing Node runtimes", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-node-floor-"));
    const bin = join(tmp, "bin");
    mkdirSync(bin, { recursive: true });
  ...
  it("persists a supported Linux Node path before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-linux-node-path-"));
    const home = join(tmp, "home");
  ...
  it("warns before redirecting an unwritable npm prefix", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-npm-prefix-"));
    const home = join(tmp, "home");
    ...
    expect(result?.stdout).toContain("npm i -g supportClaw@latest");
  ...
  it("persists npm prefix PATH before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-npm-prefix-shell-"));
    const home = join(tmp, "home");
    mkdirSync(home, { recursive: true });
    writeFileSync(
      join(home, ".bashrc"),
  ...
  it("uses a quoted absolute supportClaw path in follow-up commands when npm bin is not on the original PATH", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-command-"));
    const npmBin = join(tmp, "npm bin");
    const visibleBin = join(tmp, "visible-bin");
    mkdirSync(npmBin, { recursive: true });
    mkdirSync(visibleBin, { recursive: true });
    const supportClawBin = join(npmBin, "supportClaw");
    writeFileSync(supportClawBin, "#!/bin/sh\nexit 0\n");
    chmodSync(supportClawBin, 0o755);
    ...
    try {
      result = runInstallShell(`
        ...
        printf 'missing=%s\\n' "$(supportClaw_command_for_user "${supportClawBin}")"
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`missing=${supportClawBin.replace(/ /g, "\\ ")}`);
    expect(result?.stdout).toContain("present=supportClaw");
  });
  ...
  it("resolves requested git install versions to checkout refs", () => {
    const result = runInstallShell(`
      ...
      source "${SCRIPT_PATH}"
      npm() {
        if [[ "$1" == "view" && "$2" == "supportClaw" && "$3" == "dist-tags.beta" ]]; then
          printf '2026.5.12-beta.3\\n'
          return 0
        fi
        return 1
      }
      SUPPORT_CLAW_VERSION=v2026.5.12-beta.3
      printf 'tag=%s\\n' "$(resolve_git_supportClaw_ref)"
      SUPPORT_CLAW_VERSION=2026.5.12-beta.3
      printf 'semver=%s\\n' "$(resolve_git_supportClaw_ref)"
      SUPPORT_CLAW_VERSION=beta
      printf 'beta=%s\\n' "$(resolve_git_supportClaw_ref)"
      SUPPORT_CLAW_VERSION=main
      printf 'main=%s\\n' "$(resolve_git_supportClaw_ref)"
    `);
  ...
  it("uses the repo Corepack pnpm when a global pnpm version is already present", () => {
    const tmp = mkdtempSync(join(tmpdir(), "supportClaw-install-pnpm-version-"));
    const bin = join(tmp, "bin");
...
describe("install.sh macOS Homebrew Node behavior", () => {
  ...
  it("reruns spinner-wrapped commands when gum reports ioctl failure", () => {
    const dir = mkdtempSync(join(tmpdir(), "supportClaw-install-sh-gum-"));
    try {
...
describe("install.sh duplicate SupportClaw install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    const result = runInstallShell(`
      ...
      mkdir -p "$root/brew/supportClaw" "$root/fnm/supportClaw"
      printf '{"version":"2026.3.7"}\\n' > "$root/brew/supportClaw/package.json"
      printf '{"version":"2026.3.1"}\\n' > "$root/fnm/supportClaw/package.json"
      collect_supportClaw_npm_root_candidates() { printf '%s\\n' "$root/brew" "$root/fnm"; }
      SUPPORT_CLAW_BIN="$root/fnm/.bin/supportClaw"
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_supportClaw_global_installs
    `);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple SupportClaw global installs detected");
    expect(result.stdout).toContain("2026.3.7");
    expect(result.stdout).toContain("2026.3.1");
    expect(result.stdout).toContain("/brew/supportClaw");
    expect(result.stdout).toContain("/fnm/supportClaw");
    expect(result.stdout).toContain("Active supportClaw:");
    expect(result.stdout).toContain("npm uninstall -g supportClaw");
  });

  it("stays quiet when only one SupportClaw npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
      trap 'rm -rf "$root"' EXIT
      mkdir -p "$root/only/supportClaw"
      printf '{"version":"2026.3.7"}\\n' > "$root/only/supportClaw/package.json"
      collect_supportClaw_npm_root_candidates() { printf '%s\\n' "$root/only"; }
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_supportClaw_global_installs
    `);
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple SupportClaw global installs detected");
  });
});
