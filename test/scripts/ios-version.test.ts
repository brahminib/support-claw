...
describe("resolveIosVersion", () => {
  ...
  it("prints selected fields from the CLI", () => {
    const rootDir = writeIosFixture({
      version: "2026.4.6",
      changelog: "# SupportClaw iOS Changelog\n\n## 2026.4.6\n\nStable notes.\n",
    });
    const result = spawnSync(
      process.execPath,
      [
...
describe("renderIosVersionXcconfig", () => {
  it("renders checked-in defaults from the pinned iOS version", () => {
    ...
    expect(renderIosVersionXcconfig(version)).toContain("SUPPORT_CLAW_IOS_VERSION = 2026.4.8");
    expect(renderIosVersionXcconfig(version)).toContain("SUPPORT_CLAW_MARKETING_VERSION = 2026.4.8");
    expect(renderIosVersionXcconfig(version)).toContain("SUPPORT_CLAW_BUILD_VERSION = 1");
  });
});
