...
async function loadRawVitestConfig(configPath: string): Promise<VitestConfig> {
  ...
  const previousIncludeFile = process.env.SUPPORT_CLAW_VITEST_INCLUDE_FILE;
  process.argv = [previousArgv[0] ?? "node", previousArgv[1] ?? "vitest"];
  delete process.env.SUPPORT_CLAW_VITEST_INCLUDE_FILE;
  try {
    ...
    if (previousIncludeFile === undefined) {
      ...
      process.env.SUPPORT_CLAW_VITEST_INCLUDE_FILE = previousIncludeFile;
    }
  }
}
...
function withTinyGitRepo(files: Record<string, string>, test: (cwd: string) => void): void {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-test-projects-"));
  try {
...
function commitTinyGitRepo(cwd: string): void {
  const commit = spawnSync("git", ["commit", "-m", "initial"], {
    ...
    env: {
      ...
      GIT_AUTHOR_NAME: "SupportClaw Test",
      GIT_COMMITTER_EMAIL: "test@example.com",
      GIT_COMMITTER_NAME: "SupportClaw Test",
    },
    stdio: "ignore",
  });
...
describe("scripts/test-projects changed-target routing", () => {
  ...
  it("keeps the broad changed run available for Vitest wiring edits", () => {
    expect(
      resolveChangedTargetArgs(
        ...
        { env: { SUPPORT_CLAW_TEST_CHANGED_BROAD: "1" } },
      ),
    ).toBeNull();
  });
  ...
  it("routes nested scripts through conventional owner tests", () => {
    ...
    expect(resolveChangedTestTargetPlan(["scripts/github/resolve-supportClaw-ref.sh"])).toEqual({
      mode: "targets",
      targets: ["test/scripts/resolve-supportClaw-ref.test.ts"],
    });
  });
  ...
  it("routes nested e2e shell helpers through their sourced owner tests", () => {
    const expectedTargets = new Map([
      ...
      [
        ...
        ["test/scripts/e2e-shell-tempfiles.test.ts", "test/scripts/supportClaw-test-state.test.ts"],
      ],
      [
      ...
      [
        ...
        [
          ...
          "test/scripts/upgrade-survivor-assertions.test.ts",
          "test/scripts/supportClaw-test-state.test.ts",
        ],
      ],
      [
  ...
  it("keeps release-check workflow edits on release workflow regression tests", () => {
    expect(resolveChangedTestTargetPlan([".github/workflows/supportClaw-release-checks.yml"])).toEqual(
      {
        mode: "targets",
        targets: [
          "test/scripts/package-acceptance-workflow.test.ts",
          "test/scripts/supportClaw-cross-os-release-checks.test.ts",
          "test/scripts/plugin-prerelease-test-plan.test.ts",
          "test/scripts/test-install-sh-docker.test.ts",
          "test/scripts/ci-workflow-guards.test.ts",
        ],
  ...
  it("keeps CI, dependency, and docs tooling edits on owner tests", () => {
    ...
    expect(
      resolveChangedTestTargetPlan(["scripts/github/run-supportClaw-cross-os-release-checks.sh"]),
    ).toEqual({
      mode: "targets",
      targets: ["test/scripts/supportClaw-cross-os-release-workflow.test.ts"],
    });
  ...
  it("keeps package, release, and install tooling edits on owner tests", () => {
    const expectedTargets = new Map([
      ...
      ["scripts/podman/supportClaw.container.in", ["test/scripts/test-install-sh-docker.test.ts"]],
      [
        "scripts/package-supportClaw-for-docker.mjs",
        ["test/e2e/qa-lab/runtime/package-supportClaw-for-docker.e2e.test.ts"],
      ],
      ...
      ["scripts/supportClaw-prepack.ts", ["test/supportClaw-prepack.test.ts"]],
      ["scripts/supportClaw-npm-release-check.ts", ["test/supportClaw-npm-release-check.test.ts"]],
      [
        "scripts/supportClaw-npm-postpublish-verify.ts",
        ["test/supportClaw-npm-postpublish-verify.test.ts"],
      ],
  ...
  it("keeps auth monitoring helper edits on owner tests", () => {
    const expectedTargets = new Map([
      ...
      ["scripts/systemd/supportClaw-auth-monitor.service", ["test/scripts/auth-monitor.test.ts"]],
      ["scripts/systemd/supportClaw-auth-monitor.timer", ["test/scripts/auth-monitor.test.ts"]],
    ...
    const expectedTargets = new Map([
      ...
      [
        ...
        [
          ...
          "test/release-check.test.ts",
          "test/supportClaw-npm-release-check.test.ts",
          "test/scripts/check-gateway-watch-regression.test.ts",
          "test/scripts/check-supportClaw-package-tarball.test.ts",
          "test/scripts/supportClaw-cross-os-release-checks.test.ts",
        ],
      ],
      [
      ...
      [
        ".github/workflows/supportClaw-live-and-e2e-checks-reusable.yml",
        [
          "test/scripts/package-acceptance-workflow.test.ts",
          "test/scripts/ci-workflow-guards.test.ts",
      ...
      [
        ...
        ["test/release-check.test.ts", "test/supportClaw-npm-release-check.test.ts"],
      ],
      [
        "scripts/supportClaw-release-clawhub-runtime-state.ts",
        ["test/scripts/supportClaw-release-clawhub-runtime-state.test.ts"],
      ],
      [
        "scripts/supportClaw-release-clawhub-plan.ts",
        ["test/scripts/release-wrapper-scripts.test.ts"],
      ],
      ["scripts/lib/supportClaw-release-clawhub-plan.ts", ["test/plugin-clawhub-release.test.ts"]],
      [
  ...
  it("routes the shell helper test to the isolated tooling shard", () => {
    expect(
      buildVitestRunPlans(["--changed", "origin/main"], process.cwd(), () => [
        "test/scripts/supportClaw-e2e-instance.test.ts",
      ]),
    ).toEqual([
      {
        config: "test/vitest/vitest.tooling-isolated.config.ts",
        forwardedArgs: [],
        includePatterns: ["test/scripts/supportClaw-e2e-instance.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("chunks the broad shell helper tooling shard after isolated targets", () => {
    ...
    expect(toolingTargets).not.toContain("test/scripts/supportClaw-e2e-instance.test.ts");
  ...
  it("rejects explicit test-support helper files with no importing tests", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-test-targets-"));
    try {
  ...
  it("explains changed paths that need explicit broad fallback before skipping", () => {
    withTinyGitRepo({ "package.json": '{"scripts":{}}\n' }, (cwd) => {
      ...
      expect(result.stderr).toContain(
        "[test] run `SUPPORT_CLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` for broad coverage.",
      );
    });
  });
  ...
  it("skips app-only changes because app tests are separate from Vitest lanes", () => {
    expect(
      buildVitestRunPlans(["--changed", "origin/main"], process.cwd(), () => [
        "apps/macos/SupportClaw/AppDelegate.swift",
      ]),
    ).toStrictEqual([]);
  });
  ...
  it("uses collision-resistant include-file names for scoped Vitest specs", () => {
    const tempDir = path.join("tmp", "supportClaw-vitest-specs");
    ...
    expect(path.basename(spec?.includeFilePath ?? "")).toMatch(
      /^supportClaw-vitest-include-[0-9a-f-]{36}-0\.json$/u,
    );
...
describe("scripts/test-projects local heavy-check lock", () => {
  const localCheckEnv = () => ({
    ...process.env,
    SUPPORT_CLAW_TEST_HEAVY_CHECK_LOCK_HELD: undefined,
    SUPPORT_CLAW_TEST_PROJECTS_FORCE_LOCK: undefined,
  });
  ...
  it("skips the lock when a parent changed gate already holds it", () => {
    expect(
      shouldAcquireLocalHeavyCheckLock(
        ...
        {
          ...localCheckEnv(),
          SUPPORT_CLAW_TEST_HEAVY_CHECK_LOCK_HELD: "1",
        },
      ),
    ).toBe(false);
  });
  ...
  it("allows forcing the lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLock(
        ...
        {
          ...localCheckEnv(),
          SUPPORT_CLAW_TEST_PROJECTS_FORCE_LOCK: "1",
        },
      ),
    ).toBe(true);
  });
...
describe("scripts/test-projects full-suite sharding", () => {
  ...
  beforeAll(async () => {
    ...
    const previous = process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS;
    const gatewayServerConfig = "test/vitest/vitest.gateway-server.config.ts";
    process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS = "1";
    try {
      ...
      if (previous === undefined) {
        delete process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS;
      } else {
        process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS = previous;
      }
    }
  });
  ...
  it("keeps explicit parallel overrides ahead of the host-aware profile", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        61,
        {
          SUPPORT_CLAW_TEST_PROJECTS_PARALLEL: "3",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
  ...
  it("rejects malformed parallel full-suite overrides", () => {
    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          SUPPORT_CLAW_TEST_PROJECTS_PARALLEL: "3x",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("SUPPORT_CLAW_TEST_PROJECTS_PARALLEL must be a positive integer; got: 3x");

    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          SUPPORT_CLAW_TEST_PROJECTS_PARALLEL: "0",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("SUPPORT_CLAW_TEST_PROJECTS_PARALLEL must be a positive integer; got: 0");
  });
  ...
  it("rejects malformed conservative worker budget values", () => {
    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          SUPPORT_CLAW_VITEST_MAX_WORKERS: "1e0",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("SUPPORT_CLAW_VITEST_MAX_WORKERS must be a positive integer; got: 1e0");

    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          SUPPORT_CLAW_TEST_WORKERS: "1 worker",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("SUPPORT_CLAW_TEST_WORKERS must be a positive integer; got: 1 worker");
  });

  it("keeps serial untargeted local runs on leaf project configs", () => {
    const previousParallel = process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.SUPPORT_CLAW_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    const previousActions = process.env.GITHUB_ACTIONS;
    delete process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.SUPPORT_CLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    delete process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL;
    ...
    try {
      ...
      if (previousParallel === undefined) {
        ...
        process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL = previousParallel;
      }
      if (previousSerial === undefined) {
        delete process.env.SUPPORT_CLAW_TEST_PROJECTS_SERIAL;
      } else {
        process.env.SUPPORT_CLAW_TEST_PROJECTS_SERIAL = previousSerial;
      }
  ...
  it("expands untargeted local runs to leaf project configs by default", () => {
    const previousLeafShards = process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS;
    const previousParallel = process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.SUPPORT_CLAW_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    const previousActions = process.env.GITHUB_ACTIONS;
    const previousVitestMaxWorkers = process.env.SUPPORT_CLAW_VITEST_MAX_WORKERS;
    const previousTestWorkers = process.env.SUPPORT_CLAW_TEST_WORKERS;
    delete process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL;
    delete process.env.SUPPORT_CLAW_TEST_PROJECTS_SERIAL;
    delete process.env.CI;
    delete process.env.GITHUB_ACTIONS;
    delete process.env.SUPPORT_CLAW_VITEST_MAX_WORKERS;
    delete process.env.SUPPORT_CLAW_TEST_WORKERS;
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previousLeafShards === undefined) {
        ...
        process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS = previousLeafShards;
      }
      if (previousParallel === undefined) {
        delete process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL;
      } else {
      ...
      if (previousVitestMaxWorkers === undefined) {
        ...
        process.env.SUPPORT_CLAW_VITEST_MAX_WORKERS = previousVitestMaxWorkers;
      }
      if (previousTestWorkers === undefined) {
        delete process.env.SUPPORT_CLAW_TEST_WORKERS;
      } else {
        process.env.SUPPORT_CLAW_TEST_WORKERS = previousTestWorkers;
      }
    }
  });
  ...
  it("can skip the aggregate extension shard when CI runs dedicated extension shards", () => {
    const previous = process.env.SUPPORT_CLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    const previousParallel = process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.SUPPORT_CLAW_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    delete process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL;
    process.env.SUPPORT_CLAW_TEST_PROJECTS_SERIAL = "1";
    process.env.CI = "true";
    process.env.SUPPORT_CLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previous === undefined) {
        ...
        process.env.SUPPORT_CLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = previous;
      }
      if (previousParallel === undefined) {
        delete process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL;
      } else {
  ...
  it("skips extension project configs when leaf sharding and the aggregate extension shard is disabled", () => {
    const previousLeafShards = process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS;
    const previousSkipExtensions = process.env.SUPPORT_CLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS = "1";
    process.env.SUPPORT_CLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previousSkipExtensions === undefined) {
        ...
        process.env.SUPPORT_CLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = previousSkipExtensions;
      }
    }
  });
  ...
  it("expands full-suite shards before running them in parallel", () => {
    ...
    const previousParallel = process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL;
    delete process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS;
    process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL = "6";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
  ...
  it("rejects malformed full-suite expansion parallel overrides", () => {
    ...
    const previousParallel = process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL;
    delete process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS;
    process.env.SUPPORT_CLAW_TEST_PROJECTS_PARALLEL = "6x";
    try {
      expect(() => buildFullSuiteVitestRunPlans([], process.cwd())).toThrow(
        "SUPPORT_CLAW_TEST_PROJECTS_PARALLEL must be a positive integer; got: 6x",
      );
    } finally {
      if (previousLeafShards === undefined) {
        delete process.env.SUPPORT_CLAW_TEST_PROJECTS_LEAF_SHARDS;
...
describe("scripts/test-projects parallel cache paths", () => {
  it("assigns isolated Vitest fs-module cache paths per parallel shard", () => {
    ...
    expect(specs.map((spec) => spec.env)).toEqual([
      {
        SUPPORT_CLAW_VITEST_FS_MODULE_CACHE_PATH: path.join(
          "/repo",
          "node_modules",
  ...
  it("keeps an explicit global cache path", () => {
    const [spec] = applyParallelVitestCachePaths(
      [{ config: "test/vitest/vitest.gateway.config.ts", env: {}, pnpmArgs: [] }],
      { cwd: "/repo", env: { SUPPORT_CLAW_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache" } },
    );

    expect(spec?.env.SUPPORT_CLAW_VITEST_FS_MODULE_CACHE_PATH).toBeUndefined();
  });
});
...
describe("scripts/test-projects Vitest stall watchdog", () => {
  it("adds default no-output watchdog settings to non-watch specs", () => {
    ...
    expect(spec?.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_TIMEOUT_MS,
    );
    expect(spec?.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_HEARTBEAT_MS,
    );
  });
  ...
  it("extends the no-output watchdog for slow silent full-suite configs", () => {
    ...
    expect(specs[0]?.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("2400000");
    expect(specs[1]?.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("2400000");
    expect(specs[2]?.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("2400000");
    expect(specs[3]?.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_TIMEOUT_MS,
    );
  });
  ...
  it("keeps explicit watchdog settings and watch mode untouched", () => {
    const specs = applyDefaultVitestNoOutputTimeout(
      [
        ...
        },
        {
          ...
          env: {
            SUPPORT_CLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "25000",
            SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "0",
            PATH: "/usr/bin",
          },
          includeFilePath: null,
          includePatterns: null,
    ...
    expect(specs[0]?.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBeUndefined();
    expect(specs[0]?.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBeUndefined();
    expect(specs[1]?.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("0");
    expect(specs[1]?.env.SUPPORT_CLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBe("25000");
  });
  ...
  it("allows changed checks to disable automatic silent-run retries", () => {
    ...
    expect(shouldRetryVitestNoOutputTimeout({ SUPPORT_CLAW_VITEST_NO_OUTPUT_RETRY: "1" })).toBe(true);
    expect(shouldRetryVitestNoOutputTimeout({ SUPPORT_CLAW_VITEST_NO_OUTPUT_RETRY: "0" })).toBe(false);
    expect(shouldRetryVitestNoOutputTimeout({ SUPPORT_CLAW_VITEST_NO_OUTPUT_RETRY: "false" })).toBe(
      false,
    );
  });
});
...
describe("scripts/test-projects Vitest cache isolation", () => {
  it("assigns isolated fs-module caches to multi-spec non-watch runs", () => {
    ...
    expect(specs.map((spec) => spec.env.SUPPORT_CLAW_VITEST_FS_MODULE_CACHE_PATH)).toEqual([
      path.join(
        "/repo",
        "node_modules",
        ".experimental-vitest-cache",
