...
describe("security-sensitive guard workflow", () => {
  ...
  it("temporarily skips PR bases that predate the guard rollout commit", () => {
    ...
    expect(parsed.env?.SUPPORT_CLAW_SECURITY_SENSITIVE_GUARD_ROLLOUT_SHA).toBe(
    ...
    for (const jobName of ["security-sensitive-guard-detect", "security-sensitive-guard"]) {
      ...
      expect(rollout?.run).toContain(
        "compare/${SUPPORT_CLAW_SECURITY_SENSITIVE_GUARD_ROLLOUT_SHA}...${PR_BASE_SHA}",
      );
  ...
  it("keeps detection separate from the final required check", () => {
    ...
    expect(detectSteps.at(-1)?.env?.SUPPORT_CLAW_SECURITY_SENSITIVE_GUARD_MODE).toBe("detect");
    expect(finalSteps.at(-1)?.env?.SUPPORT_CLAW_SECURITY_SENSITIVE_GUARD_MODE).toBe("enforce");
    expect(finalSteps.at(-1)?.env?.SUPPORT_CLAW_SECURITY_TEAM_SLUG).toBe("supportClaw-secops");
    expect(finalSteps.at(-1)?.env?.SUPPORT_CLAW_SECURITY_APPROVERS).toBe(
  ...
  it("requires secops review for future workflow or guard changes", () => {
    ...
    expect(codeowners).toContain(
      "/.github/workflows/security-sensitive-guard.yml @supportclaw/supportClaw-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/security-sensitive-guard-workflow.test.ts @supportclaw/supportClaw-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/security-sensitive-guard-script.test.ts @supportclaw/supportClaw-secops",
    );
    expect(codeowners).toContain(
      "/scripts/github/security-sensitive-guard.mjs @supportclaw/supportClaw-secops",
    );
    expect(codeowners).toContain("/.gitignore @supportclaw/supportClaw-secops");
  });
});
