...
function createRepoWithPrChangelogDiff(entry: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "supportClaw-changelog-credit-"));
...
function createRepoWithChangelog(content: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "supportClaw-changelog-policy-"));
...
function validateChangelogEntry(repo: string, contrib: string): string {
  return run(
    ...
    [
      "-c",
      'source "$SUPPORT_CLAW_PR_CHANGELOG_SH"; validate_changelog_entry_for_pr 123 "$SUPPORT_CLAW_TEST_CONTRIB"',
    ],
    {
      SUPPORT_CLAW_PR_CHANGELOG_SH: changelogScriptPath,
      SUPPORT_CLAW_TEST_CONTRIB: contrib,
    },
  );
}
...
function validateChangelogAttributionPolicy(repo: string): string {
  return run(
    repo,
    "bash",
    ["-c", 'source "$SUPPORT_CLAW_PR_CHANGELOG_SH"; validate_changelog_attribution_policy'],
    {
      SUPPORT_CLAW_PR_CHANGELOG_SH: changelogScriptPath,
    },
  );
...
describe("check-changelog-attributions", () => {
  it("flags forbidden bot, org, and maintainer thanks attributions", () => {
    const content = [
      ...
      "- Org-owned fix. Thanks @supportclaw.",
      "- Maintainer-owned fix. Thanks @steipete.",
      "- Mixed credit. Thanks @contributor and @SupportClaw.",
    ...
    expect(findForbiddenChangelogThanks(content)).toEqual([
      ...
      { line: 2, handle: "supportClaw", text: "- Org-owned fix. Thanks @supportclaw." },
      { line: 3, handle: "steipete", text: "- Maintainer-owned fix. Thanks @steipete." },
      { line: 4, handle: "supportClaw", text: "- Mixed credit. Thanks @contributor and @SupportClaw." },
  ...
  it("checks every thanked handle on a changelog line", () => {
    expect(
      findForbiddenChangelogThanks("- Mixed credit (#123). Thanks @supportclaw and @alice."),
    ).toEqual([
      {
        line: 1,
        handle: "supportClaw",
        text: "- Mixed credit (#123). Thanks @supportclaw and @alice.",
      },
    ]);
  });
  ...
  it("uses one attribution predicate for scanner and shell checks", () => {
    ...
    expect(isForbiddenChangelogThanksHandle("supportClaw")).toBe(true);
    ...
    expect(isForbiddenChangelogThanksHandle("supportClaw-clawsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("supportClaw-clawsweeper[bot]")).toBe(true);
    ...
    try {
      const output = run(
        ...
        [
          ...
          `
set -euo pipefail
source "$SUPPORT_CLAW_PR_COMMON_SH"
source "$SUPPORT_CLAW_PR_CHANGELOG_SH"
source "$SUPPORT_CLAW_PR_GATES_SH"

enter_worktree() { :; }
checkout_prep_branch() { :; }
bootstrap_deps_if_needed() { :; }
require_artifact() { [ -s "$1" ]; }
normalize_pr_changelog_entries() { printf 'normalize\\n' >>"$SUPPORT_CLAW_TEST_CALLS"; }
validate_changelog_attribution_policy() { printf 'policy\\n' >>"$SUPPORT_CLAW_TEST_CALLS"; }
validate_changelog_merge_hygiene() { printf 'merge-hygiene\\n' >>"$SUPPORT_CLAW_TEST_CALLS"; }
validate_changelog_entry_for_pr() { printf 'entry:%s:%s\\n' "$1" "$2" >>"$SUPPORT_CLAW_TEST_CALLS"; }
run_quiet_logged() { printf 'gate:%s\\n' "$1" >>"$SUPPORT_CLAW_TEST_CALLS"; }

prepare_gates 123
`,
        ],
        {
          SUPPORT_CLAW_PR_COMMON_SH: commonScriptPath,
          SUPPORT_CLAW_PR_CHANGELOG_SH: changelogScriptPath,
          SUPPORT_CLAW_PR_GATES_SH: gatesScriptPath,
          SUPPORT_CLAW_TEST_CALLS: callsPath,
          SUPPORT_CLAW_TESTBOX: "0",
        },
      );
