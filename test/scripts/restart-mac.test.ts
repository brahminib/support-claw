...
function runGatewayPortCheck(fakeLsof: string) {
  const root = mkdtempSync(join(tmpdir(), "supportClaw-restart-mac-test-"));
  tempRoots.push(root);
...
function runCleanupFunction(fakePs: string) {
  ...
  const cleanupFunction = script.slice(
    script.indexOf("kill_all_supportClaw()"),
  ...
  writeFileSync(
    ...
    [
      ...
      'DEBUG_PROCESS_PATTERN="/worktree/apps/macos/.build/debug/SupportClaw"',
      'LOCAL_PROCESS_PATTERN="/worktree/apps/macos/.build-local/debug/SupportClaw"',
      'RELEASE_PROCESS_PATTERN="/worktree/apps/macos/.build/release/SupportClaw"',
      "kill() {",
      '  printf "%s\\n" "$*" >> "$SUPPORT_CLAW_TEST_KILL_CALLS"',
      "  return 0",
      "}",
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      SUPPORT_CLAW_TEST_KILL_CALLS: killCallsPath,
      PATH: `${binDir}:${process.env.PATH ?? ""}`,
    },
  });
...
describe("scripts/restart-mac.sh", () => {
  ...
  it("keeps the default restart log scoped to the current worktree lock", () => {
    ...
    expect(script).not.toContain('LOG_PATH="${SUPPORT_CLAW_RESTART_LOG:-/tmp/supportClaw-restart.log}"');
  });
  ...
  it("prefers the freshly packaged app unless an explicit app bundle is set", () => {
    ...
    expect(script).toContain('fail "SUPPORT_CLAW_APP_BUNDLE does not exist: ${APP_BUNDLE}"');
    expect(chooseBlock).toContain("canonicalize_app_bundle");
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/SupportClaw.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("/Applications/SupportClaw.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/SupportClaw.app")).toBeLessThan(
      chooseBlock.indexOf("/Applications/SupportClaw.app"),
    );
  });

  it("keeps restart cleanup scoped to known SupportClaw app and build paths", () => {
    const script = readFileSync(restartScriptPath, "utf8");
    ...
    expect(cleanupBlock).toContain('"/Applications/SupportClaw.app/${APP_EXECUTABLE_RELATIVE_PATH}"');
    ...
    expect(cleanupBlock).not.toContain('pkill -x "SupportClaw"');
    expect(cleanupBlock).not.toContain("pgrep");
    expect(cleanupBlock).not.toContain('pgrep -x "SupportClaw"');
  });

  it("stops launchd supervision before killing app processes", () => {
    const script = readFileSync(restartScriptPath, "utf8");
    const stopIndex = script.indexOf("stop_launch_agent\nlog");
    const killIndex = script.indexOf("if ! kill_all_supportClaw");
  ...
  it("normalizes custom app bundle paths before process matching", () => {
    const root = mkdtempSync(join(tmpdir(), "supportClaw-restart-mac-test-"));
    tempRoots.push(root);
    const appBundle = join(root, "dist", "SupportClaw.app");
    mkdirSync(appBundle, { recursive: true });

    const { result } = runCanonicalizeAppBundle(`${appBundle}/../SupportClaw.app/`);
  ...
  it("fails restart cleanup when scoped processes survive every kill attempt", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  321 /worktree/dist/SupportClaw.app/Contents/MacOS/SupportClaw --attach-only'",
      ].join("\n"),
    );
  ...
  it("passes restart cleanup when the final kill attempt clears the process", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        'kill_count="$(wc -l < "$SUPPORT_CLAW_TEST_KILL_CALLS" 2>/dev/null || echo 0)"',
  ...
  it("does not kill unrelated SupportClaw app bundles", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  654 /tmp/Other/SupportClaw.app/Contents/MacOS/SupportClaw'",
      ].join("\n"),
    );
