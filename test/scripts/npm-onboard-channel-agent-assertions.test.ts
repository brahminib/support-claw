...
function writeConfig(home: string, channels: Record<string, unknown>): void {
  ...
  fs.writeFileSync(path.join(configDir, "supportClaw.json"), JSON.stringify({ channels }));
}
...
function runStatusAssert(
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-status-assertions-"));
  try {
...
describe("npm onboard channel agent assertions", () => {
  it("rejects loose mock OpenAI port args", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-onboard-assertions-"));
  ...
  it("validates OpenAI env refs from the SQLite auth profile store", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-onboard-assertions-"));
    const agentDir = path.join(tempDir, ".supportClaw", "agents", "main", "agent");
      ...
      writeAuthProfileStoreSqlite(agentDir, {
        ...
        profiles: {
          "openai:api-key": {
            type: "api_key",
            provider: "openai",
            key: "sk-supportClaw-npm-onboard-e2e",
          },
        },
      });
  ...
  it("validates channel tokens in their canonical config fields", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-channel-assertions-"));
    try {
      writeConfig(tempDir, {
  ...
  it("rejects oversized plain status output before parsing it", () => {
    const result = runStatusAssert(
      ...
      { configuredChannels: ["telegram"] },
      `# SupportClaw status\n${"x".repeat(128)}`,
      { SUPPORT_CLAW_NPM_ONBOARD_STATUS_TEXT_MAX_BYTES: "64" },
    );
  ...
  it("rejects oversized channels status JSON before parsing it", () => {
    const result = runStatusAssert(
      ...
      "# Channels\ntelegram ok configured",
      { SUPPORT_CLAW_NPM_ONBOARD_JSON_ARTIFACT_MAX_BYTES: "64" },
    );
