...
function makeFakePnpm(): { binDir: string; logPath: string } {
  const root = makeTempDir(tempDirs, "supportClaw-release-preflight-");
...
appendFileSync(process.env.SUPPORT_CLAW_RELEASE_PREFLIGHT_PNPM_LOG, command + "\\n");
const failures = new Set((process.env.SUPPORT_CLAW_RELEASE_PREFLIGHT_FAIL_COMMANDS ?? "").split(";").filter(Boolean));
...
function runPreflight(
  ...
  return spawnSync(process.execPath, [SCRIPT, ...args], {
    ...
    env: {
      ...
      ...(fakePnpm
        ? {
            SUPPORT_CLAW_RELEASE_PREFLIGHT_PNPM_LOG: fakePnpm.logPath,
...
describe("scripts/release-preflight.mjs", () => {
  ...
  it("runs every check command and reports all failed release artifact checks", () => {
    ...
    const result = runPreflight(["--check"], fakePnpm, {
      SUPPORT_CLAW_RELEASE_PREFLIGHT_FAIL_COMMANDS: "plugins:sync:check;config:docs:check",
    });
  ...
  it("stops refresh mode at the first failed generator before running checks", () => {
    ...
    const result = spawnSync(process.execPath, [SCRIPT, "--fix"], {
      ...
      env: {
        ...process.env,
        SUPPORT_CLAW_RELEASE_PREFLIGHT_FAIL_COMMANDS: "deps:shrinkwrap:changed:generate",
        SUPPORT_CLAW_RELEASE_PREFLIGHT_PNPM_LOG: fakePnpm.logPath,
        PATH: `${fakePnpm.binDir}${delimiter}${process.env.PATH ?? ""}`,
      },
    });
