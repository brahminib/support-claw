...
describe("scripts/ios-release-signing.mjs", () => {
  ...
  it("emits manual App Store profile settings for every signed target", () => {
    ...
    expect(output).toContain("SUPPORT_CLAW_CODE_SIGN_STYLE = Manual");
    expect(output).toContain("SUPPORT_CLAW_CODE_SIGN_IDENTITY = Apple Distribution");
    expect(output).toContain("SUPPORT_CLAW_APP_GROUP_ID = group.ai.supportClawfoundation.app.shared");
    expect(output).toContain("SUPPORT_CLAW_APP_PROFILE = SupportClaw App Store ai.supportClawfoundation.app");
    expect(output).toContain(
      "SUPPORT_CLAW_SHARE_PROFILE = SupportClaw App Store ai.supportClawfoundation.app.share",
    );
    expect(output).toContain(
      "SUPPORT_CLAW_ACTIVITY_WIDGET_PROFILE = SupportClaw App Store ai.supportClawfoundation.app.activitywidget",
    );
    expect(output).toContain(
      "SUPPORT_CLAW_WATCH_APP_PROFILE = SupportClaw App Store ai.supportClawfoundation.app.watchkitapp",
    );
    expect(output).not.toContain("SUPPORT_CLAW_WATCH_EXTENSION_PROFILE");
  });
  ...
  it("documents the canonical release signing plan", () => {
    ...
    expect(output).toContain("Signing repo: git@github.com:supportClaw/apps-signing.git");
    expect(output).toContain("Signing branch: main");
    expect(output).toContain("Signing setup and sync: Fastlane match");
    expect(output).not.toContain("SupportClawWatchExtension");
    expect(output).toContain("capabilities: PUSH_NOTIFICATIONS, APP_GROUPS");
    expect(output).toContain("app groups: group.ai.supportClawfoundation.app.shared");
  });
});
