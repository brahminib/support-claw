...
describe("gateway CPU scenario guard", () => {
  ...
  it("prebuilds private QA dist before running QA scenarios when it is missing", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        ...
        if (args.includes("supportClaw") && args.includes("qa")) {
    ...
    expect(calls[0]?.env).toMatchObject({
      ...
      SUPPORT_CLAW_CONFIG_PATH: path.join(outputDir, "qa-state-root", "state", "supportClaw.json"),
      SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI: "1",
      SUPPORT_CLAW_HOME: path.join(outputDir, "qa-state-root", "home"),
      SUPPORT_CLAW_RUN_NODE_SKIP_DTS_BUILD: "1",
      SUPPORT_CLAW_STATE_DIR: path.join(outputDir, "qa-state-root", "state"),
      SUPPORT_CLAW_TEST_DISABLE_UPDATE_CHECK: "1",
      USERPROFILE: path.join(outputDir, "qa-state-root", "home"),
    });
    expect(calls[0]?.env?.SUPPORT_CLAW_BUNDLED_PLUGIN_BUILD_IDS).toBeUndefined();
  });
  ...
  it("does not prebuild private QA dist when the required entries already exist", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      env: {
        HOME: "/real/user/home",
        SUPPORT_CLAW_CONFIG_PATH: "/real/user/.supportClaw/supportClaw.json",
        SUPPORT_CLAW_HOME: "/real/user/home",
        SUPPORT_CLAW_STATE_DIR: "/real/user/.supportClaw",
      },
      silent: true,
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        calls.push({ args, env: opts?.env });
