...
describe("Docker E2E client scripts", () => {
  ...
  it("keeps session runtime-context checks wired to packaged transcript behavior", () => {
    ...
    expect(source).toContain("supportClaw/plugin-sdk/agent-sessions");
    ...
    expect(source).toContain("<<<BEGIN_SUPPORT_CLAW_INTERNAL_CONTEXT>>>");
    expect(source).toContain("supportClaw.runtime-context");
