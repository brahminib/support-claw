...
describe("website installer sync workflow", () => {
  ...
  it("treats all website installer scripts as SupportClaw-owned inputs", () => {
  ...
  it("verifies installers on Linux Docker plus native macOS and Windows runners", () => {
    ...
    expect(workflow).toContain("bash /tmp/install.sh --version latest && supportClaw --version");
    expect(workflow).not.toContain("bash /tmp/install.sh --no-prompt --no-onboard");
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/supportClaw");
    expect(workflow).toContain("macos-installer:");
    expect(workflow).toContain("runs-on: macos-15");
    expect(workflow).toContain("node-version: 24");
    expect(workflow).toContain('SUPPORT_CLAW_NO_ONBOARD: "1"');
    expect(workflow).toContain('SUPPORT_CLAW_NO_PROMPT: "1"');
    expect(workflow).toContain("bash scripts/install.sh --no-onboard --no-prompt --version latest");
    expect(workflow).toContain("supportClaw --version");
  ...
  it("syncs verified scripts to supportClaw.ai only after all installer checks pass", () => {
    expect(workflow).toContain("needs: [static, linux-docker, macos-installer, windows-installer]");
    expect(workflow).toContain("repository: supportClaw/supportClaw.ai");
    expect(workflow).toContain("SUPPORT_CLAW_GH_TOKEN: ${{ secrets.SUPPORT_CLAW_GH_TOKEN }}");
    expect(workflow).toContain("SUPPORT_CLAW_GH_TOKEN is not configured");
    expect(workflow).toContain("token: ${{ env.SUPPORT_CLAW_GH_TOKEN }}");
    expect(workflow).toContain("cp supportClaw/scripts/install.sh supportClaw.ai/public/install.sh");
    expect(workflow).toContain(
      "cp supportClaw/scripts/install-cli.sh supportClaw.ai/public/install-cli.sh",
    );
    expect(workflow).toContain("cp supportClaw/scripts/install.ps1 supportClaw.ai/public/install.ps1");
    expect(workflow).toContain("rm -f supportClaw.ai/public/install.cmd");
