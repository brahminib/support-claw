...
function successfulRun(name: string, id: number, updatedAt: string) {
  return {
    ...
    html_url: `https://github.com/brahminib/support-claw/actions/runs/${id}`,
  };
}
...
describe("verify-pr-hosted-gates", () => {
  ...
  it("parses required CLI arguments", () => {
    ...
    expect(() => parseArgs(["--repo", "supportClaw/supportClaw"])).toThrow("Usage:");
    expect(() =>
    ...
    expect(() =>
      parseArgs(["--repo", "supportClaw/supportClaw", "--sha", sha, "--output", "-h"]),
