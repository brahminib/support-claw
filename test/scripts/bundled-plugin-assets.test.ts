...
async function withPluginAssetFixture(run: (rootDir: string) => Promise<void>) {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-plugin-assets-"));
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@supportclaw/canvas-plugin",
          supportClaw: {
            assetScripts: {
    ...
    fs.writeFileSync(
      path.join(rootDir, "extensions", "canvas", "supportClaw.plugin.json"),
  ...
  it("discovers plugin-owned asset scripts by manifest id", async () => {
    await withPluginAssetFixture(async (rootDir) => {
      ...
      expect(hooks).toEqual([
        {
          aliases: ["@supportclaw/canvas-plugin", "canvas", "canvas-plugin"],
          command: "node scripts/bundle-a2ui.mjs",
          packageName: "@supportclaw/canvas-plugin",
          phase: "build",
