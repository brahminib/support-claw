...
function withReleaseFixture<T>(releases: unknown[], fn: (file: string) => T): T {
  const dir = mkdtempSync(path.join(tmpdir(), "supportClaw-upgrade-baselines-"));
  try {
...
describe("scripts/resolve-upgrade-survivor-baselines", () => {
  ...
  it("keeps the single fallback baseline when no expanded request is provided", () => {
    expect(resolveBaselines(new Map([["fallback", "2026.4.23"]]))).toEqual(["supportClaw@2026.4.23"]);
  });
  ...
  it("resolves release-history to last six stable releases plus explicit legacy anchors", () => {
    ...
    withReleaseFixture(releases, (file) => {
      expect(
        ...
        "supportClaw@2026.4.26",
        "supportClaw@2026.4.25",
        "supportClaw@2026.4.24",
        "supportClaw@2026.4.22",
      ...
      withJsonFixture(
        ...
        (versionsFile) => {
          expect(
            ...
            "supportClaw@2026.5.3-1",
            "supportClaw@2026.5.3",
            "supportClaw@2026.5.2",
            "supportClaw@2026.4.29",
            "supportClaw@2026.4.23",
            "supportClaw@2026.4.15",
          ]);
        },
      );
    });
