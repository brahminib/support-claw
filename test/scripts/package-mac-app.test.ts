...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "supportClaw-plistbuddy-"));
  tempDirs.push(dir);
...
function runStopPackagedAppHarness(killZeroStatus: 0 | 1) {
  const root = mkdtempSync(path.join(tmpdir(), "supportClaw-package-stop-root-"));
  const toolsDir = mkdtempSync(path.join(tmpdir(), "supportClaw-package-stop-tools-"));
  tempDirs.push(root, toolsDir);

  const appRoot = path.join(root, "dist", "SupportClaw.app");
  const appBinary = path.join(appRoot, "Contents", "MacOS", "SupportClaw");
...
function runSwiftCompatibilityHarness(buildConfig: "debug" | "release") {
  const root = mkdtempSync(path.join(tmpdir(), "supportClaw-package-swift-root-"));
  const toolsDir = mkdtempSync(path.join(tmpdir(), "supportClaw-package-swift-tools-"));
  const developerDir = path.join(root, "Xcode.app", "Contents", "Developer");
  const appRoot = path.join(root, "SupportClaw.app");
...
describe("package-mac-app plist stamping", () => {
  ...
  it("falls back to corepack pnpm when the pnpm shim is absent", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "supportClaw-package-pnpm-root-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "supportClaw-package-pnpm-tools-"));
    ...
    const result = runHelper(`
      ...
      SUPPORT_CLAW_TEST_LOG=${JSON.stringify(logPath)}
      export SUPPORT_CLAW_TEST_LOG
      PATH=${JSON.stringify(`${toolsDir}:/usr/bin:/bin`)}
      ${helperBlock}
  ...
  it("prefers repo Corepack pnpm over a global pnpm shim", () => {
    const helperBlock = getPackageManagerHelperBlock();
    const tempRoot = mkdtempSync(path.join(tmpdir(), "supportClaw-package-pnpm-root-"));
    const outerRoot = mkdtempSync(path.join(tmpdir(), "supportClaw-package-pnpm-outer-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "supportClaw-package-pnpm-tools-"));
  ...
  it("runs Sparkle build metadata derivation from the repository root", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "supportClaw-package-sparkle-root-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "supportClaw-package-sparkle-tools-"));
    tempDirs.push(tempRoot, toolsDir);
    ...
    const result = runHelper(`
      set -euo pipefail
      ROOT_DIR=${JSON.stringify(tempRoot)}
      SUPPORT_CLAW_ROOT=${JSON.stringify(tempRoot)}
      PATH=${JSON.stringify(`${toolsDir}:/usr/bin:/bin`)}
      export SUPPORT_CLAW_ROOT PATH
      cd /tmp
      ${helperBlock}
  ...
  it("does not kill unrelated SupportClaw processes during packaging", () => {
    const script = readFileSync(scriptPath, "utf8");
    ...
    expect(script).not.toContain("killall -q SupportClaw");
    expect(stopBlock).toContain('local app_binary="$APP_ROOT/Contents/MacOS/SupportClaw"');
  ...
  it("fails when the packaged app survives forced shutdown", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain("ERROR: Packaged SupportClaw bundle did not exit: 123");
  });
  ...
  it("fails closed when required Swift resources are missing", () => {
    const script = readFileSync(scriptPath, "utf8");
    const supportClawKitBlock = script.slice(
      script.indexOf(
        'SUPPORT_CLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SupportClawKit_SupportClawKit.bundle"',
      ),
      script.indexOf('echo "📦 Copying Textual resources"'),
    );

    expect(supportClawKitBlock).toContain("ERROR: SupportClawKit resource bundle not found");
    expect(supportClawKitBlock).toContain("exit 1");
    expect(supportClawKitBlock).not.toContain("WARN:");
    expect(supportClawKitBlock).not.toContain("continuing");
  });
  ...
  it.runIf(process.platform === "darwin")(
    ...
    () => {
      ...
      const result = runHelper(`
        ...
        plist_set_string_required ${JSON.stringify(plist)} CFBundleIdentifier 'ai.supportClaw.test'
      ...
      expect(result.status).toBe(1);
      expect(result.stdout).toContain("ai.supportClaw.test");
