...
describe("gh-read helpers", () => {
  ...
  it("prints wrapper usage before reading auth env", () => {
    ...
    expect(stderr).toContain("SUPPORT_CLAW_GH_READ_APP_ID");
  });

  it("finds repo from gh args", () => {
    expect(parseRepoArg(["pr", "view", "42", "-R", "supportClaw/supportClaw"])).toBe("supportClaw/supportClaw");
    expect(parseRepoArg(["run", "list", "--repo=supportClaw/docs"])).toBe("supportClaw/docs");
    expect(parseRepoArg(["pr", "view", "42"])).toBeNull();
  });

  it("normalizes repo strings from common git formats", () => {
    expect(normalizeRepo("supportClaw/supportClaw")).toBe("supportClaw/supportClaw");
    expect(normalizeRepo("github.com/brahminib/support-claw")).toBe("supportClaw/supportClaw");
    expect(normalizeRepo("https://github.com/brahminib/support-claw.git")).toBe("supportClaw/supportClaw");
    expect(normalizeRepo("git@github.com:supportClaw/supportClaw.git")).toBe("supportClaw/supportClaw");
  ...
  it("rejects invalid GitHub API timeout values", () => {
    ...
    expect(() => resolveGitHubFetchTimeoutMs("1s")).toThrow(
      /SUPPORT_CLAW_GH_READ_FETCH_TIMEOUT_MS must be an integer/u,
    );
  });
});
