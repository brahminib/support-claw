...
describe("plugin npm runtime build planning", () => {
  it("plans package-local runtime entries for every publishable plugin package", () => {
    ...
    for (const plan of resolvedPlans) {
      ...
      expect(plan.packagePeerMetadata.peerDependencies.supportClaw).toBe(
        plan.packageJson.supportClaw.compat.pluginApi,
      );
      expect(plan.packagePeerMetadata.peerDependenciesMeta.supportClaw.optional).toBe(true);
    }
  });
