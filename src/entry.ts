#!/usr/bin/env node
// Boots the SupportClaw CLI entry point under Node.
// CLI process entrypoint for SupportClaw command execution.
...
import {
  enableSupportClawCompileCache,
  resolveEntryInstallRoot,
  respawnWithoutSupportClawCompileCacheIfNeeded,
...
import { ensureSupportClawExecMarkerOnProcess } from "./infra/supportClaw-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "supportClawCli.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "supportClaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  const waitingForCompileCacheRespawn = respawnWithoutSupportClawCompileCacheIfNeeded({
    currentFile: entryFile,
    installRoot,
  });
  if (!waitingForCompileCacheRespawn) {
    process.title = "supportClaw";
    ...
    if (shouldForceReadOnlyAuthStore(process.argv)) {
      process.env.SUPPORT_CLAW_AUTH_STORE_READONLY = "1";
    }
    ...
    if (!ensureCliRespawnReady()) {
      ...
      if (!parsedContainer.ok) {
        console.error(`[supportClaw] ${parsedContainer.error}`);
        process.exit(2);
      }
      ...
      if (!parsed.ok) {
        ...
        console.error(`[supportClaw] ${parsed.error}`);
        process.exit(2);
      }

      const containerTargetName = resolveCliContainerTarget(process.argv);
      if (containerTargetName && parsed.profile) {
        console.error("[supportClaw] --container cannot be combined with --profile/--dev");
        process.exit(2);
      }
...
export async function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[supportClaw] Failed to display help:",
...
export async function tryHandlePrecomputedCommandHelpFastPath(
  ...
  if (env.SUPPORT_CLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }
