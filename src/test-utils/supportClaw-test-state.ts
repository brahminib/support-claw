// Creates isolated SupportClaw state directories for integration-style tests.
...
type SupportClawTestStateLayout = "home" | "state-only" | "split";

type SupportClawTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type SupportClawTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: SupportClawTestStateLayout;
  scenario?: SupportClawTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
export type SupportClawTestState = {
  root: string;
  home: string;
  stateDir: string;
  configPath: string;
...
const DEFAULT_PREFIX = "supportClaw-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "home") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(stateDir, "supportClaw.json"),
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "supportClaw.json"),
...
function scenarioConfig(options: SupportClawTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "supportClaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: SupportClawTestStateOptions): Record<string, string | undefined> {
...
export async function createSupportClawTestState(
  options: SupportClawTestStateOptions = {},
): Promise<SupportClawTestState> {
  ...
  const state: SupportClawTestState = {
    root,
    ...paths,
    env,
    envVars,
    ...
    applyEnv: () => {
      ...
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed SupportClaw env set, not plugin-provided host env.
        if (value === undefined) {
...
export async function withSupportClawTestState<T>(
  options: SupportClawTestStateOptions,
  fn: (state: SupportClawTestState) => Promise<T>,
): Promise<T> {
  const state = await createSupportClawTestState(options);
  try {
