...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "SUPPORT_CLAW_ENV_TEST_A";
    const keyB = "SUPPORT_CLAW_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "SUPPORT_CLAW_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "SUPPORT_CLAW_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "SUPPORT_CLAW_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "SUPPORT_CLAW_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "SUPPORT_CLAW_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "SUPPORT_CLAW_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "SUPPORT_CLAW_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "supportClaw-home");
    ...
    const previousBundledDir = process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR;
    process.env.SUPPORT_CLAW_HOME = "/srv/supportClaw-home";
    process.env.SUPPORT_CLAW_STATE_DIR = "/srv/supportClaw-state";
    process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR = "/srv/supportClaw-bundled";
    ...
    try {
      ...
      expect(env.SUPPORT_CLAW_HOME).toBeUndefined();
      expect(env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.SUPPORT_CLAW_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("SUPPORT_CLAW_HOME", previousSupportClawHome);
      restoreEnvKey("SUPPORT_CLAW_STATE_DIR", previousStateDir);
      restoreEnvKey("SUPPORT_CLAW_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processSupportClawHome: process.env.SUPPORT_CLAW_HOME,
          processBundledDir: process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processSupportClawHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.SUPPORT_CLAW_HOME).toBe("/srv/supportClaw-home");
    } finally {
      restoreEnvKey("SUPPORT_CLAW_HOME", previousSupportClawHome);
    }
  });
