...
/** Resolves `~` and SupportClaw home-relative paths with injectable env/home sources. */
...
/** Resolves the SupportClaw config directory from state/config env overrides or home. */
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".supportClaw");
  try {
...
/** Resolves the effective SupportClaw home directory, if one can be determined. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SUPPORT_CLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SUPPORT_CLAW_HOME" };
  }
  return { home, prefix: "~" };
}

/** Replaces the leading home directory in a path with `~` or `$SUPPORT_CLAW_HOME`. */
