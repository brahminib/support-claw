...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: SupportClawConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    cliUrlOverride || options.ignoreEnvUrlOverride || options.localPortOverride !== undefined
      ? undefined
      : normalizeOptionalString(process.env.SUPPORT_CLAW_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
