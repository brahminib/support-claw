...
const getRuntimeConfig = vi.hoisted(() =>
  vi.fn<() => SupportClawConfig>(() => ({}) as SupportClawConfig),
);
...
function setRuntimeConfig(config: SupportClawConfig): void {
  getRuntimeConfig.mockReturnValue(config);
}

function pluginEntries(...ids: string[]): SupportClawConfig["plugins"] {
  return {
    entries: Object.fromEntries(ids.map((id) => [id, { enabled: true }])),
  };
}

function telegramConfig(): SupportClawConfig {
  return {
    channels: {
      telegram: {
...
function telegramSlackConfig(params: { includeMainSession?: boolean } = {}): SupportClawConfig {
  return {
...
function msteamsConfig(): SupportClawConfig {
  return {
    channels: {
      msteams: {
...
function slackSynologyConfig(): SupportClawConfig {
  return {
    channels: {
      slack: {
        botToken: "xoxb-slack-token",
...
function slackConfig(params: { includeMainSession?: boolean } = {}): SupportClawConfig {
  return {
    ...(params.includeMainSession ? { session: { mainKey: "main" } } : {}),
    channels: {
      slack: {
...
describe("cron method validation", () => {
  beforeEach(() => {
    getRuntimeConfig.mockReset().mockReturnValue({} as SupportClawConfig);
