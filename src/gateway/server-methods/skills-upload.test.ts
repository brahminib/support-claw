  ...
  return {
    ...
    movePathWithCopyFallback: async (
      ...
      if (
        ...
        options.from.includes(".supportClaw-install-stage-") &&
...
let tempDirs: string[] = [];
let testStates: SupportClawTestState[] = [];
...
async function makeHarness(): Promise<{
  ...
  const testState = await createSupportClawTestState({
    layout: "state-only",
    prefix: "supportClaw-skill-upload-handler-",
  });
