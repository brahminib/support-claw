...
import {
  ...
  fetchSupportClawSkillSecurityVerdicts,
...
export const skillsHandlers: GatewayRequestHandlers = {
  ...
  "skills.securityVerdicts": async ({ params, respond, context }) => {
    ...
    try {
      ...
      if (targets.length === 0) {
        respond(true, { schema: "supportClaw.skills.security-verdicts.v1", items: [] }, undefined);
        return;
      }
      const items = await fetchSupportClawSkillSecurityVerdicts(targets);
      respond(true, { schema: "supportClaw.skills.security-verdicts.v1", items }, undefined);
    } catch (err) {
