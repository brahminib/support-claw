...
const ORIGINAL_STATE_DIR = process.env.SUPPORT_CLAW_STATE_DIR;
...
beforeEach(async () => {
  stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-gateway-tasks-"));
...
afterEach(async () => {
  ...
  if (ORIGINAL_STATE_DIR === undefined) {
    ...
    process.env.SUPPORT_CLAW_STATE_DIR = ORIGINAL_STATE_DIR;
  }
...
describe("tasks gateway handlers", () => {
  ...
  it("sanitizes task text before exposing SDK summaries", async () => {
    const task = createTaskRecord({
      ...
      runId: "run-sanitized",
      label:
        "Compile artifact\nSupportClaw runtime context (internal): Keep internal details private.",
    ...
    expect(JSON.stringify(calls[0]?.[1])).not.toContain("SupportClaw runtime context");
  });
