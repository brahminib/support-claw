...
import { isAudioFileName } from "@supportclaw/media-core/mime";
import { asOptionalRecord } from "@supportclaw/normalization-core/record-coerce";
import type { FastMode } from "@supportclaw/normalization-core/string-coerce";
...
function resolveWebchatPromptCacheKey(params: {
  ...
  const digest = createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 32);
  return `supportClaw-webchat-${digest}`;
}
...
export function buildOversizedHistoryPlaceholder(message?: unknown): Record<string, unknown> {
  ...
  const rawMetadata =
    message && typeof message === "object"
      ? (message as Record<string, unknown>)["__supportClaw"]
      : undefined;
  const metadata =
...
async function findSourceReplyTranscriptMirrorByIdempotencyKey(
  ...
  if (found?.message.provider !== "supportClaw" || found.message.model !== "delivery-mirror") {
    return null;
  }
  return found;
}
...
function sendGlobalAwareNodeChatPayload(params: {
  ...
  const deliveryKeys = resolveGlobalAwareNodeChatDeliveryKeys({
    cfg: params.context.getRuntimeConfig?.() ?? ({} as SupportClawConfig),
    sessionKey: params.sessionKey,
    agentId: params.agentId,
  });
...
function readChatHistoryMessageId(message: unknown): string | undefined {
  const metadata = asOptionalRecord(asOptionalRecord(message)?.["__supportClaw"]);
...
async function handleChatHistoryRequest({
  ...
  const requestedAgentId = resolveRequestedChatAgentId({
    cfg: (context as { getRuntimeConfig?: () => SupportClawConfig }).getRuntimeConfig?.(),
...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
      ...
      try {
        await measureDiagnosticsTimelineSpan(
          ...
          async () => {
            ...
            // attachments even when the parent SupportClaw session model is text-only.
    ...
    try {
      ...
      // See: https://github.com/brahminib/support-claw/issues/3658
        ...
        .then(async () => {
          ...
          await measureDiagnosticsTimelineSpan(
            ...
            async () => {
              ...
              // through SupportClaw runtime's SessionManager; this dispatcher only owns live delivery payloads.
                  ...
                  if (shouldAppendAssistantTranscript) {
                    ...
                    if (appended.ok) {
                      ...
                      message = {
                        ...
                        timestamp: nowValue,
                        ...(ttsSupplementMarker
                          ? { supportClawTtsSupplement: ttsSupplementMarker }
                          : {}),
