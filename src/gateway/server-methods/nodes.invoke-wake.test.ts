...
describe("node plugin surface refresh", () => {
  it("refreshes generic plugin surface capability urls", async () => {
    ...
    expect(parsedCanvasUrl.pathname.startsWith("/__supportClaw__/cap/")).toBe(true);
    const capabilityToken = parsedCanvasUrl.pathname.slice("/__supportClaw__/cap/".length);
