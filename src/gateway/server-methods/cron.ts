...
async function listConfiguredAnnounceChannelIds(cfg: SupportClawConfig): Promise<string[]> {
  return await listConfiguredMessageChannels(cfg);
}

function hasExplicitChannelConfigEntry(cfg: SupportClawConfig): boolean {
...
async function assertValidCronCreateDelivery(cfg: SupportClawConfig, jobCreate: CronJobCreate) {
