...
vi.mock("../../skills/security/clawhub-verdicts.js", () => ({
  ...
  fetchSupportClawSkillSecurityVerdicts: vi.fn(),
}));
...
describe("skills proposal gateway handlers", () => {
  beforeEach(async () => {
    testState = await createSupportClawTestState({
      layout: "state-only",
      prefix: "supportClaw-skills-proposals-gateway-state-",
    });
    ...
    mocks.workspaceDir = await tempDirs.make("supportClaw-skills-proposals-gateway-");
  ...
  it("scopes list and inspect to the resolved agent workspace", async () => {
    ...
    const secondWorkspaceDir = await tempDirs.make("supportClaw-skills-proposals-gateway-second-");
