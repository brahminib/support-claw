...
function requestModelsList(params: {
  ...
  const request = modelsHandlers["models.list"]({
    ...
    context: {
      getRuntimeConfig: () => params.runtimeConfig ?? ({} as SupportClawConfig),
...
describe("models.list", () => {
  ...
  it("marks legacy OpenAI Codex aliases available through ChatGPT OAuth", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-models-list-codex-alias-",
        agentEnv: "main",
      },
      async (state) => {
  ...
  it("does not mark catalog rows available from expired OAuth profiles", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-models-list-expired-profile-",
        agentEnv: "main",
      },
      async (state) => {
        await state.writeAuthProfiles({
  ...
  it("marks env SecretRef-backed auth profiles available", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-models-list-env-profile-",
        agentEnv: "main",
        env: {
  ...
  it("keeps non-env SecretRef-backed auth profile availability unknown", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-models-list-file-profile-",
        agentEnv: "main",
      },
      async (state) => {
        await state.writeAuthProfiles({
  ...
  it("marks auth profiles available even when provider config uses non-env SecretRef markers", async () => {
    for (const fixture of [
      ...
      await withSupportClawTestState(
        {
          layout: "state-only",
          prefix: `supportClaw-models-list-provider-${fixture.name}-profile-`,
          agentEnv: "main",
          env: {
            SUPPORT_CLAW_TEST_PROFILE_API_KEY: "test-token",
            ...
            profiles: {
              "vllm:env": {
                ...
                keyRef: {
                  source: "env",
                  provider: "default",
                  id: "SUPPORT_CLAW_TEST_PROFILE_API_KEY",
                },
              },
            },
          });
