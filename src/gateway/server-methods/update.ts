...
> {
  ...
  return {
    ...
    authoredConfig: isRecord(snapshot.parsed)
      ? (snapshot.parsed as SupportClawConfig)
...
function hasManagedServiceHandoffContext(
  ...
  if (supervisor === "launchd") {
    return Boolean(
      env.SUPPORT_CLAW_LAUNCHD_LABEL?.trim() ||
  ...
  if (supervisor === "systemd") {
    ...
    return Boolean(env.SUPPORT_CLAW_SYSTEMD_UNIT?.trim());
  }
  if (supervisor === "schtasks") {
    return Boolean(
      env.SUPPORT_CLAW_WINDOWS_TASK_NAME?.trim() ||
  ...
  "update.run": async ({ params, respond, client, context }) => {
    ...
    try {
      ...
      if (!isRestartEnabled(config) && !supervisor) {
        ...
        // The CLI `supportClaw update` resumes post-core plugin convergence after a
