...
const createdStates: SupportClawTestState[] = [];

async function createState(label: string): Promise<SupportClawTestState> {
  const state = await createSupportClawTestState({ label, layout: "state-only" });
