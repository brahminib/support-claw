...
import type { ConfigFileSnapshot, SupportClawConfig } from "../../config/types.supportClaw.js";
...
const startManagedServiceUpdateHandoffMock = vi.fn(async () => ({
  ...
  pid: 12345,
  command: "supportClaw update --yes --timeout 1800",
  logPath: "/tmp/supportClaw-update-run-handoff/handoff.log",
}));
...
vi.mock("../../infra/supportClaw-root.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/supportClaw-root.js")>(
    "../../infra/supportClaw-root.js",
  );
  return {
    ...actual,
    resolveSupportClawPackageRoot: async () => "/tmp/supportClaw",
  };
});
...
vi.mock("../../infra/update-managed-service-handoff.js", () => ({
  ...
  formatManagedServiceUpdateCommand: (params?: {
    ...
    params?.timeoutMs
      ? `supportClaw update --yes --timeout ${Math.ceil(params.timeoutMs / 1000)}`
      : "supportClaw update --yes",
  buildManagedServiceHandoffUnavailableMessage: (command: string) =>
    [
      "SupportClaw updates cannot safely run inside the live gateway process without a managed-service handoff.",
...
beforeEach(() => {
  ...
  readConfigFileSnapshotMock.mockResolvedValue({
    ...
    raw: "{}",
    parsed: {},
    resolved: {} as SupportClawConfig,
    sourceConfig: {} as SupportClawConfig,
    valid: true,
    config: {} as SupportClawConfig,
    runtimeConfig: {} as SupportClawConfig,
    issues: [],
    warnings: [],
    legacyIssues: [],
  });
...
function mockGlobalInstallSurface() {
  resolveUpdateInstallSurfaceMock.mockResolvedValueOnce({
    kind: "global",
    mode: "npm",
    root: "/tmp/supportClaw-global",
    packageRoot: "/tmp/supportClaw-global",
  });
}
...
describe("update.run restart scheduling", () => {
  ...
  it("hands managed package updates to the CLI path instead of running them in-process", async () => {
    ...
    const payload = await withProcessEnv({ SUPPORT_CLAW_LAUNCHD_LABEL: "ai.supportClaw.gateway" }, () =>
  ...
  it("keeps a startup grace before restarting after systemd handoff spawn", async () => {
    ...
    await withProcessEnv({ SUPPORT_CLAW_SYSTEMD_UNIT: "supportClaw-gateway.service" }, () =>
  ...
  it("starts managed package handoff when the gateway cwd is unavailable", async () => {
    ...
    try {
      await withProcessEnv({ SUPPORT_CLAW_LAUNCHD_LABEL: "ai.supportClaw.gateway" }, () =>
        invokeUpdateRun({}),
      );
    } finally {
      cwdSpy.mockRestore();
  ...
  it("hands supervised git/dev updates to the CLI path instead of rebuilding live dist in-process", async () => {
    detectRespawnSupervisorMock.mockReturnValueOnce("launchd");
    mockGitInstallSurface("/tmp/supportClaw-git");
    const payload = await withProcessEnv({ SUPPORT_CLAW_LAUNCHD_LABEL: "ai.supportClaw.gateway" }, () =>
      captureUpdateRunPayload(),
    );
  ...
  it("returns a safe command when package updates cannot be handed off", async () => {
    ...
    expect(payload?.handoff).toEqual({
      ...
      message:
        "SupportClaw updates cannot safely run inside the live gateway process without a managed-service handoff.\n" +
        "Run `supportClaw update --yes --timeout 1800` from a shell outside the gateway service, or restart/update from the host UI.",
    });
  });
...
describe("update.run post-core plugin finalize", () => {
  ...
  it("resumes official plugin convergence after a git/source core update", async () => {
    runPostCoreFinalizeAfterGatewayUpdateMock.mockResolvedValueOnce({
      status: "ok",
      entrypoint: "/tmp/supportClaw-git/dist/index.mjs",
    });
    mockGitOkUpdate("/tmp/supportClaw-git");
  ...
  it("carries the pre-doctor source config into the git finalizer", async () => {
    ...
    const [finalizeParams] = firstMockCall(
      runPostCoreFinalizeAfterGatewayUpdateMock,
      "post-core finalize",
    ) as [{ preUpdateConfig?: { sourceConfig?: SupportClawConfig; authoredConfig?: SupportClawConfig } }];
