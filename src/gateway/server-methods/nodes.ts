...
import { normalizeUniqueTrimmedStringList } from "@supportclaw/normalization-core/string-normalization";
import {
  type ConnectParams,
  ErrorCodes,
  errorShape,
...
export async function maybeWakeNodeWithApns(
  nodeId: string,
  opts?: { force?: boolean; wakeReason?: string; cfg?: SupportClawConfig },
  ...
  try {
    ...
    if (registration.transport === "relay") {
      ...
      result = await sendApnsAlert({
        registration,
        nodeId,
        title: "SupportClaw needs a quick reopen",
        body: "Tap to reopen SupportClaw and restore the node connection.",
        relayConfig: relay.relayConfig,
      });
    } else {
      const auth = await resolveDirectNodePushConfig();
...
export const nodeHandlers: GatewayRequestHandlers = {
  ...
  // Remove a node pairing (CLI: `supportClaw nodes remove`). For a device-backed
