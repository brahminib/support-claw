...
describe("resolveConfigOpenCommand", () => {
  it("uses open on macOS", () => {
    expect(resolveConfigOpenCommand("/tmp/supportClaw.json", "darwin")).toEqual({
      command: "open",
      args: ["/tmp/supportClaw.json"],
    });
  });

  it("uses xdg-open on Linux", () => {
    expect(resolveConfigOpenCommand("/tmp/supportClaw.json", "linux")).toEqual({
      command: "xdg-open",
      args: ["/tmp/supportClaw.json"],
    });
  });
...
describe("config.openFile", () => {
  it("opens the configured file without shell interpolation", async () => {
    await withEnvAsync({ SUPPORT_CLAW_CONFIG_PATH: "/tmp/config $(touch pwned).json" }, async () => {
      execFileMock.mockImplementation((...args: unknown[]) => {
  ...
  it("returns a detailed error and logs details when the opener fails", async () => {
    await withEnvAsync({ SUPPORT_CLAW_CONFIG_PATH: "/tmp/config.json" }, async () => {
