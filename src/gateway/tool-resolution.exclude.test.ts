...
describe("resolveGatewayScopedTools excludeToolNames", () => {
  beforeEach(() => {
    hoisted.createSupportClawToolsMock.mockClear();
  });
  ...
  function readCreateToolsArgs(): {
    ...
    if (!args || typeof args !== "object") {
      throw new Error("expected createSupportClawTools args");
    }
    return args as {
      cronCreatorToolAllowlist?: Array<string | { name: string; pluginId?: string }>;
      inheritedToolDenylist?: string[];
  ...
  it("passes final filtered tool surface to gateway cron jobs", () => {
    hoisted.createSupportClawToolsMock.mockReturnValueOnce([
