...
describe("gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-gw-"));
    try {
  ...
  test("sessions.send accepts dashboard messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-sessions-send-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.send creates a configured agent main session before sending", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-sessions-send-agent-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
  ...
  test("sessions.steer accepts dashboard follow-up messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-sessions-steer-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort stops active dashboard runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-sessions-abort-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort resolves active runs by runId without a caller session key", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-sessions-abort-runid-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-gw-"));
  ...
  test("chat.history mirrors current-session message tool sends before NO_REPLY", async () => {
    ...
    expect(
      historyMessages.some((message) => {
        ...
        const entry = message as { role?: unknown; supportClawMessageToolMirror?: unknown };
        return entry.role === "assistant" && Boolean(entry.supportClawMessageToolMirror);
      }),
    ).toBe(true);
  });
    ...
    expect(
      historyMessages.some(
        (message) =>
          ...
          Boolean((message as { supportClawMessageToolMirror?: unknown }).supportClawMessageToolMirror),
      ),
    ).toBe(true);
    expect(historyMessages).not.toContainEqual(
      expect.objectContaining({ provider: "supportClaw", model: "delivery-mirror" }),
    );
  });
  ...
  test("chat.history persists assistant image data URLs as managed image blocks", async () => {
    await withMainSessionStore(
      async (dir) => {
        ...
        process.env.SUPPORT_CLAW_STATE_DIR = dir;
        const pngB64 =
