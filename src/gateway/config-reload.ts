...
function asPluginInstallConfig(records: PluginInstallRecords): SupportClawConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: SupportClawConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: SupportClawConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: {
    ...
    compareConfig: SupportClawConfig;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: SupportClawConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (
    ...
    nextCompareConfig: SupportClawConfig,
    afterWrite?: ConfigWriteNotification["afterWrite"],
  ) => {
