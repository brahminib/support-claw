...
describe("live-agent-probes", () => {
  ...
  it("builds a retryable cron prompt with provider-specific fallback wording", () => {
    ...
    expect(claudeRetryPrompt).toContain("search/load MCP tools for `supportClaw cron` or `cron`");
    expect(claudeRetryPrompt).toContain("mcp__supportClaw__cron");
    expect(claudeRetryPrompt).toContain("Do not use Claude native `CronCreate`");
    expect(claudeRetryPrompt).not.toContain("supportClaw-tools");
    expect(
      buildLiveCronProbeMessage({
        agent: "future-agent",
        argsJson: spec.argsJson,
    ...
    ).toContain("previous SupportClaw cron MCP tool call was cancelled");
