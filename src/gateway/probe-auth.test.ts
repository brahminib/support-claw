...
function configWithDefaultEnvProvider(gateway: NonNullable<SupportClawConfig["gateway"]>) {
  return {
    gateway,
    secrets: {
      providers: {
...
function resolveSafeProbeAuth(cfg: SupportClawConfig, mode: "local" | "remote" = "local") {
...
function expectUnresolvedProbeTokenWarning(cfg: SupportClawConfig) {
  ...
  it("resolves env SecretRef token via async secret-inputs path", async () => {
    const result = await resolveGatewayProbeAuthSafeWithSecretInputs({
      cfg: configWithDefaultEnvProvider({
        auth: tokenAuthConfig("SUPPORT_CLAW_GATEWAY_TOKEN"),
      }),
      mode: "local",
      env: {
        SUPPORT_CLAW_GATEWAY_TOKEN: "test-token-from-env",
