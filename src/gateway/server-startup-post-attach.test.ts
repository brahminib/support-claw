...
import { closeSupportClawStateDatabaseForTest } from "../state/supportClaw-state-db.js";
...
const hoisted = vi.hoisted(() => {
  ...
  const ensureSupportClawModelsJson = vi.fn(async () => {});
...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/supportClaw-state",
    resolveConfigPath: vi.fn(() => "/tmp/supportClaw-state/supportClaw.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn((env: NodeJS.ProcessEnv = process.env) =>
      env.SUPPORT_CLAW_STATE_DIR?.trim() ? actual.resolveStateDir(env) : "/tmp/supportClaw-state",
    ),
  };
});
...
vi.mock("../agents/models-config.js", () => ({
  ensureSupportClawModelsJson: hoisted.ensureSupportClawModelsJson,
}));
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("SUPPORT_CLAW_SKIP_CHANNELS", "0");
    vi.stubEnv("SUPPORT_CLAW_SKIP_PROVIDERS", "0");
    ...
    hoisted.ensureSupportClawModelsJson.mockReset();
    hoisted.ensureSupportClawModelsJson.mockResolvedValue(undefined);
  ...
  it("skips heavy restart sentinel refresh when no sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-no-sentinel-"));
  ...
  it("refreshes the restart sentinel when the sentinel row exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sentinel-"));
    try {
      await writeRestartSentinel(
        ...
        { SUPPORT_CLAW_STATE_DIR: stateDir } as NodeJS.ProcessEnv,
      );
      vi.stubEnv("SUPPORT_CLAW_STATE_DIR", stateDir);
  ...
  it("detects restart sentinel rows in explicit state directories", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sentinel-state-"));
    try {
      await writeRestartSentinel(
        {
          kind: "update",
  ...
  it("avoids sync filesystem probes while checking restart sentinel presence", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-async-sentinel-"));
    try {
      await writeRestartSentinel(
        {
          kind: "update",
  ...
  it("defers qmd memory backend startup refresh until the idle delay elapses", async () => {
    ...
    try {
      await startGatewaySidecars({
        ...
        defaultWorkspaceDir: "/tmp/supportClaw-workspace",
        deps: {} as never,
  ...
  it("cleans startup session locks with bounded concurrency", async () => {
    ...
    let maxActive = 0;
    const cleanedLock = {
      lockPath: "/tmp/supportClaw-state/agents/main/sessions/a.jsonl.lock",
      pid: null,
      pidAlive: false,
      createdAt: null,
      ageMs: null,
  ...
  it("starts channels when channel startup is enabled", async () => {
    await withEnvAsync(
      {
        SUPPORT_CLAW_SKIP_CHANNELS: undefined,
        SUPPORT_CLAW_SKIP_PROVIDERS: undefined,
      },
      async () => {
  ...
  it("starts and reports plugin services after channel startup completes", async () => {
    await withEnvAsync(
      { SUPPORT_CLAW_SKIP_CHANNELS: undefined, SUPPORT_CLAW_SKIP_PROVIDERS: undefined },
      async () => {
  ...
  it("emits a startup trace span when channel startup is skipped", async () => {
    ...
    await withEnvAsync(
      { SUPPORT_CLAW_SKIP_CHANNELS: "1", SUPPORT_CLAW_SKIP_PROVIDERS: undefined },
      async () => {
        await startGatewaySidecars({
          cfg: { hooks: { internal: { enabled: false } } } as never,
          pluginRegistry: createPostAttachParams().pluginRegistry,
    ...
    expect(logChannels.info).toHaveBeenCalledWith(
      "skipping channel start (SUPPORT_CLAW_SKIP_CHANNELS=1 or SUPPORT_CLAW_SKIP_PROVIDERS=1)",
    );
  });
