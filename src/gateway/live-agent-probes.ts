...
const SUPPORT_CLAW_CLI_GATEWAY_TIMEOUT_MS = 30_000;
const SUPPORT_CLAW_CLI_CHILD_TIMEOUT_MS = SUPPORT_CLAW_CLI_GATEWAY_TIMEOUT_MS + 45_000;
...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the SupportClaw MCP cron tool from server `supportClaw`. " +
      "If it is not already visible, search/load MCP tools for `supportClaw cron` or `cron`, " +
      "then call the matching SupportClaw MCP tool; Claude-style names may appear as `mcp__supportClaw__cron`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not SupportClaw proof. " +
      ...
      "Do the actual tool call; I will verify externally with the SupportClaw cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the SupportClaw MCP cron tool from server `supportClaw` now. " +
      "If it is not already visible, search/load MCP tools for `supportClaw cron` or `cron`, " +
      "then call the matching SupportClaw MCP tool; Claude-style names may appear as `mcp__supportClaw__cron`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not SupportClaw proof. " +
      ...
      "I will verify externally with the SupportClaw cron CLI."
    );
  }
  return (
    "Your previous SupportClaw cron MCP tool call was cancelled before the job was created. " +
    "Retry the SupportClaw MCP cron tool from server `supportClaw` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__supportClaw__cron`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
    `If the cron job is created, reply exactly: ${params.exactReply}. ` +
    "If the tool call is cancelled, the job is not created, or you cannot confirm creation, " +
...
export async function runSupportClawCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  ...
  const cliArgs = args.includes("--timeout")
    ? args
    : [...args, "--timeout", String(SUPPORT_CLAW_CLI_GATEWAY_TIMEOUT_MS)];
  const { stdout, stderr } = await execFileAsync(process.execPath, ["supportClawCli.mjs", ...cliArgs], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: SUPPORT_CLAW_CLI_CHILD_TIMEOUT_MS,
    maxBuffer: 1024 * 1024,
  });
  ...
  if (!trimmed) {
    throw new Error(
      [
        `supportClaw ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `supportClaw ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runSupportClawCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
