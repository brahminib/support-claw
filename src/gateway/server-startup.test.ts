...
const ensureSupportClawModelsJsonMock = vi.fn<
  (
    config: unknown,
    agentDir: unknown,
    options?: unknown,
...
vi.mock("../agents/models-config.js", () => ({
  ensureSupportClawModelsJson: (config: unknown, agentDir: unknown, options?: unknown) =>
    ensureSupportClawModelsJsonMock(config, agentDir, options),
}));
...
function expectModelsJsonPrewarmCall(cfg: SupportClawConfig) {
  expect(ensureSupportClawModelsJsonMock).toHaveBeenCalledTimes(1);
  const [calledConfig, agentDir, options] = ensureSupportClawModelsJsonMock.mock.calls.at(0) ?? [];
...
describe("gateway startup primary model warmup", () => {
  ...
  beforeEach(() => {
    ensureSupportClawModelsJsonMock.mockClear();
  ...
  it("honors the startup model prewarm skip env", () => {
    ...
    expect(
      shouldSkipStartupModelPrewarm({
        SUPPORT_CLAW_SKIP_STARTUP_MODEL_PREWARM: "1",
      }),
    ).toBe(true);
    expect(
      shouldSkipStartupModelPrewarm({
        SUPPORT_CLAW_SKIP_STARTUP_MODEL_PREWARM: "true",
      }),
    ).toBe(true);
  });
  ...
  it("warns when scoped models.json preparation fails", async () => {
    ensureSupportClawModelsJsonMock.mockRejectedValueOnce(new Error("models write failed"));
    const warn = vi.fn();
