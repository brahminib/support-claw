...
export async function prepareGatewayStartupConfig(params: {
  ...
  const canReusePreflightPreparedSnapshot = (config: SupportClawConfig): boolean =>
    Boolean(
      preflightPrepared &&
  ...
  const activateStartupSecrets = async (config: SupportClawConfig) => {
...
function hasActiveGatewayAuthSecretRef(config: SupportClawConfig): boolean {
...
function pruneSkippedStartupSecretSurfaces(config: SupportClawConfig): SupportClawConfig {
  const skipChannels =
...
function assertRuntimeGatewayAuthNotKnownWeak(config: SupportClawConfig): void {
