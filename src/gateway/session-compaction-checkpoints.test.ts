...
import { CURRENT_SESSION_VERSION, SessionManager } from "supportClaw/plugin-sdk/agent-sessions";
import type { AssistantMessage } from "supportClaw/plugin-sdk/llm";
...
function checkpointConfig(storePath: string): SupportClawConfig {
  return {
...
describe("session-compaction-checkpoints", () => {
  ...
  test("async capture stores pre-compaction identity without copying the transcript", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-checkpoint-async-"));
    tempDirs.push(dir);
  ...
  test("async capture derives session metadata without synchronous SessionManager.open", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-checkpoint-async-metadata-"));
    tempDirs.push(dir);
  ...
  test("async capture follows terminal leaf controls instead of their inactive parent", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-checkpoint-leaf-control-"));
    tempDirs.push(dir);
  ...
  test("async leaf scans ignore controls with dangling references", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-checkpoint-invalid-leaf-"));
    tempDirs.push(dir);
    const sessionFile = path.join(dir, "session.jsonl");
    await fs.writeFile(
      sessionFile,
  ...
  test("async capture scans bounded metadata without copying oversized transcripts", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-checkpoint-async-oversized-"));
    tempDirs.push(dir);
  ...
  test("bounded capture falls back to a forkable raw tail when the leaf target is older", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-checkpoint-bounded-leaf-"));
    tempDirs.push(dir);
    const sessionFile = path.join(dir, "session.jsonl");
    await fs.writeFile(
      sessionFile,
  ...
  test("async fork creates a checkpoint branch transcript without SessionManager sync reads", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-checkpoint-fork-"));
    tempDirs.push(dir);
  ...
  test("async fork truncates mutable checkpoint sources at the stored leaf", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-checkpoint-fork-leaf-"));
    tempDirs.push(dir);
  ...
  test("async fork migrates legacy checkpoint snapshots before writing a current header", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-checkpoint-legacy-fork-"));
    tempDirs.push(dir);
  ...
  test("async fork skips JSON-valid garbage transcript entries", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-checkpoint-garbage-fork-"));
    tempDirs.push(dir);
  ...
  test("persist skips codex-style checkpoints without a stable post-compaction leaf", async () => {
    const { storePath, sessionId, sessionKey, now } = await makeTempSessionStore(
      "supportClaw-checkpoint-no-leaf-",
    );
    await writeSessionStore(storePath, sessionKey, {
      sessionId,
      updatedAt: now,
  ...
  test("persist trims retained checkpoint snapshots by total byte budget", async () => {
    const { dir, storePath, sessionId, sessionKey, now } = await makeTempSessionStore(
      "supportClaw-checkpoint-byte-trim-",
    );
