...
const mocks = vi.hoisted(() => ({
  replaceConfigFile: vi.fn(async (_params: { nextConfig: SupportClawConfig }) => {}),
}));
...
type GatewayAuthConfig = NonNullable<NonNullable<SupportClawConfig["gateway"]>["auth"]>;
...
function gatewayAuthConfig(auth: GatewayAuthConfig): SupportClawConfig {
  return {
    gateway: { auth },
  };
}

function gatewayAuthConfigWithDefaultEnvProvider(auth: GatewayAuthConfig): SupportClawConfig {
  return {
...
describe("mergeGatewayTailscaleConfig", () => {
  ...
  it("preserves explicit serviceName overrides", () => {
    expect(
      mergeGatewayTailscaleConfig(
        { mode: "serve", serviceName: "svc:old-supportClaw", resetOnExit: false },
        { serviceName: "svc:supportClaw" },
      ),
    ).toEqual({ mode: "serve", serviceName: "svc:supportClaw", resetOnExit: false });
  });
});
...
describe("ensureGatewayStartupAuth", () => {
  ...
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: SupportClawConfig) {
  ...
  async function expectNoTokenGeneration(cfg: SupportClawConfig, mode: string) {
    const result = await runStartupAuth({
      cfg,
      persist: true,
    });
  ...
  function createMissingGatewayTokenSecretRefConfig(): SupportClawConfig {
  ...
  it("resolves env-template gateway.auth.token before env-token short-circuiting", async () => {
    await expectResolvedToken({
      cfg: gatewayAuthConfig({ mode: "token", token: "${SUPPORT_CLAW_GATEWAY_TOKEN}" }),
      env: {
        SUPPORT_CLAW_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${SUPPORT_CLAW_GATEWAY_TOKEN}",
    });
  });

  it("uses SUPPORT_CLAW_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    await expectResolvedToken({
      cfg: createMissingGatewayTokenSecretRefConfig(),
      env: {
        SUPPORT_CLAW_GATEWAY_TOKEN: "token-from-env",
      } as NodeJS.ProcessEnv,
  ...
  it("uses SUPPORT_CLAW_GATEWAY_PASSWORD without resolving configured password SecretRef", async () => {
    const result = await runStartupAuth({
      cfg: gatewayAuthConfigWithDefaultEnvProvider({
        mode: "password",
      ...
      env: {
        SUPPORT_CLAW_GATEWAY_PASSWORD: "password-from-env", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      persist: true,
    });
  ...
  it("keeps startup non-breaking when hooks token reuses gateway token resolved from env", async () => {
    ...
    const result = await runStartupAuth({
      ...
      env: {
        SUPPORT_CLAW_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
      } as NodeJS.ProcessEnv,
      warn,
    });
    ...
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("supportClaw security audit"));
  });
