...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.SUPPORT_CLAW_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: SupportClawConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: SupportClawConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: SupportClawConfig, requestedAgentId: string) => {
