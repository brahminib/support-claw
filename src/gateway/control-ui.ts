...
import { detectMime } from "@supportclaw/media-core/mime";
import {
  asDateTimestampMs,
...
const ROOT_PREFIX = "/";
const CONTROL_UI_ASSISTANT_MEDIA_PREFIX = "/__supportClaw__/assistant-media";
...
const CONTROL_UI_NAMESPACE_PREFIX = "/__supportClaw__/";
...
// `/__supportClaw__/`, so a browser that opens the default entry infers
// `/__supportClaw__` as its base path (see `inferBasePathFromPathname`) and fetches
// `/__supportClaw__/control-ui-config.json`. Accept that namespaced alias so the
...
// Single-underscore `/__supportClaw` prefix used by the pre-base-path-relative
...
// `/__supportClaw/control-ui-config.json`, so current main and the v2026.6.1
// release serve and document that exact path under an empty base path.
const LEGACY_CONTROL_UI_NAMESPACE_PREFIX = "/__supportClaw";

// The old documented no-base-path bootstrap endpoint
// (`/__supportClaw/control-ui-config.json`, single underscore). It is derived from
// the legacy `/__supportClaw` namespace joined with the canonical config constant
 ...
 * single-underscore suffix `${basePath}/__supportClaw/control-ui-config.json` that
 ...
 * the default-namespace alias `/__supportClaw__/control-ui-config.json`, which is
 * what the default `/__supportClaw__/` entry requests after inferring its base path
...
function matchesControlUiBootstrapConfigPath(pathname: string, basePath: string): boolean {
  ...
  // `/__supportClaw` namespace, so joining it with the base path yields
  // `${basePath}/__supportClaw/control-ui-config.json` (or the bare legacy path
  ...
  // The default `/__supportClaw__/` namespace alias only applies when no base path
