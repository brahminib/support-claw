...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "supportClaw-test-empty-bundled-plugins");
  await fs.mkdir(bundledPluginsDir, { recursive: true });
  return bundledPluginsDir;
}

async function createGatewayConfigPath(tempHome: string): Promise<string> {
  const configPath = path.join(tempHome, ".supportClaw", "supportClaw.json");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".supportClaw", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["SUPPORT_CLAW_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);

  const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), params.prefix));
  setTestEnvValue("HOME", tempHome);
  setTestEnvValue("SUPPORT_CLAW_STATE_DIR", path.join(tempHome, ".supportClaw"));
  deleteTestEnvValue("SUPPORT_CLAW_CONFIG_PATH");
  setTestEnvValue("SUPPORT_CLAW_SKIP_CHANNELS", "1");
  setTestEnvValue("SUPPORT_CLAW_SKIP_GMAIL_WATCHER", "1");
  setTestEnvValue("SUPPORT_CLAW_SKIP_CRON", "1");
  setTestEnvValue("SUPPORT_CLAW_SKIP_CANVAS_HOST", "1");
  setTestEnvValue("SUPPORT_CLAW_SKIP_BROWSER_CONTROL_SERVER", "1");
  setTestEnvValue("SUPPORT_CLAW_SKIP_PROVIDERS", "1");
  if (params.minimalGateway) {
    setTestEnvValue("SUPPORT_CLAW_TEST_MINIMAL_GATEWAY", "1");
  } else {
    deleteTestEnvValue("SUPPORT_CLAW_TEST_MINIMAL_GATEWAY");
  }

  const workspaceDir = path.join(tempHome, "supportClaw");
  await fs.mkdir(workspaceDir, { recursive: true });
  setTestEnvValue("SUPPORT_CLAW_BUNDLED_PLUGINS_DIR", await createEmptyBundledPluginsDir(tempHome));
  setTestEnvValue("SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS", "1");
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "supportClaw-gw-mock-home-",
        minimalGateway: true,
      });

      const token = nextGatewayId("test-token");
      setTestEnvValue("SUPPORT_CLAW_GATEWAY_TOKEN", token);
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "supportClaw-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome } = await setupGatewayTempHome({
        prefix: "supportClaw-wizard-home-",
        minimalGateway: true,
      });
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-minimal-gateway-home-"));
      const configPath = await createGatewayConfigPath(tempHome);
      const bundledPluginsDir = path.join(tempHome, "supportClaw-test-no-bundled-extensions");
      setTestEnvValue("HOME", tempHome);
      setTestEnvValue("SUPPORT_CLAW_STATE_DIR", path.join(tempHome, ".supportClaw"));
      setTestEnvValue("SUPPORT_CLAW_CONFIG_PATH", configPath);
      setTestEnvValue("SUPPORT_CLAW_SKIP_CHANNELS", "1");
      ...
      setTestEnvValue("SUPPORT_CLAW_SKIP_BROWSER_CONTROL_SERVER", "1");
      setTestEnvValue("SUPPORT_CLAW_SKIP_PROVIDERS", "1");
      setTestEnvValue("SUPPORT_CLAW_BUNDLED_PLUGINS_DIR", bundledPluginsDir);
      setTestEnvValue("SUPPORT_CLAW_TEST_MINIMAL_GATEWAY", "1");
