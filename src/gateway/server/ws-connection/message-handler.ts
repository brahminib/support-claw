...
/**
 * Lazily resolve the local node host's nodeId from ~/.supportClaw/node.json.
 * Process-stable: only changes on `supportClaw node install`, which requires restart.
 */
...
function resolveTrustedProxyControlUiScopes(params: {
  ...
  const rawHeader = firstHeaderValue(params.upgradeReq.headers["x-supportClaw-scopes"]);
  ...
  const handleMessage = async (data: RawData) => {
    ...
    try {
      ...
      if (!client) {
        ...
        // matching instanceId against ~/.supportClaw/node.json nodeId). SSH-tunneled remote
