...
type GatewayReloadHandlerParams = {
  ...
  createHealthMonitor: (config: SupportClawConfig) => ChannelHealthMonitor | null;
...
> & {
  ...
  initialConfig: SupportClawConfig;
  initialCompareConfig?: SupportClawConfig;
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: SupportClawConfig) => string | undefined;
...
export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  ...
  const markActiveMainSessionsForRestart = async (nextConfig: SupportClawConfig, reason: string) => {
  ...
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: SupportClawConfig) => {
            ...
            await startGmailWatcherWithLogs({
              ...
              onSkipped: () =>
                params.logHooks.info(
                  "skipping gmail watcher restart (SUPPORT_CLAW_SKIP_GMAIL_WATCHER=1)",
                ),
            });
          }
        }
    ...
    if (channelsToRestart.size > 0) {
      if (shouldSkipChannelRestart()) {
        params.logChannels.info(
          "skipping channel reload (SUPPORT_CLAW_SKIP_CHANNELS=1 or SUPPORT_CLAW_SKIP_PROVIDERS=1)",
        );
      } else {
        if (!plan.reloadPlugins) {
          await waitForActiveWorkBeforeChannelReload(channelsToRestart, nextConfig);
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: SupportClawConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
