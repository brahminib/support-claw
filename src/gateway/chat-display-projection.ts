...
import { asFiniteNumber } from "@supportclaw/normalization-core/number-coercion";
import { asOptionalRecord as readRecord } from "@supportclaw/normalization-core/record-coerce";
import { normalizeOptionalString } from "@supportclaw/normalization-core/string-coerce";
import { SUPPORT_CLAW_RUNTIME_CONTEXT_CUSTOM_TYPE } from "../agents/internal-runtime-context.js";
...
import { isSupportClawDeliveryMirrorAssistantMessage } from "../shared/transcript-only-supportClaw-assistant.js";
...
function sanitizeChatHistoryContentBlock(
  ...
  if ("supportClawReasoningReplay" in entry) {
    delete entry.supportClawReasoningReplay;
    changed = true;
  }
...
function buildMessageToolVisibleReplyMirror(
  ...
  const transcriptMeta = readRecord((pending.completionAnchor ?? pending.anchor)["__supportClaw"]);
  if (transcriptMeta) {
    mirror["__supportClaw"] = { ...transcriptMeta };
  }
  return mirror;
}
...
function readMessageToolDeliveryMirrorText(message: Record<string, unknown>): string | undefined {
  ...
  if (!isSupportClawDeliveryMirrorAssistantMessage(message)) {
    return undefined;
  }
...
function readTtsSupplementMarker(
  ...
  const marker = message.supportClawTtsSupplement;
...
function readChatHistoryRecordTimestampMs(message: unknown): number | undefined {
  const meta = readRecord(readRecord(message)?.["__supportClaw"]);
...
function isDisplayHiddenProjectedMessage(message: Record<string, unknown>): boolean {
  ...
  return message.role === "custom" && message.customType === SUPPORT_CLAW_RUNTIME_CONTEXT_CUSTOM_TYPE;
}
...
function supportClawAssistantModel(message: Record<string, unknown>): string | undefined {
  return message.role === "assistant" &&
    message.provider === "supportClaw" &&
...
function isDuplicateAcpGatewayInjectedMessage(
  ...
  if (
    supportClawAssistantModel(previousVisible) !== "acp-runtime" ||
    supportClawAssistantModel(current) !== "gateway-injected"
  ) {
    return false;
  }
