...
function createSnapshot(config: SupportClawConfig): PreparedSecretsRuntimeSnapshot {
  return {
...
function activateSnapshot(config: SupportClawConfig) {
  activateSecretsRuntimeSnapshot(createSnapshot(config));
}

function mockResolvedSecrets(config: SupportClawConfig) {
...
// set SUPPORT_CLAW_SKIP_CHANNELS / SUPPORT_CLAW_SKIP_PROVIDERS at module load. When a
