...
export const SUPPORT_CLAW_MODEL_ID = "supportClaw";
/** Default OpenAI-compatible model alias that targets the default SupportClaw agent. */
export const SUPPORT_CLAW_DEFAULT_MODEL_ID = "supportClaw/default";
...
export class GatewaySessionKeyOverrideError extends Error {
  constructor() {
    super("`x-supportClaw-session-key` cannot use reserved internal session namespaces.");
...
function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-supportClaw-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-supportClaw-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === SUPPORT_CLAW_MODEL_ID || lowered === SUPPORT_CLAW_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^supportClaw[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
/** Validates and resolves the `x-supportClaw-model` override for OpenAI-compatible requests. */
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `supportClaw` or `supportClaw/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-supportClaw-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-supportClaw-model`." };
  }
...
function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-supportClaw-session-key")?.trim();
  if (explicit) {
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-supportClaw-message-channel")) ??
