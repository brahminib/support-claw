...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("SUPPORT_CLAW_CONFIG_PATH is required for gateway config tests");
  }
...
describe("OpenResponses HTTP API (e2e)", () => {
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "supportClaw", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `supportClaw` or `supportClaw/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "supportClaw", input: "hi" },
        { "x-supportClaw-agent-id": "beta" },
      );
      ...
      const resSessionOverride = await postResponses(
        ...
        {
          "x-supportClaw-agent-id": "beta",
          "x-supportClaw-session-key": "agent:beta:openresponses:custom",
        },
      );
      ...
      const resReservedSessionOverride = await postResponses(
        port,
        { model: "supportClaw", input: "hi" },
        { "x-supportClaw-session-key": "agent:main:subagent:spoofed" },
      );
      ...
      expect(reservedSessionJson.error?.message).toBe(
        "`x-supportClaw-session-key` cannot use reserved internal session namespaces.",
      );
      expect(agentCommand).toHaveBeenCalledTimes(0);

      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "supportClaw/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "supportClaw/default", input: "hi" });
...

      {
        ...
        const res = await postResponses(
          port,
          { model: "supportClaw", input: "hi" },
          { "x-supportClaw-agent-id": "missing-agent" },
        );
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
      ...
      {
        agentCommand.mockClear();
        const res = await postResponses(port, { model: "supportClaw/missing-agent", input: "hi" });
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Unknown agent 'missing-agent'.");
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "supportClaw", input: "hi" },
        { "x-supportClaw-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-supportClaw-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("accepts write-scoped and admin-scoped HTTP callers", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "supportClaw", input: "hi" },
      { "x-supportClaw-scopes": "operator.admin, operator.write" },
    );
