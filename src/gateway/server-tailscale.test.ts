...
describe("startGatewayTailscaleExposure preserveFunnel", () => {
  ...
  it("passes serviceName through to Tailscale Serve setup and cleanup", async () => {
    ...
    expect(mocks.enableTailscaleServe).toHaveBeenCalledWith(18789, undefined, "svc:supportClaw");
    expect(logTailscale.info).toHaveBeenCalledWith(
      "serve enabled for svc:supportClaw: https://supportClaw.tailnet.ts.net/ (WS via wss://supportClaw.tailnet.ts.net)",
    );

    await cleanup?.();

    expect(mocks.disableTailscaleServe).toHaveBeenCalledWith(undefined, "svc:supportClaw");
  });
