...
// Emits a one-shot notice for ignored pre-SupportClaw environment prefixes.
...
export function warnLegacySupportClawEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but SupportClaw only reads SUPPORT_CLAW_* names now.`,
      "Rename them by replacing the legacy prefix with SUPPORT_CLAW_; the old names are ignored.",
    ].join("\n"),
    { code: "SUPPORT_CLAW_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}

/** Resets the one-shot legacy env warning latch for tests. */
export function resetLegacySupportClawEnvWarningForTest(): void {
  warned = false;
}
