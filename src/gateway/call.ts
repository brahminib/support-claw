...
import { isLoopbackIpAddress } from "@supportclaw/net-policy/ip";
import { redactSensitiveUrlLikeString } from "@supportclaw/net-policy/redact-sensitive-url";
...
type CallGatewayBaseOptions = {
   ...
   * Bypasses SUPPORT_CLAW_GATEWAY_URL and SUPPORT_CLAW_GATEWAY_PORT for this call only.
   */
...
type GatewayRuntimeConfigLoader = () => SupportClawConfig | Promise<SupportClawConfig>;
const defaultGetRuntimeConfig = async (): Promise<SupportClawConfig> =>
...
async function loadGatewayConfig(): Promise<SupportClawConfig> {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: SupportClawConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallAuth(config: SupportClawConfig) {
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.SUPPORT_CLAW_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.SUPPORT_CLAW_TEST_HANDSHAKE_TIMEOUT_MS);
...
async function resolveGatewayCallContext(
  ...
  const envUrlOverride =
    cliUrlOverride || opts.localPortOverride !== undefined
      ? undefined
      : trimToUndefined(process.env.SUPPORT_CLAW_GATEWAY_URL);
  ...
  const config =
    opts.config ?? (canSkipConfigLoad ? ({} as SupportClawConfig) : await loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `supportClaw doctor` for diagnostics.";
  }
  return message;
}
