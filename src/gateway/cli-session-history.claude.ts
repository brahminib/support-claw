...
// Converts Claude project JSONL into SupportClaw transcript-compatible messages.
...
import { attachSupportClawTranscriptMeta } from "./session-transcript-readers.js";
...
function normalizeClaudeCliContent(
  ...
  for (const item of content) {
    ...
    if (type === "tool_use") {
      // Claude stores tool calls as `tool_use` with `input`; SupportClaw history
...
function parseClaudeCliHistoryEntry(
  ...
  if (type === "user") {
    return attachSupportClawTranscriptMeta(
      {
        role: "user",
        content,
