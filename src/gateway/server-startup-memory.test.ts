...
function createQmdConfig(
  agents: SupportClawConfig["agents"],
...
async function startMemoryBackendForTest(cfg: SupportClawConfig) {
  const log = createGatewayLogMock();
  await startGatewayMemoryBackend({ cfg, log });
  return log;
}

async function startQmdBackendWithManager(cfg: SupportClawConfig) {
...
function expectQmdManagerRequests(cfg: SupportClawConfig, agentIds: string[]) {
