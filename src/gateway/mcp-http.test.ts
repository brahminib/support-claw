...
describe("mcp loopback server", () => {
  it("passes session, account, message channel, and inbound event headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: jsonHeaders({
        ...
        "x-supportClaw-session-id": "session-123",
        "x-supportClaw-account-id": "work",
        "x-supportClaw-message-channel": "telegram",
        "x-supportClaw-current-channel-id": "telegram:chat123",
        "x-supportClaw-current-thread-ts": "42",
        "x-supportClaw-current-message-id": "reply-message-1",
        "x-supportClaw-current-inbound-audio": "true",
        "x-supportClaw-inbound-event-kind": "room_event",
        "x-supportClaw-source-reply-delivery-mode": "message_tool_only",
        "x-supportClaw-require-explicit-message-target": "true",
      }),
      body: mcpToolsListBody(),
    });
    ...
    const sendYield = async (
      ...
      return await sendLoopbackToolCall({
        ...
        headers: {
          ...
          "x-supportClaw-session-id": "session-reused",
          "x-supportClaw-cli-capture-key": captureKey,
        },
      });
    };
    ...
    const sendToolsList = async (
      ...
      await sendLoopbackToolsList({
        ...
        headers: {
          "x-session-key": "agent:main:telegram:group:chat123",
          "x-supportClaw-message-channel": "telegram",
          "x-supportClaw-inbound-event-kind": inboundEventKind,
          ...(sourceReplyDeliveryMode
            ? { "x-supportClaw-source-reply-delivery-mode": sourceReplyDeliveryMode }
            : {}),
          ...(currentInboundAudio ? { "x-supportClaw-current-inbound-audio": "true" } : {}),
    ...
    const sendToolsList = async (token?: string) =>
      await sendLoopbackToolsList({
        ...
        headers: {
          ...
          "x-supportClaw-message-channel": "matrix",
        },
      });
  ...
  it("ignores spoofed owner headers on loopback requests", async () => {
    ...
    const response = await sendLoopbackToolsList({
      ...
      headers: {
        "x-session-key": "agent:main:matrix:dm:test",
        "x-supportClaw-message-channel": "matrix",
        "x-supportClaw-sender-is-owner": "true",
      },
    });
    ...
    expect(
      (
        await sendLoopbackToolCall({
          ...
          headers: { "x-supportClaw-cli-capture-key": captureKey },
        })
      ).status,
    ).toBe(200);
  ...
  it("times out stalled request bodies and closes uploads after flushing 408", async () => {
    const previousTimeout = process.env.SUPPORT_CLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS;
    process.env.SUPPORT_CLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS = "20";
    try {
      server = await startMcpLoopbackServer(0);
      const runtime = getActiveMcpLoopbackRuntime();
      if (!runtime) {
      ...
      if (previousTimeout === undefined) {
        delete process.env.SUPPORT_CLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS;
      } else {
        process.env.SUPPORT_CLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS = previousTimeout;
      }
    }
  });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.supportClaw?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.supportClaw?.alwaysLoad).toBe(true);
    expect(config.mcpServers?.supportClaw?.headers?.Authorization).toBe(
      "Bearer ${SUPPORT_CLAW_MCP_TOKEN}",
    );
    expect(config.mcpServers?.supportClaw?.headers?.["x-supportClaw-session-id"]).toBe(
      "${SUPPORT_CLAW_MCP_SESSION_ID}",
    );
    expect(config.mcpServers?.supportClaw?.headers?.["x-supportClaw-message-channel"]).toBe(
      "${SUPPORT_CLAW_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.supportClaw?.headers?.["x-supportClaw-current-channel-id"]).toBe(
      "${SUPPORT_CLAW_MCP_CURRENT_CHANNEL_ID}",
    );
    expect(config.mcpServers?.supportClaw?.headers?.["x-supportClaw-current-thread-ts"]).toBe(
      "${SUPPORT_CLAW_MCP_CURRENT_THREAD_TS}",
    );
    expect(config.mcpServers?.supportClaw?.headers?.["x-supportClaw-current-message-id"]).toBe(
      "${SUPPORT_CLAW_MCP_CURRENT_MESSAGE_ID}",
    );
    expect(config.mcpServers?.supportClaw?.headers?.["x-supportClaw-current-inbound-audio"]).toBe(
      "${SUPPORT_CLAW_MCP_CURRENT_INBOUND_AUDIO}",
    );
    expect(config.mcpServers?.supportClaw?.headers?.["x-supportClaw-source-reply-delivery-mode"]).toBe(
      "${SUPPORT_CLAW_MCP_SOURCE_REPLY_DELIVERY_MODE}",
    );
    expect(
      config.mcpServers?.supportClaw?.headers?.["x-supportClaw-require-explicit-message-target"],
    ).toBe("${SUPPORT_CLAW_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET}");
    expect(config.mcpServers?.supportClaw?.headers?.["x-supportClaw-cli-capture-key"]).toBe(
      "${SUPPORT_CLAW_MCP_CLI_CAPTURE_KEY}",
    );
    expect(config.mcpServers?.supportClaw?.headers).not.toHaveProperty("x-supportClaw-sender-is-owner");
  });
