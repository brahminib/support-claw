...
function buildForcedConsultCheckingPrompt(): string {
  return [
    "Briefly tell the person that you are checking with SupportClaw.",
    "Do not answer the request yet. Wait for the SupportClaw result before giving the actual answer.",
  ].join(" ");
}
  ...
  session.forcedConsults.schedule(handle, FORCED_CONSULT_FALLBACK_DELAY_MS, () => {
    ...
    broadcastToOwner(session.context, session.connId, {
      ...
      args: {
        ...
        context:
          "The realtime provider produced a final user transcript without invoking supportClaw_agent_consult, so SupportClaw is forcing the consult for realtime Talk.",
