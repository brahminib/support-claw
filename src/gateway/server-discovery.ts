...
/** Formats the Bonjour instance name while preserving user-provided SupportClaw names. */
export function formatBonjourInstanceName(displayName: string) {
  const trimmed = displayName.trim();
  if (!trimmed) {
    return "SupportClaw";
  }
  if (/supportClaw/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (SupportClaw)`;
}
...
export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  ...
  const envPath = env.SUPPORT_CLAW_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "supportClaw");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.SUPPORT_CLAW_TAILNET_DNS?.trim();
