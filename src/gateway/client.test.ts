...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("supportClaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.SUPPORT_CLAW_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// hostnames with SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://supportClaw-gateway.ai:18789",
      onConnectError,
    });
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { SUPPORT_CLAW_HOME: "/tmp/custom-supportClaw-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-client-service-state",
