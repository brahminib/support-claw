...
export const CLI_BACKEND_LIVE_PROVIDER_SKIP_ENV = "SUPPORT_CLAW_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP";
export const CLI_BACKEND_LIVE_ADVISORY_ENV = "SUPPORT_CLAW_LIVE_CLI_BACKEND_ADVISORY";
...
export function resolveCliBackendLiveModelSelection(params: {
  ...
  if (!parsed) {
    throw new Error(
      `SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${params.rawModel}`,
    );
  }

  if (parsed.provider === "codex-cli") {
    throw new Error(
      "SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL=codex-cli/... is no longer supported. Use a supported CLI backend such as claude-cli or google-gemini-cli.",
    );
  }
...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.SUPPORT_CLAW_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.SUPPORT_CLAW_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray(
      "SUPPORT_CLAW_LIVE_CLI_BACKEND_ARGS",
      process.env.SUPPORT_CLAW_LIVE_CLI_BACKEND_ARGS,
    ) ?? params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(
      `SUPPORT_CLAW_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "SUPPORT_CLAW_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.SUPPORT_CLAW_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.SUPPORT_CLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.SUPPORT_CLAW_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.SUPPORT_CLAW_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.SUPPORT_CLAW_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  ...
  process.env.SUPPORT_CLAW_SKIP_PROVIDERS = "1";
  ...
  process.env.SUPPORT_CLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.SUPPORT_CLAW_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("SUPPORT_CLAW_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("SUPPORT_CLAW_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("SUPPORT_CLAW_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("SUPPORT_CLAW_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("SUPPORT_CLAW_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("SUPPORT_CLAW_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("SUPPORT_CLAW_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("SUPPORT_CLAW_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("SUPPORT_CLAW_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("SUPPORT_CLAW_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("SUPPORT_CLAW_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
