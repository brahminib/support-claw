...
const CODEX_HARNESS_LIVE = process.env.SUPPORT_CLAW_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.SUPPORT_CLAW_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["SUPPORT_CLAW_TRAJECTORY", "SUPPORT_CLAW_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-supportClaw-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.SUPPORT_CLAW_TRAJECTORY = "1";
      process.env.SUPPORT_CLAW_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".supportClaw", "trajectory-exports", "bundle");
