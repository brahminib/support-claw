...
const gatewayEnv = {
  SUPPORT_CLAW_GATEWAY_TOKEN: "env-token", // pragma: allowlist secret
  SUPPORT_CLAW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
} as NodeJS.ProcessEnv;

function makeRemoteGatewayConfig(remote: { token?: string; password?: string }): SupportClawConfig {
  return {
    gateway: {
      mode: "remote",
      remote,
...
function withGatewayAuthEnv<T>(env: NodeJS.ProcessEnv, fn: () => T): T {
  return withEnv(
    {
      SUPPORT_CLAW_GATEWAY_TOKEN: env.SUPPORT_CLAW_GATEWAY_TOKEN,
      SUPPORT_CLAW_GATEWAY_PASSWORD: env.SUPPORT_CLAW_GATEWAY_PASSWORD,
      SUPPORT_CLAW_SERVICE_KIND: env.SUPPORT_CLAW_SERVICE_KIND,
    },
    fn,
  );
}
