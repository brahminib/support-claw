...
export function resolveHooksConfig(cfg: SupportClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SupportClawConfig, defaultAgentId: string): Set<string> {
...
/** Extract the hook bearer token from Authorization or x-supportClaw-token headers. */
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-supportClaw-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-supportClaw-idempotency-key"]) ||
