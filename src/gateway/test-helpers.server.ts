...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "SUPPORT_CLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.SUPPORT_CLAW_CONFIG_PATH) {
    configPaths.add(process.env.SUPPORT_CLAW_CONFIG_PATH);
  }
  if (process.env.SUPPORT_CLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.SUPPORT_CLAW_STATE_DIR, "supportClaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "supportClaw-test-no-bundled-extensions")
    : "supportClaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".supportClaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".supportClaw-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".supportClaw-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>supportClaw-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__supportClawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__supportClawTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["SUPPORT_CLAW_GATEWAY_TOKEN"]);
  const prev = process.env.SUPPORT_CLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SUPPORT_CLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.SUPPORT_CLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
