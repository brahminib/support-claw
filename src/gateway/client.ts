// SupportClaw Gateway client facade.
// Wraps the shared gateway-client package with SupportClaw host dependencies.
import {
...
function createSupportClawGatewayClientHostDeps(
  overrides?: GatewayClientHostDeps,
): GatewayClientHostDeps {
  return {
    // This wrapper is the only place the package reaches into SupportClaw runtime
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    ...
    // SupportClaw device identity, token storage, proxy routing, and logging.
    this.#client = new BaseGatewayClient({
      ...opts,
      clientVersion: opts.clientVersion ?? VERSION,
      hostDeps: createSupportClawGatewayClientHostDeps(opts.hostDeps),
    });
  }
