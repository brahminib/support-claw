...
import {
  resetLegacySupportClawEnvWarningForTest,
  warnLegacySupportClawEnvVars,
} from "./env-deprecation.js";

describe("warnLegacySupportClawEnvVars", () => {
  ...
  it("warns with counts and prefixes instead of secret-shaped env names", () => {
    ...
    expect(message).toContain("replacing the legacy prefix with SUPPORT_CLAW_");
    ...
    expect(options).toEqual({
      code: "SUPPORT_CLAW_LEGACY_ENV_VARS",
      type: "DeprecationWarning",
    });
  });

  it("does not warn for current SUPPORT_CLAW names", () => {
    warnLegacySupportClawEnvVars({ SUPPORT_CLAW_GATEWAY_TOKEN: "token" });

    expect(emitWarning).not.toHaveBeenCalled();
  });

  it("warns only once after a successful emit", () => {
    warnLegacySupportClawEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
    warnLegacySupportClawEnvVars({ MOLTBOT_GATEWAY_TOKEN: "old-token" });
  ...
  it("retries if emitWarning throws before the warning is emitted", () => {
    ...
    expect(() => warnLegacySupportClawEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" })).toThrow(
      "warning sink failed",
    );
    warnLegacySupportClawEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
