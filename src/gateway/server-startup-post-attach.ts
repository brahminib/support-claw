...
import type { loadSupportClawPlugins } from "../plugins/loader.js";
...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "SUPPORT_CLAW_SKIP_STARTUP_MODEL_PREWARM";
...
function resolveGatewayMemoryStartupPolicy(cfg: SupportClawConfig): GatewayMemoryStartupPolicy {
...
function hasGatewayStartHooks(pluginRegistry: ReturnType<typeof loadSupportClawPlugins>): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const { normalizeProviderId } = await import("@supportclaw/model-catalog-core/provider-id");
  if (
  ...
  const { ensureSupportClawModelsJson } = await import("../agents/models-config.js");
  const agentDir = resolveDefaultAgentDir(params.cfg);
  const workspaceDir =
    params.workspaceDir ?? resolveAgentWorkspaceDir(params.cfg, resolveDefaultAgentId(params.cfg));
  try {
    await ensureSupportClawModelsJson(params.cfg, agentDir, {
      workspaceDir,
...
export async function startGatewaySidecars(params: {
  cfg: SupportClawConfig;
  pluginRegistry: ReturnType<typeof loadSupportClawPlugins>;
      ...
      await measureStartup(params.startupTrace, "sidecars.acp.identity-reconcile", async () => {
        const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] =
          await Promise.all([
            ...
            import("@supportclaw/acp-core/runtime/session-identifiers"),
          ]);
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: SupportClawConfig;
    pluginRegistry: ReturnType<typeof loadSupportClawPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
