...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.SUPPORT_CLAW_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.SUPPORT_CLAW_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: SupportClawConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpHttpBodyTimeoutMs(): number {
  return readPositiveIntEnv("SUPPORT_CLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS", DEFAULT_MCP_BODY_TIMEOUT_MS);
}

export function resolveMcpCliCaptureKey(req: IncomingMessage): string | undefined {
  return normalizeOptionalString(getHeader(req, "x-supportClaw-cli-capture-key"));
}
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    sessionId: normalizeOptionalString(getHeader(req, "x-supportClaw-session-id")),
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-supportClaw-message-channel")) ?? undefined,
    currentChannelId: normalizeOptionalString(getHeader(req, "x-supportClaw-current-channel-id")),
    currentThreadTs: normalizeOptionalString(getHeader(req, "x-supportClaw-current-thread-ts")),
    currentMessageId: normalizeOptionalString(getHeader(req, "x-supportClaw-current-message-id")),
    currentInboundAudio: normalizeMcpBooleanHeader(
      getHeader(req, "x-supportClaw-current-inbound-audio"),
    ),
    accountId: normalizeOptionalString(getHeader(req, "x-supportClaw-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-supportClaw-inbound-event-kind")),
    sourceReplyDeliveryMode: normalizeMcpSourceReplyDeliveryMode(
      getHeader(req, "x-supportClaw-source-reply-delivery-mode"),
    ),
    requireExplicitMessageTarget: normalizeMcpBooleanHeader(
      getHeader(req, "x-supportClaw-require-explicit-message-target"),
    ),
