...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn<ResolveWideAreaDiscoveryDomain>(() => "supportClaw.internal."),
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/supportClaw.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (SupportClaw)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/supportClaw"),
...
async function expectSshPortOmitted(rawPort: string) {
  ...
  process.env.SUPPORT_CLAW_SSH_PORT = rawPort;
...
function startStuckDiscovery(timeoutMs: string) {
  vi.useFakeTimers();
  useDevelopmentDiscoveryEnv();
  process.env.SUPPORT_CLAW_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = timeoutMs;
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.SUPPORT_CLAW_SSH_PORT = "2222";
    ...
    expect(bonjour.service.advertise).toHaveBeenCalledWith({
      ...
      cliPath: "/usr/local/bin/supportClaw",
      minimal: false,
    });
  ...
  it("skips local discovery services for truthy SUPPORT_CLAW_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.SUPPORT_CLAW_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "supportClaw.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(zoneParams.domain).toBe("supportClaw.internal.");
    expect(zoneParams.gatewayPort).toBe(18789);
    expect(zoneParams.gatewayDirectReachable).toBe(true);
    expect(zoneParams.displayName).toBe("Lab Mac (SupportClaw)");
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (supportClaw.internal. → /tmp/supportClaw.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });
