...
let lastCreateSupportClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/supportClaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateSupportClawToolsContext?.agentTo,
          agentThreadId: lastCreateSupportClawToolsContext?.agentThreadId,
        },
        inheritedToolDenylist: lastCreateSupportClawToolsContext?.inheritedToolDenylist,
      }),
    },
    {
      name: "sessions_send",
  ...
  return {
    createSupportClawTools: (ctx: Record<string, unknown>) => {
      lastCreateSupportClawToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateSupportClawToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-supportClaw-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-supportClaw-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateSupportClawToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateSupportClawToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateSupportClawToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateSupportClawToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateSupportClawToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(body.result?.permissionFlow).toBe(true);
    expect(lastCreateSupportClawToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
    expect(lastCreateSupportClawToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-supportClaw-message-to": "channel:24514",
        "x-supportClaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("keeps shared-secret bearer auth as owner for explicitly allowed owner-only tools", async () => {
    ...
    expect(lastCreateSupportClawToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("derives sender owner identity from HTTP auth instead of caller headers", async () => {
    ...
    expect(lastCreateSupportClawToolsContext?.senderIsOwner).toBe(false);
