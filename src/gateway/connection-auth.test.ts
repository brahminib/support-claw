...
type ConnectionAuthCase = {
  name: string;
  cfgLocal: SupportClawConfig;
  env: NodeJS.ProcessEnv;
  options?: Partial<Omit<GatewayConnectionAuthOptions, "config" | "env">>;
  expected: ResolvedAuth;
};

function cfg(input: Partial<SupportClawConfig>): SupportClawConfig {
  return input as SupportClawConfig;
}
...
const DEFAULT_ENV = {
  ...
  SUPPORT_CLAW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when SUPPORT_CLAW env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when SUPPORT_CLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when SUPPORT_CLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
