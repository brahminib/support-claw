...
function resetGatewayCallMocks() {
  ...
  connectError = null;
  const loadConfigForTests = getRuntimeConfig as unknown as () => SupportClawConfig;
...
describe("callGateway url resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot.restore();
    deleteTestEnvValue("SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS");
  ...
  it("allows Tailscale-authenticated backend calls without client-side credentials", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        remote: { url: "wss://supportClaw.example.test" },
    ...
    expect(lastClientOptions?.url).toBe("wss://supportClaw.example.test");
    expect(lastClientOptions?.token).toBeUndefined();
  ...
  it("uses SUPPORT_CLAW_GATEWAY_URL env override in remote mode when remote URL is missing", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: { mode: "remote", bind: "loopback", remote: {} },
    });
    resolveGatewayPort.mockReturnValue(18789);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.SUPPORT_CLAW_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.SUPPORT_CLAW_GATEWAY_TOKEN = "env-token";
  ...
  it("lets an explicit local port override bypass gateway env URL and port", async () => {
    ...
    resolveGatewayPort.mockImplementation((_config?: unknown, env?: unknown) => {
      ...
      return Number(candidateEnv?.SUPPORT_CLAW_GATEWAY_PORT ?? 18789);
    });
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.SUPPORT_CLAW_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.SUPPORT_CLAW_GATEWAY_PORT = "19001";
    process.env.SUPPORT_CLAW_GATEWAY_TOKEN = "env-token";
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("lets probe details local port override bypass gateway env URL and port", async () => {
    ...
    testing.setDepsForTests({
      ...
      resolveGatewayPort: (_config?: unknown, env?: NodeJS.ProcessEnv) =>
        Number(env?.SUPPORT_CLAW_GATEWAY_PORT ?? 18789),
    });
    const prevUrl = process.env.SUPPORT_CLAW_GATEWAY_URL;
    const prevPort = process.env.SUPPORT_CLAW_GATEWAY_PORT;
    try {
      process.env.SUPPORT_CLAW_GATEWAY_URL = "wss://env-gateway.example/ws";
      process.env.SUPPORT_CLAW_GATEWAY_PORT = "19001";
      ...
      if (prevUrl === undefined) {
        delete process.env.SUPPORT_CLAW_GATEWAY_URL;
      } else {
        process.env.SUPPORT_CLAW_GATEWAY_URL = prevUrl;
      }
      if (prevPort === undefined) {
        delete process.env.SUPPORT_CLAW_GATEWAY_PORT;
      } else {
        process.env.SUPPORT_CLAW_GATEWAY_PORT = prevPort;
      }
    }
  });
  ...
  it("uses env SUPPORT_CLAW_GATEWAY_URL when set", () => {
    getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
    resolveGatewayPort.mockReturnValue(18800);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    const prevUrl = process.env.SUPPORT_CLAW_GATEWAY_URL;
    try {
      process.env.SUPPORT_CLAW_GATEWAY_URL = "wss://browser-gateway.local:9443/ws";

      const details = buildGatewayConnectionDetails();

      expect(details.url).toBe("wss://browser-gateway.local:9443/ws");
      expect(details.urlSource).toBe("env SUPPORT_CLAW_GATEWAY_URL");
      expect(details.bindDetail).toBeUndefined();
    } finally {
      if (prevUrl === undefined) {
        delete process.env.SUPPORT_CLAW_GATEWAY_URL;
  ...
  it("falls back to the default config loader when test deps drift", () => {
    const tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-gateway-call-"));
    setTestEnvValue("SUPPORT_CLAW_STATE_DIR", tempStateDir);
    setTestEnvValue("SUPPORT_CLAW_CONFIG_PATH", path.join(tempStateDir, "missing-config.json"));
    try {
      getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
      resolveGatewayPort.mockReturnValue(18800);
      testing.setDepsForTests({
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("supportClaw doctor --fix");
  });
  ...
  it("allows ws:// hostname remote URLs when SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
        remote: { url: "ws://supportClaw-gateway.ai:18789" },
      },
    });
    resolveGatewayPort.mockReturnValue(18789);
    ...
    expect(details.url).toBe("ws://supportClaw-gateway.ai:18789");
    expect(details.urlSource).toBe("config gateway.remote.url");
  });
...
describe("callGateway error details", () => {
  ...
  it("keeps the default wrapper timeout aligned with env handshake timeout", async () => {
    const envSnapshot = captureEnv(["SUPPORT_CLAW_HANDSHAKE_TIMEOUT_MS"]);
    try {
      process.env.SUPPORT_CLAW_HANDSHAKE_TIMEOUT_MS = "30000";
      startMode = "silent";
      setLocalLoopbackGatewayConfig();
  ...
  it("runs the signal abort hook on the active gateway connection before teardown", async () => {
    ...
    testing.setDepsForTests({
      ...
      getRuntimeConfig: getRuntimeConfig as unknown as () => SupportClawConfig,
...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    ...
    resetGatewayCallMocks();
    delete process.env.SUPPORT_CLAW_GATEWAY_TOKEN;
    delete process.env.SUPPORT_CLAW_GATEWAY_PASSWORD;
    delete process.env.SUPPORT_CLAW_GATEWAY_URL;
    setGatewayNetworkDefaults(18789);
  });
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.SUPPORT_CLAW_GATEWAY_TOKEN = "env-token";
    process.env.SUPPORT_CLAW_GATEWAY_PASSWORD = "env-password";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
  ...
  it("throws when env URL override is set without env credentials", async () => {
    process.env.SUPPORT_CLAW_GATEWAY_URL = "wss://override.example/ws";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: { token: "local-token", password: "local-password" },
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      ...
      envKey: "SUPPORT_CLAW_GATEWAY_PASSWORD",
      envValue: "from-env",
    ...
    },
    {
      ...
      envKey: "SUPPORT_CLAW_GATEWAY_TOKEN",
      envValue: "env-token",
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.SUPPORT_CLAW_GATEWAY_PASSWORD = envPassword;
    }
  ...
  it("does not resolve local password ref when env password takes precedence", async () => {
    process.env.SUPPORT_CLAW_GATEWAY_PASSWORD = "from-env";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
