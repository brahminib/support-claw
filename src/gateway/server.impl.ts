...
import { ensureSupportClawCliOnPath } from "../infra/path-env.js";
...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.SUPPORT_CLAW_GATEWAY_STARTUP_TRACE);
  let timelineConfig: SupportClawConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.SUPPORT_CLAW_DIAGNOSTICS_EVENT_LOOP);
  ...
  return {
    setConfig(config: SupportClawConfig) {
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`supportClaw config set gateway.auth.mode token\` and \`supportClaw config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
    "In Nix mode, set gateway.auth.token in your Nix-managed SupportClaw config and rebuild.",
    "For the first-party Nix flow, see https://github.com/supportClaw/nix-supportClaw#quick-start and https://docs.supportClaw.ai/install/nix.",
  ].join(" ");
}
...
export type GatewayServerOptions = {
   ...
   * reparsing supportClaw.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.SUPPORT_CLAW_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.SUPPORT_CLAW_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "SUPPORT_CLAW_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "SUPPORT_CLAW_RAW_STREAM_PATH",
  ...
  let cfgAtStart: SupportClawConfig;
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: SupportClawConfig) =>
    ...
    const gatewayRequestContext = await startupTrace.measure(
      ...
      async () => {
        ...
        return createGatewayRequestContext({
          ...
          clients,
          enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: SupportClawConfig) => {
            enforceSharedGatewaySessionGenerationForConfigWrite({
