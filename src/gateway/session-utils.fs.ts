...
/** Attach SupportClaw metadata to a transcript message without dropping existing metadata. */
export function attachSupportClawTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record["__supportClaw"] &&
    typeof record["__supportClaw"] === "object" &&
    !Array.isArray(record["__supportClaw"])
      ? (record["__supportClaw"] as Record<string, unknown>)
      : {};
  return {
    ...record,
    __supportClaw: {
...
function buildOversizedTranscriptRecord(line: string): TailTranscriptRecord {
  ...
  const record: Record<string, unknown> = {
    ...
    message: {
      ...
      __supportClaw: { truncated: true, reason: "oversized" },
    },
  };
  return { record };
}
...
export function readRecentSessionMessagesWithStats(
  ...
  const messagesWithSeq = messages.map((message, index) =>
    attachSupportClawTranscriptMeta(message, { seq: firstSeq + index }),
  );
    ...
    const recordTimestampMs =
      typeof entry.timestamp === "string"
        ...
        : typeof entry.timestamp === "number"
          ? entry.timestamp
          : Number.NaN;
    return attachSupportClawTranscriptMeta(entry.message, {
...
function extractTranscriptTokenEstimateFromLine(line: string): {
  ...
  try {
    ...
    const isDeliveryMirror =
      role === "assistant" && modelProvider === "supportClaw" && model === "delivery-mirror";
    if (isDeliveryMirror) {
      return null;
    }
    ...
    const model =
      typeof message.model === "string"
        ...
        : typeof parsed.model === "string"
          ? parsed.model.trim()
          : undefined;
    const isDeliveryMirror = modelProvider === "supportClaw" && model === "delivery-mirror";
