...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.SUPPORT_CLAW_LIVE_CODEX_BIND);
const describeLive = LIVE && CODEX_BIND_LIVE ? describe : describe.skip;
const CODEX_BIND_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.SUPPORT_CLAW_LIVE_CODEX_BIND_TIMEOUT_MS,
  900_000,
);
const CODEX_BIND_REQUEST_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.SUPPORT_CLAW_LIVE_CODEX_BIND_REQUEST_TIMEOUT_MS,
  300_000,
);
...
async function writePluginBindingApproval(params: {
  ...
  const supportClawDir = path.join(params.homeDir, ".supportClaw");
  await fs.mkdir(supportClawDir, { recursive: true });
  await fs.writeFile(
    path.join(supportClawDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
...
function resolveCodexBindModelProvider(): string | undefined {
  const configured = process.env.SUPPORT_CLAW_LIVE_CODEX_BIND_PROVIDER?.trim();
  if (configured) {
    return configured;
  }
  return process.env.SUPPORT_CLAW_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "openai" : undefined;
}
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.SUPPORT_CLAW_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-live-codex-bind-"));
      ...
      const bindModel =
        process.env.SUPPORT_CLAW_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        try {
          ...
          restoreEnvVar("SUPPORT_CLAW_CONFIG_PATH", previous.configPath);
          restoreEnvVar("SUPPORT_CLAW_GATEWAY_TOKEN", previous.gatewayToken);
          restoreEnvVar("HOME", previous.home);
          restoreEnvVar("SUPPORT_CLAW_SKIP_CANVAS_HOST", previous.skipCanvas);
          restoreEnvVar("SUPPORT_CLAW_SKIP_CHANNELS", previous.skipChannels);
          restoreEnvVar("SUPPORT_CLAW_SKIP_CRON", previous.skipCron);
          restoreEnvVar("SUPPORT_CLAW_SKIP_GMAIL_WATCHER", previous.skipGmail);
          restoreEnvVar("SUPPORT_CLAW_STATE_DIR", previous.stateDir);
        }
      }
    },
