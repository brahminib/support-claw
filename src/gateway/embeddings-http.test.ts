...
const WRITE_SCOPE_HEADER = { "x-supportClaw-scopes": "operator.write" };
...
describe("OpenAI-compatible embeddings HTTP API (e2e)", () => {
  it("embeds string and array inputs", async () => {
    ...
    const qualified = await postEmbeddings(
      ...
      { "x-supportClaw-model": "openai/text-embedding-3-small" },
    );
    expect(qualified.status).toBe(200);
    const qualifiedJson = (await qualified.json()) as { model?: string };
    expect(qualifiedJson.model).toBe("supportClaw/default");
  ...
  it("rejects explicit unknown agent ids", async () => {
    try {
      ...
      const header = await postEmbeddings(
        { model: "supportClaw/default", input: "hello" },
        { "x-supportClaw-agent-id": "missing-agent" },
      );
      await expectInvalidEmbeddingRequest(header, "Unknown agent 'missing-agent'.");

      const model = await postEmbeddings({ model: "supportClaw/missing-agent", input: "hello" });
  ...
  it("rejects disallowed x-supportClaw-model provider overrides", async () => {
    const res = await postEmbeddings(
      {
        model: "supportClaw/default",
        input: "hello",
      },
      { "x-supportClaw-model": "ollama/nomic-embed-text" },
    );
    await expectInvalidEmbeddingRequest(
      res,
  ...
  it("rejects x-supportClaw-model for trusted write-only callers", async () => {
