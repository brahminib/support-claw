// OpenAI-compatible `/v1/models` HTTP route backed by configured SupportClaw agents.
...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([SUPPORT_CLAW_MODEL_ID, SUPPORT_CLAW_DEFAULT_MODEL_ID]);
  ids.add(`supportClaw/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`supportClaw/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== SUPPORT_CLAW_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
