...
import { asPositiveSafeInteger } from "@supportclaw/normalization-core/number-coercion";
import {
...
import {
  attachSupportClawTranscriptMeta,
...
type SessionHistoryMessage = Record<string, unknown> & {
  __supportClaw?: SessionHistoryTranscriptMeta;
};
...
function resolveMessageSeq(message: SessionHistoryMessage | undefined): number | undefined {
  return asPositiveSafeInteger(message?.["__supportClaw"]?.seq);
}

function isMessageToolMirrorMessage(message: SessionHistoryMessage): boolean {
  return message.supportClawMessageToolMirror !== undefined;
}
...
export class SessionHistorySseState {
  ...
  appendInlineMessage(update: {
    ...
    const nextMessage = attachSupportClawTranscriptMeta(update.message, {
      ...
      if (projectedMessage !== undefined) {
        const emittedMessage: SessionHistoryMessage =
          ...
          resolveMessageSeq(projectedMessage) === undefined
            ? (attachSupportClawTranscriptMeta(projectedMessage, {
                seq: this.rawTranscriptSeq,
