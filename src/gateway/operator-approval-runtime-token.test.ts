...
const envSnapshot = captureEnv(["HOME", "SUPPORT_CLAW_HOME"]);

const tempHomes: string[] = [];

function useTempHome(): string {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-approval-runtime-"));
  tempHomes.push(home);
  setTestEnvValue("HOME", home);
  setTestEnvValue("SUPPORT_CLAW_HOME", home);
  return home;
}

function execApprovalsPath(home: string): string {
  return path.join(home, ".supportClaw", "exec-approvals.json");
}

function writeExecApprovalsToken(home: string, token: string): void {
  fs.mkdirSync(path.join(home, ".supportClaw"), { recursive: true });
  fs.writeFileSync(
