...
// Bridges /v1/embeddings requests to configured SupportClaw memory providers.
...
// OpenAI-compatible `/v1/embeddings` bridge. It maps SupportClaw agent/model
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== SUPPORT_CLAW_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
        message: "Invalid `model`. Use `supportClaw` or `supportClaw/<agentId>`.",
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-supportClaw-model")) ||
