...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listSupportClawPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...
    listSupportClawPluginManifestMetadata: (
      params?: Parameters<typeof actual.listSupportClawPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listSupportClawPluginManifestMetadata(params);
      return actual.listSupportClawPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listSupportClawPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as SupportClawConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listSupportClawPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/supportClaw.plugin.json",
    ...overrides,
  };
}
