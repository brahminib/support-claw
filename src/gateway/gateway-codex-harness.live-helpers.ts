...
export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current SupportClaw session status reports the active model as:",
] as const;
...
export const EXPECTED_CODEX_STATUS_COMMAND_TEXT = [
  ...
  "SupportClaw status shown above.",
  "Status shown above.",
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsSupportClawStatus =
    normalized.includes("supportClaw is running on") ||
    /supportClaw\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("supportClaw status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsSupportClawStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from supportClaw status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live supportClaw config shows") ||
