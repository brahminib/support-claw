...
const hoisted = vi.hoisted(() => ({
  ...
  runtimeConfig: { value: { session: { store: "/tmp/active-sessions.json" } } as SupportClawConfig },
  reloadEvents: [] as string[],
  loadModelCatalog: vi.fn(async (_params: { config: SupportClawConfig }) => []),
  resetModelCatalogCache: vi.fn(() => {}),
  refreshContextWindowCache: vi.fn(async (_cfg: SupportClawConfig) => {}),
  clearCurrentProviderAuthState: vi.fn(() => {}),
  warmCurrentProviderAuthStateOffMainThread: vi.fn(async (_cfg: SupportClawConfig) => {}),
...
vi.mock("../agents/model-catalog.js", () => ({
  loadModelCatalog: (params: { config: SupportClawConfig }) => {
...
vi.mock("../agents/context.js", () => ({
  refreshContextWindowCache: async (cfg: SupportClawConfig) => {
...
vi.mock("../agents/model-provider-auth.js", () => ({
  ...
  warmCurrentProviderAuthStateOffMainThread: async (cfg: SupportClawConfig) => {
...
describe("gateway hot reload model state", () => {
  it("resets prepared model runtime state for every hot reload and rewarms after plugin reload", async () => {
    ...
    const nextConfig = { plugins: { enabled: true } } as SupportClawConfig;
  ...
  it("disposes cached MCP runtimes on MCP config hot reloads", async () => {
    ...
    const nextConfig = { mcp: { servers: {} } } as SupportClawConfig;
  ...
  it.each([
    ...
    const { applyHotReload } = createReloadHandlersForTest();
    const previousConfig = testCase.previousConfig as SupportClawConfig;
    const nextConfig = testCase.nextConfig as SupportClawConfig;
...
describe("gateway Gmail hot reload handlers", () => {
  ...
  function createGmailConfig(account: string): SupportClawConfig {
    return {
  ...
  it("aborts an in-flight managed Gmail restart when the reloader stops", async () => {
    ...
    const reloader = startManagedGatewayConfigReloader({
      ...
      watchPath: "/tmp/supportClaw.json",
      ...
      activateRuntimeSecrets: vi.fn(async (config: SupportClawConfig) => ({
        sourceConfig: config,
        config,
        authStores: [],
        warnings: [],
