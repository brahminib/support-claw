...
const resolveSupportClawPackageRootSync = vi.hoisted(() => vi.fn((_params: unknown) => "/package"));
...
vi.mock("../infra/supportClaw-root.js", () => ({
  resolveSupportClawPackageRootSync: (params: unknown) => resolveSupportClawPackageRootSync(params),
}));
...
function slackConfig(): SupportClawConfig {
  return {
    channels: {
...
describe("prepareGatewayPluginBootstrap startup plugins", () => {
  beforeEach(() => {
    ...
    resolveSupportClawPackageRootSync.mockClear().mockReturnValue("/package");
...
describe("warnUnregisteredConfiguredMemoryEmbeddingProviders", () => {
  ...
  function customOllamaConfig(source: "provider" | "fallback" = "provider"): SupportClawConfig {
    const memorySearch =
      source === "provider"
