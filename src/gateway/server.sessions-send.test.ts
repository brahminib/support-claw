...
const { createSupportClawTools } = await import("../agents/supportClaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createSupportClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createSupportClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["SUPPORT_CLAW_GATEWAY_PORT", "SUPPORT_CLAW_GATEWAY_TOKEN"]);
  ...
  process.env.SUPPORT_CLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.SUPPORT_CLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  ...
  it(
    ...
    async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-sessions-send-route-"));
