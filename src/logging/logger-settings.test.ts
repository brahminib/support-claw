...
beforeEach(() => {
  envSnapshot = captureEnv(["SUPPORT_CLAW_TEST_FILE_LOG", "SUPPORT_CLAW_LOG_LEVEL"]);
  delete process.env.SUPPORT_CLAW_TEST_FILE_LOG;
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    ...
    logging.setLoggerConfigLoaderForTests(() => ({
      level: "debug",
      file: "/tmp/supportClaw-configured.log",
      maxFileBytes: 2048,
    }));
    ...
    expect(settings.file).toBe("/tmp/supportClaw-configured.log");
  ...
  it("uses defaults when no logging config is available", () => {
    ...
    expect(path.basename(settings.file)).toMatch(/^supportClaw-vitest-\d+-\d{4}-\d{2}-\d{2}\.log$/);
    expect(settings.file).not.toBe(
      `/tmp/supportClaw/supportClaw-${new Date().toISOString().slice(0, 10)}.log`,
    );
  });
});
