...
function writeConfig(source: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-redact-config-"));
  tempDirs.push(dir);
  const configPath = path.join(dir, "supportClaw.json");
...
describe("redactSensitiveText", () => {
  ...
  it("masks URL userinfo and database connection-string passwords", () => {
    const input = [
      ...
      "postgres://dbuser:database-password-1234567890@db.example.test/supportClaw",
      "postgres://secret:secret@db.example.test/supportClaw",
    ...
    expect(output).toContain("postgres://dbuser:databa…7890@db.example.test/supportClaw");
    expect(output).toContain("postgres://secret:***@db.example.test/supportClaw");
  ...
  it("does not redact ordinary identifiers containing short token-prefix substrings", () => {
    const input =
      "npm_telegram_package_spec ask_supportClaw_query_patterns team_management risk_assessment glpat-docs dapi-example sbp_short nfp_site CCIPAT_docs ATATT-example";
    const output = redactSensitiveText(input, { mode: "tools" });
    expect(output).toBe(input);
  });
  ...
  it("masks connection-string passwords through the default options path", () => {
    expect(
      redactSensitiveText("postgres://dbuser:opaquepw12345@db.example.test/supportClaw", {
        mode: "tools",
      }),
    ).toBe("postgres://dbuser:***@db.example.test/supportClaw");
  });
  ...
  it("honors logging redaction settings from the active config path", () => {
    ...
    withEnv({ SUPPORT_CLAW_CONFIG_PATH: configPath }, () =>
