...
const SUPPORT_EXPORT_PREFIX = "supportClaw-diagnostics-";
...
function resolveBonjourEnvOverride(
  ...
  const raw = env.SUPPORT_CLAW_DISABLE_BONJOUR?.trim().toLowerCase();
  if (!raw) {
    return "unset";
  }
  switch (raw) {
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$SUPPORT_CLAW_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    "# SupportClaw Diagnostics Export",
    "",
    ...
    "- `logs/supportClaw-sanitized.jsonl`: sanitized log summaries and metadata",
