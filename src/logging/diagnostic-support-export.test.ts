...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "supportClaw.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      env: {
        ...process.env,
        HOME: tempDir,
        SUPPORT_CLAW_STATE_DIR: tempDir,
      },
      stateDir: tempDir,
      outputPath,
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["supportClaw", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              SUPPORT_CLAW_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(Object.keys(entries).toSorted()).toEqual([
      ...
      "logs/supportClaw-sanitized.jsonl",
      "manifest.json",
    ...
    expect(combined).toContain("$SUPPORT_CLAW_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/supportClaw-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.SUPPORT_CLAW_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\supportClaw`;
    const redaction = {
      env: {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$SUPPORT_CLAW_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\supportClaw\\dist\\index.js`,
              "--config",
              `${stateDir}\\supportClaw.json`,
            ],
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\supportClaw\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\supportClaw\\\\dist\\\\index.js");
    expect(serialized).toContain("$SUPPORT_CLAW_STATE_DIR\\\\supportClaw.json");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\supportClaw\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/supportClaw.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/supportClaw-sanitized.jsonl");
