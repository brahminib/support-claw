...
const logPathTracker = createSuiteLogPathTracker("supportClaw-log-redaction-");
...
describe("file log redaction", () => {
  ...
  it("uses logging.file from the active config path", () => {
    ...
    withEnv({ SUPPORT_CLAW_CONFIG_PATH: configPath, SUPPORT_CLAW_TEST_FILE_LOG: "1" }, () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    withEnv({ HOME: home }, () => {
      expect(loggerTest.resolveActiveLogFile("~/custom-supportClaw.log")).toBe(
        path.join(home, "custom-supportClaw.log"),
      );
    });
  });
