...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredSupportClawTmpDir() : POSIX_SUPPORT_CLAW_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "supportClaw.log")
    : `${POSIX_SUPPORT_CLAW_TMP_DIR}/supportClaw.log`;
}
...
type LoggerConfigLoader = () => SupportClawConfig["logging"] | undefined;
...
function canUseSilentVitestFileLogFastPath(envLevel: LogLevel | undefined): boolean {
  return (
    ...
    process.env.SUPPORT_CLAW_TEST_FILE_LOG !== "1" &&
    !envLevel &&
...
function resolveDefaultActiveLogFile(): string {
  if (process.env.VITEST === "true" && process.env.SUPPORT_CLAW_TEST_FILE_LOG === "1") {
    return path.join(
      process.cwd(),
      ".artifacts",
      "test-logs",
...
function resolveSettings(): ResolvedSettings {
  ...
  const cfg: SupportClawConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? loadLoggerConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.SUPPORT_CLAW_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (currentFileBytes > 0 && nextBytes > settings.maxFileBytes) {
        if (rotateLogFile(activeFile)) {
          ...
          process.stderr.write(
            `[supportClaw] log file rotation failed; continuing writes file=${activeFile} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
      }
