...
const logPathTracker = createSuiteLogPathTracker("supportClaw-test-env-log-level-");

describe("SUPPORT_CLAW_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["SUPPORT_CLAW_LOG_LEVEL"]);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.SUPPORT_CLAW_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.SUPPORT_CLAW_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("SUPPORT_CLAW_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid SUPPORT_CLAW_LOG_LEVEL="nope"');
  });
});
