...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-log-tail-redaction-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("readConfiguredLogTail redaction", () => {
  it("redacts raw auth headers before returning log lines", async () => {
    ...
    const logFile = path.join(dir, "supportClaw.log");
    const configFile = path.join(dir, "supportClaw.json");
    ...
    expect(text).toContain("X-SupportClaw-Token: supers…7890");
    ...
    expect(text).not.toContain(supportClawToken);
