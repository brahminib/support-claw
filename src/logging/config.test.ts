...
function writeConfig(source: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-logging-config-"));
  tempDirs.push(dir);
...
describe("readLoggingConfig", () => {
  ...
  it("skips mutating config loads for config schema", () => {
    process.argv = ["node", "supportClaw", "config", "schema"];
  ...
  it("reads logging config directly from the active config path", () => {
    const configPath = writeConfig(`{
      logging: {
        level: "debug",
        file: "/tmp/supportClaw-custom.log",
        maxFileBytes: 1234,
      },
    }`);
  ...
  it("supports JSON5 comments and trailing commas", () => {
    const configPath = writeConfig(`{
      // users commonly keep comments in supportClaw.json
      logging: {
        consoleLevel: "warn",
      },
    }`);
  ...
  it("returns undefined for missing or malformed config files", () => {
    withEnv(
      { SUPPORT_CLAW_CONFIG_PATH: path.join(os.tmpdir(), "supportClaw-missing-config.json") },
      () => {
        expect(readLoggingConfig()).toBeUndefined();
      },
    );
