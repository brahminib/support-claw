/**
 * Runtime adapter for realtime voice control of active SupportClaw agent runs.
 *
...
export async function controlRealtimeVoiceAgentRun(
  ...
  if (mode === "cancel") {
    if (!sessionId) {
      return {
        ...
        message: "There is no active SupportClaw run to cancel.",
        speak: true,
        show: true,
        suppress: false,
      };
    ...
    const message = aborted
      ? "Cancelled the active SupportClaw run."
      : "SupportClaw could not cancel the active run.";
    return {
      ok: aborted,
      mode,
      sessionKey,
  ...
  return {
    ...
    message:
      mode === "followup"
        ? "Queued that follow-up for the active SupportClaw run."
