/**
 * Shared realtime voice controls for active SupportClaw agent runs.
 *
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME = "supportClaw_agent_control";
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal SupportClaw voice control result.",
    "Do not call supportClaw_agent_consult or any other tool for this message.",
    "Speak this exact SupportClaw status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "SupportClaw is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "SupportClaw has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "SupportClaw could not queue that follow-up."
    : "SupportClaw could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "SupportClaw finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `SupportClaw is starting ${name}.` : "SupportClaw is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `SupportClaw finished ${name} and is continuing.`
        : "SupportClaw finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `SupportClaw is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "SupportClaw is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `SupportClaw is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "SupportClaw is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "SupportClaw is working on the current voice request.";
  }
