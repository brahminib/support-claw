/**
 * Realtime voice tool definition and helpers for delegating work to SupportClaw.
 *
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME = "supportClaw_agent_consult";
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Delegate the caller's request to the configured SupportClaw agent for normal tool-backed work, actions, context, memory, or reasoning before speaking.",
  parameters: {
    type: "object",
    properties: {
      question: {
...
export function buildRealtimeVoiceAgentConsultWorkingResponse(
  ...
  return {
    ...
    message: `Tell the ${audienceLabel} briefly that you are checking, then wait for the final SupportClaw result before answering with the actual result.`,
  };
}
...
/** Resolve the SupportClaw tool allowlist paired with the consult exposure policy. */
...
export function buildRealtimeVoiceAgentConsultPolicyInstructions(config: {
  ...
  if (config.consultPolicy === "always") {
    return [
      "Consult behavior:",
      "- Call supportClaw_agent_consult before every substantive answer.",
  ...
  return [
    ...
    "- Call supportClaw_agent_consult before answering requests that need facts, memory, current information, tools, workspace state, or the user's SupportClaw-specific context.",
...
/** Build the delegated SupportClaw agent prompt for a live voice consult. */
export function buildRealtimeVoiceAgentConsultPrompt(params: {
  ...
  return [
    ...
    "Act as the configured SupportClaw agent on behalf of this user. Use available tools when the request asks you to do work.",
