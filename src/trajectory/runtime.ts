...
async function replaceTrajectoryWindow(params: {
  ...
  await writeSiblingTempFile({
    ...
    chmodDir: false,
    mode: 0o600,
    tempPrefix: ".supportClaw-trajectory-",
...
export function createTrajectoryRuntimeRecorder(
  ...
  // override so operators can still disable recording with SUPPORT_CLAW_TRAJECTORY=0.
  const enabled = parseBooleanValue(env.SUPPORT_CLAW_TRAJECTORY) ?? true;
  if (!enabled) {
    return null;
  }
