...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/supportClaw.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.supportClaw/supportClaw.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          SUPPORT_CLAW_STATE_DIR: "/Users/tester/.supportClaw",
        },
      ...
      expect(harness.invocation).toEqual([
        "node",
        "~/project/supportClaw.js",
        "--api-key",
        "<redacted>",
        "--config=$SUPPORT_CLAW_STATE_DIR/supportClaw.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/supportClaw.js");
  ...
  it("redactPathForSupport returns empty string for null/undefined input", () => {
    const ctx: SupportRedactionContext = { env: {}, stateDir: "/tmp/.supportClaw" };
