...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        SUPPORT_CLAW_BASE_ENV: "base",
        SUPPORT_CLAW_TO_REMOVE: undefined,
      },
      env: {
        SUPPORT_CLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.SUPPORT_CLAW_BASE_ENV).toBe("base");
    expect(resolved.SUPPORT_CLAW_TEST_ENV).toBe("ok");
    expect(resolved.SUPPORT_CLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.SUPPORT_CLAW_CLI).toBe(SUPPORT_CLAW_CLI_ENV_VALUE);
  });
