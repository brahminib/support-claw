...
const mockConfig = vi.hoisted(() => {
  ...
  return {
    reset() {
      state.path = "/tmp/supportClaw.json";
      state.exists = true;
      state.config = {};
...
vi.mock("./overview.js", () => ({
  ...
  loadCrestodianOverview: vi.fn(async () => ({
    ...
    config: { path: "/tmp/supportClaw.json", exists: true, valid: true, issues: [], hash: null },
    tools: {
    ...
    references: {
      docsUrl: "https://docs.supportClaw.ai",
      sourceUrl: "https://github.com/brahminib/support-claw",
    },
  })),
}));
...
describe("parseCrestodianOperation", () => {
  ...
  it("parses plugin management operations", () => {
    ...
    expect(parseCrestodianOperation("install npm plugin @supportclaw/demo")).toEqual({
      kind: "plugin-install",
      spec: "npm:@supportclaw/demo",
    });
    expect(parseCrestodianOperation("plugin install clawhub:supportClaw-demo")).toEqual({
      kind: "plugin-install",
      spec: "clawhub:supportClaw-demo",
    });
    expect(parseCrestodianOperation("plugin uninstall supportClaw-demo")).toEqual({
      kind: "plugin-uninstall",
      pluginId: "supportClaw-demo",
    });
  });
  ...
  it("validates missing config without exiting the process", async () => {
    mockConfig.missing("/tmp/supportClaw.json");
    const { runtime, lines } = createCrestodianTestRuntime();
  ...
  it("applies SecretRef config set through typed deps and writes an audit entry", async () => {
    ...
    expect(runConfigSet).toHaveBeenCalledWith({
      ...
      cliOptions: {
        refProvider: "default",
        refSource: "env",
        refId: "SUPPORT_CLAW_GATEWAY_TOKEN",
      },
    });
  ...
  it("installs plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-install", spec: "clawhub:supportClaw-demo" },
      runtime,
      { deps: { runPluginInstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: install plugin clawhub:supportClaw-demo. Say yes to apply.",
    });
    ...
    expect(installCall[0]).toBe("clawhub:supportClaw-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Installed plugin clawhub:supportClaw-demo",
      },
      { rescue: true, spec: "clawhub:supportClaw-demo" },
    );
  });
  ...
  it("uninstalls plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-uninstall", pluginId: "supportClaw-demo" },
      runtime,
      { deps: { runPluginUninstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: uninstall plugin supportClaw-demo. Say yes to apply.",
    });
    ...
    expect(uninstallCall[0]).toBe("supportClaw-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Uninstalled plugin supportClaw-demo",
      },
      { rescue: true, pluginId: "supportClaw-demo" },
    );
  });
