...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, SupportClaw's ring-zero setup helper.",
  "Turn the user's request into exactly one safe SupportClaw Crestodian command.",
  ...
  "Use the provided SupportClaw docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `SupportClaw docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `SupportClaw source: ${
