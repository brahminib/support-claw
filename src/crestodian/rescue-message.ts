...
function formatUnsupportedRemoteOperation(operation: CrestodianOperation): string | null {
  if (operation.kind === "open-tui") {
    return [
      ...
      "Use local `supportClaw` for agent handoff, or ask for status, doctor, config, gateway, agents, or models.",
    ].join(" ");
  }
  if (operation.kind === "plugin-install") {
    return [
      ...
      "Use local `supportClaw crestodian` or `supportClaw plugins install` instead.",
    ].join(" ");
  }
  return null;
}
