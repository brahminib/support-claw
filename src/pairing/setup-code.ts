// Generates setup codes used to pair external channels with SupportClaw.
...
} from "@supportclaw/net-policy/ip";
import {
...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.SUPPORT_CLAW_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.SUPPORT_CLAW_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(env.SUPPORT_CLAW_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(env.SUPPORT_CLAW_GATEWAY_PASSWORD)),
  });
