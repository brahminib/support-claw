...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: SupportClawConfig): unknown =>
...
type SecretRegistryEntry = {
  id: string;
  configFile: "supportClaw.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.SUPPORT_CLAW_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_SUPPORT_CLAW_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.SUPPORT_CLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR ??= "extensions";
process.env.SUPPORT_CLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR ??= "1";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.SUPPORT_CLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function addCoveragePluginLoadPath(config: SupportClawConfig, pluginId: string): void {
...
function collectSupportClawCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "supportClaw.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_SUPPORT_CLAW_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForSupportClawTarget(
...
async function expectSupportClawCoverageBatchResolved(
  label: string,
  batch: readonly SecretRegistryEntry[],
): Promise<void> {
  logCoverageBatch(label, batch);
  const config = {} as SupportClawConfig;
  const env: Record<string, string> = {};
  for (const [index, entry] of batch.entries()) {
    const envId = toCoverageEnvRefId("SUPPORT_CLAW_SECRET_TARGET", entry.id);
    ...
    applyConfigForSupportClawTarget(config, entry, envId, wildcardToken);
  }
...
const SUPPORT_CLAW_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectSupportClawCoverageEntries({ includePluginEntries: false }),
);
const SUPPORT_CLAW_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectSupportClawCoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = SUPPORT_CLAW_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectSupportClawCoverageBatchResolved("supportClaw.json core", googleChatBatch);
    }
  });

  describe("supportClaw.json core and channel registry targets", () => {
    test.each(SUPPORT_CLAW_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectSupportClawCoverageBatchResolved("supportClaw.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("supportClaw.json plugin registry targets", () => {
    test.each(SUPPORT_CLAW_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectSupportClawCoverageBatchResolved("supportClaw.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = toCoverageEnvRefId("SUPPORT_CLAW_AUTH_SECRET_TARGET", entry.id);
        ...
        const snapshot = await prepareAuthCoverageSnapshot({
          ...
          env,
          agentDirs: ["/tmp/supportClaw-agent-main"],
