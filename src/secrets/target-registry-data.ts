...
function createPluginSupportClawConfigSecretTargetEntry(
  pluginId: string,
  configPath: string,
  ...
  for (const record of bundledPlugins) {
    for (const config of WEB_PROVIDER_SECRET_CONFIGS) {
      if (
        ...
        entries.push(createPluginSupportClawConfigSecretTargetEntry(record.id, config.configPath));
      }
    }
  }
...
function listBundledPluginConfigSecretTargetRegistryEntries(
  ...
  for (const record of bundledPlugins) {
    ...
    for (const secretInput of secretInputs) {
      const entry = createPluginSupportClawConfigSecretTargetEntry(record.id, secretInput.path);
...
export function getSourceSecretTargetRegistry(): SecretTargetRegistryEntry[] {
  return loadSecretTargetRegistryFromPluginMetadata({
    env: {
      ...process.env,
      SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR ?? "extensions",
    },
