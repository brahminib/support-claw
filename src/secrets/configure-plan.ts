/** Builds the interactive `supportClaw secrets configure` target list and apply plan. */
...
/** Credential target shown by `supportClaw secrets configure` before a SecretRef is selected. */
...
function getSecretProviders(config: SupportClawConfig): Record<string, SecretProviderConfig> {
...
/** Builds configure candidates for the current SupportClaw config only. */
export function buildConfigureCandidates(config: SupportClawConfig): ConfigureCandidate[] {
...
function configureCandidateSortKey(candidate: ConfigureCandidate): string {
  ...
  return `supportClaw:${candidate.path}`;
}
...
/** Builds configure candidates for SupportClaw config plus an optional auth-profile scope. */
export function buildConfigureCandidatesForScope(params: {
  config: SupportClawConfig;
  authoredSupportClawConfig?: SupportClawConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredSupportClawConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const supportClawCandidates = discoverConfigSecretTargets(params.config)
      ...
      return Object.assign(
        {
          ...
          label: entry.path,
          configFile: `supportClaw.json` as const,
  ...
  return [...supportClawCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "supportClaw secrets configure",
