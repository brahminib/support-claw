...
function resolvePluginContractApiPath(rootDir: string): string | null {
  ...
  // (per package.json `supportClaw.runtimeExtensions`), while flat-layout plugins keep
  // them at <rootDir>/. Search both, preferring dist/ when running from built supportClaw
...
function loadExternalChannelSecretContractFromRecord(
  ...
  try {
    ...
    if (process.env.SUPPORT_CLAW_DEBUG_CHANNEL_CONTRACT_API === "1") {
