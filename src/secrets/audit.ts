...
import { resolveStateDir, type SupportClawConfig } from "../config/config.js";
...
/** Stable finding codes emitted by `supportClaw secrets audit`. */
...
export async function runSecretsAudit(
  ...
  const config = snapshot.valid ? snapshot.config : ({} as SupportClawConfig);
  let resolution = {
    refsChecked: 0,
    skippedExecRefs: 0,
