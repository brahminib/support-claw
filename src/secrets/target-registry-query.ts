...
let compiledSecretTargetRegistryState: {
  ...
  supportClawCompiledSecretTargets: CompiledTargetRegistryEntry[];
  supportClawTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
} | null = null;

let compiledCoreSupportClawTargetState: {
  knownTargetIds: Set<string>;
  supportClawCompiledSecretTargets: CompiledTargetRegistryEntry[];
  supportClawTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledChannelSupportClawTargets = new Map<string, CompiledTargetRegistryEntry[] | null>();
...
function getCompiledSecretTargetRegistryState() {
  ...
  const supportClawCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "supportClaw.json",
  );
  ...
  compiledSecretTargetRegistryState = {
    ...
    supportClawCompiledSecretTargets,
    supportClawTargetsById: buildConfigTargetIdIndex(supportClawCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(compiledSecretTargetRegistry),
  };
  return compiledSecretTargetRegistryState;
}

function getCompiledCoreSupportClawTargetState() {
  if (compiledCoreSupportClawTargetState) {
    return compiledCoreSupportClawTargetState;
  }
  const supportClawCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "supportClaw.json")
    .map(compileTargetRegistryEntry);
  compiledCoreSupportClawTargetState = {
    knownTargetIds: new Set(supportClawCompiledSecretTargets.map((entry) => entry.id)),
    supportClawCompiledSecretTargets,
    supportClawTargetsById: buildConfigTargetIdIndex(supportClawCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(supportClawCompiledSecretTargets),
  };
  return compiledCoreSupportClawTargetState;
}

function getCompiledChannelSupportClawTargets(
  channelId: string,
  ...
  if (compiledChannelSupportClawTargets.has(normalizedChannelId)) {
    return compiledChannelSupportClawTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadChannelSecretContractApi({
      ...
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "supportClaw.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledChannelSupportClawTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreSupportClawTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
/**
 * Resolves an supportClaw.json config path to the matching plan-capable secrets target.
 */
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreSupportClawTargetState().supportClawCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitChannelEntries = explicitChannelId
    ? getCompiledChannelSupportClawTargets(explicitChannelId)
    : null;
  ...
  for (const entry of getCompiledSecretTargetRegistryState().supportClawCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
    const matched = matchPathTokens(pathSegments, entry.pathTokens);
...
/**
 * Discovers configured secret-bearing values in supportClaw.json using the full registry.
 */
...
/**
 * Discovers configured supportClaw.json targets, optionally limited to selected registry ids.
 */
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreSupportClawTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreSupportClawTargetState()
      : getCompiledSecretTargetRegistryState();
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: registryState.supportClawCompiledSecretTargets,
    entriesById: registryState.supportClawTargetsById,
  });
