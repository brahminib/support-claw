...
describe("secrets runtime snapshot request secret refs", () => {
  it("can skip auth-profile SecretRef resolution when includeAuthStoreRefs is false", async () => {
    const missingEnvVar = `SUPPORT_CLAW_MISSING_AUTH_PROFILE_SECRET_${Date.now()}`;
  ...
  it("can skip auth-profile SecretRef resolution during active runtime refresh", async () => {
    const initialEnvVar = `SUPPORT_CLAW_INITIAL_AUTH_PROFILE_SECRET_${Date.now()}`;
    const missingEnvVar = `SUPPORT_CLAW_MISSING_AUTH_PROFILE_SECRET_${Date.now()}`;
    delete process.env[missingEnvVar];
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-runtime-exec-preflight-"));
      try {
