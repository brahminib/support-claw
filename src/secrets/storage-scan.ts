...
import { isRecord as isJsonObject } from "@supportclaw/normalization-core/record-coerce";
...
export function listAuthProfileStoreAgentDirs(config: SupportClawConfig, stateDir: string): string[] {
...
function resolveActiveAgentDir(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  const override = env.SUPPORT_CLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
  if (override) {
