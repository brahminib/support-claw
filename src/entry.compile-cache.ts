...
export function shouldEnableSupportClawCompileCache(params: {
...
export function resolveSupportClawCompileCacheDirectory(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): string {
...
type SupportClawCompileCacheRespawnPlan = {
  command: string;
  args: string[];
  env: NodeJS.ProcessEnv;
...
type SupportClawCompileCacheRespawnRuntime = RespawnChildRuntime & {
  writeError: (message: string) => void;
};

export function buildSupportClawCompileCacheRespawnPlan(params: {
  currentFile: string;
  env?: NodeJS.ProcessEnv;
  execArgv?: string[];
  execPath?: string;
  ...
  nodeVersion?: string;
  platform?: NodeJS.Platform;
}): SupportClawCompileCacheRespawnPlan | undefined {
  const env = params.env ?? process.env;
...
export function respawnWithoutSupportClawCompileCacheIfNeeded(params: {
  currentFile: string;
  installRoot: string;
}): boolean {
  ...
  runSupportClawCompileCacheRespawnPlan(plan);
  return true;
}

export function runSupportClawCompileCacheRespawnPlan(
  plan: SupportClawCompileCacheRespawnPlan,
  runtime: SupportClawCompileCacheRespawnRuntime = {
    spawn,
  ...
  return runRespawnChildWithSignalBridge({
    ...
    onError: (error) => {
      runtime.writeError(
        `[supportClaw] Failed to respawn CLI without compile cache: ${
...
export function enableSupportClawCompileCache(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): void {
  if (!shouldEnableSupportClawCompileCache(params)) {
    return;
  }
  try {
    enableCompileCache(resolveSupportClawCompileCacheDirectory(params));
  } catch {
