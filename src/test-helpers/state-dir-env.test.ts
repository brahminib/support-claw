...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    supportClaw: process.env.SUPPORT_CLAW_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.SUPPORT_CLAW_STATE_DIR).toBe(snapshot.supportClaw);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips SUPPORT_CLAW_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/supportClaw-state-dir-test");
    expect(process.env.SUPPORT_CLAW_STATE_DIR).toBe("/tmp/supportClaw-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("supportClaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("supportClaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
