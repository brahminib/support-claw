...
const fixtureRoot = makeTrackedTempDir("supportClaw-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadSupportClawPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "supportClaw.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("supportClaw plugins inspect <id> --runtime --json");
    expect(summary).toContain("supportClaw plugins list");
  });
});
