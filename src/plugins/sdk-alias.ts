...
import { resolveSupportClawDevSourceRoot } from "./dev-source-root.js";
...
function hasTrustedSupportClawRootIndicator(params: {
  packageRoot: string;
  ...
  const hasSupportClawBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("supportClaw")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.supportClaw === "string");
  const hasSupportClawEntrypoint = fs.existsSync(path.join(params.packageRoot, "supportClawCli.mjs"));
  return hasCliEntryExport || hasSupportClawBin || hasSupportClawEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedSupportClawRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedSupportClawRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedSupportClawRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveDevSourceRootParam(params: { devSourceRoot?: string | null }): string | null {
  return params.devSourceRoot !== undefined
    ...
    : resolveSupportClawDevSourceRoot(process.env);
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveSupportClawPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedSupportClawRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["supportClaw/plugin-sdk", "@supportclaw/plugin-sdk"] as const;
...
const PRIVATE_PLUGIN_SDK_SUBPATH_OWNERS: readonly PrivatePluginSdkSubpathOwner[] = [
  {
    ...
    officialInstalledPackageName: "@supportclaw/codex",
...
const WORKSPACE_PACKAGE_ALIAS_ENTRIES: WorkspacePackageAliasEntry[] = [
  {
    packageName: "@supportclaw/gateway-client",
  ...
  {
    packageName: "@supportclaw/gateway-protocol",
  ...
  {
    packageName: "@supportclaw/markdown-core",
  ...
  {
    packageName: "@supportclaw/media-generation-core",
  ...
  {
    packageName: "@supportclaw/media-core",
  ...
  {
    packageName: "@supportclaw/normalization-core",
  ...
  {
    packageName: "@supportclaw/terminal-core",
  ...
  {
    packageName: "@supportclaw/net-policy",
  ...
  {
    packageName: "@supportclaw/model-catalog-core",
...
export function listWorkspacePackageExportAliasEntries(params: {
  ...
  const fallbackPackageRoot = resolveSupportClawPackageRootSync({ cwd: process.cwd() });
  const packageJson =
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@supportclaw/") ? name : null;
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.hasOwn(aliasMap, `supportClaw/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "supportClaw/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
...
export function isBundledPluginExtensionPath(params: {
  modulePath: string;
  supportClawPackageRoot: string;
  bundledPluginsDir?: string;
}): boolean {
  const normalizedModulePath = path.resolve(params.modulePath);
  const roots = [
    params.bundledPluginsDir ? path.resolve(params.bundledPluginsDir) : null,
    path.join(params.supportClawPackageRoot, "extensions"),
    path.join(params.supportClawPackageRoot, "dist", "extensions"),
    path.join(params.supportClawPackageRoot, "dist-runtime", "extensions"),
