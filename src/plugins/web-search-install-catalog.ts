...
import { normalizeOptionalString as normalizeString } from "@supportclaw/normalization-core/string-coerce";
...
function getConfigPath(config: SupportClawConfig | undefined, path: string): unknown {
...
function setConfigPath(target: SupportClawConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: SupportClawConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: SupportClawConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: SupportClawConfig) =>
