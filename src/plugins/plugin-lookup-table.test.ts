...
vi.mock("../channels/config-presence.js", () => ({
  ...
  listExplicitlyDisabledChannelIdsForConfig: (config: SupportClawConfig) =>
...
function createManifestRecord(
  ...
  return {
    ...
    manifestPath: `/plugins/${plugin.id}/supportClaw.plugin.json`,
    ...plugin,
  };
}
...
const indexDiagnostic = {
  level: "warn",
  source: "/plugins/demo/supportClaw.plugin.json",
...
describe("loadPluginLookUpTable", () => {
  beforeEach(() => {
    ...
    listPotentialConfiguredChannelIds
      .mockReset()
      .mockImplementation((config: SupportClawConfig) => Object.keys(config.channels ?? {}));
  ...
  it("does not reuse lookup tables when channel env presence changes startup scope", async () => {
    listPotentialConfiguredChannelIds.mockImplementation(
      (_config: SupportClawConfig, env: NodeJS.ProcessEnv) => (env.TELEGRAM_TOKEN ? ["telegram"] : []),
    );
    const plugins = [
      createManifestRecord({
        id: "telegram",
  ...
  it("rebuilds when a provided metadata snapshot has stale plugin paths", async () => {
    ...
    const requestedPlugins = [
      createManifestRecord({
        ...
        manifestPath: "/plugins-moved/telegram/supportClaw.plugin.json",
      }),
    ];
    const config = {
      channels: {
