...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as SupportClawConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as SupportClawConfig;
    const load = vi.fn(async (config?: SupportClawConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: SupportClawConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
