...
export type SupportClawPeerLinkAuditIssue = {
  packageName: string;
  packageDir: string;
  reason: string;
};
...
type AuditManagedNpmRootResult = {
  checked: number;
  broken: number;
  issues: SupportClawPeerLinkAuditIssue[];
};

type SupportClawPeerLinkResult = "linked" | "skipped" | "unchanged";
...
async function auditSupportClawPeerDependency(params: {
  hostRoot: string;
  packageDir: string;
  npmRoot?: string;
  packageName?: string;
}): Promise<SupportClawPeerLinkAuditIssue | null> {
  const packageName =
    params.packageName ??
    (params.npmRoot
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return {
        packageName,
        packageDir: params.packageDir,
        reason: `missing ${path.join(nodeModulesDir, "supportClaw")}`,
      };
    }
    throw error;
  }

  const linkPath = path.join(nodeModulesDir, "supportClaw");
...
export async function auditSupportClawPeerDependencyLink(params: {
  packageDir: string;
  packageName?: string;
}): Promise<SupportClawPeerLinkAuditIssue | null> {
  ...
  if (!hostRoot) {
    return {
      packageName,
      packageDir: params.packageDir,
      reason: "could not locate supportClaw package root",
    };
  }
  return await auditSupportClawPeerDependency({
    hostRoot,
    packageDir: params.packageDir,
    packageName,
  });
...
async function ensureRealNodeModulesDir(params: {
  ...
  try {
    ...
    if (!existing.isDirectory() || existing.isSymbolicLink()) {
      params.logger.warn?.(
        `Skipping supportClaw peerDependency link because ${nodeModulesDir} is not a real directory.`,
      );
      return null;
    }
...
async function linkSupportClawPeerDependency(params: {
  hostRoot: string;
  installedDir: string;
  peerName: string;
  logger: PluginPeerLinkLogger;
}): Promise<SupportClawPeerLinkResult> {
  ...
  try {
    ...
    if (existing) {
      if (!existing.isSymbolicLink()) {
        if (params.peerName === "supportClaw" && existing.isDirectory()) {
          const existingPackageName = await readPackageName(linkPath);
          if (existingPackageName === "supportClaw") {
        ...
        params.logger.warn?.(
          `Skipping supportClaw peerDependency link because ${linkPath} already exists and is not a symlink.`,
        );
        return "skipped";
      }
...
/**
 * Symlink the host supportClaw package for plugins that declare it as a peer.
 ...
 */
export async function linkSupportClawPeerDependencies(params: {
  installedDir: string;
  peerDependencies: Record<string, string>;
  logger: PluginPeerLinkLogger;
}): Promise<{ repaired: number; skipped: number }> {
  const peers = Object.keys(params.peerDependencies).filter((name) => name === "supportClaw");
  if (peers.length === 0) {
  ...
  if (!hostRoot) {
    params.logger.warn?.(
      "Could not locate supportClaw package root to symlink peerDependencies; plugin may fail to resolve supportClaw at runtime.",
    );
  ...
  for (const peerName of peers) {
    const result = await linkSupportClawPeerDependency({
      hostRoot,
      installedDir: params.installedDir,
      peerName,
      logger: params.logger,
...
export async function relinkSupportClawPeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  logger: PluginPeerLinkLogger;
  ...
  for (const packageDir of await listManagedNpmRootPackageDirs(params.npmRoot)) {
    ...
    checked += 1;
    const result = await linkSupportClawPeerDependencies({
...
export async function auditSupportClawPeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  ...
  let checked = 0;
  const issues: SupportClawPeerLinkAuditIssue[] = [];
  for (const packageDir of await listManagedNpmRootPackageDirs(params.npmRoot)) {
    const peerDependencies = await readPackagePeerDependencies(packageDir);
    if (!Object.hasOwn(peerDependencies, "supportClaw")) {
      continue;
    ...
    checked += 1;
    const issue = await auditSupportClawPeerDependency({
      hostRoot,
      npmRoot: params.npmRoot,
      packageDir,
    });
