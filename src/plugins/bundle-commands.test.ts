...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".supportClaw", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "SUPPORT_CLAW_HOME", "SUPPORT_CLAW_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("supportClaw-bundle-commands-home-");
      const workspaceDir = await createTempDir("supportClaw-bundle-commands-workspace-");
