...
export type ConfigScopedRuntimeCache<T> = WeakMap<SupportClawConfig, Map<string, T>>;
...
export type ConfigScopedPromiseLoader<T> = {
  load(config?: SupportClawConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: SupportClawConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<SupportClawConfig, Promise<T>>();

  const createPromise = (config?: SupportClawConfig): Promise<T> => {
  ...
  return {
    async load(config?: SupportClawConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<SupportClawConfig, Promise<T>>();
    },
  };
}
