...
import {
  ...
  installedPackageNeedsSupportClawPeerLinkRepair,
...
import { linkSupportClawPeerDependencies } from "./plugin-peer-link.js";
...
function compareNpmSemverForUpdate(left: string, right: string): number {
  const releaseCmp = compareSupportClawReleaseVersions(left, right);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
> {
  ...
  if (
    !parsedSpec ||
    !parsedSpec.name.startsWith("@supportclaw/") ||
...
function isNpmMetadataCompatibleWithCurrentHost(metadata: NpmSpecResolution): boolean {
  ...
  const installMetadata = metadata.packageSupportClaw?.install;
  ...
  const pluginApiRangeCheck = resolvePackagePluginApiRange(metadata.packageSupportClaw);
...
function isBundledVersionNewer(bundledVersion: string, installedVersion: string): boolean {
  const releaseCmp = compareSupportClawReleaseVersions(bundledVersion, installedVersion);
  if (releaseCmp !== null) {
    return releaseCmp > 0;
  }
...
function shouldFallbackClawHubBridgeToNpm(params: {
  ...
  if (!isSupportClawOrgNpmSpec(params.npmSpec)) {
    return false;
  }
  return (
...
function migratePluginConfigId(cfg: SupportClawConfig, fromId: string, toId: string): SupportClawConfig {
...
function withoutPluginInstallRecord(cfg: SupportClawConfig, pluginId: string): SupportClawConfig {
...
function resetDisabledPluginSlots(
  slots: NonNullable<SupportClawConfig["plugins"]>["slots"] | undefined,
  pluginId: string,
): NonNullable<SupportClawConfig["plugins"]>["slots"] | undefined {
  if (!slots) {
    return slots;
  }
  let next = slots;
...
function disablePluginConfigEntry(config: SupportClawConfig, pluginId: string): SupportClawConfig {
...
async function repairSupportClawPeerLinksForNpmInstalls(params: {
  config: SupportClawConfig;
  logger: PluginUpdateLogger;
}): Promise<boolean> {
  let repaired = false;
  for (const [pluginId, record] of Object.entries(params.config.plugins?.installs ?? {})) {
    ...
    try {
      ...
      params.logger.warn?.(
        `Could not repair supportClaw peer link for "${pluginId}" due to invalid install path: ${String(err)}`,
      );
      continue;
    }

    if (!installedPackageNeedsSupportClawPeerLinkRepair(installPath)) {
      continue;
    }

    const peerDependencies = readInstalledPackagePeerDependencies(installPath);
    if (!Object.hasOwn(peerDependencies, "supportClaw")) {
      continue;
    }
    ...
    try {
      ...
      if (peerLinkRepair.skipped > 0) {
        params.logger.warn?.(
          `Could not repair supportClaw peer link for "${pluginId}" at ${installPath}: ${warnings.join("; ") || "peer link repair was skipped"}`,
        );
        continue;
      }
      repaired = !installedPackageNeedsSupportClawPeerLinkRepair(installPath) || repaired;
    } catch (err) {
      params.logger.warn?.(
        `Could not repair supportClaw peer link for "${pluginId}" at ${installPath}: ${String(err)}`,
      );
    }
  }
  return repaired;
...
export async function updateNpmInstalledPlugins(params: {
  ...
  const recordFailure = (
    ...
    if (params.disableOnFailure && !params.dryRun) {
      const disabledMessage =
        `Disabled "${pluginId}" after plugin update failure; SupportClaw will continue without it. ` +
        message;
    ...
    if (
      ...
      if (metadataResult.ok) {
        ...
        if (
          ...
          !installedPackageNeedsSupportClawPeerLinkRepair(installPath) &&
  ...
  if (ranNpmInstaller) {
    changed =
      (await repairSupportClawPeerLinksForNpmInstalls({
        config: next,
        logger,
      })) || changed;
  }
