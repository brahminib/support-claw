/** Installs native Node resolution aliases so plugins can import the SupportClaw SDK in dev and tests. */
...
export type InstallSupportClawPluginSdkNativeResolverOptions = {
  modulePath?: string;
...
const PLUGIN_SDK_PACKAGE_PREFIXES = ["supportClaw/plugin-sdk", "@supportclaw/plugin-sdk"] as const;
const INTERNAL_CORE_PACKAGE_ALIASES = [
  ...
  {
    packageName: "@supportclaw/llm-core",
    packageDir: "llm-core",
    subpaths: [
      ["", "index.ts"],
...
function resolveLoaderModulePath(options: InstallSupportClawPluginSdkNativeResolverOptions): string {
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        if (
          packageJson.name === "supportClaw" ||
          (typeof packageJson.bin === "object" &&
            packageJson.bin !== null &&
            typeof (packageJson.bin as { supportClaw?: unknown }).supportClaw === "string")
        ) {
          return cursor;
        }
      } catch {
...
function resolveAllowedParentRoots(
  options: InstallSupportClawPluginSdkNativeResolverOptions,
): string[] {
...
export function installSupportClawPluginSdkNativeResolver(
  options: InstallSupportClawPluginSdkNativeResolverOptions = {},
): string[] {
...
export function installSupportClawInternalCorePackageNativeResolver(
  options: Pick<InstallSupportClawPluginSdkNativeResolverOptions, "moduleUrl"> = {},
): string[] {
  for (const alias of listInternalCorePackageNativeAliases(options)) {
    registerNativeAlias(alias);
  }
...
export function resetSupportClawPluginSdkNativeResolverForTest(): void {
