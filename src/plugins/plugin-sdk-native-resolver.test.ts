...
import {
  installSupportClawPluginSdkNativeResolver,
  resetSupportClawPluginSdkNativeResolverForTest,
...
function writeFakeSupportClawPackage(root: string): { distRoot: string; loaderModulePath: string } {
  ...
  fs.writeFileSync(path.join(root, "supportClawCli.mjs"), "#!/usr/bin/env node\n", "utf8");
...
describe("installSupportClawPluginSdkNativeResolver", () => {
  it("resolves installed plugin SDK imports to the dev source root", () => {
    const stableRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sdk-native-stable-"));
    const devRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sdk-native-dev-source-"));
    const { loaderModulePath } = writeFakeSupportClawPackage(stableRoot);
    writeFakeSupportClawPackage(devRoot);
    ...
    const previousDevSourceRoot = process.env.SUPPORT_CLAW_DEV_SOURCE_ROOT;
    process.env.SUPPORT_CLAW_DEV_SOURCE_ROOT = devRoot;

    try {
      const installedAliases = installSupportClawPluginSdkNativeResolver({
        modulePath: loaderModulePath,
        pluginModulePath: externalPluginEntry,
      });

      expect(installedAliases).toContain("supportClaw/plugin-sdk/agent-runtime");
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(fs.realpathSync(requireFromPlugin.resolve("supportClaw/plugin-sdk/agent-runtime"))).toBe(
        fs.realpathSync(path.join(devRoot, "dist", "plugin-sdk", "agent-runtime.js")),
      );
    } finally {
      if (previousDevSourceRoot === undefined) {
        delete process.env.SUPPORT_CLAW_DEV_SOURCE_ROOT;
      } else {
        process.env.SUPPORT_CLAW_DEV_SOURCE_ROOT = previousDevSourceRoot;
      }
    }
  });
  ...
  it("removes stale native SDK aliases when a later dev root omits a subpath", () => {
    ...
    expect(fs.realpathSync(requireFromPlugin.resolve("supportClaw/plugin-sdk/stable-extra"))).toBe(
    ...
    expect(() => requireFromPlugin.resolve("supportClaw/plugin-sdk/stable-extra")).toThrow();
  });

  it("keeps native aliases on JS dist artifacts when source files exist", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sdk-native-source-resolver-"));
    const { loaderModulePath } = writeFakeSupportClawPackage(root);
    ...
    expect(installedAliases).toContain("supportClaw/plugin-sdk/channel-outbound");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(fs.realpathSync(requireFromPlugin.resolve("supportClaw/plugin-sdk/channel-outbound"))).toBe(
      fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")),
    );
  });

  it("lets built external plugins resolve SupportClaw SDK subpaths with createRequire", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sdk-native-resolver-"));
    const { distRoot, loaderModulePath } = writeFakeSupportClawPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    ...
    try {
      ...
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(
        fs.realpathSync(requireFromPlugin.resolve("supportClaw/plugin-sdk/channel-outbound")),
      ).toBe(fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")));
      const sdk = requireFromPlugin("supportClaw/plugin-sdk/channel-outbound") as {
        defineChannelMessageAdapter?: () => string;
      };

      expect(sdk.defineChannelMessageAdapter?.()).toBe("adapter");
      expect(() => requireFromPlugin.resolve("supportClaw/not-plugin-sdk/channel-message")).toThrow();
    } finally {
      if (process.platform !== "win32") {
  ...
  beforeAll(() => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sdk-native-esm-resolver-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'import path from "node:path";',
        'import { pathToFileURL } from "node:url";',
        `import { installSupportClawPluginSdkNativeResolver, resetSupportClawPluginSdkNativeResolverForTest } from ${JSON.stringify(resolverModuleUrl)};`,
        ...
        "fs.writeFileSync(",
        "  entryPath,",
        '  "import { defineChannelMessageAdapter } from \\"supportClaw/plugin-sdk/channel-outbound\\"; export const eager = defineChannelMessageAdapter(); export const loadLazy = () => import(\\"./lazy.js\\");\\n",',
        '  "utf8",',
        ");",
        "fs.writeFileSync(",
        "  lazyPath,",
        '  "import { defineChannelMessageAdapter } from \\"supportClaw/plugin-sdk/channel-outbound\\"; export const lazy = defineChannelMessageAdapter();\\n",',
        '  "utf8",',
        ");",
        "installSupportClawPluginSdkNativeResolver({",
        "  modulePath: loaderModulePath,",
  ...
  it("does not resolve SDK aliases for parents outside registered plugin roots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sdk-native-guard-"));
    const { loaderModulePath } = writeFakeSupportClawPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sdk-native-outside-"));
    ...
    expect(requireFromPlugin.resolve("supportClaw/plugin-sdk/channel-outbound")).toBeTruthy();
    expect(() => requireFromOutside.resolve("supportClaw/plugin-sdk/channel-outbound")).toThrow();
  });

  it("resolves internal core packages only for SupportClaw-owned source parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sdk-native-core-internal-"));
    const { loaderModulePath } = writeFakeSupportClawPackage(root);
    ...
    expect(installedAliases).toContain("@supportclaw/normalization-core/string-coerce");
    expect(installedAliases).toContain("@supportclaw/media-core/mime");
    expect(installedAliases).toContain("@supportclaw/acp-core/runtime/types");
    expect(installedAliases).toContain("@supportclaw/llm-core");
    const requireFromCoreSource = createRequire(coreSourceParent);
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@supportclaw/normalization-core/string-coerce")),
    ).toBe(fs.realpathSync(normalizationSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@supportclaw/media-core/mime"))).toBe(
      fs.realpathSync(mediaCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@supportclaw/acp-core/runtime/types"))).toBe(
      fs.realpathSync(acpCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@supportclaw/llm-core"))).toBe(
      fs.realpathSync(llmCoreSource),
    );
    expect(() => requireFromPlugin.resolve("@supportclaw/normalization-core/string-coerce")).toThrow();
    expect(() => requireFromPlugin.resolve("@supportclaw/media-core/mime")).toThrow();
    expect(() => requireFromPlugin.resolve("@supportclaw/acp-core/runtime/types")).toThrow();
    expect(() => requireFromPlugin.resolve("@supportclaw/llm-core")).toThrow();
  });

  it("does not register source-only SDK subpaths for native resolution", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sdk-native-source-only-"));
    const { loaderModulePath } = writeFakeSupportClawPackage(root);
    ...
    expect(installedAliases).toContain("supportClaw/plugin-sdk/channel-outbound");
    expect(installedAliases).not.toContain("supportClaw/plugin-sdk/source-only");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(() => requireFromPlugin.resolve("supportClaw/plugin-sdk/source-only")).toThrow();
  });

  it("scopes private SSRF SDK aliases to bundled local IPC native parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sdk-native-ssrf-"));
    const { loaderModulePath } = writeFakeSupportClawPackage(root);
    ...
    expect(installedAliases).toContain("supportClaw/plugin-sdk/ssrf-runtime-internal");
    const requireFromOllama = createRequire(ollamaEntry);
    expect(
      fs.realpathSync(requireFromOllama.resolve("supportClaw/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeOllama = createRequire(runtimeOllamaEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeOllama.resolve("supportClaw/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromBrowser = createRequire(browserEntry);
    expect(
      fs.realpathSync(requireFromBrowser.resolve("supportClaw/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeBrowser = createRequire(runtimeBrowserEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeBrowser.resolve("supportClaw/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromOther = createRequire(otherEntry);
    expect(() => requireFromOther.resolve("supportClaw/plugin-sdk/ssrf-runtime-internal")).toThrow();
  });
});
