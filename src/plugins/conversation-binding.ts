...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "supportClaw-app-server:thread:",
  "supportClaw-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("supportClaw.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("supportClaw.plugins.binding.global-state");
...
function openApprovalsDatabase() {
  return openSupportClawStateDatabase();
}
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to SupportClaw instead. If this started after an update, run "supportClaw doctor --fix"; otherwise reinstall or enable the plugin.${buildDetachHintSuffix(binding.detachHint)}`;
}
