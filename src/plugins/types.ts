...
} from "@supportclaw/model-catalog-core/model-catalog-types";
...
import type {
  SupportClawPluginHookOptions,
  SupportClawPluginToolFactory,
  SupportClawPluginToolOptions,
} from "./tool-types.js";
...
export type {
  SupportClawPluginActiveModelContext,
  SupportClawPluginHookOptions,
 ...
 */
export type SupportClawPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<SupportClawConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). SupportClaw uses this to expose
  ...
  ) => Promise<SupportClawConfig | null>;
};
 ...
 *
 * Runs after SupportClaw resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common SupportClaw auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and SupportClaw wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before SupportClaw builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. SupportClaw no
...
/**
 * Provider-owned stream wrapper hook after SupportClaw applies its generic
 ...
 *
 * Return `true` or `false` to override SupportClaw's built-in provider cache TTL
 ...
 *
 * Runs only after SupportClaw exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that SupportClaw should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after SupportClaw loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, SupportClaw falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * SupportClaw calls this only from async model resolution paths. After it
   ...
   *
   * Runs after SupportClaw performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before SupportClaw registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic SupportClaw wrappers.
   *
   ...
   *
   * Called after SupportClaw resolves the raw configured credential but before the
   ...
   * hint than SupportClaw's generic auth-store guidance.
   */
   ...
   * SupportClaw's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when SupportClaw should surface a provider-specific hint for
   ...
   * merging. SupportClaw deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. SupportClaw stores the
   ...
   * without replacing the full SupportClaw-owned system prompt.
   */
   ...
   *
   * Runs after SupportClaw's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep SupportClaw's default fallback policy.
   */
   ...
   * rewrites after SupportClaw has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before SupportClaw handles
  ...
  ) => SupportClawConfig | null | undefined;
  /**
   ...
   *
   * SupportClaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `supportClaw doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `supportClaw doctor` should offer.
   *
   ...
   *
   * SupportClaw calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep SupportClaw's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * SupportClaw falls back to that stored profile.
   */
...
export type SupportClawPluginGatewayMethod = {
  method: string;
...
export type PluginCommandDiagnosticsSession = {
  ...
  sessionKey?: string;
  /** Ephemeral SupportClaw session id when available. */
  sessionId?: string;
  /** Transcript file for this SupportClaw session when available. */
  sessionFile?: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active SupportClaw session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current SupportClaw configuration */
  config: SupportClawConfig;
...
export const AGENT_PROMPT_SURFACE_KINDS = [
  "supportClaw_main",
  /** @deprecated Use supportClaw_main. */
  "pi_main",
  "codex_app_server",
  "cli_backend",
  "acp_backend",
...
export type SupportClawPluginCommandDefinition = {
...
export type SupportClawPluginHttpRouteAuth = "gateway" | "plugin";
export type SupportClawPluginHttpRouteMatch = "exact" | "prefix";
export type SupportClawPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type SupportClawPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type SupportClawPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type SupportClawPluginHttpRouteParams = {
  path: string;
  handler: SupportClawPluginHttpRouteHandler;
  handleUpgrade?: SupportClawPluginHttpRouteUpgradeHandler;
  auth: SupportClawPluginHttpRouteAuth;
  match?: SupportClawPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: SupportClawPluginGatewayRuntimeScopeSurface;
  nodeCapability?: {
    surface: string;
    ttlMs?: number;
  };
...
export type SupportClawPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type SupportClawPluginCliContext = {
  /**
   * Command object where this plugin should register its commands.
   *
   * For root CLI registrations this is the root `supportClaw` program. For nested
...
export type SupportClawPluginCliRegistrar = (ctx: SupportClawPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want SupportClaw to keep a plugin command lazy-loaded while still
 ...
 */
export type SupportClawPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type SupportClawPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `supportClaw nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `supportClaw nodes`, so a descriptor named
   * `"camera"` exposes `supportClaw nodes camera`.
   */
  descriptors?: SupportClawPluginCliCommandDescriptor[];
};

export type SupportClawPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type SupportClawPluginNodeHostCommand = {
  command: string;
  cap?: string;
  dangerous?: boolean;
...
export type SupportClawPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type SupportClawPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type SupportClawPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: SupportClawPluginNodeInvokeApprovalDecision | null;
  }>;
};

export type SupportClawPluginNodeInvokePolicyContext = {
  nodeId: string;
  command: string;
  params: unknown;
  timeoutMs?: number;
  ...
  approvals?: SupportClawPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
  }) => Promise<SupportClawPluginNodeInvokeTransportResult>;
};

export type SupportClawPluginNodeInvokePolicyResult =
  | {
      ok: true;
      payload?: unknown;
      payloadJSON?: string | null;
...
export type SupportClawPluginNodeInvokePolicy = {
  commands: string[];
  /**
  ...
  handle: (
    ctx: SupportClawPluginNodeInvokePolicyContext,
  ) => Promise<SupportClawPluginNodeInvokePolicyResult> | SupportClawPluginNodeInvokePolicyResult;
};

export type SupportClawPluginSecurityAuditContext = {
  config: SupportClawConfig;
...
export type SupportClawPluginSecurityAuditCollector = (
  ctx: SupportClawPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type SupportClawGatewayDiscoveryAdvertiseContext = {
...
export type SupportClawGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: SupportClawGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type SupportClawPluginServiceContext = {
  config: SupportClawConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type SupportClawPluginService = {
  id: string;
  start: (ctx: SupportClawPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: SupportClawPluginServiceContext) => void | Promise<void>;
};

export type SupportClawPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type SupportClawPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  /**
   * @deprecated Declare exclusive plugin kind in `supportClaw.plugin.json` via
  ...
  configSchema?: SupportClawPluginConfigSchema;
  reload?: SupportClawPluginReloadRegistration;
  nodeHostCommands?: SupportClawPluginNodeHostCommand[];
  securityAuditCollectors?: SupportClawPluginSecurityAuditCollector[];
  register?: (api: SupportClawPluginApi) => void;
  activate?: (api: SupportClawPluginApi) => void;
};

export type SupportClawPluginModule = SupportClawPluginDefinition | ((api: SupportClawPluginApi) => void);
...
export type PluginConfigMigration = (config: SupportClawConfig) =>
  | {
      config: SupportClawConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `supportClaw migrate`. */
...
export type SupportClawPluginSessionStateApi = {
...
export type SupportClawPluginSessionWorkflowApi = {
...
export type SupportClawPluginSessionControlsApi = {
...
export type SupportClawPluginSessionApi = {
  state: SupportClawPluginSessionStateApi;
  workflow: SupportClawPluginSessionWorkflowApi;
  controls: SupportClawPluginSessionControlsApi;
};

export type SupportClawPluginAgentEventsApi = {
...
export type SupportClawPluginAgentApi = {
  events: SupportClawPluginAgentEventsApi;
};

export type SupportClawPluginRunContextApi = {
...
export type SupportClawPluginLifecycleApi = {
...
export type SupportClawPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
   ...
   */
  session: SupportClawPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: SupportClawPluginAgentApi;
  /** Grouped facade for run-scoped plugin scratch state. */
  runContext: SupportClawPluginRunContextApi;
  /** Grouped facade for plugin-owned lifecycle cleanup hooks. */
  lifecycle: SupportClawPluginLifecycleApi;
  registerTool: (
    tool: AnyAgentTool | SupportClawPluginToolFactory,
    opts?: SupportClawPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: SupportClawPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: SupportClawPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: SupportClawPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: SupportClawPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: SupportClawPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every command exposed at `parentPath`, SupportClaw
  ...
  /**
   * Register a plugin-owned node feature command group under `supportClaw nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: SupportClawPluginCliRegistrar,
    opts?: SupportClawPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: SupportClawPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: SupportClawPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: SupportClawPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: SupportClawPluginSecurityAuditCollector) => void;
  registerService: (service: SupportClawPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: SupportClawGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `supportClaw migrate` (migration capability). */
   ...
   */
  registerCommand: (command: SupportClawPluginCommandDefinition) => void;
