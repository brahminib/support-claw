...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/supportClaw-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "supportClaw.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): SupportClawConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    SUPPORT_CLAW_HOME: path.join(homeDir, "home"),
    SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
    SUPPORT_CLAW_DISABLE_PERSISTED_PLUGIN_REGISTRY:
