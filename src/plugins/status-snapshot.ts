...
/** Control-plane plugin status shape used by `supportClaw plugins status` style surfaces. */
...
function isPluginLifecycleTraceEnabled(): boolean {
  const raw = process.env.SUPPORT_CLAW_PLUGIN_LIFECYCLE_TRACE?.trim().toLowerCase();
...
function buildPluginRecordFromInstalledIndex(
  ...
  const format = plugin.format ?? manifest?.format ?? "supportClaw";
