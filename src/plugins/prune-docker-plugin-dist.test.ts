...
function writeDistPluginFile(repoRoot: string, root: "dist" | "dist-runtime", pluginId: string) {
  ...
  fs.writeFileSync(path.join(pluginDir, "supportClaw.plugin.json"), "{}\n", "utf8");
}
...
describe("pruneDockerPluginDist", () => {
  ...
  it("removes package-excluded plugin runtime artifacts unless Docker explicitly opts it in", () => {
    const repoRoot = makeRepoRoot("supportClaw-docker-plugin-dist-");
    ...
    const removed = pruneDockerPluginDist({
      repoRoot,
      env: { SUPPORT_CLAW_EXTENSIONS: "diagnostics-otel" } as NodeJS.ProcessEnv,
    });
  ...
  it("honors custom bundled plugin source roots when pruning Docker runtime importers", () => {
    const repoRoot = makeRepoRoot("supportClaw-docker-plugin-source-");
    writeJsonFile(path.join(repoRoot, "package.json"), {
    ...
    const removed = pruneDockerPluginDist({
      repoRoot,
      env: {
        SUPPORT_CLAW_BUNDLED_PLUGIN_DIR: "plugins",
  ...
  it("removes node_modules dependency closure that only omitted Docker plugins need", () => {
    const repoRoot = makeRepoRoot("supportClaw-docker-plugin-node-modules-");
    writeJsonFile(path.join(repoRoot, "package.json"), {
    ...
    writeNodePackage(repoRoot, "@supportclaw/acpx");
    writeNodePackage(repoRoot, "@supportclaw/codex");
    ...
    const removed = pruneDockerPluginDist({
      repoRoot,
      env: { SUPPORT_CLAW_EXTENSIONS: "codex" } as NodeJS.ProcessEnv,
    });

    expect(removed).toEqual([
      "node_modules/@supportclaw/acpx",
