...
vi.mock("./loader.js", () => ({
  ...
  loadSupportClawPlugins: mocks.loadSupportClawPlugins,
...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadSupportClawPlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadSupportClawPlugins).cache).toBe(true);
    expect(mockArg(mocks.loadSupportClawPlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to a scoped provider load when the active runtime registry has no web providers", () => {
    ...
    mocks.loadSupportClawPlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadSupportClawPlugins).cache).toBe(false);
    expect(mockArg(mocks.loadSupportClawPlugins).onlyPluginIds).toEqual(["brave"]);
  });
});
