...
function hasPackagePluginMetadata(manifest: Record<string, unknown>): boolean {
  const supportClaw = manifest.supportClaw;
  if (!isRecord(supportClaw)) {
    return false;
  }
  const extensions = supportClaw.extensions;
...
function readManifestPluginId(packageDir: string): string | undefined {
  const manifest = readJsonObjectFileSync(path.join(packageDir, "supportClaw.plugin.json"));
