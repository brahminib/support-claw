...
import { listSupportClawPluginManifestMetadata } from "./manifest-metadata-scan.js";
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
  ...
  writePersistedInstalledPluginIndexSync(
    {
      ...
      plugins: [
        {
          pluginId: "normalizer",
          manifestPath: path.join(params.pluginDir, "supportClaw.plugin.json"),
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/supportClaw.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("reuses current metadata when callers omit config", () => {
    const config: SupportClawConfig = { plugins: { allow: ["normalizer"] } };
    setCurrentPluginMetadataSnapshot(
      createCurrentSnapshot({
        manifestHash: "alpha",
        prefix: "alpha",
  ...
  it("keeps process metadata stable across manifest edits and reflects state-dir changes", () => {
    ...
    setTestEnvValue("SUPPORT_CLAW_STATE_DIR", stateDirA);
    deleteTestEnvValue("SUPPORT_CLAW_HOME");
    setTestEnvValue("SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS", "1");
    deleteTestEnvValue("SUPPORT_CLAW_BUNDLED_PLUGINS_DIR");
    ...
    setTestEnvValue("SUPPORT_CLAW_STATE_DIR", stateDirB);
    clearPluginMetadataLifecycleCaches();
  ...
  it("reuses manifest metadata while file fingerprints are unchanged", () => {
    ...
    expect(listSupportClawPluginManifestMetadata(process.env)).toHaveLength(1);
    expect(listSupportClawPluginManifestMetadata(process.env)).toHaveLength(1);
