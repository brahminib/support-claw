...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-supportClaw",
    targetDir: params?.targetDir ?? "/tmp/opik-supportClaw",
...
function createSuccessfulClawHubUpdateResult(params?: {
  ...
  return {
    ...
    targetDir: params?.targetDir ?? "/tmp/supportClaw-plugins/legacy-chat",
...
function createCodexAppServerInstallConfig(params: {
  ...
  return {
    plugins: {
      installs: {
        "supportClaw-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/supportClaw-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createInstalledPackageDir(params: {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-plugin-update-test-"));
  tempDirs.push(dir);
  fs.writeFileSync(
...
function createSupportClawPeerLinkFixtures(plugins: Array<{ pluginId: string; packageName: string }>) {
  const peerTarget = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-peer-target-"));
  ...
  const installPaths = Object.fromEntries(
    plugins.map(({ pluginId, packageName }) => [
      ...
      createInstalledPackageDir({
        name: packageName,
        version: "2026.5.4",
        peerDependencies: { supportClaw: ">=2026.5.4" },
      }),
    ]),
  );
  const peerLinkPath = (pluginId: string) =>
    path.join(installPaths[pluginId], "node_modules", "supportClaw");
...
function mockNpmViewMetadata(params: {
  ...
  integrity?: string;
  shasum?: string;
  supportClaw?: Record<string, unknown>;
}) {
  runCommandWithTimeoutMock.mockResolvedValueOnce({
    ...
    stdout: JSON.stringify({
      ...
      ...(params.supportClaw ? { supportClaw: params.supportClaw } : {}),
    }),
    stderr: "",
  });
}
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@supportclaw/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  const install = params.result.config.plugins?.installs?.["supportClaw-codex-app-server"];
  expect(install?.source).toBe("npm");
  expect(install?.spec).toBe(params.spec);
  expect(install?.installPath).toBe("/tmp/supportClaw-codex-app-server");
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-supportClaw",
        spec: "@opik/opik-supportClaw",
        integrity: "sha512-old",
        installPath: "/tmp/opik-supportClaw",
      }),
      pluginIds: ["opik-supportClaw"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-supportClaw",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-supportClaw",
        spec: "@opik/opik-supportClaw@0.2.5",
        integrity: "sha512-old",
        installPath: "/tmp/opik-supportClaw",
      }),
      pluginIds: ["opik-supportClaw"],
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "supportClaw-codex-app-server",
        spec: "supportClaw-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/supportClaw-codex-app-server",
      }),
      pluginIds: ["supportClaw-codex-app-server"],
      specOverrides: {
        "supportClaw-codex-app-server": "supportClaw-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "supportClaw-codex-app-server",
        targetDir: "/tmp/supportClaw-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "supportClaw-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("trusts official catalog npm updates when the installed package matches the catalog", async () => {
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "acpx",
        spec: "@supportclaw/acpx",
        installPath,
        resolvedName: "@supportclaw/acpx",
        resolvedSpec: "@supportclaw/acpx@2026.5.2-beta.1",
    ...
    expect(npmInstallCall()?.spec).toBe("@supportclaw/acpx");
    expect(npmInstallCall()?.expectedPluginId).toBe("acpx");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
    expect(result.config.plugins?.installs?.acpx?.spec).toBe("@supportclaw/acpx@2026.5.2-beta.2");
  });
  ...
  it("pins unchanged official npm records during official sync", async () => {
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        installPath,
        resolvedName: "@supportclaw/acpx",
        resolvedSpec: "@supportclaw/acpx@2026.5.2",
    ...
    expect(result.config.plugins?.installs?.acpx?.spec).toBe("@supportclaw/acpx@2026.5.2");
  ...
  it("keeps integrity drift checks for exact official pins during official sync", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "acpx",
        spec: "@supportclaw/acpx@2026.5.2",
        installPath,
        resolvedName: "@supportclaw/acpx",
        resolvedSpec: "@supportclaw/acpx@2026.5.2",
        resolvedVersion: "2026.5.2",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@supportclaw/acpx",
          version: "2026.5.1",
          resolvedSpec: "@supportclaw/acpx@2026.5.1",
        },
      }),
    );
  ...
  it("keeps integrity drift checks for exact prerelease-only official pins", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@supportclaw/voice-call",
          version: "0.0.2-beta.1",
          resolvedSpec: "@supportclaw/voice-call@0.0.2-beta.1",
        },
      }),
    );
    ...
    await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "voice-call",
        spec: "@supportclaw/voice-call@0.0.2-beta.1",
        installPath,
        resolvedName: "@supportclaw/voice-call",
        resolvedSpec: "@supportclaw/voice-call@0.0.2-beta.1",
  ...
  it("keeps integrity drift checks for exact official pins during beta fallback", async () => {
    ...
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error: "No matching version found for @supportclaw/acpx@beta",
    ...
    expect(npmInstallCall(0)?.spec).toBe("@supportclaw/acpx@beta");
    expect(npmInstallCall(0)?.expectedIntegrity).toBeUndefined();
    expect(npmInstallCall(1)?.spec).toBe("@supportclaw/acpx");
  ...
  it("does not skip trusted official default updates when latest resolves to the installed prerelease", async () => {
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "acpx",
        spec: "@supportclaw/acpx@2026.5.2-beta.2",
        installPath,
        integrity: "sha512-beta",
        shasum: "beta",
        resolvedName: "@supportclaw/acpx",
  ...
  it("updates trusted official npm plugins when latest resolves to a stable correction release", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@supportclaw/acpx",
          version: "2026.5.3-1",
          resolvedSpec: "@supportclaw/acpx@2026.5.3-1",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        installPath,
        resolvedName: "@supportclaw/acpx",
        resolvedSpec: "@supportclaw/acpx@2026.5.3",
  ...
  it("does not skip unchanged npm plugins when package metadata requires a newer plugin API", async () => {
    vi.stubEnv("SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION", "2026.5.28-beta.3");
    const installPath = createInstalledPackageDir({
      name: "@supportclaw/msteams",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@supportclaw/msteams",
          version: "2026.5.28-beta.3",
          resolvedSpec: "@supportclaw/msteams@2026.5.28-beta.3",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        resolvedSpec: "@supportclaw/msteams@2026.5.28-beta.4",
        integrity: "sha512-newer",
        shasum: "newer",
      }),
      pluginIds: ["msteams"],
    ...
    expect(npmInstallCall()?.spec).toBe("@supportclaw/msteams");
  ...
  it("repairs missing supportClaw peer links before skipping unchanged npm plugins", async () => {
    const installPath = createInstalledPackageDir({
      name: "@supportclaw/codex",
      version: "2026.5.3",
      peerDependencies: { supportClaw: ">=2026.5.3" },
    });
    mockNpmViewMetadata({
      name: "@supportclaw/codex",
      version: "2026.5.3",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@supportclaw/codex",
          version: "2026.5.3",
          resolvedSpec: "@supportclaw/codex@2026.5.3",
        },
      }),
    );
    const config: SupportClawConfig = {
    ...
    expect(npmInstallCall()?.spec).toBe("@supportclaw/codex");
    expect(npmInstallCall()?.mode).toBe("update");
  ...
  it("skips unchanged npm plugins when the supportClaw peer link already resolves", async () => {
    const installPath = createInstalledPackageDir({
      name: "@supportclaw/codex",
      version: "2026.5.3",
      peerDependencies: { supportClaw: ">=2026.5.3" },
    });
    fs.mkdirSync(path.join(installPath, "node_modules", "supportClaw"), { recursive: true });
    mockNpmViewMetadata({
      name: "@supportclaw/codex",
      version: "2026.5.3",
      integrity: "sha512-same",
  ...
  it("repairs supportClaw peer links after batch npm updates prune earlier plugin links", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@supportclaw/brave-plugin" },
      { pluginId: "codex", packageName: "@supportclaw/codex" },
      { pluginId: "discord", packageName: "@supportclaw/discord" },
    ];
    const { installPaths, peerLinkPath, linkPeer } = createSupportClawPeerLinkFixtures(plugins);
  ...
  it("repairs sibling supportClaw peer links after a targeted npm update prunes the shared install tree", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@supportclaw/brave-plugin" },
      { pluginId: "codex", packageName: "@supportclaw/codex" },
      { pluginId: "discord", packageName: "@supportclaw/discord" },
      ...
      return Promise.resolve(
        createSuccessfulNpmUpdateResult({
          ...
          npmResolution: {
            name: "@supportclaw/codex",
            version: "2026.5.4",
            resolvedSpec: "@supportclaw/codex@2026.5.4",
          },
        }),
      );
    });
  ...
  it("continues repairing sibling supportClaw peer links after one recorded npm install cannot be relinked", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@supportclaw/brave-plugin" },
      { pluginId: "codex", packageName: "@supportclaw/codex" },
    ];
    ...
    const brokenInstallPath = createInstalledPackageDir({
      name: "@supportclaw/broken-plugin",
      version: "2026.5.4",
      peerDependencies: { supportClaw: ">=2026.5.4" },
    });
      ...
      config: {
        plugins: {
          installs: {
            broken: {
              source: "npm",
              spec: "@supportclaw/broken-plugin",
              installPath: brokenInstallPath,
              resolvedName: "@supportclaw/broken-plugin",
              resolvedVersion: "2026.5.4",
              resolvedSpec: "@supportclaw/broken-plugin@2026.5.4",
            },
            ...Object.fromEntries(
              plugins.map(({ pluginId, packageName }) => [
                pluginId,
    ...
    expect(warnMessages).toEqual([
      `Could not repair supportClaw peer link for "broken" at ${brokenInstallPath}: Skipping supportClaw peerDependency link because ${path.join(brokenInstallPath, "node_modules")} is not a real directory.`,
    ]);
  });
  ...
  it("expands home-relative install paths before checking installed npm versions", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-plugin-update-home-"));
    tempDirs.push(home);
    const installPath = path.join(home, ".supportClaw", "extensions", "lossless-claw");
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "lossless-claw",
        spec: "@martian-engineering/lossless-claw",
        installPath: "~/.supportClaw/extensions/lossless-claw",
        resolvedName: "@martian-engineering/lossless-claw",
        resolvedVersion: "0.9.0",
        resolvedSpec: "@martian-engineering/lossless-claw@0.9.0",
        integrity: "sha512-same",
  ...
  it("updates disabled trusted official npm installs from the channel spec when requested", async () => {
      ...
      config: {
        plugins: {
          ...
          installs: {
            codex: {
              source: "npm",
              spec: "@supportclaw/codex@2026.5.3",
              installPath,
            },
          },
        },
    ...
    expectRecordFields(result.config.plugins?.installs?.codex, {
      source: "npm",
      spec: "@supportclaw/codex@2026.5.4",
      version: "2026.5.4",
      resolvedName: "@supportclaw/codex",
      resolvedVersion: "2026.5.4",
      resolvedSpec: "@supportclaw/codex@2026.5.4",
  ...
  it("preserves exact official npm pins when official install sync is not requested", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@supportclaw/codex",
          version: "2026.5.28",
          resolvedSpec: "@supportclaw/codex@2026.5.28",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "codex",
        spec: "@supportclaw/codex@2026.5.28",
        installPath,
        resolvedName: "@supportclaw/codex",
        resolvedSpec: "@supportclaw/codex@2026.5.28",
    ...
    expect(npmInstallCall()?.spec).toBe("@supportclaw/codex@2026.5.28");
    expect(npmInstallCall()?.expectedPluginId).toBe("codex");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
    expect(result.changed).toBe(false);
    expectRecordFields(result.outcomes[0], {
  ...
  it("reinstalls missing exact official npm pins without official install sync", async () => {
    const extensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-missing-plugin-"));
  ...
  it("updates disabled trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@supportclaw/diagnostics-otel",
      }),
    );
    ...
    const config = createClawHubInstallConfig({
      ...
      clawhubFamily: "code-plugin",
      clawhubChannel: "official",
      spec: "clawhub:@supportclaw/diagnostics-otel@2026.5.3",
    });
    const result = await updateNpmInstalledPlugins({
      config: {
        ...config,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@supportclaw/diagnostics-otel");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-otel"], {
      source: "clawhub",
      spec: "clawhub:@supportclaw/diagnostics-otel",
      version: "2026.5.4",
      clawhubPackage: "@supportclaw/diagnostics-otel",
      clawhubChannel: "official",
    });
  ...
  it("updates bare trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@supportclaw/diagnostics-prometheus",
      }),
    );
        ...
        plugins: {
          installs: {
            "diagnostics-prometheus": {
              source: "clawhub",
              spec: "clawhub:@supportclaw/diagnostics-prometheus@2026.5.3",
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@supportclaw/diagnostics-prometheus");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-prometheus"], {
      source: "clawhub",
      spec: "clawhub:@supportclaw/diagnostics-prometheus",
      version: "2026.5.4",
      clawhubPackage: "@supportclaw/diagnostics-prometheus",
      clawhubChannel: "official",
    });
  ...
  it("clears stale plugin policy and slot references when disabling failed updates", async () => {
    ...
    const message =
      'Disabled "demo" after plugin update failure; SupportClaw will continue without it. Failed to update demo: security scan blocked install';
    expect(warn).toHaveBeenCalledWith(message);
    expect(result.changed).toBe(true);
    expect(result.config.plugins?.entries?.demo).toEqual({
      enabled: false,
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockImplementation(
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            integrity: "sha512-new",
            resolvedSpec: "@opik/opik-supportClaw@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-supportClaw@0.2.5",
          };
        }
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-supportClaw" (@opik/opik-supportClaw@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-supportClaw: aborted: npm package integrity drift detected for @opik/opik-supportClaw@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ok: false,
        code: "npm_package_not_found",
        error: "Package not found on npm: @supportclaw/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@supportclaw/missing",
        installPath: "/tmp/missing",
      }),
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @supportclaw/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        spec: "supportClaw-codex-app-server@beta",
        resolvedName: "supportClaw-codex-app-server",
        resolvedSpec: "supportClaw-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "supportClaw-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "supportClaw-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "supportClaw-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "supportClaw-codex-app-server",
      }),
      specOverrides: {
        "supportClaw-codex-app-server": "supportClaw-codex-app-server@beta",
      },
      expectedSpec: "supportClaw-codex-app-server@beta",
      expectedRecordSpec: "supportClaw-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "supportClaw-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "supportClaw-codex-app-server",
      });
  ...
  it("preserves explicit official npm tag overrides during manual updates", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@supportclaw/acpx",
          version: "2026.5.3-beta.1",
          resolvedSpec: "@supportclaw/acpx@2026.5.3-beta.1",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      ...
      pluginIds: ["acpx"],
      specOverrides: {
        acpx: "@supportclaw/acpx@beta",
      },
    });

    expectNpmUpdateCall({
      spec: "@supportclaw/acpx@beta",
      expectedPluginId: "acpx",
    });
  ...
  it("falls back to the default npm spec when a beta tag is unavailable", async () => {
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error:
          "npm ERR! code ETARGET\nnpm ERR! No matching version found for supportClaw-codex-app-server@beta.",
      })
      .mockResolvedValueOnce(
        createSuccessfulNpmUpdateResult({
          pluginId: "supportClaw-codex-app-server",
          ...
          npmResolution: {
            name: "supportClaw-codex-app-server",
            version: "0.2.6",
            resolvedSpec: "supportClaw-codex-app-server@0.2.6",
          },
        }),
      );
    ...
    expect(npmInstallCall(0)?.spec).toBe("supportClaw-codex-app-server@beta");
    expect(npmInstallCall(1)?.spec).toBe("supportClaw-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "supportClaw-codex-app-server" has no beta npm release for supportClaw-codex-app-server@beta; using supportClaw-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "supportClaw-codex-app-server",
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Updated supportClaw-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback used supportClaw-codex-app-server because supportClaw-codex-app-server@beta could not be used).",
    );
    expect(result.outcomes[0]?.channelFallback).toEqual({
      requestedSpec: "supportClaw-codex-app-server@beta",
      usedSpec: "supportClaw-codex-app-server",
      requestedLabel: "@beta",
      usedLabel: "@latest",
      reason: "unavailable",
      message:
        "plugin channel fallback: supportClaw-codex-app-server used @latest because @beta was unavailable",
    });
  });
  ...
  it("reports npm beta fallback as tentative during dry-run checks", async () => {
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Would update supportClaw-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback would use supportClaw-codex-app-server because supportClaw-codex-app-server@beta could not be used).",
    );
    expect(result.outcomes[0]?.channelFallback?.message).toBe(
      "plugin channel fallback: supportClaw-codex-app-server would use @latest because @beta was unavailable",
    );
  });
  ...
  it("falls back to the default npm spec when the beta package exists but is invalid", async () => {
    ...
    expect(npmInstallCall(1)?.spec).toBe("supportClaw-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "supportClaw-codex-app-server" failed beta npm update for supportClaw-codex-app-server@beta; using supportClaw-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "supportClaw-codex-app-server",
    ...
    expect(result.outcomes[0]?.channelFallback).toMatchObject({
      ...
      usedLabel: "@latest",
      reason: "failed",
      message: "plugin channel fallback: supportClaw-codex-app-server used @latest after @beta failed",
    });
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update supportClaw-codex-app-server: npm package not found for supportClaw-codex-app-server.",
        channelFallback: {
          requestedSpec: "supportClaw-codex-app-server@beta",
          usedSpec: "supportClaw-codex-app-server",
          requestedLabel: "@beta",
          ...
          reason: "failed",
          message:
            "plugin channel fallback: supportClaw-codex-app-server used @latest after @beta failed",
        },
      },
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to check supportClaw-codex-app-server: npm package not found for supportClaw-codex-app-server.",
        channelFallback: {
          requestedSpec: "supportClaw-codex-app-server@beta",
          usedSpec: "supportClaw-codex-app-server",
          requestedLabel: "@beta",
          ...
          reason: "failed",
          message:
            "plugin channel fallback: supportClaw-codex-app-server would use @latest after @beta failed",
        },
      },
    ]);
  });
  ...
  it("preserves explicit npm tags when updating on the beta channel", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createCodexAppServerInstallConfig({
        spec: "supportClaw-codex-app-server@rc",
      }),
      pluginIds: ["supportClaw-codex-app-server"],
      updateChannel: "beta",
      dryRun: true,
  ...
  it("falls back to npm for trusted official ClawHub artifact blocks", async () => {
    ...
    installPluginFromClawHubMock.mockResolvedValueOnce({
      ...
      error:
        'ClawHub artifact download for "@supportclaw/discord@2026.5.16-beta.5" is not available yet (ClawHub /api/v1/packages/%40supportClaw%2Fdiscord/versions/2026.5.16-beta.5/artifact/download failed (403): Blocked: this package release has been flagged as malicious and cannot be downloaded.). Use "npm:@supportclaw/discord@2026.5.16-beta.5" for launch installs while ClawHub artifact routing is being rolled out.',
    });
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@supportclaw/discord",
          version: "2026.5.16-beta.5",
          resolvedSpec: "@supportclaw/discord@2026.5.16-beta.5",
        },
      }),
    );
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@supportclaw/discord@beta");
    expect(npmInstallCall()?.spec).toBe("@supportclaw/discord@beta");
    ...
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@supportclaw/discord@2026.5.16-beta.5",
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        message:
          "Updated discord: 2026.5.12 -> 2026.5.16-beta.5. (warning: official ClawHub artifact fallback used @supportclaw/discord@beta).",
      },
    ]);
    expect(warnMessages).toEqual([
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@supportclaw/discord@beta; using npm @supportclaw/discord@beta instead. Core update can still complete.',
    ]);
  });
  ...
  it("uses the default npm spec when beta ClawHub falls back before an artifact block", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@supportclaw/discord",
          version: "2026.5.16",
          resolvedSpec: "@supportclaw/discord@2026.5.16",
        },
      }),
    );
    ...
    expect(clawHubInstallCall(0)?.spec).toBe("clawhub:@supportclaw/discord@beta");
    expect(clawHubInstallCall(1)?.spec).toBe("clawhub:@supportclaw/discord");
    expect(npmInstallCall()?.spec).toBe("@supportclaw/discord");
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@supportclaw/discord@2026.5.16",
      installPath: "/tmp/supportClaw-plugins/discord",
      version: "2026.5.16",
    });
    expect(result.outcomes[0]?.message).toBe(
      "Updated discord: 2026.5.12 -> 2026.5.16. (warning: official ClawHub artifact fallback used @supportclaw/discord).",
    );
    expect(warnMessages).toEqual([
      'Plugin "discord" has no beta ClawHub release for clawhub:@supportclaw/discord@beta; using clawhub:@supportclaw/discord instead. Core update can still complete.',
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@supportclaw/discord; using npm @supportclaw/discord instead. Core update can still complete.',
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        nextVersion: "2026.5.16-beta.5",
        message:
          "discord is up to date (2026.5.16-beta.5). (warning: official ClawHub artifact fallback would use @supportclaw/discord@beta).",
      },
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update discord: artifact unavailable (ClawHub clawhub:@supportclaw/discord@beta).",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@supportclaw/voice-call",
      targetDir: "/tmp/supportClaw-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(npmInstallCall()?.spec).toBe("@supportclaw/voice-call");
    expect(npmInstallCall()?.expectedPluginId).toBe("voice-call");
    expect(result.config.plugins?.allow).toEqual(["@supportclaw/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@supportclaw/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@supportclaw/voice-call");
    expect(result.config.plugins?.entries?.["@supportclaw/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expectRecordFields(result.config.plugins?.installs?.["@supportclaw/voice-call"], {
      source: "npm",
      spec: "@supportclaw/voice-call",
      installPath: "/tmp/supportClaw-voice-call",
      version: "0.0.2",
    });
  ...
  it("keeps authored plugin config shape when only the install key migrates", async () => {
    ...
    expect(result.config.plugins).toEqual({
      installs: {
        "@supportclaw/voice-call": expect.objectContaining({
          source: "npm",
          spec: "@supportclaw/voice-call",
          installPath: "/tmp/supportClaw-voice-call",
        }),
  ...
  it("migrates context engine slot when a plugin id changes during update", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@supportclaw/context-engine",
      targetDir: "/tmp/supportClaw-context-engine",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          ...
          installs: {
            "context-engine": {
              source: "npm",
              spec: "@supportclaw/context-engine",
    ...
    expect(result.config.plugins?.slots?.contextEngine).toBe("@supportclaw/context-engine");
    expectRecordFields(result.config.plugins?.installs?.["@supportclaw/context-engine"], {
      source: "npm",
      spec: "@supportclaw/context-engine",
      installPath: "/tmp/supportClaw-context-engine",
      version: "0.0.2",
    });
  ...
  it("forwards dangerous force unsafe install to plugin update installers", async () => {
    ...
    expect(npmInstallCall()?.spec).toBe("supportClaw-codex-app-server@beta");
    expect(npmInstallCall()?.dangerouslyForceUnsafeInstall).toBe(true);
    expect(npmInstallCall()?.expectedPluginId).toBe("supportClaw-codex-app-server");
  });

  it("reuses the recorded managed extensions root when updating external plugins", async () => {
    const installPath = "/var/supportClaw/extensions/demo";
    const extensionsDir = "/var/supportClaw/extensions";
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "demo",
        targetDir: installPath,
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    resolveBundledPluginSourcesMock.mockReturnValue(new Map());
    const env = { SUPPORT_CLAW_HOME: "/srv/supportClaw-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/supportClaw-home";
    mockBundledSources(
      createBundledSource({
  ...
  it("installs an externalized bundled plugin and rewrites its old bundled path plugin index", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/supportClaw-plugins/legacy-chat",
        version: "2.0.0",
        npmResolution: {
          name: "@supportclaw/legacy-chat",
          version: "2.0.0",
          resolvedSpec: "@supportclaw/legacy-chat@2.0.0",
        },
      }),
    );
      ...
      externalizedBundledPluginBridges: [
        {
          ...
          npmSpec: "@supportclaw/legacy-chat",
    ...
    expect(npmInstallCall()?.spec).toBe("@supportclaw/legacy-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      spec: "@supportclaw/legacy-chat",
      installPath: "/tmp/supportClaw-plugins/legacy-chat",
      version: "2.0.0",
      resolvedName: "@supportclaw/legacy-chat",
  ...
  it("marks official externalized bundled npm installs as trusted", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "voice-call",
        targetDir: "/tmp/supportClaw-plugins/voice-call",
        version: "0.0.2-beta.1",
      }),
    );
      ...
      externalizedBundledPluginBridges: [
        {
          ...
          npmSpec: "@supportclaw/voice-call",
  ...
  it("falls back from ClawHub to npm only when the ClawHub package is absent", async () => {
    ...
    expect(result.summary.warnings).toEqual([
      "ClawHub clawhub:legacy-chat@2026.5.1-beta.2 unavailable for legacy-chat; falling back to npm @supportclaw/legacy-chat.",
    ]);
    expect(result.summary.errors).toStrictEqual([]);
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      ...
      externalizedBundledPluginBridges: [
        {
          bundledPluginId: "voice-call",
          preferredSource: "clawhub",
          clawhubSpec: "clawhub:@supportclaw/voice-call",
          npmSpec: "@supportclaw/voice-call",
          channelIds: ["voice-call"],
        },
      ],
  ...
  it("externalizes bundled plugins that were enabled by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/supportClaw-plugins/default-chat",
        version: "2.0.0",
      }),
    );
      ...
      externalizedBundledPluginBridges: [
        {
          ...
          npmSpec: "@supportclaw/default-chat",
    ...
    expect(npmInstallCall()?.spec).toBe("@supportclaw/default-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["default-chat"], {
      source: "npm",
      spec: "@supportclaw/default-chat",
      installPath: "/tmp/supportClaw-plugins/default-chat",
      version: "2.0.0",
    });
  });
      ...
      config: {
        ...
        plugins: {
          ...
          installs: {
            "legacy-chat": {
              source: "npm",
              spec: "@supportclaw/legacy-chat@1.2.3",
              resolvedSpec: "@supportclaw/legacy-chat@1.2.3",
              installPath: "/tmp/supportClaw-plugins/legacy-chat",
            },
          },
        },
