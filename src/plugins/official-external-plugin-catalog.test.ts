...
describe("official external plugin catalog", () => {
  it("lists the externalized provider and capability plugins with install metadata", () => {
    const providers = [
      ["arcee", "@supportclaw/arcee-provider"],
      ["cerebras", "@supportclaw/cerebras-provider"],
      ["chutes", "@supportclaw/chutes-provider"],
      ["cloudflare-ai-gateway", "@supportclaw/cloudflare-ai-gateway-provider"],
      ["deepinfra", "@supportclaw/deepinfra-provider"],
      ["deepseek", "@supportclaw/deepseek-provider"],
      ["groq", "@supportclaw/groq-provider"],
      ["kilocode", "@supportclaw/kilocode-provider"],
      ["kimi", "@supportclaw/kimi-provider"],
      ["qianfan", "@supportclaw/qianfan-provider"],
      ["qwen", "@supportclaw/qwen-provider"],
    ] as const;
    const plugins = [
      ["exa", "@supportclaw/exa-plugin"],
      ["firecrawl", "@supportclaw/firecrawl-plugin"],
      ["gradium", "@supportclaw/gradium-speech"],
      ["inworld", "@supportclaw/inworld-speech"],
      ["parallel", "@supportclaw/parallel-plugin"],
      ["perplexity", "@supportclaw/perplexity-plugin"],
    ] as const;
    const newlyExternalized = [
      ["clickclack", "@supportclaw/clickclack"],
      ["fireworks", "@supportclaw/fireworks-provider"],
      ["irc", "@supportclaw/irc"],
      ["mattermost", "@supportclaw/mattermost"],
      ["moonshot", "@supportclaw/moonshot-provider"],
      ["searxng", "@supportclaw/searxng-plugin"],
      ["signal", "@supportclaw/signal"],
      ["sms", "@supportclaw/sms"],
      ["tavily", "@supportclaw/tavily-plugin"],
      ["tencent", "@supportclaw/tencent-provider"],
      ["venice", "@supportclaw/venice-provider"],
      ["vercel-ai-gateway", "@supportclaw/vercel-ai-gateway-provider"],
      ["zai", "@supportclaw/zai-provider"],
    ] as const;
  ...
  it("advertises StepFun with its ClawHub package and plugin API floor", () => {
    expect(resolveOfficialExternalPluginInstall(expectCatalogEntry("stepfun"))).toEqual({
      clawhubSpec: "clawhub:@supportclaw/stepfun-provider",
      npmSpec: "@supportclaw/stepfun-provider",
      defaultChoice: "npm",
      minHostVersion: ">=2026.6.9",
    });
  });
  ...
  it("resolves third-party channel lookup aliases to published plugin ids", () => {
    ...
    const wecomByPlugin = expectCatalogEntry("wecom-supportClaw-plugin");
    const yuanbaoByChannel = expectCatalogEntry("yuanbao");

    expect(resolveOfficialExternalPluginId(wecomByChannel)).toBe("wecom-supportClaw-plugin");
    expect(resolveOfficialExternalPluginId(wecomByPlugin)).toBe("wecom-supportClaw-plugin");
    expect(resolveOfficialExternalPluginInstall(wecomByChannel)?.npmSpec).toBe(
      "@wecom/wecom-supportClaw-plugin@2026.5.7",
    );
    expect(resolveOfficialExternalPluginId(yuanbaoByChannel)).toBe("supportClaw-plugin-yuanbao");
    expect(resolveOfficialExternalPluginInstall(yuanbaoByChannel)?.npmSpec).toBe(
      "supportClaw-plugin-yuanbao@2.15.0",
    );
  });
  ...
  it("keeps official launch package specs on the production package names", () => {
    ...
    expect(resolveOfficialExternalPluginInstall(expectCatalogEntry("diffs-language-pack"))).toEqual(
      {
        npmSpec: "@supportclaw/diffs-language-pack",
        clawhubSpec: "clawhub:@supportclaw/diffs-language-pack",
        defaultChoice: "npm",
        minHostVersion: ">=2026.5.27",
      },
    );
    expect(resolveOfficialExternalPluginInstall(expectCatalogEntry("llama-cpp"))?.npmSpec).toBe(
      "@supportclaw/llama-cpp-provider",
    );
  });
  ...
  it("keeps Groq available through the cold-install auth catalog", () => {
    ...
    const authChoice = groq.supportClaw?.providers?.find((provider) => provider.id === "groq")
      ?.authChoices?.[0];
