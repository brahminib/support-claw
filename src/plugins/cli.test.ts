...
const mocks = vi.hoisted(() => ({
  ...
  loadSupportClawPluginCliRegistry: vi.fn(),
  loadSupportClawPlugins: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadSupportClawPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadSupportClawPluginCliRegistry(...args),
  loadSupportClawPlugins: (...args: unknown[]) => mocks.loadSupportClawPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: SupportClawConfig;
  autoEnabledConfig: SupportClawConfig;
  ...
  const loadOptions = getMockCallObject(mocks.loadSupportClawPlugins);
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadSupportClawPluginCliRegistry.mockReset();
    mocks.loadSupportClawPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadSupportClawPlugins.mockReset();
    mocks.loadSupportClawPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as SupportClawConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as SupportClawConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as SupportClawConfig, env);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as SupportClawConfig);

    const loadOptions = getMockCallObject(mocks.loadSupportClawPlugins) as {
  ...
  it("reuses loaded plugin CLI entries on repeat calls for the same program", async () => {
    ...
    expect(mocks.loadSupportClawPlugins).toHaveBeenCalledTimes(1);
  });

  it("reloads plugin CLI entries when the requested primary command changes", async () => {
    const program = createProgram();

    await registerPluginCliCommands(program, {} as SupportClawConfig, undefined, undefined, {
      primary: "memory",
    });
    await registerPluginCliCommands(program, {} as SupportClawConfig);

    expect(mocks.loadSupportClawPlugins).toHaveBeenCalledTimes(2);
  });

  it("reloads plugin CLI entries when config or environment identity changes", async () => {
    const program = createProgram();
    const configA = {} as SupportClawConfig;
    const configB = { plugins: {} } as SupportClawConfig;
    const envA = { SUPPORT_CLAW_HOME: "/tmp/a" } as NodeJS.ProcessEnv;
    const envB = { SUPPORT_CLAW_HOME: "/tmp/b" } as NodeJS.ProcessEnv;
    ...
    expect(mocks.loadSupportClawPlugins).toHaveBeenCalledTimes(3);
  });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadSupportClawPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    const registryOptions = getMockCallObject(mocks.loadSupportClawPluginCliRegistry);
  ...
  it("keeps root-help descriptor load failures quiet", async () => {
    ...
    mocks.loadSupportClawPluginCliRegistry.mockImplementationOnce((options: { logger?: unknown }) => {
    ...
    await expect(
      getPluginCliCommandDescriptors({ plugins: { entries: { stale: {} } } } as SupportClawConfig),
    ).resolves.toEqual([]);
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadSupportClawPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as SupportClawConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("scopes full CLI loading through CLI metadata when manifest planning finds no plugin match", async () => {
    ...
    expect(mocks.loadSupportClawPluginCliRegistry).toHaveBeenCalled();
    const loadOptions = getMockCallObject(mocks.loadSupportClawPlugins);
    expect(loadOptions.onlyPluginIds).toEqual(["memory-core"]);
  });
  ...
  it("scopes nested CLI loading through CLI metadata parent paths", async () => {
    ...
    mocks.loadSupportClawPluginCliRegistry.mockResolvedValue(nestedRegistry);
    mocks.loadSupportClawPlugins.mockReturnValue(nestedRegistry);
    const program = createProgram("nodes");
    program.exitOverride();
  ...
  it("skips full plugin runtime loading when no metadata owns the requested primary", async () => {
    ...
    expect(mocks.loadSupportClawPluginCliRegistry).toHaveBeenCalled();
    expect(mocks.loadSupportClawPlugins).not.toHaveBeenCalled();
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as SupportClawConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
