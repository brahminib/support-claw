...
const nixEnv: NodeJS.ProcessEnv = { SUPPORT_CLAW_NIX_MODE: "1" };
...
describe("plugin hardlink policy", () => {
  ...
  it("does not treat SUPPORT_CLAW_NIX_MODE as enough by itself", () => {
    expect(
      shouldRejectHardlinkedPluginFiles({
        origin: "config",
        rootDir: "/tmp/plugin",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      expect(isNixStorePluginRoot("/nix/store/abc-supportClaw-plugin")).toBe(true);
      expect(isNixStorePluginRoot("/tmp/nix/store/abc-supportClaw-plugin")).toBe(false);
      expect(
        shouldRejectHardlinkedPluginFiles({
          origin: "config",
          rootDir: "/nix/store/abc-supportClaw-plugin",
          env: nixEnv,
        }),
      ).toBe(false);
    },
