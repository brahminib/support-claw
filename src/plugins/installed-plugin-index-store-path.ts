...
import type { SupportClawStateDatabaseOptions } from "../state/supportClaw-state-db.js";
...
function resolveStoreEnv(options: InstalledPluginIndexStoreOptions): NodeJS.ProcessEnv {
  return options.stateDir
    ? { ...(options.env ?? process.env), SUPPORT_CLAW_STATE_DIR: options.stateDir }
...
export function resolveInstalledPluginIndexStorePath(
  ...
  return resolveSupportClawStateSqlitePath(resolveStoreEnv(options));
}
...
export function resolveInstalledPluginIndexStateDatabaseOptions(
  options: InstalledPluginIndexStoreOptions = {},
): SupportClawStateDatabaseOptions {
  if (options.filePath) {
    return {
