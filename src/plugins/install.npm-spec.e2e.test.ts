...
async function makeTempDir(label: string): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `supportClaw-${label}-`));
  tempDirs.push(dir);
  return dir;
}

function configWithInstalledPackageTreeBlockPolicy(): SupportClawConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
...
async function packPlugin(params: {
  ...
  await fs.writeFile(
    path.join(packageDir, "supportClaw.plugin.json"),
    `${JSON.stringify(
      {
        id: params.pluginId,
            ...
            versions: Object.fromEntries(
              [...pkg.versionsByVersion.entries()].map(([version, entry]) => [
                version,
                {
                  name: pkg.packageName,
                  version,
                  ...(entry.supportClaw ? { supportClaw: entry.supportClaw } : {}),
...
describe("installPluginFromNpmSpec e2e", () => {
  it("installs the newest compatible stable package when npm latest requires a newer plugin API", async () => {
    ...
    const versions = [
      await packPlugin({
        ...
        version: "2026.5.26",
        rootDir,
        supportClaw: compatibleSupportClaw,
      }),
      await packPlugin({
        packageName,
        pluginId: packageName,
        version: "2026.5.27",
        rootDir,
        supportClaw: incompatibleSupportClaw,
      }),
    ];
    const registry = await startStaticRegistry([{ packageName, latest: "2026.5.27", versions }]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    const previousHostVersion = process.env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION;
    process.env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION = "2026.5.10-beta.1";
    ...
    try {
      ...
      if (previousHostVersion === undefined) {
        ...
        process.env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION = previousHostVersion;
      }
    }
  });

  it("scrubs root supportClaw materialized by required npm peers", async () => {
    ...
    const supportClawVersions = [
      await packPlugin({
        packageName: "supportClaw",
        pluginId: "registry-supportClaw-copy",
        version: "2026.0.0",
        rootDir,
      }),
    ];
    const registry = await startStaticRegistry([
      ...
      { packageName: "supportClaw", latest: "2026.0.0", versions: supportClawVersions },
    ]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    ...
    const rawSupportClawLockEntry = rawLock.packages?.["node_modules/supportClaw"] as
      | { peer?: unknown; version?: unknown }
      | undefined;
    expect(rawSupportClawLockEntry?.peer).toBe(true);
    expect(rawSupportClawLockEntry?.version).toBe("2026.0.0");
    ...
    expect(lock.packages?.["node_modules/supportClaw"]).toBeUndefined();
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", "supportClaw")),
    ).rejects.toHaveProperty("code", "ENOENT");
    await expect(
      fs
        .lstat(path.join(result.targetDir, "node_modules", "supportClaw"))
  ...
  it("plans peers from installed optional dependencies", async () => {
    ...
    const rootManifest = JSON.parse(
      ...
      dependencies?: Record<string, string>;
      supportClaw?: { managedPeerDependencies?: string[] };
    };
    expect(["1.0.0", "^1.0.0"]).toContain(rootManifest.dependencies?.[runtimePeer]);
    expect(rootManifest.supportClaw?.managedPeerDependencies ?? []).toContain(runtimePeer);
  });
  ...
  it("leaves legacy flat-root peer dependencies alone during isolated later installs", async () => {
    ...
    expect(rootManifest.supportClaw?.managedPeerDependencies ?? []).not.toContain(runtimePeer);
  });
  ...
  it("falls back to the legacy npm peer mode inside the plugin project when npm cannot plan third-party peers", async () => {
    ...
    expect(rootManifest.supportClaw?.managedPeerDependencies ?? []).not.toContain(missingPeer);
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", blockedPlugin, "package.json")),
    ).resolves.toBeTruthy();
  });
  ...
  it("does not take ownership of an existing root dependency observed as a peer", async () => {
    ...
    expect(rootManifest.dependencies?.[blockedPlugin]).toBeUndefined();
    expect(rootManifest.dependencies?.[runtimePeer]).toBeUndefined();
    expect(rootManifest.supportClaw?.managedPeerDependencies ?? []).not.toContain(
  ...
  it("scrubs host peers inside each isolated npm project", async () => {
    ...
    const registry = await startStaticRegistry([
      {
        ...
        versions: [
          await packPlugin({
            packageName: codexName,
            peerDependencies: { supportClaw: ">=2026.5.5-beta.2" },
            peerDependenciesMeta: { supportClaw: { optional: true } },
            pluginId: codexName,
            version: "1.0.0",
            rootDir,
          }),
      ...
      },
      {
        ...
        versions: [
          await packPlugin({
            packageName: opikName,
            peerDependencies: { supportClaw: ">=2026.3.2" },
            peerDependenciesMeta: {},
            pluginId: opikName,
            version: "1.0.0",
            rootDir,
    ...
    await expect(
      fs
        .lstat(path.join(first.targetDir, "node_modules", "supportClaw"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
    await expect(
      fs
        .lstat(path.join(second.targetDir, "node_modules", "supportClaw"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
  });

  it("keeps an earlier isolated supportClaw peer link after later plugin installs", async () => {
    ...
    const peerLink = path.join(first.targetDir, "node_modules", "supportClaw");
    ...
    expect(manifest.dependencies?.supportClaw).toBeUndefined();
    const lock = JSON.parse(
