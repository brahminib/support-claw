...
import { normalizeOptionalString as normalizeTrimmedString } from "@supportclaw/normalization-core/string-coerce";
...
const PLUGIN_MANIFEST_FILENAME = "supportClaw.plugin.json";
...
function resolveUserPath(value: string, env: NodeJS.ProcessEnv): string {
  if (value === "~" || value.startsWith("~/")) {
    const home = env.SUPPORT_CLAW_HOME ?? env.HOME ?? env.USERPROFILE ?? os.homedir();
...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  const override = normalizeTrimmedString(env.SUPPORT_CLAW_STATE_DIR);
  if (override) {
...
export function listSupportClawPluginManifestMetadata(
