...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "supportClaw", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".supportClaw", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".supportClaw", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "supportClaw-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".supportClaw", "extensions"),
      },
    });
  });
});
