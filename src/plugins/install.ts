...
import {
  ...
  readSupportClawManagedNpmRootOverrides,
  repairManagedNpmRootSupportClawPeer,
...
import { installedPackageNeedsSupportClawPeerLinkRepair } from "../infra/package-update-utils.js";
import {
...
import {
  ...
  type SupportClawPackageManifest,
  type PackageManifest as PluginPackageManifest,
} from "./manifest.js";
import { resolvePackagePluginApiRange } from "./package-compat.js";
import { validatePackageExtensionEntriesForInstall } from "./package-entry-resolution.js";
import {
  linkSupportClawPeerDependencies,
  relinkSupportClawPeerDependenciesInManagedNpmRoot,
...
function formatUnresolvedSupportClawPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares supportClaw as a peer dependency, but SupportClaw could not create a plugin-local node_modules/supportClaw link. Run from a packaged SupportClaw install or reinstall SupportClaw, then retry.`;
}
...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing supportClaw.extensions; update the plugin package to include supportClaw.extensions (for example ["./dist/index.js"]). See https://docs.supportClaw.ai/help/troubleshooting#plugin-install-fails-with-missing-supportClaw-extensions';
const PLUGIN_ARCHIVE_ROOT_MARKERS = [
  "package.json",
  "supportClaw.plugin.json",
  ".codex-plugin/plugin.json",
  ".claude-plugin/plugin.json",
  ".cursor-plugin/plugin.json",
];
const MANAGED_NPM_PACK_ARCHIVE_DIR = "_supportClaw-pack-archives";
const MANAGED_NPM_PROJECT_QUARANTINE_DIR = "_supportClaw-quarantined-npm-projects";
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_SUPPORT_CLAW_EXTENSIONS: "missing_supportClaw_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_SUPPORT_CLAW_EXTENSIONS: "empty_supportClaw_extensions",
  INVALID_SUPPORT_CLAW_EXTENSIONS: "invalid_supportClaw_extensions",
...
function validateSupportClawPackageCompatibility(params: {
  pluginId: string;
  currentHostVersion: string;
  packageMetadata?: SupportClawPackageManifest;
  ...
  if (!pluginApiRangeCheck.ok) {
    return {
      ok: false,
      error: `invalid package.json supportClaw.compat.pluginApi: ${pluginApiRangeCheck.error}`,
  ...
  if (pluginApiRange && !satisfiesPluginApiRange(params.currentHostVersion, pluginApiRange)) {
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires plugin API ${pluginApiRange}, but this SupportClaw runtime exposes ${params.currentHostVersion}. Upgrade SupportClaw or install a compatible plugin version and retry.`,
...
function validateSupportClawPackageInstallCompatibility(params: {
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json supportClaw.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${params.pluginId}" requires SupportClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set SUPPORT_CLAW_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires SupportClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade SupportClaw and retry.`,
      code: PLUGIN_INSTALL_ERROR_CODE.INCOMPATIBLE_HOST_VERSION,
    };
  }

  return validateSupportClawPackageCompatibility({
...
function ensureSupportClawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_SUPPORT_CLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json supportClaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_SUPPORT_CLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "invalid") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_SUPPORT_CLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
function compareNpmSemver(a: string, b: string): number {
  const releaseCmp = compareSupportClawReleaseVersions(a, b);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
async function resolveTrustedOfficialPrereleaseResolution(params: {
  ...
  if (!params.spec.name.startsWith("@supportclaw/")) {
    return null;
  }
  ...
  if (!stableVersion) {
    ...
    if (prereleaseVersion && semverVersions.every(isPrereleaseSemverVersion)) {
      if (prereleaseVersion !== params.resolvedPrereleaseVersion) {
        ...
        params.logger.warn?.(
          `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; using newest prerelease ${prereleaseSpec} because this trusted official SupportClaw package has no stable npm versions yet.`,
        );
        return { kind: "prerelease-only", resolution: metadataResult.metadata };
      }
      params.logger.warn?.(
        `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; allowing it because this trusted official SupportClaw package has no stable npm versions yet.`,
      );
  ...
  params.logger.warn?.(
    `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; falling back to stable ${stableSpec} for this trusted official SupportClaw install.`,
  );
...
function validateNpmResolutionCompatibility(params: {
  ...
  return validateSupportClawPackageInstallCompatibility({
    runtime: params.runtime,
    pluginId: params.expectedPluginId ?? params.resolution.name ?? params.parsedSpec.name,
    packageMetadata: params.resolution.packageSupportClaw as SupportClawPackageManifest | undefined,
  });
}
...
async function resolveLatestCompatibleNpmResolution(params: {
  ...
  for (const version of candidates) {
    ...
    if (!compatibilityError) {
      params.logger.warn?.(
        `Resolved ${params.parsedSpec.raw} to ${params.currentResolution.resolvedSpec ?? currentVersion}, but that version is incompatible with this SupportClaw runtime; using newest compatible ${metadataResult.metadata.resolvedSpec ?? spec}.`,
      );
...
async function rollbackManagedNpmPluginInstall(params: {
  ...
  if (params.snapshot) {
    try {
      ...
      await relinkSupportClawPeerDependenciesInManagedNpmRoot({
        npmRoot: params.npmRoot,
  ...
  if (params.packageName !== "supportClaw") {
    try {
      await repairManagedNpmRootSupportClawPeer({
        npmRoot: params.npmRoot,
        timeoutMs: params.timeoutMs,
        logger: params.logger,
      });
      ...
      params.logger.warn?.(
        `Failed to repair managed npm supportClaw peer after rollback: ${String(error)}`,
      );
    }
  }
  try {
...
async function createManagedNpmPluginInstallRollbackSnapshot(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-npm-plugin-rollback-"));
...
async function listManagedNpmRootPackageNames(npmRoot: string): Promise<Set<string>> {
  ...
  for (const entry of entries.toSorted((left, right) => left.name.localeCompare(right.name))) {
    if (entry.name === ".bin" || entry.name === "supportClaw") {
      continue;
    }
...
function resolveRequiredPlatformPackageNames(
  ...
  if (!Array.isArray(raw)) {
    return {
      ok: false,
      error: "package.json supportClaw.install.requiredPlatformPackages must be an array",
    };
  }
  const packageNames = new Set<string>();
  for (const value of raw) {
    if (typeof value !== "string") {
      return {
        ok: false,
        error:
          "package.json supportClaw.install.requiredPlatformPackages must contain only npm package names",
      };
    }
    ...
    if (specError || !parsed || parsed.selectorKind !== "none") {
      return {
        ok: false,
        error: `package.json supportClaw.install.requiredPlatformPackages contains invalid package name: ${value}`,
      };
    }
...
async function installPluginFromManagedNpmRoot(
  ...
  const runManagedNpmInstall = async (
    ...
    if (params.packageName !== "supportClaw") {
      const repairedSupportClawPeer = await repairManagedNpmRootSupportClawPeer({
        npmRoot,
        timeoutMs,
        logger,
      });
      if (repairedSupportClawPeer) {
        logger.info?.(`Repaired stale supportClaw peer dependency in ${npmRoot}`);
      }
    }
    let preInstallRootPackageNames = await listManagedNpmRootPackageNames(npmRoot);
    const managedOverrides = await readSupportClawManagedNpmRootOverrides();
    ...
    if (install.code !== 0 && isManagedNpmProjectCorruptionInstallFailure(install)) {
      ...
      try {
        ...
        return await rollbackFailedManagedNpmInstall({
          ok: false,
          error: `npm install failed with a managed npm project corruption signature, but SupportClaw could not quarantine ${npmRoot} for rebuild: ${String(error)}. Original npm error: ${originalError}`,
        });
      }
      logger.warn?.(
    ...
    if (omittedPlatformPackages.length > 0) {
      ...
      try {
        freshCacheDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-npm-cache-"));
    ...
    if (params.packageName !== "supportClaw") {
      ...
      if (repairedSupportClawPeer) {
        logger.info?.(`Repaired stale supportClaw peer dependency in ${npmRoot} after npm install`);
      }
    }
    try {
      await relinkSupportClawPeerDependenciesInManagedNpmRoot({
      ...
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: `Failed to repair supportClaw peer links after npm install: ${String(error)}`,
      });
    }
    if (installedPackageNeedsSupportClawPeerLinkRepair(installRoot)) {
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: formatUnresolvedSupportClawPeerLinkError(params.packageName),
      });
    }
...
> {
  ...
  try {
    ...
    backupTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-npm-pack-archive-"));
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "supportClaw security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function installBundleFromSourceDir(
  ...
  const packageMetadata = packageManifestResult.manifest
    ? runtime.getPackageManifestMetadata(packageManifestResult.manifest)
    : undefined;
  const compatibilityError = validateSupportClawPackageInstallCompatibility({
    runtime,
    pluginId,
    packageMetadata,
  });
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureSupportClawExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
> {
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid supportClaw.plugin.json: ${ocManifestResult.error}`,
  ...
  const extensionsResult = ensureSupportClawExtensions({
    manifest,
  });
...
async function scanAndLinkInstalledPackage(params: {
  ...
  const peerLinkRepair = await linkSupportClawPeerDependencies({
  ...
  if (peerLinkRepair.skipped > 0) {
    return {
      ok: false,
      error: formatUnresolvedSupportClawPeerLinkError(params.pluginId),
    };
  }
  return null;
}
...
export async function installPluginFromArchive(
  ...
  const result = await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "supportClaw-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(
  ...
  const policyTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-npm-policy-"));
  try {
...
export async function installPluginFromPath(
  ...
  return {
    ...
    error:
      "Plain file plugin installs are not supported. Install a plugin directory or archive that contains supportClaw.plugin.json, or list standalone plugin files in plugins.load.paths.",
  };
}
