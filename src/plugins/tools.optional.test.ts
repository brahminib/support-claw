...
vi.mock("./loader.js", () => ({
  loadSupportClawPlugins: (params: unknown) => loadSupportClawPluginsMock(params),
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadSupportClawPluginsMock.mockReturnValue(registry);
...
function expectLoaderCall(overrides: Record<string, unknown>) {
  void overrides;
  expect(loadSupportClawPluginsMock).not.toHaveBeenCalled();
}
...
function expectLoaderSelectedOnlyPluginIds(expectedPluginIds: readonly string[]) {
  const selectedPluginIds = loadSupportClawPluginsMock.mock.calls.map(
...
describe("resolvePluginTools optional tools", () => {
  ...
  beforeEach(() => {
    ...
    resolveRuntimePluginRegistryMock.mockImplementation((params) =>
      loadSupportClawPluginsMock(params),
    );
  ...
  it("does not load plugin-owned tools whose manifest metadata has no available signal", () => {
    ...
    loadSupportClawPluginsMock.mockImplementation((params) =>
  ...
  it("loads manifest-gated tools when a named account supplies required config", () => {
    ...
    loadSupportClawPluginsMock.mockReturnValue(
      createToolRegistry([
        {
          pluginId: "feishu",
          optional: false,
  ...
  it("does not reuse a partial active registry for wildcard-selected plugin tools", () => {
    ...
    loadSupportClawPluginsMock.mockReturnValue(fullRegistry);
    ...
    const loaderParams = mockCallParams(loadSupportClawPluginsMock) as {
      activate?: unknown;
      cache?: unknown;
  ...
  it("uses the fresh cold-loaded registry for diagnostics when partial active registries remain incomplete", () => {
    ...
    loadSupportClawPluginsMock.mockReturnValue(freshRegistry);
  ...
  it("does not reuse a pinned gateway registry for manifest-unavailable tools", () => {
    ...
    loadSupportClawPluginsMock.mockReturnValue({ tools: [], diagnostics: [] });
    ...
    const config = {
      ...
      secrets: {
        providers: {
          vault: {
            source: "file",
            path: "/tmp/supportClaw-secrets.json",
            mode: "json",
          },
        },
      },
  ...
  it("does not cold-load unrelated manifest-optional plugins when alsoAllow opts into one optional tool", () => {
    ...
    loadSupportClawPluginsMock.mockReturnValue(createToolRegistry([explicitOptionalEntry]));
    installToolManifestSnapshots({
      config,
      plugins: [
        {
  ...
  it.each([
    {
      ...
      params: {
        env: { SUPPORT_CLAW_HOME: "/srv/supportClaw-home" } as NodeJS.ProcessEnv,
        toolAllowlist: ["optional_tool"],
      },
      expectedLoaderCall: {
        env: { SUPPORT_CLAW_HOME: "/srv/supportClaw-home" },
      },
    },
    {
  ...
  it("loads a standalone registry when cached runtime registries lack matching tool entries", () => {
    ...
    resolveRuntimePluginRegistryMock.mockReturnValue(undefined);
    loadSupportClawPluginsMock.mockReturnValue(loadedRegistry);
  ...
  it("adds enabled non-startup tool plugins to the active tool runtime scope", () => {
    ...
    setActivePluginRegistry(activeRegistry as never, "gateway-startup", "gateway-bindable", "/tmp");
    resolveRuntimePluginRegistryMock.mockReturnValue(activeRegistry);
    loadSupportClawPluginsMock.mockReturnValue(createToolRegistry([]));
