  ...
  return listExtensionPackagePaths(extensionsDir)
    .filter((packagePath) => {
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        supportClaw?: SupportClawPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.supportClaw?.extensions).length > 0;
    })
    .map(
      (packagePath) =>
        JSON.parse(
          fs.readFileSync(path.join(path.dirname(packagePath), "supportClaw.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
