...
function createManifestPlugin(id: string): PluginManifestRecord {
  return {
    ...
    rootDir: `/tmp/${id}`,
    source: "bundled",
    manifestPath: `/tmp/${id}/supportClaw.plugin.json`,
...
describe("resolvePluginDiscoveryProvidersRuntime", () => {
  ...
  it("clears bundled dist discovery chunks from the dist root", () => {
    ...
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      ...
      manifestRegistry: {
        plugins: [
          {
            ...createManifestPlugin("deepseek"),
            rootDir: "/tmp/supportClaw/dist/extensions/deepseek",
            manifestPath: "/tmp/supportClaw/dist/extensions/deepseek/supportClaw.plugin.json",
            providerDiscoverySource: "/tmp/supportClaw/dist/extensions/deepseek/provider-discovery.js",
          },
        ],
        diagnostics: [],
      },
    ...
    expect(mocks.clearNativeRequireJavaScriptModuleCache).toHaveBeenCalledWith(
      "/tmp/supportClaw/dist/extensions/deepseek/provider-discovery.js",
      { dependencyRoot: "/tmp/supportClaw/dist" },
    );
  });
