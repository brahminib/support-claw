...
function collectExplicitEffectivePluginIds(config: SupportClawConfig): string[] {
...
function collectSelectedContextEnginePluginIds(config: SupportClawConfig): string[] {
  const plugins = normalizePluginsConfig(config.plugins);
  if (!plugins.enabled) {
    return [];
  }
