...
const resolveSupportClawPackageRootSyncMock = vi.fn();
...
vi.mock("../infra/supportClaw-root.js", () => ({
  resolveSupportClawPackageRootSync: (...args: unknown[]) =>
    resolveSupportClawPackageRootSyncMock(...args),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("supportClaw-plugin-install-npm-spec");
...
function writeInstalledNpmPlugin(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      supportClaw: params.supportClaw ?? { extensions: ["./dist/index.js"] },
      ...(params.dependency
  ...
  fs.writeFileSync(
    path.join(pluginDir, "supportClaw.plugin.json"),
    JSON.stringify({
...
type MockNpmPackage = {
  ...
  materializesRootSupportClaw?: boolean;
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootSupportClaw) {
      lockPackages["node_modules/supportClaw"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalSupportClawPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      ...
      fs.rmSync(path.join(packageNodeModulesDir, "supportClaw"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (viewPackage) {
        return successfulSpawn(
          JSON.stringify({
            ...
            ...(viewPackage.supportClaw ? { supportClaw: viewPackage.supportClaw } : {}),
          }),
        );
      }
      ...
      if (isManagedNpmInstallCommand(argv)) {
        ...
        prunePluginLocalSupportClawPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          if (packageName === "supportClaw") {
            const supportClawRoot = path.join(npmRoot, "node_modules", "supportClaw");
            fs.mkdirSync(supportClawRoot, { recursive: true });
            fs.writeFileSync(
              path.join(supportClawRoot, "package.json"),
              JSON.stringify({ name: "supportClaw", version: "0.0.0-test" }),
              "utf8",
            );
            continue;
          }
          ...
          if (pkg.materializesRootSupportClaw) {
            ...
            fs.writeFileSync(
              path.join(supportClawRoot, "package.json"),
              JSON.stringify({ name: "supportClaw", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "supportClaw") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "supportClaw"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  resolveSupportClawPackageRootSyncMock.mockReset();
  const hostRoot = suiteTempRootTracker.makeTempDir();
  fs.writeFileSync(
    path.join(hostRoot, "package.json"),
    `${JSON.stringify({ name: "supportClaw", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
  resolveSupportClawPackageRootSyncMock.mockReturnValue(hostRoot);
  vi.unstubAllEnvs();
  process.env.NPM_CONFIG_GLOBALCONFIG = npmGlobalConfigPath;
});
...
beforeAll(async () => {
  ...
  const archivePath = path.join(stateDir, "supportClaw-pack-demo-1.2.3.tgz");
  fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

  mockNpmViewAndInstallMany([
    {
      packageName: "@supportclaw/pack-demo",
      version: "1.2.3",
      pluginId: "pack-demo",
      npmRoot,
    ...
    {
      spec: "@supportclaw/voice-call@0.0.1",
      packageName: "@supportclaw/voice-call",
      version: "0.0.1",
      pluginId: "voice-call",
      npmRoot,
    },
  ...
  const dependencySpec = managedManifest.dependencies?.["@supportclaw/pack-demo"];
...
describe("installPluginFromNpmSpec", () => {
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@supportclaw/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@supportclaw/pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("supportClaw-pack-demo-1.2.3.tgz");
    expectNpmInstallIntoProject({
      calls,
      npmRoot,
      packageName: "@supportclaw/pack-demo",
    });
    expect(dependencySpec).toMatch(/^file:\.\/_supportClaw-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_supportClaw-pack-archives"))).toBe(false);
  ...
  it("updates staged npm pack archives when dangerous-looking code is present", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@supportclaw/pack-demo";
    const archiveV1Path = path.join(stateDir, "supportClaw-pack-demo-1.0.0.tgz");
    const archiveV2Path = path.join(stateDir, "supportClaw-pack-demo-2.0.0.tgz");
  ...
  it("installs staged npm pack archives with dangerous-looking code", async () => {
    ...
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@supportclaw/pack-demo";
    const archivePath = path.join(stateDir, "supportClaw-pack-demo-2.0.0.tgz");
  ...
  it("installs npm plugins into .supportClaw/npm", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@supportclaw/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@supportclaw/voice-call@0.0.1");
  ...
  it("repairs omitted current-platform packages with a fresh npm cache", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@supportclaw/codex-fixture";
    ...
    expect(repairCacheDir).toContain("supportClaw-npm-cache-");
  ...
  it("quarantines and rebuilds a corrupt managed npm project after npm from-argument failures", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@supportclaw/voice-call";
    const warnings: string[] = [];
    const npmProjectRoot = resolvePluginNpmProjectDir({ npmDir: npmRoot, packageName });
    ...
    const quarantineParent = path.join(npmProjectRoot, "_supportClaw-quarantined-npm-projects");
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed supportClaw peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstallMany([
        {
          ...
          npmRoot,
          peerDependencies: { supportClaw: "^2026.0.0" },
        },
        {
      ...
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "supportClaw")).isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/supportClaw");
      }
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "supportClaw")).isSymbolicLink(),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their supportClaw peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];

    resolveSupportClawPackageRootSyncMock.mockReturnValue(null);
    mockNpmViewAndInstall({
      spec: "@supportclaw/codex@2026.5.7",
      packageName: "@supportclaw/codex",
      version: "2026.5.7",
      pluginId: "@supportclaw/codex",
      npmRoot,
      peerDependencies: { supportClaw: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@supportclaw/codex");
    expect(result.error).toContain("plugin-local node_modules/supportClaw link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate supportClaw package root")),
    ).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@supportclaw/codex"))).toBe(false);
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@supportclaw/codex",
    });
  ...
  it("rejects exact npm plugins whose package compatibility requires a newer host", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    vi.stubEnv("SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION", "2026.5.10-beta.1");

    mockNpmViewAndInstall({
      spec: "@supportclaw/whatsapp@2026.5.27",
      packageName: "@supportclaw/whatsapp",
      version: "2026.5.27",
      pluginId: "whatsapp",
      npmRoot,
      peerDependencies: { supportClaw: ">=2026.5.27" },
      supportClaw: {
        extensions: ["./dist/index.js"],
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@supportclaw", "whatsapp"))).toBe(false);
    expect(fs.existsSync(path.join(npmRoot, "package.json"))).toBe(false);
    expect(
  ...
  it("installs the newest compatible npm version for unpinned plugins", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@supportclaw/whatsapp@2026.5.26",
        packageName: "@supportclaw/whatsapp",
        version: "2026.5.26",
        pluginId: "whatsapp",
        npmRoot,
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@supportclaw/whatsapp@2026.5.26");
    expect(result.npmResolution?.version).toBe("2026.5.26");
    expect(warnings.join("\n")).toContain("using newest compatible @supportclaw/whatsapp@2026.5.26");
    expect(
      JSON.parse(
        fs.readFileSync(
          path.join(resolveTestPluginPackageDir(npmRoot, "@supportclaw/whatsapp"), "package.json"),
          "utf8",
        ),
      ).version,
    ).toBe("2026.5.26");
  ...
  it("preserves an existing npm plugin by resolving update metadata to a compatible version", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        private: true,
        dependencies: {
          "@supportclaw/whatsapp": "2026.5.26",
        },
      }),
      "utf8",
    );
    ...
    expect(managedManifest.dependencies?.["@supportclaw/whatsapp"]).toBe("2026.5.26");
    expect(
      runCommandWithTimeoutMock.mock.calls.some(([argv]) => isManagedNpmInstallCommand(argv)),
    ).toBe(true);
  });
  ...
  it("resolves incompatible prerelease tags to a compatible prerelease version", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@supportclaw/msteams@2026.5.28-beta.3",
        packageName: "@supportclaw/msteams",
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@supportclaw/msteams@2026.5.28-beta.3");
    expect(result.npmResolution?.version).toBe("2026.5.28-beta.3");
    expect(warnings.join("\n")).toContain(
      "using newest compatible @supportclaw/msteams@2026.5.28-beta.3",
    );
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@supportclaw/msteams",
    ...
    expect(managedManifest.dependencies?.["@supportclaw/msteams"]).toBe("2026.5.28-beta.3");
  });
  ...
  it.runIf(process.platform !== "win32")(
    "repairs root supportClaw materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstall({
        ...
        npmRoot,
        peerDependencies: { supportClaw: "^2026.0.0" },
        materializesRootSupportClaw: true,
      });
      ...
      expect(fs.existsSync(path.join(npmProjectRoot, "node_modules", "supportClaw"))).toBe(false);
      const lockfile = JSON.parse(
        fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
      ) as {
        packages?: Record<string, unknown>;
      };
      expect(lockfile.packages?.["node_modules/supportClaw"]).toBeUndefined();
      expect(
        fs.lstatSync(path.join(requiredPeerPluginDir, "node_modules", "supportClaw")).isSymbolicLink(),
      ).toBe(true);
    },
  );

  it("repairs stale managed supportClaw root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@supportclaw/discord",
    });
    fs.mkdirSync(path.join(npmProjectRoot, "node_modules", "supportClaw"), { recursive: true });
    fs.writeFileSync(
      path.join(npmProjectRoot, "package.json"),
      JSON.stringify(
        {
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/supportClaw": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/supportClaw/-/supportClaw-2026.5.4.tgz",
            },
          },
          dependencies: {
            supportClaw: {
    ...
    fs.writeFileSync(
      path.join(npmProjectRoot, "node_modules", "supportClaw", "package.json"),
      JSON.stringify({
        name: "supportClaw",
        version: "2026.5.4",
      }),
    ...
    mockNpmViewAndInstall({
      spec: "@supportclaw/discord@beta",
      packageName: "@supportclaw/discord",
      version: "2026.5.5-beta.1",
      pluginId: "discord",
      npmRoot,
      peerDependencies: { supportClaw: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("supportClaw");
    expect(manifest.dependencies?.["@supportclaw/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
    ...
    expect(lockfile.packages?.["node_modules/supportClaw"]).toBeUndefined();
    expect(lockfile.dependencies?.supportClaw).toBeUndefined();
  });

  it("preserves the active host supportClaw runtime package during npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const hostPackageRoot = path.join(npmRoot, "node_modules", "supportClaw");
    ...
    resolveSupportClawPackageRootSyncMock.mockReturnValue(hostPackageRoot);
    mockNpmViewAndInstall({
      spec: "@xdarkicex/supportClaw-memory-libravdb@1.4.69",
      packageName: "@xdarkicex/supportClaw-memory-libravdb",
      version: "1.4.69",
    ...
    expect(baseManifest.dependencies?.supportClaw).toBe("2026.5.12-beta.6");
    expect(baseManifest.dependencies?.["@xdarkicex/supportClaw-memory-libravdb"]).toBeUndefined();
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@xdarkicex/supportClaw-memory-libravdb",
    });
    ...
    expect(projectManifest.dependencies?.["@xdarkicex/supportClaw-memory-libravdb"]).toBe("1.4.69");
    expect(fs.existsSync(hostPackageRoot)).toBe(true);
    expect(result.targetDir).toBe(
      resolveTestPluginPackageDir(npmRoot, "@xdarkicex/supportClaw-memory-libravdb"),
    );
    expect(
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@supportclaw/voice-call@0.0.1"))) {
          return successfulSpawn(
            JSON.stringify({
        ...
        if (argv[0] === "npm" && argv[1] === "uninstall") {
          if (!(argv as string[]).includes("--legacy-peer-deps")) {
            fs.mkdirSync(path.join(options?.cwd ?? npmRoot, "node_modules", "supportClaw"), {
              recursive: true,
            });
          }
          return successfulSpawn("");
    ...
    await expect(
      fs.promises.access(path.join(npmProjectRoot, "node_modules", "supportClaw")),
    ).rejects.toHaveProperty("code", "ENOENT");
  });

  it("does not fail rollback snapshots on plugin-local supportClaw peer symlinks", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@supportclaw/codex",
    ...
    writeNpmRootPackageLock({
      npmRoot: npmProjectRoot,
      dependencies: { "@supportclaw/codex": "0.0.1" },
      packages: [
        {
          packageName: "@supportclaw/codex",
          version: "0.0.1",
    ...
    const installedDir = writeInstalledNpmPlugin({
      ...
      packageName: "@supportclaw/codex",
      version: "0.0.1",
      peerDependencies: { supportClaw: "*" },
    });
    const peerLink = path.join(installedDir, "node_modules", "supportClaw");
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@supportclaw/codex@0.0.2"))) {
          return successfulSpawn(
            JSON.stringify({
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (isManagedNpmInstallCommand(argv)) {
          ...
          const manifest = JSON.parse(
            fs.readFileSync(path.join(npmProjectRoot, "package.json"), "utf8"),
          ) as { overrides?: Record<string, unknown>; supportClaw?: { managedOverrides?: string[] } };
          if (installAttempts === 1) {
            expect(manifest.overrides?.["node-domexception"]).toBe(
              "npm:@nolyfill/domexception@1.0.28",
            );
            expect(manifest.supportClaw?.managedOverrides).toEqual([
              "axios",
              "nested",
              "node-domexception",
            ]);
          ...
          expect(manifest.supportClaw?.managedOverrides).toEqual(["axios", "nested"]);
        }
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official SupportClaw package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = resolveTestPluginPackageDir(npmRoot, "@supportclaw/voice-call");
  ...
  it("allows duplicate npm installs in update mode", async () => {
    ...
    mockNpmViewAndInstall({
      spec: "@supportclaw/voice-call@0.0.2",
      packageName: "@supportclaw/voice-call",
      version: "0.0.2",
      pluginId: "voice-call",
      npmRoot,
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@supportclaw/whatsapp@0.0.1",
        packageName: "@supportclaw/whatsapp",
        version: "0.0.1",
        pluginId: "whatsapp",
        npmRoot,
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@supportclaw/voice-call"))).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@supportclaw/whatsapp"))).toBe(true);
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@supportclaw/voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@supportclaw/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @supportclaw/voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@supportclaw/voice-call@2026.5.3-1");
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@supportclaw/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @supportclaw/voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@supportclaw/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoProject({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
      packageName: "@supportclaw/voice-call",
