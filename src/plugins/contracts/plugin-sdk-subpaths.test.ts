...
import * as commandAuthSdk from "supportClaw/plugin-sdk/command-auth";
import type {
  ChannelMessageActionContext as CoreChannelMessageActionContext,
  SupportClawPluginApi as CoreSupportClawPluginApi,
  PluginRuntime as CorePluginRuntime,
} from "supportClaw/plugin-sdk/core";
import * as providerEntrySdk from "supportClaw/plugin-sdk/provider-entry";
import type { GetReplyOptions as ReplyRuntimeGetReplyOptions } from "supportClaw/plugin-sdk/reply-runtime";
import * as zalouserSdk from "supportClaw/plugin-sdk/zalouser";
...
import type {
  ...
  SupportClawPluginApi as SharedSupportClawPluginApi,
...
import type { SupportClawPluginApi } from "../types.js";
...
const BROWSER_HELPER_EXPORT_PARITY_CONTRACTS: readonly BrowserHelperExportParityContract[] = [
  ...
  },
  {
    ...
    expectedExports: [
      ...
      "DEFAULT_SUPPORT_CLAW_BROWSER_ENABLED",
...
describe("plugin-sdk subpath exports", () => {
  ...
  beforeAll(() => {
    deprecatedChannelRuntimeMatches = findRepoFilesContaining({
      ...
      pattern:
        /(?:from\s+|import\s+(?:type\s+)?|import\s*\(\s*)["']supportClaw\/plugin-sdk\/channel-runtime(?=["'])/u,
      exclude: [
  ...
  it("keeps shared plugin-sdk types aligned", () => {
    ...
    expectTypeOf<CoreSupportClawPluginApi>().toMatchTypeOf<SupportClawPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<PluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<ChannelMessageActionContext>();
    expectTypeOf<CoreSupportClawPluginApi>().toMatchTypeOf<SharedSupportClawPluginApi>();
  ...
  it("keeps runtime entry subpaths importable", async () => {
    const coreSdk = await importResolvedPluginSdkSubpath("supportClaw/plugin-sdk/core");
    const channelActionsSdk = await importResolvedPluginSdkSubpath(
      "supportClaw/plugin-sdk/channel-actions",
    );
    const globalSingletonSdk = await importResolvedPluginSdkSubpath(
      "supportClaw/plugin-sdk/global-singleton",
    );
    const pluginEntrySdk = await importResolvedPluginSdkSubpath("supportClaw/plugin-sdk/plugin-entry");
    const channelLifecycleSdk = await importResolvedPluginSdkSubpath(
      "supportClaw/plugin-sdk/channel-lifecycle",
    );
    const channelPairingSdk = await importResolvedPluginSdkSubpath(
      "supportClaw/plugin-sdk/channel-pairing",
    );
    const channelReplyPipelineSdk = await importResolvedPluginSdkSubpath(
      "supportClaw/plugin-sdk/channel-reply-pipeline",
    );
    const representativeModules = [];
    for (const id of representativeRuntimeSmokeSubpaths) {
      representativeModules.push(await importResolvedPluginSdkSubpath(`supportClaw/plugin-sdk/${id}`));
    }
  ...
  it("keeps repeated silent-token semantics visible through the reply-chunking subpath", async () => {
    const replyChunkingSdk = await importResolvedPluginSdkSubpath(
      "supportClaw/plugin-sdk/reply-chunking",
    );
