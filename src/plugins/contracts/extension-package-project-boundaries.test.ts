...
const MEMORY_HOST_SDK_ALLOWED_CORE_BRIDGE_FILES = [
  "packages/memory-host-sdk/src/host/supportClaw-runtime-auth.ts",
  "packages/memory-host-sdk/src/host/supportClaw-runtime-network.ts",
  "packages/memory-host-sdk/src/host/supportClaw-runtime.ts",
] as const;
const MEMORY_HOST_SDK_RUNTIME_ADAPTER_FILES = [
  "packages/memory-host-sdk/src/host/supportClaw-runtime-agent.ts",
  "packages/memory-host-sdk/src/host/supportClaw-runtime-cli.ts",
  "packages/memory-host-sdk/src/host/supportClaw-runtime-config.ts",
  "packages/memory-host-sdk/src/host/supportClaw-runtime-io.ts",
  "packages/memory-host-sdk/src/host/supportClaw-runtime-memory.ts",
  "packages/memory-host-sdk/src/host/supportClaw-runtime-session.ts",
] as const;
...
function collectSupportClawRuntimeDirectImportFiles(relativeDir: string): string[] {
  return collectCodeFiles(relativeDir).filter((file) => {
    const source = fs.readFileSync(resolve(REPO_ROOT, file), "utf8");
    return source.includes('"./supportClaw-runtime.js"');
  });
}
...
describe("opt-in extension package boundaries", () => {
  ...
  it("keeps every opt-in extension rooted inside its package and on the package sdk", () => {
    ...
    for (const extensionName of optInExtensions) {
      ...
      expect(packageJson.devDependencies?.["@supportclaw/plugin-sdk"]).toBe("workspace:*");
    }
  });
  ...
  it("keeps plugin-sdk package types generated from the package build, not a hand-maintained types bridge", () => {
    ...
    expect(packageJson.name).toBe("@supportclaw/plugin-sdk");
  ...
  it("keeps memory-host-sdk as a private package-owned contract surface", () => {
    ...
    expect(packageJson.name).toBe("@supportclaw/memory-host-sdk");
    ...
    expect(collectSupportClawRuntimeDirectImportFiles("packages/memory-host-sdk/src")).toEqual([
