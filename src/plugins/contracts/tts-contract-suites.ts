...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "supportClaw/plugin-sdk/speech-core";
import { withEnv, withEnvAsync } from "supportClaw/plugin-sdk/test-env";
...
type TtsRuntimeModule = typeof import("supportClaw/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("supportClaw/plugin-sdk/speech-core");
...
const speechCoreRuntimeApiModuleId = resolveWorkspacePackagePublicModuleUrl({
  packageName: "@supportclaw/speech-core",
...
let completeSimple: typeof import("supportClaw/plugin-sdk/llm").completeSimple;
...
vi.mock("supportClaw/plugin-sdk/llm", () => {
...
function asLegacyTtsConfig(value: unknown): SupportClawConfig {
  return value as SupportClawConfig;
}

function asLegacySupportClawConfig(value: Record<string, unknown>): SupportClawConfig {
  return value as unknown as SupportClawConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): SupportClawConfig {
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("supportClaw/plugin-sdk/speech-core");
...
function createResolvedSummarizationConfig(cfg: SupportClawConfig): ResolvedTtsConfig {
  const rawConfig =
...
async function setupSummarizationMocks() {
  ...
  ({ completeSimple } = await import("supportClaw/plugin-sdk/llm"));
    ...
    describe("getTtsProvider", () => {
      ...
      it("passes cfg into auto-selection so model-provider Google keys can configure TTS", () => {
        withIsolatedSpeechProviderEnv(
          ...
          () => {
            const cfg = asLegacySupportClawConfig({
              agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
              models: {
                providers: {
                  google: {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: SupportClawConfig = asLegacySupportClawConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.SUPPORT_CLAW_TTS_PREFS;
      process.env.SUPPORT_CLAW_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.SUPPORT_CLAW_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: SupportClawConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
