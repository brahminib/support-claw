...
const diagnosticEventsStateKey = Symbol.for("supportClaw.diagnosticEvents.state.v1");
...
function loadRootAliasWithStubs(options?: {
  ...
  const localRequire = ((id: string) => {
    ...
    if (id === "node:os") {
      return {
        tmpdir: () =>
          context.process.env.TMPDIR ?? options?.defaultTmpDir ?? "/tmp/supportClaw-root-alias-test",
      };
    }
    if (id === "jiti") {
      return {
...
function ensureDiagnosticEventsStateFixture(
  ...
  const state = vm.runInNewContext(
    `({
      marker: Symbol.for("supportClaw.diagnosticEvents.state.v1"),
      enabled: true,
      seq: 0,
      listeners: new Set(),
      dispatchDepth: 0,
...
describe("plugin-sdk root alias", () => {
  ...
  it("loads legacy root exports on demand and preserves reflection", () => {
    ...
    expect(lazyModule.createJitiOptions.at(-1)?.fsCache).toBe(
      path.join("/tmp/supportClaw-root-alias-test", "jiti", "supportClaw", "3.4.5", "12345-678"),
    );
  ...
  it("preserves jiti's tmpdir guard when root-alias TMPDIR resolves to cwd", () => {
    const lazyModule = loadRootAliasWithStubs({
      cwd: "/tmp/supportClaw-root-alias-cwd",
      defaultTmpDir: "/tmp/supportClaw-root-alias-fallback",
      env: { TMPDIR: "/tmp/supportClaw-root-alias-cwd" },
      packageVersion: "3.4.5",
    });

    expect("slowHelper" in lazyModule.moduleExports).toBe(true);
    expect(lazyModule.createJitiOptions.at(-1)?.fsCache).toBe(
      path.join("/tmp/supportClaw-root-alias-fallback", "jiti", "supportClaw", "3.4.5", "12345-678"),
    );
  });
  ...
  it("builds scoped and unscoped plugin-sdk aliases for jiti loads", () => {
    ...
    expect(aliasMap["supportClaw/plugin-sdk"]).toBe(rootAliasPath);
    expect(aliasMap["@supportclaw/plugin-sdk"]).toBe(rootAliasPath);
    expect(aliasMap["supportClaw/plugin-sdk/group-access"]).toContain(
  ...
  it("keeps agent-core package imports on the source graph when llm-core dist is missing", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["@supportclaw/llm-core"]).toBe(sourceLlmCorePath);
  });
  ...
  it("keeps bootstrap plugin-sdk aliases deterministic and ignores unsafe subpaths", () => {
    ...
    expect(aliasKeys).toEqual([
      "supportClaw/plugin-sdk/alpha",
      "@supportclaw/plugin-sdk/alpha",
      "supportClaw/plugin-sdk/group-access",
      "@supportclaw/plugin-sdk/group-access",
      "supportClaw/plugin-sdk/zeta",
      "@supportclaw/plugin-sdk/zeta",
      "@supportclaw/llm-core",
      "@supportclaw/llm-core/diagnostics",
      "@supportclaw/llm-core/event-stream",
      "@supportclaw/llm-core/types",
      "@supportclaw/llm-core/validation",
  ...
  it("ignores unsafe private local-only plugin-sdk subpaths in the CJS root alias", () => {
    ...
    const lazyModule = loadRootAliasWithStubs({
      env: { SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI: "1" },
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["supportClaw/plugin-sdk/qa-lab"]).toBe(qaLabPath);
    expect(aliasMap["@supportclaw/plugin-sdk/qa-lab"]).toBe(qaLabPath);
    expect(aliasMap).not.toHaveProperty("supportClaw/plugin-sdk/../escape");
    expect(aliasMap).not.toHaveProperty("supportClaw/plugin-sdk/nested/path");
    expect(aliasMap).not.toHaveProperty("supportClaw/plugin-sdk/ssrf-runtime-internal");
    expect(aliasMap).not.toHaveProperty("@supportclaw/plugin-sdk/ssrf-runtime-internal");
  });
  ...
  it("keeps non-QA private local-only plugin-sdk subpaths out of the CJS root alias", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap).not.toHaveProperty("supportClaw/plugin-sdk/codex-mcp-projection");
    expect(aliasMap).not.toHaveProperty("@supportclaw/plugin-sdk/codex-mcp-projection");
    expect(aliasMap).not.toHaveProperty("supportClaw/plugin-sdk/qa-runtime");
  });
  ...
  it("builds source plugin-sdk subpath aliases through the wider source extension family", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["supportClaw/plugin-sdk/channel-runtime"]).toBe(
  ...
  it("bridges diagnostic listeners through shared process state when the lazy module is isolated", () => {
    ...
    const state = lazyModule.globalContext[Symbol.for("supportClaw.diagnosticEvents.state.v1")] as {
      listeners: Set<(event: { type: string }, metadata: { trusted: boolean }) => void>;
    };
