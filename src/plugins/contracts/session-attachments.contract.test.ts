...
import { FILE_TYPE_SNIFF_MAX_BYTES } from "@supportclaw/media-core/mime";
import {
...
async function withSessionStore(
  ...
  const stateDir = await fs.mkdtemp(
    path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-session-attachments-"),
  );
...
describe("plugin session attachments", () => {
  afterEach(() => {
    ...
    delete (globalThis as { proofAttachmentApi?: SupportClawPluginApi }).proofAttachmentApi;
  ...
  it("wires sendSessionAttachment through the plugin API with stale-registry protection", async () => {
    await withSessionStore(async ({ storePath, filePath }) => {
      ...
      let capturedApi: SupportClawPluginApi | undefined;
      registerTestPlugin({
        registry,
        config,
