...
describe("plugin session actions", () => {
  ...
  it("emits plugin-attributed agent events through the plugin API", () => {
    ...
    const { config, registry } = createPluginRegistryFixture();
    let bundledApi: SupportClawPluginApi | undefined;
    let workspaceApi: SupportClawPluginApi | undefined;
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("blocks agent events from stale and non-activating plugin API closures", () => {
    ...
    try {
      ...
      let neverActiveApi: SupportClawPluginApi | undefined;
      registerTestPlugin({
      ...
      let inactiveApi: SupportClawPluginApi | undefined;
      registerTestPlugin({
