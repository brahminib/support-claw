...
function makePluginDir() {
  return makeTrackedTempDir("supportClaw-manifest-model-catalog", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "supportClaw.plugin.json"), JSON.stringify(manifest), "utf8");
}
