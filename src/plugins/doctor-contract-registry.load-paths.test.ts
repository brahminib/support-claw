...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "supportClaw-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): SupportClawConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: SupportClawConfig, pluginId: string): Record<string, unknown> {
