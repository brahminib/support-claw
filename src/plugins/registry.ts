...
import { uniqueValues } from "@supportclaw/normalization-core/string-normalization";
import {
...
import type {
  ...
  SupportClawPluginChannelRegistration,
  SupportClawPluginCliCommandDescriptor,
  SupportClawPluginCliRegistrar,
  SupportClawPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  SupportClawPluginGatewayRuntimeScopeSurface,
  SupportClawGatewayDiscoveryService,
  SupportClawPluginHostedMediaResolver,
  SupportClawPluginHttpRouteParams,
  SupportClawPluginHookOptions,
  SupportClawPluginNodeHostCommand,
  SupportClawPluginNodeInvokePolicy,
  SupportClawPluginReloadRegistration,
  SupportClawPluginSecurityAuditCollector,
...
function isOfficialCodexPluginRecord(
  ...
  if (record.packageName === "@supportclaw/codex") {
    return true;
  }
  const sourcePath = path
    ...
    .split(path.sep)
    .join("/");
  return sourcePath.includes("/node_modules/@supportclaw/codex");
}
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("supportClaw.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<SupportClawPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
  ...
  const registerAgentToolResultMiddleware = (
    record: PluginRecord,
    handler: Parameters<SupportClawPluginApi["registerAgentToolResultMiddleware"]>[0],
    options: Parameters<SupportClawPluginApi["registerAgentToolResultMiddleware"]>[1],
  ) => {
  ...
  const registerTool = (
    ...
    const factory: SupportClawPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: SupportClawPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: SupportClawPluginHookOptions | undefined,
    config: SupportClawPluginApi["config"],
  ...
  const registerHttpRoute = (record: PluginRecord, params: SupportClawPluginHttpRouteParams) => {
  ...
  const registerHostedMediaResolver = (
    record: PluginRecord,
    resolver: SupportClawPluginHostedMediaResolver,
  ) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: SupportClawPluginChannelRegistration | ChannelPlugin,
    ...
    const normalized =
      typeof (registration as SupportClawPluginChannelRegistration).plugin === "object"
        ? (registration as SupportClawPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    const descriptors = (opts?.descriptors ?? [])
      ...
      .filter(
        (descriptor): descriptor is SupportClawPluginCliCommandDescriptor => descriptor !== null,
      );
    const commands = [
      ...(opts?.commands ?? []),
  ...
  const registerReload = (record: PluginRecord, registration: SupportClawPluginReloadRegistration) => {
    const normalized: SupportClawPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: SupportClawPluginNodeHostCommand,
  ) => {
  ...
  const registerNodeInvokePolicy = (
    record: PluginRecord,
    policy: SupportClawPluginNodeInvokePolicy,
  ...
  const registerSecurityAuditCollector = (
    record: PluginRecord,
    collector: SupportClawPluginSecurityAuditCollector,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: SupportClawPluginService) => {
  ...
  const registerGatewayDiscoveryService = (
    record: PluginRecord,
    service: SupportClawGatewayDiscoveryService,
  ) => {
    const id = service.id.trim();
    if (!id) {
      return;
  ...
  const registerCommand = (record: PluginRecord, command: SupportClawPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationCapabilities.capabilityHandlers
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<SupportClawPluginApi["registerCompactionProvider"]>[0],
              ) => {
                const id = normalizeOptionalString(
                  (
                    provider as Partial<
                      Parameters<SupportClawPluginApi["registerCompactionProvider"]>[0]
                    > | null
                  )?.id,
                );
                if (!id) {
              ...
              enqueueNextTurnInjection: (injection) => {
                ...
                return enqueuePluginNextTurnInjection({
                  cfg: registryParams.runtime.config.current() as SupportClawConfig,
                  pluginId: record.id,
                  pluginName: record.name,
                  injection,
                });
              ...
              sendSessionAttachment: async (attachment) => {
                ...
                try {
                  ...
                  const runtimeConfig =
                    (registryParams.runtime.config?.current?.() as SupportClawConfig | undefined) ??
                    params.config;
