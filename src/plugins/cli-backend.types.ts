...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before SupportClaw consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether SupportClaw should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume SupportClaw's bundle
   ...
   *
   * Use this when the backend should consume a canonical SupportClaw auth profile
   ...
   * `profile-only` treats the selected SupportClaw auth profile as the sole auth
   ...
   * as static config, such as mapping SupportClaw thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside SupportClaw's tool
