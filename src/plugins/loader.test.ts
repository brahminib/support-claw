...
import {
  ...
  loadSupportClawPluginCliRegistry,
  loadSupportClawPlugins,
...
import { ensureSupportClawPluginSdkAlias } from "./plugin-sdk-dist-alias.js";
...
function updatePluginManifest(plugin: Pick<TempPlugin, "dir">, patch: Record<string, unknown>) {
  const manifestPath = path.join(plugin.dir, "supportClaw.plugin.json");
...
function setupBundledDreamingMemoryPlugins(params?: {
  ...
  fs.writeFileSync(
    path.join(memoryCoreDir, "supportClaw.plugin.json"),
    JSON.stringify(
  ...
  fs.writeFileSync(
    path.join(selectedMemoryDir, "supportClaw.plugin.json"),
    JSON.stringify(
      {
        id: selectedId,
  ...
  process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR = bundledDir;
...
function writeBundledPlugin(params: {
  ...
  delete process.env.SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS;
  process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR = bundledDir;
  return { bundledDir, plugin };
}

function makeSupportClawDevSourceRoot() {
  const root = makeTempDir();
  fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "supportClaw" }), "utf-8");
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".supportClaw", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ SUPPORT_CLAW_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadSupportClawPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          supportClaw: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadSupportClawPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadSupportClawPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadSupportClawPlugins>[0], "cache" | "config">,
) {
  return loadSupportClawPlugins({
    cache: false,
    ...options,
...
function resolveLoadedPluginSource(
  registry: ReturnType<typeof loadSupportClawPlugins>,
  pluginId: string,
) {
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadSupportClawPlugins>;
  loadSecond: () => ReturnType<typeof loadSupportClawPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadSupportClawPlugins>;
  loadVariant: () => ReturnType<typeof loadSupportClawPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const supportClawHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(supportClawHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...process.env,
    SUPPORT_CLAW_HOME: supportClawHome,
    HOME: ignoredHome,
    SUPPORT_CLAW_STATE_DIR: stateDir,
    SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
describe("loadSupportClawPlugins", () => {
  ...
  it("can load scoped plugins from a supplied manifest registry without rereading manifests", () => {
    ...
    fs.rmSync(path.join(plugin.dir, "supportClaw.plugin.json"));
  ...
  it("loads installed plugin packages discovered from persisted install records", () => {
    ...
    const registry = withEnv({ SUPPORT_CLAW_STATE_DIR: stateDir }, () =>
      loadSupportClawPlugins({
        cache: false,
        config: {
          plugins: {
  ...
  it("refreshes bundled plugin-sdk aliases without deleting the shared alias directory", () => {
    ...
    const aliasDir = path.join(distRoot, "extensions", "node_modules", "supportClaw", "plugin-sdk");
    ...
    ensureSupportClawPluginSdkAlias(distRoot);
  ...
  it("keeps private local-only plugin-sdk artifacts out of package dist aliases", () => {
    ...
    const pluginSdkDir = path.join(distRoot, "plugin-sdk");
    const aliasRoot = path.join(distRoot, "extensions", "node_modules", "supportClaw");
  ...
  it("keeps private local-only plugin-sdk artifacts out of legacy package dist aliases", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "supportClaw", version: "2026.4.22", type: "module" }, null, 2),
      "utf8",
    );
    fs.writeFileSync(path.join(pluginSdkDir, "index.js"), "export const root = true;\n", "utf8");
    fs.writeFileSync(
  ...
  it("loads bundled plugins with plugin-sdk imports from a package dist root", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "supportClaw", version: "2026.4.22", type: "module" }),
      "utf-8",
    );
    fs.writeFileSync(
    ...
    ensureSupportClawPluginSdkAlias(path.join(packageRoot, "dist"));
    fs.writeFileSync(
      path.join(pluginRoot, "index.js"),
      [
        `import { normalizeLowercaseStringOrEmpty } from "supportClaw/plugin-sdk/string-coerce-runtime";`,
        `export default {`,
        `  id: "discord",`,
        `  register(api) {`,
    ...
    fs.writeFileSync(
      path.join(pluginRoot, "supportClaw.plugin.json"),
      JSON.stringify(
        {
          id: "discord",
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadSupportClawPlugins>) => {
...
module.exports = { id: "manifest-surfaces-plugin", register() { throw new Error("manifest-only snapshot should not register"); } };`,
        ...
        fs.writeFileSync(
          path.join(plugin.dir, "supportClaw.plugin.json"),
          JSON.stringify(
            {
              id: "manifest-surfaces-plugin",
              configSchema: EMPTY_PLUGIN_SCHEMA,
  ...
  register() {},
        ...
        fs.writeFileSync(
          path.join(memoryPlugin.dir, "supportClaw.plugin.json"),
          JSON.stringify(
            {
              id: "memory-demo",
              kind: "memory",
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const importMarker = "__supportClaw_loader_import_throw_marker";
...
module.exports = { id: "throws-after-import", register() {} };`,
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__supportClaw_loader_reentry_error";
        const reenterFnMarker = "__supportClaw_loader_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          reenterFnMarker,
          (options: Parameters<typeof loadSupportClawPlugins>[0]) => loadSupportClawPlugins(options),
        );
        const pluginDir = makeTempDir();
        ...
        } satisfies Parameters<typeof loadSupportClawPlugins>[0];
        writePlugin({
  ...
  register() {
    try {
      ...
      throw error;
        ...
        const registry = loadSupportClawPlugins(nestedOptions);
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__supportClaw_runtime_registry_reentry_marker";
        const resolverMarker = "__supportClaw_runtime_registry_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          resolverMarker,
  ...
  register() {
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadSupportClawPlugins(options);
        const scoped = loadSupportClawPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadSupportClawPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("skips discovery and manifest registry loading entirely when onlyPluginIds is an explicit empty array", async () => {
    ...
    const discoverySpy = vi.spyOn(discovery, "discoverSupportClawPlugins");
  ...
  it("only publishes plugin commands to the global registry during activating loads", () => {
    ...
    const active = loadSupportClawPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("replaces prior plugin hook registrations on activating reloads", async () => {
    ...
    loadSupportClawPlugins(loadOptions);
    loadSupportClawPlugins(loadOptions);
  ...
  it("can scope bundled provider loads without hanging", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@supportclaw/scoped-provider",
        supportClaw: { extensions: ["./index.cjs"] },
      }),
      "utf-8",
    );
    const plugin = writePlugin({
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@supportclaw/unscoped-provider",
        supportClaw: { extensions: ["./index.cjs"] },
      }),
      "utf-8",
    );
  ...
  it("restores cached memory capability public artifacts on cache hits", async () => {
    ...
    const first = loadSupportClawPlugins(options);
    ...
    const second = loadSupportClawPlugins(options);
  ...
  it("uses discovery registration mode for non-activating loads", () => {
    useNoBundledPlugins();
    const marker = "__supportClawDiscoveryModeTest";
    const plugin = writePlugin({
    ...
    const snapshot = loadSupportClawPlugins({
      activate: false,
      cache: false,
      workspaceDir: plugin.dir,
      config,
  ...
  it("caches non-activating snapshots without restoring global side effects", () => {
    useNoBundledPlugins();
    clearPluginCommands();
    const marker = "__supportClawSnapshotCacheRegisterCount";
    const plugin = writePlugin({
      id: "snapshot-cache",
  ...
  it("does not re-register non-bundled plugins after gateway-bindable boot loads", () => {
    useNoBundledPlugins();
    const marker = "__supportClawGatewayBootRegisterCount";
    const plugin = writePlugin({
  ...
  it("reuses a gateway-bindable cache entry for later default-mode loads", () => {
    useNoBundledPlugins();
    const marker = "__supportClawGatewayBindableCacheRegisterCount";
    const plugin = writePlugin({
    ...
    const gatewayBindable = loadSupportClawPlugins({
      ...options,
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
      },
    });
    const defaultMode = loadSupportClawPlugins(options);
  ...
  it("keeps pinned gateway hooks and later default-mode hooks dispatchable together", () => {
    ...
    const gatewayRegistry = loadSupportClawPlugins({
    ...
    try {
      ...
      const defaultRegistry = loadSupportClawPlugins({
        ...
        return {
          ...
          loadFirst: () =>
            loadSupportClawPlugins({
              ...
              env: {
                ...process.env,
                SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadSupportClawPlugins({
              ...
              env: {
                ...process.env,
                SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadSupportClawPlugins({
              ...
              env: {
                ...
                SUPPORT_CLAW_STATE_DIR: stateDir,
                SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadSupportClawPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(supportClawHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadSupportClawPlugins(options),
          loadVariant: () =>
            loadSupportClawPlugins({
              ...options,
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadSupportClawPlugins({
      env: {
        ...
        HOME: homeDir,
        SUPPORT_CLAW_HOME: undefined,
        SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers SUPPORT_CLAW_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const supportClawHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "supportClaw-home-demo",
      dir: path.join(supportClawHome, "plugins", "supportClaw-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "supportClaw-home-demo", register() {} };`,
    });
    ...
    const registry = loadSupportClawPlugins({
      ...
      config: {
        plugins: {
          allow: ["supportClaw-home-demo"],
          entries: {
            "supportClaw-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/supportClaw-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "supportClaw-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  it("can include plugin export shape when register is missing", () => {
    ...
    const registry = withEnv({ SUPPORT_CLAW_PLUGIN_LOAD_DEBUG: "1" }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadSupportClawPlugins>) =>
  ...
  api.registerGatewayDiscoveryService({ id: "shared-discovery", advertise() {} });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadSupportClawPlugins>) => {
          expect(
  ...
  it("loads bundled channel entries through nested default export wrappers", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@supportclaw/nested-default-channel",
          supportClaw: {
            extensions: ["./index.cjs"],
          },
        },
  ...
  it("does not treat manifest channel ids as scoped plugin id matches", () => {
    ...
    fs.writeFileSync(
      path.join(unrelated.dir, "supportClaw.plugin.json"),
      JSON.stringify(
        {
          id: "unrelated-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    ...
    const broadSetupRegistry = loadSupportClawPlugins({
      cache: false,
      config,
    ...
    const scopedSetupRegistry = loadSupportClawPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(workspacePluginDir, "supportClaw.plugin.json"),
      JSON.stringify(
        {
          id: "workspace-shadow",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    api.registerChannel({
      ...
      fs.writeFileSync(
        path.join(globalDir, "supportClaw.plugin.json"),
        JSON.stringify(
          {
            id: "untrusted-global-channel",
            configSchema: EMPTY_PLUGIN_SCHEMA,
      ...
      fs.writeFileSync(
        ...
        JSON.stringify(
          {
            name: "@supportclaw/untrusted-global-channel",
            version: "0.0.0-test",
            main: "./index.cjs",
            supportClaw: {
              extensions: ["./index.cjs"],
  ...
  register(api) {
    api.registerChannel({
      ...
      fs.writeFileSync(
        ...
        JSON.stringify(
          {
            name: "@supportclaw/trusted-global-channel",
            version: "0.0.0-test",
            main: "./index.cjs",
            supportClaw: {
              extensions: ["./index.cjs"],
...
module.exports = {
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@supportclaw/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@supportclaw/setup-only-bundled-contract-test",
        fullBlurb: "full entry should not run in setup-only mode",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@supportclaw/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@supportclaw/setup-runtime-bundled-contract-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@supportclaw/setup-runtime-bundled-contract-secrets-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@supportclaw/setup-runtime-bundled-contract-runtime-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@supportclaw/setup-runtime-bundled-route-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@supportclaw/setup-runtime-bundled-runtime-merge-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@supportclaw/setup-runtime-external-deferred-test",
        fullBlurb: "full entry should defer while configured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@supportclaw/setup-runtime-not-preferred-test",
  ...
  it("applies the bundled runtime setter before loading the merged setup-runtime plugin", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@supportclaw/setup-runtime-order-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("records setup runtime setter failures without aborting the full load pass", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@supportclaw/setup-runtime-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledSetupEntryContract: true,
  ...
  it("rolls back setup-runtime registrations when setup side effects fail", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@supportclaw/setup-runtime-route-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime route",
      configured: true,
      startupDeferConfiguredChannelFullLoadUntilAfterListen: true,
  ...
  it("closes setup-runtime registration APIs after synchronous registration", async () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@supportclaw/setup-runtime-late-route-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime route",
      configured: true,
      startupDeferConfiguredChannelFullLoadUntilAfterListen: true,
  ...
  it("rejects mismatched bundled runtime entry ids before applying setup-runtime setters", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@supportclaw/setup-runtime-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("rejects mismatched bundled setup export ids before loading setup-runtime entry code", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@supportclaw/setup-export-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("isolates loadSetupPlugin errors as per-plugin diagnostics instead of crashing registry load", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@supportclaw/setup-entry-throws-test",
          supportClaw: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
  ...
  it("keeps healthy sibling channel plugins loadable when a setup entry throws", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@supportclaw/setup-entry-throws-sibling-test",
          supportClaw: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
    ...
    fs.writeFileSync(
      path.join(brokenDir, "supportClaw.plugin.json"),
      JSON.stringify(
        {
  ...
  it("records a diagnostic when registerChannel throws in the setup-entry path", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@supportclaw/register-channel-throws-test",
          supportClaw: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
...
module.exports = {
  ...
  it("prefers built bundled plugin artifacts over source TS when requested", () => {
    ...
    fs.writeFileSync(
      path.join(sourceDir, "supportClaw.plugin.json"),
      JSON.stringify(
        {
    ...
    const registry = withEnv(
      {
        SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
        SUPPORT_CLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR: "1",
    ...
    const scenarios = [
      {
        ...
        loadRegistry: () => {
          ...
          return withEnv(
            {
              SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS: "1",
              SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: undefined,
            },
            () =>
              loadSupportClawPlugins({
                cache: false,
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "supportClaw.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "supportClaw.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryLanceDir, "supportClaw.plugin.json"),
            JSON.stringify(
  ...
  it("loads dreaming sidecar metadata through a restrictive selected-memory allowlist", async () => {
    ...
    const registry = await loadSupportClawPluginCliRegistry({
      cache: false,
      config: {
        plugins: {
          allow: [selectedId],
  ...
  it("resolves duplicate plugin ids by source precedence", () => {
    const scenarios = [
      ...
      },
      {
        ...
        bundledFilename: "index.cjs",
        loadRegistry: () => {
          const devSourceRoot = makeSupportClawDevSourceRoot();
          ...
          process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
          return withEnv({ SUPPORT_CLAW_DEV_SOURCE_ROOT: devSourceRoot }, () =>
            withStateDir((stateDir) => {
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          return withStateDir((stateDir) => {
            const globalDir = path.join(stateDir, "node_modules", "@supportclaw", "memory-lancedb");
            mkdirSafe(globalDir);
            ...
            fs.writeFileSync(
              ...
              JSON.stringify(
                {
                  name: "@supportclaw/memory-lancedb",
              ...
              config: {
                plugins: {
                  ...
                  installs: {
                    "memory-lancedb": {
                      source: "npm",
                      spec: "@supportclaw/memory-lancedb",
                      resolvedName: "@supportclaw/memory-lancedb",
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ SUPPORT_CLAW_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("uses the source runtime snapshot allowlist for plugin trust checks", () => {
    useNoBundledPlugins();
    const stateDir = makeTempDir();
    withEnv({ SUPPORT_CLAW_STATE_DIR: stateDir }, () => {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("supportClaw/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadSupportClawPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__supportClawLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("supportClaw/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("supportClaw/plugin-sdk");
