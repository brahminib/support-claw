/** Loads and normalizes SupportClaw plugin manifests, including contracts and config schemas. */
import fs from "node:fs";
import path from "node:path";
import { normalizeModelCatalog } from "@supportclaw/model-catalog-core/model-catalog-normalize";
import { normalizeModelCatalogProviderId } from "@supportclaw/model-catalog-core/model-catalog-refs";
import type { ModelCatalog } from "@supportclaw/model-catalog-core/model-catalog-types";
...
export const PLUGIN_MANIFEST_FILENAME = "supportClaw.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `supportClaw qa`, for example `matrix`. */
  commandName: string;
...
// package.json "supportClaw" metadata (used for setup/catalog)
...
export type SupportClawPackageStartup = {
  /**
...
export type SupportClawPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type SupportClawPackageCompat = {
  pluginApi?: string;
};

export type SupportClawPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: SupportClawPackageSetupFeatures;
  plugin?: {
    id?: string;
    label?: string;
  };
  channel?: PluginPackageChannel;
  compat?: SupportClawPackageCompat;
  install?: PluginPackageInstall;
  startup?: SupportClawPackageStartup;
};
...
} & Partial<Record<ManifestKey, SupportClawPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): SupportClawPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
...
export function resolvePackageExtensionEntries(
  ...
  const rawSupportClaw = manifest?.[MANIFEST_KEY] as unknown;
  if (rawSupportClaw === undefined || rawSupportClaw === null) {
    return { status: "missing", entries: [] };
  }
  if (!isRecord(rawSupportClaw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json supportClaw must be an object",
    };
  }
  const raw = rawSupportClaw.extensions;
  ...
  if (!Array.isArray(raw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json supportClaw.extensions must be an array",
    };
  }
  ...
  for (const [index, entry] of raw.entries()) {
    ...
    if (!normalized) {
      return {
        status: "invalid",
        entries: [],
        error: `package.json supportClaw.extensions[${index}] must be a non-empty string`,
      };
    }
