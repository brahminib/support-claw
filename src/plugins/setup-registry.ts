...
function collectConfiguredPluginEntryIds(config: SupportClawConfig): string[] {
...
function resolveRegister(mod: SupportClawPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: SupportClawPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as SupportClawPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: SupportClawPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so SupportClaw ignored the plugin setup runtime entry. Remove setup-api/supportClaw.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
