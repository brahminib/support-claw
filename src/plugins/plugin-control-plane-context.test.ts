...
describe("plugin control-plane context", () => {
  it("resolves env-sensitive discovery roots and load paths before fingerprinting", () => {
    ...
    const envA = { HOME: "/home/a", SUPPORT_CLAW_HOME: "/supportClaw/a" } as NodeJS.ProcessEnv;
    const envB = { HOME: "/home/b", SUPPORT_CLAW_HOME: "/supportClaw/b" } as NodeJS.ProcessEnv;

    const contextA = resolvePluginDiscoveryContext({ config, env: envA });
    const contextB = resolvePluginDiscoveryContext({ config, env: envB });

    expect(contextA.loadPaths).toEqual(["/supportClaw/a/plugins", "/opt/shared"]);
    expect(contextB.loadPaths).toEqual(["/supportClaw/b/plugins", "/opt/shared"]);
  ...
  it("includes policy, inventory, and activation in one control-plane fingerprint", () => {
    ...
    const base = resolvePluginControlPlaneFingerprint({
      config,
      env: { HOME: "/home/a", SUPPORT_CLAW_HOME: "/supportClaw/a" } as NodeJS.ProcessEnv,
  ...
  it("keeps the canonical context inspectable for cache diagnostics", () => {
    const context = resolvePluginControlPlaneContext({
      ...
      env: {
        HOME: "/home/a",
        SUPPORT_CLAW_HOME: "/supportClaw/a",
    ...
    expect(context).toStrictEqual({
      discovery: {
        ...
        roots: {
          stock: path.join(os.tmpdir(), "supportClaw-empty-bundled-plugins"),
          global: "/supportClaw/a/.supportClaw/extensions",
          workspace: undefined,
        },
      },
      inventoryFingerprint: "inventory",
