...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "supportClaw-empty-bundled-plugins");
const TEST_TRUST_BUNDLED_PLUGINS_DIR_ENV = "SUPPORT_CLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR";
...
export function areBundledPluginsDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = normalizeOptionalLowercaseString(env.SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "supportClaw.plugin.json"))
      );
    });
  } catch {
    return false;
...
function resolvePackageRootsForBundledPlugins(): string[] {
  const argvRoot = resolveSupportClawPackageRootSync({ argv1: process.argv[1] });
  const moduleRoot = resolveSupportClawPackageRootSync({ moduleUrl: import.meta.url });
...
function resolveTrustedExistingOverride(resolvedOverride: string): string | null {
  ...
  const modulePackageRoot = resolveSupportClawPackageRootSync({ moduleUrl: import.meta.url });
...
function createBundledPluginsDirCacheKey(env: NodeJS.ProcessEnv): string {
  return JSON.stringify({
    disabled: env.SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS ?? "",
    override: env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR ?? "",
    ...
    supportClawHome: env.SUPPORT_CLAW_HOME ?? "",
    home: env.HOME ?? "",
...
function resolveBundledPluginsDirUncached(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR?.trim();
