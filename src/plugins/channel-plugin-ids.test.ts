...
const listExplicitlyDisabledChannelIdsForConfig = vi.hoisted(() =>
  vi.fn((config: SupportClawConfig) => {
...
function withManifestLoadPaths<T extends { id: string }>(
  ...
  return {
    ...
    manifestPath: `/tmp/plugins/${plugin.id}/supportClaw.plugin.json`,
    skills: [],
    hooks: [],
    ...plugin,
  };
...
describe("resolveGatewayStartupPluginIds", () => {
  beforeEach(() => {
    listPotentialConfiguredChannelIds.mockReset().mockImplementation((config: SupportClawConfig) => {
    ...
    listPotentialConfiguredChannelPresenceSignals
      .mockReset()
      .mockImplementation((config: SupportClawConfig) => {
  ...
  it("does not treat persisted auth alone as gateway startup intent", () => {
    listPotentialConfiguredChannelIds.mockImplementation(
      (
        configForTest: SupportClawConfig,
    ...
    expectStartupPluginIdsCase({
      ...
      env: createPluginPlanningTestEnv({
        SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-with-persisted-demo-channel",
      }),
      expected: ["browser", "memory-core"],
    });
  });
  ...
  it("does not include Codex when an OpenAI model is manually pinned to SupportClaw", () => {
    expectStartupPluginIdsCase({
      config: {
        agents: {
          defaults: {
  ...
  it("ignores env runtime overrides during startup planning", () => {
    expectStartupPluginIdsCase({
      ...
      env: { SUPPORT_CLAW_AGENT_RUNTIME: "codex" },
      expected: ["demo-channel", "browser", "memory-core"],
    });
  });
