...
export type PluginLoadConfig = NonNullable<Parameters<typeof loadSupportClawPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadSupportClawPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "supportClaw-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".supportClaw", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ SUPPORT_CLAW_STATE_DIR: stateDir, ...params.env }, () =>
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
