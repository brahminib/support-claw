...
export type PluginCliCommandGroupEntry = {
  ...
  placeholders: readonly SupportClawPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: SupportClawPluginCliContext["program"]) => Promise<void>;
};
...
export async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadSupportClawPluginCliRegistry(
...
export async function loadPluginCliCommandRegistryWithContext(params: {
  ...
  return {
    ...params.context,
    registry: loadSupportClawPlugins(
...
): Promise<SupportClawPluginCliCommandDescriptor[]> {
  try {
