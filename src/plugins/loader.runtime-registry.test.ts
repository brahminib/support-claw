...
function makeSupportClawDevSourceRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-loader-dev-source-"));
...
describe("resolveRuntimePluginRegistry", () => {
  ...
  it("keeps the full workspace registry warm when scoped cron registries churn", () => {
    ...
    try {
      ...
      const fullRegistry = loadSupportClawPlugins(loadOptions);

      loadSupportClawPlugins({ ...loadOptions, onlyPluginIds: ["alpha"] });
      loadSupportClawPlugins({ ...loadOptions, onlyPluginIds: ["bravo"] });
...
describe("loadSupportClawPlugins active runtime clearing", () => {
  it("clears plugin-owned global providers before activating a new registry", () => {
    ...
    loadSupportClawPlugins({ onlyPluginIds: [] });
...
describe("clearPluginRegistryLoadCache", () => {
  ...
  it("invalidates full-workspace load snapshots", () => {
    ...
    const registry = loadSupportClawPlugins(loadOptions);

    clearPluginRegistryLoadCache();

    expect(loadSupportClawPlugins(loadOptions)).not.toBe(registry);
  });
});
