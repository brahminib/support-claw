...
function isPackageRootSupportClawPeerSymlink(segments: string[]): boolean {
  return (
    (segments.length === 2 && segments[0] === "node_modules" && segments[1] === "supportClaw") ||
    (segments.length === 3 &&
      segments[0] === "node_modules" &&
      segments[1] === ".bin" &&
      segments[2] === "supportClaw")
  );
}
...
function isManagedNpmRootPackagePeerSymlink(segments: string[]): boolean {
  ...
  return isPackageRootSupportClawPeerSymlink(segments.slice(packageEndIndex));
}

function isTrustedSupportClawPeerSymlink(params: {
  allowManagedNpmRootPackagePeerSymlinks?: boolean;
  relativePath: string;
}): boolean {
  const segments = params.relativePath.split(/[\\/]+/);
  return (
    isPackageRootSupportClawPeerSymlink(segments) ||
    (params.allowManagedNpmRootPackagePeerSymlinks === true &&
      isManagedNpmRootPackagePeerSymlink(segments))
  );
}

async function resolveTrustedHostSupportClawRootRealPath(): Promise<string | null> {
...
function isTrustedHostSupportClawPath(params: {
  resolvedTargetPath: string;
  trustedHostSupportClawRootRealPath: string | null;
}): boolean {
  return (
    params.trustedHostSupportClawRootRealPath !== null &&
    isPathInside(params.trustedHostSupportClawRootRealPath, params.resolvedTargetPath)
  );
}
...
> {
  ...
  if (!isPathInside(params.rootRealPath, resolvedTargetPath)) {
    if (
      isTrustedSupportClawPeerSymlink({
      ...
      isTrustedHostSupportClawPath({
        resolvedTargetPath,
        trustedHostSupportClawRootRealPath: params.trustedHostSupportClawRootRealPath,
      })
    ) {
      return {};
    }
...
function collectManifestRuntimeDependencyNames(manifest: PackageManifest): string[] {
  ...
  for (const dependencyName of Object.keys(manifest.peerDependencies ?? {})) {
    if (dependencyName !== "supportClaw" && isInstallScannableDependencyName(dependencyName)) {
      dependencyNames.add(dependencyName);
    }
  }
...
async function collectPackageManifestPaths(params: {
  ...
  const trustedHostSupportClawRootRealPath = await resolveTrustedHostSupportClawRootRealPath();
      ...
      if (entry.isSymbolicLink()) {
        ...
        if (pathContainsNodeModulesSegment(relativeNextPath)) {
          const symlinkTargetInspection = await inspectNodeModulesSymlinkTarget({
            ...
            trustedHostSupportClawRootRealPath,
          });
...
function resolvePolicySource(params: {
  ...
  if (params.requestKind === "skill-install") {
    switch (params.origin?.type) {
      case "clawhub":
        return { kind: "clawhub", authority: "supportClaw", mutable: false, network: true };
      case "git":
        return {
          kind: "git",
      ...
      case "supportClaw-bundled":
        return { kind: "bundled", authority: "supportClaw", mutable: false, network: false };
      case "supportClaw-managed":
      case "supportClaw-extra":
        return { kind: "managed", authority: "supportClaw", mutable: false, network: false };
      default:
