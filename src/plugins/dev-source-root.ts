...
/** Env var that points bundled-plugin lookup at an SupportClaw source checkout. */
export const SUPPORT_CLAW_DEV_SOURCE_ROOT_ENV = "SUPPORT_CLAW_DEV_SOURCE_ROOT";
...
/** Resolves and validates the configured SupportClaw development source root. */
export function resolveSupportClawDevSourceRoot(env: NodeJS.ProcessEnv = process.env): string | null {
  const rawRoot = env[SUPPORT_CLAW_DEV_SOURCE_ROOT_ENV]?.trim();
  if (!rawRoot) {
    return null;
  }
  ...
  if (readPackageName(path.join(realRoot, "package.json")) !== "supportClaw") {
    return null;
  }
...
export function isBundledPluginInsideDevSourceRoot(params: {
  ...
  const devSourceRoot = resolveSupportClawDevSourceRoot(params.env);
  if (!devSourceRoot) {
    return false;
  }
