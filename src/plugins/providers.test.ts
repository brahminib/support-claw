...
import { sortUniqueStrings } from "@supportclaw/normalization-core/string-normalization";
...
type LoadSupportClawPlugins = typeof import("./loader.js").loadSupportClawPlugins;
...
const loadSupportClawPluginsMock = vi.fn<LoadSupportClawPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/supportClaw.plugin.json`,
  };
}
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = getLastMockCallArg(loadSupportClawPluginsMock, "SupportClaw plugin setup loader");
...
function getLastSetupLoadedPluginConfig() {
  const call = expectRecordFields(
    getLastMockCallArg(loadSupportClawPluginsMock, "SupportClaw plugin setup loader"),
    {},
  );
...
function createAutoEnabledProviderConfig() {
  const rawConfig: SupportClawConfig = {
    plugins: {},
  };
  const autoEnabledConfig: SupportClawConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadSupportClawPlugins: (...args: Parameters<LoadSupportClawPlugins>) =>
        loadSupportClawPluginsMock(...args),
