// Regresses plugin loader behavior for git-path install records.
...
function makeTempDir() {
  return makeTrackedTempDir("supportClaw-plugin-loader", tempRoots);
}
...
`,
    ...
    const script = `
      ...
      const withAlias = createJiti(${JSON.stringify(sourceLoaderBaseFile)}, {
        ...
        extensions: [".ts", ".tsx", ".mts", ".cts", ".mtsx", ".ctsx", ".js", ".mjs", ".cjs", ".json"],
        alias: {
          "supportClaw/plugin-sdk/channel-outbound": ${JSON.stringify(copiedChannelRuntimeShim)},
        },
      });
