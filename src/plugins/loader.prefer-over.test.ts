...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-plugin-prefer-over-"));
...
describe("plugin loader preferOver activation", () => {
  it("loads the preferred external channel plugin without the replaced bundled plugin tools", () => {
    ...
    const env = {
      SUPPORT_CLAW_STATE_DIR: makeTempDir(),
    ...
    expect(autoEnabled.config.plugins?.entries?.["supportClaw-qqbot"]?.enabled).toBe(true);
    expect(autoEnabled.config.plugins?.entries?.qqbot?.enabled).toBe(false);
    expect(registry.plugins.find((plugin) => plugin.id === "supportClaw-qqbot")?.status).toBe(
      "loaded",
    );
    expect(registry.plugins.find((plugin) => plugin.id === "qqbot")?.status).toBe("disabled");
    expect(registry.tools.map((tool) => tool.pluginId)).toEqual(["supportClaw-qqbot"]);
    ...
    const registry = loadSupportClawPlugins({
      ...
      config: {
        ...
        plugins: {
          ...
          entries: {
            qqbot: { enabled: true },
            "supportClaw-qqbot": { enabled: true },
          },
        },
      },
      env,
