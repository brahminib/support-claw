...
const REQUIRED_REVIEWED_PUBLISHABLE_CRITICAL_FINDINGS = new Set([
  "@supportclaw/acpx:dangerous-exec:src/codex-auth-bridge.ts",
  "@supportclaw/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs",
  "@supportclaw/codex:dangerous-exec:src/app-server/sandbox-exec-server/http.ts",
  "@supportclaw/codex:dangerous-exec:src/app-server/sandbox-exec-server/processes.ts",
  "@supportclaw/codex:dangerous-exec:src/app-server/transport-stdio.ts",
  "@supportclaw/codex:dangerous-exec:src/node-cli-sessions.ts",
  "@supportclaw/discord:dangerous-exec:src/voice/audio.ts",
  "@supportclaw/google-meet:dangerous-exec:src/node-host.ts",
  "@supportclaw/google-meet:dangerous-exec:src/realtime.ts",
  "@supportclaw/matrix:dangerous-exec:src/matrix/deps.ts",
  "@supportclaw/voice-call:dangerous-exec:src/tunnel.ts",
  "@supportclaw/voice-call:dangerous-exec:src/webhook/tailscale.ts",
]);

const OPTIONAL_REVIEWED_PUBLISHABLE_DIST_CRITICAL_FINDINGS = new Set([
  "@supportclaw/acpx:dangerous-exec:dist/mcp-proxy.mjs",
  "@supportclaw/acpx:dangerous-exec:dist/service-<hash>.js",
  "@supportclaw/codex:dangerous-exec:dist/client-<hash>.js",
  "@supportclaw/google-meet:dangerous-exec:dist/index.js",
  "@supportclaw/slack:dynamic-code-execution:dist/outbound-payload.test-harness-<hash>.js",
  "@supportclaw/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js",
]);
...
function stageScannerRelevantPackedFiles(
  ...
  const stageDir = mkdtempSync(join(tmpdir(), "supportClaw-plugin-npm-scan-"));
...
function collectPublishablePluginPackages(): PublishablePluginPackage[] {
  return listPublishablePluginPackageDirs()
    .flatMap((packageDir) => {
      ...
      let packageJson: {
        name?: unknown;
        supportClaw?: { release?: { publishToNpm?: unknown } };
      };
      try {
      ...
      if (packageJson.supportClaw?.release?.publishToNpm !== true) {
        return [];
      }
