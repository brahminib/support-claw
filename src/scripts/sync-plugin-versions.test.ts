...
describe("syncPluginVersions", () => {
  ...
  it("preserves workspace supportClaw devDependencies and plugin host floors", () => {
    const rootDir = makeTempDir(tempDirs, "supportClaw-sync-plugin-versions-");
    ...
    writeJson(path.join(rootDir, "extensions/imessage/package.json"), {
      name: "@supportclaw/imessage",
      version: "2026.3.30",
      devDependencies: {
        supportClaw: "workspace:*",
      },
      peerDependencies: {
        supportClaw: ">=2026.3.30",
      },
      supportClaw: {
        install: {
    ...
    expect(summary.updated).toContain("@supportclaw/imessage");
    expect(updatedPackage.version).toBe("2026.4.1");
    expect(updatedPackage.devDependencies?.supportClaw).toBe("workspace:*");
    expect(updatedPackage.peerDependencies?.supportClaw).toBe(">=2026.4.1");
    expect(updatedPackage.supportClaw?.install?.minHostVersion).toBe(">=2026.3.30");
    expect(updatedPackage.supportClaw?.compat?.pluginApi).toBe(">=2026.4.1");
    expect(updatedPackage.supportClaw?.build?.supportClawVersion).toBe("2026.4.1");
  });

  it("reports pending version sync without writing in check mode", () => {
    const rootDir = makeTempDir(tempDirs, "supportClaw-sync-plugin-versions-check-");
    ...
    expect(summary.updated).toEqual(["@supportclaw/discord"]);
    expect(unchangedPackage.version).toBe("2026.4.1");
    expect(unchangedPackage.peerDependencies?.supportClaw).toBe(">=2026.4.1");
    expect(unchangedPackage.supportClaw?.compat?.pluginApi).toBe(">=2026.4.1");
  });

  it("uses the base release version for beta changelog entries", () => {
    const rootDir = makeTempDir(tempDirs, "supportClaw-sync-plugin-versions-beta-changelog-");
    ...
    expect(summary.changelogged).toEqual(["@supportclaw/matrix"]);
