...
import type { SupportClawConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "supportClaw-bundled": {
    ...
    canOverride: ["supportClaw-bundled"],
    canBeOverriddenBy: ["supportClaw-managed", "supportClaw-plugin"],
  },
  "supportClaw-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["supportClaw-bundled", "supportClaw-plugin"],
    canBeOverriddenBy: ["supportClaw-managed"],
  },
  "supportClaw-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["supportClaw-bundled", "supportClaw-managed", "supportClaw-plugin"],
    canBeOverriddenBy: ["supportClaw-managed"],
  },
  "supportClaw-workspace": {
    ...
    canOverride: ["supportClaw-workspace"],
    canBeOverriddenBy: ["supportClaw-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "supportClaw-plugin") {
    return { enabled: true };
  }
