...
const fixtureRoot = path.join(process.cwd(), ".tmp", `supportClaw-hook-install-${randomUUID()}`);
...
const zipHooksBuffer = await createZipHookPackBuffer({
  packageName: "@supportclaw/zip-hooks",
  hookName: "zip-hook",
...
const npmPackHooksBuffer = await createTarGzHookPackBuffer({
  packageName: "@supportclaw/test-hooks",
  hookName: "one-hook",
...
function writeHookPackFiles(params: {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      'metadata: {"supportClaw":{"events":["command:new"]}}',
      "---",
      "",
      `# ${params.heading}`,
    ].join("\n"),
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    supportClaw: { hooks: [`./hooks/${params.hookName}`] },
  });
...
describe("installHooksFromPath", () => {
  ...
  it("blocks a staged single hook before publishing the target", async () => {
    ...
    expect(scanCall.packageDir).toContain(".supportClaw-install-stage-");
  ...
  it.each([".codex-plugin/plugin.json", "hooks/hooks.json", "supportClaw.plugin.json"])(
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "supportClaw.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "supportClaw.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  it("forwards npm install policy metadata through extracted archive validation", async () => {
          ...
          expect(params.archiveInstallParams).toEqual(
            expect.objectContaining({
              installPolicyRequest: {
                kind: "plugin-npm",
                requestedSpecifier: "@supportclaw/test-hooks@0.0.1",
                source: {
                  kind: "npm",
    ...
    try {
      const result = await installHooksFromNpmSpec({
        spec: "@supportclaw/test-hooks@0.0.1",
      });
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@supportclaw/test-hooks@0.0.1",
              name: "@supportclaw/test-hooks",
              version: "0.0.1",
              filename: packedName,
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.packageKind).toBe("hook-only");
    expect(result.npmResolution?.resolvedSpec).toBe("@supportclaw/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@supportclaw/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = runCommandWithTimeoutMock;
    mockNpmPackMetadataResult(run, {
      id: "@supportclaw/test-hooks@0.0.2-beta.1",
      name: "@supportclaw/test-hooks",
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@supportclaw/test-hooks",
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("prerelease version 0.0.2-beta.1");
      expect(result.error).toContain('"@supportclaw/test-hooks@beta"');
    }
  });
});
