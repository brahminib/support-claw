...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "supportClaw-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "supportClaw-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "supportClaw-bundled");
      const plugin = makeHookEntry("shared", "supportClaw-plugin");
      const managed = makeHookEntry("shared", "supportClaw-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("supportClaw-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "supportClaw-managed");
      const workspace = makeHookEntry("shared", "supportClaw-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "supportClaw-workspace");
      const second = makeHookEntry("shared", "supportClaw-workspace");
      second.hook.handlerPath = "/tmp/supportClaw-workspace/shared/handler-2.js";
