...
homepage: https://docs.supportClaw.ai/automation/hooks#command-logger
metadata:
  {
    "supportClaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.supportClaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.supportClaw/logs/commands.log ~/.supportClaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/supportClaw`:
   ```
   /home/username/.supportClaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
