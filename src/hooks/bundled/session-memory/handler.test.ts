...
// Avoid calling the embedded SupportClaw agent (global command lane); keep this unit test deterministic.
...
beforeAll(async () => {
  ...
  suiteWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-session-memory-"));
});
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => SupportClawConfig;
  action?: "new" | "reset";
