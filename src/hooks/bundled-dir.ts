...
export function resolveBundledHooksDir(): string | undefined {
  const override = process.env.SUPPORT_CLAW_BUNDLED_HOOKS_DIR?.trim();
  if (override) {
    return override;
  }
  ...
  // This path works when installed via npm: node_modules/supportClaw/dist/hooks/bundled-dir.js
  try {
