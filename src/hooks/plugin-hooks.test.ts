...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "supportClaw-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.SUPPORT_CLAW_BUNDLED_HOOKS_DIR;
    process.env.SUPPORT_CLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.SUPPORT_CLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.SUPPORT_CLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".supportClaw", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): SupportClawConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("supportClaw-plugin");
  ...
  it("does not treat Claude hooks.json bundles as SupportClaw hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".supportClaw", "extensions", "claude-bundle");
