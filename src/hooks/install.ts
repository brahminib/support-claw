...
async function ensureSupportClawHooks(manifest: HookPackageManifest) {
  const hooks = manifest[MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing supportClaw.hooks");
  }
  const list = normalizeTrimmedStringList(hooks);
  if (list.length === 0) {
    throw new Error("package.json supportClaw.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(
  ...
  try {
    hookEntries = await ensureSupportClawHooks(manifest);
  } catch (err) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `supportClaw.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `supportClaw.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "supportClaw-hook-",
    timeoutMs,
    logger,
...
export async function installHooksFromNpmSpec(
  ...
  return await runtime.installFromValidatedNpmSpecArchive({
    tempDirPrefix: "supportClaw-hook-pack-",
    spec,
    timeoutMs,
