...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/supportClaw-agent"),
  resolveAgentDir: vi.fn(() => "/tmp/supportClaw-agent/.supportClaw-agent"),
  resolveAgentEffectiveModelPrimary: vi.fn((cfg: SupportClawConfig) => {
...
function requireFirstRunOptions(): Record<string, unknown> {
  ...
  if (!call) {
    throw new Error("expected embedded SupportClaw agent run");
  }
  const [options] = call;
  if (!options || typeof options !== "object") {
    throw new Error("expected embedded SupportClaw agent run options");
  }
