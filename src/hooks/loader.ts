...
const LOADED_INTERNAL_HOOK_REGISTRATIONS_KEY = Symbol.for(
  "supportClaw.loadedInternalHookRegistrations",
);
 ...
 *
 * @param cfg - SupportClaw configuration
...
export async function loadInternalHooks(
  ...
  for (const handlerConfig of handlers) {
    try {
      ...
      const importUrl = buildImportUrl(safeModulePath, "supportClaw-workspace");
      const mod = (await import(importUrl)) as Record<string, unknown>;
