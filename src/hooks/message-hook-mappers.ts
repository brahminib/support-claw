...
export type CanonicalInboundMessageHookContext = {
  ...
  // `mediaPath(s)` are files SupportClaw has already staged locally. `mediaUrl(s)`
...
): MessageTranscribedHookContext & { cfg: SupportClawConfig } {
...
export function toInternalMessagePreprocessedContext(
  canonical: CanonicalInboundMessageHookContext,
  cfg: SupportClawConfig,
): MessagePreprocessedHookContext & { cfg: SupportClawConfig } {
  const shared = toInternalInboundMessageHookContextBase(canonical);
  return {
    ...shared,
    transcript: canonical.transcript,
