...
import { getRuntimeConfig, type SupportClawConfig } from "../config/config.js";
...
const SUPPORT_CLAW_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../supportClawCli.mjs", import.meta.url));
const SUPPORT_CLAW_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const SUPPORT_CLAW_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const SUPPORT_CLAW_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(SUPPORT_CLAW_DIST_ENTRY_JS_PATH) || existsSync(SUPPORT_CLAW_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(SUPPORT_CLAW_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? SUPPORT_CLAW_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? SUPPORT_CLAW_RUN_NODE_SCRIPT_PATH;
...
export function resolveLocalAuthSpawnCwd(params: { args: string[]; defaultCwd?: string }): string {
  ...
  if (entryBase === "supportClawCli.mjs") {
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run supportClaw devices list",
      pairingHint:
        "Pairing required. Run `supportClaw devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export function scheduleProcessExitAfterTuiReturn(
  ...
  const timer = setTimeoutFn(() => {
    try {
      writeStderr("supportClaw tui forcing process exit after return\n");
    } catch {
...
function resolveEmptySessionInfoDefaults(config: SupportClawConfig): SessionInfo {
  return {
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__supportClaw_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "supportClaw tui";
    header.setText(
      theme.header(
  ...
  const forceExit = () => {
    try {
      process.stderr.write("supportClaw tui forcing exit\n");
    } catch {
    ...
    void Promise.resolve()
      ...
      .catch((err: unknown) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`supportClaw tui shutdown failed: ${String(err)}\n`);
          } catch {
