...
describe("resolveTuiFooterHostLabel", () => {
  ...
  it("renders only remote hosts when explicitly enabled", () => {
    const config = { tui: { footer: { showRemoteHost: true } } } satisfies SupportClawConfig;
...
describe("resolveInitialTuiAgentId", () => {
  const cfg: SupportClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/supportClaw" },
        { id: "ops", workspace: "/tmp/supportClaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/supportClaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run supportClaw devices list");
    expect(state.pairingHint).toContain("supportClaw devices list");
  });
...
describe("TUI shutdown safety", () => {
  ...
  it("schedules a process-exit guard after standalone TUI return", () => {
    ...
    callback?.();
    expect(writeStderr).toHaveBeenCalledWith("supportClaw tui forcing process exit after return\n");
...
describe("resolveLocalAuthCliInvocation", () => {
  it("uses the source runner when dist is unavailable", () => {
    expect(
      resolveLocalAuthCliInvocation({
        ...
        wrapperPath: "/repo/supportClawCli.mjs",
  ...
  it("uses the packaged wrapper when dist is available", () => {
    expect(
      ...
      command: "/usr/bin/node",
      args: ["/repo/supportClawCli.mjs", "models", "auth", "login"],
    });
  });
});
