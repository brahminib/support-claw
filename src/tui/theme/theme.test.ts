...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      SUPPORT_CLAW_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when SUPPORT_CLAW_THEME=light", async () => {
    const mod = await importThemeWithEnv({ SUPPORT_CLAW_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when SUPPORT_CLAW_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ SUPPORT_CLAW_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats SUPPORT_CLAW_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ SUPPORT_CLAW_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("SUPPORT_CLAW_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      SUPPORT_CLAW_THEME: "dark",
      COLORFGBG: "0;15",
    });
    expect(mod.lightMode).toBe(false);
  });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ SUPPORT_CLAW_THEME: "light" });
    const darkMod = await importThemeWithEnv({ SUPPORT_CLAW_THEME: "dark" });
