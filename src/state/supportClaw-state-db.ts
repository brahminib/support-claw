// SupportClaw state database manages shared persisted state and migrations.
...
import {
  resolveSupportClawStateSqliteDir,
  resolveSupportClawStateSqlitePath,
} from "./supportClaw-state-db.paths.js";
import { SUPPORT_CLAW_STATE_SCHEMA_SQL } from "./supportClaw-state-schema.generated.js";

/**
 * Shared SupportClaw SQLite state database lifecycle and metadata writers.
 *
 ...
 */
const SUPPORT_CLAW_STATE_SCHEMA_VERSION = 1;
/** Shared timeout used by state and agent SQLite handles before surfacing busy errors. */
export const SUPPORT_CLAW_SQLITE_BUSY_TIMEOUT_MS = 30_000;
const SUPPORT_CLAW_STATE_DIR_MODE = 0o700;
const SUPPORT_CLAW_STATE_FILE_MODE = 0o600;

/** Open shared SQLite database handle plus WAL maintenance lifecycle. */
export type SupportClawStateDatabase = {
  db: DatabaseSync;
  path: string;
...
export type SupportClawStateDatabaseOptions = {
  env?: NodeJS.ProcessEnv;
  path?: string;
};

export type SupportClawStateDatabaseSchemaMigration = {
  kind: "agent-databases-composite-primary-key";
  path: string;
};

const cachedDatabases = new Map<string, SupportClawStateDatabase>();

type SupportClawStateMetadataDatabase = Pick<SupportClawStateKyselyDatabase, "schema_meta">;

function assertSupportedSchemaVersion(db: DatabaseSync, pathname: string): void {
  const userVersion = readSqliteUserVersion(db);
  if (userVersion > SUPPORT_CLAW_STATE_SCHEMA_VERSION) {
    throw new Error(
      `SupportClaw state database ${pathname} uses newer schema version ${userVersion}; this SupportClaw build supports ${SUPPORT_CLAW_STATE_SCHEMA_VERSION}.`,
    );
  }
}
...
function ensureSupportClawStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveSupportClawStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveSupportClawStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`SupportClaw state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: SUPPORT_CLAW_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, SUPPORT_CLAW_STATE_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    if (existsSync(candidate)) {
      bestEffortChmodSync(candidate, SUPPORT_CLAW_STATE_FILE_MODE);
    }
  }
}
...
function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  throw new Error(
    `SupportClaw state database ${pathname} has a legacy agent database registry schema; run supportClaw doctor --fix to migrate it.`,
  );
}

export function detectSupportClawStateDatabaseSchemaMigrations(
  options: SupportClawStateDatabaseOptions = {},
): SupportClawStateDatabaseSchemaMigration[] {
...
export function repairSupportClawStateDatabaseSchema(options: SupportClawStateDatabaseOptions = {}): {
  changes: string[];
  warnings: string[];
} {
  ...
  ensureSupportClawStatePermissions(pathname, env);
  const sqlite = requireNodeSqlite();
  const db = new sqlite.DatabaseSync(pathname);
  db.exec(`PRAGMA busy_timeout = ${SUPPORT_CLAW_SQLITE_BUSY_TIMEOUT_MS};`);
  try {
...
function ensureSchema(db: DatabaseSync, pathname: string): void {
  ...
  db.exec(SUPPORT_CLAW_STATE_SCHEMA_SQL);
  ensureAdditiveStateColumns(db);
  db.exec(`PRAGMA user_version = ${SUPPORT_CLAW_STATE_SCHEMA_VERSION};`);
  const now = Date.now();
  const kysely = getNodeSqliteKysely<SupportClawStateMetadataDatabase>(db);
  executeSqliteQuerySync(
    db,
    kysely
      ...
      .values({
        meta_key: "primary",
        role: "global",
        schema_version: SUPPORT_CLAW_STATE_SCHEMA_VERSION,
        agent_id: null,
        app_version: null,
        created_at: now,
        updated_at: now,
...
function resolveDatabasePath(options: SupportClawStateDatabaseOptions = {}): string {
  return path.resolve(options.path ?? resolveSupportClawStateSqlitePath(options.env ?? process.env));
}

/** Open or return a cached shared state database after schema and migration checks. */
export function openSupportClawStateDatabase(
  options: SupportClawStateDatabaseOptions = {},
): SupportClawStateDatabase {
  const env = options.env ?? process.env;
  const pathname = resolveDatabasePath(options);
  ...
  const walMaintenance = (() => {
    ...
    try {
      maintenance = configureSqliteConnectionPragmas(db, {
        ...
        databaseLabel: "supportClaw-state",
...
export function runSupportClawStateWriteTransaction<T>(
  operation: (database: SupportClawStateDatabase) => T,
  options: SupportClawStateDatabaseOptions = {},
): T {
  const database = openSupportClawStateDatabase(options);
  const result = runSqliteImmediateTransactionSync(database.db, () => operation(database));
  try {
    ensureSupportClawStatePermissions(database.path, options.env ?? process.env);
  } catch {
...
export function closeSupportClawStateDatabase(): void {
...
export function isSupportClawStateDatabaseOpen(): boolean {
  return Array.from(cachedDatabases.values()).some((database) => database.db.isOpen);
}

/** Test alias for closing shared state handles from teardown code. */
export const closeSupportClawStateDatabaseForTest = closeSupportClawStateDatabase;
