...
vi.mock("node:fs", async (importOriginal) => {
  ...
  const chmodSync: typeof actual.chmodSync = ((target: unknown, mode: unknown) => {
    ...
    const isProbe = String(target).includes(".supportClaw-chmod-probe-");
...
} = await import("./supportClaw-state-db.js");
...
describe("state database permission hardening without chmod support", () => {
  ...
  it("opens the state database when chmodSync throws ENOTSUP", () => {
    stateDir = fs.mkdtempSync(join(tmpdir(), "supportClaw-state-chmod-"));
    chmodFailHook.error = enotsupError();

    const database = openSupportClawStateDatabase({ env: { SUPPORT_CLAW_STATE_DIR: stateDir } });
  ...
  it("rethrows EPERM when existing permissions are too broad", () => {
    ...
    expect(() => openSupportClawStateDatabase({ env: { SUPPORT_CLAW_STATE_DIR: stateDir } })).toThrow(
      /EPERM/,
    );
  });
  ...
  it("opens when EPERM leaves existing permissions restrictive", () => {
    stateDir = fs.mkdtempSync(join(tmpdir(), "supportClaw-state-chmod-"));
    openSupportClawStateDatabase({ env: { SUPPORT_CLAW_STATE_DIR: stateDir } });
    closeSupportClawStateDatabaseForTest();
    chmodFailHook.error = chmodError("EPERM");
  ...
  it("repairs the schema when chmodSync throws ENOTSUP", () => {
    ...
    expect(() =>
      repairSupportClawStateDatabaseSchema({ env: { SUPPORT_CLAW_STATE_DIR: stateDir } }),
    ).not.toThrow();
  });
  ...
  it("commits write transactions when chmodSync throws ENOTSUP", () => {
    ...
    const result = runSupportClawStateWriteTransaction((database) => {
      expect(database.db.isOpen).toBe(true);
      return "committed";
    }, options);
