...
import { resolveSupportClawStateSqliteDir } from "./supportClaw-state-db.paths.js";
...
export type SupportClawAgentSqlitePathOptions = {
  agentId: string;
...
export function resolveSupportClawAgentSqlitePath(options: SupportClawAgentSqlitePathOptions): string {
  const agentId = normalizeAgentId(options.agentId);
  return path.resolve(
    options.path ??
      path.join(
        path.dirname(resolveSupportClawStateSqliteDir(options.env ?? process.env)),
        "agents",
        agentId,
        "agent",
