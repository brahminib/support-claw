// SupportClaw agent database tests cover agent-scoped DB storage and migrations.
...
type AgentDbTestDatabase = Pick<SupportClawAgentKyselyDatabase, "schema_meta">;
...
function createTempStateDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-agent-db-"));
}

function listRegisteredAgentDatabasesForTest(options: { env?: NodeJS.ProcessEnv } = {}) {
  const rows = openSupportClawStateDatabase(options)
    .db.prepare(
...
describe("supportClaw agent database", () => {
  it("resolves under the per-agent state directory", () => {
    ...
    ).toBe(path.join(stateDir, "agents", "worker-1", "agent", "supportClaw-agent.sqlite"));
  });
  ...
  it("keeps test default state under a worker-sharded temp directory", () => {
    expect(
      ...
      path.join(
        os.tmpdir(),
        "supportClaw-test-state",
        `${process.pid}-7`,
        "agents",
        "main",
        "agent",
        "supportClaw-agent.sqlite",
      ),
    );
  });
  ...
  it("creates the per-agent schema and registers it globally", () => {
    const stateDir = createTempStateDir();
    const database = openSupportClawAgentDatabase({
      agentId: "worker-1",
      env: { SUPPORT_CLAW_STATE_DIR: stateDir },
    });

    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./supportClaw-agent-schema.sql", import.meta.url)),
    );
    expect(database.agentId).toBe("worker-1");
    expect(database.path).toBe(
      path.join(stateDir, "agents", "worker-1", "agent", "supportClaw-agent.sqlite"),
    );
  ...
  it("keeps multiple registered paths for the same agent", () => {
    const stateDir = createTempStateDir();
    const env = { SUPPORT_CLAW_STATE_DIR: stateDir };
    const relocatedPath = path.join(stateDir, "relocated", "worker-1.sqlite");
    const relocated = openSupportClawAgentDatabase({
      agentId: "worker-1",
      env,
      path: relocatedPath,
    });
    const defaultDatabase = openSupportClawAgentDatabase({
      agentId: "worker-1",
      env,
    });
  ...
  it("rejects the legacy agent registry primary key with a doctor repair hint", () => {
    ...
    ).toThrow(/run supportClaw doctor --fix/);
  });

  it("keys explicit relative paths by resolved database pathname", () => {
    const agentModuleUrl = new URL("./supportClaw-agent-db.ts", import.meta.url).href;
    const stateModuleUrl = new URL("./supportClaw-state-db.ts", import.meta.url).href;
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-agent-db-state-"));
          const env = { SUPPORT_CLAW_STATE_DIR: stateDir };
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-agent-db-relative-"));
          ...
          try {
            ...
            const first = openSupportClawAgentDatabase({
              agentId: "worker-1",
              env,
              path: "agent.sqlite",
            });
            ...
            const second = openSupportClawAgentDatabase({
              agentId: "worker-1",
              env,
              path: "agent.sqlite",
            });
            ...
            console.log(JSON.stringify({
              ...
              registeredPaths: openSupportClawStateDatabase({ env }).db
  ...
  it("rejects explicit paths that point at the global state database", () => {
    ...
    const stateDatabase = openSupportClawStateDatabase({
      env,
      path: databasePath,
    });
    closeSupportClawStateDatabaseForTest();
    ...
    const reopenedStateDatabase = openSupportClawStateDatabase({
      env,
      path: databasePath,
    });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    ...
    const env = { SUPPORT_CLAW_STATE_DIR: parentDir };
