...
describe("completion-runtime", () => {
  ...
  it("installs PowerShell completion into the concrete profile path", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-completion-home-"));
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-completion-state-bob's-"));

    try {
      await withEnvAsync({ HOME: homeDir, SUPPORT_CLAW_STATE_DIR: stateDir }, async () => {
        const cachePath = resolveCompletionCachePath("powershell", "supportClaw");
        await fs.mkdir(path.dirname(cachePath), { recursive: true });
        await fs.writeFile(cachePath, "# powershell completion\n", "utf-8");

        await installCompletion("powershell", true, "supportClaw");

        const profilePath = resolveCompletionProfilePath("powershell");
        const profile = await fs.readFile(profilePath, "utf-8");
        expect(profile).toBe(`# SupportClaw Completion\n. '${cachePath.replace(/'/g, "''")}'\n`);
      });
    } finally {
  ...
  it("rejects install when the completion cache is missing", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-completion-home-"));
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-completion-state-"));

    try {
      await withEnvAsync({ HOME: homeDir, SUPPORT_CLAW_STATE_DIR: stateDir }, async () => {
        await expect(installCompletion("zsh", true, "supportClaw")).rejects.toThrow(
