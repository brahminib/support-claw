...
vi.mock("../terminal/links.js", () => ({
  formatDocsLink: () => "docs.supportClaw.ai/cli/skills",
}));
...
describe("skills workshop cli", () => {
  ...
  beforeEach(async () => {
    testState = await createSupportClawTestState({
      layout: "state-only",
      prefix: "supportClaw-skills-cli-workshop-state-",
    });
    mocks.workspaceDir = await tempDirs.make("supportClaw-skills-cli-workshop-");
  ...
  it("scopes list and inspect to the selected workspace", async () => {
    ...
    mocks.workspaceDir = await tempDirs.make("supportClaw-skills-cli-workshop-second-");
    await runCommand(["skills", "workshop", "list"]);
