...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-bundled-skills-test-"));
    process.env.SUPPORT_CLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
