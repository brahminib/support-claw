...
import { serveSupportClawChannelMcp } from "../mcp/channel-server.js";
      ...
      if (server.auth === "oauth") {
        ...
        if (!authStatus.hasTokens) {
          issues.push(
            issue(
              "warning",
              `OAuth credentials are not authorized; run ${formatCliCommand(`supportClaw mcp login ${name}`)}`,
            ),
          );
        }
...
async function probeMcpServerIssue(params: {
  ...
  const runtime = createSessionMcpRuntime({
    sessionId: "supportClaw-cli-mcp-doctor",
...
async function probeMcpServersOrFail(params: {
  ...
  const runtime = createSessionMcpRuntime({
    sessionId: "supportClaw-cli-mcp-probe",
    workspaceDir: process.cwd(),
...
const SUPPORT_CLAW_MCP_REGISTRY_SCOPE_NOTE =
  "Note: this command only shows SupportClaw-managed mcp.servers entries and does not include mcporter servers from config/mcporter.json.";

export function registerMcpCli(program: Command) {
  const mcp = program
    .command("mcp")
    .description("Manage SupportClaw mcp.servers config and channel bridge");

  mcp
    .command("serve")
    .description("Expose SupportClaw channels over MCP stdio")
    ...
    .action(async (opts) => {
      try {
        ...
        await serveSupportClawChannelMcp({
        ...
        defaultRuntime.error(
          `MCP server failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("supportClaw mcp list")} to inspect configured servers.`,
        );
        defaultRuntime.exit(1);
      }
    });
  ...
  mcp
    .command("list")
    .description("List SupportClaw-managed MCP servers from mcp.servers")
    ...
    .action(async (opts: { json?: boolean }) => {
      ...
      if (names.length === 0) {
        defaultRuntime.log(
          `No SupportClaw-managed MCP servers configured in ${loaded.path}. Add one with ${formatCliCommand('supportClaw mcp set <name> \'{"command":"uvx","args":["context7-mcp"]}\'')}.`,
        );
        defaultRuntime.log(SUPPORT_CLAW_MCP_REGISTRY_SCOPE_NOTE);
        return;
      }
      defaultRuntime.log(`SupportClaw-managed MCP servers (${loaded.path}):`);
  ...
  mcp
    .command("show")
    .description("Show one SupportClaw-managed MCP server or the full mcp.servers config")
    ...
    .action(async (name: string | undefined, opts: { json?: boolean }) => {
      ...
      if (name && !value) {
        fail(
          `No MCP server named "${name}" in ${loaded.path}. Run ${formatCliCommand("supportClaw mcp list")} to see configured servers.`,
        );
      }
      if (opts.json) {
        printJson(value ?? {});
      ...
      if (name) {
        defaultRuntime.log(`SupportClaw-managed MCP server "${name}" (${loaded.path}):`);
      } else {
        defaultRuntime.log(`SupportClaw-managed MCP servers (${loaded.path}):`);
      }
      printJson(value ?? {});
  ...
  mcp
    ...
    .action(async (name: string | undefined, opts: { json?: boolean }) => {
      ...
      if (name && loaded.mcpServers[name]?.enabled === false) {
        fail(
          `MCP server "${name}" is disabled in ${loaded.path}. Run ${formatCliCommand(`supportClaw mcp configure ${name} --enable`)} before probing it.`,
        );
      }
      const runtime = createSessionMcpRuntime({
        sessionId: "supportClaw-cli-mcp-probe",
  ...
  mcp
    ...
    .action(async (name: string | undefined, opts: { probe?: boolean; json?: boolean }) => {
      ...
      if (servers.length === 0) {
        defaultRuntime.log(
          `No MCP servers configured in ${loaded.path}. Add one with ${formatCliCommand("supportClaw mcp add <name> --command <command>")}.`,
        );
        return;
      }
  ...
  mcp
    ...
    .action(
      async (
        ...
        if (server.auth === "oauth") {
          defaultRuntime.log(
            `Run ${formatCliCommand(`supportClaw mcp login ${name}`)} to authorize this MCP server.`,
          );
        }
      },
    );
  ...
  mcp
    .command("set")
    .description("Set one SupportClaw-managed MCP server from a JSON object")
    .argument("<name>", "MCP server name")
  ...
  mcp
    ...
    .action(async (name: string, opts: { include?: string; exclude?: string; clear?: boolean }) => {
      ...
      if (!result.updated) {
        fail(
          `No MCP server named "${name}" in ${result.path}. Run ${formatCliCommand("supportClaw mcp list")} to see configured servers.`,
        );
      }
    ...
    .action(async (name: string, opts: { code?: string }) => {
      ...
      const result = await runMcpOAuthLogin({
        ...
        onAuthorizationUrl: (url) => {
          ...
          defaultRuntime.log(
            `After approval, run ${formatCliCommand(`supportClaw mcp login ${name} --code <code>`)}.`,
          );
        },
      });
  ...
  mcp
    .command("unset")
    .description("Remove one SupportClaw-managed MCP server")
    .argument("<name>", "MCP server name")
    .action(async (name: string) => {
      const loaded = await listConfiguredMcpServers();
      if (!loaded.ok) {
