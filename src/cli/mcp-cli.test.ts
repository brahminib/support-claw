...
const mocks = vi.hoisted(() => {
  ...
  return {
    runtime,
    serveSupportClawChannelMcp: vi.fn(),
...
const serveSupportClawChannelMcp = mocks.serveSupportClawChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveSupportClawChannelMcp: mocks.serveSupportClawChannelMcp,
}));
...
async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("supportClaw-cli-mcp-home-", async (home) => {
  ...
  it("adds a configured MCP server from flags without replacing operator knobs", async () => {
    await withTempHome("supportClaw-cli-mcp-home-", async () => {
      const workspaceDir = await createWorkspace();
      vi.spyOn(process, "cwd").mockReturnValue(workspaceDir);
  ...
  it("labels listed MCP servers as SupportClaw-managed", async () => {
    await withTempHome("supportClaw-cli-mcp-home-", async () => {
      const workspaceDir = await createWorkspace();
      vi.spyOn(process, "cwd").mockReturnValue(workspaceDir);
      ...
      expect(output).toContain("SupportClaw-managed MCP servers (");
      expect(output).toContain("- context7");
      expect(output).toContain("SupportClaw-managed mcp.servers entries");
  ...
  it("removes pure disabled tombstones when enabling MCP servers", async () => {
    await withTempHome("supportClaw-cli-mcp-home-", async () => {
      ...
      await runMcpCommand(["mcp", "list"]);
      const output = mockLog.mock.calls.map((call) => String(call[0])).join("\n");
      expect(output).toContain("No SupportClaw-managed MCP servers configured in ");
      expect(output).toContain("does not include mcporter servers from config/mcporter.json");
    });
  });
  ...
  it("fails named probes for disabled MCP servers", async () => {
    await withTempHome("supportClaw-cli-mcp-home-", async (home) => {
      ...
      expect(lastErrorLine()).toBe(
        `MCP server "docs" is disabled in ${configPath}. Run supportClaw mcp configure docs --enable before probing it.`,
      );
    });
  });
  ...
  it("fails when removing an unknown MCP server", async () => {
    await withTempHome("supportClaw-cli-mcp-home-", async (home) => {
      ...
      expect(lastErrorLine()).toBe(
        `No MCP server named "missing" in ${configPath}. Run supportClaw mcp list to see configured servers.`,
      );
    });
  });
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("supportClaw-cli-mcp-home-", async () => {
      ...
      expect(serveSupportClawChannelMcp).toHaveBeenCalledWith({
