...
const resolveShellEnvExpectedKeys = vi.fn((_env?: NodeJS.ProcessEnv) => ["SUPPORT_CLAW_GATEWAY_TOKEN"]);
...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/supportClaw-stability.json",
  path: "/tmp/supportClaw-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/supportClaw-control-ui" as string | null,
}));
...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/supportClaw-test-missing-config.json",
...
vi.mock("../../config/paths.js", () => ({
  CONFIG_PATH: "/tmp/supportClaw-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/supportClaw-control-ui";
  ...
  it("loads configured shell env fallback before final proxy refresh and gateway startup", async () => {
    await withEnvAsync({ SUPPORT_CLAW_GATEWAY_TOKEN: undefined }, async () => {
      const finalConfig = {
        env: {
          shellEnv: { enabled: true, timeoutMs: 1234 },
          vars: { SUPPORT_CLAW_GATEWAY_TOKEN: "config-token" },
        },
        gateway: {
      ...
      readConfigFileSnapshotWithPluginMetadata
        ...
        .mockImplementationOnce(async (options) => {
          expect(options?.lowerPrecedenceEnv).toEqual({
            SUPPORT_CLAW_GATEWAY_TOKEN: "shell-token",
          });
          expect(process.env.SUPPORT_CLAW_GATEWAY_TOKEN).toBe("shell-token");
          return {
            snapshot: {
              ...configState.snapshot,
              config: {
      ...
      loadShellEnvFallback.mockImplementationOnce((opts?: unknown) => {
        callOrder.push("shell-env");
        (opts as { env: NodeJS.ProcessEnv }).env.SUPPORT_CLAW_GATEWAY_TOKEN = "shell-token";
      });
      const uninstall = installGatewayRunRuntimeHooks({ refreshManagedProxy });
      try {
        await runGatewayCli(["gateway"]);
      ...
      expect(loadShellEnvFallback).toHaveBeenCalledWith({
        enabled: true,
        env: process.env,
        expectedKeys: ["SUPPORT_CLAW_GATEWAY_TOKEN"],
        logger: expect.any(Object),
        timeoutMs: 1234,
      });
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledWith(
        expect.objectContaining({
          lowerPrecedenceEnv: { SUPPORT_CLAW_GATEWAY_TOKEN: "shell-token" },
        }),
      );
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledTimes(2);
      expect(process.env.SUPPORT_CLAW_GATEWAY_TOKEN).toBe("config-token");
      expect(clearShellEnvAppliedKeys).toHaveBeenCalledWith(["SUPPORT_CLAW_GATEWAY_TOKEN"]);
    ...
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: {
            vars: {
              ...
              SUPPORT_CLAW_SHELL_ENV_TIMEOUT_MS: "4321",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        };
        ...
        shouldEnableShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.SUPPORT_CLAW_LOAD_SHELL_ENV === "1",
        );
        resolveShellEnvFallbackTimeoutMs.mockImplementationOnce((env?: NodeJS.ProcessEnv) =>
          Number(env?.SUPPORT_CLAW_SHELL_ENV_TIMEOUT_MS),
        );
        ...
        expect(process.env.SUPPORT_CLAW_LOAD_SHELL_ENV).toBe("1");
        expect(process.env.SUPPORT_CLAW_SHELL_ENV_TIMEOUT_MS).toBe("4321");
      },
    );
  });
    ...
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: {
            vars: {
              SUPPORT_CLAW_DEFER_SHELL_ENV_FALLBACK: "1",
              SUPPORT_CLAW_LOAD_SHELL_ENV: "1",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        ...
        shouldDeferShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.SUPPORT_CLAW_DEFER_SHELL_ENV_FALLBACK === "1",
        );
  ...
  it("ignores shell fallback controls from invalid config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        const invalidConfig = {
          env: { vars: { SUPPORT_CLAW_DEFER_SHELL_ENV_FALLBACK: "1" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
  ...
  it("rejects an invalid final config after a prepared config selected runtime paths", async () => {
    const selectedStateDir = "/tmp/supportClaw-prepared-selected-state";
    await withEnvAsync({ SUPPORT_CLAW_STATE_DIR: undefined }, async () => {
  ...
  it("replaces config-derived env when the final startup snapshot changes in place", async () => {
    await withEnvAsync(
      {
        SUPPORT_CLAW_GATEWAY_TOKEN: undefined,
        SUPPORT_CLAW_PROXY_URL: undefined,
        SUPPORT_CLAW_RAW_STREAM: undefined,
      },
      async () => {
        const oldConfig = {
          env: {
            vars: {
              ...
              SUPPORT_CLAW_PROXY_URL: "http://127.0.0.1:19876",
              SUPPORT_CLAW_RAW_STREAM: "1",
            },
          },
          gateway: { mode: "local" },
        };
        const newConfig = {
          env: { vars: { SUPPORT_CLAW_GATEWAY_TOKEN: "new-token" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: oldConfig,
        ...
        expect(process.env.SUPPORT_CLAW_GATEWAY_TOKEN).toBe("old-token");
        expect(process.env.SUPPORT_CLAW_PROXY_URL).toBe("http://127.0.0.1:19876");
        ...
        readConfigFileSnapshotWithPluginMetadata.mockImplementationOnce(async () => {
          expect(process.env.SUPPORT_CLAW_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.SUPPORT_CLAW_PROXY_URL).toBeUndefined();
          return { snapshot: configState.snapshot };
        });
        await runGatewayCli(["gateway", "--raw-stream"]);

        expect(process.env.SUPPORT_CLAW_GATEWAY_TOKEN).toBe("new-token");
        expect(process.env.SUPPORT_CLAW_PROXY_URL).toBeUndefined();
        expect(process.env.SUPPORT_CLAW_RAW_STREAM).toBe("1");
      },
    );
  });
  ...
  it("marks descendants when the final config supplies the service marker", async () => {
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: { vars: { SUPPORT_CLAW_SERVICE_MARKER: "supportClaw" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: finalConfig,
        ...
        expect(process.env.SUPPORT_CLAW_SERVICE_MARKER).toBe("supportClaw");
        expect(process.env[GATEWAY_SERVICE_RUNTIME_PID_ENV]).toBe(String(process.pid));
      },
    );
  });
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    process.env.SUPPORT_CLAW_SERVICE_MARKER = "gateway";
    try {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        ensureDevGatewayConfig.mockImplementationOnce(async () => {
          ...
          expect(process.env.SUPPORT_CLAW_HOME).toBeUndefined();
          expect(process.env.SUPPORT_CLAW_PROFILE).toBe("dev");
          expect(process.env.SUPPORT_CLAW_STATE_DIR).toBeUndefined();
          expect(process.env.SUPPORT_CLAW_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.SUPPORT_CLAW_WORKSPACE_DIR).toBe("/tmp/supportClaw-reset-workspace");
          configState.snapshot = {
            exists: true,
            valid: true,
            config: { gateway: { mode: "local" } },
        ...
        loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
          process.env.SUPPORT_CLAW_GATEWAY_TOKEN ??= "trusted-token";
          process.env.SUPPORT_CLAW_PROFILE ??= "dev";
          process.env.SUPPORT_CLAW_WORKSPACE_DIR ??= "/tmp/supportClaw-reset-workspace";
        });
  ...
  it("refuses dev reset if trusted dotenv retargets after pre-bootstrap", async () => {
    await withEnvAsync({ SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-reset-original" }, async () => {
      configState.snapshot = {
        config: { gateway: { mode: "local" } },
        exists: true,
        path: "/tmp/supportClaw-reset-original/supportClaw.json",
        sourceConfig: { gateway: { mode: "local" } },
        valid: true,
      };
      await prepareGatewayReset();
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        process.env.SUPPORT_CLAW_STATE_DIR = "/tmp/supportClaw-reset-retargeted";
        return {
          gatewayEnvAppliedKeys: [],
          stateEnvAppliedKeys: ["SUPPORT_CLAW_STATE_DIR"],
        };
      });
      ...
      expect(ensureDevGatewayConfig).not.toHaveBeenCalled();
      expect(process.env.SUPPORT_CLAW_STATE_DIR).toBe("/tmp/supportClaw-reset-original");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it.each([
    ...
    "SUPPORT_CLAW_INCLUDE_ROOTS",
    "SUPPORT_CLAW_NIX_MODE",
    "SUPPORT_CLAW_OAUTH_DIR",
    "SUPPORT_CLAW_PACKAGE_DIR",
    ...
    await withEnvAsync({ [selector]: "/tmp/supportClaw-reset-value" }, async () => {
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        process.env[selector] = "/tmp/supportClaw-reset-retargeted";
      });
      ...
      expect(process.env[selector]).toBe("/tmp/supportClaw-reset-value");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it("blocks a final startup snapshot that changes guarded config selection", async () => {
    await withEnvAsync({ SUPPORT_CLAW_STATE_DIR: undefined }, async () => {
      configState.snapshot = {
        ...
        config: { gateway: { mode: "local" } },
        sourceConfig: {
          env: { vars: { SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-late-selection" } },
          gateway: { mode: "local" },
        },
      };
  ...
  it("blocks a final startup snapshot that changes an already-selected config selector", async () => {
    await withEnvAsync({ SUPPORT_CLAW_STATE_DIR: undefined }, async () => {
      const guardedConfig = {
        env: { vars: { SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-guarded-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
      ...
      await selectGatewayRunEnvironment({ opts: {}, runtime: defaultRuntime });
      await prepareGatewayRunBootstrap({ opts: {}, runtime: defaultRuntime });
      expect(process.env.SUPPORT_CLAW_STATE_DIR).toBe("/tmp/supportClaw-guarded-state");

      const finalConfig = {
        env: { vars: { SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-final-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
        config: finalConfig,
  ...
  it.each([
    ...
    delete process.env.SUPPORT_CLAW_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.SUPPORT_CLAW_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `supportClaw onboard --mode local` or `supportClaw setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("blocks invalid startup config without automatic recovery", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/supportClaw-test-missing-config.json",
      config: {},
      parsed: null,
  ...
  it("allows password mode preflight when password is configured via SecretRef", async () => {
    configState.cfg = {
      gateway: {
        auth: {
          mode: "password",
          password: { source: "env", provider: "default", id: "SUPPORT_CLAW_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        defaults: {
  ...
  it("reads gateway password from --password-file", async () => {
    ...
    expect(runtimeErrors).not.toContain(
      "Warning: --password can be exposed via process listings. Prefer --password-file or SUPPORT_CLAW_GATEWAY_PASSWORD.",
    );
  });
