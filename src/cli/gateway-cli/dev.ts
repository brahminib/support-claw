...
const resolveDevWorkspaceDir = (env: NodeJS.ProcessEnv = process.env): string => {
  ...
  const profile = normalizeOptionalLowercaseString(env.SUPPORT_CLAW_PROFILE);
  if (profile === "dev") {
    return baseDir;
  }
...
async function ensureDevWorkspace(dir: string) {
  ...
  const [agents, soul, tools, identity, user] = await Promise.all([
    loadDevTemplate(
      "AGENTS.dev.md",
      `# AGENTS.md - SupportClaw Dev Workspace\n\nDefault dev workspace for supportClaw gateway --dev.\n`,
    ),
    loadDevTemplate(
      "SOUL.dev.md",
