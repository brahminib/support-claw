...
type GatewayRunFutureConfigGuardParams = {
  ...
  config?: Pick<SupportClawConfig, "env" | "meta"> | null;
};

function resolveGatewayRunFutureConfigBlock(params: GatewayRunFutureConfigGuardParams) {
  const processServiceMode = Boolean(process.env.SUPPORT_CLAW_SERVICE_MARKER?.trim());
  ...
  const candidateServiceMode =
    ...
    Boolean(
      candidateConfig
        ? createConfigRuntimeEnv(candidateConfig, process.env).SUPPORT_CLAW_SERVICE_MARKER?.trim()
        : undefined,
    );
