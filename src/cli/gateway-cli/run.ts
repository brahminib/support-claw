...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.SUPPORT_CLAW_GATEWAY_STARTUP_TRACE);
...
async function maybeLogPendingControlUiBuild(cfg: SupportClawConfig): Promise<void> {
...
function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("supportClaw setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  if (params.mode === undefined) {
    return [
      [
        ...
        `Re-run \`${formatCliCommand("supportClaw onboard --mode local")}\` or \`${formatCliCommand("supportClaw setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
      ].join(" "),
...
async function readGatewayStartupConfig(params: {
  ...
  let blockedRecoveryConfig: SupportClawConfig | null = null;
...
export async function runGatewayCommand(opts: GatewayRunOpts, hooks: GatewayRunRuntimeHooks = {}) {
  ...
  const isDevProfile = normalizeOptionalLowercaseString(process.env.SUPPORT_CLAW_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.SUPPORT_CLAW_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.SUPPORT_CLAW_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.SUPPORT_CLAW_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (snapshot) {
    ...
    const finalConfigEnteredServiceMode = Boolean(process.env.SUPPORT_CLAW_SERVICE_MARKER?.trim());
  ...
  if (process.env.SUPPORT_CLAW_SERVICE_MARKER?.trim()) {
  ...
  if (opts.force) {
    try {
      ...
      defaultRuntime.error(
        `Could not free port ${port}: ${formatErrorMessage(err)}. Run ${formatCliCommand("supportClaw gateway status --deep")} to inspect the listener.`,
      );
      defaultRuntime.exit(1);
      return;
    }
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or SUPPORT_CLAW_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set SUPPORT_CLAW_GATEWAY_TOKEN or SUPPORT_CLAW_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or SUPPORT_CLAW_GATEWAY_TOKEN/SUPPORT_CLAW_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
  ...
  const deferStartupSidecars =
    isTruthyEnvValue(process.env.SUPPORT_CLAW_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.SUPPORT_CLAW_SKIP_PROVIDERS);
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("supportClaw gateway stop")}`,
      );
      try {
    ...
    defaultRuntime.error(
      `Gateway failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("supportClaw gateway status --deep")} for diagnostics.`,
    );
    defaultRuntime.exit(1);
  }
}
