...
describe("command-path-policy", () => {
  ...
  it("applies exact overrides after broader channel plugin rules", () => {
    ...
    expect(
      channelsStatusPolicy.networkProxy({
        argv: ["node", "supportClaw", "channels", "status"],
        commandPath: ["channels", "status"],
      }),
    ).toBe("bypass");
    expect(
      channelsStatusPolicy.networkProxy({
        argv: ["node", "supportClaw", "channels", "status", "--probe"],
        commandPath: ["channels", "status"],
      }),
    ).toBe("default");
  ...
  it("keeps config-only agent commands on config-only startup", () => {
    ...
    expect(
      agentPolicy.loadPlugins({
        argv: ["node", "supportClaw", "agent", "--json"],
        commandPath: ["agent"],
        jsonOutputMode: true,
      }),
    ).toBe(false);
    expect(
      agentPolicy.loadPlugins({
        argv: ["node", "supportClaw", "agent", "--local"],
        commandPath: ["agent"],
        jsonOutputMode: true,
      }),
    ).toBe(true);
  ...
  it("defaults unknown command paths to network proxy routing", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "googlemeet", "login"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "tool", "image_generate"])).toBe(
      "bypass",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "tools", "effective"])).toBe("bypass");
  });

  it("resolves static network proxy bypass policies from the catalog", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "status"])).toBe("bypass");
    expect(
      resolveCliNetworkProxyPolicy(["node", "supportClaw", "config", "get", "proxy.enabled"]),
    ).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "proxy", "start"])).toBe("bypass");
  });

  it("resolves mixed network proxy policies from argv-sensitive catalog entries", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "gateway"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "gateway", "run"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "gateway", "health"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "node", "run"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "node", "status"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "agent", "--local"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "agent", "run"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "channels", "status"])).toBe("bypass");
    expect(
      resolveCliNetworkProxyPolicy(["node", "supportClaw", "channels", "status", "--probe"]),
    ).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "models", "status"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "models", "status", "--probe"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "skills", "info", "browser"])).toBe(
      "bypass",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "skills", "check"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "skills", "list"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "skills", "search", "browser"])).toBe(
      "default",
    );
  });
  ...
  it("uses the longest catalog command path for deep network proxy overrides", async () => {
    ...
    expect(
      resolveCliCatalogCommandPathLocal(["node", "supportClaw", "nodes", "camera", "snap"]),
    ).toEqual(["nodes", "camera", "snap"]);
    expect(resolveCliNetworkProxyPolicyLocal(["node", "supportClaw", "nodes", "camera", "snap"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicyLocal(["node", "supportClaw", "nodes", "camera", "list"])).toBe(
      "bypass",
    );
  });
  ...
  it("stops catalog command path resolution before positional arguments", () => {
    expect(
      resolveCliCatalogCommandPath(["node", "supportClaw", "config", "get", "proxy.enabled"]),
    ).toEqual(["config", "get"]);
    expect(
      resolveCliCatalogCommandPath(["node", "supportClaw", "message", "send", "--to", "demo"]),
    ).toEqual(["message"]);
  });

  it("treats bare gateway invocations with options as the gateway runtime", () => {
    const argv = ["node", "supportClaw", "gateway", "--port", "1234"];
  ...
  it("resolves gateway runs after root options with values", () => {
    const argv = ["node", "supportClaw", "--log-level", "debug", "gateway", "run"];
  ...
  it("does not let gateway run option values spoof bypass subcommands", () => {
    for (const argv of [
      ["node", "supportClaw", "gateway", "--token", "status"],
      ["node", "supportClaw", "gateway", "--token=status"],
      ["node", "supportClaw", "gateway", "--password", "health"],
      ["node", "supportClaw", "gateway", "--password-file", "status"],
      ["node", "supportClaw", "gateway", "--ws-log", "compact"],
    ]) {
  ...
  it("still resolves real gateway bypass subcommands after their command token", () => {
    expect(resolveCliCatalogCommandPath(["node", "supportClaw", "gateway", "status"])).toEqual([
      "gateway",
      "status",
    ]);
    expect(
      resolveCliCatalogCommandPath(["node", "supportClaw", "gateway", "status", "--token", "secret"]),
    ).toEqual(["gateway", "status"]);
    expect(resolveCliNetworkProxyPolicy(["node", "supportClaw", "gateway", "status"])).toBe("bypass");
  });
});
