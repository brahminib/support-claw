...
describe("pickTagline", () => {
  ...
  it("keeps SUPPORT_CLAW_TAGLINE_INDEX behavior in random mode", () => {
    const value = pickTagline({
      mode: "random",
      env: { SUPPORT_CLAW_TAGLINE_INDEX: "0" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(
  ...
  it("ignores partial SUPPORT_CLAW_TAGLINE_INDEX values", () => {
    expect(
      pickTagline({
        mode: "random",
        env: { SUPPORT_CLAW_TAGLINE_INDEX: "1abc" } as NodeJS.ProcessEnv,
        random: () => 0,
      }),
