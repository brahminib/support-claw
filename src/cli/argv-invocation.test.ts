...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "supportClaw", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "supportClaw", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "supportClaw", "--profile", "work", "gateway", "status"],
