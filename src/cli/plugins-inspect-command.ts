// `supportClaw plugins inspect`: renders plugin registry shape, capabilities, policy, diagnostics, and install records.
...
/** Options accepted by `supportClaw plugins inspect`. */
...
export async function runPluginsInspectCommand(
  ...
  if (!inspect) {
    defaultRuntime.error(
      formatMissingPluginMessage({ id, listCommand: "supportClaw plugins list --json" }),
    );
    return defaultRuntime.exit(1);
  }
  ...
  lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "supportClaw"}`);
