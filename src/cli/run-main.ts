...
function createGatewayCliMainStartupTrace(argv: string[]) {
  ...
  const enabled =
    isTruthyEnvValue(process.env.SUPPORT_CLAW_GATEWAY_STARTUP_TRACE) &&
...
function shouldBootstrapCliProxyBeforeFastPath(env: NodeJS.ProcessEnv = process.env): boolean {
  if (
    isTruthyEnvValue(env.SUPPORT_CLAW_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.SUPPORT_CLAW_DEBUG_PROXY_REQUIRE)
  ) {
    return true;
  }
...
function createAllowlistAgnosticCliLookupConfig(config: SupportClawConfig): SupportClawConfig {
...
async function resolveUnownedCliPrimaryMessage(params: {
  ...
  return [
    `Unknown command: supportClaw ${params.primary}. No built-in command or plugin CLI metadata owns "${params.primary}".`,
    suggestion,
  ]
    .filter(Boolean)
    .join("\n");
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.SUPPORT_CLAW_CONTAINER) ?? null;
  ...
  let bestEffortConfigPromise: Promise<SupportClawConfig> | null = null;
  const isolateProxyConfigEnv = isGatewayRunInvocation;
  const readBestEffortCliConfig = async (): Promise<SupportClawConfig> => {
  ...
  const replaceStartedProxy = async (config: SupportClawConfig["proxy"]) => {
  ...
  try {
    ...
    // `supportClaw <typo> --help` surfaces the same Unknown command error as
    // `supportClaw <typo>` instead of silently showing generic top-level help.
    ...
    if (shouldRunBareRootCrestodian) {
      ...
      if (!process.stdin.isTTY || !process.stdout.isTTY) {
        console.error(
          'Crestodian needs an interactive TTY. Use `supportClaw crestodian --message "status"` for one command.',
        );
        process.exitCode = 1;
        return;
      }
    ...
    try {
      ...
      process.on("uncaughtException", (error) => {
        ...
        if (isBenignUncaughtExceptionError(error)) {
          console.warn(
            "[supportClaw] Non-fatal uncaught exception (continuing):",
            formatUncaughtError(error),
          );
          return;
        }
        for (const line of formatCliFailureLines({
          title: "SupportClaw hit an unexpected runtime error.",
          error,
          argv: normalizedArgv,
        })) {
          console.error(line);
        ...
        for (const message of runFatalErrorHooks({ reason: "uncaught_exception", error })) {
          console.error("[supportClaw]", message);
        }
