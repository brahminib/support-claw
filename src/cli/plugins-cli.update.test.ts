...
function primeUpdateConfigSnapshot(params: {
  ...
  configPath?: string;
  loadedConfig?: SupportClawConfig;
  ...
  const configPath = params.configPath ?? path.join(process.cwd(), "supportClaw.json5");
...
function primeBlockedUpdateConfig(section: "hooks" | "plugins", config: SupportClawConfig): void {
...
describe("plugins cli update", () => {
  ...
  it("uses the mutation-start snapshot for updater input and hook selection", async () => {
        ...
        internal: {
          installs: {
            "new-hooks": {
              ...
              installPath: "~/.supportClaw/hooks/new-hooks",
            },
          },
        },
      },
          ...
          internal: {
            installs: {
              "new-hooks": {
                source: "npm",
                spec: "@acme/new-hooks@1.0.0",
                installPath: "/home/test/.supportClaw/hooks/new-hooks",
              },
            },
          },
        },
    ...
    updateNpmInstalledPlugins.mockImplementation(async (params: { config: SupportClawConfig }) => ({
      config: params.config,
      changed: false,
      outcomes: [],
    }));
    updateNpmInstalledHookPacks.mockImplementation(async (params: { config: SupportClawConfig }) => ({
      config: params.config,
      changed: false,
      outcomes: [],
    }));
  ...
  it("allows index-only legacy id migration when an included plugins section has no references", async () => {
    const cfg = { plugins: {} } as SupportClawConfig;
    const pluginRecords = createTrackedPluginConfig({
      pluginId: "voice-call",
      spec: "@supportclaw/voice-call@1.0.0",
    }).plugins?.installs;
    const nextConfig = {
      ...cfg,
      plugins: {
        ...
        installs: {
          "@supportclaw/voice-call": {
            source: "npm",
            spec: "@supportclaw/voice-call@1.1.0",
          },
        },
      },
    } as SupportClawConfig;
      ...
      outcomes: [
        {
          ...
          status: "updated",
          message: "Updated @supportclaw/voice-call.",
        },
      ],
    });
  ...
  it.each([
    {
      ...
      record: {
        source: "clawhub",
        spec: "clawhub:@supportclaw/voice-call",
        clawhubPackage: "@supportclaw/voice-call",
        installPath: "/tmp/voice-call",
      },
    },
    {
      ...
      record: {
        source: "git",
        spec: "https://github.com/supportClaw/voice-call.git",
        installPath: "/tmp/voice-call",
      },
    },
    {
  ...
  it("blocks possible legacy id migration when an included plugins section is unresolved", async () => {
    ...
    primeUpdateConfigSnapshot({
      ...
      sourceConfig: { plugins: { $include: externalPath } } as unknown as SupportClawConfig,
      includeFileTargetsForWrite: {
        [externalPath]: externalPath,
      },
    });
  ...
  it("preserves an include-owned plugins section during legacy-record cleanup", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-plugin-update-"));
  ...
  it("passes dangerous force unsafe install to plugin updates", async () => {
    ...
    expect(updateParams.pluginIds).toEqual(["supportClaw-codex-app-server"]);
