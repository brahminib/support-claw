...
const DEFAULT_CLI_NAME = "supportClaw";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(supportClaw)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `supportClaw`. */
...
/** Replace a leading `supportClaw` command prefix with the active CLI name. */
