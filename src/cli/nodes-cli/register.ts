    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw nodes status", "List known nodes with live status."],
          ["supportClaw nodes pairing pending", "Show pending node pairing requests."],
          ["supportClaw nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'supportClaw nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["supportClaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.supportClaw.ai/cli/nodes")}\n`,
    );
