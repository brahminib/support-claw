...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      SUPPORT_CLAW_GATEWAY_PORT: String(params.port),
      ...(params.wrapperPath ? { SUPPORT_CLAW_WRAPPER: params.wrapperPath } : {}),
      ...(params.token ? { SUPPORT_CLAW_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-daemon-cli-"));
    ...
    process.env.SUPPORT_CLAW_STATE_DIR = tmpDir;
    process.env.SUPPORT_CLAW_CONFIG_PATH = path.join(tmpDir, "supportClaw.json");
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-daemon-state",
        SUPPORT_CLAW_CONFIG_PATH: "/tmp/supportClaw-daemon-state/supportClaw.json",
        SUPPORT_CLAW_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.supportClaw.gateway.plist",
    });
  ...
  it("passes the existing service environment into the install plan on forced reinstall", async () => {
    ...
    expect((installPlanParams.env as NodeJS.ProcessEnv).SUPPORT_CLAW_WRAPPER).toBe(
      "/usr/local/bin/supportClaw-doppler",
    );
  });
