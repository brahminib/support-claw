...
import type { SupportClawConfig, ConfigFileSnapshot } from "../config/types.supportClaw.js";
...
const loadInstalledPluginIndexInstallRecords = vi.fn(
  async (params: { config?: SupportClawConfig } = {}) => params.config?.plugins?.installs ?? {},
);
...
const launchdUpdateCleanupMocks = vi.hoisted(() => ({
  disableCurrentSupportClawUpdateLaunchdJob: vi.fn(async () => false),
}));
...
vi.mock("../infra/supportClaw-root.js", () => ({
  resolveSupportClawPackageRoot: vi.fn(),
  resolveSupportClawPackageRootSync: vi.fn(() => process.cwd()),
}));

vi.mock("../config/config.js", () => ({
  assertConfigWriteAllowedInCurrentMode: () => {
    if (process.env.SUPPORT_CLAW_NIX_MODE === "1") {
      throw new Error(
        [
          ...
          "Agent-first Nix setup: https://github.com/supportClaw/nix-supportClaw#quick-start",
          "SupportClaw Nix overview: https://docs.supportClaw.ai/install/nix",
        ].join("\n"),
      );
    }
  },
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/supportClaw-config",
  };
});
...
vi.mock("../daemon/launchd.js", async (importOriginal) => ({
  ...
  disableCurrentSupportClawUpdateLaunchdJob:
    launchdUpdateCleanupMocks.disableCurrentSupportClawUpdateLaunchdJob,
}));
...
const { resolveSupportClawPackageRoot } = await import("../infra/supportClaw-root.js");
const {
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/supportClaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as SupportClawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/supportClaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveSupportClawPackageRoot).mockResolvedValue(root);
  ...
  const stripSupportClawPackageAlias = (spec: string) => {
    const trimmed = spec.trim();
    return trimmed.toLowerCase().startsWith("supportClaw@")
      ? trimmed.slice("supportClaw@".length)
      : trimmed;
  };

  const isNpmGitPackageSpec = (spec: string) => {
    const target = stripSupportClawPackageAlias(spec);
  ...
  const syncPluginCall = (index = 0) => {
    const calls = syncPluginsForUpdateChannel.mock.calls as unknown as Array<
      [{ channel?: string; config?: SupportClawConfig }]
    >;
  ...
  const npmPluginUpdateCall = (index = 0) => {
    const calls = updateNpmInstalledPlugins.mock.calls as unknown as Array<
      [{ config?: SupportClawConfig; timeoutMs?: number }]
    >;
    return calls[index]?.[0];
  };
  ...
  const setupConfigMutationWithRetryMock = () => {
    vi.mocked(mutateConfigFileWithRetry).mockImplementation(async (params) => {
      ...
      const nextConfig = structuredClone(snapshot.sourceConfig) as SupportClawConfig;
  ...
  const expectPackageInstallSpec = (spec: string) => {
    ...
    if (isNpmGitPackageSpec(spec)) {
      ...
      installSpec = path.join(packDir, "supportClaw-9999.0.0.tgz");
    } else {
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("supportClaw-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("supportClaw-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveSupportClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        if (destination) {
          await fs.writeFile(path.join(destination, "supportClaw-9999.0.0.tgz"), "packed\n", "utf8");
        }
      }
      return {
        stdout: "",
    ...
    readPackageName.mockResolvedValue("supportClaw");
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["supportClaw", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/supportClaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "supportClaw-gateway" }],
      hints: [],
    });
    ...
    checkShellCompletionStatus.mockResolvedValue({
      ...
      cacheExists: false,
      cachePath: "/tmp/supportClaw-completion.zsh",
    ...
    launchdUpdateCleanupMocks.disableCurrentSupportClawUpdateLaunchdJob.mockReset();
    launchdUpdateCleanupMocks.disableCurrentSupportClawUpdateLaunchdJob.mockResolvedValue(false);
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("supportClaw-completion-timeout");
    ...
    expect(call?.[1]).toEqual([path.join(root, "supportClawCli.mjs"), "completion", "--write-state"]);
    expect(call?.[2]?.env?.SUPPORT_CLAW_COMPLETION_SKIP_PLUGIN_COMMANDS).toBe("1");
  ...
  it("disarms legacy launchd updater jobs before refusing mutating updates in Nix mode", async () => {
    await withEnvAsync({ SUPPORT_CLAW_NIX_MODE: "1" }, async () => {
      await expect(updateCommand({ yes: true })).rejects.toThrow("SUPPORT_CLAW_NIX_MODE=1");
    });

    expect(launchdUpdateCleanupMocks.disableCurrentSupportClawUpdateLaunchdJob).toHaveBeenCalledOnce();
    expect(runGatewayUpdate).not.toHaveBeenCalled();
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("supportClaw-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logOutput).toContain("supportClaw completion --write-state");
  ...
  it("keeps update completion refresh best-effort when profile install fails", async () => {
    ...
    expect(installCompletion).toHaveBeenCalledWith("zsh", true, "supportClaw");
    const logOutput = vi
      .mocked(runtimeCapture.log)
      .mock.calls.map((call) => String(call[0]))
      .join("\n");
  ...
  it("respawns into the updated package root before running post-update tasks", async () => {
    ...
    expect(call?.[2]?.env?.SUPPORT_CLAW_UPDATE_IN_PROGRESS).toBe("1");
    expect(call?.[2]?.env?.SUPPORT_CLAW_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.SUPPORT_CLAW_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION).toBe("1.0.0");
  ...
  it("finishes package updates when the post-core process writes a result but keeps handles open", async () => {
    ...
    spawn.mockImplementationOnce((_command: unknown, _argv: unknown, options: unknown) => {
      const resultPath = (options as { env?: NodeJS.ProcessEnv }).env
        ?.SUPPORT_CLAW_UPDATE_POST_CORE_RESULT_PATH;
      if (!resultPath) {
  ...
  it("does not restart a stopped managed gateway after post-core plugin errors", async () => {
    const root = createCaseDir("supportClaw-update");
            ...
            warnings: [
              {
                ...
                guidance: ["Run supportClaw update repair to retry post-update plugin repair."],
              },
            ],
            sync: {
              changed: false,
  ...
  it("does not carry gateway service markers into the post-core update process", async () => {
    ...
    expect(spawnEnv?.SUPPORT_CLAW_SERVICE_MARKER).toBeUndefined();
    expect(spawnEnv?.SUPPORT_CLAW_SERVICE_KIND).toBeUndefined();
  ...
  it("passes pre-update plugin install records into the post-core update process", async () => {
    ...
    const pluginInstallRecords = {
      demo: {
        source: "npm",
        spec: "@supportclaw/demo@1.0.0",
        installPath: "/tmp/supportClaw-demo-plugin",
      },
    } as const;
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      const recordsPath = env?.SUPPORT_CLAW_UPDATE_POST_CORE_INSTALL_RECORDS_PATH;
      const sourceConfigPath = env?.SUPPORT_CLAW_UPDATE_POST_CORE_SOURCE_CONFIG_PATH;
      if (!recordsPath) {
  ...
  it("clears stale npm resolution metadata before post-core downgrade resume", async () => {
    ...
    const pluginInstallRecords = {
      msteams: {
        source: "npm",
        spec: "@supportclaw/msteams",
        installPath: "/tmp/supportClaw-msteams-plugin",
        version: "1.0.0",
        resolvedName: "@supportclaw/msteams",
        resolvedVersion: "1.0.0",
        resolvedSpec: "@supportclaw/msteams@1.0.0",
  ...
  it("respawns into the updated git root before requested channel persistence", async () => {
    ...
    expect(call?.[2]?.env?.SUPPORT_CLAW_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.SUPPORT_CLAW_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.SUPPORT_CLAW_UPDATE_POST_CORE_REQUESTED_CHANNEL).toBe("dev");
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(syncPluginsForUpdateChannel).not.toHaveBeenCalled();
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  });
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("supportClaw-downgraded-root");
    setupUpdatedRootRefresh({
  ...
  it("pins the compatibility host version to the downgraded target during current-process post-core plugin convergence (#87914)", async () => {
    const downgradedRoot = createCaseDir("supportClaw-downgraded-compat-root");
    setupUpdatedRootRefresh({
      gatewayUpdateImpl: async () =>
        makeOkUpdateResult({
          mode: "npm",
    ...
    delete process.env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION;
    let hostVersionDuringPluginUpdate: string | undefined = "unset";
    updateNpmInstalledPlugins.mockImplementation(async () => {
      hostVersionDuringPluginUpdate = process.env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION;
    ...
    try {
      ...
      expect(process.env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION).toBeUndefined();
    } finally {
      delete process.env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION;
    }
  });
    ...
    await withEnvAsync(
      {
        ...
        SUPPORT_CLAW_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("post-core resume children exit after writing a plugin update result", async () => {
    const resultDir = createCaseDir("supportClaw-post-core-result");
    ...
    await withEnvAsync(
      {
        SUPPORT_CLAW_UPDATE_POST_CORE: "1",
        SUPPORT_CLAW_UPDATE_POST_CORE_CHANNEL: "stable",
        SUPPORT_CLAW_UPDATE_POST_CORE_RESULT_PATH: resultPath,
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("post-core resume mode uses the parent install records snapshot for missing payload warnings", async () => {
    const resultDir = createCaseDir("supportClaw-post-core-records");
    ...
    await withEnvAsync(
      {
        SUPPORT_CLAW_UPDATE_POST_CORE: "1",
        SUPPORT_CLAW_UPDATE_POST_CORE_CHANNEL: "stable",
        SUPPORT_CLAW_UPDATE_POST_CORE_INSTALL_RECORDS_PATH: recordsPath,
      },
      async () => {
  ...
  it("post-core resume mode prefers post-doctor disk install records over the stale parent snapshot", async () => {
    const resultDir = createCaseDir("supportClaw-post-core-disk-records");
    const recordsPath = path.join(resultDir, "plugin-install-records.json");
    await fs.mkdir(resultDir, { recursive: true });
    await fs.writeFile(
      recordsPath,
    ...
    const postDoctorRecords = {
      codex: {
        source: "npm",
        spec: "@supportclaw/codex@2026.5.17",
  ...
  it("post-core resume mode persists the requested update channel with the updated process", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...
      resolved: { update: { channel: "stable" } } as SupportClawConfig,
      sourceConfig: { update: { channel: "stable" } } as SupportClawConfig,
      runtimeConfig: { update: { channel: "stable" } } as SupportClawConfig,
      config: { update: { channel: "stable" } } as SupportClawConfig,
      hash: "stable-hash",
    });
    ...
    await withEnvAsync(
      {
        SUPPORT_CLAW_UPDATE_POST_CORE: "1",
        SUPPORT_CLAW_UPDATE_POST_CORE_CHANNEL: "dev",
        SUPPORT_CLAW_UPDATE_POST_CORE_REQUESTED_CHANNEL: "dev",
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("uses a fail-closed integrity policy for post-core plugin updates", async () => {
    ...
    await expect(
      onIntegrityDrift({
        pluginId: "demo",
        spec: "@supportclaw/demo@1.0.0",
        resolvedSpec: "@supportclaw/demo@1.0.0",
  ...
  it("keeps json update output successful when post-core plugin updates warn", async () => {
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @supportclaw/demo@1.0.0"
    ...
    expect(pluginWarning(jsonOutput)?.guidance).toEqual([
      "Run supportClaw update repair to retry post-update plugin repair.",
      "Run supportClaw plugins inspect demo --runtime --json for details.",
    ]);
  ...
  it("detects missing plugin payloads from persisted records before npm updates", async () => {
    const installPath = createCaseDir("supportClaw-missing-plugin-payload");
  ...
  it("prints non-fatal plugin warnings in human update output", async () => {
    ...
    expect(logs).toContain("Run supportClaw update repair to retry post-update plugin repair.");
    expect(logs).toContain("Run supportClaw plugins inspect demo --runtime --json for details.");
  });
      ...
      outcomes: [
        {
          ...
          status: "skipped",
          message:
            'Disabled "demo" after plugin update failure; SupportClaw will continue without it. Failed to update demo: registry timeout',
        },
      ],
    });
    vi.mocked(defaultRuntime.writeJson).mockClear();
  ...
  it("preserves fresh-process plugin warning details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(() => {
        void (async () => {
          const resultPath = env?.SUPPORT_CLAW_UPDATE_POST_CORE_RESULT_PATH;
          if (resultPath) {
            await fs.writeFile(
              resultPath,
              JSON.stringify({
                ...
                warnings: [
                  {
                    ...
                    message:
                      'Plugin "demo" could not be processed after the core update: Failed to update demo: registry timeout Run supportClaw update repair to retry post-update plugin repair. Run supportClaw plugins inspect demo --runtime --json for details.',
                    guidance: [
                      "Run supportClaw update repair to retry post-update plugin repair.",
                      "Run supportClaw plugins inspect demo --runtime --json for details.",
                    ],
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("SupportClaw update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "supportClaw", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as SupportClawConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expectPackageInstallSpec("supportClaw@latest");
      }
  ...
  it("allows package updates from inherited gateway service env when the managed gateway is not running", async () => {
    mockPackageInstallStatus(createCaseDir("supportClaw-update"));
    ...
    expect(defaultRuntime.error).not.toHaveBeenCalledWith(
      [
        ...
        "That path replaces the active SupportClaw dist tree while the live gateway may still lazy-load old chunks.",
        "Run `supportClaw update` from a shell outside the gateway service, or stop the gateway service first and then update.",
      ].join("\n"),
    );
    expectPackageInstallSpec("supportClaw@latest");
  });
  ...
  it("refuses package updates from inherited gateway service env when --no-restart leaves the gateway running", async () => {
    ...
    serviceReadCommand.mockResolvedValue({
      programArguments: ["supportClaw", "gateway", "run"],
      environment: {
        SUPPORT_CLAW_SERVICE_MARKER: "supportClaw",
        SUPPORT_CLAW_SERVICE_KIND: "gateway",
      },
  ...
  it("refuses package updates from inside the active gateway process tree", async () => {
    ...
    const errors = vi.mocked(defaultRuntime.error).mock.calls.map((call) => String(call[0]));
    expect(errors.join("\n")).toContain(
      "supportClaw update detected it is running inside the gateway process tree.",
    );
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g supportClaw` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:supportClaw/supportClaw#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("supportClaw-update"));
        await updateCommand({ yes: true, tag: "github:supportClaw/supportClaw#main" });
      },
      expectedSpec: "github:supportClaw/supportClaw#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("supportClaw-update"));
        await updateCommand({ yes: true, tag: "SupportClaw@github:supportClaw/supportClaw#main" });
      },
      expectedSpec: "SupportClaw@github:supportClaw/supportClaw#main",
    },
    {
      name: "full git URL package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("supportClaw-update"));
        await updateCommand({ yes: true, tag: "https://github.com/brahminib/support-claw.git#main" });
      },
      expectedSpec: "https://github.com/brahminib/support-claw.git#main",
    },
    {
      name: "hosted GitHub URL package spec without git suffix",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("supportClaw-update"));
        await updateCommand({ yes: true, tag: "https://github.com/brahminib/support-claw#main" });
      },
      expectedSpec: "https://github.com/brahminib/support-claw#main",
    },
    {
      ...
      run: async () => {
        ...
        await updateCommand({
          yes: true,
          tag: "supportClaw@https://github.com/brahminib/support-claw#main",
        });
      },
      expectedSpec: "https://github.com/brahminib/support-claw#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("supportClaw-update"));
        await updateCommand({ yes: true, tag: "supportClaw/supportClaw#main" });
      },
      expectedSpec: "supportClaw/supportClaw#main",
    },
    {
      name: "SCP-style SSH package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("supportClaw-update"));
        await updateCommand({ yes: true, tag: "git@github.com:supportClaw/supportClaw.git#main" });
      },
      expectedSpec: "git@github.com:supportClaw/supportClaw.git#main",
    },
    {
      name: "SUPPORT_CLAW_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("supportClaw-update"));
        await withEnvAsync(
          { SUPPORT_CLAW_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/supportClaw-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/supportClaw-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "supportClaw");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "supportClaw", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await createTrackedTempDir("supportClaw-update-staged-fail-");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "supportClaw", version: "2026.4.20" }),
      "utf-8",
    );
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "supportClaw");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "supportClaw", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await createTrackedTempDir("supportClaw-update-package-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "supportClaw");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "supportClaw", version: "2026.4.21" }),
      "utf-8",
    );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.SUPPORT_CLAW_UPDATE_IN_PROGRESS,
    ).toBe("1");
  });

  it("continues package post-core work for explicit post-update doctor advisories", async () => {
    const tempDir = await createTrackedTempDir("supportClaw-update-package-doctor-warning-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "supportClaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    expect(postCoreCall?.[2]?.env?.SUPPORT_CLAW_UPDATE_POST_CORE).toBe("1");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
    const jsonOutput = lastWriteJsonCall() as UpdateRunResult | undefined;
    const doctorStep = jsonOutput?.steps.find((step) => step.name === "supportClaw doctor");
    expect(jsonOutput?.status).toBe("ok");
  ...
  it("fails package updates when the post-update doctor is killed after verification", async () => {
    const tempDir = await createTrackedTempDir("supportClaw-update-package-doctor-timeout-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "supportClaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
  ...
  it("runs package post-update doctor from the verified package root after a staged swap", async () => {
    const tempDir = await createTrackedTempDir("supportClaw-update-staged-doctor-");
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (argv[0] === "npm" && argv[1] === "i" && argv.includes("--prefix")) {
        ...
        await fs.writeFile(
          ...
          JSON.stringify({ name: "supportClaw", version: "2026.5.14" }),
          "utf-8",
        );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION,
    ).toBe("2026.5.14");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });

  it("stops a running managed gateway before package replacement", async () => {
    const tempDir = await createTrackedTempDir("supportClaw-update-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "supportClaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    const serviceStopCall = serviceStop.mock.calls[0]?.[0] as
      ...
      | undefined;
    expect(serviceStopCall?.env?.SUPPORT_CLAW_SERVICE_MARKER).toBe("supportClaw");
    expect(serviceStopCall?.env?.SUPPORT_CLAW_SERVICE_KIND).toBe("gateway");
  ...
  it("stops a running managed git gateway when wrapper commands hide the service root", async () => {
    const wrapperPath = path.join(
      createCaseDir("supportClaw-update-wrapper-service"),
      "gateway-wrapper",
    );
    serviceReadCommand.mockResolvedValue({
  ...
  it("stops a managed gateway rooted at the git checkout when switching package installs to dev", async () => {
    const packageRoot = createCaseDir("supportClaw-update-package-root");
    const gitRoot = await createTrackedTempDir("supportClaw-update-git-service-root-");
    ...
    await withEnvAsync({ SUPPORT_CLAW_GIT_DIR: gitRoot }, async () => {
  ...
  it("stops a managed gateway rooted at the package install when switching package installs to dev", async () => {
    const packageRoot = await createTrackedTempDir("supportClaw-update-package-service-root-");
  ...
  it("does not stop or restart a managed gateway owned by another git checkout", async () => {
    const otherRoot = await createTrackedTempDir("supportClaw-update-other-service-root-");
  ...
  it("keeps managed service stop output off stdout during json package updates", async () => {
    const tempDir = await createTrackedTempDir("supportClaw-update-json-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "supportClaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    ...
    serviceStop.mockImplementationOnce(async (params: { stdout?: NodeJS.WritableStream }) => {
      params.stdout?.write("Stopped systemd service: supportClaw-gateway.service\n");
    });
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
  ...
  it("disarms legacy launchd updater jobs before stopping the gateway", async () => {
    const tempDir = await createTrackedTempDir("supportClaw-update-launchd-loop-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "supportClaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    launchdUpdateCleanupMocks.disableCurrentSupportClawUpdateLaunchdJob.mockResolvedValue(true);
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
        return true;
    ...
    const cleanupOrder =
      launchdUpdateCleanupMocks.disableCurrentSupportClawUpdateLaunchdJob.mock.invocationCallOrder[0];
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await createTrackedTempDir("supportClaw-update-current-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "supportClaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "supportClaw", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
    ...
    expect(postCoreSpawn?.[2].env?.SUPPORT_CLAW_UPDATE_POST_CORE).toBe("1");
    expect(postCoreSpawn?.[2].env?.SUPPORT_CLAW_UPDATE_POST_CORE_CHANNEL).toBe("stable");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(
      vi
        .mocked(defaultRuntime.log)
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await createTrackedTempDir("supportClaw-update-optional-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "supportClaw");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "supportClaw", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses the owning npm binary for package updates when PATH npm points elsewhere", async () => {
    ...
    const pkgRoot = path.join(brewRoot, "supportClaw");
    ...
    const installCall = vi
      ...
      .mock.calls.find(
        ([argv]) =>
          ...
          argv[1] === "i" &&
          argv[2] === "-g" &&
          argv.includes("supportClaw@latest"),
      );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("supportClaw-update");
    const localAppData = createCaseDir("supportClaw-localappdata");
  ...
  it("warns when a package update targets a managed service root outside the shell root", async () => {
    const shellRoot = createCaseDir("supportClaw-shell-root");
    const serviceRoot = await createTrackedTempDir("supportClaw-service-root-");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "supportClaw", version: "2026.5.18" }),
      "utf-8",
    );
    ...
    expect(logs).toContain(
      `Shell SupportClaw root differs from the managed gateway service root: ${shellRoot}`,
    );
    expect(logs).toContain("make sure `supportClaw` on PATH resolves to the managed service root");
  ...
  it("runs managed service package follow-up commands with the service Node", async () => {
    const shellRoot = createCaseDir("supportClaw-shell-root");
    const servicePrefix = await createTrackedTempDir("supportClaw-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const serviceRoot = path.join(nodeModules, "supportClaw");
  ...
  it("uses the managed service Node when package roots match but node binaries differ", async () => {
    const root = createCaseDir("supportClaw-same-root");
  ...
  it("uses the managed service Node for follow-up commands when roots match but nodes differ", async () => {
    const servicePrefix = await createTrackedTempDir("supportClaw-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "supportClaw");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const serviceNpm = path.join(servicePrefix, "bin", "npm");
    const entrypoint = path.join(root, "dist", "index.js");
    await fs.mkdir(path.dirname(entrypoint), { recursive: true });
  ...
  it("pins package install to the service root when nodes differ and no owning npm exists at the prefix", async () => {
    const servicePrefix = await createTrackedTempDir("supportClaw-no-npm-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "supportClaw");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const entrypoint = path.join(root, "dist", "index.js");
  ...
  it("does not repair legacy config during a dry run", async () => {
    ...
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(runCommandWithTimeout).not.toHaveBeenCalled();
    expect(launchdUpdateCleanupMocks.disableCurrentSupportClawUpdateLaunchdJob).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("refreshes post-doctor config before post-update plugin sync", async () => {
    const tempDir = createCaseDir("supportClaw-update");
    mockPackageInstallStatus(tempDir);
    const preUpdateConfig = { update: { channel: "stable" } } as SupportClawConfig;
    ...
    syncPluginsForUpdateChannel.mockImplementation(async ({ config }) => ({
      ...
      config: {
        ...
        plugins: {
          ...config.plugins,
          load: { paths: ["/tmp/supportClaw-updated-plugin"] },
        },
      },
      summary: {
        switchedToBundled: [],
    ...
    const syncConfig = syncPluginCall()?.config as
      | (SupportClawConfig & { meta?: { lastTouchedVersion?: string } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          baseHash?: string;
          nextConfig?: SupportClawConfig & { meta?: { lastTouchedVersion?: string } };
        }
      | undefined;
  ...
  it("restores pre-update channels when post-core resume sees post-doctor config without them", async () => {
    const tempDir = createCaseDir("supportClaw-update");
    const configPath = path.join(tempDir, "supportClaw.json");
    const preUpdateConfig = {
      update: { channel: "stable" },
      channels: {
        whatsapp: {
    ...
    await withEnvAsync(
      {
        SUPPORT_CLAW_UPDATE_POST_CORE: "1",
        SUPPORT_CLAW_UPDATE_POST_CORE_CHANNEL: "stable",
        SUPPORT_CLAW_UPDATE_POST_CORE_STARTED_AT_MS: String(updateStartedAtMs),
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
  ...
  it("persists authored channel values when post-core restore input is resolved", async () => {
    ...
    await withEnvAsync(
      {
        SUPPORT_CLAW_UPDATE_POST_CORE: "1",
        SUPPORT_CLAW_UPDATE_POST_CORE_CHANNEL: "stable",
        SUPPORT_CLAW_UPDATE_POST_CORE_SOURCE_CONFIG_PATH: sourceConfigPath,
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
    ...
    const syncConfig = syncPluginCall()?.config as
      | (SupportClawConfig & { channels?: { whatsapp?: { token?: string } } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          nextConfig?: SupportClawConfig & {
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("supportClaw-update");
    const gitRoot = path.join(tempDir, "..", "supportClaw");
    ...
    const syncCall = syncPluginCall() as
      | { channel?: string; config?: SupportClawConfig; workspaceDir?: string }
      | undefined;
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `supportClaw update`.",
    );
    expect(serviceStop).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  ...
  it("tries the updated install restart when package service refresh fails", async () => {
    const updatedRoot = createCaseDir("supportClaw-updated-root");
  ...
  it("accepts same-version refresh failure recovery when the managed service restarts", async () => {
    ...
    await fs.writeFile(
      updatedPackageJson,
      JSON.stringify({ name: "supportClaw", version: "2026.4.24" }),
      "utf8",
    );
    setupUpdatedRootRefresh({
      entrypoints: [updatedEntrypoint],
  ...
  it("rejects same-version refresh failure recovery from a stale service definition", async () => {
    const oldRoot = createCaseDir("supportClaw-old-root");
    const updatedRoot = createCaseDir("supportClaw-updated-root");
  ...
  it("writes the control-plane update sentinel after managed package restart health passes", async () => {
    const stateDir = await createTrackedTempDir("supportClaw-update-sentinel-state-");
    const metaDir = await createTrackedTempDir("supportClaw-update-sentinel-meta-");
  ...
  it.each([
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            SUPPORT_CLAW_CONFIG_PATH: "./config/supportClaw.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedEnv: () => ({
        SUPPORT_CLAW_STATE_DIR: path.resolve("./state"),
        SUPPORT_CLAW_CONFIG_PATH: path.resolve("./config/supportClaw.json"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
    },
    {
      ...
      expectedEnv: (context?: { originalCwd: string }) => ({
        SUPPORT_CLAW_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ SUPPORT_CLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.SUPPORT_CLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("marks the whole update command as update-in-progress", async () => {
    await withEnvAsync({ SUPPORT_CLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
      ...
      vi.mocked(runGatewayUpdate).mockImplementationOnce(async () => {
        observedUpdateEnv = process.env.SUPPORT_CLAW_UPDATE_IN_PROGRESS;
  ...
  it("updateFinalizeCommand runs doctor and plugin convergence with full update env", async () => {
    await withEnvAsync(
      {
        SUPPORT_CLAW_UPDATE_IN_PROGRESS: undefined,
        SUPPORT_CLAW_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR: undefined,
        SUPPORT_CLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE: undefined,
      },
      async () => {
        ...
        expect(doctorEnv?.SUPPORT_CLAW_UPDATE_IN_PROGRESS).toBe("1");
        expect(doctorEnv?.SUPPORT_CLAW_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBe("1");
        expect(doctorEnv?.SUPPORT_CLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
        expect(process.env.SUPPORT_CLAW_UPDATE_IN_PROGRESS).toBeUndefined();
        expect(process.env.SUPPORT_CLAW_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBeUndefined();
        expect(process.env.SUPPORT_CLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBeUndefined();
  ...
  it("updateFinalizeCommand repairs doctor by default and refreshes plugin state after doctor", async () => {
    ...
    syncPluginsForUpdateChannel.mockImplementationOnce(
      async (params: { config?: SupportClawConfig }) => ({
        changed: true,
  ...
  it("updateFinalizeCommand restores channels from the RPC pre-update config payload", async () => {
    const tempDir = createCaseDir("supportClaw-rpc-finalize");
    const sourceConfigPath = path.join(tempDir, "source-config.json");
    const preUpdateConfig = {
      channels: {
        whatsapp: {
  ...
  it("updateFinalizeCommand reapplies requested channel against post-doctor config", async () => {
    const preDoctorConfig = { update: { channel: "stable" } } as SupportClawConfig;
    const postDoctorConfig = { update: { channel: "beta" } } as SupportClawConfig;
    const preDoctorSnapshot: ConfigFileSnapshot = {
      ...baseSnapshot,
      sourceConfig: preDoctorConfig,
      resolved: preDoctorConfig,
  ...
  it("updateFinalizeCommand converges on the effective channel from env without persisting update.channel", async () => {
    const noChannelConfig = {} as SupportClawConfig;
    ...
    const priorEffective = process.env.SUPPORT_CLAW_UPDATE_EFFECTIVE_CHANNEL;
    // Simulate a no-config git/source update whose effective channel is dev.
    process.env.SUPPORT_CLAW_UPDATE_EFFECTIVE_CHANNEL = "dev";
    try {
      await updateFinalizeCommand({ json: true, restart: false });
    } finally {
      if (priorEffective === undefined) {
        delete process.env.SUPPORT_CLAW_UPDATE_EFFECTIVE_CHANNEL;
      } else {
        process.env.SUPPORT_CLAW_UPDATE_EFFECTIVE_CHANNEL = priorEffective;
      }
    }
  ...
  it.each([
    ...
    },
    {
      ...
      requireTty: false,
      expectedError:
        "Update wizard requires a TTY. Use `supportClaw update --channel <stable|beta|dev>` instead.",
    },
  ] as const)(
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("supportClaw-update-wizard");
    await withEnvAsync({ SUPPORT_CLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/supportClaw as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    try {
      await withEnvAsync(
        {
          HOME: undefined,
          SUPPORT_CLAW_GIT_DIR: undefined,
          SUPPORT_CLAW_HOME: undefined,
          USERPROFILE: undefined,
        },
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "supportClaw"));
        },
      );
    } finally {
  ...
  it("uses SUPPORT_CLAW_HOME for the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    try {
      await withEnvAsync(
        { SUPPORT_CLAW_GIT_DIR: undefined, SUPPORT_CLAW_HOME: "/srv/supportClaw-home" },
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/srv/supportClaw-home", "supportClaw"));
        },
      );
    } finally {
      homedirSpy.mockRestore();
  ...
  it("creates the parent directory before cloning the default dev checkout", async () => {
    const root = await createTrackedTempDir("supportClaw-update-home-");
    const home = path.join(root, "custom-supportClaw-home");
    const checkoutDir = path.join(home, "supportClaw");

    await withEnvAsync({ SUPPORT_CLAW_GIT_DIR: undefined, SUPPORT_CLAW_HOME: home }, async () => {
    ...
    expect(cloneCall?.[0]).toEqual([
      "git",
      "clone",
      "https://github.com/brahminib/support-claw.git",
      checkoutDir,
    ]);
  });
});
