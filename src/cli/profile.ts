...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.supportClaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.SUPPORT_CLAW_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.SUPPORT_CLAW_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.SUPPORT_CLAW_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.SUPPORT_CLAW_CONFIG_PATH)) {
    env.SUPPORT_CLAW_CONFIG_PATH = path.join(stateDir, "supportClaw.json");
  }

  if (profile === "dev" && !env.SUPPORT_CLAW_GATEWAY_PORT?.trim()) {
    env.SUPPORT_CLAW_GATEWAY_PORT = "19001";
  }
}
