...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    resetRuntimeCapture();
    vi.stubEnv("SUPPORT_CLAW_GATEWAY_TOKEN", "");
    vi.stubEnv("SUPPORT_CLAW_GATEWAY_PASSWORD", "");
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("supportClaw devices approve <requestId>");
  });
  ...
  it("uses SUPPORT_CLAW_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("SUPPORT_CLAW_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
