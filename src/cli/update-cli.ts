// Commander wiring for `supportClaw update`, its status/finalize subcommands, and help text.
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw update repair", "Rerun post-update doctor and plugin convergence."],
          ["supportClaw update repair --channel beta", "Repair against the beta update channel."],
          ["supportClaw update repair --json", "JSON output for automation."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.supportClaw.ai/cli/update")}`,
    )
...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update SupportClaw and inspect update channel status")
    .option("--json", "Output result as JSON", false)
    ...
    .addHelpText("after", () => {
      const examples = [
        ["supportClaw update", "Update a source checkout (git)"],
        ["supportClaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["supportClaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["supportClaw update --tag beta", "One-off update to a dist-tag or version"],
        ["supportClaw update --tag main", "One-off package update from GitHub main"],
        ["supportClaw update --dry-run", "Preview actions without changing anything"],
        ["supportClaw update --no-restart", "Update without restarting the service"],
        ["supportClaw update --json", "Output result as JSON"],
        ["supportClaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["supportClaw update repair", "Repair stranded post-update plugin state"],
        ["supportClaw update wizard", "Interactive update wizard"],
        ["supportClaw --update", "Shorthand for supportClaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run supportClaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.supportClaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw update status", "Show channel + version status."],
          ["supportClaw update status --json", "JSON output."],
          ["supportClaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
