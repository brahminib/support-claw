...
describe("resolveCommandSecretRefsViaGateway", () => {
  function makeTalkProviderApiKeySecretRefConfig(envKey: string): SupportClawConfig {
  ...
  async function createExecProviderConfig(refId: string): Promise<{
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-command-secret-exec-"));
    tempRoots.add(root);
    ...
    const resolverScript = [
      ...
      "  fs.writeFileSync(process.env.SUPPORT_CLAW_EXEC_MARKER, 'executed');",
        ...
        secrets: {
          providers: {
            default: {
              ...
              env: { SUPPORT_CLAW_EXEC_MARKER: markerPath },
