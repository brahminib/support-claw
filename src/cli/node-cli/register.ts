    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "supportClaw node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["supportClaw node status", "Check node host service status."],
          ["supportClaw node install", "Install the node host service."],
          ["supportClaw node start", "Start the installed node host service."],
          ["supportClaw node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.supportClaw.ai/cli/node")}\n`,
    );
