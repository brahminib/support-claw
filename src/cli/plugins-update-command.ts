// `supportClaw plugins update` command implementation for tracked npm plugins and hook packs.
...
function projectUpdaterResultOntoSourceConfig(params: {
  runtimeBase: SupportClawConfig;
  sourceBase: SupportClawConfig;
  updatedConfig: SupportClawConfig;
}): SupportClawConfig {
  const updatePatch = createMergePatch(params.runtimeBase, params.updatedConfig);
  return applyMergePatch(params.sourceBase, updatePatch) as SupportClawConfig;
}
