...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.SUPPORT_CLAW_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.SUPPORT_CLAW_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.SUPPORT_CLAW_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      ...
      process.env.SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "supportClaw", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "supportClaw"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "supportClaw", "nodes", "list"], "supportClaw");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "supportClaw", "infer"], "supportClaw");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "supportClaw", "acp", "--help"], "supportClaw");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "supportClaw", "gateway", "--force"];
    process.argv = argv;
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "supportClaw", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("supportClaw");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "supportClaw", "completion", "--write-state"];
    const program = new Command().name("supportClaw");
  ...
  it.each([
    ...
    process.argv = ["node", "supportClaw", ...args];
    const program = new Command().name("supportClaw");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "supportClaw", "plugins"];
    const program = new Command().name("supportClaw");
