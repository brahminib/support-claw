...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw status", "Show channel health + session summary."],
          ["supportClaw status --all", "Full diagnosis (read-only)."],
          ["supportClaw status --json", "Machine-readable output."],
          ["supportClaw status --usage", "Show model provider usage/quota snapshots."],
          [
            "supportClaw status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["supportClaw status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.supportClaw.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.supportClaw.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw sessions", "List all sessions."],
          ["supportClaw sessions --agent work", "List sessions for one agent."],
          ["supportClaw sessions --all-agents", "Aggregate sessions across agents."],
          ["supportClaw sessions --active 120", "Only last 2 hours."],
          ["supportClaw sessions --limit 25", "Show the newest 25 sessions."],
          ["supportClaw sessions --json", "Machine-readable output."],
          ["supportClaw sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.supportClaw.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "supportClaw sessions cleanup --dry-run --fix-missing",
          ...
          ["supportClaw sessions cleanup --enforce", "Apply maintenance now."],
          ["supportClaw sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["supportClaw sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "supportClaw sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .supportClaw/trajectory-exports")
  ...
  sessionsCmd
    ...
    .action(async (key: string, opts, command) => {
      ...
      // command, e.g. `supportClaw sessions --agent work compact <key>`. Merge those
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw commitments", "List pending inferred follow-ups."],
          ["supportClaw commitments --all", "List all inferred follow-ups."],
          ["supportClaw commitments --agent work", "List one agent's inferred follow-ups."],
          ["supportClaw commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
