...
export function registerMigrateCommand(program: Command) {
  const migrate = addVerifyPluginAppsOption(
    program
      ...
      .option("--no-backup", "Skip the pre-migration SupportClaw backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw migrate list", "Show available migration providers."],
          ["supportClaw migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["supportClaw migrate hermes --dry-run", "Preview Hermes migration only."],
          [
            "supportClaw migrate apply hermes --yes",
            "Apply Hermes migration non-interactively after writing a verified backup.",
          ],
          [
            "supportClaw migrate hermes --no-auth-credentials",
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing SupportClaw state"),
  ).action(async (provider, opts) => {
    await runCommandWithRuntime(defaultRuntime, async () => {
