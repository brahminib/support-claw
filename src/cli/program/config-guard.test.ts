...
describe("ensureConfigReady", () => {
  ...
  function useTempSupportClawHome(): string {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-config-guard-"));
    tempRoots.push(root);
    setTestEnvValue("SUPPORT_CLAW_HOME", root);
    deleteTestEnvValue("SUPPORT_CLAW_STATE_DIR");
    return root;
  }

  function writeLegacyTaskSidecarMarker(root: string): void {
    const markerPath = path.join(root, ".supportClaw", "tasks", "runs.sqlite");
  ...
  function writeStateMarker(root: string, relativePath: string): void {
    const markerPath = path.join(root, ".supportClaw", relativePath);
    fs.mkdirSync(path.dirname(markerPath), { recursive: true });
    fs.writeFileSync(markerPath, "{}");
  }

  beforeEach(() => {
    envSnapshot = captureEnv(["HOME", "SUPPORT_CLAW_HOME", "SUPPORT_CLAW_STATE_DIR"]);
    vi.clearAllMocks();
    resetConfigGuardStateForTests();
    for (const root of tempRoots.splice(0)) {
      fs.rmSync(root, { recursive: true, force: true });
    }
    useTempSupportClawHome();
  ...
  it("runs doctor flow when lightweight startup detection finds legacy state", async () => {
    const root = useTempSupportClawHome();
  ...
  it("runs doctor flow for legacy sessions without task sidecars", async () => {
    const root = useTempSupportClawHome();
    fs.mkdirSync(path.join(root, ".supportClaw", "sessions"), { recursive: true });
  ...
  it("uses shared tilde expansion for SUPPORT_CLAW_HOME in the startup detector", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-config-guard-home-"));
    tempRoots.push(root);
    setTestEnvValue("HOME", root);
    setTestEnvValue("SUPPORT_CLAW_HOME", "~/svc");
    deleteTestEnvValue("SUPPORT_CLAW_STATE_DIR");
  ...
  it("exits for invalid config on non-allowlisted commands", async () => {
    ...
    expect(plainErrorCalls(runtime)).toEqual([
      "SupportClaw config is invalid",
      "File: /tmp/supportClaw.json",
      "Problem:",
      "  - channels.quietchat: invalid",
      "",
      `Fix: ${formatCliCommand("supportClaw doctor --fix")}`,
      `Inspect: ${formatCliCommand("supportClaw config validate")}`,
  ...
  it("replaces doctor fix advice for plugin packaging-only invalid config", async () => {
    ...
    expect(calls).not.toContain(`Fix: ${formatCliCommand("supportClaw doctor --fix")}`);
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("runs doctor migration flow only once per module instance", async () => {
    writeLegacyTaskSidecarMarker(useTempSupportClawHome());
