  ...
  program
    ...
    .addHelpText(
      ...
      () =>
        ...
        `  ${theme.command("supportClaw setup")}\n` +
        `    ${theme.muted("Create config, workspace, and session folders.")}\n` +
        `  ${theme.command("supportClaw setup --wizard")}\n` +
        `    ${theme.muted("Run full onboarding for auth, models, Gateway, and channels.")}\n\n` +
        `${theme.muted("Docs:")} ${formatDocsLink("/cli/setup", "docs.supportClaw.ai/cli/setup")}\n`,
    )
    .option(
      "--workspace <dir>",
      "Agent workspace directory (default: ~/.supportClaw/workspace; stored as agents.defaults.workspace)",
    )
