...
function resolveHelpCommand(argv: string[] | undefined, options?: { root?: boolean }): string {
  if (options?.root || !argv) {
    return formatCliCommand("supportClaw --help");
  }
  const commandPath = getCommandPathWithRootOptions(argv, 2);
  if (commandPath.length === 0) {
    return formatCliCommand("supportClaw --help");
  }
  return formatCliCommand(`supportClaw ${commandPath.join(" ")} --help`);
}
...
function formatDocsHint(): string {
  return `${theme.muted("Docs:")} ${formatDocsLink("/cli", "docs.supportClaw.ai/cli")}`;
}

/** Convert Commander parse errors into SupportClaw-specific help and docs guidance. */
export function formatCliParseErrorOutput(
  ...
  if (unknownCommand) {
    ...
    return lines(
      theme.error(`SupportClaw does not know the command ${quote(command)}.`),
      formatCliCommandSuggestions(command),
      formatHelpHint(options.argv, { root: true }),
      `${theme.muted("Plugin command?")} ${theme.command(formatCliCommand("supportClaw plugins list"))}`,
      formatDocsHint(),
    );
  }
  ...
  if (unknownOption) {
    ...
    return lines(
      theme.error(`SupportClaw does not recognize option ${quote(option)}.`),
  ...
  return lines(
    theme.error(`SupportClaw could not parse this command: ${message}`),
    formatHelpHint(options.argv),
  );
}
