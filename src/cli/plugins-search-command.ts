...
/** Options accepted by `supportClaw plugins search`. */
...
function formatPackageSearchLine(entry: ClawHubPackageSearchResult): string {
  ...
  return `${pkg.name}  ${theme.muted(flags.join(" | "))}${summary}\n  ${theme.muted(`Install: supportClaw plugins install clawhub:${pkg.name}`)}`;
}
...
export async function runPluginsSearchCommand(
  ...
  if (!query) {
    runtime.error("Usage: supportClaw plugins search <query>");
