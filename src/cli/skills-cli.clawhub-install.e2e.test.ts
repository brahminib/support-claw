...
async function spawnSupportClaw(
  args: string[],
...
describe("supportClaw skills install ClawHub GitHub-backed E2E", () => {
  it("installs from the install resolver and reports install telemetry", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-clawhub-cli-e2e-"));
    try {
      const result = await spawnSupportClaw(["skills", "install", "aiq-deploy", "--global"], {
        cwd: process.cwd(),
        env: {
          ...process.env,
          SUPPORT_CLAW_STATE_DIR: stateDir,
          SUPPORT_CLAW_CONFIG_PATH: path.join(stateDir, "supportClaw.json"),
          SUPPORT_CLAW_CLAWHUB_URL: registry,
          SUPPORT_CLAW_CLAWHUB_TOKEN: "test-token",
          SUPPORT_CLAW_CLAWHUB_GITHUB_CODELOAD_BASE_URL: registry,
