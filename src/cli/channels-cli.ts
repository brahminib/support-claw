...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw channels list", "List configured channels."],
          ["supportClaw channels list --all", "Show configured, bundled, and installable channels."],
          ["supportClaw channels add", "Open guided channel setup."],
          ["supportClaw channels status --probe", "Run channel status checks and probes."],
          [
            "supportClaw channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["supportClaw channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.supportClaw.ai/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw channels add", "Open guided setup for available chat channels."],
          [
            "supportClaw channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["supportClaw channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
