...
import { detectMime, extensionForMime, normalizeMimeType } from "@supportclaw/media-core/mime";
import {
...
const LOCAL_MODEL_RUN_SYSTEM_PROMPT = "You are a personal assistant running inside SupportClaw.";
...
function getAuthProfileIdsForProvider(cfg: SupportClawConfig, providerId: string): string[] {
...
function pinRuntimeConfigSnapshot(config: SupportClawConfig): void {
...
export function registerCapabilityCli(program: Command) {
  ...
  const capability = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/infer", "docs.supportClaw.ai/cli/infer")}\n`,
    );
