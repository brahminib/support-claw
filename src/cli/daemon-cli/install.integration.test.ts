...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("supportClaw-daemon-install-int-");
    configPath = path.join(tempHome, "supportClaw.json");
    process.env.HOME = tempHome;
    process.env.SUPPORT_CLAW_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer SupportClaw", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.SUPPORT_CLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
