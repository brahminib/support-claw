...
type DaemonConfigContext = {
  ...
  cliCfg: SupportClawConfig;
  daemonCfg: SupportClawConfig;
...
function resolveSnapshotRuntimeConfig(snapshot: ConfigFileSnapshot | null): SupportClawConfig | null {
...
function coerceStatusConfig(value: unknown): SupportClawConfig {
...
export type DaemonStatus = {
  ...
  service: {
    ...
    staleUpdateLaunchdJobs?: StaleSupportClawUpdateLaunchdJob[];
  };
  config?: {
    cli: ConfigSummary;
   ...
   * can happen after `npm install -g supportClaw@<v>` updates the gateway binary
   * without a corresponding `supportClaw plugins update`.
   */
  ...
  const staleUpdateLaunchdJobs =
    opts.deep && process.platform === "darwin"
      ? await loadLaunchdModule()
          .then(({ findStaleSupportClawUpdateLaunchdJobs }) =>
            findStaleSupportClawUpdateLaunchdJobs(serviceEnv),
          )
          .catch(() => [])
      : [];
