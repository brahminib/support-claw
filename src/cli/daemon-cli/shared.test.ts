...
describe("renderGatewayServiceStartHints", () => {
  ...
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        SUPPORT_CLAW_CONTAINER: "supportClaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("supportClaw-demo-container");
    expect(
      resolveDaemonContainerContext({
        SUPPORT_CLAW_CONTAINER_HINT: "supportClaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("supportClaw-demo-container");
  });

  it("prepends a single container restart hint when SUPPORT_CLAW_CONTAINER is set", () => {
    expect(
      renderGatewayServiceStartHints({
        SUPPORT_CLAW_CONTAINER: "supportClaw-demo-container",
      } as NodeJS.ProcessEnv),
  ...
  it("prepends a single container restart hint when SUPPORT_CLAW_CONTAINER_HINT is set", () => {
    expect(
      renderGatewayServiceStartHints({
        SUPPORT_CLAW_CONTAINER_HINT: "supportClaw-demo-container",
      } as NodeJS.ProcessEnv),
...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when SUPPORT_CLAW_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
          "If you're in a container, run the gateway in the foreground instead of `supportClaw gateway`.",
        ],
        { SUPPORT_CLAW_CONTAINER: "supportClaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });

  it("drops the generic container foreground hint when SUPPORT_CLAW_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
        ...
        { SUPPORT_CLAW_CONTAINER_HINT: "supportClaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });
});
