...
async function confirmGatewayReachable(params: {
  ...
  const token = normalizeOptionalString(params.auth?.token ?? process.env.SUPPORT_CLAW_GATEWAY_TOKEN);
  const password = normalizeOptionalString(
    params.auth?.password ?? process.env.SUPPORT_CLAW_GATEWAY_PASSWORD,
  );
