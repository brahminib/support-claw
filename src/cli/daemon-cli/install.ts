...
import { SUPPORT_CLAW_WRAPPER_ENV_KEY, resolveSupportClawWrapperPath } from "../../daemon/program-args.js";
...
export function mergeInstallInvocationEnv(params: {
  ...
  for (const [rawKey, rawValue] of Object.entries(params.existingServiceEnv)) {
    ...
    if (upper === SUPPORT_CLAW_WRAPPER_ENV_KEY) {
      const value = rawValue.trim();
      if (value) {
        preservedServiceEnv[normalizeInstallEnvKey(SUPPORT_CLAW_WRAPPER_ENV_KEY)] = value;
      }
      continue;
    }
    if (
      ...
      upper === "PATH" ||
      upper === "TMPDIR" ||
      upper.startsWith("SUPPORT_CLAW_")
    ) {
      continue;
    }
...
export async function runDaemonInstall(opts: DaemonInstallOptions) {
  ...
  if (opts.wrapper !== undefined) {
    try {
      wrapperPath = await resolveSupportClawWrapperPath(opts.wrapper);
      if (!wrapperPath) {
  ...
  if (!wrapperPath) {
    try {
      wrapperPath = await resolveSupportClawWrapperPath(installEnv[SUPPORT_CLAW_WRAPPER_ENV_KEY]);
    } catch (err) {
      fail(`Invalid ${SUPPORT_CLAW_WRAPPER_ENV_KEY}: ${String(err)}`);
      return;
    }
  }
  if (loaded) {
    if (!opts.force) {
      ...
      if (autoRefreshMessage) {
        ...
        if (!json) {
          ...
          defaultRuntime.log(
            `Reinstall with: ${formatCliCommand("supportClaw gateway install --force")}`,
          );
        }
        return;
      }
  ...
  try {
    ...
    if (currentEmbeddedToken) {
      ...
      const plannedEmbeddedToken = normalizeOptionalString(
        plannedInstall.environment.SUPPORT_CLAW_GATEWAY_TOKEN,
      );
      if (currentEmbeddedToken !== plannedEmbeddedToken) {
        return "Gateway service SUPPORT_CLAW_GATEWAY_TOKEN differs from the current install plan; refreshing the install.";
      }
    }
    const wrapperRequested = Boolean(
      params.wrapperPath || normalizeOptionalString(params.installEnv[SUPPORT_CLAW_WRAPPER_ENV_KEY]),
    );
    if (wrapperRequested) {
      const plannedInstall = await buildGatewayInstallPlan({
        env: params.installEnv,
      ...
      const plannedWrapperPath = normalizeOptionalString(
        plannedInstall.environment[SUPPORT_CLAW_WRAPPER_ENV_KEY],
      );
      const currentWrapperPath = normalizeOptionalString(
        currentCommand.environment?.[SUPPORT_CLAW_WRAPPER_ENV_KEY],
      );
      if (plannedWrapperPath !== currentWrapperPath) {
        return `Gateway service ${SUPPORT_CLAW_WRAPPER_ENV_KEY} differs from the current wrapper install plan; refreshing the install.`;
      }
    }
