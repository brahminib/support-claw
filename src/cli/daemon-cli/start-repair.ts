...
export async function repairLoadedGatewayServiceForStart(params: {
  ...
  const wrapperPath = await resolveSupportClawWrapperPath(installEnv[SUPPORT_CLAW_WRAPPER_ENV_KEY]);
  ...
  return {
    result: "started",
    message:
      "Gateway service definition repaired and started. Reopen the Control UI with `supportClaw dashboard` or copy a fresh auth URL with `supportClaw dashboard --no-open`.",
