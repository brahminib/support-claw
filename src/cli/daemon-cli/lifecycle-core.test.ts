...
const loadConfig = vi.fn<() => SupportClawConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
function stubServiceGatewayTokenEnv() {
  service.readCommand.mockResolvedValue({
    programArguments: [],
    environment: {
      SUPPORT_CLAW_GATEWAY_TOKEN: "service-token",
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { SUPPORT_CLAW_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
    ...
    await withUnsupportedGatewayService(async (unsupportedService) => {
      await expect(
        runServiceStart({
          ...
          renderStartHints: () => ["supportClaw gateway install"],
          opts: { json: true },
          onNotLoaded,
        }),
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("SUPPORT_CLAW_CONTAINER_HINT", "supportClaw-demo-container");

    await runServiceRestart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => [
        "Restart the container or the service that manages it for supportClaw-demo-container.",
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for supportClaw-demo-container.",
    );
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { SUPPORT_CLAW_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("SUPPORT_CLAW_GATEWAY_TOKEN", "env-token");
  ...
  it("repairs stale loaded services during start before reporting success", async () => {
    service.readCommand.mockResolvedValue({
      programArguments: ["supportClaw", "gateway"],
    ...
    const repairLoadedService = vi.fn(async () => ({
      ...
      warnings: ["service was installed by SupportClaw 2026.4.24, current CLI is 2026.5.2"],
      loaded: true,
    }));
    ...
    expect(payload.warnings?.[0]).toContain("service was installed by SupportClaw");
    expect(payload.service?.loaded).toBe(true);
  });
  ...
  it("fails start with an install hint when a stale loaded service has no repair callback", async () => {
    ...
    expect(payload.hints).toEqual(["supportClaw gateway install --force"]);
    expect(service.restart).not.toHaveBeenCalled();
  });
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    expect(payload.hints?.includes("supportClaw gateway install")).toBe(true);
    expect(
      payload.hintItems?.some(
        (item) => item.kind === "install" && item.text === "supportClaw gateway install",
      ),
    ).toBe(true);
    expect(service.restart).not.toHaveBeenCalled();
  });
