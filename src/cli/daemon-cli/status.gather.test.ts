...
import type { StaleSupportClawUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const findStaleSupportClawUpdateLaunchdJobs = vi.fn<
  (env?: NodeJS.ProcessEnv) => Promise<StaleSupportClawUpdateLaunchdJob[]>
>(async () => []);
...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-daemon",
    SUPPORT_CLAW_CONFIG_PATH: "/tmp/supportClaw-daemon/supportClaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.SUPPORT_CLAW_STATE_DIR ?? "/tmp/supportClaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.SUPPORT_CLAW_CONFIG_PATH ?? `${stateDir}/supportClaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({
    ...
    const isDaemon = configPath.includes("/supportClaw-daemon/");
...
vi.mock("../../daemon/launchd.js", () => ({
  findStaleSupportClawUpdateLaunchdJobs: (env?: NodeJS.ProcessEnv) =>
    findStaleSupportClawUpdateLaunchdJobs(env),
}));
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    setTestEnvValue("SUPPORT_CLAW_STATE_DIR", "/tmp/supportClaw-cli");
    setTestEnvValue("SUPPORT_CLAW_CONFIG_PATH", "/tmp/supportClaw-cli/supportClaw.json");
    ...
    findStaleSupportClawUpdateLaunchdJobs.mockReset();
    findStaleSupportClawUpdateLaunchdJobs.mockResolvedValue([]);
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(probeInput.configPath).toBe("/tmp/supportClaw-daemon/supportClaw.json");
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/supportClaw-cli/supportClaw.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.SUPPORT_CLAW_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.SUPPORT_CLAW_GATEWAY_PORT ?? "missing-port",
    }));
    ...
    expect(
      serviceReadRuntime.mock.calls.some(([env]) => env?.SUPPORT_CLAW_GATEWAY_PORT === "19001"),
    ).toBe(true);
  ...
  it("surfaces recent service restart handoffs only during deep status", async () => {
    ...
    expect(handoffInput.SUPPORT_CLAW_STATE_DIR).toBe("/tmp/supportClaw-daemon");
    expect(handoffInput.SUPPORT_CLAW_CONFIG_PATH).toBe("/tmp/supportClaw-daemon/supportClaw.json");
    ...
    async () => {
      serviceReadCommand.mockResolvedValueOnce({
        ...
        environment: {
          SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-daemon",
          SUPPORT_CLAW_CONFIG_PATH: "/tmp/supportClaw-daemon/supportClaw.json",
          SUPPORT_CLAW_LAUNCHD_LABEL: "ai.supportClaw.manual-update.gateway",
        },
      });
      findStaleSupportClawUpdateLaunchdJobs.mockResolvedValueOnce([
        {
      ...
      const staleScanEnv = findStaleSupportClawUpdateLaunchdJobs.mock.calls[0]?.[0];
      expect(staleScanEnv?.SUPPORT_CLAW_STATE_DIR).toBe("/tmp/supportClaw-daemon");
      expect(staleScanEnv?.SUPPORT_CLAW_CONFIG_PATH).toBe("/tmp/supportClaw-daemon/supportClaw.json");
      expect(staleScanEnv?.SUPPORT_CLAW_LAUNCHD_LABEL).toBe("ai.supportClaw.manual-update.gateway");
  ...
  it("does not read restart handoffs during normal status", async () => {
    ...
    expect(findStaleSupportClawUpdateLaunchdJobs).not.toHaveBeenCalled();
      ...
      connections: [
        {
          ...
          ppid: 1,
          command: "node",
          commandLine: "node /tmp/newer-supportClaw/dist/index.js logs --follow",
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-status-config-"));
    const configPath = path.join(tmp, "supportClaw.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    setTestEnvValue("SUPPORT_CLAW_STATE_DIR", tmp);
    setTestEnvValue("SUPPORT_CLAW_CONFIG_PATH", configPath);
    serviceReadCommand.mockResolvedValueOnce({
      programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"],
      environment: {
        SUPPORT_CLAW_STATE_DIR: tmp,
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    setTestEnvValue("SUPPORT_CLAW_GATEWAY_TOKEN", "env-token");
    setTestEnvValue("SUPPORT_CLAW_GATEWAY_PASSWORD", "env-password"); // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "supportClaw-gateway" }],
        hints: [],
      },
      healthy: false,
  ...
  it("reads install records from the merged daemon service environment, not the CLI process env", async () => {
    ...
    // The mock daemon service command sets SUPPORT_CLAW_STATE_DIR=/tmp/supportClaw-daemon,
    // distinct from the CLI process SUPPORT_CLAW_STATE_DIR=/tmp/supportClaw-cli. Drift
