...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportClaw security audit", "Run a local security audit."],
          [
            "supportClaw security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["supportClaw security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "supportClaw security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          [
            "supportClaw security audit --auth password --password <password>",
            "Audit a runtime-only password-mode Gateway secret.",
          ],
          ["supportClaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["supportClaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.supportClaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("SupportClaw security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("supportClaw security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("supportClaw security audit --fix")}`));
        if (!fixResult) {
