...
describe("plugins cli list", () => {
  ...
  it("reports stale plugin config in doctor output without claiming full plugin health", async () => {
    ...
    expect(output).toContain(
      'Run "supportClaw doctor --fix" to remove stale plugin ids and dangling channel references.',
    );
    expect(output).toContain(
  ...
  it("reports missing configured Codex runtime plugin in doctor output", async () => {
    ...
    expect(output).toContain("supportClaw doctor --fix");
    expect(output).toContain("supportClaw plugins install @supportclaw/codex");
    expect(output).toContain(
      "No plugin install-tree issues detected; configuration warnings remain.",
    );
    expect(output).not.toContain("No plugin issues detected.");
  ...
  it("reports missing configured ACPX runtime plugin in doctor output", async () => {
    ...
    expect(output).toContain("supportClaw doctor --fix");
    expect(output).toContain("supportClaw plugins install @supportclaw/acpx");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports blocked configured ACPX runtime with ACP-specific guidance", async () => {
    ...
    expect(output).not.toContain('runtime policy to "supportClaw"');
    expect(output).not.toContain("supportClaw plugins install @supportclaw/acpx");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports configured Codex runtime when the plugin record is disabled", async () => {
    ...
    expect(output).not.toContain("supportClaw plugins install @supportclaw/codex");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports blocked configured Codex runtime without install advice", async () => {
    ...
    expect(output).not.toContain('Run "supportClaw doctor --fix" to install');
    expect(output).not.toContain("supportClaw plugins install @supportclaw/codex");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports config-selected plugin source shadowing in doctor output", async () => {
    buildPluginDiagnosticsReport.mockReturnValue({
      plugins: [
        createPluginRecord({
          id: "discord",
          origin: "config",
          source: "/tmp/supportClaw-upstream/extensions/discord/index.ts",
          status: "error",
      ...
      diagnostics: [
        {
          level: "warn",
          pluginId: "discord",
          source: "/tmp/supportClaw/npm/node_modules/@supportclaw/discord/index.ts",
          message:
            "duplicate plugin id resolved by explicit config-selected plugin; global plugin will be overridden by config plugin (/tmp/supportClaw-upstream/extensions/discord/index.ts)",
        },
      ],
    });
    ...
    expect(output).toContain("active: /tmp/supportClaw-upstream/extensions/discord/index.ts");
    expect(output).toContain("shadowed: /tmp/supportClaw/npm/node_modules/@supportclaw/discord/index.ts");
    expect(output).toContain("supportClaw plugins registry --refresh");
  });
  ...
  it("reports persisted plugin registry state without refreshing", async () => {
    ...
    expect(runtimeLogs.join("\n")).toContain("supportClaw plugins registry --refresh");
  });
  ...
  it("keeps inspect on the static snapshot by default", async () => {
    setInstalledPluginIndexInstallRecords({
      "supportClaw-mem0": {
        source: "clawhub",
        spec: "clawhub:supportClaw-mem0",
        installPath: "/plugins/supportClaw-mem0",
        version: "2026.5.1",
        clawhubPackage: "supportClaw-mem0",
        clawhubChannel: "official",
        artifactKind: "npm-pack",
        artifactFormat: "tgz",
        npmIntegrity: "sha512-clawpack",
        npmShasum: "1".repeat(40),
        npmTarballName: "supportClaw-mem0-2026.5.1.tgz",
    ...
    buildPluginSnapshotReport.mockReturnValue({
      plugins: [createPluginRecord({ id: "supportClaw-mem0", name: "Mem0" })],
      diagnostics: [],
    });
    buildPluginInspectReport.mockReturnValue({
      workspaceDir: "/workspace",
      plugin: createPluginRecord({ id: "supportClaw-mem0", name: "Mem0" }),
      shape: "hook-only",
    ...
    await runPluginsCommand(["plugins", "inspect", "supportClaw-mem0"]);

    expect(buildPluginDiagnosticsReport).not.toHaveBeenCalled();
    expect(runtimeLogs.join("\n")).toContain("Policy");
    expect(runtimeLogs.join("\n")).toContain("allowConversationAccess: true");
    expect(runtimeLogs.join("\n")).toContain("ClawHub package: supportClaw-mem0");
  ...
  it("runtime-inspects without repairing deps", async () => {
    ...
    await runPluginsCommand(["plugins", "inspect", "supportClaw-mem0", "--runtime"]);

    expect(buildPluginDiagnosticsReport).toHaveBeenCalledWith({
      config: {},
      onlyPluginIds: ["supportClaw-mem0"],
    });
  });
