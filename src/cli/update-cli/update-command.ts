...
import { disableCurrentSupportClawUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const POST_CORE_UPDATE_ENV = "SUPPORT_CLAW_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "SUPPORT_CLAW_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_REQUESTED_CHANNEL_ENV = "SUPPORT_CLAW_UPDATE_POST_CORE_REQUESTED_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "SUPPORT_CLAW_UPDATE_POST_CORE_RESULT_PATH";
const POST_CORE_UPDATE_INSTALL_RECORDS_PATH_ENV = "SUPPORT_CLAW_UPDATE_POST_CORE_INSTALL_RECORDS_PATH";
const POST_CORE_UPDATE_STARTED_AT_ENV = "SUPPORT_CLAW_UPDATE_POST_CORE_STARTED_AT_MS";
...
function restoreDroppedPreUpdateChannels(
  ...
  const authoredChannels = resolveRestoredAuthoredChannels({
    ...
    currentAuthoredChannels: isRecord(snapshot.parsed)
      ? (snapshot.parsed as SupportClawConfig).channels
...
function formatPostUpdatePluginInspectGuidance(pluginId: string): string {
  return `Run supportClaw plugins inspect ${pluginId} --runtime --json for details.`;
}
...
export function buildInvalidConfigPostCoreUpdateResult(): {
  ...
  const guidance = [
    "Run `supportClaw doctor` to inspect the config validation errors.",
    "Once the config parses, rerun `supportClaw update repair`.",
  ];
  const message =
...
function formatPostUpdateGatewayRecoveryLine(platform: NodeJS.Platform): string {
  const restartCommand = replaceCliName(formatCliCommand("supportClaw gateway restart"), CLI_NAME);
  const installCommand = replaceCliName(
    formatCliCommand("supportClaw gateway install --force"),
    CLI_NAME,
  );
...
export function formatPostUpdateGatewayRecoveryInstructions(
  ...
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall SupportClaw ${beforeVersion} with the same package manager, then rerun \`${replaceCliName(formatCliCommand("supportClaw gateway install --force"), CLI_NAME)}\`.`,
    );
  }
  return lines;
}
...
class UpdateCommandAbort extends Error {
  constructor() {
    super("supportClaw-update-abort");
...
function formatGatewayAncestryBlockMessage(pid: number): string {
  return `supportClaw update detected it is running inside the gateway process tree.
Gateway PID ${pid} is an ancestor of this process, so this updater cannot safely stop or restart the gateway that owns it.
Run \`${replaceCliName(formatCliCommand("supportClaw update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`;
}
...
async function maybeStopManagedServiceBeforeMutableUpdate(params: {
  ...
  if (
    ...
    if (!params.jsonMode) {
      defaultRuntime.log(
        theme.muted(
          "Managed gateway service points at a different SupportClaw root; leaving it running during this git update.",
        ),
      );
    }
    return {
...
function isRunningInsideGatewayService(
  ...
  if (env.SUPPORT_CLAW_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.SUPPORT_CLAW_SERVICE_KIND?.trim();
...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `${runtimeLabel} is too old for supportClaw@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    runtime.nodeRunner
      ? "Upgrade the Node runtime that owns the managed Gateway service, then rerun `supportClaw update`."
      : "Upgrade Node to 22.19+ or Node 24, then rerun `supportClaw update`.",
    "Bare `npm i -g supportClaw` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g supportClaw@latest`.",
  ].join("\n");
}
...
function stripGatewayServiceMarkerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  delete resolvedEnv.SUPPORT_CLAW_SERVICE_MARKER;
  delete resolvedEnv.SUPPORT_CLAW_SERVICE_KIND;
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("supportClaw completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
        ...
        const doctorStep = await runUpdateStep({
          ...
          env: {
            ...
            ...(candidateHostVersion === null
              ? {}
              : { SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION: candidateHostVersion }),
          },
          timeoutMs: params.timeoutMs,
        });
...
export async function updatePluginsAfterCoreUpdate(params: {
  ...
  if (pluginsChanged) {
    ...
    if (params.restoredAuthoredChannels !== undefined) {
      nextConfig = {
        ...nextConfig,
        channels: structuredClone(params.restoredAuthoredChannels) as SupportClawConfig["channels"],
      };
    }
...
async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("supportClaw gateway status --deep"), CLI_NAME)}\` for details.`,
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        defaultRuntime.log("");
        await createUpdateConfigSnapshot();
        process.env.SUPPORT_CLAW_UPDATE_IN_PROGRESS = "1";
        ...
        try {
          ...
          delete process.env.SUPPORT_CLAW_UPDATE_IN_PROGRESS;
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("supportClaw gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("supportClaw doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("supportClaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("supportClaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
function withUpdateFinalizationEnv<T>(run: () => Promise<T>): Promise<T> {
  const previousUpdateInProgress = process.env.SUPPORT_CLAW_UPDATE_IN_PROGRESS;
  ...
  return run().finally(() => {
    if (previousUpdateInProgress === undefined) {
      ...
      process.env.SUPPORT_CLAW_UPDATE_IN_PROGRESS = previousUpdateInProgress;
    }
    ...
    (configSnapshot.valid
      ? {
          ...
          authoredConfig: isRecord(configSnapshot.parsed)
            ? (configSnapshot.parsed as SupportClawConfig)
...
function normalizePreUpdateConfigRestoreInput(
  ...
  if (isRecord(sourceConfig) && isRecord(authoredConfig)) {
    return {
      sourceConfig: sourceConfig as SupportClawConfig,
      authoredConfig: authoredConfig as SupportClawConfig,
    };
  }
  const authored = parsed as SupportClawConfig;
  return {
...
function resolvePreUpdateSourceConfigFromAuthored(
  ...
  try {
    ...
    return isRecord(resolved) ? (resolved as SupportClawConfig) : authoredConfig;
  } catch {
...
async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-update-post-core-"));
    ...
    const child = spawn(params.nodeRunner ?? resolveNodeRunner(), argv, {
      ...
      env: {
        ...
        ...(postCoreHostVersion === null
          ? {}
          : { SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION: postCoreHostVersion }),
...
async function updateCommandInternal(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (opts.dryRun !== true) {
    await disableCurrentSupportClawUpdateLaunchdJob().catch(() => undefined);
    assertConfigWriteAllowedInCurrentMode();
  }
  ...
  if (postCoreUpdateResume) {
    ...
    process.env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION = (await readPackageVersion(root)) ?? VERSION;
  ...
  const devTargetRef =
    channel === "dev" ? process.env.SUPPORT_CLAW_UPDATE_DEV_TARGET_REF?.trim() || undefined : undefined;
  ...
  if (updateInstallKind === "package") {
    ...
    if (managedServiceRootRedirect) {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.warn(
            `Shell SupportClaw root differs from the managed gateway service root: ${managedServiceRootRedirect.previousRoot}`,
          ),
        );
        defaultRuntime.log(
          theme.muted(
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating SupportClaw..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "dirty") {
      ...
      defaultRuntime.log(
        theme.muted("Commit, stash, or discard the local changes, then rerun `supportClaw update`."),
      );
    }
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this SupportClaw install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("supportClaw doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("supportClaw gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g supportClaw@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g supportClaw@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (!pluginsUpdatedInFreshProcess) {
    ...
    const previousCompatibilityHostVersion = process.env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION;
    if (compatibilityDowngradeTarget) {
      process.env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION = compatibilityDowngradeTarget;
    }
    try {
      ...
      if (compatibilityDowngradeTarget) {
        if (previousCompatibilityHostVersion === undefined) {
          ...
          process.env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION = previousCompatibilityHostVersion;
        }
      }
    }
  }
