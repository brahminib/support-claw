...
const mocks = vi.hoisted(() => ({
  ...
  relinkSupportClawPeerDependenciesInManagedNpmRoot: vi.fn(),
...
vi.mock("../../plugins/plugin-peer-link.js", () => ({
  relinkSupportClawPeerDependenciesInManagedNpmRoot:
    mocks.relinkSupportClawPeerDependenciesInManagedNpmRoot,
}));
...
describe("runPostCorePluginConvergence", () => {
  ...
  beforeEach(() => {
    ...
    mocks.relinkSupportClawPeerDependenciesInManagedNpmRoot.mockResolvedValue({
      checked: 0,
      attempted: 0,
      repaired: 0,
      skipped: 0,
  ...
  function makeTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-post-core-convergence-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it("calls repair with SUPPORT_CLAW_UPDATE_POST_CORE_CONVERGENCE=1 set", async () => {
    const cfg = { plugins: { entries: {} } } as unknown as SupportClawConfig;
    await runPostCorePluginConvergence({
      cfg,
      env: { SUPPORT_CLAW_UPDATE_IN_PROGRESS: "1" },
    });
    ...
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      ...
      env: {
        ...
        SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION: VERSION,
  ...
  it("uses the candidate runtime version over a stale inherited host version", async () => {
    ...
    await runPostCorePluginConvergence({
      cfg,
      env: { SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION: "2026.5.12" },
    });
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      cfg,
      env: {
  ...
  it("returns the post-repair install records so callers can re-seed pluginConfig", async () => {
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { discord: { enabled: true } } } } as unknown as SupportClawConfig,
      env: {},
    });
  ...
  it("repairs managed npm supportClaw peer links in every managed npm project before payload smoke checks", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [],
    ...
    mocks.listManagedPluginNpmRoots.mockResolvedValue([
      "/tmp/supportClaw-state/npm",
      "/tmp/supportClaw-state/npm/projects/codex",
    ]);
    mocks.relinkSupportClawPeerDependenciesInManagedNpmRoot
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { codex: { enabled: true } } } } as unknown as SupportClawConfig,
      env: { SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-state" },
    });

    expect(mocks.relinkSupportClawPeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(1, {
      npmRoot: "/tmp/supportClaw-state/npm",
      logger: {},
    });
    expect(mocks.relinkSupportClawPeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(2, {
      npmRoot: "/tmp/supportClaw-state/npm/projects/codex",
      logger: {},
    });
    expect(result.changes).toEqual([
      "Repaired SupportClaw host peer link(s) for 1 managed npm plugin package(s).",
    ]);
    expect(
      mocks.relinkSupportClawPeerDependenciesInManagedNpmRoot.mock.invocationCallOrder[0],
  ...
  it("keeps repair warnings nonblocking with actionable guidance", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "discord" from @supportclaw/discord: ENETUNREACH.',
      ],
      records: {},
    });
    const result = await runPostCorePluginConvergence({
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        message:
          'Failed to install missing configured plugin "discord" from @supportclaw/discord: ENETUNREACH.',
        guidance: ["Run `supportClaw update repair` to retry plugin repair."],
      },
    ]);
  });
  ...
  it("keeps failed configured-plugin repair fetches nonblocking", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "matrix" from clawhub:@supportclaw/matrix@beta: ClawHub ClawPack download for @supportclaw/matrix@2026.6.1-beta.1 body stalled after 30000ms.',
      ],
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        guidance: [
          "Run `supportClaw update repair` to retry plugin repair.",
          "Run `supportClaw plugins inspect brave --runtime --json` for details.",
        ],
      },
    ]);
  });
      ...
      warnings: [
        {
          ...
          guidance: ["Run `supportClaw update repair`."],
        },
        {
