...
describe("runPluginPayloadSmokeCheck", () => {
  let tmpRoot: string;
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-payload-smoke-"));
  });
  afterEach(async () => {
  ...
  function resolveTestHostRoot(): string {
    const hostRoot = resolveSupportClawPackageRootSync({
  ...
  async function linkSupportClawPeerToHost(dir: string): Promise<void> {
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(resolveTestHostRoot(), path.join(dir, "node_modules", "supportClaw"), "junction");
  }
  ...
  it("reports ok for a record whose package.json + main file exist", async () => {
    ...
    await writePackage(
      dir,
      { name: "@supportclaw/discord", main: "dist/index.js" },
      "module.exports = {};",
    );
  ...
  it("reports a failure when the main entry file is missing on disk", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, { name: "@supportclaw/brave", main: "dist/index.js" });
    const result = await runPluginPayloadSmokeCheck({
      records: { brave: { source: "npm", installPath: dir } },
      env: {},
    });
  ...
  it("accepts a manifest with no main field (SupportClaw plugins commonly use `exports` or `supportClaw.extensions`)", async () => {
    const dir = path.join(tmpRoot, "matrix");
    await writePackage(dir, { name: "@supportclaw/plugin-matrix" });
    const result = await runPluginPayloadSmokeCheck({
  ...
  it("accepts a manifest that declares an existing `supportClaw.extensions` entry and no `main`", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
  ...
  it("reports a failure when `supportClaw.extensions` contains invalid entries", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@supportclaw/brave-plugin",
      supportClaw: { extensions: ["./index.js", " "] },
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        detail:
          "Plugin extension entry validation failed: package.json supportClaw.extensions[1] must be a non-empty string",
      },
    ]);
  });
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json supportClaw.extensions is empty",
      },
    ]);
  });
  ...
  it("accepts a packaged TypeScript extension entry when compiled runtime output exists", async () => {
    ...
    await writePackage(dir, {
      ...
      supportClaw: { extensions: ["./index.ts"] },
    });
  ...
  it("reports a failure when an supportClaw peer link is missing", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
        name: "@supportclaw/codex",
        main: "dist/index.js",
        peerDependencies: { supportClaw: ">=2026.5.18-beta.1" },
      },
      "export default {};\n",
    );
    ...
    expect(result.failures).toStrictEqual([
      {
        pluginId: "codex",
        installPath: dir,
        reason: "missing-supportClaw-peer-link",
        detail: `Plugin declares peerDependency "supportClaw" but peer link audit failed: missing ${path.join(
          dir,
          "node_modules",
          "supportClaw",
        )}.`,
  ...
  it("reports a failure when an supportClaw peer link is a stale real directory", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const stalePeerDir = path.join(dir, "node_modules", "supportClaw");
  ...
  it("reports a failure when an supportClaw peer link points at the wrong package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const wrongHostRoot = path.join(tmpRoot, "old-supportClaw");
    await fs.mkdir(wrongHostRoot, { recursive: true });
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(wrongHostRoot, path.join(dir, "node_modules", "supportClaw"), "junction");
  ...
  it("accepts an supportClaw peer link when it resolves to the host package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    await linkSupportClawPeerToHost(dir);
  ...
  it("reports a failure when an `supportClaw.extensions` entry file is missing", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@supportclaw/brave-plugin",
