...
import { relinkSupportClawPeerDependenciesInManagedNpmRoot } from "../../plugins/plugin-peer-link.js";
...
const REPAIR_GUIDANCE = "Run `supportClaw update repair` to retry plugin repair.";
const inspectGuidance = (pluginId: string) =>
  `Run \`supportClaw plugins inspect ${pluginId} --runtime --json\` for details.`;

async function repairManagedNpmSupportClawPeerLinks(params: {
  ...
  try {
    ...
    const message = `Failed to repair managed npm SupportClaw host peer links: ${err instanceof Error ? err.message : String(err)}`;
    return {
      changes: [],
      warnings: [
        {
...
export async function runPostCorePluginConvergence(params: {
  ...
  const peerLinkRepair = await repairManagedNpmSupportClawPeerLinks({ env });
