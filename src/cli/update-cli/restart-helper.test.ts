...
vi.mock("node:child_process", async () => {
  const { mockNodeBuiltinModule } = await import("supportClaw/plugin-sdk/test-node-mocks");
...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    const stateCheck = "$taskState = Get-SupportClawScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-SupportClawSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "supportClaw restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-SupportClawListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-SupportClawSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'supportClaw-gateway.service'");
    ...
    it("creates restart scripts in a private temp directory with exclusive creation", async () => {
      ...
      const oldCandidatePath = path.join(os.tmpdir(), `supportClaw-restart-${timestamp}.sh`);
      const victimDir = await makeTempDir("supportClaw-restart-helper-victim-");
      ...
      try {
        ...
        expect(path.basename(scriptDir)).toMatch(/^supportClaw-restart-/);
    ...
    it("uses SUPPORT_CLAW_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        SUPPORT_CLAW_PROFILE: "default",
        SUPPORT_CLAW_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("supportClaw-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        SUPPORT_CLAW_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        SUPPORT_CLAW_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped supportClaw gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart supportClaw-gateway.service");
      expect(calls).toContain("--user is-active --quiet supportClaw-gateway.service");
      expect(calls).toContain("is-active --quiet supportClaw-gateway.service");
      expect(calls).not.toContain("--user restart supportClaw-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.supportClaw.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.supportClaw.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.supportClaw/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.supportClaw/logs/gateway-restart.log' 2>&1");
    ...
    it("uses SUPPORT_CLAW_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/supportClaw-state/logs' 2>/dev/null && : >>'/tmp/supportClaw-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/supportClaw-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain("supportClaw restart attempt source=update target=ai.supportClaw.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.supportClaw.gateway");
      expect(log).toContain("supportClaw restart failed source=update status=42");
      expect(log).not.toContain("supportClaw restart done source=update");
    });
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        SUPPORT_CLAW_PROFILE: "default",
        HOME: path.join(tmpDir, "home"),
        SUPPORT_CLAW_STATE_DIR: stateFile,
      });
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        SUPPORT_CLAW_LAUNCHD_LABEL: "ai.supportClaw.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        SUPPORT_CLAW_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.supportClaw.$(echo injected)");
      expect(log).not.toContain("target=ai.supportClaw.injected");
    });

    it("uses SUPPORT_CLAW_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        SUPPORT_CLAW_PROFILE: "default",
        SUPPORT_CLAW_LAUNCHD_LABEL: "com.custom.supportClaw",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.supportClaw'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'SupportClaw Gateway'");
      expect(content).toContain("function Invoke-SupportClawSchtasksWithTimeout");
      expect(content).toContain("function Get-SupportClawScheduledTaskState");
      expect(content).toContain("function Invoke-SupportClawStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("supportClaw restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".supportClaw\\gateway.cmd"',
      );
      expect(content).toContain("supportClaw restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      expect(content).toContain('rmdir "%SUPPORT_CLAW_RESTART_SCRIPT_DIR%" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses SUPPORT_CLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        SUPPORT_CLAW_PROFILE: "default",
        SUPPORT_CLAW_WINDOWS_TASK_NAME: "SupportClaw Gateway (custom)",
      });
      expect(content).toContain("$taskName = 'SupportClaw Gateway (custom)'");
      expect(content).toContain("Get-SupportClawScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-SupportClawSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-SupportClawStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        SUPPORT_CLAW_PROFILE: "production",
      });
      expect(content).toContain("supportClaw-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        SUPPORT_CLAW_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.supportClaw.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'SupportClaw Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("escapes single quotes in profile names for shell scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        SUPPORT_CLAW_PROFILE: "it's-a-test",
      });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        SUPPORT_CLAW_LAUNCHD_LABEL: "ai.supportClaw.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.supportClaw.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
    ...
    it("rejects unsafe batch profile names on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      const scriptPath = await prepareRestartScript({
        SUPPORT_CLAW_PROFILE: "test&whoami",
      });
