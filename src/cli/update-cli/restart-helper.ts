...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.SUPPORT_CLAW_SYSTEMD_UNIT);
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.SUPPORT_CLAW_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.SUPPORT_CLAW_LAUNCHD_LABEL);
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.SUPPORT_CLAW_PROFILE);
}

function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.SUPPORT_CLAW_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.SUPPORT_CLAW_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `supportClaw-restart-${timestamp}.sh`;
...
exec 3>&2
${logSetup}
printf '[%s] supportClaw restart attempt source=update target=%s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
if systemctl --user is-active --quiet '${escaped}' || systemctl --user is-enabled --quiet '${escaped}'; then
  if systemctl --user restart '${escaped}'; then
    status=0
    printf '[%s] supportClaw restart done source=update\\n' "$(date -u +%FT%TZ)" >&2
  else
    status=$?
    printf '[%s] supportClaw restart failed source=update status=%s\\n' "$(date -u +%FT%TZ)" "$status" >&2
  fi
elif systemctl is-active --quiet '${escaped}' || systemctl is-enabled --quiet '${escaped}'; then
  status=78
  printf '[%s] system-scoped supportClaw gateway unit detected; update cannot restart it without sudo. Run: sudo systemctl restart %s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
...
# Self-cleanup (log is retained under the SupportClaw state logs directory).
script_dir=$(dirname "$0")
rm -f "$0"
rmdir "$script_dir" 2>/dev/null || true
exit "$status"
`;
      ...
      filename = `supportClaw-restart-${timestamp}.cmd`;
...
setlocal
set "SUPPORT_CLAW_RESTART_SCRIPT=%~f0"
set "SUPPORT_CLAW_RESTART_SCRIPT_DIR=%~dp0."
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p=$env:SUPPORT_CLAW_RESTART_SCRIPT; $s=Get-Content -Raw -LiteralPath $p; $m='# POWERSHELL'; $i=$s.IndexOf($m); if ($i -lt 0) { exit 1 }; Invoke-Expression $s.Substring($i)"
set "status=%ERRORLEVEL%"
del "%~f0" >nul 2>&1
rmdir "%SUPPORT_CLAW_RESTART_SCRIPT_DIR%" >nul 2>&1
exit /b %status%
# POWERSHELL
# Wait briefly to ensure file locks are released after update.
...
try {
  ...
  Add-Content -LiteralPath $logPath -Value "[$(Get-Date -Format o)] supportClaw restart log initialized"
} catch {
...
function Join-SupportClawProcessArguments {
...
function Invoke-SupportClawSchtasksWithTimeout {
  param(
    [string[]]$Arguments,
    [int]$TimeoutSeconds
  )
  ...
  try {
    ...
    $startInfo.Arguments = Join-SupportClawProcessArguments -Arguments $Arguments
    ...
    if (-not $process.WaitForExit($TimeoutSeconds * 1000)) {
      ...
      Write-RestartLog "supportClaw restart schtasks timeout source=update args=$($Arguments -join ' ')"
      return 124
    }
    ...
    Write-RestartLog "supportClaw restart schtasks failed source=update args=$($Arguments -join ' ') error=$($_.Exception.Message)"
    return 1
  }
}

function Get-SupportClawScheduledTaskState {
...
function Get-SupportClawListenerPids {
  param([int]$Port)
  $listenerPids = @()
...
function Invoke-SupportClawStartupLauncher {
  $launcherPath = Join-Path $env:USERPROFILE ".supportClaw\\gateway.cmd"
  if (-not (Test-Path -LiteralPath $launcherPath)) {
    Write-RestartLog "supportClaw restart startup launcher missing source=update path=$launcherPath"
    return 1
  }

  try {
    Start-Process -FilePath $launcherPath -WindowStyle Hidden | Out-Null
    Write-RestartLog "supportClaw restart launched startup fallback source=update path=$launcherPath"
    return 0
  } catch {
    Write-RestartLog "supportClaw restart startup fallback failed source=update error=$($_.Exception.Message)"
    return 1
  }
}
...
$port = ${port}
Write-RestartLog "supportClaw restart attempt source=update target=$taskName"

$taskState = Get-SupportClawScheduledTaskState -TaskName $taskName
if ($taskState -eq "Running") {
  $endStatus = Invoke-SupportClawSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10
  if ($endStatus -ne 0) {
    Write-RestartLog "supportClaw restart schtasks end did not complete cleanly source=update status=$endStatus"
  }
} else {
  Write-RestartLog "supportClaw restart skipped schtasks end source=update state=$taskState"
}

for ($attempt = 1; $attempt -le 10; $attempt++) {
  $listeners = @(Get-SupportClawListenerPids -Port $port)
  ...
  if ($attempt -eq 10) {
    foreach ($listenerPid in $listeners) {
      try {
        ...
        Write-RestartLog "supportClaw restart killed stale listener source=update pid=$listenerPid"
      } catch {
        Write-RestartLog "supportClaw restart failed to kill stale listener source=update pid=$listenerPid error=$($_.Exception.Message)"
      }
    }
    break
  }
...
$status = Invoke-SupportClawSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30
if ($status -ne 0) {
  $status = Invoke-SupportClawStartupLauncher
}
if ($status -eq 0) {
  Write-RestartLog "supportClaw restart done source=update"
} else {
  Write-RestartLog "supportClaw restart failed source=update status=$status"
}
...
`;
    ...
    const scriptDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-restart-"));
