...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/supportClaw-root")).toEqual([
      path.join("/tmp/supportClaw-root", "dist", "index.js"),
      path.join("/tmp/supportClaw-root", "dist", "index.mjs"),
      path.join("/tmp/supportClaw-root", "dist", "entry.js"),
      path.join("/tmp/supportClaw-root", "dist", "entry.mjs"),
    ]);
  });
});
...
describe("resolveGatewayInstallEntrypoint", () => {
  it("prefers dist/index.js over dist/entry.js when both exist", async () => {
    const root = "/tmp/supportClaw-root";
  ...
  it("uses the managed service port ahead of the caller environment", () => {
    expect(
      resolveUpdatedGatewayRestartPort({
        ...
        processEnv: { SUPPORT_CLAW_GATEWAY_PORT: "19001" },
        serviceEnv: { SUPPORT_CLAW_GATEWAY_PORT: "19002" },
      }),
    ).toBe(19002);
  });
...
describe("resolvePostUpdateServiceStateReadEnv", () => {
  it("keeps package restart preparation anchored to the pre-update service env", () => {
    const processEnv = {
      SUPPORT_CLAW_STATE_DIR: "/source/state",
      SUPPORT_CLAW_CONFIG_PATH: "/source/supportClaw.json",
    } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = {
      SUPPORT_CLAW_STATE_DIR: "/managed/state",
      SUPPORT_CLAW_CONFIG_PATH: "/managed/supportClaw.json",
    } as NodeJS.ProcessEnv;
  ...
  it("keeps git updates tied to the caller environment", () => {
    const processEnv = { SUPPORT_CLAW_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = { SUPPORT_CLAW_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
  ...
  it("uses the managed service environment for git updates stopped by this updater", () => {
    const processEnv = { SUPPORT_CLAW_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const preManagedServiceEnv = { SUPPORT_CLAW_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
...
describe("resolvePostInstallDoctorEnv", () => {
  it("uses the managed service profile paths for post-install doctor", () => {
    const env = resolvePostInstallDoctorEnv({
      invocationCwd: "/srv/supportClaw",
      baseEnv: {
        PATH: "/bin",
        SUPPORT_CLAW_STATE_DIR: "/wrong/state",
        SUPPORT_CLAW_CONFIG_PATH: "/wrong/supportClaw.json",
        SUPPORT_CLAW_PROFILE: "wrong",
      },
      serviceEnv: {
        SUPPORT_CLAW_STATE_DIR: "daemon-state",
        SUPPORT_CLAW_CONFIG_PATH: "daemon-state/supportClaw.json",
    ...
    expect(env.SUPPORT_CLAW_STATE_DIR).toBe(path.join("/srv/supportClaw", "daemon-state"));
    expect(env.SUPPORT_CLAW_CONFIG_PATH).toBe(
      path.join("/srv/supportClaw", "daemon-state", "supportClaw.json"),
    );
    expect(env.SUPPORT_CLAW_PROFILE).toBe("work");
  });
  ...
  it("keeps the caller env when no managed service env is available", () => {
    const env = resolvePostInstallDoctorEnv({
      baseEnv: {
        PATH: "/bin",
        SUPPORT_CLAW_STATE_DIR: "/caller/state",
        SUPPORT_CLAW_PROFILE: "caller",
      },
    });

    expect(env.PATH).toBe("/bin");
    expect(env.NODE_DISABLE_COMPILE_CACHE).toBe("1");
    expect(env.SUPPORT_CLAW_STATE_DIR).toBe("/caller/state");
    expect(env.SUPPORT_CLAW_PROFILE).toBe("caller");
  });
});

describe("collectMissingPluginInstallPayloads", () => {
  it("reports tracked npm install records whose package payload is absent", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-update-plugin-payload-"));
    const presentDir = path.join(tmpDir, "state", "npm", "node_modules", "@supportclaw", "present");
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@supportclaw", "missing");
    ...
    try {
      ...
      await fs.writeFile(path.join(presentDir, "package.json"), '{"name":"@supportclaw/present"}\n');
      ...
      await expect(
        collectMissingPluginInstallPayloads({
          ...
          records: {
            present: {
              source: "npm",
              spec: "@supportclaw/present@beta",
              installPath: presentDir,
            },
            missing: {
              source: "npm",
              spec: "@supportclaw/missing@beta",
              installPath: missingDir,
            },
            "no-package-json": {
              source: "npm",
              spec: "@supportclaw/no-package-json@beta",
              installPath: noPackageJsonDir,
            },
            "missing-install-path": {
              source: "npm",
              spec: "@supportclaw/missing-install-path@beta",
            },
            local: {
              source: "path",
  ...
  it("keeps disabled trusted official npm records eligible for payload repair when requested", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-update-plugin-payload-"));
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@supportclaw", "codex");
    try {
      await expect(
        collectMissingPluginInstallPayloads({
          env: { HOME: tmpDir } as NodeJS.ProcessEnv,
...
describe("formatPostUpdateGatewayRecoveryInstructions", () => {
  ...
  it("uses systemd wording on Linux instead of macOS LaunchAgent instructions", () => {
    ...
    expect(line).toContain("supportClaw gateway restart");
    expect(line).toContain("supportClaw gateway install --force");
    expect(line).toContain("supportClaw gateway status --deep");
...
describe("recoverInstalledLaunchAgentAfterUpdate", () => {
  it("re-bootstraps an installed-but-not-loaded macOS LaunchAgent after update", async () => {
    ...
    const serviceEnv = { SUPPORT_CLAW_PROFILE: "stomme" } as NodeJS.ProcessEnv;
    const recoveredEnv = { ...serviceEnv, SUPPORT_CLAW_PORT: "18790" } as NodeJS.ProcessEnv;
  ...
  it("does not recover a loaded LaunchAgent", async () => {
    const readState = vi.fn(async () => ({
      ...
      loaded: true,
      running: true,
      env: { SUPPORT_CLAW_PROFILE: "stomme" } as NodeJS.ProcessEnv,
      command: null,
  ...
  it("does not report recovered update health until the gateway passes the post-recovery wait", async () => {
    ...
    await expect(
      recoverLaunchAgentAndRecheckGatewayHealth({
        ...
        env: { SUPPORT_CLAW_PROFILE: "stomme", SUPPORT_CLAW_PORT: "18790" },
...
describe("resolvePostCoreUpdateChildStdio", () => {
  it('returns "pipe" on Windows so the child never inherits the parent console handles', () => {
    ...
    // causing the terminal to hang after `supportClaw update` completes (#78445).
