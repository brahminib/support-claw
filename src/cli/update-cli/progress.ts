...
const STEP_LABELS: Record<string, string> = {
  ...
  "supportClaw doctor entry": "Checking doctor entrypoint",
  "supportClaw doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g supportClaw@latest");
    hints.push(
      "System install outline: supportClaw gateway stop -> sudo <system-npm> i -g supportClaw@latest -> supportClaw gateway install --force -> supportClaw gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g supportClaw@latest --omit=optional");
  }
