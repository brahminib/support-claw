...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "supportClaw", "terminal"] },
    { argv: ["node", "supportClaw", "chat"] },
    { argv: ["node", "supportClaw", "gateway"] },
    { argv: ["node", "supportClaw", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "supportClaw", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "supportClaw", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "supportClaw", "gateway", "status"] },
    { argv: ["node", "supportClaw", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "supportClaw", "--help"] },
    { argv: ["node", "supportClaw", "gateway"] },
    { argv: ["node", "supportClaw", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "supportClaw.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("supportClaw.internal");
    } finally {
      log.mockRestore();
    }
  });
