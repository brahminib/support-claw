...
export const COMPLETION_SKIP_PLUGIN_COMMANDS_ENV = "SUPPORT_CLAW_COMPLETION_SKIP_PLUGIN_COMMANDS";
...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# SupportClaw Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# SupportClaw Completion\n${sourceLine}`;
  const next = trimmed ? `${trimmed}\n\n${block}\n` : `${block}\n`;
  return { next, changed: next !== content, hadExisting };
}

/** Resolves the shell startup profile path that should contain the SupportClaw completion block. */
...
/** Returns whether a shell profile already contains an SupportClaw completion block or source line. */
 ...
 * Returns true if profile has `source <(supportClaw completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "supportClaw") {
