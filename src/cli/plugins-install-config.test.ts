...
import { bundledPluginRootAt, repoInstallSpec } from "supportClaw/plugin-sdk/test-fixtures";
import { beforeEach, describe, expect, it, vi } from "vitest";
import type { SupportClawConfig } from "../config/config.js";
import { hashConfigIncludeRaw } from "../config/includes.js";
import type { ConfigWriteOptions } from "../config/io.js";
import type { ConfigFileSnapshot } from "../config/types.supportClaw.js";
import {
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    resolved: { plugins: {} } as SupportClawConfig,
    valid: false,
    runtimeConfig: { plugins: {} } as ConfigFileSnapshot["runtimeConfig"],
    config: { plugins: {} } as SupportClawConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const discordNpmRequest = {
    rawSpec: "@supportclaw/discord",
    normalizedSpec: "@supportclaw/discord",
    installKind: "plugin",
  ...
  it("returns the source config and base hash when the snapshot is valid", async () => {
    const cfg = { plugins: { entries: { discord: { enabled: true } } } } as SupportClawConfig;
  ...
  it("allows versioned npm:-prefixed bundled-plugin reinstall recovery", async () => {
    ...
    const request = resolvePluginInstallRequestContext({
      rawSpec: "npm:@supportclaw/discord@2026.5.22",
    });
    if (!request.ok) {
  ...
  it.each(["file:@supportclaw/discord", "FILE:@supportclaw/discord"])(
  ...
  it("allows versioned official npm spec reinstall recovery", async () => {
    ...
    const request = resolvePluginInstallRequestContext({
      rawSpec: "@supportclaw/discord@2026.5.22",
    });
    if (!request.ok) {
      throw new Error(request.error);
    }
  ...
  it("rejects unattributed source-only runtime failures during official plugin recovery", async () => {
    ...
    const request = resolvePluginInstallRequestContext({
      rawSpec: "npm:@supportclaw/discord",
    });
    if (!request.ok) {
      throw new Error(request.error);
    }
  ...
  it("allows Brave official plugin reinstall recovery from source-only runtime shadows", async () => {
      ...
      makeSnapshot({
        ...
        issues: [
          ...
          },
          {
            ...
            message:
              'web_search provider is not available: brave (install or enable plugin "brave", then run supportClaw doctor --fix)',
          },
        ],
      }),
    );
    ...
    const request = resolvePluginInstallRequestContext({
      rawSpec: "@supportclaw/brave-plugin",
    });
    if (!request.ok) {
      throw new Error(request.error);
    }
  ...
  it("allows explicit repo-checkout bundled-plugin reinstall recovery", async () => {
    const snapshotCfg = { plugins: {} } as SupportClawConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("allows recovery through an exact single-file top-level plugins include", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-plugin-include-"));
  ...
  it("carries a hook-mutation block through an external hooks include", async () => {
    ...
    const snapshotCfg = { hooks: { internal: {} } } as SupportClawConfig;
    includeFileTargetsForWriteMock.mockReturnValue({
  ...
  it("blocks config mutations when plugins and hooks share one canonical include target", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-shared-include-"));
    const configPath = path.join(tempRoot, "config.json5");
    ...
    const snapshotCfg = { hooks: {}, plugins: {} } as SupportClawConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        path: configPath,
        valid: true,
  ...
  it("blocks both mutations when an external include aliases the other section target", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-aliased-include-"));
    const configPath = path.join(tempRoot, "config.json5");
    const sharedPath = path.join(tempRoot, "shared.json5");
    const externalHooksPath = path.join(
      path.parse(process.cwd()).root,
  ...
  it("blocks nested plugins includes before plugin installation", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-plugin-nested-include-"));
    const configPath = path.join(tempRoot, "config.json5");
    const pluginsPath = path.join(tempRoot, "plugins.json5");
    ...
    const snapshotCfg = { plugins: { entries: {} } } as SupportClawConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        path: configPath,
        valid: true,
  ...
  it("rejects non-Discord install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `supportClaw doctor --fix` before installing plugins.");
  });
