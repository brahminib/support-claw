...
describe("plugins search command", () => {
  ...
  it("searches ClawHub code and bundle plugin families", async () => {
      ...
      .mockResolvedValueOnce([
        {
          score: 10,
          package: {
            name: "supportClaw-calendar-bundle",
    ...
    expect(mocks.logs.join("\n")).toContain("supportClaw-calendar");
    expect(mocks.logs.join("\n")).toContain(
      "Install: supportClaw plugins install clawhub:supportClaw-calendar",
    );
  });
