...
const mocks = vi.hoisted(() => {
  ...
  let configState: SupportClawConfig = {
    tools: {
      exec: {
        host: "auto",
        security: "allowlist",
  ...
  return {
    ...
    setConfig: (next: SupportClawConfig) => {
      configState = next;
    },
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: SupportClawConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: SupportClawConfig }>
    >(async () => ({
      path: "/tmp/supportClaw.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: SupportClawConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/supportClaw.json");
    ...
    expect(output).not.toContain("/tmp/supportClaw.json\nforged");
