...
function makeStore() {
  const root = makeTempDir(cleanupDirs, "supportClaw-proxy-capture-");
  return new DebugProxyCaptureStore({ env: { SUPPORT_CLAW_STATE_DIR: root } });
}

function makeStateEnv(prefix: string): NodeJS.ProcessEnv {
  const root = makeTempDir(cleanupDirs, prefix);
  return { SUPPORT_CLAW_STATE_DIR: root };
}
...
describe("DebugProxyCaptureStore", () => {
  it("keeps the cached store open until the last lease releases", () => {
    const options = { env: makeStateEnv("supportClaw-proxy-capture-lease-") };
  ...
  it("tracks and closes cached stores independently across paths", () => {
    const first = acquireDebugProxyCaptureStore({
      env: makeStateEnv("supportClaw-proxy-capture-first-"),
    });
    const second = acquireDebugProxyCaptureStore({
      env: makeStateEnv("supportClaw-proxy-capture-second-"),
    });
  ...
  it("preserves the shipped path-based Plugin SDK overloads", () => {
    const root = makeTempDir(cleanupDirs, "supportClaw-proxy-capture-legacy-sdk-");
  ...
  it("uses rollback journaling for captures on NFS-backed volumes", () => {
    ...
    const store = new DebugProxyCaptureStore({
      env: makeStateEnv("supportClaw-proxy-capture-nfs-"),
    });
    try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const env = makeStateEnv("supportClaw-proxy-capture-permissions-");
      ...
      expect(store.dbPath).toBe(path.join(root, "state", "supportClaw.sqlite"));
