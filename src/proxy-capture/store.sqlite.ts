...
import { normalizeNullableString as normalizeObservedValue } from "@supportclaw/normalization-core/string-coerce";
...
class DebugProxyCaptureStoreImpl {
  ...
  constructor(
    ...
    const database = openSupportClawStateDatabase({ env: optionsOrDbPath.env });
    this.db = database.db;
...
function resolveDebugProxyCaptureStoreKey(
  ...
  return typeof optionsOrDbPath === "string"
    ...
    : `shared:${openSupportClawStateDatabase({ env: optionsOrDbPath.env }).path}`;
}
