...
async function makeSettings(): Promise<DebugProxySettings> {
  testRoot = await mkdtemp(join(tmpdir(), "supportClaw-debug-proxy-managed-proxy-"));
  ...
  process.env.SUPPORT_CLAW_STATE_DIR = testRoot;
  return {
    enabled: true,
    required: false,
...
describe("debug proxy managed-proxy direct upstream policy", () => {
  const originalProxyActive = process.env["SUPPORT_CLAW_PROXY_ACTIVE"];
  const originalAllowDirect =
    process.env["SUPPORT_CLAW_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];

  beforeEach(async () => {
    await cleanupTestDirs();
    delete process.env["SUPPORT_CLAW_PROXY_ACTIVE"];
    delete process.env["SUPPORT_CLAW_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];
  });
  ...
  afterEach(async () => {
    if (originalProxyActive === undefined) {
      ...
      process.env["SUPPORT_CLAW_PROXY_ACTIVE"] = originalProxyActive;
    }
  ...
  it("uses shared truthy parsing for managed proxy mode", () => {
    process.env["SUPPORT_CLAW_PROXY_ACTIVE"] = "true";
  ...
  it("allows direct upstreams with explicit diagnostic override", () => {
    process.env["SUPPORT_CLAW_PROXY_ACTIVE"] = "1";
    process.env["SUPPORT_CLAW_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"] = "1";
