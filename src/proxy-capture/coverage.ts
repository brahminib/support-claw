...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[supportClaw proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[supportClaw proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`supportClaw proxy coverage\` for details.`,
  );
}
