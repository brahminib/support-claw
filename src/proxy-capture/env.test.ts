...
describe("resolveDebugProxySettings", () => {
  ...
  it("prefers an explicit session id from the environment", () => {
    const settings = resolveDebugProxySettings({
      [SUPPORT_CLAW_DEBUG_PROXY_ENABLED]: "1",
      [SUPPORT_CLAW_DEBUG_PROXY_SESSION_ID]: "session-explicit",
    });
  ...
  it("retains deprecated capture storage settings for Plugin SDK compatibility", () => {
    const settings = resolveDebugProxySettings({
      SUPPORT_CLAW_DEBUG_PROXY_DB_PATH: "/tmp/legacy-capture.sqlite",
      SUPPORT_CLAW_DEBUG_PROXY_BLOB_DIR: "/tmp/legacy-capture-blobs",
    });
  ...
  it("does not pass obsolete capture storage overrides to child processes", () => {
    ...
    expect(env.SUPPORT_CLAW_DEBUG_PROXY_DB_PATH).toBeUndefined();
    expect(env.SUPPORT_CLAW_DEBUG_PROXY_BLOB_DIR).toBeUndefined();
  });
});
