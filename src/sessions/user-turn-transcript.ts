...
import { mimeTypeFromFilePath } from "@supportclaw/media-core/mime";
...
function buildPersistedUserTurnMessage(params: UserTurnInput): PersistedUserTurnMessage {
  ...
  // the live turn) — see https://github.com/brahminib/support-claw/issues/3658.
...
function isBeforeAgentRunBlockedMessage(message: AgentMessage): boolean {
  const marker = (message as { __supportClaw?: { beforeAgentRunBlocked?: unknown } })["__supportClaw"]
...
export async function persistUserTurnTranscript(
  ...
  const turn = await persistSessionTranscriptTurn(
    ...
    {
      ...(params.cwd ? { cwd: params.cwd } : {}),
      ...(params.config ? { config: params.config as SupportClawConfig } : {}),
      updateMode: params.updateMode ?? "inline",
      messages: [
        {
          message,
...
async function appendFileTargetUserTurnTranscript(params: {
  ...
  const appended = await appendUserTurnTranscriptMessage({
    ...
    ...(config ? { config: config as SupportClawConfig } : {}),
