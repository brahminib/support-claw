...
const ORIGINAL_ENV = {
  SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS: process.env.SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS,
  SUPPORT_CLAW_CONFIG_PATH: process.env.SUPPORT_CLAW_CONFIG_PATH,
  SUPPORT_CLAW_STATE_DIR: process.env.SUPPORT_CLAW_STATE_DIR,
  SUPPORT_CLAW_TEST_FAST: process.env.SUPPORT_CLAW_TEST_FAST,
} as const;
...
describe("plugin-sdk qa-runner-runtime linked plugin smoke", () => {
  ...
  it("loads an activated qa runner from a linked plugin path without a bundled install fallback", async () => {
    const stateDir = makeTempDir("supportClaw-qa-runner-state-");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@supportclaw/qa-linked",
        type: "module",
        supportClaw: {
          extensions: ["./index.js"],
          install: {
            npmSpec: "@supportclaw/qa-linked",
          },
        },
      }),
      "utf8",
