...
function expectPathInsideTmpRoot(resultPath: string) {
  const tmpRoot = fsSync.realpathSync(resolvePreferredSupportClawTmpDir());
...
describe("withTempDownloadPath", () => {
  it.each([
    ...
    if (expectedBasename) {
      ...
      expect(capturedPath).toContain(path.join(resolvePreferredSupportClawTmpDir(), "line-media-"));
    }
    if (expectCleanup) {
