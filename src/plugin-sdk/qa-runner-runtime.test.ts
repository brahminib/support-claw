...
const resolveSupportClawPackageRootSync = vi.hoisted(() => vi.fn());
...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  const originalPrivateQaCli = process.env.SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    ...
    resolveSupportClawPackageRootSync.mockReset().mockReturnValue(null);
    delete process.env.SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "supportClaw-qa-test-api-root-");
    resolveSupportClawPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(testApiCall?.env?.SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(testApiCall?.env?.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR).toBe(
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "supportClaw-qa-runner-root-");
    resolveSupportClawPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(manifestCall?.env?.SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(manifestCall?.env?.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
    ...
    expect(publicSurfaceCall?.env?.SUPPORT_CLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(publicSurfaceCall?.env?.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
  });
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-matrix" exported "extra" from runtime-api.js but did not declare it in supportClaw.plugin.json',
    );
  });
});
