...
export function requireRuntimeConfig(config: SupportClawConfig, context: string): SupportClawConfig {
  if (config) {
    return config;
  }
  throw new Error(
...
export function resolveLivePluginConfigObject(
  runtimeConfigLoader: (() => SupportClawConfig | undefined) | undefined,
  pluginId: string,
