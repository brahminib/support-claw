...
export { VERSION as SUPPORT_CLAW_VERSION } from "../version.js";
...
export { resolveSupportClawAgentDir } from "./agent-dir-compat.js";
export {
  resolveAgentDir,
...
export async function detectAndLoadAgentHarnessPromptImages(params: {
  ...
  config?: import("../config/types.supportClaw.js").SupportClawConfig;
  ...
  const [{ resolveImageSanitizationLimits }, { detectAndLoadPromptImages }, { MAX_IMAGE_BYTES }] =
    await Promise.all([
      ...
      import("@supportclaw/media-core/constants"),
    ]);
...
/**
 * Derive the same compact user-facing tool detail that embedded SupportClaw uses for progress logs.
 */
 ...
 * preserve the same SupportClaw-owned fallback signals as the built-in SupportClaw path
