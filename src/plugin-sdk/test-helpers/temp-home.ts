// Temp home test helpers create isolated SupportClaw home directories for plugin tests.
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked SUPPORT_CLAW_HOME.
  delete process.env.SUPPORT_CLAW_HOME;
  process.env.SUPPORT_CLAW_STATE_DIR = path.join(base, ".supportClaw");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "supportClaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".supportClaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
