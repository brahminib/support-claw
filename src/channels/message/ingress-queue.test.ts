...
type ChannelIngressTestDatabase = Pick<SupportClawStateKyselyDatabase, "channel_ingress_events">;

async function withTempState<T>(fn: (stateDir: string) => Promise<T>): Promise<T> {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-ingress-queue-"));
  try {
...
describe("channel ingress queue", () => {
  ...
  it("opens a custom state database without copying the full process env", async () => {
    ...
    await withTempState(async (stateDir) => {
      ...
      expect(env.SUPPORT_CLAW_STATE_DIR).toBe(stateDir);
      expect(env.HOME).toBe("/home/supportClaw");
      expect(Object.getPrototypeOf(env)).toBe(inheritedEnv);
      expect(Object.keys(env)).toEqual(["SUPPORT_CLAW_STATE_DIR"]);

      const database = openSupportClawStateDatabase({ env });
