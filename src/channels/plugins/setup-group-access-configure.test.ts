...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: SupportClawConfig, policy: ChannelAccessPolicy) => SupportClawConfig;
  resolveAllowlist: (params: { cfg: SupportClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: SupportClawConfig; resolved: TResolved }) => SupportClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: SupportClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: SupportClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: SupportClawConfig, policy: ChannelAccessPolicy): SupportClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: SupportClawConfig, policy: ChannelAccessPolicy): SupportClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: SupportClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: SupportClawConfig; resolved: string[] }) => {
      calls.push("apply");
