...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): SupportClawConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as SupportClawConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: SupportClawConfig) => string[];
  input: SupportClawConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: SupportClawConfig, expected: string) {
