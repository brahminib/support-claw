...
function listBuiltPluginLoaderModuleCandidateUrls(importerUrl: string): URL[] {
  ...
  // ../../plugins candidates would escape the installed supportClaw package there.
...
function getChannelConfigRecord(cfg: SupportClawConfig, channelId: string): Record<string, unknown> {
...
function listManifestChannelAccountIds(cfg: SupportClawConfig, channelId: string): string[] {
...
function rebindChannelPluginConfig(
  ...
  const rebind = (cfg: SupportClawConfig) =>
...
export function resolveReadOnlyChannelPluginsForConfig(
  ...
  if (externalPluginIds.length > 0) {
    ...
    if (missingConfiguredChannelIds.length > 0 && options.includeSetupFallbackPlugins === true) {
      ...
      const registry = loadPluginLoaderModule().loadSupportClawPlugins({
        config: cfg,
        activationSourceConfig: options.activationSourceConfig ?? cfg,
        env,
        workspaceDir,
