...
import type { SupportClawPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["SUPPORT_CLAW_PLUGIN_CATALOG_PATHS", "SUPPORT_CLAW_MPM_CATALOG_PATHS"];
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = uniqueStrings(
    [
      resolveSupportClawPackageRootSync({ cwd: process.cwd() }),
      resolveSupportClawPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<SupportClawPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
