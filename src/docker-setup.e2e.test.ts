...
const sandboxRootTracker = createSuiteTempRootTracker({ prefix: "supportClaw-docker-setup-" });

const prestartContainerEnvFlags = [
  "-e HOME=/home/node",
  "-e SUPPORT_CLAW_HOME=/home/node",
  "-e SUPPORT_CLAW_STATE_DIR=/home/node/.supportClaw",
  "-e SUPPORT_CLAW_CONFIG_PATH=/home/node/.supportClaw/supportClaw.json",
  "-e SUPPORT_CLAW_CONFIG_DIR=/home/node/.supportClaw",
  "-e SUPPORT_CLAW_WORKSPACE_DIR=/home/node/.supportClaw/workspace",
].join(" ");
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    SUPPORT_CLAW_GATEWAY_TOKEN: "test-token",
    SUPPORT_CLAW_CONFIG_DIR: join(sandbox.rootDir, "config"),
    SUPPORT_CLAW_WORKSPACE_DIR: join(sandbox.rootDir, "supportClaw"),
    SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR: join(sandbox.rootDir, "auth-profile-secrets"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("supportClaw-gateway");
}
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    ...
    SUPPORT_CLAW_CONFIG_DIR: configDir,
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_DOCKER_APT_PACKAGES: "curl wget",
      SUPPORT_CLAW_EXTRA_MOUNTS: undefined,
      SUPPORT_CLAW_HOME_VOLUME: "supportClaw-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SUPPORT_CLAW_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).toContain("SUPPORT_CLAW_EXTRA_MOUNTS=");
    expect(envFile).toContain("SUPPORT_CLAW_HOME_VOLUME=supportClaw-home"); // pragma: allowlist secret
    expect(envFile).toContain("SUPPORT_CLAW_DISABLE_BONJOUR=");
    expect(envFile).toContain(
      `SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR=${join(activeSandbox.rootDir, "auth-profile-secrets")}`,
    );
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("supportClaw-home:/home/node");
    expect(extraCompose).toContain(
      `${join(activeSandbox.rootDir, "auth-profile-secrets")}:/home/node/.config/supportClaw`,
    );
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("supportClaw-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg SUPPORT_CLAW_IMAGE_APT_PACKAGES=curl wget");
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node supportClaw-gateway dist/index.js onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env SUPPORT_CLAW_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output`,
    );
    ...
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node supportClaw-gateway dist/index.js config set --batch-json [{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]`,
    );
    expect(log).not.toContain("run --rm supportClaw-cli onboard --mode local --no-install-daemon");
  });
  ...
  it("allows ordinary spaces in host persistence paths and quotes generated mounts", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_CONFIG_DIR: configDir,
      SUPPORT_CLAW_WORKSPACE_DIR: workspaceDir,
      SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR: authProfileSecretDir,
      SUPPORT_CLAW_HOME_VOLUME: homeVolumeDir,
      SUPPORT_CLAW_EXTRA_MOUNTS: `${extraMountSource}:/mnt/extra data:ro`,
    });

    expect(result.status).toBe(0);
    expect(result.stderr).not.toContain("cannot contain whitespace");
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain(`SUPPORT_CLAW_CONFIG_DIR=${configDir}`);
    expect(envFile).toContain(`SUPPORT_CLAW_WORKSPACE_DIR=${workspaceDir}`);
    expect(envFile).toContain(`SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR=${authProfileSecretDir}`);
    ...
    expect(extraCompose).toContain(`"${configDir}:/home/node/.supportClaw"`);
    expect(extraCompose).toContain(`"${workspaceDir}:/home/node/.supportClaw/workspace"`);
    expect(extraCompose).toContain(`"${authProfileSecretDir}:/home/node/.config/supportClaw"`);
  ...
  it("persists explicit Docker Bonjour opt-in overrides", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_DISABLE_BONJOUR: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SUPPORT_CLAW_DISABLE_BONJOUR=0");
  });

  it("normalizes legacy SUPPORT_CLAW_DOCKER_APT_PACKAGES into SUPPORT_CLAW_IMAGE_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SUPPORT_CLAW_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).not.toContain("SUPPORT_CLAW_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg SUPPORT_CLAW_IMAGE_APT_PACKAGES=curl wget");
    expect(log).not.toContain("--build-arg SUPPORT_CLAW_DOCKER_APT_PACKAGES");
  });

  it("prefers SUPPORT_CLAW_IMAGE_APT_PACKAGES over legacy SUPPORT_CLAW_DOCKER_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_IMAGE_APT_PACKAGES: "curl wget httpie",
      SUPPORT_CLAW_DOCKER_APT_PACKAGES: "curl wget",
    });
    expect(result.status).toBe(0);

    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SUPPORT_CLAW_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(envFile).not.toContain("SUPPORT_CLAW_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg SUPPORT_CLAW_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(log).not.toMatch(/--build-arg SUPPORT_CLAW_IMAGE_APT_PACKAGES=curl wget(?! httpie)/);
  });

  it("explicitly empty SUPPORT_CLAW_IMAGE_APT_PACKAGES suppresses legacy fallback", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_IMAGE_APT_PACKAGES: "",
      SUPPORT_CLAW_DOCKER_APT_PACKAGES: "curl wget",
    });
    expect(result.status).toBe(0);

    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SUPPORT_CLAW_IMAGE_APT_PACKAGES=");
    expect(envFile).not.toContain("curl wget");

    const log = await readDockerLog(activeSandbox);
    expect(log).not.toContain("--build-arg SUPPORT_CLAW_IMAGE_APT_PACKAGES=curl wget");
  });

  it("avoids shared-network supportClaw-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    const prestartCliRunLines = collectMatchingLines(prestartLines, (line) =>
      /\bcompose\b.*\brun\b.*\bsupportClaw-cli\b/.test(line),
    );
  ...
  it("pins setup-time CLI state paths inside the container", async () => {
    ...
    await resetDockerLog(activeSandbox);
    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_HOME: "/mnt/c/Users/Trevor",
      SUPPORT_CLAW_STATE_DIR: "/mnt/c/Users/Trevor/.supportClaw",
      SUPPORT_CLAW_CONFIG_PATH: "/mnt/c/Users/Trevor/.supportClaw/supportClaw.json",
      SUPPORT_CLAW_SKIP_ONBOARDING: "1",
    });
    expect(result.status).toBe(0);
  ...
  it("forces BuildKit for local and sandbox docker builds", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        SUPPORT_CLAW_SANDBOX: "1",
        SUPPORT_CLAW_DOCKER_SOCKET: socketPath,
      });
  ...
  it("offline mode reuses a preloaded local image without build or pull", async () => {
    ...
    const result = runDockerSetup(
      activeSandbox,
      {
        SUPPORT_CLAW_IMAGE: "ghcr.io/supportClaw/supportClaw:latest",
        SUPPORT_CLAW_SKIP_ONBOARDING: "1",
      },
      ["--offline"],
    );
    ...
    expect(result.stdout).toContain(
      "Using preloaded Docker image: ghcr.io/supportClaw/supportClaw:latest",
    );

    const lines = await readDockerLogLines(activeSandbox);
    const log = lines.join("\n");
    expect(log).toContain("image inspect ghcr.io/supportClaw/supportClaw:latest");
  ...
  it("offline mode fails before setup when the main image is missing", async () => {
    ...
    const result = runDockerSetup(
      activeSandbox,
      {
        SUPPORT_CLAW_IMAGE: "ghcr.io/supportClaw/supportClaw:offline",
        DOCKER_STUB_MISSING_IMAGES: "ghcr.io/supportClaw/supportClaw:offline",
      },
      ["--offline"],
    );
    ...
    expect(result.stderr).toContain(
      "Offline Docker setup requires preloaded image ghcr.io/supportClaw/supportClaw:offline",
    );

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("image inspect ghcr.io/supportClaw/supportClaw:offline");
    expect(log).not.toMatch(/^build /m);
    expect(log).not.toMatch(/^pull /m);
    expect(log).not.toContain("up -d supportClaw-gateway");
  });
  ...
  it("offline sandbox stays disabled when its configured image is missing", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const defaultImage = "registry.example/supportClaw-sandbox:approved";
      const agentImage = " registry.example/supportClaw-sandbox:agent ";
      const result = runDockerSetup(
        activeSandbox,
        {
          SUPPORT_CLAW_SANDBOX: "1",
      ...
      const lines = await readDockerLogLines(activeSandbox);
      const log = lines.join("\n");
      expect(log).toContain("image inspect supportClaw:local");
      expect(log).not.toContain(`image inspect ${defaultImage}`);
      expect(log).toContain(`image inspect ${agentImage} host=unix://${socketPath}`);
      expect(log).not.toContain("image inspect supportClaw-sandbox:bookworm-slim");
      expect(log).not.toMatch(/^build /m);
      expect(log).not.toMatch(/^pull /m);
  ...
  it("offline sandbox validates only effective Docker and browser images", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const defaultImage = "registry.example/supportClaw-sandbox:default";
      const browserImage = "registry.example/supportClaw-sandbox-browser:default";
      const ignoredImages = [
        "registry.example/supportClaw-sandbox:ssh",
        "registry.example/supportClaw-sandbox:shared-agent",
        "registry.example/supportClaw-sandbox-browser:shared-agent",
        "registry.example/supportClaw-sandbox:off",
        "registry.example/supportClaw-sandbox-browser:denied",
      ];
      const result = runDockerSetup(
        activeSandbox,
        {
  ...
  it("offline sandbox rejects an incompatible browser image", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const browserImage = "registry.example/supportClaw-sandbox-browser:stale";
      const result = runDockerSetup(
        activeSandbox,
        {
          SUPPORT_CLAW_SANDBOX: "1",
  ...
  it("writes SUPPORT_CLAW_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SUPPORT_CLAW_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh supportClaw-gateway -c");
  ...
  it("precreates auth profile secret key dir outside the mounted state dir", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_CONFIG_DIR: configDir,
      SUPPORT_CLAW_WORKSPACE_DIR: workspaceDir,
      SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR: secretDir,
    });
    ...
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("find /home/node/.config/supportClaw -xdev");
  });

  it("reuses existing config token when SUPPORT_CLAW_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      ...
      async (configDir) => {
        await writeFile(
          join(configDir, "supportClaw.json"),
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("SUPPORT_CLAW_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when SUPPORT_CLAW_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "SUPPORT_CLAW_GATEWAY_TOKEN=dotenv-token-123\nSUPPORT_CLAW_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("SUPPORT_CLAW_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "SUPPORT_CLAW_GATEWAY_TOKEN=first-token",
        "SUPPORT_CLAW_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("SUPPORT_CLAW_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("SUPPORT_CLAW_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats SUPPORT_CLAW_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_SANDBOX: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SUPPORT_CLAW_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg SUPPORT_CLAW_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg SUPPORT_CLAW_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        SUPPORT_CLAW_SANDBOX: "1",
        SUPPORT_CLAW_DOCKER_SOCKET: socketPath,
        DOCKER_STUB_FAIL_MATCH: "--entrypoint docker supportClaw-gateway --version",
      });
  ...
  it("keeps offline policy when sandbox config writes fail and the gateway rolls back", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      ...
      expect(log).toContain(
        "run --pull never --rm --no-deps supportClaw-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("--force-recreate supportClaw-gateway"));
      expect(forceRecreateLine).toBe(
        `compose compose -f ${join(activeSandbox.rootDir, "docker-compose.yml")} up -d --pull never --no-build --force-recreate supportClaw-gateway`,
      );
      expect(forceRecreateLine).not.toContain("docker-compose.sandbox.yml");
      expect(log).toContain(
        `image inspect supportClaw-sandbox:bookworm-slim host=unix://${socketPath}`,
      );
      expectOfflineComposePolicy(lines);
      await expectMissingPath(join(activeSandbox.rootDir, "docker-compose.sandbox.yml"));
    });
  ...
  it("rejects injected multiline SUPPORT_CLAW_EXTRA_MOUNTS values", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SUPPORT_CLAW_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid SUPPORT_CLAW_EXTRA_MOUNTS mount format", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid SUPPORT_CLAW_HOME_VOLUME names", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SUPPORT_CLAW_HOME_VOLUME must match");
  });

  it("rejects SUPPORT_CLAW_TZ values that are not present in zoneinfo", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_TZ: "Nope/Bad",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SUPPORT_CLAW_TZ must match a timezone in /usr/share/zoneinfo");
  });

  it("skips onboarding when SUPPORT_CLAW_SKIP_ONBOARDING is set", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SUPPORT_CLAW_SKIP_ONBOARDING=1");
  });

  it("treats SUPPORT_CLAW_SKIP_ONBOARDING=0 as disabled and runs onboarding", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORT_CLAW_SKIP_ONBOARDING: "0",
    });

    expect(result.status).toBe(0);
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain(
      "onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env SUPPORT_CLAW_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output",
    );
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toMatch(/SUPPORT_CLAW_SKIP_ONBOARDING=\n/);
  });
  ...
  it("keeps docker-compose gateway Bonjour advertising in auto mode by default", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(
      compose.match(/SUPPORT_CLAW_DISABLE_BONJOUR: \$\{SUPPORT_CLAW_DISABLE_BONJOUR:-\}/g),
    ).toHaveLength(1);
  });

  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:supportClaw-gateway"');
    expect(compose).toContain("depends_on:\n      - supportClaw-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/SUPPORT_CLAW_GATEWAY_TOKEN: \$\{SUPPORT_CLAW_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose auth profile secret key source durable outside state", async () => {
    ...
    expect(
      compose.split(
        '"${SUPPORT_CLAW_AUTH_PROFILE_SECRET_DIR:-${HOME:-/tmp}/.supportClaw-auth-profile-secrets}:/home/node/.config/supportClaw"',
      ),
    ).toHaveLength(3);
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{SUPPORT_CLAW_TZ:-UTC\}/g)).toHaveLength(2);
  });
  ...
  it("pins container-side state, workspace, and config dirs on both services so host .env paths cannot leak (#77436)", async () => {
    ...
    expect(compose.match(/SUPPORT_CLAW_HOME: \/home\/node$/gm)).toHaveLength(2);
    expect(compose.match(/SUPPORT_CLAW_STATE_DIR: \/home\/node\/\.supportClaw$/gm)).toHaveLength(2);
    expect(
      compose.match(/SUPPORT_CLAW_CONFIG_PATH: \/home\/node\/\.supportClaw\/supportClaw\.json$/gm),
    ).toHaveLength(2);
    expect(compose.match(/SUPPORT_CLAW_CONFIG_DIR: \/home\/node\/\.supportClaw$/gm)).toHaveLength(2);
    expect(
      compose.match(/SUPPORT_CLAW_WORKSPACE_DIR: \/home\/node\/\.supportClaw\/workspace$/gm),
    ).toHaveLength(2);
  });

  it("Dockerfile ARG SUPPORT_CLAW_IMAGE_APT_PACKAGES must not have a default value", async () => {
    // If the ARG has a default (e.g. ARG SUPPORT_CLAW_IMAGE_APT_PACKAGES=""), Docker treats it as
    // "set" even when no --build-arg is passed. That breaks the RUN fallback expression
    // ${SUPPORT_CLAW_IMAGE_APT_PACKAGES-$SUPPORT_CLAW_DOCKER_APT_PACKAGES} because the variable is
    // never truly unset, so legacy-only callers using --build-arg SUPPORT_CLAW_DOCKER_APT_PACKAGES
    ...
    const argLine = dockerfile
      .split("\n")
      .find((line) => line.startsWith("ARG SUPPORT_CLAW_IMAGE_APT_PACKAGES"));
    expect(argLine).toBeDefined();
    // Must be bare `ARG SUPPORT_CLAW_IMAGE_APT_PACKAGES` with no default assignment
    expect(argLine).toBe("ARG SUPPORT_CLAW_IMAGE_APT_PACKAGES");
  });
});
