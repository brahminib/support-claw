...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-security-audit-async-"));
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        supportClaw: { extensions: ["../outside.js"] },
      }),
    );
  ...
  it("ignores install backup and debris dirs when scanning installed plugin roots", async () => {
    ...
    try {
      ...
      expect(findings.map((f) => f.title).join("\n")).not.toContain(".supportClaw-install-backups");
    } finally {
      scanSpy.mockRestore();
    }
  });
  ...
  it("audits canonical auth profile SQLite store permissions", async () => {
    ...
    const databasePath = path.join(agentDir, "supportClaw-agent.sqlite");
    ...
    expect(readableAuthTargets).toEqual(
      expect.arrayContaining([
        expect.stringContaining("supportClaw-agent.sqlite"),
        expect.stringContaining("supportClaw-agent.sqlite-wal"),
        expect.stringContaining("supportClaw-agent.sqlite-shm"),
        expect.stringContaining("supportClaw-agent.sqlite-journal"),
      ]),
    );
  });
});
