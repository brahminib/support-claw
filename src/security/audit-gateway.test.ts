...
describe("security audit gateway config findings", () => {
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    await Promise.all([
      ...
      (async () => {
        ...
        const resolvedConfig: SupportClawConfig = {
          gateway: {
            bind: "lan",
            auth: {},
          },
  ...
  it("warns when SUPPORT_CLAW_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: SupportClawConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from SUPPORT_CLAW_GATEWAY_TOKEN", () => {
    const cfg: SupportClawConfig = {
