...
type CollectDangerousConfigFlags = (cfg: SupportClawConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.SUPPORT_CLAW_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.SUPPORT_CLAW_GATEWAY_PASSWORD);
