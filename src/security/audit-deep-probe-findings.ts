...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("supportClaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("supportClaw security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set SUPPORT_CLAW_GATEWAY_TOKEN/SUPPORT_CLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("supportClaw security audit --deep")}".`,
    });
  }
  return findings;
}
