...
function summarizeGroupPolicy(cfg: SupportClawConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: SupportClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: SupportClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: SupportClawConfig): boolean {
...
function isBrowserEnabled(cfg: SupportClawConfig): boolean {
  return cfg.browser?.enabled !== false;
}

/** Produce a concise inventory of major security-relevant surfaces. */
export function collectAttackSurfaceSummaryFindings(cfg: SupportClawConfig): SecurityAuditFinding[] {
