...
import type { SupportClawConfig, SecurityConfig } from "../config/types.supportClaw.js";
...
export type InstallPolicySource = {
  kind:
    ...
    | "upload"
    | "workspace";
  authority: "supportClaw" | "official" | "third-party" | "unknown" | "user";
  mutable: boolean;
  network: boolean;
};
...
export async function runInstallPolicy(params: {
  ...
  if (!config) {
    try {
      ...
      return failClosed(`could not load SupportClaw config (${formatErrorMessage(err)})`);
    }
  }
  ...
  const input = JSON.stringify({
    protocolVersion: 1,
    supportClawVersion: resolveRuntimeServiceVersion(params.env ?? process.env),
    ...params.request,
  });
