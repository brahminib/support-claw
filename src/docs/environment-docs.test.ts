...
describe("environment docs", () => {
  it("documents the trusted sources for provider credentials", async () => {
    ...
    expect(markdown).toContain(
      "SupportClaw ignores provider credential environment variables from workspace `.env` files",
    );
    expect(markdown).toContain("~/.supportClaw/.env");
    expect(markdown).toContain("$SUPPORT_CLAW_STATE_DIR/.env");
    expect(markdown).toContain("The config `env` block");
    expect(markdown).toContain("SUPPORT_CLAW_LOAD_SHELL_ENV=1");
