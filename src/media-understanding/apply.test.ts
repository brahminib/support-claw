...
const TEMP_MEDIA_PREFIX = "supportClaw-media-";
...
function createGroqAudioConfig(): SupportClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): SupportClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): SupportClawConfig {
  return {
  ...
  return await withEnvAsync(
    {
      ...
      SUPPORT_CLAW_ANTIGRAVITY_CLI: undefined,
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: SupportClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredSupportClawTmpDir();
  ...
  it("uses Antigravity CLI as the last auto image fallback", async () => {
    ...
    const cfg: SupportClawConfig = { tools: { media: { image: {} } } };
    mockedResolveApiKey.mockResolvedValue({
      source: "none",
      mode: "api-key",
    });
    ...
    expect(probeOptions).toEqual({
      timeoutMs: 3000,
      cwd: expect.stringContaining("supportClaw-antigravity-probe-"),
    });
