...
const hoisted = vi.hoisted(() => ({
  completeMock: vi.fn(),
  ensureSupportClawModelsJsonMock: vi.fn(async () => {}),
...
const {
  completeMock,
  ensureSupportClawModelsJsonMock,
...
vi.mock("../agents/models-config.js", async () => ({
  ...
  ensureSupportClawModelsJson: ensureSupportClawModelsJsonMock,
}));
...
describe("describeImageWithModel", () => {
  ...
  it("resolves configured image models when discovery has not registered the provider", async () => {
    ...
    expect(resolveRequest.agentDir).toBe("/tmp/supportClaw-agent");
    expect(
      requireRecord(
        requireRecord(
