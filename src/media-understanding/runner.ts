...
import { mergeInboundPathRoots } from "@supportclaw/media-core/inbound-path-policy";
import { findNormalizedProviderValue } from "@supportclaw/model-catalog-core/provider-id";
import {
...
async function probeAntigravityCliCandidate(command: string): Promise<string | null> {
  ...
  const probeDir = await fs.mkdtemp(
    path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-antigravity-probe-"),
  );
  try {
...
async function resolveAntigravityCliBinary(): Promise<string | null> {
  ...
  const resolved = (async () => {
    const configured = process.env.SUPPORT_CLAW_ANTIGRAVITY_CLI?.trim();
