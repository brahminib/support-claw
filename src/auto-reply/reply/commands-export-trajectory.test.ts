...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-export-command-"));
  tempDirs.push(dir);
  return dir;
}
...
function makeParams(workspaceDir = makeTempDir()): HandleCommandsParams {
  return {
    cfg: {
      session: {
        store: "/tmp/supportClaw-sessions.json",
      },
    },
    ctx: {
    ...
    execute: vi.fn(async (_toolCallId: string, params: unknown) => {
      ...
      return {
        details: {
          ...
          command: "supportClaw sessions export-trajectory --session-key agent:target:session",
          cwd: "/tmp",
        },
      };
    }),
...
describe("buildExportTrajectoryCommandReply", () => {
  ...
  it("requests per-run exec approval for trajectory exports", async () => {
    ...
    expect(reply.text).toContain("https://docs.supportClaw.ai/tools/trajectory");
    ...
    expect(execCall.defaults.sessionStore).toBe("/tmp/supportClaw-sessions.json");
    ...
    expect(command).not.toContain("supportClaw sessions export-trajectory");
    ...
    expect(String(request.workspace)).toContain("supportClaw-export-command-");
  });
  ...
  it("routes group trajectory export approval privately", async () => {
    ...
    expect(privateReplies[0]?.text).toContain("supportClaw sessions export-trajectory");
