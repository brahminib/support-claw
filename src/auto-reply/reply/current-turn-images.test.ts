...
const originalStateDirEnv = process.env.SUPPORT_CLAW_STATE_DIR;

function restoreProcessState() {
  if (originalStateDirEnv === undefined) {
    delete process.env.SUPPORT_CLAW_STATE_DIR;
  } else {
    process.env.SUPPORT_CLAW_STATE_DIR = originalStateDirEnv;
  }
}
...
describe("resolveCurrentTurnImages", () => {
  ...
  it("hydrates Telegram-style state-relative media into native prompt images", async () => {
    await withTempDir({ prefix: "supportClaw-current-turn-images-" }, async (base) => {
