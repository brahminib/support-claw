...
const mocks = vi.hoisted(() => ({
  ...
  triggerSupportClawRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerSupportClawRestart: mocks.triggerSupportClawRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerSupportClawRestart.mockReset();
    mocks.triggerSupportClawRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerSupportClawRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerSupportClawRestart).not.toHaveBeenCalled();
  ...
  it("clears the success sentinel when fallback restart fails", async () => {
    mocks.triggerSupportClawRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
