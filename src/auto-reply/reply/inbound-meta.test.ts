...
describe("buildInboundMetaSystemPrompt", () => {
  it("includes stable routing fields and omits chat ids", () => {
    ...
    expect(payload["schema"]).toBe("supportClaw.inbound_meta.v2");
...
describe("buildInboundUserContextPrefix", () => {
  it("omits conversation label block for direct chats", () => {
    const text = buildInboundUserContextPrefix({
      ChatType: "direct",
      ConversationLabel: "supportClaw-tui",
    } as TemplateContext);
  ...
  it("does not treat group chats as direct based on sender id", () => {
    const text = buildInboundUserContextPrefix({
      ChatType: "group",
      SenderId: "supportClaw-control-ui",
      MessageSid: "123",
    ...
    expect(conversationInfo["sender_id"]).toBe("supportClaw-control-ui");
  ...
  it("renders chat window structured context as compact transcript text", () => {
              ...
              messages: [
                {
                  ...
                  media_path: "/home/user/.supportClaw/media/inbound/sticker.webp",
    ...
    expect(text).not.toContain("/home/user/.supportClaw/media/inbound/sticker.webp");
  ...
  it("includes inbound history media metadata without leaking paths or URLs", () => {
      ...
      InboundHistory: [
        {
          ...
          media: [
            {
              path: "/tmp/supportClaw-secret-image.png",
    ...
    expect(text).not.toContain("/tmp/supportClaw-secret-image.png");
