...
import {
  buildCurrentSupportClawCliCommand,
...
const DIAGNOSTICS_DOCS_URL = "https://docs.supportClaw.ai/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "supportClaw gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentSupportClawCliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
async function requestGatewayDiagnosticsExportApproval(
  ...
  try {
    ...
    const result = await execTool.execute("chat-diagnostics-gateway-export", {
      command,
      env: buildCurrentSupportClawCliExecEnv(),
      security: "allowlist",
      ask: "always",
      background: true,
      timeout: timeoutSec,
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this SupportClaw session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an SupportClaw session file.",
    ) === true
  );
}
