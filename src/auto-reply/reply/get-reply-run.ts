...
import { asDateTimestampMs } from "@supportclaw/normalization-core/number-coercion";
import { type FastMode, normalizeOptionalString } from "@supportclaw/normalization-core/string-coerce";
import {
...
type AgentDefaults = NonNullable<SupportClawConfig["agents"]>["defaults"];
...
type RunPreparedReplyParams = {
  ...
  sessionCfg: SupportClawConfig["session"];
...
export async function runPreparedReply(
  ...
  const useFastReplyRuntime = shouldUseReplyFastTestRuntime({
    cfg,
    isFastTestEnv: process.env.SUPPORT_CLAW_TEST_FAST === "1",
  });
  ...
  const userTurnInput =
    userTurnTranscriptText !== undefined || userTurnMediaForPersistence.length > 0
      ? {
          ...
          // replayed as history. See: https://github.com/brahminib/support-claw/issues/3658
