...
export function resolveAcpInstallCommandHint(cfg: SupportClawConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `supportClaw plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^supportClaw plugins install /u, "");
    ...
    return "supportClaw plugins install acpx";
  }
