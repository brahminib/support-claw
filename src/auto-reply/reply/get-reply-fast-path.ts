...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("supportClaw.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("supportClaw.reply.full-runtime");

type ReplyConfigWithMarker = SupportClawConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.SUPPORT_CLAW_ALLOW_SLOW_REPLY_TESTS === "1" || env.SUPPORT_CLAW_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends SupportClawConfig>(
  config: T,
...
export function withFastReplyConfig<T extends SupportClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends SupportClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

function isCompleteReplyConfig(config: unknown): config is SupportClawConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  getRuntimeConfig: () => SupportClawConfig;
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set SUPPORT_CLAW_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as SupportClawConfig;
}
