...
describe("resolveQueueSettings runtime defaults", () => {
  it("uses defaults from already-loaded channel plugins", async () => {
    ...
    expect(resolveQueueSettings({ cfg: {} as SupportClawConfig, channel: "demo" })).toEqual({
      mode: "steer",
      debounceMs: 125,
      cap: 20,
  ...
  it("falls back without loading bundled channel plugins", async () => {
    ...
    expect(resolveQueueSettings({ cfg: {} as SupportClawConfig, channel: "telegram" })).toEqual({
      mode: "steer",
      debounceMs: 500,
      cap: 20,
      dropPolicy: "summarize",
