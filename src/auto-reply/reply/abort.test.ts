...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop supportClaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@supportclaw_bot", { botUsername: "supportClaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@supportclaw_bot", { botUsername: "supportClaw_bot" })).toBe(true);
