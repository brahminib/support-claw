...
import { createSupportClawCodingTools } from "../../agents/agent-tools.js";
...
const { createSupportClawCodingToolsMock } = vi.hoisted(() => ({
  createSupportClawCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/agent-tools.js", () => ({
  createSupportClawCodingTools: createSupportClawCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createSupportClawCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    const toolParams = requireFirstArg(
      vi.mocked(createSupportClawCodingTools),
  ...
  it("uses materialized sandbox skill paths for sandbox command prompts", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-command-sandbox-skills-"));
    try {
      ...
      vi.mocked(resolveReusableWorkspaceSkillSnapshot).mockReturnValue({
        snapshot: {
          prompt:
            "<available_skills>~/.npm-global/lib/node_modules/supportClaw/skills/gog/SKILL.md</available_skills>",
          skills: [],
          resolvedSkills: [],
        },
        shouldRefresh: false,
  ...
  it("uses config-backed prompt settings for the target agent", async () => {
    ...
    createSupportClawCodingToolsMock.mockReturnValue([{ name: "sessions_spawn" }] as never);
    const params = makeParams();
    params.cfg = {
      agents: {
        defaults: {
