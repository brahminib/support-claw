...
vi.mock("../../agents/harness/builtin-supportClaw.js", () => ({
  createSupportClawAgentHarness: () => ({
    id: "supportClaw",
    label: "SupportClaw Default",
...
function saveStatusTestAuthProfiles(params: {
  ...
  const agentDir = path.join(params.dir, ".supportClaw", "agents", "main", "agent");
...
describe("buildStatusReply subagent summary", () => {
  ...
  it("does not leak internal runtime context through the task status line", async () => {
    ...
    expect(reply?.text).not.toContain("SupportClaw runtime context (internal):");
  ...
  it("shows the effective non-SupportClaw embedded harness in /status", async () => {
  ...
  it("uses Codex OAuth auth labels for openai models running on the Codex harness", async () => {
    ...
    await withTempHome(
      async (dir) => {
        const agentDir = path.join(dir, ".supportClaw", "agents", "main", "agent");
        fs.mkdirSync(agentDir, { recursive: true });
        saveAuthProfileStore(
          {
            version: 1,
  ...
  it("uses Codex OAuth auth labels for explicit OpenAI SupportClaw auth order", async () => {
    await withTempHome(
      async (dir) => {
        const agentDir = path.join(dir, ".supportClaw", "agents", "main", "agent");
        fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("uses workspace-scoped auth evidence in /status auth labels", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-status-auth-label-"));
    const workspaceDir = path.join(tempRoot, "workspace");
    const pluginDir = path.join(workspaceDir, ".supportClaw", "extensions", "workspace-auth-label");
  ...
  it("keeps /status on a session-pinned SupportClaw harness after config changes", async () => {
    registerStatusCodexHarness();
