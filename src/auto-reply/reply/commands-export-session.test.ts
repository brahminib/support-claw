...
describe("buildExportSessionReply", () => {
  ...
  it("injects scripts and session data through the real export template", async () => {
    ...
    expect(html).not.toContain("data-supportClaw-export-placeholder");
  ...
  it("suffixes colliding default export filenames instead of overwriting", async () => {
    ...
    const expectedBase = path.join(
      "/tmp/workspace",
      "supportClaw-session-session--2026-05-05T10-11-12.html",
    );
    const expectedSuffix = path.join(
      "/tmp/workspace",
      "supportClaw-session-session--2026-05-05T10-11-12-2.html",
    );
    ...
    expect(reply.text).toContain("📄 File: supportClaw-session-session--2026-05-05T10-11-12-2.html");
  });
  ...
  it("preserves replacement text with dollar sequences", async () => {
    hoisted.exportHtmlTemplateContents.set(
      "template.html",
      [
        '<style data-supportClaw-export-placeholder="CSS"></style>',
        '<script id="session-data" type="application/json" data-supportClaw-export-placeholder="SESSION_DATA"></script>',
        '<script data-supportClaw-export-placeholder="MARKED_JS"></script>',
        '<script data-supportClaw-export-placeholder="HIGHLIGHT_JS"></script>',
        '<script data-supportClaw-export-placeholder="JS"></script>',
      ].join(""),
    );
