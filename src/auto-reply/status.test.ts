...
describe("buildStatusMessage", () => {
  it("summarizes agent readiness and context usage", () => {
    ...
    expect(normalized).toContain("SupportClaw");
    ...
    expect(normalized).toContain("Runtime: SupportClaw Default");
  ...
  it("shows the default SupportClaw harness as the model runtime", () => {
    const text = buildStatusMessage({
      agent: {
        model: "openai/gpt-5.4",
      },
      sessionEntry: {
        sessionId: "supportClaw-harness",
        updatedAt: 0,
        fastMode: true,
      },
      sessionKey: "agent:main:main",
    ...
    expect(normalized).toContain("Fast");
    expect(normalized).toContain("Runtime: SupportClaw Default");
    expect(normalized).not.toContain("· supportClaw");
  });
