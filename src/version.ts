...
declare const __SUPPORT_CLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "supportClaw";
...
function readInjectedVersion(): string | undefined {
  return typeof __SUPPORT_CLAW_VERSION__ === "string" ? __SUPPORT_CLAW_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["SUPPORT_CLAW_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["SUPPORT_CLAW_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["SUPPORT_CLAW_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current SupportClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.SUPPORT_CLAW_BUNDLED_VERSION,
});
