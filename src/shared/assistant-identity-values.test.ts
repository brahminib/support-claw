...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  SupportClaw  ", 20)).toBe("SupportClaw");
    expect(coerceIdentityValue("  SupportClaw  ", 8)).toBe("SupportClaw");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  SupportClaw Assistant  ", 8)).toBe("SupportClaw");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  SupportClaw  ", 0)).toBe("");
    expect(coerceIdentityValue("  SupportClaw  ", -1)).toBe("OpenCla");
  });
});
