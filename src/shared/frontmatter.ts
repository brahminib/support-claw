...
import { normalizeCsvOrLooseStringList } from "@supportclaw/normalization-core/string-normalization";
...
/** Parses the JSON5 SupportClaw manifest block embedded inside a string frontmatter field. */
export function resolveSupportClawManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type SupportClawManifestRequires = {
...
/** Extracts normalized runtime requirement lists from an SupportClaw manifest block. */
export function resolveSupportClawManifestRequires(
  metadataObj: Record<string, unknown>,
): SupportClawManifestRequires | undefined {
  const requiresRaw =
...
export function resolveSupportClawManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
/** Extracts normalized OS allowlist entries from an SupportClaw manifest block. */
export function resolveSupportClawManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedSupportClawManifestInstallBase = {
...
export function parseSupportClawManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedSupportClawManifestInstallBase | undefined {
  ...
  const spec: ParsedSupportClawManifestInstallBase = {
    raw,
    kind,
  };
...
export function applySupportClawManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedSupportClawManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
