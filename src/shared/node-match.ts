...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current SupportClaw nodes over legacy migration ties. */
  clientId?: string;
};
...
function isCurrentSupportClawClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("supportClaw-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentSupportClawClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
  ...
  // During Clawdbot -> SupportClaw migration, a unique current client should win only
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentSupportClawClient(node.clientId)) {
    score += 10;
