...
/**
 * Runtime bridge between MCP tools and the SupportClaw Gateway channel APIs.
 *
...
export class SupportClawChannelBridge {
  ...
  constructor(
    private readonly cfg: SupportClawConfig,
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClientCtor({
      ...
      clientDisplayName: "SupportClaw MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`supportClaw mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
  ...
  private async sendNotification(notification: ServerNotification): Promise<void> {
    ...
    try {
      ...
      process.stderr.write(`supportClaw mcp: notification ${notification.method} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `supportClaw mcp: notification ${notification.method} error: ${String(error)}\n`,
        );
      }
    }
  }
