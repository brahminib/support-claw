/**
 * Standalone MCP server that exposes SupportClaw plugin-registered tools
...
function resolvePluginToolPolicy(config: SupportClawConfig): {
...
function resolveTools(config: SupportClawConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "supportClaw-plugin-tools", tools });
}
