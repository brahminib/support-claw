...
vi.mock("./channel-bridge.js", () => ({
  SupportClawChannelBridge: class MockSupportClawChannelBridge {
...
describe("serveSupportClawChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveSupportClawChannelMcp } = await import("./channel-server.js");

    const servePromise = serveSupportClawChannelMcp({ verbose: false });
