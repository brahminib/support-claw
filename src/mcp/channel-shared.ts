...
/** Resolve the visible message id, including SupportClaw metadata attached to raw entries. */
export function resolveMessageId(entry: Record<string, unknown>): string | undefined {
  return (
    toText(entry.id) ??
    (entry["__supportClaw"] && typeof entry["__supportClaw"] === "object"
      ? toText((entry["__supportClaw"] as { id?: unknown }).id)
      : undefined)
  );
}
