...
import { createSupportClawChannelMcpServer, SupportClawChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createSupportClawChannelMcpServer({
...
describe("supportClaw channel mcp server", () => {
  ...
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      ...
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new SupportClawChannelBridge({} as never, {
        ...
        expect((messages[1]?.["__supportClaw"] as { id?: string } | undefined)?.id).toBe(
          "msg-attachment",
        );
        expect(
