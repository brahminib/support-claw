...
function logMigrationHint(runtime: RuntimeEnv, candidate: ResolvedProviderCandidate): void {
  const command = formatCliCommand(`supportClaw migrate ${candidate.provider.id} --dry-run`);
 ...
 * prompt) is owned by `supportClaw migrate <provider>`; this helper only owns
 * the gate prompt.
 */
    ...
    try {
      ...
      params.runtime.log(
        `${candidate.provider.label} migration failed: ${formatErrorMessage(error)}. ` +
          `Re-run with ${formatCliCommand(`supportClaw migrate ${candidate.provider.id} --dry-run`)} to inspect.`,
      );
    } finally {
