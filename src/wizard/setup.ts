// Setup wizard orchestrates onboarding prompts and generated SupportClaw config.
...
async function writeWizardConfigFile(
  configInput: SupportClawConfig,
  opts: {
    allowConfigSizeDrop?: boolean;
    migrationBaseConfig?: SupportClawConfig;
...
export async function runSetupWizard(
  ...
  let baseConfig: SupportClawConfig = snapshot.valid
    ? snapshot.exists
  ...
  let pendingPluginInstallMigrationBaseConfig: SupportClawConfig | undefined = baseConfig;
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("supportClaw doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("supportClaw doctor")}`,
        `Inspect: ${formatCliCommand("supportClaw plugins inspect --all")}`,
      ].join("\n"),
  ...
  const quickstartHint = t("wizard.setup.flowQuickstartHint", {
    command: formatCliCommand("supportClaw configure"),
  });
  ...
  if (
    ...
    runtime.error(
      "Invalid --flow. Use quickstart, manual, advanced, or import. Example: supportClaw onboard --flow quickstart",
    );
    runtime.exit(1);
    return;
  }
  ...
  let localGatewayToken = process.env.SUPPORT_CLAW_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.SUPPORT_CLAW_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: SupportClawConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
  if (opts.skipBootstrap) {
    nextConfig = applySkipBootstrapConfig(nextConfig);
  }
