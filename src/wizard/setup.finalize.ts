...
export async function finalizeSetupWizard(
  ...
  if (!opts.skipHealth) {
    ...
    if (gatewayProbe.ok) {
      try {
        const healthConfig: SupportClawConfig =
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.startGatewayNow", {
            command: formatCliCommand("supportClaw gateway run"),
          }),
          t("wizard.finalize.rerunInstallDaemon", {
            command: formatCliCommand("supportClaw onboard --install-daemon"),
          }),
          t("wizard.finalize.skipHealthNextTime", {
            command: formatCliCommand("supportClaw onboard --skip-health"),
          }),
        ].join("\n"),
        "Gateway",
      );
    ...
    if (gatewayProbe.ok) {
      const tokenNotes = [
        ...
        t("wizard.finalize.gatewayTokenView", {
          command: formatCliCommand("supportClaw config get gateway.auth.token"),
        }),
        t("wizard.finalize.gatewayTokenGenerate", {
          command: formatCliCommand("supportClaw doctor --generate-gateway-token"),
        }),
        suppressGatewayTokenOutput ? undefined : t("wizard.finalize.dashboardTokenMemory"),
        t("wizard.finalize.dashboardOpenAnytime", {
          command: formatCliCommand("supportClaw dashboard --no-open"),
        }),
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("supportClaw configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.webSearchGetKey", {
            url: entry?.signupUrl ?? "https://docs.supportClaw.ai/tools/web",
          }),
          t("wizard.finalize.webDocs"),
        ].join("\n"),
        t("wizard.finalize.webSearchTitle"),
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.webSearchReenable", {
            command: formatCliCommand("supportClaw configure --section web"),
          }),
          "",
          t("wizard.finalize.webDocs"),
        ].join("\n"),
