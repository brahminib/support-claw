...
function hasMeaningfulConfig(config: SupportClawConfig): boolean {
...
function assertFreshSetupMigrationTarget(freshness: {
  ...
  if (freshness.fresh || process.env.SUPPORT_CLAW_MIGRATION_EXISTING_IMPORT === "1") {
    return;
  }
  throw new Error(
    [
      "Migration import during onboarding requires a fresh SupportClaw setup.",
...
export async function runSetupMigrationImport(params: {
  ...
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  commitConfigFile: (config: SupportClawConfig) => Promise<SupportClawConfig>;
}): Promise<void> {
  const [
  ...
  // can reference a concrete SupportClaw config target.
