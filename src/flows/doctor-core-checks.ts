...
import type { ConfigValidationIssue, SupportClawConfig } from "../config/types.supportClaw.js";
...
export type CoreHealthCheckDeps = {
  readonly detectUnavailableSkills: (cfg: SupportClawConfig) => Promise<readonly SkillStatusEntry[]>;
  readonly collectSecurityWarnings: (cfg: SupportClawConfig) => Promise<readonly string[]>;
...
async function collectSecurityWarningsWithRuntime(cfg: SupportClawConfig): Promise<readonly string[]> {
...
const gatewayConfigCheck: HealthCheck = {
  ...
  kind: "core",
  description: "supportClaw.jsonc gateway block is set and unambiguous.",
  source: "doctor",
  async detect(ctx) {
    ...
    if (!ctx.cfg.gateway?.mode) {
      findings.push({
        ...
        path: "gateway.mode",
        fixHint:
          "Run `supportClaw configure` and set Gateway mode (local/remote), or `supportClaw config set gateway.mode local`.",
      });
    }
    if (ctx.cfg.gateway?.mode !== "remote" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
      findings.push({
        ...
        fixHint:
          "Set an explicit mode: `supportClaw config set gateway.auth.mode token` or `... password`.",
      });
    }
    return findings;
  },
    ...
    return [
      {
        ...
        fixHint:
          "Set commands.ownerAllowFrom to your channel user id, e.g. `supportClaw config set commands.ownerAllowFrom '[\"telegram:123456789\"]'`.",
      },
    ];
  },
};

function resolveDoctorMode(cfg: SupportClawConfig): "local" | "remote" {
...
export function buildGatewayTokenSecretRefFixHint(ref: SecretRef): string {
  if (ref.source === "exec") {
    return "Run `supportClaw doctor --allow-exec` to verify exec SecretRefs during doctor, or `supportClaw secrets audit --allow-exec` to audit all exec SecretRefs.";
  }
    ...
    return [
      {
        ...
        path: "gateway.auth",
        fixHint: "Run `supportClaw doctor --fix --generate-gateway-token` to generate a token.",
      },
    ];
  },
};
  ...
  async detect(ctx) {
    ...
    return detected.preview.map(
      (line): HealthFinding => ({
        ...
        fixHint: "Run `supportClaw doctor --fix` to migrate legacy state.",
      }),
    );
  },
};
      ...
      (issue): HealthFinding => ({
        ...
        fixHint: issue.blockedOutsideEntry
          ? [
              ...
              "or set the affected OpenAI models to an SupportClaw runtime policy.",
            ].join(" ")
          : [
              "Run `supportClaw doctor --fix`: it enables plugins.entries.codex,",
              "or set the affected OpenAI models to an SupportClaw runtime policy.",
            ].join(" "),
      }),
    );
...
function unavailableSkillToFinding(skill: SkillStatusEntry): HealthFinding {
  return {
    ...
    fixHint:
      "Install/configure the missing requirement, or run `supportClaw doctor --fix` to disable unused unavailable skills.",
  };
}
...
function browserResidueFinding(residue: LegacyClawdBrowserProfileResidue): HealthFinding {
  return {
    ...
    fixHint:
      "Run `supportClaw doctor --fix` to archive the stale clawd profile safely instead of deleting it in place.",
  };
}
...
const browserClawdProfileResidueCheck: HealthCheck = {
  ...
  kind: "core",
  description:
    "Legacy clawd managed browser profile residue has been archived after the SupportClaw rename.",
  source: "doctor",
  async detect(ctx, scope) {
...
const finalConfigValidationCheck: HealthCheck = {
  ...
  kind: "core",
  description: "Active supportClaw.jsonc parses and conforms to the config schema.",
  source: "doctor",
  async detect() {
