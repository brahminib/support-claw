...
type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<SupportClawConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<SupportClawConfig["tools"]>["web"]>["search"]>;
...
const WEB_SEARCH_DOCS_URL = "https://docs.supportClaw.ai/tools/web";
...
function rawKeyValue(config: SupportClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: SupportClawConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: SupportClawConfig, provider: SearchProvider): SecretRef {
  const entry =
...
export function applySearchKey(
  ...
  const nextBase: SupportClawConfig = {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: SupportClawConfig, result: SupportClawConfig): SupportClawConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as SupportClawConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "SupportClaw will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.supportClaw.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — SupportClaw will store a reference instead of the API key.",
