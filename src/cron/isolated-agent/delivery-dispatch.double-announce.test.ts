...
describe("dispatchCronDelivery — double-announce guard", () => {
  ...
  it("keeps same-recipient message-tool awareness separate across channels", async () => {
    vi.mocked(resolveOutboundSessionRoute)
      ...
      .mockResolvedValueOnce({
        sessionKey: "agent:main:supportClaw-weixin:direct:123456",
        baseSessionKey: "agent:main:supportClaw-weixin:direct:123456",
        peer: { kind: "direct", id: "123456" },
        chatType: "direct",
        from: "supportClaw-weixin:123456",
        to: "123456",
      });
    ...
    expect(enqueueSystemEvent).toHaveBeenCalledWith(
      ...
      {
        sessionKey: "agent:main:supportClaw-weixin:direct:123456",
        contextKey: "cron-direct-delivery:v1:cron:test-job:1000:supportClaw-weixin::123456:",
      },
    );
  });
  ...
  it("queues message-tool awareness for explicit off-plan message-tool deliveries", async () => {
    mockResolvedOutboundRoute({
      sessionKey: "agent:main:supportClaw-weixin:direct:user-123",
      baseSessionKey: "agent:main:supportClaw-weixin:direct:user-123",
      to: "user-123",
    });
    ...
    expect(enqueueSystemEvent).toHaveBeenCalledExactlyOnceWith(
      ...
      {
        sessionKey: "agent:main:supportClaw-weixin:direct:user-123",
        contextKey: "cron-direct-delivery:v1:cron:test-job:1000:supportClaw-weixin::user-123:",
      },
    );
  });
  ...
  it("keeps unresolved message-tool delivery out of delivered status", async () => {
    ...
    expect(state.result?.error).toContain(
      "the agent used the message tool, but SupportClaw could not verify",
    );
  });
