...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses SUPPORT_CLAW_HOME for tilde expansion", () => {
    vi.stubEnv("SUPPORT_CLAW_HOME", "/srv/supportClaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/supportClaw-home", "cron", "jobs.json"));
  });
});
