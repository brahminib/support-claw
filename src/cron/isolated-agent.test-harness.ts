...
/** Runs a test callback with an isolated SupportClaw home for cron tests. */
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "supportClaw-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".supportClaw", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<SupportClawConfig> = {},
): SupportClawConfig {
  const base: SupportClawConfig = {
    agents: {
      defaults: {
