...
function listAgents(cfg: SupportClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: SupportClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<SupportClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: SupportClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: SupportClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: SupportClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<SupportClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  SupportClawConfig,
  {
    bindingsRef: SupportClawConfig["bindings"];
    agentsRef: SupportClawConfig["agents"];
    sessionRef: SupportClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: SupportClawConfig): Map<string, ResolvedAgentRoute> {
