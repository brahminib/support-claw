...
const createTempDir = () => tempDirs.make("supportClaw-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets SUPPORT_CLAW_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "supportClaw",
    });

    expect(env.SUPPORT_CLAW_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("supportClaw");
  });

  it("overrides pre-existing SUPPORT_CLAW_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      SUPPORT_CLAW_SHELL: "wrong",
    });
    expect(env.SUPPORT_CLAW_SHELL).toBe("acp-client");
  });
  ...
  it("preserves SUPPORT_CLAW_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        SUPPORT_CLAW_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["SUPPORT_CLAW_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default SupportClaw bridge", () => {
    ...
    expect(env.SUPPORT_CLAW_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.SUPPORT_CLAW_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "supportClaw",
        serverArgs: ["acp"],
        defaultServerCommand: "supportClaw",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("SUPPORT_CLAW_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "supportClaw", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "supportClaw", "dist", "entry.js");
    const shimPath = path.join(dir, "supportClaw.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\supportClaw\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/supportClaw-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/supportClaw-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/supportClaw-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
