...
import { AcpRuntimeError } from "@supportclaw/acp-core/runtime/errors";
...
export function isAcpEnabledByPolicy(cfg: SupportClawConfig): boolean {
  return cfg.acp?.enabled !== false;
}

/** Resolves the effective dispatch policy state for inbound ACP routing. */
function resolveAcpDispatchPolicyState(cfg: SupportClawConfig): AcpDispatchPolicyState {
...
export function resolveAcpDispatchPolicyMessage(cfg: SupportClawConfig): string | null {
...
export function resolveAcpDispatchPolicyError(cfg: SupportClawConfig): AcpRuntimeError | null {
...
export function resolveAcpExplicitTurnPolicyError(cfg: SupportClawConfig): AcpRuntimeError | null {
  if (isAcpEnabledByPolicy(cfg)) {
    return null;
  }
...
function isAcpAgentAllowedByPolicy(cfg: SupportClawConfig, agentId: string): boolean {
