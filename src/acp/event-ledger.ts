...
import { resolveIntegerOption } from "@supportclaw/acp-core/numeric-options";
...
/** Resolves the legacy file-backed ACP ledger path under the SupportClaw state directory. */
...
export async function migrateFileAcpEventLedgerToSqlite(
  params: { filePath: string; archiveSource?: boolean } & SupportClawStateDatabaseOptions,
...
export function createSqliteAcpEventLedger(
  params: SupportClawStateDatabaseOptions & LedgerOptions = {},
): AcpEventLedger {
  ...
  const mutate = (fn: (db: DatabaseSync) => void) =>
    runSupportClawStateWriteTransaction((database) => fn(database.db), dbOptions);
  const read = <T>(fn: (db: DatabaseSync) => T): T => fn(openSupportClawStateDatabase(dbOptions).db);
