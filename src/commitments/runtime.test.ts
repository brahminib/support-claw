...
function requireFirstEmbeddedAgentRequest(): {
  ...
  if (!call) {
    throw new Error("expected embedded SupportClaw agent extraction request");
  }
  const [request] = call;
...
describe("commitment extraction runtime", () => {
  ...
  async function createConfig(): Promise<SupportClawConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-commitment-runtime-"));
    tmpDirs.push(tmpDir);
    stateDirEnvSnapshot ??= captureEnv(["SUPPORT_CLAW_STATE_DIR"]);
    setTestEnvValue("SUPPORT_CLAW_STATE_DIR", tmpDir);
    return {
      commitments: {
        enabled: true,
      },
