...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "SUPPORT_CLAW_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "supportClaw", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("supportClaw channels login --channel whatsapp");
  });
