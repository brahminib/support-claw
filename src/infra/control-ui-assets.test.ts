...
vi.mock("./supportClaw-root.js", () => ({
  resolveSupportClawPackageRoot: vi.fn(async () => null),
  resolveSupportClawPackageRootSync: vi.fn(() => null),
}));
...
let supportClawRoot: typeof import("./supportClaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    supportClawRoot = await import("./supportClaw-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/supportClaw");
    const wrapperArgv1 = abs("fixtures/bin/supportClaw");
  ...
  it("uses resolveSupportClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/supportClaw");
    (
      supportClawRoot.resolveSupportClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/supportClaw"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "supportClaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "supportClawCli.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-supportClaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", () => {
    const pkgRoot = abs("fixtures/supportClaw-bundle");
    (
      supportClawRoot.resolveSupportClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/SupportClaw.app/Contents/MacOS/SupportClaw");
    const bundledUiDir = abs("fixtures/SupportClaw.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/supportClaw-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
