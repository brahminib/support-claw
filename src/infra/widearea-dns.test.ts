...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "supportClaw.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (SupportClaw)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "supportClaw.internal", expected: "supportClaw.internal." },
    { value: "supportClaw.internal.", expected: "supportClaw.internal." },
    { value: "  supportClaw.internal  ", expected: "supportClaw.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { SUPPORT_CLAW_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { SUPPORT_CLAW_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
    ...
    },
    {
      ...
      params: {
        env: { SUPPORT_CLAW_WIDE_AREA_DOMAIN: "foo/bar" } as NodeJS.ProcessEnv,
      },
      expected: null,
    },
  ...
  it("builds valid zone paths under the DNS config directory", () => {
    ...
    const zonePath = getWideAreaZonePath("supportClaw.internal.");

    expect(zonePath).toBe(path.join(dnsDir, "supportClaw.internal.db"));
    expect(path.relative(dnsDir, zonePath)).toBe("supportClaw.internal.db");
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN supportClaw.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_supportClaw-gw._tcp IN PTR studio-london._supportClaw-gw._tcp`,
      `studio-london._supportClaw-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (SupportClaw)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/supportClaw`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN supportClaw.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._supportClaw-gw._tcp IN TXT`,
        `displayName=Mac Studio (SupportClaw)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("supportClaw.internal."),
      changed: false,
    });
    expect(writeSpy).not.toHaveBeenCalled();
  });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("supportClaw.internal."),
      expectedZoneText,
      "utf-8",
    );
  });
