...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "SUPPORT_CLAW_LIVE_ANTHROPIC_KEYS",
  "SUPPORT_CLAW_LIVE_GEMINI_KEY",
  "SUPPORT_CLAW_LIVE_OPENAI_KEY",
  "SUPPORT_CLAW_GATEWAY_TOKEN",
  "SUPPORT_CLAW_GATEWAY_PASSWORD",
  "SUPPORT_CLAW_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "SUPPORT_CLAW_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "SUPPORT_CLAW_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "SUPPORT_CLAW_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "SUPPORT_CLAW_BUNDLED_SKILLS_DIR=./attacker-skills",
  "SUPPORT_CLAW_SKIP_BROWSER_CONTROL_SERVER=1",
] as const;
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-dotenv-test-"));
  const cwdDir = path.join(base, "cwd");
  const stateDir = path.join(base, "state");
  setTestEnvValue("SUPPORT_CLAW_STATE_DIR", stateDir);
...
describe("loadDotEnv", () => {
  it("loads ~/.supportClaw/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.supportClaw/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        setTestEnvValue("HOME", base);
        const defaultStateDir = path.join(base, ".supportClaw");
        setTestEnvValue("SUPPORT_CLAW_STATE_DIR", defaultStateDir);
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "supportClaw", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "SUPPORT_CLAW_STATE_DIR=./evil-state",
            "SUPPORT_CLAW_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.SUPPORT_CLAW_STATE_DIR).toBe(stateDir);
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "SUPPORT_CLAW_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "SUPPORT_CLAW_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "SUPPORT_CLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "SUPPORT_CLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "SUPPORT_CLAW_GATEWAY_TOKEN=attacker-token",
            "SUPPORT_CLAW_GATEWAY_PASSWORD=attacker-password",
            "SUPPORT_CLAW_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks state-directory controls from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.SUPPORT_CLAW_STATE_DIR).toBeUndefined();
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "SUPPORT_CLAW_AGENT_DIR=./evil-agent",
            `SUPPORT_CLAW_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "SUPPORT_CLAW_OAUTH_DIR=./evil-oauth",
            "PI_CODING_AGENT_DIR=./evil-pi-agent",
          ].join("\n"),
        );

        deleteTestEnvValue("SUPPORT_CLAW_AGENT_DIR");
        delete process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR;
        delete process.env.SUPPORT_CLAW_OAUTH_DIR;
        delete process.env.PI_CODING_AGENT_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.SUPPORT_CLAW_AGENT_DIR).toBeUndefined();
        expect(process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.SUPPORT_CLAW_OAUTH_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
      });
    });
  });

  it("blocks SUPPORT_CLAW_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "SUPPORT_CLAW_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.SUPPORT_CLAW_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.SUPPORT_CLAW_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "SUPPORT_CLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1",
            'SUPPORT_CLAW_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );

        delete process.env.SUPPORT_CLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES;
        delete process.env.SUPPORT_CLAW_PLUGIN_INSTALL_OVERRIDES;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.SUPPORT_CLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.SUPPORT_CLAW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "SUPPORT_CLAW_PINNED_PYTHON=./attacker-python",
            "SUPPORT_CLAW_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.SUPPORT_CLAW_PINNED_PYTHON;
        delete process.env.SUPPORT_CLAW_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.SUPPORT_CLAW_PINNED_PYTHON).toBeUndefined();
        expect(process.env.SUPPORT_CLAW_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "SUPPORT_CLAW_PINNED_PYTHON=/trusted/python",
            "SUPPORT_CLAW_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.SUPPORT_CLAW_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.SUPPORT_CLAW_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "SUPPORT_CLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "SUPPORT_CLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "SUPPORT_CLAW_GATEWAY_TOKEN=trusted-token",
            "SUPPORT_CLAW_GATEWAY_PASSWORD=trusted-password",
            "SUPPORT_CLAW_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.SUPPORT_CLAW_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.SUPPORT_CLAW_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.SUPPORT_CLAW_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.SUPPORT_CLAW_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.SUPPORT_CLAW_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.SUPPORT_CLAW_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.SUPPORT_CLAW_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "SUPPORT_CLAW_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks SUPPORT_CLAW_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "SUPPORT_CLAW_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when SUPPORT_CLAW_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        setTestEnvValue("HOME", base);
        setTestEnvValue("SUPPORT_CLAW_STATE_DIR", customStateDir);
        await writeEnvFile(
          path.join(base, ".config", "supportClaw", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.FOO).toBeUndefined();
        expect(process.env.SUPPORT_CLAW_STATE_DIR).toBe(customStateDir);
        expect(process.env.BAR).toBeUndefined();
      });
    });
  });
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
      const legacyStateDir = path.join(base, ".clawdbot");
      setTestEnvValue("HOME", base);
      deleteTestEnvValue("SUPPORT_CLAW_STATE_DIR");
      delete process.env.SUPPORT_CLAW_TEST_FAST;
      await fs.mkdir(cwdDir, { recursive: true });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        const plugin: PluginManifestRecord = {
          ...
          manifestPath: "/plugins/runtime-cloud/supportClaw.plugin.json",
          providerAuthEnvVars: {
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "SUPPORT_CLAW_MDNS_HOSTNAME",
          "SUPPORT_CLAW_SESSION_CACHE_TTL_MS",
          "SUPPORT_CLAW_UPDATE_PACKAGE_SPEC",
          "SUPPORT_CLAW_GATEWAY_PORT",
          "SUPPORT_CLAW_GATEWAY_URL",
          "SUPPORT_CLAW_CLAWHUB_URL",
          "CLAWHUB_URL",
          "SUPPORT_CLAW_CLAWHUB_TOKEN",
          "CLAWHUB_TOKEN",
          "CLAWHUB_AUTH_TOKEN",
          "CLAWHUB_CONFIG_PATH",
          "SUPPORT_CLAW_DISABLE_BUNDLED_PLUGINS",
          "SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS",
          "SUPPORT_CLAW_BROWSER_EXECUTABLE_PATH",
          "EXAMPLE_API_HOST",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "IRC_HOST",
          ...
          "SUPPORT_CLAW_SKIP_PROVIDERS",
          "SUPPORT_CLAW_SKIP_CRON",
          "SUPPORT_CLAW_RAW_STREAM",
          "SUPPORT_CLAW_RAW_STREAM_PATH",
          "SUPPORT_CLAW_CACHE_TRACE",
          "SUPPORT_CLAW_CACHE_TRACE_FILE",
          "SUPPORT_CLAW_CACHE_TRACE_MESSAGES",
          "SUPPORT_CLAW_CACHE_TRACE_PROMPT",
          "SUPPORT_CLAW_CACHE_TRACE_SYSTEM",
          "SUPPORT_CLAW_SHOW_SECRETS",
          "SUPPORT_CLAW_PLUGIN_CATALOG_PATHS",
          "SUPPORT_CLAW_MPM_CATALOG_PATHS",
          "SUPPORT_CLAW_NODE_EXEC_HOST",
          "SUPPORT_CLAW_NODE_EXEC_FALLBACK",
          "SUPPORT_CLAW_ALLOW_PROJECT_LOCAL_BIN",
          "PATH",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "SystemRoot",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("supportClaw/supportClaw");
