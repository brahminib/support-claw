...
function normalizeNpmViewMetadata(value: unknown): NpmSpecResolution | null {
  ...
  return {
    ...
    ...(isRecord(rec.supportClaw) ? { packageSupportClaw: rec.supportClaw } : {}),
  };
}
...
> {
  ...
  if (res.code !== 0) {
    ...
    if (/E404|is not in this registry/i.test(raw)) {
      return {
        ok: false,
        error: `Package not found on npm: ${params.spec}. See https://docs.supportClaw.ai/tools/plugin for installable plugins.`,
      };
    }
