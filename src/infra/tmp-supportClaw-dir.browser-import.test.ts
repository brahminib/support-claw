...
describe("tmp-supportClaw-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__supportClawTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_SUPPORT_CLAW_TMP_DIR, resolvePreferredSupportClawTmpDir } from "./src/infra/tmp-supportClaw-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_SUPPORT_CLAW_TMP_DIR,
            resolverType: typeof resolvePreferredSupportClawTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-supportClaw-dir-browser-entry.ts",
      },
      write: false,
    });
