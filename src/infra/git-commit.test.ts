...
async function makeTempDir(label: string): Promise<string> {
  return await tempDirs.make(`supportClaw-${label}-`);
}
...
async function makeFakeSupportClawPackage(root: string) {
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the supportClaw package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "supportClaw");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "supportClaw", version: "2026.3.10" }),
      "utf-8",
    );
