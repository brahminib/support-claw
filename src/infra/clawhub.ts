...
import { readResponseWithLimit } from "@supportclaw/media-core/read-response-with-limit";
...
// Keep aligned with @supportclaw/plugin-package-contract ExternalPluginCompatibility.
export type ClawHubPackageCompatibility = {
  pluginApiRange?: string;
  builtWithSupportClawVersion?: string;
...
function normalizeBaseUrl(baseUrl?: string): string {
  const envValue =
    normalizeOptionalString(process.env.SUPPORT_CLAW_CLAWHUB_URL) ||
...
function normalizeGitHubCodeloadBaseUrl(): string {
  const value =
    normalizeOptionalString(process.env.SUPPORT_CLAW_CLAWHUB_GITHUB_CODELOAD_BASE_URL) ||
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    normalizeOptionalString(process.env.SUPPORT_CLAW_CLAWHUB_CONFIG_PATH) ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    normalizeOptionalString(process.env.SUPPORT_CLAW_CLAWHUB_TOKEN) ||
...
function normalizePluginApiVersionForComparator(version: string, target: string): string {
  const normalizedCorrection = normalizeSupportClawNumericCorrectionForPluginApi(version);
  if (normalizedCorrection) {
    return normalizedCorrection;
  }
  return shouldPreservePluginApiPrereleaseFloor(target)
    ? version
    : normalizeSupportClawReleaseSuffixForPluginApi(version);
}
...
const SUPPORT_CLAW_RELEASE_SUFFIX_PATTERN =
  /^[vV]?(\d{4}\.[1-9]\d?\.[1-9]\d*)(?:-\d+|-(?:alpha|beta|rc)\.\d+)$/i;
const SUPPORT_CLAW_NUMERIC_CORRECTION_PATTERN = /^[vV]?(\d{4}\.[1-9]\d?\.[1-9]\d*)-\d+$/;

function normalizeSupportClawNumericCorrectionForPluginApi(
  pluginApiVersion: string,
): string | undefined {
  return SUPPORT_CLAW_NUMERIC_CORRECTION_PATTERN.exec(pluginApiVersion.trim())?.[1];
}

function normalizeSupportClawReleaseSuffixForPluginApi(pluginApiVersion: string): string {
  const match = SUPPORT_CLAW_RELEASE_SUFFIX_PATTERN.exec(pluginApiVersion.trim());
...
export async function downloadClawHubPackageArchive(params: {
  ...
  if (params.artifact === "clawpack") {
    ...
    const target = await createTempDownloadTarget({
      prefix: "supportClaw-clawhub-clawpack",
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "supportClaw-clawhub-package",
...
export async function downloadClawHubSkillArchive(params: {
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "supportClaw-clawhub-skill",
...
export async function downloadClawHubGitHubSkillArchive(params: {
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "supportClaw-clawhub-github-skill",
