...
// update.
//
// `runGatewayUpdate` (git mode) runs `supportClaw doctor --fix` with
// `SUPPORT_CLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1`, which makes the doctor
// pass DEFER configured-plugin repair to a later convergence step (see
// `shouldDeferConfiguredPluginInstallRepair`). The `supportClaw update` CLI resumes
...
// binary's hidden `supportClaw update finalize` entrypoint — the designed
...
function buildFinalizeEnv(
  ...
  delete env.SUPPORT_CLAW_SERVICE_MARKER;
  delete env.SUPPORT_CLAW_SERVICE_KIND;
  delete env[GATEWAY_SERVICE_RUNTIME_PID_ENV];
  env[UPDATE_EFFECTIVE_CHANNEL_ENV] = effectiveChannel;
  if (compatHostVersion) {
    env.SUPPORT_CLAW_COMPATIBILITY_HOST_VERSION = compatHostVersion;
  }
  if (sourceConfigPath) {
...
// re-enters the full `supportClaw update` CLI. Re-run convergence on no-op retries:
...
export async function runPostCoreFinalizeAfterGatewayUpdate(params: {
  ...
  try {
    ...
    if (params.preUpdateConfig) {
      sourceConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-update-post-core-"));
  ...
  return {
    ...
    steps: [
      ...result.steps,
      {
        ...
        command: "supportClaw update finalize",
