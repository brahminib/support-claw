...
import { closeSupportClawAgentDatabasesForTest } from "../state/supportClaw-agent-db.js";
import {
  closeSupportClawStateDatabase,
  openSupportClawStateDatabase,
} from "../state/supportClaw-state-db.js";
import { withSupportClawTestState } from "../test-utils/supportClaw-test-state.js";
import {
...
function makeResult(overrides: Partial<BackupCreateResult> = {}): BackupCreateResult {
  return {
    ...
    archiveRoot: "supportClaw-backup-2026-01-01",
    archivePath: "/tmp/supportClaw-backup.tar.gz",
    dryRun: false,
...
describe("formatBackupCreateSummary", () => {
  const backupArchiveLine = "Backup archive: /tmp/supportClaw-backup.tar.gz";
        ...
        skipped: [
          {
            ...
            displayPath: "~/Projects/supportClaw",
            reason: "covered",
            coveredBy: "~/.supportClaw",
          },
        ],
      }),
      expected: [
        ...
        "- state: ~/.supportClaw",
        "Skipped 1 path:",
        "- workspace: ~/Projects/supportClaw (covered by ~/.supportClaw)",
        "Created /tmp/supportClaw-backup.tar.gz",
        ...
        assets: [
          {
            ...
            displayPath: "~/.supportClaw/config.json",
          },
          {
            kind: "credentials",
            sourcePath: "/oauth",
            archivePath: "archive/oauth",
            displayPath: "~/.supportClaw/oauth",
          },
        ],
      }),
      expected: [
        backupArchiveLine,
        "Included 2 paths:",
        "- config: ~/.supportClaw/config.json",
        "- credentials: ~/.supportClaw/oauth",
  ...
  it("surfaces the volatile skip count in the summary", () => {
    expect(
      ...
      "Backup archive: /tmp/supportClaw-backup.tar.gz",
      "Included 1 path:",
      "- state: ~/.supportClaw",
      "Created /tmp/supportClaw-backup.tar.gz",
...
describe("buildExtensionsNodeModulesFilter", () => {
  it("excludes dependency trees only under state extensions", () => {
    ...
    expect(filter("/state/extensions/demo/supportClaw.plugin.json")).toBe(true);
  ...
  it("normalizes Windows path separators", () => {
    const filter = buildExtensionsNodeModulesFilter("C:\\Users\\me\\.supportClaw\\");

    expect(filter(String.raw`C:\Users\me\.supportClaw\extensions\demo\index.js`)).toBe(true);
    expect(
      filter(String.raw`C:\Users\me\.supportClaw\extensions\demo\node_modules\dep\index.js`),
    ).toBe(false);
  });
});
...
describe("createBackupArchive", () => {
  it("falls back when injected nowMs is outside Date range", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-invalid-now-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        try {
          ...
          expect(path.basename(result.archivePath)).toContain("supportClaw-backup.tar.gz");
  ...
  it("falls back to epoch when injected nowMs and Date.now are outside Date range", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-invalid-fallback-now-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("skips current live volatile state files while preserving workspace locks", async () => {
    await withSupportClawTestState(
      {
        layout: "split",
        prefix: "supportClaw-backup-volatile-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("scrubs transient SQLite delivery queue rows from archive snapshots", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-sqlite-queue-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const { db } = openSupportClawStateDatabase({ env: state.env });
        db.prepare(
          `
        ...
        try {
          ...
          const archivedDbEntry = entries.find((entry) =>
            entry.endsWith("/state/state/supportClaw.sqlite"),
          );
          expect(archivedDbEntry).toBeDefined();
          expect(entries.some((entry) => entry.endsWith("/state/state/supportClaw.sqlite-wal"))).toBe(
            false,
          );
  ...
  it("snapshots per-agent SQLite auth stores without deleted secret pages", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-agent-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const sqlite = requireNodeSqlite();
        const liveDbPath = path.join(state.agentDir(), "supportClaw-agent.sqlite");
        const deletedSecretMarker = "SUPPORT_CLAW_DELETED_SECRET_PAGE_MARKER";
        ...
        const entries = await listArchiveEntries(result.archivePath);
        const archivedDbEntry = entries.find((entry) =>
          entry.endsWith("/state/agents/main/agent/supportClaw-agent.sqlite"),
        );
        expect(archivedDbEntry).toBeDefined();
        expect(
          entries.some((entry) =>
            entry.endsWith("/state/agents/main/agent/supportClaw-agent.sqlite-wal"),
          ),
        ).toBe(false);
  ...
  it("snapshots nested live SQLite databases with transaction continuity", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-nested-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("fails instead of raw-copying malformed nested SQLite databases", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-malformed-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it.each(["late.sqlite", "late.sqlite-wal"])(
    ...
    async (lateName) => {
      await withSupportClawTestState(
        {
          layout: "state-only",
          prefix: "supportClaw-backup-late-sqlite-",
          scenario: "minimal",
        },
        async (state) => {
          const outputDir = state.path("backups");
  ...
  it("omits pre-existing orphan SQLite sidecars without failing backup", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-orphan-sqlite-sidecars-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("omits transient memory reindex databases and sidecars", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-memory-reindex-lock-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("preserves noncanonical symlinked SQLite paths without dereferencing them", async () => {
    ...
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-symlinked-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("snapshots the canonical global SQLite symlink as a complete regular file", async () => {
    ...
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-global-sqlite-symlink-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const linkedDbPath = state.statePath("state", "supportClaw.sqlite");
        await state.writeConfig({
          agents: {
            list: [{ id: "main", default: true, workspace: state.workspaceDir }],
          },
        ...
        try {
          ...
          const entries = await listArchiveEntryDetails(result.archivePath);
          const archivedDbEntries = entries.filter((entry) =>
            entry.path.endsWith("/state/state/supportClaw.sqlite"),
          );
  ...
  it("fails when the canonical global SQLite path is not a file", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-global-sqlite-directory-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        const globalDbPath = state.statePath("state", "supportClaw.sqlite");
  ...
  it("omits installed plugin node_modules from the real archive while keeping plugin files", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-plugin-deps-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        await fs.writeFile(
          path.join(stateDir, "extensions", "demo", "supportClaw.plugin.json"),
          '{"id":"demo"}\n',
          "utf8",
        );
        await fs.writeFile(
        ...
        expect(entrySuffixes).toContain("/state/extensions/demo/supportClaw.plugin.json");
  ...
  it("dereferences hardlinks instead of emitting restore-hostile Link entries", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-hardlink-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
        const outputDir = state.path("backups");
        const sourcePath = path.join(stateDir, "workspace-adx", "supportClaw-src", "node_modules");
  ...
  it("does not duplicate the root manifest when the system tempdir lives inside the state dir", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-tmp-overlap-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
  ...
  it("does not duplicate the root manifest when the system tempdir is the state dir itself", async () => {
    await withSupportClawTestState(
      {
        layout: "state-only",
        prefix: "supportClaw-backup-tmp-equals-state-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        try {
          ...
          expect(entries.some((entry) => entry.includes("/supportClaw-state-db-"))).toBe(false);
