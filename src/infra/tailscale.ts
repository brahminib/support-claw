...
import { asNullableObjectRecord as readRecord } from "@supportclaw/normalization-core/record-coerce";
import {
...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.SUPPORT_CLAW_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
