...
describe("isMainModule", () => {
  ...
  it("returns true for configured wrapper-to-entry pairs", () => {
    expect(
      isMainModule({
        ...
        argv: ["node", "/repo/supportClawCli.mjs"],
        cwd: "/repo",
        env: {},
        wrapperEntryPairs: [{ wrapperBasename: "supportClawCli.mjs", entryBasename: "entry.js" }],
      }),
    ).toBe(true);
  });
  ...
  it("returns false when this module is only imported under PM2", () => {
    expect(
      isMainModule({
        currentFile: "/repo/node_modules/supportClaw/dist/index.js",
