...
export function emitGatewayRestart(
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerSupportClawRestart() uses schtasks to restart the gateway.
...
export function triggerSupportClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.SUPPORT_CLAW_SYSTEMD_UNIT,
      process.env.SUPPORT_CLAW_PROFILE,
    );
  ...
  const label =
    process.env.SUPPORT_CLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SUPPORT_CLAW_PROFILE);
