...
async function withRestartSentinelStateDir(run: () => Promise<void>): Promise<void> {
  await withTempDir({ prefix: "supportClaw-sentinel-" }, async (tempDir) => {
    try {
      await withEnvAsync({ SUPPORT_CLAW_STATE_DIR: tempDir }, run);
    } finally {
      closeSupportClawStateDatabaseForTest();
    }
  });
...
describe("restart sentinel", () => {
  ...
  it("imports a legacy file sentinel into sqlite once", async () => {
    await withRestartSentinelStateDir(async () => {
      ...
      const legacyPath = path.join(process.env.SUPPORT_CLAW_STATE_DIR ?? "", "restart-sentinel.json");
  ...
  it("keeps old config restart sentinels readable without restart-required stats", async () => {
    await withRestartSentinelStateDir(async () => {
      const filePath = path.join(process.env.SUPPORT_CLAW_STATE_DIR ?? "", "restart-sentinel.json");
      const payload = {
  ...
  it("formats config write success notices as restart required when marked", () => {
    const payload = {
      ...
      doctorHint: "Run supportClaw doctor --non-interactive",
    ...
    expect(formatRestartSentinelMessage(payload)).toBe(
      [
        ...
        "Run supportClaw doctor --non-interactive",
      ].join("\n"),
    );
  ...
  it("formats summary, distinct reason, and doctor hint together", () => {
    const payload = {
      ...
      doctorHint: "Run supportClaw doctor",
...
describe("restart sentinel message dedup", () => {
  ...
  it("formats the non-interactive doctor command as actionability guidance", () => {
    expect(formatDoctorNonInteractiveHint({ PATH: "/usr/bin:/bin" })).toBe(
      "Recommended follow-up: run supportClaw doctor --non-interactive in a terminal or approvals-capable SupportClaw surface.",
    );
  });
  ...
  it("keeps profile-aware doctor guidance actionable outside constrained delivery surfaces", () => {
    expect(
      formatDoctorNonInteractiveHint({
        SUPPORT_CLAW_PROFILE: "isolated",
        PATH: "/usr/bin:/bin",
      }),
    ).toBe(
      "Recommended follow-up: run supportClaw --profile isolated doctor --non-interactive in a terminal or approvals-capable SupportClaw surface.",
    );
  });
});
