...
const DEFAULT_TRIGGERS = ["supportClaw", "claude", "computer"];
const VOICEWAKE_CONFIG_KEY = "default";

type VoiceWakeDatabase = Pick<SupportClawStateKyselyDatabase, "voicewake_triggers">;
...
function openStateDatabase(stateDir?: string) {
  return openSupportClawStateDatabase({
    env: stateDir ? { ...process.env, SUPPORT_CLAW_STATE_DIR: stateDir } : process.env,
  });
}
...
export async function setVoiceWakeTriggers(
  ...
  runSupportClawStateWriteTransaction(
    ...
    baseDir ? { env: { ...process.env, SUPPORT_CLAW_STATE_DIR: baseDir } } : {},
  );
  return {
    triggers: sanitized,
    updatedAtMs,
