...
import { normalizeOptionalString as readOptionalString } from "@supportclaw/normalization-core/string-coerce";
...
import { resolveSupportClawPackageRootSync } from "./supportClaw-root.js";
...
type ManagedNpmRootSupportClawMetadata = {
...
type ManagedNpmRootSupportClawHostState = "none" | "managed-active-host" | "linked-active-host";
...
function buildManagedSupportClawMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
  managedPeerDependencyKeys?: string[];
}): ManagedNpmRootSupportClawMetadata | undefined {
  const metadata: ManagedNpmRootSupportClawMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
function applyManagedNpmRootOverrides(params: {
  ...
  for (const key of readManagedOverrideKeys(params.manifest.supportClaw)) {
...
/** Read host SupportClaw pnpm overrides for reuse inside a managed npm root. */
export async function readSupportClawManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
  ...
  const packageRoot =
    params?.packageRoot ??
    resolveSupportClawPackageRootSync({
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  const managedDependencyNames = new Set(readManagedPeerDependencyKeys(manifest.supportClaw));
  ...
  const supportClawMetadata = buildManagedSupportClawMetadata({
    current: manifest.supportClaw,
    managedOverrideKeys,
  ...
  if (supportClawMetadata) {
    next.supportClaw = supportClawMetadata;
  } else {
    delete next.supportClaw;
  }
...
function scrubHostPeerFromLockPackage(value: unknown): boolean {
  ...
  let changed = false;
  if (isRecord(value.peerDependencies) && "supportClaw" in value.peerDependencies) {
    const peerDependencies = { ...value.peerDependencies };
    delete peerDependencies.supportClaw;
  ...
  if (isRecord(value.peerDependenciesMeta) && "supportClaw" in value.peerDependenciesMeta) {
    const peerDependenciesMeta = { ...value.peerDependenciesMeta };
    delete peerDependenciesMeta.supportClaw;
...
function isHostPeerResolutionFailure(
  ...
  return /(^|[^@\w.-])supportClaw(?=$|[@\s:,"'])/i.test(output);
}
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.supportClaw);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-managed-peer-plan-"));
  try {
    delete dependencies.supportClaw;
    await writeJson(
    ...
    await copyPathIfExists(
      path.join(params.npmRoot, "_supportClaw-pack-archives"),
      path.join(tempRoot, "_supportClaw-pack-archives"),
    );
...
export async function readManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"));
  const dependencies = readDependencyRecord(manifest.dependencies);
  const managedPeerDependencies = readManagedPeerDependencyKeys(manifest.supportClaw).toSorted();
...
export async function restoreManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  for (const packageName of readManagedPeerDependencyKeys(manifest.supportClaw)) {
    delete dependencies[packageName];
  }
  Object.assign(dependencies, params.snapshot.dependencies);
  const overrides = readOverrideRecord(manifest.overrides);
  const currentManagedOverrideKeys = readManagedOverrideKeys(manifest.supportClaw);
...
/** Remove stale managed-root supportClaw peer installs while preserving active host links. */
export async function repairManagedNpmRootSupportClawPeer(params: {
  npmRoot: string;
  packageRoot?: string | null;
  timeoutMs?: number;
  ...
  const activeHostState = await readManagedNpmRootSupportClawHostState({
    npmRoot: params.npmRoot,
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "supportClaw" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasSupportClawPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "supportClaw"));
  ...
  if (preserveActiveHostLink) {
    await scrubManagedNpmRootSupportClawPeer({
      npmRoot: params.npmRoot,
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall supportClaw" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall supportClaw" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootSupportClawPeer({ npmRoot: params.npmRoot });
  return true;
}

async function readManagedNpmRootSupportClawHostState(params: {
  npmRoot: string;
  packageRoot?: string | null;
}): Promise<ManagedNpmRootSupportClawHostState> {
  const packageRoot =
  ...
  const managedSupportClawPackageDir = path.join(params.npmRoot, "node_modules", "supportClaw");
  const [hostPackageRoot, managedPackageRoot, managedPackageStat] = await Promise.all([
    realpathIfExists(packageRoot),
    realpathIfExists(managedSupportClawPackageDir),
    lstatIfExists(managedSupportClawPackageDir),
  ]);
...
async function managedNpmRootLockfileHasSupportClawPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "supportClaw" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/supportClaw" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "supportClaw" in parsed.dependencies;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return false;
    }
...
async function scrubManagedNpmRootSupportClawPeer(params: {
  npmRoot: string;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("supportClaw" in dependencies) {
    const { supportClaw: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (isRecord(rootPackage) && isRecord(rootPackage.dependencies)) {
        ...
        if ("supportClaw" in dependenciesValue) {
          delete dependenciesValue.supportClaw;
          parsed.packages[""] = { ...rootPackage, dependencies: dependenciesValue };
          lockChanged = true;
        }
      }
      if ("node_modules/supportClaw" in parsed.packages) {
        delete parsed.packages["node_modules/supportClaw"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "supportClaw" in parsed.dependencies) {
      const dependenciesLocal = { ...parsed.dependencies };
      delete dependenciesLocal.supportClaw;
  ...
  const supportClawPackageDir = path.join(params.npmRoot, "node_modules", "supportClaw");
  if (!params.preservePackageDir && (await pathExists(supportClawPackageDir))) {
    await fs.rm(supportClawPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["supportClaw", "supportClaw.cmd", "supportClaw.ps1"].map((binName) =>
