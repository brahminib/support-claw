...
import { readRootJsonObjectSync } from "@supportclaw/fs-safe/json";
...
/** Return true when an installed package needs an supportClaw peer link repair. */
export function installedPackageNeedsSupportClawPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "supportClaw"));
    return false;
  } catch (error) {
