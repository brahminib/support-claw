// Applies persisted state migrations across SupportClaw config files.
...
import {
  detectSupportClawStateDatabaseSchemaMigrations,
  repairSupportClawStateDatabaseSchema,
...
type LegacyPluginStateImportDatabase = Pick<SupportClawStateKyselyDatabase, "plugin_state_entries">;
type LegacyVoiceWakeImportDatabase = Pick<
  SupportClawStateKyselyDatabase,
  "voicewake_routing_config" | "voicewake_routing_routes" | "voicewake_triggers"
>;
type LegacyUpdateCheckImportDatabase = Pick<SupportClawStateKyselyDatabase, "update_check_state">;
type LegacyConfigHealthImportDatabase = Pick<SupportClawStateKyselyDatabase, "config_health_entries">;
...
function resolveDefaultExecApprovalsStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env, homedir), ".supportClaw");
}
...
function detectLegacyExecApprovalsMigration(params: {
  ...
  return {
    ...
    targetPath,
    hasLegacy:
      Boolean(params.env.SUPPORT_CLAW_STATE_DIR?.trim()) &&
...
async function migrateLegacyTaskRunsSidecar(params: {
  ...
  try {
    ...
    runSupportClawStateWriteTransaction(
      ...
      { env: { ...process.env, SUPPORT_CLAW_STATE_DIR: params.stateDir } },
    );
    if (importedTasks > 0) {
      changes.push(
...
const DEFAULT_VOICEWAKE_TRIGGERS = ["supportClaw", "claude", "computer"];
...
function migrateLegacyVoiceWakeSettings(params: {
  ...
  const changes: string[] = [];
  const warnings: string[] = [];
  const env = { ...process.env, SUPPORT_CLAW_STATE_DIR: params.stateDir };
...
async function withPluginStateImportEnv<T>(
  ...
  process.env.SUPPORT_CLAW_STATE_DIR = plan.stateDir;
  try {
    return await run();
  } finally {
...
export async function autoMigrateLegacyStateDir(params: {
  ...
  const warnings: string[] = [];
  const changes: string[] = [];
  const hasCustomStateDir = Boolean(env.SUPPORT_CLAW_STATE_DIR?.trim());
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set SUPPORT_CLAW_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function detectLegacyStateMigrations(params: {
  ...
  const stateSchemaMigrations = detectSupportClawStateDatabaseSchemaMigrations({
    env: { ...env, SUPPORT_CLAW_STATE_DIR: stateDir },
  });
...
async function migrateLegacySessions(
  ...
  if (!targetReadable) {
    if (options.recoverCorruptTargetStore) {
      ...
      warnings.push(
        `Target sessions store unreadable; left untouched to avoid overwriting at ${detected.sessions.targetStorePath}. Run supportClaw doctor --fix to archive it and retry the legacy merge.`,
      );
    }
  }
...
function migrateLegacyStateSchema(detected: LegacyStateDetection): {
  ...
  return repairSupportClawStateDatabaseSchema({
    env: { ...process.env, SUPPORT_CLAW_STATE_DIR: detected.stateDir },
  });
}
...
export async function autoMigrateLegacyState(params: {
  ...
  const stateSchema = repairSupportClawStateDatabaseSchema({
    env: { ...env, SUPPORT_CLAW_STATE_DIR: stateDir },
  });
  ...
  const hasCustomAgentDir = env.SUPPORT_CLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
