...
function resolveLegacyDebugProxyCapturePaths(
  ...
  return {
    sourcePath: env.SUPPORT_CLAW_DEBUG_PROXY_DB_PATH?.trim() || path.join(rootDir, "capture.sqlite"),
    blobDir: env.SUPPORT_CLAW_DEBUG_PROXY_BLOB_DIR?.trim() || path.join(rootDir, "blobs"),
  };
}
...
export function detectLegacyDebugProxyCaptureSidecar(
  ...
  if (
    ...
    path.resolve(resolveSupportClawStateSqlitePath({ ...env, SUPPORT_CLAW_STATE_DIR: stateDir }))
  ) {
