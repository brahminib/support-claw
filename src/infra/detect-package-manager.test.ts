...
async function withPackageManagerRoot<T>(
  ...
  return await withTempDir({ prefix: "supportClaw-detect-pm-" }, async (root) => {
...
async function writePublishedSupportClawRoot(root: string): Promise<void> {
...
describe("detectPackageManager", () => {
  ...
  it("keeps pnpm-owned direct package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "supportClaw-detect-pm-pnpm-direct-" }, async (base) => {
      const nodeModulesRoot = path.join(base, "pnpm-global", "node_modules");
      const packageRoot = path.join(nodeModulesRoot, "supportClaw");
      await writePublishedSupportClawRoot(packageRoot);
  ...
  it("keeps pnpm-owned virtual-store package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "supportClaw-detect-pm-pnpm-virtual-" }, async (base) => {
      const nodeModulesRoot = path.join(base, "project", "node_modules");
      const packageRoot = path.join(
        nodeModulesRoot,
        ".pnpm",
        "supportClaw@2026.5.27",
        "node_modules",
        "supportClaw",
      );
      await writePublishedSupportClawRoot(packageRoot);
  ...
  it("keeps bun-owned global package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "supportClaw-detect-pm-bun-" }, async (base) => {
      const bunInstall = path.join(base, "bun-home");
      await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
        const packageRoot = path.join(bunInstall, "install", "global", "node_modules", "supportClaw");
        await writePublishedSupportClawRoot(packageRoot);
