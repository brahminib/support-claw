...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "supportClaw-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".supportClaw");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".supportClaw", "agents", "main", "agent");
  }
