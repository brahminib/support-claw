// Creates temporary SupportClaw directories for runtime scratch work.
import fs from "node:fs";
import { tmpdir as getOsTmpDir } from "node:os";
import path from "node:path";

/** Preferred shared SupportClaw temp root on POSIX systems when ownership and permissions are safe. */
export const POSIX_SUPPORT_CLAW_TMP_DIR = "/tmp/supportClaw";
...
export type ResolvePreferredSupportClawTmpDirOptions = {
...
/** Resolves a safe SupportClaw temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredSupportClawTmpDir(
  options: ResolvePreferredSupportClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "supportClaw" : `supportClaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[supportClaw] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback SupportClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback SupportClaw temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_SUPPORT_CLAW_TMP_DIR;
