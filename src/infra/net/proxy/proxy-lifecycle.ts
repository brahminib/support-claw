...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    SUPPORT_CLAW_PROXY_ACTIVE: process.env["SUPPORT_CLAW_PROXY_ACTIVE"],
    SUPPORT_CLAW_PROXY_LOOPBACK_MODE: process.env["SUPPORT_CLAW_PROXY_LOOPBACK_MODE"],
    SUPPORT_CLAW_PROXY_CA_FILE: process.env["SUPPORT_CLAW_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["SUPPORT_CLAW_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["SUPPORT_CLAW_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["SUPPORT_CLAW_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["SUPPORT_CLAW_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or SUPPORT_CLAW_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["SUPPORT_CLAW_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["SUPPORT_CLAW_PROXY_CA_FILE"],
  });
 ...
 * loopback CDP probe to a Chromium instance SupportClaw spawned itself.
 *
