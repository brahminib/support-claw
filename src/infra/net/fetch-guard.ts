...
const SUPPORT_CLAW_DEBUG_PROXY_ENABLED = "SUPPORT_CLAW_DEBUG_PROXY_ENABLED";
...
function isManagedProxyActive(): boolean {
  return process.env["SUPPORT_CLAW_PROXY_ACTIVE"] === "1";
}
...
async function captureGuardedFetchExchange(params: {
  ...
  if (params.capture === false || !isTruthyEnvValue(process.env[SUPPORT_CLAW_DEBUG_PROXY_ENABLED])) {
    return;
  }
