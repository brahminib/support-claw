...
type GatewayRestartHandoffDatabase = Pick<SupportClawStateKyselyDatabase, "gateway_restart_handoff">;

function createHandoffEnv(): NodeJS.ProcessEnv {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-restart-handoff-"));
  tempDirs.push(dir);
  return {
    ...process.env,
...
function legacyHandoffPath(env: NodeJS.ProcessEnv): string {
  return path.join(env.SUPPORT_CLAW_STATE_DIR ?? "", "gateway-supervisor-restart-handoff.json");
}
