...
function createSupportClawBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "supportClaw-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation((command: unknown) => {
    ...
    call += 1;
    if (call === 1) {
      return createSupportClawBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses supportClaw-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "supportClaw-gateway" },
          { pid: process.pid, cmd: "supportClaw-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("verifies argv when lsof reports the node process name instead of supportClaw", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        if (command === "ps") {
          return {
            error: null,
            status: 0,
            stdout: "node /opt/supportClaw/dist/entry.js gateway\n",
            stderr: "",
          };
        }
        return {
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: supportClaw-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "supportClaw-gateway" },
          { pid: unrelatedStalePid, cmd: "supportClaw-gateway" },
        ]),
        stderr: "",
      });
      ...
      () => {
        ...
        mockReadFileSync.mockImplementation((path: unknown): string => {
          if (path === `/proc/${directParentPid}/status`) {
            return `Name:\tsupportClaw-gateway\nPid:\t${directParentPid}\nPPid:\t${grandparentPid}\n`;
          }
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "supportClaw-gateway" },
            { pid: grandparentPid, cmd: "supportClaw-gateway" },
            { pid: benignStalePid, cmd: "supportClaw-gateway" },
          ]),
          stderr: "",
        });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "supportClaw-gateway" },
            { pid: gatewayGrandparentPid, cmd: "supportClaw-gateway" },
          ]),
          stderr: "",
        });
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            ...
            status: 0,
            stdout: lsofOutput([
              { pid: toolHostPid, cmd: "supportClaw-gateway" },
              { pid: gatewayGrandparentPid, cmd: "supportClaw-gateway" },
              { pid: benignStalePid, cmd: "supportClaw-gateway" },
            ]),
            stderr: "",
    ...
    it("excludes pids whose command does not include 'supportClaw'", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: process.pid + 3152, cmd: "supportClaw-gateway" }]),
            stderr: "",
          };
        });
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["supportClaw", "gateway"]);
        expect(findGatewayPidsOnPortSync(18789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18789, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple supportClaw pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "supportClaw-gateway" },
          { pid: pid2, cmd: "supportClaw-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-supportClaw pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks supportClaw processes — non-supportClaw listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty supportClaw stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty supportClaw stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + supportClaw pid in stdout — container-restricted lsof reports partial results
          return createSupportClawBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "supportClaw-gateway" }]),
          stderr: "",
        };
      });
    ...
    it("does not kill a protected gateway pid after reparenting", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        return lsofCall === 1
          ? createLsofResult({
              stdout: lsofOutput([
                { pid: protectedPid, cmd: "supportClaw-gateway" },
                { pid: stalePid, cmd: "supportClaw-gateway" },
              ]),
            })
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'supportClaw'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("supportClaw")
      // inside the mid-loop flush: a non-supportClaw cmd between two entries must not
      // be pushed, but the following supportClaw entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-supportClaw entry first, then supportClaw entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid supportClaw entry must still be found.
      const stdout = `p0\ncsupportClaw-gateway\np${stalePid}\ncsupportClaw-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero supportClaw pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-supportClaw stdout (line 145)", () => {
    it("treats status 1 + non-supportClaw stdout as port-free (not an supportClaw process)", () => {
      // status 1 + non-empty stdout where no supportClaw pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill supportClaw pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-supportClaw output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // No supportClaw pids in status-1 output means the port is free for this cleanup.
      expect(cleanStaleGatewayProcessesSync()).toContain(stalePid);
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          if (command === "lsof") {
            ...
            return createLsofResult({
              stdout: lsofOutput([{ pid: gatewayParentPid, cmd: "supportClaw-gateway" }]),
            });
          }
          return createLsofResult();
        });
