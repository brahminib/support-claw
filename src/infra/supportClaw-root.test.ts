// Covers SupportClaw package root resolution.
...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__supportClaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "supportClaw-root");
...
function setPackageRoot(root: string, name = "supportClaw") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./supportClaw-root.js").resolveSupportClawPackageRootSync,
  asyncResolver: typeof import("./supportClaw-root.js").resolveSupportClawPackageRoot,
  opts: Parameters<typeof import("./supportClaw-root.js").resolveSupportClawPackageRootSync>[0],
...
vi.mock("./supportClaw-root.fs.runtime.js", () => ({
  supportClawRootFsSync: mockFsModule(),
  supportClawRootFs: mockFsPromisesModule(),
}));

describe("resolveSupportClawPackageRoot", () => {
  let resolveSupportClawPackageRoot: typeof import("./supportClaw-root.js").resolveSupportClawPackageRoot;
  let resolveSupportClawPackageRootSync: typeof import("./supportClaw-root.js").resolveSupportClawPackageRootSync;
  let clearSupportClawPackageRootCaches: typeof import("./supportClaw-root.js").testing.clearSupportClawPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveSupportClawPackageRoot,
      resolveSupportClawPackageRootSync,
      testing: { clearSupportClawPackageRootCaches },
    } = await import("./supportClaw-root.js"));
  });

  beforeEach(() => {
    clearSupportClawPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "supportClaw");
        const pkgRoot = path.join(project, "node_modules", "supportClaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "supportClaw");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "supportClawCli.mjs")));
    ...
    {
      name: "falls through from a non-supportClaw moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-supportClaw");
    ...
    {
      name: "returns null for non-supportClaw package roots",
      setup: () => {
        const pkgRoot = fx("not-supportClaw");
        setPackageRoot(pkgRoot, "not-supportClaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "supportClawCli.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "supportClaw");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
