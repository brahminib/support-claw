...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".supportClaw");
...
function sharedMainOpsConfig(sharedStorePath: string): SupportClawConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: SupportClawConfig = OPS_WORK_CONFIG) {
