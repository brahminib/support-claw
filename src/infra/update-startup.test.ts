...
const {
  ...
  startManagedServiceUpdateHandoffMock: vi.fn(async () => ({
    ...
    pid: 12345,
    command: "supportClaw update --yes --channel beta --timeout 2700",
    logPath: "/tmp/supportClaw-handoff.log",
  })),
}));

vi.mock("./supportClaw-root.js", async () => {
  const actual = await vi.importActual<typeof import("./supportClaw-root.js")>("./supportClaw-root.js");
  return {
    ...actual,
...
type UpdateCheckStateDatabase = Pick<SupportClawStateKyselyDatabase, "update_check_state">;
...
describe("update-startup", () => {
  let tempDir: string;
  let testState: SupportClawTestState;
  ...
  function readPersistedUpdateCheckState(): PersistedUpdateCheckState | null {
    const { db } = openSupportClawStateDatabase();
  ...
  function writePersistedUpdateCheckState(state: PersistedUpdateCheckState): void {
    runSupportClawStateWriteTransaction(({ db }) => {
      const stateDb = getNodeSqliteKysely<UpdateCheckStateDatabase>(db);
      executeSqliteQuerySync(
        db,
  ...
  beforeEach(async () => {
    ...
    testState = await createSupportClawTestState({
      layout: "state-only",
      prefix: "supportClaw-update-check-suite-",
      env: {
        SUPPORT_CLAW_NO_AUTO_UPDATE: undefined,
        SUPPORT_CLAW_SERVICE_KIND: undefined,
        SUPPORT_CLAW_SERVICE_MARKER: undefined,
        SUPPORT_CLAW_GATEWAY_SERVICE_PID: undefined,
        SUPPORT_CLAW_LAUNCHD_LABEL: undefined,
        SUPPORT_CLAW_SYSTEMD_UNIT: undefined,
        SUPPORT_CLAW_WINDOWS_TASK_NAME: undefined,
    ...
    if (!loaded) {
      ({ resolveSupportClawPackageRoot } = await import("./supportClaw-root.js"));
    ...
    vi.mocked(resolveSupportClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveSupportClawPackageRoot).mockResolvedValue("/opt/supportClaw");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/supportClaw",
      installKind: "package",
      packageManager: "npm",
  ...
  it.each([
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (latest): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("supportClaw update")}`,
    );
  ...
  it("honors SUPPORT_CLAW_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.SUPPORT_CLAW_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    const disabledLogCall = log.info.mock.calls.find(
      ([message]) => message === "auto-update disabled by SUPPORT_CLAW_NO_AUTO_UPDATE",
    );
    expect(disabledLogCall).toEqual([
      "auto-update disabled by SUPPORT_CLAW_NO_AUTO_UPDATE",
      {
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/supportClaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
    ...
    expect(options.env).toEqual({ SUPPORT_CLAW_AUTO_UPDATE: "1" });
  });
