// Resolves the SupportClaw package root from runtime and package metadata.
import path from "node:path";
import { fileURLToPath } from "node:url";
import { supportClawRootFs, supportClawRootFsSync } from "./supportClaw-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["supportClaw"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  ...
  try {
    const name = parsePackageName(await supportClawRootFs.readFile(packageJsonPath, "utf-8"));
...
function readPackageNameSync(dir: string): string | null {
  ...
  try {
    const name = parsePackageName(supportClawRootFsSync.readFileSync(packageJsonPath, "utf-8"));
    packageNameCache.set(packageJsonPath, name);
    return name;
  } catch {
    packageNameCache.set(packageJsonPath, null);
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = supportClawRootFsSync.realpathSync(normalized);
...
export async function resolveSupportClawPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveSupportClawPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
...
export const testing = {
  clearSupportClawPackageRootCaches(): void {
