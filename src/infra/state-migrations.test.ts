...
vi.mock("../channels/plugins/bundled.js", () => {
  ...
  function resolveChatAppAccountId(cfg: SupportClawConfig): string {
  ...
  return {
    ...
    listBundledChannelLegacyStateMigrationDetectors: vi.fn(() => [
      ...
      ({ cfg, env }: { cfg: SupportClawConfig; env: NodeJS.ProcessEnv }) => {
        const root = env.SUPPORT_CLAW_STATE_DIR;
        if (!root) {
          return [];
        }
...
type PluginBindingApprovalsDatabase = Pick<SupportClawStateKyselyDatabase, "plugin_binding_approvals">;
...
const createTempDir = () => tempDirs.make("supportClaw-state-migrations-test-");
...
describe("state migrations", () => {
  ...
  it("migrates legacy plugin binding approvals from the home state dir when using a custom state dir", async () => {
    ...
    const env = createEnv(stateDir);
    const cfg = createConfig();
    const sourcePath = path.join(root, ".supportClaw", "plugin-binding-approvals.json");
