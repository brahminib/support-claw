...
const testEnvSnapshot = captureEnv(["SUPPORT_CLAW_HOME", "SUPPORT_CLAW_STATE_DIR"]);
...
function createHomeDir(): string {
  ...
  tempDirs.push(dir);
  setTestEnvValue("SUPPORT_CLAW_HOME", dir);
  deleteTestEnvValue("SUPPORT_CLAW_STATE_DIR");
  return dir;
}

function approvalsFilePath(homeDir: string): string {
  return path.join(homeDir, ".supportClaw", "exec-approvals.json");
}
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".supportClaw", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".supportClaw", "exec-approvals.sock")),
    );
    expect(resolveExecApprovalsDisplayPath()).toBe("~/.supportClaw/exec-approvals.json");
  });

  it("uses SUPPORT_CLAW_STATE_DIR for default file and socket paths", () => {
    const dir = createHomeDir();
    ...
    expect(resolveExecApprovalsTranscriptPath()).toBe("$SUPPORT_CLAW_STATE_DIR/exec-approvals.json");
  ...
  it("fails closed without writing target approvals before state migration runs", () => {
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({
        version: 1,
        socket: {
          path: path.join(dir, ".supportClaw", "exec-approvals.sock"),
          token: "legacy-token",
        },
        defaults: {
          security: "deny",
  ...
  it("accepts a symlinked SUPPORT_CLAW_HOME as the trusted approvals root", () => {
    ...
    setTestEnvValue("SUPPORT_CLAW_HOME", linkedHome);

    saveExecApprovals({ version: 1, defaults: { security: "full" }, agents: {} });

    expect(
      fs.readFileSync(path.join(realHome, ".supportClaw", "exec-approvals.json"), "utf8"),
  ...
  it("refuses to traverse symlinked approvals components below a symlinked home", () => {
    ...
    fs.symlinkSync(realHome, linkedHome, "dir");
    fs.symlinkSync(linkedStateTarget, path.join(realHome, ".supportClaw"), "dir");
    setTestEnvValue("SUPPORT_CLAW_HOME", linkedHome);
