...
describe("logAcceptedEnvOption", () => {
  it("logs accepted env options once with redaction and formatting", async () => {
    ...
    withEnv(
      {
        ...
        SUPPORT_CLAW_TEST_ENV: "  line one\nline two  ",
      },
      () => {
        logAcceptedEnvOption({
          key: "SUPPORT_CLAW_TEST_ENV",
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith(
      "env: SUPPORT_CLAW_TEST_ENV=<redacted> (test option)",
    );
  });
    ...
    withEnv(
      {
        VITEST: "1",
        NODE_ENV: "development",
        SUPPORT_CLAW_BLANK_ENV: "value",
      },
      () => {
        logAcceptedEnvOption({
          key: "SUPPORT_CLAW_BLANK_ENV",
