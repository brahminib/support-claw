...
import { resolveSupportClawStateSqlitePath } from "../state/supportClaw-state-db.paths.js";
import {
...
type GatewayRestartSentinelDatabase = Pick<SupportClawStateKyselyDatabase, "gateway_restart_sentinel">;
...
function writeRestartSentinelRow(env: NodeJS.ProcessEnv, sentinel: unknown): void {
  const { db } = openSupportClawStateDatabase({ env });
...
async function runHelperWithExistingSentinel(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-handoff-helper-test-"));
  tempDirs.add(tmpDir);
  ...
  await startManagedServiceUpdateHandoff({
    ...
    argv1: "/opt/supportClaw/supportClawCli.mjs",
  ...
  const helperParams = JSON.parse(await fs.readFile(args[1] ?? "", "utf-8")) as Record<
    ...
    unknown
  >;
  const env = { SUPPORT_CLAW_STATE_DIR: tmpDir } as NodeJS.ProcessEnv;
    ...
    `${JSON.stringify(
      {
        ...
        stateDatabasePath: resolveSupportClawStateSqlitePath(env),
...
async function createLegacyRestartSentinelTable(env: NodeJS.ProcessEnv): Promise<void> {
  ...
  const stateDatabasePath = resolveSupportClawStateSqlitePath(env);
...
async function runHelperWithCommand(params: {
  ...
  const { execFile } =
    await vi.importActual<typeof import("node:child_process")>("node:child_process");
  const { startManagedServiceUpdateHandoff } = await import("./update-managed-service-handoff.js");
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-handoff-recovery-test-"));
  tempDirs.add(tmpDir);
    ...
    `${JSON.stringify(
      {
        ...
        stateDatabasePath: resolveSupportClawStateSqlitePath({ SUPPORT_CLAW_STATE_DIR: tmpDir }),
        logPath: path.join(tmpDir, "handoff.log"),
        sensitivePaths: [],
...
async function writeFakeSystemctl(): Promise<{ binDir: string; recordPath: string }> {
  const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-recovery-bin-"));
  tempDirs.add(binDir);
...
async function writeFakeLaunchctl(): Promise<{ binDir: string; recordPath: string }> {
  const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-launchctl-bin-"));
  tempDirs.add(binDir);
...
describe("managed service update handoff", () => {
  it("strips process supervisor hints while preserving service identity for the CLI handoff", async () => {
    ...
    const serviceIdentityEnv = {
      SUPPORT_CLAW_LAUNCHD_LABEL: "com.example.supportClaw.test",
      SUPPORT_CLAW_SYSTEMD_UNIT: "supportClaw-test.service",
      SUPPORT_CLAW_WINDOWS_TASK_NAME: "SupportClaw Test Gateway",
    ...
    expect(result.command).toBe("supportClaw update --yes --timeout 1800");
    ...
    expect(helperParams.stateDatabasePath).toMatch(/supportClaw\.sqlite$/u);
    ...
    expect(options.env.SUPPORT_CLAW_UPDATE_RUN_HANDOFF).toBe("1");
  ...
  it("launches systemd handoffs through a transient user scope", async () => {
    const { startManagedServiceUpdateHandoff } =
      await import("./update-managed-service-handoff.js");
    const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-systemd-run-bin-"));
    tempDirs.add(binDir);
    ...
    const result = await startManagedServiceUpdateHandoff({
      ...
      env: {
        PATH: binDir,
        SUPPORT_CLAW_SYSTEMD_UNIT: "supportClaw-gateway.service",
    ...
    expect(args.slice(0, 4)).toEqual([
      ...
      "--scope",
      "--collect",
      "--unit=supportClaw-update-handoff-123.scope",
    ]);
    ...
    expect(options.detached).toBe(true);
    expect(options.env.SUPPORT_CLAW_SYSTEMD_UNIT).toBe("supportClaw-gateway.service");
  ...
  it("starts the managed gateway service when the update command fails after handoff", async () => {
    ...
    const result = await runHelperWithCommand({
      ...
      serviceRecovery: { kind: "systemd", unit: "supportClaw-gateway.service" },
      pathPrepend: binDir,
    });

    expect(result.code).toBe(7);
    await expect(fs.readFile(recordPath, "utf-8")).resolves.toBe(
      "--user start supportClaw-gateway.service\n",
    );
  });
  ...
  it("retries launchd start when bootstrap reports an already-loaded label", async () => {
    ...
    const result = await runHelperWithCommand({
      ...
      serviceRecovery: {
        kind: "launchd",
        uid: 501,
        label: "com.example.supportClaw",
        plistPath: "/Users/test/Library/LaunchAgents/com.example.supportClaw.plist",
      },
      pathPrepend: binDir,
    });
    ...
    await expect(fs.readFile(recordPath, "utf-8")).resolves.toBe(
      [
        "kickstart gui/501/com.example.supportClaw",
        "enable gui/501/com.example.supportClaw",
        "bootstrap gui/501 /Users/test/Library/LaunchAgents/com.example.supportClaw.plist",
        "kickstart gui/501/com.example.supportClaw",
        "",
      ].join("\n"),
    );
    ...
    const cases = [
      {
        ...
        env: { SUPPORT_CLAW_LAUNCHD_LABEL: "com.example.supportClaw.test", HOME: "/Users/test" },
        expected: {
          kind: "launchd",
          uid: typeof process.getuid === "function" ? process.getuid() : 501,
          label: "com.example.supportClaw.test",
          plistPath: "/Users/test/Library/LaunchAgents/com.example.supportClaw.test.plist",
        },
      },
      {
        supervisor: "schtasks" as const,
        env: { SUPPORT_CLAW_WINDOWS_TASK_NAME: "SupportClaw Test Gateway" },
        expected: { kind: "schtasks", taskName: "SupportClaw Test Gateway" },
      },
    ];
  ...
  it("writes a fallback update failure when no restart sentinel row exists", async () => {
    ...
    if (process.platform !== "win32") {
      const mode = (await fs.stat(resolveSupportClawStateSqlitePath(env))).mode & 0o777;
  ...
  it("sweeps stale handoff temp directories while keeping fresh handoff logs", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-handoff-cleanup-test-"));
    tempDirs.add(tmpDir);
    ...
    const unrelatedDir = path.join(tmpDir, "supportClaw-other-temp");
