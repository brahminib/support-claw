...
async function createTimelineEnv() {
  const dir = await mkdtemp(join(tmpdir(), "supportClaw-diagnostics-timeline-"));
  tempDirs.push(dir);
  return {
    env: {
      SUPPORT_CLAW_DIAGNOSTICS: "timeline",
      SUPPORT_CLAW_DIAGNOSTICS_RUN_ID: "run-1",
      SUPPORT_CLAW_DIAGNOSTICS_ENV: "env-1",
      SUPPORT_CLAW_DIAGNOSTICS_TIMELINE_PATH: join(dir, "nested", "timeline.jsonl"),
...
describe("diagnostics timeline", () => {
  it("detects when timeline output is enabled", async () => {
    ...
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, SUPPORT_CLAW_DIAGNOSTICS: "1" } })).toBe(true);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, SUPPORT_CLAW_DIAGNOSTICS: "yes" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, SUPPORT_CLAW_DIAGNOSTICS: "on" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, SUPPORT_CLAW_DIAGNOSTICS: "all" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, SUPPORT_CLAW_DIAGNOSTICS: "*" } })).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, SUPPORT_CLAW_DIAGNOSTICS: "diagnostics.timeline" },
      }),
    ).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({ env: { ...env, SUPPORT_CLAW_DIAGNOSTICS: "telegram.http" } }),
    ).toBe(false);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, SUPPORT_CLAW_DIAGNOSTICS: "0" } })).toBe(
      false,
    );
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, SUPPORT_CLAW_DIAGNOSTICS_TIMELINE_PATH: "" },
      }),
    ).toBe(false);
  });
  ...
  it("honors config diagnostics flags after config is available", async () => {
    ...
    delete envWithoutFlag.SUPPORT_CLAW_DIAGNOSTICS;
    const configWithTimeline = { diagnostics: { flags: ["timeline"] } } as SupportClawConfig;
    const configWithWildcard = { diagnostics: { flags: ["*"] } } as SupportClawConfig;
    const configWithoutTimeline = { diagnostics: { flags: ["telegram.http"] } } as SupportClawConfig;
  ...
  it("lets false-like env diagnostics disable config-enabled timeline output", async () => {
    ...
    expect(
      isDiagnosticsTimelineEnabled({
        ...
        env: { ...env, SUPPORT_CLAW_DIAGNOSTICS: "0" },
      }),
    ).toBe(false);
  });
  ...
  it("writes JSONL diagnostic events with the stable envelope", async () => {
    ...
    expect(event?.schemaVersion).toBe("supportClaw.diagnostics.v1");
  ...
  it("records span start and end events around successful work", async () => {
    ...
    delete configOnlyEnv.SUPPORT_CLAW_DIAGNOSTICS;

    await expect(
      measureDiagnosticsTimelineSpan("runtimeDeps.stage", () => "ok", {
        phase: "startup",
        attributes: { pluginCount: 3 },
        config: { diagnostics: { flags: ["timeline"] } } as SupportClawConfig,
        env: configOnlyEnv,
      }),
    ).resolves.toBe("ok");
