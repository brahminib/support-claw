...
const SUPERVISOR_HINTS = {
  launchd: ["SUPPORT_CLAW_LAUNCHD_LABEL"],
  systemd: ["SUPPORT_CLAW_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["SUPPORT_CLAW_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "SUPPORT_CLAW_SERVICE_MARKER",
  "SUPPORT_CLAW_SERVICE_KIND",
] as const;
...
export interface DetectRespawnSupervisorOptions {
  includeLinuxSupportClawGatewayServiceMarker?: boolean;
}
...
function hasSupportClawGatewayServiceMarker(env: NodeJS.ProcessEnv): boolean {
  return (
    env.SUPPORT_CLAW_SERVICE_MARKER?.trim() === "supportClaw" &&
    env.SUPPORT_CLAW_SERVICE_KIND?.trim() === "gateway"
  );
}

function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.SUPPORT_CLAW_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "linux") {
    return hasAnyHint(env, SUPERVISOR_HINTS.systemd) ||
      (options.includeLinuxSupportClawGatewayServiceMarker === true &&
        hasSupportClawGatewayServiceMarker(env))
      ? "systemd"
      : null;
  }
  if (platform === "win32") {
    ...
    const marker = env.SUPPORT_CLAW_SERVICE_MARKER?.trim();
