...
const SUPPORT_CLAW_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "supportClaw.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.SUPPORT_CLAW_DIAGNOSTICS)) &&
    typeof env.SUPPORT_CLAW_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.SUPPORT_CLAW_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: SUPPORT_CLAW_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.SUPPORT_CLAW_DIAGNOSTICS_RUN_ID ? { runId: env.SUPPORT_CLAW_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.SUPPORT_CLAW_DIAGNOSTICS_ENV ? { envName: env.SUPPORT_CLAW_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.SUPPORT_CLAW_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
