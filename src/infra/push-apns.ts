...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open SupportClaw to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "supportClaw.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.SUPPORT_CLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.SUPPORT_CLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set SUPPORT_CLAW_APNS_TEAM_ID and SUPPORT_CLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.SUPPORT_CLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.SUPPORT_CLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.SUPPORT_CLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set SUPPORT_CLAW_APNS_PRIVATE_KEY_P8 or SUPPORT_CLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading SUPPORT_CLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
