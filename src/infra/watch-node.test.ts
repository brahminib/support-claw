...
import { bundledPluginFile } from "supportClaw/plugin-sdk/test-fixtures";
...
const VOICE_CALL_MANIFEST = bundledPluginFile("voice-call", "supportClaw.plugin.json");
...
describe("watch-node script", () => {
  it("wires chokidar watch to run-node with watched source/config paths", async () => {
    ...
    await withTempDir({ prefix: "supportClaw-watch-node-" }, async (cwd) => {
      ...
      expect(spawnEnv.SUPPORT_CLAW_WATCH_MODE).toBe("1");
      expect(spawnEnv.SUPPORT_CLAW_WATCH_SESSION).toBe("1700000000000-4242");
      expect(spawnEnv.SUPPORT_CLAW_NO_RESPAWN).toBe("1");
      expect(spawnEnv.SUPPORT_CLAW_WATCH_COMMAND).toBe("gateway --force");
      expect(spawnEnv.SUPPORT_CLAW_TRACE_SYNC_IO).toBeUndefined();
  ...
  it("preserves explicit sync I/O trace overrides for gateway watch", async () => {
    ...
    await withTempDir({ prefix: "supportClaw-watch-node-" }, async (cwd) => {
      const runPromise = runWatch({
        ...
        cwd,
        createWatcher,
        env: { SUPPORT_CLAW_TRACE_SYNC_IO: "0" },
        lockDisabled: true,
        process: fakeProcess,
        spawn,
      });
      ...
      expect(spawnCall[0]).toBe("/usr/local/bin/node");
      expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
      expect(requireSpawnEnv(spawn, 0).SUPPORT_CLAW_TRACE_SYNC_IO).toBe("0");
  ...
  it("does not run doctor after a gateway failure when auto doctor is disabled", async () => {
    ...
    const runPromise = runWatch({
      args: ["gateway", "--force"],
      createWatcher,
      env: { SUPPORT_CLAW_GATEWAY_WATCH_AUTO_DOCTOR: "0" },
      lockDisabled: true,
      process: fakeProcess,
      spawn,
    });
  ...
  it("forces no-respawn for watch children even when supervisor hints are inherited", async () => {
    ...
    expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
    const spawnEnv = requireSpawnEnv(spawn, 0);
    expect(spawnEnv.LAUNCH_JOB_LABEL).toBe("ai.supportClaw.gateway");
    expect(spawnEnv.SUPPORT_CLAW_NO_RESPAWN).toBe("1");
  ...
  it("prints recovery guidance when chokidar fails with invalid package config", async () => {
    const error = Object.assign(
      new Error(
        'Invalid package config /tmp/supportClaw/.pnpm/chokidar/package.json while importing "chokidar" from /tmp/supportClaw/scripts/watch-node.mjs.',
      ),
    ...
    try {
      ...
      expect(errorSpy.mock.calls).toEqual([
        [""],
        [
          "[supportClaw] gateway:watch could not start because a dependency package config looks corrupted.",
        ],
        ["[supportClaw] Invalid package config: /tmp/supportClaw/.pnpm/chokidar/package.json"],
        ["[supportClaw] This usually means a file in node_modules is empty or truncated."],
        ["[supportClaw] Recommended recovery:"],
        ["[supportClaw]   rm -rf node_modules"],
        ["[supportClaw]   pnpm store prune"],
        ["[supportClaw]   pnpm install"],
        [""],
        ["[supportClaw] Original error:"],
        [error],
      ]);
    } finally {
  ...
  it("replaces an existing watcher lock holder before starting", async () => {
    const { child, spawn, watcher, createWatcher, fakeProcess } = createWatchHarness();
    await withTempDir({ prefix: "supportClaw-watch-node-lock-" }, async (cwd) => {
