...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/supportClaw-state", "devices")).toEqual({
      dir: path.join("/tmp/supportClaw-state", "devices"),
      pendingPath: path.join("/tmp/supportClaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/supportClaw-state", "devices", "paired.json"),
    });
  });
