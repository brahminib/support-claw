// Computes git, dependency, and registry update status for SupportClaw installs.
import fs from "node:fs/promises";
import path from "node:path";
import { runCommandWithTimeout } from "../process/exec.js";
import { fetchWithTimeout } from "../utils/fetch-timeout.js";
import { detectPackageManager as detectPackageManagerImpl } from "./detect-package-manager.js";
import { compareSupportClawReleaseVersions } from "./npm-registry-spec.js";
...
function packageTargetSpec(params: { target: string; spec?: string }): string {
  ...
  return spec || `supportClaw@${params.target.trim() || "latest"}`;
}
...
async function fetchPublicNpmPackageTargetStatus(params: {
  ...
  try {
    res = await fetchWithTimeout(
      `https://registry.npmjs.org/supportClaw/${encodeURIComponent(params.target)}`,
      {},
...
export function compareSemverStrings(a: string | null, b: string | null): number | null {
  if (a && b) {
    const supportClawReleaseCmp = compareSupportClawReleaseVersions(a, b);
    if (supportClawReleaseCmp != null) {
      return supportClawReleaseCmp;
    }
  }
