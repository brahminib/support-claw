...
function keyPairMatches(publicKeyPem: string, privateKeyPem: string): boolean {
  try {
    const payload = Buffer.from("supportClaw-device-identity-self-check", "utf8");
    ...
    if (normalized?.kind === "identity") {
      if (normalized.stored) {
        try {
          ...
          // Keep using recognized SupportClaw key material even if best-effort normalization fails.
        }
      }
