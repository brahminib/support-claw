// Covers SupportClaw's default fs-safe Python helper configuration.
...
vi.mock("@supportclaw/fs-safe/config", () => ({
...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.SUPPORT_CLAW_FS_SAFE_PYTHON_MODE;
  });

  it("disables the Python helper by default in SupportClaw", async () => {
  ...
  it("honors the SupportClaw-specific env mode override", async () => {
    process.env.SUPPORT_CLAW_FS_SAFE_PYTHON_MODE = "auto";
