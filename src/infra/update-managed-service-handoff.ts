...
const SERVICE_IDENTITY_ENV_VARS = new Set<string>([
  ...
  "SUPPORT_CLAW_SYSTEMD_UNIT",
  "SUPPORT_CLAW_WINDOWS_TASK_NAME",
] as const);
...
export function formatManagedServiceUpdateCommand(params?: {
  ...
  const args = ["supportClaw", "update", "--yes"];
  if (params?.channel) {
...
function resolveGatewayServiceRecovery(
  ...
  if (supervisor === "systemd") {
    const override = env.SUPPORT_CLAW_SYSTEMD_UNIT?.trim();
    const unit = override
      ? override.endsWith(".service")
        ? override
        : `${override}.service`
      : `${resolveGatewaySystemdServiceName(env.SUPPORT_CLAW_PROFILE)}.service`;
    return { kind: "systemd", unit };
  }
  if (supervisor === "launchd") {
    const label =
      env.SUPPORT_CLAW_LAUNCHD_LABEL?.trim() || resolveGatewayLaunchAgentLabel(env.SUPPORT_CLAW_PROFILE);
  ...
  if (supervisor === "schtasks") {
    const taskName =
      env.SUPPORT_CLAW_WINDOWS_TASK_NAME?.trim() || resolveGatewayWindowsTaskName(env.SUPPORT_CLAW_PROFILE);
...
function buildSystemdHandoffUnitName(handoffId: string | undefined): string {
  const suffix =
    ...
    "handoff";
  return `supportClaw-update-${suffix}.scope`;
}
...
async function resolveHandoffSpawn(params: {
  ...
  if (!systemdRunPath) {
    throw new Error(
      "systemd-run is required to start the managed update handoff outside supportClaw-gateway.service",
    );
  }
...
export async function startManagedServiceUpdateHandoff(params: {
  ...
  const helperParams = {
    ...
    logPath,
    metaPath,
    stateDatabasePath: resolveSupportClawStateSqlitePath(params.env ?? process.env),
  ...
  const env = {
    ...
    SUPPORT_CLAW_UPDATE_RUN_HANDOFF: "1",
  };
