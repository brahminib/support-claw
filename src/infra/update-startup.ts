...
import { resolveSupportClawPackageRoot } from "./supportClaw-root.js";
...
function resolveAutoUpdatePolicy(cfg: SupportClawConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: SupportClawConfig): number {
...
async function runAutoUpdateCommand(params: {
  ...
  if (argv.length === 0) {
    argv.push("supportClaw", ...baseArgs);
  }
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.SUPPORT_CLAW_NO_AUTO_UPDATE);
  ...
  const root = await resolveSupportClawPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("supportClaw update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by SUPPORT_CLAW_NO_AUTO_UPDATE", {
