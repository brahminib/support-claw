...
const TEMP_DIR_PREFIX = "supportClaw-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("supportClaw-install-source-utils-");
  const archivePath = path.join(cwd, "supportClaw-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("supportClaw-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
  ...
  it.each([
    {
      ...
      path: async () => "/tmp/does-not-exist-supportClaw-archive.tgz",
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "supportClaw-plugin@1.2.3",
          name: "supportClaw-plugin",
          version: "1.2.3",
          filename: "supportClaw-plugin-1.2.3.tgz",
    ...
    const result = await runPack("supportClaw-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "supportClaw-plugin",
        version: "1.2.3",
        resolvedSpec: "supportClaw-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "supportClaw-plugin@1.2.3", "--ignore-scripts", "--json"],
      {
        cwd,
        timeoutMs: 300_000,
        env: {
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "supportClaw-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nsupportClaw-plugin-1.2.3.tgz\n",
    });
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"supportClaw-plugin@1.2.3","name":"supportClaw-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "supportClaw-plugin",
        version: "1.2.3",
        resolvedSpec: "supportClaw-plugin@1.2.3",
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@supportclaw/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@supportclaw/whatsapp", cwd);
    expectPackError(result, [
      "Package not found on npm",
      "@supportclaw/whatsapp",
      "docs.supportClaw.ai/tools/plugin",
    ]);
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("supportClaw-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "supportClaw-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@supportclaw/plugin-demo@2.0.0",
            filename: "supportClaw-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@supportclaw/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "supportClaw-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@supportclaw/plugin-demo@2.0.0",
      },
    });
  });
