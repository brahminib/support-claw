 ...
 * returning control to the caller (typically `triggerSupportClawRestart` →
...
function parsePidsFromLsofOutput(
  ...
  for (const entry of parseLsofEntries(stdout)) {
    ...
    if (entry.cmd && normalizeLowercaseStringOrEmpty(entry.cmd).includes("supportClaw")) {
      pids.push(entry.pid);
      continue;
    }
...
/**
 * Windows: find listening PIDs on the port, then verify each is an supportClaw
...
function findGatewayPidsOnPortWithProtectedPidSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only supportClaw gateway processes.
 ...
 * Returns only PIDs that belong to supportClaw gateway processes (not the current process).
 */
