...
function resolveAvailableKnownChannel(params: {
  ...
  // been eagerly loaded (e.g. `supportClaw agent --local`). Already-loaded and
...
export function isConfiguredChannel(cfg: SupportClawConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: supportClaw doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run supportClaw channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use supportClaw channels list --all to see available channel ids.",
  ].join(" ");
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: SupportClawConfig): Promise<boolean> {
