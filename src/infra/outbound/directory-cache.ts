...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: SupportClawConfig | null = null;
   ...
   */
  get(key: string, cfg: SupportClawConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: SupportClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
   ...
   */
  clear(cfg?: SupportClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: SupportClawConfig): void {
