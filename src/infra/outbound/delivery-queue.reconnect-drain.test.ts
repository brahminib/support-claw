...
const stubCfg = {} as SupportClawConfig;
...
function readOutboundQueueStatus(tmpDir: string, id: string): string | undefined {
  const { db } = openSupportClawStateDatabase({
    env: { ...process.env, SUPPORT_CLAW_STATE_DIR: tmpDir },
  });
  const row = db
...
describe("drainPendingDeliveries for reconnect", () => {
  ...
  it("skips entries that an in-flight live delivery has actively claimed", async () => {
    // Regression for supportClaw/supportClaw#70386: a reconnect drain that runs
