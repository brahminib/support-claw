...
export function installDeliveryQueueTmpDirHooks(): { readonly tmpDir: () => string } {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-dq-suite-"));
  });
...
export function readQueuedEntry(tmpDir: string, id: string): Record<string, unknown> {
  const { db } = openSupportClawStateDatabase({ env: { ...process.env, SUPPORT_CLAW_STATE_DIR: tmpDir } });
  const row = db
    .prepare(
