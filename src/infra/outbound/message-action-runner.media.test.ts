...
async function withTempSupportClawStateDir<T>(test: (stateDir: string) => Promise<T>): Promise<T> {
...
describe("runMessageAction media behavior", () => {
  ...
  it("materializes buffer-only send attachments into outbound media paths", async () => {
    ...
    await withTempSupportClawStateDir(async () => {
  ...
  it("does not stage buffer-only send attachments before target validation passes", async () => {
    ...
    await withTempSupportClawStateDir(async (stateDir) => {
      await expect(
        runMessageAction({
          cfg: workspaceConfig,
          action: "send",
  ...
  describe("reply hydration", () => {
    ...
    // params as send. Before supportClaw#79864 the runner only hydrated
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred SupportClaw tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideSupportClaw = path.join(os.tmpdir(), "outside-supportClaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
            params: {
              channel: "workspace",
              target: "12345678",
              media: hostTmpOutsideSupportClaw,
              message: "",
            },
            sandboxRoot: sandboxDir,
            dryRun: true,
