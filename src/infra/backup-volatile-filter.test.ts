...
const stateDir = "/opt/supportClaw/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/supportClaw/state/sessions/../config.jsonl` resolves to
    // `/opt/supportClaw/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\supportClaw\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/supportClaw/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
