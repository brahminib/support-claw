...
describe("compareSemverStrings", () => {
  ...
  it("treats SupportClaw stable correction releases as newer than their base release", () => {
...
describe("resolveNpmChannelTag", () => {
  ...
  it("delegates package target metadata to npm view with global config scope", async () => {
    ...
    const env = { ...process.env, NPM_CONFIG_USERCONFIG: "/tmp/supportClaw-user-npmrc" };

    await expect(
      fetchNpmPackageTargetStatus({
        target: "latest",
        spec: "supportClaw@latest",
        command: "/opt/supportClaw/node/bin/npm",
        timeoutMs: 1000,
        cwd: "/tmp/supportClaw-project",
        env,
        runCommand,
      }),
    ).resolves.toEqual({
  ...
  it("uses npm global scope, user config auth, and ignores project npmrc for real metadata", async () => {
    await withTempDir({ prefix: "supportClaw-update-check-npm-view-" }, async (base) => {
          ...
          JSON.stringify({
            ...
            versions: {
              "2026.6.6": {
                ...
                engines: { node: ">=22.19.0" },
                dist: {
                  tarball: "http://example.invalid/supportClaw-2026.6.6.tgz",
                  shasum: "0".repeat(40),
                },
              },
            },
      ...
      try {
        ...
        expect(requests.some((request) => request.url.startsWith("/user/supportClaw"))).toBe(true);
  ...
  it("uses the public registry when no npm command is available", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://registry.npmjs.org/supportClaw/latest",
...
describe("checkDepsStatus", () => {
  it("reports unknown, missing, stale, and ok states from lockfile markers", async () => {
    await withTempDir({ prefix: "supportClaw-update-check-" }, async (base) => {
  ...
  it("uses npm-shrinkwrap as the npm dependency lock marker when present", async () => {
    await withTempDir({ prefix: "supportClaw-update-check-shrinkwrap-" }, async (root) => {
...
describe("checkUpdateStatus", () => {
  ...
  it("detects package installs for non-git roots", async () => {
    await withTempDir({ prefix: "supportClaw-update-check-" }, async (root) => {
      await fs.writeFile(
  ...
  it("detects npm package installs that ship pnpm package metadata with shrinkwrap", async () => {
    await withTempDir({ prefix: "supportClaw-update-check-npm-shrinkwrap-" }, async (root) => {
      await fs.writeFile(
        path.join(root, "package.json"),
        JSON.stringify({ name: "supportClaw", packageManager: "pnpm@11.2.2" }),
        "utf8",
      );
  ...
  it("treats symlinked git installs as git roots", async () => {
    await withTempDir({ prefix: "supportClaw-update-check-git-" }, async (base) => {
      const repoRoot = path.join(base, "repo");
      const linkedRoot = path.join(base, "linked-supportClaw");
      await fs.mkdir(repoRoot, { recursive: true });
      await fs.writeFile(
        path.join(repoRoot, "package.json"),
        JSON.stringify({ name: "supportClaw", packageManager: "pnpm@10.0.0" }),
        "utf8",
      );
