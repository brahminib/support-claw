...
describe("command explainer tree-sitter runtime", () => {
  ...
  it("reports missing parser packages and wasm files with explainer context", () => {
    expect(() =>
      resolvePackageFileForCommandExplanation(
        "definitely-missing-supportClaw-parser-package",
        "parser.wasm",
      ),
    ).toThrow("Unable to resolve definitely-missing-supportClaw-parser-package");

    expect(() =>
      resolvePackageFileForCommandExplanation("web-tree-sitter", "missing-supportClaw-parser.wasm"),
    ).toThrow("Unable to locate missing-supportClaw-parser.wasm in web-tree-sitter");
  });
  ...
  it("detects eval, source, aliases, and carrier shell wrappers", async () => {
    const evalCommand = await explainShellCommand('eval "$SUPPORT_CLAW_CMD"');
