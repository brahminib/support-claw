  ...
  try {
    return await new Promise<ProbeApnsHttp2ReachabilityViaProxyResult>((resolve, reject) => {
      ...
      const request = session.request({
        ...
        authorization: "bearer intentionally.invalid.supportClaw.proxy.validation",
        "apns-topic": "ai.supportClaw.ios",
      ...
      request.end(JSON.stringify({ aps: { alert: "SupportClaw APNs proxy validation" } }));
    });
  } finally {
