...
describe("resolveVapidKeys", () => {
  it("generates and persists VAPID keys on first call", async () => {
    ...
    expect(keys.subject).toBe("https://supportClaw.ai");
    const persistedKeys = JSON.parse(
      await fs.readFile(path.join(tmpDir, "push", "vapid-keys.json"), "utf8"),
    ) as { subject?: string };
    expect(persistedKeys.subject).toBe("https://supportClaw.ai");
  ...
  it("prefers env vars over persisted keys", async () => {
    ...
    // Set env overrides.
    process.env.SUPPORT_CLAW_VAPID_PUBLIC_KEY = "env-public";
    process.env.SUPPORT_CLAW_VAPID_PRIVATE_KEY = "env-private";
    process.env.SUPPORT_CLAW_VAPID_SUBJECT = "mailto:env@test.com";
    try {
      const keys = await resolveVapidKeys(tmpDir);
      ...
      delete process.env.SUPPORT_CLAW_VAPID_PUBLIC_KEY;
      delete process.env.SUPPORT_CLAW_VAPID_PRIVATE_KEY;
      delete process.env.SUPPORT_CLAW_VAPID_SUBJECT;
    }
  });
});
