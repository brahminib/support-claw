...
import { timestampMsToIsoString } from "@supportclaw/normalization-core/number-coercion";
import {
...
function resolveHeartbeatTimeoutOverrideSeconds(cfg: SupportClawConfig, heartbeat?: HeartbeatConfig) {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SupportClawConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: SupportClawConfig) {
...
function resolveHeartbeatAgents(cfg: SupportClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: SupportClawConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: SupportClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: SupportClawConfig, heartbeat?: HeartbeatConfig) {
...
function resolveHeartbeatAckMaxChars(cfg: SupportClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: SupportClawConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: SupportClawConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SupportClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
