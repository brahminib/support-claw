...
import { bundledDistPluginFile } from "supportClaw/plugin-sdk/test-fixtures";
...
import {
  ...
  isMainPackageTarget,
  SUPPORT_CLAW_MAIN_PACKAGE_SPEC,
...
const execFileSyncMock = vi.hoisted(() => vi.fn(() => "/tmp/supportClaw-test-global-npmrc\n"));
...
async function writeGlobalPackageJson(packageRoot: string, version = "1.0.0") {
  await fs.writeFile(
    ...
    JSON.stringify({ name: "supportClaw", version }),
    "utf-8",
  );
}
...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["SUPPORT_CLAW_UPDATE_PACKAGE_SPEC"]);
    process.env.SUPPORT_CLAW_UPDATE_PACKAGE_SPEC = "file:/tmp/supportClaw.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "supportClaw", tag: "latest" })).toBe(
      "file:/tmp/supportClaw.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "supportClaw",
        tag: "beta",
        env: { SUPPORT_CLAW_UPDATE_PACKAGE_SPEC: "supportClaw@next" },
      }),
    ).toBe("supportClaw@next");
  });
  ...
  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "supportClaw", tag: "main" })).toBe(
      SUPPORT_CLAW_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "supportClaw",
        tag: "github:supportClaw/supportClaw#feature/my-branch",
      }),
    ).toBe("github:supportClaw/supportClaw#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "supportClaw",
        tag: "https://example.com/supportClaw-main.tgz",
      }),
    ).toBe("https://example.com/supportClaw-main.tgz");
  });
  ...
  it("resolves portable Git paths from process-local app data only", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "supportClaw-update-portable-git-" }, async (base) => {
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:supportClaw/supportClaw#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/supportClaw-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/supportClaw-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("/tmp/supportClaw-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("supportClaw-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:supportClaw/supportClaw#main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("/tmp/supportClaw-main.tgz")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "supportClaw-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "supportClaw");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "supportClaw"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "supportClaw"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "supportClaw"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "supportClaw"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "supportClaw-update-npm-prefix-" }, async (base) => {
        ...
        expect(globalInstallArgs("npm", "supportClaw@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "supportClaw@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "supportClaw@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "supportClaw@latest",
  ...
  it("keeps npm ownership but avoids per-Node npm commands for reinstall", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "supportClaw-update-node-version-prefix-" }, async (base) => {
        ...
        for (const layout of layouts) {
          ...
          const pkgRoot = path.join(nodeManagedRoot, "supportClaw");
          ...
          expect(globalInstallArgs("npm", "supportClaw@latest", pkgRoot), layout.name).toEqual([
            "npm",
            "i",
            "-g",
            "supportClaw@latest",
          ...
          expect(globalInstallFallbackArgs("npm", "supportClaw@latest", pkgRoot), layout.name).toEqual(
            [
              "npm",
              "i",
              "-g",
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "supportClaw-update-npm-missing-bin-" }, async (base) => {
  ...
  it("honors an explicitly selected direct npm node_modules package root", async () => {
    await withTempDir({ prefix: "supportClaw-update-managed-service-root-" }, async (base) => {
      const managedNpmRoot = path.join(base, ".supportClaw", "npm", "node_modules");
      const pkgRoot = path.join(managedNpmRoot, "supportClaw");
      ...
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(otherPnpmRoot, "supportClaw"), { recursive: true });
  ...
  it("preserves bun ownership for direct node_modules package roots", async () => {
    await withTempDir({ prefix: "supportClaw-update-managed-bun-root-" }, async (base) => {
      envSnapshot = captureEnv(["BUN_INSTALL"]);
      process.env.BUN_INSTALL = path.join(base, ".bun");
      const bunRoot = path.join(process.env.BUN_INSTALL, "install", "global", "node_modules");
      const pkgRoot = path.join(bunRoot, "supportClaw");
      const pathNpmRoot = path.join(base, "shell", "lib", "node_modules");
      await fs.mkdir(pkgRoot, { recursive: true });
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "supportClaw-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "supportClaw");
  ...
  it("detects custom pnpm global layouts from the running package root", async () => {
    await withTempDir({ prefix: "supportClaw-update-pnpm-custom-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "supportClaw");
  ...
  it("detects custom pnpm global layouts from virtual-store package roots", async () => {
    await withTempDir({ prefix: "supportClaw-update-pnpm-virtual-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(
        ...
        "5",
        ".pnpm",
        "supportClaw@file+..+pack+supportClaw-2026.5.6.tgz",
        "node_modules",
        "supportClaw",
      );
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        command: "pnpm",
        globalRoot: customGlobalRoot,
        packageRoot: path.join(customGlobalRoot, "supportClaw"),
      });
    });
  });

  it("does not infer pnpm ownership without pnpm node_modules metadata", async () => {
    await withTempDir({ prefix: "supportClaw-update-pnpm-shape-only-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "supportClaw");
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        packageRoot: path.join(defaultPnpmRoot, "supportClaw"),
      });
    });
  });
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "supportClaw@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "supportClaw@latest",
    ...
    expect(globalInstallArgs("pnpm", "supportClaw@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "supportClaw@latest",
    ]);
    expect(globalInstallArgs("pnpm", "github:supportClaw/supportClaw#release/2026.5.12")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=supportClaw",
      "github:supportClaw/supportClaw#release/2026.5.12",
    ]);
    expect(
      globalInstallArgs("pnpm", "supportClaw@git+https://github.com/brahminib/support-claw.git"),
    ).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=supportClaw",
      "supportClaw@git+https://github.com/brahminib/support-claw.git",
    ]);
    expect(globalInstallArgs("bun", "supportClaw@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "supportClaw@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "supportClaw@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "supportClaw@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "supportClaw@latest")).toBeNull();
    expect(
      globalInstallArgs({ manager: "pnpm", command: "/opt/homebrew/bin/pnpm" }, "supportClaw@latest"),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "supportClaw@latest"]);
    expect(globalInstallArgs("pnpm", "supportClaw@latest", null, "/opt/pnpm-global")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--global-dir",
  ...
  it("builds npm staged install argv with an explicit prefix", () => {
    expect(globalInstallArgs("npm", "supportClaw@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
    ...
    expect(globalInstallFallbackArgs("npm", "supportClaw@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
  ...
  it("resolves npm prefix layouts for normal global roots", () => {
    expect(resolveNpmGlobalPrefixLayoutFromGlobalRoot("/opt/supportClaw/lib/node_modules")).toEqual({
      prefix: "/opt/supportClaw",
      globalRoot: "/opt/supportClaw/lib/node_modules",
      binDir: "/opt/supportClaw/bin",
    });
  ...
  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "supportClaw-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".supportClaw-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".supportClaw-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".supportClaw-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "supportClaw"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".supportClaw-123", ".supportClaw-456"],
      });
      const packageDirStat = await fs.stat(path.join(root, "supportClaw"));
      const markerFileStat = await fs.stat(path.join(root, ".supportClaw-file"));
  ...
  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "supportClaw-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("reports bundled plugin install stages during installed dist verification", async () => {
    await withTempDir({ prefix: "supportClaw-update-global-plugin-stage-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await fs.mkdir(path.join(packageRoot, "dist", "extensions", "brave"), { recursive: true });
      await writePackageDistInventory(packageRoot);

      for (const stageDir of [".supportClaw-install-stage", ".supportClaw-install-stage-retry"]) {
      ...
      await expect(collectInstalledGlobalPackageErrors({ packageRoot })).resolves.toEqual([
        "unexpected packaged dist file dist/extensions/brave/.supportClaw-install-stage-retry/node_modules/typebox/build/compile/code.mjs",
        "unexpected packaged dist file dist/extensions/brave/.supportClaw-install-stage/node_modules/typebox/build/compile/code.mjs",
      ]);
    });
  });

  it("flags global package roots that resolve into source checkouts", async () => {
    await withTempDir({ prefix: "supportClaw-update-global-source-checkout-" }, async (base) => {
      const checkoutRoot = path.join(base, "checkout");
      const globalRoot = path.join(base, "prefix", "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "supportClaw");
  ...
  it("does not require private QA sidecars when the inventory is missing", async () => {
    await withTempDir({ prefix: "supportClaw-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "supportClaw-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "supportClaw-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir({ prefix: "supportClaw-update-global-legacy-plugin-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeBundledPluginPackageJson(packageRoot, "telegram", "@supportclaw/telegram");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "supportClaw-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "telegram", "@supportclaw/telegram");
        await writePackageDistInventory(packageRoot);
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "supportClaw-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@supportclaw/qa-lab");
        await writePackageDistInventory(packageRoot);
