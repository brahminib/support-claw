// Builds PATH values for SupportClaw child processes.
...
type EnsureSupportClawPathOpts = {
...
function candidateBinDirs(
  opts: EnsureSupportClawPathOpts,
  existingPathParts: ReadonlySet<string>,
  ...
  // subprocesses keep using the same Node/Bun the current SupportClaw process is on.
  try {
  ...
  // Bundled macOS app: `supportClaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "supportClaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.SUPPORT_CLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "supportClaw"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `supportClaw`
...
/**
 * Best-effort PATH bootstrap so skills that require the `supportClaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSupportClawCliOnPath(opts: EnsureSupportClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SUPPORT_CLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  // Mark before filesystem probing so repeated calls from nested bootstraps do
  // not keep reshuffling PATH.
  process.env.SUPPORT_CLAW_PATH_BOOTSTRAPPED = "1";
