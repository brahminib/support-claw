...
const DEFAULT_EXEC_APPROVALS_STATE_DIR = "~/.supportClaw";
...
export function resolveExecApprovalsTranscriptPath(): string {
  return process.env.SUPPORT_CLAW_STATE_DIR?.trim()
    ? `$SUPPORT_CLAW_STATE_DIR/${EXEC_APPROVALS_FILE}`
...
function hasUnmigratedLegacyExecApprovals(filePath: string): boolean {
  if (!process.env.SUPPORT_CLAW_STATE_DIR?.trim()) {
    return false;
  }
...
function isReadOnlySecurityAuditSuppressionInspection(argv: string[]): boolean {
  ...
  let offset = command === "pnpm" && argv[1] === "supportClaw" ? 1 : 0;
  if (normalizeCommandName(argv[offset]) !== "supportClaw") {
    return false;
  }
  offset += 1;
