// Validates the current runtime against SupportClaw's Node engine floor.
...
/** Returns whether a detected runtime meets SupportClaw's minimum runtime contract. */
...
/** Checks a Node version label against SupportClaw's current minimum Node version. */
...
export function assertSupportedRuntime(
  ...
  runtime.error(
    [
      "supportClaw requires Node >=22.19.0.",
      `Detected: ${runtimeLabel} (exec: ${execLabel}).`,
      `PATH searched: ${details.pathEnv}`,
      "Install Node: https://nodejs.org/en/download",
      "Upgrade Node and re-run supportClaw.",
    ].join("\n"),
  );
  runtime.exit(1);
}
