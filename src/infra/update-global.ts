...
/** Supported package managers for SupportClaw global install and update flows. */
...
const PRIMARY_PACKAGE_NAME = "supportClaw";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
/** npm-compatible spec used when the user asks to install the moving main branch. */
export const SUPPORT_CLAW_MAIN_PACKAGE_SPEC = "github:supportClaw/supportClaw#main";
const COREPACK_ENABLE_DOWNLOAD_PROMPT_DEFAULT = "0";
const NPM_GLOBAL_INSTALL_QUIET_FLAGS = ["--no-fund", "--no-audit", "--loglevel=error"] as const;
const PNPM_SUPPORT_CLAW_BUILD_ALLOWLIST_FLAG = `--allow-build=${PRIMARY_PACKAGE_NAME}`;
...
function isPnpmSupportClawSourceInstallSpec(spec: string): boolean {
...
/**
 * Extracts a pinned installed version from package specs like `supportClaw@1.2.3`.
...
/**
 * Verifies that a global package root looks like a packaged SupportClaw install
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "supportClaw.plugin.json")))
      ) {
...
async function resolvePortableGitPathPrepend(): Promise<string[]> {
  ...
  const portableGitRoot = path.join(localAppData, "SupportClaw", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.SUPPORT_CLAW_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.SUPPORT_CLAW_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return SUPPORT_CLAW_MAIN_PACKAGE_SPEC;
  }
 ...
 * Used for staged installs where SupportClaw creates the prefix itself.
 */
...
/**
 * Detects an installed global SupportClaw package by probing package-manager roots
...
/**
 * Builds the primary package-manager argv for a global SupportClaw install.
...
export function globalInstallArgs(
  ...
  if (resolved.manager === "pnpm") {
    return [
      ...
      ...(isPnpmSupportClawSourceInstallSpec(spec) ? [PNPM_SUPPORT_CLAW_BUILD_ALLOWLIST_FLAG] : []),
      spec,
    ];
  }
  if (resolved.manager === "bun") {
