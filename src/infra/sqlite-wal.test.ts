...
describe("sqlite WAL maintenance", () => {
  ...
  it("uses rollback journaling for databases on NFS-backed volumes", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sqlite-nfs-"));
    try {
      const db = createMockDb();
      ...
      const maintenance = configureSqliteWalMaintenance(db, {
        ...
        databasePath: path.join(tempDir, "missing", "supportClaw.sqlite"),
      });
  ...
  it.each([
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sqlite-network-"));
    try {
      const db = createMockDb();
      ...
      configureSqliteWalMaintenance(db, {
        checkpointIntervalMs: 0,
        databasePath: path.join(tempDir, "supportClaw.sqlite"),
      });
  ...
  it.each([
    String.raw`\\server\share\supportClaw.sqlite`,
    String.raw`\\?\UNC\server\share\supportClaw.sqlite`,
    "//server/share/supportClaw.sqlite",
    "//?/UNC/server/share/supportClaw.sqlite",
  ...
  it("uses rollback journaling for mapped Windows network drives", () => {
    const db = createMockDb();
    const databasePath = String.raw`Z:\state\supportClaw.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    const realpath = vi
      .spyOn(fs.realpathSync, "native")
      .mockReturnValue(String.raw`\\server\share\state\supportClaw.sqlite`);
  ...
  it("does not treat namespaced Windows local drives as UNC paths", () => {
    const db = createMockDb();
    const databasePath = String.raw`\\?\C:\state\supportClaw.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
  ...
  it("uses rollback journaling when Windows cannot classify an opened drive path", () => {
    const db = createMockDb();
    const databasePath = String.raw`Z:\restricted\supportClaw.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
  ...
  it("refuses fuse.sshfs mountinfo entries", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sqlite-sshfs-"));
    try {
      const db = createMockDb();
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
      vi.spyOn(fs, "readFileSync").mockReturnValue(
  ...
  it("refuses symlinked paths into fuse.sshfs mounts", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sqlite-sshfs-link-"));
    ...
    try {
      ...
      expect(() =>
        configureSqliteWalMaintenance(createMockDb(), {
          checkpointIntervalMs: 0,
          databasePath: path.join(linkedDir, "supportClaw.sqlite"),
        }),
  ...
  it("matches raw mount paths when the existing path canonicalizes elsewhere", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sqlite-sshfs-prefix-"));
    ...
    try {
      ...
      expect(() =>
        configureSqliteWalMaintenance(createMockDb(), {
          checkpointIntervalMs: 0,
          databasePath: path.join(rawMountDir, "supportClaw.sqlite"),
        }),
      ).toThrow(/SSHFS.*refusing to open/);
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
  ...
  it("uses macOS SMB mount filesystem names", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sqlite-smb-"));
    try {
      const db = createMockDb();
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
      vi.spyOn(fs, "readFileSync").mockImplementation(() => {
  ...
  it.each([
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sqlite-sshfs-macfuse-"));
    try {
      const db = createMockDb();
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
      vi.spyOn(fs, "readFileSync").mockImplementation(() => {
  ...
  it("keeps WAL enabled for non-remote macFUSE mounts", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sqlite-macfuse-"));
    try {
      const db = createMockDb();
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
      vi.spyOn(fs, "readFileSync").mockImplementation(() => {
