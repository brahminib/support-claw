...
const { triggerSupportClawRestart } = await import("./restart.js");
...
describe.runIf(process.platform !== "win32")("findGatewayPidsOnPortSync", () => {
  it("parses lsof output and filters non-supportClaw/current processes", () => {
  ...
  it("kills stale gateway pids discovered on the gateway port", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePidA}`, "csupportClaw", `p${stalePidB}`, "csupportClaw-gateway"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
  ...
  it("uses explicit port override when provided", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePid}`, "csupportClaw"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
...
describe("triggerSupportClawRestart", () => {
  it("does not kickstart after bootstrap registers an unloaded LaunchAgent", () => {
    setPlatform("darwin");
    withEnv(
      { VITEST: undefined, NODE_ENV: undefined, HOME: "/Users/test", SUPPORT_CLAW_PROFILE: "default" },
      () => {
        ...
        const result = triggerSupportClawRestart();
        ...
        expect(result).toEqual({
          ...
          tried: [
            ...
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.supportClaw.gateway.plist`,
          ],
        });
      },
    );
      ...
      () => {
        ...
        expect(result).toEqual({
          ...
          tried: [
            `launchctl kickstart -k gui/${uid}/ai.supportClaw.gateway`,
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.supportClaw.gateway.plist`,
            `launchctl kickstart gui/${uid}/ai.supportClaw.gateway`,
          ],
        });
      },
    );
