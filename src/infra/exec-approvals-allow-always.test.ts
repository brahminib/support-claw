...
describe("resolveAllowAlwaysPatterns", () => {
  ...
  it("returns direct executable paths for non-shell segments", () => {
    const exe = path.join("/tmp", "supportClaw-tool");
      ...
      segments: [
        {
          ...
          resolution: makeMockCommandResolution({
            execution: makeMockExecutableResolution({
              rawExecutable: exe,
              resolvedPath: exe,
              executableName: "supportClaw-tool",
            }),
          }),
        },
      ],
  ...
  it("rejects startup shell inline payloads for allow-always and inline-chain allowlist fallback", async () => {
    ...
    const dir = makeTempDir();
    const tool = makeExecutable(dir, "supportClaw-ok");
    const env = { PATH: `${dir}${path.delimiter}${process.env.PATH ?? ""}` };
    const safeBins = resolveSafeBins(undefined);

    for (const command of [
      `bash --login -c "supportClaw-ok && supportClaw-ok"`,
      `bash -i -c "supportClaw-ok && supportClaw-ok"`,
      `bash -lc "supportClaw-ok && supportClaw-ok"`,
