...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.supportClaw.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  ...
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const supportClawPayload = requireRecord(payload.supportClaw, "supportClaw payload");
    expectRecordFields(supportClawPayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof supportClawPayload.ts).toBe("number");
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.supportClaw.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open SupportClaw to review this request.",
      },
      sound: "default",
      category: "supportClaw.exec-approval",
      "content-available": 1,
    });
    const supportClawPayload = requireRecord(payload.supportClaw, "supportClaw payload");
    expectRecordFields(supportClawPayload, {
    ...
    expect(typeof supportClawPayload.ts).toBe("number");
    expectNoProperties(supportClawPayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    expectRecordFields(requireRecord(payload.supportClaw, "supportClaw payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });
