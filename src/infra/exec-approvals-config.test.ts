...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    const prevSupportClawHome = process.env.SUPPORT_CLAW_HOME;

    try {
      process.env.SUPPORT_CLAW_HOME = dir;
      const approvalsPath = path.join(dir, ".supportClaw", "exec-approvals.json");
      ...
      if (prevSupportClawHome === undefined) {
        delete process.env.SUPPORT_CLAW_HOME;
      } else {
        process.env.SUPPORT_CLAW_HOME = prevSupportClawHome;
      }
    }
  });
});
