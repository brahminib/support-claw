...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SUPPORT_CLAW_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*supportClaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "supportClaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "supportClaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "supportClaw-gw");
  ...
  records.push(`_supportClaw-gw._tcp IN PTR ${instanceLabel}._supportClaw-gw._tcp`);
  records.push(`${instanceLabel}._supportClaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._supportClaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; supportClaw-content-hash: ${contentHash}\n${contentBody}`;
}
