...
import { triggerSupportClawRestart } from "./restart.js";
...
const PNPM_VERSIONED_SUPPORT_CLAW_ENTRY_PATTERN =
  /^(.*?)([\\/])node_modules\2\.pnpm\2supportClaw@[^\\/]+\2node_modules\2supportClaw\2.+$/;

function rewritePnpmVersionedSupportClawEntryPath(entryPath: string): string {
  // pnpm can expose argv[1] as a versioned realpath that self-update removes.
  // Respawn through the stable SupportClaw package wrapper instead.
  return entryPath.replace(
    PNPM_VERSIONED_SUPPORT_CLAW_ENTRY_PATTERN,
    "$1$2node_modules$2supportClaw$2supportClawCli.mjs",
  );
}
...
function spawnDetachedGatewayProcess(opts: GatewayRespawnOptions = {}): {
  ...
  const args = [
    ...process.execArgv,
    ...(entryArg ? [rewritePnpmVersionedSupportClawEntryPath(entryArg)] : []),
    ...entryArgs,
  ];
 ...
 * - SUPPORT_CLAW_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
...
export function restartGatewayProcessWithFreshPid(
  ...
  if (isTruthy(process.env.SUPPORT_CLAW_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerSupportClawRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
...
export function respawnGatewayProcessForUpdate(
  ...
  if (isTruthy(process.env.SUPPORT_CLAW_NO_RESPAWN)) {
    return { mode: "disabled", detail: "SUPPORT_CLAW_NO_RESPAWN" };
  }
