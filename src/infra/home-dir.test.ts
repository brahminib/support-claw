// Tests SupportClaw home directory resolution.
...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers SUPPORT_CLAW_HOME over HOME and USERPROFILE",
      env: {
  ...
  it("expands SUPPORT_CLAW_HOME when set to ~", () => {
    const env = {
      SUPPORT_CLAW_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for SUPPORT_CLAW_HOME",
      env: { SUPPORT_CLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when SUPPORT_CLAW_HOME is tilde-only and no fallback home exists",
      env: { SUPPORT_CLAW_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores SUPPORT_CLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          SUPPORT_CLAW_HOME: "/srv/supportClaw-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/supportClaw-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/supportClaw-home " },
      expected: "/srv/supportClaw-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/supportClaw-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of SUPPORT_CLAW_HOME", () => {
    expect(
