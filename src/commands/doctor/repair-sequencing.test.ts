...
const mocks = vi.hoisted(() => ({
  ...
  maybeRepairManagedNpmSupportClawPeerLinks: vi.fn(),
...
vi.mock("../doctor-plugin-registry.js", () => ({
  maybeRepairManagedNpmSupportClawPeerLinks: mocks.maybeRepairManagedNpmSupportClawPeerLinks,
...
vi.mock("./shared/channel-doctor.js", () => ({
  collectChannelDoctorRepairMutations: ({ cfg }: { cfg: SupportClawConfig }) => {
...
vi.mock("./shared/empty-allowlist-scan.js", () => ({
  scanEmptyAllowlistPolicyWarnings: (cfg: SupportClawConfig) =>
    cfg.channels?.signal
...
vi.mock("./shared/allowlist-policy-repair.js", () => ({
  maybeRepairAllowlistPolicyAllowFrom: async (cfg: SupportClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/bundled-plugin-load-paths.js", () => ({
  maybeRepairBundledPluginLoadPaths: (cfg: SupportClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/invalid-plugin-config.js", () => ({
  maybeRepairInvalidPluginConfig: (cfg: SupportClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/legacy-tools-by-sender.js", () => ({
  maybeRepairLegacyToolsBySenderKeys: (cfg: SupportClawConfig) => {
...
vi.mock("./shared/exec-safe-bins.js", () => ({
  maybeRepairExecSafeBinProfiles: (cfg: SupportClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
describe("doctor repair sequencing", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.applyPluginAutoEnable.mockImplementation((params: { config: SupportClawConfig }) => ({
      config: params.config,
      changes: [],
    }));
    mocks.materializePluginAutoEnableCandidates.mockImplementation(
      (params: { config: SupportClawConfig }) => ({
        config: params.config,
        changes: [],
      }),
    );
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementation((cfg: SupportClawConfig) => ({
      config: cfg,
      changes: [],
    }));
    mocks.maybeRepairManagedNpmSupportClawPeerLinks.mockResolvedValue(false);
    ...
    mocks.maybeRepairOpenAICodexAuthConfig.mockImplementation((cfg: SupportClawConfig) => ({
      changes: [],
      config: cfg,
      warnings: [],
    }));
    ...
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementation((cfg: SupportClawConfig) => ({
      config: cfg,
      changes: [],
    }));
    ...
    mocks.maybeRepairStalePluginConfig.mockImplementation((cfg: SupportClawConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("applies ordered repairs and sanitizes empty-allowlist warnings", async () => {
    const result = await runDoctorRepairSequence({
      ...
      doctorFixCommand: "supportClaw doctor --fix",
    });
  ...
  it("repairs managed npm plugin drift before missing plugin install repair", async () => {
    ...
    mocks.maybeRepairManagedNpmSupportClawPeerLinks.mockImplementation(async () => {
      events.push("supportClaw-peer-links");
      return true;
    });
    ...
    expect(events).toEqual(["bundled-shadow-cleanup", "supportClaw-peer-links", "missing-installs"]);
    ...
    expect(mocks.maybeRepairManagedNpmSupportClawPeerLinks).toHaveBeenCalledOnce();
    const peerLinkCall = mocks.maybeRepairManagedNpmSupportClawPeerLinks.mock.calls[0]?.[0];
  ...
  it("repairs stale OAuth shadows before importing and removing auth JSON", async () => {
    ...
    const result = await runDoctorRepairSequence({
      state: {
        ...
        candidate: {} as SupportClawConfig,
        pendingChanges: false,
        fixHints: [],
      },
      doctorFixCommand: "supportClaw doctor --fix",
  ...
  it("auto-enables newly installed configured plugins after doctor repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: ['Installed missing configured plugin "brave" from @supportclaw/brave-plugin.'],
      warnings: [],
    });
    mocks.applyPluginAutoEnable.mockImplementationOnce((params: { config: SupportClawConfig }) => ({
      config: {
        ...params.config,
        plugins: {
    ...
    expect(result.changeNotes).toStrictEqual([
      'Installed missing configured plugin "brave" from @supportclaw/brave-plugin.',
  ...
  it("explicitly enables plugins repaired from env-only configuration", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: ['Installed missing configured plugin "exa" from @supportclaw/exa-plugin.'],
      warnings: [],
    ...
    expect(result.changeNotes).toStrictEqual([
      'Installed missing configured plugin "exa" from @supportclaw/exa-plugin.',
  ...
  it("moves legacy Codex routes to canonical OpenAI before missing plugin install repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockImplementationOnce(
      async (params: { cfg: SupportClawConfig }) => {
  ...
  it("runs group allowFrom fallback migration after open-policy allowFrom repair", async () => {
    const events: string[] = [];
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementationOnce((cfg: SupportClawConfig) => {
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementationOnce((cfg: SupportClawConfig) => {
  ...
  it("does not remove deferred configured plugins during the package update doctor pass", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [
        'Skipped package-manager repair for configured plugin "brave" during package update; rerun "supportClaw doctor --fix" after the update completes.',
      ],
      warnings: [],
    });
    const result = await runDoctorRepairSequence({
  ...
  it("preserves configured plugins when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "brave" from @supportclaw/brave-plugin: package install failed',
      ],
  ...
  it("preserves configured channels when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured channel plugin "whatsapp" from @supportclaw/whatsapp: package install failed',
      ],
