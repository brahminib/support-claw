...
export function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("supportClaw cron list")} and ${formatCliCommand("supportClaw cron show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.supportClaw/bin/ensure-whatsapp.sh` is not maintained by current SupportClaw and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("supportClaw channels status --probe")}, ${formatCliCommand("supportClaw doctor")}, and ${formatCliCommand("supportClaw gateway status")} for current health checks.`,
