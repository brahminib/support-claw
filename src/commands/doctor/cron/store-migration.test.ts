...
describe("normalizeStoredCronJobs", () => {
  ...
  it("converts legacy agent command prompts into command cron payloads", () => {
    const command =
      "cd /home/supportClaw/.razor/quant && ./scripts/system/run_position_control.sh --write-card --silent-token NO_REPLY";
    const { job, result } = normalizeOneJob(
      makeLegacyJob({
        ...
        payload: {
          ...
          message: [
            ...
            "Command to run:",
            `- command: ${command}`,
            "- workdir: /home/supportClaw/.razor/quant",
            "- background: false",
            "- timeout: 840",
            "",
    ...
    expect(payload).toEqual({
      kind: "command",
      argv: ["sh", "-lc", command],
      cwd: "/home/supportClaw/.razor/quant",
      timeoutSeconds: 900,
    });
  });
