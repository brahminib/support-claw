...
async function makeTempStorePath() {
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-doctor-cron-"));
...
function createCronConfig(storePath: string): SupportClawConfig {
  return {
    cron: {
      store: storePath,
...
describe("maybeRepairLegacyCronStore", () => {
  ...
  it("advises on isolated shell-prompt jobs without a non-actionable --fix repair note (#94655)", async () => {
    ...
    expectNoNoteContaining("supportClaw doctor --fix", "Cron");
    ...
    const commandPromptJob = createCurrentCronJob({
      ...
      payload: {
        ...
        message: [
          ...
          "- workdir: /home/supportClaw/.razor/clawd",
        ].join("\n"),
  ...
  it("removes inert legacy notify:true for delivery.mode none when cron.webhook is unset and stops looping (#44460)", async () => {
    ...
    const cfg = { cron: { store: storePath } } as SupportClawConfig;
    await maybeRepairLegacyCronStore({
      cfg,
      options: {},
      prompter: makePrompter(true),
  ...
  it("rewrites stale managed dreaming jobs to the isolated agentTurn shape", async () => {
    ...
    const payload = requireRecord(job.payload, "cron payload");
    expect(payload.kind).toBe("agentTurn");
    expect(payload.message).toBe("__supportClaw_memory_core_short_term_promotion_dream__");
    ...
    const warning = await collectLegacyWhatsAppCrontabHealthWarning({
      ...
      readCrontab: async () => ({
        stdout: [
          ...
          "*/5 * * * * ~/.supportClaw/bin/ensure-whatsapp.sh >> ~/.supportClaw/logs/whatsapp-health.log 2>&1",
          "0 9 * * * /usr/bin/true",
          "",
        ].join("\n"),
      }),
  ...
  it("ignores malformed crontab output instead of crashing", async () => {
    ...
    await expect(
      noteLegacyWhatsAppCrontabHealthCheck({
        platform: "linux",
        readCrontab: async () => ({
          stdout: { lines: ["*/5 * * * * ~/.supportClaw/bin/ensure-whatsapp.sh"] },
        }),
      }),
    ).resolves.toBeUndefined();
