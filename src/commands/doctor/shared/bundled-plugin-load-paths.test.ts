...
import { bundledDistPluginRootAt, bundledPluginRootAt } from "supportClaw/plugin-sdk/test-fixtures";
...
describe("bundled plugin load path repair", () => {
  beforeEach(() => {
    const packageRoot = "/app/node_modules/supportClaw";
  ...
  it("detects legacy bundled plugin paths that still point at source extensions", () => {
    const packageRoot = path.resolve("app-node-modules", "supportClaw");
  ...
  it("removes stale bundled paths from old versioned SupportClaw package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "supportClaw");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
      "supportClaw@2026.3.28_@napi-rs+canvas@0.1.97",
      "node_modules",
      "supportClaw",
    );
  ...
  it("removes stale legacy bundled paths from old versioned SupportClaw package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "supportClaw");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
  ...
  it("formats a doctor hint for legacy bundled plugin paths", () => {
    ...
    expect(warnings).toEqual([
      `- plugins.load.paths: bundled plugin path "${legacyPath}" still aliases feishu; SupportClaw loads the packaged bundled plugin from "${bundledPath}".`,
      '- Run "supportClaw doctor --fix" to remove these redundant bundled plugin paths.',
    ]);
  });

  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "supportClaw", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
