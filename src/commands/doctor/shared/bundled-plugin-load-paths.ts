...
function resolveBundledWorkspaceDir(cfg: SupportClawConfig): string | undefined {
  return resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg)) ?? undefined;
}

function isSupportClawNodeModulesPackageRoot(packageRoot: string): boolean {
  ...
  return packageDir === "supportClaw" && parentDir === "node_modules";
}

/** Find configured plugin load paths that alias bundled plugins already shipped by SupportClaw. */
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed SupportClaw package; user plugin paths stay intact.
        oldPackageRoot && oldBundledLeaf && isSupportClawNodeModulesPackageRoot(oldPackageRoot)
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; SupportClaw loads the packaged bundled plugin from "${hit.toPath}".`,
  );
