...
import { isRecord as hasRecord } from "@supportclaw/normalization-core/record-coerce";
...
function listAgentRecords(cfg: SupportClawConfig): Record<string, unknown>[] {
  return Array.isArray(cfg.agents?.list) ? cfg.agents.list.filter(hasRecord) : [];
}

function hasChannels(cfg: SupportClawConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: SupportClawConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: SupportClawConfig): boolean {
...
function hasSubagentAllowlistConfig(cfg: SupportClawConfig): boolean {
...
function hasConfiguredSafeBins(cfg: SupportClawConfig): boolean {
...
function sourceReplyRuntimeMayAllowMessageTool(cfg: SupportClawConfig): boolean {
...
function resolveGroupVisibleReplyProvenance(cfg: SupportClawConfig): {
...
export function collectVisibleReplyToolPolicyWarnings(cfg: SupportClawConfig): string[] {
  const groupPolicy = resolveGroupVisibleReplyProvenance(cfg);
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; SupportClaw falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    ...
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; SupportClaw falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
export function collectChannelBoundMessageToolPolicyWarnings(cfg: SupportClawConfig): string[] {
...
export function collectProfileConfiguredToolSectionWarnings(cfg: SupportClawConfig): string[] {
  const warnings: string[] = [];
...
async function resolveDoctorChannelPreviewConfig(params: {
  ...
  allowExec?: boolean;
}): Promise<{ cfg: SupportClawConfig; diagnostics: string[] }> {
