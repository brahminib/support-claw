...
function isCodexRuntimeConfigured(cfg: SupportClawConfig, _env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg).includes("codex");
}

function isCodexPluginConfigured(cfg: SupportClawConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: SupportClawConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      "- Personal Codex CLI assets were found, but native Codex-mode SupportClaw agents use isolated per-agent Codex homes.",
      ...
      "- If the Codex plugin is not installed, run `supportClaw plugins install npm:@supportclaw/codex` first. Then run `supportClaw migrate plan codex` to inventory them. Applying that migration copies skills into the current SupportClaw agent workspace; Codex plugins, hooks, and config stay manual-review only.",
    ].join("\n"),
  ];
}
