...
function migrateLegacyConfigForTest(raw: unknown): {
  ...
  return changes.length === 0
    ? { config: null, changes }
    : { config: next as SupportClawConfig, changes };
}
  ...
  it("removes sidecar memory search index paths", () => {
    const res = migrateLegacyConfigForTest({
      memorySearch: {
        provider: "openai",
        store: {
          path: "/tmp/supportClaw-memory-{agentId}.sqlite",
    ...
    const res = migrateLegacyConfigForTest({
      agents: {
        ...
        list: [
          {
            id: "reviewer",
            agentRuntime: { fallback: "supportClaw" },
            embeddedHarness: {
              runtime: "codex",
              fallback: "none",
            },
...
describe("legacy migrate MCP server type aliases", () => {
  it("moves CLI-native http type to SupportClaw streamable HTTP transport", () => {
    const res = migrateLegacyConfigForTest({
      mcp: {
        servers: {
          silo: {
