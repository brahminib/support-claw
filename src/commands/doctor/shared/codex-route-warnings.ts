...
import { AGENT_MODEL_CONFIG_KEYS } from "@supportclaw/model-catalog-core/configured-model-refs";
import { normalizeProviderId } from "@supportclaw/model-catalog-core/provider-id";
import { asOptionalRecord as asMutableRecord } from "@supportclaw/normalization-core/record-coerce";
import { normalizeOptionalLowercaseString as normalizeString } from "@supportclaw/normalization-core/string-coerce";
...
function resolveImplicitDefaultAgentModelRef(cfg: SupportClawConfig): string {
...
function collectConfigModelRefs(cfg: SupportClawConfig): CodexRouteHit[] {
...
function codexPluginAllowlistIsRestrictive(cfg: SupportClawConfig): boolean {
  const allow = cfg.plugins?.allow;
  return Array.isArray(allow) && allow.length > 0 && !pluginIdListIncludes(allow, "codex");
}

function isCodexPluginUnavailableByConfig(cfg: SupportClawConfig): boolean {
...
function codexPluginIsBlockedOutsideEntry(cfg: SupportClawConfig): boolean {
...
function collectDisabledCodexPluginRouteHits(cfg: SupportClawConfig): DisabledCodexPluginRouteHit[] {
...
}): { cfg: SupportClawConfig; changes: string[] } {
...
function rewriteAgentModelRefs(params: {
  cfg: SupportClawConfig;
  preRepairCfg: SupportClawConfig;
  hits: CodexRouteHit[];
...
function clearConfigLegacyAgentRuntimePolicies(cfg: SupportClawConfig): string[] {
  const changes: string[] = [];
...
function configRepairWouldClearLegacyRuntimePins(params: { cfg: SupportClawConfig }): boolean {
...
function rewriteConfigModelRefs(params: { cfg: SupportClawConfig }): ConfigRouteRepairResult {
...
function formatUnsupportedCompactionWarning(params: {
  ...
  return [
    "- Codex runtime uses native server-side compaction and ignores SupportClaw compaction summarizer overrides.",
    ...params.hits.map(
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `supportClaw doctor --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function formatDisabledCodexPluginWarning(params: {
  ...
  const fixHint = params.blockedOutsideEntry
    ? "- Enable plugin loading and remove `codex` from plugins.deny, or set the affected OpenAI models to an SupportClaw runtime policy."
    : "- Run `supportClaw doctor --fix`: it enables plugins.entries.codex, or set the affected OpenAI models to an SupportClaw runtime policy.";
  return [
...
function collectCodexAppServerCommandWarnings(cfg: SupportClawConfig): string[] {
  const plugins = asMutableRecord(cfg.plugins);
  const entries = asMutableRecord(plugins?.entries);
  ...
  if (hits.length > 0) {
    warnings.push(
      [
        ...
        "- Run `supportClaw doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
      ].join("\n"),
    );
  }
  ...
  if (fixableHits.length > 0) {
    warnings.push(
      formatUnsupportedCompactionWarning({
        hits: fixableHits,
        fixHint:
          "- Run `supportClaw doctor --fix`: it removes unsupported Codex compaction overrides.",
      }),
    );
  }
  return warnings;
...
}): { cfg: SupportClawConfig; warnings: string[]; changes: string[] } {
  const hits = collectConfigModelRefs(params.cfg);
  const disabledCodexPluginHits = collectDisabledCodexPluginRouteHits(params.cfg);
  const ignoreLegacyAgentRuntimePins = configRepairWouldClearLegacyRuntimePins(params);
  const unsupportedCompactionOverrides = collectUnsupportedCodexCompactionOverrides({
...
function clearStaleSessionRuntimePins(entry: SessionEntry): boolean {
  ...
  let changed = false;
  if (entry.agentHarnessId !== undefined && harnessRuntime !== "supportClaw") {
    delete entry.agentHarnessId;
    changed = true;
  }
  if (entry.agentRuntimeOverride !== undefined && overrideRuntime !== "supportClaw") {
        ...
        stale.length > 0
          ? [
              [
                ...
                "- Run `supportClaw doctor --fix` to rewrite stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : [],
      changes: [],
