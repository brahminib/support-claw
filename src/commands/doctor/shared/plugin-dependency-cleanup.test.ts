...
describe("cleanupLegacyPluginDependencyState", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-plugin-deps-cleanup-"));
  });
  ...
  it("collects and removes legacy plugin dependency state roots", async () => {
    ...
    const explicitStageDir = path.join(stateDir, ".supportClaw-install-stage-explicit");
    ...
    const env = {
      ...
      SUPPORT_CLAW_PLUGIN_STAGE_DIR: explicitStageDir,
  ...
  it("removes configured plugin stage roots outside SupportClaw roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    ...
    await fs.writeFile(
      path.join(stageRoot, "node_modules", "ansi-escapes", ".supportClaw-rename-tmp"),
    ...
    const result = await cleanupLegacyPluginDependencyState({
      env: {
        SUPPORT_CLAW_STATE_DIR: stateDir,
        SUPPORT_CLAW_PLUGIN_STAGE_DIR: stageRoot,
      },
      packageRoot,
    });
  ...
  it("refuses arbitrary explicit plugin stage roots outside SupportClaw roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
  ...
  it("refuses explicit plugin stage paths with parent segments", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    const dotDotStage = `${stateDir}${path.sep}..${path.sep}.supportClaw-install-stage-dotdot`;
    ...
    const result = await cleanupLegacyPluginDependencyState({
      env: {
        SUPPORT_CLAW_STATE_DIR: stateDir,
        SUPPORT_CLAW_PLUGIN_STAGE_DIR: dotDotStage,
      },
      packageRoot,
    });
  ...
  it("does not follow symlinked extension roots outside SupportClaw roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    ...
    await fs.writeFile(path.join(externalPlugin, ".supportClaw-runtime-deps.json"), "{}");
  ...
  it("refuses legacy roots that resolve outside SupportClaw roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    const legacyRuntimeRoot = path.join(stateDir, "plugin-runtime-deps");
    ...
    expect(result.changes).toStrictEqual([]);
    expect(result.warnings).toContain(
      `Skipped legacy plugin dependency state ${legacyRuntimeRoot}: resolved outside SupportClaw cleanup roots`,
    );
  ...
  it("does not unlink global runtime symlinks through unsafe cleanup roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "prefix", "lib", "node_modules", "supportClaw");
    const nodeModulesRoot = path.dirname(packageRoot);
    const legacyRuntimeRoot = path.join(stateDir, "plugin-runtime-deps");
    const externalRuntimeRoot = path.join(tempDir, "external-runtime");
    const activeRuntimeTarget = path.join(
      externalRuntimeRoot,
      "supportClaw-external",
      "node_modules",
      "left-pad",
    );
