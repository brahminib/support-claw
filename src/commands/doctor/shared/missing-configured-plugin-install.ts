...
const SUPPORT_CLAW_BETA_COMPANION_VERSION_RE = /^(\d{4}\.[1-9]\d?\.[1-9]\d?)-beta\.[1-9]\d*$/;
const SUPPORT_CLAW_STABLE_OR_BETA_COMPANION_VERSION_RE =
...
function addConfiguredAgentRuntimePluginIds(ids: Set<string>, cfg: SupportClawConfig): void {
...
function addConfiguredSpeechProviderPluginIds(ids: Set<string>, cfg: SupportClawConfig): void {
  for (const pluginId of resolveOfficialExternalProviderContractPluginIds({
...
function addConfiguredWebFetchProviderPluginIds(ids: Set<string>, cfg: SupportClawConfig): void {
...
function collectConfiguredPluginIds(cfg: SupportClawConfig, env?: NodeJS.ProcessEnv): Set<string> {
...
function collectBlockedPluginIds(cfg: SupportClawConfig): Set<string> {
  const ids = new Set<string>();
...
function collectConfiguredChannelIds(cfg: SupportClawConfig, env?: NodeJS.ProcessEnv): Set<string> {
  const ids = new Set<string>();
...
function installedRuntimePackageVersionIsStale(params: {
  ...
  const comparison = compareSupportClawReleaseVersions(params.installedVersion, params.currentVersion);
...
function betaCompanionMatchesCurrentStableVersion(params: {
  ...
  const installedBase = SUPPORT_CLAW_BETA_COMPANION_VERSION_RE.exec(params.installedVersion)?.[1];
  const currentBase = SUPPORT_CLAW_STABLE_OR_BETA_COMPANION_VERSION_RE.exec(params.currentVersion)?.[1];
...
/** Repair missing installs inferred from the current SupportClaw config. */
...
async function repairMissingPluginInstalls(params: {
  ...
  if (shouldDeferConfiguredPluginInstallRepair(env)) {
    ...
    for (const pluginId of updateDeferredPluginIds) {
      ...
      const detail = `Skipped package-manager repair for configured plugin "${pluginId}" during package update; rerun "supportClaw doctor --fix" after the update completes.`;
      changes.push(detail);
