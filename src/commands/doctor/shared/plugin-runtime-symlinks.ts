...
export async function noteStalePluginRuntimeSymlinks(
  ...
  const lines = [
    ...
    `  ${PLUGIN_RUNTIME_DEPS_MARKER} directories from a previous SupportClaw install.`,
  ...
  lines.push("- Repair: run `supportClaw doctor --fix` to remove the dangling symlinks.");
