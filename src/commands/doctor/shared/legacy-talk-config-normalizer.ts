...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<SupportClawConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as SupportClawConfig["talk"])?.realtime;
}
...
export function normalizeLegacyTalkConfig(cfg: SupportClawConfig, changes: string[]): SupportClawConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as SupportClawConfig["talk"]) ?? {};
