...
import { normalizeNullableString as normalizeId } from "@supportclaw/normalization-core/string-coerce";
...
import { compareSupportClawVersions } from "../../../config/version.js";
import {
...
function isPluginsGloballyDisabled(cfg: SupportClawConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: SupportClawConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: SupportClawConfig): string[] {
...
function isPluginEntryDisabled(cfg: SupportClawConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: SupportClawConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: SupportClawConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: SupportClawConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: SupportClawConfig): string[] {
...
function collectConfiguredChannelIds(cfg: SupportClawConfig, env: NodeJS.ProcessEnv): string[] {
  const ids = new Set<string>();
  const channels = asObjectRecord(cfg.channels);
  if (channels) {
...
function collectWebSearchPluginIds(cfg: SupportClawConfig): string[] {
...
function collectEnvWebSearchPluginIds(cfg: SupportClawConfig, env: NodeJS.ProcessEnv): string[] {
  if (cfg.tools?.web?.search?.enabled === false) {
    return [];
  }
...
function collectWebFetchPluginIds(cfg: SupportClawConfig): string[] {
...
function collectEnvWebFetchPluginIds(cfg: SupportClawConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectSpeechPluginIds(cfg: SupportClawConfig): string[] {
  return resolveOfficialExternalProviderContractPluginIds({
...
function collectAcpRuntimePluginIds(cfg: SupportClawConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: SupportClawConfig): string[] {
...
function addEligiblePluginId(cfg: SupportClawConfig, pluginIds: Set<string>, pluginId: string): void {
...
export function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareSupportClawVersions(
  ...
  const touchedComparedToRelease = compareSupportClawVersions(params.touchedVersion, releaseVersion);
