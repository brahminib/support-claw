...
describe("maybeRepairSandboxImages", () => {
  ...
  function createSandboxConfig(mode: "off" | "all" | "non-main"): SupportClawConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
  ...
  function createSandboxConfigWithDockerNetwork(network: string): SupportClawConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
...
describe("maybeRepairSandboxRegistryFiles", () => {
  ...
  it("warns about legacy registry files without migrating outside doctor --fix", async () => {
    inspectLegacySandboxRegistryFiles.mockResolvedValue([
      {
        kind: "containers",
        registryPath: "/tmp/supportClaw/sandbox/containers.json",
        shardedDir: "/tmp/supportClaw/sandbox/containers",
        source: "monolithic",
        exists: true,
        valid: true,
        entries: 2,
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "- containers monolithic: /tmp/supportClaw/sandbox/containers.json (2 entries)",
        "Run supportClaw doctor --fix to migrate them to SQLite.",
      ].join("\n"),
      "Sandbox",
    );
  });
  ...
  it("maps legacy registry files to structured findings and dry-run effects", () => {
    ...
    expect(legacySandboxRegistryInspectionToHealthFinding(monolithicFile)).toEqual(
      expect.objectContaining({
        ...
        severity: "warning",
        path: "/tmp/supportClaw/sandbox/containers.json",
        fixHint: expect.stringContaining("supportClaw doctor --fix"),
      }),
    );
    expect(legacySandboxRegistryInspectionToRepairEffect(monolithicFile)).toEqual({
      kind: "state",
      action: "would-migrate-legacy-sandbox-registry",
      target: "/tmp/supportClaw/sandbox/containers.json",
      dryRunSafe: false,
    });
    expect(legacySandboxRegistryInspectionToHealthFinding(shardedFile)).toEqual(
      expect.objectContaining({
        path: "/tmp/supportClaw/sandbox/containers",
        message: expect.stringContaining(
          "- containers sharded: /tmp/supportClaw/sandbox/containers (2 entries)",
        ),
      }),
    );
    expect(legacySandboxRegistryInspectionToRepairEffect(shardedFile)).toEqual(
      expect.objectContaining({
        target: "/tmp/supportClaw/sandbox/containers",
      }),
    );
  });
  ...
  it("maps invalid legacy registry files to quarantine effects", () => {
    expect(
      legacySandboxRegistryInspectionToRepairEffect({
        kind: "browsers",
        registryPath: "/tmp/supportClaw/sandbox/browsers.json",
        shardedDir: "/tmp/supportClaw/sandbox/browsers",
        source: "monolithic",
        exists: true,
        valid: false,
        entries: 0,
      ...
      expect.objectContaining({
        ...
        target: "/tmp/supportClaw/sandbox/browsers.json",
      }),
    );
  });
