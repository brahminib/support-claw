...
export async function loadValidConfigOrThrow(): Promise<SupportClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: SupportClawConfig, context: UpdateConfigContext) => SupportClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: SupportClawConfig }): {
  provider: string;
  model: string;
} {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("supportClaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
