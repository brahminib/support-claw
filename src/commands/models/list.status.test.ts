...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/supportClaw-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/supportClaw-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/supportClaw-agent"}/auth-profiles.json`,
    ),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/supportClaw-dev/supportClaw.json",
    }),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/supportClaw-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/supportClaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/supportClaw-dev/supportClaw.json");
    expect(payload.auth.storePath).toBe("/tmp/supportClaw-agent/auth-profiles.json");
  ...
  it("honors SUPPORT_CLAW_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync({ SUPPORT_CLAW_AGENT_DIR: "/tmp/supportClaw-isolated-agent" }, async () => {
      await modelsStatusCommand({ json: true }, localRuntime as never);
    });

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/supportClaw-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/supportClaw-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/supportClaw-isolated-agent/auth-profiles.json");
  });

  it("honors deprecated PI_CODING_AGENT_DIR when SUPPORT_CLAW_AGENT_DIR is unset", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync(
      {
        SUPPORT_CLAW_AGENT_DIR: undefined,
    ...
    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/supportClaw-legacy-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/supportClaw-legacy-agent");
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/supportClaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/supportClaw-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/supportClaw-agent-custom/auth-profiles.json",
        });
      },
    );
  });
  ...
  it("reports unresolved Codex OAuth sidecars as missing for OpenAI Codex runtime routes", async () => {
    ...
    mocks.store.profiles = {
      "openai-codex:default": {
        ...
        expires: Date.now() + 60_000,
        oauthRef: {
          source: "supportClaw-credentials",
          provider: "openai-codex",
  ...
  it("reports Gemini CLI OAuth for canonical Google text routed through the CLI runtime", async () => {
    ...
    try {
      ...
      expect(
        ...
        kind: "profiles",
        detail: "/tmp/supportClaw-agent/auth-profiles.json",
      });
      expect(payload.auth.runtimeAuthRoutes).toEqual([
        {
          provider: "google",
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/supportClaw-agent/workspace"
          ? {
