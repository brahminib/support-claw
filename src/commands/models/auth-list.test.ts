...
const mocks = vi.hoisted(() => ({
  ...
  resolveModelsTargetAgent: vi.fn((_cfg: SupportClawConfig, rawAgentId?: string) => {
    const agentId = rawAgentId ?? "main";
    return { agentDir: `/tmp/supportClaw/agents/${agentId}`, agentId };
  }),
}));

vi.mock("../../agents/agent-scope.js", () => ({
  resolveAgentDir: (_cfg: SupportClawConfig, agentId: string) => `/tmp/supportClaw/agents/${agentId}`,
...
vi.mock("../../agents/auth-profiles.js", () => ({
  ...
  resolveAuthStatePathForDisplay: (agentDir: string) => `${agentDir}/supportClaw-agent.sqlite`,
}));
...
describe("modelsAuthListCommand", () => {
  beforeEach(() => {
    mocks.loadModelsConfig.mockReset().mockResolvedValue({} as SupportClawConfig);
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "coder",
        authStatePath: "/tmp/supportClaw/agents/coder/supportClaw-agent.sqlite",
        profiles: [
          {
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "main",
        authStatePath: "/tmp/supportClaw/agents/main/supportClaw-agent.sqlite",
        profiles: [
          {
  ...
  it("prints an empty profile list without failing", async () => {
    ...
    expect(runtime.logs).toEqual([
      "Agent: main",
      "Auth state store: /tmp/supportClaw/agents/main/supportClaw-agent.sqlite",
      "Profiles: (none)",
    ]);
  });
