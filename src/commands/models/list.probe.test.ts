...
describe("runAuthProbes", () => {
  it("runs Codex auth probes through raw SupportClaw model-run mode", async () => {
    ...
    try {
      ...
      const result = await module.runAuthProbes({
        cfg: {} as never,
        agentId: "probe-agent",
        agentDir: "/tmp/supportClaw-probe-agent",
        workspaceDir: "/tmp/supportClaw-probe-workspace",
        providers: ["openai"],
