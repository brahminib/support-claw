...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:supportClaw security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:supportClaw security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:supportClaw status --all",
      "  Need to debug live? cmd:supportClaw logs --follow",
      "  Fix reachability first: cmd:supportClaw gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:supportClaw devices approve req-123)",
      "muted(Fallback: cmd:supportClaw devices approve --latest)",
      "muted(Inspect: cmd:supportClaw devices list)",
    ]);
  });
