// Builds the data model for the standard `supportClaw status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("supportClaw security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("supportClaw status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("supportClaw tasks maintenance --apply")}`,
