...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("supportClaw tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("supportClaw tasks flow cancel <flow-id>")}`,
    ]
...
function notePluginVersionDrift(drift: PluginVersionDriftReport | undefined) {
  ...
  const lines = [
    ...
    } not on SupportClaw ${drift.gatewayVersion}`,
    ...
    singleDrift
      ? `Fix: ${formatCliCommand(
          `supportClaw plugins update ${singleDrift.pluginId}`,
        )} && ${formatCliCommand("supportClaw gateway restart")}.`
      : `Fix: ${formatCliCommand(
          "supportClaw plugins update <plugin-id>",
        )} for each drifted plugin, then ${formatCliCommand("supportClaw gateway restart")}.`,
  ];
  note(lines.join("\n"), "Plugin version drift");
}

/** Emits workspace, skills, plugin, and TaskFlow recovery status notes for doctor. */
export function noteWorkspaceStatus(cfg: SupportClawConfig, options: NoteWorkspaceStatusOptions = {}) {
