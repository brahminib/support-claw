...
const originalEnv = captureEnv(["HOME", "SUPPORT_CLAW_STATE_DIR", "SHELL"]);
...
describe("shell completion health mapping", () => {
  it("checks an explicit shell instead of the detected environment shell", async () => {
    ...
    const current = await checkShellCompletionStatus("supportClaw", { shell: "fish" });

    expect(current.shell).toBe("fish");
    expect(current.cachePath).toBe(path.join(stateDir, "completions", "supportClaw.fish"));
