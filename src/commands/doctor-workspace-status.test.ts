...
async function runNoteWorkspaceStatusForTest(
  ...
  const cfg: SupportClawConfig = opts?.cfg ?? {};
...
describe("noteWorkspaceStatus", () => {
  ...
  it("surfaces active official managed plugin version drift", async () => {
    ...
    try {
      ...
      expect(body).toContain("1 active official plugin not on SupportClaw 2026.6.1");
      expect(body).toContain("codex: 2026.5.30-beta.1 (npm) -> expected 2026.6.1");
      expect(body).toContain("supportClaw plugins update codex");
      expect(body).toContain("supportClaw gateway restart");
    } finally {
      noteSpy.mockRestore();
    }
  });
  ...
  it("adds TaskFlow recovery hints for broken blocked flows", async () => {
    ...
    try {
      ...
      expect(body).toContain("supportClaw tasks flow show <flow-id>");
    } finally {
      noteSpy.mockRestore();
    }
  });
