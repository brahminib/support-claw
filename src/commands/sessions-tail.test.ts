...
describe("sessionsTailCommand", () => {
  ...
  beforeEach(() => {
    ...
    previousTrajectoryDir = process.env.SUPPORT_CLAW_TRAJECTORY_DIR;
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sessions-tail-"));
    process.env.SUPPORT_CLAW_STATE_DIR = path.join(tmpDir, "state");
    delete process.env.SUPPORT_CLAW_TRAJECTORY_DIR;
  ...
  afterEach(() => {
    ...
    if (previousTrajectoryDir === undefined) {
      ...
      process.env.SUPPORT_CLAW_TRAJECTORY_DIR = previousTrajectoryDir;
    }
  ...
  it("uses a session trajectory pointer for relocated runtime files", async () => {
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({
        traceSchema: "supportClaw-trajectory-pointer",
        schemaVersion: 1,
        sessionId: "session-one",
  ...
  it("resolves the target store from a fully qualified non-default agent session key", async () => {
    ...
    const opsSessionsDir = path.join(process.env.SUPPORT_CLAW_STATE_DIR!, "agents", "ops", "sessions");
