...
import type { SupportClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: SupportClawConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: SupportClawConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "SupportClaw default (full)";
  const defaultRequestedAskSource = "SupportClaw default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("supportClaw approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: SupportClawConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: SupportClawConfig): string[] {
...
function collectPlaintextConfigSecretWarnings(cfg: SupportClawConfig): string[] {
  ...
  return [
    "- WARNING: supportClaw.json contains plaintext secret-bearing config fields.",
    `  Paths: ${pathLine}`,
    "  Agents or workspace tools that can read config files may see these API keys/tokens.",
    `  Migrate them to SecretRefs with ${formatCliCommand("supportClaw secrets configure")} or ${formatCliCommand("supportClaw secrets apply")}, then verify with ${formatCliCommand("supportClaw secrets audit --check")}.`,
  ];
}
...
export async function collectSecurityWarnings(
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      `  Check local policy with: ${formatCliCommand("supportClaw approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.supportClaw.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("supportClaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("supportClaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("supportClaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("supportClaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('supportClaw config set session.dmScope "per-channel-peer"') +
...
export async function noteSecurityWarnings(cfg: SupportClawConfig) {
  const warnings = await collectSecurityWarnings(cfg);
  const auditHint = `- Run: ${formatCliCommand("supportClaw security audit --deep")}`;
