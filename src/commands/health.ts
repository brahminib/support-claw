...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.SUPPORT_CLAW_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: SupportClawConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: SupportClawConfig) => {
...
/** Runs the `supportClaw health` command against the gateway and renders JSON or text. */
export async function healthCommand(
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.SUPPORT_CLAW_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<SupportClawConfig> {
  const { readBestEffortConfig } = await loadConfigRuntime();
  return await readBestEffortConfig();
}

async function readRuntimeHealthConfig(): Promise<SupportClawConfig> {
