...
beforeAll(() => {
  envSnapshot = captureEnv(["SUPPORT_CLAW_PROFILE"]);
  process.env.SUPPORT_CLAW_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedBySupportClaw: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/supportClaw/pnpm-lock.yaml",
        markerPath: "/tmp/supportClaw/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.SUPPORT_CLAW_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.supportClaw.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.supportClaw.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.SUPPORT_CLAW_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/supportClaw-root.js", () => ({
  resolveSupportClawPackageRoot: vi.fn().mockResolvedValue("/tmp/supportClaw"),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expectLogsMatch(logs, /supportClaw (?:--profile isolated )?status --all/);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("supportClaw --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("SUPPORT_CLAW_GATEWAY_TOKEN", "abcd1234", async () => {
