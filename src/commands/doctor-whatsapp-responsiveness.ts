...
function isLocalTuiCommand(command: string): boolean {
  ...
  if (executable === "supportClaw-tui") {
    return true;
  }
  return executable === "supportClaw" && LOCAL_TUI_SUBCOMMANDS.has(argv[1] ?? "");
}
...
/** Lists local SupportClaw TUI processes that can contend with gateway responsiveness. */
...
function hasWhatsappEnabled(cfg: SupportClawConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("supportClaw doctor --fix")}.`,
      );
    }
  }
