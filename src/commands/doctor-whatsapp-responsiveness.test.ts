...
vi.mock("node:child_process", async () => {
  const { mockNodeChildProcessSpawnSync } = await import("supportClaw/plugin-sdk/test-node-mocks");
...
describe("doctor WhatsApp responsiveness", () => {
  ...
  it("lists only verified local TUI processes", () => {
    spawnSyncMock.mockReturnValue({
      ...
      stdout: [
        " 101 supportClaw-tui",
        " 102 /usr/bin/node /usr/lib/node_modules/supportClaw/dist/index.js gateway --port 18789",
        " 103 supportClaw channels",
        " 104 supportClaw tui --local",
        " 105 /usr/bin/supportClaw chat",
        " 106 helper --note 'supportClaw tui'",
        " 107 supportClaw-helper supportClaw terminal",
        " 108 supportClaw --flag tui",
      ].join("\n"),
    });

    expect(listLocalTuiProcesses()).toEqual([
      { pid: 101, command: "supportClaw-tui" },
      { pid: 104, command: "supportClaw tui --local" },
      { pid: 105, command: "/usr/bin/supportClaw chat" },
    ]);
  });
  ...
  it("terminates stale local TUI processes with a kill fallback", async () => {
    ...
    await expect(
      terminateLocalTuiProcesses({
        processes: [{ pid: 101, command: "supportClaw-tui" }],
        controller,
        graceMs: 0,
      }),
  ...
  it("warns and repairs local TUI pressure when WhatsApp is enabled and the gateway is degraded", async () => {
    ...
    const cfg = { channels: { whatsapp: { enabled: true } } } as SupportClawConfig;
    ...
    await noteWhatsappResponsivenessHealth({
      ...
      shouldRepair: true,
      listLocalTuiProcesses: () => [{ pid: 101, command: "supportClaw-tui" }],
