...
describe("noteSessionLockHealth", () => {
  let state: SupportClawTestState;

  beforeEach(async () => {
    note.mockClear();
    state = await createSupportClawTestState({
      layout: "state-only",
      prefix: "supportClaw-doctor-locks-",
    });
  });
  ...
  it("uses configured stale threshold without removing live SupportClaw lock files", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("removes fresh live locks when the owner is not an SupportClaw process", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
    ...
    expect(note).toHaveBeenCalledTimes(1);
    const [message] = firstNoteCall();
    expect(message).toContain("stale=yes (non-supportClaw-owner)");
    expect(message).toContain("[removed]");
    expect(message).toContain("Removed 1 stale session lock file");
