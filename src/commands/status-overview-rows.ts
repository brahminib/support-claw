// Builds overview table rows for `supportClaw status` and `supportClaw status --all`.
...
/** Builds the default `supportClaw status` overview rows from scan, health, memory, and session inputs. */
  ...
  return buildStatusOverviewRowsFromSurface({
    ...
    middleRows: [
      ...(params.updateRestartValue
        ? [{ Item: "Update restart", Value: params.updateRestartValue }]
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("supportClaw security audit --deep")}` },
    ],
