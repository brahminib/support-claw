...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentDir: (configForTest: unknown, agentId: string) =>
    `${process.env.SUPPORT_CLAW_STATE_DIR ?? "/tmp/supportClaw-state"}/agents/${agentId}/agent`,
  resolveAgentWorkspaceDir: (configForTest: unknown, agentId: string) =>
    `/tmp/supportClaw-workspaces/${agentId}`,
...
vi.mock("../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: () => "/tmp/supportClaw-workspace",
}));
...
function resolveTestAuthStoreKey(agentDir?: string): string {
  return agentDir?.trim() || process.env.SUPPORT_CLAW_AGENT_DIR || "__main__";
}
...
async function createApiKeyProvider(params: {
  ...
  noteMessage?: string;
  noteTitle?: string;
  applyConfig?: Partial<SupportClawConfig>;
    ...
    auth: [
      {
        ...
        run: async (ctx) => {
          ...
          return {
            ...
            ...(params.applyConfig ? { configPatch: params.applyConfig as SupportClawConfig } : {}),
  ...
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as SupportClawConfig,
...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    process.env.SUPPORT_CLAW_AGENT_DIR = agentDir;
  }
  ...
  async function readAuthProfiles() {
    return readTestAuthProfileStore(resolveAgentDir({} as SupportClawConfig, "main"));
  }
  ...
  function expectAuthProfileConfig(
    result: { config: SupportClawConfig },
    profileId: string,
  ...
  beforeAll(async () => {
    authTestRoot = (await setupAuthTestEnv("supportClaw-auth-")).stateDir;
  ...
  it("fails fast when a removed provider auth choice is passed to the interactive flow", async () => {
    ...
    try {
      await expect(
        ...
        'Auth choice "openai-chatgpt-import" is no longer supported. Use "openai" instead, or run supportClaw onboard to choose interactively.',
      );
    } finally {
      spy.mockRestore();
    }
  ...
  it("escapes removed provider auth choice guidance for terminal output", async () => {
    ...
    try {
      await expect(
        ...
        'Auth choice "legacy\\u001b[31mchoice" is no longer supported. Use "modern\\nchoice" instead, or run supportClaw onboard to choose interactively.',
      );
    } finally {
      spy.mockRestore();
    }
  ...
  it("guides external provider auth-choice replacements before the plugin is installed", async () => {
    ...
    try {
      await expect(
        ...
        'Auth choice "modelstudio-api-key" is no longer supported. Use "qwen-api-key" instead, or run supportClaw onboard to choose interactively.',
      );
    } finally {
