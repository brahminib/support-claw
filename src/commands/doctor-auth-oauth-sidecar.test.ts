...
const states: SupportClawTestState[] = [];
...
async function makeTestState(seed = "legacy-oauth-seed"): Promise<SupportClawTestState> {
  const state = await createSupportClawTestState({
    layout: "state-only",
    prefix: "supportClaw-doctor-oauth-sidecar-",
    env: {
      SUPPORT_CLAW_AGENT_DIR: undefined,
      SUPPORT_CLAW_AUTH_PROFILE_SECRET_KEY: seed,
    },
  });
  states.push(state);
  return state;
...
function encryptLegacySidecarMaterial(params: {
  ref: { source: "supportClaw-credentials"; provider: "openai-codex"; id: string };
  profileId: string;
  provider: string;
  seed: string;
...
describe("maybeRepairLegacyOAuthSidecarProfiles", () => {
  ...
  it("scans SUPPORT_CLAW_AGENT_DIR before treating sidecars as unreferenced", async () => {
    const state = await makeTestState();
    const previousAgentDir = process.env.SUPPORT_CLAW_AGENT_DIR;
    ...
    try {
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.SUPPORT_CLAW_AGENT_DIR = previousAgentDir;
      }
    }
  });
