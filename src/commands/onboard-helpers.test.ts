...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".supportClaw-work");
    const defaultStateDir = path.join(homeDir, ".supportClaw");
    const profileConfigPath = path.join(profileStateDir, "supportClaw.json");
      ...
      await withEnvAsync(
        {
          ...
          SUPPORT_CLAW_STATE_DIR: profileStateDir,
          SUPPORT_CLAW_CONFIG_PATH: profileConfigPath,
        },
...
describe("moveToTrash", () => {
  it("uses fs-safe trash instead of resolving a PATH trash command", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-trash-helper-"));
  ...
  it("allows fs-safe trash to move a symlink whose target resolves outside the parent", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-trash-symlink-"));
    const targetPath = path.join(testRoot, "target-link");
    const outsideTarget = path.join(os.tmpdir(), "supportClaw-trash-symlink-target");
  ...
  it("canonicalizes a symlinked parent before calling fs-safe trash", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-trash-parent-link-"));
    const lexicalParent = path.join(testRoot, "state-link");
    const realParent = path.join(testRoot, "state-real");
    const targetPath = path.join(lexicalParent, "supportClaw.json");
    const sourcePath = path.join(realParent, "supportClaw.json");
