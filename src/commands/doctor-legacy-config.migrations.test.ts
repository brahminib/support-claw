...
vi.mock("../plugins/setup-registry.js", () => ({
  ...
  runPluginSetupConfigMigrations: ({ config }: { config: SupportClawConfig }) => ({
    config,
    changes: [],
  }),
}));
...
vi.mock("./doctor/shared/channel-legacy-config-migrate.js", () => ({
  applyChannelDoctorCompatibilityMigrations: (cfg: SupportClawConfig) => ({
    next: cfg,
    changes: [],
  }),
}));
...
vi.mock("../secrets/target-registry.js", () => {
  ...
  return {
    discoverConfigSecretTargets: (cfg: SupportClawConfig) => {
      const targets: Array<{
        entry: typeof entry;
        path: string;
        pathSegments: string[];
...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  beforeAll(() => {
    previousOauthDir = process.env.SUPPORT_CLAW_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-oauth-"));
    process.env.SUPPORT_CLAW_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterAll(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.SUPPORT_CLAW_OAUTH_DIR = previousOauthDir;
    }
    fs.rmSync(tempOauthDir, { recursive: true, force: true });
  });
        ...
        list: [
          {
            ...
            models: {
              "anthropic/claude-opus-4-7": { agentRuntime: { id: "supportClaw" } },
            },
          },
        ],
      },
  ...
  it("does not report talk provider normalization for realtime voice aliases", () => {
    ...
    const res = normalizeCompatibilityConfigValues(input as SupportClawConfig);
