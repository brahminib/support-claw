...
function isTempBackedSupportClawRoot(segments: readonly string[]): boolean {
  const lower = segments.map((segment) => segment.toLowerCase());
  const supportClawIndex = lower.lastIndexOf("supportClaw");
  if (supportClawIndex < 1) {
    return false;
  }
  return lower[supportClawIndex - 1] === "tmp" || lower[supportClawIndex - 1] === "temp";
}
...
function isBundledRuntimeSkillsPath(cachedPath: string, skillRootIndex: number): boolean {
  ...
  return (
    lower.some(
      (segment) =>
        segment === "dist-runtime" || segment === "node_modules" || segment.startsWith("supportClaw@"),
    ) || isTempBackedSupportClawRoot(beforeSkillRoot)
  );
}
