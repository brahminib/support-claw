...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.SUPPORT_CLAW_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.SUPPORT_CLAW_GATEWAY_PASSWORD ?? configuredPassword;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.supportClaw.ai/gateway/health",
        "https://docs.supportClaw.ai/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from supportClaw.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<SupportClawConfig["tools"]>["web"]>["search"];
  ...
  note(
    [
      ...
      "Docs: https://docs.supportClaw.ai/tools/web",
    ].join("\n"),
    "Web search",
  );
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "SupportClaw update wizard" : "SupportClaw configure");
    ...
    const baseConfig: SupportClawConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.supportClaw.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("supportClaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const promptGatewayRunMode = async (): Promise<OnboardMode> => {
      ...
      const localProbePromise = (async () => {
        ...
        return probeGatewayReachable({
          url: localUrl,
          token: process.env.SUPPORT_CLAW_GATEWAY_TOKEN ?? baseLocalProbeToken,
          password: process.env.SUPPORT_CLAW_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    ...
    if (shouldSkipGatewaySummary) {
      ...
      if (remoteUrl) {
        note(
          ["Remote Gateway:", remoteUrl, "Docs: https://docs.supportClaw.ai/gateway/remote"].join(
            "\n",
          ),
          "Gateway",
        );
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.supportClaw.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
