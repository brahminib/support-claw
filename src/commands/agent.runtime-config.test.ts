...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: SupportClawConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => SupportClawConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: SupportClawConfig, sourceConfig: SupportClawConfig) => void>(),
);
...
const resolveCommandConfigWithSecretsMock = vi.hoisted(() =>
  vi.fn<
    (params: ResolveCommandConfigParams) => Promise<{
      resolvedConfig: SupportClawConfig;
      effectiveConfig: SupportClawConfig;
      diagnostics: never[];
    }>
  >(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "supportClaw-agent-" });
}
...
function mockConfig(home: string, storePath: string): SupportClawConfig {
  const cfg = {
    agents: {
      defaults: {
