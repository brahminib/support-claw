...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    promptRemoteGatewayConfig: vi.fn(async (cfg: SupportClawConfig) => ({
      ...cfg,
      gateway: { mode: "remote", remote: { url: "wss://gateway.example.test" } },
    })),
    isCodexNativeWebSearchRelevant: vi.fn(({ config }: { config: SupportClawConfig }) =>
      Boolean(config.auth?.profiles?.["openai:default"]),
    ),
    setupChannels: vi.fn(async (cfg: SupportClawConfig) => cfg),
  };
});
...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.supportClaw/supportClaw.json",
  createConfigIO: () => ({
    readConfigFileSnapshotForWrite: async () => ({
      snapshot: await mocks.readConfigFileSnapshot(),
      writeOptions: {
        assertConfigPathForWrite: mocks.assertConfigPathForWrite,
        expectedConfigPath: "/tmp/supportClaw.json",
        ownedConfigPathForWrite: "/tmp/supportClaw.json",
      },
    }),
  }),
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.supportClaw/workspace",
  applyWizardMetadata: (cfg: SupportClawConfig) => cfg,
...
function setupBaseWizardState(config: SupportClawConfig = {}) {
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: SupportClawConfig) => cfg);
    mocks.promptAuthConfig.mockReset();
    mocks.promptAuthConfig.mockImplementation(async (cfg: SupportClawConfig) => cfg);
    mocks.promptGatewayConfig.mockReset();
    mocks.promptGatewayConfig.mockImplementation(async (cfg: SupportClawConfig) => ({
      config: cfg,
      port: 18789,
    }));
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: SupportClawConfig) => {
  ...
  it("still supports keyless web search providers through the shared setup flow", async () => {
    ...
    mocks.resolveSearchProviderOptions.mockReturnValue([
      createSearchProviderOption({
        ...
        docsUrl: "https://docs.supportClaw.ai/tools/web",
        credentialPath: "",
      }),
    ]);
    mocks.setupSearch.mockImplementation(async (cfg: SupportClawConfig) =>
  ...
  it("retries without dropping nested plugin config written during wizard flow (issue #64188)", async () => {
    ...
    expect(String(defaults.workspace)).toContain("/.supportClaw/workspace");
