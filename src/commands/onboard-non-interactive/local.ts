...
export async function runNonInteractiveLocalSetup(params: {
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `supportClaw gateway run` or `supportClaw agent --local`.",
              ]
            : [`Run \`${formatCliCommand("supportClaw gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
    ...
    if (!probe.ok) {
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              "Non-interactive local setup only waits for an already-running gateway unless you pass `--install-daemon` to `supportClaw onboard`.",
              `Fix: start \`${formatCliCommand("supportClaw gateway run")}\`, re-run \`${formatCliCommand("supportClaw onboard --install-daemon")}\`, or use \`${formatCliCommand("supportClaw onboard --skip-health")}\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("supportClaw configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.supportClaw.ai/tools/web`,
    );
  }
}
