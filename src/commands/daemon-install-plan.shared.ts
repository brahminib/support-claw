...
function isSupportClawCommandBasename(basename: string, platform: NodeJS.Platform): boolean {
  if (basename === "supportClaw") {
    return true;
  }
  if (platform === "win32") {
    return (
      basename === "supportClaw.cmd" || basename === "supportClaw.ps1" || basename === "supportClaw.exe"
    );
  }
  return false;
}
...
/** Resolve the SupportClaw CLI binary directory from argv/PATH for daemon PATH. */
export function resolveDaemonSupportClawBinDir(
  params: {
    argv?: string[];
    env?: Record<string, string | undefined>;
  ...
  if (
    ...
    isSupportClawCommandBasename(path.basename(argv1), platform)
  ) {
  ...
  for (const rawSegment of (env.PATH ?? "").split(path.delimiter)) {
    ...
    const candidate = path.join(segment, platform === "win32" ? "supportClaw.cmd" : "supportClaw");
...
/** Merge Node and SupportClaw binary directories for the daemon service PATH. */
export function resolveDaemonServicePathDirs(params: {
  ...
  for (const dir of resolveDaemonSupportClawBinDir(params) ?? []) {
    addUniquePathDir(dirs, dir);
  }
  return dirs.length > 0 ? dirs : undefined;
}
