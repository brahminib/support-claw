...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "SupportClaw", ".supportClaw");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/supportClaw-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".supportClaw");
  ...
  it("anchors cloud detection to OS homedir when SUPPORT_CLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".supportClaw");
    const originalSupportClawHome = process.env.SUPPORT_CLAW_HOME;
    process.env.SUPPORT_CLAW_HOME = "/tmp/supportClaw-home-override";
