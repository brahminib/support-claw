 ...
 *
 *   - RED: ACP-keyed session resolves to `id: "supportClaw"`, `source: "implicit"`.
...
/**
 * Build a minimal `SupportClawConfig` that mirrors the deployed scenario:
 ...
 *
 * Result: the old metadata resolver fell through to the implicit "supportClaw"
 * branch — which is the bug under test.
 */
function buildConfigWithoutAgentRuntimePolicy(): SupportClawConfig {
  return {
    agents: {
      list: [
        {
...
describe("sessions --json agentRuntime classifier (catalog #18)", () => {
  it("RED→GREEN: ACP session key is no longer misclassified (overlay applies)", () => {
    ...
    // resolved metadata said id="supportClaw", source="implicit".
    ...
    expect(
      agentRuntime.id,
      `ACP session ${ACP_SESSION_KEY} should no longer be misclassified as "auto" or "supportClaw". ` +
  ...
  it("FIX-SHAPE expectation: ACP session should resolve to 'acpx'", () => {
    ...
    // from "supportClaw" and reflects the actual runtime driving the session.
