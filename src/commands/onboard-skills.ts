...
export async function setupSkills(
  ...
  let next: SupportClawConfig = cfg;
  if (installable.length === 0 && missing.length === 0) {
    await prompter.note(
      [
        "No missing skill dependencies to install.",
        `To inspect available skills, run: ${formatCliCommand("supportClaw skills list --verbose")}`,
        `To check skill status, run: ${formatCliCommand("supportClaw skills check")}`,
      ].join("\n"),
  ...
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      // visible through `supportClaw skills list --verbose`.
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("supportClaw doctor")}\` to review skills + requirements.`,
      );
