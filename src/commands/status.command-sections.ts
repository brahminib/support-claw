...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("supportClaw security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("supportClaw security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusModelSelectionLines(params: {
  ...
  for (const sess of mismatches.slice(0, limit)) {
    ...
    lines.push(
      ...
      "  Docs: https://docs.supportClaw.ai/concepts/models#selection-source-and-fallback-behavior",
    );
  }
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.supportClaw.ai/faq",
    "Troubleshooting: https://docs.supportClaw.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("supportClaw status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("supportClaw logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("supportClaw node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("supportClaw status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("supportClaw gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`supportClaw devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("supportClaw devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("supportClaw devices list")}`),
  ];
}
