// Implements guided and non-interactive `supportClaw channels add` account setup.
...
async function resolveCatalogChannelEntry(raw: string, cfg: SupportClawConfig | null) {
...
async function channelsAddCommandImpl(
  ...
  const cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as SupportClawConfig;
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run supportClaw doctor --fix, then retry supportClaw channels add.`
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("supportClaw channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }
