...
describe("describeHeartbeatSessionTargetIssues", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-heartbeat-doctor-"));
  });
  ...
  function cfgWithSession(session: string, target: string | null = "slack"): SupportClawConfig {
  ...
  function writeStore(cfg: SupportClawConfig, entries: Record<string, unknown>) {
