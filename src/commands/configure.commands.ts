 ...
 * Mirrors the real `supportClaw config <sub>` surface so the message only ever
...
const CONFIGURE_NON_TTY_HINT = [
  ...
  `  ${formatCliCommand("supportClaw config set <path> <value>")}  write a config entry`,
  `  ${formatCliCommand("supportClaw config get <path>")}          read a config entry`,
  `  ${formatCliCommand("supportClaw config patch")}              apply a JSON patch`,
  `  ${formatCliCommand("supportClaw config validate")}           validate configuration`,
].join("\n");
...
export async function configureCommandFromSectionsArg(
  ...
  // Fail closed once at the shared entry: both `supportClaw configure` and the
  // no-subcommand `supportClaw config` route here, so a single guard keeps them
  ...
  if (invalid.length > 0) {
    runtime.error(
      `Invalid --section: ${invalid.join(", ")}. Expected one of: ${CONFIGURE_WIZARD_SECTIONS.join(", ")}. Run ${formatCliCommand("supportClaw configure")} without --section to use the full wizard.`,
    );
    runtime.exit(1);
    return;
  }
