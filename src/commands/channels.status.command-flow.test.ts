...
const mocks = vi.hoisted(() => ({
  ...
  readConfigFileSnapshot: vi.fn(async () => ({ path: "/tmp/supportClaw.json" })),
  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          channelId,
          label: "Feishu",
          installSpec: "@supportclaw/feishu",
          installCommand: "supportClaw plugins install @supportclaw/feishu",
          doctorFixCommand: "supportClaw doctor --fix",
          repairHint:
            "Install the official external plugin with: supportClaw plugins install @supportclaw/feishu, or run: supportClaw doctor --fix.",
        }
      : null,
}));
...
describe("channelsStatusCommand SecretRef fallback flow", () => {
  ...
  it("shows missing official external plugin repair hints in config-only output", async () => {
    ...
    expect(joined).toContain(
      "Feishu: Install the official external plugin with: supportClaw plugins install @supportclaw/feishu, or run: supportClaw doctor --fix.",
    );
  });
    ...
    mocks.callGateway.mockRejectedValue(
      new Error(
        [
          ...
          "Source: env SUPPORT_CLAW_GATEWAY_URL",
        ].join("\n"),
      ),
    );
    mocks.requireValidConfigSnapshot.mockResolvedValue({ secretResolved: false, channels: {} });
