...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.supportClaw.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.supportClaw.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        SUPPORT_CLAW_STATE_DIR: "/tmp/supportClaw-service",
        SUPPORT_CLAW_CONFIG_PATH: "/tmp/supportClaw-service/supportClaw.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { SUPPORT_CLAW_STATE_DIR?: string; SUPPORT_CLAW_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.SUPPORT_CLAW_STATE_DIR).toBe("/tmp/supportClaw-service");
    expect(handoffEnv?.SUPPORT_CLAW_CONFIG_PATH).toBe("/tmp/supportClaw-service/supportClaw.json");
    expect(note).toHaveBeenCalledWith(
      ...
      connections: [
        {
          pid: 4242,
          command: "node",
          commandLine: "/tmp/newer-supportClaw/bin/supportClaw logs --follow",
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "supportClaw-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "supportClaw-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "supportClaw-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("supportClaw gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system SupportClaw gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "supportClaw-gateway.service",
        detail: "unit: /etc/systemd/system/supportClaw-gateway.service",
        scope: "system",
        marker: "supportClaw",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        "System-level SupportClaw gateway service detected while the user gateway service is not installed.",
        "- supportClaw-gateway.service (unit: /etc/systemd/system/supportClaw-gateway.service)",
        "SupportClaw will not install a second user-level gateway service automatically.",
        "Run `supportClaw gateway status --deep` or `supportClaw doctor --deep` to inspect duplicate services.",
