...
function setupSessionState(cfg: SupportClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("SUPPORT_CLAW_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: SupportClawConfig) {
...
async function runStateIntegrityText(cfg: SupportClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-doctor-state-integrity-"));
    const stateDir = path.join(tempHome, ".supportClaw");
    setTestEnvValue("HOME", tempHome);
    setTestEnvValue("SUPPORT_CLAW_HOME", tempHome);
    setTestEnvValue("SUPPORT_CLAW_STATE_DIR", stateDir);
    deleteTestEnvValue("SUPPORT_CLAW_OAUTH_DIR");
  ...
  it("prompts for oauth dir when SUPPORT_CLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.SUPPORT_CLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: SupportClawConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(hasRepairPromptMessage(confirmRuntimeRepair, "Create OAuth dir at")).toBe(true);
  ...
  it("does not warn when SUPPORT_CLAW_AGENT_DIR points at the live compatibility agent dir", async () => {
    ...
    setTestEnvValue("SUPPORT_CLAW_AGENT_DIR", legacyAgentDir);
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("supportClaw tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.SUPPORT_CLAW_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        const symlinkStateDir = path.join(symlinkHome, ".supportClaw");
        setTestEnvValue("HOME", symlinkHome);
        setTestEnvValue("SUPPORT_CLAW_HOME", symlinkHome);
        setTestEnvValue("SUPPORT_CLAW_STATE_DIR", symlinkStateDir);
  ...
  it("prints supportClaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: SupportClawConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/supportClaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/supportClaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /supportClaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-tui-pointer-clear-"));
    try {
