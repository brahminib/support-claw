...
const resolveSupportClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadSupportClawPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/supportClaw.json",
    createConfigIO,
...
vi.mock("supportClaw/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("supportClaw/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("supportClaw/plugin-sdk/provider-stream-shared", () => ({
...
vi.mock("../infra/supportClaw-root.js", () => ({
  ...
  resolveSupportClawPackageRootSync: vi.fn(() => "/tmp/supportClaw"),
}));
...
beforeEach(() => {
  ...
  resolveSupportClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SUPPORT_CLAW_STATE_DIR;
  originalUpdateInProgress = process.env.SUPPORT_CLAW_UPDATE_IN_PROGRESS;
  process.env.SUPPORT_CLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-doctor-state-"));
