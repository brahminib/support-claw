...
let replyPayloadModulePromise:
  | Promise<typeof import("supportClaw/plugin-sdk/reply-payload")>
  | undefined;
...
async function loadRuntimeConfig(): Promise<SupportClawConfig> {
...
function loadReplyPayloadModule() {
  replyPayloadModulePromise ??= import("supportClaw/plugin-sdk/reply-payload");
...
function missingAgentMessageError(): Error {
  return new Error(
    `Missing message. Use ${formatCliCommand('supportClaw agent --message "..." --agent <id>')} or ${formatCliCommand("supportClaw agent --message-file <path> --agent <id>")}.`,
  );
}
...
function parseTimeoutSeconds(opts: { cfg: SupportClawConfig; timeout?: string }) {
  const raw =
...
async function agentViaGatewayCommand(
  ...
  if (!opts.to && !opts.sessionId && !opts.agent && !explicitSessionKey) {
    throw new Error(
      `No target session selected. Use --agent <id>, --session-key <key>, --session-id <id>, or --to <E.164>. Run ${formatCliCommand("supportClaw agents list")} to see agents.`,
    );
  }
  ...
  if (agentId) {
    ...
    if (!knownAgents.includes(agentId)) {
      throw new Error(
        `Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("supportClaw agents list")}" to see configured agents.`,
      );
    }
  }
  ...
  const dispatchGatewayAgentCall = async (activeCfg: SupportClawConfig) =>
    await withProgress(
      {
...
export async function agentCliCommand(
  ...
  if (isCompactControlCommand(messageOpts.message)) {
    runtime.error?.(
      "Slash commands cannot be executed via --message from the CLI. Use: supportClaw sessions compact <key>",
    );
    runtime.exit(1);
    return undefined;
  }
