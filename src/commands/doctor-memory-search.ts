...
import {
  ...
  normalizeProviderId,
} from "@supportclaw/model-catalog-core/provider-id";
...
function isOpenAICompatibleMemoryProvider(providerId: string, cfg: SupportClawConfig): boolean {
...
function isKeyOptionalMemoryProvider(providerId: string, cfg: SupportClawConfig): boolean {
  return (
    providerId === "local" ||
    providerId === "ollama" ||
    providerId === "lmstudio" ||
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("supportClaw doctor --fix")} or ${formatCliCommand("supportClaw memory status --fix")}`
    : `Verify: ${formatCliCommand("supportClaw memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: SupportClawConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: SupportClawConfig): boolean {
 ...
 * Runs as part of `supportClaw doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
      ...
      note(
        [
          ...
          workspaceProbeFailed
            ...
            : `- Set an explicit binary path: ${formatCliCommand("supportClaw config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("supportClaw config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("supportClaw memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (provider === "local") {
    ...
    note(
      [
        ...
        "",
        "Fix (pick one):",
        `- Install the llama.cpp provider plugin: ${formatCliCommand("supportClaw plugins install @supportclaw/llama-cpp-provider")}`,
        `- Set a local GGUF model path in config`,
        suggestedRemoteProvider
          ? `- Switch to a remote provider: ${formatCliCommand(`supportClaw config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
  ...
  if (
    ...
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("supportClaw config set agents.defaults.memorySearch.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("supportClaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("supportClaw config set agents.defaults.memorySearch.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("supportClaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `supportClaw doctor` without
  ...
  note(
    [
      ...
      `- Configure credentials: ${formatCliCommand("supportClaw configure --section model")}`,
      `- To disable: ${formatCliCommand("supportClaw config set agents.defaults.memorySearch.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("supportClaw memory status --deep")}`,
    ].join("\n"),
    "Memory search",
