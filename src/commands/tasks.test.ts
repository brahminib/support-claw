...
import type { SupportClawTestState } from "../test-utils/supportClaw-test-state.js";
import {
  tasksAuditCommand,
  tasksCancelCommand,
...
async function withTaskCommandStateDir(
  run: (state: SupportClawTestState) => Promise<void>,
): Promise<void> {
  await withSupportClawTestState(
    { layout: "state-only", prefix: "supportClaw-tasks-command-" },
    async (state) => {
