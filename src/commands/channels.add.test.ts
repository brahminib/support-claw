...
const channelWizardMocks = vi.hoisted(() => {
  ...
  return {
    prompter,
    setupChannels: vi.fn(async (...args: unknown[]) => args[0] as SupportClawConfig),
  };
});
    ...
    await channelsAddCommand(
      {
        channel: "whatsapp",
        account: "work",
        authDir: "/tmp/supportClaw-wa-auth",
      },
      runtime,
      { hasFlags: true },
    );
