...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/supportClaw-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = SupportClawConfig>(
  overrides: T = {} as T,
): SupportClawConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): SupportClawConfig {
