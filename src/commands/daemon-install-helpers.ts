...
import {
  SUPPORT_CLAW_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveSupportClawWrapperPath,
...
// they share the SUPPORT_CLAW_ prefix that is otherwise stripped from preserved
...
const PRESERVED_SUPPORT_CLAW_OPERATOR_OPT_IN_ENV_KEYS = new Set([
  "SUPPORT_CLAW_CLI_CONTAINER_BYPASS",
  "SUPPORT_CLAW_CONTAINER_HINT",
]);
...
export function collectPreservedExistingServiceEnvVars(
  ...
  for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
    ...
    if (
      ...
      upper === "PATH" ||
      upper === "TMPDIR" ||
      (upper.startsWith("SUPPORT_CLAW_") && !PRESERVED_SUPPORT_CLAW_OPERATOR_OPT_IN_ENV_KEYS.has(upper))
    ) {
      continue;
    }
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperInput = params.wrapperPath ?? params.env[SUPPORT_CLAW_WRAPPER_ENV_KEY];
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${SUPPORT_CLAW_WRAPPER_ENV_KEY} because it points to the Windows task script; using the SupportClaw gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
  const wrapperPath = wrapperPointsAtWindowsTaskScript
    ? undefined
    : await resolveSupportClawWrapperPath(wrapperInput);
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [SUPPORT_CLAW_WRAPPER_ENV_KEY]: wrapperPath }
    : wrapperPointsAtWindowsTaskScript
      ? omitEnvKey(params.env, SUPPORT_CLAW_WRAPPER_ENV_KEY)
      : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.SUPPORT_CLAW_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("supportClaw gateway install")}\` after fixing the error.`;
}
