...
describe("setupSkills", () => {
  ...
  it("hides brew-only installs in Linux containers when brew is missing", async () => {
    await withPlatform("linux", async () => {
      ...
      await setupSkills({} as SupportClawConfig, "/tmp/ws", runtime, prompter);
  ...
  it("displays a clear empty state note when all skill dependencies are ready", async () => {
    ...
    expect(emptyStateNote?.message).toContain("supportClaw skills list --verbose");
    expect(emptyStateNote?.message).toContain("supportClaw skills check");
  });
