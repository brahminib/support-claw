...
const mocks = vi.hoisted(() => ({
  ...
  resolveSupportClawWrapperPath: vi.fn(),
...
vi.mock("../daemon/program-args.js", () => ({
  SUPPORT_CLAW_WRAPPER_ENV_KEY: "SUPPORT_CLAW_WRAPPER",
  resolveGatewayProgramArguments: mocks.resolveGatewayProgramArguments,
  resolveSupportClawWrapperPath: mocks.resolveSupportClawWrapperPath,
}));
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { SUPPORT_CLAW_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveSupportClawWrapperPath.mockImplementation(async (value: string | undefined) =>
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.supportClaw/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ SUPPORT_CLAW_PORT: "3000" });
  ...
  it("adds the active supportClaw command bin directory to the managed service PATH", async () => {
    mockNodeGatewayPlanFixture();
    const originalArgv = process.argv;
    const supportClawBinPath = path.join(isolatedHome, ".npm-global", "bin", "supportClaw");
    process.argv = ["node", supportClawBinPath, "gateway", "install"];
    ...
    ).toStrictEqual(["/opt/homebrew/opt/node/bin", path.dirname(supportClawBinPath)]);
  });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".supportClaw"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes SUPPORT_CLAW_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/supportClaw-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        SUPPORT_CLAW_PORT: "3000",
        SUPPORT_CLAW_WRAPPER: wrapperPath,
      },
    });
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.SUPPORT_CLAW_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.SUPPORT_CLAW_WRAPPER).toBe(wrapperPath);
  });

  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    const selfWrapperPath = path.join(isolatedHome, ".supportClaw", "gateway.cmd");
    const warn = vi.fn();
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        SUPPORT_CLAW_PORT: "3000",
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        SUPPORT_CLAW_WRAPPER: selfWrapperPath,
      }),
      port: 3000,
      runtime: "node",
      platform: "win32",
    ...
    expect(plan.environment.SUPPORT_CLAW_WRAPPER).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Ignoring SUPPORT_CLAW_WRAPPER because it points to the Windows task script",
      ),
    );
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.SUPPORT_CLAW_PORT).toBe("3000");
    expect(plan.environment.SUPPORT_CLAW_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("includes env SecretRef values from config into the service environment", async () => {
    ...
    expect(plan.environment.SUPPORT_CLAW_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("allows safe inherited passEnv names while blocking dangerous exec SecretRef env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BASH_ENV: "/tmp/supportClaw-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/supportClaw-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        SUPPORT_CLAW_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
        gateway: {
          auth: {
            token: { source: "env", provider: "default", id: "SUPPORT_CLAW_GATEWAY_TOKEN" },
          },
        },
      },
    });

    expect(plan.environment.SUPPORT_CLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.SUPPORT_CLAW_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".supportClaw"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        SUPPORT_CLAW_PORT: "3000",
    ...
    expect(plan.environment.SUPPORT_CLAW_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("tracks .env vars with config while preserving service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".supportClaw"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.SUPPORT_CLAW_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("works when .env file does not exist", async () => {
    mockNodeGatewayPlanFixture({ serviceEnvironment: { SUPPORT_CLAW_PORT: "3000" } });
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.SUPPORT_CLAW_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        SUPPORT_CLAW_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.SUPPORT_CLAW_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.SUPPORT_CLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.SUPPORT_CLAW_GATEWAY_TOKEN).toBeUndefined();
  });
  ...
  it("does not embed auth-profile env refs when the key is already durable", async () => {
    ...
    expect(plan.environment.SUPPORT_CLAW_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
});
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:supportClaw|supportClaw)( --profile isolated)? gateway install/,
    );
  });
});
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("continues to drop stale SUPPORT_CLAW_ALLOW_ROOT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      { SUPPORT_CLAW_ALLOW_ROOT: "1" },
      managedKeys,
    );
    expect(result.SUPPORT_CLAW_ALLOW_ROOT).toBeUndefined();
  });

  it("preserves SUPPORT_CLAW_CLI_CONTAINER_BYPASS and SUPPORT_CLAW_CONTAINER_HINT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        SUPPORT_CLAW_CLI_CONTAINER_BYPASS: "1",
        SUPPORT_CLAW_CONTAINER_HINT: "ci",
      },
      managedKeys,
    );
    expect(result.SUPPORT_CLAW_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.SUPPORT_CLAW_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary SUPPORT_CLAW_FOO", () => {
    const result = collectPreservedExistingServiceEnvVars({ SUPPORT_CLAW_FOO: "bar" }, managedKeys);
    expect(result.SUPPORT_CLAW_FOO).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated SUPPORT_CLAW_* keys", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        SUPPORT_CLAW_CLI_CONTAINER_BYPASS: "1",
        SUPPORT_CLAW_CONTAINER_HINT: "ci",
        SUPPORT_CLAW_BAZ: "qux",
      },
      managedKeys,
    );
    expect(result.SUPPORT_CLAW_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.SUPPORT_CLAW_CONTAINER_HINT).toBe("ci");
    expect(result.SUPPORT_CLAW_BAZ).toBeUndefined();
  });
});
