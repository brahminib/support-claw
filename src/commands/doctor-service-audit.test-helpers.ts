...
export function readEmbeddedGatewayTokenForTest(
  ...
  return isEnvironmentFileOnlySource(command?.environmentValueSources?.SUPPORT_CLAW_GATEWAY_TOKEN)
    ? undefined
    : normalizeOptionalString(command?.environment?.SUPPORT_CLAW_GATEWAY_TOKEN);
}
