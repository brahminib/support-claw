...
import { resolvePrimaryStringValue } from "@supportclaw/normalization-core/string-coerce";
...
function isUpdateInProgress(): boolean {
  const value = process.env.SUPPORT_CLAW_UPDATE_IN_PROGRESS;
 ...
 */
export function stripUnknownConfigKeys(config: SupportClawConfig): {
  ...
  const parsed = SupportClawSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }
...
export function noteOpencodeProviderOverrides(cfg: SupportClawConfig): void {
...
export function collectImplicitFallbackClobberWarnings(cfg: SupportClawConfig): string[] {
...
export function noteImplicitFallbackClobberWarnings(cfg: SupportClawConfig): void {
