...
vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.SUPPORT_CLAW_STATE_DIR ?? "/tmp/supportClaw-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  const upsert = (params: { profileId: string; credential: unknown; agentDir?: string }) => {
    const stateDir = process.env.SUPPORT_CLAW_STATE_DIR ?? "/tmp/supportClaw-state";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under the default agent dir", async () => {
    const env = await setupAuthTestEnv("supportClaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-oauth-sync-"));
    ...
    setTestEnvValue("SUPPORT_CLAW_AGENT_DIR", kidAgentDir);
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-oauth-nosync-"));
    lifecycle.setStateDir(tempStateDir);
    setTestEnvValue("SUPPORT_CLAW_STATE_DIR", tempStateDir);
  ...
  it("syncs siblings from explicit agentDir outside SUPPORT_CLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-oauth-external-"));
    lifecycle.setStateDir(tempStateDir);
    setTestEnvValue("SUPPORT_CLAW_STATE_DIR", tempStateDir);

    // Create standard-layout agents tree *outside* SUPPORT_CLAW_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("supportClaw-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("supportClaw-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
...
describe("upsertApiKeyProfile", () => {
  const lifecycle = createAuthTestLifecycle(["SUPPORT_CLAW_STATE_DIR", "SUPPORT_CLAW_AGENT_DIR"]);
  ...
  it("writes to the default agent dir", async () => {
    const env = await setupAuthTestEnv("supportClaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
    const defaultAgentDir = path.join(env.stateDir, "agents", "main", "agent");
