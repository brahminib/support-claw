...
describe("onboard-hooks", () => {
  ...
  const createMockHook = (
    ...
    ...params,
    source: "supportClaw-bundled" as const,
    pluginId: undefined,
    homepage: undefined,
    always: false,
  ...
  const createMockHookReport = (eligible = true): HookStatusReport => ({
    ...
    managedHooksDir: "/mock/.supportClaw/hooks",
    hooks: [
      createMockHook(
        {
          name: "session-memory",
  ...
  describe("setupInternalHooks", () => {
    ...
    it("localizes built-in hook prompts when SUPPORT_CLAW_LOCALE is set", async () => {
      ...
      expect(noteCalls).toEqual([
        [
          [
            ...
            "",
            "Learn more: https://docs.supportClaw.ai/automation/hooks",
          ].join("\n"),
          "Hooks",
        ],
        [
