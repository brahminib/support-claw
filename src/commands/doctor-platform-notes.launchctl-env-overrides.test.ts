...
import {
  ...
  collectMacStaleSupportClawUpdateLaunchdJobsWarning,
  noteMacLaunchctlGatewayEnvOverrides,
  noteMacStaleSupportClawUpdateLaunchdJobs,
...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("collects clear unsetenv instructions for token override", async () => {
    const getenv = vi.fn(async (name: string) =>
      name === "SUPPORT_CLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(warning).toContain("SUPPORT_CLAW_GATEWAY_TOKEN");
    expect(warning).toContain("launchctl unsetenv SUPPORT_CLAW_GATEWAY_TOKEN");
    expect(warning).not.toContain("SUPPORT_CLAW_GATEWAY_PASSWORD");
  });
  ...
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    expect(message).toContain("SUPPORT_CLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SUPPORT_CLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("SUPPORT_CLAW_GATEWAY_PASSWORD");
  });
  ...
  it("treats SecretRef-backed credentials as configured", async () => {
    const noteFn = vi.fn();
    const getenv = vi.fn(async (name: string) =>
      name === "SUPPORT_CLAW_GATEWAY_PASSWORD" ? "launchctl-password" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SUPPORT_CLAW_GATEWAY_PASSWORD");
  });
...
describe("noteMacStaleSupportClawUpdateLaunchdJobs", () => {
  it("collects stale updater job cleanup guidance on macOS", async () => {
    ...
    const warning = await collectMacStaleSupportClawUpdateLaunchdJobsWarning({
      platform: "darwin",
      findJobs,
      env,
    });
    ...
    expect(warning).toContain("Stale SupportClaw updater launchd job(s) detected");
    expect(warning).toContain("ai.supportClaw.update.2026.5.12");
    expect(warning).toContain("ai.supportClaw.manual-update.1717168800");
    expect(warning).toContain("launchctl remove <label>");
    expect(warning).toContain("supportClaw gateway restart");
  });
  ...
  it("uses service env for gateway platform stale updater warnings", async () => {
    ...
    await collectMacGatewayPlatformWarnings({} as SupportClawConfig, {
      platform: "darwin",
      service,
      findJobs,
    });
  ...
  it("uses service env for doctor stale updater notes", async () => {
    ...
    await noteMacStaleSupportClawUpdateLaunchdJobs({
      platform: "darwin",
      service,
      findJobs,
    });
  ...
  it("prints stale updater job cleanup guidance on macOS", async () => {
    ...
    const [message, title] = requireNoteCall(noteFn);
    expect(title).toBe("Gateway (macOS)");
    expect(message).toContain("Stale SupportClaw updater launchd job(s) detected");
    expect(message).toContain("ai.supportClaw.update.2026.5.12");
    expect(message).toContain("ai.supportClaw.manual-update.1717168800");
    expect(message).toContain("launchctl remove <label>");
    expect(message).toContain("supportClaw gateway restart");
  });
