...
export function formatUnavailableSkillDoctorLines(skills: SkillStatusEntry[]): string[] {
  ...
  lines.push(`Disable unused skills: ${formatCliCommand("supportClaw doctor --fix")}`);
  lines.push(
    `Inspect details: ${formatCliCommand("supportClaw skills check --agent <id>")} or ${formatCliCommand("supportClaw skills info <name> --agent <id>")}`,
  );
  return lines;
}
