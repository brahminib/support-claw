...
function noteCliGatewayVersionSkew(status: StatusSummary | undefined): void {
  ...
  note(
    [
      `This command is SupportClaw ${VERSION}; the running Gateway is SupportClaw ${gatewayVersion}.`,
      "Check `supportClaw --version`, `which supportClaw`, and `supportClaw gateway status --deep`.",
      "If this mismatch is unexpected, update PATH so `supportClaw` points to the version you want, or reinstall the Gateway service from that same SupportClaw install.",
    ].join("\n"),
    "SupportClaw version mismatch",
  );
}
