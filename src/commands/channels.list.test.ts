...
function createCatalogEntry(id: string, label: string): ChannelPluginCatalogEntry {
  return {
    ...
    install: { npmSpec: `@supportclaw/${id}` },
...
describe("channels list", () => {
  ...
  it("default output shows configured official external channels when the plugin is missing", async () => {
    ...
    mocks.resolveMissingOfficialExternalChannelPluginRepairHint.mockReturnValue({
      ...
      channelId: "discord",
      label: "Discord",
      installSpec: "@supportclaw/discord",
      installCommand: "supportClaw plugins install @supportclaw/discord",
      doctorFixCommand: "supportClaw doctor --fix",
      repairHint:
        "Install the official external plugin with: supportClaw plugins install @supportclaw/discord, or run: supportClaw doctor --fix.",
    });
    mocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
    ...
    expect(output).toContain(
      "run supportClaw plugins install @supportclaw/discord or supportClaw doctor --fix",
    );
