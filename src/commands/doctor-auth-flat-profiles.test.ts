...
async function makeTestState(): Promise<SupportClawTestState> {
  const state = await createSupportClawTestState({
    layout: "state-only",
    prefix: "supportClaw-doctor-flat-auth-",
    env: {
...
describe("maybeMigrateAuthProfileJsonStoresToSqlite", () => {
  ...
  it("infers config credential provider and mode before stripping config", async () => {
    const cases: Array<{ profileId: string; cfg: SupportClawConfig; now: number }> = [
      {
...
describe("maybeRepairOpenAICodexAuthConfig", () => {
  it("renames legacy OpenAI Codex config profiles and merges auth order", () => {
    ...
    const migrated = result.config as SupportClawConfig & {
      agents?: {
        defaults?: {
