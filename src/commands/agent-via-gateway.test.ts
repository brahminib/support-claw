...
function mockConfig(storePath: string, overrides?: Partial<SupportClawConfig>) {
  const config = {
    agents: {
      defaults: {
        timeoutSeconds: 600,
...
async function withTempStore(
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-agent-cli-"));
...
describe("agentCliCommand", () => {
  ...
  for (const message of [
    ...
    it(`rejects ${JSON.stringify(message)} from the CLI before any gateway or embedded turn`, async () => {
      ...
      expect(errorMessages.some((m) => m.includes("supportClaw sessions compact"))).toBe(true);
