...
vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => "/tmp/supportClaw-migrate-command-test",
}));
  ...
  const items: MigrationPlan["items"] = [
    {
      ...
      target: "/tmp/supportClaw/workspace/skills/alpha",
      details: {
        skillName: "alpha",
    ...
    },
    {
      ...
      target: "/tmp/supportClaw/workspace/skills/beta",
      details: {
        skillName: "beta",
...
describe("migrateApplyCommand", () => {
  ...
  beforeEach(async () => {
    await fs.rm("/tmp/supportClaw-migrate-command-test", { force: true, recursive: true });
    ...
    mocks.backupCreateCommand.mockResolvedValue({ archivePath: "/tmp/supportClaw-backup.tgz" });
  });
  ...
  it("creates a verified backup before applying a conflict-free migration", async () => {
    ...
    expect(applyContext.backupPath).toBe("/tmp/supportClaw-backup.tgz");
    expect(String(applyContext.reportDir)).toContain("/migration/hermes/");
    expect(firstAppliedPlan()).toBe(planned);
    expect(result.backupPath).toBe("/tmp/supportClaw-backup.tgz");
  });
  ...
  it("prints only the final result for root apply in JSON mode", async () => {
    ...
    expect(logPayload.providerId).toBe("hermes");
    expect(logPayload.backupPath).toBe("/tmp/supportClaw-backup.tgz");
    expect(logPayload.items?.[0]?.details?.value?.time?.env?.OPENAI_API_KEY).toBe("[redacted]");
