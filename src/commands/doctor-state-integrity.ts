...
import { asNullableObjectRecord } from "@supportclaw/normalization-core/record-coerce";
...
import { resolveSupportClawAgentDir } from "../plugin-sdk/agent-dir-compat.js";
...
function listOrphanAgentDirs(cfg: SupportClawConfig, stateDir: string): OrphanAgentDir[] {
  ...
  const liveCompatibilityAgentDir = resolveSupportClawAgentDir();
  try {
...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".supportClaw"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for SUPPORT_CLAW_STATE_DIR.",
  ].join("\n");
}
...
export function formatLinuxVolatileStateDirWarning(
  ...
  return [
    ...
    "- Move SUPPORT_CLAW_STATE_DIR to a persistent filesystem to avoid data loss.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // SUPPORT_CLAW_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: SupportClawConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.SUPPORT_CLAW_OAUTH_DIR?.trim()) {
    return true;
  }
...
function shouldSuppressOrphanTranscriptWarning(cfg: SupportClawConfig, agentId: string): boolean {
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.supportClaw).",
        `  Set locally: SUPPORT_CLAW_STATE_DIR=~/.supportClaw ${formatCliCommand("supportClaw doctor")}`,
      ].join("\n"),
    );
  }
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`supportClaw sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`supportClaw sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
          `  Prune missing entries: ${formatCliCommand(`supportClaw sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
    ...
    if (wedgedSubagentSessions.length > 0) {
      ...
      warnings.push(
        [
          ...
          "  SupportClaw will not auto-resume these child sessions on restart; reconcile their task records instead.",
          `  Examples: ${wedgedSubagentSessions
            .slice(0, 3)
            .map(([key]) => key)
            .join(", ")}`,
          `  Fix: ${formatCliCommand("supportClaw tasks maintenance --apply")}`,
        ].join("\n"),
      );
...
export function collectWorkspaceBackupTip(workspaceDir: string): string | null {
  ...
  return [
    ...
    "- Keep ~/.supportClaw out of git; it contains credentials and session history.",
