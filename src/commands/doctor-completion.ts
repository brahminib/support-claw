...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(supportClaw completion ...)` */
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: "Run `supportClaw doctor --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: `Run \`supportClaw completion --write-state\` or \`supportClaw doctor --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
