...
export function summarizeExistingConfig(config: SupportClawConfig): string {
  const rows: string[] = [];
...
function summarizeGatewayConfig(config: SupportClawConfig): string | null {
...
export function printWizardHeader(runtime: RuntimeEnv) {
  ...
  const title = supportsDecorativeEmoji() && icon ? `${icon} SUPPORT_CLAW ${icon}` : "SUPPORT_CLAW";
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    "Docs:",
    "https://docs.supportClaw.ai/gateway/remote",
    "https://docs.supportClaw.ai/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
