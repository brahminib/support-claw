...
import { canonicalizeBase64, estimateBase64DecodedBytes } from "@supportclaw/media-core/base64";
import { parseMediaContentLength } from "@supportclaw/media-core/content-length";
...
export async function fetchWithGuard(params: {
  ...
  const { response, release } = await fetchWithSsrFGuard({
    ...
    init: { headers: { "User-Agent": "SupportClaw-Gateway/1.0" } },
  });
