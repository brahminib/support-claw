...
import { maxBytesForKind, type MediaKind } from "@supportclaw/media-core/constants";
import { basenameFromAnyPath, extnameFromAnyPath } from "@supportclaw/media-core/file-name";
import {
  detectMime,
  extensionForMime,
  getFileExtension,
...
async function isTrustedGeneratedHostReadHtmlPath(filePath: string | undefined): Promise<boolean> {
  ...
  const [resolvedFilePath, resolvedTmpRoot] = await Promise.all([
    ...
    realpath(resolvePreferredSupportClawTmpDir()).catch(() => undefined),
  ]);
  return Boolean(
