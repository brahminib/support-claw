...
describe("local media roots", () => {
  function withStateDir<T>(stateDir: string, run: () => T): T {
    return withEnv({ SUPPORT_CLAW_STATE_DIR: stateDir }, run);
  }
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "supportClaw-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "supportClaw-agent-media-roots-state"),
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "supportClaw-flexible-media-roots-state"),
      cfg: {},
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "supportClaw-messaging-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "supportClaw-messaging-fs-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "supportClaw-messaging-read-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
  ...
  it("keeps the config-dir media cache root when state and config paths differ", () => {
    const stateDir = path.join("/tmp", "supportClaw-legacy-state");
    const configDir = path.join("/tmp", "supportClaw-current-config");
