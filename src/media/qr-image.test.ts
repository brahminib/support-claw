...
describe("renderQrPngBase64", () => {
  const tmpRoot = path.join(os.tmpdir(), "supportClaw-qr-image-tests");
  ...
  it("delegates PNG rendering to qrcode", async () => {
    await expect(renderQrPngBase64("supportClaw", { scale: 8, marginModules: 2 })).resolves.toBe(
      MOCK_PNG_BASE64,
    );
    expect(toDataURL).toHaveBeenCalledWith("supportClaw", {
      margin: 2,
      scale: 8,
      type: "image/png",
    });
  ...
  it("uses the default PNG rendering options", async () => {
    await renderQrPngBase64("supportClaw");
    expect(toDataURL).toHaveBeenCalledWith("supportClaw", {
      margin: 4,
      scale: 6,
      type: "image/png",
  ...
  it("floors finite PNG rendering options before delegating", async () => {
    await renderQrPngBase64("supportClaw", { scale: 8.9, marginModules: 2.9 });
    expect(toDataURL).toHaveBeenCalledWith("supportClaw", {
      margin: 2,
      scale: 8,
      type: "image/png",
  ...
  it.each([
    ...
    await expect(renderQrPngBase64("supportClaw", { scale, marginModules })).rejects.toThrow(message);
    expect(toDataURL).not.toHaveBeenCalled();
  });

  it("rejects non-PNG qrcode data URLs", async () => {
    toDataURL.mockResolvedValue("data:image/svg+xml;base64,PHN2Zz4=");
    await expect(renderQrPngBase64("supportClaw")).rejects.toThrow(
  ...
  it("formats QR PNG data URLs", async () => {
    ...
    await expect(renderQrPngDataUrl("supportClaw")).resolves.toBe(
  ...
  it("writes QR PNGs to a scoped temp file", async () => {
    ...
    const result = await writeQrPngTempFile("supportClaw", {
      tmpRoot,
      dirPrefix: "pair-",
  ...
  it.each([
    ...
    await expect(
      writeQrPngTempFile("supportClaw", {
        tmpRoot,
