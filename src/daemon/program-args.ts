...
export const SUPPORT_CLAW_WRAPPER_ENV_KEY = "SUPPORT_CLAW_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/supportClaw -> .pnpm/supportClaw@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // supportClaw from node_modules/.bin points at the package root sibling.
...
export async function resolveSupportClawWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${SUPPORT_CLAW_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveSupportClawWrapperPath(params.wrapperPath);
  if (wrapperPath) {
