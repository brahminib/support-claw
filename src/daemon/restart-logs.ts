...
function resolveGatewayLogPrefix(env: GatewayServiceEnv): string {
  return env.SUPPORT_CLAW_LOG_PREFIX?.trim() || "gateway";
}

function resolveMacLaunchAgentLogPrefix(env: GatewayServiceEnv): string {
  return (
    env.SUPPORT_CLAW_LOG_PREFIX?.trim() || `gateway${resolveGatewayProfileSuffix(env.SUPPORT_CLAW_PROFILE)}`
  );
}
...
export function resolveMacLaunchAgentLogPaths(env: GatewayServiceEnv): GatewayLogPaths {
  ...
  const logDir = path.posix.join(home, "Library", "Logs", "supportClaw");
...
export function resolveGatewaySupervisorLogPaths(
  ...
  // SupportClaw state dir so generated service users can create the directory.
...
export function renderCmdRestartLogSetup(env: GatewayServiceEnv): {
  ...
  return {
    ...
    lines: [
      ...
      `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] supportClaw restart log initialized`,
    ],
  };
}
