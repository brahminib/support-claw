...
const SUPPORT_CLAW_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.supportClaw.update.";
const SUPPORT_CLAW_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN = /^ai\.supportClaw\.manual-update\.\d+$/;

export type StaleSupportClawUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

function normalizeSupportClawUpdateLaunchdLabel(label: unknown): string | null {
  if (typeof label !== "string") {
    return null;
  }
  const trimmed = label.trim();
  if (trimmed.startsWith(SUPPORT_CLAW_UPDATE_LAUNCHD_LABEL_PREFIX)) {
    return trimmed;
  }
  // Manual update jobs include a timestamp-like suffix and should be cleaned up
  // without matching arbitrary ai.supportClaw labels.
  return SUPPORT_CLAW_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN.test(trimmed) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.SUPPORT_CLAW_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.SUPPORT_CLAW_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.SUPPORT_CLAW_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.SUPPORT_CLAW_LAUNCHD_LABEL?.trim();
  return Boolean(configuredLabel && label === configuredLabel);
}

function resolveCurrentSupportClawUpdateLaunchdJobLabel(
  ...
  for (const label of [
    ...
    env.SUPPORT_CLAW_LAUNCHD_LABEL,
  ]) {
    const normalized = normalizeSupportClawUpdateLaunchdLabel(label);
    if (normalized) {
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.SUPPORT_CLAW_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.SUPPORT_CLAW_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by SupportClaw. Do not edit while the gateway service is installed.",
...
export function parseLaunchctlListSupportClawUpdateJobs(
  output: string,
): StaleSupportClawUpdateLaunchdJob[] {
  const jobs: StaleSupportClawUpdateLaunchdJob[] = [];
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    const label = normalizeSupportClawUpdateLaunchdLabel(labelParts.join(" "));
    if (!label) {
      continue;
    }
...
export async function findStaleSupportClawUpdateLaunchdJobs(
  env: NodeJS.ProcessEnv = process.env,
): Promise<StaleSupportClawUpdateLaunchdJob[]> {
  ...
  return parseLaunchctlListSupportClawUpdateJobs(result.stdout).filter(
    (job) => !isCurrentGatewayLaunchdLabel(job.label, env),
  );
}

export async function disableSupportClawUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeSupportClawUpdateLaunchdLabel(label);
...
export async function disableCurrentSupportClawUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const label = resolveCurrentSupportClawUpdateLaunchdJobLabel(env);
  if (!label) {
    return false;
  }
  return await disableSupportClawUpdateLaunchdJob(label);
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseTcpPort(command?.environment?.SUPPORT_CLAW_GATEWAY_PORT ?? "");
  if (fromServiceEnv !== null) {
    return fromServiceEnv;
  }
  return parseTcpPort(env.SUPPORT_CLAW_GATEWAY_PORT ?? "");
}
...
export function formatLaunchAgentGuiSessionError(params: {
  ...
  return [
    ...
    "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.supportClaw.ai/gateway",
  ].join("\n");
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `supportClaw gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.SUPPORT_CLAW_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "supportClaw gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "supportClaw gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `supportClaw gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      serviceTarget,
      plistPath,
      actionHint: "supportClaw gateway restart",
    });
