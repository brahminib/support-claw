...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when SUPPORT_CLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".supportClaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when SUPPORT_CLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", SUPPORT_CLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".supportClaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers SUPPORT_CLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        SUPPORT_CLAW_PROFILE: "rescue",
        SUPPORT_CLAW_STATE_DIR: "C:\\State\\supportClaw",
      },
      expected: path.join("C:\\State\\supportClaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", SUPPORT_CLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".supportClaw", "gateway.cmd"),
    },
    {
      name: "uses a custom task script file name inside the state directory",
      env: {
        USERPROFILE: "C:\\Users\\test",
        SUPPORT_CLAW_TASK_SCRIPT_NAME: "gateway-node.cmd",
      },
      expected: path.join("C:\\Users\\test", ".supportClaw", "gateway-node.cmd"),
    },
  ])("$name", ({ env, expected }) => {
  ...
  it.each([
    ...
    expect(() =>
      resolveTaskScriptPath({
        USERPROFILE: "C:\\Users\\test",
        SUPPORT_CLAW_TASK_SCRIPT_NAME: scriptName,
      }),
    ).toThrow("SUPPORT_CLAW_TASK_SCRIPT_NAME must be a file name only");
  });
});
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem SupportClaw Gateway",
          "cd /d C:\\Projects\\supportClaw",
          "set NODE_ENV=production",
          "set SUPPORT_CLAW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\supportClaw",
          environment: {
            NODE_ENV: "production",
            SUPPORT_CLAW_PORT: "18789",
          },
          environmentValueSources: {
            NODE_ENV: "inline",
            SUPPORT_CLAW_PORT: "inline",
          },
          sourcePath: resolveTaskScriptPath(env),
        });
      },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\supportClaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\SupportClaw Share\\node.exe" "\\\\fileserver\\SupportClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\SupportClaw Share\\node.exe",
            "\\\\fileserver\\SupportClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from SUPPORT_CLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ SUPPORT_CLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
