...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds SUPPORT_CLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `supportClaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service SUPPORT_CLAW_GATEWAY_TOKEN does not match gateway.auth.token in supportClaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.SUPPORT_CLAW_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.SUPPORT_CLAW_GATEWAY_TOKEN);
}
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  const serviceVersion = command?.environment?.SUPPORT_CLAW_SERVICE_VERSION?.trim();
  ...
  issues.push({
    ...
    message: `Gateway service was installed by SupportClaw ${serviceVersion}; current CLI is ${VERSION}.`,
