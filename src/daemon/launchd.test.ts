...
import {
  ...
  disableSupportClawUpdateLaunchdJob,
  findStaleSupportClawUpdateLaunchdJobs,
  parseLaunchctlPrint,
  parseLaunchctlListSupportClawUpdateJobs,
...
function expectLaunchctlEnableBootstrapOrder(env: Record<string, string | undefined>) {
  ...
  const label = "ai.supportClaw.gateway";
...
describe("launchctl list detection", () => {
  it("parses stale SupportClaw updater jobs from launchctl list", () => {
    const jobs = parseLaunchctlListSupportClawUpdateJobs(
      [
        "123 0 ai.supportClaw.gateway",
        "- 127 ai.supportClaw.update.2026.5.12",
        "- 0 ai.supportClaw.manual-update.1717168800",
        "8142 0 ai.supportClaw.update.2026.5.13-beta.1",
        "- 0 ai.supportClaw.manual-updater.1717168800",
        "- 0 com.example.other",
      ].join("\n"),
    );
    ...
    expect(jobs).toEqual([
      {
        label: "ai.supportClaw.manual-update.1717168800",
        lastExitStatus: 0,
      },
      {
        label: "ai.supportClaw.update.2026.5.12",
        lastExitStatus: 127,
      },
      {
        label: "ai.supportClaw.update.2026.5.13-beta.1",
        pid: 8142,
        lastExitStatus: 0,
      },
    ]);
  ...
  it.runIf(process.platform === "darwin")(
    "finds stale SupportClaw updater jobs via launchctl list",
    async () => {
      state.listOutput = "- 127 ai.supportClaw.update.2026.5.12\n";

      const jobs = await findStaleSupportClawUpdateLaunchdJobs();
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      state.listOutput = [
        "- 0 ai.supportClaw.manual-update.1717168800",
        "812 0 ai.supportClaw.manual-update.profile",
        "913 0 ai.supportClaw.manual-update.custom-label",
      ].join("\n");

      const jobs = await findStaleSupportClawUpdateLaunchdJobs({
        SUPPORT_CLAW_PROFILE: "manual-update.profile",
        SUPPORT_CLAW_LAUNCHD_LABEL: "ai.supportClaw.manual-update.custom-label",
        SUPPORT_CLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
        SUPPORT_CLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentSupportClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.supportClaw.update.2026.5.12",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.supportClaw.update.2026.5.12`,
      ]);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentSupportClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.supportClaw.manual-update.1717168800",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.supportClaw.manual-update.1717168800`,
      ]);
      expect(launchctlCommandNames()).not.toContain("remove");
    },
  );
  ...
  it.runIf(process.platform === "darwin")(
    "disables the current legacy updater launchd job from SupportClaw label env",
    async () => {
      await expect(
        disableCurrentSupportClawUpdateLaunchdJob({
          SUPPORT_CLAW_LAUNCHD_LABEL: "ai.supportClaw.update.2026.5.12",
        }),
      ).resolves.toBe(true);
  ...
  it.runIf(process.platform === "darwin")(
    "does not let non-update launchd markers mask the SupportClaw update label",
    async () => {
      await expect(
        disableCurrentSupportClawUpdateLaunchdJob({
          XPC_SERVICE_NAME: "0",
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentSupportClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.supportClaw.update.2026.5.12",
          SUPPORT_CLAW_PROFILE: "update.2026.5.12",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentSupportClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.supportClaw.manual-update.1717168800",
          SUPPORT_CLAW_PROFILE: "manual-update.1717168800",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentSupportClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.supportClaw.manual-update.gateway",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")("disables explicit legacy updater jobs", async () => {
    await expect(disableSupportClawUpdateLaunchdJob("ai.supportClaw.update.2026.5.12")).resolves.toBe(
      true,
    );
  ...
  it.runIf(process.platform === "darwin")("disables explicit manual updater jobs", async () => {
    await expect(
      disableSupportClawUpdateLaunchdJob("ai.supportClaw.manual-update.1717168800"),
    ).resolves.toBe(true);
...
describe("launchd install", () => {
  ...
  it("writes LaunchAgent environment to an owner-only env file when provided", async () => {
    const env = createDefaultLaunchdEnv();
    const tmpDir = "/Users/test/.supportClaw/tmp";
    ...
    const plistPath = resolveLaunchAgentPlistPath(env);
    const envFilePath = "/Users/test/.supportClaw/service-env/ai.supportClaw.gateway.env";
    const wrapperPath = "/Users/test/.supportClaw/service-env/ai.supportClaw.gateway-env-wrapper.sh";
    ...
    expect(state.dirModes.get("/Users/test/.supportClaw/service-env")).toBe(0o700);
  ...
  it("repairs a mangled label-derived service-env wrapper path on restart", async () => {
    ...
    const serviceEnv = {
      ...callerEnv,
      SUPPORT_CLAW_STATE_DIR: "/Users/test/service-env/custom-state",
    };
    await installLaunchAgent({
      env: serviceEnv,
      stdout: new PassThrough(),
      ...
      environment: {
        ...
        SUPPORT_CLAW_STATE_DIR: serviceEnv.SUPPORT_CLAW_STATE_DIR,
      },
    });

    const plistPath = resolveLaunchAgentPlistPath(callerEnv);
    const envFilePath = "/Users/test/service-env/custom-state/service-env/ai.supportClaw.gateway.env";
    const wrapperPath =
      "/Users/test/service-env/custom-state/service-env/ai.supportClaw.gateway-env-wrapper.sh";
    const callerEnvFilePath = "/Users/test/.supportClaw/service-env/ai.supportClaw.gateway.env";
    const callerWrapperPath =
      "/Users/test/.supportClaw/service-env/ai.supportClaw.gateway-env-wrapper.sh";
    const mangledEnvFilePath =
      "/Users/test/service-env/custom-state/service-env/[ai.supportClaw.gateway.env](http:/ai.supportClaw.gateway.env)";
    const mangledWrapperPath =
      "/Users/test/service-env/custom-state/service-env/[ai.supportClaw.gateway-env-wrapper.sh](http:/ai.supportClaw.gateway-env-wrapper.sh)";
    state.files.set(
      plistPath,
    ...
    expect(command?.programArguments).toEqual(defaultProgramArguments);
    expect(command?.environment?.SUPPORT_CLAW_GATEWAY_PORT).toBe("18789");
    expect(command?.environment?.SUPPORT_CLAW_STATE_DIR).toBe(serviceEnv.SUPPORT_CLAW_STATE_DIR);
    expect(command?.environmentValueSources?.SUPPORT_CLAW_GATEWAY_PORT).toBe("file");
    ...
    expect(rewrittenEnv).toContain("export SUPPORT_CLAW_GATEWAY_PORT='18789'");
    expect(rewrittenEnv).toContain(
      "export SUPPORT_CLAW_STATE_DIR='/Users/test/service-env/custom-state'",
    );
  });
  ...
  it("writes KeepAlive=true policy with shutdown and throttle limits", async () => {
    ...
    expect(plist).toContain("<string>/Users/test/Library/Logs/supportClaw/gateway.log</string>");
  ...
  it("rewrites the plist before bootstrap during restart fallback", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "  <dict>",
        "    <key>Label</key>",
        "    <string>ai.supportClaw.gateway</string>",
  ...
  it("stops LaunchAgent via bootout by default, preserving KeepAlive for future crashes", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.supportClaw.gateway`;
    ...
    await withProcessEnv(
      ...
      async () => {
        await expect(stopLaunchAgent({ env, stdout: new PassThrough() })).rejects.toThrow(
          "Refusing to stop LaunchAgent ai.supportClaw.gateway from inside the same launchd service",
        );
      },
    );
  ...
  it("allows external LaunchAgent label overrides to stop the selected target", async () => {
    const env = {
      ...
      SUPPORT_CLAW_LAUNCHD_LABEL: "com.example.supportClaw.gateway",
    };
    const stdout = new PassThrough();
    let output = "";
    stdout.on("data", (chunk: Buffer) => {
    ...
    await withProcessEnv(
      {
        ...
        SUPPORT_CLAW_LAUNCHD_LABEL: undefined,
        SUPPORT_CLAW_SERVICE_MARKER: undefined,
        SUPPORT_CLAW_SERVICE_KIND: undefined,
      },
      async () => {
        await stopLaunchAgent({ env, stdout });
      },
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/com.example.supportClaw.gateway`;
    expect(state.launchctlCalls).toEqual([["bootout", serviceId]]);
    expect(output).toContain("Stopped LaunchAgent");
  });
  ...
  it("verifies the configured gateway port is released before reporting stop success", async () => {
    const env = {
      ...createDefaultLaunchdEnv(),
      SUPPORT_CLAW_GATEWAY_PORT: "19003",
    };
    const stdout = new PassThrough();
    let output = "";
    stdout.on("data", (chunk: Buffer) => {
  ...
  it("resolves the stop postcondition port from the stored LaunchAgent environment", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { SUPPORT_CLAW_GATEWAY_PORT: "19006" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("fails stop when the verified gateway port remains busy after cleanup", async () => {
    const env = {
      ...createDefaultLaunchdEnv(),
      SUPPORT_CLAW_GATEWAY_PORT: "19004",
    };
    const stdout = new PassThrough();
    let output = "";
    stdout.on("data", (chunk: Buffer) => {
  ...
  it("verifies the configured gateway port is released before reporting disable stop success", async () => {
    const env = {
      ...createDefaultLaunchdEnv(),
      SUPPORT_CLAW_GATEWAY_PORT: "19005",
    };
    const stdout = new PassThrough();
    let output = "";
    stdout.on("data", (chunk: Buffer) => {
  ...
  it("does not report degraded stop success when fallback cleanup leaves the port busy", async () => {
    const env = {
      ...createDefaultLaunchdEnv(),
      SUPPORT_CLAW_GATEWAY_PORT: "19008",
    };
    const stdout = new PassThrough();
    let output = "";
    state.disableError = "Operation not permitted";
  ...
  it("reloads launchd after rewriting an existing plist", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "    <string>/Users/test/.supportClaw-default/logs/gateway.log</string>",
        "  </dict>",
        "</plist>",
      ].join("\n"),
    );
  ...
  it("ignores invalid configured gateway ports for stale cleanup", async () => {
    const env = {
      ...createDefaultLaunchdEnv(),
      SUPPORT_CLAW_GATEWAY_PORT: "65536",
    };
    state.files.clear();
  ...
  it("uses the stored LaunchAgent environment port for restart stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { SUPPORT_CLAW_GATEWAY_PORT: "19007" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("ignores invalid stored LaunchAgent environment ports for stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { SUPPORT_CLAW_GATEWAY_PORT: "65536" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("fails restart before kickstart when the configured gateway port remains busy", async () => {
    const env = {
      ...createDefaultLaunchdEnv(),
      SUPPORT_CLAW_GATEWAY_PORT: "19002",
    };
    const plistPath = resolveLaunchAgentPlistPath(env);
    const originalPlist = [
      '<?xml version="1.0" encoding="UTF-8"?>',
  ...
  it("hands restart off to a detached helper when invoked from the current LaunchAgent", async () => {
    ...
    const result = await withProcessEnv({ LAUNCH_JOB_LABEL: "ai.supportClaw.gateway" }, async () =>
      restartLaunchAgent({
        env,
        stdout: new PassThrough(),
      }),
  ...
  it("hands plist reload off when current LaunchAgent needs rewritten paths", async () => {
    ...
    expect(state.files.get(plistPath)).toContain("/Users/test/Library/Logs/supportClaw/gateway.log");
    expect(state.launchctlCalls).toStrictEqual([]);
  });
  ...
  it("surfaces detached handoff failures", async () => {
    ...
    await expect(
      withProcessEnv({ LAUNCH_JOB_LABEL: "ai.supportClaw.gateway" }, async () =>
        restartLaunchAgent({
          env,
          stdout: new PassThrough(),
        }),
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.supportClaw.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when SUPPORT_CLAW_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.supportClaw.gateway.plist",
    },
    {
      name: "uses profile-specific label when SUPPORT_CLAW_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", SUPPORT_CLAW_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.supportClaw.jbphoenix.plist",
    },
    {
      name: "prefers SUPPORT_CLAW_LAUNCHD_LABEL over SUPPORT_CLAW_PROFILE",
      env: {
        HOME: "/Users/test",
        SUPPORT_CLAW_PROFILE: "jbphoenix",
        SUPPORT_CLAW_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from SUPPORT_CLAW_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        SUPPORT_CLAW_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty SUPPORT_CLAW_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        SUPPORT_CLAW_PROFILE: "myprofile",
        SUPPORT_CLAW_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.supportClaw.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        SUPPORT_CLAW_LAUNCHD_LABEL: "../evil/label",
      }),
