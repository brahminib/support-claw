...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SUPPORT_CLAW_GATEWAY_PORT).toBe("18789");
    expect(env.SUPPORT_CLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.SUPPORT_CLAW_SERVICE_MARKER).toBe("supportClaw");
    expect(env.SUPPORT_CLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SUPPORT_CLAW_SERVICE_VERSION).toBe("string");
    expect(env.SUPPORT_CLAW_SYSTEMD_UNIT).toBe("supportClaw-gateway.service");
    expect(env.SUPPORT_CLAW_WINDOWS_TASK_NAME).toBe("SupportClaw Gateway");
    if (process.platform === "darwin") {
      expect(env.SUPPORT_CLAW_LAUNCHD_LABEL).toBe("ai.supportClaw.gateway");
    }
  });

  it("sets the SupportClaw-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 18789,
      platform: "darwin",
  ...
  it("passes through SUPPORT_CLAW_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        SUPPORT_CLAW_WRAPPER: " /usr/local/bin/supportClaw-doppler ",
      },
      port: 18789,
    });

    expect(env.SUPPORT_CLAW_WRAPPER).toBe("/usr/local/bin/supportClaw-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".supportClaw", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SUPPORT_CLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SUPPORT_CLAW_SYSTEMD_UNIT).toBe("supportClaw-gateway-work.service");
    expect(env.SUPPORT_CLAW_WINDOWS_TASK_NAME).toBe("SupportClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.SUPPORT_CLAW_LAUNCHD_LABEL).toBe("ai.supportClaw.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        ...
        SUPPORT_CLAW_SYSTEMD_UNIT: "supportClaw-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.SUPPORT_CLAW_SYSTEMD_UNIT).toBe("supportClaw-gateway-maintenance.service");
  });
  ...
  it("preserves explicit systemd unit overrides with service suffix", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        SUPPORT_CLAW_SYSTEMD_UNIT: "supportClaw-gateway-maintenance.service",
      },
      port: 18789,
      platform: "linux",
    });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", SUPPORT_CLAW_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        SUPPORT_CLAW_PROXY_URL: " http://127.0.0.1:3128 ",
      },
      port: 18789,
    });

    expect(env.SUPPORT_CLAW_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the SupportClaw-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });

    expect(env.SUPPORT_CLAW_LAUNCHD_LABEL).toBe("ai.supportClaw.node");
  });

  it("passes through SUPPORT_CLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", SUPPORT_CLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.SUPPORT_CLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits SUPPORT_CLAW_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.SUPPORT_CLAW_WINDOWS_TASK_NAME).toBe("SupportClaw Node");
    expect(env.SUPPORT_CLAW_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.SUPPORT_CLAW_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".supportClaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", SUPPORT_CLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".supportClaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", SUPPORT_CLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".supportClaw"));
  });

  it("uses SUPPORT_CLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", SUPPORT_CLAW_STATE_DIR: "/var/lib/supportClaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/supportClaw"));
  });

  it("expands ~ in SUPPORT_CLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", SUPPORT_CLAW_STATE_DIR: "~/supportClaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/supportClaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { SUPPORT_CLAW_STATE_DIR: "C:\\State\\supportClaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\supportClaw");
  });
});
