...
describe("resolveGatewayService", () => {
  ...
  it("guards mutating service adapters when config was written by a newer SupportClaw", async () => {
    const tempHome = await makeTempWorkspace("supportClaw-service-future-config-");
    const stateDir = path.join(tempHome, ".supportClaw");
    const configPath = path.join(stateDir, "supportClaw.json");
    const envSnapshot = captureEnv(["HOME", "SUPPORT_CLAW_STATE_DIR", "SUPPORT_CLAW_CONFIG_PATH"]);
    try {
...
describe("readGatewayServiceState", () => {
  it("tracks installed, loaded, and running separately", async () => {
    ...
    const state = await readGatewayServiceState(service, {
      env: { SUPPORT_CLAW_GATEWAY_PORT: "1" },
    });

    expect(state.installed).toBe(true);
    expect(state.loaded).toBe(true);
    expect(state.running).toBe(true);
    expect(state.env.SUPPORT_CLAW_GATEWAY_PORT).toBe("18789");
  });
  ...
  it("keeps the caller-selected service identity when merging persisted env", async () => {
    ...
    const state = await readGatewayServiceState(service, {
      env: { SUPPORT_CLAW_SYSTEMD_UNIT: "supportClaw-gateway-maintenance.service" },
    });

    expect(state.env.SUPPORT_CLAW_SYSTEMD_UNIT).toBe("supportClaw-gateway-maintenance.service");
...
describe("startGatewayService", () => {
  ...
  it("requests repair before start when the loaded service version is stale", async () => {
    const service = createService({
      readCommand: vi.fn(async () => ({
        programArguments: ["supportClaw", "gateway", "run"],
        environment: { SUPPORT_CLAW_SERVICE_VERSION: "2026.4.24" },
      })),
      isLoaded: vi.fn(async () => true),
    ...
    if (result.outcome === "repair-required") {
      expect(formatGatewayServiceStartRepairIssues(result.issues)).toContain(
        "service was installed by SupportClaw 2026.4.24",
      );
    }
    expect(service.restart).not.toHaveBeenCalled();
  });
  ...
  it("requests repair before start when the loaded service points at temporary install paths", async () => {
    const service = createService({
      readCommand: vi.fn(async () => ({
        programArguments: [
          "/private/tmp/supportClaw-ai-install-cli-pr118/tools/node/bin/node",
          "/tmp/supportClaw-ai-install-cli-pr118/lib/node_modules/supportClaw/dist/index.js",
          "gateway",
        ],
        environment: {},
      })),
