...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.SUPPORT_CLAW_LAUNCHD_LABEL);
  if (!configuredLabel || configuredLabel !== label) {
    return false;
  }
  if (
    normalizeOptionalString(env.SUPPORT_CLAW_SERVICE_MARKER) === "supportClaw" &&
    Boolean(normalizeOptionalString(env.SUPPORT_CLAW_SERVICE_KIND))
  ) {
