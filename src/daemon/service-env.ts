...
function readServiceProxyEnvironment(
  ...
  // Service env intentionally preserves only the canonical SupportClaw proxy knob;
  // generic shell proxy vars are audited but not frozen into services.
  const proxyUrl = normalizeOptionalString(env.SUPPORT_CLAW_PROXY_URL);
  return proxyUrl ? { SUPPORT_CLAW_PROXY_URL: proxyUrl } : {};
}
...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.SUPPORT_CLAW_PROFILE;
  const wrapperPath = normalizeOptionalString(env.SUPPORT_CLAW_WRAPPER);
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = resolveGatewaySystemdUnitEnv(env);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    SUPPORT_CLAW_PROFILE: profile,
    SUPPORT_CLAW_WRAPPER: wrapperPath,
    SUPPORT_CLAW_GATEWAY_PORT: String(port),
    SUPPORT_CLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    SUPPORT_CLAW_SYSTEMD_UNIT: systemdUnit,
    SUPPORT_CLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    SUPPORT_CLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    SUPPORT_CLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    SUPPORT_CLAW_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.SUPPORT_CLAW_GATEWAY_TOKEN);
  const allowInsecurePrivateWs = normalizeOptionalString(env.SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    SUPPORT_CLAW_GATEWAY_TOKEN: gatewayToken,
    SUPPORT_CLAW_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    SUPPORT_CLAW_STATE_DIR: sharedEnv.stateDir,
    SUPPORT_CLAW_CONFIG_PATH: sharedEnv.configPath,
    ...sharedEnv.proxyEnv,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.SUPPORT_CLAW_STATE_DIR;
