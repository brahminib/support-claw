/** Inspects installed platform services for extra SupportClaw or legacy gateway jobs. */
...
const EXTRA_MARKERS = ["supportClaw", "clawdbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  ...
  const markerKeys = ["supportClaw_service_marker"];
  const kindKeys = ["supportClaw_service_kind"];
...
function isSupportClawGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (detectLaunchdGatewayExecutionMarker(contents) !== "supportClaw") {
    return false;
  }
  return label.startsWith("ai.supportClaw.");
}

function isSupportClawGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("supportClaw-gateway")) {
    return false;
  }
  return normalizeLowercaseStringOrEmpty(contents).includes("gateway");
}

function isSupportClawGatewayTaskName(name: string): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(name);
  if (!normalized) {
    return false;
  }
  // Windows schtasks /Query returns task names prefixed with \ (e.g.
  // \SupportClaw Gateway for root-folder tasks). Strip the leading
  ...
  return stripped === defaultName || /^supportClaw gateway \(.+\)$/.test(stripped);
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    const marker =
      hasGatewayServiceMarker(contents) || executionMarker === "supportClaw"
        ? "supportClaw"
    ...
    if (marker === "supportClaw" && isSupportClawGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "supportClaw" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  dir: string;
  scope: "user" | "system";
  includeManagedSupportClaw?: boolean;
}): Promise<ExtraGatewayService[]> {
  const results: ExtraGatewayService[] = [];
  const candidates = await collectServiceFiles({
    dir: params.dir,
    extension: ".service",
    isIgnoredName: params.includeManagedSupportClaw ? () => false : isIgnoredSystemdName,
  });
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (
      !params.includeManagedSupportClaw &&
      marker === "supportClaw" &&
      isSupportClawGatewaySystemdService(name, contents)
    ) {
      continue;
    }
    results.push({
    ...
    for (const dir of ["/etc/systemd/system", "/usr/lib/systemd/system", "/lib/systemd/system"]) {
      results.push(
        ...(await scanSystemdDir({
          dir,
          scope: "system",
          includeManagedSupportClaw: true,
        })),
      );
    }
  } catch {
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isSupportClawGatewayTaskName(name)) {
        continue;
      }
