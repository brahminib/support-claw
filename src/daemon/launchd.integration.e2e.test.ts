...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `supportClaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      SUPPORT_CLAW_LAUNCHD_LABEL: `ai.supportClaw.launchd-int-${testId}`,
      SUPPORT_CLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
