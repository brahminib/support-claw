...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/supportClaw-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): SupportClawConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: SupportClawConfig) {
...
let unauthenticatedProviderWritePlan: Awaited<ReturnType<typeof planSupportClawModelsJsonWithDeps>>;
...
beforeAll(async () => {
  ...
  unauthenticatedProviderWritePlan = await planSupportClawModelsJsonWithDeps(
    {
...
describe("models-config", () => {
  it("keeps the implicit provider catalog when explicit baseUrl is blank", async () => {
    let observedConfig: SupportClawConfig | undefined;
  ...
  it("threads workspace scope into implicit provider discovery", async () => {
    ...
    expect(observedWorkspaceDir).toBe("/tmp/supportClaw-workspace");
  });
  ...
  it("threads plugin metadata snapshots through models.json planning", async () => {
    ...
    await planSupportClawModelsJsonWithDeps(
      {
        cfg: { models: { providers: {} } },
        agentDir: "/tmp/supportClaw-models-config-env-vars-test",
        env: {},
  ...
  it("keeps google-vertex static catalog rows when an auth profile supplies the API key", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-google-vertex-models-"));
    try {
  ...
  it("keeps google-vertex static catalog rows when ADC auth evidence supplies the marker", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-google-vertex-adc-models-"));
    ...
    try {
      const plan = await withEnvAsync(
        {
          SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: BUNDLED_PLUGINS_DIR,
