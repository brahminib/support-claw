// Verifies SupportClaw-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "supportClaw/plugin-sdk/agent-core";
import {
  installSupportClawOwnedToolHooks,
  resetSupportClawOwnedToolHooks,
  textToolResult,
} from "supportClaw/plugin-sdk/agent-runtime-test-contracts";
import type { ExtensionContext } from "supportClaw/plugin-sdk/agent-sessions";
...
describe("SupportClaw-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetSupportClawOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installSupportClawOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installSupportClawOwnedToolHooks();
    ...
    const originalParams = {
      ...
      mediaUrl: "/tmp/supportClaw-reply.png",
      provider: "telegram",
      to: "chat-1",
    };
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/supportClaw-reply.png"]);
    expect(
    ...
    ).toEqual([
      {
        ...
        mediaUrls: ["/tmp/supportClaw-reply.png"],
      },
    ]);
    const [afterPayload, afterContext] = await waitForAfterToolCall(hooks);
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installSupportClawOwnedToolHooks({ blockReason: "blocked by policy" });
