...
import { normalizeTrimmedStringList } from "@supportclaw/normalization-core/string-normalization";
import { listSupportClawPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
...
const SUPPORT_CLAW_ATTRIBUTION_PRODUCT = "SupportClaw";
const SUPPORT_CLAW_ATTRIBUTION_ORIGINATOR = "supportClaw";
...
function formatSupportClawUserAgent(version: string): string {
  return `${SUPPORT_CLAW_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listSupportClawPluginManifestMetadata()) {
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: SUPPORT_CLAW_ATTRIBUTION_PRODUCT,
...
function buildOpenRouterAttributionPolicy(
  ...
  return {
    ...
    reviewNote: "Documented app attribution headers. Verified in SupportClaw runtime wrapper.",
    ...identity,
    headers: {
...
function buildNvidiaAttributionPolicy(
  ...
  return {
    ...
    headers: {
      "X-BILLING-INVOKE-ORIGIN": SUPPORT_CLAW_ATTRIBUTION_PRODUCT,
    },
  };
}
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: SUPPORT_CLAW_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatSupportClawUserAgent(identity.version),
    },
  };
}
...
function buildXaiAttributionPolicy(
  ...
  return {
    ...
    hook: "request-headers",
    reviewNote:
      "xAI api.x.ai accepts a standard supportClaw User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
    ...identity,
    headers: {
      originator: SUPPORT_CLAW_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
