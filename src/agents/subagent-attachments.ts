...
function resolveAttachmentLimits(config: SupportClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".supportClaw", "attachments");
  const relDir = path.posix.join(".supportClaw", "attachments", attachmentId);
