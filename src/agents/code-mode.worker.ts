...
(() => {
  ...
  const catalog = Array.isArray(globalThis.__supportClawCatalog) ? globalThis.__supportClawCatalog : [];
  const apiFiles = Array.isArray(globalThis.__supportClawApiFiles) ? globalThis.__supportClawApiFiles : [];
  const namespaceDescriptors = Array.isArray(globalThis.__supportClawNamespaces) ? globalThis.__supportClawNamespaces : [];
  ...
  function request(method, args) {
    const id = String(globalThis.__supportClawHostRequest(String(method), JSON.stringify(safe(args ?? []))));
  ...
  Object.defineProperties(globalThis, {
    ...
    __supportClawSettleBridge: { value: settle },
    __supportClawTakeOutput: { value: () => output.splice(0) },
  });
})();
`;

function buildUserSource(code: string): string {
  return `globalThis.__supportClawResult = (async () => {\n${code}\n})()`;
}
...
async function createVm(params: {
  ...
  try {
    vm.setProp(vm.global, "__supportClawHostRequest", hostRequest);
  } finally {
    hostRequest.dispose();
  }
  vm.evalCode(CONTROLLER_SOURCE, "supportClaw-code-mode:controller.js").dispose();
...
function takeOutput(vm: QuickJS): unknown[] {
  const take = vm.global.getProp("__supportClawTakeOutput");
  try {
...
function getResultHandle(vm: QuickJS): JSValueHandle {
  return vm.global.getProp("__supportClawResult");
}
...
async function runResume(input: Extract<CodeModeWorkerInput, { kind: "resume" }>) {
  ...
  return runVmExecution({
    ...
    config: input.config,
    prepare: () => {
      const settle = vm.global.getProp("__supportClawSettleBridge");
      try {
