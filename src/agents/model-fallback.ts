...
function hasConfiguredProvider(params: { cfg?: SupportClawConfig; provider: string }): boolean {
  const normalizedProvider = normalizeProviderId(params.provider);
...
function isCliAgentRuntime(runtime: string | undefined, cfg: SupportClawConfig | undefined): boolean {
...
async function resolveModelFallbackCandidateHarnessAuthPrecheck(
  ...
  if (isCliAgentRuntime(agentRuntime, params.cfg)) {
    // CLI runtimes own their transport/auth, so stale SupportClaw provider
    // profile state must not block the candidate before the CLI starts.
    return { skipsProviderAuthCooldown: true };
  }
  if (agentRuntime === "supportClaw") {
    return { skipsProviderAuthCooldown: false };
  }
  ...
  // Explicit non-Codex plugin harnesses own transport/auth; stale SupportClaw
...
export function resolveImageFallbackDefaultProvider(cfg: SupportClawConfig | undefined): string {
...
function resolveFallbackCandidateModelProviderCacheParts(cfg: SupportClawConfig | undefined): unknown {
