...
import { resolveOptionalMediaToolFactoryPlan } from "./supportClaw-tools.media-factory-plan.js";
...
type CreateSupportClawToolsOptions = Parameters<
  typeof import("./supportClaw-tools.js").createSupportClawTools
>[0];
let createSupportClawToolsForTestModule: typeof import("./supportClaw-tools.js").createSupportClawTools;
let legacyComfyToolNames: string[];

async function createSupportClawToolsForTest(options?: CreateSupportClawToolsOptions) {
  return createSupportClawToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/supportClaw.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): SupportClawConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createSupportClawTools: createSupportClawToolsForTestModule } =
      await import("./supportClaw-tools.js"));
    ...
    legacyComfyToolNames = (
      await createSupportClawToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: SupportClawConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createSupportClawToolsForTest({
      config,
      agentDir: "/tmp/supportClaw-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: SupportClawConfig = {};
    const workspaceDir = "/tmp/supportClaw-workspace";
