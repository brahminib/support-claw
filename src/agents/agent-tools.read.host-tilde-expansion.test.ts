...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("supportClaw-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("supportClaw-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("supportClaw-tilde-test-mkdir-");
  ...
  it("ignores SUPPORT_CLAW_HOME for write operations", async () => {
    const supportClawHome = await createTempDir("supportClaw-home-override-", os.tmpdir());
    const dir = await createTempDir("supportClaw-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");

    await withEnvAsync({ SUPPORT_CLAW_HOME: supportClawHome }, async () => {
      createHostWorkspaceWriteTool(supportClawHome, { workspaceOnly: false });
      ...
      await expectMissingPath(fs.access(path.join(supportClawHome, path.basename(testFile))));
    });
  });

  it("ignores SUPPORT_CLAW_HOME for mkdir operations", async () => {
    const supportClawHome = await createTempDir("supportClaw-home-override-", os.tmpdir());
    const dir = await createTempDir("supportClaw-tilde-test-mkdir-");
    ...
    await withEnvAsync({ SUPPORT_CLAW_HOME: supportClawHome }, async () => {
      ...
      expect((await fs.stat(newDir)).isDirectory()).toBe(true);
      await expectMissingPath(fs.access(path.join(supportClawHome, path.basename(newDir))));
    });
  });

  it("ignores SUPPORT_CLAW_HOME for readFile operations", async () => {
    const supportClawHome = await createTempDir("supportClaw-home-override-", os.tmpdir());
    const dir = await createTempDir("supportClaw-tilde-test-edit-");
    ...
    await withEnvAsync({ SUPPORT_CLAW_HOME: supportClawHome }, async () => {
      createHostWorkspaceEditTool(supportClawHome, { workspaceOnly: false });
      const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores SUPPORT_CLAW_HOME for access operations", async () => {
    const supportClawHome = await createTempDir("supportClaw-home-override-", os.tmpdir());
    const dir = await createTempDir("supportClaw-tilde-test-edit-");
