...
describe("runExecProcess POSIX command wrapper", () => {
  ...
  it("wraps command with PATH export if SUPPORT_CLAW_PREPEND_PATH is present", async () => {
    ...
    expect(commandStr).toContain(
      'export PATH="${SUPPORT_CLAW_PREPEND_PATH}${PATH:+:$PATH}"; unset SUPPORT_CLAW_PREPEND_PATH; echo test',
    );
  });
