...
describe("gateway-hosted exec approvals", () => {
  ...
  it(
    "lets SupportClaw-style gateway tool calls request and wait for approval over separate connections",
    async () => {
      const envSnapshot = captureEnv(TEST_ENV_KEYS);
      cleanup.push(() => envSnapshot.restore());

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-exec-approval-e2e-"));
      ...
      setTestEnvValue("SUPPORT_CLAW_GATEWAY_PORT", String(port));
