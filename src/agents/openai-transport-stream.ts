 ...
 * provider-specific payload policy before converting SDK streams into SupportClaw assistant events.
 */
...
const OPENAI_RESPONSES_REASONING_REPLAY_META_KEY = "__supportClaw_replay";
const OPENAI_RESPONSES_REASONING_REPLAY_BLOCK_META_KEY = "supportClawReasoningReplay";
...
type BaseStreamOptions = {
  ...
  supportClawCodeModeToolSurface?: boolean;
...
function createResponsesFirstEventTimeoutError(model: Model, timeoutMs: number): Error {
  return new Error(
    `Azure OpenAI Responses stream did not deliver a first event within ${timeoutMs}ms after HTTP streaming headers. ` +
      ...
      "The provider may be stalled while parsing the tool payload; retry with a smaller tool surface or enable SUPPORT_CLAW_DEBUG_MODEL_PAYLOAD=tools to inspect exposed tools.",
  );
}
  ...
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (
          (options as { supportClawCodeModeToolSurface?: unknown } | undefined)
            ?.supportClawCodeModeToolSurface === true
        ) {
...
function isOpenAICodexResponsesModel(model: Model): boolean {
  return (
    ...
    (model.api === "openai-chatgpt-responses" ||
      model.api === "supportClaw-openai-responses-transport")
  );
}
...
function getCompletionsContentDeltas(content: unknown): CompletionsReasoningDelta[] {
  ...
  // Preserve provider reasoning as SupportClaw thinking blocks so channel/UI
