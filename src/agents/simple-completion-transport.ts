 ...
 *
 * Registers provider-specific stream functions and rewrites models that need SupportClaw-managed transport semantics.
 */
...
const PROVIDER_SIMPLE_COMPLETION_API_PREFIX = "supportClaw-provider-simple:";
...
function resolveAnthropicVertexSimpleApi(baseUrl?: string): Api {
  ...
  return `supportClaw-anthropic-vertex-simple:${suffix}`;
}
...
function applyProviderSimpleCompletionWrapper(model: Model, cfg?: SupportClawConfig): Model {
...
function prepareCodexSimpleTransportModel<TApi extends Api>(
  ...
  // completion path must use SupportClaw's transport so resolved request auth is applied.
  const transportModel = {
    ...model,
    baseUrl: normalizeCodexResponsesBaseUrlForOpenAISdk(model.baseUrl),
  } as Model;
  const api = resolveTransportAwareSimpleApi(model.api);
  const streamFn = createSupportClawTransportStreamFnForModel(transportModel, { cfg });
  if (!api || !streamFn) {
    return undefined;
  }
