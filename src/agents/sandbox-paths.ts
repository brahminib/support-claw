...
import { isPassThroughRemoteMediaSource } from "@supportclaw/media-core/media-source-url";
...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const supportClawTmpDir = path.resolve(resolvePreferredSupportClawTmpDir());
  if (!isPathInside(supportClawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: supportClawTmpDir });
  return resolved;
}
