...
const CAPTURE_MARKER = "__SUPPORT_CLAW_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__SUPPORT_CLAW_SHELL_SNAPSHOT_ENV__";
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-shell-snapshot-"));
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // SUPPORT_CLAW_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.SUPPORT_CLAW_SHELL === "exec") {
    env.SUPPORT_CLAW_SHELL = "exec";
  }
  return env;
}
