...
async function createBundleMcpFixture(params: {
  ...
  setTestEnvValue("SUPPORT_CLAW_DISABLE_PERSISTED_PLUGIN_REGISTRY", "1");
  ...
  const fakeClaudePidPath = params.liveSession
    ...
    : undefined;
  const pluginRoot = path.join(tempHome, ".supportClaw", "extensions", "bundle-probe");
...
describe("CLI bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const fixture = await createBundleMcpFixture({
        tempPrefix: "supportClaw-cli-bundle-mcp-",
      });
  ...
  it(
    ...
    async () => {
      ...
      const fixture = await createBundleMcpFixture({
        liveSession: true,
        tempPrefix: "supportClaw-cli-live-cleanup-",
      });
      const context = await prepareBundleMcpExecutionContext({
