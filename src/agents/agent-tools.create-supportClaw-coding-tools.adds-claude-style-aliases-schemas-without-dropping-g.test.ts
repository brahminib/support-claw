...
import type { AgentTool, AgentToolResult } from "supportClaw/plugin-sdk/agent-core";
import { Type } from "typebox";
import { describe, expect, it, vi } from "vitest";
import * as windowsEncoding from "../infra/windows-encoding.js";
import { createSupportClawReadTool, createSandboxedReadTool } from "./agent-tools.read.js";
...
describe("createSupportClawCodingTools read behavior", () => {
  it("uses host decoding only for host-backed sandbox paths", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-sbx-encoding-"));
  ...
  it("applies sandbox path guards to canonical path", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "supportClaw-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-read-autopage-"));
  ...
  it("maps inbound media refs to sandbox-staged media for reads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-read-media-sbx-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-read-cap-"));
  ...
  it("returns empty content for explicit offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-read-offset-eof-"));
  ...
  it("returns empty content for adaptive offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-read-offset-adaptive-"));
    await fs.writeFile(path.join(tmpDir, "notes.txt"), "one\ntwo\nthree\n", "utf8");
    try {
      const readTool = createSandboxedReadTool({
        root: tmpDir,
  ...
  it("returns already-read adaptive content when pagination reaches EOF", async () => {
    ...
    const readTool = createSupportClawReadTool({
      name: "read",
      label: "read",
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createSupportClawReadTool(
      baseRead as unknown as Parameters<typeof createSupportClawReadTool>[0],
    );
