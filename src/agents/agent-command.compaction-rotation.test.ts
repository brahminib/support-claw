...
const state = vi.hoisted(() => ({
  cfg: undefined as SupportClawConfig | undefined,
...
vi.mock("./agent-scope.js", async () => {
  ...
  return {
    ...
    resolveAgentDir: () => state.agentDir ?? "/tmp/supportClaw-agent",
    resolveDefaultAgentId: () => "main",
    resolveEffectiveModelFallbacks: () => undefined,
    resolveSessionAgentId: () => "main",
    resolveAgentWorkspaceDir: () => state.workspaceDir ?? "/tmp/supportClaw-workspace",
  };
});
...
beforeEach(async () => {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-rotation-e2e-"));
