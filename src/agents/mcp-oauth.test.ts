...
describe("MCP OAuth provider", () => {
  it("stores token state under the SupportClaw state directory with restricted permissions", async () => {
    await withTempHome(
      async (home) => {
        ...
        const tokenDir = `${home}/.supportClaw/mcp-oauth`;
  ...
  it("isolates token state by configured server URL", async () => {
    await withTempHome(
      ...
      {
        prefix: "supportClaw-mcp-oauth-url-",
        skipSessionCleanup: true,
        env: {
          SUPPORT_CLAW_CONFIG_PATH: undefined,
          SUPPORT_CLAW_STATE_DIR: undefined,
  ...
  it("does not persist localhost when the fallback attempt fails", async () => {
    await withTempHome(
      async (home) => {
        ...
        await expect(fs.readdir(`${home}/.supportClaw/mcp-oauth`)).rejects.toThrow();
      },
      {
        prefix: "supportClaw-mcp-oauth-localhost-failure-",
        skipSessionCleanup: true,
        env: {
          SUPPORT_CLAW_CONFIG_PATH: undefined,
          SUPPORT_CLAW_STATE_DIR: undefined,
  ...
  it("persists localhost redirect for a later code exchange login", async () => {
    await withTempHome(
      ...
      {
        prefix: "supportClaw-mcp-oauth-localhost-persist-",
        skipSessionCleanup: true,
        env: {
          SUPPORT_CLAW_CONFIG_PATH: undefined,
          SUPPORT_CLAW_STATE_DIR: undefined,
  ...
  it("does not start hidden authorization flows without an authorization callback", async () => {
    ...
    await withTempHome(
      async () => {
        ...
        await expect(provider.state?.()).rejects.toThrow("Run supportClaw mcp login Remote Docs.");
        await expect(provider.saveCodeVerifier?.("verifier")).rejects.toThrow(
          "Run supportClaw mcp login Remote Docs.",
        );
        await expect(
          provider.redirectToAuthorization?.(new URL("https://auth.example.com/authorize")),
        ).rejects.toThrow("Run supportClaw mcp login Remote Docs.");
      },
      {
        prefix: "supportClaw-mcp-oauth-noninteractive-",
        skipSessionCleanup: true,
        env: {
          SUPPORT_CLAW_CONFIG_PATH: undefined,
          SUPPORT_CLAW_STATE_DIR: undefined,
  ...
  it("clears stored credentials for a configured server URL", async () => {
    await withTempHome(
      ...
      {
        prefix: "supportClaw-mcp-oauth-clear-",
        skipSessionCleanup: true,
        env: {
          SUPPORT_CLAW_CONFIG_PATH: undefined,
          SUPPORT_CLAW_STATE_DIR: undefined,
