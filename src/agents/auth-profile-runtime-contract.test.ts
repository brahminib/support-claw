...
function providerRuntimeConfig(provider: string, runtime: string): SupportClawConfig {
  return {
    models: {
      providers: {
        [provider]: {
          baseUrl: "https://api.supportClaw.test/v1",
...
async function runAuthContractAttempt(params: {
  ...
  const cfg = params.cfg ?? ({} as SupportClawConfig);
...
describe("Auth profile runtime contract - embedded SupportClaw and CLI adapter", () => {
  let tmpDir: string;
  let storePath: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-auth-contract-"));
  ...
  it("forwards a legacy OpenAI Codex auth profile through the embedded SupportClaw path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
  ...
  it("forwards an OpenAI auth profile through the explicit embedded OpenAI SupportClaw path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      ...
      authProfileProvider: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
      authProfileOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProfileId,
      cfg: providerRuntimeConfig(AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider, "supportClaw"),
    });
  ...
  it("routes explicit OpenAI SupportClaw runs with legacy Codex OAuth through OpenAI transport", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      providerOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
