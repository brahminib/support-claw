...
const originalConfigPath = process.env.SUPPORT_CLAW_CONFIG_PATH;
let tempDirs: string[] = [];
...
function initializeTempPlugin(params: { tmpPrefix: string; id: string; body: string }) {
  ...
  process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
...
afterEach(() => {
  ...
  if (originalConfigPath === undefined) {
    ...
    process.env.SUPPORT_CLAW_CONFIG_PATH = originalConfigPath;
  }
...
describe("tool_result_persist hook", () => {
  ...
  it("keeps sensitive parent keys when custom value patterns match the key probe", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-redact-config-"));
    tempDirs.push(tempDir);
    process.env.SUPPORT_CLAW_CONFIG_PATH = path.join(tempDir, "supportClaw.json");
    fs.writeFileSync(
      process.env.SUPPORT_CLAW_CONFIG_PATH,
  ...
  it("loads tool_result_persist hooks without breaking persistence", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-toolpersist-"));
    process.env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("reapplies the cap after tool_result_persist expands a tool result", () => {
    initializeTempPlugin({
      tmpPrefix: "supportClaw-toolpersist-expand-",
      id: "persist-expand",
  ...
  it("reapplies the details cap after tool_result_persist expands details", () => {
    initializeTempPlugin({
      tmpPrefix: "supportClaw-toolpersist-details-expand-",
  ...
  it("reapplies the details cap after redaction expands hook details", () => {
    ...
    initializeTempPlugin({
      tmpPrefix: "supportClaw-toolpersist-details-redaction-expand-",
...
describe("before_message_write hook", () => {
  it("continues persistence when a before_message_write hook throws", () => {
    initializeTempPlugin({
      tmpPrefix: "supportClaw-before-write-",
  ...
  it("reapplies the cap after before_message_write expands a tool result", () => {
    initializeTempPlugin({
      tmpPrefix: "supportClaw-before-write-expand-",
