 ...
 *
 * Routes models that need SupportClaw-managed proxy/TLS/local-service semantics onto built-in transport implementations.
 */
...
const SIMPLE_TRANSPORT_API_ALIAS: Record<string, Api> = {
  "openai-responses": "supportClaw-openai-responses-transport",
  "openai-chatgpt-responses": "supportClaw-openai-responses-transport",
  "openai-completions": "supportClaw-openai-completions-transport",
  "azure-openai-responses": "supportClaw-azure-openai-responses-transport",
  "anthropic-messages": "supportClaw-anthropic-messages-transport",
  "google-generative-ai": "supportClaw-google-generative-ai-transport",
};
...
function hasSupportClawTransportRequirement(model: Model): boolean {
  const request = getModelProviderRequestTransport(model);
  return Boolean(request?.proxy || request?.tls || getModelProviderLocalService(model));
}

/** Returns whether SupportClaw has a managed transport implementation for this API. */
...
export function createTransportAwareStreamFnForModel(
  ...
  if (!hasSupportClawTransportRequirement(model)) {
    return undefined;
  }
...
/** Creates a managed SupportClaw transport stream for explicit fallback/runtime callers. */
export function createSupportClawTransportStreamFnForModel(
  model: Model,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need SupportClaw's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // HTTP streams preserve the same SupportClaw request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);
