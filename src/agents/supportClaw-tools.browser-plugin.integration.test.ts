// Verifies SupportClaw plugin tools are resolved with browser/runtime context.
...
function firstResolvePluginToolsParams(): Record<string, unknown> {
  // Captures the plugin runtime contract passed from SupportClaw tool resolution.
...
describe("createSupportClawTools browser plugin integration", () => {
  afterEach(() => {
  ...
  it("keeps the browser tool returned by plugin resolution", () => {
    ...
    const tools = resolveSupportClawPluginToolsForOptions({
      options: { config },
  ...
  it("does not pass a stale active snapshot as plugin runtime config for a resolved run config", () => {
    ...
    let capturedRuntimeConfig: SupportClawConfig | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      capturedRuntimeConfig = (params as { context?: { runtimeConfig?: SupportClawConfig } }).context
        ?.runtimeConfig;
      return [];
    });
  ...
  it("does not let a source-less pinned config snapshot override explicit plugin tool config", () => {
    ...
    let capturedRuntimeConfig: SupportClawConfig | undefined;
    let getRuntimeConfig: (() => SupportClawConfig | undefined) | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      const context = (
        params as {
          context?: {
            runtimeConfig?: SupportClawConfig;
            getRuntimeConfig?: () => SupportClawConfig | undefined;
          };
        }
      ).context;
  ...
  it("exposes a live runtime config getter to plugin tool factories", () => {
    ...
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      getRuntimeConfig = (
        params as { context?: { getRuntimeConfig?: () => SupportClawConfig | undefined } }
