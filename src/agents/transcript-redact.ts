...
} from "@supportclaw/media-core/inline-image-data-url";
...
function redactTranscriptOptions(cfg?: SupportClawConfig) {
...
function isTranscriptRedactionDisabled(cfg?: SupportClawConfig): boolean {
  return (cfg?.logging?.redactSensitive ?? readLoggingConfig()?.redactSensitive) === "off";
}

function redactTranscriptText(value: string, cfg?: SupportClawConfig): string {
...
const OPENAI_RESPONSES_APIS = new Set([
  ...
  "supportClaw-azure-openai-responses-transport",
]);
...
const ANTHROPIC_REASONING_APIS = new Set([
  ...
  "supportClaw-anthropic-messages-transport",
]);
const OPENAI_COMPLETIONS_APIS = new Set([
  "openai-completions",
  "supportClaw-openai-completions-transport",
]);
...
function isGitHubCopilotResponsesRoute(route: TranscriptAssistantRoute | undefined): boolean {
  return (
    (route?.api === "openai-responses" || route?.api === "supportClaw-openai-responses-transport") &&
...
const OPENAI_REASONING_REPLAY_METADATA_KEY = "__supportClaw_replay";
...
function redactTranscriptStructuredValue(
  ...
  for (const [key, item] of Object.entries(source)) {
    if (
      ...
      key === "supportClawReasoningReplay"
    ) {
...
export function redactTranscriptMessage(message: AgentMessage, cfg?: SupportClawConfig): AgentMessage {
