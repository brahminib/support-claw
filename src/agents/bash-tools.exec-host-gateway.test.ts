...
describe("processGatewayAllowlist", () => {
  ...
  it("does not require suppression edit approval for read-only suppression inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      allowlistSatisfied: true,
      segments: [
        { resolution: null, argv: ["supportClaw", "config", "get", "security.audit.suppressions"] },
      ],
      segmentAllowlistEntries: [],
    });
    resolveExecHostApprovalContextMock.mockReturnValue({
    ...
    await runGatewayAllowlist({
      command: "supportClaw config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("does not require suppression edit approval for profile-scoped read-only inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      segments: [
        {
          resolution: null,
          argv: ["supportClaw", "--profile", "rescue", "config", "get", "security.audit.suppressions"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    await runGatewayAllowlist({
      command: "supportClaw --profile rescue config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a mutating segment follows read-only inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      segments: [
        ...
        {
          resolution: null,
          argv: ["supportClaw", "config", "set", "security.audit.suppressions", "[]"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    const result = await runGatewayAllowlist({
      command:
        "supportClaw config get security.audit.suppressions; supportClaw config set security.audit.suppressions '[]'",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a heredoc patch follows read-only inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      segments: [
        {
          raw: "supportClaw config get security.audit.suppressions",
          resolution: null,
          argv: ["supportClaw", "config", "get", "security.audit.suppressions"],
        },
        {
          raw: "supportClaw config patch --stdin <<'EOF'",
          resolution: null,
          argv: ["supportClaw", "config", "patch", "--stdin"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    const result = await runGatewayAllowlist({
      command: `supportClaw config get security.audit.suppressions; supportClaw config patch --stdin <<'EOF'
...
EOF`,
  ...
  it("keeps webchat diagnostics approvals as direct pasteable followups", async () => {
    ...
    const outcome = {
      ...
      aggregated: JSON.stringify({
        path: "/tmp/supportClaw-diagnostics.zip",
        bytes: 1234,
        manifest: {
          generatedAt: "2026-04-28T20:58:29.311Z",
          supportClawVersion: "2026.4.27",
          contents: [
    ...
    const approvalFollowup = vi.fn<ExecApprovalFollowupFactory>(async () =>
      [
        ...
        "Session 1",
        "Channel: telegram",
        "SupportClaw session id: `session-1`",
        "Codex thread id: `thread-1`",
      ].join("\n"),
    );

    const result = await runGatewayAllowlist({
      command: "supportClaw gateway diagnostics export --json",
      trigger: "diagnostics",
    ...
    expect(followupText).toContain("Path: /tmp/supportClaw-diagnostics.zip");
  ...
  it("uses async agent followups for explicit webchat approval mode", async () => {
    ...
    const result = await runGatewayAllowlist({
      command: "supportClaw sessions export-trajectory --json",
      approvalFollowupMode: "agent",
      sessionId: "approval-session",
      sessionStore: "/tmp/supportClaw-sessions.json",
      turnSourceChannel: "webchat",
    });
