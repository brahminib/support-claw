...
const ensureSupportClawModelsJsonMock = vi.fn(async () => ({ wrote: false }));
const loggerWarnMock = vi.fn();
let refreshRuntimeAuthOnFirstPromptError = false;
let clearRuntimeConfigSnapshot: typeof import("../config/config.js").clearRuntimeConfigSnapshot;
let setRuntimeConfigSnapshot: typeof import("../config/config.js").setRuntimeConfigSnapshot;

vi.mock("supportClaw/plugin-sdk/llm", async () => {
  const actual =
    await vi.importActual<typeof import("supportClaw/plugin-sdk/llm")>("supportClaw/plugin-sdk/llm");
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureSupportClawModelsJson: (...args: Parameters<typeof ensureSupportClawModelsJsonMock>) =>
        ensureSupportClawModelsJsonMock(...args),
    };
  });
};

let runEmbeddedAgent: typeof import("./embedded-agent-runner/run.js").runEmbeddedAgent;
let SessionManager: typeof import("supportClaw/plugin-sdk/agent-sessions").SessionManager;
...
beforeAll(async () => {
  ...
  ({ SessionManager } = await import("supportClaw/plugin-sdk/agent-sessions"));
  e2eWorkspace = await createEmbeddedAgentRunnerTestWorkspace("supportClaw-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureSupportClawModelsJsonMock.mockReset();
  ensureSupportClawModelsJsonMock.mockResolvedValue({ wrote: false });
...
describe("runEmbeddedAgent", () => {
  ...
  it("skips models.json generation when dynamic model resolution succeeds", async () => {
    ...
    expect(ensureSupportClawModelsJsonMock).not.toHaveBeenCalled();
  });

  it("resolves explicit OpenAI SupportClaw runs through Codex when auth order starts with Codex OAuth", async () => {
    const sessionFile = nextSessionFile();
    ...
    await runEmbeddedAgent({
      sessionId: "codex-first-supportClaw",
      sessionFile,
      workspaceDir,
      config: cfg,
      prompt: "hello",
      ...
      timeoutMs: 5_000,
      agentDir,
      runId: nextRunId("codex-first-supportClaw"),
      enqueue: immediateEnqueue,
    });
