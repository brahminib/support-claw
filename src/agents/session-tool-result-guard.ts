 ...
 */
import { resolveIntegerOption } from "@supportclaw/normalization-core/number-coercion";
...
const PERSISTED_DETAIL_REDACTION_BOUNDARY = "\u0000SUPPORT_CLAW_PERSISTED_DETAIL_BOUNDARY\u0000";
...
function redactPersistedDetailString(
  ...
  const redactedPrefix =
    boundaryIndex >= 0
      ...
      : "[SupportClaw persisted detail redacted: boundary marker removed]";
  ...
  const persistedPrefix =
    ...
    PARTIAL_PRIVATE_KEY_BLOCK_RE.test(initialPersistedPrefix)
      ? "[SupportClaw persisted detail redacted: partial secret span omitted]"
      : initialPersistedPrefix;
  const boundaryNotice = "[SupportClaw persisted detail redacted: boundary overlap omitted]";
  return `${persistedPrefix}${persistedPrefix ? "\n" : ""}${boundaryNotice}\n\n[SupportClaw persisted detail truncated: ${Math.max(
    0,
...
function redactPersistedDetailValue(
  ...
  if (depth >= 8) {
    return "[SupportClaw persisted detail redacted: max depth exceeded]";
  }
...
function isTranscriptOnlySupportClawAssistantMessage(message: AgentMessage): boolean {
  ...
  return isTranscriptOnlySupportClawAssistantModel(provider, model);
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage, callerOptions?: AppendMessageOptions) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlySupportClawAssistantMessage(nextMessage);
    if (
