...
const NATIVE_HOOK_RELAY_STATE_SYMBOL = Symbol.for("supportClaw.nativeHookRelay.state");
        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by SupportClaw",
                },
        },
      })}\n`,
      stderr: "",
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveSupportClawCliExecutable();
  const argv =
...
function registerNativeHookRelayBridge(registration: ActiveNativeHookRelayRegistration): void {
  ...
  // directory is scoped by OS user (uid) and is shared across all SupportClaw
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `supportClaw-native-hook-relays-${uid}`);
}
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("supportClaw:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
function readNativeHookRelayApprovalMode(rawPayload: JsonValue): "report" | undefined {
  const payload = isJsonObject(rawPayload) ? rawPayload : {};
  return payload.supportClaw_approval_mode === "report" ? "report" : undefined;
}
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `supportClaw-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveSupportClawCliExecutable(): string {
  const envPath = process.env.SUPPORT_CLAW_CLI_PATH?.trim();
  ...
  if (packageRoot) {
    for (const candidate of [
      path.join(packageRoot, "supportClawCli.mjs"),
  ...
  throw new Error("Cannot resolve SupportClaw CLI executable path for native hook relay");
}
