...
const agentRunAttempt = vi.fn<AgentHarness["runAttempt"]>(async () =>
  createAttemptResult("supportClaw"),
);
...
vi.mock("./builtin-supportClaw.js", () => ({
  createSupportClawAgentHarness: (): AgentHarness => ({
    id: "supportClaw",
...
const originalRuntime = process.env.SUPPORT_CLAW_AGENT_RUNTIME;
...
afterEach(() => {
  ...
  if (originalRuntime == null) {
    delete process.env.SUPPORT_CLAW_AGENT_RUNTIME;
  } else {
    process.env.SUPPORT_CLAW_AGENT_RUNTIME = originalRuntime;
  }
});

function createAttemptParams(config?: SupportClawConfig): EmbeddedRunAttemptParams {
  return {
    prompt: "hello",
    sessionId: "session-1",
    runId: "run-1",
...
function groupSenderDenyAllConfig(): SupportClawConfig {
...
function groupDenyAllConfig(): SupportClawConfig {
  return {
    channels: {
      telegram: {
        groups: {
...
function providerRuntimeConfig(provider: string, runtime: string): SupportClawConfig {
  return {
    models: {
      providers: {
        [provider]: {
...
describe("runAgentHarnessAttempt", () => {
  it("fails when a forced plugin harness is unavailable and fallback is omitted", async () => {
    process.env.SUPPORT_CLAW_AGENT_RUNTIME = "codex";
  ...
  it("falls back to the SupportClaw harness in auto mode when no plugin harness matches", async () => {
    const result = await runAgentHarnessAttempt(createAttemptParams());

    expect(result.sessionIdUsed).toBe("supportClaw");
    expect(agentRunAttempt).toHaveBeenCalledTimes(1);
  });

  it("allows the selected SupportClaw harness to satisfy context-engine pre-prompt assembly", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("codex", "supportClaw")),
  ...
  it("surfaces an auto-selected plugin harness failure instead of replaying through SupportClaw", async () => {
  ...
  it("surfaces a forced plugin harness failure instead of replaying through SupportClaw", async () => {
    registerFailingCodexHarness();
  ...
  it("falls back to SupportClaw when the implicit OpenAI Codex harness is unavailable", async () => {
    expect(resolveAgentHarnessPolicy({ provider: "openai", modelId: "gpt-5.4" })).toEqual({
      runtime: "codex",
      runtimeSource: "implicit",
    });
  ...
  it("honors explicit SupportClaw runtime for OpenAI agent model runs", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("openai", "supportClaw")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("supportClaw");
  ...
  it("honors provider wildcard SupportClaw runtime policy for OpenAI agent model runs", async () => {
    registerSuccessfulCodexHarness();

    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(agentModelRuntimeConfig("openai/*", "supportClaw")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("supportClaw");
  ...
  it.each([
    {
      ...
      config: { tools: { allow: ["message"] } } as SupportClawConfig,
    },
    {
      name: "specific denylist",
      config: { tools: { deny: ["exec"] } } as SupportClawConfig,
    },
    {
      name: "narrow profile",
      config: { tools: { profile: "coding" } } as SupportClawConfig,
    },
  ...
  it.each([
    { name: "full tool profile", config: { tools: { profile: "full" } } as SupportClawConfig },
    { name: "explicit empty allowlist", config: { tools: { allow: [] } } as SupportClawConfig },
  ...
  it("leaves SupportClaw harness params unchanged for channel group sender deny-all policy", async () => {
    await runAgentHarnessAttempt({
      ...createAttemptParams(groupSenderDenyAllConfig()),
      sessionKey: "agent:main:telegram:group:test-deny-room",
      messageProvider: "telegram",
  ...
  it("does not let a strict agent model plugin runtime fall back to SupportClaw", async () => {
    await expect(
      runAgentHarnessAttempt({
...
describe("selectAgentHarness", () => {
  ...
  it("ignores session-level SupportClaw pins when selecting a harness", () => {
    const supports = vi.fn(() => ({ supported: true as const, priority: 100 }));
    registerAgentHarness({
      id: "codex",
      label: "Codex",
  ...
  it("honors explicit SupportClaw runtime overrides when selecting a harness", async () => {
    registerSuccessfulCodexHarness();

    const harness = selectAgentHarness({
      provider: "openai",
      modelId: "gpt-5.4",
      agentHarnessRuntimeOverride: "supportClaw",
    });

    expect(harness.id).toBe("supportClaw");
  ...
  it("treats legacy PI runtime overrides as the built-in SupportClaw harness", async () => {
    registerSuccessfulCodexHarness();
  ...
  it("selects SupportClaw when the implicit OpenAI Codex harness is unavailable", () => {
    expect(selectAgentHarness({ provider: "openai", modelId: "gpt-5.4" }).id).toBe("supportClaw");
  });
  ...
  it("ignores existing session SupportClaw pins when provider policy forces a plugin harness", () => {
    registerFailingCodexHarness();
  ...
  it("ignores env-forced SupportClaw for OpenAI default runtime selection", () => {
    process.env.SUPPORT_CLAW_AGENT_RUNTIME = "supportClaw";
    registerFailingCodexHarness();
      ...
      maybeCompactAgentHarnessSession({
        ...
        config: {
          agents: {
            ...
            defaults: {
              models: {
                "openai/gpt-5.5": { agentRuntime: { id: "supportClaw" } },
              },
            },
          },
        } as SupportClawConfig,
  ...
  it("keeps compaction recoverable when auth profile lookup fails", async () => {
    ...
    await expect(
      maybeCompactAgentHarnessSession({
        ...
        agentHarnessId: "codex",
        config: agentModelRuntimeConfig("openai/gpt-5.5", "supportClaw"),
      }),
    ).resolves.toEqual({ ok: true, compacted: false });
    expect(compact).toHaveBeenCalledTimes(1);
  ...
  it("does not compact a selected plugin harness through SupportClaw when the plugin has no compactor", async () => {
    registerFailingCodexHarness();
  ...
  it.each([
    ...
    "returns SupportClaw for explicit CLI runtime alias $alias on $provider instead of throwing MissingAgentHarnessError",
