...
describe("AgentHarness lifecycle runner", () => {
  ...
  it("advertises SupportClaw embedded host capabilities", async () => {
    const harness = createSupportClawAgentHarness();

    expect(harness.contextEngineHostCapabilities).toEqual(
      SUPPORT_CLAW_EMBEDDED_CONTEXT_ENGINE_HOST.capabilities,
    );
  });
