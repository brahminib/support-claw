...
import { createSupportClawAgentHarness } from "./builtin-supportClaw.js";
...
type AgentHarnessSelectionDecision = {
  ...
  selectedReason:
    | "forced_supportClaw"
    | "forced_plugin"
    // Implicit Codex preference found no registered Codex harness, so SupportClaw handled the run.
    | "implicit_plugin_unavailable_supportClaw"
    // Provider-owned CLI runtime aliases have no agent harness plugin counterpart.
    | "cli_runtime_passthrough_supportClaw"
    // Auto mode chose a registered plugin harness that supports the provider/model.
    | "auto_plugin"
    // Auto mode found no supporting plugin harness, so SupportClaw handled the run.
    | "auto_supportClaw";
...
function selectAgentHarnessDecision(params: {
  ...
  const policy =
    runtimeOverride && !isDefaultAgentRuntimeId(runtimeOverride)
      ...
      : resolvedPolicy;
  // SupportClaw's built-in harness is intentionally not part of the plugin candidate list. Explicit plugin
  // runtimes fail closed; only `auto` may route an unmatched turn to SupportClaw.
  const pluginHarnesses = listPluginAgentHarnesses();
  const supportClawHarness = createSupportClawAgentHarness();
  const runtime = policy.runtime;
  if (runtime === "supportClaw") {
    return buildSelectionDecision({
      harness: supportClawHarness,
      policy,
      selectedReason: "forced_supportClaw",
  ...
  if (runtime !== "auto") {
    ...
    if (forced) {
      ...
      if (isCliRuntimeAliasForProvider({ runtime, provider: params.provider })) {
        return buildSelectionDecision({
          ...
          selectedReason: "cli_runtime_passthrough_supportClaw",
          candidates: listHarnessCandidates(pluginHarnesses),
        });
      }
      throw new Error(
    ...
    if (runtime === "codex" && policy.runtimeSource === "implicit") {
      return buildSelectionDecision({
        ...
        selectedReason: "implicit_plugin_unavailable_supportClaw",
        candidates: listHarnessCandidates(pluginHarnesses),
      });
    }
    if (
  ...
  return buildSelectionDecision({
    harness: supportClawHarness,
    policy,
    selectedReason: "auto_supportClaw",
    candidates: candidates.map(toSelectionCandidate),
  });
}
...
export async function runAgentHarnessAttempt(
  ...
  const attemptParams =
    harness.id === "supportClaw" ? params : applyPluginHarnessDenyAllToolPolicy(params);
  ...
  try {
    ...
    log.warn(`${harness.label} failed; not falling back to embedded SupportClaw backend`, {
