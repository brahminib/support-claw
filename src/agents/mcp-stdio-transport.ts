/**
 * SupportClaw stdio transport wrapper for MCP server subprocesses.
 */
...
type SupportClawStdioServerParameters = {
  command: string;
  args?: string[];
...
export class SupportClawStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: SupportClawStdioServerParameters) {
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "SupportClawStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }
