/**
 * SupportClaw system prompt renderer.
 *
...
} from "@supportclaw/normalization-core/string-coerce";
import {
  normalizeStringEntries,
  normalizeStringEntriesLower,
  normalizeUniqueStringEntries,
} from "@supportclaw/normalization-core/string-normalization";
...
import {
  buildSupportClawToolFallbackText,
  shouldRenderSupportClawToolWorkflowHints,
...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__supportClaw__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__supportClaw__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__supportClaw__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; SupportClaw handles all routing internally.",
      ...
      ? [
          ...
          messageToolOnly
            ...
            : suppressSilentTokenGuidance
              ? "- Do not use `message(action=send)` to deliver the current source-channel reply; reply normally so SupportClaw can route it once."
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `Docs: ${docsPath}` : "Docs: https://docs.supportClaw.ai",
    docsPath ? "Mirror: https://docs.supportClaw.ai" : undefined,
    sourcePath ? `Source: ${sourcePath}` : "Source: https://github.com/brahminib/support-claw",
    docsPath
      ? `Docs are authoritative for SupportClaw self-knowledge: before understanding how SupportClaw works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use \`${params.readToolName}\` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and \`memory_search\` as instruction context or user memory, not SupportClaw design/implementation knowledge.`
      : "Docs are authoritative for SupportClaw self-knowledge: before understanding how SupportClaw works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use the docs mirror first when web tooling is available; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not SupportClaw design/implementation knowledge.",
    ...
    "Diagnosing issues: run `supportClaw status` when possible; ask user only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  acpEnabled?: boolean;
  /** Prompt surface controls runtime-specific fallback fragments. Defaults to SupportClaw main. */
  ...
  const promptSurface = params.promptSurface ?? "supportClaw_main";
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running SupportClaw process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List SupportClaw agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List SupportClaw agent ids allowed for sessions_spawn",
  ...
  const renderSupportClawToolWorkflowHints = shouldRenderSupportClawToolWorkflowHints({
  ...
  const workspaceOnlyGuidance =
    params.fsWorkspaceOnly === true
      ? "tools.fs.workspaceOnly is enabled: scratch/temp/meta files that file tools must later read/write/edit must stay inside the workspace, preferably as relative paths under `.supportClaw/tmp/`. Do not write files to `/tmp/...` with exec if a later read/write/edit/apply_patch tool needs them; use `.supportClaw/tmp/...` instead."
      : "";
  const safetySection = [
    "## Safety",
  ...
  if (promptMode === "none") {
    return ["You are a personal assistant running inside SupportClaw.", modelIdentityLine]
      .filter(Boolean)
      .join("\n");
  }
  ...
  const stablePrefixCacheKey = hashStablePromptInput({
    ...
    renderSupportClawToolWorkflowHints,
    hasGateway,
    readToolName,
    execToolName,
    processToolName,
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      ...
      "CLI lifecycle only on explicit user request: `supportClaw gateway status|restart|start|stop`.",
      "`restart`, not stop+start.",
      "",
      ...skillsSection,
      ...skillWorkshopSection,
      ...memorySection,
      hasGateway && !isMinimal ? "## SupportClaw Self-Update" : "",
      hasGateway && !isMinimal
        ? [
            ...
            "After restart, SupportClaw pings the last active session automatically.",
          ].join("\n")
        : "",
      hasGateway && !isMinimal ? "" : "",
      "",
      ...
      "These user-editable files are loaded by SupportClaw and included below in Project Context.",
      "",
