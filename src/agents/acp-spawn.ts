...
} from "@supportclaw/acp-core/runtime/session-identifiers";
import type { AcpRuntimeSessionMode } from "@supportclaw/acp-core/runtime/types";
import {
...
function resolveHeartbeatConfigForAgent(params: {
  cfg: SupportClawConfig;
  agentId: string;
}): NonNullable<NonNullable<SupportClawConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an SupportClaw config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for SupportClaw config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: SupportClawConfig, agentId: string): boolean {
...
function resolveConfiguredAcpSubagentTargetIds(cfg: SupportClawConfig): string[] {
