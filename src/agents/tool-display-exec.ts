 ...
 */
import { asOptionalObjectRecord as asRecord } from "@supportclaw/normalization-core/record-coerce";
...
function summarizeKnownExec(words: string[]): string {
  ...
  if (bin === "supportClaw") {
    ...
    return sub ? `run supportClaw ${sub}` : "run supportClaw";
  }
...
function classifyWorkspacePath(
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    if (segment === ".supportClaw" && segments[index + 1] === "workspace") {
      return "agent";
    }
    if (segment === ".supportClaw" && segments[index + 1] === "sandboxes") {
      return "sandbox";
    }
