 ...
 *
 * @see https://github.com/brahminib/support-claw/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /supportClaw\.json|supportClaw gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify supportClaw.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
