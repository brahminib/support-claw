...
async function resolveContext(config: SupportClawConfig, sessionKey: string, workspaceDir: string) {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): SupportClawConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
    ...
    const cfg: SupportClawConfig = {
      agents: {
        ...
        list: [
          {
            id: "isolated",
            workspace: "~/supportClaw-isolated",
            sandbox: {
              mode: "all",
              scope: "agent",
