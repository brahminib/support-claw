...
describe("subscribeEmbeddedAgentSession", () => {
  ...
  async function captureToolLifecycleLogSubsystems(messageChannel?: string): Promise<string[]> {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-tool-log-attribution-"));
    const logFile = path.join(tempDir, "supportClaw.log");
    try {
      setLoggerOverride({
        level: "debug",
        consoleLevel: "silent",
