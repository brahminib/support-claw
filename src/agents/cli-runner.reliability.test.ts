...
const hookRunnerGlobalStateKey = Symbol.for("supportClaw.plugins.hook-runner-global-state");
...
function createSessionFile(params?: { history?: Array<{ role: "user"; content: string }> }) {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-cli-hooks-"));
...
function buildPreparedContext(params?: {
  ...
  runId?: string;
  lane?: string;
  supportClawHistoryPrompt?: string;
  provider?: string;
  model?: string;
  ...
  return {
    ...
    ...(params?.supportClawHistoryPrompt
      ? { supportClawHistoryPrompt: params.supportClawHistoryPrompt }
      : {}),
    authEpochVersion: 2,
  };
}
...
const CLI_RESEED_PROMPT =
  "Continue this conversation using the SupportClaw transcript below as prior session history.\n\n<conversation_history>\nUser: earlier context\n</conversation_history>\n\n<next_user_message>\nhi\n</next_user_message>";
...
describe("runCliAgent reliability", () => {
  ...
  it("preserves fresh retry for direct CLI callers without a pre-clear hook", async () => {
    ...
    const context = buildPreparedContext({
      ...
      provider: "claude-cli",
      model: "opus",
      supportClawHistoryPrompt: CLI_RESEED_PROMPT,
    });
  ...
  it("does not retry or fail over after a confirmed message send", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      const captureKey = input.env?.SUPPORT_CLAW_MCP_CLI_CAPTURE_KEY ?? "";
  ...
  it("preserves first-turn delivery through cleanup without binding the SupportClaw session id", async () => {
    supervisorSpawnMock.mockClear();
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      const input = args[0] as Parameters<ReturnType<typeof getProcessSupervisor>["spawn"]>[0];
      const captureHandle = markMcpLoopbackToolCallStarted({
  ...
  it("bounds admitted requests that have not finished uploading", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      const captureHandle = markMcpLoopbackRequestStarted(
        input.env?.SUPPORT_CLAW_MCP_CLI_CAPTURE_KEY ?? "",
      );
      if (!captureHandle) {
  ...
  it("marks CLI runs as paused after sessions_yield", async () => {
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      const input = args[0] as Parameters<ReturnType<typeof getProcessSupervisor>["spawn"]>[0];
      const captureHandle = markMcpLoopbackRequestStarted(input.env?.SUPPORT_CLAW_MCP_CLI_CAPTURE_KEY);
  ...
  it("seeds fresh CLI sessions from the SupportClaw transcript", async () => {
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        reason: "exit",
        exitCode: 0,
    ...
    const result = await runPreparedCliAgent(
      buildPreparedContext({
        supportClawHistoryPrompt:
          "Continue this conversation using the SupportClaw transcript below.\n\nUser: earlier ask\n\nAssistant: earlier answer\n\n<next_user_message>\nhi\n</next_user_message>",
      }),
    );
  ...
  it("keeps resumed CLI sessions on native resume history", async () => {
    ...
    const result = await runPreparedCliAgent(
      buildPreparedContext({
        ...
        supportClawHistoryPrompt: "User: earlier ask",
      }),
    );
  ...
  it("passes cwd to approved CLI user-turn persistence", async () => {
    ...
    const { dir, sessionFile } = createSessionFile();
    const taskDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-cli-persist-cwd-"));
  ...
  it("does not execute the CLI when approved user turn persistence fails", async () => {
    supervisorSpawnMock.mockClear();
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-cli-persist-fail-"));
  ...
  it("blocks CLI runs before llm_input and model execution when before_agent_run blocks", async () => {
    ...
    try {
      ...
      expect(blockedLine.message["__supportClaw"].beforeAgentRunBlocked.blockedBy).toBe(
        "policy-plugin",
      );
      expect(blockedLine.message["__supportClaw"].beforeAgentRunBlocked).not.toHaveProperty("reason");
      expect(Object.hasOwn(blockedLine.message["__supportClaw"], "beforeAgentRunBlocked")).toBe(true);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("builds fresh-session history reseed prompts from hook-mutated prompts", async () => {
    ...
    try {
      ...
      expect(context.supportClawHistoryPrompt).toContain("Compaction summary: compacted earlier ask");
      expect(context.supportClawHistoryPrompt).toContain("hook context");
      expect(context.supportClawHistoryPrompt).toContain("current ask");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
