/**
 * Broad coverage for createSupportClawCodingTools.
...
import "./test-helpers/fast-supportClaw-tools.js";
import { wrapToolWithBeforeToolCallHook } from "./agent-tools.before-tool-call.js";
import { createSupportClawCodingTools } from "./agent-tools.js";
import type { AuthProfileStore } from "./auth-profiles/types.js";
import * as supportClawPluginTools from "./supportClaw-plugin-tools.js";
...
function createToolsForStoredSession(storeTemplate: string, sessionKey: string) {
  return createSupportClawCodingTools({
    sessionKey,
    config: {
      session: {
        store: storeTemplate,
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createSupportClawCodingTools>) {
...
type SupportClawCodingTool = ReturnType<typeof createSupportClawCodingTools>[number];
type SupportClawToolsOptions = NonNullable<Parameters<typeof createSupportClawTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: SupportClawCodingTool[], name: string): SupportClawCodingTool {
...
function requireToolExecute(tool: SupportClawCodingTool): NonNullable<SupportClawCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateSupportClawToolsOptions(): SupportClawToolsOptions {
  const calls = vi.mocked(createSupportClawTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createSupportClawTools call");
  }
  return options;
}
...
function cronCreatorToolNames(
  list: SupportClawToolsOptions["cronCreatorToolAllowlist"] | undefined,
): string[] | undefined {
  return list?.map((entry) => (typeof entry === "string" ? entry : entry.name));
}

describe("createSupportClawCodingTools", () => {
  const testConfig: SupportClawConfig = {};
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createSupportClawCodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-hook-channel-"));
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createSupportClawTools).mockReturnValueOnce([wrapped as never]);

    const tools = createSupportClawCodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core SupportClaw tools are not materialized", () => {
    const createSupportClawToolsMock = vi.mocked(createSupportClawTools);
    createSupportClawToolsMock.mockClear();
    ...
    expect(createSupportClawToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createSupportClawToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateSupportClawToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:supportClaw"], ["*"]]) {
      const tools = createSupportClawCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to SupportClaw tool construction", () => {
    const createSupportClawToolsMock = vi.mocked(createSupportClawTools);
    createSupportClawToolsMock.mockClear();
    ...
    expect(createSupportClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateSupportClawToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into SupportClaw tool construction plans", () => {
    const createSupportClawToolsMock = vi.mocked(createSupportClawTools);
    createSupportClawToolsMock.mockClear();
    ...
    expect(createSupportClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateSupportClawToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the SupportClaw core factory", () => {
    const createSupportClawToolsMock = vi.mocked(createSupportClawTools);
    createSupportClawToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(supportClawPluginTools, "resolveSupportClawPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createSupportClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateSupportClawToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to SupportClaw tool factory planning", () => {
    const createSupportClawToolsMock = vi.mocked(createSupportClawTools);
    createSupportClawToolsMock.mockClear();
    ...
    expect(createSupportClawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateSupportClawToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to SupportClaw plugin discovery", async () => {
    const createSupportClawToolsMock = vi.mocked(createSupportClawTools);
    createSupportClawToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `supportClaw-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createSupportClawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateSupportClawToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createSupportClawToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateSupportClawToolsOptions().inheritedToolAllowlist;
  ...
  it("passes group-restricted tool surface to cron-created agent turns", () => {
    ...
    expect(createSupportClawToolsMock).toHaveBeenCalledTimes(1);
    const cronAllow = latestCreateSupportClawToolsOptions().cronCreatorToolAllowlist;
  ...
  it("lets embedded attempts refresh a caller-owned cron creator tool surface", () => {
    ...
    const cronCreatorToolAllowlistRef: NonNullable<
      SupportClawToolsOptions["cronCreatorToolAllowlist"]
    > = [];
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("supportClaw-tools:test-helper")).toBeLessThan(
      stages.indexOf("supportClaw-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createSupportClawCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createSupportClawCodingTools({
      config: testConfig,
    ...
    const codexTools = createSupportClawCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: SupportClawConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createSupportClawCodingTools({
    ...
    const anthropicTools = createSupportClawCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: SupportClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createSupportClawCodingTools({
    ...
    const denied = createSupportClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createSupportClawCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createSupportClawCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    vi.mocked(createSupportClawTools).mockClear();
    ...
    expect(latestCreateSupportClawToolsOptions().agentChannel).toBe("discord");
  });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createSupportClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createSupportClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createSupportClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createSupportClawCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createSupportClawCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createSupportClawCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createSupportClawCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createSupportClawCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createSupportClawCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-read-"));
    try {
      ...
      const contents = "Hello from supportClaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "supportClaw-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "supportClaw-workspace"),
    ...
    const tools = createSupportClawCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-canonical-"));
    try {
      const tools = createSupportClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-legacy-alias-"));
    try {
      const tools = createSupportClawCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-structured-write-"));
    try {
      const tools = createSupportClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-structured-edit-"));
    try {
