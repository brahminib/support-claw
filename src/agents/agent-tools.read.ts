...
type SupportClawReadToolOptions = {
...
function resolveAdaptiveReadMaxBytes(options?: SupportClawReadToolOptions): number {
...
function withWorkspaceSafeTempHint(error: unknown): unknown {
  ...
  const message = error.message.includes(".supportClaw/tmp/")
    ? error.message
    : `${error.message}. Use a relative path under \`.supportClaw/tmp/\` inside the workspace for scratch/temp/meta files that file tools need to read or write later.`;
...
/** Create a sandbox-backed read tool with SupportClaw result normalization. */
export function createSandboxedReadTool(params: SandboxToolParams) {
  const base = createReadTool(params.root, {
    operations: createSandboxReadOperations(params),
  }) as unknown as AnyAgentTool;
  return createSupportClawReadTool(base, {
...
/** Wrap the base read tool with SupportClaw paging, MIME, and image handling. */
export function createSupportClawReadTool(
  base: AnyAgentTool,
  options?: SupportClawReadToolOptions,
): AnyAgentTool {
  return {
    ...base,
