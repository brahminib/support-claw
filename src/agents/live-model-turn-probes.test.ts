...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "SUPPORT_CLAW_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { SUPPORT_CLAW_LIVE_MODEL_IMAGE_PROBE: "false" },
        "SUPPORT_CLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { SUPPORT_CLAW_LIVE_MODEL_IMAGE_PROBE: "1" },
        "SUPPORT_CLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
