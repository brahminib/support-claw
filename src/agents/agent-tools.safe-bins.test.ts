...
let createSupportClawCodingTools: typeof import("./agent-tools.js").createSupportClawCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "supportClaw-test-no-bundled-extensions"),
    },
    async () => {
      ({ createSupportClawCodingTools } = await import("./agent-tools.js"));
    },
  );
});
...
vi.mock("supportClaw/plugin-sdk/agent-sessions", () => ({
  AuthStorage: vi.fn(),
...
async function withSafeBinsExecTool(
  ...
  try {
    await withEnvAsync(
      {
        SUPPORT_CLAW_SHELL_ENV_TIMEOUT_MS: "1",
        PATH: "/usr/bin:/bin",
        SHELL: "/bin/sh",
      },
      async () => {
...
describe("createSupportClawCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "supportClaw-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "supportClaw-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "supportClaw-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "supportClaw-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "supportClaw-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "supportClaw-safe-bins-grep-",
        safeBins: ["grep"],
