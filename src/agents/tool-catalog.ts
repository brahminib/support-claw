...
type CoreToolDefinition = {
  ...
  includeInSupportClawGroup?: boolean;
};
...
const CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  ...
  },
  {
    ...
    sectionId: "runtime",
    profiles: ["coding"],
    includeInSupportClawGroup: true,
  },
  {
    id: "web_search",
    label: "web_search",
...
function buildCoreToolGroupMap() {
  ...
  const supportClawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInSupportClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:supportClaw": supportClawTools,
