...
describe("isSilentOverflowProneModel", () => {
  ...
  // supportClaw#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable SupportClaw runtime's secondary compaction path;
  // SupportClaw's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of supportClaw#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering SupportClaw runtime's
  ...
  // state.messages intact; SupportClaw's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep SupportClaw runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove SupportClaw runtime's
