...
async function withTempExecApprovalsFile(
  ...
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-cli-exec-approvals-"));
...
async function withTempSupportClawHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-cli-home-"));
  try {
    await withEnvAsync({ SUPPORT_CLAW_HOME: home }, async () => run(home));
  } finally {
    await fs.rm(home, { recursive: true, force: true });
  }
}
...
describe("runCliAgent spawn path", () => {
  ...
  it("includes the SupportClaw skills prompt in CLI system prompts", () => {
  ...
  it("passes Claude system prompts through a file instead of argv", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(systemPromptPath).toContain("supportClaw-cli-system-prompt-");
  ...
  it("passes prepared backend env to the spawned CLI process", async () => {
    ...
    await executePreparedCliRun(
      buildPreparedCliRunContext({
        ...
        preparedEnv: {
          GEMINI_CLI_HOME: "/tmp/supportClaw-gemini-profile-home",
          GEMINI_CLI_SYSTEM_SETTINGS_PATH: "/tmp/supportClaw-gemini-system-settings.json",
        },
      }),
    );
    ...
    expect(input.env?.GEMINI_CLI_HOME).toBe("/tmp/supportClaw-gemini-profile-home");
    expect(input.env?.GEMINI_CLI_SYSTEM_SETTINGS_PATH).toBe(
      "/tmp/supportClaw-gemini-system-settings.json",
    );
  });

  it("passes SupportClaw skills to Claude as a session plugin", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-cli-skills-"));
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(manifest.name).toBe("supportClaw-skills");
  ...
  it("keeps captured live prepared backend cleanup with the whole-run owner", async () => {
    const mcpConfigDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "supportClaw-cli-captured-mcp-config-"),
    );
  ...
  it("preserves completed output when system prompt cleanup fails after delivery", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      const captureHandle = markMcpLoopbackToolCallStarted({
        captureKey: input.env?.SUPPORT_CLAW_MCP_CLI_CAPTURE_KEY ?? "",
        toolName: "message",
  ...
  it("preserves Claude resume args when building live session argv", () => {
    ...
    expect(args).not.toContain("supportClaw-session");
...
`,
  ...
  it("does not create exec approvals file while resolving Claude live policy", async () => {
    await withTempSupportClawHome(async (home) => {
      const approvalsPath = path.join(home, ".supportClaw", "exec-approvals.json");
      let stdoutListener: ((chunk: string) => void) | undefined;
      const stdin = {
...
`,
  ...
  it("answers Claude live control_request can_use_tool with allow when SupportClaw exec is YOLO despite raw --permission-mode default", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    const writes: string[] = [];
    const stdin = {
      write: vi.fn((data: string, cb?: (err?: Error | null) => void) => {
...
`,
    ...
    // and SupportClaw policy is authoritative over raw Claude permission-mode
  ...
  it("restarts Claude live sessions when selected skills change", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-live-skills-"));
  ...
  it("can preserve selected clearEnv keys for live CLI backend probes", async () => {
    try {
      process.env.SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV = '["SAFE_CLEAR"]';
      process.env.SAFE_CLEAR = "from-base";
      mockSuccessfulCliRun();
      await executePreparedCliRun(
        buildPreparedCliRunContext({
      ...
      delete process.env.SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV;
  ...
  it("loads workspace bootstrap files into the Claude CLI system prompt", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "supportClaw-cli-bootstrap-context-"),
    );
