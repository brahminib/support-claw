...
function createClaudeCliOverrideConfig(config: CliBackendConfig): SupportClawConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from SupportClaw's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__supportClaw__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__supportClaw__*");
  ...
  it("keeps Claude permission mode unset when SupportClaw exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("claude-cli", {
