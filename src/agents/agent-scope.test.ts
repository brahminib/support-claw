...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: SupportClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/supportClaw" }],
      },
    };
        ...
        list: [
          {
            ...
            agentDir: "~/.supportClaw/agents/main",
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: SupportClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: SupportClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: SupportClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: SupportClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: SupportClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: SupportClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: SupportClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: SupportClawConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: SupportClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: SupportClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: SupportClawConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/supportClaw-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/supportClaw");
  });

  it("uses SUPPORT_CLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "supportClaw-home");
    withEnv({ SUPPORT_CLAW_HOME: home }, () => {
      const workspace = resolveAgentWorkspaceDir({} as SupportClawConfig, "main");
      expect(workspace).toBe(path.join(path.resolve(home), ".supportClaw", "workspace"));
    });
  });

  it("uses SUPPORT_CLAW_WORKSPACE_DIR for default agent workspace", () => {
  ...
  it("uses SUPPORT_CLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "supportClaw-home");
    withEnv({ SUPPORT_CLAW_HOME: home, SUPPORT_CLAW_STATE_DIR: "" }, () => {
      const agentDir = resolveAgentDir({} as SupportClawConfig, "main");
      expect(agentDir).toBe(path.join(path.resolve(home), ".supportClaw", "agents", "main", "agent"));
    });
  });
  ...
  it("resolves default agentDir from the configured default agent", () => {
    ...
    const agentDir = withEnv({ SUPPORT_CLAW_STATE_DIR: stateDir }, () => resolveDefaultAgentDir(cfg));
  ...
  it("non-default agent without defaults.workspace falls back to stateDir", () => {
    ...
    const workspace = withEnv({ SUPPORT_CLAW_STATE_DIR: stateDir }, () =>
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/supportClaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/supportClaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-agent-scope-"));
