...
function policyDeniesTool(policy: { deny?: string[] } | undefined, toolName: string): boolean {
  return (
    ...
    listIncludesTool(policy?.deny, "group:supportClaw")
  );
}

function hasExplicitBrowserIntent(cfg: SupportClawConfig): boolean {
...
export function resolveEffectiveToolInventory(
  ...
  const effectiveTools = createSupportClawCodingTools({
    agentId,
