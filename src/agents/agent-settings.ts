...
function resolveCompactionReserveTokensFloor(cfg?: SupportClawConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: SupportClawConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers SupportClaw runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (supportClaw#75799).
 *
...
/**
 * Disable SupportClaw runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when SupportClaw or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (supportClaw#75799).
 * Default-mode runs against ordinary providers keep SupportClaw runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores SupportClaw runtime's
