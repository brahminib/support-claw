/**
 * Builds the effective SupportClaw agent tool surface.
 * Assembles core, shell, channel, SupportClaw, plugin, and Tool Search tools, then
...
function resolveExecConfig(params: { cfg?: SupportClawConfig; agentId?: string }) {
...
export type SupportClawCodingToolConstructionPlan = {
  includeBaseCodingTools: boolean;
  includeShellTools: boolean;
  includeChannelTools: boolean;
  includeSupportClawTools: boolean;
  includePluginTools: boolean;
};

/** Build the runtime tool list for one agent run. */
export function createSupportClawCodingTools(options?: {
  agentId?: string;
  ...
  modelCompat?: ModelCompatConfig;
  /** If false, keep SupportClaw web_search even when a provider-native search tool is active. */
  suppressManagedWebSearch?: boolean;
  /**
  ...
  toolConstructionPlan?: SupportClawCodingToolConstructionPlan;
  ...
  const toolConstructionPlan = options?.toolConstructionPlan ?? {
    ...
    includeSupportClawTools: includeCoreTools,
  ...
  const includeSupportClawTools = includeCoreTools && toolConstructionPlan.includeSupportClawTools;
  ...
  if (includeBaseCodingTools) {
    for (const tool of createCodingTools(codingRoot) as unknown as AnyAgentTool[]) {
      if (tool.name === "read") {
        ...
        const wrapped = createSupportClawReadTool(freshReadTool, {
          modelContextWindowTokens: options?.modelContextWindowTokens,
          imageSanitization,
        });
        base.push(
  ...
  const pluginToolsOnly =
    includeSupportClawTools || !includePluginTools
      ? []
  ...
  options?.recordToolPrepStage?.("supportClaw-tools");
  ...
  // Always normalize tool JSON Schemas before handing them to SupportClaw model runtime.
