...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to SupportClaw transport aliases when needed.
    const cases = [
      {
        ...
        alias: "supportClaw-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "supportClaw-openai-completions-transport",
      },
      {
        ...
        alias: "supportClaw-azure-openai-responses-transport",
      },
      {
        ...
        alias: "supportClaw-anthropic-messages-transport",
      },
      {
        ...
        alias: "supportClaw-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on SupportClaw transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createSupportClawTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the SupportClaw simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("supportClaw-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the SupportClaw transport until SupportClaw preserves attribution", () => {
