...
let testState: SupportClawTestState | undefined;

beforeEach(async () => {
  testState = await createSupportClawTestState({
    layout: "state-only",
    prefix: "supportClaw-workspace-state-",
  });
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SUPPORT_CLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SUPPORT_CLAW_HOME: "/srv/supportClaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/supportClaw-home"), ".supportClaw", "workspace"));
  });

  it("prefers SUPPORT_CLAW_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SUPPORT_CLAW_WORKSPACE_DIR: "/srv/supportClaw-workspace",
      SUPPORT_CLAW_HOME: "/srv/supportClaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/supportClaw-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = ["supportClaw-workspace-state.json"] as const;
const LEGACY_WORKSPACE_STATE_PATH_SEGMENTS = [".supportClaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("supportClaw-workspace-");
  ...
  it("does not overwrite a foreign root workspace-state.json file", async () => {
    ...
    const foreignState = "not supportClaw state\n";
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".supportClaw"), { recursive: true });
  ...
  it("does not overwrite a sibling file that is not an SupportClaw attestation marker", async () => {
    const tempDir = await makeTempWorkspace("supportClaw-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-workspace-hardlink-"));
    try {
