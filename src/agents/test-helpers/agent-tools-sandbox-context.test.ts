...
describe("createAgentToolsSandboxContext", () => {
  it("provides stable defaults for agent-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("supportClaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("supportClaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("supportClaw-sbx-");
