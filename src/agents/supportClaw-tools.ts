/**
 * SupportClaw built-in and plugin tool assembly.
 *
...
import { resolveSupportClawPluginToolsForOptions } from "./supportClaw-plugin-tools.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./supportClaw-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./supportClaw-tools.nodes-workspace-guard.js";
import {
  collectPresentSupportClawTools,
  shouldIncludeUpdatePlanToolForSupportClawTools,
} from "./supportClaw-tools.registration.js";
...
type SupportClawToolsDeps = {
  callGateway: typeof callGateway;
  config?: SupportClawConfig;
};

const defaultSupportClawToolsDeps: SupportClawToolsDeps = {
  callGateway,
};

let supportClawToolsDeps: SupportClawToolsDeps = defaultSupportClawToolsDeps;

export function createSupportClawTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? supportClawToolsDeps.config;
  ...
  options?.recordToolPrepStage?.("supportClaw-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("supportClaw-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("supportClaw-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("supportClaw-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("supportClaw-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("supportClaw-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("supportClaw-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("supportClaw-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("supportClaw-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("supportClaw-tools:nodes-tool");
  ...
  const effectiveCallGateway = embedded
    ...
    : supportClawToolsDeps.callGateway;
  const includeUpdatePlanTool = shouldIncludeUpdatePlanToolForSupportClawTools({
    config: resolvedConfig,
    agentSessionKey: options?.agentSessionKey,
    agentId: options?.requesterAgentIdOverride,
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentSupportClawTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentSupportClawTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...(embedded
      ...
      : [
          createSessionsSendTool({
            ...
            sandboxed: options?.sandboxed,
            config: resolvedConfig,
            callGateway: supportClawToolsDeps.callGateway,
          }),
        ]),
    ...
    ...collectPresentSupportClawTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("supportClaw-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    allTools = [
      ...tools,
      ...resolveSupportClawPluginToolsForOptions({
        options,
        resolvedConfig,
        existingToolNames,
      }),
    ];
    options?.recordToolPrepStage?.("supportClaw-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("supportClaw-tools:tool-hooks");
...
export const testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<SupportClawToolsDeps>) {
    supportClawToolsDeps = overrides
      ? {
          ...defaultSupportClawToolsDeps,
          ...overrides,
        }
      : defaultSupportClawToolsDeps;
  },
