...
export function resolveModelRuntimePolicy(params: {
  ...
  if (process.env.SUPPORT_CLAW_BUILD_PRIVATE_QA === "1") {
    const forcedRuntime = process.env.SUPPORT_CLAW_QA_FORCE_RUNTIME?.trim().toLowerCase();
    if (forcedRuntime === "supportClaw" || forcedRuntime === "codex") {
