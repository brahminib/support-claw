...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousSupportClawAgentDir: string | undefined;
  const originalFetch = globalThis.fetch;
  const shouldResetPluginLoaderState = opts?.resetPluginLoaderState !== false;

  beforeEach(() => {
    previousHome = process.env.HOME;
    previousSupportClawAgentDir = process.env.SUPPORT_CLAW_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousSupportClawAgentDir === undefined) {
      delete process.env.SUPPORT_CLAW_AGENT_DIR;
    } else {
      process.env.SUPPORT_CLAW_AGENT_DIR = previousSupportClawAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "SUPPORT_CLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: SupportClawConfig = {
  models: {
    providers: {
      "custom-proxy": {
