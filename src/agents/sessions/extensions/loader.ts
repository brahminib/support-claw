...
import * as bundledLlm from "supportClaw/plugin-sdk/llm";
...
import { installSupportClawInternalCorePackageNativeResolver } from "../../../plugins/plugin-sdk-native-resolver.js";
import {
...
const VIRTUAL_MODULES: Record<string, unknown> = {
  ...
  "supportClaw/plugin-sdk/agent-core": bundledAgentCore,
  "@supportclaw/plugin-sdk/agent-core": bundledAgentCore,
  "supportClaw/plugin-sdk/llm": bundledLlm,
  "@supportclaw/plugin-sdk/llm": bundledLlm,
  "supportClaw/plugin-sdk/agent-sessions": bundledAgentSessions,
  "@supportclaw/plugin-sdk/agent-sessions": bundledAgentSessions,
};
...
const EXTENSION_LOADER_ALIAS_IMPORT_PATTERN =
  /(?:@supportclaw\/plugin-sdk|supportClaw\/plugin-sdk|@sinclair\/typebox|typebox)(?:\/[A-Za-z0-9_-]+)?/u;
...
function getExtensionLoaderAliases(): Record<string, string> {
  ...
  aliases = {
    ...
    "supportClaw/plugin-sdk/agent-sessions": agentSessionsEntry,
    "@supportclaw/plugin-sdk/agent-sessions": agentSessionsEntry,
...
async function loadExtensionSourceTransformModule(
  ...
  if (!extensionSourceTransformLoader) {
    installSupportClawInternalCorePackageNativeResolver({ moduleUrl: import.meta.url });
...
function readResourceManifest(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    if (pkg.supportClaw && typeof pkg.supportClaw === "object") {
      return pkg.supportClaw as ResourceManifest;
    }
    return null;
  } catch {
    return null;
 ...
 *
 * Checks for:
 * 1. package.json with "supportClaw.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
  // Check for package.json with "supportClaw" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "supportClaw" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
  ...
  for (const p of configuredPaths) {
    ...
    if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
      // Check for package.json with SupportClaw manifest or index.ts
