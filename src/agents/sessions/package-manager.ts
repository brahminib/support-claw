...
type SkillDiscoveryMode = "supportClaw" | "agents";
...
function readResourceManifestFile(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    const pkg = JSON.parse(content) as { supportClaw?: ResourceManifest };
    return pkg.supportClaw ?? null;
  } catch {
    return null;
  }
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
  if (resourceType === "skills") {
    return collectSkillEntries(dir, "supportClaw");
  }
...
export class DefaultPackageManager implements PackageManager {
  ...
  private getTemporaryDir(prefix: string, suffix?: string): string {
    const hash = createHash("sha256")
      ...
      .digest("hex")
      .slice(0, 8);
    return join(tmpdir(), "supportClaw-resources", prefix, hash, suffix ?? "");
  }
  ...
  private addAutoDiscoveredResources(
    ...
    addResources(
      "skills",
      collectAutoSkillEntries(projectDirs.skills, "supportClaw"),
      projectMetadata,
    ...
    // User extensions from ~/.supportClaw/agent/
    addResources(
      "extensions",
    ...
    // User skills from ~/.supportClaw/agent/
    addResources(
      "skills",
      collectAutoSkillEntries(userDirs.skills, "supportClaw"),
      userMetadata,
