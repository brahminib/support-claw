...
describe("read tool", () => {
  ...
  it("reads managed inbound media refs as image files", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-read-media-"));
  ...
  it("uses the shared Windows decoder for local filesystem reads", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-read-encoding-"));
