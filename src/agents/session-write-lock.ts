...
const CLEANUP_STATE_KEY = Symbol.for("supportClaw.sessionWriteLockCleanupState");
const WATCHDOG_STATE_KEY = Symbol.for("supportClaw.sessionWriteLockWatchdogState");
...
const SESSION_LOCKS = createFileLockManager("supportClaw.session-write-lock");
...
const SESSION_WRITE_LOCK_ENV: Record<SessionWriteLockMsKey, string> = {
  acquireTimeoutMs: "SUPPORT_CLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS",
  staleMs: "SUPPORT_CLAW_SESSION_WRITE_LOCK_STALE_MS",
  maxHoldMs: "SUPPORT_CLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS",
};
...
function shouldStartBackgroundWatchdog(): boolean {
  return process.env.VITEST !== "true" || process.env.SUPPORT_CLAW_TEST_SESSION_LOCK_WATCHDOG === "1";
}
...
function isSupportClawSessionOwnerArgv(args: string[]): boolean {
  const normalized = args.map(normalizeOwnerProcessArg).filter(Boolean);
  if (normalized.length === 0) {
    return false;
  }
  const exe = (normalized[0] ?? "").replace(/\.(bat|cmd|exe)$/i, "");
  if (exe === "supportClaw" || exe.endsWith("/supportClaw") || exe.endsWith("/supportClaw-gateway")) {
    return true;
  }
  if (
    normalized.some(
      (arg) =>
        arg === "supportClaw" ||
        arg.endsWith("/supportClaw") ||
        arg === "supportClawCli.mjs" ||
        arg.endsWith("/supportClawCli.mjs"),
    )
  ) {
    return true;
  }
  ...
  const hasSupportClawCommandToken = normalized.some((arg) => arg === "gateway" || arg === "agent");
  return normalized.some(
    (arg) => entryCandidates.some((entry) => arg.endsWith(entry)) && hasSupportClawCommandToken,
  );
}
...
function shouldTreatAsNonSupportClawOwner(params: {
  payload: LockFilePayload | null;
  ...
  return !isSupportClawSessionOwnerArgv(args);
}
...
function inspectLockPayloadForSession(params: {
  ...
  if (
    shouldTreatAsNonSupportClawOwner({
      payload: params.payload,
      inspected,
      heldByThisProcess: params.heldByThisProcess,
    ...
    return {
      ...inspected,
      stale: true,
      staleReasons: [...inspected.staleReasons, "non-supportClaw-owner"],
    };
  }
