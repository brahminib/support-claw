...
import { isInboundPathAllowed } from "@supportclaw/media-core/inbound-path-policy";
...
type CreateSupportClawCodingToolsArgs = Parameters<typeof createSupportClawCodingTools>[0];
type MockSupportClawToolsOptions = {
...
vi.mock("../auth-profiles.js", () => ({
  ...
  ensureAuthProfileStore: (agentDir?: string) => {
    ...
    if (process.env.SUPPORT_CLAW_TEST_CODEX_CLI_OAUTH === "1") {
...
vi.mock("../auth-profiles/external-cli-sync.js", () => ({
  resolveExternalCliAuthProfiles: (
    ...
    if (
      process.env.SUPPORT_CLAW_TEST_CODEX_CLI_OAUTH !== "1" ||
...
vi.mock("../model-auth.js", () => ({
  ...
  hasRuntimeAvailableProviderAuth: (params: {
    ...
    if (params.provider === "codex") {
      return process.env.SUPPORT_CLAW_TEST_CODEX_ROUTE === "1";
    }
  ...
  hasUsableCustomProviderApiKey: (cfg?: SupportClawConfig, provider?: string) => {
...
vi.mock("../supportClaw-tools.js", async () => {
  const { createImageTool: createImageToolLocal } = await import("./image-tool.js");
  return {
    createSupportClawTools: vi.fn((options?: MockSupportClawToolsOptions) => {
...
async function createSupportClawCodingToolsWithFreshModules(options?: CreateSupportClawCodingToolsArgs) {
  ...
  return createSupportClawCodingTools(options);
}

async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-image-"));
  try {
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(parentDir, "supportClaw-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): SupportClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempSandboxState(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-image-sandbox-"));
...
describe("image tool implicit imageModel config", () => {
  ...
  registerImageToolEnvReset(priorFetch, [
    ...
    "ZAI_API_KEY",
    "Z_AI_API_KEY",
    "SUPPORT_CLAW_TEST_CODEX_CLI_OAUTH",
    "SUPPORT_CLAW_TEST_CODEX_ROUTE",
  ...
  it.each(implicitImageRoutingCases)(
    ...
    async ({ cfg, profiles, codexRoute, openAiApiKey, expected }) => {
      if (codexRoute) {
        vi.stubEnv("SUPPORT_CLAW_TEST_CODEX_ROUTE", "1");
      }
      if (openAiApiKey) {
  ...
  it("lets external CLI Codex OAuth survive the candidate auth filter", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("SUPPORT_CLAW_TEST_CODEX_CLI_OAUTH", "1");
      vi.stubEnv("SUPPORT_CLAW_TEST_CODEX_ROUTE", "1");
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-outside-"));
  ...
  it("still rejects non-workspace local image paths when workspaceOnly is disabled", async () => {
    ...
    await withTempAgentDir(async (agentDir) => {
      const cfg = createMinimaxImageConfig();
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-image-outside-"));
      const outsideImage = path.join(outsideDir, "secret.png");
      await fs.writeFile(outsideImage, Buffer.from(ONE_PIXEL_PNG_B64, "base64"));
      try {
        const tool = createRequiredImageTool({
  ...
  it("allows image paths from the current iMessage account attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-imessage-root-"));
  ...
  it("allows image paths from current iMessage wildcard attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRootParent = await fs.mkdtemp(
        path.join(os.tmpdir(), "supportClaw-imessage-wildcard-root-"),
      );
  ...
  it("allows workspace images via createSupportClawCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createSupportClawCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.supportClaw/media/inbound/photo.png",
      });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg = createMinimaxImageConfig();
    const tool = createRequiredImageTool({ config: cfg, agentDir });
    return { fetch, tool };
...
describe("image tool managed inbound media", () => {
  ...
  async function withManagedInboundPng(
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-image-managed-inbound-"));
