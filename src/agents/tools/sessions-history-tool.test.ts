...
describe("sessions_history redaction", () => {
  beforeAll(async () => {
    previousConfigPath = process.env.SUPPORT_CLAW_CONFIG_PATH;
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-sessions-history-redact-"));
  ...
  afterAll(() => {
    if (previousConfigPath === undefined) {
      ...
      process.env.SUPPORT_CLAW_CONFIG_PATH = previousConfigPath;
    }
    if (tempDir) {
