...
async function createStoreConfig(): Promise<{ config: SupportClawConfig; template: string }> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-goal-tools-"));
  const template = path.join(dir, "{agentId}", "sessions.json");
  return {
    config: { session: { store: template } } as SupportClawConfig,
    template,
  };
}
