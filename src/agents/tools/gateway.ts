...
function resolveLocalGatewayUrlKeys(cfg: SupportClawConfig): Set<string> {
...
function resolveConfiguredRemoteGatewayKey(cfg: SupportClawConfig): string | undefined {
...
export function resolveGatewayOptions(opts?: GatewayCallOptions) {
  ...
  const timeoutMs =
    typeof opts?.timeoutMs === "number" && Number.isFinite(opts.timeoutMs)
      ...
      : 30_000;
  const envGatewayUrl = trimToUndefined(process.env.SUPPORT_CLAW_GATEWAY_URL);
  const target =
  ...
  try {
    ...
    throw new Error(
      [
        ...
        "Fix the SupportClaw state directory permissions or use the local approval-runtime gateway.",
      ].join(" "),
      { cause: error },
    );
  }
