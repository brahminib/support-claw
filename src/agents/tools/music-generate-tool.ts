...
function hasExplicitMusicGenerationModelConfig(cfg?: SupportClawConfig): boolean {
...
async function executeMusicGenerationJob(params: {
  effectiveCfg: SupportClawConfig;
  prompt: string;
  agentDir?: string;
  model?: string;
  lyrics?: string;
...
export function createMusicGenerateTool(options?: {
  ...
  const cfg: SupportClawConfig = options?.config ?? getRuntimeConfig();
  if (
    !hasGenerationToolAvailability({
      cfg,
