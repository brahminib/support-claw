...
describe("resolveMediaToolLocalRoots", () => {
  it("does not widen default local roots from media sources", () => {
    const stateDir = path.join("/tmp", "supportClaw-media-tool-roots-state");
    const picturesDir =
    ...
    const roots = withEnv({ SUPPORT_CLAW_STATE_DIR: stateDir }, () =>
  ...
  it("keeps channel inbound attachment roots separate from local roots", () => {
    ...
    const accountRoot = path.join("/tmp", "supportClaw-imessage-work");
    const sharedRoot = path.join("/tmp", "supportClaw-imessage-shared");
    const cfg = {
      channels: {
        imessage: {
