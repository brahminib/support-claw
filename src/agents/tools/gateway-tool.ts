...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `supportClaw security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as SupportClawConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as SupportClawConfig);
