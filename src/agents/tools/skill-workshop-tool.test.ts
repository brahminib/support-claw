...
describe("skill_workshop tool", () => {
  it("is exposed in the SupportClaw tool set", async () => {
    const workspaceDir = await tempDirs.make("supportClaw-skill-workshop-tool-");
  ...
  it("is not exposed from sandboxed SupportClaw tool sets", async () => {
    const workspaceDir = await tempDirs.make("supportClaw-skill-workshop-tool-");
    const tools = createSupportClawTools({
      workspaceDir,
      config: {},
  ...
  it("scopes proposal discovery to the tool workspace", async () => {
    const firstWorkspaceDir = await tempDirs.make("supportClaw-skill-workshop-tool-first-");
    const secondWorkspaceDir = await tempDirs.make("supportClaw-skill-workshop-tool-second-");
