...
describe("buildAgentSystemPrompt", () => {
  it("resolves helper session keys to scoped prompt surfaces", () => {
    ...
    expect(resolveAgentPromptSurfaceForSessionKey("agent:main")).toBe("supportClaw_main");
    expect(resolveAgentPromptSurfaceForSessionKey(undefined)).toBe("supportClaw_main");
  });
  ...
  it("includes an SupportClaw control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/supportClaw",
    });

    expect(prompt).toContain("## SupportClaw Control");
    expect(prompt).toContain("prefer `gateway` tool");
    expect(prompt).toContain("CLI lifecycle only on explicit user request");
    expect(prompt).toContain("supportClaw gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__supportClaw__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("keeps the SupportClaw empty-tool fallback on the main prompt surface", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/supportClaw",
      toolNames: [],
    });

    expect(prompt).toContain("SupportClaw lists the standard tools above");
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- sessions_spawn: Spawn an isolated sub-agent session");
    expect(prompt).toContain("- agents_list: List SupportClaw agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/supportClaw/docs");
    expect(prompt).toContain(
      "Docs are authoritative for SupportClaw self-knowledge: before understanding how SupportClaw works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use `Read` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not SupportClaw design/implementation knowledge.",
    );
  });
  ...
  it("includes docs guidance when docsPath is provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/supportClaw",
      docsPath: "/tmp/supportClaw/docs",
      sourcePath: "/tmp/supportClaw",
    });

    expect(prompt).toContain("## Documentation");
    expect(prompt).toContain("Docs: /tmp/supportClaw/docs");
    expect(prompt).toContain("Source: /tmp/supportClaw");
    expect(prompt).toContain(
      "Docs are authoritative for SupportClaw self-knowledge: before understanding how SupportClaw works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use `read` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not SupportClaw design/implementation knowledge.",
    );
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.supportClaw.ai");
    expect(prompt).toContain("Source: https://github.com/brahminib/support-claw");
    expect(prompt).toContain(
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/supportClaw",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/supportClaw/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/supportClaw/BOOTSTRAP.md");
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## SupportClaw Self-Update");
  ...
  it("describes Telegram rich text only for rich Telegram runtimes", () => {
    ...
    expect(telegramPrompt).toContain("SupportClaw renders Telegram-safe rich messages");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/supportClaw. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSections", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/supportClaw/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`supportClaw ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for SupportClaw subagents");
