...
vi.mock("../plugins/manifest-metadata-scan.js", () => ({
  listSupportClawPluginManifestMetadata: () =>
...
describe("provider attribution", () => {
  it("resolves the canonical SupportClaw product and runtime version", () => {
  ...
  it("returns a documented NVIDIA attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      version: "2026.3.22",
      headers: {
        "X-BILLING-INVOKE-ORIGIN": "SupportClaw",
      },
    });
  });
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { SUPPORT_CLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(
      resolveProviderAttributionPolicy("openai", { SUPPORT_CLAW_VERSION: "2026.3.22" })?.headers,
    ).toEqual({
      originator: "supportClaw",
      version: "2026.3.22",
      "User-Agent": "supportClaw/2026.3.22",
  ...
  it("returns a hidden-spec xAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("xai", { SUPPORT_CLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "xai",
      enabledByDefault: true,
      verification: "vendor-hidden-api-spec",
      hook: "request-headers",
    ...
    expect(
      resolveProviderAttributionPolicy("xai", { SUPPORT_CLAW_VERSION: "2026.3.22" })?.headers,
    ).toEqual({
      originator: "supportClaw",
      version: "2026.3.22",
      "User-Agent": "supportClaw/2026.3.22",
  ...
  it("lists the current attribution support matrix", () => {
    ...
    expect(
      listProviderAttributionPolicies({ SUPPORT_CLAW_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
