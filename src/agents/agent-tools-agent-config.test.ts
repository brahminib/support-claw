...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-agent-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: SupportClawConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<SupportClawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<SupportClawConfig["agents"]>["list"]>[number]["tools"];
  }): SupportClawConfig {
    return {
      tools: params.tools,
      agents: {
