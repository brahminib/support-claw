...
vi.mock("../process/supervisor/index.js", () => {
  ...
  const applySegmentShellEffects = (segment: string, env: NodeJS.ProcessEnv) => {
    if (segment === 'export PATH="${SUPPORT_CLAW_PREPEND_PATH}${PATH:+:$PATH}"') {
      const prepend = env.SUPPORT_CLAW_PREPEND_PATH ?? "";
      const current = readEnvPath(env);
      writeEnvPath(env, `${prepend}${current ? `:${current}` : ""}`);
      return;
    }
    if (segment === "unset SUPPORT_CLAW_PREPEND_PATH") {
      delete env.SUPPORT_CLAW_PREPEND_PATH;
    }
  };
...
const defaultShell = isWin
  ? undefined
  : process.env.SUPPORT_CLAW_TEST_SHELL || resolveShellFromPath("bash") || process.env.SHELL || "sh";
...
const SHELL_ENV_KEYS = ["SUPPORT_CLAW_EXEC_SHELL_SNAPSHOT", "SHELL"] as const;
const PATH_SHELL_ENV_KEYS = ["SUPPORT_CLAW_EXEC_SHELL_SNAPSHOT", "PATH", "SHELL"] as const;
...
const notifyCfg = {} as SupportClawConfig;
