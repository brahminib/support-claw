...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-managed-media-"));
  try {
    return await withEnvAsync({ SUPPORT_CLAW_STATE_DIR: stateDir }, async () => {
...
async function withOutsideHardlinkInSupportClawTmp<T>(
  params: {
    supportClawTmpDir: string;
  ...
  const hardlinkPath = path.join(params.supportClawTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.supportClawTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.supportClawTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.supportClawTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const supportClawTmpDir = resolvePreferredSupportClawTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred SupportClaw tmp root",
      media: path.join(supportClawTmpDir, "image.png"),
      expected: path.join(supportClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred SupportClaw tmp root",
      media: pathToFileURL(path.join(supportClawTmpDir, "photo.png")).href,
      expected: path.join(supportClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred SupportClaw tmp root",
      media: path.join(supportClawTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(supportClawTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(supportClawTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside supportClaw tmp root",
      media: path.join(os.tmpdir(), "outside-supportClaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked SupportClaw tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(supportClawTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(supportClawTmpDir, { recursive: true });
      const symlinkPath = path.join(supportClawTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked SupportClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInSupportClawTmp(
      {
        supportClawTmpDir,
  ...
  it("rejects symlinked SupportClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInSupportClawTmp(
