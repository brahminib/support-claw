...
export async function markRestartAbortedMainSessions(params: {
  ...
  additionalCfgs?: Iterable<SupportClawConfig | undefined>;
  stateDir?: string;
  ...
  const env =
    params.stateDir === undefined
      ? process.env
      : { ...process.env, SUPPORT_CLAW_STATE_DIR: params.stateDir };
  const stateDir = resolveStateDir(env);
  const configs = [params.cfg, ...(params.additionalCfgs ?? [])].filter(
    (cfg): cfg is SupportClawConfig => Boolean(cfg),
  );
...
function buildResumeMessage(pendingFinalDeliveryText?: string | null): string {
  const base =
    ...
    "SupportClaw was waiting on tool/model work. Continue from the existing " +
