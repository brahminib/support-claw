 ...
 *
 * Presents large SupportClaw/MCP/client tool inventories through search, describe, call, and optional code-mode tools.
 */
...
type CatalogSource = "supportClaw" | "mcp" | "client";
...
function buildModelScriptSource(code) {
  return "(async (supportClaw, console) => {\n" + code + "\n})(supportClaw, console)";
}
...
function buildControllerSource() {
  ...
  return (
    ...
    "});\n" +
    "const supportClaw = Object.freeze({\n" +
    "  tools: Object.freeze({\n" +
...
async function runModelCode(code, timeoutMs) {
  ...
  Object.defineProperties(sandbox, {
    ...
    supportClaw: { value: controller.supportClaw, enumerable: true },
  });
...
const SESSION_CATALOGS_KEY = Symbol.for("supportClaw.toolSearch.sessionCatalogs");
...
function readToolSearchConfig(config?: SupportClawConfig): Record<string, unknown> {
...
export function resolveToolSearchConfig(config?: SupportClawConfig): ToolSearchConfig {
...
function classifyTool(tool: CatalogTool): {
  ...
  if (pluginId) {
    return { source: "supportClaw", sourceName: pluginId };
  }
  return { source: "supportClaw", sourceName: "core" };
}
  ...
  return [
    {
      ...
      description:
        "Run JavaScript in an isolated Node subprocess with supportClaw.tools.search, supportClaw.tools.describe, and supportClaw.tools.call for large tool catalogs.",
      parameters: Type.Object({
        code: Type.String({
          description:
            "JavaScript body for an async function. Use return to return the final value. The supportClaw.tools bridge is available.",
        }),
      }),
      execute: async (
        toolCallId: string,
    ...
    },
    {
      name: TOOL_CALL_RAW_TOOL_NAME,
      label: "Tool Call",
      description: "Call a selected Tool Search catalog entry through SupportClaw.",
      parameters: Type.Object({
        id: Type.String({ description: "Tool search result id or tool name." }),
        args: Type.Optional(
