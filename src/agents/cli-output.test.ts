    ...
    const result = parseCliJsonl(
      [
        ...
        JSON.stringify({
          type: "tool_use",
          timestamp: "2026-06-16T19:36:47.000Z",
          tool_name: "mcp_supportClaw_create_goal",
          tool_id: "tool-1",
...
describe("createCliJsonlStreamingParser", () => {
  ...
  it("streams Gemini message deltas and tool events", () => {
    ...
    expect(starts).toEqual([
      {
        toolCallId: "tool-1",
        name: "mcp_supportClaw_create_goal",
        args: { objective: "Update files" },
      },
    ]);
    expect(results).toEqual([
      { toolCallId: "tool-1", name: "mcp_supportClaw_create_goal", isError: false, result: "created" },
    ]);
  ...
  it.each(["server_tool_use", "mcp_tool_use"])("recognizes %s blocks", (type) => {
      ...
      [
        ...
        JSON.stringify({
          ...
          event: {
            type: "content_block_delta",
            index: 0,
            delta: { type: "input_json_delta", partial_json: '{"query":"supportClaw"}' },
          },
        }),
        JSON.stringify({
          type: "stream_event",
    ...
    expect(starts).toEqual([
      { toolCallId: "toolu_hosted", name: "web_search", args: { query: "supportClaw" } },
    ]);
  });
  ...
  it.each([
    {
      ...
      name: "web_search",
      input: { query: "supportClaw" },
      result: [{ type: "web_search_result", title: "SupportClaw", url: "https://example.com" }],
      isError: false,
    },
    {
