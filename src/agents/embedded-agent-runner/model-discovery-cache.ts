...
function authFingerprint(agentDir: string): object {
  return {
    authProfilesSqlite: fileFingerprint(path.join(agentDir, "supportClaw-agent.sqlite")),
    authProfilesSqliteWal: fileFingerprint(path.join(agentDir, "supportClaw-agent.sqlite-wal")),
  };
}
