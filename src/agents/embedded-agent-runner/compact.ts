...
import { createSupportClawCodingTools, resolveProcessToolScopeKey } from "../agent-tools.js";
...
async function compactEmbeddedAgentSessionDirectOnce(
  ...
  try {
    ...
    const toolsRaw = createSupportClawCodingTools({
      exec: {
    ...
    const filteredBundledTools = applyFinalEffectiveToolPolicy({
      ...
      // createSupportClawCodingTools(...) also omit it, so both paths resolve
    ...
    try {
      ...
      // exact SupportClaw-managed registrations so custom tools survive startup.
