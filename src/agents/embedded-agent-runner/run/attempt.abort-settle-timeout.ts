...
type AbortSettleTimeoutEnv = Partial<
  Pick<NodeJS.ProcessEnv, "SUPPORT_CLAW_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS" | "SUPPORT_CLAW_TEST_FAST">
>;
...
export function resolveEmbeddedAbortSettleTimeoutMs(
  ...
  const override = parseStrictPositiveInteger(env.SUPPORT_CLAW_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS);
  if (override !== undefined) {
    return override;
  }
  return env.SUPPORT_CLAW_TEST_FAST === "1" ? 250 : 2_000;
}
