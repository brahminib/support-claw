...
import type { AgentMessage } from "supportClaw/plugin-sdk/agent-core";
...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      toolSearchControlsCase = mockParams(
        hoisted.createSupportClawCodingToolsMock,
        0,
        "createSupportClawCodingTools options",
      );
    } finally {
  ...
  it("enables Tool Search controls for embedded SupportClaw runs when configured", async () => {
  ...
  it("defaults local-model lean embedded runs to Tool Search controls", async () => {
    ...
    expect(hoisted.createSupportClawCodingToolsMock).toHaveBeenCalledTimes(1);
    ...
    const optionsConfig = requireRecord(options.config, "createSupportClawCodingTools config");
    const toolsConfig = requireRecord(
      optionsConfig.tools,
      "createSupportClawCodingTools tools config",
    );
  ...
  it("keeps Tool Search controls off for lean message-tool-only delivery", async () => {
    hoisted.createSupportClawCodingToolsMock.mockReturnValueOnce([
      {
        name: "message",
        label: "Message",
  ...
  it("quarantines unsupported tool schemas before creating the model session", async () => {
    hoisted.createSupportClawCodingToolsMock.mockReturnValue([
      {
        name: "healthy_lookup",
  ...
  it("enforces code-mode payload surface from active-agent config during an embedded attempt", async () => {
    ...
    expect(observedOptions.at(-1)?.supportClawCodeModeToolSurface).toBe(true);
  ...
  it("sends transcriptPrompt visibly and keeps runtime context out of transcript messages", async () => {
    ...
    const result = await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        prompt: [
          "visible ask",
          "",
          "<<<BEGIN_SUPPORT_CLAW_INTERNAL_CONTEXT>>>",
          "secret runtime context",
          "<<<END_SUPPORT_CLAW_INTERNAL_CONTEXT>>>",
        ].join("\n"),
    ...
    expectFields(
      findRecord(
        ...
        (message) => message.customType === "supportClaw.runtime-context",
        "runtime context message",
      ),
      {
        role: "custom",
        customType: "supportClaw.runtime-context",
        display: false,
      },
    );
    ...
    for (const value of [
      ...
      expect(String(value)).not.toContain("SUPPORT_CLAW_INTERNAL_CONTEXT");
  ...
  it("rebuilds skill prompt inputs from the sandbox workspace for non-rw sandbox runs", async () => {
    const sandboxWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-sandbox-skills-"));
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        skillsSnapshot: {
          prompt:
            "<available_skills><skill><location>~/.supportClaw/skills/smaug/SKILL.md</location></skill></available_skills>",
          skills: [{ name: "smaug" }],
          resolvedSkills: [
            {
              name: "smaug",
              ...
              filePath: "/Users/alice/.supportClaw/skills/smaug/SKILL.md",
              baseDir: "/Users/alice/.supportClaw/skills/smaug",
              source: "supportClaw-workspace",
              sourceInfo: {
                path: "/Users/alice/.supportClaw/skills/smaug/SKILL.md",
                source: "supportClaw-workspace",
                scope: "project",
                origin: "top-level",
                baseDir: "/Users/alice/.supportClaw/skills/smaug",
  ...
  it("keeps bootstrap truncation warnings out of WebChat runtime context", async () => {
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "AGENTS.md",
          path: "/tmp/supportClaw-warning-workspace/AGENTS.md",
          content: "A".repeat(200),
          missing: false,
        },
      ],
      contextFiles: [
        { path: "/tmp/supportClaw-warning-workspace/AGENTS.md", content: "A".repeat(20) },
      ],
    });
  ...
  it("preserves bootstrap system context in the assembled system prompt", async () => {
    ...
    hoisted.createSupportClawCodingToolsMock.mockImplementationOnce(() => [
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/supportClaw-bootstrap-workspace/BOOTSTRAP.md",
  ...
  it("includes hook-adjusted bootstrap files preloaded before routing", async () => {
    const workspaceDir = "/tmp/supportClaw-hook-workspace";
  ...
  it("adds current-turn context to the current model input without exposing internal runtime context", async () => {
    ...
    expect(seenPrompt).not.toContain("SUPPORT_CLAW_INTERNAL_CONTEXT");
  ...
  it("submits runtime-only context through system prompt without visible prompt", async () => {
    ...
    expect(seenPrompt).toBe("Continue the SupportClaw runtime event.");
    expect(result.finalPromptText).toBe("Continue the SupportClaw runtime event.");
  ...
  it("keeps current inbound context visible on runtime-only turns", async () => {
    ...
    expect(seenPrompt).toContain("Continue the SupportClaw runtime event.");
    expect(result.finalPromptText).toBe(seenPrompt);
    const trajectoryEvents = (
      await fs.readFile(path.join(tempPaths[0] ?? "", "session.trajectory.jsonl"), "utf8")
    )
  ...
  it("submits suppressed room event context as the model prompt", async () => {
    ...
    expect(seenPrompt).toContain("[SupportClaw room event]");
    ...
    expect(seenPrompt?.trim().endsWith("[SupportClaw room event]")).toBe(true);
    expect(seenPrompt).not.toBe("Continue the SupportClaw runtime event.");
    ...
    expect(contextCompiled?.data?.prompt).toContain("[SupportClaw room event]");
  });
