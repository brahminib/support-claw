...
type PromptReleaseStreamFn = ((...args: unknown[]) => unknown) & {
  __supportClawSessionLockPromptReleaseInstalled?: boolean;
};
...
function parsePromptReleasedMessageLine(
  ...
  try {
    ...
    const isSupportClawTranscriptOnlyAssistant = isTranscriptOnlySupportClawAssistantMessage(message);
    if (
      typeof message.role !== "string" ||
      (!options?.allowAnyMessage && !isSupportClawTranscriptOnlyAssistant)
    ) {
      return undefined;
    }
    return {
...
// Controllers in the same SupportClaw process can legitimately take turns writing
// the same session file while another attempt is released for model I/O. Track
// only fingerprints that changed while SupportClaw held the write lock so the
...
const EMBEDDED_ATTEMPT_SESSION_FILE_OWNER_STATE_KEY = Symbol.for(
  "supportClaw.embeddedAttemptSessionFileOwnerState",
);
...
export function installPromptSubmissionLockRelease(params: {
  ...
  if (currentStreamFn["__supportClawSessionLockPromptReleaseInstalled"] === true) {
    return;
  }
  ...
  wrappedStreamFn["__supportClawSessionLockPromptReleaseInstalled"] = true;
