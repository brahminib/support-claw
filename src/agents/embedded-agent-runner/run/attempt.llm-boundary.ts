...
function stripUnsafeBlockedRunMetadata(messages: AgentMessage[]): AgentMessage[] {
  ...
  const nextMessages = messages.map((message) => {
    const supportClaw = (message as unknown as Record<string, unknown>)["__supportClaw"];
    if (!supportClaw || typeof supportClaw !== "object") {
      return message;
    }
    const beforeAgentRunBlocked = (supportClaw as { beforeAgentRunBlocked?: unknown })
    ...
    const nextSupportClaw = {
      ...(supportClaw as Record<string, unknown>),
      beforeAgentRunBlocked: safeBlocked,
    };
    changed = true;
    return {
      ...(message as unknown as Record<string, unknown>),
      __supportClaw: nextSupportClaw,
