...
function isSupportClawCliImageCachePath(filePath: string): boolean {
  const parts = filePath.replaceAll("\\", "/").split("/");
  return parts.some((part, index) => {
    if (part === ".supportClaw-cli-images") {
      return true;
    }
    const parent = parts[index - 1] ?? "";
    return part === "supportClaw-cli-images" && /^supportClaw(?:-\d+)?$/.test(parent);
  });
}
...
export function detectImageReferences(prompt: string): DetectedImageRef[] {
  ...
  const addPathRef = (raw: string) => {
    ...
    if (isSupportClawCliImageCachePath(resolved)) {
      return;
    }
    seen.add(dedupeKey);
