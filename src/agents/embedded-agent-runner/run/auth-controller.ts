...
export function createEmbeddedRunAuthController(params: {
  ...
  const applyApiKeyInfo = async (candidate?: string): Promise<void> => {
    ...
    if (!apiKeyInfo.apiKey) {
      ...
      // SupportClaw runtime's authStorage considers the provider authenticated.  Try
      ...
      // sentinel so SupportClaw runtime's hasConfiguredAuth() passes and the AWS SDK default
