...
import {
  ...
  SUPPORT_CLAW_EMBEDDED_CONTEXT_ENGINE_HOST,
...
import { isTranscriptOnlySupportClawAssistantMessage } from "../../../shared/transcript-only-supportClaw-assistant.js";
    ...
    params.sessionManager.removeTrailingEntries(
      (entry) => entry.type === "message" && isMidTurnPrecheckAssistantError(entry.message),
      {
        preserveTrailing: (entry) =>
          ...
          (entry.type === "message" && isTranscriptOnlySupportClawAssistantMessage(entry.message)),
      },
    ) > 0;
  ...
  try {
    ...
    if (activeContextEngine && activeContextEngine.info.id !== "legacy") {
      assertContextEngineHostSupport({
        ...
        operation: "agent-run",
        host: SUPPORT_CLAW_EMBEDDED_CONTEXT_ENGINE_HOST,
      });
    }
    ...
    const toolsRaw = !shouldConstructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-supportClaw-coding-tools");
    ...
    const supportClawReferences = await resolveSupportClawReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      await withOwnedSessionWriteLock(async () => {
        await runAttemptContextEngineBootstrap({
          ...
          contextEngineHostSupport: SUPPORT_CLAW_EMBEDDED_CONTEXT_ENGINE_HOST,
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop SupportClaw
      // compaction overrides applied in createPreparedEmbeddedAgentSettingsManager — same
      // rehydration also restores SupportClaw runtime's auto-compaction (supportClaw#75799), so re-apply
      // both guards.
      applyAgentCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      // exact SupportClaw-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal SupportClaw agent/tool prompt when `session.prompt()` starts.
      ...
      if (activeContextEngine?.info.ownsCompaction === true) {
        ...
        const contextEngineLoopRuntimeSettings = buildContextEngineRuntimeSettings({
          contextEngineHost: SUPPORT_CLAW_EMBEDDED_CONTEXT_ENGINE_HOST,
          provider: params.provider,
      ...
      try {
        if (isRawModelRun) {
          ...
          if (params.sessionKey && params.config && !isRawModelRun) {
            // Capability guidance must include deferred SupportClaw tools without
      ...
      toolSearchCatalogExecutor = async (toolParams) => {
        try {
          if (toolParams.source === "supportClaw" && toolParams.sourceName === "core") {
      ...
      try {
        ...
        await withOwnedSessionWriteLock(async () => {
          ...
          // double-compaction. See: https://github.com/brahminib/support-claw/issues/9282
          ...
          if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
            try {
              activeSessionManager.appendCustomEntry("supportClaw:prompt-error", {
                timestamp: Date.now(),
                runId: params.runId,
                sessionId: params.sessionId,
                provider: params.provider,
      ...
      // See: https://github.com/brahminib/support-claw/issues/8643
