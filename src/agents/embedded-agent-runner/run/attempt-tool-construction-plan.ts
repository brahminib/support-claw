...
import type { SupportClawCodingToolConstructionPlan } from "../../agent-tools.js";
...
// Names here must be emitted directly by createSupportClawTools(). Catalog entries
...
const SUPPORT_CLAW_TOOL_FACTORY_NAMES = new Set([
  "agents_list",
  "canvas",
  "cron",
  "gateway",
...
const ALL_CODING_TOOL_CONSTRUCTION_PLAN: SupportClawCodingToolConstructionPlan = {
...
const NO_CODING_TOOL_CONSTRUCTION_PLAN: SupportClawCodingToolConstructionPlan = {
...
function cloneCodingToolConstructionPlan(
  plan: SupportClawCodingToolConstructionPlan,
): SupportClawCodingToolConstructionPlan {
  return { ...plan };
}
...
function isKnownLocalCodingToolName(normalized: string): boolean {
  ...
  return (
    ...
    SUPPORT_CLAW_TOOL_FACTORY_NAMES.has(normalized)
  );
}
...
function resolveCodingToolConstructionPlanForAllowlist(
  ...
  const includeSupportClawTools = normalized.some((name) => SUPPORT_CLAW_TOOL_FACTORY_NAMES.has(name));
...
export function resolveEmbeddedAttemptToolConstructionPlan(params: {
  ...
  codingToolConstructionPlan: SupportClawCodingToolConstructionPlan;
} {
  ...
  const includeCoreTools =
    ...
    codingToolConstructionPlan.includeSupportClawTools;
