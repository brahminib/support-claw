...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createSupportClawCodingToolsMock: UnknownMock;
...
const hoisted = vi.hoisted((): AttemptSpawnWorkspaceHoisted => {
  ...
  return {
    ...
    createSupportClawCodingToolsMock,
...
vi.mock("../../docs-path.js", () => ({
  resolveSupportClawReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../agent-tools.js", () => ({
  createSupportClawCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createSupportClawCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("supportClaw.cache-ttl", data),
  ...
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "supportClaw.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createSupportClawCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.SUPPORT_CLAW_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.SUPPORT_CLAW_TRAJECTORY = "0";
  }
  try {
    return await (
      await loadRunEmbeddedAttempt()
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.SUPPORT_CLAW_TRAJECTORY;
    } else {
      process.env.SUPPORT_CLAW_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
