...
let supportClawNativeCodexResponsesStreamFnForTest: StreamFn | undefined;
...
function isDefaultSupportClawStreamFnForModel(
...
function resolveSupportClawNativeCodexResponsesStreamFn(params: {
  model: EmbeddedRunAttemptParams["model"];
  ...
  if (!isDefaultSupportClawStreamFnForModel(params.model, params.currentStreamFn)) {
    return undefined;
  }
  return supportClawNativeCodexResponsesStreamFnForTest ?? params.currentStreamFn ?? streamSimple;
}
...
export function describeEmbeddedAgentStreamStrategy(params: {
  ...
  if (
    resolveSupportClawNativeCodexResponsesStreamFn({
      model: params.model,
      currentStreamFn: params.currentStreamFn,
    })
  ) {
    return "supportClaw-native-codex-responses";
  }
  if (isDefaultSupportClawStreamFnForModel(params.model, params.currentStreamFn)) {
...
export function resolveEmbeddedAgentStreamFn(params: {
  ...
  const supportClawNativeCodexResponsesStreamFn = resolveSupportClawNativeCodexResponsesStreamFn({
    model: params.model,
    currentStreamFn: params.currentStreamFn,
  });
  if (supportClawNativeCodexResponsesStreamFn) {
    return wrapEmbeddedAgentStreamFn(supportClawNativeCodexResponsesStreamFn, {
      runSignal: params.signal,
      resolvedApiKey: params.resolvedApiKey,
      authProfileId: params.authProfileId,
      authStorage: params.authStorage,
  ...
  if (
    isDefaultSupportClawStreamFnForModel(params.model, params.currentStreamFn) ||
    hasResolvedRuntimeApiKey(params.resolvedApiKey)
  ) {
    const boundaryAwareStreamFn = createBoundaryAwareStreamFnForModel(params.model);
    if (boundaryAwareStreamFn) {
      // Some SupportClaw session factories return a provider-specific stream wrapper
      // once runtime auth is resolved. Keep transport-supported APIs on
      // SupportClaw's HTTP transport so provider-specific auth/header semantics
...
export const testing = {
  setSupportClawNativeCodexResponsesStreamFnForTest(streamFn: StreamFn | undefined): void {
    supportClawNativeCodexResponsesStreamFnForTest = streamFn;
  },
  resetSupportClawNativeCodexResponsesStreamFnForTest(): void {
    supportClawNativeCodexResponsesStreamFnForTest = undefined;
  },
};
