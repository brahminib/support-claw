...
vi.mock("../../plugins/manifest-metadata-scan.js", () => ({
  listSupportClawPluginManifestMetadata: manifestMocks.listSupportClawPluginManifestMetadata,
}));
...
function setManifestPlugins(plugins: unknown[]) {
  ...
  manifestMocks.listSupportClawPluginManifestMetadata.mockReturnValue(
  ...
  manifestMocks.loadPluginManifest.mockImplementation((pluginDir: string) => {
    ...
    return plugin
      ...
      : { ok: false, error: "missing manifest", manifestPath: `${pluginDir}/supportClaw.plugin.json` };
  });
}
...
beforeEach(() => {
  manifestMocks.listSupportClawPluginManifestMetadata.mockReset();
...
describe("resolveBundledStaticCatalogModel", () => {
  it("reuses one manifest scan across prepared lookups", () => {
    ...
    expect(manifestMocks.listSupportClawPluginManifestMetadata).toHaveBeenCalledTimes(1);
  });
