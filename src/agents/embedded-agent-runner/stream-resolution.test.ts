...
import type { StreamFn } from "supportClaw/plugin-sdk/agent-core";
...
afterEach(() => {
  testing.resetSupportClawNativeCodexResponsesStreamFnForTest();
});
...
describe("describeEmbeddedAgentStreamStrategy", () => {
  ...
  it("describes default Codex fallback as SupportClaw native", () => {
    expect(
      describeEmbeddedAgentStreamStrategy({
        currentStreamFn: undefined,
        model: {
    ...
    ).toBe("supportClaw-native-codex-responses");
  });
...
describe("resolveEmbeddedAgentStreamFn", () => {
  ...
  it("routes Codex responses fallbacks through SupportClaw native transport", async () => {
    // Codex OAuth models use the SupportClaw native transport, with prompt-cache
    ...
    testing.setSupportClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
      currentStreamFn: undefined,
      sessionId: "session-1",
      model: {
  ...
  it("routes SupportClaw native OpenAI-compatible provider streams through boundary-aware transports", async () => {
  ...
  it("injects the resolved run api key into the SupportClaw native Codex Responses fallback", async () => {
  ...
  it("falls back to authStorage when no resolved api key is available for SupportClaw native fallback", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const authStorage = {
  ...
  it("forwards the run abort signal into the SupportClaw native fallback when callers omit one", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    testing.setSupportClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
  ...
  it("does not overwrite an explicit signal on the SupportClaw native fallback path", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    const explicitSignal = new AbortController().signal;
    testing.setSupportClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
  ...
  it("forwards the run signal on the sync SupportClaw native fallback path without auth credentials", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    testing.setSupportClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
  ...
  it("strips cache boundary markers on the SupportClaw native fallback path", async () => {
