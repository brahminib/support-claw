 ...
 * See: https://github.com/brahminib/support-claw/issues/60552
 */
...
async function runEmbeddedAgentInternal(
  ...
  return enqueueSession(async () => {
    ...
    return enqueueGlobal(async () => {
      ...
      const pluginHarnessOwnsTransport = agentHarness.id !== "supportClaw";
      ...
      for (const candidateProvider of modelResolutionProviders) {
        const candidateResolution = await resolveModelAsync(
          ...
          params.config,
          {
            ...
            // first generating SupportClaw models.json. This keeps one-shot model runs from
      ...
      if (!modelResolution) {
        await ensureSupportClawModelsJson(params.config, agentDir, {
          workspaceDir: resolvedWorkspace,
        });
        for (const candidateProvider of modelResolutionProviders) {
          const candidateResolution = await resolveModelAsync(
      ...
      const pluginHarnessNeedsSupportClawAuthBootstrap =
        pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.api === "openai-chatgpt-responses";
      const supportClawNativeCodexResponsesNeedsAuthBootstrap =
        !pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.api === "openai-chatgpt-responses";
      let piExternalCliAuthScope = pluginHarnessOwnsTransport
        ? { ignoreAutoPreferredProfile: false }
        : supportClawNativeCodexResponsesNeedsAuthBootstrap
          ? {
      ...
      if (
        !pluginHarnessOwnsTransport &&
        !pluginHarnessNeedsSupportClawAuthBootstrap &&
      ...
      const authStore =
        pluginHarnessOwnsTransport && !pluginHarnessNeedsSupportClawAuthBootstrap
          ? createEmptyAuthProfileStore()
          : pluginHarnessNeedsSupportClawAuthBootstrap
      ...
      // Plugin harnesses own their model transport/auth. Running SupportClaw's generic
      ...
      if (!pluginHarnessOwnsTransport || pluginHarnessNeedsSupportClawAuthBootstrap) {
      ...
      const runAttemptAuthProfileStore = pluginHarnessOwnsTransport
        ...
        : attemptAuthProfileStore;
      const harnessBuildsSupportClawTools =
      ...
      const maybeMarkAuthProfileFailure = async (failure: {
        ...
        if (pluginHarnessOwnsTransport && reason === "timeout") {
          ...
          // credential evidence. Do not poison SupportClaw auth cooldowns.
          return;
        }
      ...
      try {
        ...
        while (true) {
          ...
          const rawAttempt = await runEmbeddedAttemptWithBackend({
            ...
            // attempt too. Otherwise plugin-owned transports can skip SupportClaw auth
            // bootstrap but drift back to SupportClaw when the attempt is created.
            agentHarnessId: agentHarness.id,
            ...(params.sessionKey
              ? {
            ...
            // These harnesses build SupportClaw tools internally. Keep transport auth
            // scoped while letting tool construction see plugin/provider creds.
            toolAuthProfileStore: harnessBuildsSupportClawTools ? attemptAuthProfileStore : undefined,
            modelRegistry,
            agentId: workspaceResolution.agentId,
