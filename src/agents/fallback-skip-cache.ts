 ...
 * SUPPORT_CLAW_FALLBACK_SKIP_TTL_MS.
 */
const DEFAULT_FALLBACK_SKIP_TTL_MS = 0;
const FALLBACK_SKIP_TTL_ENV = "SUPPORT_CLAW_FALLBACK_SKIP_TTL_MS";
...
function getState(): SkipCacheState {
  const globalStore = globalThis as typeof globalThis & {
    supportClawFallbackSkipCache?: SkipBySession;
    supportClawFallbackSkipCacheState?: SkipCacheState;
  };
  if (!globalStore.supportClawFallbackSkipCacheState) {
    ...
    const buckets = globalStore.supportClawFallbackSkipCache ?? new Map();
    globalStore.supportClawFallbackSkipCacheState = {
      buckets,
      lastGlobalPruneAtMs: 0,
    };
    globalStore.supportClawFallbackSkipCache = buckets;
  }
  return globalStore.supportClawFallbackSkipCacheState;
}
