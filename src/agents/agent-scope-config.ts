...
export function listAgentEntries(cfg: SupportClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: SupportClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: SupportClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: SupportClawConfig, agentId: string): AgentEntry | undefined {
