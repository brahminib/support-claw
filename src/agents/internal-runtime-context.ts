 ...
 */
/** Opening delimiter for protected SupportClaw runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_SUPPORT_CLAW_INTERNAL_CONTEXT>>>";
/** Closing delimiter for protected SupportClaw runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_SUPPORT_CLAW_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[SUPPORT_CLAW_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[SUPPORT_CLAW_INTERNAL_CONTEXT_END]]";

/** Notice inserted into runtime-generated context blocks. */
export const SUPPORT_CLAW_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
/** Header for context attached to the immediately preceding user message. */
export const SUPPORT_CLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "SupportClaw runtime context for the immediately preceding user message.";
/** Header for runtime events passed as prompt context. */
export const SUPPORT_CLAW_RUNTIME_EVENT_HEADER = "SupportClaw runtime event.";
/** Custom message type used for structured runtime-context messages. */
export const SUPPORT_CLAW_RUNTIME_CONTEXT_CUSTOM_TYPE = "supportClaw.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["SupportClaw runtime context (internal):", SUPPORT_CLAW_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === SUPPORT_CLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === SUPPORT_CLAW_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === SUPPORT_CLAW_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(
      `${SUPPORT_CLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER}\n${SUPPORT_CLAW_RUNTIME_CONTEXT_NOTICE}`,
    ) ||
    text.includes(`${SUPPORT_CLAW_RUNTIME_EVENT_HEADER}\n${SUPPORT_CLAW_RUNTIME_CONTEXT_NOTICE}`)
  );
}

function isSupportClawRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === SUPPORT_CLAW_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

/** Remove all structured runtime-context custom messages. */
export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isSupportClawRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isSupportClawRuntimeContextCustomMessage(message));
}
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  for (let index = lastUserIndex - 1; index >= 0; index -= 1) {
    if (!isSupportClawRuntimeContextCustomMessage(messages[index])) {
      break;
    }
    currentRuntimeContextIndexes.add(index);
  }
  return messages.filter((message, index) => {
    if (!isSupportClawRuntimeContextCustomMessage(message)) {
      return true;
    }
