...
let prepareSupportClawModelsJsonSourceMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    prepareSupportClawModelsJsonSourceMock = vi.fn().mockResolvedValue({
      agentDir: "/tmp/supportClaw",
    ...
    vi.doMock("./models-config.js", () => ({
      ...
      prepareSupportClawModelsJsonSource: prepareSupportClawModelsJsonSourceMock,
    }));
    ...
    vi.doMock("./agent-scope.js", () => ({
      resolveAgentWorkspaceDir: (cfg: SupportClawConfig, agentId: string) => {
        const entry = cfg.agents?.list?.find((entryEntry) => entryEntry.id === agentId);
        return entry?.workspace ?? cfg.agents?.defaults?.workspace ?? "/tmp/supportClaw-workspace";
      },
      resolveDefaultAgentDir: () => "/tmp/supportClaw",
      resolveDefaultAgentId: (cfg: SupportClawConfig) =>
  ...
  beforeEach(() => {
    ...
    prepareSupportClawModelsJsonSourceMock.mockReset();
    prepareSupportClawModelsJsonSourceMock.mockResolvedValue({
      agentDir: "/tmp/supportClaw",
      fingerprint: "source-fingerprint",
      workspaceDir: "/tmp/supportClaw-workspace",
      wrote: false,
  ...
  it("uses the state cached catalog before runtime discovery", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as SupportClawConfig });
    ...
    expect(prepareSupportClawModelsJsonSourceMock).not.toHaveBeenCalled();
  ...
  it("bypasses the state cached catalog when a refresh is requested", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as SupportClawConfig, useCache: false });
  ...
  it("does not publish a catalog load from an invalidated generation", async () => {
    ...
    const staleLoad = loadModelCatalog({ config: {} as SupportClawConfig });
    resetModelCatalogCache();
    await expect(loadModelCatalog({ config: {} as SupportClawConfig })).resolves.toBe(freshCatalog);
  ...
  it("misses the state cached catalog when source freshness changes", async () => {
    ...
    await expect(loadModelCatalog({ config: {} as SupportClawConfig })).resolves.toEqual([
      { id: "cached-stale", name: "Cached Stale", provider: "openai" },
    ]);
    resetModelCatalogCacheForTest();
    mockAgentDiscoveryModels([{ id: "fresh-fast", name: "Fresh Fast", provider: "openai" }]);
  ...
  it("reloads dynamic registry entries after clearing the cache", async () => {
    ...
    const first = await loadModelCatalog({ config: {} as SupportClawConfig });
    expect(first).toStrictEqual([
      {
        id: "existing",
        name: "Existing",
    ...
    const second = await loadModelCatalog({ config: {} as SupportClawConfig });
  ...
  it("filters suppressed built-ins from persisted read-only catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as SupportClawConfig, readOnly: true });
  ...
  it("loads generated plugin catalog rows in read-only mode", async () => {
    const catalogPath = "/tmp/supportClaw/plugins/read-only-shard/catalog.json";
    mkdirSync("/tmp/supportClaw/plugins/read-only-shard", { recursive: true });
    ...
    try {
      ...
      rmSync("/tmp/supportClaw/plugins/read-only-shard", { recursive: true, force: true });
    }
  });
  ...
  it("loads manifest catalog rows from the current metadata snapshot without provider runtime", () => {
    ...
    const result = loadManifestModelCatalog({ config: {} as SupportClawConfig });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/supportClaw-model-catalog-env" } as NodeJS.ProcessEnv;
