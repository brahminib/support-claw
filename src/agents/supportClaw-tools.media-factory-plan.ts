...
function hasExplicitImageModelConfig(config: SupportClawConfig | undefined): boolean {
  return hasExplicitToolModelConfig(config?.agents?.defaults?.imageModel);
}

function hasExplicitPdfModelConfig(config: SupportClawConfig | undefined): boolean {
  return (
