...
function resolveAnthropicOpusThinking(cfg: SupportClawConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: SupportClawConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicOpus48Thinking(cfg: SupportClawConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicVertexOpus48Thinking(cfg: SupportClawConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic-vertex",
    model: "claude-opus-4-8",
...
function resolveClaudeCliOpus48Thinking(cfg: SupportClawConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "claude-cli",
    model: "claude-opus-4-8",
...
function resolveConfiguredRefForTest(cfg: Partial<SupportClawConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as SupportClawConfig,
...
describe("model-selection", () => {
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<SupportClawConfig> = {
        agents: {
          defaults: {
            models: {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("should fall back to the configured default provider and warn if provider is missing for non-alias", async () => {
      const warnLogs = createWarnLogCapture("supportClaw-model-selection-test");
      try {
        const cfg: Partial<SupportClawConfig> = {
          agents: {
            defaults: {
