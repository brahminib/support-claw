...
import { planSupportClawModelsJson } from "./models-config.plan.js";
import {
...
export type PreparedSupportClawModelsJsonSource = ModelsJsonReadyResult & {
  fingerprint: string;
  workspaceDir?: string;
};

type EnsureSupportClawModelsJsonOptions = {
...
function resolveModelsConfigInput(config?: SupportClawConfig): {
  config: SupportClawConfig;
  sourceConfigForSecrets: SupportClawConfig;
} {
...
export async function prepareSupportClawModelsJsonSource(
  config?: SupportClawConfig,
  agentDirOverride?: string,
  options: EnsureSupportClawModelsJsonOptions = {},
): Promise<PreparedSupportClawModelsJsonSource> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  ...
  const pending: Promise<ModelsJsonReadyState> = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planSupportClawModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
...
/** Ensures models.json and plugin catalog sidecars are current for an agent. */
export async function ensureSupportClawModelsJson(
  config?: SupportClawConfig,
  agentDirOverride?: string,
  options: EnsureSupportClawModelsJsonOptions = {},
): Promise<ModelsJsonReadyResult> {
  const prepared = await prepareSupportClawModelsJsonSource(config, agentDirOverride, options);
