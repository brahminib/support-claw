...
function isSupportClawLoopbackMcpServer(name: string, server: BundleMcpServerConfig): boolean {
  return (
    name === "supportClaw" &&
...
export function normalizeCodexMcpServerConfig(
  ...
  } else if (isSupportClawLoopbackMcpServer(name, server)) {
    // SupportClaw's loopback MCP exposes local tools; Codex should ask for approval
