...
import { resolveClaudeFable5ModelIdentity } from "@supportclaw/llm-core";
...
import {
  ...
  prepareSupportClawModelsJsonSource,
...
let manifestModelCatalogCache = new WeakMap<SupportClawConfig, ManifestModelCatalogCacheEntry>();
...
function shouldLogModelCatalogTiming(): boolean {
  return process.env.SUPPORT_CLAW_DEBUG_INGRESS_TIMING === "1";
}
...
function hasConfiguredProviderRowsNeedingManifestLookup(cfg: SupportClawConfig): boolean {
...
export async function loadModelCatalog(params?: {
  ...
  const loadCatalog = async () => {
    ...
    try {
      ...
      if (!readOnly) {
        const preparedSource = await prepareSupportClawModelsJsonSource(cfg, agentDir, {
          pluginMetadataSnapshot: params?.metadataSnapshot,
          workspaceDir,
        });
