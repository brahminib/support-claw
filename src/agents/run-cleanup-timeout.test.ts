...
describe("agent cleanup timeout", () => {
  ...
  it("uses the trajectory flush timeout environment override for trajectory cleanup", async () => {
    ...
    const result = runAgentCleanupStep({
      ...
      step: "supportClaw-trajectory-flush",
      cleanup,
      log,
      env: {
        SUPPORT_CLAW_TRAJECTORY_FLUSH_TIMEOUT_MS: "25000",
      },
    });
    ...
    expect(cleanup).toHaveBeenCalledTimes(1);
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-trajectory sessionId=session-trajectory step=supportClaw-trajectory-flush timeoutMs=25000",
    );
  });
  ...
  it("includes cleanup timeout details when the cleanup step exposes them", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-trajectory sessionId=session-trajectory step=supportClaw-trajectory-flush timeoutMs=5 details=pendingWrites=2 queuedBytes=128 activeOperation=file-append",
    );
  });
  ...
  it("does not fail cleanup when timeout details throw", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-trajectory sessionId=session-trajectory step=supportClaw-trajectory-flush timeoutMs=5 detailsError=details unavailable",
    );
  });
  ...
  it("uses the general cleanup timeout environment override for other cleanup steps", async () => {
    ...
    const result = runAgentCleanupStep({
      ...
      log,
      env: {
        SUPPORT_CLAW_AGENT_CLEANUP_TIMEOUT_MS: "1500",
      },
    });
  ...
  it("prefers explicit cleanup timeout values over environment overrides", async () => {
    ...
    const result = runAgentCleanupStep({
      ...
      env: {
        SUPPORT_CLAW_TRAJECTORY_FLUSH_TIMEOUT_MS: "25000",
        SUPPORT_CLAW_AGENT_CLEANUP_TIMEOUT_MS: "15000",
      },
    });
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-explicit sessionId=session-explicit step=supportClaw-trajectory-flush timeoutMs=2000",
    );
  });
  ...
  it("keeps explicit zero cleanup timeouts as a one millisecond timeout", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-zero sessionId=session-zero step=supportClaw-trajectory-flush timeoutMs=1",
    );
  });
  ...
  it.each([
    {
      ...
      env: {
        SUPPORT_CLAW_TRAJECTORY_FLUSH_TIMEOUT_MS: "0",
        SUPPORT_CLAW_AGENT_CLEANUP_TIMEOUT_MS: "not-a-number",
      },
    },
    {
      ...
      env: {
        SUPPORT_CLAW_TRAJECTORY_FLUSH_TIMEOUT_MS: "1e3",
        SUPPORT_CLAW_AGENT_CLEANUP_TIMEOUT_MS: "0x10",
      },
    },
  ])(
    ...
    async ({ runId, sessionId, env }) => {
      ...
      expect(log.warn).toHaveBeenCalledWith(
        `agent cleanup timed out: runId=${runId} sessionId=${sessionId} step=supportClaw-trajectory-flush timeoutMs=10000`,
      );
    },
  );
