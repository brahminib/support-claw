...
function writeExecApprovalsFixture(root: string, file: Record<string, unknown>): void {
  const stateDir = process.env.SUPPORT_CLAW_STATE_DIR ?? path.join(root, "state");
...
describe("exec security floor", () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-exec-security-floor-"));
    setTestEnvValue("HOME", tempRoot);
    setTestEnvValue("USERPROFILE", tempRoot);
    setTestEnvValue("SUPPORT_CLAW_HOME", tempRoot);
    setTestEnvValue("SUPPORT_CLAW_STATE_DIR", path.join(tempRoot, "state"));
