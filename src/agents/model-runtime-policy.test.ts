...
const ORIGINAL_BUILD_PRIVATE_QA = process.env.SUPPORT_CLAW_BUILD_PRIVATE_QA;
const ORIGINAL_QA_FORCE_RUNTIME = process.env.SUPPORT_CLAW_QA_FORCE_RUNTIME;
...
function restoreEnv(
  name: "SUPPORT_CLAW_BUILD_PRIVATE_QA" | "SUPPORT_CLAW_QA_FORCE_RUNTIME",
...
function makeProviderRuntimeConfig(runtime: string): SupportClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
afterEach(() => {
  restoreEnv("SUPPORT_CLAW_BUILD_PRIVATE_QA", ORIGINAL_BUILD_PRIVATE_QA);
  restoreEnv("SUPPORT_CLAW_QA_FORCE_RUNTIME", ORIGINAL_QA_FORCE_RUNTIME);
});

describe("resolveModelRuntimePolicy", () => {
  it("ignores the QA force-runtime override when the private QA gate is unset", () => {
    deleteTestEnvValue("SUPPORT_CLAW_BUILD_PRIVATE_QA");
    setTestEnvValue("SUPPORT_CLAW_QA_FORCE_RUNTIME", "supportClaw");
  ...
  it("respects the QA force-runtime override when the private QA gate is set", () => {
    ...
    setTestEnvValue("SUPPORT_CLAW_BUILD_PRIVATE_QA", "1");
    setTestEnvValue("SUPPORT_CLAW_QA_FORCE_RUNTIME", "supportClaw");
  ...
  it("ignores invalid QA force-runtime values even when the private QA gate is set", () => {
    setTestEnvValue("SUPPORT_CLAW_BUILD_PRIVATE_QA", "1");
    setTestEnvValue("SUPPORT_CLAW_QA_FORCE_RUNTIME", "bogus");
  ...
  it("honors provider wildcard agent model runtime policy entries", () => {
    const config = {
      agents: {
        defaults: {
          models: {
            "vllm/*": { agentRuntime: { id: "supportClaw" } },
          },
        },
      },
    } as SupportClawConfig;
