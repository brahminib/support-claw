...
const asConfig = (cfg: SupportClawConfig): SupportClawConfig => cfg;
...
describe("memory search config", () => {
  ...
  function configWithDefaultProvider(provider: string): SupportClawConfig {
    return asConfig({
      agents: {
        defaults: {
          memorySearch: {
  ...
  it("defaults provider to openai when unspecified", () => {
    ...
    expect(resolved?.store.databasePath).toBe(resolveSupportClawAgentSqlitePath({ agentId: "main" }));
  });
