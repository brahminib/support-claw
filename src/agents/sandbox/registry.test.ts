...
const {
  TEST_STATE_DIR,
  PREVIOUS_SUPPORT_CLAW_STATE_DIR,
  ...
  const baseDir = mkdtempSync(nodePath.join(tmpdir(), "supportClaw-sandbox-registry-"));
  const previousStateDir = process.env.SUPPORT_CLAW_STATE_DIR;
  Reflect.set(process.env, "SUPPORT_CLAW_STATE_DIR", baseDir);

  return {
    TEST_STATE_DIR: baseDir,
    PREVIOUS_SUPPORT_CLAW_STATE_DIR: previousStateDir,
...
afterAll(async () => {
  ...
  if (PREVIOUS_SUPPORT_CLAW_STATE_DIR === undefined) {
    deleteTestEnvValue("SUPPORT_CLAW_STATE_DIR");
  } else {
    setTestEnvValue("SUPPORT_CLAW_STATE_DIR", PREVIOUS_SUPPORT_CLAW_STATE_DIR);
  }
});
