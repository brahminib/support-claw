      ...
      defaults: {
        sandbox: {
          ...
          ssh: {
            ...
            command: "ssh",
            workspaceRoot: "/remote/supportClaw",
...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "supportClaw-test-ssh-config"),
    host: "supportClaw-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "supportClaw-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "supportClaw-ssh-worker-abcd1234",
    ...
    expect(sessionSettings.workspaceRoot).toBe("/remote/supportClaw");
    const commandParams = requireSshRunCommandParams();
    expect(commandParams.remoteCommand).toContain("/remote/supportClaw/supportClaw-ssh-agent-worker");
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const skillsWorkspaceDir = await createTempDir("supportClaw-ssh-skills-");
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        browser: {
          enabled: false,
          image: "supportClaw-browser",
          containerPrefix: "supportClaw-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/supportClaw/supportClaw-ssh-agent-worker");
    ...
    expect(skillsUploadParams.remoteDir).toContain("/workspace/.supportClaw/sandbox-skills");
  ...
  it("clears stale remote materialized skills when the local copy is missing", async () => {
    const tmpDir = await createTempDir("supportClaw-ssh-skills-");
    ...
    expect(commandParams.remoteCommand).toContain("supportClaw-sandbox-clear");
    expect(commandParams.remoteCommand).toContain("/workspace/.supportClaw/sandbox-skills");
    await backend.finalizeExec?.({
      status: "completed",
      exitCode: 0,
      timedOut: false,
