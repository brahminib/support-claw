...
describe("sandbox ssh helpers", () => {
  it("materializes inline ssh auth data into a temp config", async () => {
    ...
    expect(config).toContain("Host supportClaw-sandbox");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-ssh-real-"));
      ...
      const target = path.join(root, "workspace", ".supportClaw", "sandbox-skills");
      await execFileAsync("/bin/sh", [
        "-c",
        ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
        "supportClaw-remote-dir",
        target,
        root,
      ]);

      await expect(
        fs.stat(path.join(realParent, "runtime", "workspace", ".supportClaw", "sandbox-skills")),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = makeTempDir(tempDirs, "supportClaw-ssh-real-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.symlink(realParent, path.join(root, "workspace", ".supportClaw"));
      ...
      await expect(
        execFileAsync("/bin/sh", [
          ...
          ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
          "supportClaw-remote-dir",
          path.join(root, "workspace", ".supportClaw", "sandbox-skills"),
          root,
        ]),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-ssh-upload-"));
      ...
      await expect(
        uploadDirectoryToSshTarget({
          session: {
            command: "ssh",
            configPath: "/tmp/supportClaw-test-ssh-config",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-ssh-upload-safe-"));
      tempDirs.push(localDir);
