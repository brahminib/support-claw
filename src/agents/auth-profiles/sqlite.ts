...
import type { DB as SupportClawAgentKyselyDatabase } from "../../state/supportClaw-agent-db.generated.js";
import {
  runSupportClawAgentWriteTransaction,
  type SupportClawAgentDatabase,
} from "../../state/supportClaw-agent-db.js";
import { SUPPORT_CLAW_SQLITE_BUSY_TIMEOUT_MS } from "../../state/supportClaw-state-db.js";
import { resolveUserPath } from "../../utils.js";
import { resolveRegisteredAgentIdForDir } from "../agent-dir-registry.js";
import { resolveDefaultAgentDir } from "../agent-scope-config.js";

type AuthProfileDatabase = Pick<
  SupportClawAgentKyselyDatabase,
...
// The auth database lives in the agent dir and shares the supportClaw-agent schema
...
function resolveAuthProfileDatabaseOptions(agentDir?: string) {
  ...
  return {
    ...
    path: path.join(dir, "supportClaw-agent.sqlite"),
  };
}
...
export function readPersistedAuthProfileStoreRaw(
  agentDir?: string,
  database?: SupportClawAgentDatabase,
): unknown {
  if (database) {
...
export function writePersistedAuthProfileStoreRaw(
  ...
  const write = (target: SupportClawAgentDatabase) => {
  ...
  runSupportClawAgentWriteTransaction(write, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function deletePersistedAuthProfileStoreRaw(
  ...
  const remove = (target: SupportClawAgentDatabase) => {
    const db = getAuthProfileKysely(target.db);
    executeSqliteQuerySync(
      target.db,
  ...
  runSupportClawAgentWriteTransaction(remove, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function runAuthProfileWriteTransaction<T>(
  ...
  return runSupportClawAgentWriteTransaction(operation, resolveAuthProfileDatabaseOptions(agentDir));
}
