...
export async function writeGeminiSystemSettings(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-gemini-mcp-"));
...
export async function writeGeminiMcpCaptureSettings(params: {
  ...
  const supportClaw = isRecord(mcpServers.supportClaw) ? mcpServers.supportClaw : {};
  const headers = normalizeStringRecord(supportClaw.headers) ?? {};
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-gemini-mcp-attempt-"));
  const settingsPath = path.join(tempDir, "settings.json");
  await writeJson(
    settingsPath,
    {
