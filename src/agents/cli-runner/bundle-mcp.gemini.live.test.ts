...
const LIVE = isLiveTestEnabled(["SUPPORT_CLAW_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "supportClaw-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("supportClaw_live_probe", "SupportClaw Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an SupportClaw-configured streamable-http server", async () => {
    const geminiCommand = process.env.SUPPORT_CLAW_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("supportClawLiveProbe");
