  ...
  return {
    mcpServers: {
      supportClaw: {
        ...
        alwaysLoad: true,
        headers: {
          Authorization: "Bearer ${SUPPORT_CLAW_MCP_TOKEN}",
          "x-session-key": "${SUPPORT_CLAW_MCP_SESSION_KEY}",
          "x-supportClaw-session-id": "${SUPPORT_CLAW_MCP_SESSION_ID}",
          "x-supportClaw-agent-id": "${SUPPORT_CLAW_MCP_AGENT_ID}",
          "x-supportClaw-account-id": "${SUPPORT_CLAW_MCP_ACCOUNT_ID}",
          "x-supportClaw-message-channel": "${SUPPORT_CLAW_MCP_MESSAGE_CHANNEL}",
          "x-supportClaw-current-channel-id": "${SUPPORT_CLAW_MCP_CURRENT_CHANNEL_ID}",
          "x-supportClaw-current-thread-ts": "${SUPPORT_CLAW_MCP_CURRENT_THREAD_TS}",
          "x-supportClaw-current-message-id": "${SUPPORT_CLAW_MCP_CURRENT_MESSAGE_ID}",
          "x-supportClaw-current-inbound-audio": "${SUPPORT_CLAW_MCP_CURRENT_INBOUND_AUDIO}",
          "x-supportClaw-inbound-event-kind": "${SUPPORT_CLAW_MCP_INBOUND_EVENT_KIND}",
          "x-supportClaw-source-reply-delivery-mode": "${SUPPORT_CLAW_MCP_SOURCE_REPLY_DELIVERY_MODE}",
          "x-supportClaw-require-explicit-message-target":
            "${SUPPORT_CLAW_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET}",
          "x-supportClaw-cli-capture-key": "${SUPPORT_CLAW_MCP_CLI_CAPTURE_KEY}",
        },
      },
    },
  };
...
function createSessionFile() {
  // Prepare tests use canonical SupportClaw session paths because several cases
  // assert that external or stale transcript paths are ignored.
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-cli-prepare-"));
  sessionFileEnvSnapshot ??= captureEnv(["SUPPORT_CLAW_STATE_DIR"]);
  setTestEnvValue("SUPPORT_CLAW_STATE_DIR", dir);
...
describe("shouldSkipLocalCliCredentialEpoch", () => {
  beforeEach(() => {
    ...
    setCliRunnerPrepareTestDeps({
      ...
      resolveSupportClawReferencePaths: vi.fn(async () => ({ docsPath: null, sourcePath: null })),
  ...
  it("lets Gemini CLI preparation override generated MCP system settings auth", async () => {
    ...
    try {
      ...
      expect(generatedSettings.mcp?.allowed).toEqual(["supportClaw"]);
      expect(generatedSettings.mcpServers?.supportClaw?.url).toBe("http://127.0.0.1:31783/mcp");
  ...
  it("prepares side questions without agent-turn context, tools, hooks, or reusable sessions", async () => {
    ...
    setCliRunnerPrepareTestDeps({
      ...
      resolveSupportClawReferencePaths: vi.fn(async () => ({ docsPath: "docs", sourcePath: "src" })),
    });
    ...
    expect(context.supportClawHistoryPrompt).toBeUndefined();
  ...
  it("uses compact current-turn context when a room event resumes a CLI session", async () => {
    ...
    try {
      ...
      // richer room context in SupportClaw history for reseed and audits.
      const context = await prepareCliRunContext({
        sessionId: "session-test",
        sessionKey: "agent:main:test",
        agentId: "main",
      ...
      expect(context.params.prompt).toBe("Current event:\nBob: yes\n\n[SupportClaw room event]");
      expect(context.supportClawHistoryPrompt).toContain("Room context:\nAlice: lunch?");
      expect(context.supportClawHistoryPrompt).toContain("Current event:\nBob: yes");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the base prompt when prompt-build hooks fail", async () => {
    ...
    try {
      ...
      expect(context.systemPrompt).toContain(
        "You are a personal assistant running inside SupportClaw.",
      );
  ...
  it("does not allocate a non-legacy context engine before fallible CLI preparation finishes", async () => {
    ...
    setCliRunnerPrepareTestDeps({
      resolveSupportClawReferencePaths: vi.fn(async () => {
        ...
        info: {
          ...
          hostRequirements: {
            "agent-run": {
              ...
              unsupportedMessage: "Use the native Codex or SupportClaw embedded runtime.",
            },
          },
        },
        ingest: vi.fn(async () => ({ ingested: true })),
  ...
  it("uses cwd for CLI system prompt workspace guidance", async () => {
    const { dir, sessionFile } = createSessionFile();
    const taskDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportClaw-cli-task-"));
    try {
      const context = await prepareCliRunContext({
        sessionId: "session-test",
        sessionFile,
  ...
  it("prepares raw-tail history for safe invalidations only when the backend opts in", async () => {
    ...
    try {
      ...
      expect(context.reusableCliSession).toEqual({ invalidatedReason: "system-prompt" });
      expect(context.supportClawHistoryPrompt).toContain("prior no-compaction ask");
      expect(context.supportClawHistoryPrompt).toContain("latest ask");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("prepares opted-in raw-tail history for session-expired retry without disabling native resume", async () => {
    ...
    try {
      ...
      expect(context.reusableCliSession).toEqual({ sessionId: "cli-session" });
      expect(context.supportClawHistoryPrompt).toContain("prior resumable ask");
      expect(context.supportClawHistoryPrompt).toContain("latest ask");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  ...
  it("passes current turn kind into bundle MCP loopback env", async () => {
    ...
    try {
      ...
      expect(context.preparedBackend.env).toMatchObject({
        SUPPORT_CLAW_MCP_SESSION_ID: "session-test",
        SUPPORT_CLAW_MCP_MESSAGE_CHANNEL: "telegram",
        SUPPORT_CLAW_MCP_CURRENT_CHANNEL_ID: "telegram:-100123:topic:42",
        SUPPORT_CLAW_MCP_CURRENT_THREAD_TS: "42",
        SUPPORT_CLAW_MCP_CURRENT_MESSAGE_ID: "reply-message-1",
        SUPPORT_CLAW_MCP_CURRENT_INBOUND_AUDIO: "true",
        SUPPORT_CLAW_MCP_INBOUND_EVENT_KIND: "room_event",
        SUPPORT_CLAW_MCP_SOURCE_REPLY_DELIVERY_MODE: "message_tool_only",
        SUPPORT_CLAW_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET: "true",
        SUPPORT_CLAW_MCP_CLI_CAPTURE_KEY: "",
      });
  ...
  it("renders CLI skills from sandbox-readable paths instead of persisted host snapshots", async () => {
    const { dir, sessionFile } = createSessionFile();
    const hostSkillDir = "/home/tzdai/.npm-global/lib/node_modules/supportClaw/skills/gog";
    ...
    try {
      ...
      expect(context.systemPrompt).toContain(
        "/workspace/.supportClaw/sandbox-skills/skills/gog/SKILL.md",
      );
  ...
  it("omits Claude CLI prompt skills when the native skills plugin can carry them", async () => {
    ...
    try {
      ...
      setCliRunnerPrepareTestDeps({
        prepareClaudeCliSkillsPlugin: vi.fn(async () => ({
          args: ["--plugin-dir", path.join(dir, "supportClaw-skills")],
          cleanup: vi.fn(async () => undefined),
          pluginDir: path.join(dir, "supportClaw-skills"),
        })),
      });
      ...
      expect(context.claudeSkillsPluginArgs).toEqual([
        "--plugin-dir",
        path.join(dir, "supportClaw-skills"),
      ]);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  ...
  it("uses a larger automatic reseed history cap for Claude CLI", async () => {
    ...
    try {
      ...
      expect(context.supportClawHistoryPrompt).toBeDefined();
      expect(context.supportClawHistoryPrompt).toContain(summaryMarker);
      expect(context.supportClawHistoryPrompt).not.toContain("SupportClaw reseed history truncated");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("keeps the default reseed history cap for non-Claude CLI backends", async () => {
    ...
    try {
      ...
      expect(context.supportClawHistoryPrompt).toBeDefined();
      expect(context.supportClawHistoryPrompt).toContain("SupportClaw reseed history truncated");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("uses the automatic Claude CLI cap through the raw-tail reseed path", async () => {
    ...
    try {
      ...
      expect(context.reusableCliSession).toEqual({ sessionId: "cli-session" });
      expect(context.supportClawHistoryPrompt).toBeDefined();
      expect(context.supportClawHistoryPrompt).toContain(recentMarker);
      expect(context.supportClawHistoryPrompt).toContain("EARLIEST_USER");
      expect(context.supportClawHistoryPrompt).not.toContain("SupportClaw reseed history truncated");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
