...
describe("buildCliAgentSystemPrompt", () => {
  ...
  it("uses CLI backend tool fallback instead of SupportClaw tool assumptions", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/supportClaw",
      tools: [],
      modelDisplay: "test/model",
    ...
    expect(prompt).not.toContain("SupportClaw lists the standard tools above");
    expect(prompt).not.toContain("This runtime enables:");
    expect(prompt).not.toContain("For long waits, avoid rapid poll loops");
    expect(prompt).not.toContain("Larger work: use `sessions_spawn`");
    expect(prompt).not.toContain("Do not poll `subagents list` / `sessions_list` in a loop");
    expect(prompt).toContain("No SupportClaw tool list is injected");
  });

  it("uses cwd, not bootstrap workspace, for CLI workspace guidance", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/supportClaw-agent",
      cwd: "/tmp/task-repo",
      tools: [],
      modelDisplay: "test/model",
    });
    ...
    expect(prompt).not.toContain("Your working directory is: /tmp/supportClaw-agent");
  });

  it("includes CLI-scoped plugin command guidance", () => {
    // Plugin command guidance is surface-filtered; CLI prompts must not leak
    // SupportClaw-main command text into external CLI backends.
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "SupportClaw-only command guidance.",
    ...
    expect(prompt).not.toContain("SupportClaw-only command guidance.");
  });
