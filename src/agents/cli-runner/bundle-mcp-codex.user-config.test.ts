/** Tests projecting SupportClaw user MCP servers into Codex app-server config. */
...
describe("buildCodexUserMcpServersThreadConfigPatch", () => {
  it("returns undefined when cfg has no mcp.servers (regression: #80814)", () => {
    ...
    expect(buildCodexUserMcpServersThreadConfigPatch({} as SupportClawConfig)).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: {} } as SupportClawConfig),
    ).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: { servers: {} } } as SupportClawConfig),
    ).toBeUndefined();
  });
  ...
  it("filters Codex-scoped user MCP servers by SupportClaw agent id", () => {
    // Agent-scoped MCP servers should follow the active SupportClaw agent, while
  ...
  it("omits scoped Codex MCP servers when no SupportClaw agent id is available", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          atlas: {
