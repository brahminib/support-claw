...
import { extensionForMime } from "@supportclaw/media-core/mime";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalLowercaseString,
} from "@supportclaw/normalization-core/string-coerce";
import { KeyedAsyncQueue } from "supportClaw/plugin-sdk/keyed-async-queue";
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".supportClaw-cli-images");
  }
  return path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const workspace = await tempWorkspace({
    ...
    prefix: "supportClaw-cli-system-prompt-",
  });
