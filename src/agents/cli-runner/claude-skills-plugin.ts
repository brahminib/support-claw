/**
 * Materializes selected SupportClaw skills as a temporary Claude CLI plugin.
 */
...
const SUPPORT_CLAW_CLAUDE_PLUGIN_NAME = "supportClaw-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredSupportClawTmpDir(), "supportClaw-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, SUPPORT_CLAW_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: SUPPORT_CLAW_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped SupportClaw skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
