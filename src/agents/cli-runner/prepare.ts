...
const prepareDeps = {
  ...
  resolveSupportClawReferencePaths: async (
    params: Parameters<typeof import("../docs-path.js").resolveSupportClawReferencePaths>[0],
  ) => (await import("../docs-path.js")).resolveSupportClawReferencePaths(params),
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          SUPPORT_CLAW_MCP_TOKEN: prepareDeps.resolveMcpLoopbackBearerToken(
            mcpLoopbackRuntime,
            params.senderIsOwner === true,
          ),
          SUPPORT_CLAW_MCP_AGENT_ID: sessionAgentId ?? "",
          SUPPORT_CLAW_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          SUPPORT_CLAW_MCP_SESSION_KEY: params.sessionKey ?? "",
          SUPPORT_CLAW_MCP_SESSION_ID: params.sessionId,
          SUPPORT_CLAW_MCP_MESSAGE_CHANNEL: params.messageChannel ?? params.messageProvider ?? "",
          SUPPORT_CLAW_MCP_CURRENT_CHANNEL_ID: params.currentChannelId ?? "",
          SUPPORT_CLAW_MCP_CURRENT_THREAD_TS: params.currentThreadTs ?? "",
          SUPPORT_CLAW_MCP_CURRENT_MESSAGE_ID:
            params.currentMessageId != null ? String(params.currentMessageId) : "",
          SUPPORT_CLAW_MCP_CURRENT_INBOUND_AUDIO: params.currentInboundAudio === true ? "true" : "",
          SUPPORT_CLAW_MCP_INBOUND_EVENT_KIND: params.currentInboundEventKind ?? "",
          SUPPORT_CLAW_MCP_SOURCE_REPLY_DELIVERY_MODE: params.sourceReplyDeliveryMode ?? "",
          SUPPORT_CLAW_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET: requireExplicitMessageTarget ? "true" : "",
  ...
  let supportClawHistoryMessages: unknown[] | undefined;
  const loadSupportClawHistoryMessages = async () => {
    supportClawHistoryMessages ??= await loadCliSessionHistoryMessages({
      sessionId: params.sessionId,
    ...
    return supportClawHistoryMessages;
  };
  ...
  const supportClawReferences = isSideQuestion
    ? { docsPath: null, sourcePath: null }
    : await prepareDeps.resolveSupportClawReferencePaths({
        workspaceDir,
  ...
  const builtSystemPrompt = isSideQuestion
    ...
    : buildCliAgentSystemPrompt({
        ...
        heartbeatPrompt,
        docsPath: supportClawReferences.docsPath ?? undefined,
        sourcePath: supportClawReferences.sourcePath ?? undefined,
  ...
  if (!isSideQuestion) {
    ...
    try {
      const hookResult = await resolvePromptBuildHookResult({
        ...
        messages: await loadSupportClawHistoryMessages(),
        hookCtx: {
          runId: params.runId,
          agentId: sessionAgentId,
          sessionKey: params.sessionKey,
  ...
  const shouldPrepareSupportClawHistoryPrompt =
    !isSideQuestion && (!reusableCliSession.sessionId || allowRawTranscriptReseed);
  const supportClawHistoryPrompt = shouldPrepareSupportClawHistoryPrompt
    ? buildCliSessionHistoryPrompt({
  ...
  try {
    ...
    return {
      ...
      claudeSkillsPluginArgs: claudeSkillsPlugin.args,
      bootstrapPromptWarningLines: bootstrapPromptWarning.lines,
      ...(supportClawHistoryPrompt ? { supportClawHistoryPrompt } : {}),
      heartbeatPrompt,
      authEpoch,
      authEpochVersion: CLI_AUTH_EPOCH_VERSION,
      extraSystemPromptHash,
