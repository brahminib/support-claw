...
describe("prepareCliBundleMcpConfig", () => {
  it("injects a strict empty --mcp-config overlay for bundle-MCP-enabled backends without servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "supportClaw-cli-bundle-mcp-empty-",
    );
  ...
  it("loads workspace bundle MCP plugins from the configured workspace root", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "supportClaw-cli-bundle-mcp-workspace-root-",
    );
    const pluginRoot = path.join(workspaceDir, ".supportClaw", "extensions", "workspace-probe");
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    const prepared = await prepareBundleProbeCliConfig({
      additionalConfig,
      env: {
        SUPPORT_CLAW_MCP_TOKEN: "loopback-token-123",
    ...
    const otherEnvPrepared = await prepareBundleProbeCliConfig({
      additionalConfig,
      env: {
        SUPPORT_CLAW_MCP_TOKEN: "other-loopback-token",
        SUPPORT_CLAW_MCP_CLI_CAPTURE_KEY: "",
      },
    });
    ...
    expect(Object.keys(raw.mcpServers ?? {}).toSorted()).toEqual(["bundleProbe", "supportClaw"]);
    expect(raw.mcpServers?.supportClaw?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.supportClaw?.headers?.Authorization).toBe("Bearer loopback-token-123");
    expect(raw.mcpServers?.supportClaw?.headers?.["x-supportClaw-cli-capture-key"]).toBe("");
    ...
    expect(attemptRaw.mcpServers?.supportClaw?.headers?.Authorization).toBe(
      "Bearer loopback-token-123",
    );
    expect(attemptRaw.mcpServers?.supportClaw?.headers?.["x-supportClaw-cli-capture-key"]).toBe(
      "attempt-123",
    );
  ...
  it("preserves extra env values alongside generated MCP config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "supportClaw-cli-bundle-mcp-env-",
    );
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      env: {
        SUPPORT_CLAW_MCP_TOKEN: "loopback-token-123",
        SUPPORT_CLAW_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
      },
    });
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/supportClaw-bundle-mcp-disabled",
    });
