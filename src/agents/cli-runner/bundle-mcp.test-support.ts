...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    envSnapshot = captureEnv(["SUPPORT_CLAW_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("supportClaw-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("supportClaw-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("supportClaw-cli-bundle-mcp-bundled-");
    setTestEnvValue("SUPPORT_CLAW_BUNDLED_PLUGINS_DIR", emptyBundledDir);
...
function createEnabledBundleProbeConfig(): SupportClawConfig {
  return {
    plugins: {
      entries: {
