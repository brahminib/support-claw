...
function normalizeSupportClawLoopbackUrl(value: string): string {
  const match =
    /^(http:\/\/(?:127\.0\.0\.1|localhost|\[::1\])):\d+(\/mcp)$/.exec(value.trim()) ?? undefined;
  if (!match) {
    return value;
  }
  return `${match[1]}:<supportClaw-loopback>${match[2]}`;
}

function canonicalizeBundleMcpConfigForResume(config: BundleMcpConfig): BundleMcpConfig {
  // The SupportClaw loopback MCP port changes across runs. Replace it before
  ...
  const canonicalServers = Object.fromEntries(
    Object.entries(config.mcpServers).map(([name, server]) => {
      if (name !== "supportClaw" || typeof server.url !== "string") {
      ...
      return [
        ...
        sortJsonValue({
          ...server,
          url: normalizeSupportClawLoopbackUrl(server.url),
        }),
      ];
    }),
...
const SUPPORT_CLAW_MCP_ENV_TEMPLATE_PATTERN = /\$\{(SUPPORT_CLAW_MCP_[A-Z0-9_]+)\}/g;

function resolveSupportClawMcpEnvTemplates(value: unknown, env?: Record<string, string>): unknown {
  if (!env) {
    return value;
  }
  if (typeof value === "string") {
    return value.replace(SUPPORT_CLAW_MCP_ENV_TEMPLATE_PATTERN, (match, name: string) => {
      return Object.hasOwn(env, name) ? env[name] : match;
    });
  }
  if (Array.isArray(value)) {
    return value.map((entry) => resolveSupportClawMcpEnvTemplates(entry, env));
  }
  if (!isRecord(value)) {
    return value;
  }
  return Object.fromEntries(
    Object.entries(value).map(([key, entry]) => [key, resolveSupportClawMcpEnvTemplates(entry, env)]),
  );
}
...
async function prepareModeSpecificBundleMcpConfig(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-cli-mcp-"));
  const mcpConfigPath = path.join(tempDir, "mcp.json");
  const runtimeConfig = resolveSupportClawMcpEnvTemplates(
    params.mergedConfig,
    params.env,
  ) as BundleMcpConfig;
...
export async function prepareCliBundleMcpCaptureAttempt(params: {
  ...
  return {
    env: {
      ...params.env,
      SUPPORT_CLAW_MCP_CLI_CAPTURE_KEY: params.captureKey,
    },
  };
}
