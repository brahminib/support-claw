...
const SUPPORT_CLAW_MCP_TOOL_PREFIX = "mcp__supportClaw__";

function normalizeCliMessagingToolName(toolName: string): string {
  return toolName.startsWith(SUPPORT_CLAW_MCP_TOOL_PREFIX)
    ? toolName.slice(SUPPORT_CLAW_MCP_TOOL_PREFIX.length)
    : toolName;
}
...
const CLI_BACKEND_PRESERVE_ENV = "SUPPORT_CLAW_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.SUPPORT_CLAW_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.SUPPORT_CLAW_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.SUPPORT_CLAW_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.SUPPORT_CLAW_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.SUPPORT_CLAW_MCP_MESSAGE_CHANNEL || "<empty>"}`,
  ].join(" ");
}
...
export async function executePreparedCliRun(
  ...
  const basePrompt = cliSessionIdToUse
    ? params.prompt
    : (context.supportClawHistoryPrompt ?? params.prompt);
      ...
      try {
        cliBackendLog.info(
          buildCliExecLogLine({
            ...
            hasHistoryPrompt: Boolean(context.supportClawHistoryPrompt),
          }),
        );
        const logOutputText =
        ...
        if (logOutputText) {
          ...
          if (env.SUPPORT_CLAW_MCP_TOKEN || env.SUPPORT_CLAW_MCP_SESSION_KEY || env.SUPPORT_CLAW_MCP_AGENT_ID) {
          ...
          if (result.exitCode !== 0 || result.reason !== "exit") {
            if (result.reason === "no-output-timeout" || result.noOutputTimedOut) {
              ...
              const deferWatchdogNoticeForFreshRetry =
                ...
                Boolean(context.supportClawHistoryPrompt) &&
