...
import {
  compareSupportClawVersions,
  isSameSupportClawStableFamily,
  parseSupportClawVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseSupportClawVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseSupportClawVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseSupportClawVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseSupportClawVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseSupportClawVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseSupportClawVersion("2026.3")).toBeNull();
    expect(parseSupportClawVersion("latest")).toBeNull();
  });
});

describe("compareSupportClawVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareSupportClawVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareSupportClawVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareSupportClawVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareSupportClawVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareSupportClawVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareSupportClawVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameSupportClawStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameSupportClawStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameSupportClawStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameSupportClawStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameSupportClawStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
