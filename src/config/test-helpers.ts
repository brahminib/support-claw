...
import { resetConfigRuntimeState, type SupportClawConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        SUPPORT_CLAW_MPM_CATALOG_PATHS: undefined,
...
export async function writeSupportClawConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.SUPPORT_CLAW_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected SUPPORT_CLAW_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeSupportClawConfig(home, config);
