...
import { normalizeConfiguredProviderCatalogModelId } from "@supportclaw/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  supportClaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  supportClaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
export function unsetPathForWrite(
  root: SupportClawConfig,
  pathSegments: string[],
): { changed: boolean; next: SupportClawConfig } {
