...
/**
 * Nix mode detection: When SUPPORT_CLAW_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.SUPPORT_CLAW_NIX_MODE === "1";
}
...
const NEW_STATE_DIRNAME = ".supportClaw";
const CONFIG_FILENAME = "supportClaw.json";
...
/** Build a homedir thunk that respects SUPPORT_CLAW_HOME for the given env. */
 ...
 * Can be overridden via SUPPORT_CLAW_STATE_DIR.
 * Default: ~/.supportClaw
 */
export function resolveStateDir(
  ...
  const override = env.SUPPORT_CLAW_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, effectiveHomedir);
  }
  const newDir = newStateDir(effectiveHomedir);
  if (env.SUPPORT_CLAW_TEST_FAST === "1") {
    return newDir;
  }
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const supportClawOverride = env.SUPPORT_CLAW_STATE_DIR?.trim();
  if (supportClawOverride) {
    env.SUPPORT_CLAW_STATE_DIR = resolveUserPath(supportClawOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `SUPPORT_CLAW_INCLUDE_ROOTS` as a
 ...
 * directory containing `supportClaw.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.SUPPORT_CLAW_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via SUPPORT_CLAW_CONFIG_PATH.
 * Default: ~/.supportClaw/supportClaw.json (or $SUPPORT_CLAW_STATE_DIR/supportClaw.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.SUPPORT_CLAW_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.SUPPORT_CLAW_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.SUPPORT_CLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const supportClawStateDir = env.SUPPORT_CLAW_STATE_DIR?.trim();
  if (supportClawStateDir) {
    const resolved = resolveUserPath(supportClawStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/supportClaw-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `supportClaw-${uid}` : "supportClaw";
 ...
 *
 * Precedence:
 * - `SUPPORT_CLAW_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.SUPPORT_CLAW_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.SUPPORT_CLAW_GATEWAY_PORT?.trim();
