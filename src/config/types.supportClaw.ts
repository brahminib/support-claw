// Defines the top-level SupportClaw configuration type.
...
/** Top-level SupportClaw config as read from user/project config files. */
export type SupportClawConfig = {
  /** JSON schema URL used by editors and generated config files. */
  $schema?: string;
  meta?: {
    /** Last SupportClaw version that wrote this config. */
  ...
  wizard?: {
    ...
    lastRunAt?: string;
    /** SupportClaw version used by the last completed wizard run. */
  ...
  ui?: {
    /** Accent color for SupportClaw UI chrome (hex). */
    seamColor?: string;
    assistant?: {
...
export type SupportClawConfigInput = Omit<SupportClawConfig, "models"> & {
  models?: ModelsConfigInput;
};

declare const supportClawConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = SupportClawConfig & {
  readonly [supportClawConfigStateBrand]?: TState;
};
