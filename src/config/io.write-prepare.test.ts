...
describe("config io write prepare", () => {
      ...
      resolvePersistCandidateForWrite({
        ...
        sourceConfig: {
          plugins: {
            entries: {},
            installs: {
              "supportClaw-web-search": {
                source: "npm",
                spec: "@ollama/supportClaw-web-search",
                installPath: "/tmp/supportClaw-web-search",
                resolvedName: "@ollama/supportClaw-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "supportClaw-web-search": {
                source: "npm",
                spec: "@ollama/supportClaw-web-search@0.2.2",
                installPath: "/tmp/supportClaw-web-search",
                resolvedName: "@ollama/supportClaw-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("supportClaw config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('supportClaw config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/supportClaw.json",
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: SupportClawConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://supportClaw.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
