...
function isBlockedConfigEnvVar(key: string): boolean {
  return (
    ...
    key.toUpperCase() === "SUPPORT_CLAW_INCLUDE_ROOTS" ||
...
function collectConfigEnvVarsByTarget(cfg?: SupportClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: SupportClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** Collects config env vars safe to persist into managed service environments. */
export function collectConfigServiceEnvVars(cfg?: SupportClawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.SUPPORT_CLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
