...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.SUPPORT_CLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveSupportClawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    SUPPORT_CLAW_STATE_DIR: path.join(os.tmpdir(), "supportClaw-config-doc-baseline-state"),
