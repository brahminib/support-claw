...
import type { DB as SupportClawStateKyselyDatabase } from "../state/supportClaw-state-db.generated.js";
import {
...
import type { ConfigFileSnapshot, SupportClawConfig } from "./types.supportClaw.js";

const CONFIG_CLOBBER_SNAPSHOT_LIMIT = 32;
type ConfigHealthDatabase = Pick<SupportClawStateKyselyDatabase, "config_health_entries">;
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "supportClaw-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    return withEnvAsync(
      {
        SUPPORT_CLAW_DEFER_SHELL_ENV_FALLBACK: undefined,
        SUPPORT_CLAW_LOAD_SHELL_ENV: undefined,
        SUPPORT_CLAW_SHELL_ENV_TIMEOUT_MS: undefined,
      },
      () => fn(home),
    );
  }
  ...
  function readConfigHealthRow(home: string, configPath: string) {
    const { db } = openSupportClawStateDatabase({ env: { HOME: home } as NodeJS.ProcessEnv });
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { SUPPORT_CLAW_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("writes health state to SQLite through public config reads", async () => {
    await withSuiteHome(async (home) => {
      ...
      const healthPath = path.join(home, ".supportClaw", "logs", "config-health.json");
  ...
  it("loads shipped plugin install config records without mutating config or plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const pluginDir = path.join(home, ".supportClaw", "plugins", "demo");
      const manifestPath = path.join(pluginDir, "supportClaw.plugin.json");
      ...
      try {
        ...
        await expect(
          readPersistedInstalledPluginIndex({
            stateDir: path.join(home, ".supportClaw"),
          }),
        ).resolves.toBeNull();
        await expect(fs.readFile(configPath, "utf-8")).resolves.toBe(initialRaw);
      } finally {
  ...
  it("retains included shipped plugin install records in write snapshots", async () => {
    await withSuiteHome(async (home) => {
      const configDir = path.join(home, ".supportClaw");
  ...
  it("migrates shipped plugin install config records during explicit writes even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const pluginDir = path.join(home, ".supportClaw", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const unwritableStatePath = path.join(home, ".supportClaw");
  ...
  it("keeps writes inside an SUPPORT_CLAW_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".supportClaw", "supportClaw.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "supportClaw.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "supportClaw.json"), "utf-8"),
      ) as {
  ...
  it("prints missing-meta write anomalies when anomaly logging is requested", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          SUPPORT_CLAW_CONFIG_WRITE_ANOMALY_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          SUPPORT_CLAW_TEST_CONFIG_OVERWRITE_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("preserves root $schema during partial writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(persisted.$schema).toBe("https://supportClaw.ai/config.json");
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/supportClaw-main" }] },
  ...
  it("keeps authored agent provider params during narrowed internal agent writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as SupportClawConfig;
  ...
  it("returns the read-time environment snapshot for invalid config repairs", async () => {
    await withSuiteHome(async (home) => {
        ...
        `${JSON.stringify(
          {
            gateway: {
              mode: "local",
              auth: { mode: "token", token: "${SUPPORT_CLAW_GATEWAY_TOKEN}" },
            },
            channels: { "test-plugin-channel": { enabled: true } },
          },
          null,
      ...
      const io = createConfigIO({
        env: {
          SUPPORT_CLAW_GATEWAY_TOKEN: "gateway-token-at-read",
          SUPPORT_CLAW_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(result.writeOptions.envSnapshotForRestore?.SUPPORT_CLAW_GATEWAY_TOKEN).toBe(
  ...
  it("returns the snapshot-time hash when an included file is malformed", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const includePath = path.join(home, ".supportClaw", "plugins.json5");
  ...
  it("returns a write guard that rejects a changed active config path", async () => {
    await withSuiteHome(async (home) => {
      const firstConfigPath = path.join(home, ".supportClaw", "first.json");
      const secondConfigPath = path.join(home, ".supportClaw", "second.json");
      ...
      const env = {
        SUPPORT_CLAW_CONFIG_PATH: firstConfigPath,
        SUPPORT_CLAW_TEST_FAST: "1",
      } as NodeJS.ProcessEnv;
      const io = createConfigIO({ env, homedir: () => home, logger: silentLogger });

      const result = await io.readConfigFileSnapshotForWrite();
      env.SUPPORT_CLAW_CONFIG_PATH = secondConfigPath;
  ...
  it("rejects local write ownership when config env changes path selection during the read", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const configuredNextPath = path.join(home, ".supportClaw", "next.json");
      const sourceConfig = {
        env: { SUPPORT_CLAW_CONFIG_PATH: configuredNextPath },
        gateway: { mode: "local" },
  ...
  it("follows config env path selection before returning global write ownership", async () => {
    await withSuiteHome(async (home) => {
      ...
      const initialConfig = JSON.parse(await fs.readFile(configPath, "utf-8")) as SupportClawConfig;
  ...
  it("does not use expectedConfigPath as the write destination", async () => {
    await withSuiteHome(async (home) => {
      const expectedConfigPath = path.join(home, ".supportClaw", "expected.json");
      const activeConfigPath = path.join(home, ".supportClaw", "active.json");
      ...
      await withEnvAsync(
        {
          SUPPORT_CLAW_CONFIG_PATH: activeConfigPath,
          SUPPORT_CLAW_TEST_FAST: "1",
        },
        async () => {
          await writeConfigFile(
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const includePath = path.join(home, ".supportClaw", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://supportClaw.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
  ...
  it("rejects invalid include-backed repairs instead of persisting substituted secrets", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const includePath = path.join(home, ".supportClaw", "gateway.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify(
      ...
      const io = createConfigIO({
        env: {
          SUPPORT_CLAW_GATEWAY_TOKEN: "gateway-token-runtime",
          SUPPORT_CLAW_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("repairs invalid root-authored siblings without flattening included config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const includePath = path.join(home, ".supportClaw", "agent-defaults.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
  ...
  it("rejects repairs that would flatten a valid outer include with a broken nested include", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const pluginsPath = path.join(home, ".supportClaw", "plugins.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        pluginsPath,
  ...
  it("preserves escaped root literals before validating unrelated includes", async () => {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/supportClaw-test-literal-plugin",
          source: "/tmp/supportClaw-test-literal-plugin/index.ts",
          manifestPath: "/tmp/supportClaw-test-literal-plugin/supportClaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const agentsPath = path.join(home, ".supportClaw", "agents.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        agentsPath,
  ...
  it("repairs invalid config without flattening array-nested includes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const includePath = path.join(home, ".supportClaw", "main-agent.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ id: "main", workspace: "${SUPPORT_CLAW_AGENT_WORKSPACE}" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      const io = createConfigIO({
        env: {
          SUPPORT_CLAW_AGENT_WORKSPACE: "/resolved/agent-workspace",
          SUPPORT_CLAW_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      await expect(fs.readFile(includePath, "utf-8")).resolves.toContain(
        '"workspace": "${SUPPORT_CLAW_AGENT_WORKSPACE}"',
      );
    });
  });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/supportClaw-test-required-plugin",
          source: "/tmp/supportClaw-test-required-plugin/index.ts",
          manifestPath: "/tmp/supportClaw-test-required-plugin/supportClaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
  ...
  it("writes runtime-derived edits back to source SecretRef markers", async () => {
    await withSuiteHome(async (home) => {
      ...
      await withEnvAsync({ SUPPORT_CLAW_CONFIG_PATH: configPath }, async () => {
        setRuntimeConfigSnapshot(
          {
            gateway: { mode: "local" },
            models: {
      ...
      try {
        await withEnvAsync(
          ...
          async () => {
            ...
            expect(persisted.gateway?.auth?.token).toBe("${SUPPORT_CLAW_GATEWAY_TOKEN}");
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/supportClaw-test-demo",
          source: "/tmp/supportClaw-test-demo/index.ts",
          manifestPath: "/tmp/supportClaw-test-demo/supportClaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
      ...
      try {
        await withEnvAsync({ SUPPORT_CLAW_CONFIG_PATH: configPath }, async () => {
          ...
          const postWriteSnapshot = await createConfigIO({
            env: { SUPPORT_CLAW_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
            homedir: () => home,
            logger: silentLogger,
  ...
  it("rolls back the root config when post-write runtime refresh fails", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies SupportClawConfig;
  ...
  it("keeps plugin install index migration when runtime refresh fails", async () => {
    await withSuiteHome(async (home) => {
      const stateDir = path.join(home, ".supportClaw");
      const configPath = path.join(stateDir, "supportClaw.json");
  ...
  it("blocks runtime preflight failures before committing root writes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const initialRaw = `${JSON.stringify({ gateway: { mode: "local" } }, null, 2)}\n`;
      let observedSource: SupportClawConfig | undefined;
  ...
  it("restores config env vars when post-write runtime refresh rollback succeeds", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const envKey = "SUPPORT_CLAW_TEST_RUNTIME_ROLLBACK_ENV";
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies SupportClawConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
  ...
  it("rolls back root writes when canonical reread changes config path ownership", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const nextConfigPath = path.join(home, ".supportClaw", "next.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies SupportClawConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        async () => {
          ...
          await expect(
            writeConfigFile(
              {
                gateway: { mode: "local", port: 19001 },
                env: { SUPPORT_CLAW_CONFIG_PATH: nextConfigPath },
              },
              {
                baseSnapshot: prepared.snapshot,
                ...prepared.writeOptions,
  ...
  it("uses injected filesystem operations when rolling back ownership loss", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".supportClaw", "supportClaw.json");
      const otherConfigPath = path.join(home, ".supportClaw", "other.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies SupportClawConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        promises: {
          ...
          rename: async (from, to) => {
            ...
            if (!committed && to === configPath) {
              committed = true;
              env.SUPPORT_CLAW_CONFIG_PATH = otherConfigPath;
            }
          },
        },
      } as typeof fsNode;
    ...
    await withSuiteHome(async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/supportClaw-test-strict-plugin",
            source: "/tmp/supportClaw-test-strict-plugin/index.ts",
            manifestPath: "/tmp/supportClaw-test-strict-plugin/supportClaw.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await withEnvAsync({ SUPPORT_CLAW_CONFIG_PATH: configPath }, async () => {
          ...
          await expect(
            writeConfigFile({ agents: { list: "not-array" } } as unknown as SupportClawConfig, {
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/supportClaw-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "supportClaw-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
        { baseSnapshot: snapshot },
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as SupportClawConfig;
      expect(persisted.logging?.file).toBe("~/supportClaw-upgrade-survivor/gateway.jsonl");
