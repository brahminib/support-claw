...
function isOpenAiCodexDefaultRuntimeSelection(params: {
  ...
  // Custom OpenAI-compatible base URLs stay on the SupportClaw runtime path.
...
function codexPluginEntryEnabled(cfg: SupportClawConfig): boolean | undefined {
...
function openAiProviderRuntimePolicy(cfg: SupportClawConfig): AgentRuntimePolicyConfig | undefined {
...
function listConfiguredAgentIds(cfg: SupportClawConfig): Array<string | undefined> {
...
function openAiHasCodexDefaultRuntimePolicy(cfg: SupportClawConfig): boolean {
  for (const [providerId, providerConfig] of Object.entries(cfg.models?.providers ?? {})) {
    if (normalizeProviderId(providerId) !== OPENAI_PROVIDER_ID) {
      continue;
    }
    ...
    // A model-scoped explicit "auto"/"default" overrides provider-wide PI/SupportClaw
...
function openAiDefaultRouteKeepsCodexUnavailable(cfg: SupportClawConfig): boolean {
  const policy = openAiDefaultRouteRuntimePolicy(cfg);
  if (!policy?.id?.trim()) {
    // With no explicit runtime policy, the OpenAI route only needs Codex on the
    // official OpenAI endpoint. OpenAI-compatible proxies stay on SupportClaw.
    return !openAIProviderUsesCodexRuntimeByDefault({
      provider: OPENAI_PROVIDER_ID,
      config: cfg,
    });
 ...
 */
export function configExplicitlyKeepsCodexUnavailableForOpenAi(cfg: SupportClawConfig): boolean {
 ...
 * all default routes would otherwise stay on the SupportClaw runtime.
 */
export function shouldSuppressMissingCodexPluginDiagnostics(cfg: SupportClawConfig): boolean {
