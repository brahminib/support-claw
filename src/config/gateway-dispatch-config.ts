...
function projectGatewayDispatchConfig(value: unknown): SupportClawConfig {
  if (!isPlainRecord(value)) {
    return {};
  }
  ...
  return projected as SupportClawConfig;
}

// Main session keys are process-local; Gateway dispatch always sees the canonical main key.
function applyGatewayDispatchSessionDefaults(config: SupportClawConfig): SupportClawConfig {
...
function resolveGatewayDispatchEnvVars(config: unknown, env: NodeJS.ProcessEnv): unknown {
  if (isPlainRecord(config) && Object.hasOwn(config, "env")) {
    applyConfigEnvVars(config as SupportClawConfig, env);
  }
