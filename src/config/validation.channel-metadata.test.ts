...
describe("validateConfigObjectRawWithPlugins channel metadata", () => {
    ...
    if (!result.ok) {
      expect(result.issues).toContainEqual(
        expect.objectContaining({
          ...
          message:
            'invalid config for plugin supportClaw-lark: must not have additional properties: "unsupportedField"',
        }),
      );
    }
  });
  ...
  it("sanitizes the schema owner in validation diagnostics", () => {
    const unsafeId = `supportClaw${String.fromCharCode(10)}${String.fromCharCode(27)}[31m-lark`;
    const registry = createExternalFeishuSchemaRegistry();
    registry.plugins[0] = {
      ...registry.plugins[0],
      id: unsafeId,
  ...
  it("keeps raw channel validation diagnostics plugin-agnostic", () => {
    ...
    if (!result.ok) {
      ...
      expect(result.issues[0]?.message).not.toContain("supportClaw doctor --fix");
    }
  });
});
