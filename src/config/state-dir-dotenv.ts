...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, SupportClaw env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still SupportClaw-managed keys:
 ...
 * literal references for keys SupportClaw previously managed.
 */
...
/**
 * Read and parse `~/.supportClaw/.env` (or `$SUPPORT_CLAW_STATE_DIR/.env`), returning
