...
import { withTempHome, writeSupportClawConfig } from "./test-helpers.js";
...
describe("readBestEffortConfig", () => {
  ...
  it("can read snapshots without updating config observation state", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeSupportClawConfig(home, {
        gateway: { mode: "local" },
      });

      await readConfigFileSnapshot({ observe: false });

      const healthPath = `${home}/.supportClaw/logs/config-health.json`;
  ...
  it("can read snapshots without applying config env vars to the process", async () => {
    await withTempHome(async (home) => {
      const key = "SUPPORT_CLAW_ISOLATED_CONFIG_READ_TEST";
      await withEnvAsync({ [key]: undefined }, async () => {
        await writeSupportClawConfig(home, {
  ...
  it("resolves config env above exact lower-precedence values in isolated snapshots", async () => {
    await withTempHome(async (home) => {
      const key = "SUPPORT_CLAW_GATEWAY_TOKEN";
  ...
  it("can read best-effort config without applying env vars or recording observation", async () => {
    await withTempHome(async (home) => {
      const key = "SUPPORT_CLAW_ISOLATED_BEST_EFFORT_CONFIG_TEST";
      await withEnvAsync({ [key]: undefined }, async () => {
        await writeSupportClawConfig(home, {
          env: { vars: { [key]: "from-config" } },
          gateway: { mode: "local" },
        ...
        expect(process.env[key]).toBeUndefined();
        await expect(fs.stat(`${home}/.supportClaw/logs/config-health.json`)).rejects.toMatchObject({
          code: "ENOENT",
        });
      });
    });
  ...
  it("preserves Windows case-insensitive env lookup in isolated reads", async () => {
    await withTempHome(async (home) => {
      const mixedCaseKey = "SupportClaw_Config_Path";
      const customConfigPath = `${home}/custom-supportClaw.json`;
  ...
  it("does not restore suspicious direct edits from .bak during ordinary reads", async () => {
    await withTempHome(async (home) => {
      ...
      const entries = await fs.readdir(`${home}/.supportClaw`);
      expect(entries.some((entry) => entry.startsWith("supportClaw.json.clobbered."))).toBe(false);
    });
  });
