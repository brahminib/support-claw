...
vi.mock("../plugins/bundled-dir.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveBundledPluginsDir: (env: NodeJS.ProcessEnv = process.env) =>
      env.SUPPORT_CLAW_BUNDLED_PLUGINS_DIR,
  };
});
...
function makeTempDir(): string {
  ...
  const dir = fs.mkdtempSync(path.join(trustedRoot, ".supportClaw-plugin-prefer-over-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin auto-enable preferOver", () => {
  it("tolerates bundled channel id metadata drift during auto-enable", async () => {
    ...
    vi.stubEnv("SUPPORT_CLAW_BUNDLED_PLUGINS_DIR", rootDir);
    ...
    vi.stubEnv("SUPPORT_CLAW_BUNDLED_PLUGINS_DIR", path.join(rootDir, "missing"));
    await setBundledPluginsDirFixture(undefined);
    ...
    const result = materializePluginAutoEnableCandidates({
      ...
      env: {
        ...
        SUPPORT_CLAW_BUNDLED_PLUGINS_DIR: path.join(rootDir, "missing"),
      },
