...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ SUPPORT_CLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-supportClaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-supportClaw#general");
  });
  ...
  it("derives session transcripts dir from SUPPORT_CLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SUPPORT_CLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when SUPPORT_CLAW_STATE_DIR differs from stored paths", () => {
