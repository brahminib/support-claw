...
type SupportClawMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_SUPPORT_CLAW_TRANSPORT: Record<string, SupportClawMcpHttpTransport | "stdio"> = {
...
/** Maps CLI-native MCP type aliases to SupportClaw HTTP transport names. */
export function resolveSupportClawMcpTransportAlias(
  value: unknown,
): SupportClawMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_SUPPORT_CLAW_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

/** Checks whether a raw MCP `type` value is a legacy CLI alias SupportClaw can rewrite. */
export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_SUPPORT_CLAW_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveSupportClawMcpTransportAlias(next.type);
  // `transport` is SupportClaw's canonical field; legacy `type` only fills a gap.
