...
import { isSensitiveUrlConfigPath } from "@supportclaw/net-policy/redact-sensitive-url";
...
describe("mapSensitivePaths", () => {
  ...
  it("main schema yields correct hints (samples)", () => {
    const schema = SupportClawSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "SupportClawConfig";
    const hints = mapSensitivePaths(SupportClawSchema, "", {});
...
describe("collectMatchingSchemaPaths", () => {
  it("finds base-config URL fields that may embed secrets", () => {
    const paths = collectMatchingSchemaPaths(SupportClawSchema, "", isSensitiveUrlConfigPath);
