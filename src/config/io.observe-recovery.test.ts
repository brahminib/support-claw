...
describe("config observe recovery", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "supportClaw-config-observe-recovery-"));
  });
  ...
  function makeDeps(
    ...
    return {
      ...
      configPath,
      auditPath: path.join(home, ".supportClaw", "logs", "config-audit.jsonl"),
      warn,
    };
  }
  ...
  it("loadConfig clears env vars from the discarded clobbered config before rereading backup", async () => {
    await withSuiteHome(async (home) => {
      ...
      await writeConfigRaw(configPath, {
        meta: { lastTouchedVersion: "2026.5.28" },
        env: { vars: { SUPPORT_CLAW_CLOBBER_ONLY: "bad" } },
      });

      const config = io.loadConfig();

      expect(config.gateway?.mode).toBe("local");
      expect(env.SUPPORT_CLAW_CLOBBER_ONLY).toBeUndefined();
    });
  });
  ...
  it("validates backup candidates without leaking their env into live state", async () => {
    await withSuiteHome(async (home) => {
      ...
      await seedConfigBackup(configPath, {
        gateway: { mode: "local" },
        env: { vars: { SUPPORT_CLAW_BACKUP_ONLY: "stale" } },
        agents: { defaults: { model: 123 } },
      });
      await writeConfigRaw(configPath, {
        meta: { lastTouchedVersion: "2026.5.28" },
      ...
      expect(env.SUPPORT_CLAW_BACKUP_ONLY).toBeUndefined();
    });
  });
  ...
  it("writes async health state to SQLite", async () => {
    await withSuiteHome(async (home) => {
      ...
      await expectPathMissing(path.join(home, ".supportClaw", "logs", "config-health.json"));
