...
import { normalizeOptionalLowercaseString } from "@supportclaw/normalization-core/string-coerce";
...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: SupportClawConfig): boolean {
...
function isProviderConfigured(cfg: SupportClawConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: SupportClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: SupportClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: SupportClawConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: SupportClawConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: SupportClawConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: SupportClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredSpeechProviderSelection(cfg: SupportClawConfig): boolean {
  return collectConfiguredSpeechProviderIds(cfg).size > 0;
}

function hasConfiguredPluginConfigEntry(cfg: SupportClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: SupportClawConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: SupportClawConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: SupportClawConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: SupportClawConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: SupportClawConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: SupportClawConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: SupportClawConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: SupportClawConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: SupportClawConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: SupportClawConfig): boolean {
  return (
...
function hasPluginEntries(cfg: SupportClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: SupportClawConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: SupportClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: SupportClawConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: SupportClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: SupportClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: SupportClawConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: SupportClawConfig, pluginId: string): boolean {
...
function disableImplicitPreferredOverPlugin(params: {
  config: SupportClawConfig;
  originalConfig: SupportClawConfig;
  pluginId: string;
...
function isBuiltInChannelAlreadyEnabled(cfg: SupportClawConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
