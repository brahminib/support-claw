...
/** Parses stable, prerelease, and legacy dot-beta SupportClaw versions. */
export function parseSupportClawVersion(raw: string | null | undefined): SupportClawVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeSupportClawVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseSupportClawVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameSupportClawStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseSupportClawVersion(a);
  const parsedB = parseSupportClawVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareSupportClawVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseSupportClawVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseSupportClawVersion(current);
  const parsedTouched = parseSupportClawVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameSupportClawStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareSupportClawVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: SupportClawVersion): number {
