...
const mockReadSourceConfigSnapshot = vi.hoisted(() => async () => {
  ...
  const configPath = pathValue.join(process.env.SUPPORT_CLAW_STATE_DIR ?? "", "supportClaw.json");
  try {
...
const mockReplaceConfigFile = vi.hoisted(() => async ({ nextConfig }: { nextConfig: unknown }) => {
  ...
  const configPath = pathLocal.join(process.env.SUPPORT_CLAW_STATE_DIR ?? "", "supportClaw.json");
