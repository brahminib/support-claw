...
import { SENSITIVE_URL_HINT_TAG } from "@supportclaw/net-policy/redact-sensitive-url";
...
describe("config schema", () => {
  ...
  it("validates MCP OAuth client metadata URLs against the SDK contract", () => {
        ...
        mcp: {
          servers: {
            docs: {
              ...
              auth: "oauth",
              oauth: {
                clientMetadataUrl: "https://client.example.com/supportClaw-mcp.json",
              },
            },
          },
        },
    ...
    for (const clientMetadataUrl of [
      "http://client.example.com/supportClaw-mcp.json",
  ...
  it("accepts exec command highlighting config in global and agent scopes", () => {
    ...
    const config = SupportClawSchema.parse({
      agents: {
        list: [
          {
            id: "main",
  ...
  it("accepts install policy exec config in the runtime zod schema", () => {
    const parsed = SupportClawSchema.parse({
      security: {
        installPolicy: {
          enabled: true,
          ...
          exec: {
            ...
            passEnv: ["SUPPORT_CLAW_STATE_DIR"],
    ...
    expect(parsed.security?.installPolicy?.exec?.command).toBe(
      "/usr/local/bin/supportClaw-install-policy",
    );
  });
