...
import type { SupportClawConfig } from "./types.js";

describe("config env vars", () => {
  it("applies env vars from env block when missing", async () => {
    await withEnvOverride({ OPENROUTER_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { OPENROUTER_API_KEY: "config-key" } } } as SupportClawConfig);
  ...
  it("applies config env above normalized lower-precedence aliases", () => {
    ...
    applyConfigEnvVars({ env: { vars: { Z_AI_API_KEY: "config-key" } } } as SupportClawConfig, env, {
  ...
  it("preserves a higher-precedence normalized alias", () => {
    ...
    applyConfigEnvVars({ env: { vars: { ZAI_API_KEY: "config-key" } } } as SupportClawConfig, env, {
      lowerPrecedenceEnv: { ZAI_API_KEY: "shell-key" },
    });
  ...
  it.runIf(process.platform !== "win32")("keeps unrelated POSIX env casing distinct", () => {
    ...
    applyConfigEnvVars({ env: { vars: { foo: "config-key" } } } as SupportClawConfig, env);
  ...
  it("applies env vars from env.vars when missing", async () => {
    await withEnvOverride({ GROQ_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { GROQ_API_KEY: "gsk-config" } } } as SupportClawConfig);
  ...
  it("preserves Windows case-insensitive env precedence in merged runtime env", () => {
    ...
    try {
      const merged = createConfigRuntimeEnv(
        { env: { vars: { SUPPORT_CLAW_LOAD_SHELL_ENV: "1" } } } as SupportClawConfig,
        { SupportClaw_Load_Shell_Env: "0" },
      );

      expect(merged.SUPPORT_CLAW_LOAD_SHELL_ENV).toBe("0");
      expect(Object.keys(merged)).toEqual(["SupportClaw_Load_Shell_Env"]);
    } finally {
    ...
    await withEnvOverride(
      {
        ...
        supportClaw_allow_older_binary_destructive_actions: undefined,
      ...
      async () => {
        const config = {
          env: {
            vars: {
              ...
              SUPPORT_CLAW_INCLUDE_ROOTS: "/tmp/evil-include-root",
              supportClaw_allow_older_binary_destructive_actions: "1",
              OPENROUTER_API_KEY: "config-key",
            },
          },
        };
        const entries = collectConfigRuntimeEnvVars(config as SupportClawConfig);
        ...
        expect(entries.SUPPORT_CLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
        expect(entries.SUPPORT_CLAW_INCLUDE_ROOTS).toBeUndefined();
        expect(entries.supportClaw_allow_older_binary_destructive_actions).toBeUndefined();
        expect(entries.OPENROUTER_API_KEY).toBe("config-key");

        applyConfigEnvVars(config as SupportClawConfig);
        ...
        expect(process.env.supportClaw_allow_older_binary_destructive_actions).toBeUndefined();
        expect(process.env.OPENROUTER_API_KEY).toBe("config-key");
      },
    );
  });
  ...
  it("loads ${VAR} substitutions from ~/.supportClaw/.env on repeated runtime loads", async () => {
    await withTempHome(async (_home) => {
      await withEnvOverride({ BRAVE_API_KEY: undefined }, async () => {
        const stateDir = process.env.SUPPORT_CLAW_STATE_DIR?.trim();
        if (!stateDir) {
          throw new Error("Expected SUPPORT_CLAW_STATE_DIR to be set by withTempHome");
        }
        ...
        const first = resolveConfigEnvVars(config, process.env) as SupportClawConfig;
        expect(first.tools?.web?.search?.apiKey).toBe("from-dotenv");

        delete process.env.BRAVE_API_KEY;
        loadDotEnv({ quiet: true });
        const second = resolveConfigEnvVars(config, process.env) as SupportClawConfig;
  ...
  it("drops dangerous and empty values from the state-dir .env file", async () => {
    await withTempHome(async (_home) => {
      ...
      expect(vars.SUPPORT_CLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
      expect(vars.EMPTY).toBeUndefined();
      expect(vars.VALID).toBe("ok");
    });
  });

  it("respects SUPPORT_CLAW_STATE_DIR when reading state-dir .env vars", async () => {
    await withTempHome(async (_home) => {
      const customStateDir = path.join(process.env.SUPPORT_CLAW_STATE_DIR ?? "", "custom-state");
      await writeStateDirDotEnv("CUSTOM_KEY=from-override\n", {
        stateDir: customStateDir,
      });
      expect(
        readStateDirDotEnvVars({
          SUPPORT_CLAW_STATE_DIR: customStateDir,
        }).CUSTOM_KEY,
      ).toBe("from-override");
    });
  });
