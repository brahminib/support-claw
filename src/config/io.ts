// Loads, validates, migrates, snapshots, and writes SupportClaw config files.
import crypto from "node:crypto";
import fs from "node:fs";
import os from "node:os";
import path from "node:path";
import { collectManifestModelIdNormalizationPolicies } from "@supportclaw/model-catalog-core/provider-model-id-normalization";
...
import type { SupportClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
export type ConfigWriteResult = { persistedHash: string; persistedConfig: SupportClawConfig };
...
export type ConfigWriteOptions = {
   ...
   */
  explicitSetValueSource?: SupportClawConfig;
  /**
   ...
   */
  preCommitRuntimePreflight?: (sourceConfig: SupportClawConfig) => Promise<unknown>;
...
function coerceConfig(value: unknown): SupportClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SupportClawConfig;
}
...
export type ConfigSnapshotReadOptions = {
  ...
  allowSuspiciousRecovery?: (
    candidate: SupportClawConfig,
    current: SupportClawConfig,
...
function stampConfigVersion(cfg: SupportClawConfig, version?: string): SupportClawConfig {
  return stampConfigWriteMetadata(cfg, new Date().toISOString(), version);
}

function warnIfConfigFromFuture(cfg: SupportClawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      [
        `Your SupportClaw config was written by version ${touched}, but this command is running ${VERSION}.`,
...
function shouldSuppressFutureVersionWarningForEnv(env: NodeJS.ProcessEnv): boolean {
  return (
    isTruthyEnvValue(env.SUPPORT_CLAW_UPDATE_IN_PROGRESS) ||
    isTruthyEnvValue(env.SUPPORT_CLAW_UPDATE_POST_CORE)
  );
}
...
export function parseConfigJson5(
  ...
  try {
    ...
    // on the JSON files SupportClaw writes itself.
  }
  try {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as SupportClawConfig, env, { lowerPrecedenceEnv });
  }
...
function createConfigFileSnapshot(params: {
  ...
  sourceConfig: SupportClawConfig;
  valid: boolean;
  runtimeConfig: SupportClawConfig;
  hash?: string;
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: SupportClawConfig): SupportClawConfig {
  ...
  function createValidationPluginMetadataSnapshotLoader(params: {
    ...
    load: (config: SupportClawConfig) => PluginMetadataSnapshot;
  ...
  function resolveRuntimePreflightSourceConfig(candidate: SupportClawConfig): SupportClawConfig {
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run supportClaw plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  function resolveSuspiciousRecoveryBackupCandidate(parsed: unknown): SupportClawConfig | null {
    try {
  ...
  function loadConfigLocal(options: { skipSuspiciousRecovery?: boolean } = {}): SupportClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        validationConfigRaw as SupportClawConfig,
        {
          env: deps.env,
          homedir: deps.homedir,
        },
  ...
  async function readConfigFileSnapshotInternal(
    ...
    let fallbackSourceConfig: SupportClawConfig = {};
    ...
    try {
      ...
      if (
        ...
        let recoveryCandidate: SupportClawConfig | null = null;
  ...
  async function readBestEffortConfigLocal(): Promise<SupportClawConfig> {
    return (await readBestEffortConfigSnapshotLocal()).config;
  }

  async function readSourceConfigBestEffortLocal(): Promise<SupportClawConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFileLocal(
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as SupportClawConfig, unsetPaths);
    ...
    const validationCandidate = containsConfigIncludeDirective(validationSourceCandidate)
      ? resolveRuntimePreflightSourceConfig(validationSourceCandidate as SupportClawConfig)
    ...
    let cfgToWrite = persistCandidate as SupportClawConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.SUPPORT_CLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      if (!isVerbose() && deps.env.SUPPORT_CLAW_CONFIG_OVERWRITE_LOG !== "1" && !shouldLogInVitest) {
        return;
      }
      deps.logger.warn(
        formatConfigOverwriteLogMessage({
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.SUPPORT_CLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      const shouldLogBenignMissingMeta =
        isVerbose() || deps.env.SUPPORT_CLAW_CONFIG_WRITE_ANOMALY_LOG === "1" || shouldLogInVitest;
    ...
    const preCommitRuntimePreflight =
      ...
      (async (sourceConfig: SupportClawConfig) => {
...
// module scope. `SUPPORT_CLAW_CONFIG_PATH` (and friends) are expected to work even
...
export function loadConfig(options?: {
  ...
  // reparsing supportClaw.json on hot code paths.
...
export async function readSourceConfigBestEffort(): Promise<SupportClawConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: SupportClawConfig = nextCfg;
