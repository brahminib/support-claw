...
export function listConfiguredSessionStoreAgentIds(cfg: SupportClawConfig): string[] {
...
export function resolveSessionStoreTargets(
  ...
  if (hasAgent) {
    ...
    if (!knownAgents.includes(requested)) {
      throw new Error(
        `Unknown agent id "${opts.agent}". Use "supportClaw agents list" to see configured agents.`,
      );
    }
    return [
      {
