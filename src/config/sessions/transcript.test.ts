...
describe("appendAssistantMessageToSessionTranscript", () => {
  ...
  it("emits transcript update events for delivery mirrors", async () => {
    ...
    expect(message?.provider).toBe("supportClaw");
  ...
  it("idempotently appends identified channel finals while preserving repeated replies", async () => {
    ...
    if (first.ok && replay.ok && nextTurn.ok) {
      ...
      expect(JSON.parse(lines[1]).message.supportClawDeliveryMirror).toEqual({
        kind: "channel-final",
  ...
  it("skips transcript-only SupportClaw assistant entries when reading recent prompt context", async () => {
    writeTranscriptStore();
    const sessionFile = resolveSessionTranscriptPathInDir(sessionId, fixture.sessionsDir());
  ...
  it("skips transcript-only SupportClaw assistant entries when reading latest assistant text", async () => {
    writeTranscriptStore();
  ...
  it("does not report transcript-only SupportClaw assistant entries as latest assistant text", async () => {
    writeTranscriptStore();
  ...
  it("keeps transcript-only SupportClaw assistant entries available to the tail reader", async () => {
    writeTranscriptStore();
  ...
  it("scans past trailing non-assistant entries (e.g. supportClaw.cache-ttl) to find the latest assistant text", async () => {
    // Regression for supportClaw/supportClaw#83427: the cache-ttl custom entry was
  ...
  it("does not reuse an older matching assistant message across turns", async () => {
    ...
    if (olderResult.ok && latestResult.ok && mirrorResult.ok) {
      ...
      expect(messageLine.message.provider).toBe("supportClaw");
