...
import { isTranscriptOnlySupportClawAssistantModel } from "../../shared/transcript-only-supportClaw-assistant.js";
...
function parseAssistantTranscriptText(
  line: string,
  options?: { excludeTranscriptOnlySupportClawAssistant?: boolean },
  ...
  if (
    options?.excludeTranscriptOnlySupportClawAssistant &&
    isTranscriptOnlySupportClawAssistantMessage(message)
  ) {
    return undefined;
  }
...
function isTranscriptOnlySupportClawAssistantMessage(message: {
  provider?: unknown;
  model?: unknown;
}): boolean {
  return isTranscriptOnlySupportClawAssistantModel(message.provider, message.model);
}
...
function parseRecentConversationText(line: string): SessionRecentConversationText | undefined {
  ...
  if (message.role === "assistant" && isTranscriptOnlySupportClawAssistantMessage(message)) {
    return undefined;
  }
  const text =
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      const assistantText = parseAssistantTranscriptText(line, {
        excludeTranscriptOnlySupportClawAssistant: true,
      });
      if (assistantText) {
...
export async function readTailAssistantTextFromSessionTranscript(
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      ...
      // Skip non-message entries (e.g. `supportClaw.cache-ttl` custom events) so
...
export async function appendAssistantMessageToSessionTranscript(params: {
  ...
  return appendExactAssistantMessageToSessionTranscript({
    ...
    message: {
      ...
      timestamp: Date.now(),
      ...(params.deliveryMirror ? { supportClawDeliveryMirror: params.deliveryMirror } : {}),
...
function isRedundantDeliveryMirror(message: SessionTranscriptAssistantMessage): boolean {
  return message.provider === "supportClaw" && message.model === "delivery-mirror";
}

function isChannelFinalDeliveryMirror(message: SessionTranscriptAssistantMessage): boolean {
  const marker = (message as { supportClawDeliveryMirror?: SessionTranscriptDeliveryMirror })
    .supportClawDeliveryMirror;
