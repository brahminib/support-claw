...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.supportClaw) || !isRecord(entry.supportClaw.channel)) {
      continue;
    }
    const channel = entry.supportClaw.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: SupportClawConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: SupportClawConfig, pluginId: string) => boolean;
