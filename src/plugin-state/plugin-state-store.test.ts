...
beforeAll(async () => {
  testState = await createSupportClawTestState({ label: "plugin-state-store" });
  rmSync(path.dirname(resolveSupportClawStateSqlitePath()), { recursive: true, force: true });
});
...
describe("plugin state keyed store", () => {
      ...
      async (stateA) => {
        await withSupportClawTestState(
          ...
          async (stateB) => {
            ...
            expect(resolveSupportClawStateSqlitePath(stateA.env)).not.toBe(
              resolveSupportClawStateSqlitePath(stateB.env),
            );
          },
        );
      },
  ...
  it("reopens after the shared state DB cache closes its handle", async () => {
    await withPluginStateTestState(async () => {
      ...
      const secondary = await createSupportClawTestState({
        label: "plugin-state-cache-secondary",
        applyEnv: false,
      });
      try {
        openSupportClawStateDatabase({ env: secondary.env });
        testState?.applyEnv();
        await expect(store.lookup("k")).resolves.toEqual({ ok: true });
      } finally {
  ...
  it.runIf(process.platform !== "win32")("hardens DB directory and file permissions", async () => {
    await withPluginStateTestState(async () => {
      ...
      const databasePath = resolveSupportClawStateSqlitePath();
