// Plugin state SQLite helpers persist plugin state in the SupportClaw state database.
...
import {
  ...
  isSupportClawStateDatabaseOpen,
...
type PluginStateEntriesTable = SupportClawStateKyselyDatabase["plugin_state_entries"];
type PluginStateStoreDatabase = Pick<SupportClawStateKyselyDatabase, "plugin_state_entries">;
...
function wrapPluginStateError(
  ...
  message: string,
  pathname = resolveSupportClawStateSqlitePath(process.env),
): PluginStateStoreError {
...
function parseStoredJson(raw: string, operation: PluginStateStoreOperation): unknown {
  try {
    ...
    throw createPluginStateError({
      ...
      path: resolveSupportClawStateSqlitePath(process.env),
      cause: error,
    });
  }
}
...
function openPluginStateDatabase(
  ...
  const pathname = resolveSupportClawStateSqlitePath(env);
...
function envOptions(env?: NodeJS.ProcessEnv): SupportClawStateDatabaseOptions {
  return env ? { env } : {};
}
...
function runWriteTransaction<T>(
  ...
  return runSupportClawStateWriteTransaction(() => {
...
export function probePluginStateStore(): PluginStateStoreProbeResult {
  const databasePath = resolveSupportClawStateSqlitePath(process.env);
  const steps: PluginStateStoreProbeStep[] = [];
  const wasOpen = cachedDatabase !== null;
  const stateWasOpen = isSupportClawStateDatabaseOpen();
  ...
  try {
    ...
    openSupportClawStateDatabase().walMaintenance.checkpoint();
    pushOk("checkpoint");
  } catch (error) {
