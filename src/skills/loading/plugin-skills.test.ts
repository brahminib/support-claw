    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "supportClaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "supportClaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "supportClaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("supportClaw-");
  const acpxRoot = await tempDirs.make("supportClaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("supportClaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("supportClaw-");
  const pluginRoot = await tempDirs.make("supportClaw-plugin-");
  const outsideDir = await tempDirs.make("supportClaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("supportClaw-");
    const pluginRoot = await tempDirs.make("supportClaw-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("supportClaw-");
    const pluginRoot = await tempDirs.make("supportClaw-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
