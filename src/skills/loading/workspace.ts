...
import { resolveSupportClawMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
const SKILL_SOURCE_ORIGIN_RELATIVE_PATH = path.join(".supportClaw", "source-origin.json");
...
function isContainerStateHomeWherePromptTildeEscapes(home: string): boolean {
  ...
  return home === "/data" && (configDir === "/data/.supportClaw" || isPathInside("/data/.supportClaw", configDir));
}
...
function resolveSkillsLimits(config?: SupportClawConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "supportClaw-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "supportClaw-bundled") {
    return {
...
function resolveSkillEntryMetadata(params: {
  frontmatter: ParsedSkillFrontmatter;
  skillDir: string;
}): SupportClawSkillMetadata | undefined {
  const metadata = resolveSupportClawMetadata(params.frontmatter);
...
function loadGeneratedPluginSkillRecords(params: {
  ...
  for (const name of childDirs) {
    ...
    // Plugin skills live as symlinks under ~/.supportClaw/plugin-skills/, so
  ...
  const loadSkills = (params: { dir: string; source: string }): LoadedSkillRecord[] => {
    ...
    for (const candidate of scanQueue) {
      ...
      const maxGroupedDepth =
        params.source === "supportClaw-extra" &&
...
function buildSkillsLimitNote(params: {
  ...
  if (params.truncated) {
    return `⚠️ Skills truncated: included ${params.included} of ${params.total}${params.compact ? " (compact format, descriptions omitted)" : ""}. Run \`supportClaw skills check\` to audit.`;
  }
  if (params.compact) {
    return `⚠️ Skills catalog using compact format (descriptions omitted). Run \`supportClaw skills check\` to audit.`;
  }
  return "";
}
