...
beforeAll(async () => {
  fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "supportClaw-plugin-skills-sync-"));
});
...
describe("syncSkillsToWorkspace for plugin skills", () => {
  it("syncs plugin skills from symlinked directories to sandbox workspace", async () => {
    ...
    const pluginSkillsDir = path.join(sourceWorkspace, ".supportClaw", "plugin-skills");
