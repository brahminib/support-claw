...
async function installArchiveResolution(params: {
  ...
  archivePath: string;
  registry: string;
  authority: "supportClaw" | "third-party";
  force?: boolean;
  logger?: Logger;
  config?: SupportClawConfig;
}) {
  return await withExtractedArchiveRoot({
    ...
    tempDirPrefix: "supportClaw-skill-clawhub-",
    timeoutMs: 120_000,
...
async function installGitHubResolution(params: {
  ...
  return await withExtractedArchiveRoot({
    archivePath: params.archivePath,
    tempDirPrefix: "supportClaw-skill-clawhub-github-",
    timeoutMs: 120_000,
...
function assertInstallResolutionAllowed(
  ...
  if (resolution.reason === "ambiguous_slug") {
    ...
    throw new Error(
      `Skill "${resolution.slug}" is ambiguous on ClawHub. Install an owner-qualified skill, for example: supportClaw skills install @owner/${resolution.slug}.${message}`,
    );
  }
