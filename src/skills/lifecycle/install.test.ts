...
import { resolveSupportClawMetadata, resolveSkillInvocationPolicy } from "../loading/frontmatter.js";
...
metadata: {"supportClaw":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
function loadTestWorkspaceSkillEntries(workspaceDir: string): SkillEntry[] {
  ...
  return skills.map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveSupportClawMetadata(frontmatter),
      invocation,
      exposure: {
...
const workspaceSuite = createFixtureSuite("supportClaw-skills-install-");
...
describe("installSkill before_install hooks", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("SUPPORT_CLAW_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an SupportClaw-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["SUPPORT_CLAW_STATE_DIR", "SUPPORT_CLAW_CONFIG_PATH"]);
    try {
      process.env.SUPPORT_CLAW_STATE_DIR = "/tmp/untrusted-state";
      process.env.SUPPORT_CLAW_CONFIG_PATH = "/tmp/untrusted-config/supportClaw.json";
  ...
  it("surfaces plugin hook findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("supportClaw-workspace");
