...
describe("skills-clawhub", () => {
  ...
  it("installs owner-qualified ClawHub skills without using owner as a local path", async () => {
    const workspaceDir = await tempDirs.make("supportClaw-owner-skill-");
  ...
  it("formats ambiguous ClawHub slug responses with owner-qualified guidance", async () => {
    ...
    expect(result.error).toContain("supportClaw skills install @owner/weather");
  ...
  it("persists install artifact and verification provenance in the ClawHub lockfile", async () => {
    const workspaceDir = await tempDirs.make("supportClaw-skills-lock-");
  ...
  it("persists the source URL from server-resolved verification provenance", async () => {
    const workspaceDir = await tempDirs.make("supportClaw-skills-source-");
    const sourceUrl = "https://github.com/supportClaw/skills/tree/main/agentreceipt";
    const verifiedSourceUrl =
      "https://github.com/supportClaw/skills/tree/0123456789abcdef0123456789abcdef01234567/agentreceipt";
  ...
  it("does not treat detail metadata as verified source provenance", async () => {
    ...
    fetchClawHubSkillDetailMock.mockResolvedValueOnce({
      skill: {
        ...
        createdAt: 1,
        updatedAt: 2,
        sourceUrl: "https://github.com/supportClaw/skills/tree/latest/agentreceipt",
      },
      latestVersion: {
        version: "1.0.0",
        createdAt: 3,
  ...
  it("does not trust URLs from unavailable verification provenance", async () => {
    ...
    fetchClawHubSkillVerificationMock.mockResolvedValueOnce({
      ...
      provenance: {
        source: "unavailable",
        url: "https://github.com/supportClaw/skills/tree/unverified/agentreceipt",
      },
      security: { status: "clean" },
      signature: { status: "unsigned" },
    });
  ...
  it("updates owner-qualified ClawHub skills with the stored owner namespace", async () => {
    const workspaceDir = await tempDirs.make("supportClaw-owner-update-");
  ...
  it("updates owner-qualified ClawHub skills when the requested owner matches tracking", async () => {
    const workspaceDir = await tempDirs.make("supportClaw-owner-update-request-");
    await writeClawHubOriginFixture({
      workspaceDir,
      slug: "weather",
      ownerHandle: "demo-owner",
  ...
  it("rejects owner-qualified ClawHub updates when the requested owner does not match tracking", async () => {
    const workspaceDir = await tempDirs.make("supportClaw-owner-update-mismatch-");
    await writeClawHubOriginFixture({
      workspaceDir,
      slug: "weather",
  ...
  describe("legacy tracked slugs remain updatable", () => {
    async function createLegacyTrackedSkillFixture(slug: string) {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-skills-clawhub-"));
    ...
    it("does not install configured skills during update all without ClawHub tracking", async () => {
      const workspaceDir = await tempDirs.make("supportClaw-configured-update-");
      const results = await updateSkillsFromClawHub({
        workspaceDir,
        config: {
          agents: {
    ...
    it("rejects untracked requested updates instead of installing by slug", async () => {
      const workspaceDir = await tempDirs.make("supportClaw-untracked-update-");
  ...
  describe("verification target resolution", () => {
    it("uses installed origin registry and installed version by default", async () => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-skill-verify-"));
      try {
...
describe("ClawHub origin provenance readback", () => {
  ...
  it("restores matching provenance and rejects one-sided origin edits", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-origin-prov-"));
    try {
      const artifact = {
