...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "supportClaw");
  }
  return path.join(homedir(), ".supportClaw");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "supportClaw-bundled"
        ? { kind: "bundled", authority: "supportClaw", mutable: false, network: false }
        : skillSource === "supportClaw-managed" || skillSource === "supportClaw-extra"
          ? { kind: "managed", authority: "supportClaw", mutable: false, network: false }
  ...
  const trustedInstallSources = new Set(["supportClaw-bundled", "supportClaw-managed", "supportClaw-extra"]);
