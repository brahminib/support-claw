...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-skill-upload-store-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("skill upload store", () => {
  ...
  beforeAll(async () => {
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-skill-upload-store-"));
    try {
