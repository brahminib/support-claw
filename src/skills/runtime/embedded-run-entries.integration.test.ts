...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("supportClaw-bundled-");
  const workspaceDir = await createTempDir("supportClaw-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: SupportClawConfig) {
