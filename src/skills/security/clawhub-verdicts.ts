...
type SupportClawSkillSecurityVerdictItem = Omit<
...
function projectClawHubVerdictItem(
  ...
  registry: string,
): SupportClawSkillSecurityVerdictItem {
  const projected: SupportClawSkillSecurityVerdictItem = {
    registry,
    ok: item.ok,
  ...
  if (item.error) {
    const error: SupportClawSkillSecurityVerdictItem["error"] = {};
...
export async function fetchSupportClawSkillSecurityVerdicts(
  targets: Array<{ registry: string; slug: string; version: string }>,
): Promise<SupportClawSkillSecurityVerdictItem[]> {
  ...
  const items: SupportClawSkillSecurityVerdictItem[] = [];
