...
beforeEach(async () => {
  testState = await createSupportClawTestState({
    layout: "state-only",
    prefix: "supportClaw-skill-workshop-state-",
  });
...
async function makeWorkspace(): Promise<string> {
  return await tempDirs.make("supportClaw-skill-workshop-");
}
...
describe("skill workshop proposals", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("supportClaw-skill-workshop-target-skills-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("supportClaw-skill-workshop-readonly-skills-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("supportClaw-skill-workshop-support-trusted-");
      const untrustedSkillsDir = await tempDirs.make("supportClaw-skill-workshop-support-untrusted-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("supportClaw-skill-workshop-untrusted-skills-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
      const proposal = await proposeCreateSkill({
        workspaceDir,
