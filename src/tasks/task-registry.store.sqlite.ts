// Persists task registry records and events through the SupportClaw SQLite state database.
...
type TaskRunsTable = SupportClawStateKyselyDatabase["task_runs"];
type TaskDeliveryStateTable = SupportClawStateKyselyDatabase["task_delivery_state"];
...
// SQLite-backed task store mirrors task records and delivery state into supportClaw-state.db.
...
export function saveTaskRegistryStateToSqlite(snapshot: TaskRegistryStoreSnapshot) {
  withWriteTransaction(({ db }) => {
    ...
    pruneRowsNotInSnapshot({
      ...
      tableName: "task_runs",
      columnName: "task_id",
      tempTableName: "supportClaw_live_task_run_ids",
      ids: taskIds,
    });
    ...
    if (deliveryTaskIds.length === 0) {
      ...
      pruneRowsNotInSnapshot({
        ...
        columnName: "task_id",
        tempTableName: "supportClaw_live_task_delivery_ids",
        ids: deliveryTaskIds,
      });
    }
