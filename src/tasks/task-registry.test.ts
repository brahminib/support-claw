...
function configureTaskRegistryMaintenanceRuntimeForTest(params: {
  ...
  setTaskRegistryMaintenanceRuntimeForTests({
    ...
    resolveCronJobsStorePath: () => "/tmp/supportClaw-test-cron/jobs.json",
...
function createAcpSessionStoreEntry(params: {
  ...
  return {
    cfg: {} as never,
    storePath: "/tmp/supportClaw-test-sessions.json",
...
async function withTaskRegistryTempDir<T>(
  ...
  return await withTempDir({ prefix: "supportClaw-task-registry-" }, async (root) => {
    return await withEnvAsync({ SUPPORT_CLAW_STATE_DIR: root }, async () => {
...
describe("task-registry", () => {
  ...
  it("cancels childless codex-native tasks without routing through SupportClaw subagent sessions", async () => {
    await withTaskRegistryTempDir(async () => {
      resetTaskRegistryForTests();
      const task = createTaskRecord({
        runtime: "subagent",
