...
const ORIGINAL_ENV = captureEnv(["SUPPORT_CLAW_STATE_DIR"]);
...
describe("task-registry store runtime", () => {
  ...
  it("rejects corrupt persisted task rows during sqlite restore", async () => {
    await withSupportClawTestState(
      { layout: "state-only", prefix: "supportClaw-task-store-corrupt-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("drops invalid requester origins during sqlite restore", async () => {
    await withSupportClawTestState(
      { layout: "state-only", prefix: "supportClaw-task-store-invalid-origin-" },
      async () => {
        resetTaskRegistryForTests();
        const created = createTaskRecord({
          runtime: "acp",
  ...
  it("persists executor and requester agent ids in sqlite task rows", async () => {
    await withSupportClawTestState(
      { layout: "state-only", prefix: "supportClaw-task-agent-id-" },
      async () => {
        const created = createTaskRecord({
          runtime: "subagent",
  ...
  it("persists requester origin atomically when creating sqlite tasks", async () => {
    await withSupportClawTestState(
      { layout: "state-only", prefix: "supportClaw-task-create-origin-" },
      async () => {
        const created = createTaskRecord({
          runtime: "acp",
  ...
  it("prunes stale sqlite delivery state while retaining current rows", async () => {
    await withSupportClawTestState(
      { layout: "state-only", prefix: "supportClaw-task-delivery-prune-" },
      async () => {
  ...
  it("prunes large sqlite snapshots without binding every task id at once", async () => {
    await withSupportClawTestState(
      { layout: "state-only", prefix: "supportClaw-task-large-prune-" },
      async () => {
  ...
  it("reopens after the shared state database is closed", async () => {
    await withSupportClawTestState(
      { layout: "state-only", prefix: "supportClaw-task-store-" },
      async () => {
  ...
  it("hardens the sqlite task store directory and file modes", async () => {
    ...
    await withSupportClawTestState(
      ...
      async () => {
        ...
        expect(databasePath.endsWith(path.join("state", "supportClaw.sqlite"))).toBe(true);
