// Persists managed task-flow records through the SupportClaw SQLite state database.
...
type FlowRunsTable = SupportClawStateKyselyDatabase["flow_runs"];
type FlowRegistryStoreDatabase = Pick<SupportClawStateKyselyDatabase, "flow_runs">;
...
// SQLite-backed task-flow store mirrors the in-process registry into supportClaw-state.db.
...
function pruneFlowsNotInSnapshot(params: { db: DatabaseSync; ids: readonly string[] }) {
  const tempTableName = "supportClaw_live_flow_ids";
