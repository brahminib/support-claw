...
type TaskFlowRegistryTestDatabase = Pick<SupportClawStateKyselyDatabase, "flow_runs">;
...
async function withFlowRegistryTempDir<T>(run: (root: string) => Promise<T>): Promise<T> {
  return await withSupportClawTestState(
    {
      layout: "state-only",
      prefix: "supportClaw-task-flow-store-",
    },
    async (state) => {
      const root = state.stateDir;
      resetTaskFlowRegistryForTests();
      try {
        return await withEnvAsync({ SUPPORT_CLAW_STATE_DIR: root }, async () => await run(root));
      } finally {
        resetTaskFlowRegistryForTests();
      }
    },
