...
const TASK_REGISTRY_DELIVERY_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "supportClaw.taskRegistry.deliveryRuntimeOverride",
);
const TASK_REGISTRY_CONTROL_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "supportClaw.taskRegistry.controlRuntimeOverride",
);
  ...
  try {
    if (task.runtime !== "cli") {
      ...
      if (task.runtime === "cron") {
        ...
        // not have SupportClaw child sessions to terminate. Cancellation clears
