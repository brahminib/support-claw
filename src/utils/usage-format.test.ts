...
describe("usage-format", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["SUPPORT_CLAW_AGENT_DIR", "SUPPORT_CLAW_STATE_DIR"]);
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportClaw-usage-format-"));
  ...
  it("prefers models.json pricing over supportClaw config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to supportClaw config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
