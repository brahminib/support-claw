...
type SupportClawSimpleStreamOptions = SimpleStreamOptions & {
...
function isCodeModeEnabled(config?: SupportClawConfig): boolean {
...
export function createCodexNativeWebSearchWrapper(
  ...
  return (model, context, options) => {
    if (
      ...
      const originalOnPayload = options?.onPayload;
      const codeModeOptions: SupportClawSimpleStreamOptions = {
        ...options,
