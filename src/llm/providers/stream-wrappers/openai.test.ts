...
import { createAssistantMessageEventStream } from "supportClaw/plugin-sdk/llm";
...
describe("createCodexNativeWebSearchWrapper", () => {
  ...
  it("does not enable code-mode transport enforcement when config is on but controls are inactive", () => {
    ...
    expect(observedOptions[0]?.supportClawCodeModeToolSurface).toBeUndefined();
  ...
  it("enforces the code-mode transport surface when the run enables it at agent scope", () => {
    ...
    expect(observedOptions[0]?.supportClawCodeModeToolSurface).toBe(true);
    expect(payloads[0]?.tools).toEqual([
      { type: "function", name: "exec" },
      { type: "function", name: "wait" },
    ]);
...
describe("createOpenAIAttributionHeadersWrapper", () => {
  it("routes native Codex traffic through the SupportClaw transport so attribution survives SupportClaw defaults", () => {
    let codexCalls = 0;
    ...
    expect(capturedHeaders?.originator).toBe("supportClaw");
    expect(capturedHeaders?.["User-Agent"]).toMatch(/^supportClaw\//);
  });
  ...
  it("keeps existing wrapped Codex streams so runtime OAuth injection is preserved", () => {
    ...
    expect(capturedOptions?.headers?.originator).toBe("supportClaw");
    expect(capturedOptions?.headers?.["User-Agent"]).toMatch(/^supportClaw\//);
  });
});
