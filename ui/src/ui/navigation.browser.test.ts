...
function expectConfirmedGatewayChange(app: ReturnType<typeof mountApp>) {
  expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/supportClaw");
...
describe("control UI routing", () => {
  ...
  it("hydrates hash tokens, preserves same-scope URL edits, and reloads after gateway changes", async () => {
    ...
    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("supportClaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
    ...
    gatewayUrlInput.value = "wss://missing-token.example/supportClaw";
    gatewayUrlInput.dispatchEvent(new Event("input", { bubbles: true }));
    await refreshed.updateComplete;

    expect(refreshed.settings.gatewayUrl).toBe("wss://missing-token.example/supportClaw");
    expect(refreshed.settings.token).toBe("");

    sessionStorage.setItem(
      "supportClaw.control.token.v1:wss://other-gateway.example/supportClaw",
      "other-token",
    );
    gatewayUrlInput.value = "wss://other-gateway.example/supportClaw";
    gatewayUrlInput.dispatchEvent(new Event("input", { bubbles: true }));
    await refreshed.updateComplete;

    expect(refreshed.settings.gatewayUrl).toBe("wss://other-gateway.example/supportClaw");
  ...
  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/supportClaw#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/supportClaw");
