...
import "../app.ts";
import type { SupportClawApp } from "../app.ts";
...
const mountedApps = new Set<SupportClawApp>();

function collectMountedApps() {
  return new Set<SupportClawApp>([
    ...mountedApps,
    ...document.querySelectorAll<SupportClawApp>("supportclaw-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<SupportClawApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<SupportClawApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("supportclaw-app") as SupportClawApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window["__SUPPORT_CLAW_CONTROL_UI_BASE_PATH__"] = undefined;
