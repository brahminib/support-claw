...
function isPendingSendMessage(message: unknown): boolean {
  return asRecord(asRecord(message)?.["__supportClaw"])?.kind === "pending-send";
}
...
function sourceMessageId(message: unknown): string | null {
  ...
  const supportClawId = asRecord(record["__supportClaw"])?.id;
  if (typeof supportClawId === "string" && supportClawId.trim()) {
    return supportClawId.trim();
  }
...
export function buildChatItems(props: BuildChatItemsProps): Array<ChatItem | MessageGroup> {
  ...
  for (let i = historyStart; i < history.length; i++) {
    ...
    const marker = raw["__supportClaw"] as Record<string, unknown> | undefined;
