...
const SKIP_DELETE_CONFIRM_KEY = "supportClaw:skipDeleteConfirm";
...
function isLocalAssistantAttachmentSource(source: string): boolean {
  ...
  if (/^\/(?:__supportClaw__|media|api\/chat\/media\/outgoing)\//.test(trimmed)) {
    return false;
  }
  return (
...
function buildAssistantAttachmentUrl(
  ...
  return `${normalizedBasePath}/__supportClaw__/assistant-media?${params.toString()}`;
}
  ...
  if (!pending) {
    pending = (async () => {
      ...
      if (requesterSessionKey) {
        headers.set("x-supportClaw-requester-session-key", requesterSessionKey);
      }
...
function renderGroupedMessage(
  ...
  const transcriptMeta =
    m["__supportClaw"] && typeof m["__supportClaw"] === "object" && !Array.isArray(m["__supportClaw"])
      ? (m["__supportClaw"] as Record<string, unknown>)
      : null;
  ...
  const shouldFetchFullMessage = Boolean(
    sidebarMessageId &&
    !m.supportClawMessageToolMirror &&
