  ...
  const guidance =
    {
      ...
      "not-git-install":
        "Not a git checkout. Run `supportClaw update` from the CLI for a global reinstall.",
      "not-supportClaw-root":
        "Run the update from an SupportClaw checkout or use the CLI global reinstall path.",
      ...
      "restart-disabled":
        "The update was not applied because gateway restarts are disabled. Enable restarts in config, then retry — or run `supportClaw update` from the CLI.",
      "restart-unavailable":
        "This global install cannot be safely replaced while restarts are disabled and no supervisor is present.",
      "restart-unhealthy":
        "The replacement process never became healthy. The previous process stayed up so you can recover.",
      "doctor-failed": "Doctor repair failed. Run `supportClaw doctor --non-interactive` and retry.",
