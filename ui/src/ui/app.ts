...
declare global {
  interface Window {
    __SUPPORT_CLAW_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
export class SupportClawApp extends LitElement {
  ...
  async toggleRealtimeTalk() {
    ...
    const session = new RealtimeTalkSession(
      ...
      this.sessionKey,
      {
        ...
        onTranscript: (entry) => {
          this.realtimeTalkTranscript = `${entry.role === "user" ? "You" : "SupportClaw"}: ${entry.text}`;
...
if (!customElements.get("supportclaw-app")) {
  customElements.define("supportclaw-app", SupportClawApp);
}
