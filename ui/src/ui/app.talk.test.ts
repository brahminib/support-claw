...
describe("SupportClawApp Talk controls", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("retries Talk immediately when the previous session is already in error state", async () => {
    ...
    const app = Object.create(SupportClawApp.prototype) as {
      client: unknown;
      connected: boolean;
    ...
    await SupportClawApp.prototype.toggleRealtimeTalk.call(app as never);
  ...
  it("keeps the Talk options toggle inside the open-panel click guard", async () => {
    await import("./app.ts");
    const app = document.createElement("supportclaw-app");
  ...
  it("clears stale Talk catalog providers but preserves selection when a refresh fails", async () => {
    ...
    await SupportClawApp.prototype.fetchRealtimeTalkCatalog.call(app as never);
