...
describe("Control UI Vite config", () => {
  it("resolves root tsconfig package aliases for source imports", () => {
    expect(findStringAlias("@supportclaw/net-policy/ip")?.replacement).toBe(
  ...
  it("resolves Control UI dev-server source aliases for internal packages", () => {
    ...
    expect(
      aliases.find((alias) => alias.find === "@supportclaw/normalization-core/string-coerce"),
    )?.toEqual({
  ...
  it("keeps specific tsconfig aliases ahead of broad package aliases", () => {
    ...
    const netPolicyIpIndex = aliases.findIndex((alias) => alias.find === "@supportclaw/net-policy/ip");
    const netPolicyPackageIndex = aliases.findIndex(
      (alias) => alias.find === "@supportclaw/net-policy",
    );
    ...
    const broadSupportClawWildcardIndex = aliases.findIndex(
    ...
    expect(broadSupportClawWildcardIndex).toBeGreaterThanOrEqual(0);
    expect(netPolicyIpIndex).toBeLessThan(netPolicyPackageIndex);
    expect(netPolicyWildcardIndex).toBeLessThan(broadSupportClawWildcardIndex);
  });
