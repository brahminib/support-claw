...
import type { SupportClawModalDialog } from "./modal-dialog.ts";
...
function expectShadowElement(modal: SupportClawModalDialog, id: string): HTMLElement {
...
describe("supportclaw-modal-dialog", () => {
  beforeEach(() => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("supportclaw-modal-dialog-label");
    expect(descriptionId).toBe("supportclaw-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "supportclaw-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "supportclaw-modal-dialog-description").textContent).toBe(
