...
import type { SupportClawFilePreviewModal } from "./file-preview-modal.ts";
...
async function renderPreview(query = "") {
  render(
    html`
      <supportclaw-file-preview-modal
        .files=${files}
  ...
  const modal = container.querySelector<SupportClawFilePreviewModal>("supportclaw-file-preview-modal");
...
function shadowText(modal: SupportClawFilePreviewModal): string {
  return modal.shadowRoot?.textContent ?? "";
}

describe("supportclaw-file-preview-modal", () => {
  beforeEach(() => {
