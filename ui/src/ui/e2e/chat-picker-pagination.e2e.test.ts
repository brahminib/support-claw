...
describeControlUiE2e("Control UI chat picker mocked Gateway E2E", () => {
  beforeAll(async () => {
    if (!chromiumAvailable) {
      throw new Error(
        `Playwright Chromium is not installed or cannot start at ${chromiumExecutablePath}. Run \`pnpm --dir ui exec playwright install --with-deps chromium\`, or set SUPPORT_CLAW_UI_E2E_ALLOW_MISSING_CHROMIUM=1 only when intentionally skipping this lane.`,
      );
    }
  ...
  it("skips hidden subagent-only pages when loading more chat sessions through the GUI", async () => {
    ...
    try {
      ...
      if (process.env.SUPPORT_CLAW_CAPTURE_UI_PROOF === "1") {
