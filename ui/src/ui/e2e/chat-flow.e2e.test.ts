...
const allowMissingChromium = process.env.SUPPORT_CLAW_UI_E2E_ALLOW_MISSING_CHROMIUM === "1";
...
async function waitForChatScrollIdle(page: Page): Promise<void> {
  await expect
    .poll(
      () =>
        page.evaluate(() => {
          const app = document.querySelector("supportclaw-app") as
            | (Element & {
...
describeControlUiE2e("Control UI mocked Gateway E2E", () => {
  beforeAll(async () => {
    if (!chromiumAvailable) {
      throw new Error(
        `Playwright Chromium is not installed or cannot start at ${chromiumExecutablePath}. Run \`pnpm --dir ui exec playwright install --with-deps chromium\`, set PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH to a compatible browser, or set SUPPORT_CLAW_UI_E2E_ALLOW_MISSING_CHROMIUM=1 only when intentionally skipping this lane.`,
      );
    }
  ...
  it("sends the first chat turn while agents startup loading is still pending", async () => {
    ...
    try {
      ...
      await gateway.resolveDeferred("chat.startup", {
        agentsList: {
          agents: [{ id: "ops", name: "SupportClaw" }],
          defaultId: "ops",
          mainKey: "main",
          scope: "agent",
        },
