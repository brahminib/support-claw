...
export async function startControlUiE2eServer(): Promise<ControlUiE2eServer> {
  ...
  const server = await createServer({
    ...
    configFile: false,
    define: {
      SUPPORT_CLAW_CONTROL_UI_BUILD_ID: JSON.stringify("e2e"),
    },
    logLevel: "error",
    optimizeDeps: {
      include: [
...
function normalizeScenario(
  ...
  return {
    ...
    assistantName: scenario.assistantName?.trim() || "SupportClaw",
    defaultAgentId,
...
function installControlUiMockGateway(input: {
  ...
  type WindowWithGateway = Window & {
    supportClawControlUiE2eGateway?: ExposedGateway;
  };
  ...
  (window as WindowWithGateway).supportClawControlUiE2eGateway = exposed;
  ...
  const emitGatewayEvent = async (event: string, payload?: unknown) => {
    await page.evaluate(
      ({ eventName, eventPayload }) => {
        const gateway = (
          window as Window & {
            supportClawControlUiE2eGateway?: {
              emit: (event: string, payload?: unknown) => void;
            };
          }
        ).supportClawControlUiE2eGateway;
