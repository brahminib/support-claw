...
function resolveControlUiBuildId(): string {
  const explicit =
    process.env.SUPPORT_CLAW_CONTROL_UI_BUILD_ID?.trim() || process.env.SUPPORT_CLAW_VERSION?.trim();
  if (explicit) {
...
function controlUiServiceWorkerBuildIdPlugin(buildId: string): Plugin {
  return {
    ...
    closeBundle() {
      ...
      const placeholder = '"__SUPPORT_CLAW_CONTROL_UI_BUILD_ID__"';
...
export default function controlUiViteConfig(): UserConfig {
  const envBase = process.env.SUPPORT_CLAW_CONTROL_UI_BASE_PATH?.trim();
  ...
  return {
    base,
    define: {
      SUPPORT_CLAW_CONTROL_UI_BUILD_ID: JSON.stringify(controlUiBuildId),
    },
